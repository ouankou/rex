...
Edison Design Group (legacy frontend) frontend.  We want to thank the original
...
legacy frontend's AST to SAGE II was derived loosely from the legacy frontend C++ source generator and has
formed the basis of the SAGE III translator from legacy frontend to SAGE III's IR. Under this license
we exclude the legacy frontend source code and the translation from the legacy frontend AST in distributions 
and make available only a binary of those parts with use legacy frontend (front-end AST translation),
and the source to all of ROSE (which does not depend on legacy frontend). No part of the legacy frontend work is 
visible to the user of ROSE.  We can make the legacy frontend source available only to those who have 
the free legacy frontend research license.  We want to thank the developers at Edison Design Group
(legacy frontend) for making their work so widely available under their research license program.
