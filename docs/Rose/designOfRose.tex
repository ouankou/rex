...
legacy frontend only builds its own AST and it is difficult to modify it (I think)
...
(since it completely hides the legacy frontend AST and does not need it after translation to
SAGE).  SAGE is independent of legacy frontend by design. Current work with Rice is using the 
...
these ways that what we have done adds to legacy frontend and
hopefully provides a more useful infrastructure for the automated transformation of
C++ applications beyond that of the design goals of legacy frontend.  I understand that the
design goals of legacy frontend are only to provide a front-end to compiler vendors, a critical
...
legacy frontend directly, but certainly it is not required, many people use legacy frontend by itself.
We, like the rest of the world, of course has the deepest respect for legacy frontend.
For our purposes we need something like legacy frontend's front-end to support our own research work
on program optimization. I don't know of any alternatives to using legacy frontend given that 
...
specified by the C++ standard (much of this we get because we use legacy frontend and it implements
...
number of CMake, LaTeX, dot, etc.  ROSE distributions appear in
...
to the legacy frontend front-end used in parts of ROSE) from binary versions (all source code to ROSE
but a binary of any legacy frontend specific parts or parts of ROSE which use the legacy frontend header files).
...
version of the legacy frontend work used by ROSE.  We protect the source code of legacy frontend by saving only 
the binary libraries that we build from the legacy frontend work (the legacy frontend source code and any parts 
of ROSE that use the legacy frontend header files).  
