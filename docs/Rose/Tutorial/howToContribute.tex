...
\begin{verbatim}
-u --exclude 'CMakeCache.txt' --exclude 'CMakeFiles' --exclude 'cmake_install.cmake'
--exclude 'compile_commands.json' --exclude 'build'
\end{verbatim}
...
\begin{verbatim}
$ cd /home/me/research
$ diff -ur --exclude 'CMakeCache.txt' --exclude 'CMakeFiles' --exclude 'cmake_install.cmake'
  --exclude 'compile_commands.json' --exclude 'build' ROSE-orig ROSE > my-changes.patch
\end{verbatim}
...
\begin{verbatim}
$ cd src
$ cvs diff -u --exclude 'CMakeCache.txt' --exclude 'CMakeFiles' --exclude 'cmake_install.cmake'
  --exclude 'compile_commands.json' --exclude 'build' frontend > my-changes.patch
\end{verbatim}
...
You will need to create a CMakeLists.txt which integrates with the ROSE build process. The best way to learn about these is by looking at other CMakeLists.txt files in sibling directories to yours. Remember to add your new directory via \verb|add_subdirectory()| in the parent directory's CMakeLists.txt, and update any target or install lists as needed.

If your project is in \verb|src|, your test cases should go into a subdirectory of \verb|tests/nonsmoke/functional/roseTests|, which mirrors the \verb|src| directory structure. Otherwise they can go into your project directory. Again, look at other CMakeLists.txt files for information on how test cases work.
