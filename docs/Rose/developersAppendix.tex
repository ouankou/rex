...
\item Get content in the repository:
  \begin{itemize}
          ...
          directory named localRoseName will show up after this 
           ...
           \item Retrieve legacy frontend source files: we use a separated git
           repository for legacy frontend to protect its proprietary source files. The
           legacy frontend git repository is linked to the ROSE git repository as a
           submodule. You have to explicitly get the submodule if you want to
           modify the legacy frontend source files. Just type:
...
\item Pulling and Merging the shared master branch: It is recommended to synchronize the master branches often and merge new commits from others to  your local branch. 
   \begin{itemize}
          ...
          \item Don't forget to update the link to the legacy frontend submodule: \textit{git submodule update}
          \item Switch to your local branch and merge with local master: \textit{git checkout your-local-branch-name; git merge master}
    \item Alternatively, you can stay in your local branch and directly pull the remote master and update the legacy frontend submodule: 
...
\item Modifying the legacy frontend submodule. In rare cases, you may have to modify the legacy frontend files within the submodule and update the link between ROSE and the legacy frontend module. Please be {\bf EXTREMELY cautious} when you have to do this and always
ask for help from senior LLNL developers for your first attempt. Here are some brief instructions about how to do this right.
  ...
  \item First, make sure you have tried to pull the content from the legacy frontend submodule. You can run \textit{git submodule init; git submodule update} again from the top level of your private ROSE git repository to be safe. 
  \item By default, the legacy frontend submodule files checked out via \textit{git submodule init; git submodule update} is not on any branch (You can verify this by typing \textit{git status} within \textit{src/frontend/CxxFrontend/legacy frontend}.  
You can create a local branch based on the current content before you modify the legacy frontend related files. Just type \textit{git checkout -b local-branch-name} under \textit{src/frontend/CxxFrontend/legacy frontend} so you can create a local branch off the current content and switch to the branch.
   \item We have access control over the legacy frontend git repo the same way we have
   for the ROE git repo. You have to have your own remote branch in
   the shared legacy frontend git repo so you can push any contributions to your own
   branch. The initial content of your remote branch
   MUST be the same as the current legacy frontend version linked to ROSE. This
   should be the default legacy frontend version checked out and also the local branch
   you just created from the previous step. To create a remote branch for
   yourself based the latest linked legacy frontend version, type 
   \textit{git push origin HEAD:refs/heads/your\_LLNL\_OUN-main-rc} within \textit{src/frontend/CxxFrontend/legacy frontend}.
   \item You can then modify legacy frontend files and commit changes to your private repository of the submodule. 
         You should also be able to push your commits to your own branch
         within the remote legacy frontend repository.
         Again, always be sure do all commits and push within \textit{src/frontend/CxxFrontend/legacy frontend} for legacy frontend related changes. 
  ...
  $ git add legacy frontend  # Do not type 'git add legacy frontend/', 
                 # which will add all files under legacy frontend/ to the super project!!!
  $ git commit -m "Updated submodule legacy frontend."
  ...
  of the legacy frontend repo will not automatically be merged into the master of the legacy frontend repo. So
  the legacy frontend branch linked to the parent ROSE repo often is often the master of
  the legacy frontend repo. This should not be a problem since two repositories are
  ...
  We maintain an external subversion repository for ROSE at SciDAC Outreach
    ...
    If you are our external (non-LLNL) users who make contributions to ROSE,
       we highly recommend you to work on a dedicated branch of the external repository. 
         ...
         \htmladdnormallink{https://outreach.scidac.gov/account/}{https://outreach.scidac.gov/account/}
  ...
  \item {\tt svn commit} will start your favorite editor where you should enter a
    ...
    a short, one-line summary ({\it i.e.}, a title with just the first word capitalized),
      followed by a blank line, and as much detail as necessary. There is generally
        ...
        {\indent
          \begin{verbatim}
          Adjusted test case for new function detection

            This test case assumed that the only functions were those with explicit
            definitions in the translation unit. This is no longer true since we now
            include declarations from headers.
            \end{verbatim} 
        }
  \end{itemize}
...
access to the legacy frontend source code, we sometime have to update their
version of the parts of ROSE that are not released publicly
(e.g. legacy frontend and the legacy frontend/ROSE translation work which uses legacy frontend).
...
with the binary distribution of the legacy frontend and legacy frontend/ROSE translation code,
and so they need a most recent version of the full distribution of
ROSE so that they can build legacy frontend and the legacy frontend/ROSE translation fresh
...
\begin{enumerate}
   ...
   \item Ask for a tarball of the full source code of ROSE from our internal SVN repository.
      ...
      For example, it shows the lat log entry of the main trunk on the page of (\htmladdnormallink{https://outreach.scidac.gov/plugins/scmsvn/}{https://outreach.scidac.gov/plugins/scmsvn/}):
\begin{verbatim}
...
which we then send to you.  This is a full source code release of ROSE which includes the
    protected legacy frontend source code, we will know if you have a license for this.  
    {\em You should not distribute this to anyone who does not have an legacy frontend license.}
   ...
   A typical  command to generate a patch looks like the following:
    ...
    diff -NaU5 -rbB -x \*.orig -x \*.o -x \*.swp -x \*.bak -x \*.pdf \
      -x \*.html -x \*.rej -x \*~ -x \*.gz \
      -x CMakeCache.txt -x CMakeFiles -x cmake_install.cmake -x compile_commands.json \
      -x build -x .git -x .svn external_trunk your_updated_branch > my.patch
    \end{verbatim}
   ...
   \item Apply that patch to the tarball of the internal ROSE's trunk that we sent you (representing
     the full source code for legacy frontend and everything) and you how have a way to test
     your work and recompile the legacy frontend work for either a new machine or with the IR changes
     that you have added.
     \begin{verbatim}
   ...
   patched internal ROSE trunk and it's original form. 
    ...
    diff -NaU5 -rbB -x \*.orig -x \*.o -x \*.swp -x \*.bak -x \*.pdf \
      -x \*.html -x \*.rej -x \*~ -x \*.gz \
      -x CMakeCache.txt -x CMakeFiles -x cmake_install.cmake -x compile_commands.json \
      -x build -x .git -x .svn internal_ROSE_trunk_orig
...
% \item Apply that patch to your external branch.  This will update your branch with everything required, even though   it may not pass tests using the released binary of legacy frontend.
...
LaTeX, MrDocs and DOT to be installed (check the list of dependences in the 
{\tt ROSE/ChangeLog}). If you want to build the reference manual of LaTeX documentation
generated from the API reference (not suggested) you may have to tailor your version of LaTeX to 
...
\end{verbatim} 
     ...
     \item Creating the new IR node at some point, such as somewhere within frontend, midend, or even backend if desired.
           ...
           If the new types of IR come from their counterparts in legacy frontend, then modifications to the legacy frontend/SAGE connection code are needed.
           If you are trying to represent some pragma information, you don't need to touch legacy frontend and its connection. 
           You just parse the pragma string in the original AST and create your own nodes to get a new version of AST. 
           Then it should be done.

           Here are the instructions if the new IR node is connected to legacy frontend IR node.
           Modify the legacy frontend/SAGE connection code to have the new IR node built in the
           translation from legacy frontend to SAGE III.  This step often requires a bit of expertise
           in working with the legacy frontend/SAGE connection code. In general, it requires no great
           depth of knowledge of legacy frontend.

           Two source files are usually involved: a)
           \textit{src/frontend/CxxFrontend/legacy frontend\_SAGE\_Connection/sage\_gen\_be.C}
           which converts IL tree to SAGE III AST and is derived from legacy frontend's 
           C++/C-generating back end \textit{cp\_gen\_be.c}; b)
           \textit{sage\_il\_to\_str.C} contains helper functions forming SAGE
           III AST from various legacy frontend IL entries. It is derived from legacy frontend's
           \textit{il\_to\_str.c}.  For the {\em SgVarArgOp} example, the
           following legacy frontend-SAGE connection code is needed in
...
\end{verbatim}
    ...
    the documentation for that specific class (in the class documentation comments,
    which are the source for the API reference, not in the
    {\tt ROSE/src/frontend/SageIII/docs} directory, which is frequently overwritten).  See
    details on how to document ROSE in {\tt ROSE/docs/README}.
{\indent
{\mySmallFontSize
\begin{verbatim}
...
\begin{enumerate}
  ...
  # compilation options: turn off ROSE frontend warnings 
...
It conducts a set of sanity checks and postprocessing before the actual importing. e.g. remove .svn and other undesired directories or files, and verify the package contents.
%------------------------------------------------------------
%}}}
...
filter out the automatically generated commits updating legacy frontend binary files.
...
tarball of ROSE source (with legacy frontend binary), or against the HEAD revision of the
...
\begin{itemize}
 ...
 \item {\tt PREREQS} - Define what software this run needs. The prereqs available
                       ...
                       \url{http://nmi-s005.cs.wisc.edu/nmi/} \\
 ...
 \item {\tt JAVA\_HOME} - If the JVM runtime (used by the Fortran OFP frontend)
                         is included in the prereqs, {\tt JAVA\_HOME} should be
                         specified. This will be passed to the environment of
                         the running test.
...
DESCRIPTION="minimal configuration options, gcc 4.2.4"
\end{verbatim}
...
\url{http://nmi-web.cs.wisc.edu/nmi/}.
...
\begin{enumerate}
   \item ROSE Frontend \\
      Generation of Abstract Syntax Tree (AST) from source code.
         ...
         \begin{enumerate}
            ...
            \begin{enumerate}
               ...
               \item FunctionCallNormalization \\
                  This is a library of function call normalizations to support analysis.
...
available in other documentation or via the API reference.
...
processing of source code.  Specialized projects may
...
\fixme{Liao will be proposing namespace names for us to agree upon separately.}
    The frontend of ROSE takes the source code
...
We separate out the API that is specific for source code.
...
data structures. We separate out the API that is specific for source code.
\begin{enumerate}
   \item Source (TP) \\
   \begin{enumerate}
...
\begin{enumerate}
   ...
   generated source code. UnparseDelegate is currently ignored.
...
<HEADER_GRAPH_START>
 ...
 //! Support for minimum spanning tree.
...
IR.  However this the IR is perhaps best represented by the API reference
web pages which present both the hierarchy of C++ classes 
