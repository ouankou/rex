...
of the legacy frontend front-end, and dropped the handling of Fortran, 
...
SAGE III continues to use the legacy frontend frontend and has updated
the versions of legacy frontend in use (over SAGE II) and separated
out the legacy frontend work so that the connection of SAGE III to legacy frontend
...
\begin{itemize}
    ...
    \item Complete language support for C, C99, C++, Fortran 66, Fortran 77, 
    ...
    \item Full IR documentation via the API reference (web pages).
...
legacy frontend does not preserve comments at this time, and we felt it was important to preserve them
...
The legacy frontend documentation refers to them as pragma declarations, so they should be 
...
decision on the part of legacy frontend to provide this as a default behavior and our decision 
to use it.  More recent work to get the template as an AST is underway, using some 
of the options in legacy frontend to support this.  This later work is not robust enough to be 
...
%                    compiles with either legacy frontend or GNU g++).  No other declaration is 
     ...
     \item Transformation of templated classes is enabled via generated specializations. \\
          ...
          \begin{itemize}
               \item Generation of code for transformed templated class. \\
                     ...
                     involve a call to the legacy frontend function:
                     \begin{itemize}
                   ...
                   % *.ti files (I guess), and might require a modification to the legacy frontend prelink
...
ROSE controls automatic template instantiation using the option
{\tt -rose:instantiation <mode>}, where the mode is either:
\begin{enumerate}
   \item none (default) \\
...
\commentout{
\subsection{Problems with the legacy frontend/ROSE Template Instantiation Approach}

    First lets mention that legacy frontend does a great job and that the problems that
we have within template instantiation in ROSE are a result of trying to
convert template instantiations in legacy frontend to be formal template specializations 
in ROSE.  This approach presents a better user interface and is substantially 
different to what legacy frontend provides.

    For member functions legacy frontend adds to the legacy frontend AST the instantiations of the member functions
...
\begin{enumerate}
   \item Transformations on templates \\
         ...
         templates.  However, due to legacy frontend dropping the inline member function template
   ...
   \item used (same as legacy frontend) \\
   ...
   \item local (same as legacy frontend) \\
...
then we would trigger the mechanism (in the legacy frontend front-end) for the front-end to ask for a template to be
...
the legacy frontend front-end generates a {\em template instantiation} file (file.ti).
...
and deletes the {\em template instantiation} file generated by legacy frontend (file.ti).  Thus ROSE
...
\begin{verbatim}
cmd:./translator -rose:verbose 2 -I/home/dquinlan/ROSE/NEW_ROSE/tests/nonsmoke/functional/CompileTests/A++Code -c -rose:prelink
...
to communicate to the legacy frontend front-end what templates should be instantiated during the 
...
the file associated with that symbol and the legacy frontend front-end then reads the template 
...
Note that legacy frontend also uses a {\em definition list file}  I am unclear what its significance
is in the template instantiation process (it is however documented in the legacy frontend manual).
}
...
\begin{enumerate}
 ...
 %       should not be an issue.
   \item static const data members defined in the class definition (floats only)
         legacy frontend accepts static and g++ supports const, and neither accepts what the other
         considers correct. ROSE generates code specific for the back-end and so the
         back-end must be specified at CMake configuration time (e.g.,
         {\tt -DWITH-ALT-BACKEND-CXX-COMPILER=<path>}).  We
         don't currently support legacy frontend as a back-end, though we support Intel C++ as a
         back-end and they use legacy frontend, so this should work.
...
\begin{enumerate}
     ...
     \item Which access functions are simple and which do meaningful computation. \\
          ...
          See the API reference for more details on the IR
...
difference is due to normalizations internally within the legacy frontend front-end.  Current 
...
\begin{enumerate}
   ...
   %  Although this could be changed within legacy frontend, we currently use legacy frontend with
   ...
   %  non-template classes.  The default must be changed in legacy frontend for this to work
   ...
   %  Clearly, access to the legacy frontend source code is required for this.  The default within ROSE
...
\end{verbatim}
     ...
     This is not something that we can fix or change. legacy frontend simply represents 
     ...
     generated by legacy frontend, and legacy frontend ignores the comments.  We currently output the comments 
...
\end{verbatim}
     ...
     legacy frontend appears to normalize all C++ style cases to C style casts. We are
     ...
     Floating-point literals are internally represented in legacy frontend as float, double, or long
     double (dependent on the type), thus the exact string representing the floating point
     literal is lost.  We have modified legacy frontend to save the string representation (from the
...
\end{verbatim}
     ...
     \item Array indexing represented as pointer arithmetic. \\
           Array indexing is translated by legacy frontend into pointer arithmetic.  It
...
\item Unnamed typedefs of enums are normalized to enums. \\
     legacy frontend appears to normalize unnamed typedefs to be enums, and the information about the 
     origin as an unnamed typedef is lost.  Since there appears to be no difference in the
     legacy frontend AST, ROSE is unable to recover when the {\em typedef} keyword was used.  This is not
...
\end{verbatim}
     ...
     The reasons this is done is because this is that legacy frontend stores the packing alignment
...
normalization appears to be a result of the internal working of legacy frontend 
...
legacy frontend (only one that I know of) and a fair number by g++.  Because in many cases these features 
...
differences between how they are supported within both the legacy frontend front-end and the
     ...
     class X
        {
          public:
            ...
            // Illegal C++ code (non-standard, does not compile with legacy frontend, but does with g++)
               static const double doubleValueConstant1 = 3.14; 
            // Illegal C++ code (non-standard, but compiles with legacy frontend, and does not with g++)
...
\end{verbatim}
           ...
           as a non-standard extension by both legacy frontend and GNU g++ (though in different ways).
           This is a little corner of C++ which is truly obscure, but shows up in some
           large applications at LLNL.  Since the code that works with legacy frontend does not work
           with GNU g++ (and vice versa),
           there is no common ground.  So we assume that the code will compile using legacy frontend
           (we have no choice) and then generate code that will compile with GNU g++.
           This means that we generate C++ code that can't be compiled with legacy frontend, but this 
...
\section{Notes on ROSE-specific Header Files }
    ...
    used by ROSE (by the legacy frontend front-end) from those used by the target back-end.
  ...
  % reference.
    We use the {\tt --preinclude} mechanism in legacy frontend to force a specific generated header file 
    to be read ahead of any ROSE system header files (translated from the back-end system
    header files by the ROSE build header-staging mechanism).  This head file contains
    all the back-end specific macros definitions. The file name is:
    {\tt rose\_required\_macros\_and\_functions.h} and is placed in the install
...
\commentout{
    ...
    given their definition withn the specificaiton of the commandline internally to legacy frontend
    (not really a commandline, but a string handed to the legacy frontend front-end withi a function
    call interface).

    The full commandline internally required to legacy frontend to make it emulate g++ is:
    ...
    \begin{enumerate}
        ...
        {\tt \_\_flexarr[1]} to compile properly with legacy frontend.
        ...
        must be defined to avoid code which uses GNU extensions and would not compile with legacy frontend
        (not legal C++ code).
...
extensions to C and C++ compilers (even certain standard following front-ends such as legacy frontend).}
...
appeared in the original code).
   ...
   These pointers are initialized in the legacy frontend/Sage interface code and are in a few cases
...
is a specific C++ construct) presented by the C++ program.
     ...
     Note that none of the {\tt get\_mangled()} functions are called within the legacy frontend/Sage 
...
\section{Passing Options to ROSE}

    By default, all command line options (except ROSE-specific options) are 
...
compiler ({\tt CC} or {\tt CXX}).
    ...
    ROSE-specific command line options are prefixed using {\tt -rose:xxx}
and only interpreted by ROSE (not passed on to the back-end compiler).  To see 
...
\section{How to Control Language Specific Modes: C++, C, C99}

   ROSE supports a number of different modes internally (within ROSE and the SAGE III
IR).  There are three modes supported:
\begin{enumerate}
   \item C++ mode. \\
        This mode is used when compiling all files when no command line options are specified.

   \item C mode. \\
        This is the mode used when compiling with the {\tt -rose:C\_only} or
        {\tt -rose:C89\_only} options (works best if files have ".c" filename extension).
        This implies conformance with the C89 ANSI standard.

   \item C99 mode. \\
        This is the mode used when compiling with the {\tt -rose:C99\_only} option
        (file must have ".c" filename extension). This implies conformance with the
        C99 standard.
...
a *.c (lower case {\em c}) as a filename extension (suffix).  This is a frontend
issue related to the front-end parsing and the language rules that are selected
(seemingly independent of the options specified to legacy frontend and based partly on the
...
higher-level interfaces to the rewrite mechanisms.
