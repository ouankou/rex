...
%%    It has been reported that our binary version of legacy frontend requires Mac OS X v10.5
...
\begin{enumerate}
 ...
 % compiling fine now.
   ...
   This message from running the {\tt configure} command in ROSE (an initial step in building
   ROSE) indicates that your {\tt LD\_LIBRARY\_PATH} (environment variable) is not set to 
   the location of {\em libjvm.so} or that {\tt JAVA\_HOME} points to a JVM installation that
   does not provide the required runtime libraries. The JVM dependency exists only for the
   Open Fortran Parser (OFP) frontend; REX does not support Java as a source language.
...
%% separately so that ROSE can find and download it's required legacy frontend 
%% binaries from the web (ROSE web site).
%% 
%% \paragraph{Mac OS X v10.5, Leopard}
%%    We only build legacy frontend binaries for the default GNU 4.0 compiler.
...
distribute the Windows generated legacy frontend binaries that would be required.  Also the
...
\item Hudson client : The client can be started through the web page. All that is required is a JVM runtime.
\end{itemize}

Currently under tux270-0 Flex, CMake, and Bison are installed under c:/ROSE. In
