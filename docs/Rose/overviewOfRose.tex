...
languages and use different parsers and/or frontends to address each one.
For C and C++, REX uses the Clang/LLVM frontend.  For Fortran we
use the Open Fortran Parser and build the frontend required.
ROSE contains a midend, where analysis support is made available and a backend 
...
back-end (which has language specific support).
   ...
   More languages could be added to ROSE, ROSE is designed to be language
neutral, but it is implemented in C++.

\section{Design}

Fundamentally, ROSE has three parts:
\begin{enumerate}
   \item frontend, which addresses language specific parsers/frontend issues;
...
all the documentation is in {\tt docs}.  REX uses CMake; build helpers live in
{\tt build-rex.sh} and {\tt cmake/}. The {\tt config} directory contains
compiler header staging assets used by the build.
