...
/* This file is required to avoid compiling VLA
   ...
   array is not allowed struct __gconv_info __cd;
                          ^

   Change this path if your C compiler include files are not in the normal
   place. This should be the standard location for this file, but it will have
   to be modified if it is in a different place.
 */
 ...
 *
 * The create_system_headers build script will detect this placeholder doesn't
 * exist and automatically find the correct arch-specific cdefs.h for the build
 * system:
 ...
 * The script now excludes problematic newlib paths and prefers *-linux-gnu
 * paths.
 */
