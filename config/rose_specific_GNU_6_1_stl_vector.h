...
template <typename _Tp, typename _Alloc> struct _Vector_base {
  ...
  struct _Vector_impl : public _Tp_alloc_type {
    ...
    _Vector_impl(_Tp_alloc_type &&__a) noexcept
        : _Tp_alloc_type(std::move(__a)), _M_start(), _M_finish(),
          _M_end_of_storage() {}
#endif
...
public:
   ...
   */
  const_reference back() const _GLIBCXX_NOEXCEPT { return *(end() - 1); }
...
#if __cplusplus >= 201103L
  template <typename _Up> _Up *_M_data_ptr(_Up *__ptr) const { return __ptr; }
