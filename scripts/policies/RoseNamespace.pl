...
my @policies = (
	...
	include a Rose header and then check whether its corresponding ROSE_*
	symbol is defined in order to determine whether that capability is
	available. The
	...
	reason =>
	'Some common file system configurations are case-insensitive
        and are thus unable to handle some situations where two files have names
...
my %bad_words = (
    ...
    'uninit'            => 'Uninitialized',	  # Compass, ROSETTA-generated code, Flang frontend, generic data-flow, etc.
...
while (my $filename = $files->next_file) {
	...
	if (! -r $parentHeader) {
		...
		# to have both <rose.h> and <Rose.h> on case-insensitive file systems.
	    } else {
