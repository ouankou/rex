...
public:
...
#endif
  inline globalID &operator=(const globalID &g) {
    m_gid = g.m_gid;
    return *this;
  }
  inline globalID &operator+=(const globalID &g) {
  ...
  inline globalID &operator+=(const int &g) {
    m_gid += g;
    return *this;
  }
  inline globalID &operator-=(const int &g) {
  ...
  inline globalID operator--(int) { return globalID(m_gid--); }
  inline globalID operator+(const globalID &g) const {
    return globalID(*this) += g;
  }
  inline globalID operator-(const globalID &g) const {
    return globalID(*this) -= g;
  }
  inline globalID operator%(const globalID &g) const {
    return globalID(m_gid % g.m_gid);
  }
  inline globalID operator/(const globalID &g) const {
    return globalID(m_gid / g.m_gid);
  }
  ...
  inline int operator%(const int &i) const {
    return static_cast<int>(m_gid % i);
  }
  ...
  inline int Int() const { return (int)m_gid; }
