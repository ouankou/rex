 ...
 *  container, load the specimen into memory, disassemble, and discover functions. The results are stored in a
 *  `Partitioner2::Partitioner` ("Partitioner") object:
 *
 *  @snippet binaryFunctionCfg.C partition
 *
 *  Finally, we'll iterate over those functions to create function control flow graphs and print each graph.  The
 ...
 *
 *  In the above code, the `Partitioner2::ControlFlowGraph` is a specialization of `Sawyer::Graph`, which is a generic
 ...
 *
 *  The `Partitioner2::DataFlow` namespace has a `Partitioner2::DataFlow::buildDfCfg` ("buildDfCfg") function that creates a
 ...
 *
 *  This example is similar to the hello-world example, but demonstrates how to analyze function call information to
 ...
 *
 *  This time we'll use a few namespaces to reduce our typing. The `Rose::Diagnostics` namespace brings into scope the
 ...
 *
 *  The hello-world example showed the simplest way to parse a command-line, but this time we'll do something a
 ...
 *
 *  Instead of calling `Partitioner2::Engine::frontend` ("engine.frontend"), this time we call
 ...
 *
 *  The partitioner knows how to construct a call graph from its internal data structures.  The
 ...
 *
 *  Finally we can emit the call graph as a GraphViz file. This is done through a `Partitioner2::GraphViz::CgEmitter`
 ...
 *
 *  Yet another way to parse a command-line.  This time we're trying to avoid calling `Partitioner2::Engine::frontend`
 ...
 *
 *  The `Partitioner2::Engine::loadSpecimens` ("engine.loadSpecimens") method parses the speicmen container if present (e.g.,
 *  Linux ELF) and determines how the specimen should be mapped into memory.  The result is a `MemoryMap` that describes
 *  memory segments with addresses, permissions, names, etc.  Try inserting a call to `MemoryMap::dump` here to get an
 ...
 *  instantiates the analyzer, configures it, calls some method to perform the analysis (here it's
 *  `Strings::StringFinder::find` ("find")), and then queries the results.
 *
 *  The `MemoryMap::require` and `MemoryMap::prohibit` methods are a form of filtering. They're filtering the memory
 ...
 *
 *  ROSE uses the `Sawyer::Container::Graph` ("Sawyer Graph API") for all binary analysis graphs, and Sawyer has functions for
 ...
 *
 *  The `Rose::initialize` ("ROSE_INITIALIZE") macro is always the first ROSE-related statement and checks that this tool was
 ...
 *  dumps, S-Records, and/or process IDs.  The `Rose::BinaryAnalysis::Partitioner2::Engine::partition` ("engine.partition")
 *  call does all the hard work of parsing containers, loading data into the virtual address space, decoding CPU instructions,
 *  and creating basic blocks and functions. It returns a `Rose::BinaryAnalysis::Partitioner2::Partitioner` ("partitioner")
 ...
 *
 *  The `Sawyer::Container::Algorithm::graphDominators` ("graphDominators") function can handle any type of Sawyer graph, thus
 ...
 *  `Rose::BinaryAnalysis::Partitioner2::Partitioner::vertexName` ("vertexName") static method to print some info about
 ...
 *
 *  Most binary analysis capabilities are documented in the `Rose::BinaryAnalysis` namespace. The test directory,
