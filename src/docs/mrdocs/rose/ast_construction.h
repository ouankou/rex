...
#ifndef ROSE_DOCS_ROSE_AST_CONSTRUCTION_H
#define ROSE_DOCS_ROSE_AST_CONSTRUCTION_H

/** @brief AST construction
 *
 ...
 * manipulation interfaces defined in the `SageBuilder` and `SageInterface`
 * namespaces.
 *
 ...
 *
 * Building the variable declaration using low-level member functions of SAGE
 * III node classes. The low-level approach requires manual handling of scope,
 * parent pointers, and symbol tables.
 *
 *  Full listing: `tutorial/addVariableDeclaration.C`
 *
 * @section expr_addition Adding Expressions
 *
 * A translator using the high-level AST builder interface can be used to add an
 * assignment statement before the last statement in a `main()` function.
 *
 ...
 *
 * Adding a function at the top of a global scope using both high-level and
 ...
 *
 * @subsection func_decl_scope_stack High-Level Function Declaration with Scope
 * Stack
 *
 ...
 *
 * Adding function calls to instrument code using the AST string-based rewrite
 * mechanism or the AST builder interface.
 *
 ...
 * to support Charm++. The translator also updates all references to global
 * variables so that they reference the variables indirectly through the
 * `struct`. This is a preprocessing step required to use Charm++ and AMPI.
 *
 * The example uses low-level AST manipulation at the level of the IR. More
 * concise versions using `SageInterface` and `SageBuilder` functions should be
 * considered for high-level manipulation.
 *
 * @subsection global_struct_example1 Repackaging Global Variables
 *
 * The following example repackages global variables in an application into a
 * `struct`.
 *
 *  Full listing: `tutorial/CharmSupport.C`
 *
 * @subsection global_struct_example2 Referencing Global Variables via Struct
 * (Part 2)
 *
 * This part shows the continuation of the repackaging of global variables into
 * a `struct`.
 *
 *  Full listing: `tutorial/CharmSupport.C`
 *
 ...
 *
 * The third part of the example, continuing the transformation of global
 * variables into a `struct`.
 *
 *  Full listing: `tutorial/CharmSupport.C`
 *
 * @subsection global_struct_example4 Final Steps of Global Variable Repackaging
 * (Part 4)
 *
 * This part completes the transformation and repackaging of global variables
 * into a `struct`.
 *
 *  Full listing: `tutorial/CharmSupport.C`
 *
 ...
 *
 * This is the final part of the global variable repackaging and struct
 ...
 *
 * Example source code used as input to the translator for repackaging global
 * variables into a `struct`.
 *
 *  Full listing: `tutorial/inputCode_ExampleCharmSupport.C`
 *
 * @subsection global_struct_output Example Output of Global Variable
 * Repackaging
 *
 * The output of the input after the translator repackages the global variables
 * into a `struct`.
 *
