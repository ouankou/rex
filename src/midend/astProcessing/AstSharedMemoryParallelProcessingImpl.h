...
// Perhaps this code should be fixed so it works on a single thread when
// multi-thread support is disabled by the user. Until then, I am commenting out
// this entire file when multi-threading support is not configured. [Robb P.
// Matzke 2015-03-14]
#ifdef _REENTRANT // Does the user want multi-threaded support? (e.g., g++
                  // -pthread)
...
AstSharedMemoryParallelizableTopDownBottomUpProcessing<I, S>::
    AstSharedMemoryParallelizableTopDownBottomUpProcessing(
        ...
        const typename AstSharedMemoryParallelizableTopDownBottomUpProcessing<
            I, S>::TraversalPtrList &t)
...
template <class I, class S>
void AstSharedMemoryParallelizableTopDownBottomUpProcessing<
    I, S>::set_runningParallelTraversal(bool val) {
  runningParallelTraversal = val;
}

template <class I, class S>
typename AstSharedMemoryParallelizableTopDownBottomUpProcessing<
    I, S>::InheritedAttributeTypeList *
AstSharedMemoryParallelizableTopDownBottomUpProcessing<I, S>::
    evaluateInheritedAttribute(
        SgNode *astNode,
        typename AstSharedMemoryParallelizableTopDownBottomUpProcessing<
            I, S>::InheritedAttributeTypeList *inheritedValues) {
...
void *parallelTopDownBottomUpProcessingThread(void *p) {
  AstSharedMemoryParallelTopDownBottomUpThreadArgs<I, S> *threadArgs =
      (AstSharedMemoryParallelTopDownBottomUpThreadArgs<I, S> *)p;

  AstSharedMemoryParallelizableTopDownBottomUpProcessing<I, S> *traversal =
      threadArgs->traversal;
  SgNode *basenode = threadArgs->basenode;
  typename AstSharedMemoryParallelizableTopDownBottomUpProcessing<
      I, S>::InheritedAttributeTypeList *inheritedValues =
      threadArgs->inheritedValues;
  delete threadArgs;
...
template <class I, class S>
typename AstSharedMemoryParallelTopDownBottomUpProcessing<
    I, S>::SynthesizedAttributeTypeList *
AstSharedMemoryParallelTopDownBottomUpProcessing<I, S>::traverseInParallel(
    SgNode *basenode, typename AstSharedMemoryParallelTopDownBottomUpProcessing<
                          I, S>::InheritedAttributeTypeList *inheritedValues) {
  // GB (09/27/2007): Is this really required? Inheritance of templated classes
  // is just weird.
  const typename Superclass::TraversalPtrList &traversals =
      Superclass::traversals;

  size_t numberOfTraversals = traversals.size();
  size_t i;

  AstSharedMemoryParallelProcessingSynchronizationInfo syncInfo(
      numberOfThreads, synchronizationWindowSize);
  ...
  std::vector<InheritedAttributeTypeList *> parallelInheritedValues(
      numberOfThreads);
  size_t begin = 0, end;
  for (i = 0; i < numberOfThreads; i++) {
    ...
    parallelTraversals[i] =
        new AstSharedMemoryParallelizableTopDownBottomUpProcessing<I, S>(
            syncInfo, std::vector<TraversalPtr>(traversals.begin() + begin,
                                                traversals.begin() + end));
    parallelInheritedValues[i] = new InheritedAttributeTypeList(
        inheritedValues->begin() + begin, inheritedValues->begin() + end);
    begin = end;
  }

#if defined(_REENTRANT) &&                                                     \
    defined(ROSE_HAVE_PTHREAD_H) // user wants multi-thread support and POSIX
  ...
  for (i = 0; i < numberOfThreads; i++) {
    pthread_create(
        &threads[i], NULL, parallelTopDownBottomUpProcessingThread<I, S>,
  ...
  SynthesizedAttributeTypeList *flatFinalResults =
      new SynthesizedAttributeTypeList;
  for (i = 0; i < numberOfThreads; i++)
    std::copy(finalResults[i]->begin(), finalResults[i]->end(),
              std::back_inserter(*flatFinalResults));
...
AstSharedMemoryParallelTopDownBottomUpProcessing<
    I, S>::AstSharedMemoryParallelTopDownBottomUpProcessing()
    : numberOfThreads(2), synchronizationWindowSize(100000) {}

template <class I, class S>
AstSharedMemoryParallelTopDownBottomUpProcessing<I, S>::
    AstSharedMemoryParallelTopDownBottomUpProcessing(
        const AstSharedMemoryParallelTopDownBottomUpProcessing<
            I, S>::TraversalPtrList &traversals)
    : AstCombinedTopDownBottomUpProcessing<I, S>(traversals),
      numberOfThreads(2), synchronizationWindowSize(100000) {}

template <class I, class S>
void AstSharedMemoryParallelTopDownBottomUpProcessing<
    I, S>::set_numberOfThreads(size_t threads) {
#if !USE_ROSE
...
template <class I, class S>
void AstSharedMemoryParallelTopDownBottomUpProcessing<
    I, S>::set_synchronizationWindowSize(size_t windowSize) {
#if !USE_ROSE
  // DQ (11/3/2011): legacy frontend compilains about this (but GNU allowed it,
  // I think that legacy frontend might be correct since it is a private
  // variable.  But since we are only trying to compile ROSE with ROSE (using
...
AstSharedMemoryParallelizableTopDownProcessing<I>::
    AstSharedMemoryParallelizableTopDownProcessing(
        const AstSharedMemoryParallelProcessingSynchronizationInfo &syncInfo,
        const typename AstSharedMemoryParallelizableTopDownProcessing<
            I>::TraversalPtrList &t)
...
template <class I>
void AstSharedMemoryParallelizableTopDownProcessing<
    I>::set_runningParallelTraversal(bool val) {
  runningParallelTraversal = val;
}

template <class I>
typename AstSharedMemoryParallelizableTopDownProcessing<
    I>::InheritedAttributeTypeList *
AstSharedMemoryParallelizableTopDownProcessing<I>::evaluateInheritedAttribute(
    SgNode *astNode, typename AstSharedMemoryParallelizableTopDownProcessing<
                         I>::InheritedAttributeTypeList *inheritedValues) {
  if (runningParallelTraversal && ++visitedNodes > synchronizationWindowSize) {
    synchronize();
    visitedNodes = 0;
  }
...
template <class I>
void AstSharedMemoryParallelizableTopDownProcessing<I>::atTraversalEnd() {
  // delegate the call to the superclass
  Superclass::atTraversalEnd();
...
// traversal class that contains a number of traversals, the node to start the
// traversal from, and the list of initial inherited values.
template <class I> struct AstSharedMemoryParallelTopDownThreadArgs {
...
// result is returned. Synchronization is built into the parallelizable
// processing classes.
template <class I> void *parallelTopDownProcessingThread(void *p) {
  AstSharedMemoryParallelTopDownThreadArgs<I> *threadArgs =
      (AstSharedMemoryParallelTopDownThreadArgs<I> *)p;
...
template <class I>
void AstSharedMemoryParallelTopDownProcessing<I>::traverseInParallel(
    SgNode *basenode, typename AstSharedMemoryParallelTopDownProcessing<
                          I>::InheritedAttributeTypeList *inheritedValues) {
  const typename Superclass::TraversalPtrList &traversals =
      Superclass::traversals;
  ...
  for (i = 0; i < numberOfThreads; i++) {
    ...
    parallelTraversals[i] =
        new AstSharedMemoryParallelizableTopDownProcessing<I>(
            syncInfo, std::vector<TraversalPtr>(traversals.begin() + begin,
                                                traversals.begin() + end));
    parallelInheritedValues[i] = new InheritedAttributeTypeList(
        inheritedValues->begin() + begin, inheritedValues->begin() + end);
  ...
  for (i = 0; i < numberOfThreads; i++) {
    pthread_create(
        &threads[i], NULL, parallelTopDownProcessingThread<I>,
...
AstSharedMemoryParallelTopDownProcessing<
    I>::AstSharedMemoryParallelTopDownProcessing()
    : numberOfThreads(2), synchronizationWindowSize(100000) {}

template <class I>
AstSharedMemoryParallelTopDownProcessing<I>::
    AstSharedMemoryParallelTopDownProcessing(
        const AstSharedMemoryParallelTopDownProcessing<I>::TraversalPtrList
            &traversals)
    : AstCombinedTopDownProcessing<I>(traversals), numberOfThreads(2),
      synchronizationWindowSize(100000) {}

template <class I>
void AstSharedMemoryParallelTopDownProcessing<I>::set_numberOfThreads(
    size_t threads) {
#if !USE_ROSE
  // DQ (11/3/2011): legacy frontend compilains about this (but GNU allowed it,
  // I think that legacy frontend might be correct since it is a private
...
template <class I>
void AstSharedMemoryParallelTopDownProcessing<I>::set_synchronizationWindowSize(
    size_t windowSize) {
#if !USE_ROSE
  // DQ (11/3/2011): legacy frontend compilains about this (but GNU allowed it,
  // I think that legacy frontend might be correct since it is a private
...
AstSharedMemoryParallelizableBottomUpProcessing<S>::
    AstSharedMemoryParallelizableBottomUpProcessing(
        const AstSharedMemoryParallelProcessingSynchronizationInfo &syncInfo,
        const typename AstSharedMemoryParallelizableBottomUpProcessing<
            S>::TraversalPtrList &t)
...
template <class S>
void AstSharedMemoryParallelizableBottomUpProcessing<
    S>::set_runningParallelTraversal(bool val) {
  runningParallelTraversal = val;
}

template <class S>
typename AstSharedMemoryParallelizableBottomUpProcessing<
    S>::SynthesizedAttributeTypeList *
AstSharedMemoryParallelizableBottomUpProcessing<S>::
    evaluateSynthesizedAttribute(
        SgNode *astNode,
        typename AstSharedMemoryParallelizableBottomUpProcessing<
            S>::SynthesizedAttributesList synthesizedAttributes) {
  if (runningParallelTraversal && ++visitedNodes > synchronizationWindowSize) {
    synchronize();
    visitedNodes = 0;
  }
  ...
  // let the superclass handle actual attribute evaluation
  return Superclass::evaluateSynthesizedAttribute(astNode,
                                                  synthesizedAttributes);
}

template <class S>
void AstSharedMemoryParallelizableBottomUpProcessing<S>::atTraversalEnd() {
  // delegate the call to the superclass
  Superclass::atTraversalEnd();
...
// the traversal from.
template <class S> struct AstSharedMemoryParallelBottomUpThreadArgs {
...
// result is returned. Synchronization is built into the parallelizable
// processing classes.
template <class S> void *parallelBottomUpProcessingThread(void *p) {
  AstSharedMemoryParallelBottomUpThreadArgs<S> *threadArgs =
      (AstSharedMemoryParallelBottomUpThreadArgs<S> *)p;
...
template <class S>
typename AstSharedMemoryParallelBottomUpProcessing<
    S>::SynthesizedAttributeTypeList *
AstSharedMemoryParallelBottomUpProcessing<S>::traverseInParallel(
    SgNode *basenode) {
  const typename Superclass::TraversalPtrList &traversals =
      Superclass::traversals;
  ...
  for (i = 0; i < numberOfThreads; i++) {
    ...
    parallelTraversals[i] =
        new AstSharedMemoryParallelizableBottomUpProcessing<S>(
            syncInfo, std::vector<TraversalPtr>(traversals.begin() + begin,
                                                traversals.begin() + end));
    begin = end;
  }
  ...
  pthread_t *threads = new pthread_t[numberOfThreads];
  for (i = 0; i < numberOfThreads; i++) {
    pthread_create(&threads[i], NULL, parallelBottomUpProcessingThread<S>,
...
AstSharedMemoryParallelBottomUpProcessing<
    S>::AstSharedMemoryParallelBottomUpProcessing()
    : numberOfThreads(2), synchronizationWindowSize(100000) {}

template <class S>
AstSharedMemoryParallelBottomUpProcessing<S>::
    AstSharedMemoryParallelBottomUpProcessing(
        const AstSharedMemoryParallelBottomUpProcessing<S>::TraversalPtrList
            &traversals)
    : AstCombinedBottomUpProcessing<S>(traversals), numberOfThreads(2),
      synchronizationWindowSize(100000) {}

template <class S>
void AstSharedMemoryParallelBottomUpProcessing<S>::set_numberOfThreads(
    size_t threads) {
#if !USE_ROSE
  // DQ (11/3/2011): legacy frontend compilains about this (but GNU allowed it,
  // I think that legacy frontend might be correct since it is a private
...
template <class S>
void AstSharedMemoryParallelBottomUpProcessing<
    S>::set_synchronizationWindowSize(size_t windowSize) {
#if !USE_ROSE
  // DQ (11/3/2011): legacy frontend compilains about this (but GNU allowed it,
  // I think that legacy frontend might be correct since it is a private
  // variable.  But since we are only trying to compile ROSE with ROSE (using
