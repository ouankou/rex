...
std::vector<std::string>
    PluginActions; // one or more actions defined in the shared libs
std::map<std::string, std::vector<std::string>> PluginArgs; // plugin arguments

template class Registry<PluginAction>;

// CommandlineProcessing::isOptionWithParameter(argv,"-rose:","plugin_lib",
// lib_name, true) does not work. it will extract the last hit and remove all
// matched pairs!!
//
// A helper function to extract -rose:xx value pair from command lines
// each call will extract the first match and return.
bool extractOptionWithParameter(vector<string> &argv, const string &option,
                                string &value) {
...
//! Hooked up with SgProject::frontend(..) to process command line options
//! related to plugins
/*
-rose:plugin_lib filename //  multiple .so, each is loaded with -load file.so
-rose:plugin_action xxx // one or more actions to be called, each is specified
with -plugin action_name -rose:plugin_arg_xxx // arguments to each action, each
is specified with -plugin-arg-name …
 */
void processPluginCommandLine(vector<string> &argv) {
  ...
  do {
    string act_name;
    // if
    // (CommandlineProcessing::isOptionWithParameter(argv,"-rose:","plugin_action",
    // act_name, true))
  ...
  for (size_t i = 0; i < PluginActions.size(); i++) {
    ...
    do {
      string arg;
      // if (CommandlineProcessing::isOptionWithParameter(argv,"-rose:",
      // "plugin_arg_"+act_name, arg, true))
      if (extractOptionWithParameter(argv, "-rose:plugin_arg_" + act_name,
                                     arg)) {
        args.push_back(arg);
        found = true;
      } else
...
// int obtainAndExecuteActions(const char* lib_file_name, string action_name,
// SgNode* n)
int obtainAndExecuteActions(SgProject *n) {
  ...
  for (size_t i = 0; i < PluginLibs.size(); i++) {
    ...
    if (!handle) {
      ...
      cout << "Error in dlopen: error code: " << errKode << " when loading "
           << lib_file_name << endl;
      exit(1);
    }
  }
