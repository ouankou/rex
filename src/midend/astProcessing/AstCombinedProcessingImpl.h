...
SgCombinedTreeTraversal<I, S>::SgCombinedTreeTraversal() : traversals() {}
...
template <class I, class S>
void SgCombinedTreeTraversal<I, S>::addTraversal(
    typename SgCombinedTreeTraversal<I, S>::TraversalPtr t) {
  traversals.push_back(t);
}

template <class I, class S>
typename SgCombinedTreeTraversal<I, S>::TraversalPtrList &
SgCombinedTreeTraversal<I, S>::get_traversalPtrListRef() {
  return traversals;
}

template <class I, class S>
typename SgCombinedTreeTraversal<I, S>::InheritedAttributeTypeList *
SgCombinedTreeTraversal<I, S>::evaluateInheritedAttribute(
    SgNode *astNode,
    typename SgCombinedTreeTraversal<I, S>::InheritedAttributeTypeList
        *inheritedValues) {
  InheritedAttributeTypeList *result = new InheritedAttributeTypeList();
  ...
  typename InheritedAttributeTypeList::iterator i = inheritedValues->begin();
...
template <class I, class S>
typename SgCombinedTreeTraversal<I, S>::SynthesizedAttributeTypeList *
SgCombinedTreeTraversal<I, S>::evaluateSynthesizedAttribute(
    SgNode *astNode,
    typename SgCombinedTreeTraversal<I, S>::InheritedAttributeTypeList
        *inheritedValues,
    typename SgCombinedTreeTraversal<I, S>::SynthesizedAttributesList
  ...
  typename TraversalType::SynthesizedAttributesList attributesForTraversal(M);
  // Create a list for the traversal results.
  SynthesizedAttributeTypeList *result = new SynthesizedAttributeTypeList();
  result->reserve(numberOfTraversals);
  ...
  for (i = 0; i < numberOfTraversals; i++) {
    ...
    result->push_back(traversals[i]->evaluateSynthesizedAttribute(
        astNode, (*inheritedValues)[i], attributesForTraversal));
  }
...
template <class I, class S>
typename SgCombinedTreeTraversal<I, S>::SynthesizedAttributeTypeList *
SgCombinedTreeTraversal<I, S>::defaultSynthesizedAttribute(
    typename SgCombinedTreeTraversal<I, S>::InheritedAttributeTypeList
        *inheritedValues) {
...
template <class I, class S>
AstCombinedTopDownBottomUpProcessing<I,
                                     S>::AstCombinedTopDownBottomUpProcessing()
    : traversals() {}

template <class I, class S>
AstCombinedTopDownBottomUpProcessing<I, S>::
    AstCombinedTopDownBottomUpProcessing(
        const typename AstCombinedTopDownBottomUpProcessing<
            I, S>::TraversalPtrList &t)
    : traversals(t) {}

template <class I, class S>
void AstCombinedTopDownBottomUpProcessing<I, S>::addTraversal(
    typename AstCombinedTopDownBottomUpProcessing<I, S>::TraversalPtr t) {
  traversals.push_back(t);
}

template <class I, class S>
typename AstCombinedTopDownBottomUpProcessing<I, S>::TraversalPtrList &
AstCombinedTopDownBottomUpProcessing<I, S>::get_traversalPtrListRef() {
  return traversals;
}

template <class I, class S>
typename AstCombinedTopDownBottomUpProcessing<I,
                                              S>::InheritedAttributeTypeList *
AstCombinedTopDownBottomUpProcessing<I, S>::evaluateInheritedAttribute(
    SgNode *astNode, typename AstCombinedTopDownBottomUpProcessing<
...
AstCombinedTopDownBottomUpProcessing<I, S>::evaluateSynthesizedAttribute(
    SgNode *astNode,
    typename AstCombinedTopDownBottomUpProcessing<
        I, S>::InheritedAttributeTypeList *inheritedValues,
    typename AstCombinedTopDownBottomUpProcessing<
        I, S>::SynthesizedAttributesList synthesizedAttributes) {
  // Let N = number of traversals, M = number of node successors.
  // synthesizedAttributes is a list of M lists of N attributes each; we
  // want to call each of the N traversals with the corresponding M
  // synthesized attributes.
...
typename AstCombinedTopDownBottomUpProcessing<I,
                                              S>::SynthesizedAttributeTypeList *
AstCombinedTopDownBottomUpProcessing<I, S>::defaultSynthesizedAttribute(
    typename AstCombinedTopDownBottomUpProcessing<
        I, S>::InheritedAttributeTypeList *inheritedValues) {
  // Create a list for the default attributes.
  SynthesizedAttributeTypeList *result =
...
template <class I>
AstCombinedTopDownProcessing<I>::AstCombinedTopDownProcessing()
    : traversals() {}
...
template <class I>
void AstCombinedTopDownProcessing<I>::addTraversal(
    typename AstCombinedTopDownProcessing<I>::TraversalPtr t) {
  traversals.push_back(t);
}

template <class I>
typename AstCombinedTopDownProcessing<I>::TraversalPtrList &
AstCombinedTopDownProcessing<I>::get_traversalPtrListRef() {
  return traversals;
}

template <class I>
typename AstCombinedTopDownProcessing<I>::InheritedAttributeTypeList *
AstCombinedTopDownProcessing<I>::evaluateInheritedAttribute(
    SgNode *astNode,
    typename AstCombinedTopDownProcessing<I>::InheritedAttributeTypeList
        *inheritedValues) {
  InheritedAttributeTypeList *result = new InheritedAttributeTypeList();
...
template <class I> void AstCombinedTopDownProcessing<I>::atTraversalStart() {
  // Compute some values that are constant during the traversal, saving
  // lots of method calls. We cannot reliably compute these earlier
  // because the user is allowed to mess with the traversal container
  // using the reference we provide.
...
template <class I> void AstCombinedTopDownProcessing<I>::atTraversalEnd() {
  // Call this function for all traversals.
  typename TraversalPtrList::iterator t;
  for (t = tBegin; t != tEnd; ++t)
    (*t)->atTraversalEnd();
...
template <class I>
void AstCombinedTopDownProcessing<I>::destroyInheritedValue(
    SgNode *node,
    typename AstCombinedTopDownProcessing<I>::InheritedAttributeTypeList
        *inheritedValues) {
  typename TraversalPtrList::iterator t = tBegin;
...
template <class S>
AstCombinedBottomUpProcessing<S>::AstCombinedBottomUpProcessing()
    : traversals() {}
...
template <class S>
void AstCombinedBottomUpProcessing<S>::addTraversal(
    typename AstCombinedBottomUpProcessing<S>::TraversalPtr t) {
  traversals.push_back(t);
}

template <class S>
typename AstCombinedBottomUpProcessing<S>::TraversalPtrList &
AstCombinedBottomUpProcessing<S>::get_traversalPtrListRef() {
  return traversals;
}

template <class S>
typename AstCombinedBottomUpProcessing<S>::SynthesizedAttributeTypeList *
AstCombinedBottomUpProcessing<S>::evaluateSynthesizedAttribute(
    SgNode *astNode,
    typename AstCombinedBottomUpProcessing<S>::SynthesizedAttributesList
        synthesizedAttributes) {
  // Let N = number of traversals, M = number of node successors.
  // synthesizedAttributes is a list of M lists of N attributes each; we
  // want to call each of the N traversals with the corresponding M
  ...
  // Create a container of size M in which we will store the synthesized
  // attributes for each traversal.
  typename TraversalType::SynthesizedAttributesList attributesForTraversal(
      M, NULL);
  // Create a list for the traversal results.
  SynthesizedAttributeTypeList *result =
      new SynthesizedAttributeTypeList(numberOfTraversals);
  ...
  for (i = 0; i < numberOfTraversals; i++) {
    ...
    (*result)[i] = traversals[i]->evaluateSynthesizedAttribute(
        astNode, attributesForTraversal);
  }
  ROSE_ASSERT(result->size() == numberOfTraversals);
...
template <class S>
typename AstCombinedBottomUpProcessing<S>::SynthesizedAttributeTypeList *
AstCombinedBottomUpProcessing<S>::defaultSynthesizedAttribute() {
  // Create a list for the default attributes.
  SynthesizedAttributeTypeList *result =
      new SynthesizedAttributeTypeList(numberOfTraversals);
...
template <class S> void AstCombinedBottomUpProcessing<S>::atTraversalStart() {
  // Compute some values that are constant during the traversal, saving
  // lots of method calls. We cannot reliably compute these earlier
  // because the user is allowed to mess with the traversal container
  // using the reference we provide.
...
template <class S> void AstCombinedBottomUpProcessing<S>::atTraversalEnd() {
  // Call this function for all traversals.
  typename TraversalPtrList::iterator t;
  for (t = tBegin; t != tEnd; ++t)
    (*t)->atTraversalEnd();
