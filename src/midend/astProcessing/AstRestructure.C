...
void AstRestructure::lock(SgNode *astNode) {
  ...
  if (f == lockedNodes.end()) {
    ...
    cerr << "ERROR: attempted to re-lock node " << astNode->sage_class_name()
         << " @" << astNode << endl;
    ROSE_ABORT();
  }
}
...
void AstRestructure::unlock(SgNode *astNode) {
  ...
  if (f != lockedNodes.end()) {
    ...
    cerr << "ERROR: attempted to unlock non-locked node "
         << astNode->sage_class_name() << " @" << astNode << endl;
    ROSE_ABORT();
  }
}

void AstRestructure::delayedReplace(SgNode *astNode, string s) {
...
void AstRestructure::immediateReplace(SgStatement *astNode, string s) {
  ...
  cout << s << endl;
  // TODO: comment out the MiddleLevelRewrite::insert call since the legacy
  // rewrite system is removed.
  cerr << "MiddleLevelRewrite::insert is unavailable; legacy rewrite system "
          "removed."
       << endl;
  ROSE_ABORT();
...
cout << "IMMEDIATE REPLACE RESULT: PARENT's NEW AST-SOURCE: " <<
...
AstRestructure::evaluateRewriteInheritedAttribute (SgNode* astNode,
                                                   RestructureInhType
inheritedValue ) { RestructureInhType inh(inheritedValue,astNode); return inh;
}

RestructureSynType
AstRestructure::evaluateRewriteSynthesizedAttribute (SgNode* astNode,
                                                      RestructureInhType
inheritedValue, SubTreeSynthesizedAttributes synList ) { RestructureSynType
sa(astNode); for (SubTreeSynthesizedAttributes::iterator i = synList.begin(); i
!= synList.end(); i++) { sa+=*i;
  }
  ...
  if(astNode==targetNode) {
    ...
    AST_Rewrite::AST_FragmentString
sourceFragmentSpecifier1(includeHeaders,inheritedValue, AST_Rewrite::Preamble,
                                                            AST_Rewrite::PreamblePositionInScope
                                                             );
    sa.addSourceCodeString(sourceFragmentSpecifier1);
    MidL
    AST_Rewrite::AST_FragmentString
sourceFragmentSpecifier2(sourceFragment,inheritedValue, AST_Rewrite::LocalScope,
