// Author: Gergo Barany
// $Id: AstSharedMemoryParallelProcessing.h,v 1.1 2008/01/08 02:56:39 dquinlan
// Exp $
...
// this (they all need the same pointers!), the default copy mechanism takes
// care of that.
...
#warning "POSIX threads are unavailable on this platform."
  ...
  // We can't implement this as private since it is required.  So we have a
  // potential double free error here. private: DQ (9/13/2011): This copy
  // constructor was built because static analysis tools (made it private to
  // force compile time error if used).
  AstSharedMemoryParallelProcessingSynchronizationInfo(
      const AstSharedMemoryParallelProcessingSynchronizationInfo &X);
};

// Class containing the code needed for synchronization of parallelizable
// traversals. The parallelizable processing classes inherit privately from this
// because the code is the same for all of them.
...
// Class representing a traversal that can run in parallel with some other
// instances of the same type. It is basically a combined processing class with
// a thin synchronization layer. The user will probably never need to
// instantiate this class, they should use the
// AstSharedMemoryParallel*Processing classes instead.
template <class InheritedAttributeType, class SynthesizedAttributeType>
class AstSharedMemoryParallelizableTopDownBottomUpProcessing
    : public AstCombinedTopDownBottomUpProcessing<InheritedAttributeType,
                                                  SynthesizedAttributeType>,
      private AstSharedMemoryParallelProcessingSynchronizationBase {
public:
  typedef AstCombinedTopDownBottomUpProcessing<InheritedAttributeType,
                                               SynthesizedAttributeType>
      Superclass;
  ...
  typedef typename Superclass::InheritedAttributeTypeList
      InheritedAttributeTypeList;
  typedef typename Superclass::SynthesizedAttributeTypeList
...
// replacement for the corresponding AstCombined*Processing class, the usage is
// identical except that you call traverseInParallel() instead of traverse().
// (Calling traverse() is identical to AstCombined*Processing, i.e. it will not
// run in parallel.)
template <class InheritedAttributeType, class SynthesizedAttributeType>
...
public:
  ...
  typedef AstSharedMemoryParallelizableTopDownBottomUpProcessing<
      InheritedAttributeType, SynthesizedAttributeType>
      *ParallelizableTraversalPtr;
  typedef std::vector<ParallelizableTraversalPtr>
      ParallelizableTraversalPtrList;

  SynthesizedAttributeTypeList *
  traverseInParallel(SgNode *basenode,
...
public:
  ...
  typedef AstSharedMemoryParallelizableTopDownProcessing<InheritedAttributeType>
      *ParallelizableTraversalPtr;
  typedef std::vector<ParallelizableTraversalPtr>
      ParallelizableTraversalPtrList;
...
public:
  ...
  typedef AstSharedMemoryParallelizableBottomUpProcessing<
      SynthesizedAttributeType> *ParallelizableTraversalPtr;
  typedef std::vector<ParallelizableTraversalPtr>
      ParallelizableTraversalPtrList;
