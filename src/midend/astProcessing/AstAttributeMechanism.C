...
static std::string attributeFullName(const std::string &name,
                                     AstAttribute *value) {
  std::string retval;
  ...
  if (value == NULL) {
    ...
    retval = "attribute " + name + "(" + value->toString() + ") of " +
             className + " type";
  }
  return retval;
}

static void deleteAttributeValue(const std::string &name, AstAttribute *value) {
  if (value != NULL) {
    ...
    case AstAttribute::NO_OWNERSHIP:
      MLOG_WARN_CXX("midend:astProcessing")
          << attributeFullName(name, value) << "is leaked\n";
      break;
    ...
    case AstAttribute::UNKNOWN_OWNERSHIP: {
      MLOG_WARN_CXX("midend:astProcessing")
          << "attribute " << attributeFullName(name, value)
          << "ownership is unknown and possibly leaked\n";

      // Show details about how to fix this only once per program.
      static bool detailsShown = false;
      if (!detailsShown) {
        MLOG_WARN_CXX("midend:astProcessing")
            << "    This attribute's class should include a definition for the "
               "virtual\n"
            << "    \"getOwnershipPolicy\" based on the intention the author "
               "had for how\n"
            << "    memory is managed for this attribute type.  If the author "
               "intended\n"
            << "    attribute memory to be managed by ROSE's "
               "AstAttributeMechanism, then\n"
            << "    the following definition should be added to the "
               "attribute's class:\n"
            << "        virtual AstAttribute::OwnershipPolicy\n"
            << "        getOwnershipPolicy() const override {\n"
            << "            return CONTAINER_OWNERSHIP;\n"
            << "        }\n"
            << "    and the attribute should not be explicitly deleted by the "
               "attribute\n"
            << "    creator or any users of the attribute. See documentation "
               "for\n"
...
AstAttributeMechanism::~AstAttributeMechanism() { clearAttributes(); }

void AstAttributeMechanism::clearAttributes() {
...
bool AstAttributeMechanism::add(const std::string &name, AstAttribute *value) {
  auto it = attributes_.find(name);
...
size_t AstAttributeMechanism::size() const { return attributes_.size(); }

// Construction and assignment. Must be exception-safe.
void AstAttributeMechanism::assignFrom(const AstAttributeMechanism &other) {
  if (this == &other)
    return;
  AstAttributeMechanism tmp; // for exception safety
  for (auto it = other.attributes_.begin(); it != other.attributes_.end();
       ++it) {
    /*!const*/ AstAttribute *attr = it->second;
    ASSERT_not_null(attr);

    // Copy the attribute. This might throw, which is why we're using "tmp". If
    // it throws, then we don't ever make it to the std::swap below, and the
    // destination is unchanged and tmp will be cleaned up according to the
    // ownership policy of each attribute. Also, as mentioned in the header
    // file, be sure we invoke the non-const version of "copy" for the sake of
    // subclasses that have not been modified to define the const version.
    AstAttribute *copied = attr->copy();

    // Check various things about the attribute.  Some of these might be
    // important in places other than copying, but we report them here because
    // there's a large amount of user code that doesn't follow the rules and we
    // don't want to be completely obnoxious.
    if (!copied)
      MLOG_WARN_CXX("midend:astProcessing")
          << attributeFullName(it->first, it->second)
          << " was not copied; no virtual copy function?\n";
    ...
    case AstAttribute::CONTAINER_OWNERSHIP:
      ASSERT_require2(
          copied != attr,
          "virtual copy function for \"%s\" did not copy attribute\n",
          attributeFullName(it->first, it->second).c_str());
      break;
    case AstAttribute::NO_OWNERSHIP:
      // copy() is allowed to return itself; we don't care since we won't ever
      // delete either attribute A leak will be reported instead of deleting the
      // attribute.
      break;
    case AstAttribute::CUSTOM_OWNERSHIP:
      // copy() is allowed to return itself; we don't care since we won't ever
      // delete either attribute Assume attribute ownership is properly
      // implemented without leaking memory.
      break;
    case AstAttribute::UNKNOWN_OWNERSHIP:
      ...
      if (copied != attr)
        MLOG_WARN_CXX("midend:astProcessing")
            << attributeFullName(it->first, it->second)
            << " virtual copy function did not copy the attribute (unknown "
               "ownership)\n";
      break;
    }

    if (copied)
      tmp.set(it->first, copied);
  }
...
AstAttribute::OwnershipPolicy AstAttribute::getOwnershipPolicy() const {
  return UNKNOWN_OWNERSHIP;
}

std::string AstAttribute::toString() {
  return StringUtility::numberToString((void *)(this));
}

int AstAttribute::packed_size() { return 0; }

char *AstAttribute::packed_data() { return NULL; }

void AstAttribute::unpacked_data(int /*size*/, char * /*data*/) {}

std::string AstAttribute::additionalNodeOptions() { return ""; }
...
bool AstAttribute::commentOutNodeInGraph() { return false; }

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                      MetricAttribute
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

AstAttribute *MetricAttribute::copy() const {
  return new MetricAttribute(*this);
}

std::string MetricAttribute::attribute_class_name() const {
...
char *MetricAttribute::packed_data() {
  // Reimplemented because old version returned a dangling pointer. [Robb Matzke
  // 2015-11-10]
...
void MetricAttribute::unpacked_data(int size, char *data) {
  ...
  if (string_size_minus_one >= 0) {
    ...
    std::cout << "Error: In MetricAttribute::unpacked_data(): data string is "
                 "zero length"
              << std::endl;
    ROSE_ABORT();
  }
