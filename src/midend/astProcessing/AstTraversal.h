...
//! Usage:
//! AstCycleTest t;
//! t.traverse(SgNode* n); // where n is the root node of the subtree to be
//! tested.
class AstCycleTest : public AstPrePostOrderTraversal {
public:
  //! determines whether the given sequence l of nodes extended by node creates
  //! a cycle the found cycle is returned. If no cycle is found, the returned
  //! list is empty.
