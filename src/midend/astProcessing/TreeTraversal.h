...
template <typename TreeNode> class TreeTraversal {
public:
  TreeTraversal();
  virtual ~TreeTraversal();
  virtual void traverse(TreeNode node) = 0;
  virtual bool isNullNode(TreeNode node) {
    return node == 0; /* as default we assume the comparison with 0 to succeed
                         on a null-node */
  }
...
protected:
  typedef typename std::vector<TreeNode>
      ChildrenContainer; /* will be made a template parameter */
...
template <class TreeNode> TreeTraversal<TreeNode>::TreeTraversal() {}

template <class TreeNode> TreeTraversal<TreeNode>::~TreeTraversal() {}

template <class TreeNode>
void PreOrderTraversal<TreeNode>::traverse(TreeNode node) {
  ...
  if (!this->skipSubTreeOfNode(node)) {
    ...
    for (typename PreOrderTraversal<TreeNode>::ChildrenContainer::iterator
             iter = c.begin();
         iter != c.end(); iter++) {
      traverse(*iter);
    }
...
template <class TreeNode>
void InOrderTraversal<TreeNode>::traverse(TreeNode node) {
  ...
  if (!skipSubTreeOfNode(node)) {
    ...
    for (typename InOrderTraversal<TreeNode>::ChildrenContainer::iterator iter =
             c.begin();
         iter != c.end();) {
      traverse(*iter);
      iter++;
      if (iter != c.end) {
        if (!skipNode(node)) {
          inOrderVisit(*iter); // only inorder-visit if the node is not first
                               // and not last of children
        }
      }
    }
  }
...
template <class TreeNode>
void PostOrderTraversal<TreeNode>::traverse(TreeNode node) {
  ...
  if (!skipSubTreeOfNode(node)) {
    setChildrenContainer(node, c);
    for (typename PostOrderTraversal<TreeNode>::ChildrenContainer::iterator
             iter = c.begin();
         iter != c.end(); iter++) {
      traverse(*iter);
    }
...
template <class TreeNode>
void PrePostOrderTraversal<TreeNode>::traverse(TreeNode node) {
  ...
  if (!this->skipSubTreeOfNode(node)) {
    this->setChildrenContainer(node, c);
    for (typename PrePostOrderTraversal<TreeNode>::ChildrenContainer::iterator
             iter = c.begin();
         iter != c.end(); iter++) {
      traverse(*iter);
    }
