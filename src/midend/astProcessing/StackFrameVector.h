...
private:
  explicit StackFrameVector(const BufferType &otherBuffer,
...
StackFrameVector<T>::StackFrameVector()
    : buffer(new BufferType()), framePtr(buffer->begin()),
      stackPtr(buffer->end()), deleteBufferWhenDone(true) {}

template <class T>
StackFrameVector<T>::StackFrameVector(const StackFrameVector<T> &v)
    : buffer(NULL), framePtr(v.framePtr), stackPtr(v.stackPtr),
      deleteBufferWhenDone(false) // don't delete the pointer, it's not ours
{}

template <class T>
StackFrameVector<T>::StackFrameVector(typename StackFrameVector<T>::size_type n)
    : buffer(new BufferType(n)), framePtr(buffer->begin()),
      stackPtr(buffer->end()), deleteBufferWhenDone(true) {}
...
StackFrameVector<T>::StackFrameVector(
    ...
    : buffer(new BufferType(n, initValue)), framePtr(buffer->begin()),
      stackPtr(buffer->end()), deleteBufferWhenDone(true) {}
...
StackFrameVector<T>::StackFrameVector(
    ...
    : buffer(new BufferType(otherBuffer)),
      framePtr(buffer->begin() + framePtrOffset), stackPtr(buffer->end()),
      deleteBufferWhenDone(true) {}

template <class T> StackFrameVector<T>::~StackFrameVector() {
...
template <class T> StackFrameVector<T> *StackFrameVector<T>::deepCopy() const {
  ...
  return new StackFrameVector(BufferType(buffer->begin(), stackPtr),
                              framePtr - buffer->begin());
}

template <class T>
typename StackFrameVector<T>::iterator StackFrameVector<T>::begin() {
  return framePtr;
}
...
template <class T>
typename StackFrameVector<T>::iterator StackFrameVector<T>::end() {
  return stackPtr;
}

template <class T>
typename StackFrameVector<T>::const_iterator StackFrameVector<T>::end() const {
  return stackPtr;
}

template <class T>
typename StackFrameVector<T>::reverse_iterator StackFrameVector<T>::rbegin() {
...
template <class T>
typename StackFrameVector<T>::reverse_iterator StackFrameVector<T>::rend() {
...
template <class T>
typename StackFrameVector<T>::size_type StackFrameVector<T>::size() const {
  return end() - begin();
}

template <class T>
typename StackFrameVector<T>::size_type StackFrameVector<T>::max_size() const {
  return size();
}

template <class T>
typename StackFrameVector<T>::size_type StackFrameVector<T>::capacity() const {
  return size();
}

template <class T> bool StackFrameVector<T>::empty() const {
  return size() == 0;
}
...
template <class T>
typename StackFrameVector<T>::const_reference StackFrameVector<T>::operator[](
    typename StackFrameVector<T>::size_type n) const {
  return framePtr[n];
}
...
template <class T>
typename StackFrameVector<T>::reference StackFrameVector<T>::front() {
  return *framePtr;
}
...
template <class T>
typename StackFrameVector<T>::reference StackFrameVector<T>::back() {
  return *(stackPtr - 1);
}
...
template <class T> StackFrameVector<T>::operator std::vector<T>() {
  // return a copy of the objects in the current stack frame
  return BufferType(begin(), end());
}

template <class T> void StackFrameVector<T>::push(const T &x) {
...
template <class T>
typename StackFrameVector<T>::size_type StackFrameVector<T>::debugSize() const {
  ROSE_ASSERT(buffer != NULL);
  return stackPtr - buffer->begin();
}

template <class T> void StackFrameVector<T>::resetStack() {
  ROSE_ASSERT(buffer != NULL);
  framePtr = stackPtr = buffer->begin();
}

template <class T>
typename StackFrameVector<T>::value_type StackFrameVector<T>::pop() {
...
template <class T> void StackFrameVector<T>::debugDump(std::ostream &s) {
  // this function should only be called for debugging
  s << "\n"
    << "framePtr offset: " << framePtr - buffer->begin()
    << " stackPtr offset: " << stackPtr - buffer->begin() << " size: " << size()
