...
AstNodePtrSynAttr
AstNodePtrs::evaluateSynthesizedAttribute(SgNode *node,
  ...
  // -Werror=unused-but-set-variable. bool nullValueExists=false;
  for (SynthesizedAttributesList::iterator i = l.begin(); i != l.end(); i++) {
    if ((*i).node == NULL) {
      // DQ (12/10/2016): Eliminating a warning that we want to be an error:
      // -Werror=unused-but-set-variable. nullValueExists=true;
      s += ", NULL";
    } else {
