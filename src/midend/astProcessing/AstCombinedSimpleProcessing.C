...
AstCombinedSimpleProcessing::AstCombinedSimpleProcessing() : traversals() {}

AstCombinedSimpleProcessing::AstCombinedSimpleProcessing(
    const AstCombinedSimpleProcessing::TraversalPtrList &t)
    : traversals(t) {}

void AstCombinedSimpleProcessing::addTraversal(
    AstCombinedSimpleProcessing::TraversalPtr t) {
...
void AstCombinedSimpleProcessing::visit(SgNode *astNode) {
...
void AstCombinedSimpleProcessing::atTraversalStart() {
...
void AstCombinedSimpleProcessing::atTraversalEnd() {
  // Call this function for all traversals.
  TraversalPtrList::iterator t;
  for (t = tBegin; t != tEnd; ++t)
    (*t)->atTraversalEnd();
...
AstCombinedPrePostProcessing::AstCombinedPrePostProcessing() : traversals() {}

AstCombinedPrePostProcessing::AstCombinedPrePostProcessing(
    const AstCombinedPrePostProcessing::TraversalPtrList &t)
    : traversals(t) {}

void AstCombinedPrePostProcessing::addTraversal(
    AstCombinedPrePostProcessing::TraversalPtr t) {
  traversals.push_back(t);
}

AstCombinedPrePostProcessing::TraversalPtrList &
AstCombinedPrePostProcessing::get_traversalPtrListRef() {
  return traversals;
}

void AstCombinedPrePostProcessing::preOrderVisit(SgNode *astNode) {
  // Visit this node with each of the traversals.
  TraversalPtrList::iterator t;
  for (t = tBegin; t != tEnd; ++t)
...
void AstCombinedPrePostProcessing::postOrderVisit(SgNode *astNode) {
  // Visit this node with each of the traversals.
  TraversalPtrList::iterator t;
  for (t = tBegin; t != tEnd; ++t)
...
void AstCombinedPrePostProcessing::atTraversalStart() {
  // Compute some values that are constant during the traversal, saving
  // lots of method calls. We cannot reliably compute these earlier
  // because the user is allowed to mess with the traversal container
  // using the reference we provide.
...
void AstCombinedPrePostProcessing::atTraversalEnd() {
  // Call this function for all traversals.
  TraversalPtrList::iterator t;
  for (t = tBegin; t != tEnd; ++t)
    (*t)->atTraversalEnd();
