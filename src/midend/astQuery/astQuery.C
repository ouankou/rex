...
//     AstQuery<AstQuerySynthesizedAttributeType>::VariantVector
// **********************************************************
...
VariantVector VariantVector::operator=(const VariantVector &X) {
  // printf ("Top of
  // AstQuery<AstQuerySynthesizedAttributeType>::VariantVector::operator= (X)
  // \n"); printVariantVector(*this);
...
// (vector of variants)
void VariantVector::printVariantVector(const VariantVector &X) {
...
void Merge(Rose_STL_Container<int> &mergeWith,
           Rose_STL_Container<int> mergeTo) {
  // mergeWith.merge(mergeTo);
  mergeWith.insert(mergeWith.end(), mergeTo.begin(), mergeTo.end());
}

void Merge(Rose_STL_Container<std::string> &mergeWith,
           Rose_STL_Container<std::string> mergeTo) {
  // mergeWith.merge(mergeTo);
  mergeWith.insert(mergeWith.end(), mergeTo.begin(), mergeTo.end());
}

void Merge(Rose_STL_Container<SgNode *> &mergeWith,
           Rose_STL_Container<SgNode *> mergeTo) {
  // mergeWith.merge(mergeTo);
  mergeWith.insert(mergeWith.end(), mergeTo.begin(), mergeTo.end());
}

void Merge(Rose_STL_Container<SgFunctionDeclaration *> &mergeWith,
...
template <typename AstQuerySynthesizedAttributeType> struct testFunctionals {
  using FunctorType =
      std::function<std::list<AstQuerySynthesizedAttributeType>(SgNode *)>;
