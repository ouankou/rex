...
Lattice *nodeConstLattice::copy() const { return new nodeConstLattice(*this); }
...
bool nodeConstLattice::meetUpdate(Lattice *that_arg) {
  ...
  // if both are initialized, perform the meet
    ...
    // make this object top since we're at a meet of a set value and an unset
    // value
    if (level == bottom) {
  ...
  return (oldValID != valID) || (oldLevel != level);
}
...
// If this lattice is at level valKnown, increments the value by the given
// amount returns true if this causes this to change and false otherwise
...
bool nodeConstLattice::operator==(Lattice *that_arg) {
  ...
  return (valID == that->valID) && (level == that->level);
}

short nodeConstLattice::getValConst() const { return level; }
...
// vector<Lattice*> nodeConstAnalysis::genInitState(const Function& func, const
// DataflowNode& n, const NodeState& state)
void nodeConstAnalysis::genInitState(const Function &func,
  ...
  FiniteVariablesProductLattice *l =
      new FiniteVariablesProductLattice(true, false, new nodeConstLattice(),
                                        constVarLattices, NULL, func, n, state);
...
bool nodeConstAnalysis::transfer(const Function &func, const DataflowNode &n,
                                 NodeState &state,
  ...
  n.getNode()->class_name().c_str()); printf("isSgBinaryOp(n.getNode())=%p\n",
  ...
  if ((asgnLHS = cfgUtils::getAssignmentLHS(n.getNode())) != NULL) {
    ...
    if (varID::isValidVarExp(asgnLHS)) {
      ...
      // ---------------------------------------------------------------------------------
      // Update the left-hand side variable with the max of the lattices of the
      // right-hand side variables or set it to 1 if the right hand side has no
      // variable reads (i.e. its a constant)
      // WARNING: The RHS MAY CONTAIN FUNCTION CALLS, WHICH WE'RE NOT HANDLING
      // RIGHT NOW
      set<SgNode *> rhs;
      cfgUtils::getAssignmentRHS(n.getNode(), rhs);
      varIDSet rhsReads;
      // printf("rhs=\n");
      //  Iterate through the SgNodes in rhs and collect all their variable read
      //  references
      for (set<SgNode *>::iterator it = rhs.begin(); it != rhs.end(); it++) {
        ...
        for (varIDSet::iterator it2 = rhsReads_it.begin();
             it2 != rhsReads_it.end(); it2++)
          rhsReads.insert(*it2);
        ...
        // rhsReads_it.size(), rhsWrites_it.size());
      }
      ...
      if (rhsReads.size() > 0) {
        maxLat.initialize();
        for (varIDSet::iterator it = rhsReads.begin(); it != rhsReads.end();
             it++) {
          nodeConstLattice *rhsVarLat = dynamic_cast<nodeConstLattice *>(
              prodLat->getVarLattice(func, *it));
      ...
      nodeConstLattice *lhsVarLat = dynamic_cast<nodeConstLattice *>(
          prodLat->getVarLattice(func, lhsVar));
  ...
  // If this is a variable declaration
    ...
    nodeConstLattice *ncLat =
        dynamic_cast<nodeConstLattice *>(prodLat->getVarLattice(func, var));
  ...
  // If this is a function call to a function that doesn't have a body, assume
  // that all the variables passed by reference were assigned
    ...
    if (!callee.get_definition()) {
      ...
      const SgExpressionPtrList &args =
          isSgFunctionCallExp(n.getNode())->get_args()->get_expressions();
      ...
      for (itParams = params.begin(), itArgs = args.begin();
        ...
        // If the current variable is being passed by reference through a
        // pointer
        if (isSgPointerType(param->get_type()) && isSgAddressOfOp(arg) &&
          ...
          nodeConstLattice *ncLat = dynamic_cast<nodeConstLattice *>(
              prodLat->getVarLattice(func, var));
          ROSE_ASSERT(ncLat != NULL);
          ncLat->set(/*uids.getUID()*/ (unsigned long)n.getNode());
        }
        // If the current variable is being passed by reference throgh a
        // reference-typed argument
        else if (isSgReferenceType(param->get_type()) &&
                 varID::isValidVarExp(arg)) {
          // printf("arg = <%s|%s>\n", arg->unparseToString().c_str(),
...
// runs the nodeConstAnalysis on the project and returns the resulting
// nodeConstAnalysis object
...
void printNodeConstAnalysisStates(nodeConstAnalysis *nca, string indent) {
  ...
  printAnalysisStates pas(nca, factNames, latticeNames,
