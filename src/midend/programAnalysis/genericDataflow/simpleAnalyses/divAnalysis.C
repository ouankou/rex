...
Lattice *DivLattice::copy() const { return new DivLattice(*this); }
...
// returns true if the given value matches the given div, rem combo and false
// otherwise
bool DivLattice::matchDiv(long value, long div, long rem) {
  return (value % div) == rem;
}

// Takes two lattices at level divKnown. If the two objects have matching div,
// rem pairs, returns true and sets div and rem to those mathching values.
// Otherwise, returns false.
bool DivLattice::matchDiv(DivLattice *one, DivLattice *two, long &div,
                          long &rem) {
  ...
  if (one->div == two->div) {
    ...
    // else, if the rems are different
      ...
      int divremgcd;
      // if one rem is 0 and the other 0 shares factors with the common div,
      // those factors can be the new common div
...
// Takes two lattices at level divKnown. If the two objects have div, rem pairs
// that make it possible to add or subtract them them and produce div/rem
// information where div>1, returns true and sets div and rem to correspond to
// the sum of these values. Otherwise, returns false. plus - true if the caller
// want to see one+two and false if one-two
bool DivLattice::matchDivAddSubt(DivLattice *one, DivLattice *two, long &div,
                                 long &rem, bool plus) {
  // If these object have the same div
  if (one->div == two->div) {
    Dbg::dbg << "DivLattice::matchDivAddSubt() one=" << one->str("")
             << " two=" << two->str("") << "\n";
    div = one->div;
    if (plus)
      rem = (one->rem + two->rem) % div;
    else
      rem = (one->rem - two->rem + div) % div;
    return true;
  }
  // Else, if these object have the different divs
  else {
    div = gcd(one->div, two->div);
    // If their divisors have a factor in common, their sum can be expressed
    // using a divisor that has this factor
    if (div > 1) {
      if (plus)
        rem = (one->rem % div + two->rem % div) % div;
      else
        rem = (one->rem % div - two->rem % div + div) % div;
      return true;
    }
    // Else, if they're not compatible
    else
  ...
  // else, if both are above bottom, perform the meet
    ...
    if (level == valKnown && that->level == valKnown) {
      ...
      if (value != that->value) {
        ...
        // If the gcd is > 1, then we know a useful divisor and the level is
        // divKnown otherwise, the divisor info is useless and the level becomes
        // top
        if (div != 1) {
          level = divKnown;
          rem = 0;
          ...
          // If we cannot find a good divisor, we guess that the variable's new
          // value was produced via an iteration where the variable's value is
          // repeatedly incremented by a constant. If this is the case, the
          // divisor is the difference between the two values. If this guess is
          // wrong, the next time we update this lattice, we'll update it to
          // top.
          level = divKnown;
      ...
      // Dbg::dbg << "(level==valKnown && that->level==divKnown)
      // value%that->div="<<(value%that->div)<<" that->rem="<<that->rem<<"\n";
      ...
      // Dbg::dbg << "(level==divKnown && that->level==valKnown)
      // that->value%div="<<that->value%div<<" == rem="<<rem<<"\n";
      //  if this can be divided by that->div to get that->rem as the remainder,
...
// the various cases that do not result in this object becoming top goto this
// label
Done:

  // return whether this object was modified
  return (oldValue != value) || (oldDiv != div) || (oldRem != rem) ||
         (oldLevel != level);
}
...
bool DivLattice::operator==(Lattice *that_arg) {
  ...
  /*Dbg::dbg << "operator == ("<<str()<<", "<<that->str()<<"): "<<((value ==
     that->value) && (div == that->div) && (rem == that->rem) && (level ==
     that->level))<<"\n";*/

  return (value == that->value) && (div == that->div) && (rem == that->rem) &&
         (level == that->level);
}
...
long DivLattice::getValue() const { return value; }

long DivLattice::getDiv() const { return div; }

long DivLattice::getRem() const { return rem; }

short DivLattice::getLevel() const { return level; }
...
bool DivLattice::set(long div, long rem) {
  bool modified =
      this->level != divKnown || this->div != div || this->rem != rem;
  value = 0;
  this->div = div;
  this->rem = rem;
  level = divKnown;
...
Lattice* DivAnalysis::genInitVarState(const Function& func, const DataflowNode&
...
newLattice->str().c_str()); return newLattice;
}

// generates the initial non-variable-specific lattice state for a dataflow node
Lattice* DivAnalysis::genInitNonVarState(const Function& func, const
...
// vector<Lattice*> DivAnalysis::genInitState(const Function& func, const
// DataflowNode& n, const NodeState& state)
void DivAnalysis::genInitState(const Function &func, const DataflowNode &n,
  ...
  FiniteVarsExprsProductLattice *l = new FiniteVarsExprsProductLattice(
      (Lattice *)new DivLattice(), emptyM /*genConstVarLattices()*/,
...
/*map<varID, Lattice*>& DivAnalysis::genConstVarLattices() const
{
        if(!constVars_init)
        {
                ...
                / *
                Dbg::dbg << "constVars:\n";
                for(map<varID, Lattice*>::iterator it = constVars.begin();
it!=constVars.end(); it++) { Dbg::dbg << it->first.str() << ": " <<
it->second->str("") << "\n"; }
...
DivAnalysisTransfer::DivAnalysisTransfer(const Function &func,
                                         const DataflowNode &n,
                                         NodeState &state,
                                         const vector<Lattice *> &dfInfo)
    : VariableStateTransfer<DivLattice>(func, n, state, dfInfo,
                                        divAnalysisDebugLevel) {}

// Integral Numeric Constants
template <class T> void DivAnalysisTransfer::visitIntegerValue(T *sgn) {
...
void DivAnalysisTransfer::visit(SgLongLongIntVal *sgn) {
  visitIntegerValue(sgn);
}
...
void DivAnalysisTransfer::visit(SgUnsignedLongLongIntVal *sgn) {
  visitIntegerValue(sgn);
}
void DivAnalysisTransfer::visit(SgUnsignedLongVal *sgn) {
  visitIntegerValue(sgn);
}
void DivAnalysisTransfer::visit(SgUnsignedIntVal *sgn) {
  visitIntegerValue(sgn);
}
void DivAnalysisTransfer::visit(SgUnsignedShortVal *sgn) {
  visitIntegerValue(sgn);
}
...
void DivAnalysisTransfer::transferAdditive(DivLattice *arg1Lat,
                                           DivLattice *arg2Lat,
                                           DivLattice *resLat,
                                           bool isAddition) {
  ...
  if (arg1Level == DivLattice::divKnown && arg2Level == DivLattice::divKnown) {
    ...
    if (DivLattice::matchDivAddSubt(arg1Lat, arg2Lat, newDiv, newRem,
                                    isAddition)) {
  ...
  /// XXX: Copy result from DivKnown, then increment by value of ValKnown. Need
  /// to fix DivLattice::incr to handle negatives correctly
  if (arg1Level == DivLattice::valKnown && arg2Level == DivLattice::divKnown) {
    long rem = (isAddition ? arg1Lat->getValue() + arg2Lat->getRem()
                           : arg1Lat->getValue() % arg2Lat->getDiv() -
                                 arg2Lat->getRem() + arg2Lat->getDiv()) %
               arg2Lat->getDiv();
    updateModified(resLat->set(arg2Lat->getDiv(), rem));
  }
  // Arg1 DivKnown, Arg2 ValKnown
  else if (arg1Level == DivLattice::divKnown &&
           arg2Level == DivLattice::valKnown) {
    long rem = (isAddition ? (arg2Lat->getValue() + arg1Lat->getRem())
                           : (arg1Lat->getRem() -
                              (arg2Lat->getValue() % arg1Lat->getDiv()) +
                              arg2Lat->getDiv())) %
               arg1Lat->getDiv();
...
void DivAnalysisTransfer::visit(SgPlusAssignOp *sgn) {
  transferArith(sgn,
                std::bind(&DivAnalysisTransfer::transferAdditive,
                          std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4, true));
}
void DivAnalysisTransfer::visit(SgMinusAssignOp *sgn) {
  transferArith(sgn,
                std::bind(&DivAnalysisTransfer::transferAdditive,
                          std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4, false));
}
void DivAnalysisTransfer::visit(SgAddOp *sgn) {
  transferArith(sgn,
                std::bind(&DivAnalysisTransfer::transferAdditive,
                          std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4, true));
}
void DivAnalysisTransfer::visit(SgSubtractOp *sgn) {
  transferArith(sgn,
                std::bind(&DivAnalysisTransfer::transferAdditive,
                          std::placeholders::_1, std::placeholders::_2,
                          std::placeholders::_3, std::placeholders::_4, false));
...
void DivAnalysisTransfer::visit(SgMinusOp *sgn) {
  ...
  if (resLat->getLevel() == DivLattice::valKnown)
    ...
    resLat->set(resLat->getDiv(),
                (resLat->getDiv() - resLat->getRem()) % resLat->getDiv());
  if (arg2Lat != NULL) {
    delete arg2Lat;
  }
}

void DivAnalysisTransfer::transferMultiplicative(DivLattice *arg1Lat,
                                                 DivLattice *arg2Lat,
                                                 DivLattice *resLat) {
...
printf("arg2Lat = %s\n", arg2Lat->str().c_str());*/
  ...
  // Both Bottom
  if (arg1Lat->getLevel() == DivLattice::bottom ||
      arg2Lat->getLevel() == DivLattice::bottom)
    updateModified(resLat->setBot());
  // Both ValKnown
  else if (arg1Lat->getLevel() == DivLattice::valKnown &&
           arg2Lat->getLevel() == DivLattice::valKnown)
    updateModified(resLat->set(arg1Lat->getValue() * arg2Lat->getValue()));
  // Arg1 ValKnown, Arg2 DivKnown, Arg1
  else if (arg1Lat->getLevel() == DivLattice::valKnown &&
           arg2Lat->getLevel() == DivLattice::divKnown) {
  ...
  // Arg1 DivKnown, Arg2 ValKnown
  else if (arg1Lat->getLevel() == DivLattice::divKnown &&
           arg2Lat->getLevel() == DivLattice::valKnown) {
...
void DivAnalysisTransfer::visit(SgMultiplyOp *sgn) {
  transferArith(sgn, &DivAnalysisTransfer::transferMultiplicative);
}
void DivAnalysisTransfer::visit(SgMultAssignOp *sgn) {
  transferArith(sgn, &DivAnalysisTransfer::transferMultiplicative);
}

void DivAnalysisTransfer::transferDivision(DivLattice *arg1Lat,
                                           DivLattice *arg2Lat,
                                           DivLattice *resLat) {
  if (divAnalysisDebugLevel >= 1)
    Dbg::dbg << "   case i = j / k\n";
  ...
  // Arg1 ValKnown, Arg2 DivKnown, Arg1 divisible by Arg2 Div
           ...
           arg2Lat->getLevel() == DivLattice::divKnown &&
           arg2Lat->getRem() == 0 &&
           arg1Lat->getValue() % arg2Lat->getDiv() == 0)
  ...
  // Arg1 DivKnown, Arg2 ValKnown, Arg1 Div/Rem divisible by Arg2
           ...
           (arg1Lat->getRem() % arg2Lat->getValue() == 0))
    updateModified(resLat->set(arg1Lat->getDiv() / arg2Lat->getValue(),
                               arg1Lat->getRem() / arg2Lat->getValue()));
  // Both DivKnown, Arg1 Div/Rem divisible by Arg2 Div
  else if (arg1Lat->getLevel() == DivLattice::divKnown &&
           arg2Lat->getLevel() == DivLattice::divKnown &&
           arg2Lat->getRem() == 0 &&
           (arg1Lat->getDiv() % arg2Lat->getDiv() == 0) &&
           (arg1Lat->getRem() % arg2Lat->getDiv() == 0))
    updateModified(resLat->set(arg1Lat->getDiv() / arg2Lat->getDiv(),
                               arg1Lat->getRem() / arg2Lat->getDiv()));
  // Else => Top
  else
    updateModified(resLat->setTop());
}
void DivAnalysisTransfer::visit(SgDivideOp *sgn) {
  transferArith(sgn, &DivAnalysisTransfer::transferDivision);
}
void DivAnalysisTransfer::visit(SgDivAssignOp *sgn) {
  transferArith(sgn, &DivAnalysisTransfer::transferDivision);
}
...
void DivAnalysisTransfer::transferArith(SgBinaryOp *sgn,
                                        TransferOp transferOp) {
  transferArith(sgn, std::mem_fn(transferOp));
}

void DivAnalysisTransfer::transferMod(DivLattice *arg1Lat, DivLattice *arg2Lat,
                                      DivLattice *resLat) {
  if (divAnalysisDebugLevel >= 1)
    Dbg::dbg << "   case i = j %% k\n";
...
void DivAnalysisTransfer::visit(SgModOp *sgn) {
  transferArith(sgn, &DivAnalysisTransfer::transferMod);
}
void DivAnalysisTransfer::visit(SgModAssignOp *sgn) {
  transferArith(sgn, &DivAnalysisTransfer::transferMod);
}
...
void printDivAnalysisStates(DivAnalysis *da, string indent) {
  ...
  printAnalysisStates pas(da, factNames, latticeNames,
