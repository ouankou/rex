...
// vector<Lattice*> ConstrGraphAnalysis::genInitState(const Function& func,
// const DataflowNode& n, const NodeState& state)
void ConstrGraphAnalysis::genInitState(const Function &func,
  ...
  // Create a constraint graph from the divisiblity and sign information at this
  // CFG node
  FiniteVarsExprsProductLattice *divProdL =
      dynamic_cast<FiniteVarsExprsProductLattice *>(
          state.getLatticeBelow(divAnalysis, 0));
  /*if(isSgIntVal(n.getNode()))
          cout << "    divProdL="<<divProdL->str("        ")<<"\n";*/
  // FiniteVarsExprsProductLattice* sgnProdL =
  // dynamic_cast<FiniteVarsExprsProductLattice*>(state.getLatticeBelow(sgnAnalysis,
  // 0));
  ConstrGraph *cg = new ConstrGraph(
      func, n, state, ldva, divProdL,
      /* GB : 2011-03-05 (Removing Sign Lattice Dependence) sgnProdL, */ false,
      "");
  ...
  // }
  //  Create a product lattice that will maintain for each variable a
  //  ConstraintGraph that stores the constraints represented by the variable
  ...
  //  ???                                      (Lattice*)new ConstrGraph(func,
  //  n, state, ldva, divProdL, sgnProdL, true, ""),
  //  ???                                      emptyM, (Lattice*)NULL, ldva, n,
  //  state);
...
// Returns a map of special constant variables (such as zeroVar) and the
// lattices that correspond to them These lattices are assumed to be constants:
// it is assumed that they are never modified and it is legal to
//    maintain only one copy of each lattice may for the duration of the
//    analysis.
...
bool ConstrGraphAnalysis::transfer(const Function &func, const DataflowNode &n,
                                   NodeState &state,
                                   const vector<Lattice *> &dfInfo) {
  string indent = "            ";
  printf("%s-----------------------------------\n", indent.c_str());
  printf("%sConstrGraphAnalysis::transfer() function %s() node=<%s | %s>\n",
         indent.c_str(), func.get_name().str(),
  ...
  /* ??? InfiniteVarsExprsProductLattice* prodLat =
  dynamic_cast<InfiniteVarsExprsProductLattice*>(dfInfo[1]);
  ...
  it!=lattices.end(); it++) { (dynamic_cast<ConstrGraph*>(*it))->initialize("");
  }*/
  ...
  if (isSgAssignOp(n.getNode())) {
    ...
    cout << indent << "res=" << res.str() << " lhs=" << lhs.str()
         << " rhs=" << rhs.str() << "\n";
    ...
    // ??? ConstrGraph* lhsLat =
    // dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(lhs));
    // ??? ConstrGraph* rhsLat =
    // dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(rhs));
    // ???
    // ??? if(resLat) cout << "resLat=\n    "<<resLat->str(indent+"    ")<<"\n";
    // ??? if(lhsLat) cout << "lhsLat=\n    "<<lhsLat->str(indent+"    ")<<"\n";
    // ??? if(rhsLat) cout << "rhsLat=\n    "<<rhsLat->str(indent+"    ")<<"\n";

    // Copy the lattice of the right-hand-side to both the left-hand-side
    // variable and to the assignment expression itself
    // ??? if(resLat) {
    if (resLive) {
      ...
      //??? modified = resLat->copyFromReplace(*rhsLat, res, rhs, indent+"    ")
      //|| modified;
    }
    ...
    if (lhsLive) {
      ...
      //??? modified = resLat->copyFromReplace(*asgnLat, lhs, rhs, indent+" ")
      //|| modified;
    }
    ...
    // ??? ConstrGraph* asgnLat =
    // dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(asgn));
    // ??? ConstrGraph* resLat =
    // dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(res));
    ...
    if (liveVars.find(res) != liveVars.end()) {
      ...
      // Copy the constraints represetented by asgn to lhs and record that res =
      // asgn
      //??? modified = resLat->copyFromReplace(*asgnLat, res, asgn, indent+" ")
      //|| modified;
    }
    ...
    varID var(initName);
    // ??? ConstrGraph* varLat =
    // dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(var));

    // cout << "DivAnalysis::transfer() isSgInitializedName var="<<var.str()<<"
    // varLat="<<varLat<<"\n";

    // If this is a scalar that we care about, initialize it to Bottom
    // ??? if(varLat)
    if (liveVars.find(var) != liveVars.end()) {
      // if(divAnalysisDebugLevel>=1) cout << "Variable declaration:
      // "<<var.str()<<",
      // get_initializer()="<<initName->get_initializer()<<"\n";
      //  If there is NO initializer
      if (initName->get_initializer() == NULL) {
        // Initialize the constraints on var's value and constraints represented
        // by var to Bottom
        ...
        // ??? ConstrGraph* initLat =
        // dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(init));
        // if(divAnalysisDebugLevel>=1) cout << "    init="<<init.str()<<"
        // initLat="<<initLat<<"\n";
        // ??? if(initLat) {
        if (liveVars.find(init) != liveVars.end()) {
          // Add the constraint var=init
          modified = cg->assign(var, init, 1, 1, 0) || modified;
          // Copy the constraints represented by init to var and record that var
          // = init
          //??? modified = varLat->copyFromReplace(*initLat, var, init, indent+"
          //") || modified;
        }
      }
    }
    // Integral Numeric Constants
             ...
             isSgUnsignedLongLongIntVal(n.getNode()) ||
             isSgUnsignedLongVal(n.getNode()) ||
             isSgUnsignedIntVal(n.getNode()) ||
             isSgUnsignedShortVal(n.getNode())) {
    ...
    if (liveVars.find(res) != liveVars.end()) {
      ...
      if (isSgLongLongIntVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgLongLongIntVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgLongIntVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgLongIntVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgIntVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgIntVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgShortVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgShortVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgUnsignedLongLongIntVal(n.getNode()))
        modified =
            cg->assign(res, zeroVar, 1, 1,
                       isSgUnsignedLongLongIntVal(n.getNode())->get_value()) ||
            modified;
      else if (isSgUnsignedLongVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgUnsignedLongVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgUnsignedIntVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgUnsignedIntVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgUnsignedShortVal(n.getNode()))
        modified = cg->assign(res, zeroVar, 1, 1,
                              isSgUnsignedShortVal(n.getNode())->get_value()) ||
                   modified;

      // Update resLat to represent the logical expression "val != 0"
      // ??? ConstrGraph* resLatCopy =
      // dynamic_cast<ConstrGraph*>(resLat->copy()); // Copy of the old value of
      // resLat
      // ??? resLat->setToBottom();
      // ???      if(isSgLongLongIntVal(n.getNode())) resLat->assign(res,
      // zeroVar, 1, 1, isSgLongLongIntVal(n.getNode())->get_value());
      // ??? else if(isSgLongIntVal(n.getNode())) resLat->assign(res, zeroVar,
      // 1, 1, isSgLongIntVal(n.getNode())->get_value());
      // ??? else if(isSgIntVal(n.getNode())) resLat->assign(res, zeroVar, 1, 1,
      // isSgIntVal(n.getNode())->get_value());
      // ??? else if(isSgShortVal(n.getNode())) resLat->assign(res, zeroVar, 1,
      // 1, isSgShortVal(n.getNode())->get_value());
      // ??? else if(isSgUnsignedLongLongIntVal(n.getNode()))
      // resLat->assign(res, zeroVar, 1, 1,
      // isSgUnsignedLongLongIntVal(n.getNode())->get_value());
      // ??? else if(isSgUnsignedLongVal(n.getNode())) resLat->assign(res,
      // zeroVar, 1, 1, isSgUnsignedLongVal(n.getNode())->get_value());
      // ??? else if(isSgUnsignedIntVal(n.getNode())) resLat->assign(res,
      // zeroVar, 1, 1, isSgUnsignedIntVal(n.getNode())->get_value());
      // ??? else if(isSgUnsignedShortVal(n.getNode())) resLat->assign(res,
      // zeroVar, 1, 1, isSgUnsignedShortVal(n.getNode())->get_value());
      // ??? resLat->negate();
      // ???
    ...
    // ??? if(resLat) {
    if (liveVars.find(res) != liveVars.end()) {
      // Add the constraint res=Top, since res is not representable as a numeric
      // integer
      ...
      // ???  // True if equivalent to no constraints at all. True or CG = True
      // / Bottom or CG = Bottom. True AND CG = CG / Bottom AND CG = CG
      // ???  modified = resLat->setToBottom() || modified;
      // ??? // Boolean = FALSE
      // ??? else
      // ???  // False if equivalent to impossible constraints. False or CG = CG
      // / Top or CG = CG. False AND CG = False / Top AND CG = Top
    ...
    // ??? // If the result expression is live
    // ??? if(resLat) {
    // ???  // Add the constraint res=Top, since res is not representable as a
    // numeric integer
    // ???  modified = cg->assignTop(res) || modified;
    // ??? }
    ...
    // Set up the information on the arguments and target of the arithmetic
    // operation
    if (isSgBinaryOp(n.getNode())) {
      if (isSgCompoundAssignOp(n.getNode())) {
        ...
        // cout << "lhs="<<lhs.str()<<" arg1="<<arg1.str()<<"
        // arg2="<<arg2.str()<<"\n";
      } else {
      ...
      // cout << "res="<<res.str()<<" arg1="<<arg1.str()<<"
      // arg1Lat="<<arg1Lat<<", arg2Lat="<<arg2Lat<<"\n";
    }
    // ??? resLat = dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(res));
    cout << indent << "res=" << res << " resLive=" << resLive << " lhs=" << lhs
         << " lhsLive=" << lhsLive << " arg1=" << arg1
         << " arg1Live=" << arg1Live << " arg2=" << arg2
         << " arg2Live=" << arg2Live << "\n";

    // If the result expression is dead but the left-hand-side of the expression
    // is live, update the left-hand-side with the result
    // ??? if(resLat==NULL &&
    ...
    //  If the result or left-hand-side expression as well as the arguments are
    //  live
    ...
    if ((resLive || lhsLive) && arg1Live && arg2Live) {
      ...
      if (isSgPlusAssignOp(n.getNode()) || isSgAddOp(n.getNode()) ||
        ...
        // If either arg1 or arg2 is a constant, these variables hold the
        // relationship between it and zeroVar
        int a_arg1, b_arg1, c_arg1, a_arg2, b_arg2, c_arg2;
        bool resToTop = false; // True if the expression arg1+arg2 turns out to
                               // be too complex to be represented
        varID var; // The variable that res will be related to after the
                   // addition, or zeroVar if res will be a constant
        int b_res = 1, c_res = 0; // The value of b and c used in the assignment
                                  // res = var*b + c
        ...
        if (isSgPlusAssignOp(n.getNode()) || isSgAddOp(n.getNode()) ||
          ...
          if (cg->isEqVars(arg1, zeroVar, a_arg1, b_arg1, c_arg1)) {
            ...
            ROSE_ASSERT(isSgPlusPlusOp(n.getNode()) ||
                        isSgMinusMinusOp(n.getNode()));
            ...
            // ??? } else if(arg2Lat->isEqVars(arg2, zeroVar, a_arg2, b_arg2,
            // c_arg2)) {
          } else if (cg->isEqVars(arg2, zeroVar, a_arg2, b_arg2, c_arg2)) {
            // arg2*a_arg2 = 0 + c_arg2 => arg2 = c_arg2/a_arg2
            ...
            // There is no x*a<=y*b+c constraint that describes res's
            // relationship to arg1 or arg2
          }
          // MULTIPLICATION
        } else if (isSgMultAssignOp(n.getNode()) ||
                   isSgMultiplyOp(n.getNode())) {
          // ??? if(arg1Lat->isEqVars(arg1, zeroVar, a_arg1, b_arg1, c_arg1)) {
          if (cg->isEqVars(arg1, zeroVar, a_arg1, b_arg1, c_arg1)) {
            // If both arg1 and arg2 are constants
            // ??? if(arg2Lat->isEqVars(arg2, zeroVar, a_arg2, b_arg2, c_arg2))
          ...
          if (cg->isEqVars(arg1, zeroVar, a_arg1, b_arg1, c_arg1)) {
            ...
            // There is no clean x*a<=y*b+c constraint that describes res's
            // relationship to arg1 or arg2 THIS MAY BE POSSIBLE BY USING
            // DIVISIBILITY VARIABLES BUT SINCE WE'RE ALREADY INCORPORATING THE
            // RESULTS OF DivAnalysis, THIS IS PROBABLY NOT NEEDED.
            // Else, if neither input variable is a constant
          } else {
            // There is no x*a<=y*b+c constraint that describes res's
            // relationship to arg1 or arg2
        ...
        // If we couldn't find a way to represent the results of the arithmetic
        // operation statically, Represent it as top
        cout << indent << " Assigning " << res << "(resLive=" << resLive
             << ") and/or " << lhs << "(lhsLive=" << lhsLive << ") to Top\n";
        if (resLive)
          modified = cg->assignTop(res, indent + "    ") || modified;
        if (lhsLive)
          modified = cg->assignTop(lhs, indent + "    ") || modified;
        resToTop = true;

      // The code above skips here if it was able to represent the results of
      ...
      DONE_NO_TOP:
        ...
        // If the expression can be represented.
          ...
          cout << indent << " Assigning " << res << "(resLive=" << resLive
               << ") and/or " << lhs << "(lhsLive=" << lhsLive
               << ") to = " << var << "*" << b_res << " + " << c_res << "\n";
          if (resLive)
            modified = cg->assign(res, var, 1, b_res, c_res, indent + "    ") ||
                       modified;
          if (lhsLive)
            modified = cg->assign(lhs, var, 1, b_res, c_res, indent + "    ") ||
                       modified;
...
if(!modified) resLatCopy = dynamic_cast<ConstrGraph*>(resLat->copy());
          ...
          //??? modified = resLat->copyFromReplace(*resLat, res, var, indent+"
          //") || modified;
        ...
        //??? modified = resLat->copyFromReplace(*asgnLat, res, arg1, indent+"
        //") || modified;
      }

      // If there is a left-hand side, copy the final lattice to the lhs
      // variable
      /* ??? if(isSgPlusAssignOp(n.getNode()) || isSgMinusAssignOp(n.getNode())
      || isSgMultAssignOp(n.getNode()) || isSgDivAssignOp(n.getNode()) ||
              isSgModAssignOp(n.getNode())) {
      if(lhsLive) {
              // If we didn't use the lhs lattice as resLat, copy resLat into
      lhsLat
              //cout <<
      "prodLat->getVarLattice("<<res.str()<<")="<<prodLat->getVarLattice(res)<<"\n";
              if(prodLat->getVarLattice(res)!=NULL) {
                      //ConstrGraph* lhsLat =
      dynamic_cast<ConstrGraph*>(prodLat->getVarLattice(lhs));
                      //cout <<
      "prodLat->getVarLattice("<<lhs.str()<<")="<<lhsLat<<"\n";
                      //if(lhsLat){ // If the left-hand-side contains an
      identifiable, live variable

                              cg->assign(lhs, res, 1, 1, 0);

                              //??? modified = lhsLat->copyFromReplace(*resLat,
      lhs, res, indent+"    ") || modified;
                      }
              }
      }*/
    }
...
cout << indent << cg->str(indent+"    ") << "\n";*/
  ...
  // NEED TO PERFORM CLOSURE HERE BECAUSE WE HAVEN'T YET CONNECTED DIVISIBILITY
  // VARIABLES TO ANYTHING ELSE.
...
/* // Incorporates the current node's inequality information from conditionals
(ifs, fors, etc.) into the current node's
// constraint graph.
// returns true if this causes the constraint graph to change and false
otherwise bool ConstrGraphAnalysis::incorporateConditionalsInfo(const Function&
func, const DataflowNode& n, NodeState& state, const vector<Lattice*>& dfInfo)
{
        bool modified = false;
...
(affineInequalityFact*)state.getFact(affIneqPlacer, 0); if(ineqFact)
        {
...
varIneq.getY(), "    ") << "\n"; modified = cg->setVal(varIneq.getX(),
varIneq.getY(), varIneq.getIneq().getA(), varIneq.getIneq().getB(),
varIneq.getIneq().getC()) || modified;
                }
        }
        return modified;
}*/

// Incorporates the current node's divisibility information into the current
// node's constraint graph returns true if this causes the constraint graph to
// change and false otherwise
/*
bool ConstrGraphAnalysis::incorporateDivInfo(const Function& func, const
DataflowNode& n, NodeState& state, const vector<Lattice*>& dfInfo, string
indent)
{
        bool modified = false;
        ConstrGraph* cg = dynamic_cast<ConstrGraph*>(dfInfo.front());
        ...
        for(varIDSet::iterator it = liveVars.begin(); it!=liveVars.end(); it++)
        {
                //cout << indent << "    "<<(*it)<<"\n";
                varID var = *it;
                // Re-add the connection between var and its divisibility
variable. The divisibility variable
                // needs to be disconnected from the other variables so that we
can re-compute its new relationships
                // solely based on its relationship with var. If we didn't do
this, and var's relationship to its
                // divisibility variable changed from one CFG node to the next
(i.e. from var=divvar to var = b*divvar),
                ...
                // DivLattice* varDivL =
dynamic_cast<DivLattice*>(prodL->getVarLattice(func, var));

             // incorporate this variable's divisibility information (if any)
             // if(varDivL->getLevel() == DivLattice::divKnown &&
!(varDivL->getDiv()==1 && varDivL->getRem()==0))
             // {
             //         modified = cg->addDivVar(var, varDivL->getDiv(),
varDivL->getRem()) || modified;
             // }
...
// For any variable for which we have divisibility info, remove its constraints
// to other variables (other than its divisibility variable)
bool ConstrGraphAnalysis::removeConstrDivVars(const Function &func,
                                              const DataflowNode &n,
                                              NodeState &state,
                                              const vector<Lattice *> &dfInfo,
                                              string indent) {
  ...
  for (varIDSet::iterator it = liveVars.begin(); it != liveVars.end(); it++) {
    ...
    /*DivLattice* varDivL = dynamic_cast<DivLattice*>(prodL->getVarLattice(func,
    var));

    // incorporate this variable's divisibility information (if any)
    if(varDivL->getLevel() == DivLattice::divKnown && !(varDivL->getDiv()==1 &&
    varDivL->getRem()==0))
    {
...
void printConstrGraphAnalysisStates(ConstrGraphAnalysis *cga, string indent) {
  ...
  printAnalysisStates pas(cga, factNames, latticeNames,
