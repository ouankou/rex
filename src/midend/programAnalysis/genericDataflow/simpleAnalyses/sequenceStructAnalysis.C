...
SeqStructLattice::SeqStructLattice(ConstrGraph *cg, const DataflowNode &n,
                                   string indent)
    : cg(cg), n(n) {
  ROSE_ASSERT(cg);
  init(indent);
...
SeqStructLattice::SeqStructLattice(ConstrGraph *cg, const DataflowNode &n,
                                   const varID &vInit, int startOffset,
                                   string indent)
    : cg(cg), n(n) {
  ROSE_ASSERT(cg);
  init(indent);
...
// Initializes the lattice to level=seqKnown and this->vInit=initV
// this->vFin=finV, this->s=s
SeqStructLattice::SeqStructLattice(ConstrGraph *cg, const DataflowNode &n,
                                   const varID &initV, const varID &finV, int s,
                                   string indent)
    : cg(cg), n(n) {
  ROSE_ASSERT(cg);
  init(indent);
...
// this->vFin=finV, this->s=s
SeqStructLattice::SeqStructLattice(ConstrGraph *cg, const DataflowNode &n,
                                   int initV, int finV, int s, string indent)
    : cg(cg), n(n) {
  ROSE_ASSERT(cg);
  init(indent);
...
// Copy constructor
SeqStructLattice::SeqStructLattice(const SeqStructLattice &that, string indent)
    : n(that.n) {
  copyFrom(that, indent);
}

// Copies the state from That sequence to This. Returns true if this causes This
...
Lattice *SeqStructLattice::copy() const { return new SeqStructLattice(*this); }
...
void SeqStructLattice::remapVars(const map<varID, varID> &varNameMap,
  ...
  // Do nothing here since each SeqStructLattice will always be used in
  // conjuction with a ConstrGraph and this call on the ConstrGraph will perform
  // the appropriate replacement since the only meaningful relationships that
  // vInit and vFin have are through the ConstrGraph.
}
...
bool SeqStructLattice::meetUpdate(Lattice *that_arg) {
  ...
  if (level <= bottom) {
    ...
    // If the application always executes the predecessor before this current
    // node and is not part of a loop
    if (dominates(that->n, this->n) && !dominates(this->n, that->n)) {
      if (*this != that)
        modified = setToTop() || modified;
      // If the application must execute the current node before the predecessor
      // in a loop
    } else if (dominates(this->n, that->n)) {

      // DQ (12/10/2016): Eliminating a warning that we want to be an error:
      // -Werror=maybe-uninitialized. int a, b, c;
      int a = 0, b = 0, c = 0;

      // If the current state is a constant and the state carried by the
      // predecessor from around the loop is a different constant
      if (level == startKnown && that->level == startKnown &&
          cg->eqVars(this->vInit, that->vInit, indent + "    ") &&
          cg->eqVars(this->vFin, that->vFin, a, b, c, indent + "    ") &&
          a == 1 && b == 1) {
...
// Set the level of this object to startKnown, with the given variable starting
// point (vInit + startOffset). Return true if this causes ths Lattice to change
// and false otherwise.
...
// Set the level of this object to startKnown, with the given constant starting
// point. Return true if this causes ths Lattice to change and false otherwise.
...
// Set the level of this object to seqKnown, with the given final point and
// stride Return true if this causes ths Lattice to change and false otherwise.
...
SeqStructLattice::seqLevel SeqStructLattice::getLevel() const { return level; }

bool SeqStructLattice::operator==(Lattice *that_arg) {
  SeqStructLattice *that = dynamic_cast<SeqStructLattice *>(that_arg);
  return (level == that->level) && (cg == that->cg) && (dom == that->dom) &&
         (n == that->n) && cg->eqVars(this->vInit, that->vInit, "    ") &&
         cg->eqVars(this->vFin, that->vFin, "    ") && (s == that->s);
}
...
string SeqStructLattice::str(string indent) {
  ...
  if (level == uninitialized)
    oss << "SeqStructLattice: uninitialized>";
  else if (level == bottom)
    oss << "SeqStructLattice: bottom>";
  else if (level == startKnown)
    oss << "SeqStructLattice: startKnown: vInit=" << vInit << ", vFin=" << vFin
        << ">";
  else if (level == seqKnown)
    oss << "SeqStructLattice: seqKnown: vInit=" << vInit << ", vFin=" << vFin
        << ", s=" << s << ">";
  else if (level == top)
    oss << "SeqStructLattice: top>";
...
void SeqStructAnalysis::genInitState(const Function &func,
  ...
  // Get the initial state from the ConstrGraphAnalysis, which will create a
  // constraint graph in initLattices and nothing in initFacts
  cgAnalysis->genInitState(func, n, state, initLattices, initFacts);
  ROSE_ASSERT(initLattices.size() == 0 &&
              dynamic_cast<ConstrGraph *>(initLattices[0]) &&
              initFacts.size() == 0);
  ...
  FiniteVarsExprsProductLattice *l = new FiniteVarsExprsProductLattice(
      (Lattice *)new SeqStructLattice(cg, n), emptyM /*genConstVarLattices()*/,
      (Lattice *)NULL, ldva, /*func, */ n, state);

  // Get all the array reference expressions in the application and add a
  // SeqStructLattice for each one
  ROSE_ASSERT(func.get_definition());
  Rose_STL_Container<SgNode *> arrayRefs = NodeQuery::querySubTree(
      func.get_definition()->get_body(), V_SgPntrArrRefExp);
  for (Rose_STL_Container<SgNode *>::iterator aref = arrayRefs.begin();
       aref != arrayRefs.end(); aref++) {
...
bool SeqStructAnalysis::transfer(const Function &func, const DataflowNode &n,
                                 NodeState &state,
                                 const vector<Lattice *> &dfInfo) {
  bool modified = false;

  ConstrGraph *cg =
      dynamic_cast<ConstrGraph *>(state.getLatticeAbove(cgAnalysis, 0));
  ...
  if (isSgPntrArrRefExp(n.getNode()) || isSgAssignOp(n.getNode())) {
    // Variable that corresponds to the value referenced by the array index or
    // the new value of the assigned variable
    varID refVar;
    // The SeqStructLattice associated with refVar
    SeqStructLattice *ssL = 0;

    if (isSgPntrArrRefExp(n.getNode())) {
      ssL = dynamic_cast<SeqStructLattice *>(
          prodLat->getVarLattice(SgExpr2Var(isSgPntrArrRefExp(n.getNode()))));
      ROSE_ASSERT(ssL);
      refVar = SgExpr2Var(isSgPntrArrRefExp(n.getNode())->get_rhs_operand());
    } else if (isSgAssignOp(n.getNode())) {
      ssL = dynamic_cast<SeqStructLattice *>(prodLat->getVarLattice(
          SgExpr2Var(isSgAssignOp(n.getNode())->get_lhs_operand())));
    ...
    if (ssL->getLevel() == SeqStructLattice::bottom) {
      ...
      // If we've identified a sequence after analyzing multiple instances of
      // this array access
      ...
      if (cg->isEqVars(refVar, ssL->vInit, a, b, c) && a == 1 && b == 1 &&
        ...
        // If the stride is different from what we've identified, this sequence
        // is too complex to be represented
      } else
        modified = ssL->setToTop() || modified;
    }
  }
...
DONE:
  ...
  // Finally, call the transfer function of the ConstrGraph to make sure that
  // the current statement's effects are incorporated
...
void printSeqStructAnalysisStates(SeqStructAnalysis *ssa, string indent) {
  ...
  printAnalysisStates pas(ssa, factNames, latticeNames,
