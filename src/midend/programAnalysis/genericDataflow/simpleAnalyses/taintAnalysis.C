...
void TaintLattice::copy(/*const*/ Lattice *other_) {
...
bool TaintLattice::operator==(/*const*/ Lattice *other_) /*const*/ {
  TaintLattice *other = dynamic_cast<TaintLattice *>(other_);
  assert(other != NULL);
...
std::string TaintLattice::to_string() const {
  ...
  case VERTEX_UNTAINTED:
    return std::string("untainted");
  case VERTEX_TAINTED:
    return std::string("tainted");
  default:
    return std::string("");
  }
}
...
bool TaintLattice::meetUpdate(/*const*/ Lattice *other_) {
  TaintLattice *other = dynamic_cast<TaintLattice *>(other_);
  assert(other != NULL);
...
void TaintAnalysis::genInitState(const Function &func, const DataflowNode &node,
  ...
  if (debug) {
    *debug << "TaintAnalysis::genInitState(func=" << func.get_name() << ",\n"
           << "                            node={"
           << StringUtility::makeOneLine(node.toString()) << "},\n"
           << "                            state={"
           << StringUtility::makeOneLine(state.str(this, "")) << "},\n"
...
bool TaintAnalysis::transfer(const Function &func, const DataflowNode &node_,
                             NodeState &state,
                             const std::vector<Lattice *> &dfInfo) {
  static size_t ncalls = 0;
  if (debug) {
    *debug << "TaintAnalysis::transfer-" << ++ncalls
           << "(func=" << func.get_name() << ",\n"
           << "                        node={"
           << StringUtility::makeOneLine(node_.toString()) << "},\n"
           << "                        state={"
           << state.str(this, "                            ") << ",\n"
           << "                        dfInfo[" << dfInfo.size()
           << "]={...})\n";
  ...
  FiniteVarsExprsProductLattice *prodLat =
      dynamic_cast<FiniteVarsExprsProductLattice *>(dfInfo.front());
  bool modified = magic_tainted(
      node,
      prodLat); // some values are automatically tainted based on their name

  // Process AST nodes that transfer taintedness.  Most of these operations have
  // one or more inputs from which a result is always calculated the same way.
  // So we just gather up the inputs and do the calculation at the very end of
  // this function.  The other operations are handled individually within their
  // "if" bodies.
  ...
  if (isSgAssignInitializer(node)) {
    ...
    TaintLattice *in1 = dynamic_cast<TaintLattice *>(
        prodLat->getVarLattice(SgExpr2Var(xop->get_operand())));
    ...
    const SgExpressionPtrList &exprs =
        xop->get_initializers()->get_expressions();
    for (size_t i = 0; i < exprs.size(); ++i) {
      varID in_id = SgExpr2Var(exprs[i]);
      TaintLattice *in =
          dynamic_cast<TaintLattice *>(prodLat->getVarLattice(in_id));
      inputs.push_back(in);
    }
    ...
    if (xop->get_initializer()) {
      ...
      TaintLattice *in1 =
          dynamic_cast<TaintLattice *>(prodLat->getVarLattice(in1_id));
      inputs.push_back(in1);
    }
    ...
    TaintLattice *in2 =
        dynamic_cast<TaintLattice *>(prodLat->getVarLattice(in2_id));
    ...
    // as in "return a".  The result will always be dead, so we're just doing
    // this to get some debugging output.  Most of our test inputs are
    // functions, and the test examines the function's returned taintedness.
  ...
  // Update the result lattice (unless dead) with the inputs (unless dead) by
  // using the meedUpdate() method.  All this means is that the new result will
  // be the maximum of the old result and all inputs, where "maximum" is defined
  // such that "tainted" is greater than "untainted" (and both of them are
  // greater than bottom/unknown).
  for (size_t i = 0; i < inputs.size(); ++i)
    if (debug)
      *debug << "TaintAnalysis::transfer: input " << (i + 1) << " is "
             << lattice_info(inputs[i]) << "\n";
  if (!result && varID::isValidVarExp(node)) {
    varID result_id(node); // NOTE: constructor doesn't handle all SgExpression
                           // nodes, thus the next "if"
  ...
  if (debug)
    *debug << "TaintAnalysis::transfer: result is " << lattice_info(result)
...
bool TaintAnalysis::magic_tainted(SgNode *node,
  ...
  if (isSgInitializedName(node)) {
    ...
    TaintLattice *tlat =
        dynamic_cast<TaintLattice *>(prodLat->getVarLattice(varID(iname)));
    if (tlat && 0 == vname.compare(0, 7, "TAINTED")) {
      bool modified = tlat->set_vertex(TaintLattice::VERTEX_TAINTED);
      if (debug) {
        *debug << "TaintAnalysis::magic_tainted: lattice is magically "
               << tlat->to_string()
               << (modified ? " (modified)" : " (not modified)") << "\n";
      }
      return modified;
    ...
    TaintLattice *tlat =
        dynamic_cast<TaintLattice *>(prodLat->getVarLattice(varID(vref)));
    if (tlat && 0 == vname.compare(0, 7, "TAINTED")) {
      bool modified = tlat->set_vertex(TaintLattice::VERTEX_TAINTED);
      if (debug) {
        *debug << "TaintAnalysis::magic_tainted: lattice is magically "
    ...
    for (size_t i = 0; i < inames.size(); ++i) {
      ...
      TaintLattice *tlat = dynamic_cast<TaintLattice *>(
          prodLat->getVarLattice(varID(inames[i])));
      if (tlat && 0 == vname.compare(0, 7, "TAINTED")) {
        bool modified = tlat->set_vertex(TaintLattice::VERTEX_TAINTED);
        if (debug) {
          *debug << "TaintAnalysis::magic_tainted: lattice is magically "
