...
Lattice *SgnLattice::copy() const { return new SgnLattice(*this); }
...
bool SgnLattice::copyMod(Lattice *that_arg) {
  SgnLattice *that = dynamic_cast<SgnLattice *>(that_arg);
  bool modified =
      (this->sgnState != that->sgnState) || (this->level != that->level);
...
bool SgnLattice::meetUpdate(Lattice *that_arg) {
  ...
  return (oldSgnState != sgnState) || (oldLevel != level);
}

bool SgnLattice::operator==(Lattice *that_arg) {
  SgnLattice *that = dynamic_cast<SgnLattice *>(that_arg);

  return (sgnState == that->sgnState) && (level == that->level);
}

// returns the current state of this object
SgnLattice::sgnStates SgnLattice::getSgnState() const { return sgnState; }

SgnLattice::sgnLevels SgnLattice::getLevel() const { return level; }
...
bool SgnLattice::plus(long increment) {
  if (level == eqZero) {
    // if the increment is non-zero, the variable's value is !=0, so we simply
    // record the variable's new sign
    if (increment != 0) {
      if (increment > 0)
        return setSgnKnown(posZero);
      else if (increment < 0)
        return setSgnKnown(negZero);
    }
  } else if (level == sgnKnown) {
    // if the increment moves in the opposite direction from the current sign,
    // raise the sign info to top
...
// returns true if this causes the lattice's state to change, false otherwise
bool SgnLattice::minus(long decrement) { return plus(0 - decrement); }
...
// Applies a generic complex operation to this and that objects, storing the
// results in this object returns true if this causes the lattice's state to
...
// vector<Lattice*> SgnAnalysis::genInitState(const Function& func, const
// DataflowNode& n, const NodeState& state)
void SgnAnalysis::genInitState(const Function &func, const DataflowNode &n,
  ...
  // FiniteVariablesProductLattice* l = new FiniteVariablesProductLattice(true,
  // false, new SgnLattice(), constVars, NULL, func, n, state);
  map<varID, Lattice *> emptyM;
  FiniteVarsExprsProductLattice *l = new FiniteVarsExprsProductLattice(
      (Lattice *)new SgnLattice(), emptyM, (Lattice *)NULL, ldva, n, state);
  initLattices.push_back(l);
  /*cout << "    #initLattices="<<initLattices.size()<<":\n";
  for(vector<Lattice*>::const_iterator it=initLattices.begin();
  it!=initLattices.end(); it++) { cout << "        "<<(*it)<<"\n"; cout.flush();
          cout << "        "<<(*it)->str("            ")<<"\n"; cout.flush();
  }*/
  // return initLattices;
}

bool SgnAnalysis::transfer(const Function &func, const DataflowNode &n,
  ...
  for(vector<Lattice*>::const_iterator it=dfInfo.begin(); it!=dfInfo.end();
  it++) { cout << "        "<<(*it)->str("            ")<<"\n"; cout.flush();
  }*/
  // FiniteVariablesProductLattice* prodLat =
  // dynamic_cast<FiniteVariablesProductLattice*>(*(dfInfo.begin()));
  ...
  if (isSgAssignOp(n.getNode())) {
    ...
    if (sgnAnalysisDebugLevel >= 1) {
      cout << "res=" << res.str() << " lhs=" << lhs.str()
           << " rhs=" << rhs.str() << "\n";
    }

    SgnLattice *resLat =
        dynamic_cast<SgnLattice *>(prodLat->getVarLattice(res));
    SgnLattice *lhsLat =
        dynamic_cast<SgnLattice *>(prodLat->getVarLattice(lhs));
    SgnLattice *rhsLat =
        dynamic_cast<SgnLattice *>(prodLat->getVarLattice(rhs));
    ...
    SgnLattice *asgnLat =
        dynamic_cast<SgnLattice *>(prodLat->getVarLattice(asgn));
    SgnLattice *resLat =
        dynamic_cast<SgnLattice *>(prodLat->getVarLattice(res));
    if (sgnAnalysisDebugLevel >= 1) {
      if (asgnLat)
        cout << "asgn=" << asgn << " Lat=    " << asgnLat->str("    ") << "\n";
      if (resLat)
        cout << "res=" << res << " Lat=    " << resLat->str("    ") << "\n";
    }
    ...
    varID var(initName);
    SgnLattice *varLat =
        dynamic_cast<SgnLattice *>(prodLat->getVarLattice(var));
    ...
    if (varLat) {
      // if(sgnAnalysisDebugLevel>=1) cout << "Variable declaration:
      // "<<var.str()<<",
      ...
      if (initName->get_initializer() == NULL)
        ...
        SgnLattice *initLat =
            dynamic_cast<SgnLattice *>(prodLat->getVarLattice(init));
        // if(sgnAnalysisDebugLevel>=1) cout << "    init="<<init.str()<<"
        // initLat="<<initLat<<"\n";
        if (initLat) {
          varLat->copy(initLat);
          modified = true;
    ...
    if (resLat) {
      if (isSgLongLongIntVal(n.getNode()))
        ...
        modified =
            resLat->set(isSgLongIntVal(n.getNode())->get_value()) || modified;
      else if (isSgIntVal(n.getNode()))
        modified =
            resLat->set(isSgIntVal(n.getNode())->get_value()) || modified;
      else if (isSgShortVal(n.getNode()))
        modified =
            resLat->set(isSgShortVal(n.getNode())->get_value()) || modified;
      else if (isSgUnsignedLongLongIntVal(n.getNode()))
        modified =
            resLat->set(isSgUnsignedLongLongIntVal(n.getNode())->get_value()) ||
            modified;
      else if (isSgUnsignedLongVal(n.getNode()))
        modified = resLat->set(isSgUnsignedLongVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgUnsignedIntVal(n.getNode()))
        modified = resLat->set(isSgUnsignedIntVal(n.getNode())->get_value()) ||
                   modified;
      else if (isSgUnsignedShortVal(n.getNode()))
        modified =
            resLat->set(isSgUnsignedShortVal(n.getNode())->get_value()) ||
            modified;
    }
    ...
    if (isSgBinaryOp(n.getNode())) {
      // Tested that we're dealing with arithmetic above, so no worries about
      // getting the logic operators by accident
  ...
  //      if((dynamic_cast<SgnLattice*>(*it))->getLevel() ==
  //      SgnLattice::uninitialized)
  //              (dynamic_cast<SgnLattice*>(*it))->initialize();
  // }
  //
  // //SgNode* asgn;
  ...
  //      {
  //              /*printf("cfgUtils::parseAssignment(asgn, op, i, j, c) =
  //              1\n"); printf("        op=%s i=%s, j=%s%s, k=%s%s, c=%d\n",
  //                     (op==cfgUtils::add? "+": (op==cfgUtils::subtract? "-":
  //                     (op==cfgUtils::mult? "*": (op==cfgUtils::divide? "/":
  //                     (op==cfgUtils::none? "~": "???"))))), i.str().c_str(),
  //                     negJ?"-":"", j.str().c_str(), negK?"-":"",
  //                     k.str().c_str(), c);*/
  //
  //              SgnLattice* iLattice =
  //              dynamic_cast<SgnLattice*>(prodLat->getVarLattice(func, i));
  ...
  //                      {
  //                              SgnLattice* jLattice =
  //                              dynamic_cast<SgnLattice*>(prodLat->getVarLattice(func,
  //                              j)); newILattice.copy(jLattice);
  ...
  //      varID var(initName);
  //
  //      SgnLattice* varLattice =
  //      dynamic_cast<SgnLattice*>(prodLat->getVarLattice(func, var));
  //
...
void printSgnAnalysisStates(SgnAnalysis *sa, string indent) {
  ...
  printAnalysisStates pas(sa, factNames, latticeNames,
