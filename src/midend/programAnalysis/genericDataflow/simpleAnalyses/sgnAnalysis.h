...
// Maintains sign information about live variables. If a given variable may be
// either positive or negative, this object becomes top. There is one SgnLattice
...
public:
  ...
  SgnLattice(long val) {
    if (val == 0)
      ...
      this->level = sgnKnown;
      if (val > 0)
        this->sgnState = posZero;
      else
        this->sgnState = negZero;
    }
  }
  ...
  // Sets the state of this lattice to sgnKnown, with the sign of the given
  // value. returns true if this causes the lattice's state to change, false
  // otherwise
  bool set(int val);
