...
/* Assigns a unique numeric ID to each DataflowNode. IDs are unique only within
   each function and may be repeated across functions. */

/**********
 * NodeID *
 **********/
int NodeID::getID() { return id; }
...
NodeFact *NodeID::copy() const { return new NodeID(id); }

/******************
 * placeUniqueIDs *
 ******************/

void placeUniqueIDs::visit(const Function &func, const DataflowNode &n,
                           NodeState &state) {
...
// Runs the placeUniqueIDs analysis to associate a unique ID with each
// DataflowNode within each function
