...
// Maintains value information about live variables. If a given variable may
// have more than one value,
//    this object maintains divisibility information about all the possible
//    values (i.e. they're all divisible by x, with y as the remainder), with
//    the divisior,remainder = (1,0) if nothing better can be found.
...
private:
  ...
  long value;
  // the <divisor, remainder> pair where for all possible values v of the
  // variable there exists a multiplier m s.t v = div * m + rem
  long div;
  long rem;
...
public:
  ...
  // value is unknown but the divisibility (div and rem) of this variable is
  // known
...
private:
  // this object's current level in the lattice: (bottom, valKnown, divKnown,
  // top)
  short level;
...
public:
  ...
  // Takes two lattices at level divKnown. If the two objects have div, rem
  // pairs that make it possible to add or subtract them them and produce
  // div/rem information where div>1, returns true and sets div and rem to
  // correspond to the sum of these values. Otherwise, returns false. plus -
  // true if the caller want to see one+two and false if one-two
  static bool matchDivAddSubt(DivLattice *one, DivLattice *two, long &div,
                              long &rem, bool plus);
...
class DivAnalysisTransfer : public VariableStateTransfer<DivLattice> {
  ...
  typedef void (DivAnalysisTransfer::*TransferOp)(DivLattice *, DivLattice *,
                                                  DivLattice *);
  ...
  void transferAdditive(DivLattice *arg1Lat, DivLattice *arg2Lat,
                        DivLattice *resLat, bool isAddition);
  void transferMultiplicative(DivLattice *arg1Lat, DivLattice *arg2Lat,
                              DivLattice *resLat);
  void transferDivision(DivLattice *arg1Lat, DivLattice *arg2Lat,
                        DivLattice *resLat);
  void transferMod(DivLattice *arg1Lat, DivLattice *arg2Lat,
                   DivLattice *resLat);
...
public:
  ...
  DivAnalysisTransfer(const Function &func, const DataflowNode &n,
...
public:
  DivAnalysis(LiveDeadVarsAnalysis *ldva) { this->ldva = ldva; }

  /*// generates the initial variable-specific lattice state for a dataflow node
  Lattice* genInitVarState(const Function& func, const DataflowNode& n, const
  NodeState& state);

  // generates the initial non-variable-specific lattice state for a dataflow
  node Lattice* genInitNonVarState(const Function& func, const DataflowNode& n,
