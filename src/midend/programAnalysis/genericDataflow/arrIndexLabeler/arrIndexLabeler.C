...
// the traversal's inherited attribute that determines whether a given SgNode is
// part of an array index expression
class aiRecord {
public:
  // Pointers to the subtrees of SgPntrArrRefExp that corresponds to its index
  // part and its array name part. This field is =NULL in all other nodes.
  ...
  // true to represent that this is a top-level SgPntrArrRefExp that is not part
  // of a larger SgPntrArrRefExp (i.e. given a[b[i][j]][k][l], it is either
  // a[b[i][j]][k][l] or b[i][j])
  bool topArrayRefExpFlag;

  // if arrayIndexFlag==true, the depth of the current index expression (i.e.
  // its dimension)
  // int indexDim;
...
// the traversal's synthesizes attribute that determines the dimensionality of a
// given array expression and records a pointer to the array name expression
class adRecord {
public:
  ...
  adRecord(SgNode *origin) {
    this->origin = origin;
    // copy n's inherited attributes directly from the persistent attribute left
    // by the Top-Down pass
  ...
  adRecord(SgNode *origin, bool arrayIndexFlag, bool topArrayRefExpFlag,
           SgExpression *arrayNameExp, int arrayDim) {
    this->origin = origin;
...
public:
  ...
  arrIndexAttribute(bool arrayIndexFlag, bool topArrayRefExpFlag,
                    SgExpression *arrayNameExp, int arrayDim) {
    this->arrayIndexFlag = arrayIndexFlag;
    this->topArrayRefExpFlag = topArrayRefExpFlag;
    this->arrayNameExp = arrayNameExp;
...
class arrayIndexLabeler
    : public SgTopDownBottomUpProcessing<aiRecord, adRecord> {
public:
  aiRecord evaluateInheritedAttribute(SgNode *n, aiRecord inherited) {
    ...
    if ((arrRef = isSgPntrArrRefExp(n))) {
      ...
      // this SgPntrArrRefExp is the top-most isSgPntrArrRefExp if it is not the
      // top of the arrayNameSubtree of a higher-level SgPntrArrRefExp
      ir.topArrayRefExpFlag = (n != inherited.arrayNameSubtree);
    }

    // if this node is the top of the index subtree of an SgPntrArrRefExp,
    // record this in ir
    ...
    /*              if(isSgPntrArrRefExp(n))
                            printf("SgPntrArrRefExp:0x%x DOWN arrayIndexFlag=%d
       indexSubtree=0x%x\n", n, aiAttr->arrayIndexFlag, ir.indexSubtree); else
                            printf("SgNode:0x%x DOWN arrayIndexFlag=%d
  ...
  adRecord
  evaluateSynthesizedAttribute(SgNode *n, aiRecord inherited,
    ...
    if ((arrRef = isSgPntrArrRefExp(n))) {
      // printf("evaluateSynthesizedAttribute() arrRef->lhs=%p,
      // arrRef->rhs=%p\n", arrRef->get_lhs_operand(), );
      bool found = false;
      // look through both the synthesized attributes
      // (one from the SgPntrArrRefExp's array name subtree and one from the
      // index subtree)

      // get the name subtree
      for (SubTreeSynthesizedAttributes::iterator it = synthesized.begin();
           it != synthesized.end(); it++) {
        // if this synthesized attribute comes from SgPntrArrRefExp's array name
        // subtree (there should be only one such subtree)
        if ((*it).origin == arrRef->get_lhs_operand()) {
          ...
          // otherwise, the real array name expression is deeper in the array
          // name subtree, so just copy it over
          else
            dr.arrayNameExp = (*it).arrayNameExp;

          // initialize this SgPntrArrRefExp's list of indexes from the
          // lower-level list
      ...
      for (SubTreeSynthesizedAttributes::iterator it = synthesized.begin();
           it != synthesized.end(); it++) {
        // if this synthesized attribute comes from SgPntrArrRefExp's index
        // subtree (there should be only one such subtree)
        if ((*it).origin == arrRef->get_rhs_operand()) {
          found = true;
          // add the current index expression to this SgPntrArrRefExp's list of
          // indexes
    ...
    /*              if(isSgPntrArrRefExp(n))
                            printf("SgPntrArrRefExp:0x%x UP arrayIndexFlag=%d
       arrayNameExp=0x%x arrayDim=%d\n", n, dr.arrayIndexFlag, dr.arrayNameExp,
       dr.arrayDim); else printf("SgNode:0x%x UP arrayIndexFlag=%d
       arrayNameExp=0x%x arrayDim=%d\n", n, dr.arrayIndexFlag, dr.arrayNameExp,
       dr.arrayDim); printf("         <%s>\n", n->unparseToString().c_str());*/

    // label this node with its read/write access information
    arrIndexAttribute *aiAttr = new arrIndexAttribute(dr);
    char attrName[100];
    snprintf(attrName, sizeof(attrName), "ArrayIndex: [%d, %d, %p, %d]",
             dr.arrayIndexFlag, dr.topArrayRefExpFlag, dr.arrayNameExp,
             dr.arrayDim);
...
// labels the portion of the AST rooted at root with annotations that identify
// the various portions the read/write sides of SgAssignOp nodes (if they are)
...
// returns true if the given node is part of an array index expression and false
// otherwise
...
// returns true the given SgPntrArrRefExp node this is a top-level
// SgPntrArrRefExp that is not part of a larger SgPntrArrRefExp (i.e. given
// a[b[i][j]][k][l], it is either a[b[i][j]][k][l] or b[i][j])
bool isTopArrayRefExp(const SgNode *n) {
  ROSE_ASSERT(n->getAttribute("ArrayIndex"));
  return ((arrIndexAttribute *)(n->getAttribute("ArrayIndex")))
      ->topArrayRefExpFlag;
}

// returns the SgExpression node that contains the name of the array in the
// given SgPntrArrRefExp or NULL if the node is not a SgPntrArrRefExp
SgExpression *getArrayNameExp(SgNode *n) {
  ROSE_ASSERT(n->getAttribute("ArrayIndex"));
  return ((arrIndexAttribute *)(n->getAttribute("ArrayIndex")))->arrayNameExp;
}

// returns the dimensionality of the array reference in the given
// SgPntrArrRefExp
