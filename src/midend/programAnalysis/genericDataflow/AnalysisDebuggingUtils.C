...
private:
  ...
  void printNode(const DataflowNode &n,
                 std::string state_string); // print date flow node
...
void analysisStatesToDOT::printEdge(const DataflowEdge &e) {
  (*ostr) << e.source().id() << " -> " << e.target().id() << " [label=\""
          << escapeString(e.toString()) << "\", style=\"" << "solid"
          << "\"];\n";
}

void analysisStatesToDOT::printNode(const DataflowNode &n,
...
void analysisStatesToDOT::visit(const Function &func, const DataflowNode &n,
                                NodeState &state) {
...
public:
  ...
  IntraAnalysisResultsToDotFiles(IntraProceduralAnalysis &funcToDot)
      : InterProceduralAnalysis(&funcToDot),
        UnstructuredPassInterAnalysis(funcToDot) {}
  // For each function, call function level Dot file generator to generate
  // separate dot file
  void runAnalysis();
};
...
void IntraAnalysisResultsToDotFiles::runAnalysis() {
  ...
  for (set<FunctionState *>::iterator it = allFuncs.begin();
    ...
    assert(proc != NULL);
    string file_name = StringUtility::stripPathFromFileName(
        proc->get_file_info()->get_filename());
    string file_func_name =
        file_name + "_" + proc->get_mangled_name().getString();

    string full_output = file_func_name + "_cfg.dot";
    std::ofstream ostr(full_output.c_str());

    ostr << "digraph " << "mygraph" << " {\n";
    analysisStatesToDOT *dot_analysis =
        dynamic_cast<analysisStatesToDOT *>(intraAnalysis);
...
{
        string indent="";
        for(list<string>::iterator i=indents.begin(); i!=indents.end(); i++)
indent+=*i; int r = baseBuf->sputn(indent.c_str(), indent.length());
...
int dbgBuf::printString(string s) {
  ...
  if (((size_t)r) != s.length())
    return -1;
  return 0;
}

streamsize dbgBuf::xsputn(const char *s, streamsize n) {
  //      cout << "xputn() ownerAccess="<<ownerAccess<<" n="<<n<<"
  //      s=\""<<string(s)<<"\"\n";
    ...
    while (i < n) {
      ...
      if (j < n) {
        ...
        if (s[j] == '\n') {
          ...
          // If we're at a space and not inside an HTML tag, replace it with an
          // HTML space escape code
          if (numOpenAngles == 0) {
            ret = baseBuf->sputn(spaceHTML, sizeof(spaceHTML) - 1);
            if (ret != sizeof(spaceHTML) - 1)
              return 0;
            // If we're inside an HTML tag, emit a regular space character
          } else {
            ret = baseBuf->sputn(space, sizeof(space) - 1);
            if (ret != sizeof(space) - 1)
              return 0;
          }
        } else if (s[j] == '\t') {
          // If we're at a tab and not inside an HTML tag, replace it with an
          // HTML tab escape code
          if (numOpenAngles == 0) {
            ret = baseBuf->sputn(tabHTML, sizeof(tabHTML) - 1);
            if (ret != sizeof(tabHTML) - 1)
              return 0;
...
void dbgBuf::enterFunc(string funcName /*, string indent*/) {
  ...
  // cout << "Incrementing parentDivs (len="<<parentDivs.size()<<") from
  // "<<*(parentDivs.rbegin())<<" to ";
  (*(parentDivs.rbegin()))++;
...
void dbgBuf::exitFunc(string funcName) {
  if (funcName != funcs.back()) {
    cout << "dbgStream::exitFunc() ERROR: exiting from function " << funcName
         << " which is not the most recent function entered!\n";
    cout << "funcs=\n";
...
dbgStream::dbgStream(string title, string dbgFileName, string workDir,
                     string imgPath)
    : std::ostream(&buf) {
  init(title, dbgFileName, workDir, imgPath);
}

void dbgStream::init(string title, string dbgFileName, string workDir,
                     string imgPath) {
  ...
  try {
    ...
    cout << "dbgStream::init() ERROR opening file \"" << dbgFileName
         << "\" for writing!";
    exit(-1);
  }
  ...
  try {
    ...
    cout << "dbgStream::init() ERROR opening file \""
         << workDir + "/summary.html\" for writing!";
    exit(-1);
  }
  ...
  summaryF
      << "\t\t\t<tr width=\"100%\"><td width=50></td><td width=\"100%\">\n";
...
void dbgStream::printDetailFileHeader(string title) {
  ...
  *(this) << "\t\t\tparentDiv.className=(parentDiv.className=='hidden')?'"
             "unhidden':'hidden';\n";
  ...
  *(this) << "\t\t\t\t// Set the visibility status of each child table to be "
             "the same as its parent div\n";
  *(this) << "\t\t\t\tif (\"table\"+divID!=child.id && child.nodeType==1 && "
             "child.id!=undefined && child.id.match(condition)) {\n";
  ...
  *(this) << "\t\tvar sumLink = "
             "top.summary.document.getElementById(\"link\"+divID);\n";
  ...
  *(this) << "\t\tf.width = \"100%\"\n";
  *(this) << "\t\tf.height = 100;\n";
  *(this) << "\t\tf = (f.contentWindow ? f.contentWindow : "
             "(f.contentDocument.document ? f.contentDocument.document : "
             "f.contentDocument));\n";
  *(this) << "\t\tf.document.open();\n";
  *(this) << "\t\tf.document.write(\"~/Compilers/focusOnNodes.pl " << imgPath
          << "/image_\"+imgNum+\".dot \"+str+\" > " << imgPath
          << "/image_\"+imgNum+\".focus.dot ; dot -Tsvg -o" << imgPath
          << "/image_\"+imgNum+\".focus.svg " << imgPath
          << "/image_\"+imgNum+\".focus.dot\");\n";
...
void dbgStream::enterFunc(string funcName /*, string indent*/) {
  ...
  ostringstream divName;
  for (list<int>::iterator d = buf.parentDivs.begin();
       d != buf.parentDivs.end();) {
    divName << *d;
    d++;
  ...
  *(this) << "\t\t\t" << tabs(buf.funcs.size())
          << "<tr width=\"100%\"><td width=50></td><td width=\"100%\">\n";
  *(this)
      << "\t\t\t" << tabs(buf.funcs.size() + 1) << "<table bgcolor=\"#"
      << colors[(buf.funcs.size() - 1) % colors.size()]
      << "\" width=\"100%\" id=\"table" << divName.str()
      << "\" style=\"border:1px solid white\" "
         "onmouseover=\"this.style.border='1px solid black'; highlightLink('"
      << divName.str()
      << "', '#F4FBAA');\" onmouseout=\"this.style.border='1px solid white'; "
         "highlightLink('"
      << divName.str() << "', '#FFFFFF');\" onclick=\"focusLink('"
      << divName.str() << "', event);\">\n";
  ...
  *(this) << "</a></h2></td></tr>\n";
  *(this)
      << "\t\t\t" << tabs(buf.funcs.size() + 1)
      << "<tr width=\"100%\"><td width=50></td><td width=\"100%\"><div id=\"div"
      << divName.str() << "\" class=\"unhidden\">\n";
  this->flush();

  // summaryF << "<li><a target=\"detail\"
  // href=\"detail.html#"<<divName.str()<<"\">"<<funcName<<"</a><br>\n";
  // summaryF << "<ul>\n";
  summaryF << "\t\t\t" << tabs(buf.funcs.size()) << "</td></tr>\n";
  summaryF << "\t\t\t" << tabs(buf.funcs.size())
           << "<tr width=\"100%\"><td width=50></td><td width=\"100%\">\n";
  summaryF << "\t\t\t" << tabs(buf.funcs.size() + 1)
           << "<table width=\"100%\" style=\"border:0px\">\n";
  summaryF << "\t\t\t" << tabs(buf.funcs.size() + 1)
           << "<tr width=\"100%\"><td width=50></td><td id=\"link"
           << divName.str() << "\" width=\"100%\"><a name=\"anchor"
           << divName.str() << "\" target=\"detail\" href=\"detail.html#"
           << divName.str() << "\">" << funcName << "</a> (" << divName.str()
           << ")</td></tr>\n";
  summaryF << "\t\t\t" << tabs(buf.funcs.size() + 1)
           << "<tr width=\"100%\"><td width=50></td><td width=\"100%\">\n";
  summaryF.flush();
...
// Adds an image to the output with the given extension and returns the path of
// this image so that the caller can write to it.
string dbgStream::addImage(string ext) {
  ostringstream imgFName;
  imgFName << imgPath << "/image_" << numImages << "." << ext;
  buf.ownerAccessing();
  *(this) << "\t\t\t" << tabs(buf.funcs.size() + 1)
          << "<img src=" << imgFName.str() << ">\n";
  buf.userAccessing();
  return imgFName.str();
}

// Given a reference to an object that can be represented as a dot graph, create
// an image from it and add it to the output. Return the path of the image.
string dbgStream::addDOT(dottable &obj) {
  ostringstream imgFName;
  imgFName << "dbg_imgs/image_" << numImages << ".svg";
  ostringstream graphName;
  graphName << "graph_" << numImages;
  ostringstream ret;
...
// Given a reference to an object that can be represented as a dot graph, create
// an image of it and return the string that must be added to the output to
// include this image.
...
// Given a representation of a graph in dot format, create an image from it and
...
void dbgStream::addDOT(string imgFName, string graphName, string dot,
                       ostream &ret) {
  ostringstream dotFName;
  dotFName << imgPath << "/image_" << numImages << ".dot";
  ostringstream mapFName;
  mapFName << imgPath << "/image_" << numImages << ".map";
  ...
  {*/
  ostringstream cmd;
  cmd << "dot -Tsvg -o" << imgPath << "/image_" << numImages << ".svg "
      << dotFName.str() << "&"; // -Tcmapx -o"<<mapFName.str()<<"
  ...
  // ostringstream namesFName; namesFName << imgPath << "/image_" << numImages
  // << ".names";
  //{
  //       ostringstream cmd; cmd << "~/Compilers/dot2Nodes.pl
  //       "<<dotFName.str()<<" > "<<namesFName.str(); // -Tcmapx
  //       -o"<<mapFName.str()<<"
  //       //cout << "Command \""<<cmd.str()<<"\"\n";
  //       system(cmd.str().c_str());
  // }
  ...
  // }
  // else {
  //       cout << "dbgStream::addDOT() ERROR opening file
  //       \""<<namesFName.str()<<"\" for reading!"<<endl;
  // }

  ret << "\t\t\t" << tabs(buf.funcs.size() + 1) << "image_" << numImages
      << ":<img src=\"" << imgFName
      << "\" "; // <a href=\"image_" << numImages << ".dot\">
  // usemap=\""<<graphName.str()<<"\"
  ret << "><br>\n"; // </a>

  // ret << "\t\t\t"<<tabs(buf.funcs.size()+1)<<"<embed height=\"100%\"
  // width=\"100%\" pluginspage=\"http://www.adobe.com/svg/viewer/install\"
  // src=\""<<imgFName<<"\">"<<endl; ret <<
  // "\t\t\t"<<tabs(buf.funcs.size()+1)<<"<object height=\"100%\" width=\"100%\"
  // data=\""<<imgFName<<"\" type=\"image/svg+xml\"></object>"<<endl;
...
// Given a representation of a graph in dot format, create an image of it and
...
// Given a string, returns a version of the string with all the control
// characters that may appear in the string escaped to that the string can be
