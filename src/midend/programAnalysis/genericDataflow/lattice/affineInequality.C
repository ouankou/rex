...
// points trueIneqAttr and falseIneqAttr to freshly allocated objects that
represent the true and false
// branches of the control flow guarded by the given expression. They are set to
NULL if our representation
// cannot represent one of the expressions.
// doFalseBranch - if =true, falseIneqAttr is set to the correct false-branch
condition and to NULL otherwise static void setTrueFalseIneq(SgExpression* expr,
                             affineInequalityAttr **trueIneqAttr,
affineInequalityAttr **falseIneqAttr, bool doFalseBranch);

// Looks over all the conditional statements in the application and associates
appropriate
//    affine inequalities with the SgNodes that depend on these statements. Each
inequality
//    is a must: it must be true of the node that it is associated with.
void initAffineIneqs(SgProject* project)
{
        Rose_STL_Container<SgNode*> ifs = NodeQuery::querySubTree(project,
V_SgIfStmt); for(Rose_STL_Container<SgNode*>::iterator it = ifs.begin();
it!=ifs.end(); it++)
        {
                SgIfStmt* ifStmt = isSgIfStmt(*it);
                ROSE_ASSERT(isSgExprStatement(ifStmt->get_conditional()));

                // generate the inequality attributes that will be associated
with this if statement's
                ...
                if(trueIneqAttr)  annotateSubTreeIneq(ifStmt->get_true_body(),
trueIneqAttr); if(falseIneqAttr) annotateSubTreeIneq(ifStmt->get_false_body(),
falseIneqAttr);
        ...
        Rose_STL_Container<SgNode*> fors = NodeQuery::querySubTree(project,
V_SgForStatement); for(Rose_STL_Container<SgNode*>::iterator it = fors.begin();
it!=fors.end(); it++)
        {
...
with this if statement's
                ...
                if(trueIneqAttr)
                {
                        annotateSubTreeIneq(forStmt->get_loop_body(),
trueIneqAttr); delete trueIneqAttr;
                }
        }
}
                ...
                *trueIneqAttr = new affineInequalityAttr();
                ...
                if(doFalseBranch)
                {
                        *falseIneqAttr = new affineInequalityAttr();
                        affineInequality ineqFalse(negY?-1:1, negX?-1:1,
(0-c)-1); varAffineInequality tf(y, x, ineqFalse);
                        (*falseIneqAttr)->ineqs.insert(tf);
                }
        }
        // if this is a valid equality condition of the form x == y + c
        else if(cfgUtils::computeTermsOfIfCondition_EQ(expr, x, negX, y, negY,
c))
        {
                // for equalities only bother with the true branch, since the
false branch corresponds to
                // a disjunction of two affine inequalities and we don't support
disjunctions right now *trueIneqAttr = new affineInequalityAttr();
...
static void annotateSubTreeIneq(SgNode* tree, affineInequalityAttr* ineqAttr)
{
        Rose_STL_Container<SgNode*> nodes = NodeQuery::querySubTree(tree,
V_SgNode); for(Rose_STL_Container<SgNode*>::iterator it = nodes.begin();
it!=nodes.end(); it++)
        {
                SgNode* n = *it;
                ...
                {
                        AstAttribute *curAttr =
n->getAttribute("affineInequality");

                        ROSE_ASSERT(curAttr);
                        affineInequalityAttr* curIneqAttr =
(affineInequalityAttr*)curAttr; set<varAffineInequality> newIneqs;

cout << "Merging at node <"<<n->class_name()<<" |
"<<n->unparseToString()<<">\n"; cout << "    Current inequalities:\n";
for(set<varAffineInequality>::iterator it = curIneqAttr->ineqs.begin();
it!=curIneqAttr->ineqs.end(); it++)
{
...
for(set<varAffineInequality>::iterator it = ineqAttr->ineqs.begin();
it!=ineqAttr->ineqs.end(); it++)
{
        varAffineInequality varIneq = *it;
        cout << varIneq.getIneq().str(varIneq.getX(), varIneq.getY(), " ") <<
"\n";
                        ...
                        // initialize newIneqs with all the inequalities in
curIneqAttr

                        // iterate over curIneqAttr's inequalities
                        for(set<varAffineInequality>::iterator itCur =
curIneqAttr->ineqs.begin(); itCur!=curIneqAttr->ineqs.end(); itCur++)
                        {
                                varAffineInequality curIneq = *itCur;

                                // check if the new ineqAttr has an inequality
for the same variables as curIneq set<varAffineInequality>::iterator itNew;
                                for(itNew = ineqAttr->ineqs.begin();
itNew!=ineqAttr->ineqs.end(); itNew++)
                                {
                                        varAffineInequality newIneq = *itNew;

                                        // if we have a match
                                        if(curIneq.getX() == newIneq.getX() &&
curIneq.getY() == newIneq.getY())
                                        {
                                                // replace the constraint in
itCur with the intersection of the two constraints,
                                                // since both are known to be
true at this node
                                                //curIneq.getIneq() *=
newIneq.getIneq();

                                                affineInequality
newi(curIneq.getIneq()); newi *= newIneq.getIneq(); varAffineInequality
newVarIneq(curIneq.getX(), curIneq.getY(), newi); newIneqs.insert(newVarIneq);

                                                break;
                                        }
                                }

                                // if there is no varAffineInequality in
ineqAttr that corresponds to curIneq if(itNew == ineqAttr->ineqs.end())
                                        // add curIneq with no modifications to
newIneqs newIneqs.insert(curIneq); cout << "    current inequality
"<<curIneq.str("")<<", found match in new="<<(itNew ==
ineqAttr->ineqs.end())<<"\n"; cout << "
newIneqs.size()="<<newIneqs.size()<<"\n";
                        }

                        // now add to newIneqs any varAffineInequalities in
ineqAttr for variable pairs that do not
                        // appear in curIneqAttr

                        for(set<varAffineInequality>::iterator itNew =
ineqAttr->ineqs.begin(); itNew!=ineqAttr->ineqs.end(); itNew++)
                        {
                                varAffineInequality newIneq = *itNew;

                                // check if the current ineqAttr has an
inequality for the same variables as newIneq set<varAffineInequality>::iterator
itCur; for(itCur = curIneqAttr->ineqs.begin(); itCur!=curIneqAttr->ineqs.end();
itCur++)
                                {
                                        varAffineInequality curIneq = *itCur;
...
curIneq.getY() == newIneq.getY())
                                ...
                                // if there is no varAffineInequality in
curIneqAttr that corresponds to newIneq if(itCur == curIneqAttr->ineqs.end())
                                        // add curIneq with no modifications to
newIneqs newIneqs.insert(newIneq);
                        }

                        // replace this attribute's current inequalities with
the new set curIneqAttr->ineqs = newIneqs;
...
"\n";
                ...
                else
                {
                        ...
                        n->addNewAttribute("affineInequality", new
affineInequalityAttr(*ineqAttr));
                }
        }
}

// return the set of varAffineInequalities associated with this node or NULL if
none are available set<varAffineInequality>* getAffineIneq(SgNode* n)
{
        if(n->attributeExists("affineInequality"))
        {
...
// points trueIneqFact and falseIneqFact to freshly allocated objects that
// represent the true and false branches of the control flow guarded by the
// given expression. They are set to NULL if our representation cannot represent
// one of the expressions. doFalseBranch - if =true, falseIneqFact is set to the
// correct false-branch condition and to NULL otherwise
void setTrueFalseIneq(SgExpression *expr, affineInequalityFact **trueIneqFact,
  ...
  *falseIneqFact = NULL;
  ...
  if (cfgUtils::computeTermsOfIfCondition_LTEQ(expr, x, negX, y, negY, c)) {
    ...
    *trueIneqFact = new affineInequalityFact();
    // !!!! WE MAY WANT TO BE MORE PRECISE HERE BY ADDING SIGN INFORMATION TO
    // THESE INEQUALITIES !!!!
    affineInequality ineqTrue(negX ? -1 : 1, negY ? -1 : 1, c, x == zeroVar,
                              y == zeroVar, affineInequality::unknownSgn,
                              affineInequality::unknownSgn);
    varAffineInequality tt(x, y, ineqTrue);
    ...
    if (doFalseBranch) {
      *falseIneqFact = new affineInequalityFact();
      affineInequality ineqFalse(
          negY ? -1 : 1, negX ? -1 : 1, (0 - c) - 1, y == zeroVar, x == zeroVar,
  ...
  // if this is a valid equality condition of the form x == y + c
    ...
    // for equalities only bother with the true branch, since the false branch
    // corresponds to a disjunction of two affine inequalities and we don't
    // support disjunctions right now
    *trueIneqFact = new affineInequalityFact();

    // x <= y + c
    affineInequality ineqLTEQ(negX ? -1 : 1, negY ? -1 : 1, c, x == zeroVar,
                              y == zeroVar, affineInequality::unknownSgn,
                              affineInequality::unknownSgn);
    varAffineInequality tLTEQ(x, y, ineqLTEQ);
    ...
    // x >= y + c => y <= x -c
    if (doFalseBranch) {
      affineInequality ineqGTEQ(
          negY ? -1 : 1, negX ? -1 : 1, 0 - c, y == zeroVar, x == zeroVar,
          affineInequality::unknownSgn, affineInequality::unknownSgn);
      varAffineInequality tGTEQ(y, x, ineqGTEQ);
...
void affineInequalitiesPlacer::visit(const Function &func,
...
/*printAffineInequalities::printAffineInequalities(affineInequalitiesPlacer
*placer)
{
        this->placer = placer;
}

void printAffineInequalities::visit(const Function& func, const DataflowNode& n,
NodeState& state)
{
        //set<varAffineInequality> *ineqs = getAffineIneq(n.getNode());
        affineInequalityFact* ineqFact =
(affineInequalityFact*)state.getFact(placer, 0); if(ineqFact)
        {
                cout << "Node <"<<n.getNode()->class_name()<<" |
"<<n.getNode()->unparseToString()<<">\n"; for(set<varAffineInequality>::iterator
it = ineqFact->ineqs.begin(); it!=ineqFact->ineqs.end(); it++)
                {
                        varAffineInequality varIneq = *it;
                        cout << varIneq.getIneq().str(varIneq.getX(),
varIneq.getY(), "    ") << "\n";
                }
        }
}*/
...
const set<varAffineInequality> &getAffineIneq(const DataflowNode &n) {
  ...
  affineInequalityFact *fact =
      dynamic_cast<affineInequalityFact *>(ns->getFact(aip, 0));
...
// Returns the set of inequalities known to be true at the given DataflowNode's
// descendants
list<set<varAffineInequality>> getAffineIneqDesc(const DataflowNode &n) {
  if (analysisDebugLevel > 1)
    printf("getAffineIneqDesc node=<%s | %s>\n",
...
// represents a full affine inequality, including the variables involved in the
// inequality
/***************************
 *** varAffineInequality ***
 ***************************/
...
varAffineInequality::varAffineInequality(const varID &x, const varID &y,
                                         const affineInequality &ineq)
    : x(x), y(y), ineq(ineq) {}

varAffineInequality::varAffineInequality(const varID &x, const varID &y, int a,
                                         int b, int c, bool xZero, bool yZero) {
  ...
  if (a < 0 && b < 0) {
    ...
    a = 0 - a;
    b = 0 - b;
    if (this->ineq.xSign == affineInequality::posZero)
      this->ineq.xSign = affineInequality::negZero;
    else if (this->ineq.xSign == affineInequality::negZero)
      this->ineq.xSign = affineInequality::posZero;
    if (this->ineq.ySign == affineInequality::posZero)
      this->ineq.ySign = affineInequality::negZero;
    else if (this->ineq.ySign == affineInequality::negZero)
      this->ineq.ySign = affineInequality::posZero;
  } else {
    this->x = x;
    this->y = y;
...
const varID &varAffineInequality::getX() const { return x; }

const varID &varAffineInequality::getY() const { return y; }

int varAffineInequality::getA() const { return ineq.getA(); }

int varAffineInequality::getB() const { return ineq.getB(); }

int varAffineInequality::getC() const { return ineq.getC(); }

const affineInequality &varAffineInequality::getIneq() const { return ineq; }
...
const bool varAffineInequality::setA(int a) { return this->ineq.setA(a); }

const bool varAffineInequality::setB(int b) { return this->ineq.setB(b); }

const bool varAffineInequality::setC(int c) { return this->ineq.setC(c); }
...
bool varAffineInequality::operator==(const varAffineInequality &that) const {
  return x == that.x && y == that.y && ineq == that.ineq;
}
...
// Parses expr and returns the corresponding varAffineInequality if expr can be
// represented as such and NULL otherwise.
...
{
        //ineq.notUpd();
        // The negation of [x*a <= y*b + c] is [x*a > y*b + c] == [y*b + c <
x*a] == [y*b <= x*a - c - 1]
...
/*void varAffineInequality::andUpd(LogicalCond& that_arg)
{
        ...
        //cout << "varAffineInequality::andUpd() x="<<x.str()<<"
that.x="<<that.x.str()<<" y="<<y.str()<<" that.y="<<that.y.str()<<"\n"; cout <<
"varAffineInequality::andUpd() \n"; cout << "     this="<<str()<<"\n"; cout << "
that="<<that.str()<<"\n";
        // The two varAffineInequalities must refer to the same pair of
variables ROSE_ASSERT((x==that.x && y==that.y) || (x==that.y && y==that.x));
...
variables ROSE_ASSERT((x==that.x && y==that.y) || (x==that.y && y==that.x));
        ...
        {
                // We can't compare these two expressions for information
content ineq.setToTop();
        }
}
...
// no constraint known since current constraints are not representable using a
// single affine inequality
...
affineInequality::affineInequality(int a, int b, int c, bool xZero, bool yZero,
                                   signs xSign, signs ySign) {
  ROSE_ASSERT(!xZero || !yZero);
  if (xZero)
    a = 1;
  if (yZero)
...
// given a constraint on x, z and a constraint on z, y, infers the corresponding
// constraint on x, y and sets this constraint to it
affineInequality::affineInequality(const affineInequality &xz, const affineInequality &zy /*, bool xZero, bool yZero, DivLattice* divX, DivLattice* divY, varID z*/) {
  ...
  /*      else if(xz.level == falseConstr || zy.level == falseConstr)
    ...
    /* // if the connection is being made through Zero, incorporate the
    divisibility information if(z == zeroVar &&
    divX->getLevel()==DivLattice::divKnown && divX->getRem()==0 &&
                       divY->getLevel()==DivLattice::divKnown &&
    divY->getRem()==0 && c==0)
    {
            a*=divX->getDiv();
            b*=divY->getDiv();
...
bool affineInequality::operator==(const affineInequality &that) const {
  return // y==that.y &&
      a == that.a && b == that.b && c == that.c && level == that.level;
}
...
// returns true if this affineInequality represents more information (less
// information is top, more information is bottom) than that for all values of x
// and y and false otherwise
// bool affineInequality::operator<<(const affineInequality& that) const
// bool affineInequality::semLessThan(const affineInequality& that) const
bool affineInequality::semLessThan(const affineInequality &that, bool xEqZero,
                                   bool yEqZero) const {
  ...
  if (level < that.level)
    return true;
  /*              // top corresponds to an unrepresentable constraint, so we
     can't make judgements about how that relates to this if(that.level == top)
  ...
  if (level == bottom || level == top /* || level == falseConstr*/)
    return false;

  // Case 3: level == that.level == constrKnown, so we worry about a, b and c

  // if the two constraints are equal, then one is not strictly less than the
  // other
  if (*this == that)
    return false;
  ...
  /*if(xEqZero && yEqZero)
  {
          // The constraint that has multiplier information is considered to be
  tighter.
          // Technically, either constraint is equivalent, since 0*a=0*b=0.
  However, it is
          // important to explicitly maintain a and b for the future
          if(a==1 && b==1) return false;
          if(that.a==1 && that.b==1) return true;

          // if both constraints have multiplier information pick the current
  constraint as the more informative one return true;
  ...
  if (!xZero && !yZero /* && xSign!=eqZero && ySign!=eqZero*/) {
    ...
    if (b * that.a == a * that.b) {
      ...
      //  this has more information than that if its' y-intercept is lower,
    ...
    {
            // if both lines have the same y-intercept, the line with the
    smaller slope is lower,
            // since it leaves x with fewer valid points relative to y
    ...
    // 0 <= y*b + c AND 0 <= y*b' + c'
    // -c <= y*b        -c' <= y*b'
    // if c == c', then the constraint with the smaller b has more info, since
    // this inequality admits fewer valid values of x
    if (c == that.c)
      return b < that.b;
    // -c <= y*b AND -c' <= y*b'
    ...
    // printf("yZero, c == that.c=%d, a<that.a=%d, a==1 && that.a==1=%d,
    // c<that.c=%d\n", c == that.c, a<that.a, a==1 && that.a==1, c<that.c);
    //  x*a <= c AND x*a' <= c'
    //  if c == c', then the constraint with the smaller a has more info, since
    //  the inequality admits fewer valid values of x
    if (c == that.c)
      return a < that.a;
    // x <= c AND x <= c'
  ...
  // If the slopes are not the same, these two affineInequalities are not
  // strictly ordered for all values of x and y
  return false;
}
...
bool affineInequality::semLessThan(const affineInequality &that,
  ...
  bool xConst = (xZero && xZero->getA() == 1 && xZero->getB() == 1 && zeroX &&
                 zeroX->getA() == 1 && zeroX->getB() == 1 &&
                 xZero->getC() == (0 - zeroX->getC()));
  bool yConst = (yZero && yZero->getA() == 1 && yZero->getB() == 1 && zeroY &&
                 zeroY->getA() == 1 && zeroY->getB() == 1 &&
                 yZero->getC() == (0 - zeroY->getC()));

  // If x is a constant, the constraint with the tighter lower bound on y is
  // semantically smaller
  if (xConst) {
    // cout << indent << "x is constant, This bound:
    // "<<((xZero->getC()*a-c)/b)<<" <= y, That bound:
    // "<<((xZero->getC()*that.a-that.c)/that.b)<<" <= y\n";
    //  xConst*a <= y*b + c AND xConst*a' <= y*b' + c'
    //  xConst*a-c <= y*b       xConst*a'-c' <= y*b'
    //  (xConst*a-c) / b <= y*b (xConst*a'-c') / b' <= y
    //  The constraint with the larger lhs is tighter since it admits fewer
    //  values of y
    return (xZero->getC() * a - c) * that.b >
           (xZero->getC() * that.a - that.c) * b;
    /* // if xConst*a-c == xConst*a'-c', then the constraint with the larger b
    has more info, since this inequality
  ...
  // Else, if y is a constant
    ...
    // printf("yZero, c == that.c=%d, a<that.a=%d, a==1 && that.a==1=%d,
    // c<that.c=%d\n", c == that.c, a<that.a, a==1 && that.a==1, c<that.c); cout
    // << indent << "x is constant, This bound: x <=
    // "<<((yZero->getC()*b+c)/a)<<", That bound: x <=
    // "<<((yZero->getC()*that.b+that.c)/that.a)<<"\n";
    //  x*a <= yConst*b + c    AND x*a' <= yConst*b + c'
    //  x <= (yConst*b + c)/a      x <= (yConst*b + c')/a'
    //  The constraint with the smaller rhs is tighter since it admits fewer
    //  values of x
    return (yZero->getC() * b + c) * that.a <
           (yZero->getC() * that.b + that.c) * a;
    /* // if c == c', then the constraint with the smaller a has more info,
    ...
    if (b * that.a == a * that.b) {
      // printf("affineInequality::operator<<  c=%d, that.c=%d\n", c, that.c);
      // cout << indent << "neither constant, slopes equal,
      // c*that.a="<<c*that.a<<" that.c*a="<<that.c*a<<"\n";
      //  this has more information than that if its' y-intercept is lower,
      //  since it leaves x with fewer valid points relative to y
      return c * that.a < that.c * a;
    } else {
      // cout << indent << "sem< slopes!= this="<<str("")<<"
      // that="<<that.str("")<<"\n";
      //  Identify the y-coordinate of the intersection of This line and THAT
      //  line
      //       this                                        that
      //  x*a = y*b + c          AND x*a' = y*b' + c'
      ...
      // cout << indent << "    yIntersect="<<yIntersect<<" this
      // slope="<<(b/a)<<" that slope="<<(that.b/that.a)<<"\n"; if(zeroY &&
      // yZero) cout << indent << "    zeroY="<<zeroY->str("")<<"
      // yZero="<<yZero->str("")<<"\n";

      // If the slope of This line is higher than the slope of That line
      // (equivalent to b/a > that.b/that.a)
      if (b * that.a > a * that.b) {
        // If the only valid points are above the y-intersection, This is
        // tighter than That
        if (zeroY && zeroY->level == constrKnown &&
            zeroY->c > (0 - yIntersect * zeroY->b)) {
          // cout << indent << "    " << "only valid points are above the
          // y-intersection, This is tighter than That\n";
          return true;
          // If the only valid points are below the y-intersection, That is
          // tighter than This
        } else if (yZero && yZero->level == constrKnown &&
                   yZero->c < yIntersect * yZero->a) {
          // cout << indent << "    " << "only valid points are below the
          // y-intersection, That is tighter than This\n";
          return false;
        }
      }
      // Else, If the slope of This line is lower than the slope of That line
      // (equivalent to b/a > that.b/that.a)
      else if (b * that.a > a * that.b) {
        // If the only valid points are above the y-intersection, That is
        // tighter than This
        if (zeroY && zeroY->level == constrKnown &&
            zeroY->c > (0 - yIntersect * zeroY->b)) {
          // cout << indent << "    " << "only valid points are above the
          // y-intersection, That is tighter than This\n";
          return false;
          // If the only valid points are below the y-intersection, This is
          // tighter than That
        } else if (yZero && yZero->level == constrKnown &&
                   yZero->c < yIntersect * yZero->a) {
          // cout << indent << "    " << "only valid points are below the
      ...
      // If we don't have enough information about y to conclude anything about
      // its relationship with the y-coordinate of the intersection of the
      // lines, try the same with the x-coordinate

      // Identify the x-coordinate of the intersection of This line and THAT
      // line
      //      this                                        that
      // x*a = y*b + c          AND x*a' = y*b' + c'
      ...
      // cout << indent << "    xIntersect="<<xIntersect<<" this
      // slope="<<(b/a)<<" that slope="<<(that.b/that.a)<<"\n";
      if (zeroX && xZero)
        cout << indent << "    zeroX=" << zeroX->str("")
             << " xZero=" << xZero->str("") << "\n";

      // If the slope of This line is higher than the slope of That line
      // (equivalent to b/a > that.b/that.a)
      if (b * that.a > a * that.b) {
        // If the only valid points are above the x-intersection, This is
        // tighter than That
        if (zeroX && zeroX->level == constrKnown &&
            zeroX->c > (0 - xIntersect * zeroX->b)) {
          // cout << indent << "    " << "only valid points are above the
          // x-intersection, This is tighter than That\n";
          return true;
          // If the only valid points are below left the c-intersection, That is
          // tighter than This
        } else if (xZero && xZero->level == constrKnown &&
                   xZero->c < xIntersect * xZero->a) {
          // cout << indent << "    " << "only valid points are below left the
          // c-intersection, That is tighter than This\n";
          return false;
        }
      }
      // Else, If the slope of This line is lower than the slope of That line
      // (equivalent to b/a > that.b/that.a)
        ...
        // If the only valid points are above the x-intersection, That is
        // tighter than This
        if (zeroX && zeroX->level == constrKnown &&
            zeroX->c > (0 - yIntersect * zeroX->b)) {
          // cout << indent << "    " << "only valid points are above the
          // x-intersection, That is tighter than This\n";
          return false;
          // If the only valid points are below the x-intersection, This is
          // tighter than That
        } else if (xZero && xZero->level == constrKnown &&
                   xZero->c < yIntersect * xZero->a) {
          // cout << indent << "    " << "only valid points are below the
          // x-intersection, This is tighter than That\n";
          return true;
        }
    ...
    // If the slopes are not the same and we don't have enough information about
    // how x and y are related to the intersection of the lines
    return false;
  }
}

bool affineInequality::semLessThanEq(
    const affineInequality &that, bool xIsZeroVar,
    const affineInequality *xZero, const affineInequality *zeroX,
    bool yIsZeroVar, const affineInequality *yZero,
    const affineInequality *zeroY, string indent) const {
  /*ROSE_ASSERT(xIsZeroVar || (xZero && zeroX));
  ROSE_ASSERT(yIsZeroVar || (yZero && zeroY));*/
  ...
  if (!xIsZeroVar && !yIsZeroVar) {
    ...
    if (b * that.a == a * that.b) {
      ...
      // y*zyA <= ZERO + zyA     AND ZERO <= y*zyB' + zyC'
      double yIntersect = (that.c * a - c * that.a) / (b * that.a - that.b * a);
      cout << indent << "    yIntersect=" << yIntersect
           << " this slope=" << (b / a) << " that slope=" << (that.b / that.a)
           << "\n";
      if (zeroY)
        cout << indent << "    zeroY=" << zeroY->str("")
             << " yZero=" << yZero->str("") << "\n";
      ...
      if (b * that.a > a * that.b) {
        ...
        if (zeroY && zeroY->level == constrKnown &&
            zeroY->c >= (0 - yIntersect * zeroY->b)) {
          cout << indent << "    "
               << "only valid points are above the y-intersection, This is "
                  "tighter than That\n";
          return true;
          // If the only valid points are below the y-intersection, That is
          // tighter than This
                   ...
                   yZero->c <= yIntersect * yZero->a) {
          cout << indent << "    "
               << "only valid points are below the y-intersection, That is "
                  "tighter than This\n";
          return false;
        }
      }
      ...
      // (equivalent to b/a > that.b/that.a)
        ...
        if (zeroY && zeroY->level == constrKnown &&
            zeroY->c >= (0 - yIntersect * zeroY->b)) {
          cout << indent << "    "
               << "only valid points are above the y-intersection, That is "
                  "tighter than This\n";
          return false;
          // If the only valid points are below the y-intersection, This is
          // tighter than That
                   ...
                   yZero->c <= yIntersect * yZero->a) {
          cout << indent << "    "
               << "only valid points are below the y-intersection, This is "
                  "tighter than That\n";
          return true;
        }
      }
      ...
      // x*zxA <= ZERO + zxC     AND ZERO <= x*zxB' + zxC'
      double xIntersect = (that.c * b - c * that.b) / (b * that.a - that.b * a);
      cout << indent << "    xIntersect=" << xIntersect
           << " this slope=" << (b / a) << " that slope=" << (that.b / that.a)
           << "\n";
      if (zeroX)
        cout << indent << "    zeroX=" << zeroX->str("")
      ...
      if (b * that.a > a * that.b) {
        ...
        if (zeroX && zeroX->level == constrKnown &&
            zeroX->c >= (0 - xIntersect * zeroX->b)) {
          cout << indent << "    "
               << "only valid points are above the x-intersection, This is "
                  "tighter than That\n";
          return true;
          // If the only valid points are below left the c-intersection, That is
          // tighter than This
                   ...
                   xZero->c <= xIntersect * xZero->a) {
          cout << indent << "    "
               << "only valid points are below left the c-intersection, That "
      ...
      // (equivalent to b/a > that.b/that.a)
        ...
        if (zeroX && zeroX->level == constrKnown &&
            zeroX->c >= (0 - yIntersect * zeroX->b)) {
          cout << indent << "    "
               << "only valid points are above the x-intersection, That is "
                  "tighter than This\n";
          return false;
          // If the only valid points are below the x-intersection, This is
          // tighter than That
                   ...
                   xZero->c <= yIntersect * xZero->a) {
          cout << indent << "    "
               << "only valid points are below the x-intersection, This is "
                  "tighter than That\n";
          return true;
        }
      }
...
// Semantic affineInequality ordering (partial order), focused on negated
// inequalities Returns true if the negation of this affineInequality represents
// more information (less information is top, more information is bottom) than
// the nagation of that for all values of x and y and false otherwise
...
bool affineInequality::semLessThanNeg(const affineInequality &that,
                                      bool xEqZero, bool yEqZero) const {
  ROSE_ASSERT(xZero == that.xZero && yZero == that.yZero);
  ...
  // All top inequalities are incomparable since we their constraints are too
  // complex to be represented and thus unknown
  if (level == bottom || level == top /* || level == falseConstr*/)
    return false;
  ...
  if (!xZero && !yZero /* && xSign!=eqZero && ySign!=eqZero*/) {
    ...
    if (b * that.a == a * that.b) {
      // printf("affineInequality::operator<<  c=%d, that.c=%d\n", c, that.c);
      //  Neg(This) has more information than neg(that) if its' y-intercept is
      //  higher, since it leaves x with fewer valid points relative to y
      return c * that.a > that.c * a;
    }
    ...
    if((xSign==posZero && a*that.a>0) ||
            (xSign==negZero && a*that.a<0))
    {
            // If both lines have the same y-intercept, the line with the higher
    slope is more constrained,
            // since it leaves x with fewer valid points relative to y in the
    negated inequality return b*that.a > that.b*a;
    }*/
  } else if (xZero /* || xSign==eqZero*/) {
    // 0 <= y*b + c AND 0 <= y*b' + c'
    // -c <= y*b        -c' <= y*b'
    // If c == c', then the constraint with the larger b has more info, since
    // this negated inequality admits fewer valid values of x
    if (c == that.c)
      return b > that.b;
    // -c <= y*b AND -c' <= y*b'
    // The larger constant (smaller -c/-c') represents the tighter constraint in
    ...
    // c<that.c=%d\n", c == that.c, a<that.a, a==1 && that.a==1, c<that.c);
    //  x*a <= c AND x*a' <= c'
    //  If c == c', then the constraint with the larger a has more info, since
    //  the negated inequality admits fewer valid values of x
    if (c == that.c)
      return a > that.a;
    // x <= c AND x <= c'
    // The larger constant represents the tighter constraint in the negated
    // inequality
    else if (a == that.a)
      return c > that.c;
  }
...
bool affineInequality::set(const affineInequality &that) {
  bool modified = (this->a != that.a) || (this->b != that.b) ||
                  (this->c != that.c) || (this->xZero != that.xZero) ||
                  (this->yZero != that.yZero) || (this->xSign != that.xSign) ||
                  (this->ySign != that.ySign) || (this->level != that.level);
...
bool affineInequality::set(int a, int b, int c) {
  bool modified = (this->a != a) || (this->b != b) || (this->c != c) ||
                  (this->level != constrKnown);
  this->a = a;
  this->b = b;
  this->c = c;
...
bool affineInequality::set(int a, int b, int c, bool xZero, bool yZero,
                           signs xSign, signs ySign) {
  bool modified = (this->a != a) || (this->b != b) || (this->c != c) ||
                  (this->xZero != xZero) || (this->yZero != yZero) ||
                  (this->xSign != xSign) || (this->ySign != ySign) ||
                  (this->level != constrKnown);
  this->a = a;
  this->b = b;
  this->c = c;
...
// returns y, a, b or c
int affineInequality::getA() const { return a; }

int affineInequality::getB() const { return b; }

int affineInequality::getC() const { return c; }

short affineInequality::getLevel() const { return level; }

bool affineInequality::isXZero() const { return xZero; }

bool affineInequality::isYZero() const { return yZero; }

affineInequality::signs affineInequality::getXSign() const { return xSign; }

affineInequality::signs affineInequality::getYSign() const { return ySign; }
...
bool affineInequality::normalize() {
  ...
  if (xZero /* || xSign==eqZero*/) {
    a = 1;
    int gcd_bc = gcd(abs(b), abs(c));
    b = b / gcd_bc;
    c = c / gcd_bc;

    // if c=0 or -1, [0 <= y*b + c] implies [0 <= y + c], which is a stronger
    // constraint
    /*if(c==0 || c==-1)
            b=1;*/
  }
  // x*a <= 0 + c
    ...
    // if c=0 or c=-1, [x*a <= c] implies [x <= c], which is a stronger
    // constraint
    /*if(c==0 || c==-1)
            a=1;*/
  }
  ...
  /*      // Now consider the fact that x and y are integers. Thus, if
          ...
          // then [x*a <= y*b + c] implies [x*l*m <= y*l*n + l*p] == [x*m <= y*n
     + p] long gcd_ab = gcd(a,b); int div, rem; div = c/gcd_ab; rem = c%gcd_ab;
...
// Intersection = the affine inequality that contains the constraint information
// of both
...
bool affineInequality::intersectUpd(const affineInequality &that) {
  ...
  // if both levels are constrKnown
    ...
    // otherwise, the lower-bound cannot be represented as a line
      ...
      // setToTop();
      //  This lattice is conservative in the sense of containg no more
      //  information than is known to be true. As such, because we know that
      //  either this' or that's constraints are true, we'll leave this with its
      //  current constraints.
    }
  }
...
bool affineInequality::unionUpd(const affineInequality &that) {
  ...
  // If both levels are constrKnown
    ...
    // cout << "                Both inequalities are constrKnown.
    // b("<<b<<")*that.a("<<that.a<<")="<<(b*that.a)<<"
    // a("<<a<<")*that.b("<<that.b<<")="<<(a*that.b)<<"\n";
    ...
    // Otherwise, the upper-bound cannot be represented as a line
      ...
      //  approximation (doesn't contain more information that is defninitely
      //  true)
      modified = setToTop() || modified;
    }
  }
...
/*void affineInequality::operator^=(const affineInequality& that)
{
        // if this constraint is being widened to something that has more
information (higher in the lattice)
        // than it currently has, shift directly to top to avoid passing this
constraint through an infinite
        // number of lattice steps on its way to top
        if(*this << that)
                setToTop();
}*/

// returns true if the x-y constraint constrXY is consistent with the y-x
// constraint constrYX for some values of x and y and false otherwise. Since
// affineInequalities are conservative in the sense that they don't contain any
// more information than is definitely true, it may be that the true constraints
// are in fact inconsistent but we do not have enough information to prove this.
bool affineInequality::mayConsistent(const affineInequality &constrXY,
  ...
  if ((constrXY.getA() * constrYX.getA() - constrXY.getB() * constrYX.getB() ==
       0) &&
      (0 - constrYX.getC() * constrXY.getB()) >
    ...
    if (analysisDebugLevel >= 1) {
      ...
      printf("constrXY.getA()*constrYX.getA()-constrXY.getB()*constrYX.getB() "
             "= %d\n",
             (constrXY.getA() * constrYX.getA() -
              constrXY.getB() * constrYX.getB()));
...
string affineInequality::str(string indent) // const
{
  ...
  if (level == bottom)
    ...
    if (xSign == unknownSgn && ySign == unknownSgn)
      ...
      outs << "[aI: x*" << a << " <= y*" << b << " + " << c
           << "]"; //|"<<signToString(xSign)<<"|"<<signToString(ySign)<<">";
  else if (level == top)
...
/*// the basic logical operations that must be supported by any implementation
of
// a logical condition: NOT, AND and OR
