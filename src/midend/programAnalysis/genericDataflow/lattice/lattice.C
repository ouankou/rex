...
Lattice *BoolAndLattice::copy() const { return new BoolAndLattice(state); }
...
bool BoolAndLattice::meetUpdate(Lattice *that) {
  int newState = (state > dynamic_cast<BoolAndLattice *>(that)->state
                      ? state
                      : dynamic_cast<BoolAndLattice *>(that)->state);
...
{
        int newState = (state > (dynamic_cast<BoolAndLattice*>(that)->state ?
state : (dynamic_cast<BoolAndLattice*>(that)->state); return new
BoolAndLattice(newState);
}*/
...
// returns true if this causes the BoolAndLattice state to change, false
// otherwise
...
// sets the state of this lattice to the conjunction of the BoolAndLattice's
// current state and the given value returns true if this causes the
// BoolAndLattice state to change, false otherwise
...
// returns a copy of this lattice
Lattice *IntMaxLattice::copy() const { return new IntMaxLattice(state); }
...
bool IntMaxLattice::meetUpdate(Lattice *that) {
  int newState = (state > dynamic_cast<IntMaxLattice *>(that)->state
                      ? state
                      : dynamic_cast<IntMaxLattice *>(that)->state);
  bool ret = newState != state;
  state = newState;
  return ret;
}
...
{
        int newState = (state > (dynamic_cast<IntMaxLattice*>(that)->state ?
state : (dynamic_cast<IntMaxLattice*>(that)->state); return new
IntMaxLattice(newState);
}*/
...
bool IntMaxLattice::widenUpdate(InfiniteLattice *that) {
  if (state == -1) {
    ...
    int newState =
        (state == dynamic_cast<IntMaxLattice *>(that)->state ? state
                                                             : infinity);
    // printf("IntMaxLattice::widenUpdate() widening from %d to %d ==> \n",
...
// returns the current state of this object
int IntMaxLattice::get() const { return state; }
...
// computes the maximum of the given value and the state of this lattice and
// saves
...
ProductLattice::ProductLattice() { level = uninitialized; }
...
void ProductLattice::initialize() {
  ...
  if (level != initialized) {
    level = initialized;
    for (vector<Lattice *>::iterator it = lattices.begin();
         it != lattices.end(); it++) {
...
const vector<Lattice *> &ProductLattice::getLattices() { return lattices; }
...
bool ProductLattice::meetUpdate(Lattice *that_arg) {
  ...
  for (it = lattices.begin(), itThat = that->lattices.begin();
       it != lattices.end() && itThat != that->lattices.end(); it++, itThat++)
...
string ProductLattice::str(string indent) {
  ostringstream outs;
  outs << indent << "[ProductLattice: level="
       << (level == uninitialized ? "uninitialized" : "initialized") << "\n";
  for (vector<Lattice *>::const_iterator it = lattices.begin();
       it != lattices.end(); it++)
    outs << (*it)->str(indent + "    ") << "\n";
  outs << indent << "]\n";
...
bool InfiniteProductLattice::widenUpdate(InfiniteLattice *that) {
  bool modified = false;
  vector<Lattice *>::iterator it, itThat;
  for (it = lattices.begin(),
      itThat = (dynamic_cast<InfiniteProductLattice *>(that))->lattices.begin();
       it != lattices.end() &&
       itThat != (dynamic_cast<InfiniteProductLattice *>(that))->lattices.end();
       it++, itThat++) {
    /*Lattice* thisLat = *it;
    Lattice* thatLat = *itThat;*/
    modified = (dynamic_cast<InfiniteLattice *>(*it))
                   ->widenUpdate(dynamic_cast<InfiniteLattice *>(*itThat)) ||
               modified;
  }
  return modified;
}
...
// maps variables to the index of their respective Lattice objects in a given
// function
...
//     either used internally by this VariablesProductLattice object or
//     deallocated
// constVarLattices - map of additional variables and their associated lattices,
// that will be
//     incorporated into this VariablesProductLattice in addition to any other
...
VariablesProductLattice::VariablesProductLattice(
  ...
  func.get_name().str()); for(varIDSet::iterator it = refVars.begin(); it!=
  refVars.end(); it++) { printf("    %s\n", (*it).str().c_str()); }*/

  // iterate over all the variables (arrays and/or scalars) referenced in this
  // function adding their initial lattices to initState
  ...
  for(map<varID, Lattice*>::iterator it = constVarLattices.begin();
  it!=constVarLattices.end(); it++) lattices.push_back(it->second);*/
  // We don't add constVarLattices because they never change and thus, we don't
  // need to perform standard dataflow operations on them, such as meets and
  // widenings
...
// copy constructor
VariablesProductLattice::VariablesProductLattice(
    const VariablesProductLattice &that)
    : ProductLattice(), func(that.func) {
  ...
  // this->allVarLattice); printf("VariablesProductLattice() 2 that=%p,
  // that.allVarLattice=%p\n", &that, that.allVarLattice);
}
...
// returns the set of global variables(scalars and/or arrays)
varIDSet &VariablesProductLattice::getGlobalVars(bool includeScalars,
                                                 bool includeArrays) {
  if (includeScalars) {
    if (includeArrays)
      return varSets::getGlobalVars(cfgUtils::getProject());
...
Lattice *VariablesProductLattice::getVarLattice(const Function &func,
                                                const varID &var) {
  ...
  // else, if this is a constant variable
    ...
    /*printf("getVarLattice() var=%s -> const lattice %p, var==constIt->first =
    %d   < =%d, > = %d\n", var.str().c_str(), constIt->second,
    var==constIt->first, var<constIt->first, var>constIt->first); for(map<varID,
    Lattice*>::iterator it = constVarLattices.begin();
    it!=constVarLattices.end(); it++) { cout << "   constant
  ...
  // else, if this is a regular variable
    ...
    map<varID, int>::iterator it =
        (varLatticeIndex.find(func)->second).find(var);
    // printf("getVarLattice() it != (varLatticeIndex.find(func)->second).end()
    ...
    // otherwise, return NULL, since there is no lattice
      ...
      func.get_name().str(), var.str().c_str()); for(map<varID, int>::iterator
      it = (varLatticeIndex.find(func)->second).begin();
...
// or -1 otherwise
int VariablesProductLattice::getVarIndex(const Function &func,
                                         const varID &var) {
  map<varID, Lattice *>::iterator constIt;
  // if this is allVar
  if (var == allVar) {
...
void VariablesProductLattice::copy(Lattice *that_arg) {
  VariablesProductLattice *that =
      dynamic_cast<VariablesProductLattice *>(that_arg);
...
/*Lattice**/ void
VariablesProductLattice::remapVars(const map<varID, varID> &varNameMap,
                                   const Function &newFunc) {
  //      printf("remapVars(%s()), func=%s\n", newFunc.get_name().str(),
  //      func.get_name().str());

  // VariablesProductLattice* newVPL =
  // dynamic_cast<VariablesProductLattice*>(Lattice::copy());
  ...
  for (varIDSet::iterator it = newRefVars.begin(); it != newRefVars.end();
    ...
    if (!found) {
      ...
      cout << "      getVarIndex(newFunc, newVar)=" << getVarIndex(newFunc,
      newVar) << "\n";*/
...
void VariablesProductLattice::incorporateVars(Lattice *that_arg) {
  ...
  for (map<varID, int>::iterator it = varLatticeIndex[func].begin();
       it != varLatticeIndex[func].end(); it++) {
    ...
    if (that->lattices[varLatticeIndex[func][it->first]] != NULL) {
      ...
      lattices[varLatticeIndex[func][it->first]] =
          that->lattices[varLatticeIndex[func][it->first]]->copy();
    }
...
string VariablesProductLattice::str(string indent) {
  ...
  ostringstream outs;
  outs << indent << "[VariablesProductLattice: level="
  ...
  /*      vector<Lattice*>::const_iterator it = lattices.begin();
          while(it!=lattices.end())
          {
                  ...
                  it++;
                  // print the current variable, but skip the allVarLattice if
     we're doing it if(!allVarLattice || it!=lattices.end()) outs << indent << "
     " << it->first.str() << ": " << (*curIt)->str("") << "\n";
          }*/
  if (allVarLattice)
