 ...
 * exprLeaf *
 ************/
IntArithLogical::exprLeaf::exprLeaf(SpearOp cmp, int a, varID x, int b, varID y,
                                    int c)
    : cmp(cmp) {
  this->a = a;
  this->x = x;
...
IntArithLogical::exprLeaf::exprLeaf(cmpOps cmp, int a, varID x, int b, varID y,
                                    int c)
    : cmp(cmp) {
  this->a = a;
  this->x = x;
...
// divide out from a, b and c any common factors, reducing the triple to its
// normal form return true if this modifies this constraint and false otherwise
bool IntArithLogical::exprLeaf::normalize() {
  ...
  if (c == 0) {
    ...
    modified = (a == a / gcd_ab) && (b == b / gcd_ab);
    ...
    modified = (a == a / gcd_abc) && (b == b / gcd_abc) && (c == c / gcd_abc);
...
void IntArithLogical::exprLeaf::computeVarsExpr() {
  if (!varsExprInitialized) {
    ...
    // if vars contains SpearVars from the lass call to computeVarsExpr, delete
    // these SpearVars
    for (list<SpearAbstractVar *>::iterator it = vars.begin(); it != vars.end();
         it++) {
      if (outVar == *it)
        outVarInVars = true;
      delete *it;
    ...
    vars.clear();
    // if outVar was not in vars (i.e. its a constant set when level==itTrue or
    // level==isFalse), delete it
    if (outVarInVars)
      delete outVar;
    ...
    if (level == isTrue) {
      ...
      ostringstream ss;
      ss << "c " << aMxSVar->getName() << " "
         << SpearOp::opStr(SpearOp::Multiply) << " " << xSVar->getName() << " "
         << a << ":" << SpearType::typeName(32) << "\n";
      ss << "c " << bMySVar->getName() << " "
         << SpearOp::opStr(SpearOp::Multiply) << " " << ySVar->getName() << " "
         << b << ":" << SpearType::typeName(32) << "\n";
      ss << "c " << bMyPcSVar->getName() << " " << SpearOp::opStr(SpearOp::Add)
         << " " << bMySVar->getName() << " " << c << ":"
         << SpearType::typeName(32) << "\n";
      ss << "c " << relSVar->getName() << " " << cmp.opStr() << " "
         << aMxSVar->getName() << " " << bMyPcSVar->getName() << "\n";
      logExpr = ss.str();
...
IntArithLogical::exprLeafOrNode::infContent
IntArithLogical::exprLeaf::getLevel() {
  return level;
}

// returns a list of the names of all the variables needed to represent the
// logical expression in this object
...
// returns the SPEAR Format expression that represents the constraint in this
// object
...
// multi-line string.
string IntArithLogical::exprLeaf::str(string indent) { return genStr(); }

string IntArithLogical::exprLeaf::str(string indent) const { return genStr(); }
...
SpearExpr *IntArithLogical::exprLeaf::copy() { return new exprLeaf(*this); }
...
// Return true if the conjunction is possible and this now contains the
// conjunction expression and
//        false if it is not possible.
// If the conjunction is possible, modified is set to true if the conjunction
// causes
...
bool IntArithLogical::exprLeaf::andExprs(const exprLeaf &that, bool &modified) {
  ...
  if (andExprsTest(that)) {
    ...
    if (cmp.isOp(SpearOp::Equal)) {
      ...
      // The conjunction of two different not-equality expressions can be
      // conservatively represented (i.e. without introducing new information)
      // as either of the not-equality expressions.
    } else if (cmp.isOp(SpearOp::SgnLTE)) {
      // a*x <= b*y + c AND a'*x <= b'*y + c'
      // if a/b==a'/b' (a*b' == a'*b, AKA same slope), the AND is the tighter
      // affine inequality: the one with the smaller x-intercept
      ...
      // Otherwise, the two lines cross each other, meaning that either line
      // conservatively approximates the set of impossible points (x,y) in that
      // neither line accepts rejects any point accepted by the conjunction.
      // Thus, we leave this alone
    } else if (cmp.isOp(SpearOp::SgnGT)) {
      // a*x > b*y + c AND a'*x > b'*y + c'
      // if a/b==a'/b' (a*b' == a'*b, AKA same slope), the AND is the tighter
      // affine inequality: the one with the larger x-intercept
      if (a * that.b == b * that.a) {
        // x-intercept = c/a, c'/a'.
        //     c/a <= c'/a' == c*a' <= c'*a
...
// returns the same thing as andExprs but doesn't actually perform the
// conjunction
...
// Return true if the disjunction is possible and this now contains the
// disjunction expression and
//        false if it is not possible.
// If the conjunction is possible, modified is set to true if the conjunction
// causes
//    this to change and false otherwise.
bool IntArithLogical::exprLeaf::orExprs(const exprLeaf &that, bool &modified) {
  ...
  if (orExprsTest(that)) {
    ...
    if (cmp.isOp(SpearOp::Equal)) {
      // if the two lines never intersect (same slope, different y-intercept)
      if (a * that.b == that.a * b && c != that.c) {
        // The two lines can be represented as an inequality relation that
        // includes in its acceptance set all the points on both lines. Thus, we
        // simply need to use the a, b and c of the line with the larger
        // x-intercept.
      ...
      // The disjunction of two different equality expressions can only be
      // conservatively represented as True
    } else if (cmp.isOp(SpearOp::NotEqual)) {
      // the disjunction of two different not-equality expressions is True (i.e.
      // all points (x,y) are accepted)
      modified = setToTrue() || modified;
    } else if (cmp.isOp(SpearOp::SgnLTE)) {
      // a*x <= b*y + c OR a'*x <= b'*y + c'
      // if a/b==a'/b' (a*b' == a'*b, AKA same slope), the OR is the looser
      // affine inequality: the one with the larger x-intercept
      if (a * that.b == b * that.a) {
        // x-intercept = c/a, c'/a'.
        //     c/a <= c'/a' == c*a' <= c'*a
      ...
      // Otherwise, the two lines cross each other, meaning that the only
      // conservative affine relationship that covers both is one where c =
      // infinity == True
      else
        modified = setToTrue() || modified;
    } else if (cmp.isOp(SpearOp::SgnGT)) {
      // a*x > b*y + c AND a'*x > b'*y + c'
      // if a/b==a'/b' (a*b' == a'*b, AKA same slope), the AND is the looser
      // affine inequality: the one with the smaller x-intercept
      if (a * that.b == b * that.a) {
        // x-intercept = c/a, c'/a'.
        //     c/a <= c'/a' == c*a' <= c'*a
...
// returns the same thing as orExprs but doesn't actually perform the
// disjunction
...
bool IntArithLogical::exprLeaf::operator==(exprLeafOrNode &that_arg) {
  ...
  return a == that.a && b == that.b && c == that.c && x == that.x &&
         y == that.y && cmp == that.cmp && level == that.level;
}

// Information order on affine relationships with the least information (True)
// being highest and most information (False) being lowest. Some expressions are
// not comparable since their sets of accepted points are not strictly related
// to each other.
bool IntArithLogical::exprLeaf::operator<=(exprLeafOrNode &that_arg) {
  ROSE_ASSERT(that_arg.elType() == IntArithLogical::exprLeafOrNode::eLeaf);
  exprLeaf &that = dynamic_cast<exprLeaf &>(that_arg);
  ...
  if (that.level == isTrue)
    return true;
  if (level == isTrue)
    return false;
  if (that.level == isFalse)
    return false;
  if (level == isFalse)
    return true;
  ...
  // (this vs that) LTE vs LTE, Eq vs LTE, LTE vs Eq, GT vs GT, Eq vs GT, GT vs
  // Eq, where the slopes are the same The comparable cases where this may
  // contain the the same amount or more information than that: (this vs that)
  // LTE vs LTE, Eq vs LTE,            GT vs GT, Eq vs GT,           where the
  // slopes are the same The comparable cases that are also non-trivially
  // handled by andExprs and orExprs: (this vs that) LTE vs LTE, GT vs GT, where
  // the slopes are the same if the slopes are the same
  if (a * that.b == that.a * b) {
    ...
    /*else if(cmp.isOp(SpearOp::Eq)
    {
            if(that.cmp.isOp(SpearOp::SgnLTE))
            {
                    ...
                    // if this represents a line with an x-intercept <= that's
    x-intercept return c*that.a <= that.c*a;
            }
 ...
 * logicNode *
 *************/
// An internal node that represents a logical connective between the affine
// relations
...
IntArithLogical::logicNode::logicNode(logOps logOp) : logOp(logOp) {
  ...
  /*      ROSE_ASSERT((level == exprKnown && children.size()>0) ||
                      ((level == isTrue || level == isFalse) && children.size()
     == 0));*/
}

// constructor for a logicNode with a single child
IntArithLogical::logicNode::logicNode(logOps logOp, exprLeafOrNode *child)
    : logOp(logOp) {
...
IntArithLogical::logicNode::logicNode(logOps logOp, exprLeafOrNode *childA,
                                      exprLeafOrNode *childB)
    : logOp(logOp) {
  // ROSE_ASSERT(logOp==SpearOp::AndOp || logOp==SpearOp::OrOp/* ||
  // logOp==SpearOp::XorOp*/);
...
IntArithLogical::logicNode::logicNode(logOps logOp,
                                      const list<exprLeafOrNode *> &children)
    : logOp(logOp) {
  ROSE_ASSERT(logOp == SpearOp::AndOp ||
              logOp == SpearOp::OrOp /* || logOp==SpearOp::XorOp*/);
...
IntArithLogical::logicNode::logicNode(const logicNode &that)
  ...
  for (list<exprLeafOrNode *>::const_iterator it = that.children.begin();
       it != that.children.end(); it++)
    children.push_back(dynamic_cast<exprLeafOrNode *>((*it)->copy()));
  varsExprInitialized = false;
  outVar = NULL;
  logExpr = "";

  // printf("level=%d that.level=%d logOp=%d that.logOp=%d
  // children.size()=%d\n", level, that.level, logOp.getOp(),
  // that.logOp.getOp(), children.size());
  ROSE_ASSERT((level == exprKnown && children.size() > 0) ||
              ((level == isTrue || level == isFalse) && children.size() == 0));
}
...
IntArithLogical::logicNode::~logicNode() {
  ...
  for (list<exprLeafOrNode *>::iterator it = children.begin();
       it != children.end(); it++)
    delete *it;
}
...
void IntArithLogical::logicNode::addChild(exprLeafOrNode *child) {
  if (level == exprKnown || (level == isTrue && logOp.isOp(SpearOp::AndOp)) ||
...
// propagates all the true and false sub-terms upwards through the tree, pruning
// the appropriate portions returns true if this causes the logicNode to change
// and false otherwise
  ...
  if (level == isTrue)
    ...
    for (list<exprLeafOrNode *>::iterator it = children.begin();
      ...
      if (logOp.isOp(SpearOp::OrOp)) {
        // first, normalize the current child conjunct so that we can know
        // whether this child is true or false
        ROSE_ASSERT((*it)->elType() == IntArithLogical::exprLeafOrNode::lNode);
        logicNode *ln = dynamic_cast<logicNode *>(*it);

        ln->normalize();
      } else
        ROSE_ASSERT(logOp.isOp(SpearOp::AndOp));
    ...
    ROSE_ASSERT(
        (level == exprKnown && children.size() > 0) ||
        ((level == isTrue || level == isFalse) && children.size() == 0));
...
IntArithLogical::exprLeafOrNode::infContent
IntArithLogical::logicNode::getLevel() {
  return level;
}
...
// The last character of the returned string should not be '\n', even if it is a
// multi-line string.
string IntArithLogical::logicNode::str(string indent) { return genStr(); }

string IntArithLogical::logicNode::str(string indent) const { return genStr(); }
...
string IntArithLogical::logicNode::genStr(string indent) const {
  ...
  if (level == exprKnown) {
    ...
    for (list<exprLeafOrNode *>::const_iterator it = children.begin();
         it != children.end();) {
...
// Aggregates the conjunction or disjunction of the results of a number of
// expressions. Specifically, we will use a binary reduction tree, where n
// expressions will be reduced as (in the idealized case of n=2**k: l{k}_{1} cmp
// e{1} e{2} l{k}_{2} cmp e{3} e{4}
// ...
...
// op [IN] - Spear operation we'll use to aggregate individual expression
// results ss [OUT] - the stringstream that we'll be writing our final result to
// baseRedVarName [IN] - prefix of all the reduction variable names
// newVars [OUT] - all the new vars generated as part of the reduction tree will
// be added to newVars inputVars [IN] - list of variables to be reduced uid [IN]
// - the unique ID to be assigned to all the variables generated inside this
// call or -1 if no unique id is to be used returns a pointer to the variable
// that represents the entire reduction tree
SpearAbstractVar *genReductionTree(SpearOp op, ostream &ss,
  ...
  // list of variables to be reduced in the current level of the reduction
  // hierarchy
  list<SpearAbstractVar *> redVars = inputVars;

  // Iterate through the reduction tree, printing out expressions and adding the
  // reduction variables to vars
  int curLevel = log2N;
  while (redVars.size() > 1) {
    int varIndex = 0;
    // The reduction variables for this level. Will become redVars for the level
    // above it.
    list<SpearAbstractVar *> newRedVars;
    for (list<SpearAbstractVar *>::iterator it = redVars.begin();
         it != redVars.end(); it++, varIndex++) {
    ...
    // if the number of variables at this level is odd, the last reduction
    // variable reduces only one variable, so its second argument must be a noop
    // constant
    if (varIndex % 2 == 1) {
...
void IntArithLogical::logicNode::computeVarsExpr() {
  if (!varsExprInitialized) {
    ...
    bool outVarInVars = false;
    // if vars contains SpearVars from the last call to computeVarsExpr, delete
    // these SpearVars
    for (list<SpearAbstractVar *>::iterator it = vars.begin(); it != vars.end();
         it++) {
      if (outVar == *it)
    ...
    if (level == isTrue) {
      ...
      for (list<exprLeafOrNode *>::iterator it = children.begin();
        ...
        for (list<SpearAbstractVar *>::const_iterator itSV = sv.begin();
             itSV != sv.end(); itSV++) {
          vars.push_back(*itSV);
        }
      }
      ...
      // We'll need to aggregate the conjunction or disjunction of the results
      // of all the children's expressions. Specifically, we will use a binary
      // reduction tree, using the function genReductionTree()
      string baseRedVarName = "";
      if (logOp.isOp(SpearOp::AndOp))
      ...
      outVar = genReductionTree(logOp, ss, baseRedVarName, vars, inputVars,
                                getUID());
...
bool IntArithLogical::logicNode::notUpd() {
  if (level == exprKnown) {
    if (logOp.isOp(SpearOp::AndOp)) {
      ...
      // to conjunctive normal form. We need to reorganize it to get it back in
      // form.
...
// given a 2-level expression tree, collects all the permutations of
// grandchildren from each child branch into conjunct logicNodes and saves them
// in newChildren. Thus, (A^B^C)v(D^E^F) would become the list
// (A^D),(A^E),(A^F),(B^D),(B^E),(B^F),(C^D),(C^E),(C^F). The function works
// recursively, one child at a time. newChildren: The list where we'll save the
// new conjuncts newConjOrig: Accumulated stack of grandchildren that will be
// combined to form each conjunct
//              Thus, for input (A^B)v(C^D)v(D^E)v(F^G)v(H^I), the contents of
//              newConjOrig may be A,C,E,F, with H or I to be added when the
//              function recurses again.
// curChild: Iterator that refers to the current child we're processing. The
// conjuncts
//           are formed whenever curChild reaches the end of children.
void IntArithLogical::logicNode::genChildrenConj(
    list<exprLeafOrNode *> &newChildren, list<exprLeafOrNode *> newConjOrig,
    list<exprLeafOrNode *>::const_iterator curChild) {
  // if newConj contains pointers to all the leaves that will make up this new
  // conjunct
  if (curChild == children.end()) {
    // create a set of new children that corresponds to this conjunct
    logicNode *conj = new logicNode(andOp);
    for (list<exprLeafOrNode *>::iterator it = newConjOrig.begin();
         it != newConjOrig.end(); it++) {
  ...
  // else, if we're only partly through the children list
    ...
    ROSE_ASSERT((*curChild)->elType() ==
                IntArithLogical::exprLeafOrNode::lNode);
    logicNode *ln = dynamic_cast<logicNode *>(*curChild);
    // advance the child iterator so that for all the recursive calls in this
    // function it refers to the next child
    curChild++;
    for (list<exprLeafOrNode *>::iterator it = ln->children.begin();
         it != ln->children.end(); it++) {
...
bool IntArithLogical::logicNode::andUpd(logicNode &that) {
  ...
  // that.logOp.getOp()); printf("andUpd() level=%d that.level=%d\n", level,
  // that.level);
  ...
  if (level == exprKnown && that.level == exprKnown) {
    ...
    if (logOp.isOp(SpearOp::AndOp)) {
      ...
      for (list<exprLeafOrNode *>::iterator itThat = that.children.begin();
           itThat != that.children.end(); itThat++) {
        bool mergeable = false;
        ROSE_ASSERT((*itThat)->elType() ==
                    IntArithLogical::exprLeafOrNode::eLeaf);
        exprLeaf *thatEL = dynamic_cast<exprLeaf *>(*itThat);

        // first, check if this new child can be merged with this' existing
        // children
        for (list<exprLeafOrNode *>::iterator itThis = children.begin();
             itThis != children.end(); itThis++) {
          ROSE_ASSERT((*itThis)->elType() ==
                      IntArithLogical::exprLeafOrNode::eLeaf);
      ...
      for (list<exprLeafOrNode *>::iterator itThis = children.begin();
        ...
        for (list<exprLeafOrNode *>::iterator itThat = that.children.begin();
          ...
          // thisC->logOp.getOp(), thatC->logOp.getOp(),
          // newTerm->logOp.getOp()); cout << "andUpd::Or recursive call to
          // andUpd::And\n";
          newTerm->andUpd(*thatC);
          // cout << "    newTerm = "<<newTerm->str("                  ")<<"\n";

          // insert newTerm into newChildren, taking into account any
          // redundancies relative to previously inserted terms
          insertNewChildOr(newChildren, newTerm);
        }
      }

      // this object has been modified iff the new children list differs from
      // the old children list
  ...
  // the cases where this must become a copy of that
    ...
    for (list<exprLeafOrNode *>::iterator it = that.children.begin();
         it != that.children.end(); it++)
      children.push_back(dynamic_cast<exprLeafOrNode *>((*it)->copy()));
    level = exprKnown;
  } else
  ...
  else if(logOp.isOp(SpearOp::OrOp))
  {
          ...
          {
                  ROSE_ASSERT(that.children.size()>0);
                  for(list<exprLeafOrNode*>::iterator it =
  that.children.begin(); it != that.children.end(); it++)
                          children.push_back(dynamic_cast<exprLeafOrNode*>((*it)->copy()));
                  level = exprKnown;
          }
          else
...
bool IntArithLogical::logicNode::orUpd(logicNode &that) {
  ...
  if (level == exprKnown && that.level == exprKnown) {
    if (logOp.isOp(SpearOp::AndOp)) {
      ...
      for (list<exprLeafOrNode *>::iterator it = that.children.begin();
           it != that.children.end(); it++) {
        // insert newTerm into newChildren, taking into account any redundancies
        // relative to previously inserted terms
        insertNewChildOr(newChildren,
                         dynamic_cast<exprLeafOrNode *>((*it)->copy()));
...
// with other facts that do not involve the variable but could be inferred
// through the removed facts. (i.e. if we have x<y ^ y<z and wish to remove y,
// the removed expressions may be replaced with x<z or just True) Returns true
// if this causes the logicNode object to change and false otherwise.
bool IntArithLogical::logicNode::removeVar(varID var) {
  ...
  if (level == exprKnown) {
    if (logOp.isOp(SpearOp::AndOp)) {
      ...
      // var to some other variable. Thus, for each pair (x rel var), (y rel
      // var) we will check whether we can derive a replacement expression (x
      // rel y).
      for (list<list<exprLeafOrNode *>::iterator>::iterator it1 =
               taintedExprs.begin();
           it1 != taintedExprs.end(); it1++) {
        ROSE_ASSERT((**it1)->elType() ==
                    IntArithLogical::exprLeafOrNode::eLeaf);
        exprLeaf *one = dynamic_cast<exprLeaf *>(**it1);

        for (list<list<exprLeafOrNode *>::iterator>::iterator it2 =
                 taintedExprs.begin();
             it2 != taintedExprs.end(); it2++) {
          ROSE_ASSERT((**it2)->elType() ==
                      IntArithLogical::exprLeafOrNode::eLeaf);
          ...
          if (it1 != it2) {
            ...
            if (one->cmp.isOp(SpearOp::SgnLTE) &&
                two->cmp.isOp(SpearOp::SgnGT)) {
              if (((one->x != var && one->y == var) ||
                   (one->x == var && one->y != var)) &&
                  two->x == one->x && two->y == one->y) {
                // This conjunction is now false and we're done
                // First, delete all the newly-inferred expressions
                for (list<exprLeaf *>::iterator it = inferredExprs.begin();
                     it != inferredExprs.end(); it++) {
                  delete *it;
                }
            ...
            // Case 2c: a*var == b*x+c      ^ a'*var == b'*y+c' [Equivalent to
            // Case 2a for two ^ one combination] Case 2d: a*var == b*x+c      ^
            // a'*y == b'*var+c' [Equivalent to Case 2b for two ^ one
            // combination] Case 2w: a*x <=/>/!= b*var+c ^ a'*var == b'*y+c' [
            // the flip case two ^ one ] Case 2x: a*x <=/>/!= b*var+c ^ a'*y ==
            // b'*var+c' [ is handled when that    ] Case 2y: a*var <=/>/!=
            // b*x+c ^ a'*var == b'*y+c' [ combination comes up in ] Case 2z:
            // a*var <=/>/!= b*x+c ^ a'*y == b'*var+c' [ this inner loop ] Case
            // 3:  a*x > b*var+c       ^ a'*var > b'*y+c' Case 4a: a*x !=
            // b*var+c      ^ a'*var != b'*y+c Case 4b: a*x != b*var+c      ^
            // a'*y != b'*var+c Case 4c: a*var != b*x+c      ^ a'*var != b'*y+c
            // [Equivalent to Case 4a for two ^ one combination] Case 4d: a*var
            // != b*x+c      ^ a'*y != b'*var+c  [Equivalent to Case 4b for two
            // ^ one combination]

            // Thus, we provide code for cases 1, 2a, 2b, 2w, 2x, 2y, 2z, 3, 4a,
            // 4b
            ...
            else if ((one->x != var && one->y == var && one->y == two->x &&
                      two->y != var) &&
            ...
            // Cases 2b, 2x, 2y, 2z
              ...
              if ((one->x != var && one->y == var && one->y == two->y &&
                   two->x != var)) {
            ...
            // if we were able to infer something, add it to the list of
            // inferred expressions
            if (inferred)
              inferredExprs.push_back(inferred);
          }
        }
      }
      ...
      for (list<list<exprLeafOrNode *>::iterator>::iterator it =
               taintedExprs.begin();
      ...
      // 2. Insert all the expressions that were inferred via var
      // (inferredExprs)
      for (list<exprLeaf *>::iterator it = inferredExprs.begin();
           it != inferredExprs.end(); it++)
        children.push_back(*it);

      // if we've added anything, normalize this conjunction to remove any
      // redundancies
      if (inferredExprs.size() > 0)
        normalize();

      // This logicNode was modified iff it contained any conjuncts that used
      // var If it did, these conjuncts got removed and the other logic above
      // definitely
      //    did not bring them back, so the logicNode was definitely modified.
      // If it didn't, no conjuncts got removed, inferredExprs is empty and
      // normalize
      //    was not called.
...
// Removes all instances of p*var with (q*var + r) and adjusts all relations
// that involve var accordingly. Returns true if this causes the logicNode
...
bool IntArithLogical::logicNode::replaceVar(varID var, int p, int q, int r) {
  ...
  if (level == exprKnown) {
    if (logOp.isOp(SpearOp::AndOp)) {
      ...
      // iterate over this disjunction's children, telling each child to perform
...
// generic function used by orUpd and andUpd to insert new term into a
// disjunction newChildren - The new list of children that will eventually
// replace children. We will be trying to
//               insert newTerm into newChildren.
void IntArithLogical::logicNode::insertNewChildOr(
    list<exprLeafOrNode *> &newChildren, exprLeafOrNode *newTerm) {
  ...
  //    the case that A <= newConj <= B but not A<=B, where are already in
  //    newChildren. This may happen due to inaccuracies in the <= operator or
  //    our formula representation.
  bool addNewTerm = true;
  list<list<exprLeafOrNode *>::iterator> newToDelete;
  for (list<exprLeafOrNode *>::iterator it = newChildren.begin();
       it != newChildren.end(); it++) {
    /*cout << "insertNewChildOr() *it = "<<(*it)->str()<<"\n";
    cout << "      newTerm="<<newTerm->str()<<", <= "<<(*newTerm <= *(*it))<<",
    >= "<<(*(*it) <= *newTerm)<<"\n";*/
    if (*newTerm <= *(*it))
      addNewTerm = false;
    else if (*(*it) <= *newTerm)
      newToDelete.push_back(it);
  }
  // add the newly-created term as another child of this
  if (addNewTerm)
    newChildren.push_back(newTerm);
  // delete any newly-added terms that had more information than newConj and
  // remove them from newChildren
  for (list<list<exprLeafOrNode *>::iterator>::iterator it =
           newToDelete.begin();
       it != newToDelete.end(); it++) {
    delete *(*it);
    newChildren.erase(*it);
  }
}

// compares two different children lists, returning true if they're equal and
// false otherwise
bool IntArithLogical::logicNode::eqChildren(list<exprLeafOrNode *> &one,
                                            list<exprLeafOrNode *> &two) {
  ...
  if (one.size() == two.size()) {
    ...
    for (list<exprLeafOrNode *>::iterator itNew = two.begin();
         itNew != two.end(); itNew++) {
    ...
    // if we didn't match all the members of one to all the members of two, this
...
bool IntArithLogical::logicNode::operator==(exprLeafOrNode &that_arg) {
  ...
  if (logOp == that.logOp && level == that.level &&
      children.size() == that.children.size()) {
    list<exprLeafOrNode *>::iterator itThis, itThat;
    for (itThis = children.begin(), itThat = that.children.begin();
         itThis != children.end(); itThis++, itThat++) {
      if (!(*(*itThis) == *(*itThat))) {
        return false;
      }
    }
...
bool IntArithLogical::logicNode::operator<=(exprLeafOrNode &that_arg) {
  ...
  if (logOp.isOp(SpearOp::AndOp)) {
    // this <= that iff this contains the same amount or more information than
    // that, iff for every element of that there exist one or more elements of
    // this s.t. thisEL <= thatEL (thisEL contains same or more info than
    // thatEL) the simplest example of this is (A^B1^B2^C^D^...^Z) <= (A^B3^C),
    // where B1<=B3 and B2<=B3
    for (list<exprLeafOrNode *>::iterator itThat = that.children.begin();
         itThat != that.children.end(); itThat++) {
      ROSE_ASSERT((*itThat)->elType() ==
                  IntArithLogical::exprLeafOrNode::eLeaf);
      ...
      // if we couldn't find an element of this <= thatEL, then we cannot prove:
      // this <= that
      if (!foundLE)
        return false;
    }
    // since we got here without finding counter-examples to the assertion this
    // <= that, it must be true
    return true;
  } else if (logOp.isOp(SpearOp::OrOp)) {
    // this <= that iff this contains the same amount or more information than
    // that, iff for every element of this there exist one or more elements of
    // that s.t. thisEL <= thatEL (thisEL contains same or more info than
    // thatEL) the simplest example of this is (A1vA2vA3vB1vB2) <=
    // (AvBvCv...vZ), where Ai<=A and Bi<=B
    for (list<exprLeafOrNode *>::iterator itThis = children.begin();
         itThis != children.end(); itThis++) {
      ROSE_ASSERT((*itThis)->elType() ==
                  IntArithLogical::exprLeafOrNode::eLeaf);
      ...
      // if we couldn't find an element of that >= thisEL, then we cannot prove:
      // this <= that
      if (!foundLE)
        return false;
    }
...
// Creates a logical expression that is either True or False, depending on the
// value argument
...
IntArithLogical::IntArithLogical(cmpOps cmp, int a, varID x, int b, varID y,
                                 int c) {
  level = known;
  expr = new logicNode(orOp,
                       new logicNode(andOp, new exprLeaf(cmp, a, x, b, y, c)));
...
// initializes this Lattice to its default bottom state if it has not yet been
// initialized if the given new level is higher than bottom, expr is also
...
// initializes this Lattice to its default state, with a specific value (true or
// false)
...
Lattice *IntArithLogical::copy() const { return new IntArithLogical(*this); }
...
bool IntArithLogical::operator==(Lattice *that_arg) {
  ...
  return this->level == that->level && this->expr == that->expr;
}
...
// Returns true if this causes the IntArithLogical object to change and false
// otherwise.
...
// through the removed facts. (i.e. if we have x<y ^ y<z and wish to remove y,
// the removed expressions may be replaced with x<z or just True) Returns true
// if this causes the IntArithLogical object to change and false otherwise.
...
LogicalCond *IntArithLogical::copy() { return new IntArithLogical(*this); }

bool SAV_LT(SpearAbstractVar *v1, SpearAbstractVar *v2) { return *v1 < *v2; }

bool SAV_EQ(SpearAbstractVar *v1, SpearAbstractVar *v2) { return *v1 == *v2; }

// Writes the full expression that corresponds to this object, including any
// required declarations and range constraints to os. Returns that variable that
// summarizes this expression. otherVars - list of variables that also need to
// be declared and ranged createProposition - if true, outputSpearExpr() creates
// a self-contained proposition. If false, no
//       proposition is created; it is presumed that the caller will be using
//       the expression as part of a larger proposition.
SpearVar IntArithLogical::outputSpearExpr(exprLeaf *otherExpr, ofstream &os,
                                          bool createProposition) {
  const list<SpearAbstractVar *> *otherVars = NULL;
  if (otherExpr)
    otherVars = &(otherExpr->getVars());
  ...
  if (otherExpr) {
    ...
    for (list<SpearAbstractVar *>::const_iterator it = otherVars->begin();
         it != otherVars->end(); it++)
      vars.push_back(*it);
  }
  ...
  // Generate the variables that define the ranges of all the non-binary
  // variables
  list<SpearAbstractVar *> rangeVars;
  for (list<SpearAbstractVar *>::const_iterator it = vars.begin();
       it != vars.end(); it++) {
  ...
  ostringstream redSS;
  SpearAbstractVar *rangesVar =
      genReductionTree(SpearOp::AndOp, redSS, "Ranges", vars, rangeVars, -1);
  ...
  for (list<SpearAbstractVar *>::const_iterator it = vars.begin();
    ...
    if ((*it)->getType().numBits() > 1) {
      ...
      SpearVar minRVar((*it)->getName() + "_Min", 1);
      SpearVar rangeVar((*it)->getName() + "_Range", 1);
      // os << "d " << maxRVar.varDecl() << " " << minRVar.varDecl() << " " <<
      // rangeVar->varDecl() << " " << "\n";
      os << "c " << minBound.getName() << " "
         << SpearOp::opStr(SpearOp::Subtract) << " 0:i32 " << maxRange << ":"
         << minRConst.getType().typeName() << "\n";
      os << "c " << maxRVar.getName() << " " << SpearOp::opStr(SpearOp::SgnLTE)
         << " " << (*it)->getName() << " " << maxRange << ":"
         << maxRConst.getType().typeName() << "\n";
      os << "c " << minRVar.getName() << " " << SpearOp::opStr(SpearOp::SgnGTE)
         << " " << (*it)->getName() << " " << minBound.getName() << "\n";
      os << "c " << rangeVar.getName() << " " << SpearOp::opStr(SpearOp::AndOp)
         << " " << (*it)->getName() << "_Min" << " " << (*it)->getName()
         << "_Max" << "\n";
    }
  }
  ...
  for (list<SpearAbstractVar *>::iterator it = rangeVars.begin();
       it != rangeVars.end(); it++)
    delete *it;
  for (list<SpearAbstractVar *>::iterator it = reductionVars.begin();
       it != reductionVars.end(); it++)
    delete *it;

  // Generate the formulas
  if (otherExpr)
    os << otherExpr->getExpr();

  SpearAbstractVar *outVar = expr->getOutVar();
  // printf("outVar = %p\n", outVar);
  if (createProposition) {
    os << "p " << SpearOp::opStr(SpearOp::AndOp) << " " << outVar->getName()
       << " " << rangesVar->getName() << "\n";
  } else {
    os << "c " << summaryVar.getName() << " " << SpearOp::opStr(SpearOp::AndOp)
       << " " << outVar->getName() << " " << rangesVar->getName() << "\n";
  }

  return summaryVar;
}

// Runs Spear on the given input file. Returns true if the file's conditions are
...
// Queries whether the given affine relation is implied by this
// arithmetic/logical constrains. Returns true if yes and false otherwise
bool IntArithLogical::isImplied(cmpOps cmp, int a, varID x, int b, varID y,
                                int c) {
  if (level == uninitialized)
    return false;
  ...
  // precondVar summarizes this expression and is the precondition for the
  // implication created by this function
  SpearVar precondVar = outputSpearExpr(&tgtExpr, spearInput, false);

  spearInput << "c " << finalVar.getName() << " "
             << SpearOp::opStr(SpearOp::Implies) << " " << precondVar.getName()
             << " " << tgtOutVar->getName() << "\n";
...
// Queries whether the arithmetic/logical constrains may be consistent with the
// given affine relation. Returns true if yes and false otherwise
bool IntArithLogical::mayConsistent(cmpOps cmp, int a, varID x, int b, varID y,
                                    int c) {
  if (level == uninitialized)
    return true;
  else if (expr->getLevel() == exprLeafOrNode::isFalse)
  ...
  spearInput << "c " << finalVar.getName() << " "
             << SpearOp::opStr(SpearOp::AndOp) << " " << precondVar.getName()
             << " " << tgtOutVar->getName() << "\n";
  spearInput << "p " << SpearOp::opStr(SpearOp::NotOp) << " final\n";
...
// Updates the expression with the information that x*a has been assigned to
// y*b+c returns true if this causes the expression to change and false
// otherwise
bool IntArithLogical::assign(int a, varID x, int b, varID y, int c) {
  ...
  if (x != y) {
    ...
    // First, remove any mappings to x in anticipation of it's old value
    // dissapearing
    removeVar(x);
    ...
    logicNode *newEq = new logicNode(
        orOp,
        new logicNode(andOp, new exprLeaf(IntArithLogical::eq, a, x, b, y, c)));
...
// points trueFact and falseFact to freshly allocated objects that represent the
// true and false branches of the control flow guarded by the given expression.
// They are set to NULL if our representation cannot represent one of the
// expressions. doFalseBranch - if =true, falseFact is set to the correct
// false-branch condition and to NULL otherwise
void IntArithLogicalPlacer::setTrueFalseBranches(
    SgExpression *expr, IntArithLogicalFact **trueFact,
    IntArithLogicalFact **falseFact, bool doFalseBranch) {
  varID x, y;
  bool negX, negY;
  long c;
  *trueFact = NULL;
...
void IntArithLogicalPlacer::visit(const Function &func, const DataflowNode &n,
                                  NodeState &state) {
  if (analysisDebugLevel > 0)
    printf("IntArithLogicalPlacer::visit() function %s() node=<%s | %s>\n",
           func.get_name().str(), n.getNode()->class_name().c_str(),
           n.getNode()->unparseToString().c_str());
  ...
  // generate the inequality attributes that will be associated with this if/for
  // statement's true and false branches
  ...
  for (vector<DataflowEdge>::iterator ei = edges.begin(); ei != edges.end();
       ei++) {
...
void printIntArithLogicals(IntArithLogicalPlacer *aip, string indent) {
  ...
  printAnalysisStates pas(aip, factNames, latticeNames,
                          printAnalysisStates::below, indent);
...
// returns the set of IntArithLogical expressions known to be true at the given
// DataflowNode
const IntArithLogical &getIntArithLogical(const DataflowNode &n) {
  NodeState *ns = NodeState::getNodeState(n);
  IntArithLogicalFact *fact =
      dynamic_cast<IntArithLogicalFact *>(ns->getFact(aip, 0));
