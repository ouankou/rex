...
public:
  // Possible levels of this constraint. Above bottom the levels are ordered
  // according to their
  //    information content. False has the most content (admits fewest points),
  //    then a specific affine inequality and finally, top, which represents a
  //    state where the true inequality is too complex to be represented as an
  //    affine inequality, meaning that the only affine inequality that can do
  //    the job is x <= y + infinity.
  // no constraint known since current constraints are not representable using a
  // single affine inequality (i.e. x <= y + infinity)
  ...
  // (effectively, bottom == uninitialized) (this object would not even be
...
public:
  ...
  affineInequality(int a, int b, int c, bool xZero, bool yZero, signs xSign,
                   signs ySign);

  // given a constraint on x, z and a constraint on z, y, infers the
  // corresponding constraint on x, y and sets this constraint to it
  ...
  // Returns true if this affineInequality represents less or more information
  // (less information is top, more information is bottom) than that for all
  // values of x and y and false otherwise
  // bool operator<<(const affineInequality& that) const;
  // !!! NOTE: WE MAY WANT A SEMANTIC LESS-THAN-OR-EQ SINCE THAT CAN BE COMPUTED
  // MORE PRECISELY BASED IN <= INFORMATION
  bool semLessThan(const affineInequality &that, bool xEqZero,
                   bool yEqZero) const;
  bool semLessThan(const affineInequality &that, const affineInequality *xZero,
                   const affineInequality *zeroX, const affineInequality *yZero,
                   const affineInequality *zeroY,
                   std::string indent = "") const;
  bool semLessThanEq(const affineInequality &that, bool xIsZeroVar,
                     const affineInequality *xZero,
                     const affineInequality *zeroX, bool yIsZeroVar,
                     const affineInequality *yZero,
                     const affineInequality *zeroY,
                     std::string indent = "") const;

  // Semantic affineInequality ordering (partial order), focused on negated
  // inequalities Returns true if the negation of this affineInequality
  // represents more information (less information is top, more information is
  // bottom) than the nagation of that for all values of x and y and false
  // otherwise
  // bool affineInequality::semLessThanNeg(const affineInequality& that) const
  bool semLessThanNeg(const affineInequality &that, bool xEqZero,
                      bool yEqZero) const;

  bool set(const affineInequality &that);

  bool set(int a, int b, int c);
  bool set(int a, int b, int c, bool xZero, bool yZero, signs xSign,
           signs ySign);
...
public:
  ...
  // Intersection = the affine inequality that contains the constraint
  // information of both
  //         this AND that (i.e. the line that is lower than both lines)
  // (moves this/that upward in the lattice)
  void operator*=(const affineInequality &that);

  // Just like *=, except intersectUpd() returns true if the operation causes
  // this affineInequality to change and false otherwise.
  ...
  // affineInequalities are conservative in the sense that they don't contain
  // any more information than is definitely true, it may be that the true
  // constraints are in fact inconsistent but we do not have enough information
  // to prove this.
  static bool mayConsistent(const affineInequality &constrXY,
...
public:
  ...
  // of a logical condition: NOT, AND and OR
  /*void notUpd();
...
public:
  ...
  varAffineInequality(const varID &x, const varID &y,
                      const affineInequality &ineq);

  varAffineInequality(const varID &x, const varID &y, int a, int b, int c,
  ...
  // Parses expr and returns the corresponding varAffineInequality if expr can
  // be represented as such and NULL otherwise.
...
void setTrueFalseIneq(SgExpression *expr, affineInequalityFact **trueIneqFact,
                      affineInequalityFact **falseIneqFact, bool doFalseBranch);
...
none are available std::set<varAffineInequality>* getAffineIneq(SgNode* n);*/
...
/*class printAffineInequalities : public UnstructuredPassIntraAnalysis
{
        ...
        void visit(const Function& func, const DataflowNode& n, NodeState&
state);
};*/
...
// descendants
std::list<std::set<varAffineInequality>>
