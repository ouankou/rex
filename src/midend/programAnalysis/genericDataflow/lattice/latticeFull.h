...
public:
  BoolAndLattice() { state = -1; }

private:
  BoolAndLattice(int state) { this->state = state; }

public:
  BoolAndLattice(bool state) { this->state = state; }
  ...
  // These are empty in this lattice since it is now explicitly aware of
  // variables.
...
public:
  ...
  IntMaxLattice() { state = -1; }

  IntMaxLattice(int state) { this->state = state; }
...
public:
  ...
  void verifyFinite() {
    for (std::vector<Lattice *>::iterator it = lattices.begin();
         it != lattices.end(); it++)
      ROSE_ASSERT((*it)->finiteLattice());
  }

  // Returns a copy of this lattice
  Lattice *copy() const { return new FiniteProductLattice(*this); }
};
...
public:
  ...
  //     be either used internally by this VariablesProductLattice object or
  //     deallocated
  ...
  VariablesProductLattice(bool includeScalars, bool includeArrays,
                          Lattice *perVarLattice,
                          const std::map<varID, Lattice *> &constVarLattices,
                          Lattice *allVarLattice, const Function &func,
...
public:
  ...
  // returns the set of variables(scalars and/or arrays) visible in this
  // function
...
class FiniteVariablesProductLattice : public virtual VariablesProductLattice,
...
public:
  ...
  FiniteVariablesProductLattice(
      ...
      const std::map<varID, Lattice *> &constVarLattices,
      Lattice *allVarLattice, const Function &func, const DataflowNode &n,
      const NodeState &state)
      : VariablesProductLattice(includeScalars, includeArrays, perVarLattice,
                                constVarLattices, allVarLattice, func, n,
                                state),
  ...
  FiniteVariablesProductLattice(const FiniteVariablesProductLattice &that)
      : ProductLattice(), VariablesProductLattice(that),
        FiniteProductLattice() {
    verifyFinite();
  }

  // returns a copy of this lattice
  Lattice *copy() const { return new FiniteVariablesProductLattice(*this); }
};

class InfiniteVariablesProductLattice : public virtual VariablesProductLattice,
...
public:
  ...
  // returns a copy of this lattice
  Lattice *copy() const { return new InfiniteVariablesProductLattice(*this); }
};
