...
// adds to declaredVars the set of all variables declared within the given AST
// sub-tree onlyGlobal - if onlyGlobal==true, only global variables are added
// getCompilerGen - if =true, the returned set includes compiler-generated
// variables and doesn't if =false
void getDeclaredVars(SgNode *root, bool onlyGlobal, varIDSet &declaredVars,
                     bool getCompilerGen);
...
void initLocalVars(const Function &func, bool getCompilerGen = false) {
  if (localVars_initialized.find(func) == localVars_initialized.end()) {
    ...
    getDeclaredVars(func.get_declaration(), false, localVars[func],
                    getCompilerGen);
    getDeclaredVars(func.get_definition(), false, localVars[func],
                    getCompilerGen);
...
void initFuncParams(const Function &func, bool getCompilerGen) {
  if (funcParams_initialized.find(func) == funcParams_initialized.end()) {
    ...
    for (SgInitializedNamePtrList::iterator itParams = params.begin();
      ...
      if (!getCompilerGen &&
          (*itParams)->get_file_info()->isCompilerGenerated())
        continue;
...
void getDeclaredVars(SgNode *root, bool onlyGlobal, varIDSet &declaredVars,
  ...
  /*!!! NOTE: THIS CODE DOES NOT CAPTURE FIELDS WITHIN STRUCTS, UNIONS AND
   * CLASSES !!! */
...
void getReferencedVars(SgFunctionDefinition *func, varIDSet &refVars) {
  ...
  //  func->get_file_info()->isCompilerGenerated()? "Compiler generated": "In
  //  Original Code");
