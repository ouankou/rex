...
public:
  ...
  SgType *varType;
  // special variables do not have associated SgInitializedName nodes and thus,
  // we just give them a name
  std::string name;
  ...
  varID(const varID &that) : variable() { init(that); }
  ...
  // recursive function that pulls the SgInitializedNames of all the
  // SgVarRefExps inside this SgDotExp returns true on success, false on failure
...
protected:
  ...
  // components vector
  SgExpression *toSgExpression_rec(
      std::vector<SgInitializedName *>::const_iterator rest) const;
...
public:
  ...
  // Remove the given annotation from this variable. Returns true if this
  // variable previously had this annotation and false otherwise.
  ...
  // Swap this variables annotations, removing [fromAnnotName -> fromAnnotVal]
  // and replacing it with [toAnnotName -> toAnnotVal]. Returns true if this was
  // successful (i.e. the variable does have the [fromAnnotName -> fromAnnotVal]
  // annotation) and false otherwise. If the replacement occurs and this
  // variable already has an annotation named
  ...
  // If this variable has the annotation with EACH name in the given set,
...
public:
  ...
  // returns true if the last field in this variable has an array or pointer
  // type
  ...
  {
          return
  isSgReferenceType(components[components.size()-1]->get_typeptr());
  }*/
...
// ostream &operator<<(ostream &stream, const std::set<varID>::const_iterator&
// v);
...
// set of all 0, 1, -1, all constants being used in the program +/-1 and the
sums of all the above
...
protected:
  ...
  SgPntrArrRefExp *toSgExpression_rec(
      std::list<SgExpression *>::reverse_iterator itIndexes) const;
