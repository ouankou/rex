...
// initializes this object from the given expression (assumed that
// isValidVarExp(n) is true) returns true on success, false on failure
...
// initializes this object from the given expression (assumed that
// isValidVarExp(exp) is true) returns true on success, false on failure
bool varID::init(const SgExpression *exp) {
  exp = cfgUtils::unwrapCasts(exp);
  if (isValidVarExp(exp)) {
    // if this is an array reference expression, the variable in question is the
    // array itself
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    // !!! NOTE: IN THE FUTURE WE MAY WANT TO BE MORE PRECISE ABOUT THIS BUT
    // WE'LL NEED TO ADD THE NOTION OF ARRAYS TO OUR varIDs !!!
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
// initializes this object from the given SgInitializedName (assumed that
// isValidVarExp(name) is true)
...
// recursive function that pulls the SgInitializedNames of all the SgVarRefExps
// inside this SgDotExp returns true on success, false on failure
bool varID::collectDotComponents(const SgDotExp *dotExp) {
  //              printf("isSgVarRefExp(dotExp->get_lhs_operand())=%d
  //              isSgDotExp(dotExp->get_lhs_operand())=%d\n",
  //              isSgVarRefExp(dotExp->get_lhs_operand()),
  //              isSgDotExp(dotExp->get_lhs_operand()));
  //              printf("isSgVarRefExp(dotExp->get_rhs_operand())=%d
  //              isSgDotExp(dotExp->get_rhs_operand())=%d\n",
  //              isSgVarRefExp(dotExp->get_rhs_operand()),
  //              isSgDotExp(dotExp->get_rhs_operand()));
...
// for compound variables (i.e. those composed of dot expressions), it is the
// scope of the leftmost name
...
// returns a SgExpression that corresponds to an access of this variable,
// including only the components at or after the iterator rest into the
// components vector
SgExpression *varID::toSgExpression_rec(
    vector<SgInitializedName *>::const_iterator rest) const {
...
// returns true if the given expression is one that can be represented as a
// variable in our representation
bool varID::isValidVarExp(const SgExpression *exp) {
  ...
  // array itself
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // !!! NOTE: IN THE FUTURE WE MAY WANT TO BE MORE PRECISE ABOUT THIS BUT WE'LL
  // NEED TO ADD THE NOTION OF ARRAYS TO OUR varIDs !!!
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  /*if(isTopArrayRefExp(exp))
          exp = getArrayNameExp(exp);*/
  ...
  /*              printf("isValidVarExp() isSgVarRefExp(exp)=%d
  ...
  return // exp is valid if it is not part of a larger dot expression
      ...
      ((isSgVarRefExp(exp) && !isSgDotExp(exp->get_parent())) ||
       // exp is a dot expression that is a valid expression (evaluated
       // recursively using isValidVarExp_rec())
       (isSgDotExp(exp) && isValidVarExp_rec(exp)));
}

bool varID::isValidVarExp(const SgInitializedName *exp) { return true; }
...
bool varID::isValidVarExp_rec(const SgExpression *exp) {
  ...
  return // exp is a variable reference expression
      isSgVarRefExp(exp) ||
      // exp is a dot expression with an lhs that is a reference expression and
      // an rhs that is a valid expression
      (isSgDotExp(exp) &&
       isValidVarExp_rec(((SgDotExp *)exp)->get_lhs_operand()) &&
       isSgVarRefExp(((SgDotExp *)exp)->get_rhs_operand()));
}

void varID::add(SgInitializedName *name) { components.push_back(name); }

// Adds the given annotation to this variable. Returns true if this causes the
// variable's annotation state to change and false otherwise.
...
// Swap this variables annotations, removing [fromAnnotName -> fromAnnotVal] and
// replacing it with [toAnnotName -> toAnnotVal]. Returns true if this was
// successful (i.e. the variable does have the [fromAnnotName -> fromAnnotVal]
// annotation) and false otherwise. If the replacement occurs and this variable
// already has an annotation named
...
bool varID::swapAnnotations(const string &fromAnnotName, void *fromAnnotVal,
  ...
  for(map<string, void*>::iterator it=annotations.begin();
  it!=annotations.end(); it++) cout << it->first<<"->"<<it->second<<" "; cout <<
  "\n";*/
...
// If this variable has the annotation with the given name, returns it.
...
// If this variable has the annotation with the given name, returns true.
...
// If this variable has the annotation with ANY name in the given set, returns
// true. Otherwise, returns false.
bool varID::hasAnyAnnotation(const set<string> &aNames) const {
  for (set<string>::const_iterator it = aNames.begin(); it != aNames.end();
       it++)
...
// If this variable has the annotation with EACH name in the given set, returns
// true. Otherwise, returns false.
...
int varID::numAnnotations() const { return annotations.size(); }

// Returns the full map of all the annotations associated with this variable
const map<string, void *> &varID::getAnnotations() const { return annotations; }
...
bool varID::equal(const varID &two) const {
  ...
  it_one->second!=it_two->second) return false;
          }
  }*/

  return components == two.components && annotations == two.annotations &&
         name == two.name;
}
...
bool varID::lessThan(const varID &two) const {
  /*      // if these two variables have no components, just compare their names
          ...
          // we've iterated to the end of one of the lists and thus far they're
     equal
  ...
  if (components < two.components)
    return true;
  else if (components == two.components) {
    if (annotations < two.annotations)
      return true;
    else if (annotations == two.annotations)
      return name < two.name;
  }
  return false;
}
...
bool varID::operator!=(const varID &that) const { return !(*this == that); }
...
bool operator!=(const varID &var, SgExpression *expr) { return !(var == expr); }
bool operator==(SgExpression *expr, const varID &var) { return var == expr; }
bool operator!=(SgExpression *expr, const varID &var) { return !(var == expr); }
...
string varID::str() const { return str(false); }
string varID::str(bool noAnnot) const {
  ...
  if (!noAnnot) {
    for (map<string, void *>::const_iterator it = annotations.begin();
...
// string representation of the variable reference, with variable/field names
// augmented with the line numbers where they were defined. File names are
// omitted for brevity
...
string varID::str_linenum() const {
  ...
  if (components.size() > 0) {
    for (vector<SgInitializedName *>::const_iterator it = components.begin();
      ...
      ConvertStream << "[" << (*it)->get_name().getString() << "|"
                    << (*it)->get_file_info()->get_line() << "]";
      ConvertStream >> out;
      //                      printf("(*it)->get_name().str()=%s\n",
      //                      (*it)->get_name().getString().c_str());
...
// string representation of the variable reference, with the variable names
// replaced with the pointers to their declarations
...
// returns true if any of its constituent SgInitializedNames are
// compiler-generated
bool varID::isCompilerGenerated() const {
  for (vector<SgInitializedName *>::const_iterator it = components.begin();
       it != components.end(); it++) {
    if ((*it)->get_file_info()->isCompilerGenerated())
      return true;
  }
  return false;
}
...
bool varID::isGlobal() const {
  ...
  // components[0] refers to a variable declaration with a globally visible
  // scope if ...
  return
      // initName refers to a variable rather than a function AND
      !isSgFunctionType(components[0]->get_type()) &&
      // this SgInitializedName is part of a variable declaration rather than a
      // function argument list AND
...
long varID::getID() const { return ID; }

// translate from an expression that uses a variable to that variable's unique
// id currently supported: a (SgVarRefExp), a.i (SgDotExp),
/*varID
...
getVarReference( SgDotExp *exp )
{
        ...
        if ( r && l )
                {
                        varID x = (( (varID)(l->get_symbol()->get_declaration())
) << 32) ^ (varID)( r->get_symbol()->get_declaration() ); return x;
                }
        else
                {
...
// converts from a pointer to SgInitializedName (32-bits) to a full 64-bit
unique id varID getVarReference( SgInitializedName *iN )
{
        ROSE_ASSERT ( iN );
        return (varID)iN;
}*/
...
// variable
bool isVarExpr(SgExpression *expr) { return varID::isValidVarExp(expr); }
...
void addPredefinedVars(varIDSet &vars) { vars.insert(zeroVar); }
...
// returns whether this is a variable reference or dot expression (field
// reference)
...
// inherited attribute that allows us to tell whether a given SgVarRefExp is
// independent or part of some SgDotExp
/*class varStatus
{
        public;
        bool independent;
...
public:
  ...
  varIDSet evaluateSynthesizedAttribute(SgNode *n,
    ...
    // if n is not a variable node, res = union of children lists
      ...
      for (SynthesizedAttributesList::iterator it = children.begin();
           it != children.end(); it++) {
        for (varIDSet::iterator it2 = (*it).begin(); it2 != (*it).end();
             it2++) {
          res.insert(*it2);
        }
      }
...
// returns a set of varIDs that correspond to all the variables (of
// SgDotExp/SgVarRefExp form) that were referenced in the given subtree
...
// returns a set of varIDs that correspond to all the variables (of
// SgDotExp/SgVarRefExp form) that were read from in the given subtree
...
// returns a set of varIDs that correspond to all the variables (of
// SgDotExp/SgVarRefExp form) that were written to in the given subtree
...
// returns a set of varIDs that correspond to all the variables (of
// SgDotExp/SgVarRefExp form) that were referenced as arrays (i.e. var[i]) in
// the given subtree
...
string arrayElt::str() const {
  ...
  outs << arrayVar.str();
  for (list<SgExpression *>::const_iterator it = indexExprs->begin();
       it != indexExprs->end(); it++)
...
bool arrayElt::operator==(const variable &that_arg) const {
  ...
  if (arrayVar != that.arrayVar)
    return false;
  if (indexExprs != that.indexExprs)
    return false;
...
bool arrayElt::operator<(const variable &that_arg) const {
  ...
  if (arrayVar > that.arrayVar)
    return false;
  if (indexExprs->size() < that.indexExprs->size())
    return true;
  if (indexExprs->size() > that.indexExprs->size())
    return false;
...
// for compound variables (i.e. those composed of dot expressions), it is the
// scope of the leftmost name
SgScopeStatement *arrayElt::getScope() const { return arrayVar.getScope(); }

// returns a referenec to the array variable, without any indexes
const varID &arrayElt::getArrayVar() { return arrayVar; }

// returns a reference to the index expressions
list<SgExpression *> *arrayElt::getIndexExprs() { return indexExprs; }
...
// returns the SgPntrArrRefExp that corresponds to the index expressions in
// indexExprs that start with itIndexes precondition : itIndexes!=index.end()
SgPntrArrRefExp *arrayElt::toSgExpression_rec(
    list<SgExpression *>::reverse_iterator itIndexes) const {
  SgTreeCopy tc;
...
// returns true if this variable is global and false otherwise
bool arrayElt::isGlobal() const { return arrayVar.isGlobal(); }

class ArrayEltCollector : public AstBottomUpProcessing<set<arrayElt>> {
public:
  set<arrayElt>
  evaluateSynthesizedAttribute(SgNode *n, SynthesizedAttributesList children) {
    // list of variables inside this subtree
    set<arrayElt> res;
    ...
    // if n is not a variable node, res = union of children lists
      ...
      for (SynthesizedAttributesList::iterator it = children.begin();
           it != children.end(); it++) {
        for (set<arrayElt>::iterator it2 = (*it).begin(); it2 != (*it).end();
             it2++) {
          res.insert(*it2);
        }
      }
...
// returns a set of arrayElts that correspond to all the arrays (of
// SgDotExp/SgVarRefExp[SgExpression][][][]... form) that were referenced in the
// given subtree
