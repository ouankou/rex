...
Function &FunctionState::getFunc() { return func; }
...
FunctionState *FunctionState::getDefinedFuncState(const Function &func) {
  for (set<FunctionState *>::iterator it = allDefinedFuncs.begin();
       it != allDefinedFuncs.end(); it++)
    if ((*it)->func == func)
      return *it;
  return NULL;
}
...
FunctionState *FunctionState::getFuncState(const Function &func) {
  if (!allFuncsComputed) // Liao, 4/6/2012, make sure the internal set is
                         // computed first.
  {
    getAllFuncs();
  }
...
// given a function call, sets argParamMap to map all simple arguments to this
...
void FunctionState::setArgParamMap(SgFunctionCallExp *call,
  ...
  for (itA = args.begin(), itP = params.begin();
       itA != args.end() && itP != params.end(); ++itA, ++itP) {
    // cout << "    itA="<<(*itA)->unparseToString()<<"
    // itP="<<(*itP)->unparseToString()<<"
    // isValid="<<varID::isValidVarExp(*itA)<<"\n";
...
// given a function call, sets argParamMap to map all the parameters of this
// function to their corresponding simple arguments, if those arguments are
// passed by reference
void FunctionState::setParamArgByRefMap(SgFunctionCallExp *call,
  ...
  for (itParams = params.begin(), itArgs = args.begin();
       itParams != params.end() && itArgs != args.end(); itParams++, itArgs++) {
    ...
    isValidVar=%d\n", (*itArgs)->unparseToString().c_str(),
    (*itArgs)->class_name().c_str(), varID::isValidVarExp(*itArgs)); printf("
    typeArg=<%s | %s>\n", typeArg->unparseToString().c_str(),
    typeArg->class_name().c_str()); printf(" itParams=<%s | %s>\n",
    (*itParams)->unparseToString().c_str(), (*itParams)->class_name().c_str());
    printf("                                     typeParam=<%s | %s>
    isReference=%d\n", typeParam->unparseToString().c_str(),
    typeParam->class_name().c_str(), isSgReferenceType(typeParam));*/

    /*      // if the argument is a named variable AND
    if(varID::isValidVarExp(*itArgs) &&
             // if the argument has an array type, it's contents will be passed
    by reference OR
            ((isSgArrayType(typeArg) || isSgPointerType(typeArg)) ||
             // if the argument is a regular variable, by the parameter has a
    reference type,
             // or the variable is being passed via a pointer, the variable must
    be passed by reference isSgReferenceType(typeParam)))
    {
            varID argVar(*itArgs);
