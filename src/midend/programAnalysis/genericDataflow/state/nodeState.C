...
// Returns true if this analysis has initialized its state at this node and
// false otherwise
...
#else
  return initializedAnalyses.find((Analysis *)analysis) !=
         initializedAnalyses.end();
#endif
}

/*// adds the given lattice, organizing it under the given analysis and lattice
name void NodeState::addLattice(const Analysis* analysis, int latticeName,
Lattice* l)
{
...
// Set dfInfoAbove and dfInfoBelow to lattices
  ...
  // Now iterate through dfInfoAbove, copying each lattice in dfInfoAbove over
  // to dfInfoBelow
  ...
  for (vector<Lattice *>::const_iterator it = rA->second.begin();
       it != rA->second.end(); it++) {
    Lattice *l = (*it)->copy();
    // Dbg::dbg << "NodeState::setLattices pushing dfInfoBelow:
    // "<<l->str("")<<"\n";
  ...
  for (vector<Lattice *>::iterator it =
           dfInfoAbove[(Analysis *)analysis].begin();
...
void NodeState::setLatticeAbove(const Analysis *analysis,
  ...
  /*printf("Lattices above:\n");
  for(vector<Lattice*>::iterator it = w->second.begin(); it!=w->second.end();
  it++)
  {
          Dbg::dbg << (*it)->str("    ") << "\n";
  }*/
...
//! returns all the lattices from above the CFG node (corresponding to SgNode
//! and an CFG index) that are owned by the given analysis
// (read-only access)
const std::vector<Lattice *> &
NodeState::getLatticeAbove(const Analysis *a, SgNode *n, unsigned int index) {
  assert(a != NULL);
  assert(n != NULL);
...
// returns all the lattices from below the CFG node (corresponding to SgNode and
// an CFG index) that are owned by the given analysis (read-only access)
const std::vector<Lattice *> &
NodeState::getLatticeBelow(const Analysis *a, SgNode *n, unsigned int index) {
  assert(a != NULL);
  assert(n != NULL);
...
// returns the given lattice from above the node, which owned by the given
// analysis
...
// returns the map containing all the lattices from above the node that are
// owned by the given analysis (read-only access)
const vector<Lattice *> &
NodeState::getLatticeAbove(const Analysis *analysis) const {
#ifdef THREADED
  LatticeMap::const_accessor r;
  // if this analysis has registered some lattices at this node, return their
  // vector
  ...
  Dbg::dbg << "Analysis "<<analysis<<"
  found="<<(dfInfoAbove.find((Analysis*)analysis)!=dfInfoAbove.end())<<"\n";*/
  if (dfInfoAbove.find((Analysis *)analysis) != dfInfoAbove.end())
...
// returns the map containing all the lattices from above the node that are
// owned by the given analysis (read/write access)
...
// returns the given lattice from below the node, which owned by the given
// analysis
...
// returns the map containing all the lattices from below the node that are
// owned by the given analysis (read-only access)
const vector<Lattice *> &
...
bool NodeState::eqLattices(const vector<Lattice *> &latticesA,
  ...
  for (itA = latticesA.begin(), itB = latticesB.begin();
       itA != latticesA.end() && itB != latticesB.end(); ++itA, ++itB) {
    Lattice *lA = *itA;
    Lattice *lB = *itB;
    /*Dbg::dbg << "        lA = "<<lA->str("    ")<<"\n";
    Dbg::dbg << "        lB = "<<lB->str("    ")<<"\n";
...
void NodeState::cloneAnalysisState(const Analysis *srcA, const Analysis *tgtA) {
  ...
  for (vector<NodeFact *>::const_iterator it = srcFacts.begin();
    ...
    // it->first, it->second); tgtFacts[it->first] = it->second->copy();
...
// Given a set of analyses, one of which is designated as a master, unions
// together the lattices associated with each of these analyses. The results are
// associated on each CFG node with the master analysis.
void NodeState::unionLattices(set<Analysis *> &unionSet,
  ...
  // unionSet.size(), master, this); printf("        masterLatAbv.size()=%d,
  // masterLatBel.size()=%d\n", masterLatAbv.size(), masterLatBel.size());
  for (set<Analysis *>::iterator it = unionSet.begin(); it != unionSet.end();
       it++) {
    Analysis *curA = *it;
    // printf("        curA=%p\n", curA);
    if (curA != master) {
...
// adds the given lattice to the given dfInfo structure (dfInfoAbove or
dfInfoBelow),
...
dfLattices->second.at(latticeName).str(""));
                ...
                if(dfLattices->second.at(latticeName) !=
dfLattices->second.end())
                {
                        //printf("NodeState::addLattice_ex() deleting old
...
// returns true if the given lattice was found and removed and false if it was
not found bool NodeState::removeLattice_ex(map <Analysis*, vector<Lattice*> >&
dfMap, const Analysis* analysis, int latticeName)
{
        map <Analysis*, vector<Lattice*> >::iterator dfLattices;
        // if this analysis has registered some Lattices at this node
        if((dfLattices = dfMap.find((Analysis*)analysis)) != dfMap.end())
        {
                ...
                if((it = dfLattices->second.find(latticeName)) !=
dfLattices->second.end())
                {
                        delete *it;
...
void NodeState::setFacts(const Analysis *analysis,
                         const vector<NodeFact *> &newFacts) {
#ifdef THREADED
  NodeFactMap::accessor factsIt;
  // if this analysis has registered some facts at this node
  if (facts.find(factsIt, (Analysis *)analysis))
...
#endif
  {
    // delete the old facts (if any) and associate the analysis with the new set
    // of facts
...
NodeFact *NodeState::getFact(const Analysis *analysis, int factName) const {
...
#endif
  {
    ...
    // factName, factsIt->second.size()); if((it =
    // factsIt->second.find(factName)) != factsIt->second.end())
    if ((unsigned int)factName < factsIt->second.size()) {
      // return it->second;
      // return *it;
...
// returns the map of all the facts owned by the given analysis at this
// NodeState (read-only access)
const vector<NodeFact *> &NodeState::getFacts(const Analysis *analysis) const {
  ...
  // facts.find((Analysis*)analysis)==facts.end());
  //  if this analysis has registered some facts at this node, return their map
...
// returns true if the given fact was found and removed and false if it was not
// found
/*bool NodeState::removeFact(const Analysis* analysis, int factName)
{
        ...
        #endif
        {
                ...
                //if((it = factsIt->second.find(factName)) !=
factsIt->second.end()) if(factName < factsIt->second.size())
                {
                        //delete it->second;
                        delete *it;
...
// returns a vector of NodeState objects associated with the given dataflow
// node.
...
void NodeState::initNodeStateMap(bool (*filter)(CFGNode cfgn)) {
  ...
  for (set<FunctionState *>::iterator it = allFuncs.begin();
    ...
    // DQ (12/10/2016): If this function has not side-effects then we could also
    // eliminate the function call as well.
    cfgUtils::getFuncEndCFG(func.get_definition(), filter);
    ...
    for (VirtualCFG::iterator it(funcCFGStart);
      ...
      /*// if this is a function call, it has 3 states: one for the call, one
      for the body and one for the return if(isSgFunctionCallExp(n.getNode()))
              numStates=3;*/
  ...
  /*for(set<FunctionState*>::iterator it=allFuncs.begin(); it!=allFuncs.end();
  it++) { const Function& func = (*it)->func; DataflowNode funcCFGStart =
  cfgUtils::getFuncStartCFG(func.get_definition()); DataflowNode funcCFGEnd =
  cfgUtils::getFuncEndCFG(func.get_definition()); for(VirtualCFG::iterator
  it(funcCFGStart); it!=VirtualCFG::dataflow::end(); it++) { DataflowNode n =
  *it;

                  Dbg::dbg << "    NodeState::initNodeStateMap() ###
  sgn="<<n.getNode()<<"=<"<<Dbg::escape(n.getNode()->unparseToString())<<" |
  "<<n.getNode()->class_name()<<" | "<<n.getIndex()<<">\n";

                  int numStates=1;
                  for(int i=0; i<numStates; i++) {
                          Dbg::dbg << "NodeState::initNodeStateMap() ###
  nodeStateMap[n]["<<i<<"]="<<&(nodeStateMap[n][i])<<"
  nodeStateMap[n]["<<i<<"].initializedAnalyses.size()="<<nodeStateMap[n][i]->initializedAnalyses.size()<<"\n";
                          Dbg::dbg << "NodeState::str() ###
  #initializedAnalyses="<<nodeStateMap[n][i]->initializedAnalyses.size()<<"\n";
                          for(BoolMap::const_iterator
  it=nodeStateMap[n][i]->initializedAnalyses.begin();
  it!=nodeStateMap[n][i]->initializedAnalyses.end(); it++) Dbg::dbg << " ###
  "<<it->first<<" : "<<it->second<<"\n";
                  }
          }
  }*/
...
void NodeState::copyFacts(NodeState &that)
{
        ...
        {
                for(map <int, NodeFact*>::iterator it2 = it1->second.begin();
it2 != it1->second.end(); it2++)
        ...
        for(NodeFactMap::it1 = that.facts.begin(); it1!=that.facts.end(); it++)
        {
                ...
                for(map <int, NodeFact*>::iterator it2 = it1->second.begin();
it2 != it1->second.end(); it2++) newMap[it2->first] = it2->second;
...
void NodeState::copyLatticesAbove(const map <Analysis*, vector<Lattice*> >&
thatInfo)
{
...
void NodeState::copyLatticesBelow(const map <Analysis*, vector<Lattice*> >&
thatInfo)
{
                             ...
                             const map <Analysis*, vector<Lattice*> >& thatInfo)
{
            ...
            it++, itThat++)
        {
                ...
                vector<Lattice*>::iterator itL;
                for(itL = itThat->second.begin(); itL != itThat->second.end();
itL) newInfo.push_back((*itL)->copy()); dfInfo[it->first] = newInfo;
        }
}*/

// copies from's above lattices for the given analysis to to's above lattices
// for the same analysis
void NodeState::copyLattices_aEQa(Analysis *analysis, NodeState &to,
                                  const NodeState &from) {
#ifdef THREADED
  LatticeMap::accessor wTo;
  to.dfInfoAbove.find(wTo, analysis);
  LatticeMap::const_accessor rFrom;
  from.dfInfoAbove.find(rFrom, analysis);
  copyLattices(wTo->second, rFrom->second);
#else
  ROSE_ASSERT(to.dfInfoAbove.find(analysis) != to.dfInfoAbove.end());
  copyLattices(to.dfInfoAbove.find(analysis)->second,
               from.dfInfoAbove.find(analysis)->second);
#endif
}

// copies from's above lattices for analysisA to to's above lattices for
// analysisB
void NodeState::copyLattices_aEQa(Analysis *analysisA, NodeState &to,
                                  Analysis *analysisB, const NodeState &from) {
  // Dbg::dbg << "        to = "<<to.str(analysisA, "    ")<<"\n";

#ifdef THREADED
  LatticeMap::accessor wTo;
  to.dfInfoAbove.find(wTo, analysisA);
  LatticeMap::const_accessor rFrom;
  from.dfInfoAbove.find(rFrom, analysisB);
  copyLattices(wTo->second, rFrom->second);
#else
  ROSE_ASSERT(to.dfInfoAbove.find(analysisA) != to.dfInfoAbove.end());
  ROSE_ASSERT(to.dfInfoAbove.find(analysisB) != to.dfInfoAbove.end());

  // Dbg::dbg << "    copyLattices_aEQa()
  // #to.above="<<to.dfInfoAbove.find(analysisA)->second.size()<<"
  // #from.above="<<from.dfInfoAbove.find(analysisB)->second.size()<<"
  // analysisA="<<analysisA<<" analysisB="<<analysisB<<"\n";

  vector<Lattice *>::const_iterator itX, itY;
  for (itX = to.dfInfoAbove.find(analysisA)->second.begin(),
      itY = from.dfInfoAbove.find(analysisB)->second.begin();
       itX != to.dfInfoAbove.find(analysisA)->second.end() &&
       itY != from.dfInfoAbove.find(analysisB)->second.end();
       itX++, itY++) {
    // Dbg::dbg << "        itX = "<<*itX<<" = "<<(*itX)->str(" ")<<"\n";
    // Dbg::dbg.flush(); Dbg::dbg << "        itY = "<<*itY<<" =
    // "<<(*itY)->str("            ")<<"\n"; Dbg::dbg.flush(); Dbg::dbg << "
    // ---------------------------\n"; Dbg::dbg.flush();
    (*itX)->copy(*itY);
    // dfInfoX.push_back((*itY)->copy());
  }

  copyLattices(to.dfInfoAbove.find(analysisA)->second,
               from.dfInfoAbove.find(analysisB)->second);
#endif
}

// copies from's above lattices for the given analysis to to's below lattices
// for the same analysis
void NodeState::copyLattices_bEQa(Analysis *analysis, NodeState &to,
                                  const NodeState &from) {
#ifdef THREADED
  LatticeMap::accessor wTo;
  to.dfInfoBelow.find(wTo, analysis);
  LatticeMap::const_accessor rFrom;
  from.dfInfoAbove.find(rFrom, analysis);
  copyLattices(wTo->second, rFrom->second);
#else
  ...
  ROSE_ASSERT(to.dfInfoAbove.find(analysis) != to.dfInfoAbove.end());
  copyLattices(to.dfInfoBelow.find(analysis)->second,
               from.dfInfoAbove.find(analysis)->second);
#endif
}

// copies from's above lattices for analysisA to to's below lattices for
// analysisB
void NodeState::copyLattices_bEQa(Analysis *analysisA, NodeState &to,
                                  Analysis *analysisB, const NodeState &from) {
#ifdef THREADED
  LatticeMap::accessor wTo;
  to.dfInfoBelow.find(wTo, analysisA);
  LatticeMap::const_accessor rFrom;
  from.dfInfoAbove.find(rFrom, analysisB);
  copyLattices(wTo->second, rFrom->second);
#else
  ...
  ROSE_ASSERT(to.dfInfoAbove.find(analysisB) != to.dfInfoAbove.end());
  copyLattices(to.dfInfoBelow.find(analysisA)->second,
               from.dfInfoAbove.find(analysisB)->second);
#endif
}

// copies from's below lattices for the given analysis to to's below lattices
// for the same analysis
void NodeState::copyLattices_bEQb(Analysis *analysis, NodeState &to,
                                  const NodeState &from) {
#ifdef THREADED
  LatticeMap::accessor wTo;
  to.dfInfoBelow.find(wTo, analysis);
  LatticeMap::const_accessor rFrom;
  from.dfInfoBelow.find(rFrom, analysis);
  copyLattices(wTo->second, rFrom->second);
#else
  ROSE_ASSERT(to.dfInfoBelow.find(analysis) != to.dfInfoBelow.end());
  copyLattices(to.dfInfoBelow.find(analysis)->second,
               from.dfInfoBelow.find(analysis)->second);
#endif
}

// copies from's below lattices for the given analysis to to's above lattices
// for the same analysis
void NodeState::copyLattices_aEQb(Analysis *analysis, NodeState &to,
                                  const NodeState &from) {
#ifdef THREADED
  LatticeMap::accessor wTo;
  to.dfInfoAbove.find(wTo, analysis);
...
void NodeState::copyLattices(vector<Lattice *> &dfInfoX,
                             const vector<Lattice *> &dfInfoY) {
  /*      // empty lattices
          for(vector<Lattice*>::iterator itX = dfInfoX.begin();
     itX!=dfInfoX.end(); itX++)
          {
  ...
  for (itX = dfInfoX.begin(), itY = dfInfoY.begin();
       itX != dfInfoX.end() && itY != dfInfoY.end(); itX++, itY++) {
    // Dbg::dbg << "        itX = "<<*itX<<" = "<<(*itX)->str(" ")<<"\n";
...
string NodeState::str(Analysis *analysis, string indent) const {
  ...
  if (initializedAnalyses.find(analysis) == initializedAnalyses.end()) {
    ...
    for (lAbv = latticesAbove.begin(), lBel = latticesBelow.begin();
         lAbv != latticesAbove.end(); lAbv++, lBel++, i++) {
      oss << indent << "    Lattice " << i << " Above: " << *lAbv << " = "
          << (*lAbv)->str(indent + "        ") << "\n";
      oss << indent << "    Lattice " << i << " Below: " << *lBel << " = "
          << (*lBel)->str(indent + "        ") << "\n";
    }
    ...
    for (vector<NodeFact *>::const_iterator fact = aFacts.begin();
         fact != aFacts.end(); fact++, i++)
      oss << indent << "    Fact " << i << ": "
          << (*fact)->str(indent + "        ") << "\n";
    oss << indent << "]";
  }
