...
// wrapper around pointers that allows for regular == matching as well as
// wild-card matching
class fancyPointer {
public:
  void *ptr;
  // indicates whether matching on this pointer will be exact (=true) or
  // wildcard(=false)
  bool exactMatch;
  ...
  const bool operator!=(const fancyPointer &that) { return !(*this == that); }
  ...
  void wildMatch() { exactMatch = false; }
};
...
bool operator==(SgNode *&that, fancyPointer &me) { return me == that; }

bool operator!=(fancyPointer &me, SgNode *&that) { return !(me == that); }

bool operator!=(SgNode *&that, fancyPointer &me) { return me != that; }

class lrRecord {
public:
  // Pointers to the read, write and read-write subtrees of an assignment node.
  // Used by its children to determine whether they lie on the node's reading,
  // writing side or both. They are only set in SgNodes that perform,
  // assignments, like SgAssignNode or SgPlusPlusOp, so these fields are =NULL
  // in all other nodes.
  fancyPointer readSubtree;
  fancyPointer writeSubtree;
  fancyPointer rwSubtree;

  // The types of accesses performed on this side of the nearest SgAssignOp
  // node, if any
  accessType access;
  ...
  lrRecord(lrRecord &parent, SgNode *n) {
    ...
    // if this node is on the read, write or read-write side of an assignment
    // operation, set its access appropriately
    ...
    if ((binOp = isSgBinaryOp(n))) {
      ...
      // rwSubtree op= readSubtree
        ...
        // all the references involved in an array reference, whether they are
        // used to compute the array name or used in the argument, are read-only
        writeSubtree = (void *)NULL;
        readSubtree = (void *)NULL;
      ...
      // unOp->unparseToString().c_str());
    } else if ((funcCall = isSgFunctionCallExp(n))) {
      // all the references involved in a function call, whether they are used
      // to compute the function pointer or used in the argument, are read-only
      writeSubtree = (void *)NULL;
      readSubtree = (void *)NULL;
      readSubtree.wildMatch();
      rwSubtree = (void *)NULL;
...
public:
  rwAccessAttribute(lrRecord r) { access = r.access; }

  rwAccessAttribute(const rwAccessAttribute &a) { access = a.access; }

  rwAccessAttribute(rwAccessAttribute *a) { access = a->access; }

  void setAccess(accessType access) { this->access = access; }

  accessType getAccess() { return access; }
  ...
  bool operator==(rwAccessAttribute &that) { return access == that.access; }

  bool operator!=(rwAccessAttribute &that) { return !(*this == that); }
};
...
public:
  lrRecord evaluateInheritedAttribute(SgNode *n, lrRecord inherited) {
    ...
    if (!n->attributeExists("rwAccess"))
      ...
      rwAccessAttribute *oldAttr =
          (rwAccessAttribute *)n->getAttribute("rwAccess");
      // if it does, make sure that the old and the new attributes agree
      if (*oldAttr != *rwAttr)
        fprintf(stderr,
                "ERROR: old and new rwAttributes differ (Old: %s, New: %s)in "
                "SgNode <%s>!\n",
                accessTypeToStr(oldAttr->getAccess()).c_str(),
                accessTypeToStr(rwAttr->getAccess()).c_str(),
                n->unparseToString().c_str());
    }
...
void cloneAccessType(SgNode *tgt, SgNode *src) {
  ...
  if (src->attributeExists("rwAccess")) {
    if (tgt->attributeExists("rwAccess"))
      ((rwAccessAttribute *)tgt->getAttribute("rwAccess"))
          ->setAccess(((rwAccessAttribute *)(src->getAttribute("rwAccess")))
                          ->getAccess());
    else
      tgt->addNewAttribute(
          "rwAccess",
          new rwAccessAttribute(
              ((rwAccessAttribute *)(src->getAttribute("rwAccess")))));
  }
}
