...
Function::Function(string name) {
  ...
  Rose_STL_Container<SgNode *> functions =
      NodeQuery::querySubTree(cfgUtils::project, V_SgFunctionDeclaration);
  for (Rose_STL_Container<SgNode *>::const_iterator it = functions.begin();
       it != functions.end(); it++) {
    ROSE_ASSERT(isSgFunctionDeclaration(*it));

    if (!isSgTemplateFunctionDeclaration(*it) &&
        isSgFunctionDeclaration(*it)->get_name().getString() == name) {
      // decl will be initialized to the defining declaration of this function
      // or the first non-defining declaratio,n if there is no definition
      ...
      if(isSgFunctionDeclaration(*it)->get_definition())
      {
              ...
              // ensure that either def has not been set yet or that there is a
      unique definition if(def==NULL) def =
      isSgFunctionDeclaration(*it)->get_definition(); else ROSE_ASSERT(def ==
      isSgFunctionDeclaration(*it)->get_definition());
      }*/
    }
  }
...
Function::Function(SgFunctionDefinition *sample) {
  ...
  // ensure that the definition found by init() is the same as what got passed
  // in here
...
void Function::init(SgFunctionDeclaration *sample) {
  ...
  // printf("Function::init() name=%s=%s\n", this, sample->get_name().str(),
  // mangledName.str());

  // decl will be initialized to the defining declaration or the first
  // non-defining declaration if there is no definition
  decl = getCanonicalDecl(sample);

  // insert the sample declaration into decls, in case this is a name-less
  // function declaration (i.e. a function pointer) which won't show up in an
  // AST query
  // decls.insert(sample);

  /*Rose_STL_Container<SgNode*> functions =
  NodeQuery::querySubTree(cfgUtils::project, V_SgFunctionDeclaration); for
  (Rose_STL_Container<SgNode*>::const_iterator it = functions.begin(); it !=
  functions.end(); it++)
  {
          ROSE_ASSERT(isSgFunctionDeclaration(*it));
          ...
          if(isSgFunctionDeclaration(*it)->get_mangled_name() == mangledName)
          {
                  ...
                  {
                          // save the current function's definition inside def
                          // ensure that either def has not been set yet or that
  there is a unique definition if(def==NULL) def =
  isSgFunctionDeclaration(*it)->get_definition(); else ROSE_ASSERT(def ==
  isSgFunctionDeclaration(*it)->get_definition());
                  }
          }
  ...
  //printf("Function::init() this: 0x%x, name=%s *(decls.begin())=0x%x
  decls.size()=%d def=0x%x\n", this, get_name().str(), *(decls.begin()),
  decls.size(), def);
...
Function::Function(const Function &that) {
  ...
  /*def = that.def;
  for(set<SgFunctionDeclaration*>::const_iterator it = that.decls.begin();
  it!=that.decls.end(); it++) {       //printf("                  declaration:
  0x%x\n", *it); decls.insert(*it);
  }*/
  decl = that.decl;
}
...
Function::getCanonicalDecl(SgFunctionDeclaration *sampleDecl) {
  ...
  if (sampleDecl->get_definingDeclaration())
    canonicalDecl =
        isSgFunctionDeclaration(sampleDecl->get_definingDeclaration());
  else
    canonicalDecl =
        isSgFunctionDeclaration(sampleDecl->get_firstNondefiningDeclaration());
...
bool Function::eq(const Function &that) const {
  ...
  // check that that's set of declarations is a subset of this's set of
  declarations for(set<SgFunctionDeclaration*>::const_iterator it =
  that.decls.begin(); it!=that.decls.end(); it++)
  {
          printf("               (decls.find(*it) == decls.end()) = %d\n",
  (decls.find(*it) == decls.end())); if(decls.find(*it) == decls.end()) return
  false;
  }

  // check that this's set of declarations is a subset of that's set of
  declarations for(set<SgFunctionDeclaration*>::iterator it = decls.begin();
  it!=decls.end(); it++)
  {
          printf("               (that.decls.find(*it) == that.decls.end()) =
  %d\n", (that.decls.find(*it) == that.decls.end())); if(that.decls.find(*it) ==
  that.decls.end()) return false;
  }*/
  if (decl != that.decl)
    return false;
...
bool Function::operator==(const Function &that) const { return eq(that); }
...
bool Function::lessThan(const Function &that) const {
  // compare the declarations, since every Function object must have a unique
  ...
  // if both functions have the same number of declarations, compare the
  declarations themselves set<SgFunctionDeclaration*>::iterator itThis;
...
bool Function::operator<(const Function &that) const { return lessThan(that); }
bool Function::operator>(const Function &that) const {
...
//printf("Function::get_name() this: 0x%x, *(decls.begin())=0x%x decls.size()=%d
def=0x%x\n", this, *(decls.begin()), decls.size(), def); return
(*(decls.begin()))->get_name();*/
  if (decl)
...
// returns one of this function's declarations. it is guaranteed to be the same
// each time get_declaration is called
...
// returns the file_info of the definition or one of the declarations if there
// is no definition
...
string Function::str(string indent) const {
  ...
  for (SgInitializedNamePtrList::iterator in = params.begin();
       in != params.end();) {
...
CGFunction::CGFunction(string name, SgIncidenceDirectedGraph *graph)
    : Function(name) {
  this->graph = graph;
  initCGNodes();
}
...
CGFunction::CGFunction(SgGraphNode *sample, SgIncidenceDirectedGraph *graph)
    : Function(isSgFunctionDeclaration(sample->get_SgNode())) {
...
CGFunction::CGFunction(const CGFunction &that) : Function(that) {
  ...
  for (set<SgGraphNode *>::iterator it = that.cgNodes.begin();
       it != that.cgNodes.end(); it++)
    cgNodes.insert(*it);
}
...
void CGFunction::initCGNodes() {
  ...
  for (set<SgGraphNode *>::iterator itn = nodes.begin(); itn != nodes.end();
       itn++) {
    SgNode *n = (*itn)->get_SgNode();
    ROSE_ASSERT(isSgFunctionDeclaration(n));
    assert(!isSgTemplateFunctionDeclaration(n));

    SgFunctionDeclaration *cfgDecl =
        getCanonicalDecl(isSgFunctionDeclaration(n));
    assert(!isSgTemplateFunctionDeclaration(cfgDecl));

    /*// if this declaration is in the list of known declarations for this
    function if(decls.find(decl) != decls.end())*/
...
bool CGFunction::operator==(const CGFunction &that) const {
  if (!this->Function::eq(that))
    return false;

  // check that that's set of SgGraphNodes is a subset of this's set of
  // SgGraphNodes
  for (set<SgGraphNode *>::const_iterator it = that.cgNodes.begin();
       it != that.cgNodes.end(); it++)
    if (cgNodes.find(*it) == cgNodes.end())
      return false;

  // check that this's set of SgGraphNodes is a subset of that's set of
  // SgGraphNodes
  for (set<SgGraphNode *>::iterator it = cgNodes.begin(); it != cgNodes.end();
       it++)
    if (that.cgNodes.find(*it) == that.cgNodes.end())
      return false;
...
TraverseCallGraph::TraverseCallGraph(SgIncidenceDirectedGraph *graph) {
  ...
  for (set<CGFunction>::iterator it = functions.begin(); it != functions.end();
       it++) {
    noPred.insert((&*it));
    numCallers[(&*it)] = 0;
  }
  ...
  for (set<CGFunction>::iterator func = functions.begin();
       func != functions.end(); func++) {
    for (CGFunction::iterator it = (*func).successors(); it != (*func).end();
         it++) {
...
// returns a pointer to a CGFunction that matches the given declaration or NULL
// if no objects do the memory of the object persists for the entire lifetime of
// this TraverseCallGraph object
...
// returns a pointer to a CGFunction object that matches the given Function
// object the memory of the object persists for the entire lifetime of this
// TraverseCallGraph object
...
void TraverseCallGraphUnordered::traverse() {
  for (set<CGFunction>::iterator f = functions.begin(); f != functions.end();
       f++) {
    visit(&(*f));
  }
}
...
void TraverseCallGraphTopDown<InheritedAttribute>::traverse() {
  ...
  for (set<const CGFunction *>::iterator it = noPred.begin();
       it != noPred.end(); it++) {
    // printf("        *it=0x%x\n", *it);
...
template <class InheritedAttribute>
void TraverseCallGraphTopDown<InheritedAttribute>::traverse_rec(
    const CGFunction *fd, map<const CGFunction *, funcRecord> &visitRecords,
    set<pair<const CGFunction *, const CGFunction *>> &touchedEdges,
    InheritedAttribute &fromCaller) {
  /*printf("traverse_rec: func: 0x%x\n", fd);
  printf("traverse_rec: func->get_declaration(): 0x%x\n",
  fd->get_declaration()); printf("traverse_rec: func->get_definition(): 0x%x\n",
  fd->get_definition());*/
  ...
  visitRecords[fd].fromCallers.size()); printf(" numCallers[fd]=%d\n",
  numCallers[fd]);*/
  ...
  if (visitRecords[fd].fromCallers.size() >= (unsigned int)numCallers[fd]) {
    ...
    for (CGFunction::iterator it = fd->successors(); it != fd->end(); it++) {
      ...
      if (touchedEdges.find(edge) == touchedEdges.end()) {
        ...
        target->get_declaration()); printf("    calls: target->get_definition()
        = 0x%x\n", target->get_definition()); printf("    calls: unparse =
...
TraverseCallGraphBottomUp<SynthesizedAttribute>::traverse_rec(
    const CGFunction *fd,
    map<const CGFunction *, SynthesizedAttribute> &visitRecords,
    set<pair<const CGFunction *, const CGFunction *>> &touchedEdges) {
  ...
  // printf("<<<\n");
  //  traverse over the callees and compute the list of their synthesized
  //  attributes (fromCallees)
  for (CGFunction::iterator it = fd->successors(); it != fd->end(); it++) {
    const CGFunction *target = it.getTarget(functions);
    ...
    // if we haven't yet touched this edge
    if (touchedEdges.find(edge) == touchedEdges.end()) {
      // add the current callee's synthesized attribute into the list for the
      // current function
  ...
  // if this function has no callees, fill fromCallees with just a single
  // default instance of SynthesizedAttribute
  ...
  // add the current function's result to the visitRecords for use by this
  // function's callers
  visitRecords[fd] = res;
  // printf(">>>\n");
...
void TraverseCallGraphDataflow::addToRemaining(const CGFunction *func) {
  for (list<const CGFunction *>::iterator it = remaining.begin();
       it != remaining.end(); it++)
...
public:
  ...
  numCallersAnnotator_Int() { val = 0; }

  numCallersAnnotator_Int(int val) { this->val = val; }
};

class numCallersAnnotator
    : public virtual TraverseCallGraphTopDown<numCallersAnnotator_Int> {
public:
  numCallersAnnotator(SgIncidenceDirectedGraph *graph)
      : TraverseCallGraphTopDown<numCallersAnnotator_Int>(graph) {}

  numCallersAnnotator_Int visit(const CGFunction *func,
                                list<numCallersAnnotator_Int> &fromCallers) {
    // printf("visit: fromCallers.size()=%d\n", fromCallers.size());

    int totalCallers = 0;
    for (list<numCallersAnnotator_Int>::iterator it = fromCallers.begin();
         it != fromCallers.end(); it++) {
      totalCallers += (*it).val;
    }
    // printf("annotating %s / 0x%x\n", func->get_name().str(),
    // func->get_declaration());
    func->get_declaration()->addNewAttribute(
        "numCallers", new numCallersAttribute(totalCallers));
...
// annotates every fucntion's SgFunctionDefinition node with a
// numCallersAttribute that contains the number of functions that call the given
// function
...
// returns the number of functions that call this function or 0 if the function
// is compiler-generated
int getNumCallers(const Function *func) {
  ROSE_ASSERT(calledAnnotateNumCallers);

  if (func->get_declaration()->get_file_info()->isCompilerGenerated())
    return 0;
  ...
  return ((numCallersAttribute *)func->get_declaration()->getAttribute(
              "numCallers"))
      ->getNumCallers();
}
