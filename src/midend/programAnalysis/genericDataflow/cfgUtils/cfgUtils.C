...
SgProject *getProject() { return project; }

// parses a logical condition, determines whether it is in the form (x <= y + c)
// and if it is, sets x, y, c to be the unique ids of the relevant variables If
// the condition is in a similar form (ex: x<y, x>c), x, y and c are normalized
// to the form x<=y+c negX=true if x is supposed to be negated and false
// otherwise negY=true if y is supposed to be negated and false otherwise
bool computeTermsOfIfCondition_LTEQ(SgExpression *expr, varID &x, bool &negX,
                                    varID &y, bool &negY, long &c) {
  // skip if this is not a binary comparison condition
  SgBinaryOp *b_cond = isSgBinaryOp(expr);
  // printf("computeTermsOfIfCondition() A\n");
  if (!b_cond || (b_cond->variantT() != V_SgLessOrEqualOp &&
  ...
  {
  x = zeroVar;
  if(varID::isValidVarExp(rhs)) // if the rhs is a variable reference of dot
  expression
  {
  ...
  parseAddition(rhs, y, c)); if(!parseAddition(rhs, y, c))
  {
  ...
  */
  // if the comparison is >/>=, rather than </<=, exchange x and y and flip the
  // sign of c
...
#ifdef DEBUG_FLAG1
  cout << "TERMS OF CONDITION ARE x = " << x.getString()
       << "\ty = " << y.getString() << "\tc = " << c;
#endif
  return true;
}
...
// parses a logical condition, determines whether it is in the form (x == y + c)
// and if it is, sets x, y, c to be the unique ids of the relevant variables
// negX=true if x is supposed to be negated and false otherwise
// negY=true if y is supposed to be negated and false otherwise
bool computeTermsOfIfCondition_EQ(SgExpression *expr, varID &x, bool &negX,
                                  varID &y, bool &negY, long &c) {
  // skip if this is not a binary comparison condition
  ...
  if (!b_cond) {
#ifdef DEBUG_FLAG1
    cout << "Not a binary equality comparison condition: "
         << expr->unparseToString() << " has type: " << expr->sage_class_name()
         << "\n";
#endif
    return false;
  }
...
// wrIndex - map of i+c pairs that contains for each variable in arrays all the
// indexes
//    used to access the array variable for a Write operation
// rdIndex - map of i+c pairs that containsfor each variable in arrays all the
// indexes
//    used to access the array variable for a Read operation
// rdFromExp - map of flags indicating for each variable in arrays whether
// ast_node
//    contains an array Read expression of this array with its index in the
//    wrong format: something other than i or i+c
// wrFromExp - map of flags indicating for each variable in arrays whether
// ast_node
//    contains an array Write expression with its index in the wrong format:
//    something other than i or i+c
bool findArrayAccesses(SgNode *ast_node, m_varID2varID2quad &wrIndex,
                       m_varID2varID2quad &rdIndex, m_varID2bool &rdFromExp,
                       m_varID2bool &wrFromExp, varIDSet arrays,
                       m_varID2str vars2Name) {
  ROSE_ABORT();
  /*  SgNode* lhs, rhs;
  // indicates whether this is an expression that reads the left-hand-side (i.e.
i++) bool readlhs;
...
#ifdef DEBUG_FLAG1X
cout << "        LHS accesses: " << arraysLhs.size() << "\tRHS accesses: " <<
arraysRhs.size() << "\n"; #endif
  // iterate over all the array accesses on the left-hand-side
  for ( Rose_STL_Container<SgNode *>::iterator it = arraysLhs.begin(); it !=
arraysLhs.end(); it++ )
  {
  ROSE_ASSERT ( *it );
  SgPntrArrRefExp *arr = isSgPntrArrRefExp( *it );
  ROSE_ASSERT ( arr );
  // convert the lhs of the array access expression into either a variable
reference or
  // a dot expression
...
// parsing rhs
for ( Rose_STL_Container<SgNode *>::iterator it = arraysRhs.begin(); it !=
arraysRhs.end(); it++ )
{
ROSE_ASSERT ( isSgPntrArrRefExp( *it ) );
// convert the rhs of the array access expression into either a variable
reference or
// a dot expression
...
{
#ifdef DEBUG_FLAG1X
  cout << "        C RHS vars2Name["<<v<<"]="<<vars2Name[v]<<"
curArray="<<vars2Name[*curArray]<<"\n"; #endif

  // if we care about the array being accessed
  if ( vars2Name[v] == vars2Name[*curArray] )
  {
#ifdef DEBUG_FLAG1X
    cout << "        C.1 access found!  parseAddition( isSgBinaryOp( *it
)->get_rhs_operand(), &j, &c )="<<parseAddition( isSgBinaryOp( *it
)->get_rhs_operand(), &j, &c )<<"\n"; #endif
...
for ( set<variable>::iterator curArray = arrays.begin();
  curArray != arrays.end(); curArray++ )
cout << "findArrayAccesses END
rdIndex["<<vars2Name[*curArray]<<"].size()="<<rdIndex[*curArray].size()<<"
wrIndex["<<vars2Name[*curArray]<<"].size()="<<wrIndex[*curArray].size()<<"
found["<<vars2Name[*curArray]<<"]="<<found<<"
rdFromExp["<<vars2Name[*curArray]<<"]="<<rdFromExp[*curArray]<<"
wrFromExp["<<vars2Name[*curArray]<<"]="<<wrFromExp[*curArray]<<"\n"; #endif
return found;*/
}

// examines the given expression.
// If it is an array access, fills wrIndex with the ids of variables on the
// left-hand-side
//    of the assignment, fills rdIndex with the ids of variables on the
//    right-hand-side of the assignment and returns true.
// Otherwise, returns false.
// wrIndexSimp, rdIndexSimp - maps of array variables to variable/constant
// pairs. Each pair
//    contains i,c that represent the fact that the given array variable was
//    accessed (for writing or reading, respectively using the index expression
//    [i+c], where i is a variable and c is a constant
// wrIndexCpx, rdIndexCpx -  lists of array variables that were accessed (for
// writing or
//    reading, respectively) using a complex index expression
bool parseArrayAccess(SgNode *ast_node, m_varID2varID2quad &wrIndexSimp,
                      varIDlist &wrIndexCpx, m_varID2varID2quad &rdIndexSimp,
                      varIDlist &rdIndexCpx, m_varID2str vars2Name) {
  ROSE_ABORT();
  /*
     SgNode* lhs, rhs;
  // indicates whether this is an expression that reads the left-hand-side (i.e.
...
cout << "parseArrayAccess B LHS vars2Name["<<v<<"]="<<vars2Name[v]<<"\n";
  ...
  {
  // add v as an array variable that is being accessed using a complex index
expression wrIndexCpx.push_front(v);
...
// return true if this expression involved an array access (use or def) and
false otherwise return (arraysLhs.size() + arraysRhs.size() > 0);*/
}
...
/*SgExpression**/ bool isAssignment(SgNode *n) {
  if (isSgAssignOp(n) || isSgPlusAssignOp(n) || isSgMinusAssignOp(n) ||
      isSgMultAssignOp(n) || isSgDivAssignOp(n) || isSgMinusMinusOp(n) ||
      isSgPlusPlusOp(n) /* || isSgAssignInitializer(n)*/)
  ...
  // variable declaration with an initalization
    ...
    // return
    // isSgExpression(isSgAssignInitializer(isSgVariableDeclaration(n)->get_definition()->get_vardefn()->get_initializer())->get_operand());
    return true;
  else
    // return NULL;
    return false;
...
// if the given SgNode is an assignment operation, returns an SgExpression that
// is the left-hand side of this assignment and NULL otherwise
SgNode *getAssignmentLHS(SgNode *n) {
  ...
  if (isAssignment(n)) {
    ...
    /*else if(isSgAssignInitializer(asgn))
      ...
      if (isSgAssignInitializer(isSgVariableDeclaration(n)
                                    ->get_definition()
                                    ->get_vardefn()
                                    ->get_initializer()))
        return isSgVariableDeclaration(n)->get_definition()->get_vardefn();
      /*else if(SgAggregateInitializer
        (isSgVariableDeclaration(n)->get_definition()->get_vardefn()->get_initializer()))
        return isSgVariableDeclaration(n)->get_definition()->get_vardefn();*/
      else
        return NULL;
    }
...
// if the given SgNode is an assignment operation, adds to rhs the set of
// SgNodes that comprise the right-hand side of this assignment
void getAssignmentRHS(SgNode *n, set<SgNode *> &rhs) {
  ...
  if (isAssignment(n)) {
    if (isSgBinaryOp(n)) {
      ...
      if (isSgPlusAssignOp(n) || isSgMinusAssignOp(n) || isSgMultAssignOp(n) ||
          isSgDivAssignOp(n))
    ...
    /*else if(isSgAssignInitializer(asgn))
      ...
      if (isSgAssignInitializer(isSgVariableDeclaration(n)
                                    ...
                                    ->get_vardefn()
                                    ->get_initializer())) {
        rhs.insert(isSgAssignInitializer(isSgVariableDeclaration(n)
                                             ->get_definition()
                                             ->get_vardefn()
                                             ->get_initializer())
                       ->get_operand());
      ...
      /*else if(SgAggregateInitializer
        (isSgVariableDeclaration(n)->get_definition()->get_vardefn()->get_initializer()))
        return
        SgAggregateInitializer(isSgVariableDeclaration(n)->get_definition()->get_vardefn()->get_initializer())->get_initializers();*/
    }
  }
...
//    and sets op, i, j, k and c appropriately to represent an expression of the
//    form i = j op k op c where op may be either + (add), * (mult) or /
//    (divide) op may be = none if the rhs has only one term
// This function parses expressions such as i = j op k, i = j op c, i op= j,
// i++)
bool parseAssignment(/*SgExpression*/ SgNode *expr, short &op, varID &i,
                     varID &j, bool &negJ, varID &k, bool &negK, long &c) {
  SgBinaryOp *binOp;
  SgUnaryOp *upOp;
  ...
  if ((binOp = isSgBinaryOp(expr))) {
    ...
    if (isSgAssignOp(binOp)) {
      ...
      // varID::isValidVarExp(lhs), lhs->class_name().c_str(),
      // lhs->unparseToString().c_str());
      //  if the lhs is not a variable that we can understand, break
      if (!varID::isValidVarExp(lhs))
        return false;

      /*                        printf("parseAssignment isSgAssignOp  valid
         rhs=%d (%s)\n", parseAddition(rhs, j, c), lhs->class_name().c_str());
                                printf("parseAssignment isSgAssignOp  i=%s j=%s
         c=%d  &i=0x%x &j=0x%x\n", i.str().c_str(), j.str().c_str(), c, &i,
         &j);*/
      ...
      /*printf("1-------\n");
        ...
        &i); printf("parseAssignment isSgAssignOp  j=%s &j=0x%x\n",
        j.str().c_str(), &j); printf("1-------\n");*/
    }
    // if this is +=, *= or /= update operation
    else if (isSgPlusAssignOp(binOp) || isSgMinusAssignOp(binOp) ||
      ...
      if (isSgPlusAssignOp(binOp) || isSgMinusAssignOp(binOp))
        op = add;
  ...
  // !!!! This should eventually be removed as it has been superceded by the
  // case below
  else if (isSgAssignInitializer(expr)) {
    SgAssignInitializer *asgnInit = isSgAssignInitializer(expr);
    // printf("parseAssignment() isSgAssignInitializer:
    // asgnInit->get_operand_i() = <%s | %s>\n",
    // asgnInit->get_operand_i()->class_name().c_str(),
    // asgnInit->get_operand_i()->unparseToString().c_str());
    ...
    if ((asgnInit = isSgAssignInitializer(isSgVariableDeclaration(expr)
                                              ->get_definition()
                                              ->get_vardefn()
                                              ->get_initializer()))) {
      if (IsConstInt(asgnInit->get_operand(), c)) {
        SgInitializedName *initName =
            isSgVariableDeclaration(expr)->get_definition()->get_vardefn();
        ROSE_ASSERT(initName);
        i.init(initName);
        j.init(zeroVar);
        k.init(zeroVar);
...
// Returns true if the expression is of the permitted type
//    and sets op, i, j and c appropriately to represent an expression of the
//    form i op j op c where op may be either + (add), * (mult) or / (divide) op
//    may be = none if the rhs has only one term
// This function parses non-assignment expressions such as i op j or c but not i
// = j op c, i op= j or i++
bool parseExpr(const SgExpression *expr, short &op, varID &i, bool &negI,
               varID &j, bool &negJ, long &c) {
...
bool parseAddition(const SgExpression *expr, varID &j, bool &negJ, varID &k,
                   bool &negK, long &c) {
  ...
  if (varID::isValidVarExp(expr)) {
    ...
    if (IsConstInt(isSgUnaryOp(expr)->get_operand(), c) && isSgMinusOp(expr)) {
      c = -c;
      j = zeroVar;
      k = zeroVar;
    } else
    ...
    if (varID::isValidVarExp(
            unwrapCasts(isSgBinaryOp(expr)->get_lhs_operand()))) {
      // j +/- c?
      // printf("   parseAddition add/subtract const rhs=%d\n", IsConstInt(
      // isSgBinaryOp(expr)->get_rhs_operand(), c));
      ...
      // j +/- k
      else if (varID::isValidVarExp(
                   unwrapCasts(isSgBinaryOp(expr)->get_rhs_operand()))) {
    ...
    // c +/- ???
    else if (IsConstInt(unwrapCasts(isSgBinaryOp(expr)->get_lhs_operand()),
                        c)) {
      // c +/- j
      if (varID::isValidVarExp(
              unwrapCasts(isSgBinaryOp(expr)->get_rhs_operand()))) {
...
//    (expressions accepted: c, -c, j, c * j, j * c, j * k
// and sets *j, *k and *c appropriately
bool parseMultiplication(const SgExpression *expr, varID &j, varID &k,
                         long &c) {
...
// returns true if the given SgValueExp is some type of integral value, rather
// than a string or something more complex
/*bool isIntegralVal(SgValueExp* exp)
  {
  switch(exp->variantT)
  {
  case V_SgBoolVal: case V_SgCharVal: case V_SgDoubleVal: case V_SgEnumVal:
  case V_SgFloatVal: case V_SgIntVal: case V_SgLogDoubleVal: case
V_SgLongIntVal: case V_SgLongLongIntVal: case V_SgShortVal: case V_SgStringVal:
case V_SgUnsignedCharVal: case V_SgUnsignedIntVal: case V_SgUnsignedLongLongVal:
case V_SgUnsignedLongVal: case V_SgUnsignedShortVal: case V_SgWcharVal: return
true;
...
case V_SgLongLongIntVal: case V_SgShortVal: case V_SgStringVal: case
V_SgUnsignedCharVal: case V_SgUnsignedIntVal: case V_SgUnsignedLongLongVal: case
V_SgUnsignedLongVal: case V_SgUnsignedShortVal: case V_SgWcharVal: return
(int)exp->get_value();
...
// sets val to be the numeric value of that integer (all integer types are
// included but not floating point, characters, etc.)
bool IsConstInt(const SgExpression *rhs, long &val) {
  ...
  /*SgCastExp* cast;
    ...
    {
    printf("cast = %s: %s\n", cast->get_type()->unparseToString().c_str(),
    cast->get_type()->class_name().c_str());
    }*/
...
// pulls off all the SgCastExps that may be wrapping the given expression,
// returning the expression that is being wrapped
...
// returns the DataflowNode that represents that start of the CFG of the given
// function's body
DataflowNode cfgUtils::getFuncStartCFG(SgFunctionDefinition *func,
                                       bool (*f)(CFGNode) /*= defaultFilter*/) {
  return DataflowNode(func->cfgForBeginning(), f);
}

// returns the DataflowNode that represents that end of the CFG of the given
// function's body
...
// returns a string containing a unique name that is not otherwise used inside
// this project
string cfgUtils::genUniqueName() {
  string name = "temp_";

  Rose_STL_Container<SgNode *> initNames =
      NodeQuery::querySubTree(project, V_SgInitializedName);
  for (Rose_STL_Container<SgNode *>::iterator it = initNames.begin();
       it != initNames.end(); it++) {
    SgInitializedName *curName = isSgInitializedName(*it);
    ROSE_ASSERT(curName);
    // while our chosen "unique" name conflicts with the current
    // SgInitializedName keep adding random numbers to the end of the the
    // "unique" name until it becomes unique
    //          printf("SgInitializedName: name<%s> ==
    //          curName->get_name().getString()<%s> = %d\n", name.c_str(),
    //          curName->get_name().getString().c_str(), name ==
    //          curName->get_name().getString());
  ...
  for (Rose_STL_Container<SgNode *>::iterator it = funcDecls.begin();
       it != funcDecls.end(); it++) {
    SgFunctionDeclaration *curDecl = isSgFunctionDeclaration(*it);
    ROSE_ASSERT(curDecl);
    assert(!isSgTemplateFunctionDeclaration(curDecl));
    // while our chosen "unique" name conflicts with the current
    // SgFunctionDeclaration keep adding random numbers to the end of the the
    // "unique" name until it becomes unique
    //          printf("SgFunctionDeclaration: name<%s> ==
    //          curDecl->get_name().getString()<%s> = %d\n", name.c_str(),
    //          curDecl->get_name().getString().c_str(), name ==
    //          curDecl->get_name().getString());
...
SgFunctionDeclaration *cfgUtils::getFuncDecl(string name) {
  ...
  for (Rose_STL_Container<SgNode *>::iterator it = funcDecls.begin();
    ...
    if (!isSgTemplateFunctionDeclaration(*it) &&
        name == curDecl->get_name().getString()) {
      return curDecl;
    }
  }
  return NULL;
...
SgFunctionDefinition *cfgUtils::funcDeclToDef(SgFunctionDeclaration *decl) {
  ...
  if (decl->get_definition())
    ...
    for (Rose_STL_Container<SgNode *>::iterator it = funcDefs.begin();
         it != funcDefs.end(); it++) {
      ROSE_ASSERT(isSgFunctionDefinition(*it));
      // if the current definition has the same mangled name as the function
      // declaration, we've found it
      if (isSgFunctionDefinition(*it)->get_mangled_name() ==
          decl->get_mangled_name())
