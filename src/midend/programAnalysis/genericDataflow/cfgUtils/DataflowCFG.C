...
std::string DataflowNode::str(std::string indent) const {
  ...
  outs << "<" << getNode() << " | " << getNode()->class_name() << " | "
       << getNode()->unparseToString() << " | " << getIndex() << ">";
  return outs.str();
}
...
// nodes which are to be filtered out. The method used is to connect them into
// CFG paths so src and dest nodes of each path are interesting, skipping
// intermediate filtered nodes)
vector<DataflowEdge> makeClosureDF(
    const vector<CFGEdge> &orig, // raw in or out edges to be processed
    vector<CFGEdge> (CFGNode::*closure)()
        const, // find successor edges from a node, CFGNode::outEdges() for
               // example
    CFGNode (CFGPath::*otherSide)()
        const, // node from the other side of the path: CFGPath::target()
    CFGPath (*merge)(const CFGPath &,
                     const CFGPath &), // merge two paths into one
    bool (*filter)(CFGNode))           // filter function
{
  ...
  top:
    ...
    for (size_t i = 0; i < currentPaths.size();
         ++i) { // check each of the current paths
      ...
      if (!filter((currentPaths[i].*otherSide)())) {
        unsigned int oldSize =
            currentPaths.size(); // the number of unique paths before merge
        ...
        for (unsigned int j = 0; j < currentPaths2.size(); ++j) {
          ...
          if (std::find(currentPaths.begin(), currentPaths.end(), merged) ==
              currentPaths.end()) { // find a new path? push it to the working
                                    // set of initial edges
            currentPaths.push_back(
                merged); // a new path will be inserted. Old path ending with
                         // non-interesting node still exists
          }
        }
  ...
  for (vector<CFGPath>::const_iterator i = currentPaths.begin();
  ...
  //     e.target().getNode()->class_name().c_str());
#ifndef NDEBUG
  for (vector<DataflowEdge>::const_iterator i = edges.begin(); i != edges.end();
       ++i) {
...
bool isDataflowInteresting(CFGNode cn) {
  ...
  return (cn.getNode()->cfgIsIndexInteresting(cn.getIndex()) &&
         ...
         // The line breaks in the following logic clause make it look like the
