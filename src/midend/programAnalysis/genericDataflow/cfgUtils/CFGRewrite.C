...
void initCFGRewrite(SgProject *project) {
  // add the annotations that identify each part of the AST as used for read or
  // write accesses
  addRWAnnotations(project);
}

// returns true if the given SgNode is a value expression or a computational
...
public:
  bool evaluateSynthesizedAttribute(SgNode *n,
                                    SynthesizedAttributesList children) {
    // if n has side-effects
    if (!isNonMutatingOperator(n))
      return true;
...
// if the given SgNode is an SgStatement, returns that SgStatement. Otherwise,
// if it is an
//    gExpression or SgInitializedName, wraps it in an SgStatement and returns
//    that.
...
// Replace the expression from with the expression to in the SgNode parent,
// which
//    must be from's parent. Function checks to ensure that it is used properly.
// Note that if parent is an SgInitializedName, to must be SgInitializer in
// order
//    for the replacement to work properly.
void replaceExpressionChecked(SgNode *parent, SgExpression *from,
                              SgExpression *to) {
  ROSE_ASSERT(from);
  ROSE_ASSERT(to);
  ROSE_ASSERT(parent);
  ...
  if (!foundFrom) {
    cerr << "foundFrom failed: looking for " << from->unparseToString()
         << " in " << parent->unparseToString() << endl;
  ...
  if (isSgExprStatement(parent)) {
    ...
    // ensure that we're only trying to replace the for statement's increment
    // expression as the for statement's other components are statements, not
    // expressions
    ROSE_ASSERT(isSgForStatement(parent)->get_increment() == from);
    isSgForStatement(parent)->set_increment(to);
    to->set_parent(parent);
  } else {
    cerr << "replaceExpressionChecked: parent is <" << parent->class_name()
         << " | " << parent->unparseToString() << ">, from is <"
         << from->class_name() << "> \n";
...
string getFileInfoString(CFGNode n) { return getFileInfoString(n.getNode()); }
...
SgExpression *insertBeforeExpression(SgExpression *expr,
...
template <class Cont>
inline typename Cont::value_type &lastElementOfContainer(Cont &c) {
...
// newName: the name of the temporary variable. If newName=="", a random name is
// generated. varType: the type of the temporary variable, if byReference==true,
// the new variable's type is a reference type of varType
//
...
void createTmpVarInit(SgType *varType, string newName, bool byReference,
                      SgName &varName, SgInitializedName *&initName,
                      SgType *&newType, SgVariableDeclaration *&varDecl) {
...
// creates and returns a statement contains a call to the given function with no
// arguments
...
// given a SgInitializedName, returns a SgVariableSymbol for the variable
// declared in the SgInitializedName
...
// given a SgInitializedName, returns a SgVarRefExp to the variable declared in
// the SgInitializedName
...
// if byReference==true, the new variable carries the reference to from, rather
// than its value
//    (useful for expressions that identify an address to be modified, like the
//    "a" in a=???)
// Assumptions: from is not within the test of a loop or if
//              not currently traversing from or the statement it is in
SgAssignInitializer *splitExpression_GB(SgExpression *from,
                                        SgVarRefExp *&replacement,
                                        string newName, bool byReference) {
  if (!SageInterface::isCopyConstructible(from->get_type())) {
    std::cerr << "Type " << from->get_type()->unparseToString()
              << " of expression " << from->unparseToString()
              << " is not copy constructible" << std::endl;
    ROSE_ABORT();
  }
  assert(SageInterface::isCopyConstructible(
      from->get_type())); // How do we report errors?
  SgStatement *stmt = getStatementOfExpression(from);
  assert(stmt);

  // if the from's surrounding statement is the test condition of a for loop,
  // let the for loop be the surrounding statement, since we'll be inserting the
  // new variable declaration before the for loop
  if (isSgForStatement(stmt->get_parent()) &&
      isSgForStatement(stmt->get_parent())->get_test() == stmt)
  ...
  // ancestor(s)" << endl;
  for (list<SgExpression *>::iterator ai = ancestors.begin();
       ai != ancestors.end(); ++ai) {
  ...
  createTmpVarInit(from->get_type(), newName, byReference, varName, initName,
                   varType, varDecl);
  ...
  // from->sage_class_name() << endl; std::cout << " --- " <<
  // isSgExpressionRoot(fromparent)->get_operand_i()->unparseToString() << " ---
  // " << isSgExpressionRoot(fromparent)->get_statement()->unparseToString() <<
  // std::endl;

  // Create the temporary variable's initialization
  SgAssignInitializer *ai =
      new SgAssignInitializer(SgDefaultFile, from, varType);
  from->set_parent(ai);
  ...
  isSgScopeStatement(stmt->get_parent())->getStatementList();
          for(SgStatementPtrList::iterator it=siblings_ptr.begin();
  it!=siblings_ptr.end(); it++)
          {
                  printf("                sibling=<%s | %s>\n",
...
void replaceExpressionWithExpression_GB(SgExpression *from, SgExpression *to,
                                        SgNode *fromParent = NULL) {
  if (fromParent == NULL)
...
SgAssignOp *replaceExprWithAssignOp(SgExpression *orig, SgVarRefExp *lhsVar) {
  ...
  SgAssignOp *asgn =
      new SgAssignOp(SgDefaultFile, lhsVar, orig, orig->get_type());
  // copy from's access type into its corresponding temporary variable reference
...
// Creates a declaration of a new temporary variable of type varType and inserts
// it before anchor if before=true, the temporary variable in inserted before
// anchor and otherwise, after anchor sets initName to the SgInitializedName of
// the declaration
void insertVarDecl(SgStatement *anchor, SgType *varType, bool before,
                   SgInitializedName *&initName) {
  SgName varName;
  SgType *newType;
  SgVariableDeclaration *varDecl;
...
// inserts the given expression before or after a given SgVariableDeclaration
// that appears inside a SgForStatement
void insertAroundForInit(SgVariableDeclaration *n, SgExpression *newNode,
                         bool before) {
  ...
  // create a new loop initializer that only contains the declarations that
  // follow n
  ...
  // if we want to insert after n, do not leave it in the for loop's
  // initializers list
  if (!before)
    if (it != initStmts.end())
      it++;
...
string cfgRWTransaction::insertRequest::str() {
  ...
  if (type == insBef)
    ...
    c << "ins_Aft ";
  c << "<" << getTgtNode()->unparseToString() << " | "
    << getTgtNode()->class_name() << ">";
...
void cfgRWTransaction::beginTransaction() { requests.clear(); }
...
void cfgRWTransaction::transform(CFGTransform callbackFunc, SgNode *target,
                                 SgNode *newNode, void *data = NULL) {
  // printf("transform(%s)\n", newNode->unparseToString().c_str());
...
void cfgRWTransaction::insertAlong(DataflowEdge e, SgExpression *newNode) {
  // cerr << "insertAlong: " << e.source().toString() << " -> " <<
  // e.target().toString() << " at " << getFileInfoString(e.source()) << ",
  // adding " << newNode->unparseToString() << endl;
  if (e.target().inEdges().size() == 1) {
    insertBefore(e.target(), newNode);
  } else if (e.source().outEdges().size() == 1) {
    insertAfter(e.source(), newNode);
  } else
    ROSE_ASSERT(!"Cannot handle source and target with multiple edges");
}
...
void cfgRWTransaction::commitTransaction() {
  ...
  while (requests.size() > 0) {
    ...
    if (req->getType() == insBef) {
      ...
      // req->str().c_str());
      do_insertBefore(((insertRequest *)req)->getTgtNode(),
                      ((insertRequest *)req)->newNode);
    } else if (req->getType() == insAft) {
      // printf("   next request 0x%x: %d, %s\n", req, req->getType(),
      // req->str().c_str());
      do_insertAfter(((insertRequest *)req)->getTgtNode(),
                     ((insertRequest *)req)->newNode);
    } else if (req->getType() == callback) {
      // printf("   next request 0x%x: %d, %s\n", req, req->getType(),
      // req->str().c_str());
      (*((transformRequest *)req)->callbackFunc)(
          ((transformRequest *)req)->target, ((transformRequest *)req)->newNode,
...
void cfgRWTransaction::do_insertBefore(SgNode *n, SgExpression *newNode) {
  ...
  if (isSgExpression(n)) {
    if (isSgAssignInitializer(n)) {
      ...
      myStatementInsert(isSgVariableDeclaration(parent->get_parent()),
                        convertToStatement(newNode), true, true);
    } else {
      ...
      /*                      // else, if its a SgStatement, insert it
         immediately before n's surrounding statement else
         if(isSgStatement(newNode))
                              {
                                      // split off all the children expressions
         that have side-effects vector<SgNode*> children =
         n->get_traversalSuccessorContainer(); for(vector<SgNode*>::iterator it
         = children.begin(); it!=children.end(); it++) { printf("Node <%s|%s>,
         isNonMutatingSubTree=%d\n", (*it)->class_name().c_str(),
         (*it)->unparseToString().c_str(),isNonMutatingSubTree(*it));
                                              if(!isNonMutatingSubTree(*it))
                                              {
                                                      SgVarRefExp* replacement;
                                                      SgAssignInitializer* init
         = splitExpression_GB(isSgExpression(*it), replacement, "", true);
                                              }
                                      }

                                      myStatementInsert(parentStmt,
         isSgStatement(newNode), true, true);
                              }*/
    }
  } else if (isSgInitializedName(n)) {
    ...
    n->unparseToString().c_str(), n->class_name().c_str()); printf(" parent: <%s
    | %s>\n", n->get_parent()->unparseToString().c_str(),
    n->get_parent()->class_name().c_str()); printf(" initializer: 0x%x\n",
    isSgInitializedName(n)->get_initializer());*/

    /*if(isSgInitializedName(n)->get_initializer())
    {
            printf("                 initializer: <%s | %s>\n",
    isSgInitializedName(n)->get_initializer()->unparseToString().c_str(),
    isSgInitializedName(n)->get_initializer()->class_name().c_str());
            do_insertBefore(isSgInitializedName(n)->get_initializer(), newNode);
    }
    else*/
    do_insertBefore(isSgInitializedName(n)->get_parent(), newNode);

    /*** THIS DOESN'T WORK RIGHT NOW BECAUSE EACH isSgInitializedName THAT HAS A
    isSgAssignInitializer
     *** ACTUALLY GETS TWO DataflowNodes *** /
    myStatementInsert(isSgVariableDeclaration(parent),
    convertToStatement(newNode), true, true);*/
    ...
    // -Werror=maybe-uninitialized. SgVarRefExp* replacement;
    SgVarRefExp *replacement = NULL;

    // SgAssignInitializer* init =
    // splitExpression_GB(isSgReturnStmt(n)->get_expression(), replacement, "",
    // getAccessType(n)!=readAccess);
    ...
    if (isSgVariableDeclaration(n) && n->get_parent() &&
        isSgForInitStatement(n->get_parent())) {
    ...
    // if this is a function definition
      ...
      // The problem with SgFunctionDefinitions is that each function's CFG
      // starts and ends with its SgFunctionDefinition as such, if the
      // application wants to place something before a SgFunctionDefinition, it
      // just means that they want to place something immediately before the end
      // of the function.
      ...
      myStatementInsert(isSgStatement(n), convertToStatement(newNode), true,
                        true);
    }
  } else
  ...
  if (isSgExpression(n)) {
    ...
    // else, if this is a regular expression
      ...
      // if this is the only expression inside a SgExprStatement
        ...
        parent->class_name().c_str()); if(isSgForStatement(grandparent))
                printf("n=0x%x  get_increment()=0x%x  get_test()=0x%x\n", n,
        isSgForStatement(grandparent)->get_increment(),
        isSgForStatement(grandparent)->get_test());*/

        // if this is the test expression inside a for loop
        if (isSgForStatement(grandparent) &&
            isSgForStatement(grandparent)->get_test() == parent) {
          // printf("insertAfter Node: (for-test) <%s | %s>\n",
          // n->unparseToString().c_str(), n->class_name().c_str());
          SgInitializedName *initName;
          // declare a new temporary variable
          insertVarDecl(isSgForStatement(grandparent),
                        isSgExpression(n)->get_type(), true, initName);
          // replace n with (temp = n)
          SgAssignOp *asgn = replaceExprWithAssignOp(
              isSgExpression(n), varRefFromInitName(initName));
          // insert newNode immediately after (temp = n)
          SgExpression *newExpr = insertAfterExpression(asgn, newNode);

          // insert a reference to the temporary variable after the assignment
          // and the newNode
          ...
          // insertAfterExpression(isSgExpression(n), newNode);
          myStatementInsert(isSgExprStatement(parent),
                            new SgExprStatement(SgDefaultFile, newNode), false,
                            true);
        }
      }
      // if this expression is the right-hand side of a comma expression
      else if (isSgCommaOpExp(parent) &&
               isSgCommaOpExp(parent)->get_rhs_operand() == isSgExpression(n)) {
        printf("insertAfter Node: (comma_rhs) <%s | %s>\n",
               n->unparseToString().c_str(), n->class_name().c_str());
        fflush(stdout);

        // get the root of the nest of SgCommaOpExp where n appears as the last
        // expression
        SgNode *grandparent = parent->get_parent();
        while (isSgCommaOpExp(grandparent) &&
               isSgCommaOpExp(grandparent)->get_rhs_operand() ==
                   isSgExpression(parent)) {
        ...
        grandparent->unparseToString().c_str(),
        grandparent->class_name().c_str());
        // if the parent SgCommaOpExp is alone inside an SgExprStatement that is
        free
        //   (e.g. not inside a SgForStatement)
        if(isSgExprStatement(grandparent) &&
        !isSgForStatement(grandparent->get_parent()))
        {
                myStatementInsert(isSgExprStatement(grandparent), new
        SgExprStatement(SgDefaultFile, newNode), false, true);
        }
        else
        {
                ...
                SgVarRefExp* replacement;
                SgAssignInitializer* init =
        splitExpression_GB(isSgCommaOpExp(parent), replacement, "", true);
                insertBeforeExpression(replacement, newNode);
        }*/
      } else {
        // SgStatement* parentStmt=getStatementOfExpression(n);
        ...
        // SgAssignInitializer* init = splitExpression_GB(isSgExpression(n),
        // replacement, "", getAccessType(n)!=readAccess);
        ROSE_ASSERT(replacement != NULL);
        insertBeforeExpression(replacement, newNode);
      }
    }
    ...
    /*printf("                        parent = <%s | %s>\n",
    n->get_parent()->unparseToString().c_str(),
    n->get_parent()->class_name().c_str()); printf(" grandparent = <%s | %s>\n",
    n->get_parent()->get_parent()->unparseToString().c_str(),
    n->get_parent()->get_parent()->class_name().c_str());*/
    ...
    // if this is a function definition
      ...
      // The problem with SgFunctionDefinitions is that each function's CFG
      // starts and ends with its SgFunctionDefinition as such, if the
      // application wants to place something after a SgFunctionDefinition, it
      // just means that they want to place something immediately after the
      // start of the function.
      SgBasicBlock *funcBody = isSgFunctionDefinition(n)->get_body();
      funcBody->insertStatementInScope(convertToStatement(newNode), true);
    } else
      myStatementInsert(isSgStatement(n), convertToStatement(newNode), false,
                        true);
  } else
    ROSE_ASSERT(!"Error: unhandled insert-before case!");
}
...
/*void insertAfter(CFGNode n, SgNode* newNode) {
        top:
        cerr << "insertAfter: " << n.toString() << " at " <<
getFileInfoString(n) << ", adding " << newNode->unparseToString() << endl; n =
walkForwardToStatementBoundary(n); if (isSgBasicBlock(n.getNode())) { // Does
not invalidate CFG node n SgStatement* s = convertToStatement(newNode);
                ...
                if (isSgForInitStatement(n.getNode())) {
                        ...
                        myStatementInsert(isSgStatement(n.getNode()), s, false,
true); } else if (isSgExpression(n.getNode())) { ROSE_ASSERT
(isSgExpression(newNode)); SgNode* oldParent = n.getNode()->get_parent(); if
(isSgExpression(oldParent) && isSgExpression(oldParent->get_parent())) { CFGNode
newN = theOutEdge(n); insertBefore(newN, newNode); return; } else ROSE_ASSERT
(!"Bad case for inserting after expression"); } else if
(isSgInitializedName(n.getNode())) { SgInitializedName* in =
isSgInitializedName(n.getNode()); ROSE_ASSERT
(!isSgForInitStatement(in->get_parent())); ROSE_ASSERT
(isSgInitializedName(newNode)); SgInitializedName* newVar =
isSgInitializedName(newNode); SgVariableDeclaration* decl =
isSgVariableDeclaration(in->get_declaration()); ROSE_ASSERT (decl); // FIXME --
should handle parameter lists SgInitializedNamePtrList& vars =
decl->get_variables(); SgInitializedNamePtrList::iterator i = find(vars.begin(),
vars.end(), in); ROSE_ASSERT (i != vars.end());
                        ++i;
                        vars.insert(i, newVar);
                        newVar->set_parent(decl);
                } else ROSE_ASSERT (!"Bad n.getNode() case");
        } else {
                cerr << "insertAfter: bad position " << n.toString() << " to
insert " << newNode->unparseToString() << endl; ROSE_ABORT();
        }
}*/
...
void prependToScopeStmt(SgNode *target, SgNode *newNode, void *data) {
  ...
  isSgScopeStatement(target)->insertStatementInScope(
      convertToStatement(newNode), true);
}
...
void appendToScopeStmt(SgNode *target, SgNode *newNode, void *data) {
  ...
  isSgScopeStatement(target)->insertStatementInScope(
      convertToStatement(newNode), false);
}
