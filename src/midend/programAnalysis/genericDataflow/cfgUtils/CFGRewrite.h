...
// inserts the given expression before or after a given SgDeclaration that
// appears inside a SgForStatement
...
class cfgRWTransaction {
  ...
  public:
    transformRequest(CFGTransform callbackFunc, SgNode *&target,
                     SgNode *newNode, void *data) //: modRequest(callback)
    {
      this->type = callback;
      this->target = target;
      this->newNode = newNode;
...
public:
  ...
  void transform(CFGTransform callbackFunc, SgNode *n, SgNode *newNode,
                 void *data);
