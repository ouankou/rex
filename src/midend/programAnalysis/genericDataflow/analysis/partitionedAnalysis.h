...
// The original analysis that was split is explicitly identified as the "master"
// of the split.
class partSplit {
public:
  ...
  void addChild(IntraPartitionDataflow *child) { splitSet.insert(child); }
  ...
  std::string str(std::string indent = "") {
    ...
    oss << indent << "    splitSet = <";
    for (std::set<IntraPartitionDataflow *>::iterator it = splitSet.begin();
         it != splitSet.end();) {
      oss << (*it);
      it++;
      if (it != splitSet.end())
...
class PartitionedAnalysis : virtual public IntraProceduralAnalysis {
  ...
  // the set of partitions that have called join and are simply waiting to be
  // joined to the master partitions that they split from
  ...
  std::map<IntraPartitionDataflow *, IntraPartitionDataflowCheckpoint *>
      parts2chkpts;

  // Sample interprocedural analysis object that we'll use as a factory to
...
public:
  ...
  // Splits the given dataflow analysis partition into several partitions, one
  // for each given checkpoint The partition origA will be assigned the last
  ...
  // current split. If newPartActive==true, the newly-generated partitions will
  // be made initially active. If not,
  //    they will start out in joined status.
  // Returns the set of newly-created partitions.
  std::set<IntraPartitionDataflow *>
  split(IntraPartitionDataflow *origA,
  ...
  void join(IntraPartitionDataflow *joinA,
            IntraPartitionDataflowCheckpoint *chkpt, const Function &func,
            NodeState *fState);

  // Called by the base PartitionedAnalysis class when all partitions in a given
  // split have decided
  //    to join to decide whether they should be joined or all released. It may
  //    also do some processing of their state prior to the release or join.
  // Returns the set of partitions that will remain in joined status after this
  // join. If all partitions in the split
  //    set are on this list, they are all joined(all but one will be deleted).
  //    Any remaining partitions will be released.
  virtual std::set<IntraPartitionDataflow *>
  preJoin(partSplit *s, const Function &func, NodeState *fState,
...
// a given function so that the target analysis gets copies of the source
// analysis' state.
...
// together the dataflow states associated on each CFG node with each of these
// analyses. The results are associated on each CFG node with the master
// analysis.
class unionDFAnalysisStatePartitions
    : virtual public UnstructuredPassIntraAnalysis {
  std::set<Analysis *> unionSet;
  Analysis *master;

public:
  unionDFAnalysisStatePartitions(std::set<Analysis *> &unionSet,
                                 Analysis *master) {
    for (std::set<Analysis *>::iterator it = unionSet.begin();
         it != unionSet.end(); it++) {
...
public:
  ...
  void visit(const Function &func, const DataflowNode &n, NodeState &state) {
    for (std::set<IntraPartitionDataflow *>::iterator it = tgtA.begin();
         it != tgtA.end(); it++)
...
public:
  // The logic expression that describes the invariant that holds for this
  // partition
  /*LogicalCond*/ printable *partitionCond;

  IntraPartitionDataflow(const IntraPartitionDataflow &that) : Analysis() {
    parent = that.parent;
  ...
  ~IntraPartitionDataflow() { delete partitionCond; }
  ...
  PartitionedAnalysis *getParent() const { return parent; }
  ...
  // Creates a new instance of the derived object that is a copy of the original
  // instance. This instance will be used to instantiate a new partition of the
  // analysis.
  virtual IntraPartitionDataflow *copy() = 0;

  virtual bool runAnalysis(const Function &func, NodeState *fState,
                           bool &splitPart, bool &joinPart,
                           IntraPartitionDataflowCheckpoint *&outChkpt) = 0;
  ...
  // joinPart - set by the call to indicate that it exited because it wishes to
  // join the partitions that it split from outChkpt - set by the call to be the
  // checkpoint that representing the state of this analysis at the time of the
  // exit
  //            set to NULL in the case of a normal exit or a split exit
  virtual bool
  runAnalysisResume(const Function &func, NodeState *fState,
                    IntraPartitionDataflowCheckpoint *chkpt, bool &splitPart,
                    bool &joinPart,
                    IntraPartitionDataflowCheckpoint *&outChkpt) = 0;

  // Called when a partition is created to allow a specific analysis to
  // initialize its dataflow information from the partition condition
  virtual void initDFfromPartCond(const Function &func, const DataflowNode &n,
                                  NodeState &state,
...
class IntraPartitionDataflowCheckpoint {
public:
  // A checkpoint of the dataflow state associated with the given state of the
  // dataflow analysis
  ...
  // The DataflowNode that that analysis was processing when the checkpoint was
  // taken
  DataflowNode *curNode;
  // The logical condition that is an invariant of all the states of the
  // dataflow analysis
  /*LogicalCond*/ printable *partitionCond;
  // Given that the current node in the dataflow analysis has one or more
  // successors, the index of the given node's successor
  int partitionIndex;
  ...
  IntraPartitionDataflowCheckpoint(dataflow::checkpoint &dfChkpt,
                                   const std::set<DataflowNode> &joinNodes,
                                   const DataflowNode *curNode,
                                   /*LogicalCond*/ printable *partitionCond,
                                   int partitionIndex, const Function &func,
                                   NodeState *fState)
  ...
  std::string str(std::string indent = "") {
    ...
    outs << indent << "    dfChkpt = \n"
         << dfChkpt.str(indent + "    ") << "\n";
    if (curNode)
      outs << indent << "    curNode = <" << curNode->getNode()->class_name()
           << " | " << curNode->getNode()->unparseToString() << " | "
           << curNode->getIndex() << ">\n";
    else
    ...
    if (joinNodes.size() == 0)
      ...
      for (std::set<DataflowNode>::iterator it = joinNodes.begin();
           it != joinNodes.end(); it++) {
    ...
    if (partitionIndex >= 0)
      ...
      outs << indent << "    partitionIndex = all descendants ("
           << partitionIndex << ")]";
    return outs.str();
  }
};
...
public:
  ...
  //    - splitNew if the analysis wants to perform a split and place the
  //    newly-generated partitions into
  //      a fresh split set that is nested inside this partition's current split
  //    - splitParent if the analysis wants to perform a split and place the
  //    newly-generated partitions
  //      into this partition's current split (i.e. on the same split level as
  //      the current partition)
  typedef enum { noSplit, splitNew, splitParent } splitType;
  // splitConditions - if splitAnalysis==splitNew or ==splitParent, the analysis
  // sets this vector to the conditions for all the
  //                   descendant CFG nodes in the split
  /* // joinAnalysis - set to true if the analysis wants to perform a join */
  // joinNode - set to true if progress along the given dataflow node needs to
  // be blocked until the next join point.
  //            If all paths of dataflow progress are blocked in this analysis,
  //            this is the same as the analysis requesting to be joined.
  // Returns true if any of the input lattices changed as a result of the
  // transfer function and
  //    false otherwise.
  virtual bool
  transfer(const Function &func, const DataflowNode &n, NodeState &state,
           const std::vector<Lattice *> &dfInfo, splitType &splitAnalysis,
           std::vector</*LogicalCond*/ printable *> &splitConditions,
           /*bool& joinAnalysis, */ bool &joinNode) = 0;
  ...
  // false otherwise. state - the function's NodeState
  bool runAnalysis(const Function &func, NodeState *fState, bool &splitPart,
  ...
  partitionTranferRet
  partitionTranfer(const Function &func, NodeState *fState,
                   const DataflowNode &n, NodeState *state,
                   VirtualCFG::dataflow &dfIt,
                   const std::vector<Lattice *> &dfInfoBelow, bool &splitPart,
                   std::set<DataflowNode> &joinNodes,
                   IntraPartitionDataflowCheckpoint *&outChkpt);

  // propagates the forward dataflow info from the current node's NodeState
  // (curNodeState) to the next node's NodeState (nextNodeState)
  bool propagateFWStateToNextNode(const std::vector<Lattice *> &curNodeState,
