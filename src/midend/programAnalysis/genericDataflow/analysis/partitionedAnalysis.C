...
// master analysis partition and creating an initial split that only contains
// the master.
...
// Returns a reference to the master dataflow analysis. At the end of the
// partitioned analysis, it is this object that owns all the dataflow state
// generated by the overall analysis.
IntraPartitionDataflow *PartitionedAnalysis::getMasterDFAnalysis() {
  // printf("parts2splits() = %d\n", parts2splits.size());
  return parts2splits.begin()->first;
}

// Activates the given partition, returning true if it was previously inactive
// and false otherwise.
...
// Splits the given dataflow analysis partition into several partitions, one for
// each given checkpoint. The partition origA will be assigned the last
// checkpoint in partitionChkpts. If newSplit==true, this split operation
// creates a new split within origA's current split and place
//    the newly-generated partitions into this split.
// If newSplit==false, the newly-generated partitions will be added to origA's
// current split. If newPartActive==true, the newly-generated partitions will be
// made initially active. If not,
//    they will start out in joined status.
// Returns the set of newly-created partitions.
set<IntraPartitionDataflow *> PartitionedAnalysis::split(
    IntraPartitionDataflow *origA,
    vector<IntraPartitionDataflowCheckpoint *> partitionChkpts,
    const Function &func, NodeState *fState, bool newSplit,
    bool newPartActive) {
  /*printf("PartitionedAnalysis::split() origA=%p\n", origA);

  for(vector<IntraPartitionDataflowCheckpoint*>::iterator it =
  partitionChkpts.begin(); it!=partitionChkpts.end(); it++) { printf("
  chkpt=%s\n", (*it)->str("    ").c_str()); }*/
  ...
  // Only partitions that are either active or joined may call split. Joined
  // partitions may do so during the joining process, which can result in the
  ...
  cout << "split = " << split->str()
       << ", partitionChkpts.size()=" << partitionChkpts.size() << "\n";
  fflush(stdout);

  set<IntraPartitionDataflow *> newParts;

  // Generate the new analysis partitions. The last checkpoint goes to the
  // master partition and we create fresh partitions for the other checkpoints
  for (vector<IntraPartitionDataflowCheckpoint *>::iterator it =
           partitionChkpts.begin();
    ...
    if (it != partitionChkpts.end()) {
      ...
      // The splits list only contains the split (which may be newly-created or
      // a copy of parts2splits[origA]) since it will cease to exist after the
      // next successful join of split
      list<partSplit *> splitsList;
      splitsList.push_back(split);
      parts2splits[curPartA] = splitsList;

      // Set the current partition's logical condition (can't AND it because the
      // variables involved in the condition may have changed between the two
      // split points)
      ...
      // condition = %s\n", curPartA, origA,
      // curPartA->partitionCond->str("").c_str()); }
      {
        printf("        Creating analysis partition %p (master = %p)\n",
               curPartA, origA);
      }

      // create a copy of the original partition's dataflow state for the new
      // partition
      ...
      //// AND this the first partition's new logical condition to the original
      /// partition's condition
      // set the first partition's logical condition (can't AND it because the
      // variables involved in the condition may have changed between the two
      // split points)
      /*printf("origA->partitionCond=%p\n", origA->partitionCond);
      printf("    partitionChkpts[0]=%s\n", partitionChkpts[0]->str("
      ").c_str());*/
      ...
      /*if(origA->partitionCond != NULL)
      {
              ...
              // we already have a checkpoint object for the origA partition, so
      we can delete this new checkpoint
      ...
      // Delete the previous checkpoint, assuming that one exists AND we're not
      // reusing the old checkpoint as the new checkpoint
      ...
      origA->partitionCond->str("").c_str()); }*/
    }

    cout << "Updating current partition's dataflow state, "
            "parts2chkpts[curPartA]->joinNodes.size()="
         << parts2chkpts[curPartA]->joinNodes.size() << "\n";
    cout << "    curPartA->partitionCond=" << curPartA->partitionCond << "\n";
    fflush(stdout);
    // -----------------------------------------------------------------------------------------------
    // Update the current partition's current dataflow state (state at the nodes
    // in its joinNodes set) with its new partition condition

    // joinNodes
    /*              cout << "parts2chkpts[curPartA]->joinNodes.size()=" <<
       parts2chkpts[curPartA]->joinNodes.size() << "\n";
                    for(set<DataflowNode>::iterator
       itJN=parts2chkpts[curPartA]->joinNodes.begin();
                        itJN!=parts2chkpts[curPartA]->joinNodes.end(); itJN++)
                            cout << "    itJN =
       "<<(*itJN).getNode()->unparseToString()<<"\n";*/

    for (set<DataflowNode>::iterator itJN =
             parts2chkpts[curPartA]->joinNodes.begin();
      ...
      vector<NodeState *>::const_iterator itS = nodeStates.begin();
      {
        NodeState *state = *itS;
        Analysis *a = curPartA;
        curPartA->initDFfromPartCond(func, n, *state, state->getLatticeBelow(a),
                                     state->getFacts(a),
                                     curPartA->partitionCond);
      }
      itJN++;
    }
    ...
    if (parts2chkpts[curPartA]->curNode) {
      ...
      vector<NodeState *>::const_iterator itS = nodeStates.begin();
      {
        ...
        /*ConstrGraph* cg =
        dynamic_cast<ConstrGraph*>(state->getLatticeBelow(a).front()); cout <<
        "Pre-initDFfromPartCond CG="<<cg->str()<<"\n";*/
  ...
  // printf("    partitionChkpts[0]=%s\n", partitionChkpts[0]->str("
  // ").c_str()); printf("    parts2chkpts[origA]=%s\n",
...
// Joins all the analysis partitions in a given split into a single partition,
// unioning their respective dataflow information
void PartitionedAnalysis::join(IntraPartitionDataflow *joinA,
  ...
  if (allJoining) {
    // First check if all the joined partitions have in fact fully completed
    // their executions (a partition with a completed execution will have an
    // empty joinNodes list)
    bool allFinished = true;
    for (set<IntraPartitionDataflow *>::iterator it = s->splitSet.begin();
         it != s->splitSet.end(); it++) {
      ROSE_ASSERT(parts2chkpts[*it]);
      allFinished = allFinished && parts2chkpts[*it]->joinNodes.size() ==
                                       0; //(parts2chkpts[*it]==NULL);
    }
    ...
    if (allFinished) {
      ...
      // Check with our derived class to see if the partitions that requested
      // the join should truly be joined
    ...
    // if we're supposed to join all the split partitions back into the master
    // partition
    if (joinSplit) {
      if (analysisDebugLevel >= 1) {
      ...
      // Remove the master from the set of split partitions to simplify
      // subsequent code
      s->splitSet.erase(s->master);
      // ROSE_ASSERT(parts2chkpts.find(s->master) != parts2chkpts.end());
      // !!! May not be true in nested partition cases because after the first
      // inner join the master's partitionCond==NULL
     ...
     it!=s->splitSet.end(); it++)
      ...
      // the Disjunction of all the split partition conditions will refer to
      // variables that may have been modified since the CFG node where the
      // split occured
      ...
      for (set<IntraPartitionDataflow *>::iterator it = s->splitSet.begin();
        ...
        // This partition is no longer in joining status because it is being
        // made active (master) or is being killed (!master)
        joinParts.erase(curPart);

        // parts2splits.erase(curPart);
        //  Pop off the inner-most split associated with the joined analysis
        //  partitions
      ...
      // Else, if this split has not finished or this split's master is part of
      // a higher-level unfinished split
      else {
        // Activate the master analysis since it still needs to continue with
    ...
    // Otherwise, release all joining threads so that they can resume execution
      ...
      if (analysisDebugLevel >= 1) {
        printf(
            "        Releasing all members of split, s->splitSet.size()=%lu\n",
            (unsigned long)(s->splitSet.size()));
      }

      // Note that this mass-release does not apply to any partitions spawned by
      // the partitions in the split
      for (set<IntraPartitionDataflow *>::iterator it = s->splitSet.begin();
           it != s->splitSet.end(); it++) {
        IntraPartitionDataflow *curPart = *it;
        // If the current partition is not supposed to remained in joining
        // status
...
bool PartitionedAnalysis::runAnalysis(const Function &func, NodeState *fState) {
  ...
  while (activeParts.size() > 0) {
    ...
    if (parts2chkpts[nextA] == NULL) {
      // run it from start
      ret = nextA->runAnalysis(func, fState, splitPart, joinPart, outChkpt) ||
            ret;
    }
    // Otherwise, if it has a checkpoint
    else {
      // Resume from the checkpoint
      ret = nextA->runAnalysisResume(func, fState, parts2chkpts[nextA],
                                     splitPart, joinPart, outChkpt) ||
            ret;
    }

    // printf("joinPart=%d, activeParts=%d\n", joinPart, splitPart);

    // if the dataflow partition wants to join itself to the partitions that it
    // was split from
    if (joinPart) {
    ...
    printf("@ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ "
           "@\n");
    printf("PartitionedAnalysis::runAnalysis activeParts.size()=%lu, "
           "joinParts.size()=%lu\n",
           (unsigned long)(activeParts.size()),
           (unsigned long)(joinParts.size()));
  }

  ROSE_ASSERT(activeParts.size() == 0);
  ROSE_ASSERT(joinParts.size() == 0);
  // Return true if any partition caused the function's final dataflow state to
...
void unionDFAnalysisStatePartitions::visit(const Function &func,
                                           const DataflowNode &n,
                                           NodeState &state) {
  // printf("unionDFAnalysisStatePartitions::visit() function %s() node=<%s |
...
bool IntraPartitionFWDataflow::runAnalysis(
    const Function &func, NodeState *fState, bool &splitPart, bool &joinPart,
...
bool IntraPartitionFWDataflow::runAnalysisResume(
    const Function &func, NodeState *fState,
    IntraPartitionDataflowCheckpoint *chkpt, bool &splitPart, bool &joinPart,
    IntraPartitionDataflowCheckpoint *&outChkpt) {
  ...
  if (chkpt == NULL) {
    ...
    // funcCFGStart.getNode()->class_name().c_str(),
    // funcCFGStart.getNode()->unparseToString().c_str());
  ...
  // Set of nodes that this analysis has blocked progress on until the next join
  // point
  set<DataflowNode> joinNodes;

  // Set of nodes that were on the joinNodes list before a join and must now be
  // resumed. For each such node we must skip the call to the transfer function.
  ...
  if (chkpt) {
    ...
    // Add all the previously-joined nodes to the dataflow iterator to be
    // resumed
    for (set<DataflowNode>::iterator it2 = resumeNodes.begin();
         it2 != resumeNodes.end(); it2++)
      dfIt.add(*it2);
  ...
  for (; dfIt != VirtualCFG::dataflow::end(); dfIt++) {
    ...
    // nodeStates.size()); int i=0; NodeState* state =
    // NodeState::getNodeState(n, 0);
    ...
    vector<NodeState *>::const_iterator itS = nodeStates.begin();
    {
      ...
      // reset the modified state, since only the last NodeState's change
      // matters
      // modified = false;
      ...
      // ---------------------------------------------------------------------------------------
      // If we're restarting following a split or we're restarting from a join
      // and this dataflow node was previously blocked waiting on this join
      if (chkpt && chkpt->partitionCond) {
        goto postSplit;
      } else if (resumeNodes.find(n) != resumeNodes.end()) {
        cout << "Resuming previously joined node <"
      ...
      {
        // printf("                 dfInfoAbove.size()=%d,
        // dfInfoBelow.size()=%d, this=%p\n", dfInfoAbove.size(),
        // dfInfoBelow.size(), this);
        ...
        /*
        // If we're resuming this dataflow node from a previous join (we blocked
        on this node until the join and
        // we're now restarting from the join), do not call the transfer
        function on this dataflow node and instead
        ...
        // inter-procedural analysis to determine the effect of this function
        // call on the dataflow state.
        if (isSgFunctionCallExp(sgn)) {
          ...
          if (calledFunc.get_definition()) {
            ...
            // NEED TO INCORPORATE INFORMATION ABOUT RETURN INTO DATAFLOW
            // SOMEHOW
            ROSE_ABORT();
          } else {
            partitionTranferRet ret =
                partitionTranfer(func, fState, n, state, dfIt, dfInfoBelow,
                                 splitPart, joinNodes, outChkpt);
            if (ret == retFalse)
              return false;
            else if (ret == cont)
              continue;
        ...
        // Otherwise, call the user's transfer function
          ...
          if (ret == retFalse)
            ...
            continue;
          /*IntraPartitionFWDataflow::splitType
          splitAnalysis=IntraPartitionFWDataflow::noSplit; bool
          joinAnalysis=false, joinNode=false; vector<LogicalCond printable*>
          splitConditions; modified = transfer(func, n, *state, dfInfoBelow,
          splitAnalysis, splitConditions, joinAnalysis, joinNode);
          if(partitionCond != NULL)
                  cout << "after transfer: partitionCond =
          "<<partitionCond->str()<<"\n";
          ...
          if(splitAnalysis!=IntraPartitionFWDataflow::noSplit)
          {
                  ...
                  for(vector<LogicalCond printable*>::iterator
          splitIt=splitConditions.begin(); splitIt!=splitConditions.end();
          splitIt++, i++)
                  {
                          dataflow::checkpoint dfChkpt = it.getChkpt();
                          // Add the checkpoint that corresponds to the next
          split to the vector of checkpoints chkpts.push_back(new
          IntraPartitionDataflowCheckpoint(dfChkpt, joinNodes, *splitIt, i,
          func, fState));
          ...
          *splitIt); }
                  ...
                  if(partitionCond != NULL)
                          cout << "after loop: partitionCond =
          "<<partitionCond->str()<<"\n";

                  IntraPartitionDataflow* tmp = this;
                  printf("parent = %p, this=%p, tmp=%p\n", parent, this, tmp);

                  parent->split(tmp, chkpts, func, fState,
          splitAnalysis!=IntraPartitionFWDataflow::splitNew, true);
                  // After the split, break out of the analysis. The parent
          PartitionedAnalysis will schedule
                  // this analysis appropriately.
                  outChkpt=NULL;
                  return false;
          }
          // If the analysis wants to block progress along the current dataflow
          node. else if(joinNode)
          {
                  joinNodes.insert(n);
                  // Move on to the next dataflow node without propagating state
          to this node's descendants
                  // !!! What happens when the same dataflow node gets put onto
          joinNodes twice? It
                  // !!! shouldn't make a difference that it will be resumed
          only once.... I think. continue;
          }
          // If the analysis wants to be joined to the partitions that it was
          split from else if(joinAnalysis)
          {
                  if(analysisDebugLevel>=1)
                  ...
                  joinPart=true;
                  dataflow::checkpoint dfChkpt = it.getChkpt();
                  outChkpt = new IntraPartitionDataflowCheckpoint(dfChkpt,
          joinNodes, NULL, -1, func, fState); return false;
          }*/
        }
        // >>>>>>>>>>>>>>>>>>> TRANSFER FUNCTION >>>>>>>>>>>>>>>>>>>
      }

    // This is the point in the code where we resume execution following a
    // checkpoint
    postSplit:
      /* If we're restarting from a checkpoint after a split
      if(chkpt!=NULL && chkpt->partitionCond)
      {
              // Inform the newly-created active partition about its new
      partition condition initDFfromPartCond(func, n, *state,
      state->getLatticeBelow(this), state->getFacts(this));
      }*/

      if (analysisDebugLevel >= 1) {
        j = 0;
        for (itB = dfInfoBelow.begin(); itB != dfInfoBelow.end(); itB++, j++) {
    ...
    if (/*modified && */ n != funcCFGEnd) {
      ...
      /*cout << " first Descendant
      A:"<<(*(edges.begin())).target().getNode()->unparseToString()<<"\n";
      {
              DataflowNode desc=n;
              int i=0;
              ...
              {
                      cout << " A
      d->d:"<<(*(desc.outEdges().begin())).target().getNode()->unparseToString()<<"\n";
      ...
      if (chkpt != NULL) {
        ...
        if (chkpt->partitionIndex >= 0) {
          // Remove all entries from edges other than the one that corresponds
          // to this partition
        ...
        // Now that we've finished the restart, delete the checkpoint and reset
        // it to NULL
        // delete chkpt;
        // Now that we've finished the restart, reset the checkpoint to NULL
        chkpt = NULL;
      ...
      // cout << " first Descendant
      // B:"<<(*(edges.begin())).target().getNode()->unparseToString()<<"\n";
      ...
      for (vector<DataflowEdge>::iterator ei = edges.begin(); ei != edges.end();
        ...
        if (analysisDebugLevel >= 1)
          printf("    Descendant: %p<%s | %s>\n", nextSgNode,
                 nextSgNode->class_name().c_str(),
                 nextSgNode->unparseToString().c_str());
  ...
  outChkpt = new IntraPartitionDataflowCheckpoint(dfChkpt, joinNodes, NULL,
                                                  NULL, -1, func, fState);
  cout << "Joined DataflowNodes:\n";
  for (set<DataflowNode>::iterator itDF = outChkpt->joinNodes.begin();
       itDF != outChkpt->joinNodes.end(); itDF++) {
    DataflowNode n = *itDF;
    printf("    <%s | %s | %d>\n", n.getNode()->unparseToString().c_str(),
           n.getNode()->class_name().c_str(), n.getIndex());
  }

  joinPart = true;

  // We've finished going through all the nodes in this functions in dataflow
  // order. However, if the only reason why we're done is because the analysis
  // has chosen to block progress along some dataflow nodes until a join point,
  // we must perform this join and resume if possible.
  if (joinNodes.size() > 0) {
    if (analysisDebugLevel >= 1) {
      printf("    Joining analysis partition %p because all nodes are blocked "
             "until this join\n",
             this);
    }
    return false;
  ...
  // Else, if we're truly done with this function
    ...
    // Test if the Lattices at the bottom of the function after the forward
    // analysis are equal to their original values in the function state.
    bool modified =
        !NodeState::eqLattices((*(NodeState::getNodeStates(funcCFGEnd).begin()))
                                   ->getLatticeAbove(this),
                               fState->getLatticeBelow(/*interAnalysis*/ this));

    // Update the the function's exit NodeState with the final state of this
    // function's dataflow analysis.
    NodeState *exitState = *(NodeState::getNodeStates(funcCFGEnd).begin());
    NodeState::copyLattices_bEQa(/*interAnalysis*/ this, *fState, this,
                                 *exitState);

    return modified;
  }

  // PartitionedAnalysis::runAnalysis will join this partition with its peers
  // within its split. If all the peers are done then this split will be joined
  // together. If not, all peers will be resumed from their joinNodes sets.
}

IntraPartitionFWDataflow::partitionTranferRet
IntraPartitionFWDataflow::partitionTranfer(
    const Function &func, NodeState *fState, const DataflowNode &n,
    NodeState *state, VirtualCFG::dataflow &dfIt,
    const vector<Lattice *> &dfInfoBelow, bool &splitPart,
    set<DataflowNode> &joinNodes, IntraPartitionDataflowCheckpoint *&outChkpt) {
  IntraPartitionFWDataflow::splitType splitAnalysis =
      IntraPartitionFWDataflow::noSplit;
  bool /*joinAnalysis=false, */ joinNode = false;
  vector</*LogicalCond*/ printable *> splitConditions;
  /*modified = */ transfer(func, n, *state, dfInfoBelow, splitAnalysis,
                           splitConditions, /*joinAnalysis, */ joinNode);
  if (partitionCond != NULL)
  ...
  if (splitAnalysis != IntraPartitionFWDataflow::noSplit) {
    ...
    int i = 0;
    vector<IntraPartitionDataflowCheckpoint *> chkpts;
    for (vector</*LogicalCond*/ printable *>::iterator splitIt =
             splitConditions.begin();
         splitIt != splitConditions.end(); splitIt++, i++) {
      dataflow::checkpoint dfChkpt = dfIt.getChkpt();
      // Add the checkpoint that corresponds to the next split to the vector of
      // checkpoints
      chkpts.push_back(new IntraPartitionDataflowCheckpoint(
          dfChkpt, joinNodes, &n, *splitIt, i, func, fState));

      /*if(analysisDebugLevel>=1)
      { printf("        Split condition %d: \n%s\n", i, ((*splitIt)->str("
      ")).c_str()); }*/
      cout << "chkpt = "
           << (new IntraPartitionDataflowCheckpoint(dfChkpt, joinNodes, &n,
                                                    *splitIt, i, func, fState))
                  ->str()
           << "\n";
    }
    if (partitionCond != NULL)
    ...
    parent->split(tmp, chkpts, func, fState,
                  splitAnalysis != IntraPartitionFWDataflow::splitNew, true);
    // After the split, break out of the analysis. The parent
    // PartitionedAnalysis will schedule this analysis appropriately.
    outChkpt = NULL;
    return retFalse;
  }
  // If the analysis wants to block progress along the current dataflow node.
    ...
    joinNodes.insert(n);
    // Move on to the next dataflow node without propagating state to this
    // node's descendants
    // !!! What happens when the same dataflow node gets put onto joinNodes
    // twice? It
    // !!! shouldn't make a difference that it will be resumed only once.... I
    // think.
    return cont;
  }
  ...
  /*else if(joinAnalysis)
  {
          ...
          joinPart=true;
          dataflow::checkpoint dfChkpt = it.getChkpt();
          outChkpt = new IntraPartitionDataflowCheckpoint(dfChkpt, joinNodes,
  NULL, -1, func, fState); return false;
  }*/
  return normal;
}
...
bool IntraPartitionFWDataflow::propagateFWStateToNextNode(
  ...
  if (analysisDebugLevel >= 1) {
    printf("\n        Propagating to Next Node: %p[%s]<%s>\n",
           nextNode.getNode(), nextNode.getNode()->class_name().c_str(),
           nextNode.getNode()->unparseToString().c_str());
    int j = 0;
