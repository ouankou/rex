...
class Analysis {
public:
  // a filter function to decide which raw CFG node to show (if return true) or
  // hide (otherwise) This is required to support custom filters of virtual CFG
  // Custom filter is set inside the intra-procedural analysis.
  // Inter-procedural analysis will copy the filter from its intra-procedural
  // analysis during the call to its constructor.
...
protected:
  ...
  InterProceduralAnalysis(IntraProceduralAnalysis *intraAnalysis) {
    ...
    // inform the intra-procedural analysis that this inter-procedural analysis
    // will be running it
...
// A driver class which simply iterates through all CFG nodes of a specified
// function
...
// A driver class which simply iterates all function definitions one by one and
// call intra-procedural analysis on each of them.
...
public:
  UnstructuredPassInterAnalysis(IntraProceduralAnalysis &intraAnalysis)
      : InterProceduralAnalysis(&intraAnalysis) {}
