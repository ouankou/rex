...
public:
  virtual ~dottable() {}
  // Returns a string that containts the representation of the object as a graph
  // in the DOT language that has the given name
...
//! for now.
// For each function, a dot graph file will be generated. The CFG node will
// contain lattices information. The dot file will have a name like:
// original_full_filename_managed_func_name_cfg.dot
...
class dbgBuf : public std::streambuf {
  ...
  // The number of observed '<' characters that have not yet been balanced out
  // by '>' characters.
...
private:
  ...
  // Prints the indent to the stream buffer, returns 0 on success non-0 on
  // failure
  // int printIndent();
...
public:
  ...
  // ostreams.
  dbgStream();
  dbgStream(std::string title, std::string dbgFileName, std::string workDir,
            std::string imgPath);
  void init(std::string title, std::string dbgFileName, std::string workDir,
            std::string imgPath);
  ~dbgStream();
  ...
  // Adds an image to the output with the given extension and returns the path
  // of this image so that the caller can write to it.
  std::string addImage(std::string ext = ".gif");

  // Given a reference to an object that can be represented as a dot graph,
  // create an image from it and add it to the output. Return the path of the
  // image.
  std::string addDOT(dottable &obj);
  // Given a reference to an object that can be represented as a dot graph,
  // create an image of it and return the string that must be added to the
  // output to include this image.
  std::string addDOTStr(dottable &obj);
  // Given a representation of a graph in dot format, create an image from it
  // and add it to the output. Return the path of the image.
  std::string addDOT(std::string dot);
  // The common work code for all the addDOT methods
  void addDOT(std::string imgFName, std::string graphName, std::string dot,
              std::ostream &ret);
};
