...
class ReachingDefinitionBase
    : private BitVectorReprBase<std::string,
                                std::pair<AstNodePtr, AstNodePtr>> {
public:
  typedef BitVectorReprBase<
      std::string, std::pair<AstNodePtr, AstNodePtr>>::iterator iterator;
  void collect_refs(AstInterface &fa, const AstNodePtr &h,
                    FunctionSideEffectInterface *a = 0,
                    AstInterface::AstNodeList *in = 0);
  void add_ref(const std::string &name, const AstNodePtr &scope,
  ...
  void finalize() {
    BitVectorReprBase<std::string,
                      std::pair<AstNodePtr, AstNodePtr>>::finalize();
  }
  iterator begin() const {
    return BitVectorReprBase<std::string,
                             std::pair<AstNodePtr, AstNodePtr>>::begin();
  }
  iterator end() const {
    return BitVectorReprBase<std::string,
                             std::pair<AstNodePtr, AstNodePtr>>::end();
  }
  std::pair<AstNodePtr, AstNodePtr> get_ref(iterator p) const {
    return BitVectorReprBase<std::string,
                             std::pair<AstNodePtr, AstNodePtr>>::get_data(p);
  }
...
public:
  ...
  ReachingDefinitions get_empty_set() const {
    return BitVectorReprGenerator<
        std::string, std::pair<AstNodePtr, AstNodePtr>>::get_empty_set();
  }
  ReachingDefinitions get_def_set(const std::string &varname,
                                  const AstNodePtr &scope) const;

  void collect_member(
      const ReachingDefinitions &repr,
      CollectObject<std::pair<AstNodePtr, AstNodePtr>> &collect) const {
    BitVectorReprGenerator<std::string, std::pair<AstNodePtr, AstNodePtr>>::
        collect_member(repr, collect);
  }

  const ReachingDefinitionBase &get_base() const {
    return static_cast<const ReachingDefinitionBase &>(
        BitVectorReprGenerator<std::string,
                               std::pair<AstNodePtr, AstNodePtr>>::get_base());
  }
};
...
public:
  virtual ReachingDefinitions get_entry_data() const { return in; }
  virtual void set_entry_data(const ReachingDefinitions &_in) { in = _in; }
  virtual ReachingDefinitions get_exit_data() const { return out; }
  ...
  ReachingDefNode(MultiGraphCreate *c) : DataFlowNode<ReachingDefinitions>(c) {}
...
class ReachingDefinitionAnalysis
  ...
  virtual ReachingDefinitions meet_data(const ReachingDefinitions &d1,
...
public:
  ...
  const ReachingDefinitionGenerator *get_generator() const { return g; }
};
