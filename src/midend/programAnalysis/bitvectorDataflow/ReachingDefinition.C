...
public:
  bool operator()(AstNodePtr mod_first, AstNodePtr mod_second) {
    ...
    if (fa.IsVarRef(mod.first, 0, &varname, &scope)) {
      ...
      if (DebugReachingDef())
        std::cerr << "collecting var ref: " << varname << ":"
                  << AstInterface::AstToString(mod.second) << std::endl;
    } else {
      base.add_unknown_def(mod);
      if (DebugReachingDef())
        std::cerr << "collecting unknown ref: "
                  << AstInterface::AstToString(mod.first) << ":"
                  << AstInterface::AstToString(mod.second) << std::endl;
    }
    return true;
  }
...
void ReachingDefinitionBase::add_ref(
    const std::string &varname, const AstNodePtr &scope,
...
void ReachingDefinitionBase::collect_refs(AstInterface &fa, const AstNodePtr &h,
                                          FunctionSideEffectInterface *a,
                                          AstInterface::AstNodeList *in) {

  for (AstInterface::AstNodeList::iterator p = in->begin(); p != in->end();
       ++p) {
    AstNodePtr cur = *p;
    std::string varname;
    AstNodePtr scope;
  ...
  std::function<bool(AstNodePtr, AstNodePtr)> collect_f =
...
void ReachingDefinitionGenerator::add_def(
    ReachingDefinitions &repr, const std::string &varname,
...
ReachingDefinitions
ReachingDefinitionGenerator::get_def_set(const std::string &varname,
...
class CollectLocalDefinitions {
  AstInterface &fa;
  std::map<std::string,
           std::pair<AstNodePtr, std::pair<AstNodePtr, AstNodePtr>>>
      defvars;
...
public:
  CollectLocalDefinitions(AstInterface &_fa,
  ...
  ReachingDefinitions get_gen() {
    for (std::map<std::string,
                  std::pair<AstNodePtr, std::pair<AstNodePtr, AstNodePtr>>>::
             const_iterator p = defvars.begin();
         p != defvars.end(); ++p) {
      std::pair<std::string,
                std::pair<AstNodePtr, std::pair<AstNodePtr, AstNodePtr>>>
          cur = *p;
...
public:
  CollectKillDefinitions(AstInterface &_fa,
                         const ReachingDefinitionGenerator &_g)
      : fa(_fa), g(_g) {
    init();
  }
  ReachingDefinitions get_kill() const { return kill; }
  void init() { kill = g.get_empty_set(); }
};
...
void ReachingDefNode::finalize(AstInterface &fa,
  ...
  std::function<bool(AstNodePtr, AstNodePtr)> collectgen_f =
      [&collectgen](AstNodePtr first, AstNodePtr second) {
        return collectgen(first, second);
      };
...
void ReachingDefinitionAnalysis::FinalizeCFG(AstInterface &fa) {
  ...
  for (AstInterface::AstNodeList::iterator p = pars.begin(); p != pars.end();
       ++p) {
    AstNodePtr cur = *p;
    std::string name;
    AstNodePtr scope;
    if (fa.IsVarRef(cur, 0, &name, &scope))
      g->add_def(in, name, scope,
                 std::pair<AstNodePtr, AstNodePtr>(cur, AST_NULL));
  }
...
void ReachingDefinitionAnalysis::operator()(AstInterface &fa,
                                            const AstNodePtr &h,
                                            FunctionSideEffectInterface *anal) {
  assert(g == 0 && pars.size() == 0);

  AstNodePtr body = h;
  if (!fa.IsFunctionDefinition(h, 0, &pars, 0, &body)) { /* assert(false); */
  }
...
void ReachingDefinitionAnalysis::collect_ast(
