...
template <class Node>
void DumpDefSet(const std::vector<Node *> &defvec,
                const ReachingDefinitions &in) {
...
template <class Node>
void DefUseChain<Node>::build(AstInterface &fa, AstNodePtr root,
...
template <class Node>
void DefUseChain<Node>::build(AstInterface &fa, ReachingDefinitionAnalysis &r,
                              AliasAnalysisInterface &alias,
                              FunctionSideEffectInterface *f) {
  ...
  for (ReachingDefinitionBase::iterator p = base.begin(); p != base.end();
       ++p) {
    std::pair<AstNodePtr, AstNodePtr> cur = base.get_ref(p);
    if (DebugDefUseChain())
      std::cerr << "creating def node : "
                << AstInterface::AstToString(cur.first) << " : "
                << AstInterface::AstToString(cur.second) << std::endl;
  ...
  for (ReachingDefinitionAnalysis::NodeIterator p = r.GetNodeIterator();
    ...
    auto CreateEdges = [&fa, &g, &alias, this, &in,
                        &defvec](Node *cur, const AstNodePtr &ref) {
      ...
      for (size_t i = 0; i < defvec.size(); ++i) {
        Node *def = defvec[i];
        assert(def != 0);
        if (known.has_member(i) || (unknown.has_member(i) &&
                                    alias.may_alias(fa, ref, def->get_ref()))) {
          if (DebugDefUseChain())
    ...
    std::function<bool(AstNodePtr, AstNodePtr)> opread =
        [&defvec, this, &g, &alias, &fa, &in,
         &CreateEdges](AstNodePtr read_first, AstNodePtr read_second) {
          if (DebugDefUseChain()) {
            std::cerr << "processind read info : "
                      << AstInterface::AstToString(read_first) << " : "
                      << AstInterface::AstToString(read_second) << std::endl;
            DumpDefSet(defvec, in);
          }
    ...
    std::function<bool(AstNodePtr, AstNodePtr)> opgen =
        [this, &defvec, &g, &alias, &fa, &defmap, &in,
         &CreateEdges](AstNodePtr mod_first, AstNodePtr mod_second) {
          std::string varname;
          AstNodePtr scope;
          if (DebugDefUseChain()) {
            std::cerr << "processing gen mod info : "
                      << AstInterface::AstToString(mod_first) << " : "
                      << AstInterface::AstToString(mod_second) << std::endl;
            DumpDefSet(defvec, in);
          }
          typename std::map<AstNodePtr, Node *>::const_iterator p =
              defmap.find(mod_first);
          assert(p != defmap.end());
          Node *cur = (*p).second;
          ...
          if (DebugDefUseChain()) {
            std::cerr << "finish processing gen mod info : "
                      << AstInterface::AstToString(mod_first) << " : "
                      << AstInterface::AstToString(mod_second) << std::endl;
            DumpDefSet(defvec, in);
          }
    ...
    std::function<bool(AstNodePtr, AstNodePtr)> opkill =
        [this, &defvec, g, &alias, &fa, &defmap, &in](AstNodePtr mod_first,
                                                      AstNodePtr mod_second) {
          std::string varname;
          AstNodePtr scope;
          if (DebugDefUseChain()) {
            std::cerr << "processing kill mod info : "
                      << AstInterface::AstToString(mod_first) << " : "
                      << AstInterface::AstToString(mod_second) << std::endl;
            DumpDefSet(defvec, in);
          }
          ...
          if (DebugDefUseChain()) {
            std::cerr << "finish processing kill mod info : "
                      << AstInterface::AstToString(mod_first) << " : "
                      << AstInterface::AstToString(mod_second) << std::endl;
            DumpDefSet(defvec, in);
          }
    ...
    for (std::list<AstNodePtr>::iterator p = stmts.begin(); p != stmts.end();
      ...
      AstNodePtr cur = *p;
      if (DebugDefUseChain())
        std::cerr << "processing stmt : " << AstInterface::AstToString(cur)
                  << std::endl;
...
template <class Node> class AppendWorkListWrap : public CollectObject<Node *> {
...
void DefUseChainNode::Dump() const {
  if (isdef)
    ...
    std::cerr << "use: ";
  std::cerr << AstInterface::AstToString(ref) << " : "
...
std::string DefUseChainNode::toString() const {
  ...
  if (isdef)
    ...
    result = "use:";
  result = result + AstInterface::AstToString(ref) + ":" +
