...
class ECR : public UF_elem {
  ...
  ECR *find_group() { return static_cast<ECR *>(UF_elem::find_group()); }
...
public:
  ...
  void x_eq_op_y(Variable x, const std::list<Variable> &y) {
    ECR *t1 = get_ECR(x)->get_type();
    for (std::list<Variable>::const_iterator yp = y.begin(); yp != y.end();
         ++yp) {
  ...
  void function_def_x(Variable x, const std::list<Variable> &inParams,
  ...
  // x = p (y)
  void function_call_p(Variable p, const std::list<Variable> &x,
  ...
  int find_LOC(std::ostream &out, std::map<ECR *, int> &locmap, int &loc,
               ECR *p) {
    int cur = -1;
  ...
  void outputLOC(std::ostream &out, std::map<ECR *, int> &locmap, int &loc,
                 ECR *p) {
    int max = 0;
  ...
  void output(std::ostream &out) {
    ...
    int loc = 0;
    for (std::map<Variable, ECR *>::iterator itMap = table.begin();
         itMap != table.end(); itMap++) {
...
private:
  ...
  void set_lambda(Lambda *l, const std::list<Variable> &inParams,
                  const std::list<Variable> &outParams) {
  ...
  void join(ECR *e1, ECR *e2) {
    ...
    std::list<ECR *> *pending1 = &e1->get_pending(),
                     *pending2 = &e2->get_pending();
