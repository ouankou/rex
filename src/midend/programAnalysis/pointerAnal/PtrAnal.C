...
static std::string Local_GetVarName(AstInterface &fa, const AstNodePtr &scope,
                                    std::string name) {
  assert(name != "");
...
static std::string Local_GetVarName(AstInterface &fa, const AstNodePtr &var) {
  std::string res =
      "v:" + InterProcVariableUniqueRepr::get_unique_name(fa, var);
  return res;
}

std::string PtrAnal::Get_VarName(AstInterface &fa, const AstNodePtr &rhs) {
  ...
  if (fa.IsVarRef(rhs, 0, &readname, &readscope)) {
  ...
  } else if (fa.IsUnaryOp(rhs) || fa.IsBinaryOp(rhs) ||
             fa.IsFunctionCall(rhs)) {
    readname = Local_GetVarName(fa, rhs);
    ...
    if (cur.name == "") {
      std::cerr << "does not have map for " << AstInterface::AstToString(lhs1)
                << " in " << AstInterface::AstToString(rhs) << "\n";
      ROSE_ABORT();
    }
...
void PtrAnal::ProcessExpression(AstInterface &fa, const std::string &_modname,
  ...
  if (fa.IsFunctionCall(rhs, &p, &args, &out)) {
    ...
    stmts_pushback(stmts, stmt);
    for (std::list<RefRec>::const_iterator p3 = refs.begin(); p3 != refs.end();
         ++p3) {
      RefRec cur = *p3;
...
void PtrAnal::ProcessMod(AstInterface &fa, const std::string &readname,
  ...
  if (fa.IsVarRef(mod) || fa.IsArrayAccess(mod, &p)) {
    ...
    Stmt stmt_last = fields.size() ? field_x_eq_y(modname, fields, readname)
    ...
    if (fa.IsUnaryOp(mod, &op, &p) && op == AstInterface::UOP_DEREF) {
      ...
      if (op == AstInterface::BOP_DOT_ACCESS) {
        ...
        std::string lhs = Get_VarName(fa, p),
                    field = Local_GetFieldName(fa, p2);
        fields.push_front(field);
        Stmt stmt_last = deref_x_eq_y(lhs, fields, readname);
        stmts_pushback(stmts, stmt_last);
        namemap[p.get_ptr()] = VarRef(stmt_last, lhs);
...
void PtrAnal::ProcessAssign(AstInterface &fa, const AstNodePtr &mod,
                            const AstNodePtr &rhs, bool readlhs) {
...
void PtrAnal::operator()(AstInterface &fa, const AstNodePtr &funcdef) {
  ...
  if (!fa.IsFunctionDefinition(funcdef, &fname, &params, &outpars, &body)) {
    std::cerr
        << "Error: analysis requires function definition as input instead of "
  ...
  ProcessAstTreeBase<AstNodePtr>::operator()(fa, body);
  ControlFlowAnalysis(fa, body, stmt_last);
}

bool PtrAnal::ProcessTree(AstInterface &fa, const AstNodePtr &s,
