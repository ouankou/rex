...
public:
  ...
  virtual Stmt x_eq_op_y(OpType op, const std::string &x,
  ...
  virtual Stmt funccall_x(const std::string &x,
...
private:
  ...
  typedef std::unordered_map<void *, std::pair<size_t, size_t>, PtrAnal::hash>
      StmtMap;
...
protected:
  void ProcessAssign(AstInterface &fa, const AstNodePtr &mod,
                     const AstNodePtr &rhs, bool readlhs = 0);
  void ProcessExpression(AstInterface &fa, const std::string &modname,
