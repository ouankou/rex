...
public:
  PtrAnalBuildCFG(AstInterface &_fa, PtrAnal &_m, const std::string &_fname,
                  PtrAnal::Stmt _d)
  ...
  virtual void AddNodeStmt(Node *n, const AstNodePtr &s) {
    ...
    if (p_pending != pending_in.end()) {
      ...
      for (std::list<std::pair<Node *, EdgeType>>::const_iterator p_edge =
               cur.begin();
           p_edge != cur.end(); ++p_edge) {
        Node *n1 = (*p_edge).first;
        if (n1->size())
          m.contrl_flow(n1->back(), n->front(), (*p_edge).second);
        else
          pending_out[n1].push_back(
              std::pair<Node *, EdgeType>(n, (*p_edge).second));
      }
...
void PtrAnal::ControlFlowAnalysis(AstInterface &fa, const AstNodePtr &head,
                                  Stmt defn) {
  if (stmts.size()) {
