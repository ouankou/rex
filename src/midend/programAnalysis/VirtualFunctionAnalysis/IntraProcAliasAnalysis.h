...
struct AliasCfgFilter {
  /** Determines if the provided CFG node should be traversed during Alias
   * Analysis
   *
   * @param cfgn The node in question.
   * @return Whether it should be traversed.
   */
  bool operator()(StaticCFG::CFGNode cfgn) const {
...
public:
  ...
  virtual void computeAliases(SgVariableSymbol *var, int derefLevel,
                              std::vector<SgGraphNode *> &) = 0;

  //! Given a GraphNode and Dereference level compute the Aliases
  virtual void computeAliases(SgGraphNode *node, int derefLevel,
                              std::vector<SgGraphNode *> &) = 0;

  //! Add a must alias relation
  virtual void addMustAliasRelation(const AliasRelationNode &left,
                                    const AliasRelationNode &right) = 0;

  //! Add a may alias relation eg. conditionals
  virtual void addMayAliasRelation(const AliasRelationNode &left,
                                   const AliasRelationNode &right) = 0;
...
protected:
   ...
   */
  // DQ (12/6/2016): Aded virtual to fix compiler warning that we want to
  // considered an error: -Wdelete-non-virtual-dtor
...
class CompactRepresentation : public CompReprBase {
  ...
  //! Process Nodes
  void processNodes(std::ostream &o, SgGraphNode *n,
...
public:
  ...
  std::unordered_map<SgNode *, SgGraphNode *> getNodesMapping() {
    return all_nodes;
  }
...
public:
  ...
  CompReprPtr(CompactRepresentation *repr) {
    ptr = std::shared_ptr<CompReprBase>(repr);
  }
...
public:
  ...
  void addNewAliasRelation(
      SgGraphNode *node,
      std::pair<AliasRelationNode, AliasRelationNode> a_relation);

  //! Adds a return statement. Return Statements are handled separately from
  //! others because of backward propagation
...
private:
  ...
  void recursiveCollect(
      SgGraphNode *,
      std::unordered_map<SgGraphNode *, CollectAliasRelations::COLOR> &);
};

//! IntraProcedurial DataFlow Analysis to compute exit and entry from all the
//! function which
...
protected:
  ...
  std::vector<SgGraphNode *> cfgNodes;
  ...
  //! Retrieve Alias Relations from Function Parameters. Map data between actual
  //! and formal parameters. Handles return values as well.
  void getFunctionParametersAliasRelations(
  ...
  void getConstructorParametersAliasRelations(
      SgConstructorInitializer *f_exp, SgFunctionDeclaration *funcDecl,
      std::vector<std::pair<AliasRelationNode, AliasRelationNode>>
          &arg_relations);

  //! Given a callSite and type of object add appropriate virtual function
  //! declaration
  bool addVirtualFunction(SgType *type, SgFunctionCallExp *funcExp);

  //! Given a CallSite update with all the virtual function calls
  bool updateVirtualFunctionInCallGraph(SgFunctionCallExp *funcCall,
                                        CompReprPtr &callSiteIN);

  // Get all the aliases
  void getAliases(CompReprPtr &ptr, AliasRelationNode &node,
                  std::vector<SgVariableSymbol *> &aliases);

public:
  // ReachingDefinitionAnalysis(SgNode *head) :
  // DataFlowAnalysis<ReachingDefNode, ReachingDefinitions>(head),  g(0) {
  ...
  //! Meet_data for intra procedural dataflow analysis. This function is called
  //! when two path merges in CFG
