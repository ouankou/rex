...
struct AliasRelationNode{
    ...
    bool operator==(const AliasRelationNode &that) const {
        ASSERT_require(reinterpret_cast<std::intptr_t>(this) != reinterpret_cast<std::intptr_t>(nullptr));
...
public:
    ...
    virtual void computeAliases (SgVariableSymbol *var, int derefLevel, std::vector<SgGraphNode *> &) = 0;

    //! Given a GraphNode and Dereference level compute the Aliases
    virtual void computeAliases (SgGraphNode *node, int derefLevel, std::vector<SgGraphNode *> &) = 0;
...
public:
    ...
    void computeAliases (SgVariableSymbol *var, int derefLevel, std::vector<SgGraphNode *> &nodes);

    //! Given a graph node and Dereference level return list of aliases
    void computeAliases (SgGraphNode *node, int derefLevel, std::vector<SgGraphNode *> &);
