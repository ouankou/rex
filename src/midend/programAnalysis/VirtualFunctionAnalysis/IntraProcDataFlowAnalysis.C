...
IntraProcDataFlowAnalysis<Node, Data>::IntraProcDataFlowAnalysis(
    SgNode *_head) {
  head = _head;
}
...
void IntraProcDataFlowAnalysis<Node, Data>::run() {
  ...
  while (change) {
    ...
    for (unsigned int index = 0; index < nodes.size(); index++) {
      ...
      /*
           std::stringstream ss;
           ss << isSgFunctionDeclaration(head)->get_mangled_name().getString()<<
         "_" << iteration<< "_"<< index << ".dot"; std::string name; ss >> name;
