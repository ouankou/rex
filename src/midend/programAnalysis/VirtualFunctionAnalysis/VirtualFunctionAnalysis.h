 ...
 *  as described in
 *
 * "Flow-Sensitive Interprocedural Type Analysis for C++”, TechReport ‘95  Paul
 * Carini Harini Srinivasan
 *
 * So we perform Pointer Alias Analysis and Virtual Function Analysis
 * simultenously which in turn complements each other to be more precise. The
 * basic  idea is as follows:
 *
 * 1. Compute a call graph ignoring all the virtual function calls.
 * 2. Iterate over the nodes of the call graph topologically and reverse
 * topologically in alternative iteration until no Alias information is
 * modified. i. For each function:
 *            -  Compute a Control Flow Graph consists of nodes which can
 * generate new aliases eg. SgAssignOp, SgAssignInitializer, SgFunctionCallExp,
 ...
 *            - Propagate the Alias relation over the ControlFlow Graph to
 * generate entry and exit CompactRepresentation for each node. iii. Using the
 * Alias Information resolve the possible function callsites within this
 * function
 *             - Handle the Function Parameters, return types and Constructor
 * parameters correctly. iv. Compute Entry for all the functions called from
 * this function
 *
 *
 *
...
public:
  ...
  //! Given a Callsite(SgFunctionCallExp or SgConstructionInitializer) it
  //! resolves the function calls. For Virtual Functions it returns the minimal
  //! set of functions that may be execute at runtime.
  void resolveFunctionCall(SgExpression *,
