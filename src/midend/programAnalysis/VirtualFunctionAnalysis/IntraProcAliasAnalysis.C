...
void CompactRepresentation::computeAliases(SgGraphNode *node, int derefLevel,
                                           vector<SgGraphNode *> &sol) {
...
void CompactRepresentation::merge(SgIncidenceDirectedGraph *thatGraph) {
  ...
  for (SgGraphNode *node : nodes) {
    ...
    SgGraphNode *g_from = getGraphNode(node->get_SgNode());
    for (SgDirectedGraphEdge *ed : edges) {
...
CompactRepresentation::CompactRepresentation(
    const CompactRepresentation &copy) {
  if (graph != NULL) {
    delete graph;
    graph = NULL;
  }
...
CompactRepresentation::operator=(const CompactRepresentation &p) {
  ASSERT_require(reinterpret_cast<std::intptr_t>(this) !=
...
void CompactRepresentation::addMustAliasRelation(
    const AliasRelationNode &left, const AliasRelationNode &right) {
  ...
  //        std::cout << "Add a relation from " <<
  //        left.var->get_name().getString() << "
  //        "<<right.var->get_name().getString() << std::endl;
  ...
  for (i = 0; i < leftNodes.size(); i++) {
    std::set<SgDirectedGraphEdge *> edges =
        graph->computeEdgeSetOut(leftNodes[i]);
    if (edges.size() == 1) {

      updateHash((*(edges.begin()))->get_from()->get_SgNode(),
                 (*(edges.begin()))->get_to()->get_SgNode());
...
void CompactRepresentation::addMayAliasRelation(
    const AliasRelationNode &left, const AliasRelationNode &right) {
...
bool CompactRepresentation::operator==(
    const CompactRepresentation &that) const {
  ASSERT_require(reinterpret_cast<std::intptr_t>(this) !=
                 reinterpret_cast<std::intptr_t>(nullptr));
...
void CompactRepresentation::processNodes(std::ostream &o, SgGraphNode *n,
                                         std::set<SgGraphNode *> &explored) {
...
void CompactRepresentation::printNode(std::ostream &o, SgGraphNode *node) {
  ...
  std::string label = escapeString(
      ((SgVariableSymbol *)node->get_SgNode())
          ->get_scope()
          ->unparseToCompleteString() +
      "\n\n" +
...
void AliasInfoGenerator::addNewAliasRelation(
    SgGraphNode *node,
    std::pair<AliasRelationNode, AliasRelationNode> a_relation) {
  bool found = false;
  for (unsigned int i = 0; i < aliasRelations.at(node).size(); i++) {
    std::pair<AliasRelationNode, AliasRelationNode> relation =
        aliasRelations.at(node)[i];
    if (relation.first == a_relation.first &&
        relation.second.var == a_relation.second.var) {
      found = true;
      break;
    }
  }
...
std::vector<std::pair<AliasRelationNode, AliasRelationNode>>
AliasInfoGenerator::getAliasRelations(SgGraphNode *node) {
  return aliasRelations.at(node);
}

void IntraProcAliasAnalysis::buildCFG() {
  SgFunctionDefinition *defn = isSgFunctionDefinition(
      isSgFunctionDeclaration(
          isSgFunctionDeclaration(head)->get_definingDeclaration())
          ->get_definition());
  ...
  while (!workQ.empty()) {
    ...
    for (unsigned int i = 0; i < outs.size(); i++)
      if (find(visited.begin(), visited.end(), outs[i]->get_to()) ==
          visited.end()) {
...
IntraProcAliasAnalysis::IntraProcAliasAnalysis(
    SgNode *head, ClassHierarchyWrapper *_classHierarchy,
    CallGraphBuilder *_cgBuilder,
    std::unordered_map<SgFunctionDeclaration *, IntraProcAliasAnalysis *>
        &_mapping,
    std::unordered_map<SgExpression *, std::vector<SgFunctionDeclaration *>>
        &_resolver)
    : IntraProcDataFlowAnalysis<SgGraphNode, CompReprPtr>(head),
      classHierarchy(_classHierarchy), cfg(0), cgBuilder(_cgBuilder),
...
void IntraProcAliasAnalysis::run() {
  ...
  for (unsigned int i = 0; i < cfgNodes.size(); i++) {
    ...
    ss << isSgFunctionDeclaration(head)->get_mangled_name().getString() << "in_"
       << i << ".dot";
    std::string name, name2;
    ss >> name;

    ss2 << isSgFunctionDeclaration(head)->get_mangled_name().getString()
        << "out_" << i << ".dot";
    ss2 >> name2;
    ...
    // std::cout << i<< " in:" << getCFGInData((cfgNodes[i])).get()->getHash()
    // << std::endl; std::cout << i<< " out:" <<
    // getCFGOutData((cfgNodes[i])).get()->getHash() << std::endl;
  }
...
bool IntraProcAliasAnalysis::runCheck() {
  ...
  vector<SgFunctionCallExp *> functionCalls =
      SageInterface::querySubTree<SgFunctionCallExp>(head, V_SgFunctionCallExp);
  CFGNode cfgn;
  ...
  for (unsigned index = 0; index < functionCalls.size(); index++) {
    ...
    if (graphNode == NULL) {
      // example: function call inside catch block is not added on controlflow
      // graph
      continue;
    }
    ...
    for (unsigned j = 0; j < resolver.at(funcCall).size(); j++) {
      SgFunctionDeclaration *declaration =
          isSgFunctionDeclaration(resolver.at(funcCall)[j]);
      if (declaration == NULL) {
        std::cout << "Defining declaration not found for "
                  << funcCall->unparseToCompleteString() << std::endl;
        continue;
      }
      ...
      std::vector<std::pair<AliasRelationNode, AliasRelationNode>>
          arg_relations, return_relations;

      getFunctionParametersAliasRelations(funcCall, resolver.at(funcCall)[j],
                                          arg_relations, return_relations);
      // add the parameters to the alias relations to the function entry
      for (unsigned int i = 0; i < arg_relations.size(); i++)
        mapping.at(declaration)
            ->getFunctionEntry()
            .get()
            ->addMustAliasRelation(arg_relations[i].first,
                                   arg_relations[i].second);
  ...
  vector<SgConstructorInitializer *> constrs =
      SageInterface::querySubTree<SgConstructorInitializer>(
          head, V_SgConstructorInitializer);
  for (unsigned index = 0; index < constrs.size(); index++) {
    ...
    for (unsigned j = 0; j < resolver.at(con).size(); j++) {
      SgFunctionDeclaration *declaration =
          isSgFunctionDeclaration(resolver.at(con)[j]);

      if (declaration == NULL) {
        std::cout << "Defining declaration not found for "
                  << con->unparseToCompleteString() << std::endl;
        continue;
      }
      ...
      getConstructorParametersAliasRelations(con, resolver.at(con)[j],
                                             arg_relations);
...
void IntraProcAliasAnalysis::getFunctionParametersAliasRelations(
    SgFunctionCallExp *f_exp, SgFunctionDeclaration *funcDecl,
  ...
  for (; i != actual.end() && j != formal.end(); ++j, i++) {
    ...
    if ((*i)->variantT() == V_SgFunctionCallExp) {
      ...
      std::vector<std::pair<AliasRelationNode, AliasRelationNode>> relations =
          gen->getAliasRelations(graphNode);
      for (index = 0; index < relations.size(); index++) {
        rightARNode = relations[index].second;
        if (leftARNode.var != NULL && rightARNode.var != NULL) {
          arg_relations.push_back({leftARNode, rightARNode});
        }
      }
  ...
  std::vector<AliasRelationNode> returns =
      mapping.at(funcDecl)->getReturnStmts();

  for (unsigned int index = 0; index < returns.size(); index++) {
    return_relations.push_back({arNode, returns[index]});
  }
}
void IntraProcAliasAnalysis::getConstructorParametersAliasRelations(
    SgConstructorInitializer *f_exp, SgFunctionDeclaration *funcDecl,
    std::vector<std::pair<AliasRelationNode, AliasRelationNode>>
        &arg_relations) {
...
/**
 * Adds a resolved virtual function to the callgraph and also update the
 * resolver
 * @param type
 * @param right
 * @return
...
bool IntraProcAliasAnalysis::addVirtualFunction(SgType *type,
  ...
  SgExpression *right =
      isSgBinaryOp(funcExp->get_function())->get_rhs_operand();
  ...
  SgFunctionDeclaration *functionDeclaration =
      CallTargetSet::getFirstVirtualFunctionDefinitionFromAncestors(
          classType, memberFunctionDeclaration, classHierarchy);

  if (functionDeclaration == NULL)
    return false;

  // std::cout << "Decl::\n"<<functionDeclaration->unparseToCompleteString() <<
  // std::endl;
  ...
  if (cgBuilder->getGraphNodesMapping().count(functionDeclaration) == 0) {
    toNode =
        new SgGraphNode(functionDeclaration->get_qualified_name().getString());
    ;
...
bool IntraProcAliasAnalysis::updateVirtualFunctionInCallGraph(
    SgFunctionCallExp *funcCall, CompReprPtr &callSiteIN) {
...
void IntraProcAliasAnalysis::getAliases(
    CompReprPtr &ptr, AliasRelationNode &node,
    std::vector<SgVariableSymbol *> &aliases) {
  ...
  for (unsigned int i = 0; i < graph_aliases.size(); i++) {
    aliases.push_back(
        static_cast<SgVariableSymbol *>(graph_aliases[i]->get_SgNode()));
  }
}
...
IntraProcAliasAnalysis::getPredecessors(SgGraphNode *n) {
  std::vector<SgGraphNode *> nodes;
  cfg->getGraph()->getPredecessors(n, nodes);
  return nodes;
}

void ProcessExpression::processLHS(SgNode *node,
                                   struct AliasRelationNode &arNode) {
  ...
  case V_SgInitializedName: {
    ...
    ASSERT_not_null(init_exp);

    sym = static_cast<SgVariableSymbol *>(
        init_exp->get_symbol_from_symbol_table());

  } break;

  case V_SgVarRefExp: {
    var_exp = isSgVarRefExp(node);
    ASSERT_not_null(var_exp);
  ...
  if (sym != NULL && (SageInterface::isPointerType(sym->get_type()) == false ||
                      SageInterface::isReferenceType(sym->get_type())))
    sym = NULL;
...
void ProcessExpression::processRHS(SgNode *node,
                                   struct AliasRelationNode &arNode) {
  SgVariableSymbol *sym = NULL;
  SgVarRefExp *var_exp;
  int derefLevel = 0;
  ...
  case V_SgNewExp: {
    ...
    if (new_variables.count(new_exp) == 0) {
      ...
      // Create a temporary variable so that every memory location has an
      // assigned name
...
void CollectAliasRelations::processNode(SgGraphNode *g_node) {
  ...
  case V_SgAssignInitializer: {
    ...
    if (SgInitializedName *init_name =
            isSgInitializedName(assgn_i->get_parent())) {
      lhs = init_name;
    } else {
  ...
  case V_SgReturnStmt: {
    SgFunctionDeclaration *functionDeclaration = isSgFunctionDeclaration(
        isSgFunctionDefinition(cfg->getEntry()->get_SgNode())
            ->get_declaration());
    ...
    if (SageInterface::isPointerType(returnType) == false &&
        SageInterface::isReferenceType(returnType) == false)
      return;

    SgReturnStmt *returnStmt = isSgReturnStmt(node);
    rhs = returnStmt->get_expression();

    // std::cout << "Return for processing : " << rhs->unparseToCompleteString()
    // << std::endl;
  ...
  if (leftARNode.var != NULL && rightARNode.var != NULL) {
    g->addNewAliasRelation(g_node, {leftARNode, rightARNode});
  }
}

void CollectAliasRelations::recursiveCollect(
    SgGraphNode *node, std::unordered_map<SgGraphNode *, COLOR> &colors) {
...
void IntraProcAliasAnalysis::applyCFGTransferFunction(SgGraphNode *s) {
  ...
  ASSERT_not_null(in.get());

  repr += in;

  if (s->get_SgNode()->variantT() == V_SgFunctionCallExp ||
      s->get_SgNode()->variantT() == V_SgConstructorInitializer) {
    // CompactRepresentation *repr = new CompactRepresentation();
    ...
    for (unsigned int index = 0; index < resolver.at(exp).size(); index++) {
      ...
      SgFunctionDeclaration *decl =
          isSgFunctionDeclaration(resolver.at(exp)[index]);

      if (decl == NULL)
        continue;

      // std::cout << "CFG Xfer function for: "<<
      // exp->unparseToCompleteString()<< ":->"<<
      // decl->unparseToCompleteString()<< std::endl;
