...
public:
  ...
  static SgNode *identifyEntryNode(SgFunctionDefinition *def) { return def; }

public:
  // ! the nodes required to fully represent a call site in the PDG
  struct CallSiteStructure {
    //! SgNode containing the functioncall, attention: this is not the
    //! SgFunctionCall-Node, this is the first node encountered form the
    //! functioncall towards the head of the function
  ...
  SgNode *getActualReturn(int id) { return callSites[id].actual_return; }
  SgNode *getActualIn(int id, int varNr) {
    return callSites[id].actual_in[varNr];
  }
  int getActualInCount(int id) { return callSites[id].actual_in.size(); }
  ...
  std::set<SgNode *> getExitNodes() { return exitNodes; }
  void addParameterToFunctionCall(SgNode *, SgExpression *) {}

  int callSiteCount() { return callSites.size(); }
  SgNode *getFunctionCallExpNode(int i) {
    return callSites[i].sgFunctionCallExpNode;
  }
  SgNode *getFunctionEntry() { return entry; }
  void setEllipse(SgNode *formal) { ellipseNode = formal; }
  SgNode *getEllipse() { return ellipseNode; }
...
public:
  ...
  int getFormalCount() { return formal.size(); }
  ...
  void setFormalReturn(SgNode *node) { formal_return = node; }
  SgNode *getFormalReturn() { return formal_return; }
  // add this DependenceNode to the list of nodes which lead to exiting this
  // function
...
#ifdef VERBOSE_DEBUG
    std::cout << "creating interprocedural info for "
  ...
  /* ! \brief Gets the function declaration that the InterproceduralInfo object
     is for. Returns: The SgFunctionDeclaration node that is associated with
     this object */
  SgFunctionDeclaration *foo() { return decl; }
  SgFunctionDefinition *getFunctionDefinition() { return def; }
  SgFunctionDeclaration *getFunctionDeclaration() { return decl; }

  // adds an function call to the tracking list, this will alter on be used to
  // analyse the calls site in the ...
  //      void addFunctionCall(SgNode * sliceImportantNode,SgNode *
  //      functionCall,SgNode * actualReturnIdentifier=NULL)
  ...
  /*! \brief Extracs all the function calls from an AST subtree which may
     contain function calls Params: - SgNode * node: The root of the subtree we
     want to analyze. Return: a list of all SgFunctionCallExp nodes that are in
     the subtree. */
  /*    static std::list < SgFunctionCallExp * >extractFunctionCalls(SgNode *
     node)
      {
          std::list < SgFunctionCallExp * >retval;
          std::list < SgNode * >calls = NodeQuery::querySubTree(node,
     V_SgFunctionCallExp); for (std::list < SgNode * >::iterator i =
     calls.begin(); i != calls.end(); i++)
          {
