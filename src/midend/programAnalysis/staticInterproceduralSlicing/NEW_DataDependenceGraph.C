...
#ifdef NEWDU
DataDependenceGraph::DataDependenceGraph(SgNode *head, EDefUse *du,
...
void DataDependenceGraph::buildDDG() {
  ...
  for (Rose_STL_Container<SgNode *>::iterator i = varUse.begin();
    ...
    // Now, grab the proper use node in the system dependence graph, or create
    // it if it doesn't exist uw-mist replaced the call if
    // (isSgFunctionCallExp(getNextParentInterstingNode(interestingUseParent->get_parent())))
    if (isSgFunctionCallExp(interestingUseParent)) {
      // check if the variable is used as argument in a function call (special
      // case in system dependence graph)
    ...
    if (DUVariableAnalysisExt::isUse(varRef) ||
      ...
      for (unsigned int j = 0; j < defPlaces.size(); j++) {
        ...
        else if (isSgFunctionParameterList(
                     interestingDefParent->get_parent())) {
  ...
  for (int i = 0; i < _interprocedural->callSiteCount(); i++) {
    ...
    for (int j = 0; j < _interprocedural->getActualInCount(i); j++) {
      ...
      // if it is not a reference expression, search to find the lower reference
      // expressions in the complex arg uw-mist TODO: (If neccessary) for now,
      // just skip
      if (!isSgVarRefExp(actOut))
        continue;

      // search the def/use map
      SgInitializedName *actOutInitName =
          isSgVarRefExp(actOut)->get_symbol()->get_declaration();
      DependenceNode *actOutDep = getNode(DependenceNode::ACTUALOUT, actOut);

      // connect to all potential definitions (not as exact as possible, but
      // should work for first pass uw-mist TODO: connect directly to all
      // potential use paths instead
      std::vector<SgNode *> defPlaces =
          defuse->getDefFor(actOut, actOutInitName);
      for (unsigned int k = 0; k < defPlaces.size(); k++) {

        establishEdge(actOutDep,
                      getNode(getNextParentInterstingNode(defPlaces[k])), DATA);
      }
    }
  }
  ...
  for (Rose_STL_Container<SgNode *>::iterator returnS = returnStmts.begin();
    ...
    // the formal out
    if (isSgReturnStmt(
            stmt) /*&& isSgReturnStmt(stmt)->get_expression ()!=NULL*/) {
      ...
      Rose_STL_Container<SgNode *> returnRefs =
          NodeQuery::querySubTree(stmt, V_SgVarRefExp);
      for (Rose_STL_Container<SgNode *>::iterator refIt = returnRefs.begin();
           refIt != returnRefs.end(); refIt++) {
        establishEdge(
            getNode(getNextParentInterstingNode(*refIt)),
