...
std::vector<SgNode *> EDefUse::getDefFor(SgNode *node,
  ...
  std::vector<SgNode *> retVec,
      defVec = internalDefUse->getDefFor(node, initName);
  for (unsigned int i = 0; i < defVec.size(); i++) {
    ...
    if (isSgFunctionCallExp(defNode)) {
      ...
      Rose_STL_Container<SgNode *> varRefList =
          NodeQuery::querySubTree(call, V_SgVarRefExp);
      // bool foundDefinition=false;
      for (Rose_STL_Container<SgNode *>::iterator varRefIt = varRefList.begin();
           varRefIt != varRefList.end(); varRefIt++) {
        ...
        SgNode *hiker = varRef;
        while (!isSgExprListExp(hiker->get_parent()))
          hiker = hiker->get_parent();
        // hiker is the element just beneath the exprListExp
        if (!isSgFunctionCallExp(hiker->get_parent()->get_parent()) &&
            hiker->get_parent()->get_parent() != call) {
...
EDefUse::getDefMultiMapFor(SgNode *node) {
