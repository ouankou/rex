...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nr_of_bcasts; ++i) {
    ...
    if (rank == 0) {
      ...
      // as long as the data that will be broadcasted is changed before the
      // actual Broadcast starts
      //  it doesn't violate the rule to be a "real" Broadcast and it's one of
      //  the things to check before we can do a Source-Code-Transformation
      // -> These checks (if it is a Broadcast and if it is safe to transform
      // the code using MPI_Bcast
  ...
  if (rank != 0) {
    fprintf(stderr, "Proc %d: Sum of data %d.\n", rank, sum);
  }
  ...
  if (rank == 0) {
    ...
    fprintf(stderr,
            "MASTER: Time for simulating an MPI_BCAST on %d machines with %d "
            "enumerations : %lf [sec].\n",
