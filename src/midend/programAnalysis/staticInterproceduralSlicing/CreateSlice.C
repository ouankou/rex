  ...
  if (isSgBasicBlock(node) || isSgGlobal(node)) {
    ...
    for (std::list<SgNode *>::iterator cand =
      ...
      if (isSgVariableDeclaration(*cand)) {
        ...
        for (Rose_STL_Container<SgNode *>::iterator varUse = varUseList.begin();
             varUseList.end() != varUse; varUse++) {
          //                              cout <<"\t\tchecking:
          //                              "<<(*varUse)->unparseToString()<<endl;
          if (isSgVarRefExp(*varUse)->get_symbol()->get_declaration() ==
            ...
            //                                                              use
            //                                                              in
            //                                                              "<<(*varUse)->get_parent()->unparseToString()<<endl;
  ...
  if (isSgStatement(node)) {
    ...
    if (!partOfSlice.boolean) {
      if (isSgVariableDefinition(node)) {
        ...
        // SgNode *
        // toDelNode=isSgVariableDeclaration(node)->get_definition()->get_vardefn()->get_initializer();
        // cleanly delete this opject!!
        //                                      delete(toDelNode);
        isSgVariableDeclaration(node)
            ->get_definition()
            ->get_vardefn()
            ->set_initializer(NULL);
