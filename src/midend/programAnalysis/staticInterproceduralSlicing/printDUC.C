...
void printNode(std::ofstream &f, SgInitializedName *name,
               SgNode *interestingNode, set<SgNode *> *printedSet) {
  // if the node is in the printed set, skip
  if (printedSet->count(interestingNode))
    return;
...
void printEdge(std::ofstream &f, SgNode *source, SgNode *dest, int type,
               set<pair<SgNode *, SgNode *>> *printedSet) {
  ...
  if (type)
    f << "\"" << defPtrStr << "\" -> \"" << usePtrStr
      << "\"[label=\"DefUse\",color=green];" << endl;

  else if (dest != source)
    f << "\"" << defPtrStr << "\" -> \"" << usePtrStr
      << "\"[label=\"Use\",color=blue];" << endl;
}

void analyseFunction(SgFunctionDefinition *def, EDefUse *defUseAnalysis,
                     std::ofstream &f) {
  char defPtrStr[100], usePtrStr[100];
  ;
  set<SgNode *> *printedNodeSet = new set<SgNode *>();
  set<pair<SgNode *, SgNode *>> *printedDataEdgeSet =
      new set<pair<SgNode *, SgNode *>>();
  ...
  for (list<SgNode *>::iterator i = varUse.begin(); i != varUse.end(); i++) {
    ...
    SgNode *interstingUseParent =
        DUVariableAnalysisExt::getNextParentInterstingNode(use);
    ...
    for (int j = 0; j < defPlaces.size(); j++) {
      interstingDefParent =
          DUVariableAnalysisExt::getNextParentInterstingNode(defPlaces[j]);
      printNode(f, initName, interstingUseParent, printedNodeSet);
      printNode(f, initName, interstingDefParent, printedNodeSet);
      printEdge(f, interstingDefParent, interstingUseParent, 1,
                printedDataEdgeSet);
    }
  }
  for (list<SgNode *>::iterator i = varUse.begin(); i != varUse.end(); i++) {
    ...
    for (int j = 0; j < defPlaces.size(); j++) {
      ...
      printNode(f, initName, interstingUseParent, printedNodeSet);
      printNode(f, initName, interstingDefParent, printedNodeSet);
      printEdge(f, interstingUseParent, interstingDefParent, 0,
                printedDataEdgeSet);
    }
  }
  //                      test(use);
  ...
  if (DUVariableAnalysisExt::isIDef(use))
  {
          ...
          SgNode * interstingDefParent;
          std::vector< SgNode* >
defPlaces=defUseAnalysis->getDefFor(use,initName); for (int
j=0;j<defPlaces.size();j++)
          {
...
//                              cout <<"\t*next intersting node is:
"<<interstingDefParent->unparseToString()<<endl;
  ...
  if (DUVariableAnalysisExt::isIUse(use))
  {
          ...
          SgNode * interstingDefParent;
          std::vector< SgNode* >
defPlaces=defUseAnalysis->getUseFor(use,initName); for (int
j=0;j<defPlaces.size();j++)
          {
                  def=isSgVarRefExp(defPlaces[j]);
                  cout <<"\t\t*def in: "<<defPlaces[j]->unparseToString()<<endl;
...
int main(int argc, char *argv[]) {
  ...
  for (list<SgNode *>::iterator i = functionDeclarations.begin();
    ...
    filename =
        ...
        "." + isSgFunctionDeclaration(*i)->get_name().getString() + ".duc.dot";
