...
std::set<SgNode *> CreateSliceSet::computeSliceSet() {
  ...
  for (list<SgNode *>::iterator i = sliceTargetNodes.begin();
       i != sliceTargetNodes.end(); i++) {
...
std::set<SgNode *> CreateSliceSet::computeSliceSet(SgNode *node) {
  ...
  for (set<DependenceNode *>::iterator i = returnedDepSet.begin();
       i != returnedDepSet.end(); i++) {
...
CreateSliceSet::getSliceDepNodes(set<DependenceNode *> searchSet,
                                 set<DependenceGraph::EdgeType> allowedEdges) {
  ...
  for (set<DependenceNode *>::iterator i = searchSet.begin();
       searchSet.end() != i; i++) {
  ...
  while (workList.size()) {
    ...
    // since a set contains only one copy for each node it is safe to insert the
    // sgNode
    ...
    for (set<SimpleDirectedGraphNode *>::iterator i = preds.begin();
      ...
      set<DependenceGraph::EdgeType> connectingSet =
          sdg->edgeType(pred, current);
      for (set<DependenceGraph::EdgeType>::iterator j = connectingSet.begin();
           j != connectingSet.end(); j++) {
...
std::set<SgNode *>
CreateSliceSet::computeForwardSliceSet(DependenceNode *node) {
  std::set<SgNode *> returnSet;
  ...
  // transform the depset to sgSet
  for (std::set<DependenceNode *>::iterator i = returnedDepSet.begin();
       i != returnedDepSet.end(); i++) {
...
std::set<DependenceNode *> CreateSliceSet::getForwardSliceDepNodes(
    std::set<DependenceNode *> searchSet,
    std::set<DependenceGraph::EdgeType> allowedEdges) {
  std::set<DependenceNode *> reachableNodes;
  std::vector<DependenceNode *> workList;
  std::set<DependenceNode *> visitedSet;

  for (std::set<DependenceNode *>::iterator i = searchSet.begin();
       searchSet.end() != i; i++) {
    workList.push_back(*i);
  }
  // initialize process loop
  ...
  while (workList.size()) {
    ...
    for (std::set<SimpleDirectedGraphNode *>::iterator i = succs.begin();
         i != succs.end(); i++) {
      DependenceNode *succ = dynamic_cast<DependenceNode *>(*i);
      std::set<DependenceGraph::EdgeType> connectingSet =
          sdg->edgeType(current, succ);
