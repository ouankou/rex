...
void analyseFunction(SgFunctionDefinition *def, DFAnalysis *defUseAnalysis) {
  ...
  for (list<SgNode *>::iterator i = varUse.begin(); i != varUse.end(); i++) {
    ...
    cout << "SgVarRefExpr: <" << use->get_symbol()->get_name().str()
         << "> in: " << interstingUseParent->unparseToString() << endl;
    bool eval = true;
    ...
    if (defUseAnalysis->isNodeGlobalVariable(
            use->get_symbol()->get_declaration())) {
    ...
    if (eval) {
      ...
      if (isUse(use)) {
        ...
        std::vector<SgNode *> defPlaces =
            defUseAnalysis->getDefFor(use, initName);
        for (int j = 0; j < defPlaces.size(); j++) {
          ...
          if (isSgInitializedName(interstingDefParent))
            cout << "\t\t*def in: "
                 << isSgInitializedName(interstingDefParent)
                        ->get_name()
                        .getString()
                 << endl;
          else
            cout << "\t\t*def in: " << interstingDefParent->unparseToString()
                 << endl;
        }
      }
      if (isIDef(use)) {
      ...
      if (isIUse(use)) {
        ...
        for (int j = 0; j < defPlaces.size(); j++) {
          ...
          if (isIDef(def))
            interstingDefParent =
                DUVariableAnalysisExt::getNextParentInterstingNode(
                    defPlaces[j]);
          if (isSgInitializedName(interstingDefParent))
            cout << "\t\t*idef in: "
                 << isSgInitializedName(interstingDefParent)
            ...
            cout << "\t\t*idef in: " << interstingDefParent->unparseToString()
                 << endl;
        }
      }
      //                      test(use);
