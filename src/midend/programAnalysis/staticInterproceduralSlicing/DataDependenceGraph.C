...
/*SgNode * getNextParentInterstingNode(SgNode* node)
  {
  SgNode * tmp=node;
  while(!IsImportantForSliceSgFilter(tmp) && !isSgFunctionCallExp(tmp)
  &&!isSgFunctionDefinition(tmp))        { tmp=tmp->get_parent();   } return
  tmp;
...
void outputNodeInfo(std::ostream &os, SgNode *defSgNode) {
  if (isSgInitializedName(defSgNode) != NULL) {
    os << "\""
       << isSgInitializedName(defSgNode)->get_qualified_name().getString()
       << "\" of class " << defSgNode->class_name();
  } else {
    os << "\"" << defSgNode->unparseToString() << "\" of class "
       << defSgNode->class_name();
  }
}

void DataDependenceGraph::buildDDG() {
  std::set<SgInitializedName *> functionParameterSet(
      _head->get_declaration()->get_args().begin(),
      _head->get_declaration()->get_args().end());
  // SgNode *tmp;

  //      bool varRefIsDef;
  DependenceNode *defDepNode = NULL;
  DependenceNode *useDepNode = NULL;
  // this graph is intersted of all ueses of variables, up so far NO POINTER
  // analysis is done...
  Rose_STL_Container<SgNode *> varUse =
      NodeQuery::querySubTree(_head, V_SgVarRefExp);
  for (Rose_STL_Container<SgNode *>::iterator i = varUse.begin();
       i != varUse.end(); i++) {
    ...
    SgNode *varDef;
    SgNode *interestingUseParent, *interestingDefParent;

    // get the correct interesting node for the use (This is returning varRef
    // for some dumb reason)
    interestingUseParent = getNextParentInterstingNode(varRef);

    //        cout <<"processing: "<<varRef->unparseToString()<<endl;
    // cout <<"\t 1: use interesting parent is of
    // class:"<<interestingUseParent->class_name()<<
    ...
    if (isSgFunctionCallExp(
            getNextParentInterstingNode(interestingUseParent->get_parent()))) {
    ...
    // Jim: What's the point of jumping straight to the decleration here?  We
    // should want the def. from the defuse chains...
    ...
    if (defuse->isNodeGlobalVariable(initName)) {
      ...
      // the variable is a global variable, now connect the statement with the
      // global def
      if (DUVariableAnalysisExt::isIDef(varRef) ||
          DUVariableAnalysisExt::isDef(varRef)) {
        // create a ade from the ref to the globalInitialisation
        establishEdge(useDepNode, getNode(initName->get_declaration()),
                      GLOBALVAR_HELPER);
      }
      if (DUVariableAnalysisExt::isIUse(varRef) ||
          DUVariableAnalysisExt::isUse(varRef)) {
        // create a ade from the ref to the globalInitialisation
        establishEdge(getNode(initName->get_declaration()), useDepNode,
                      GLOBALVAR_HELPER);
      }
    }

    //      if the current use of the variable is a use
    if (DUVariableAnalysisExt::isUse(varRef) ||
        DUVariableAnalysisExt::isIUse(varRef)) {
      // Jim: Debug lines, just uncommented
      // cout <<"initname: 2: intersetingUseParent for "<<
      // initName->get_name().getString() <<
      //   " is "<<interestingUseParent->unparseToString()<<" and is of type
      //   "<<interestingUseParent->class_name()<<endl;
      ...
      // Does the below not over write the above?  Curious.  Removing the below
      // creates a false self-ref link on x+=5;
      ...
      for (unsigned int j = 0; j < defPlaces.size(); j++) {
        ...
        // cout <<"3: intersetingDefParent for "<< initName->get_name
        // ().getString() << " is "<<interestingDefParent->unparseToString()<<"
        // and is of type "<<interestingDefParent->class_name()<<endl; cout
        // <<"4: next inreresting parent :"
        // <<getNextParentInterstingNode(interestingDefParent->get_parent())->unparseToString()<<
        // " and is of type
        // "<<getNextParentInterstingNode(interestingDefParent->get_parent())->class_name()<<endl;

        defDepNode = NULL;
        // if the next interesting node is a function call, this definition
        // comes from a ACTUAL_OUT
        if (isSgFunctionCallExp(getNextParentInterstingNode(
                interestingDefParent->get_parent()))) {
          defDepNode = getNode(DependenceNode::ACTUALOUT, interestingDefParent);
          establishEdge(defDepNode, useDepNode, DATA);
        } else if (isSgFunctionDeclaration(getNextParentInterstingNode(
                       interestingDefParent->get_parent())))
        //)||                 isSgFunctionDefinition(interestingDefParent))
        {
          ...
          establishEdge(defDepNode, useDepNode, DATA);
            ...
            else if (isSgFunctionParameterList(interestingDefParent) ||
            isSgFunctionParameterList(interestingDefParent) ||
            isSgFunctionParameterList(defPlaces[j]->get_parent()) &&
            isSgFunctionDeclaration(defPlaces[j]->get_parent()->get_parent()))
            {
            //                                       cout <<"\t*isIntersting is
            FunctionParameterList"<<endl;
        ...
        // check if this variable is a variable from the function declaration,
        // and if so mark that variable as dirty
        //
    ...
    if (DUVariableAnalysisExt::isIUse(varRef)) {
      ...
      for (unsigned int j = 0; j < usePlaces.size(); j++) {
        ...
        //                          cout<<"\tdef "<<j<<":
        //                          "<<defPlaces[j]->unparseToString()<<endl;
        //                          cout<<"\tclass:"<<defPlaces[j]->class_name()<<endl;
        if (varRef == use) {
          //                                        cout <<"use and def in same
          //                                        node, skipping"<<endl;
          continue;
        } else if (DUVariableAnalysisExt::isIDef(use)) {
          // this is an idef, if the interesting paren is a function call get
          // the
          interestingDefParent = getNextParentInterstingNode(use);
          //                                        cout <<"\tnextInteresting:
          //                                        "<<interestingDefParent->unparseToString()<<endl;
          ...
          if (isSgFunctionCallExp(getNextParentInterstingNode(
            ...
            //                                              <<"\t*isIntersting
            //                                              is
            //                                              FunctionCallExp"<<endl;
            // since the use is the sgVarRefexp traverse upwards to find the
            // correct parameter in the functioncall
            defDepNode =
                getNode(DependenceNode::ACTUALOUT, interestingDefParent);
            establishEdge(defDepNode, useDepNode, DATA);
          } else {
    ...
    if (DUVariableAnalysisExt::isIDef(varRef)) {
      ...
      // DQ (12/6/2016): Fixed compiler warning we want to have be an error:
      // -Wsign-compare for (int j=0;j<defPlaces.size();j++)
      for (size_t j = 0; j < defPlaces.size(); j++) {
        //                          cout<<"\tdef "<<j<<":
        //                          "<<defPlaces[j]->unparseToString()<<endl;
        //                          cout<<"\tclass:"<<defPlaces[j]->class_name()<<endl;
        interestingDefParent = getNextParentInterstingNode(defPlaces[j]);
        if (isSgFunctionParameterList(interestingDefParent) ||
            isSgFunctionParameterList(interestingDefParent) ||
            (isSgFunctionParameterList(defPlaces[j]->get_parent()) &&
             isSgFunctionDeclaration(
                 defPlaces[j]->get_parent()->get_parent()))) {
          defDepNode = getNode(DependenceNode::FORMALIN, defPlaces[j]);
        } else {
          defDepNode = getNode(interestingDefParent);
        }
  ...
  Rose_STL_Container<SgNode *> functionCall =
      NodeQuery::querySubTree(_head, V_SgFunctionCallExp);
  for (Rose_STL_Container<SgNode *>::iterator call = functionCall.begin();
       call != functionCall.end(); call++) {
    ...
    SgNode *callTmp = (*call)->get_parent(); // if the paren is the interesting
                                             // node or a call break
    while (!IsImportantForSliceSgFilter(callTmp) &&
           !isSgFunctionCallExp(callTmp)) {
  ...
  Rose_STL_Container<SgNode *> returnStmts =
      NodeQuery::querySubTree(_head, V_SgReturnStmt);
  // if the last stmnt is not a return stmt, add that statement to the list,
  // such that any modified formal_in parameters are linked correctly
  if (_head->get_body()->get_statements().size() &&
      !isSgReturnStmt(*(_head->get_body()->get_statements().rbegin()))) {

    // cout <<"last stmt is "<<(*(_head->get_body ()->get_statements
    // ().rbegin()))->unparseToString()<<endl; cout <<"last stmt is
    // "<<_head->get_body ()->unparseToString()<<endl;
    //    cout <<"cfg says: "<<_head->get_declaration()->cfgForEnd
    //    ().getNode()->unparseToString()<<" is the end"<<endl;
    //   returnStmts.push_back(_head->get_declaration()->cfgForEnd
    //   ().getNode());
    ...
    //    returnStmts.push_back(*(_head->get_body ()->get_statements
    //    ().rbegin()));
  }

  // process return statements
  for (Rose_STL_Container<SgNode *>::iterator returnS = returnStmts.begin();
       returnS != returnStmts.end(); returnS++) {
    SgNode *stmt = *returnS;
    // cout <<"processing end-stantement: "<<stmt->unparseToString()<<endl;
    // if the return statement has actually some return values, connect those to
    // the formal out
    if (isSgReturnStmt(stmt) &&
        isSgReturnStmt(stmt)->get_expression() != NULL) {
      // cout <<"is return,creating out edge"<<endl;
      // check if the sub-expression of the return contains any function calls
      // for wich we have to create data-dependencys .. later
      establishEdge(
          getNode(*returnS),
          getNode(DependenceNode::FORMALRETURN, _head->get_declaration()),
          DATA);
    } else {
      ...
      //      for
      //      (list<SgNode*>::refIt=varRefList.begin();refIt!=varRefList.end();refIt++)
      //    {
      // if this varaible is not from the initial name list
      //  }
    }

    // second step: check if any of the formal in-variables have been changes,
    // if so, create a dependency from the specific cange point to the parameter
    ...
    for (SgInitializedNamePtrList::iterator paramIt = params.begin();
      ...
      for (unsigned int i = 0; i < defList.size(); i++) {
        if (defList[i] == param) {
          ...
          // cout
          // <<"\t"<<getNextParentInterstingNode(defList[i])->unparseToString()<<"
          // is valid"<<endl; connect the next interesting node ..
          establishEdge(getNode(getNextParentInterstingNode(defList[i])),
                        getNode(DependenceNode::FORMALOUT, param), DATA);
        }
      }
      ...
      for (unsigned int i = 0; i < useList.size(); i++) {
        // get the statement
        // cout <<"\tuse use stmt is: "<<useList[i]->unparseToString() <<" next
        // interesting:"<<
        // getNextParentInterstingNode(useList[i])->unparseToString()<<endl;
        if (DUVariableAnalysisExt::isIDef(useList[i])) {
          establishEdge(getNode(getNextParentInterstingNode(useList[i])),
                        getNode(DependenceNode::FORMALOUT, param), DATA);
          // cout <<"\t\tIDEF"<<endl;
        }
  ...
  // since not all functionparameters may be requried for this function, but the
  // stack has to be valid, force a data depenancy form the fucntion enrty to
  // the formal in
  for (std::set<SgInitializedName *>::iterator param =
           functionParameterSet.begin();
       param != functionParameterSet.end(); param++) {
    //            cout <<"adding backedge for slicing purpose"<<endl;
    establishEdge(getNode(DependenceNode::FORMALIN, *param),
                  getNode(DependenceNode::ENTRY, functionDef),
                  COMPLETENESS_HELPER);
  };
}

void DataDependenceGraph::computeInterproceduralInformation(
    InterproceduralInfo *) {
  // cout <<"since not all functionparameters may be requried for this function,
  // but the stack has to be valid, force a data depenancy form the fucntion
  // enrty to the formal in"<<endl;
}
...
void DataDependenceGraph::_processReturns() {
  ...
  /*  set < SimpleDirectedGraphNode * >::iterator node_iterator;
      for (node_iterator = _nodes.begin(); node_iterator != _nodes.end();
     node_iterator++)
      {
...
SgFunctionCallExp *DataDependenceGraph::_findArgExprFromRef(SgNode *&funcArg,
  ...
  while (!isSgExprListExp(funcArg->get_parent()) &&
         (funcArg->get_parent() != use)) {
  ...
  SgFunctionCallExp *funcCall =
      isSgFunctionCallExp(funcArg->get_parent()->get_parent());
