...
structureTreeAnn::structureTreeAnn(const parserID *id,
                                   structuretree_list *targets, Operator op,
                                   bool target_is_new)
    : Ann(id->line()), _name(id->name()), _targets(targets), _operator(op),
      _target_is_new(target_is_new), _is_io(id->is_io()) {}

// Single target constructor

structureTreeAnn::structureTreeAnn(const parserID *id, structureTreeAnn *target,
                                   Operator op, bool target_is_new)
    : Ann(id->line()), _name(id->name()), _targets(new structuretree_list()),
      _operator(op), _target_is_new(target_is_new), _is_io(id->is_io()) {
...
structureTreeAnn::~structureTreeAnn() {
  ...
  if (_targets) {
    for (p = _targets->begin(); p != _targets->end(); ++p)
      delete (*p);
  }
...
structureTreeAnn::structureTreeAnn(const structureTreeAnn &X) : Ann(X) {
...
void structureTreeAnn::print(int depth) const {
  ...
  if (_targets) {
    ...
    for (structuretree_list_p p = _targets->begin(); p != _targets->end();
         ++p) {
...
structureAnn::structureAnn(annVariable *source, annVariable *target,
    ...
    : Ann(line), _source(source), _target(target), _field_name() {
  if (field_name)
