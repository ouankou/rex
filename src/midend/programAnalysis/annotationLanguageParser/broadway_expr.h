...
public:
  ...
  exprAnn(Broadway::Operator op, int line) : Ann(line), _op(op) {}

  // DQ (12/6/2016): Added explicit virtual destructor to eliminate warning:
  ...
  virtual void lookup(propertyAnn *default_property, procedureAnn *procedure,
...
#ifdef __PROCLOCATION
  virtual void report(std::ostream &out, bool is_error,
...
#ifdef __PROCLOCATION
  virtual void diagnostic(std::ostream &out, procLocation *where,
...
public:
  ...
  connectiveExprAnn(Broadway::Operator op, exprAnn *lhs, exprAnn *rhs,
                    int line);
...
#ifdef __PROCLOCATION
  virtual bool test(procLocation *where, propertyAnalyzer *property_analyzer);
#endif
  virtual void lookup(propertyAnn *default_property, procedureAnn *procedure,
...
public:
  ...
  enumPropertyExprAnn(Broadway::FlowSensitivity flow_sensitivity,
                      const parserID *property, const parserID *lhs,
                      Broadway::Operator op, const parserID *rhs, int line);
...
public:
  ...
  setPropertyExprAnn(Broadway::FlowSensitivity flow_sensitivity,
                     const parserID *setProperty, const parserID *lhs,
                     Broadway::Operator op, const parserID *rhs, int line);
...
#ifdef __EXPRNODE
  numericExprAnn(exprNode *expr, int line);
#endif
...
#ifdef __EXPRNODE
  numericExprAnn(const parserID *lhs, exprNode *expr, int line);
#endif
...
public:
  ...
  pointerExprAnn(Broadway::Operator op, const parserID *lhs,
                 const parserID *rhs);
