...
public:
  ...
  analyzeAnn(Annotations *anns, const parserID *property_name, rule_list *rules,
             int line);
  ...
  void test(procLocation *where, propertyAnalyzer *property_analyzer);
#endif
...
#ifdef __PROCLOCATION
  void compute_next(procLocation *where, propertyAnalyzer *property_analyzer);
#endif
...
#ifdef __PROCLOCATION
  void apply_next(procLocation *where, propertyAnalyzer *property_analyzer,
