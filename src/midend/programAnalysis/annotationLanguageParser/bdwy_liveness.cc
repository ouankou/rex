...
bdwyLivenessAnalyzer::bdwyLivenessAnalyzer()
    : livenessAnalyzer(), _library_stmt(0), _automatically_live(false) {}

void bdwyLivenessAnalyzer::clear() { livenessAnalyzer::clear(); }
...
void bdwyLivenessAnalyzer::record_defs(procLocation *libproc_location,
  ...
  if (library_procedure->allocates_memory() ||
    ...
    for (var_set_cp p = var_defs.begin(); p != var_defs.end(); ++p) {
      annVariable *var = (*p);
