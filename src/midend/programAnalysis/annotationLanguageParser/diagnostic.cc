...
private:
  ...
  Segment(list<Location *> locs)
      : locations(locs), _id(++id_count), _flow_insensitive_property(NULL) {}
...
public:
  ...
  inline void remove_succ(Segment *s) {
    _succs.remove(s);
    s->_preds.remove(this);
  }
  inline void remove_pred(Segment *p) {
    _preds.remove(p);
    p->_succs.remove(this);
  }
...
void Diagnostic::trace(ostream &out, Location *loc, pointerValue &pv) {
  // Performs "backward" tracing from loc. The property value of interest is in
  // pv.
  ...
  for (memoryblock_set_p r = pv.blocks.begin(); r != pv.blocks.end(); ++r) {
    ...
    if (property_block) {
      ...
      enumvalue_set values =
          _property->construct_now_value(loc, property_block, false);
      if (!contain_diagnostic_value(values))
        continue;

      out << "      property_block " << property_block << " "
          << property_block->name() << endl;
  ...
  shortest.clear();
  for (Segments::iterator s = starts.begin(); s != starts.end(); s++) {
    if (*s == Segment::brokenSegment() || (*s)->flow_insensitive_property())
      continue;
    Segments visited;
    find_shortest(*s, visited, shortest);
  }
...
Diagnostic::Segment *Diagnostic::trace(Location *from, memoryDef *def) {
  ...
  if (_def2uses[step].find(def) != _def2uses[step].end())
    ...
    for (def2uses::iterator m = _def2uses[step].begin();
      ...
      cout << "   " << m->first << " " << *m->first->where()
           << " owner=" << m->first->owner() << "-" << m->first->owner()->name()
           << endl;
    }
    assert(false);
  }
...
Diagnostic::Segment *Diagnostic::trace(Location *from, Location *to) {
  ...
  for (list<Location *>::reverse_iterator l = l_to.rbegin(); l != l_to.rend();
       l++)
    result.push_back(*l);
...
#define print_location                                                         \
  ...
  case Location::Procedure:                                                    \
    out << ((procLocation *)*l)->proc()->decl()->name();                       \
  }

  out << "{" << id() << "} ";
  for (list<Location *>::iterator l = locations.begin(); l != locations.end();
       l++) {
    if (l != locations.begin())
      out << " --> ";
    print_location
  }
} // trace(Location)
...
void Diagnostic::find_shortest(Segment *cur, Segments path,
                               Segments &shortest) {
