...
// DQ (12/10/2016): Added assert.h to support fix for warning we want to make an
// error.
...
Annotations::Annotations(string &filename, str_list *cpp_flags)
    : _filename(filename), _globals(), _init(0), _enum_properties(),
      _constants(0), _set_properties(), _procedures(), _errors(0),
#ifdef __HEADER
      _header(0),
#endif
...
#endif
{
  ...
  _init = new procedureAnn(&init_name, (parserid_list *)0, this, 0);
...
Annotations::~Annotations() {
  ...
  for (var_map_p p = _globals.begin(); p != _globals.end(); ++p) {
  ...
  for (enum_property_map_p p = _enum_properties.begin();
       p != _enum_properties.end(); ++p)
    delete (*p).second;

  for (set_property_map_p p = _set_properties.begin();
       p != _set_properties.end(); ++p)
    delete (*p).second;

  // -- Delete all procedure annotations

  for (procedures_map_p p = _procedures.begin(); p != _procedures.end(); ++p)
    delete (*p).second;
}
...
void Annotations::add_procedure(procedureAnn *new_procedure) {
  ...
  if (old != 0)
    Error(new_procedure->line(),
          string("Procedure ") + new_procedure->name() + " is already defined");
  ...
  if (decl) {
    if (decl->type()->typ() != Func)
      Warning(new_procedure->line(),
              string("Annotation ") + new_procedure->name() +
                  " is not a function in the header -- annotation ignored.");
    else {
      ...
      if (func->args().size() != new_procedure->formal_params().size()) {
        ...
        if (!(func->is_void_args() &&
              (new_procedure->formal_params().size() == 0)))
          Warning(new_procedure->line(),
                  string("In procedure ") + new_procedure->name() +
                      ", number of arguments does not match header -- "
                      "annotation ignored.");
        else
          ok_to_add = true;
      } else {
        funcNode *type_copy =
            (funcNode *)ref_clone_changer::clone(decl->type(), false);
    ...
    Warning(new_procedure->line(),
            string("Procedure ") + new_procedure->name() +
                " is not declared in the header -- annotation ignored.");
#else
  ok_to_add = true;
#endif
...
void Annotations::add_globals(structuretree_list *structures) {
  if (structures) {
    for (structuretree_list_p p = structures->begin(); p != structures->end();
      ...
      annVariable *top_var =
          add_one_global(top_node->name(), top_node->is_io());
...
annVariable *Annotations::new_global(const string &varname, bool is_external,
                                     bool is_io) {
  bool is_new = false;
  bool is_global = true;
  bool is_formal = false;
  ...
  annVariable *g = new annVariable(varname, procname, is_new, is_global,
                                   is_external, is_formal, is_io);
...
void Annotations::add_enum_property(enumPropertyAnn *property) {
  ...
  if (!existing) {
    ...
    Error(property->line(),
          string(" property \"") + property->name() + "\" already defined.");
}
...
void Annotations::add_set_property(setPropertyAnn *setProperty) {
  ...
  if (!existing) {
    ...
    Error(setProperty->line(), string(" set property \"") +
                                   setProperty->name() + "\" already defined.");
}
...
void Annotations::Error(int where, const string &msg) {
  ...
  cout << "Error in \"" << _filename << "\" line " << where << ": " << msg
       << endl;
...
void Annotations::Warning(int where, const string &msg) {
  cout << "Warning: in \"" << _filename << "\" line " << where << ": " << msg
       << endl;
...
void Annotations::print(ostream &o) const {
  ...
  o << "global {";
  for (var_map_cp p = _globals.begin(); p != _globals.end(); ++p) {
  ...
  for (procedures_map_cp p = _procedures.begin(); p != _procedures.end(); ++p) {
...
void Annotations::store_header(decl_list *decls) {
  ...
  for (decl_list_p p = decls->begin(); p != decls->end(); ++p)
...
FILE *Annotations::_open_input_file() {
  ...
  if (!std_in && (access(_filename.c_str(), F_OK) != 0) &&
  ...
  if (CBZ::Preprocess && !std_in) {
    ...
    for (str_list_p p = _cpp_flags->begin(); p != _cpp_flags->end(); ++p)
      command += " " + (*p);
