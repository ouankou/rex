...
setPropertyAnn::setPropertyAnn(const parserID *id, Direction direction,
                               SetPropertyKind kind, MeetFunction meet)
    : propertyAnn(id, direction, SetProperty), _kind(kind),
      _meet_function(meet), _property_variable(0)
#ifdef __FOO
      ,
      _block(0), _set(), _equivalence(), _position_of_memoryblock(),
      _memoryblock_at_position(), _change(false)
#endif
{
  if (kind == Set)
...
void setPropertyAnn::meet(memoryblock_bitset &dest, memoryblock_bitset &src) {
  if (_meet_function == Union)
    dest |= src;
...
bool setPropertyAnn::is_element_of(pointerValue &var) {
  ...
  current_set_value(current_bits, found_reaching_value);
...
void setPropertyAnn::apply_set_merge(memoryuse_list &phi_uses,
  ...
  if (direction() == Forward) {
    ...
    current_set_value(merged_val, found_reaching_value);
...
void setPropertyAnn::to_top(memoryblock_bitset_list &bits) { bits.clear(); }

/** @brief To bottom
 *
 * Make the given equivalence into the lattice "BOTTOM" value. */

void setPropertyAnn::to_bottom(memoryblock_bitset_list &bits) { bits.clear(); }
...
void setPropertyAnn::meet(memoryblock_bitset_list &dest,
  ...
  for (memoryblock_bitset_list_cp p = src.begin(); p != src.end(); ++p) {
...
bool setPropertyAnn::update_equivalence_classes(
    memoryAccess *def_or_use, memoryblock_bitset_list &new_set) {
  if (Annotations::Verbose_properties) {
    cout << "      + Update " << name() << endl;
...
bool setPropertyAnn::update_current_equivalence_classes(
    memoryblock_bitset_list &new_classes) {
  // -- Get the property variable object
...
bool setPropertyAnn::is_equivalent(pointerValue &left, pointerValue &right) {
  ...
  // -- Visit each class to see if there is one that contains all of the
  // variables

  for (memoryblock_bitset_list_p p = equiv.begin(); p != equiv.end(); ++p) {
...
void setPropertyAnn::add_equivalence(pointerValue &left, pointerValue &right) {
  // -- Get the current list of equivalence classes
...
void setPropertyAnn::apply_equivalence_merge(memoryuse_list &phi_uses,
  ...
  if (direction() == Forward) {
    ...
    current_equivalence_classes(merged_val, found_reaching_value);
...
void setPropertyAnn::self_assignment(memoryblock_set &changes) {
  ...
  if (kind() == Set) {
    ...
    current_set_value(value, found_reaching_value);
  ...
  if (kind() == Equivalence) {
    ...
    current_equivalence_classes(value, found_reaching_value);
  ...
  for (memoryblock_set_p p = variables.blocks.begin();
    ...
    if (q == _position_of_memoryblock.end()) {
      ...
      if (position == 1024) {
        ...
        cerr << "INTERNAL ERROR: Too many objects in the bitset representation "
                "of "
             << name() << endl;
      } else {
        ...
        // cout << "Property " << name() << ": position of " << block->name() <<
        // " = " << position << endl;
      }
    } else
      position = (*q).second;
...
void setPropertyAnn::add_equivalence_class(
    memoryblock_bitset_list &equiv, const memoryblock_bitset &new_class) {
  // -- Algorithm:
  //
...
bool setPropertyAnn::is_same_equivalence(memoryblock_bitset_list &equiv1,
  ...
  for (memoryblock_bitset_list_p p = equiv1.begin(); p != equiv1.end(); ++p) {
    memoryblock_bitset &look_for = *p;

    bool found = false;

    for (memoryblock_bitset_list_p q = equiv2.begin(); q != equiv2.end(); ++q) {
...
void setPropertyAnn::print(ostream &o) const { o << name() << endl; }
...
void setPropertyAnn::print_memoryblock_bitset_list(
    const string &label, memoryblock_bitset_list &bits, ostream &out) {
  int size = _memoryblock_at_position.size();

  out << label << "{ ";

  for (memoryblock_bitset_list_p p = bits.begin(); p != bits.end(); ++p) {
