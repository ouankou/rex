...
pointerRuleAnn::pointerRuleAnn(exprAnn *condition,
                               structuretree_list *effects_tree, int line)
    : Ann(line), _condition(condition), _is_condition_true(false),
      _effects_tree(effects_tree), _effects() {}
...
pointerRuleAnn::~pointerRuleAnn() {
  ...
  for (p = _effects.begin(); p != _effects.end(); ++p)
    delete (*p);
}

pointerRuleAnn::pointerRuleAnn(const pointerRuleAnn &X) : Ann(X) {
...
void pointerRuleAnn::lookup(procedureAnn *procedure, Annotations *annotations) {
