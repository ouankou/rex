...
procedureAnn::procedureAnn(parserID *id, parserid_list *params,
    ...
    : Ann(line), _name(id->name()), _variables(), _formal_params(), _uses(),
      _defs(), _global_uses(), _global_defs(), _property_uses(),
      _property_defs(), _property_weak_defs(), _on_entry(), _on_exit(),
      _pointer_rules(), _default_pointer_rule(0), _analyses(), _reports(),
#ifdef __FOO
      _actions(), _proc(0),
#endif
      _numeric_identifiers(), _annotations(anns),
#ifdef __FOO
      _callsites(),
#endif
      _pure_function(false), _allocates_memory(false),
      _deallocates_memory(false), _modifies_globals(false),
      _temp_defs(new parserid_list()), _temp_uses(new parserid_list()),
  ...
  if (params) {
    int count = 0;
    for (parserid_list_p p = params->begin(); p != params->end(); ++p) {
      parserID &id = *p;
...
procedureAnn::~procedureAnn() {
  ...
  for (var_map_p p = _variables.begin(); p != _variables.end(); ++p) {
  ...
  for (structure_list_p p = _on_entry.begin(); p != _on_entry.end(); ++p)
    delete (*p);

  for (structure_list_p p = _on_entry.begin(); p != _on_entry.end(); ++p)
    delete (*p);

  for (pointerrule_list_p p = _pointer_rules.begin(); p != _pointer_rules.end();
       ++p)
    delete (*p);

  // -- Delete analysis annotations

  for (analyze_map_p p = _analyses.begin(); p != _analyses.end(); ++p)
    delete (*p).second;

  // -- Delete report annotations

  for (report_list_p p = _reports.begin(); p != _reports.end(); ++p)
    delete (*p);

  // -- Delete action annotations

#ifdef __ACTIONS
  for (action_list_p p = _actions.begin(); p != _actions.end(); ++p)
    delete (*p);
#endif
}
...
annVariable *procedureAnn::lookup(const string &variable, bool create,
  ...
  if (p == _variables.end()) {
    ...
    if (create) {
      ...
      annVariable *new_var = new annVariable(
          variable, name(), is_new, is_global, is_external, is_formal, is_io);
...
void procedureAnn::add_defs(parserid_list *variables) {
  ...
  for (parserid_list_p p = variables->begin(); p != variables->end(); ++p)
...
void procedureAnn::add_delete(parserID *id) { _temp_deletes->push_back(*id); }
...
void procedureAnn::postprocess() {
  ...
  for (parserid_list_p i = _temp_deletes->begin(); i != _temp_deletes->end();
       ++i) {
    parserID &id = *i;
    int line = id.line();
  ...
  for (report_list_p q = _reports.begin(); q != _reports.end(); ++q)
    (*q)->lookup(this, _annotations);

  // --- Post-process the action list

#ifdef __ACTIONS
  for (action_list_p q = _actions.begin(); q != _actions.end(); ++q)
    (*q)->lookup(this, _annotations);
#endif
  ...
  for (var_set_p p = _global_defs.begin(); p != _global_defs.end(); ++p)
    if ((*p)->is_io())
      _accesses_io = true;

  for (var_set_p p = _global_uses.begin(); p != _global_uses.end(); ++p)
    if ((*p)->is_io())
      _accesses_io = true;

  if (!_allocates_memory && !_deallocates_memory && !_modifies_globals &&
      !_accesses_io)
    _pure_function = true;
  ...
  if (_pure_function && _defs.empty() && _property_defs.empty())
    cout << "WARNING: procedure \"" << name()
         << "\" appears to have no effects at all." << endl;
}
...
void procedureAnn::postprocess_variables(parserid_list &uses_or_defs,
  ...
  for (parserid_list_p p = uses_or_defs.begin(); p != uses_or_defs.end(); ++p) {
    parserID &id = *p;
    int line = id.line();
    string &varname = id.name();
...
void procedureAnn::add_on_entry(structuretree_list *structures) {
  if (structures) {
    for (structuretree_list_p p = structures->begin(); p != structures->end();
         ++p) {
      ...
      if (top_var)
        ...
        _annotations->Error(top_node->line(),
                            string("Could not find target \"") +
                                top_node->name() + string("\""));
...
void procedureAnn::add_structures(bool is_on_entry, structure_list &the_list,
  ...
  for (structuretree_list_p p = structures->begin(); p != structures->end();
    ...
    if (parent_node->op() == structureTreeAnn::Arrow) {
      ...
      if (child_var)
        new_struc =
            new structureAnn(parent_var, child_var, 0, child_node->line());
    }
    ...
    if (parent_node->op() == structureTreeAnn::Dot) {
      ...
      if (child_var)
        new_struc = new structureAnn(parent_var, child_var, &child_node->name(),
                                     child_node->line());
    }
    ...
    if (new_struc) {
      ...
      if (parent_var->name() == "null")
        _annotations->Error(child_node->line(),
                            string("the \"null\" object cannot point to "
                                   "anything or be decomposed."));
...
void procedureAnn::add_global_structures(Annotations *annotations,
  ...
  if (!pointer_rule) {
    ...
    _default_pointer_rule = new pointerRuleAnn(
        (exprAnn *)0, (structuretree_list *)0, parent_node->line());
  ...
  for (structuretree_list_p p = structures->begin(); p != structures->end();
    ...
    if (parent_node->op() == structureTreeAnn::Arrow) {
      child_var =
          annotations->add_one_global(child_node->name(), child_node->is_io());
      new_struc =
          new structureAnn(parent_var, child_var, 0, child_node->line());
    ...
    if (parent_node->op() == structureTreeAnn::Dot) {
      ...
      string child_name = parent_var->name() + "." + child_node->name();
      child_var = annotations->add_one_global(child_name, child_node->is_io());
      new_struc = new structureAnn(parent_var, child_var, &child_node->name(),
                                   child_node->line());
    ...
    // DQ (9/12/2011): Static analysis reports that this could be NULL, check
    // for it explicitly.
    assert(child_node != NULL);
    if (child_node->targets()) {
      // DQ (9/13/2011): Static analysis reports that this could be NULL, check
      // for it explicitly.
...
void procedureAnn::add_report(reportAnn *report) { _reports.push_back(report); }

#ifdef __FOO

void procedureAnn::add_action(actionAnn *action) { _actions.push_back(action); }
...
void procedureAnn::clear() {
  ...
  for (callsite_map_p p = _callsites.begin(); p != _callsites.end(); ++p)
    delete (*p).second;
...
void procedureAnn::process_reports(Analyzer *analyzer) {
  ...
  for (callsite_map_cp ccp = callsites().begin(); ccp != callsites().end();
       ++ccp) {
    ...
    for (proclocation_set_cp pp = proclocations.begin();
      ...
      for (report_list_cp p = reports().begin(); p != reports().end(); ++p)
...
void procedureAnn::test_actions(Analyzer *analyzer) {
  ...
  for (callsite_map_cp ccp = callsites().begin(); ccp != callsites().end();
    ...
    for (proclocation_set_cp pp = proclocations.begin();
      ...
      for (action_list_cp p = actions().begin(); p != actions().end(); ++p)
...
void procedureAnn::print(ostream &o) const {
  ...
  for (var_list_cp p = _formal_params.begin(); p != _formal_params.end(); ++p) {
    annVariable *decl = *p;
  ...
  o << "  on_entry {" << endl;
  for (structure_list_cp p = _on_entry.begin(); p != _on_entry.end(); ++p) {
    structureAnn *sa = *p;
    o << "    " << *sa << ";" << endl;
  }
  o << "  }" << endl;
  ...
  o << "  access { ";
  for (var_set_cp p = _uses.begin(); p != _uses.end(); ++p) {
    annVariable *decl = *p;
    if (p != _uses.begin())
      o << ", ";
    o << decl->name();
  ...
  o << "  modify { ";
  for (var_set_cp p = _defs.begin(); p != _defs.end(); ++p) {
    annVariable *decl = *p;
    if (p != _defs.begin())
      o << ", ";
    o << decl->name();
  ...
  o << "  on_exit {" << endl;
  for (structure_list_cp p = _on_exit.begin(); p != _on_exit.end(); ++p) {
    structureAnn *sa = *p;
    o << "    " << *sa << ";" << endl;
  }
  for (pointerrule_list_cp r = _pointer_rules.begin();
       r != _pointer_rules.end(); r++) { // TB
    if ((*r)->condition()) {
      cout << "    if ( ";
  ...
  o << "  // variables: " << endl;
  for (var_map_cp p = _variables.begin(); p != _variables.end(); ++p) {
  ...
  for (var_set_cp p = _global_defs.begin(); p != _global_defs.end(); ++p) {
    annVariable *glob = *p;
    o << "  //    " << *glob << endl;
  }
  o << endl;
  ...
  for (var_set_cp p = _global_uses.begin(); p != _global_uses.end(); ++p) {
    annVariable *glob = *p;
    o << "  //    " << *glob << endl;
  }
  o << endl;
