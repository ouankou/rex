...
public:
  ...
  setPropertyAnn(const parserID *name, Direction direction,
...
#ifdef __FOO
  void set_property_variable_block(memoryBlock *block) { _block = block; }
  ...
  /** @brief To top
   *
   * Make the given set into the lattice "TOP" value. For union sets, this is
   * the empty set. For intersection sets, this is the universe set. */
  ...
  void meet(memoryblock_bitset &dest, memoryblock_bitset &src);
  ...
  void lookup_set_value(memoryAccess *def_or_use, memoryblock_bitset &bits);
  ...
  bool update_set_value(memoryAccess *def_or_use, memoryblock_bitset &new_set);
  ...
  void current_set_value(memoryblock_bitset &bits, bool &found_reaching_value);
  ...
  void apply_set_merge(memoryuse_list &phi_uses, memoryblock_set &changes);
  //}
  ...
  void meet(memoryblock_bitset_list &dest, const memoryblock_bitset_list &src);
   ...
   * using the given input bitset. If the def or use is additive, meet the value
   * in with the existing value, otherwise just overwrite it. */
  ...
  bool is_equivalent(pointerValue &left, pointerValue &right);

  /** @brief Add an equivalence
   *
   * */

  void add_equivalence(pointerValue &left, pointerValue &right);
  ...
  void apply_merge(memoryuse_list &phi_uses, memoryblock_set &changes);
...
private:
  ...
  void print_memoryblock_bitset(const std::string &label,
                                memoryblock_bitset &bits, std::ostream &out);
