...
propertyAnn::propertyAnn(const parserID *id, Direction direction,
                         bool enforce_weak_updates,
                         const parserID *default_name,
                         propertyvalue_list *lowest_values)
    : Ann(id->line()), _name(id->name()), _is_enabled(true),
      _direction(direction), _enforce_weak_updates(enforce_weak_updates),
      _values(0), _top(0), _value_index(), _now_values(), _always_values(),
      _ever_values() {
...
void propertyAnn::number_values(propertyValueAnn *prop, int &cur_index,
...
propertyValueAnn *propertyAnn::lookup(const string &name) {
  for (propertyvalue_vec_p p = _value_index.begin(); p != _value_index.end();
       ++p)
    if ((*p)->name() == name)
      return *p;
...
bool propertyAnn::at_least(propertyValueAnn *higher, propertyValueAnn *lower) {
  // X <= Y <==> X ^ Y == X
...
bool propertyAnn::test(exprAnn::Operator op,
                       ...
                       pointerValue &lhs, string &lhs_name, pointerValue &rhs,
                       string &rhs_name) {
  bool result = false;
  ...
  case exprAnn::Ever: {
    ...
    for (propertyvalue_set_p p = rhs_vals.begin(); p != rhs_vals.end(); ++p) {
      rhs_value = *p;
  ...
  default:
    cerr << "ERROR: propertyAnn: invalid flow sensitivity mode passed to test()"
         << endl;
  }
...
bool propertyAnn::test(exprAnn::Operator op,
  ...
  case exprAnn::Ever: {
    ...
    for (propertyvalue_set_p p = lhs_vals.begin(); p != lhs_vals.end(); ++p) {
      lhs_value = *p;
...
bool propertyAnn::test(exprAnn::Operator op, propertyValueAnn *lhs_value,
                       propertyValueAnn *rhs_value) {
  bool result = false;
...
bool propertyAnn::update_now_value(memoryBlock *block, memoryAccess *def_or_use,
...
propertyValueAnn *propertyAnn::construct_now_value(memoryBlock *block) {
  ...
  if (Annotations::debug)
    cout << "        - Lookup \"Now\" value of " << block->decl()->name()
         << endl;
...
void propertyAnn::construct_ever_value(pointerValue &variable, string &name,
  ...
  for (memoryblock_set_p p = variable.blocks.begin();
    ...
    if (q != _ever_values.end()) {
      ...
      for (propertyvalue_set_p w = vals.begin(); w != vals.end(); ++w) {
  ...
  if (Annotations::debug) {
    cout << "          = { ";
    for (propertyvalue_set_p p = values.begin(); p != values.end(); ++p)
      cout << (*p)->name() << " ";
    cout << "}" << endl;
  }
}

propertyValueAnn *propertyAnn::construct_weak_now_value(pointerValue &variable,
                                                        string &name) {
  propertyValueAnn *result = top();
  ...
  for (memoryblock_set_p p = variable.blocks.begin();
    ...
    if (block->current_def() && block->current_def()->is_weak()) {
...
void propertyAnn::apply_assignment(pointerValue &left, pointerValue &right,
  ...
  if (!right.is_address) {
    ...
    for (memoryblock_set_p p = blocks.begin(); p != blocks.end(); ++p) {
      memoryBlock *block = *p;
...
void propertyAnn::report(ostream &out,
  ...
  default:
    cerr << "ERROR: propertyAnn: invalid flow sensitivity mode passed to "
            "report()"
         << endl;
  }
}
...
void propertyAnn::print(ostream &o, propertyValueAnn *prop, int depth) const {
  ...
  for (propertyvalue_list_cp p = vals.begin(); p != vals.end(); ++p)
    print(o, *p, depth + 2);
}
