...
annVariable::annVariable(string name, string procedure_name, bool is_new,
                         bool is_global, bool is_external, bool is_formal,
                         bool is_io)
    : _name(name),
#ifdef __BINDINGS
      _bindings(),
      ...
      _decl(0),
#endif
      _is_new(is_new), _is_used(false), _is_defined(false),
      _is_global(is_global), _is_external(is_external), _is_formal(is_formal),
      _is_io(is_io), _is_deleted(false), _is_property_variable(false) {
...
#ifdef __DECL
    _decl = new declNode(variable_name.c_str(), declNode::NONE, (typeNode *)0,
...
pointerValue & annVariable::binding(procLocation * loc)
{
  if ((loc == 0) &&
      ! _is_global)
    cerr << "annVariable: ERROR: Looking for global binding of a local
variable!" << endl;
