...
bool UnificationBasedPtr_Ann::isField(declNode *f,
                                      bool &from_annotation) const {
...
void UnificationBasedPtr_Ann::mark_alloc(stmtNode *stmt, declNode *source_decl,
                                         declNode *target_decl) {
  if (!stmt || stmt->typ() != ThreeAddr)
    return;
...
void UnificationBasedPtr_Ann::annotation_call_arg(procNode *p, int arg,
                                                  typeNode *arg_type,
                                                  Alpha *alpha) {
  if (!p)
    return;
  ...
  for (pointerrule_list_p r = pointer_rules.begin(); r != pointer_rules.end();
       r++)
  ...
  for (var_list_p f = formals.begin(); i <= arg; i++) {
    if (i == arg) {
      ...
      process_struct_rule(*f, (*f)->decl(), arg_type, struct_rules, tao->ecr());
    }
    if (*f != formals.back())
      f++;
  }
...
void UnificationBasedPtr_Ann::annotation_ret_val(procNode *p, Unify_ECR *taoR,
  ...
  typeNode *ret_type =
      ((funcNode *)p->decl()->type())->returns()->follow_tdefs();
        ...
        if (!(*e)->target()->is_new()) {
          ...
          if (global_var) { // yes, it is global
            ...
            if (!target_ecr) {
              ...
              bool is_synthetic;
              declNode *decl =
                  linker.lookup_symbol(cur_unit, target_name, is_synthetic);
              if (decl) {
          ...
          if (target_ecr) {
            ret_alpha =
                new Alpha(target_ecr, new Unify_Offset(Unify_Offset::zero));
...
void UnificationBasedPtr_Ann::process_struct_rule(
    annVariable *source, declNode *decl, typeNode *dtype,
    const structure_list &struct_rules, Unify_ECR *ecr) {
  assert(ecr);
    ...
    if ((*r)->source() == source && (*r)->target()->name() != "null") {
      ...
      if ((*r)->field_name().empty()) { // operator is "-->" ie. dereference.
        ...
        if (dtype && (dtype->typ() == Struct || dtype->typ() == Union)) {
          ...
          for (decl_list_p f = sue->fields().begin(); f != sue->fields().end();
               f++)
  ...
  if (decl) {
    ...
    if (source->is_global()) {
      if (!global_var_ecr[source])
        global_var_ecr[source] = _ecr[decl];
      if (!global_var_type[source])
        global_var_type[source] = dtype;
    }
    _uses[decl] = true;
  }
} // process_struct_rule
