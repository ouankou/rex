...
ruleAnn::ruleAnn(exprAnn *condition, exprann_list *effects, int line)
    : Ann(line), _condition(condition), _is_condition_true(false), _effects() {
  if (effects) {
...
void ruleAnn::test(procLocation *where, propertyAnalyzer *property_analyzer) {
...
void ruleAnn::compute_next(procLocation *where) {
  for (exprann_list_p p = _effects.begin(); p != _effects.end(); ++p)
...
void ruleAnn::lookup(propertyAnn *default_property, procedureAnn *procedure,
