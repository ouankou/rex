...
public:
  ...
  enumvalue_set() : bitset32() {}

  /** @brief Construct a single element set
   *
   * Contains the single given element */

  enumvalue_set(enumValueAnn *val) : bitset32() { set(val->id()); }
...
private:
  ...
  /** @brief Diagnostic
   *
   * When the property declares that error diagnostic on some values are
   desired, an instance of Diagnostic is created; else it is NULL. */
...
public:
  ...
  enumPropertyAnn(const parserID *name, Direction direction,
                  bool enforce_weak_updates, const parserID *default_name,
  ...
  enumValueAnn *meet(enumValueAnn *one, enumValueAnn *two);
  ...
  bool test(Broadway::Operator op, Broadway::FlowSensitivity flow_sensitivity,
            Location *where, pointerValue &lhs, std::string &lhs_name,
  ...
  bool test(Broadway::Operator op, enumvalue_set lhs_value_set,
            enumvalue_set rhs_value_set);
...
#ifdef __MEMORYACCESS
  bool update_now_value(Location *where, memoryBlock *property_block,
                        memoryAccess *def_or_use, enumvalue_set value,
...
#ifdef __MEMORYBLOCK
  enumvalue_set construct_now_value(Location *where, pointerValue &variable,
  ...
  enumvalue_set construct_after_value(pointerValue &variable,
                                      std::string &name);
...
#ifdef __MEMORYBLOCK
  enumvalue_set compute_next(Location *where, ruleAnn *rule, exprAnn *expr,
                             pointerValue &right, std::string &right_name,
...
#ifdef __MEMORYBLOCK
  void apply_next(Location *where, stmtLocation *parameter_callsite,
                  ruleAnn *rule, enumPropertyExprAnn *expr, pointerValue &left,
                  std::string &left_name, pointerValue &right,
                  enumvalue_set new_value, bool rhs_lost_information,
                  bool rhs_changed, // TB_unify
                  memoryblock_set &complicit_property_blocks,
                  enumvalue_set ever_values, memoryblock_set &changes);
#endif
  /** @brief Apply merge
   *
...
#ifdef __MEMORYBLOCK
  void apply_merge(Location *where, memoryBlock *property_block,
                   memoryuse_list &phi_uses, memoryblock_set &changes);
#endif
...
#ifdef __MEMORYBLOCK
  void apply_assignment(Location *where, stmtLocation *parameter_callsite,
                        pointerValue &left, pointerValue &right,
                        bool is_parameter, memoryblock_set &changes);
#endif
...
#ifdef __MEMORYBLOCK
  void self_assignment(Location *source, Location *target,
                       memoryBlock *property_block, memoryblock_set &changes,
                       bool is_input);
#endif
  ...
  void report(std::ostream &out, bool is_error, procLocation *where,
              Broadway::FlowSensitivity flow_sensitivity, pointerValue &lhs,
              std::string &lhs_name);
...
#ifdef __FLOW_SENSITIVE_SET
  inline int count_flow_sensitive_objects() {
...
private:
  ...
  void number_values(enumValueAnn *prop, int &cur_index, int height,
                     int &max_height);
...
#ifdef __MEMORYBLOCK
  void track_destructive_assignments(
      Analyzer *analyzer, Location *where, memoryBlock *block,
      ...
      memoryblock_vector &chain, std::string &indent);
#endif
...
#ifdef __MEMORYBLOCK
  void record_tested_objects(Location *where, pointerValue &ptr,
...
#ifdef __MEMORYBLOCK
  void trace_object(std::ostream &out, memoryBlock *property_block,
                    memoryblock_set &already_seen, std::string &indent);
#endif
...
#ifdef __MEMORYBLOCK
  bool evaluate_context_sensitivity(Analyzer *analyzer, Location *target,
                                    memoryBlock *block, procLocation *procloc,
                                    bool destructive,
                                    block_proc_set &eval_cs_seen_destructive,
...
#ifdef __MEMORYBLOCK
  bool validate_pointer_fs(memoryBlock *block, stmtLocation *where,
                           std::string &indent);
#endif
...
#ifdef __MEMORYBLOCK
  bool is_location_reachable(Location *source, Location *target,
                             memoryBlock *block);
#endif
