...
public:
  ...
  callsiteEntry(threeAddrNode *stmt, operandNode *call)
      : _stmt(stmt), _call(call), _contexts() {}
...
public:
  ...
  inline const pointerrule_list &pointer_rules() const {
    return _pointer_rules;
  }
  inline pointerRuleAnn *default_pointer_rule() const {
    return _default_pointer_rule;
  }
   ...
   *  @param create pass true to create a new variable if one doesn't already
   * exist
   *  @param is_new pass true if the object is allocated in the annoation
   */

  annVariable *lookup(const std::string &variable, bool create,
                      bool is_new = false);
  ...
  inline const variable_property_map &property_uses() const {
    return _property_uses;
  }
  ...
  inline const variable_property_map &property_defs() const {
  ...
  inline const variable_property_map &property_weak_defs() const {
    return _property_weak_defs;
  }
  ...
  void add_global_structures(Annotations *annotations, annVariable *parent_var,
...
private:
  void add_structures(bool is_on_entry, structure_list &the_list,
                      annVariable *parent_var, structureTreeAnn *parent_node);
