...
exprPatternNode::exprPatternNode(const parserID *var)
    : exprNode(Meta, 0, Coord(var->line(), 0, -1)), _name(var->name()),
      _variable(0) {}
...
void exprPatternNode::lookup(procedureAnn *procedure,
  ...
  if (!_variable)
    annotations->Error(coord().line(),
                       string("Undeclared identifier \"") + name() + "\"");
}
...
Node *exprPatternNode::change(Changer &the_changer, bool redispatch) {
  ...
  if ((ord == Changer::Preorder || ord == Changer::Both) && !redispatch)
    the_exprpattern = (exprPatternNode *)the_changer.at_expr(the_exprpattern,
                                                             Changer::Preorder);
  ...
  if ((ord == Changer::Postorder || ord == Changer::Both) && !redispatch)
    the_exprpattern = (exprPatternNode *)the_changer.at_expr(
        the_exprpattern, Changer::Postorder);

  return the_exprpattern;
}

void exprPatternNode::output_expr(output_context &ct, Node *par, int prec,
                                  Assoc assoc) {}
...
stmtPatternNode::stmtPatternNode(const parserID *var)
    : stmtNode(Meta, Coord(var->line(), 0, -1)), _name(var->name()),
      _variable(0) {}
...
Node *stmtPatternNode::change(Changer &the_changer, bool redispatch) {
  ...
  if ((ord == Changer::Preorder || ord == Changer::Both) && !redispatch)
    the_stmtpattern = (stmtPatternNode *)the_changer.at_stmt(the_stmtpattern,
                                                             Changer::Preorder);
  ...
  if ((ord == Changer::Postorder || ord == Changer::Both) && !redispatch)
    the_stmtpattern = (stmtPatternNode *)the_changer.at_stmt(
        the_stmtpattern, Changer::Postorder);
...
Node *typePatternNode::change(Changer &the_changer, bool redispatch) {
  ...
  if ((ord == Changer::Preorder || ord == Changer::Both) && !redispatch)
    the_typepattern = (typePatternNode *)the_changer.at_type(the_typepattern,
                                                             Changer::Preorder);
  ...
  if ((ord == Changer::Postorder || ord == Changer::Both) && !redispatch)
    the_typepattern = (typePatternNode *)the_changer.at_type(
        the_typepattern, Changer::Postorder);

  return the_typepattern;
}

void typePatternNode::output_type(output_context &ct, Node *par, Assoc context,
                                  Type_qualifiers q) {}
...
patternExpander::patternExpander(callsiteEntry *callsite,
                                 ...
                                 Annotations *annotations)
    : Changer(Postorder, Subtree, true), _callsite(callsite),
      _analyzer(analyzer), _annotations(annotations) {}
...
Node *patternExpander::at_type(typeNode *the_type, Order ord) {
  ...
  if (the_type->typ() == Meta) {
    ...
    if (var->name() == "return") {
      ...
      if (stmt->lhs() && stmt->op() &&
          stmt->op()->id() == Operator::FUNC_CALL && stmt->lhs()->type())
        // -- We have a left-hand side, check for a type
        return ref_clone_changer::clone(stmt->lhs()->type(), false);

      // -- Fall through: no left-hand side, so issue an error

      ost << "Expansion of $type:return failed: no left-hand side at the "
             "callsite "
          << _callsite->call()->coord();
      ...
      if (var_binding.blocks.size() != 1) {
        ost << "Improper binding for replacement of \"" << var->name()
            << "\" at " << _callsite->call()->coord();
        _annotations->Error(the_type_pattern->coord().line(), ost.str());
      }

      memoryBlock *mb = *(var_binding.blocks.begin());

      // -- The expansion is a copy of the type on the declNode of the
      // memoryBlock.
