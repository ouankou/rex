...
public:
  reportElementAnn(int line) : Ann(line) {}

  virtual void lookup(enumPropertyAnn *default_property,
                      procedureAnn *procedure, Annotations *annotations) = 0;

#ifdef NONEMPTY
  virtual void report(std::ostream &out, bool is_error, Analyzer *analyzer,
...
public:
  ...
  reportAnn(exprAnn *condition, bool is_error, report_element_list *elements,
            int line);

  virtual void lookup(procedureAnn *procedure, Annotations *annotations);

#ifdef NONEMPTY
  virtual void report(std::ostream &out, Analyzer *analyzer,
                      procLocation *where, propertyAnalyzer *property_analyzer);
#endif
};
