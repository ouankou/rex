...
enumValueAnn::enumValueAnn(const parserID *id, enumvalue_list *more_specific)
    : Ann(id->line()), _name(id->name()), _more_general(0), _more_specific(),
      _id(0), _is_leaf(false) {
  if (more_specific) {
    _more_specific.swap(*more_specific);

    for (enumvalue_list_p p = _more_specific.begin(); p != _more_specific.end();
         ++p)
...
enumPropertyAnn::enumPropertyAnn(const parserID *id, Direction direction,
    ...
    : propertyAnn(id, direction, EnumProperty),
      _is_may_property(is_may_property), _enforce_weak_updates(true),
      _values(0), _top(0), _value_index()
#ifdef __FOO
      ,
      _now_values(), _ever_values(), _flow_sensitive_objects(),
      _tested_objects(), _accuracy(), _error_statements(),
      _number_of_flowvalues(0), _error_procedures(), _diagnostic(NULL)
#endif
{
  ...
  if (Annotations::Diagnostic && diagnostic_values &&
    ...
    for (parserid_list_p d = diagnostic_values->begin();
         d != diagnostic_values->end(); d++) {
      enumValueAnn *v = lookup(d->name());
      if (!v) {
        cout << "Error: unknown property value " << d->name() << " at line "
             << d->line() << endl;
        exit(1);
      }
...
void enumPropertyAnn::number_values(enumValueAnn *prop, int &cur_index,
  ...
  if (more_specific.empty()) {
    ...
    for (enumvalue_list_p p = more_specific.begin(); p != more_specific.end();
         ++p)
...
enumValueAnn *enumPropertyAnn::lookup(const string &name) {
  for (enumvalue_vec_p p = _value_index.begin(); p != _value_index.end(); ++p)
    if ((*p)->name() == name)
      return *p;
...
enumValueAnn *enumPropertyAnn::meet(enumValueAnn *one, enumValueAnn *two) {
  // X ^ BOTTOM == BOTTOM
...
bool enumPropertyAnn::at_least(enumValueAnn *left, enumValueAnn *right) {
...
bool enumPropertyAnn::at_most(enumValueAnn *left, enumValueAnn *right) {
...
void enumPropertyAnn::add_property_block(memoryBlock *real_block,
...
bool enumPropertyAnn::test(Broadway::Operator op,
                           ...
                           Location *where, pointerValue &lhs, string &lhs_name,
  ...
  case Broadway::Ever: {
    ...
    for (int i = 0; i < rhs_value_set.size(); i++) {
      if (rhs_value_set.test(i)) {
        ...
        bool one_result =
            test(op, flow_sensitivity, where, lhs, lhs_name, temp);
  ...
  default:
    cerr << "ERROR: enumPropertyAnn: invalid flow sensitivity mode passed to "
            "test()"
         << endl;
  }
...
bool enumPropertyAnn::test(Broadway::Operator op, enumvalue_set lhs_value_set,
                           enumvalue_set rhs_value_set) {
  bool result = false;
...
bool enumPropertyAnn::update_now_value(Location *where,
  ...
  if ((old_count > 0) && (proposed_newval.count() > 0) &&
...
enumvalue_set enumPropertyAnn::construct_now_value(
    Location *where, pointerValue &variable, string &variable_name,
    bool &lost_information, memoryblock_set &complicit_property_blocks) {
  ...
  if (!variable.blocks.empty()) {
    ...
    if (Annotations::Verbose_properties) {
      ...
      cout << "         [ min count = " << min_local_count
  ...
  if (direction() == Forward) {
    ...
    if (current_def) {
      ...
      if (uses.empty()) {
        ...
        for (memoryuse_list_p u = uses.begin(); u != uses.end(); ++u) {
          memoryUse *use = *u;
          ...
          if (Annotations::Verbose_properties)
            cout << "              Use at " << *(use->where())
                 << " value = " << to_string(local_val) << endl;
        }
      }
    } else {
...
enumvalue_set
enumPropertyAnn::construct_after_value(memoryBlock *property_block) {
  enumvalue_set result;
...
enumvalue_set enumPropertyAnn::compute_next(
    Location *where, ruleAnn *rule, exprAnn *expr, pointerValue &right,
    string &right_name, bool &rhs_lost_information,
    memoryblock_set &complicit_property_blocks, enumvalue_set &ever_values) {
  // -- The new property value is just the value of the right-hand side

  enumvalue_set result =
      construct_now_value(where, right, right_name, rhs_lost_information,
                          complicit_property_blocks);
...
void enumPropertyAnn::apply_next(
    Location *where, stmtLocation *parameter_callsite, ruleAnn *rule,
    enumPropertyExprAnn *expr, pointerValue &left, string &left_name,
    pointerValue &right, enumvalue_set proposed_new_value,
    bool rhs_lost_information, bool rhs_changed,
    memoryblock_set &complicit_property_blocks, enumvalue_set ever_values,
  ...
  for (memoryblock_set_p p = left.blocks.begin(); p != left.blocks.end(); ++p) {
    memoryBlock *real_block = *p;
      ...
      if (property_block) {
        ...
        if (enforce_weak_updates()) {
          ...
          if (attach_value_to->is_weak()) {
            ...
            enumvalue_set previous_value =
                construct_now_value(where, property_block, true);
        ...
        bool change_now =
            update_now_value(where, property_block, attach_value_to, new_value,
                             lost_information);
        if (Annotations::Verbose_properties) { // debug
          cout << "  inside apply_next, attach_value_to= " << attach_value_to
               << " def=" << property_block->current_def()
               << " use=" << property_block->current_use() << endl;
        }
        ...
        if (_diagnostic && property_block->is_flow_sensitive()) {
          ...
          if (loc->kind() != Location::Statement &&
            ...
            cout << "wrong assumption: apply_next @ " << *loc
                 << " kind=" << loc->kind() << endl;
            cout << "property_block " << property_block->name() << " real "
                 << real_block->name() << endl;
            cout << "where @" << *where << endl;
            if (parameter_callsite)
              cout << "callsite? @" << *parameter_callsite << endl;
            // exit(1);
          }
          ...
          if (_diagnostic->contain_diagnostic_value(new_value)) {
            ...
            if (def->where() != loc) {
              ...
              cout << "apply_next loc " << loc << " " << *loc << " loc1 "
                   << loc1 << *loc1 << endl;
              exit(1);
            }
                ...
                if (_diagnostic->contain_diagnostic_value(local_val)) {
                  ...
                  if (Annotations::Verbose_properties)
                    cout << "  diagnostic add_defuse (apply_next) " << " @"
                         << *loc << " def=" << def
                         << " use=" << rhs_block->current_use()
                         << " l_property " << property_block << " r_property "
                         << rhs_block << endl;
                  any_rhs_diagnostic_value = true;
                }
              }
        ...
        if ((change_now || rhs_changed) &&
            change_def) { // || change_always || change_ever || pass_evers) &&
                          // change_def)
          if (Annotations::Verbose_properties)
            if (rhs_changed)
        ...
        if (pointerOptions::Monitor_precision) {
          ...
          if ((parameter_callsite) && (where->kind() == Location::Procedure)) {
            procLocation *procloc = (procLocation *)where;

            memorydef_set reaching_defs;

            property_block->add_parameter_assignment(
                procloc->proc(), parameter_callsite, right_property_blocks);
          }
          ...
          if (proposed_new_value.count() > 1) {
            ...
            property_block->add_complicit_assignment(where,
                                                     complicit_property_blocks);
          }
          ...
          if (weak_update_lost_information && !forced_weak) {
            ...
            if ((multiplicity == Single) && (left.blocks.size() > 1)) {
              ...
              property_block->add_destructive_assignment(
                  where, memoryBlock::Weak_update);
            }
            ...
            if (lost_information) {
              ...
              if ((where->kind() == Location::Statement) || (rule != 0)) {
                ...
                property_block->add_destructive_assignment(
                    where, memoryBlock::Parameter_pass);
              }
            }
          }
        } // -- END monitor
...
void enumPropertyAnn::apply_merge(Location *where, memoryBlock *property_block,
  ...
  if (direction() == Forward) {
    ...
    for (memoryuse_list_p p = phi_uses.begin(); p != phi_uses.end(); ++p) {
      memoryUse *phi_use = *p;
      ...
      if (reaching_def) {
        ...
        if (Annotations::Verbose_properties)
          cout << "   = " << to_string(reaching_val) << " at "
               << *(reaching_def->where()) << endl;
      } else {
      ...
      if (_diagnostic) { // note: check with reaching_val, not merged_val
        ...
        if (_diagnostic->contain_diagnostic_value(reaching_val)) {
          // new_value contains diagnostic value; remember the def @lhs and use
          // @rhs.
          _diagnostic->add_defuse(loc, def, phi_use);
          if (def->where() != loc) {
            Location *loc1 = def->where();
            cout << "apply_merge loc " << loc << " " << *loc << " loc1 " << loc1
                 << *loc1 << endl;
            exit(1);
          }
          if (Annotations::Verbose_properties)
    ...
    if ((min_reaching_count == 1) && (merged_val.count() > 1))
      lost_information = true;

    // -- Did anything change?

    bool changed =
        update_now_value(where, property_block, def, merged_val, throw_away);
    if (changed) {
    ...
    for (memoryuse_list_p p = phi_uses.begin(); p != phi_uses.end(); ++p) {
      ...
      bool changed = update_now_value(where, property_block, phi_use,
                                      merged_val, lost_information);
  ...
  if (pointerOptions::Monitor_precision) {
    ...
    if (lost_information)
      property_block->add_destructive_assignment(where,
                                                 memoryBlock::Control_flow);
  }
...
void enumPropertyAnn::apply_assignment(Location *where,
  ...
  if (!right.is_address) {
    ...
    if (direction() == Forward) {
      ...
      new_value =
          construct_now_value(where, right, rhs_name, rhs_lost_information,
                              complicit_property_blocks);
      ever_values = construct_ever_value(right, rhs_name);

      // -- Perform the actual update

      apply_next(where, parameter_callsite, (ruleAnn *)0,
                 (enumPropertyExprAnn *)0, left, lhs_name, right, new_value,
      ...
      apply_next(where, parameter_callsite, (ruleAnn *)0,
                 (enumPropertyExprAnn *)0, right, rhs_name, left, new_value,
                 rhs_lost_information, false, complicit_property_blocks,
                 ever_values, changes);
    }
  }
...
void enumPropertyAnn::self_assignment(Location *source, Location *target,
                                      memoryBlock *property_block,
                                      memoryblock_set &changes, bool is_input) {
  if (Annotations::Verbose_properties)
    cout << "  + Pass " << name() << endl;

  // -- Get the current reaching value

  enumvalue_set current_value =
      construct_now_value(source, property_block, true);
  ...
  bool lost_information = false;
  bool change = update_now_value(target, property_block, attach_value_to,
                                 current_value, lost_information);
  if (Annotations::Verbose_properties) { // debug
  ...
  if (/*change_now &&*/ _diagnostic && property_block->is_flow_sensitive()) {
    // as suggested by Sam, use attach_value_to's location which is a
    // stmtLocation instead of `where' which is a procLocation. Location *loc =
    // attach_value_to->where();
    if (loc->kind() != Location::Statement &&
        loc->kind() != Location::Procedure) { // hack
      cout << "wrong assumption: self_assign @ " << *loc
           << " kind=" << loc->kind() << endl;
      cout << "property_block " << property_block->name() << endl;
      cout << "source @" << *source << " target @" << *target << endl;
      memoryUse *use = property_block->use_at(source);
      memoryDef *def = property_block->current_def();
      cout << "    use " << use << " @" << *use->where() << " def " << def
           << " @" << *def->where() << endl;
      return;
      exit(1);
    }
    if (_diagnostic->contain_diagnostic_value(current_value)) {
      /* cout << "  diagnostic (self_assignment) " << " source@" << *source << "
      target@" << *target
      << " attach_value_to @" << *loc << " " << loc
      << " property_block=" << property_block
      << " " << property_block->name()
      << " def=" << property_block->current_def() << endl; */
      // current_value contains diagnostic value; remember the def @lhs and use
      // @rhs.
      // cout << "    property_block " << property_block->name() << " " <<
      ...
      if (def->where() != loc) {
        Location *loc1 = def->where();
        cout << "self-assignment loc " << loc << " " << *loc << " loc1 " << loc1
             << " " << *loc1 << endl;
...
void enumPropertyAnn::report(ostream &out, bool is_error, procLocation *where,
                             Broadway::FlowSensitivity flow_sensitivity,
  ...
  case Broadway::Trace: {
    if (Annotations::Skip_traces)
      ...
      for (memoryblock_set_p p = lhs.blocks.begin(); p != lhs.blocks.end();
           ++p) {
        memoryBlock *real_block = *p;
        memoryBlock *property_block = lookup_property_block(real_block);
  ...
  case Broadway::Confidence: {
    ...
    if (dsize == 0)
      ...
      confidence =
          (_number_of_flowvalues - dsize) / (_number_of_flowvalues - 1.0);
...
void enumPropertyAnn::print(ostream &o, enumValueAnn *prop, int depth) const {
  ...
  o << prop->name() << ", id = " << prop->id()
    << ", height = " << prop->height() << endl;

  const enumvalue_list &vals = prop->more_specific();
  for (enumvalue_list_cp p = vals.begin(); p != vals.end(); ++p)
    print(o, *p, depth + 2);
}
  ...
  if (Annotations::Adaptivity == 3) {
    ...
    if (Annotations::Adapt_precision == Annotations::UNIFY) { // TB_unify
      ...
      for (tested_objects_map_p p = _tested_objects.begin();
        ...
        while (!objects.empty()) {
          ...
          for (memoryblock_set_p o = objects.begin(); o != objects.end(); o++) {
            memoryBlock *real = *o;
            if (un_unified.find(real) != un_unified.end())
              continue;
            ...
            if (Annotations::Show_adaptivity)
              cout << "AT " << *target << " non-unify " << real->name() << endl;
      ...
      for (tested_objects_map_p p = _tested_objects.begin();
           p != _tested_objects.end(); ++p) {
        Location *target = (*p).first;
        procLocation *target_proc_loc =
            Location::procedure(Location::procedure(target)->stmt_location());
      ...
      if (Annotations::Show_adaptivity)
        cout << "#test_objects=" << tested_objs.size() << " #locs=" << locs
             << endl;
      ...
      for (tested_objects_map_p p = _tested_objects.begin();
        ...
        if (Annotations::Show_adaptivity)
          cout << "AT " << *target << ": num objects = " << objects.size()
               << endl;

        if (Annotations::Show_adaptivity)
          cout << "target " << *target << " #objects " << objects.size()
               << " #seen=" << already_seen.size() << endl; // debug
        // if(already_seen.size() > 178*952) exit(1);
        for (memoryblock_set_p q = objects.begin(); q != objects.end(); ++q) {
          memoryBlock *real_block = *q;
          memoryBlock *property_block = lookup_property_block(real_block);
          if (Annotations::Show_adaptivity)
            cout << " property " << property_block << " "
                 << property_block->name() // debug
                 << " fs="
                 << (made_flow_sensitive.find(property_block) !=
                     made_flow_sensitive.end())
                 << endl;
          ...
          if (property_block && !real_block->write_protected()) {
            track_destructive_assignments(
                analyzer, target, property_block, false, already_seen,
                seen_stack, made_flow_sensitive, made_fs_destructive,
                made_context_sensitive, eval_cs_seen_destructive,
                eval_cs_seen_complicit, eval_made_cs, toss1, toss2, chain,
                indent);
          }
        }
      }
          ...
          if (!block->is_flow_sensitive()) {
            ...
            if (def) {
              ...
              if (pt.size() > 0) {
                ...
                for (memoryblock_set_cp i = pt.begin(); i != pt.end(); ++i) {
                  cout << (*i)->name() << " ";
                }
                cout << endl;
              } else
      ...
      if (pointerOptions::Slice_analysis) {
        ...
        for (proc_info_map_p p = all_procs.begin(); p != all_procs.end(); p++) {
          ...
          if (analyzer->annotations()->lookup_procedure(P->name()))
            continue;
          if (pointerOptions::No_Skip_procs.find(P) ==
                  pointerOptions::No_Skip_procs.end() &&
              do_not_skip.find(P) == do_not_skip.end()) {
            bool ancestor = false;
            for (set<procedureInfo *>::iterator t =
                     pointerOptions::No_Skip_procs.begin();
...
double enumPropertyAnn::compute_accuracy(Analyzer *analyzer) {
  ...
  for (accuracy_map_p p = _accuracy.begin(); p != _accuracy.end(); ++p) {
  ...
  for (stmt_accuracy_map_p p = by_statement.begin(); p != by_statement.end();
       ++p) {
    ...
    // cout << "ACCURACY: " << name() <<  " at " << call->coord() << " = " <<
    // local_accuracy << endl;
    cout << "ACCURACY: " << name() << " at " << stmt->coord() << " = "
         << local_accuracy << endl;
  ...
  if (count > 0) {
    ...
    cout << "STAT-accuracy-property-all-" << name() << " " << overall_accuracy
         << endl;
  }

  if (error_count > 0)
    cout << "STAT-accuracy-property-errors-" << name() << " "
         << (error_total / ((double)error_count)) << endl;
...
void enumPropertyAnn::track_destructive_assignments(
    Analyzer *analyzer, Location *target, memoryBlock *block,
    bool disallow_context_sensitivity, block_loc_set &already_seen,
    block_loc_set seen_stack, memoryblock_set &made_flow_sensitive,
    memoryblock_set &made_fs_destructive, block_loc_set &made_context_sensitive,
    block_proc_set &eval_cs_seen_destructive,
    block_proc_set &eval_cs_seen_complicit, block_proc_set &eval_made_cs,
    bool &make_chain_flow_sensitive, bool &make_chain_context_sensitive,
    memoryblock_vector &chain, string &indent) {
  if (pointerOptions::Slice_analysis) {
    memoryBlock *B = block;
    do {
      procedureInfo *info;
      ...
      if (B->allocation_site()) {
        procNode *alloc_proc =
            Location::procedure(B->allocation_site())->proc();
  ...
  if (!TB_changes || !block->is_flow_sensitive() || // 20030915
      Annotations::Adapt_precision == Annotations::FICI ||
  ...
  if (seen) {
    ...
    if (Annotations::Show_adaptivity) {
      if (fs)
        cout << indent << "+ (Already made FS: block " << block->name() << ")"
             << endl;
      if (TB_changes) {
        if (cs)
          cout << indent << "+ (Already made CS: block " << block->name() << ")"
               << endl;
        if (!fs && !cs)
          cout << indent << "+ (Already seen: block " << block->name() << ")"
               << endl;
      }
    }
#if 0
...
*/
  ...
  const memoryBlock::destructive_assignment_map &destructive =
  ...
  for (memoryBlock::destructive_assignment_map_cp p = destructive.begin();
    ...
    if (pointerOptions::Slice_analysis) {
      ...
      if (analyzer->annotations()->lookup_procedure(proc->decl()->name()))
        proc =
            Location::procedure(Location::procedure(where)->parent())->proc();
      procedureInfo *info = analyzer->lookup_procedure(proc);
      pointerOptions::No_Skip_procs.insert(info);
    }

    if (TB_changes && Annotations::Aggressive_pruning) {
      if (!is_location_reachable(where, target, block))
        continue;
    }
    ...
    case memoryBlock::Parameter_pass: {
      ...
      if (where->kind() == Location::Statement) {
        ...
        if (!disallow_context_sensitivity) {
          bool m = evaluate_context_sensitivity(
              analyzer, target, block,
              /*already_seen,*/ procloc, true, eval_cs_seen_destructive,
              eval_cs_seen_complicit, eval_made_cs, indent);
          make_context_sensitive = make_context_sensitive || m;
          if (Annotations::Show_adaptivity) // debug
            cout << "Parameter_pass(" << block->name()
                 << ") make_context_sensitive("
                 << procloc->proc()->decl()->name()
                 << ")=" << make_context_sensitive << endl;
          if (m && Annotations::Adaptor_statistics)
            Annotations::Adaptor_statistics->make_CS(
                block, analyzer->lookup_procedure(procloc->proc()), where,
    ...
    case memoryBlock::Weak_update: {
      if (!already_made_FS) {
        if (Annotations::Show_adaptivity) {
          cout << indent << "  --> needs lower multiplicity at " << *where
               << endl;
          cout << indent
               << "      (allocation site = " << block->allocation_site()
      ...
      if (TB_changes && !disallow_context_sensitivity) {
        ...
        if (!m || previously_cs ||
            analyzer->annotations()->lookup_procedure(
                procloc->proc()->decl()->name()))
          ;
        else {
          cout << *where << endl;
          cout << block << " " << block->name()
               << " proc=" << procloc->proc()->decl()->name() << endl;
        }
    ...
    case memoryBlock::Additive:
      ...
      if (!already_made_FS) {
        if (Annotations::Show_adaptivity)
          cout << indent << "  --> needs flow sensitivity at " << *where
               << endl;
      ...
      if ( ! block->property) {
        ...
        if (where->kind() == Location::Statement) {
          ...
          make_flow_sensitive = validate_pointer_fs(block, assignment_loc,
      indent);
        }
      }
      */
  ...
  */
  ...
  if (!already_made_FS && make_flow_sensitive) {
    ...
    for (memoryblock_vector_p p = chain.begin(); p != chain.end(); ++p) {
      ...
      if (!make_fs_block->is_single_assignment()) {
        ...
        if (Annotations::Show_adaptivity)
          cout << indent << " --> set flow-sensitive " << make_fs_block->name()
               << endl;
        ...
        if (make_fs_block->property) {
          ...
          make_fs_block->add_to_flow_sensitive_list(
              pointerOptions::Flow_sensitive_objects);
        }

        if (make_fs_block->is_heap_object()) {
          memoryBlock *alloc_object = make_fs_block->allocation_object();
          alloc_object->add_to_flow_sensitive_list(
              pointerOptions::Flow_sensitive_allocation_objects);
        }
        ...
        if (Annotations::Show_adaptivity)
          cout << indent << " --> skip single-assign " << make_fs_block->name()
               << endl;
      }
    }
  }
  ...
  for (memoryBlock::complicit_assignment_map_cp p = complicit.begin();
    ...
    for (memoryblock_set_cp q = blocks.begin(); q != blocks.end(); ++q) {
      memoryBlock *comp = *q;

      // TB_unify
      if (/*Annotations::Adapt_uses_uba &&*/ comp == block)
        continue;

      if (Annotations::Show_adaptivity)
        cout << indent << "  - Complicit at " << *where << ": assignment \""
             << block->name() << " = " << comp->name() << "\"" << endl;
      ...
      // TB 20030725 short-circuit
      {
        ...
        if (!TB_changes || !comp->is_flow_sensitive() || // 20030915
            Annotations::Adapt_precision == Annotations::FICI ||
            Annotations::Adapt_precision == Annotations::FICS) {
          if (!seen) {
            for (block_loc_set::iterator s = already_seen.begin();
        ...
        if (seen) {
          if (Annotations::Show_adaptivity)
            cout << block << " seen complicit comp=" << comp << " "
                 << comp->name() << endl;
          ...
          if (made_context_sensitive.find(sp) != made_context_sensitive.end()) {
            if (!seen_in_stack || pointerOptions::Recursion_Context_sensitive) {
              bool m = false; // 20030904
              if (/*!seen_in_stack &&*/ !disallow_context_sensitivity) {
                procLocation *procloc = Location::procedure(where);
                procNode *proc = procloc->proc();
                procedureInfo *info = analyzer->lookup_procedure(proc);
                m = evaluate_context_sensitivity(
                    analyzer, target,
                    block, // comp /*TB block*/ ,
                    /*already_seen,*/ procloc, false, eval_cs_seen_destructive,
                    eval_cs_seen_complicit, eval_made_cs, indent);
                if (m && Annotations::Adaptor_statistics)
                  Annotations::Adaptor_statistics->make_CS(
                      block, analyzer->lookup_procedure(procloc->proc()), where,
      ...
      if (comp == block) {
        ...
        if (Annotations::Show_adaptivity) {
          if (reachable)
            cout << indent << "    [ " << * target << " is reachable from " << *
        where << " ]" << endl; else cout << indent << "    [ " << * target << "
        is NOT reachable from " << * where << " ]" << endl;
        }
        */
        ...
        if (reachable) {
          ...
          track_destructive_assignments(
              analyzer, target /* where */, comp, throttle_context_sensitivity,
              already_seen, seen_stack, made_flow_sensitive,
              made_fs_destructive, made_context_sensitive,
              eval_cs_seen_destructive, eval_cs_seen_complicit, eval_made_cs,
              chain_flow_sensitivity, chain_context_sensitivity, chain,
              new_indent);
          ...
          if (chain_context_sensitivity) {
            ...
            if (m) {
              ...
              if (Annotations::Show_adaptivity)
                cout << indent << "  --> Chain context-sensitivity of "
                     << proc->decl()->name() << " for assignment \""
                     << block->name() << " = " << comp->name() << "\"" << endl;
  ...
  if (!already_made_FS && make_flow_sensitive) {
    ...
    if (!block->is_single_assignment()) {
      ...
      if (block->property) {
        ...
        block->add_to_flow_sensitive_list(
            pointerOptions::Flow_sensitive_objects);
      }
...
void enumPropertyAnn::record_tested_objects(
    Location *where, pointerValue &ptr, enumvalue_set &value_set,
    memoryblock_set &complicit_property_blocks) {
  if ((Annotations::Adaptivity == 3) && Annotations::Record_tested_objects) {

    // -- Remember that these objects were tested

    for (memoryblock_set_p q = complicit_property_blocks.begin();
         q != complicit_property_blocks.end(); ++q) {
    ...
    if (value_set.count() > 1) {
      ...
      for (memoryblock_set_p p = ptr.blocks.begin(); p != ptr.blocks.end();
           ++p) {
        memoryBlock *real_block = *p;
        memoryBlock *property_block = lookup_property_block(real_block);
...
void enumPropertyAnn::trace_object(ostream &out, memoryBlock *property_block,
                                   memoryblock_set &already_seen,
                                   string &indent) {
  // -- Check for cycles
  ...
  const memoryBlock::destructive_assignment_map &destructive =
      property_block->destructive_assignments();

  // -- Get the complicit assignment list

  const memoryBlock::complicit_assignment_map &complicit =
      property_block->complicit_assignments();

  // -- Visit all the defs and report

  const memorydef_list defs = property_block->defs();
  for (memorydef_list_cp mp = defs.begin(); mp != defs.end(); ++mp) {
...
bool enumPropertyAnn::evaluate_context_sensitivity(
    Analyzer *analyzer, Location *target, memoryBlock *block,
    procLocation *procloc, bool destructive,
    block_proc_set &eval_cs_seen_destructive,
    block_proc_set &eval_cs_seen_complicit, block_proc_set &evaled_cs,
    string &indent) {
  procNode *proc = procloc->proc();
  ...
  if (Annotations::Prune_context_sensitive) {
    ...
    if (analyzer->annotations()->lookup_procedure(proc->decl()->name())) {
      ...
      if (assignments.empty()) {
        ...
        if (Annotations::Show_adaptivity)
          cout << indent
               << "    ==> (No callsite: leave it context-insensitive)" << endl;
         ...
         if(! is_recursive_context && assignments.size()==1) {
           ...
           procLocation *caller =
   Location::procedure(assignments.begin()->first); is_recursive_context =
   ...
   cout << "#assignments = " << assignments.size() << endl;
           ...
           if (Annotations::Show_adaptivity)
             cout << indent << "    ==> (One callsite: leave it
   context-insensitive)" << endl; } else make_context_sensitive = true; // TB
   new
       } */
      else {
        ...
        if (info->is_recursive() &&
          ...
          if (Annotations::Show_adaptivity)
            cout << indent
                 << "    ==> (Recursive: leave it context-insensitive)" << endl;
        } else {
          ...
          if (destructive || !TB_changes) { // TB 20030910
            if (block->property) {
              ...
              make_context_sensitive =
                  validate_property_cs(block, assignments, info, indent);
            } else if (block->is_allocation_object()) {

              // -- Allocation object: check multiplicity

              make_context_sensitive =
                  validate_multiplicity_cs(block, assignments, info, indent);
            } else {

              // -- Regular pointer object: count points-to sets

              make_context_sensitive =
                  validate_pointer_cs(block, assignments, info, indent);
            }
          } else if (Annotations::Show_adaptivity)
  ...
  if (make_context_sensitive && (Annotations::Error_oriented_adaptivity ||
    ...
    if (Annotations::Show_adaptivity)
      cout << indent << "      Error-oriented check: is " << info->name()
           << " an ancestor of " << target_info->name() << endl;

    // -- See if the current procedure is an ancestor of the target

    const procedureinfo_set &ancestors = target_info->ancestors();
    if ((info == target_info) || (ancestors.find(info) != ancestors.end()))
      is_ancestor = true;
    /* TB
  ...
  if (make_context_sensitive) {
    ...
    if (0) {
      ...
      if (1) {
        ...
        subtree.insert(info);
        subtree.insert(descendants.begin(), descendants.end());
        ...
        for (procedureinfo_set_p pp = descendants.begin();
          ...
          set_difference(des_ancestors.begin(), des_ancestors.end(),
                         subtree.begin(), subtree.end(), back_inserter(diffs));

          if (Annotations::Show_adaptivity) {
            cout << indent << "        + " << name << " has " << diffs.size()
                 << " other contexts." << endl;
            if (diffs.size() == 1)
              cout << indent << "          - Diff is "
                   << diffs.front()->proc()->decl()->name() << endl;
          }
...
bool enumPropertyAnn::validate_property_cs(
    memoryBlock *block, const callsite_objects_map &assignments,
  ...
  if (Annotations::Show_adaptivity)
    cout << indent << "    Property validation for block " << block->name()
         << endl;
  ...
  for (callsite_objects_map_cp rdp = assignments.begin();
    ...
    for (memoryblock_set_cp mdp = reaching_blocks.begin();
         mdp != reaching_blocks.end(); ++mdp) {
      memoryBlock *property_block = (*mdp);

      property_block->set_current_def_use(attach_to);
      enumvalue_set local_val =
          construct_now_value(attach_to, property_block, false);

      meet_with(vals, local_val);

      if (Annotations::Show_adaptivity) {
        cout << indent << "      Callsite " << *callsite << "  block "
             << property_block->name() << " = "
  ...
  for (stmt_value_map_p p = stmt_values.begin(); p != stmt_values.end(); ++p) {
    stmtNode *stmt = (*p).first;
    enumvalue_set vals = (*p).second;

    if (Annotations::Show_adaptivity) {
      cout << indent << "      Statement " << stmt->coord()
           << " reaching values = " << to_string(vals) << endl;
    }
  ...
  if (Annotations::Show_adaptivity)
    cout << indent << "    ==> Property " << block->property->name()
         << " : Min = " << min << " , Vals = " << to_string(collected_vals)
         << endl;
...
bool enumPropertyAnn::validate_multiplicity_cs(
    memoryBlock *block, const callsite_objects_map &assignments,
    procedureInfo *procedure, // TB
    string &indent) {
  ...
  if (Annotations::Show_adaptivity)
    cout << indent << "    Multiplicity validation for block " << block->name()
         << endl;
...
bool enumPropertyAnn::validate_pointer_cs(
    memoryBlock *block, const callsite_objects_map &assignments,
    procedureInfo *procedure, string &indent) {
#if 0
  ...
  if (Annotations::Show_adaptivity)
    cout << indent << "    Points-to validation for block " << block->name()
         << endl;
  ...
  for (callsite_objects_map_cp rdp = assignments.begin();
    ...
    enumvalue_set vals = reachable_values(callsite, reaching_blocks, procedure);

    if (Annotations::Show_adaptivity) {
      cout << indent << "      Callsite " << *callsite
           << " : [ Values = " << to_string(vals) << " -- blocks = ";
      for (memoryblock_set_cp mbp = reaching_blocks.begin();
           mbp != reaching_blocks.end(); ++mbp)
        cout << (*mbp)->name() << " ";
      cout << "]" << endl;
    }
  ...
  if (Annotations::Show_adaptivity)
    cout << indent << "    ==> [ Values = " << to_string(collected_vals) << " ]"
         << endl;
...
bool enumPropertyAnn::validate_pointer_fs(memoryBlock *block,
                                          stmtLocation *where, string &indent) {
  // memoryblock_set collected_points_to;
  enumvalue_set collected_vals;
  ...
  for (memoryblock_set_cp p = points_to.begin(); p != points_to.end(); ++p) {
...
bool enumPropertyAnn::is_location_reachable(Location *source, Location *target,
                                            memoryBlock *block) {
  ...
  while (not_done) {
    ...
    for (memorydef_set_p p = defs.begin(); p != defs.end(); p++) {
      memoryDef *cur_def = *p;
    ...
    for (memoryuse_set_p w = uses.begin(); w != uses.end(); ++w) {
      memoryUse *cur_use = *w;
...
enumvalue_set enumPropertyAnn::reachable_values(stmtLocation *where,
  ...
  for (memoryblock_set_cp p = blocks.begin(); p != blocks.end(); ++p) {
  ...
  while (!worklist.empty()) {
    ...
    if (mb->is_flow_sensitive() || !callee ||
  ...
  found.insert(blocks.begin(), blocks.end());

  // -- Collect all the property blocks...

  memoryblock_set property_blocks;

  for (memoryblock_set_p p = found.begin(); p != found.end(); ++p) {
    memoryBlock *real_block = *p;
    memoryBlock *property_block = lookup_property_block(real_block);
...
bool enumPropertyAnn::add_context_sensitive_proc(
    procedureInfo *info, procedureinfo_set &already_seen,
    procedureinfo_set &make_cs) {
  // -- Visit all of it's descendants

  const procedureinfo_set &calls = info->calls();

  for (procedureinfo_set_cp p = calls.begin(); p != calls.end(); ++p) {
    procedureInfo *cur = *p;
    ...
    if (already_seen.find(cur) == already_seen.end()) {
      ...
      bool cur_recursive =
          add_context_sensitive_proc(cur, already_seen, make_cs);
...
void enumPropertyAnn::diagnostic(ostream &out, procLocation *where,
                                 pointerValue &pv, enumvalue_set values) const {
  if (!_diagnostic)
    return;
...
enumvalue_set
enumPropertyAnn::lookup_input_to_value(procedureInfo *info,
                                       memoryBlock *property_block) {
