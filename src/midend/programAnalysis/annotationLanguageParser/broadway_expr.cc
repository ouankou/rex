...
void exprAnn::apply_next(procLocation *where,
                         propertyAnalyzer *property_analyzer, ruleAnn *rule,
...
connectiveExprAnn::connectiveExprAnn(Broadway::Operator op, exprAnn *lhs,
                                     exprAnn *rhs, int line)
    : exprAnn(op, line), _lhs(lhs), _rhs(rhs) {}
...
bool connectiveExprAnn::test(procLocation *where,
  ...
  case Broadway::And:
    result = (lhs()->test(where, property_analyzer) &&
              rhs()->test(where, property_analyzer));
    break;
  case Broadway::Or:
    result = (lhs()->test(where, property_analyzer) ||
              rhs()->test(where, property_analyzer));
...
// begin TB new
void connectiveExprAnn::diagnostic(ostream &out, procLocation *where,
...
void connectiveExprAnn::lookup(propertyAnn *default_property,
  ...
  if (_lhs)
    _lhs->lookup(default_property, procedure, annotations);

  if (_rhs)
    _rhs->lookup(default_property, procedure, annotations);
}

#ifdef __PROCLOCATION

void connectiveExprAnn::report(ostream &out, bool is_error,
                               procLocation *where) {
...
enumPropertyExprAnn::enumPropertyExprAnn(
    Broadway::FlowSensitivity flow_sensitivity, const parserID *property,
    const parserID *lhs, Broadway::Operator op, const parserID *rhs, int line)
    : exprAnn(op, line), _flow_sensitivity(flow_sensitivity), _property_name(),
      _property(0), _lhs_name(lhs->name()), _lhs(0), _lhs_value(), _rhs_name(),
      _rhs_is_property_value(false), _rhs(0), _rhs_value(),
      _rhs_lost_information(false),
#ifdef __COMPLICIT
      _complicit_property_blocks(),
#endif
      _rhs_ever_values(), _is_weak(false) {
  if (property)
...
// begin TB new
void enumPropertyExprAnn::diagnostic(
    ostream &out, procLocation *where,
    propertyAnalyzer *property_analyzer) const {
  if (_rhs_is_property_value)
...
void enumPropertyExprAnn::compute_next(procLocation *where, ruleAnn *rule) {
  ...
  if (!_rhs_is_property_value) {
    ...
    _rhs_value = _property->compute_next(
        where, rule, this, right, _rhs->name(), _rhs_lost_information,
        _complicit_property_blocks, _rhs_ever_values);
  }

  if (Annotations::Verbose_properties)
    cout << "  + Update " << _lhs_name << " <- "
         << _property->to_string(_rhs_value) << endl;
}

void enumPropertyExprAnn::apply_next(procLocation *where,
                                     propertyAnalyzer *property_analyzer,
                                     ruleAnn *rule, memoryblock_set &changes) {
  // -- Get the left-hand-side binding

  pointerValue &left = _lhs->get_binding(where);

  bool rhs_changed =
      Annotations::Adapt_uses_uba && (_previous_rhs_value[where] != _rhs_value);
  ...
  if (!_rhs_is_property_value) {
    ...
    _property->apply_next(
        where, (stmtLocation *)0, rule, this, left, _lhs->name(), right,
        _rhs_value, _rhs_lost_information, rhs_changed,
        _complicit_property_blocks, _rhs_ever_values, changes);
  } else {
    ...
    _property->apply_next(where, (stmtLocation *)0, rule, this, left,
                          _lhs->name(), right, _rhs_value, false, rhs_changed,
                          _complicit_property_blocks, _rhs_ever_values,
                          changes);
  }
...
void enumPropertyExprAnn::lookup(propertyAnn *default_property,
  ...
  if (_property == 0) {
    annotations->Error(line(),
                       string("Unknown property \"") + _property_name + "\"");
    return;
  }
  ...
  if (default_property && (default_property->direction() == Forward) &&
      (_property->direction() == Backward))
    annotations->Error(line(), "NOTE: Results for backward analysis \"" +
                                   _property_name +
  ...
  if (!_rhs_name.empty()) {
    ...
    if (_rhs)
      ...
      if (!val)
        annotations->Error(line(), string("Unknown value or variable\"") +
                                       _rhs_name + "\"");
      else
  ...
  if ((_flow_sensitivity == Broadway::After) && (default_property != 0))
    annotations->Error(line(), string("Flow sensitivity operator \"@after\" "
                                      "only allowed in report annotations"));
  ...
  if (((op() == Broadway::Is_Exactly) ||
      ...
      ( ! _rhs && ! _rhs_value))
    annotations->Error(line(), string("Binary test operator missing right-hand
  side."));
  */
  ...
  if ((op() == Broadway::Assign) && (_property) &&
    ...
    annotations->Error(
        line(), string("Actions in this context may only modify property \"") +
                    _property->name() + "\"");
  ...
  if ((op() == Broadway::Assign) && (_property) &&
    ...
    annotations->Error(
        line(),
        string("Actions in the constant analysis may not change property \"") +
            _property->name() + "\"");
}

#ifdef __PROCLOCATION

void enumPropertyExprAnn::report(ostream &out, bool is_error,
                                 procLocation *where) {
  ...
  _property->report(out, is_error, where, _flow_sensitivity, var_binding,
                    _lhs->name());
}
...
void enumPropertyExprAnn::print(ostream &out) {
  ...
  if ((op() != Broadway::Is_Bottom) && (op() != Broadway::Report))
    out << " " << _rhs_name;
}
...
setPropertyExprAnn::setPropertyExprAnn(
    Broadway::FlowSensitivity flow_sensitivity, const parserID *setProperty,
    const parserID *lhs, Broadway::Operator op, const parserID *rhs, int line)
    : exprAnn(op, line), _flow_sensitivity(flow_sensitivity), _property_name(),
      _property(0), _lhs_name(), _lhs(0), _rhs_name(), _rhs(0) {
  if (setProperty)
...
// begin TB new
void setPropertyExprAnn::diagnostic(ostream &out, procLocation *where,
                                    propertyAnalyzer *property_analyzer) const {
  assert(false); // TBD
}
// end TB new
...
void setPropertyExprAnn::lookup(propertyAnn *default_property,
  ...
  if (_property == 0) {
    annotations->Error(line(), string("Unknown set property \"") +
                                   _property_name + "\"");
    return;
  }
  ...
  if (!_rhs_name.empty()) {
    ...
    if (!_rhs)
      annotations->Error(line(),
                         string("Undeclared identifier \"") + _rhs_name + "\"");
  }
...
void setPropertyExprAnn::report(ostream &out, bool is_error,
                                procLocation *where) {
...
numericExprAnn::numericExprAnn(exprNode *expr, int line)
    : exprAnn(Broadway::Evaluate, line), _lhs_name(), _lhs(0), _expr(expr) {}

#endif

numericExprAnn::numericExprAnn(const parserID *lhs, int line)
    : exprAnn(Broadway::Is_Constant, line), _lhs_name(), _lhs(0)
#ifdef __EXPR
      ,
      _expr(0)
#endif
...
#ifdef __EXPRNODE
numericExprAnn::numericExprAnn(const parserID *lhs, exprNode *expr, int line)
    : exprAnn(Broadway::Assign, line), _lhs_name(), _lhs(0), _expr(expr) {
  if (lhs)
    _lhs_name = lhs->name();
}
...
bool numericExprAnn::test(procLocation *where,
  ...
  if (op() == Broadway::Evaluate) {
    ...
    return result;
  } else if (op() == Broadway::Is_Constant) {
...
public:
  ...
  idNodeAnnWalker(procedureAnn *procedure, Annotations *annotations)
      : Walker(Preorder, Subtree), _procedure(procedure),
  ...
  virtual void at_id(idNode *the_id, Order ord) {
    ...
    if (!ida->variable())
      _annotations->Error(ida->line(), string("Undeclared identifier \"") +
                                           ida->name() + "\"");
...
void numericExprAnn::lookup(propertyAnn *default_property,
                            procedureAnn *procedure, Annotations *annotations) {
  if (!_lhs_name.empty()) {
  ...
  if ((op() == Broadway::Assign) &&
      (default_property != annotations->constants_property()))
    annotations->Error(line(),
                       string("Actions may only update constant values in the "
                              "\"constants\" analyze annotation."));
}
...
pointerExprAnn::pointerExprAnn(Broadway::Operator op, const parserID *lhs,
                               const parserID *rhs)
    : exprAnn(op, myFetchLine(lhs)), _lhs_name(), _lhs(0), _rhs_name(),
      _rhs(0) {
  if (lhs)
    _lhs_name = lhs->name();
...
bool pointerExprAnn::test(procLocation *where,
  ...
  if (op() == Broadway::Is_AliasOf) {
    ...
    if (Annotations::Verbose_properties)
      if (result)
        cout << "      pointerExprAnn : " << _lhs->name() << " aliases "
             << _rhs->name() << endl;
      else
        cout << "      pointerExprAnn : " << _lhs->name() << " does not alias "
             << _rhs->name() << endl;
  }
  ...
  if (op() == Broadway::Is_SameAs) {
    ...
    if (Annotations::Verbose_properties)
      if (result)
        cout << "      pointerExprAnn : " << _lhs->name() << " is the same as "
             << _rhs->name() << endl;
      else
        cout << "      pointerExprAnn : " << _lhs->name()
             << " is different from " << _rhs->name() << endl;
  }
  ...
  if (op() == Broadway::Is_Empty) {
    ...
    if (Annotations::Verbose_properties)
      if (result)
        cout << "      pointerExprAnn : " << _lhs->name() << " is empty."
             << endl;
      else
        cout << "      pointerExprAnn : " << _lhs->name() << " is not empty."
             << endl;
  }
...
idNodeAnn::idNodeAnn(const parserID *id)
    : idNode(id->name().c_str()), _variable(0), _line(id->line()) {}

void idNodeAnn::lookup(procedureAnn *procedure, Annotations *annotations) {
...
void idNodeAnn::output_expr(output_context &ct, Node *parent, int prec,
                            Assoc assoc) {
