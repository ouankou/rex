...
void Adaptor_Statistics::make_CS(memoryBlock *block, procedureInfo *proc,
                                 Location *site, memoryBlock *complicit) {
...
void Adaptor_Statistics::print_cs_statistics(ostream &out) {
  ...
  for (procedureinfo_set_p p = cs_procs.begin(); p != cs_procs.end(); p++) {
    ...
    out << " cg depth=" << cg_depth_min << "/" << cg_depth_max
        ...
        << " recursion=" << (*p)->is_recursive()
        << " #callsites=" << unique_callers.size() << "/" << total_callsites
        << " calls=" << (*p)->calls().size() << "/\n";

    decl_list args = ((funcNode *)proc->decl()->type())->args();
    set<int> destructive_args;
    memoryblock_set blocks = cs_blocks[*p], printed;
    ...
    if (proc_site_property.find(*p) != proc_site_property.end()) {
      ...
      for (Site_Property::iterator sp = proc_site_property[*p].begin();
        ...
        for (Arg_Property::iterator ap = sp->second.begin();
             ap != sp->second.end(); ap++)
          site_partition[Arg_Property_Pair(ap->first, ap->second)].insert(
              sp->first);
      ...
      for (Site_Partition::iterator part = site_partition.begin();
        ...
        ostringstream ost;
        ost << "<" << part->first.first.first << "," << part->first.first.second
            << "|";
        for (enumvalue_sets::iterator v = part->first.second.begin();
             v != part->first.second.end(); v++) {
          if (v != part->first.second.begin())
            ost << ",";
          ost << *v;
        }
...
void Adaptor_Statistics::print_call_args(stmtLocation *stmt_loc,
                                         ...
                                         procedureInfo *callee, int arg,
                                         declNode *formal, ostream &out) {
  ...
  for (parameter = callsite->arg_list().begin();
    ...
    ;
  if (parameter == callsite->arg_list().end())
    return;
  ...
  do {
    ...
    memoryuse_set uses;
    bool never_returns;
    _analyzer->eval(caller, stmt_loc, *parameter, stmt_loc->stmt(), defs, uses,
                    changes, parameter_value, never_returns);
...
void Adaptor_Statistics::print_call_arg(
    declNode *formal, int arg, operandNode *parameter, int dereference,
    pointerValue &parameter_value, stmtLocation *stmt_loc,
    procedureInfo *caller, procedureInfo *callee,
    const enum_property_map &props, ostream &out) {
  if (parameter_value.blocks.empty())
    return;
      ...
      for (enum_property_map_cp p = props.begin(); p != props.end(); ++p) {
        ...
        if (property_block) {
          ...
          enumvalue_set values =
              property->construct_now_value(stmt_loc, property_block, false);
  ...
  if (parameter_value.blocks == star.blocks)
    return;
  print_call_arg(formal, arg, parameter, dereference + 1, star, stmt_loc,
                 caller, callee, props, out);
} // print_call_arg
...
void Adaptor_Statistics::print_block_chain(memoryBlock *block,
  ...
  if (destructions.find(block) != destructions.end()) {
    ...
    for (Locations::iterator loc = destructions[block].begin();
      ...
      case memoryBlock::Parameter_pass:
        ...
        break;
      case memoryBlock::Weak_update:
        out << " multiplicity weak update";
        break;
      case memoryBlock::Additive:
        out << " flow-insensitive assignment";
        break;
      default:
        out << " -- ERROR unknown cause" << r << endl;
        assert(false);
      }
      out << " @" << **loc << ", ";
    }
  ...
  if (complicits.find(block) != complicits.end()) {
    ...
    for (Complicit_assigns::iterator a = complicits[block].begin();
      ...
      print_block_chain(a->second, destructions, complicits, printed, indent,
                        block_indent, out);
    }
  }
} // print_block_chain
