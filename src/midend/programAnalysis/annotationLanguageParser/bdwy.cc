...
public:
  static labelNode *get_label(basicblockNode *bb) {
    for (stmt_list_p p = bb->stmts().begin(); p != bb->stmts().end(); ++p) {
      if ((*p)->typ() == Label)
        return (labelNode *)(*p);
    }
    return (labelNode *)0;
  ...
  static void fix(unitNode *Unit) {
    for (def_list_p dp = Unit->defs().begin(); dp != Unit->defs().end(); ++dp) {
      defNode *dn = (*dp);
      if (dn->typ() == Proc) {
          ...
          if (stmt->typ() == Block) {
            ...
            if (bb->stmts().size() == 1) {
              ...
              if (label) {
                ...
                if (succ_label && (!references.empty())) {
...
private:
  ...
  TreeFixer() : Changer(Preorder, Subtree, false), _nodes() {}
...
public:
  static void fix(unitNode *node) {
    ...
    if (simp._count > 0)
      cout << "Simplifier: removed " << simp._count << " branches from "
           << node->input_file() << endl;
  }

  bdwySimplifier() : Changer(Preorder, Subtree, true), _count(0) {}
...
public:
  ...
  static void fix_unit(unitNode *u) {
    ...
    // TB: cfg_changer::generate_cfg (u, DEFAULT_DISMANTLER_FLAGS |
    // INDEX_IS_PRIMITIVE);
  ...
  void run() {
    ...
    for (unit_list_p up = CBZ::Program.begin(); up != CBZ::Program.end();
         ++up) {
      unitNode *u = (*up);
      fix_unit(u);
    }
    ...
    for (unit_list_p up = CBZ::Program.begin(); up != CBZ::Program.end();
      ...
      for (def_list_p dp = Unit->defs().begin(); dp != Unit->defs().end();
           ++dp) {
        defNode *dn = (*dp);
        if (dn->typ() == Proc) {
          procNode *pr = (procNode *)dn;
          ...
          if (pr->decl()->name() == "main") {
            if (Main) {
              cout << "Warning: Found another main() at " << pr->coord()
                   << endl;
...
public:
  static void cleanup(unitNode *u) {
    ...
    for (def_list_p dp = u->defs().begin(); dp != u->defs().end(); ++dp) {
      defNode *dn = (*dp);
      if (dn->typ() == Proc) {
        procNode *pr = (procNode *)dn;
        ...
        for (stmt_list_p q = body->stmts().begin(); q != body->stmts().end();
             ++q) {
          if ((*q)->typ() == Block) {
...
public:
  void get_flags(str_list_p &arg) {
    ...
    do {
      ...
      if ((flag == "adaptive") || (flag == "adaptive-3")) {
  ...
  void run() {
    ...
    if (a->errors() == 0) {
      ...
      cout << "STAT-memory-parsing     " << (after_parse_c - initial_memory)
           << endl;
      cout << "STAT-memory-annotations " << (after_parse_pal - after_parse_c)
           << endl;
      cout << "STAT-memory-fixup       " << (after_fixup - after_parse_pal)
           << endl;
      ...
      if (!Annotations::Enabled_properties.empty()) {
        ...
        for (str_list_p p = Annotations::Enabled_properties.begin();
             p != Annotations::Enabled_properties.end(); ++p) {
      ...
      for (str_list_p p = Annotations::Disabled_properties.begin();
           p != Annotations::Disabled_properties.end(); ++p) {
        propertyAnn *property = anns->lookup_property(*p);
        if (property)
          property->disable();
        else
      ...
      for (pass_list_p p = _passes.begin(); p != _passes.end(); ++p) {
        PassKind kind = *p;
        ...
        case Reports: {
          ...
          if (Annotations::Adaptivity == 3) {
            ...
            cout << "recursion-cs = "
                 << pointerOptions::Recursion_Context_sensitive << endl;
            while (!done) {
              ...
              analyzer = new Analyzer(anns, fixupPhase::Main, linker,
                                      fixupPhase::Unit);
              ...
              for (error_report_map_p p = Annotations::Error_reports.begin();
                   p != Annotations::Error_reports.end(); ++p) {
              ...
              cout << "STAT-errors-total " << Annotations::Error_reports.size()
                   << endl;

              // (hack) print now in case stuck in adaptor
              double analysis_time1(analysis_timer);
              cout << "STAT-time-analysis                 " << analysis_time1
                   << endl;
              ...
              */
              ...
              double start_time = (double) tv_start.tv_sec +
              ((double)tv_start.tv_usec)/1000000.0; double end_time = (double)
              tv_end.tv_sec + ((double)tv_end.tv_usec)/1000000.0; double total =
              end_time - start_time;
              */
              // -- Report the new precision parameters

              int new_cs_procedures =
                  pointerOptions::Context_sensitive_procedures.size();
              int new_fs_objects =
                  pointerOptions::Flow_sensitive_objects.size();
              int new_ps_basicblocks =
                  pointerOptions::Path_sensitive_branches.size();
              int new_fsp_objects;
              ...
              analyzer->number_of_procedures(
                  total_procedures, analyzed_procedures, actual_ci_procedures,
                  recursive_procedures, unanalyzed_procedures, program_size);

              int actual_cs_procedures =
                  analyzed_procedures - actual_ci_procedures;

              double analysis_time(analysis_timer);
              double adapt_time(adapt_timer);

              cout << " -- Precision ------------------------------" << endl;
              // cout << "STAT-time-analysis                 " << analysis_time
              // << endl;
              cout << "STAT-time-adapt                    " << adapt_time
                   << endl;
              cout << "STAT-procedures-total              "
                   << analyzed_procedures << endl;
              cout << "STAT-procedures-unanalyzed         "
                   << unanalyzed_procedures << endl;
              cout << "STAT-program-size                  " << program_size
                   << endl;
              cout << "STAT-procedures-contextinsensitive "
                   << actual_ci_procedures << endl;
              cout << "STAT-procedures-recursive          "
                   << recursive_procedures << endl;
              cout << "STAT-procedures-contextsensitive   "
                   << actual_cs_procedures << endl;
              ...
              if (adapt_use_unify) // TB_unify
                cout << "STAT-unify-objects               "
                     << pointerOptions::Unify_objects << " out of "
                     << analyzer->memory_size() << endl;
              ...
              for (enum_property_map_cp p = props.begin(); p != props.end();
                ...
                if (property->is_enabled() &&
                  ...
                  cout << "STAT-flowsensitive-property-" << property->name()
                       << " " << temp << endl;
                }
              }
              ...
              for(map<procNode*,int>::iterator b=bb_count.begin();
              b!=bb_count.end(); b++) { procedureInfo *info =
              analyzer->lookup_procedure(b->first); cout << "  " <<
              b->first->decl()->name() << " " << b->second << " "
              ...
              // // hack
              /*
              pointerOptions::Context_sensitive_procedures.insert("show_readme");
              // hack
              pointerOptions::Context_sensitive_procedures.insert("expand_id");
              // hack
              ...
              for (str_list_p f = force_cs_proc.begin();
                   f != force_cs_proc.end(); f++) {
              ...
              if (1 || Annotations::Show_adaptivity) {
                ...
                cout << "Set context-sensitive:             "
                     << new_cs_procedures << endl;
                for (str_set_p sp =
                         pointerOptions::Context_sensitive_procedures.begin();
                ...
                for (str_set_p sp = Annotations::Pruned_procedures.begin();
                     sp != Annotations::Pruned_procedures.end(); ++sp) {
              ...
              if (Annotations::Adaptor_statistics) {
                if (TB_changes)
                  cout << "TB's new adaptor algorithm\n";
                else
                  cout << "Sam's adaptor algorithm with (minor) change\n";
              ...
              cout << "OLD accuracy = " << total_accuracy
                   << ", NEW accuracy = " << new_total_accuracy << endl;
            ...
            analyzer =
                new Analyzer(anns, fixupPhase::Main, linker, fixupPhase::Unit);
            analysis_timer.start();
            ...
            cout << "STAT-program-size                  " << program_size
                 << endl;
            cout << "STAT-procedures-total              " << analyzed_procedures
                 << endl;
            cout << "STAT-procedures-unanalyzed         "
                 << unanalyzed_procedures << endl;
            cout << "STAT-procedures-contextinsensitive "
