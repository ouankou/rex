...
public:
  ...
  virtual void output_expr(output_context &ct, Node *par, int prec,
                           Assoc assoc);
};
...
public:
  ...
  /** @name AST Traversal
   *
   *  Methods to uniformly traverse the AST. See the documentation in the Node
   * class. */
  //@{
...
public:
  ...
  bool qualified_equal_to(typeNode *node2, bool strict_toplevel,
  ...
  virtual void output_type(output_context &ct, Node *par, Assoc context,
                           Type_qualifiers q);
};
...
public:
  patternExpander(callsiteEntry *callsite, propertyAnalyzer *analyzer,
