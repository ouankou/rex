...
callingContext::callingContext(procLocation *where, operandNode *call,
                               propertyAnalyzer *property_analyzer,
                               procedureAnn *procedure_annotations,
                               procedureInfo *caller, Annotations *annotations)
    : _where(where), _call(call), _property_analyzer(property_analyzer),
      _procedure_annotations(procedure_annotations), _caller(caller),
...
void callingContext::set_current_def_and_use(pointerValue &ptr) {
  ...
  for (memoryblock_set_p p = ptr.blocks.begin(); p != ptr.blocks.end(); ++p)
