...
actionChanger::actionChanger(Analyzer *analyzer, Annotations *annotations)
    : Changer(Preorder, Subtree, true),
      _analyzer(analyzer->property_analyzer()), linker(analyzer->getLinker()),
...
Node *actionChanger::at_threeAddr(threeAddrNode *the_3addr, Order ord) {
  ...
  for (procedures_map_cp p = procs.begin(); p != procs.end(); ++p) {
    proc_anns = (*p).second;
  ...
  if (callsite) {
    ...
    if (action) {
      stmtNode *repl = action->generate_replacement(callsite, _analyzer, linker,
                                                    _annotations);
      if (repl) {
