...
literalReportElementAnn::literalReportElementAnn(const parserID *literal)
    : reportElementAnn(literal->line()), _literal(literal->name()) {}
...
void literalReportElementAnn::report(ostream &out, bool is_error,
                                     Analyzer *analyzer, procLocation *where) {
...
expressionReportElementAnn::expressionReportElementAnn(
    Broadway::FlowSensitivity flow_sensitivity, exprAnn *expression, int line)
    : reportElementAnn(line), _flow_sensitivity(flow_sensitivity),
      _expr(expression) {}
...
void expressionReportElementAnn::report(ostream &out, bool is_error,
                                        Analyzer *analyzer,
                                        procLocation *where) {
...
public:
  ...
  static int count_paths(basicblockNode *bb, basicblock_set &visited) {
    int total = 1;
    ...
    for (basicblock_list_p p = bb->preds().begin(); p != bb->preds().end(); ++p)
...
locationReportElementAnn::locationReportElementAnn(const parserID *id)
    : reportElementAnn(id->line()), _kind_name(id->name()), _kind(Callsite) {}

void locationReportElementAnn::lookup(enumPropertyAnn *default_property,
                                      procedureAnn *procedure,
                                      Annotations *annotations) {
  if (_kind_name == "callsite")
    _kind = Callsite;
  else if (_kind_name == "context")
    _kind = Context;
  else if (_kind_name == "numcontexts")
    _kind = Num_Contexts;
  else
...
void locationReportElementAnn::report(ostream &out, bool is_error,
                                      Analyzer *analyzer, procLocation *where) {
  if (Annotations::Quiet_reports && !is_error)
    return;
  ...
  if (_kind == Callsite) {
    // callNode * call =
    // findCallNodeWalker::find(where->stmt_location()->stmt());
  ...
  if (_kind == Context) {
    ...
    while (cur->stmt_location()) {
      ...
      callNode * call =
    findCallNodeWalker::find(callnode(cur->stmt_location()->stmt());
...
bindingReportElementAnn::bindingReportElementAnn(const parserID *varname,
                                                 bool size_only)
    : reportElementAnn(varname->line()), _variable_name(varname->name()),
      _size_only(size_only), _variable(0) {}
...
void bindingReportElementAnn::lookup(enumPropertyAnn *default_property,
  ...
  if (!_variable)
    annotations->Error(line(), string("Undeclared identifier \"") +
                                   _variable_name + "\"");
}

#ifdef __PROCLOCATION

void bindingReportElementAnn::report(ostream &out, bool is_error,
                                     Analyzer *analyzer, procLocation *where) {
  if (Annotations::Quiet_reports && !is_error)
    return;
...
reportAnn::reportAnn(exprAnn *condition, bool is_error,
                     report_element_list *elements, int line)
    : Ann(line), _condition(condition), _elements(), _is_error(is_error) {
  if (elements)
    _elements.swap(*elements);
}

void reportAnn::lookup(procedureAnn *procedure, Annotations *annotations) {
  ...
  for (report_element_list_p p = _elements.begin(); p != _elements.end(); ++p)
    (*p)->lookup((enumPropertyAnn *)0, procedure, annotations);
}

#ifdef __PROCLOCATION

void reportAnn::report(ostream &out, Analyzer *analyzer, procLocation *where,
  ...
  if (trigger) {
    ...
    if (_is_error) {
      ...
      if (_condition)
        _condition->diagnostic(out, where, property_analyzer);
      // end TB new
    }
  }
