...
actionAnn::actionAnn(exprAnn *condition, int line)
    : Ann(line), _kind(Inline), _condition(condition), _replacement(0),
      _candidates() {}

actionAnn::actionAnn(exprAnn *condition, stmtNode *replacement, int line)
    : Ann(line), _kind(Replace), _condition(condition),
      _replacement(replacement), _candidates() {}
...
public:
  miniIdLookupWalker() : Walker(Both, Subtree), _ids() {}
...
public:
  actionLookupWalker(procedureAnn *procedure, Annotations *annotations)
...
void actionAnn::lookup(procedureAnn *procedure, Annotations *annotations) {
...
void actionAnn::test(callsiteEntry *callsite, procLocation *where,
...
stmtNode *actionAnn::generate_replacement(callsiteEntry *callsite,
  ...
  if (_kind == Replace) {
    if (_replacement) {
      ...
      stmtNode *replacement_copy =
          (stmtNode *)ref_clone_changer::clone(_replacement, false);
  ...
  if (_kind == Inline) {
    ...
    if (proc) {
      ...
      //    Clone the call so we can canibalize the actual arguments (memory
      //    leak?)

      operandNode *dup_call =
          (operandNode *)ref_clone_changer::clone(call, false);
      ...
      while (!formal_args.empty()) {
        ...
        operandNode *one_act =
            (operandNode *)ref_clone_changer::clone(actual_args.back(), false);
      ...
      if (ret->expr() && callsite->stmt()->lhs()) {
        ...
        operandNode *lhs = (operandNode *)ref_clone_changer::clone(
            callsite->stmt()->lhs(), false);

        // -- Create a new assignment statement and place it at the end of
        // the basic block.

        threeAddrNode *assignment = new threeAddrNode(
            lhs, new operandNode((indexNode *)returned_expr), call->coord());
