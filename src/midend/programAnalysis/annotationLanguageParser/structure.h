...
public:
  ...
  structureTreeAnn(const parserID *name, structuretree_list *targets,
                   Operator op, bool target_is_new);

  /** Single target constructor */

  structureTreeAnn(const parserID *name, structureTreeAnn *target, Operator op,
                   bool target_is_new);
...
private:
  ...
  // tools (made it private to force compile time error if used).
...
public:
  structureAnn(annVariable *source, annVariable *target,
               const std::string *field_name, const int line);
