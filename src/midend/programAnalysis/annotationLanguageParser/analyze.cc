...
analyzeAnn::analyzeAnn(Annotations *anns, const parserID *property,
                       rule_list *rules, int line)
    : Ann(line), _property(0), _rules(), _default(0), _applicable_rule(0) {
  if (rules) {
    _rules.swap(*rules);
    delete rules;
  }
  ...
  for (rule_list_p p = _rules.begin(); p != _rules.end(); ++p) {
    ruleAnn *rule = *p;
    ...
    if (!rule->has_condition()) {
      ...
      if (_default)
        anns->Error(line,
                    "Each analyze annotation may only have one default rule.");
      else
        _default = rule;
    }
  }
...
void analyzeAnn::compute_next(procLocation *where,
  ...
  if (property()->is_enabled() &&
    ...
    for (rule_list_p p = _rules.begin(); p != _rules.end(); ++p) {
      ...
      if ((rule != _default) && rule->is_condition_true()) {

        if (Annotations::Verbose_properties)
          cout << "- Proposed updates to \"" << property()->name() << "\""
               << endl;
...
void analyzeAnn::lookup(procedureAnn *procedure, Annotations *annotations) {
