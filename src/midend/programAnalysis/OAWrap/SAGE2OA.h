...
public:
  ...
  SymHandle GetSymHandle(LeafHandle /*vh*/) {
    ...
    return 0; // MS: SymHandle is not a pointer, does not compile with 2.96;
              // NULL;
  }

  // for now
  SymHandle GetProcSymHandle(ProcHandle) { return 0; }

  // Given a SymHandle, return the textual name.
  // DQ (1/13/2006): Andreas has pointed out that the return type should be
  // "const char*" instead of "char*"
  const char *GetSymNameFromSymHandle(SymHandle /*sh*/) {
    // ST *st = (ST *) sh;
    // return (char *) ST_name (st);
    return 0; // MS: SymHandle is not a pointer, does not compile
              // with 2.96;NULL;
  }
