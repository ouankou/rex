// tps (1/14/2010) : Switching from rose.h to sage3 changed size from 17,4 MB to
// 7,1MB
#include "SAGE2OA.h"
...
void SageIRStmtIterator::Reset() { st_iter = st_list.begin(); }

SageIRStmtIterator::SageIRStmtIterator(SgStatementPtrList &lst) : st_list(lst) {
...
//-----------------------------------------------------------------------------
SageIRCallsiteParamIterator::SageIRCallsiteParamIterator(
...
void SageIRCallsiteParamIterator::operator++() { arg_iter++; }

void SageIRCallsiteParamIterator::Reset() { arg_iter = begin; }
...
IRProcType SageIRInterface::GetProcType(ProcHandle h) {
  ...
  if (SgFunctionCallExp *fncall = isSgFunctionCallExp((SgNode *)h)) {
    ...
    // computed, most often from operands) SgType *returnType =
    // fncall->get_expression_type();
...
IRStmtIterator *
SageIRInterface::Body(StmtHandle h) // for now same as GetFirstInCompound
{
  SgStatement *sptr = (SgStatement *)h;
  if (SgScopeStatement *scope = isSgScopeStatement(sptr)) {
...
IRStmtIterator *
SageIRInterface::LoopBody(StmtHandle h) // for now same as GetFirstInCompound
{
  SgStatement *sptr = (SgStatement *)h;
  if (SgScopeStatement *scope = isSgScopeStatement(sptr)) {
    SgStatementPtrList &stptr = scope->getStatementList();
...
IRStmtIterator *SageIRInterface::TrueBody(StmtHandle h) {
  ...
  // DQ (11/27/2005): I assume that the function should return something if the
  // conditional is false and that it is an error!
  else {
    printf(
        "Note: return not handled by author for else case, assumed error! \n");
    ROSE_ABORT();
  }
}
...
ExprHandle SageIRInterface::GetUMultiCondition(StmtHandle,
                                               int /*targetIndex*/) {
...
ExprHandle SageIRInterface::GetMultiExpr(StmtHandle) {
  // Whirl does not appear to have a structured switch statement. And what about
  // Sage??
  ROSE_ABORT();
}
}; // namespace OpenAnalysis
