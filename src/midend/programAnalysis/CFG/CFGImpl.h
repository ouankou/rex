...
class CFGImplTemplate : public VirtualGraphCreateTemplate<Node, Edge>,
                        public BuildCFGConfig<Node> {
protected:
  ...
  virtual void AddNodeStmt(Node *n, const AstNodePtr &s) { n->AddNodeStmt(s); }

public:
  typedef typename VirtualGraphCreateTemplate<Node, Edge>::NodeIterator
      NodeIterator;
  typedef typename VirtualGraphCreateTemplate<Node, Edge>::EdgeIterator
      EdgeIterator;

  CFGImplTemplate(BaseGraphCreate *_impl = 0)
      : VirtualGraphCreateTemplate<Node, Edge>(_impl) {}

  NodeIterator GetPredecessors(Node *n) {
    return new GraphNodePredecessorIterator<CFGImplTemplate<Node, Edge>>(this,
                                                                         n);
  }
  NodeIterator GetSuccessors(Node *n) {
    return new GraphNodeSuccessorIterator<CFGImplTemplate<Node, Edge>>(this, n);
  }
};
...
public:
  ...
  virtual void Dump() const { write(std::cerr); }
...
public:
  ...
  virtual std::string toString() const {
    switch (t) {
    case CFGConfig::COND_TRUE:
      return "true";
    case CFGConfig::COND_FALSE:
      return "false";
    case CFGConfig::ALWAYS:
      return "always";
    default:
      ROSE_ABORT();
    }
