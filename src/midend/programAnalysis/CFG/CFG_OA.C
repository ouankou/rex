...
#ifndef TEMPLATE_ONLY
OpenAnalysis::ROSE_CFG_Wrap ::~ROSE_CFG_Wrap() { delete oaCfg; }
#endif
...
// DQ (1/9/2010): The Intel compiler also catches this problem (class Edge
// template parameter not used in the template declaration) template <class
// Node, class Edge>
template <class Node>
void OpenAnalysis::BuildCFG(AstInterface &fa, const AstNodePtr &head,
                            BuildCFGConfig<Node> &ng) {
...
// Node, class Edge>
template <class Node>
void OpenAnalysis::OA2ROSE_CFG_Translate(ROSE_CFG_Wrap &wrap,
                                         BuildCFGConfig<Node> &ng) {
  ...
  for (CFG::DFSIterator nodeIter1(g); nodeIter1; ++nodeIter1) {
    ...
    for (DGraph::OutgoingEdgesIterator edgeIter(n); edgeIter; ++edgeIter) {
      ...
      switch (e->getType()) {
      case CFG::TRUE_EDGE:
        t = CFGConfig::COND_TRUE;
        break;
      case CFG::FALSE_EDGE:
        t = CFGConfig::COND_FALSE;
        break;
      default:
        ROSE_ASSERT(!"Bad CFG edge case");
      }
