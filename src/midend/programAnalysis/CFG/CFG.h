...
template <class Node> class BuildCFGConfig : public CFGConfig {
public:
...
template <class Node>
void BuildCFG(AstInterface &fa, const AstNodePtr &head,
              BuildCFGConfig<Node> &g);
template <class Node>
void BuildCFG(AstInterface &fa, const AstInterface::AstNodeList &head,
              BuildCFGConfig<Node> &g);
}; // namespace ROSE_Analysis
...
public:
  ...
  void operator()(AstInterface &fa, const AstNodePtr &head) {
    ProcessAstTree<AstNodePtr>::operator()(fa, head);
  }
...
private:
  ...
  //   each compound statement (eg, loop, if, basic-block) maps to the last
  //   graph node containing its statements
  // startMap  and exitMap map stmt to the start graph node and to the exit
  // (next)graph node respectively
  ...
  bool IsBodyEmpty(AstInterface &fa, const AstNodePtr &s) {
    ...
    if (fa.IsBlock(s) && (first = fa.GetBlockFirstStmt(s)) == AST_NULL) {
      if (debug_cfg())
        std::cerr << "block " << AstInterface::AstToString(s) << " is empty "
                  << std::endl;
      return true;
    } else {
      if (debug_cfg()) {
        std::cerr << "block " << AstInterface::AstToString(s)
                  << " is not empty ";
        if (first != AST_NULL)
          std::cerr << "first statement: " << AstInterface::AstToString(first)
                    << std::endl;
        else
          std::cerr << std::endl;
      }
  ...
  Node *MapStmt(AstInterface &fa, const AstNodePtr &s, MapType t,
                bool add = false) {
    assert(s != AST_NULL);
  ...
  Node *AddStmt(AstInterface &fa, Node *n,
                const AstNodePtr &s) // add s to the node n
  {
  ...
  void AddBranch(Node *cur, Node *dest, CFGConfig::EdgeType val) {
    ...
    if (debug_cfg())
      std::cerr << "add " << CFGConfig::EdgeType2String(val) << " edge from "
                << cur << " to " << dest << std::endl;
  }
  ...
  virtual bool ProcessFunctionDefinition(AstInterface &fa, const AstNodePtr &s,
    ...
    if (debug_cfg()) {
      ...
      std::cerr << ((t == AstInterface::PreVisit) ? " previsit "
                                                  : " postvisit ")
                << std::endl;
    }
    ...
    return ProcessAstTree<AstNodePtr>::ProcessFunctionDefinition(fa, s, body,
                                                                 t);
  }
  ...
  virtual bool ProcessLoop(AstInterface &fa, const AstNodePtr &s,
    ...
    if (t == AstInterface::PreVisit) {
      ...
      Node *bodyNode = (isPostLoop && IsCurNodeEmpty())
          ? SetStmtNode(body, lastNode, START),
           lastNode : MapStmt(fa, body, START);
      Node *testNode = 0;
      if (cond != AST_NULL) {
        testNode =
            (!isPostLoop && IsCurNodeEmpty()) ? AddStmt(fa, lastNode, cond),
        lastNode                              : MapStmt(fa, cond, START, true);
  ...
  virtual bool ProcessIf(AstInterface &fa, const AstNodePtr &s,
    ...
    if (t == AstInterface::PreVisit) {
      ...
      Node *testNode = IsCurNodeEmpty() ? AddStmt(fa, lastNode, cond)
                                        : MapStmt(fa, cond, START, true);
      AddBranch(lastNode, testNode, CFGConfig::ALWAYS);
      Node *falseNode =
          IsBodyEmpty(fa, falseBody) ? 0 : MapStmt(fa, falseBody, START);
      if (falseNode != 0) {
        AddBranch(testNode, falseNode, CFGConfig::COND_FALSE);
        SetStmtNode(falseBody, exitNode, EXIT);
      }
      Node *trueNode =
          IsBodyEmpty(fa, trueBody) ? 0 : MapStmt(fa, trueBody, START);
      if (trueNode != 0) {
      ...
      bool empty1 = IsBodyEmpty(fa, trueBody),
           empty2 = IsBodyEmpty(fa, falseBody);
    ...
    return ProcessAstTree<AstNodePtr>::ProcessIf(fa, s, cond, trueBody,
                                                 falseBody, t);
  }
      ...
      if (n1 != 0) {
        ...
        if (lastStmt != AST_NULL)
          SetStmtNode(lastStmt, n1, EXIT);
               ...
               0) { // use exitMap to remember the last node of this block
      SetStmtNode(s, lastNode, EXIT);
    } else if ((lastNode = GetStmtNode(s, EXIT)) != 0)
      SetCurNode(lastNode);
    return ProcessAstTree<AstNodePtr>::ProcessBlock(fa, s, t);
  }
  ...
  virtual bool ProcessStmt(AstInterface &fa, const AstNodePtr &s) {
    if (debug_cfg())
      std::cerr << "processing stmt " << AstInterface::AstToString(s)
                << std::endl;
    ...
    return ProcessAstTree<AstNodePtr>::ProcessStmt(fa, s);
  }
};

template <class Node>
void ROSE_Analysis::BuildCFG(AstInterface &fa, const AstNodePtr &head,
                             BuildCFGConfig<Node> &g) {
...
template <class Node>
void ROSE_Analysis::BuildCFG(AstInterface &fa,
                             const AstInterface::AstNodeList &stmts,
                             BuildCFGConfig<Node> &g) {
  BuildCFGTraverse<Node> op(g);
  for (AstInterface::AstNodeList::const_iterator p = stmts.begin();
       p != stmts.end(); ++p) {
