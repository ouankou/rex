...
void HasValueMap::set_val(const AstNodePtr &ast,
  ...
  if (DebugValuePropogate()) {
    std::cerr << "set value for " << ast.get_ptr() << ":"
              << AstInterface::AstToString(ast) << " : ";
    valmap[ast].Dump();
    std::cerr << std::endl;
  }
}
...
void HasValueMapReplace ::VisitFunction(const SymbolicFunction &u) {
  ...
  if (dot != 0 && dot->first_arg().GetValType() == VAL_AST) {
    AstNodePtr curast;
    if (!dot->first_arg().isAstWrap(curast))
      ROSE_ABORT();
    ...
    if (hasval && curval.has_value(field, &replval) && !replval.is_top() &&
        !replval.is_bottom()) {
      repl = replval;
    } else if (usedefault &&
               ValueAnnotation::get_inst()->known_type(fa, curast, &curval) &&
...
public:
  ...
  bool operator()(const AstNodePtr &curast, const HasValueDescriptor &curval) {
    ...
    if (nodeCollect != 0) {
      std::map<AstNodePtr, ValuePropagateNode *>::const_iterator p =
          nodemap.find(curast);
      if (p != nodemap.end()) {
        ...
        if (DebugValuePropogate()) {
          std::cerr << "found node for ref: "
                    << AstInterface::AstToString(curast) << std::endl;
          (*p).second->Dump();
        }
      }
    }
...
ValuePropagateNode *ValuePropagate::CreateNode(AstInterface &fa,
...
void ValuePropagate::build(AstInterface &fa, const AstNodePtr &h,
...
void ValuePropagate::build(AstInterface &fa, const AstNodePtr &head,
                           AliasAnalysisInterface &alias,
                           FunctionSideEffectInterface *f) {
  ReachingDefinitionAnalysis r;
  if (DebugValuePropogate())
...
void HasValueMap::ObserveCopyAst(AstInterfaceImpl &fa, const AstNodePtr &orig,
                                 const AstNodePtr &copy) {
  copy_value(fa, orig, copy);
}

void HasValueMap::copy_value(AstInterfaceImpl &fa, const AstNodePtr &orig,
                             const AstNodePtr &copy) {
  HasValueDescriptor desc;
  if (has_value(orig, &desc)) {
    set_val(copy, desc);
    if (DebugValuePropogate()) {
      std::cerr << "copying ast: " << AstInterface::AstToString(copy)
                << copy.get_ptr() << " to have value " << desc.toString()
                << std::endl;
    }
  }
}
...
bool ValuePropagate::known_value(const AstNodePtr &exp,
                                 HasValueDescriptor *result, bool *change) {
  std::map<AstNodePtr, ValuePropagateNode *>::const_iterator p =
      nodemap.find(exp);
  if (p == nodemap.end()) {
