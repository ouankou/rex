...
void ArrayShapeDescriptor::push_back(
    const ReadSymbolicFunctionDeclaration &cur) {
...
void ArrayElemDescriptor::push_back(
    const ReadSymbolicFunctionDeclaration &cur) {
  std::string annot = cur.first.first;
  if (annot == "elem") {
...
void ArrayShapeDescriptor ::Dump() const { write(std::cerr); }
...
void ArrayElemDescriptor ::Dump() const { write(std::cerr); }
...
void ArrayDescriptor ::Dump() const { write(std::cerr); }

void ArrayDefineDescriptor ::replace_var(const std::string &varname,
                                         const SymbolicVal &repl) {
  ArrayDescriptor::replace_var(varname, repl);
  reshape.replace_var(varname, repl);
}

void ArrayDefineDescriptor ::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
  ArrayDescriptor ::replace_val(repl);
  reshape.replace_val(repl);
}

void ArrayDefineDescriptor::push_back(
    const ReadSymbolicFunctionDeclaration &cur) {
  std::string annot = cur.first.first;
  if (annot == "reshape") {
    ...
    reshape.push_back(
        SymbolicFunctionDeclaration(cur.first.second, cur.second));
  } else
...
void ArrayDefineDescriptor ::Dump() const { write(std::cerr); }
...
void ArrayOptDescriptor ::replace_var(const std::string &varname,
                                      const SymbolicVal &repl) {
  ArrayDescriptor::replace_var(varname, repl);
  for (DefContainer::iterator p = defs.begin(); p != defs.end(); ++p) {
    DefineVariableDescriptor &cur = *p;
    cur.replace_var(varname, repl);
  }
}

void ArrayOptDescriptor ::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
  ArrayDescriptor ::replace_val(repl);
  for (DefContainer::iterator p = defs.begin(); p != defs.end(); ++p) {
    DefineVariableDescriptor &cur = *p;
...
void ArrayOptDescriptor ::Dump() const { write(std::cerr); }
...
bool ArrayAnnotation ::known_array_type(CPPAstInterface &fa,
                                        const AstNodeType &type,
                                        ArrayDefineDescriptor *r) {
  return arrays.known_type(fa, type, r);
}

bool ArrayAnnotation ::known_unique_array(CPPAstInterface &fa,
                                          const AstNodePtr &array,
                                          ArrayDefineDescriptor *d) {
  std::string varname;
  AstNodePtr scope;
  if (!fa.IsVarRef(array, 0, &varname, &scope, 0,
...
bool ArrayAnnotation ::known_array(CPPAstInterface &fa, const AstNodePtr &array,
                                   ArrayDefineDescriptor *r) {
...
bool ArrayAnnotation ::has_array_opt(CPPAstInterface &fa,
                                     const AstNodePtr array,
                                     ArrayOptDescriptor *r) {
  return arrayopt.known_type(fa, array, r);
}

bool ArrayAnnotation::is_array_mod_op(CPPAstInterface &fa,
                                      const AstNodePtr &arrayExp,
                                      AstNodePtr *arrayp,
                                      ArrayDescriptor *descp, bool *reshape,
                                      ReplaceParams *repl) {
  ...
  if (reshape != 0) {
    *reshape = !desc.second.get_dimension().is_top() ||
               desc.second.get_length().size();
  }
...
bool ArrayAnnotation::is_array_construct_op(CPPAstInterface &fa,
                                            const AstNodePtr &arrayExp,
                                            CPPAstInterface::AstNodeList *alias,
                                            ArrayDescriptor *descp,
                                            ReplaceParams *repl) {
  CPPAstInterface::AstNodeList args;
  ...
  if (alias != 0) {
    for (std::list<SymbolicValDescriptor>::const_iterator p =
             desc.first.begin();
         p != desc.first.end(); ++p) {
...
AstNodePtr ArrayAnnotation::create_access_array_elem(
    CPPAstInterface &fa, const AstNodePtr &array,
    const AstInterface::AstNodeList &args) {
  return arrays.create_known_member_function(fa, array, "elem", args);
}

SymbolicVal ArrayAnnotation ::create_access_array_elem(
    const AstNodePtr &array, const SymbolicFunction::Arguments &args) {
  return arrays.create_known_member_function(array, "elem", args);
}

AstNodePtr ArrayAnnotation ::create_reshape_array(
    CPPAstInterface &fa, const AstNodePtr &array,
...
SymbolicVal
ArrayAnnotation::create_access_array_length(const AstNodePtr &array,
                                            const SymbolicVal &dim) {
  SymbolicFunction::Arguments args;
  args.push_back(dim);
  return arrays.create_known_member_function(array, "length", args);
}

AstNodePtr ArrayAnnotation::create_access_array_length(CPPAstInterface &fa,
                                                       const AstNodePtr &array,
                                                       int dim) {
  CPPAstInterface::AstNodeList args;
...
bool ArrayAnnotation ::is_access_array_length(CPPAstInterface &fa,
                                              const SymbolicVal &orig,
                                              AstNodePtr *arrayp,
                                              SymbolicVal *dim) {
  SymbolicFunction::Arguments args;
...
bool ArrayAnnotation ::is_access_array_elem(CPPAstInterface &fa,
                                            const SymbolicVal &orig,
                                            AstNodePtr *array,
                                            SymbolicFunction::Arguments *args) {
...
bool ArrayAnnotation ::is_access_array_elem(
    CPPAstInterface &fa, const AstNodePtr &orig, AstNodePtr *arrayp,
    CPPAstInterface::AstNodeList *args) {
  if (arrays.is_known_member_function(fa, orig, arrayp, args) == "elem") {
    if (DebugArrayAnnot()) {
      std::cerr << "Recognized access-array-elem operator: "
                << AstInterface::AstToString(orig);
      std::cerr << std::endl;
    }
    return true;
  }
  if (DebugArrayAnnot()) {
    std::cerr << "NOT access-array-elem operator: "
              << AstInterface::AstToString(orig);
    std::cerr << std::endl;
  }
  return false;
...
bool ArrayAnnotation ::is_reshape_array(CPPAstInterface &fa,
                                        const AstNodePtr &orig,
                                        AstNodePtr *arrayp,
                                        CPPAstInterface::AstNodeList *args) {
...
bool ArrayAnnotation ::may_alias(
    AstInterface &_fa, const AstNodePtr &fc, const AstNodePtr &result,
  ...
  if (is_access_array_elem(fa, fc) || is_access_array_length(fa, fc) ||
    ...
    return true;
  return OperatorAliasAnnotation::get_inst()->may_alias(fa, fc, result,
                                                        collectalias);
}

bool ArrayAnnotation ::allow_alias(
...
bool ArrayAnnotation ::get_read(AstInterface &_fa, const AstNodePtr &fc,
                                CollectObject<AstNodePtr> *collect) {
  CPPAstInterface &fa = static_cast<CPPAstInterface &>(_fa);
  AstNodePtr dim;
