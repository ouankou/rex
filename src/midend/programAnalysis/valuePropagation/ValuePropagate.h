...
public:
  ...
  void copy_value(AstInterfaceImpl &fa, const AstNodePtr &orig,
                  const AstNodePtr &copy);
  friend class ValuePropagate;
};

class HasValueMapReplace : public MapObject<SymbolicVal, SymbolicVal>,
...
public:
  ValuePropagate(BaseGraphCreate *c = 0) : DefUseChain<ValuePropagateNode>(c) {}
  void build(AstInterface &fa, const AstNodePtr &head,
             ReachingDefinitionAnalysis &r, AliasAnalysisInterface &alias,
             FunctionSideEffectInterface *f = 0);
  void build(AstInterface &fa, const AstNodePtr &head,
             AliasAnalysisInterface &alias, FunctionSideEffectInterface *f = 0);

  bool known_value(const AstNodePtr &exp, HasValueDescriptor *result = 0,
                   bool *change = 0);
