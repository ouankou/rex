...
void HasValueDescriptor ::replace_val(
...
void RestrictValueOpDescriptor ::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
  for (iterator p = begin(); p != end(); ++p) {
...
void RestrictValueOpDescriptor::replace_val(ReplaceParams &repl) {
  for (RestrictValueDescriptor &current : *this) {
    repl.replace_target(current.first);
    current.second.replace_val(repl);
  }
}

std::string HasValueCollection::is_known_member_function(
    AstInterface &fa, const SymbolicVal &exp, AstNodePtr *objp,
    SymbolicFunction::Arguments *argsp, HasValueDescriptor *descp) {
  std::string op1, op2;
...
bool ValueAnnotation ::known_type(AstInterface &fa, const AstNodePtr &exp,
                                  HasValueDescriptor *r) {
...
class ReplaceValue : public MapObject<SymbolicVal, SymbolicVal>,
  ...
  for (RestrictValueOpDescriptor::const_iterator p = desc.begin();
    ...
    if (repl(curval)) {
      if (DebugValuePropogate()) {
        std::cerr << "found restrict value : "
                  << AstInterface::AstToString(curast) << ":"
                  << AstInterface::AstToString(exp);
        curval.Dump();
        std::cerr << std::endl;
      }
      (*descp)(curast, curval);
      ...
      if (DebugValuePropogate()) {
        std::cerr << "discard restrict value : "
                  << AstInterface::AstToString(curast) << ":"
                  << AstInterface::AstToString(exp);
        curval.Dump();
        std::cerr << std::endl;
...
bool ValueAnnotation::is_access_value(CPPAstInterface &fa,
                                      const AstNodePtr &exp, AstNodePtr *obj,
                                      std::string *name,
                                      AstInterface::AstNodeList *args,
                                      HasValueDescriptor *desc) {
  std::string funcname =
      values.is_known_member_function(fa, exp, obj, args, desc);
  if (funcname != "") {
    if (name != 0)
      *name = funcname;
    return true;
