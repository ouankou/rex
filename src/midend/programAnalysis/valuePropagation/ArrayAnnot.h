...
class ArrayDescriptor : public ArrayShapeDescriptor,
                        public ArrayElemDescriptor {
public:
  void push_back(const ReadSymbolicFunctionDeclaration &cur);
  bool read(std::istream &in);
  void write(std::ostream &out) const;
...
public:
  ...
  SymbolicFunctionDeclarationGroup get_reshape() const { return reshape; }
};
...
public:
  ...
  InitVarIterator init_var_begin() { return defs.begin(); }
  InitVarIterator init_var_end() { return defs.end(); }
...
public:
  void replace_val(MapObject<SymbolicVal, SymbolicVal> &repl) {
    for (std::list<SymbolicValDescriptor>::iterator p = first.begin();
         p != first.end(); ++p) {
      (*p).replace_val(repl);
    }
...
class ArrayModifyDescriptor
    : public OPDescriptorTemp<
          CollectPair<CloseDescriptor<SymbolicValDescriptor, '(', ')'>,
                      ArrayDescriptor, 0>> {
  typedef OPDescriptorTemp<CollectPair<
      CloseDescriptor<NameDescriptor, '(', ')'>, ArrayDescriptor, 0>>
      BaseClass;
...
class UniqueArrayCollection : public TypeAnnotCollection<ArrayDefineDescriptor>,
                              public CPPTypeCollection<ArrayDefineDescriptor> {
  typedef TypeAnnotCollection<ArrayDefineDescriptor> BaseClass;
  virtual bool read_annot_name(const std::string &annotName) const {
...
class ArrayModifyOpCollection
    : public OperatorAnnotCollection<ArrayModifyDescriptor> {
  virtual bool read_annot_name(const std::string &annotName) const {
...
class ArrayAnnotation : public FunctionSideEffectInterface,
...
public:
  ...
  bool known_array(CPPAstInterface &fa, const AstNodePtr &array,
                   ArrayDefineDescriptor *d = 0);
  bool known_unique_array(CPPAstInterface &fa, const AstNodePtr &array,
                          ArrayDefineDescriptor *d = 0);
  bool known_array_type(CPPAstInterface &fa, const AstNodeType &array,
                        ArrayDefineDescriptor *d = 0);
  bool has_array_opt(CPPAstInterface &fa, const AstNodePtr array,
                     ArrayOptDescriptor *d = 0);
  ...
  bool is_access_array_length(CPPAstInterface &fa, const AstNodePtr &orig,
                              AstNodePtr *array = 0, AstNodePtr *dimast = 0,
                              int *dim = 0);
  bool is_access_array_elem(CPPAstInterface &fa, const SymbolicVal &orig,
                            AstNodePtr *array = 0,
                            SymbolicFunction::Arguments *args = 0);
  bool is_access_array_length(CPPAstInterface &fa, const SymbolicVal &orig,
                              AstNodePtr *array = 0, SymbolicVal *dim = 0);

  SymbolicVal create_access_array_elem(const AstNodePtr &array,
                                       const SymbolicFunction::Arguments &args);
  SymbolicVal create_access_array_length(const AstNodePtr &array,
  ...
  AstNodePtr create_access_array_length(CPPAstInterface &fa,
