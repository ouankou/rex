...
// DQ (6/30/2015): This is a new call graph analysis designed to support
// intremental generation of the a dot file representing the call graph of an
// application as it is compiled file by file.  It is the start of a different
// sort fo ultifile support in ROSE.
...
public:
  ...
  // Avoid redundant nodes (e.g. inlined functions that would be seen in each
  // translation unit).
...
public:
  ...
  // Avoid redundant edges (e.g. function call in inlined functions that would
  // be seen in each translation unit).
  bool skip_writing_out_to_file;
};
...
class CallGraphFileStructure {
  ...
  // Note:
  //   1) If we want to write and read variable length strings then we would
  //   have
  ...
  //   2) We could also write out a table into the top of the binary file to
  //   hold
  // all of the file names, directory names, and command lines used to compile
  // the source files to generate each function.  This would be a space
...
public:
  ...
  char function_name[functionNameLength];      // If this is an edge this is the
                                               // "from" function.
  char function_call_name[functionNameLength]; // if this is an edge then this
                                               // is the "to" function.
  char file_name[fileNameLength]; // if this is a node then this is the absolute
                                  // path with the filename for the function.
  // If this is an node then this is the location from which to compile the
  // source code (using the saved command line).
  char directoryToUseInCompilingSourceFile[directoryLength];

  // This is a potentially very long string.
  // If this is an node then this is the command line required to compile the
  // source file to generate the AST for the defining function declaration.
...
std::vector<CallGraphFileStructure> *
readCallGraphFile(std::string binaryFilename);

// Write out the dot file for the call graph (uses data from whatever is written
// in the binary file).
