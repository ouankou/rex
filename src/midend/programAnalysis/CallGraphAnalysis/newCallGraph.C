...
// Things to consider:
//   1) Generate subgraphs to show the separate translation units (for each
//   file). 2) Build a single data structure to represent the call graph and
//   associated data:
//         Source position of function declaration, function definition, and
//         function calls
...
CallGraphEdgeInfo::CallGraphEdgeInfo(CallGraphNodeInfo *from,
...
CallGraphFileStructure::CallGraphFileStructure()
    ...
    // : isNode(true), function_name({0}), function_call_name({0}),
    // file_name({0})
  ...
  fill(directoryToUseInCompilingSourceFile,
       directoryToUseInCompilingSourceFile + directoryLength, 0);
  fill(commandlineToCompileSourceFile,
       commandlineToCompileSourceFile + commandlineLength, 0);
}

void CallGraph::visit(SgNode *astNode) {
  ...
  if (functionDeclaration != NULL) {
    ...
    if (functionDeclaration->get_definingDeclaration() != NULL) {
      if (functionDeclaration ==
        ...
        // Check for if this function is in the nodeMap and if so update the
        // pointer to the function declaration with the defining function
        // declaration.
        printf(
            "This IS the defining function declaration: mangled_name = %s \n",
            name.c_str());
        if (nodeMap.find(name) != nodeMap.end()) {
          printf("Update function declaration in CallGraphNodeInfo to use the "
  ...
  if (functionRefExpression != NULL) {
    ...
    printf("Found a function ref expression: functionRefExpression = %p = %s = "
           "%s \n",
           functionRefExpression, functionRefExpression->class_name().c_str(),
           functionSymbol->get_name().str());

    // How do we handle a function call from global scope (e.g. constructor
    // initialization)?
    SgFunctionDeclaration *from_functionDeclaration =
        SageInterface::getEnclosingFunctionDeclaration(functionRefExpression);
    ...
    string from_function_name =
        SageInterface::get_name(from_functionDeclaration);
    ...
    // CallGraphNodeInfo* from = new
    // CallGraphNodeInfo(from_functionDeclaration);
    CallGraphNodeInfo *from = NULL;
    if (nodeMap.find(from_function_name) != nodeMap.end()) {
      from = nodeMap[from_function_name];
    } else {
      // This might be a case of a C language implicit function call (function
      // without a prtotype).
      from = new CallGraphNodeInfo(from_functionDeclaration);

      // We might want to insert a call graph node to stand for an implicit
      // function. edgeMap.insert(pair<string,CallGraphEdgeInfo*>(name,new
      // CallGraphEdgeInfo(from,to)));
    ...
    if (nodeMap.find(to_function_name) != nodeMap.end()) {
      ...
      // This might be a case of a C language implicit function call (function
      // without a prototype). Or in C++ when a function call appears before
      // it's prototype or definition in a class or structure.
...
int NewCallGraph::getLock() {
  ...
  // open("rose_callgraph_lockfile.lock", O_WRONLY | O_CREAT | O_EXCL)) == -1 )
  while ((fd = open("rose_callgraph_lockfile.lock", O_WRONLY | O_CREAT | O_EXCL,
                    0666)) == -1) {
...
void NewCallGraph::releaseLock(int fd) {
  close(fd);
...
NewCallGraph::readCallGraphFile(string binaryFilename) {
  // Open the callgraph file and fill in the nodeMap and edgeMap in the
  // CallGraph.

  // Open the file and put the file cursor at the end of the file so that the
  // file size can be computed.
  ifstream binaryStream(binaryFilename.c_str(),
  ...
  // This should make sure that we have a file with a size that is a whole
  // multiple of the CallGraphFileStructure data members.
  ...
  vector<CallGraphFileStructure> *callGraphVector =
      new vector<CallGraphFileStructure>(numberOfEntries);

  // Build a pointer to the first element of the STL vector of entries allocated
  // above.
  ...
  // Close the file (for reading), we will re-open it for writing (overwriting
  // with new data)
  binaryStream.close();
...
void NewCallGraph::generateCallGraphFile(SgProject *project, CallGraph &cg) {
  ...
  // This file is to be used for debugging (a simple text form of the binary
  // file's data. This file in regenerated from scratch the same as the binary
  // file. ofstream datafile ( output_filename.c_str() , ios::out | ios::app );
  ofstream datafile(output_filename.c_str(), ios::out);

  printf("WARNING: part 1: text file is not yet implemented to be consistatn "
  ...
  // data (e.g. where we have located defining declaration in the current
  // translation unit that were referenced using non-defining declaration in the
  // entries from the binary file). ofstream
  // binaryStream(binaryFilename.c_str(), std::ios::binary); ofstream
  // binaryStream(binaryFilename.c_str(), ofstream::ate | ios::binary); ofstream
  // binaryStream(binaryFilename.c_str(), ofstream::app | ios::binary);
  ...
  // This assumes that there is only one source file per invokation of ROSE on
  // the command line.
  ...
  if (callGraphVector != NULL) {
    ...
    // Iterate over the data from the file of previously build call graph data
    // (from the saved file).
    while (i != callGraphVector->end()) {
      if (i->isNode == true) {
        ...
        if (nodeMap.find(function_name) != nodeMap.end()) {
          ...
          // If this translation unit has the defining declaration then update
          // the source file in the date we write (update) in the saved binary
          // file.
          if (functionDeclaration ==
              functionDeclaration->get_definingDeclaration()) {
            if (i->isDefiningDeclaration == false) {
              // Change the file name associated with this function to the
              // current translation unit.
              filename = source_file;
            } else {
              // Use the file name recorded from the first translation using to
              // see the function definition.
              printf(
                  "Note: This node (function_name = %s) was previously defined "
                  "in filename = %s but is also defined in source_file = %s \n",
        ...
        // This function from the binary file is not in the current translation
        // unit (and we can't add it to the CallGraphNodeInfo because the
        // function declaration is not in this translation unit).  So we can
        // skip processing it here.
        CallGraphFileStructure function;

        function.isNode = true;
        function.isDefiningDeclaration = i->isDefiningDeclaration;

        memcpy(function.function_name, i->function_name,
               strlen(i->function_name) + 1);
        // memcpy (function.file_name, i->file_name, strlen(i->file_name)+1);
        memcpy(function.file_name, filename.c_str(), filename.length());

        binaryStream.write(reinterpret_cast<const char *>(&function),
                           sizeof(CallGraphFileStructure));
        SgName mangledName = i->function_name;

        ROSE_ASSERT(mangledName.get_length() < functionNameLength);

        string s = "ORIGINAL NODE: filename," + source_file + ", function, " +
                   mangledName.str() + "\n";
        ...
        if (edgeMap.find(edge_name) != edgeMap.end()) {
          // This edge can be marked to not be updated from the current
          // translation unit.
        ...
        // This function call (edge) from the binary file is not in the current
        // translation unit (and we can't add it to the CallGraphNodeInfo
        // because the edge is not is not in this translation unit).  So we need
        // to write it into the binary file.
        ...
        // memcpy (functionCall.function_name,      i->function_name,
        // strlen(i->function_name)+1); memcpy (functionCall.function_call_name,
        // to_function_name, strlen(to_function_name)+1);
        memcpy(functionCall.function_name, from_function_name.c_str(),
               from_function_name.length());
        memcpy(functionCall.function_call_name, to_function_name.c_str(),
               to_function_name.length());

        binaryStream.write(reinterpret_cast<const char *>(&functionCall),
                           sizeof(CallGraphFileStructure));
        // string edge_name = (*i_edge).first;
        // SgName from_mangledName =
        // (*i_edge).second->from->functionDeclaration->get_mangled_name();
        // SgName to_mangledName   =
        // (*i_edge).second->to->functionDeclaration->get_mangled_name();

        // string s = "ORIGINAL EDGE: filename," + source_file +
        //            ", from function, " + from_function_name.c_str() +
        //            ", to function, " + to_function_name.c_str() + "\n";
        string s = "ORIGINAL EDGE: filename," + source_file + ", edge, " +
                   edge_name + "\n";
  ...
  while (i_node != nodeMap.end()) {
    // SgName mangledName =
    // (*i_node).second->functionDeclaration->get_mangled_name();
    SgName mangledName =
        SageInterface::get_name((*i_node).second->functionDeclaration);

    ROSE_ASSERT(mangledName.get_length() < functionNameLength);

    // string s = "filename," + source_file + ", function, " +
    // (*i_node).second->functionDeclaration->get_name().str() + "\n"; datafile
    // << s; The data structure is initialized to zero in the constuctor.
    // CallGraphFileStructure function = { true, {0}, {0}, {0} };
    CallGraphFileStructure function;

    // This test avoid recording nodes in the call graph redundantly in the
    // binary file.
    if ((*i_node).second->skip_writing_out_to_file == false) {
      function.isNode = true;
      if ((*i_node).second->functionDeclaration ==
          (*i_node).second->functionDeclaration->get_definingDeclaration()) {
        function.isDefiningDeclaration = true;
      } else {
        function.isDefiningDeclaration = false;
      }
      memcpy(function.function_name, mangledName.str(),
             mangledName.get_length());
      ...
      // string s = "original entry: filename," + source_file + ", function, " +
      // mangledName.str() + "\n"; string s = "NEW NODE: in this tranalation
      // unit: filename," + source_file + ", function, " +
      // (*i_node).second->functionDeclaration->get_name().str() + "\n";
      string s = "NEW NODE: in this tranalation unit: filename," + source_file +
                 ", function, " + mangledName.str() + "\n";
      datafile << s;
    } else {
    }
  ...
  while (i_edge != edgeMap.end()) {
    ...
    // SgName from_mangledName =
    // (*i_edge).second->from->functionDeclaration->get_mangled_name(); SgName
    // to_mangledName   =
    // (*i_edge).second->to->functionDeclaration->get_mangled_name();
    SgName from_mangledName =
        SageInterface::get_name((*i_edge).second->from->functionDeclaration);
    SgName to_mangledName =
        SageInterface::get_name((*i_edge).second->to->functionDeclaration);
    CallGraphFileStructure functionCall;
    ...
    if ((*i_edge).second->skip_writing_out_to_file == false) {
      ...
      memcpy(functionCall.function_name, from_mangledName.str(),
             from_mangledName.get_length());
      memcpy(functionCall.function_call_name, to_mangledName.str(),
             to_mangledName.get_length());

      binaryStream.write(reinterpret_cast<const char *>(&functionCall),
                         sizeof(CallGraphFileStructure));
      // string s = "NEW EDGE: filename," + source_file +
      //       ", from function, " +
      //       (*i_edge).second->from->functionDeclaration->get_name().str() +
      //       ", to function, "   +
      //       (*i_edge).second->to->functionDeclaration->get_name().str() +
      //       "\n";
      string s =
          "NEW EDGE: filename," + source_file + ", edge, " + edge_name + "\n";

      datafile << s;
    } else {
      printf(
          "Skip output of this edge for a function call in this translation "
          "unit) to the binary file (redundantly present in existing file) \n");
      printf("   --- edge_namee = %s \n", edge_name.c_str());
...
int NewCallGraph::buildCallGraph(SgProject *project) {
  int status = 0;
