...
protected:
  ...
  // at any point in CFG, the variable in question and a relevant node impacting
  // it: def vs. use for example a single CFG location may have info. about
  // multiple variables. So it is a vector of pairs.
  typedef std::vector<std::pair<SgInitializedName *, SgNode *>> multitype;
  typedef std::map<SgNode *, multitype>
      tabletype; // store all nodes' analysis results
  ...
  // obtain SgInitializedName from an input expression, such as array ref or
  // pointer dereference
  SgInitializedName *getInitName(SgNode *l_expr);

  SgExpression *resolveCast(SgExpression *expr);
  bool isDoubleExactEntry(const multitype *multi, SgInitializedName *name,
                          SgNode *sgNode);
   ...
   *********************************************************/
  template <typename T> SgNode *getOtherInNode(T cfgNode, SgNode *oneNode) {
    SgNode *otherNode = NULL;
    std::vector<filteredCFGEdgeType> in_edges = cfgNode.inEdges();
    for (std::vector<filteredCFGEdgeType>::const_iterator i = in_edges.begin();
         i != in_edges.end(); ++i) {
    ...
    if (DEBUG_MODE_EXTRA)
      std::cout << "getOtherInNode:: other: " << otherNode
                << "  previous: " << oneNode
  ...
  template <typename T> void printCFGVector(std::vector<T> worklist) {
    int pos = 0;
    std::cout << "\n!! Worklist : ";
    for (typename std::vector<T>::iterator i = worklist.begin();
         i != worklist.end(); ++i, pos++) {
      T node = *i;
...
public:
  ...
  virtual FilteredCFGNode<IsDFAFilter> run(SgFunctionDefinition *function,
                                           bool &abortme) = 0;
};
