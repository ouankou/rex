...
void testOneFunction(std::string funcParamName, int argc, char *argv[],
                     bool debug, int nrOfNodes, multimap<string, int> results,
  ...
  for (; i != vars.end(); ++i) {
    ...
    if (maxNodes != nrOfNodes) {
      cout << " Error: Test should have " << nrOfNodes
    ...
    std::multimap<SgInitializedName *, SgNode *> map =
    ...
    if (map.size() > 0) {
      ...
      for (; j != map.end(); ++j) {
        ...
        for (; k != results.end(); ++k) {
          ...
          if (name == resName)
            if (debug)
              cout << " ... defNr: " << resNr << "  inTable: " << tableNr
                   << endl;
          if (name == resName && tableNr == resNr) {
            hit++;
            if (debug)
              cout << " Hit " << hit << "/" << results.size() << " - (" << name
                   << "," << resNr << ")" << endl;
          }
        }
      }
      ...
      if (results.size() != 0) {
        cout << " Error: Test node " << defuse->getIntForSgNode(func)
             << " should have a multimap. " << endl;
        exit(0);
      }
    }
