// tps (1/14/2010) : Switching from rose.h to sage3 changed size from 18,5 MB to
// 8,1MB
...
std::string getEdgeString(const EdgeT &e, DefUseAnalysis *dfa,
  ...
  if (live == NULL) {
    ...
    std::vector<std::pair<SgInitializedName *, SgNode *>> multi =
        dfa->getDefMultiMapFor(n);
    for (multitype::const_iterator j = multi.begin(); j != multi.end(); ++j) {
      ...
      string temp = "DEF: " + name + " ( " +
                    ToString(dfa->getIntForSgNode(sgInitMM)) + " )  - " +
...
inline void printDFANode(ostream &o, const NodeT &n, DefUseAnalysis *dfa) {
  ...
  if (isSgFunctionDefinition(n.getNode())) {
    funcName = isSgFunctionDefinition(n.getNode())
                   ->get_declaration()
                   ->get_qualified_name();
  }
  o << id << " [label=\"" << funcName << " : "
    << escapeStringDFA(getNodeString(n, dfa) + n.toStringForDebugging())
    << "\", color=\"" << nodeColor << "\", style=\""
    << (n.isInteresting() ? "solid" : "dotted") << "\"];\n";
}

template <typename EdgeT>
inline void printDFAEdge(ostream &o, const EdgeT &e, bool isInEdge,
  ...
  o << e.source().id() << " -> " << e.target().id() << " [label=\""
    << escapeString(getEdgeString(e, dfa, live)) << "\", color=\"" << nodeColor
...
void printDFANodePlusEdges(ostream &o, NodeT n, DefUseAnalysis *dfa,
  ...
  if (/* Debug */ isSgFunctionCallExp(n.getNode()) && n.getIndex() == 3) {
    ...
    printDFAEdge<EdgeT>(o,
                        CFGPath(CFGEdge(CFGNode(n.getNode(), 2), n.toNode())),
                        false, dfa, live);
  }
}

template <typename NodeT, typename EdgeT> class DfaToDotImpl {
  multimap<SgNode *, NodeT>
      exploredNodes; // One SgNode can have multiple CFG nodes of type NodeT.
...
void DfaToDotImpl<NodeT, EdgeT>::explore(NodeT n) {
  ...
  // search through the multimap to make sure CFGNode is not visited before, if
  // so,  return.
  pair<typename multimap<SgNode *, NodeT>::const_iterator,
       typename multimap<SgNode *, NodeT>::const_iterator>
      ip = exploredNodes.equal_range(n.getNode());
  for (typename multimap<SgNode *, NodeT>::const_iterator i = ip.first;
       i != ip.second; ++i) {
    if (i->second == n)
      return;
  }
  ...
  if (getenv("BUGGY_INTERPROCEDURAL_CFG") && isSgFunctionCallExp(n.getNode()) &&
      n.getIndex() == 2)
    explore(CFGNode(n.getNode(), 3));
}

template <typename NodeT, typename EdgeT>
void DfaToDotImpl<NodeT, EdgeT>::processNodes(SgNode *) {
  for (typename multimap<SgNode *, NodeT>::const_iterator it =
           exploredNodes.begin();
...
std::ostream &dfaToDot(std::ostream &o, std::string graphName,
                       std::vector<FilteredCFGNode<FilterFunction>> start,
                       DefUseAnalysis *dfa) {
  o << "digraph " << graphName << " {\n";
  DfaToDotImpl<FilteredCFGNode<FilterFunction>, FilteredCFGEdge<FilterFunction>>
      impl(o, dfa, NULL);
...
template ROSE_DLL_API std::ostream &
dfaToDot(std::ostream &, std::string, std::vector<FilteredCFGNode<IsDFAFilter>>,
         DefUseAnalysis *dfa);
template ROSE_DLL_API std::ostream &
dfaToDot(std::ostream &, std::string, std::vector<FilteredCFGNode<IsDFAFilter>>,
         DefUseAnalysis *dfa, LivenessAnalysis *live);
