 ...
 * persistent attribute. This is used to assign absolute names to VarRefExp
 * nodes during VariableRenaming.
 *
...
public:
   ...
   */
  VarUniqueName(const std::vector<SgInitializedName *> &prefix,
  ...
  VarUniqueName(const VarUniqueName &other)
      : AstAttribute(other), usesThis(false) {
...
struct IsDefUseFilter {
  ...
  bool operator()(VirtualCFG::CFGNode cfgn) const {
    ...
    // The begin edges of basic blocks are not considered interesting, but we
    // would like to keep them This is so we can propagate reachable defs to the
    // top of a basic block
    ...
    // We only want the middle appearance of the teritatry operator - after its
    // conditional expression and before the true and false bodies. This makes
    // it behave as an if statement for data flow purposes
 ...
 * program. For this class, we do not actually transform the AST directly,
 * rather we perform the analysis and add attributes to the AST nodes so that
 * later optimizations can access the results of this analysis while still
 * preserving the original AST.
 */
...
public:
   ...
   */
  typedef std::unordered_map<VarName, SgNode *, VectorHash<SgInitializedName *>>
      FirstDefTable;
   ...
   */
  typedef std::unordered_map<VarName, NodeNumRenameEntry,
                             VectorHash<SgInitializedName *>>
      NodeNumRenameTable;
   ...
   */
  typedef std::unordered_map<VarName, NumNodeRenameEntry,
                             VectorHash<SgInitializedName *>>
      NumNodeRenameTable;
...
private:
   ...
   * the VarDefUseTraversal. It is later used to populate the actual def/use
   * table.
   */
  ...
  /** This is the table that is populated with all the def information for all
   * the variables at all the nodes. It is populated during the runDefUse
   * function, and is done with the steady-state dataflow algorithm.
   */
  DefUseTable defTable;

  /** This is the table that is populated with all the use information for all
   * the variables at all the nodes. It is populated during the runDefUse
   * function, and is done with the steady-state dataflow algorithm.
   */
  ...
  /** This holds the mapping between variables and the nodes where they are
   * renumbered. Given a name and a node, we can get the number of the name that
   * is defined at that node. Nodes which do not define a name are not in the
   * table.
   */
  NodeNumRenameTable nodeRenameTable;

  /** This holds the mapping between variables and the nodes where they are
   * renumbered. Given a name and a number, we can get the node where that
   * number is defined. Nodes which do not define a name are not in the table.
   */
  NumNodeRenameTable numRenameTable;

public:
  VariableRenaming(SgProject *proj)
      : project(proj), DEBUG_MODE(SgProject::get_verbose() > 0),
        DEBUG_MODE_EXTRA(SgProject::get_verbose() > 1) {}
...
private:
   ...
   * defs at this node, and update this node's table if needed. If it locates a
   * def that is not in the table, it will attempt to find the location where
   * that member was first defined and insert a definition there. It will then
   * set the outParameter to indicate that it back-inserted a def.
   *
   * @param curNode The node to merge defs onto.
   * @param memberRefInserted Reference that indicates whether the function
   * back-inserted a definition.
   ...
   * propogated from previous nodes. If it locates a use of a def that did not
   * exist, it will attempt to locate the definition site of the base variable
   * and insert a def there. It will then set the outParameter to indicate that
   * it back-inserted a def.
   *
   ...
   */
  bool resolveUses(cfgNode curNode, bool *memberRefInserted,
                   NodeVec &changedNodes);

  /** Trace backwards in the cfg one step and return an aggregate of all
   * previous defs.
   *
  ...
  /** Expand all member definitions (chained names) to define every name in the
   * chain.
   *
  ...
  /** Insert defs for member uses (chained names) that do not have an explicit
   * def.
   *
   * When a member of a struct/class is used and that member does not have a
   * propogated def on the current node, this will find the closest definition
   * of a member in the ref chain and insert a definition for this use at that
   * member's def.
   *
   ...
   *     Obj o;         //Declare o of type o, Def for o.a, def for o.b, def for
   * o.b.x o.a = 5;       //Def for o.a, use for o, use for o.a, def for o.a.x
   *     int i = o.b.x; //Def for i, use for o.b.x
   *     int j = o.a.x; //Def for j, use for o.a.x
   *
   ...
   */
  bool insertExpandedDefsForUse(cfgNode curNode, VarName name,
                                NodeVec &changedNodes);

  /** Expand all member uses (chained names) to explicitly use every name in the
   * chain.
   *
...
public:
   ...
   *
   * This will return the number of the given variable as it is defined on the
   * given node. If the provided node does not define the variable, the function
   * will return -1.
   *
   ...
   * It will be the defining node for the variable renumbered with num of the
   * variable var. If the provided number does not exist for var, it will return
   * NULL.
   *
   * @param var The variable to get the defining node for.
   * @param num The renumbering of the defining node to get.
   * @return The defining node of var:num, or NULL if the renumbering does not
   * exist.
   */
   ...
   * @return A vector containing the usage nodes of the variable. Empty vector
   * otherwise.
   */
   ...
   *
   * This will retrieve a list of nodes of type T that use the specified var:num
   * combo. ex.  int i = s2.y1;  //Search for s:y1,AssignOp will yield the
   * AssignOp
   *
   * @param var The variable name to find.
   * @param num The revision of the variable to find.
  ...
  /** Get name:num mappings for all reaching definitions of all variables at the
   * node.
   *
  ...
  /** Get name:num mapping for all reaching definitions of the given variable at
   * the node.
   *
   * @param node The node to retrieve reaching definitions for.
   * @param var The variable to retrieve definitions of.
   * @return A table of (num, defNode) for the given variable. Empty table
   * otherwise.
   */
  NumNodeRenameEntry getReachingDefsAtNodeForName(SgNode *node,
                                                  const VarName &var);

  /** Get the final versions if all variables at the end of the given scope.
   *
   * @param bb The scope to get variables for.
   * @return A table of VarName->(num, defNode) for all variables at the end of
   * the scope. Empty table otherwise.
   */
   ...
   * @return A table of VarName->(num, defNode) for all variables at the end of
   * the function. Empty table otherwise.
   */
   ...
   */
  NumNodeRenameEntry
  getReachingDefsAtFunctionEndForName(SgFunctionDefinition *node,
                                      const VarName &var);

  /** Gets the versions of all variables reaching a statment before its
   * execution. Notice that this method and getReachingDefsAtNode potentially
   * return different values for loops. With loops, variable values from the
   * body of the loop flow to the top; hence getReachingDefsAtNode returns
   * definitions from the loop body. On the other hand,
   * getReachingDefsAtStatementStart does not return definitions coming in from
   * a loop body.
   *
   * @param statement
   * @return A table of VarName->(num, defNode) for all variables at the
   ...
   *
   * @param node The function to get variables for.
   * @return A table of VarName->(num, defNode) for all variables at the start
   * of the function. Empty table otherwise.
   */
   ...
   */
  NumNodeRenameEntry
  getReachingDefsAtFunctionStartForName(SgFunctionDefinition *node,
                                        const VarName &var);

  /** Get name:num mappings for all uses at this node. For example, if p.x
   * appears, there will be a use for both p and p.x
   *
   * @param node The node to get uses for.
   * @return A table mapping VarName->(num, defNode) for every varName used at
   * node. Empty table otherwise.
   */
  NumNodeRenameTable getUsesAtNode(SgNode *node);

  /** Get name:num mappings for the original uses at this node. For example, if
   * p.x appears, there will be a use for p.x, but not for p.
   *
   * @param node The node to get uses for.
   * @return A table mapping VarName->(num, defNode) for every varName used at
   * node. Empty table otherwise.
   ...
   * @return  A table of (num, defNode) for the given varName used at node.
   ...
   *
   * This will return the combination of original and expanded defs on this
   * node.
   *
   * ex. s.x = 5;  //This will return s.x and s  (s.x is original & s is
   * expanded)
   *
   * @param node The node to get defs for.
   * @return A table mapping VarName->(num, defNode) for every varName defined
   * at node. Empty table otherwise.
   */
  NumNodeRenameTable getDefsAtNode(SgNode *node);

  /** Get name:num mapping for def of the specified variable at the given node.
   *
   * This will return the combination of original and expanded defs of the given
   * variable on this node.
   *
   * ex. s.x = 5;   //(s is expanded & s.x is original)
   *     Looking for s will return this node, even though s is an expanded
   * definition.
   *
   * @param node The node to get defs for.
   ...
   *
   * @param node The node to get defs for.
   * @return A table mapping VarName->(num, defNode) for every varName
   * originally defined at node. Empty table otherwise.
   */
  NumNodeRenameTable getOriginalDefsAtNode(SgNode *node);

  /** Get name:num mapping for def of the specified variable at the given node.
   *
   * This will return the combination of original defs of the given variable on
   * this node.
   *
   * ex. s.x = 5;   //(s is expanded & s.x is original)
   ...
   * at node. Empty table otherwise.
   */
  NumNodeRenameEntry getOriginalDefsAtNodeForName(SgNode *node,
                                                  const VarName &var);
   ...
   * @param node The node to get defs for.
   * @return A table mapping VarName->(num, defNode) for every varName defined
   * via expansion at node. Empty table otherwise.
   */
  NumNodeRenameTable getExpandedDefsAtNode(SgNode *node);

  /** Get name:num mapping for def of the specified variable at the given node.
   *
   * This will return the combination of expanded defs of the given variable on
   * this node.
   *
   * ex. s.x = 5;   //(s is expanded & s.x is original)
   ...
   * at node. Empty table otherwise.
   */
  NumNodeRenameEntry getExpandedDefsAtNodeForName(SgNode *node,
                                                  const VarName &var);

  /** Get all definitions for the subtree rooted at this node. If m.x is
   * defined, the resulting table will also include a definition for m.
   *
  ...
  /** Get all original definitions for the subtree rooted at this node. No
   * expanded definitions will be included - for example, if m.x is defined,
   * there will be no definition for the structure m.
   *
   * @param node The root of the subtree to get definitions for.
   * @return The table mapping VarName->(num, node) for every definition.
   */
   ...
   * filename has "/include/" in it. If so, it will return true. Otherwise, it
   * returns false.
   *
   ...
   */
  static SgExpression *buildVariableReference(const VarName &var,
...
private:
  ...
  public:
     ...
     */
    VarRefSynthAttr(SgNode *thisNode) { refs.push_back(thisNode); }
  ...
  class UniqueNameTraversal
    ...
    /** Finds initialized names that are "fake" (refer to p_prev_decl_item in
     * the SgInitializedName docs) and replaces them with the true declaration.
     */
  ...
  public:
     ...
     *
     * This function will handle passing all variables that are referenced by a
     * given expression.
     *
  ...
  class ChildUses {
  private:
    /** An assignment to the current expression in the AST would define this
     * variable */
  ...
  /** This class collects all the defs and uses associated with each node in the
   * traversed CFG. Note that this does not compute reachability information; it
  ...
  public:
    ...
    /** Called to evaluate the synthesized attribute on every node.
     *
     * This function will handle passing all variables that are defined and used
