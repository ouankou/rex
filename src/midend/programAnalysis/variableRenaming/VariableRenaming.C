...
void VariableRenaming::printDefs(SgNode *node) {
  std::cout << "Def Table for [" << node->class_name() << ":" << node
            << "]:" << std::endl;

  for (TableEntry::value_type &entry : defTable[node]) {
    std::cout << "  Defs for [" << keyToString(entry.first)
              << "]:" << std::endl;

    for (NodeVec::value_type &iter : entry.second) {
      std::cout << "    -[" << iter->class_name() << ":" << iter << "]"
                << std::endl;
    }
  }
}

void VariableRenaming::printDefs(
    const std::map<std::vector<SgInitializedName *>, std::vector<SgNode *>>
        &table) {
...
void VariableRenaming::printOriginalDefs(SgNode *node) {
  std::cout << "Original Def Table for [" << node->class_name() << ":" << node
            << "]:" << std::endl;
...
void VariableRenaming::printOriginalDefTable() {
  ...
  for (DefUseTable::value_type &node : originalDefTable) {
    std::cout << "  Original Def Table for [" << node.first->class_name() << ":"
              << node.first << "]:" << std::endl;
...
void VariableRenaming::printUses(SgNode *node) {
  std::cout << "Use Table for [" << node->class_name() << ":" << node
            << "]:" << std::endl;

  for (TableEntry::value_type &entry : useTable[node]) {
    std::cout << "  Uses for [" << keyToString(entry.first)
              << "]:" << std::endl;
...
void VariableRenaming::printUses(
    const std::map<std::vector<SgInitializedName *>, std::vector<SgNode *>>
        &table) {
...
void VariableRenaming::printRenameTable() { printRenameTable(numRenameTable); }
...
void VariableRenaming::printRenameTable(const NumNodeRenameTable &table) {
  ...
  for (const NumNodeRenameTable::value_type &entry : table) {
    std::cout << "  Names for [" << keyToString(entry.first)
              << "]:" << std::endl;
...
VarUniqueName *VariableRenaming::getUniqueName(SgNode *node) {
  ...
  VarUniqueName *uName = dynamic_cast<VarUniqueName *>(
      node->getAttribute(VariableRenaming::varKeyTag));
  return uName;
}
...
bool VariableRenaming::isFromLibrary(SgFunctionDeclaration *node) {
  ...
  // cout << "Filename string '" << filename << "' for " << node->class_name()
  // << node << endl;
...
void VariableRenaming::run() {
  ...
  UniqueNameTraversal uniqueTrav(this,
                                 SageInterface::querySubTree<SgInitializedName>(
                                     project, V_SgInitializedName));
  std::vector<SgFunctionDefinition *> funcs =
  ...
  for (; iter != funcs.end(); ++iter) {
    ...
    if (!isFromLibrary(func)) {
      if (DEBUG_MODE)
        cout << "Running UniqueNameTrav on " << func->get_name().getString()
             << func << endl;
  ...
  for (iter = funcs.begin(); iter != funcs.end(); ++iter) {
    ...
    if (!isFromLibrary(func)) {
      if (DEBUG_MODE)
        cout << "Running defUseTrav on function: "
             << func->get_name().getString() << func << endl;
  ...
  for (iter = funcs.begin(); iter != funcs.end(); ++iter) {
    ...
    if (!isFromLibrary(func->get_declaration())) {
      ...
      if (DEBUG_MODE)
        cout << "Running DefUse on function: "
             << func->get_declaration()->get_name().getString() << func << endl;
      runDefUse(func);
    }
  }
}
...
void VariableRenaming::insertDefsForExternalVariables(
  ...
  TableEntry &originalVarsAtFunctionEntry =
      originalDefTable[function->get_definition()];
  TableEntry &expandedVarsAtFunctionEntry =
      expandedDefTable[function->get_definition()];

  // Iterate over each used variable and check it it is declared outside of the
  // function scope
  ...
  for (const VarName &usedVar : usedNames) {
    ...
    if (varScope == functionScope ||
      ...
      // function so we don't need to insert one
      if (SageInterface::isAncestor(function->get_definition(), rootName[0])) {
        ...
        if (SgVariableDeclaration *varDecl =
                isSgVariableDeclaration(rootName[0]->get_parent())) {
          ASSERT_require(varDecl->get_declarationModifier()
                             .get_storageModifier()
                             .isExtern());
          continue;
        } else if (isSgFunctionParameterList(rootName[0]->get_parent())) {
          // x can also have global scope when it appears as a formal parameter
          // in an extern function declaration, as in:  void f() { void g(int
          // x); ... }
#ifndef NDEBUG
          SgFunctionDeclaration *fdecl =
              isSgFunctionDeclaration(rootName[0]->get_parent()->get_parent());
          assert(fdecl != NULL);
    ...
    // The variable is not in local scope; we need to insert a def for it at the
    // function definition
    for (size_t i = 0; i < usedVar.size(); i++) {
      ...
      if (originalVarsAtFunctionEntry[newName].empty()) {
        originalVarsAtFunctionEntry[newName].push_back(
...
set<VariableRenaming::VarName>
VariableRenaming::getVarsUsedInSubtree(SgNode *root) {
  ...
  public:
    ...
    void visit(SgNode *node) {
      ...
      if (defEntry != ssa->originalDefTable.end()) {
        ...
        for (TableEntry::value_type &varDefsPair :
             ssa->originalDefTable[node]) {
          usedNames.insert(varDefsPair.first);
        }
      }
    }
...
void VariableRenaming::toDOT(const std::string fileName) {
  ...
  fileVec files =
      SageInterface::querySubTree<SgSourceFile>(project, V_SgSourceFile);

  // print all the functions in each file

  for (fileVec::value_type &file : files) {
    ofstream outFile(
        (StringUtility::stripPathFromFileName(file->getFileName()) + "_" +
         fileName)
            .c_str());
...
void VariableRenaming::toFilteredDOT(const std::string fileName) {
  ...
  for (fileVec::value_type &file : files) {
    ofstream outFile(
        (StringUtility::stripPathFromFileName(file->getFileName()) +
         "_filtered_" + fileName)
            .c_str());
...
void VariableRenaming::printToDOT(SgSourceFile *source,
  ...
  funcDefVec funcs = SageInterface::querySubTree<SgFunctionDefinition>(
      source, V_SgFunctionDefinition);
  ...
  for (funcDefVec::value_type &func : funcs) {
    ...
    while (!traverse.empty()) {
      ...
      if (std::count(visited.begin(), visited.end(), current) == 0) {
        ...
        bool uniqueName =
            current.getNode()->attributeExists(VariableRenaming::varKeyTag);
        ...
        // Print this node
        outFile << id << " [label=\"<"
                << escapeString(current.getNode()->class_name()) << ">:"
                << current.getNode()
...
void VariableRenaming::printToFilteredDOT(SgSourceFile *source,
                                          std::ofstream &outFile) {
  if (!outFile.good()) {
    if (DEBUG_MODE)
      cout << "Error: Bad ofstream in printToDOT()" << endl;
...
VariableRenaming::UniqueNameTraversal::resolveTemporaryInitNames(
  ...
  // Initialized names are children of varRefs when names are used before they
  // are declared (possible in class definitions)
  ...
  // Class variables defined in constructor pre-initializer lists have an extra
  // initialized name there (so there can be an assign
  //  initializer). Track down the real initialized name corresponding to the
  //  declaration of the variable inside a class scope
...
VariableRenaming::UniqueNameTraversal::evaluateSynthesizedAttribute(
    SgNode *node, SynthesizedAttributesList attrs) {
  ...
  if (isSgInitializedName(node)) {
    SgInitializedName *name =
        resolveTemporaryInitNames(isSgInitializedName(node));

    // We want to assign this node its unique name, as well as adding it to the
    // defs.
  ...
  // Next, see if it is a varRef
    ...
    VarUniqueName *uName = new VarUniqueName(
        resolveTemporaryInitNames(var->get_symbol()->get_declaration()));
    var->setAttribute(VariableRenaming::varKeyTag, uName);

    return VariableRenaming::VarRefSynthAttr(var);
  }
  // We check if it is a 'this' expression, since we want to be able to version
  // 'this' as well. We don't have an SgInitializedName for 'this', so we use a
  // flag in the unique names
  ...
  // Now we check if we have reached a Dot Expression, where we have to merge
  // names.
    ...
    if (attrs[0].getRefs().size() > 0) {
      ...
      // Get the unique name from the highest varRef in the LHS, since this will
      // have the most fully qualified UniqueName.
      VarUniqueName *lhsName;
      if (!thisExp) {
        lhsName = dynamic_cast<VarUniqueName *>(
            attrs[0].getRefs()[attrs[0].getRefs().size() - 1]->getAttribute(
                VariableRenaming::varKeyTag));
        ROSE_ASSERT(lhsName);
      }
      ...
      if (attrs[1].getRefs().size() == 1) {
        ...
        if (varRef) {
          if (!thisExp) {
            // Create the uniqueName from the uniqueName of the lhs prepended to
            // the rhs uniqueName
            VarUniqueName *uName =
                new VarUniqueName(lhsName->getKey(),
            ...
            // Create the UniqueName from the current varRef, and stores that it
            // uses 'this'
            VarUniqueName *uName = new VarUniqueName(resolveTemporaryInitNames(
                varRef->get_symbol()->get_declaration()));
      ...
      // The LHS has no varRefs, so there can be no explicit naming of varRefs
      // above this point.
  ...
  // Now we hit the default case. We should return a merged list.
    ...
    for (unsigned int i = 0; i < attrs.size(); i++) {
      names.insert(names.end(), attrs[i].getRefs().begin(),
                   attrs[i].getRefs().end());
    }
...
void VariableRenaming::runDefUse(SgFunctionDefinition *func) {
  ...
  while (!worklist.empty()) {
    ...
    // We don't want to do def_use on the ending CFGNode of the function
    // definition so if we see it, continue. If we do this, then incorrect
    // information will be propogated to the beginning of the function
    ...
    // If memberRefs were inserted, then there are nodes previous to this one
    // that are different. Thus, we need to add those nodes to the working list
    if (memberRefInserted) {
      ...
      for (SgNode *chNode : changedNodes) {
        ...
        if (DEBUG_MODE)
          cout << "Member Ref Inserted: Added "
               << nextNode.getNode()->class_name() << nextNode.getNode()
    ...
    // For every edge, add it to the worklist if it is not seen or something has
    // changed
    ...
    for (auto it = outEdges.rbegin(); it != outEdges.rend(); ++it) {
      ...
      // Only insert the node in the worklist if it isn't there already.
      if (std::find(worklist.begin(), worklist.end(), nextNode) ==
          worklist.end()) {
        if (changed) {
          // Add the node to the worklist
          worklist.push_back(nextNode);
          if (DEBUG_MODE)
            cout << "Defs Changed: Added " << nextNode.getNode()->class_name()
                 << ": Line " << nextNode.getNode()->get_file_info()->get_line()
                 << ", " << nextNode.getNode() << " to the worklist." << endl;
        }
        // If the next node has not yet been visited
          ...
          worklist.push_back(nextNode);
          if (DEBUG_MODE)
            cout << "Next unvisited: Added " << nextNode.getNode()->class_name()
                 << ": Line " << nextNode.getNode()->get_file_info()->get_line()
                 << ", " << nextNode.getNode() << " to the worklist." << endl;
        }
      }
...
bool VariableRenaming::defUse(FilteredCFGNode<IsDefUseFilter> node,
                              bool *memberRefInserted, NodeVec &changedNodes) {
  SgNode *current = node.getNode();

  // Handle each type of node
  if (DEBUG_MODE)
    cout << "Performing DefUse on " << current->class_name() << ": Line "
         << current->get_file_info()->get_line() << ", " << current << endl;
...
bool VariableRenaming::mergeDefs(cfgNode curNode, bool *memberRefInserted,
   ...
   *        1. Copy the definitions from the previous node wholesale to a
   * staging table.
   *        2. Copy in the original definitions from the current node,
   * overwriting those from the previous node.
   *        3. Compare the staging and current tables, and only overwrite if
   * needed.
   */

  /*2+ Edges: When we have branched control flow, we do the following:
   *       1. Copy the definitions from the previous node(s) wholesale to a
   * staging table. Be careful to not insert duplicates.
   *       2. Copy in the original definitions from the current node,
   * overwriting those from the previous node(s).
   *       3. Compare the staging and current tables, and only overwrite if
   * needed.
   */
  ...
  if (!isSgFunctionDefinition(node) &&
      (!isSgInitializedName(node) ||
       isSgCtorInitializerList(node->get_parent()))) {

    for (TableEntry::value_type &entry : originalDefTable[node]) {
      if (propDefs.find(entry.first) == propDefs.end()) {
        *memberRefInserted =
            insertExpandedDefsForUse(curNode, entry.first, changedNodes) ||
            *memberRefInserted;
        if (*memberRefInserted)
          break;
      }
  ...
  for (TableEntry::value_type &entry : originalDefTable[node]) {
    ...
    for (TableEntry::value_type &propEntry : propDefs) {
      ...
      // If the original def is a prefix of the propagated def, add a def at
      // this node Compare sizes to guard against inserting original def in
      // expanded table
      if (isPrefixOfName(propEntry.first, entry.first) &&
          (propEntry.first.size() > entry.first.size())) {
        ...
        if (DEBUG_MODE_EXTRA) {
          cout << "Inserted expandedDef for [" << keyToString(propEntry.first)
               << "] with originalDef prefix [" << keyToString(entry.first)
               << "]" << endl;
        }
      }
    }
  ...
  // If there is an initial definition of a name at this node, we should insert
  // it in the table
...
void VariableRenaming::aggregatePreviousDefs(cfgNode curNode,
  ...
  if (inEdges.size() == 1) {
    ...
    if(DEBUG_MODE)
    {
        cout << "Merging defs from " << prev->class_name() << prev << " to " <<
    node->class_name() << node << endl; printDefs(prev);
    }*/
    ...
    for (unsigned int i = 0; i < inEdges.size(); i++) {
      ...
      for (TableEntry::value_type &entry : defTable[prev]) {
        ...
        results[entry.first].insert(results[entry.first].end(),
  ...
  for (TableEntry::value_type &entry : results) {
    ...
    entry.second.resize(std::unique(entry.second.begin(), entry.second.end()) -
                        entry.second.begin());
  }
...
bool VariableRenaming::expandMemberDefinitions(cfgNode curNode) {
  ...
  for (TableEntry::value_type &entry : originalDefTable[node]) {
    ...
    // Start at one so we don't get the same defs in the original and expanded
    // defs
    for (unsigned int i = 1; i < entry.first.size(); i++) {
      ...
      if (DEBUG_MODE_EXTRA) {
        cout << "Testing for presence of [" << keyToString(newName) << "]"
             << endl;
      }
      ...
      if (originalDefTable[node].count(newName) == 0 &&
        ...
        if (DEBUG_MODE_EXTRA) {
          cout << "Inserted new name [" << keyToString(newName)
               << "] into defs." << endl;
        }
      }
    }
...
bool VariableRenaming::resolveUses(FilteredCFGNode<IsDefUseFilter> curNode,
  ...
  for (TableEntry::value_type &entry : useTable[node]) {
    // Check the defs that are active at the current node to find the reaching
    // definition We want to check if there is a definition entry for this use
    // at the current node
    if (defTable[node].find(entry.first) != defTable[node].end()) {
      // There is a definition entry. Now we want to see if the use is already
      // up to date
      ...
      // If there are no defs for this use at this node, then we have a
      // multi-part name that has not been expanded. Thus, we want to expand it.
      *memberRefInserted =
          insertExpandedDefsForUse(curNode, entry.first, changedNodes);
      if (*memberRefInserted)
        break;
  ...
  for (TableEntry::value_type &entry : useTable[node]) {
    ...
    if (originalDefTable[node].count(entry.first) != 0 ||
        expandedDefTable[node].count(entry.first) != 0) {
...
bool VariableRenaming::insertExpandedDefsForUse(cfgNode curNode, VarName name,
                                                NodeVec &changedNodes) {
  SgNode *node = curNode.getNode();
  ...
  if (DEBUG_MODE_EXTRA) {
    cout << "Checking for needed extra defs for uses at " << node->class_name()
         << node << endl;
  ...
  // No def for this name at this node, so we need to insert a def at the
  // location where the first part of this name was defined. eg. s.a.b = x;
  // (insert definition of s.a & s.a.b where s is first defined.)
  ...
  if (firstDefList.count(rootName) == 0) {
    ...
    // Check if the variable is declared in a class scope
    else if (isSgClassDefinition(SageInterface::getScope(rootName[0])) !=
             NULL) {
      // It is declared in class scope.
      // Get our enclosing function definition to insert the first definition
      // into.
      ...
      if (func == NULL) {
        // In this case, we're not inside the function defintion; we're inside
        // the parameter list
        SgFunctionDeclaration *declaration =
            SageInterface::getEnclosingNode<SgFunctionDeclaration>(node);
    ...
    // Otherwise, see if it is in namespace scope
    else if (isSgNamespaceDefinitionStatement(
                 SageInterface::getScope(rootName[0])) != NULL) {
      ...
      cout << "Error: Found variable with no firstDef point that is not a "
              "class or namespace member."
           << endl;
      cout << "Variable Scope: "
           << SageInterface::getScope(rootName[0])->class_name()
           << SageInterface::getScope(rootName[0]) << endl;
      cout << rootName[0]->get_name().str() << rootName[0] << "@"
           << rootName[0]->get_file_info()->get_line() << ":"
           << rootName[0]->get_file_info()->get_col() << endl;
      ROSE_ABORT();
    }
  }
  ...
  for (int i = 0; i < (signed int)name.size(); i++) {
    ...
    if (DEBUG_MODE_EXTRA) {
      cout << "Testing for def of [" << keyToString(newName)
           << "] at var initial def." << endl;
    }

    if (originalDefTable[firstDef].count(newName) == 0 &&
        expandedDefTable[firstDef].count(newName) == 0) {
  ...
  if (DEBUG_MODE_EXTRA)
    cout << "Finished inserting references. Changed: "
         << ((changed) ? "true" : "false") << endl;
...
int VariableRenaming::addRenameNumberForNode(const VarName &var, SgNode *node) {
  ...
  if (DEBUG_MODE)
    cout << "Renaming Added:[" << keyToString(var) << "]:" << nextNum << " - "
         << node << endl;

  return nextNum;
}

VariableRenaming::NodeVec VariableRenaming::getAllUsesForDef(const VarName &var,
                                                             int num) {
  NodeVec res;
  ...
  for (DefUseTable::value_type &entry : useTable) {
    ...
    if (entry.second.count(var) != 0) {
      ...
      if (std::find(entry.second[var].begin(), entry.second[var].end(),
                    defNode) != entry.second[var].end()) {
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getReachingDefsAtNode(SgNode *node) {
  // We want to get all the reaching defs at this node and insert them into the
  // result table
  ...
  for (TableEntry::value_type &entry : defTable[node]) {
    ...
    for (NodeVec::value_type &defEntry : entry.second) {
      ...
      if (renameNum > 0) {
        ...
        cout << "Error: Found propogated def with no entry in rename table."
             << endl;
        ROSE_ABORT();
      }
    }
...
VariableRenaming::NumNodeRenameEntry
VariableRenaming::getReachingDefsAtNodeForName(SgNode *node,
                                               const VarName &var) {
  // We want to get all the reaching defs at this node and insert them into the
  // result table
  ...
  for (TableEntry::value_type &entry : defTable[node]) {
    ...
    for (NodeVec::value_type &defEntry : entry.second) {
      ...
      if (renameNum > 0) {
        ...
        cout << "Error: Found propagated def with no entry in rename table."
             << endl;
        ROSE_ABORT();
      }
    }
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getUsesAtNode(SgNode *node) {
  // We want to get all the uses at this node and insert them into the result
  // table
  ...
  for (TableEntry::value_type &entry : useTable[node]) {
    ...
    for (NodeVec::value_type &defEntry : entry.second) {
      ...
      if (renameNum > 0) {
        ...
        /* This situation can happen in certain cases, so we don;t want to
         * assert.
         *
...
VariableRenaming::NumNodeRenameEntry
VariableRenaming::getUsesAtNodeForName(SgNode *node, const VarName &var) {
  // We want to get all the uses at this node and insert them into the result
  // table
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getOriginalUsesAtNode(SgNode *node) {
  // The original variables are always attached to higher levels in the AST. For
  // example, if we have p.x, the dot expression has the varname p.x attached to
  // it, while its left child has the varname p. Hence, if we get the top nodes
  // in the AST that are variables, we'll have all the original (not exanded)
  ...
  public:
    ...
    virtual bool evaluateInheritedAttribute(SgNode *node,
                                            bool isParentVariable) {
      if (isParentVariable) {
        return true;
      }
  ...
  for (VariableRenaming::VarName varName :
       originalVariableUsesTraversal.originalVariablesUsed) {
    ROSE_ASSERT(result.count(varName) == 0);
    VariableRenaming::NumNodeRenameEntry varDefs =
        getUsesAtNodeForName(node, varName);
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getDefsAtNode(SgNode *node) {
    ...
    // Or merge it with an existing one
      ...
      for (NumNodeRenameEntry::value_type &tableEntry : entry.second) {
        ...
        if (original[entry.first].count(tableEntry.first) == 0) {
          ...
          cout << "Error: Same renaming present in original and expanded defs."
               << endl;
...
VariableRenaming::NumNodeRenameEntry
VariableRenaming::getDefsAtNodeForName(SgNode *node, const VarName &var) {
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getOriginalDefsAtNode(SgNode *node) {
  // We want to get all the original defs at this node and insert them into the
  // result table
    ...
    for (NodeVec::value_type &defEntry : entry.second) {
      ...
      if (renameNum > 0) {
        ...
        if (res[entry.first].count(renameNum) == 0) {
          ...
          cout << "Error: Same def originally defined twice on same node."
               << endl;
          ROSE_ABORT();
        }
      } else {
         ...
         *     }
         *
         * The i++ will have an original def for i. However, it will not be in
         * the rename table for i. This is not technically wrong, since control
         * will never reach that i++, so it will not have an ordering wrt. the
         * other definitions.
         */
        if (DEBUG_MODE) {
          cout << "Warning: Found original def with no entry in rename table."
               << endl;
        }
      }
    }
...
VariableRenaming::NumNodeRenameEntry
VariableRenaming::getOriginalDefsAtNodeForName(SgNode *node,
                                               const VarName &var) {
  // We want to get all the original defs at this node and insert them into the
  // result table
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getExpandedDefsAtNode(SgNode *node) {
  // We want to get all the expanded defs at this node and insert them into the
  // result table
  ...
  for (TableEntry::value_type &entry : expandedDefTable[node]) {
    ...
    for (NodeVec::value_type &defEntry : entry.second) {
      ...
      if (renameNum > 0) {
        ...
        cout << "Error: Found expanded def with no entry in rename table."
             << endl;
        ROSE_ABORT();
      }
    }
...
VariableRenaming::NumNodeRenameEntry
VariableRenaming::getExpandedDefsAtNodeForName(SgNode *node,
                                               const VarName &var) {
  // We want to get all the uses at this node and insert them into the result
  // table
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getDefsForSubtree(SgNode *node) {
  ...
  public:
    ...
    virtual void visit(SgNode *node) {
      ...
      VariableRenaming::NumNodeRenameTable defsAtNode =
          varRenamingAnalysis->getDefsAtNode(node);

      // Traverse the defs

      for (VariableRenaming::NumNodeRenameTable::value_type &entry :
           defsAtNode) {
        ...
        for (VariableRenaming::NumNodeRenameEntry::value_type &tableEntry :
             entry.second) {
          if (result[entry.first].count(tableEntry.first) == 0) {
            result[entry.first][tableEntry.first] = tableEntry.second;
          } else {
            cout << "Error: Same rename number defined on multiple nodes."
                 << endl;
            ROSE_ABORT();
          }
        }
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getOriginalDefsForSubtree(SgNode *node) {
  ...
  public:
    ...
    virtual void visit(SgNode *node) {
      // Look up defs at this particular node
      VariableRenaming::NumNodeRenameTable defsAtNode =
          varRenamingAnalysis->getOriginalDefsAtNode(node);
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getReachingDefsAtScopeEnd(SgScopeStatement *bb) {
  ...
  while (!worklist.empty()) {
    ...
    for (cfgEdge edge : outEdges) {
      ...
      if (!SageInterface::isAncestor(bb, targetNode)) {
        // This edge leads outside the basic block! Gotta save the variable
        // versions here
          ...
          // Or merge it with an existing one
            ...
            for (NumNodeRenameEntry::value_type &tableEntry : entry.second) {
              ...
              if (result[entry.first].count(tableEntry.first) == 0) {
                ...
                if (result[entry.first][tableEntry.first] !=
                    tableEntry.second) {
                  cout << "Error: Same Renaming number has two different "
                          "definition points."
                       << endl;
                  ROSE_ABORT();
                }
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getReachingDefsAtFunctionEnd(SgFunctionDefinition *node) {
  ROSE_ASSERT(node);
  NumNodeRenameTable result;
  ...
  // DQ (12/10/2016): Added return statement, but because this was not
  // specified, we want to make it an error to preserve the previous semantics.
...
VariableRenaming::getReachingDefsAtFunctionEndForName(
    SgFunctionDefinition *node, const VarName &var) {
  ROSE_ASSERT(node);
  ...
  if (lastEdges.size() == 0) {
    ...
    for (cfgEdgeVec::value_type &edge : lastEdges) {
      NumNodeRenameEntry temp =
          getReachingDefsAtNodeForName(edge.source().getNode(), var);
      ...
      for (NumNodeRenameEntry::value_type &tableEntry : temp) {
        ...
        if (result.count(tableEntry.first) == 0) {
          ...
          if (result[tableEntry.first] != tableEntry.second) {
            cout << "Error: Same Renaming number has two different definition "
                    "points."
                 << endl;
            ROSE_ABORT();
          }
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getReachingDefsAtStatementStart(SgStatement *statement) {
  ROSE_ASSERT(statement);
  NumNodeRenameTable result;

  // Get all the incoming edges for the statement. These are UNFILTERED nodes,
  // so we get the true beginning of the statement
  vector<CFGEdge> inEdges = statement->cfgForBeginning().inEdges();

  // Trace back from an unfiltered node to all the filtered nodes immediately
  // leading to it
  ...
  // Merge in the defs from the reaching CFG nodes that are not children of the
  // current statement

  for (CFGNode cfgNode : filteredNodes) {
    SgNode *sourceNode = cfgNode.getNode();
    if (statement == sourceNode ||
...
VariableRenaming::NumNodeRenameTable
VariableRenaming::getReachingDefsAtFunctionStart(SgFunctionDefinition *node) {
...
SgExpression *
VariableRenaming::buildVariableReference(const VarName &var,
...
VariableRenaming::DefsAndUsesTraversal::evaluateSynthesizedAttribute(
    SgNode *node, SynthesizedAttributesList attrs) {
  if (ssa->getDebug()) {
    cout << "---------<" << node->class_name() << node << ">-------" << node
         << endl;
  }

  // We want to propagate the def/use information up from the varRefs to the
  // higher expressions.
  if (SgInitializedName *initName = isSgInitializedName(node)) {
    VarUniqueName *uName = VariableRenaming::getUniqueName(node);
    ROSE_ASSERT(uName);

    // Add this as a def, unless this initialized name is a preinitialization of
    // a parent class. For example, in the base of B : A(3) { ... } where A is a
    // superclass of B, an initialized name for A appears. Clearly, the
    // initialized name for the parent class is not a real variable
    if (initName->get_preinitialization() !=
            SgInitializedName::e_virtual_base_class &&
        initName->get_preinitialization() !=
  ...
  // Catch all variable references
    ...
    // In some cases, a varRef isn't actually part of a variable name. For
    // example, foo().x where foo returns a structure. x is an SgVarRefExp, but
    // is not part of a variable name.
    if (uName == NULL) {
      return ChildUses();
    }

    // Add this as a use. If it's not a use (e.g. target of an assignment),
    // we'll fix it up later.
    ssa->getUseTable()[node][uName->getKey()].push_back(node);

    if (ssa->getDebug()) {
      cout << "Found use for " << uName->getNameString() << " at "
           << node->cfgForBeginning().toStringForDebugging() << endl;
    }
  ...
  // Catch all types of Binary Operations
    ...
    ROSE_ASSERT(attrs.size() == 2 &&
                "Error: BinaryOp without exactly 2 children.");
    ChildUses &lhs = attrs[0];
    ChildUses &rhs = attrs[1];

    // If we have an assigning operation, we want to list everything on the LHS
    // as being defined Otherwise, everything is being used.
    vector<SgNode *> uses;
    ...
    case V_SgAssignOp: {
      ...
      if (currentVar != NULL) {
        vector<SgNode *>::iterator currVarUse =
            find(uses.begin(), uses.end(), currentVar);

        // An assign op doesn't use the var it's defining. So, remove that var
        // from the uses
        if (isSgAssignOp(binaryOp)) {
          ...
          // Also remove the use from the varRef node, because it's not really a
          // use.
          ssa->getUseTable()[currentVar].clear();
        }
        // All the other ops always use the var they're defining (+=, -=, /=,
        // etc)
        else {
          if (currVarUse == uses.end()) {
    ...
    default: {
      ...
      // Propagate the current variable up. The rhs variable is the one that
      // could be potentially defined up the tree
  ...
  // Catch all unary operations here.
    ...
    if (isSgAddressOfOp(unaryOp) &&
        isSgPointerMemberType(unaryOp->get_type())) {
      // SgAddressOfOp is special; it's not always a use of its operand. When
      // creating a reference to a member variable, we create reference without
      // providing a variable instance. For example,
      //               struct foo { int bar; };
      //
      //               void test()
      //               {
      //                       int foo::*v = &foo::bar;  <---- There is no use
      //                       of foo.bar on this line foo b; b.*v = 3;
      //               }
      // In this case, there are no uses in the operand. We also want to delete
      // any uses for the children
      vector<SgNode *> successors =
          SageInterface::querySubTree<SgNode>(unaryOp);

      for (SgNode *successor : successors) {
        ssa->getUseTable()[successor].clear();
      }
    } else {
      // Guard agains unary ops that have no children (exception rethrow
      // statement)
      if (attrs.size() > 0) {
        uses.insert(uses.end(), attrs[0].getUses().begin(),
                    attrs[0].getUses().end());
      }
    }

    // For these two definition operations, we want to insert a def for the
    // operand
    SgVarRefExp *currentVar = NULL;
    if (isSgMinusMinusOp(unaryOp) || isSgPlusPlusOp(unaryOp)) {
      currentVar = attrs[0].getCurrentVar();

      // The defs can be empty. For example, foo()++ where foo returns a
      // reference
      if (currentVar != NULL) {
    ...
    // Some other ops also preserve the current var. We don't really distinguish
    // between the pointer variable and the value to which it points
    else if (isSgCastExp(unaryOp) || isSgPointerDerefExp(unaryOp) ||
    ...
    // For the default case, we merge the uses of every attribute and pass them
    // upwards
    std::vector<SgNode *> uses;
    for (unsigned int i = 0; i < attrs.size(); i++) {
      if (ssa->getDebug()) {
      ...
      uses.insert(uses.end(), attrs[i].getUses().begin(),
                  attrs[i].getUses().end());
    }
...
void VariableRenaming::DefsAndUsesTraversal::addUsesToNode(
...
void VariableRenaming::DefsAndUsesTraversal::addDefForVarAtNode(
    SgVarRefExp *currentVar, SgNode *defNode) {
  const VariableRenaming::VarName &varName =
      VariableRenaming::getVarName(currentVar);
  ...
  if (ssa->getDebug()) {
    cout << "Found def for " << VariableRenaming::keyToString(varName) << " at "
         << defNode->cfgForBeginning().toStringForDebugging() << endl;
  }
}
