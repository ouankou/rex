...
void TemplatedDominatorTree<CFGFilterFunction>::writeDot(char *filename) {
  ...
  for (int i = 0; i < dtSize; i++) {
    f << "\"" << i << "\" [label = \"ID " << i << "\\n"
      << escapeString(getCFGNodeFromID(i).toString()) << "\"];" << std::endl;
    if (i > 0)
...
template <typename CFGFilterFunction>
TemplatedDominatorTree<CFGFilterFunction>::TemplatedDominatorTree(SgNode *head,
                                                                  Direction d)
    : DominatorForwardBackwardWrapperClass<CFGFilterFunction>(d),
      cfgRoot((d == PRE_DOMINATOR) ? (head->cfgForBeginning())
                                   : (head->cfgForEnd())) //,treeDirection(d)
{
  //        treeDirection = d;
  init();
  depthFirstSearch();
...
template <typename CFGFilterFunction>
void TemplatedDominatorTree<CFGFilterFunction>::depthFirstSearch() {
  ...
  while (workList.size() > 0) {
    ...
    workList.pop_back();
    parent = parentWorkList.back();
    parentWorkList.pop_back();
#ifdef DEBUG
    std::cout << "DFS: processing node <" << current.toString() << ">"
              << nodeCount << std::endl;
#endif
    // if the node is not in the map, it has not been processed
    if (nodeToIdMap.count(current) == 0) {
#ifdef DEBUG
      std::cout << "\tThis node has not been process, assigning ID "
                << nodeCount << std::endl;
#endif
  ...
  //      typename std::map < VirtualCFG::FilteredCFGNode < CFGFilterFunction >,
  //      int>::iterator mapTest;
  typename std::map<VirtualCFG::FilteredCFGNode<CFGFilterFunction>,
                    int>::iterator i;
  //, std::less < VirtualCFG::FilteredCFGNode < CFGFilterFunction > >
  //>::iterator i;
...
//! the preformance of the DT-algorithm can be improved by implementing a
//! tree-averaging/redistributing algorithm in eval or link, usually it is done
//! in eval
template <typename CFGFilterFunction>
void TemplatedDominatorTree<CFGFilterFunction>::link(int ancest, int current) {
  ancestor[current] = ancest;
}

template <typename CFGFilterFunction>
int TemplatedDominatorTree<CFGFilterFunction>::eval(int node) {
  int cmp = node;
...
template <typename CFGFilterFunction>
void TemplatedDominatorTree<CFGFilterFunction>::init() {
  semi.clear();
  idom.clear();
  ancestor.clear();
  dfsParent.clear();
...
template <typename CFGFilterFunction>
void TemplatedDominatorTree<CFGFilterFunction>::calculateImmediateDominators() {
  ...
  for (int i = idToNode.size() - 1; i > 0; i--) {
    ...
    for (unsigned int j = 0; j < inEdges.size(); j++) {
      ...
      std::cout << "u=EVAL(" << nodeID << ")=" << possiblySmallestSemiDom
                << std::endl;
      std::cout << "semi(" << possiblySmallestSemiDom
                << ")=" << semi[possiblySmallestSemiDom] << " < semi(" << i
                << ")=" << semi[i];
    ...
    // semidomniator
    std::cout << "add " << i << " to bucket semi[" << i << "]" << semi[i]
              << std::endl;
