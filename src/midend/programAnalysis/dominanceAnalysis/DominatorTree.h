...
/* ! \class TemplatedDominatorTree
         This class constructs either a dominator or a post-dominator tree for a
   control-flow-graph.  */
...
//! This class provides a uniform view of the CFG dissregarding the direction of
//! the CFG-traversal (forward/backward)
...
public:
  ...
  Direction getDirection() { return treeDirection; }

protected:
  //! helperfunctions to toggle beween pre and post dominator tree
  std::vector<VirtualCFG::FilteredCFGEdge<CFGFilterFunction>>
  getDirectionModifiedOutEdges(
      VirtualCFG::FilteredCFGNode<CFGFilterFunction> current) {
  ...
  // ! treeDirection stores the traversal direction and indicates construction
  // of a dominator or post-dominator tree
  Direction treeDirection;
};

// CI (01/23/2007): Implemented the DT for the VirtualCFG interface with
// the Lingauer-Tarjan algorithm
//! TemplatedDominatorTree constructs a dominator/postdominator tree for a cfg.
//! For the template parameter any cfg following Jeremias interface may be used
template <typename CFGFilterFunction>
class TemplatedDominatorTree
...
public:
  ...
  TemplatedDominatorTree(SgNode *head,
  ...
  /*
                          //! returns true if b is dominated by a (a is on the
     path form b to the root) int isDomintated(int a, int b)
                          {
  ...
  */
  ...
  int getImDomID(int i) { return idom[i]; }
  ...
  bool dominates(VirtualCFG::FilteredCFGNode<CFGFilterFunction> a,
...
private:
  ...
  void printInfo() {
    ...
    for (unsigned int i = 0; i < idToNode.size(); i++) {
      std::cout << i << "\t" << ancestor[i] << "\t" << semi[i] << "\t"
                << idom[i] << "\t" << buckets[i] << std::endl;
    }
  }
};
...
//! STD-Dominator-Tree
typedef TemplatedDominatorTree<DefaultBasicDominatorTreeIsStatementFilter>
    DominatorTree;
