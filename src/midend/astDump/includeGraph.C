...
// DQ (11/11/2018): We need a list to make sure that we don't add nodes twice
// (once when traversing the source sequence lists and a second time when
// traversing the AST). We need to show the AST plus the edges that are specific
...
void IncludeFileGraph::insertGraphNode(void *node) {
...
void IncludeFileGraph::graph_include_files(SgSourceFile *sourceFile) {
  ...
  if (graphNodeSet.find(sourceFile) == graphNodeSet.end()) {
    ...
    file << "\"" << StringUtility::numberToString(sourceFile) << "\"["
         << "label=\"" << name << "\\n"
         << StringUtility::numberToString(sourceFile) << "\" color=\"green\" ];"
         << endl;

    insertGraphNode(sourceFile);

#if DEBUG_HEADER_GRAPH_SUPPPORT
    printf("In graph_include_files(): sourceFile (node = %p): "
           "graphNodeSet.size() = %" PRIuPTR " \n",
           sourceFile, graphNodeSet.size());
#endif
  } else {
#if DEBUG_HEADER_GRAPH_SUPPPORT
    printf("In graph_include_files(): graph_headers previously processed, "
           "sourceFile in graphNodeSet: sourceFile = %p \n",
           sourceFile);
#endif
    return;
  }
...
#endif
    file << "\"" << StringUtility::numberToString(sourceFile) << "\" -> \""
         << StringUtility::numberToString(includeFile)
         // << "\"[label=\"" << "including : " +
         // StringUtility::numberToString(42) << ":"
         // << "\"[label=\"" << "assoc-include"
         << "\"[label=\""
         << "assoc-include"
         // << StringUtility::numberToString(includeFile) << "\" color=\"red\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         // << StringUtility::numberToString(8) << "\" color=\"red\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         << "\" color=\"red\" labelfontcolor=\"blue4\" weight=1 len=2];"
         << endl;
...
void IncludeFileGraph::graph_include_files(SgIncludeFile *includeFile) {
  ...
  if (graphNodeSet.find(includeFile) == graphNodeSet.end()) {
    ...
    // name = name + "\\n name_as_written = " <<
    // includeFile->get_name_used_in_include_directive() << "\""; name = name +
    // "\\n name_as_written = " <<
    // includeFile->get_name_used_in_include_directive().str() << " "; name =
    // name + "\\n name_as_written = "; // <<
    ...
    // DQ (11/16/2018): Added reference to the application root (only output for
    // the root of the include tree). SgSourceFile* source_file =
    // includeFile->get_source_file(); SgSourceFile* source_file =
    // includeFile->get_including_source_file(); SgSourceFile*
    // parent_source_file = includeFile->get_including_source_file();
    ...
    if (source_file != NULL && source_file->get_isHeaderFile() == false) {
      name = name + "\\n applicationRootDirectory = " +
             includeFile->get_applicationRootDirectory().str() + "      ";
    }

    name = name + "\\n requires_explict_path_for_unparsed_headers = " +
           (includeFile->get_requires_explict_path_for_unparsed_headers()
                ? "true"
                : "false") +
           "      ";
    ...
    if (includeFile->get_can_be_supported_using_token_based_unparsing() ==
        false) {
    ...
    if (node_color_set == false &&
        includeFile->get_isRoseSystemInclude() == true) {
      fill_color = ",fillcolor=\"orange\" ";
      node_color_set = true;
    }

    // DQ (11/21/2018): Mark those header files included from system paths
    // (internal default paths searched by the compiler).
    if (node_color_set == false &&
        includeFile->get_from_system_include_dir() == true) {
    ...
    // (internal default paths searched by the compiler).
    if (node_color_set == false &&
        includeFile->get_preinclude_macros_only() == true) {
    ...
    // file << "\"" << StringUtility::numberToString(includeFile) << "\"[" <<
    // "label=\"" << name << "\\n"
    //      << StringUtility::numberToString(includeFile) << "\"
    //      color=\"purple\" ];" << endl;
    file << "\"" << StringUtility::numberToString(includeFile) << "\"["
         << "label=\"" << name << "\\n"
         << StringUtility::numberToString(includeFile) << "\" color=\"purple\""
         << fill_color << " ];" << endl;

    insertGraphNode(includeFile);

#if DEBUG_HEADER_GRAPH_SUPPPORT
    printf("In graph_include_files(): includeFile (node = %p): "
           "graphNodeSet.size() = %" PRIuPTR " \n",
...
#if DEBUG_HEADER_GRAPH_SUPPPORT
    printf("In graph_include_files(): graph_headers previously processed, "
           "SgIncludeFile in graphNodeSet: includeFile = %p \n",
           includeFile);
#endif
    return;
  }
  ...
  if (sourceFile != NULL) {
    // DQ (11/14/2018): This is no longer true for the input file which now has
    // a SgIncludeFile associated with it so that we can build a SgIncludeFile
    // tree that we can operate upon with ROSE traversals.
    // ROSE_ASSERT(sourceFile->get_isHeaderFile() == true);

    file << "\"" << StringUtility::numberToString(includeFile) << "\" -> \""
         << StringUtility::numberToString(sourceFile)
         // << "\"[label=\"" << "file : " + StringUtility::numberToString(42) <<
         // ":"
         // << "\"[label=\"" << "assoc-file"
         << "\"[label=\""
         << "file   "
         // << StringUtility::numberToString(sourceFile) << "\" color=\"red\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         // << StringUtility::numberToString(9) << "\" color=\"red\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
  ...
  if (parent_include_file != NULL) {
    file << "\"" << StringUtility::numberToString(includeFile) << "\" -> \""
         << StringUtility::numberToString(parent_include_file)
         ...
         // << StringUtility::numberToString(10) << "\" color=\"red\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         << "\" color=\"blue\" labelfontcolor=\"blue4\" weight=1 len=2];"
         << endl;
...
// DQ(11/14/2018): Build a simpler graph while debugging include tree.
  ...
  // "source_file_of_translation_unit", " = NULL",
  ...
  if (translation_unit_source_file != NULL) {
    file << "\"" << StringUtility::numberToString(includeFile) << "\" -> \""
         << StringUtility::numberToString(translation_unit_source_file)
         // << "\"[label=\"" << "parent : " + StringUtility::numberToString(42)
         // << ":"
         << "\"[label=\""
         << "translation unit"
         // << StringUtility::numberToString(parent_include_file) << "\"
         // color=\"red\" labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         // << StringUtility::numberToString(10) << "\" color=\"black\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         << "\" color=\"black\" labelfontcolor=\"blue4\" weight=1 len=2];"
         << endl;
  ...
  for (size_t i = 0; i < includeFile->get_include_file_list().size(); i++) {
    ...
    file << "\"" << StringUtility::numberToString(includeFile) << "\" -> \""
         << StringUtility::numberToString(nested_includeFile)
         ...
         // << StringUtility::numberToString(nested_includeFile) << "\"
         // color=\"red\" labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         // << StringUtility::numberToString(11) << "\" color=\"red\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         << "\" color=\"blue\" labelfontcolor=\"blue4\" weight=1 len=2];"
         << endl;
...
void IncludeFileGraph::graph_include_files(SgProject *project) {
#if DEBUG_HEADER_GRAPH_SUPPPORT
  printf(
      "In graph_include_files(): project = %p: graphNodeSet.size() = %" PRIuPTR
      " \n",
  ...
  if (graphNodeSet.find(project) == graphNodeSet.end()) {
    ...
    file << "\"" << StringUtility::numberToString(project) << "\"["
         << "label=\"" << name << "\\n"
         << StringUtility::numberToString(project) << "\" color=\"blue\" ];"
         << endl;

    insertGraphNode(project);

#if DEBUG_HEADER_GRAPH_SUPPPORT
    printf("In graph_include_files(): project (node = %p): graphNodeSet.size() "
           "= %" PRIuPTR " \n",
           project, graphNodeSet.size());
#endif
  } else {
#if DEBUG_HEADER_GRAPH_SUPPPORT
    printf("In graph_include_files(): graph_headers previously processed, "
           "project in graphNodeSet: project = %p \n",
           project);
#endif
    return;
  }
  ...
  for (size_t i = 0; i < fileList->get_listOfFiles().size(); ++i) {
    ...
    file << "\"" << StringUtility::numberToString(project) << "\" -> \""
         << StringUtility::numberToString(sourceFile) << "\"[label=\""
         << "source_file : " + StringUtility::numberToString(i)
         // << StringUtility::numberToString(sourceFile) << "\" color=\"blue\"
         // labelfontcolor=\"blue4\" weight=1 len=2];" << endl;
         << "\" color=\"blue\" labelfontcolor=\"blue4\" weight=1 len=2];"
         << endl;
...
void generateGraphOfIncludeFiles(SgProject *project, std::string filename) {
  // DQ (11/10/2018): Build a dot graph of the header files used and where they
  // are included from (should be a tree, except where headers might be included
  // multiple times).
  ...
  // Calling recursive function to build dot file for scope, starting at the top
  // level scope.
