...
template <class Node> DOTGeneration<Node>::DOTGeneration() { init(); }

template <class Node> void DOTGeneration<Node>::init() {
  tdTrace = 0;
  buTrace = 0;
  tdbuTrace = 0;
  dotrep.clear();
...
template <class Node>
void DOTGeneration<Node>::generate(std::string filename, Node node,
                                   traversalType tt) {
  init();
  traversal = tt;
...
DOTGeneration<Node>::evaluateInheritedAttribute(Node node,
                                                DOTInheritedAttribute ia) {
...
template <class Node>
DOTSynthesizedAttribute DOTGeneration<Node>::evaluateSynthesizedAttribute(
    Node node, DOTInheritedAttribute ia, SubTreeSynthesizedAttributes l) {
  ...
  for (iter = l.begin(); iter != l.end(); iter++) {
    // DQ (3/5/2007): Added addional function to add more edge information to
    // the label and option string
    std::string edgelabel = "";
    std::string edgeoption = "";

    // Use the label to identify the edge (in general, even though in this case
    // the label is "")
...
template <class Node>
std::string DOTGeneration<Node>::additionalNodeInfo(Node node) {
  return "";
}

template <class Node>
std::string DOTGeneration<Node>::additionalNodeOptions(Node node) {
  return "";
}

// DQ (3/5/2007): Added to support edge options (e.g. colors)
template <class Node>
std::string DOTGeneration<Node>::additionalEdgeInfo(Node from, Node to,
                                                    std::string label) {
  return "";
}
...
// DQ (3/5/2007): Added to support edge options (e.g. colors)
template <class Node>
std::string DOTGeneration<Node>::additionalEdgeOptions(Node from, Node to,
                                                       std::string label) {
  return "";
}
