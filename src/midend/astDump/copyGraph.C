...
// This file contains support for the visualization of an AST divided into two
// sets of nodes. It is used to visualize the results of the AST Copy mechanism
// and to visualize the results of outlining in the special case where the
// outlined function (and supporting declarations) are moved to a separate file.

// This visualization presents the AST as two sets (using the DOT subgraph
// mechanism). Connections between the subgraphs represent shared IR nodes.  In
// a properly separate AST, the edges connectiong the subgraphs will only be for
// types (yellow).
...
set<SgNode *> getAllNodes(SgNode *node) {
...
void graphNodesAfterCopy(const set<SgNode *> &oldNodes, string filename) {
  ...
  generateWholeGraphOfAST(temp_filename.c_str());
  {
    ...
    for (set<SgNode *>::const_iterator i = oldNodes.begin();
         i != oldNodes.end(); ++i) {
      if (i != oldNodes.begin())
        fs << ' ';
      fs << (uintptr_t)(*i);
