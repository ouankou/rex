...
public:
  ...
  // DQ (12/20/2018): Skip template instantiations that can make the DOT files
  // too large to generate or look at easily.
  bool skipTemplateInstantiations;

  DOTInheritedAttribute()
      : skipSubTree(false), skipTemplateInstantiations(false), tdTracePos(0),
        tdbuTracePos(0) {}
  DOTRepresentation<SgNode *>::TraceType tdTracePos; // tdTrace value at node
  DOTRepresentation<SgNode *>::TraceType
      tdbuTracePos; // tdbuTrace value at node
};
...
public:
  ...
  DOTSynthesizedAttribute(SgNode *n) : node(n), defaultVal(false) {
    dotstring = "";
  };
...
template <class Node>
class DOTGeneration
    : public SgTopDownBottomUpProcessing<DOTInheritedAttribute,
                                         DOTSynthesizedAttribute> {
public:
  ...
  DOTGeneration();
  // generates a visualization of an AST subtree with root node node. The
  // filename must be specified. DQ (10/30/2003) Make generate a virtual
  // function MS 10/22/2015: this function is never overwritten, but overloaded
  // in subclasses. Let's keep it non-virtual for that reason.
  void generate(std::string filename, Node node,
                traversalType tt = TOPDOWNBOTTOMUP);
  virtual DOTInheritedAttribute
  evaluateInheritedAttribute(Node node, DOTInheritedAttribute ia);
  virtual DOTSynthesizedAttribute
  evaluateSynthesizedAttribute(Node node, DOTInheritedAttribute ia,
                               SubTreeSynthesizedAttributes l);

  // this virtual function allows to customize the dot output for each node by
  // overriding it.
  ...
  virtual std::string additionalEdgeOptions(Node from, Node to,
                                            std::string label);
