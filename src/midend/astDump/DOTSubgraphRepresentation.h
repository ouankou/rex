...
public:
  ...
  // DQ (8/16/2004): Added to refactor code in support of graph hierarcies
  // (nested subgraphs)
  ...
  void addEdge(NodeType node1, TraceType downtrace, TraceType uptrace,
               std::string edgelabel, NodeType node2, std::string option = "");
  void addEdge(NodeType node1, TraceType downtrace, std::string edgelabel,
               NodeType node2, std::string option = "");
  void addEdge(NodeType node1, std::string edgelabel, NodeType node2,
               std::string option = "");

  void addNullValue(NodeType node, TraceType trace, std::string varname,
                    std::string option = "");
  void addNullValue(NodeType node, std::string nodelabel, std::string edgelabel,
                    std::string option = "");
  void addEmptyContainer(NodeType node, TraceType trace, std::string varname,
                         std::string option = "");

  std::string containerEdgeLabel(std::string containerVarName,
  ...
  // DQ (8/14/2004): Removed return value! fixes warning and implements better
  // code not clear if the semantics really should be to return a value. int
  // addSubgraph(int id, string name)
  void addSubgraph(int id, std::string name);

  // DQ (2/8/2005): Moved this to the source file instead of placing it in the
  // header
...
private:
  ...
  //! use the automatically assigned subgraph id's ?, should not be mixed with
  //! manual assignments
  bool mAutoSubgraphIds;

  //! use the manual assigned of subgraph id's ?, should not be mixed with auto
  //! assignments
