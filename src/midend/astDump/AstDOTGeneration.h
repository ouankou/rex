...
public:
  // traverse full AST , nodes which represent code from include files . name
  // postfix does not need an ending "."
  void generate(SgProject *node, traversalType tt = TOPDOWNBOTTOMUP,
                std::string filenamePostfix = "");
  void generate(SgNode *node, std::string filename,
                traversalType tt = TOPDOWNBOTTOMUP,
                std::string filenamePostfix = "");
  void generate(SgProject *project, std::string const &filename);

  // DQ (12/20/2018): Added support to exclude template instantiations to make
  // the graphs more tractable within large C++ applications. traverse the
  // subtree of AST which represents the files specified on the command line
  ...
  void generateWithinFile(SgFile *node, traversalType tt = TOPDOWNBOTTOMUP,
                          std::string filenamePostfix = "");
  void writeIncidenceGraphToDOTFile(SgIncidenceDirectedGraph *graph,
                                    const std::string &filename);
  void addAdditionalNodesAndEdges(SgNode *node);

protected:
  virtual DOTInheritedAttribute
  evaluateInheritedAttribute(SgNode *node, DOTInheritedAttribute ia);
  virtual DOTSynthesizedAttribute
  ...
  // DQ (3/5/2007) added mechanism to add edge options (to add color, etc.)
  std::string additionalEdgeOptions(SgNode *from, SgNode *to,
                                    std::string label);
...
struct DefaultExtraNodeOptions {
  std::string operator()(SgNode *) { return std::string(); }
};

struct DefaultExtraEdgeInfo {
  std::string operator()(SgNode * /*from*/, SgNode * /*to*/,
                         std::string /*label*/) {
    return std::string();
  }
};
...
struct DefaultExtraEdgeOptions {
  std::string operator()(SgNode * /*node*/, SgNode * /*to*/,
                         std::string /*label*/) {
    return std::string();
  }
};
...
public:
  AstDOTGenerationExtended(ExtraNodeInfo_t eni_ = ExtraNodeInfo_t(),
  ...
  // virtual DOTInheritedAttribute evaluateInheritedAttribute(SgNode* node,
