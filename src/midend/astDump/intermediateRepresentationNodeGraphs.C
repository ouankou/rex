...
void intermediateRepresentationNodeGraph(
    SgProject *project, const std::vector<VariantT> &nodeKindList) {
  // Build filename...
...
IntermediateRepresentationNodeGraph::IntermediateRepresentationNodeGraph(
    ofstream &inputFile, SgProject *project,
    const std::vector<VariantT> &inputNodeKindList)
    : file(inputFile), nodeKindList(inputNodeKindList) {
  for (size_t i = 0; i < nodeKindList.size(); i++) {
    printf("Adding nodeKindList[%" PRIuPTR "] = %d = %s to nodeKindSet \n", i,
           Cxx_GrammarTerminalNames[nodeKindList[i]].variant,
           Cxx_GrammarTerminalNames[nodeKindList[i]].name.c_str());
  ...
  Rose_STL_Container<SgNode *> functionDeclarationList =
      NodeQuery::querySubTree(project, V_SgStatement);
  ...
  for (Rose_STL_Container<SgNode *>::iterator i =
    ...
    // isSgDeclarationStatement(*i); SgFunctionDeclaration*  functionDeclaration
    ...
    if (statement != NULL &&
        statement->get_file_info()->isCompilerGenerated() == false) {
  ...
  if (numberOfNodes <= maxNumberOfNodes) {
    // Ouput nodes.
    for (std::set<SgNode *>::iterator i = include_nodeSet.begin();
         i != include_nodeSet.end(); i++) {
      SgNode *node = *i;
      file << "\"" << StringUtility::numberToString(node) << "\"[" << "label=\""
           << node->class_name() << "\\n"
    ...
    for (std::set<SgNode *>::iterator i = include_nodeSet.begin();
      ...
      std::vector<std::pair<SgNode *, std::string>>::iterator j =
          listOfIRnodes.begin();
      while (j != listOfIRnodes.end()) {
        if (include_nodeSet.find(j->first) != include_nodeSet.end()) {
          file << "\"" << StringUtility::numberToString(node) << "\" -> \""
               << StringUtility::numberToString(j->first) << "\"[label=\""
               << j->second << "\"];" << endl;
        }
    ...
    printf("WARNING: IntermediateRepresentationNodeGraph is too large to "
           "generate: numberOfNodes = %d (max size = %d) \n",
           numberOfNodes, maxNumberOfNodes);
  }
}

void IntermediateRepresentationNodeGraph::visit(SgNode *node) {
  // digraph "Gtest2013_322_WholeAST.dot" {
  // "0x2b6c15bf1010"[label="\n13SgSymbolTable\n  Unnamed Symbol Table\n
  // 0x2b6c15bf1010  " shape=house,regular=0,URL="\N",tooltip="more info at
  // \N",sides=5,peripheries=1,color="blue",fillcolor=orange,fontname="7x13bold",fontcolor=black,style=filled];
  // "0x2b6c15bf1010" -> "0x1535c1c0"[label="foo" dir=forward ];

  // "0x2b6c15cb3138" -> "0x152fc188"[label="storageModifier" dir=forward ];
  // "0x2b6c15cb3138" -> "0x2b6c15bb0010"[label="scope" color="green" decorate
  // dir=forward ]; "0x2b6c15cb3138" -> "0x2b6c15c1a280"[label="parent"
  // color="blue" decorate dir=forward ];
  // }
}
