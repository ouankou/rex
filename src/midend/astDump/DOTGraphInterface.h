...
template <class VertexType, class EdgeType, class VertexIterator,
          class EdgeIterator>
...
protected:
  //! directions of an edge: no direction, towards target, towards source  or
  //! bidirectional
  typedef enum { none = 1, target, source, both } EdgeDirection;
  ...
  //! get unique string representation of a vertex, NOTE - this function works
  //! with a pointer to the vertex!
  ...
  //! get the target vertex of an edge, NOTE - this function returns a pointer
  //! to a vertex for use with vertexToString!
  ...
  //! get a list of the subgraph names, together with their ID's (see also
  //! getVertexSubgraphId)
  //  this function should construct a map containing the ID a the name as a
  //  string subgraph Id's have to be >0 to be valid only subgraphs entered in
  //  this list will be displayed (the name may also be empty)
...
template <class VertexType, class EdgeType, class VertexIterator,
          class EdgeIterator>
void DOTGraphInterface<VertexType, EdgeType, VertexIterator,
                       EdgeIterator>::writeToDOTFile(std::string filename) {
  bool debug = false;
  ...
  for (std::map<int, std::string>::iterator s = subgraphs.begin();
       s != subgraphs.end(); s++) {
  ...
  for (VertexIterator i = getVertices(); i != getVerticesEnd(); i++) {
    ...
    if (debug) {
      std::cerr << " add node " << vertexToString(&(*i)) << " as "
                << getVertexName((*i)) << std::endl; // debug
    }
  }
  ...
  for (VertexIterator i = getVertices(); i != getVerticesEnd(); i++) {
    ...
    for (EdgeIterator j = estart; j != eend; j++) {
      ...
      // AS(032006) Fixed it so that the corect edge is made. Earlier a call was
      // made towards mDotRep.addEdge(
      // vertexToString(&(*i)),vertexToString(getTargetVertex(e)),"" ) which
      // would result in the second paramater implemented as a label and third
      // as a node id. Therefore I decided to simplify the implementation
      mDotRep.addEdge(vertexToString(&(*i)), label,
                      vertexToString(getTargetVertex(e)), "");
      if (debug)
        std::cerr << " add edge from " << vertexToString(&(*i)) << " to "
                  << vertexToString(getTargetVertex(e)) << " as " << label
                  << std::endl; // debug
      if (debug)
