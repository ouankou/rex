...
public:
  ...
  // DQ (3/5/2007): Added label string (mostly for symmetry with
  ...
  CustomAstDOTGenerationEdgeType(SgNode *n1, SgNode *n2, std::string label = "",
                                 std::string options = "")
      : start(n1), end(n2), labelString(label), optionString(options) {}

  // DQ (3/6/2007): Modified to only focus on the start and end nodes, actually
  // we need the label test These are implemented to support the STL find
  // function (STL algorithm) bool operator!=(const
  // CustomAstDOTGenerationEdgeType & edge) const { return (edge.start != start)
  // || (edge.end != end) || (edge.optionString != optionString); } bool
  // operator==(const CustomAstDOTGenerationEdgeType & edge) const { return
  // (edge.start == start) && (edge.end == end) && (edge.optionString ==
  // optionString); } bool operator!=(const CustomAstDOTGenerationEdgeType &
  // edge) const { return (edge.start != start) || (edge.end != end); } bool
  // operator==(const CustomAstDOTGenerationEdgeType & edge) const { return
  ...
  // bool operator< (const CustomAstDOTGenerationEdgeType & edge) const { return
  // (edge.start < start) || (edge.start == start) && (edge.end < end); }
  bool operator<(const CustomAstDOTGenerationEdgeType &edge) const {
    return (edge.start < start) ||
           ((edge.start == start) && (edge.labelString == labelString)) &&
               (edge.end < end);
  }
};

class CustomAstDOTGenerationNodeType {
  // NodeType holdes information required to specify additional info for any
  // node to DOT
...
public:
  ...
  CustomAstDOTGenerationNodeType(const CustomAstDOTGenerationNodeType &X)
      : target(X.target), labelString(X.labelString),
        optionString(X.optionString) {
    ROSE_ASSERT(X.optionString.size() < 4000);
    ROSE_ASSERT(optionString.size() < 4000);
  }

  // These are implemented to support the STL find function (STL algorithm)
  bool operator!=(const CustomAstDOTGenerationNodeType &node) const {
...
class CustomAstDOTGenerationData : public AstDOTGeneration {
  // Specialization of AST DOT graphics output class to support the overloaded
  // operators Note that this class is derived from a traversal and so can not
  // be the class that is used in the multiple inhertance design where
  // CustomDOTGeneration is used.
...
public:
  ...
  // void generateInputFiles(SgProject* node, traversalType tt=TOPDOWNBOTTOMUP,
  ...
  virtual std::string additionalEdgeInfo(SgNode *from, SgNode *to,
                                         std::string label);
  virtual std::string additionalEdgeOptions(SgNode *from, SgNode *to,
                                            std::string label);

  // This generates code for the DOT graph node
  std::string unparseToCompleteStringForDOT(SgNode *astNode);

  // Wrapup details and output the file (generates the rest of the nodes in the
  // AST)
...
// Build a similar coloring mechanism as for the ROSE traversals developed by
// Markus, but do this for the Memory Pool based traversals.
...
public:
  ...
  // Nodes to ignore when building the DOT graph, we have to remove it from the
  // nodeList if it is there.
...
// In order to get the CustomAstDOTGenerationData data member into the traversal
// we will represent it in a class and use multiple inheritance.
// This is a copy of CustomDOTGeneration using a
// CustomMemoryPoolDOTGenerationData instead of a CustomAstDOTGenerationData
// data member
...
public:
  ...
  // Mechanism to ignore all SgFile Info IR nodes (often they just get in the
  // way)
...
public:
  ...
  static void
  generateGraph(std::string filename,
                const std::set<SgNode *> &firstAST = defaultSetOfIRnodes);

  void markFirstAST();
  void buildExcludeList();

  SimpleColorMemoryPoolTraversal(
      const std::set<SgNode *> &s = defaultSetOfIRnodes)
      : setOfIRnodes(s) {};
...
public:
  ...
  static void generateGraph(SgProject *project, std::string filename,
                            std::set<SgNode *> &specialNodeList =
