...
std::string
AstTerm::astTermWithNullValuesToString(SgNode *node, bool withNumbers,
                                       bool withTypes,
    ...
    if (SgExpression *exp = isSgExpression(node)) {
      ...
      if (expType) {
        if (visited.find(expType) == visited.end()) {
          visited.insert(
              expType); // for detecting sharing and/or cycle in type info
          typeTerm += "type:" + astTermWithNullValuesToString(
                                    expType, withNumbers, withTypes, visited);
          if (SgArrayType *arrayType = isSgArrayType(expType)) {
            typeTerm += ",basetype:" + astTermWithNullValuesToString(
                                           arrayType->get_base_type(),
                                           withNumbers, withTypes, visited);
          }
        } else {
  ...
  if (arity > 0) {
    ...
    for (int i = 0; i < arity; i++) {
      ...
      if (i != 0)
        s += ",";
      s +=
          astTermWithNullValuesToString(child, withNumbers, withTypes, visited);
    }
    s += ")";
  }
...
std::string AstTerm::pointerExprToString(SgNode *node) {
  // MS: TODO: type check is required to ensure the expression is indeed a
  // pointer expression e.g. for return 0 it produces "null" even if the integer
  // 0 is returned.

  if (isSgExprListExp(node)) {
    return string("EXPRLIST ") + "[" +
  ...
  if (SgVarRefExp *varRefExp = isSgVarRefExp(node))
    return "\"" +
           SgNodeHelper::symbolToString(
               SgNodeHelper::getSymbolOfVariable(varRefExp)) +
           "\" ";

  string s;
  if (SgVariableDeclaration *varDecl = isSgVariableDeclaration(node)) {
    SgInitializedName *initName =
        SgNodeHelper::getInitializedNameOfVariableDeclaration(varDecl);
    string lhs = "\"" +
                 SgNodeHelper::symbolToString(
                     SgNodeHelper::getSymbolOfInitializedName(initName)) +
                 "\"";
...
std::string AstTerm::astTermWithoutNullValuesToDot(SgNode *root) {
  ...
  std::stringstream ss;
  ss << "digraph G {\n ordering=out;\n";
  for (RoseAst::iterator i = ast.begin().withoutNullValues(); i != ast.end();
       ++i) {
    ss << "\"" << *i << "\"" << "[label=\"" << nodeTypeName(*i) << "\"];"
       << std::endl;
    if (*i != root) {
      ss << "\"" << i.parent() << "\"" << " -> " << "\"" << *i << "\"" << ";"
         << std::endl;
...
std::string AstTerm::astTermWithNullValuesToDotFragment(SgNode *root) {
  RoseAst ast(root);
  std::stringstream ss;
  for (RoseAst::iterator i = ast.begin().withNullValues(); i != ast.end();
       ++i) {
...
std::string AstTerm::astTermToDot(RoseAst::iterator start,
                                  RoseAst::iterator end) {
  std::stringstream ss;
  SgNode *root = *start;
  long int visitCnt = 1;
  ss << "digraph G {\n";
