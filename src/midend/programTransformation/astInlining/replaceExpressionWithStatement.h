...
// King84: there seems to be a circular dependency with including sage interface
// (which happens before this file is included), so we just predeclare the one
// class we use from it.
...
// DQ (12/14/2006): Modified this function to find the root node of an
// expression, which is not always a SgExpressionRoot, now that we have removed
// these from the AST.
//! Get the topmost enclosing expression of a given expression.  Needs to be
//! moved.
