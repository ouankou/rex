...
//
// An array reference is strided if the last element is not equal to the loop
// index
//
...
void OmpSimdCompiler::omp_simd_build_math(VariantT op_type, SgType *type) {
  ...
  switch (op_type) {
  case V_SgAddOp:
    op = buildAddOp(var1, var2);
    break;
  case V_SgSubtractOp:
    op = buildSubtractOp(var1, var2);
    break;
  case V_SgMultiplyOp:
    op = buildMultiplyOp(var1, var2);
    break;
  case V_SgDivideOp:
    op = buildDivideOp(var1, var2);
    break;
  default: {
  }
  }
...
char OmpSimdCompiler::omp_simd_get_reduction_mod(SgVarRefExp *var) {
  ...
  for (size_t i = 0; i < clauses.size(); i++) {
    ...
    for (size_t j = 0; j < vars.size(); j++) {
      ...
      if (v_current->get_symbol()->get_name() ==
          var->get_symbol()->get_name()) {
        found = true;
        break;
      }
    }
...
bool OmpSimdCompiler::omp_simd_pass1() {
  ...
  for (Rose_STL_Container<SgNode *>::iterator i = bodyList.begin();
    ...
    Rose_STL_Container<SgNode *> exprBody =
    ...
    if (orig->variantT() == V_SgVarRefExp) {
      ...
      SgPntrArrRefExp *array =
          static_cast<SgPntrArrRefExp *>(copyExpression(orig));
      type = array->get_type();
    ...
    if (partial_vec != "")
      lhs = buildVarRefExp(partial_vec, new_block);
    ...
    if (isSgAssignOp(expr2))
      assign_list.push_back(isSgAssignOp(expr2));
  ...
  for (Rose_STL_Container<SgStatement *>::iterator i = bodyList.begin();
    ...
    if (lval->variantT() == V_SgVarRefExp &&
      ...
      if (lvar->get_type()->variantT() == V_SgPointerType) {
        ...
        if (pntr->get_rhs_operand()->variantT() == V_SgPntrArrRefExp) {
          SgSIMDGather *ld = buildBinaryExpression<SgSIMDGather>(
              deepCopy(lval), deepCopy(rval));
          ir_block->push_back(ld);
        } else if (pntr->get_lhs_operand()->variantT() == V_SgPntrArrRefExp) {
          SgSIMDExplicitGather *ld =
              buildBinaryExpression<SgSIMDExplicitGather>(deepCopy(lval),
                                                          deepCopy(rval));
          ir_block->push_back(ld);
        } else {
          SgSIMDLoad *ld =
              buildBinaryExpression<SgSIMDLoad>(deepCopy(lval), deepCopy(rval));
          ir_block->push_back(ld);
        }
      }
      ...
      if (pntr->get_rhs_operand()->variantT() == V_SgPntrArrRefExp) {
        ...
        SgSIMDStore *str =
            buildBinaryExpression<SgSIMDStore>(deepCopy(lval), deepCopy(rval));
        ir_block->push_back(str);
      }
      ...
      if (!isSgVarRefExp(lval) || !isSgVarRefExp(rval)) {
        std::cout << "Error: invalid arguments in math expression."
                  << std::endl;
        continue;
      }
      ...
      if (name.rfind("__part", 0) == 0) {
        SgSIMDPartialStore *str =
            buildBinaryExpression<SgSIMDPartialStore>(deepCopy(lval), NULL);
        ir_block->push_back(str);
      }
      ...
      case V_SgAddOp:
        math = buildBinaryExpression<SgSIMDAddOp>(dest, parameters);
        break;
      case V_SgSubtractOp:
        math = buildBinaryExpression<SgSIMDSubOp>(dest, parameters);
        break;
      case V_SgMultiplyOp:
        math = buildBinaryExpression<SgSIMDMulOp>(dest, parameters);
        break;
      case V_SgDivideOp:
        math = buildBinaryExpression<SgSIMDDivOp>(dest, parameters);
        break;
      default:
        std::cout << "Error: Invalid math." << std::endl;
      }
...
// If none is provided, we return -1, which means the compiler should use the
// default length If we return -2, an error has occurred
int OmpSimdCompiler::omp_simd_get_simdlen(bool safelen) {
  ...
  for (size_t i = 0; i < clauses.size(); i++) {
    ...
    SgOmpExpressionClause *sl_clause =
        static_cast<SgOmpExpressionClause *>(clauses.at(i));
    SgExpression *sl_expr = sl_clause->get_expression();

    if (sl_expr->variantT() != V_SgIntVal) {
      std::cout << "Error: SIMDLEN value must currently only be an integer."
                << std::endl;
      return -1;
    }
...
//
// This is platform-specific. Currently, I haven't decided what to with Arm yet.
// For intel:
// <= 4 -> SSE (Not sure why someone would want to use this...)
// > 4 <= 8 -> AVX2
// > 8 <= 16 -> AVX-512
// For now, ignore anything greater than 16. At some point, we may want to
// implement some kind of lowering
...
SgBasicBlock *OmpSimdCompiler::getBlock() { return new_block; }

SgOmpSimdStatement *OmpSimdCompiler::getTarget() { return target; }

SgForStatement *OmpSimdCompiler::getForLoop() { return for_loop; }

Rose_STL_Container<SgNode *> *OmpSimdCompiler::getIR() { return ir_block; }

bool OmpSimdCompiler::isArm() { return arm; }
...
void OmpSimdCompiler::addIR(SgNode *ir) { ir_block->push_back(ir); }
...
void OmpSupport::transOmpSimd(SgNode *node) {
  ...
  bool isArm = false;
  if (simd_arch == ArmAddr3 || simd_arch == Arm_SVE2)
    isArm = true;
