...
SgType *intel_simd_type(SgType *type, SgScopeStatement *new_block,
  ...
  if (len == 16) {
    ...
    case V_SgTypeInt:
      vector_type = buildOpaqueType("__m512i", global_scope);
      break;
    case V_SgTypeFloat:
      vector_type = buildOpaqueType("__m512", global_scope);
      break;
    case V_SgTypeDouble:
      vector_type = buildOpaqueType("__m512d", global_scope);
      break;
    default:
      vector_type = type;
    }
    ...
    switch (type->variantT()) {
    case V_SgTypeInt:
      vector_type = buildOpaqueType("__m256i", new_block);
      break;
    case V_SgTypeFloat:
      vector_type = buildOpaqueType("__m256", new_block);
      break;
    case V_SgTypeDouble:
      vector_type = buildOpaqueType("__m256d", new_block);
      break;
    default:
      vector_type = type;
    }
...
std::string intel_simd_func(OpType op_type, SgType *type,
                            bool half_type = false) {
  int len = simd_len;
  if (half_type)
    len /= 2;
...
SgAssignInitializer *intel_write_load(SgBinaryOp *op,
...
//
SgAssignInitializer *intel_write_broadcast(SgBinaryOp *op,
                                           SgOmpSimdStatement *target,
                                           SgBasicBlock *new_block) {
  SgExpression *lval = op->get_lhs_operand();
  SgExpression *rval = op->get_rhs_operand();
  ...
  SgType *vector_type =
      intel_simd_type(v_dest->get_type(), target->get_scope());
...
//
SgAssignInitializer *intel_write_exp_gather(SgBinaryOp *op,
                                            SgOmpSimdStatement *target,
                                            SgBasicBlock *new_block,
  ...
  SgVariableDeclaration *loop_inc_vd = buildVariableDeclaration(
      "__i", buildIntType(), buildAssignInitializer(buildIntVal(0)));
  SgLessThanOp *loop_cmp = buildLessThanOp(buildVarRefExp("__i"), length);
  SgPlusAssignOp *loop_inc =
      buildPlusAssignOp(buildVarRefExp("__i"), buildIntVal(1));
  ...
  SgPntrArrRefExp *index_pntr = buildPntrArrRefExp(
      buildVarRefExp(name, new_block), buildVarRefExp("__i", new_block));
  ...
  index_pntr =
      buildPntrArrRefExp(buildVarRefExp(name, new_block), buildIntVal(0));
  ...
  if (simd_len == 8) {
    parameters = buildExprListExp(pntr2->get_lhs_operand(),
                                  buildVarRefExp(vindex_name, new_block),
                                  buildIntVal(4));
  } else {
    parameters = buildExprListExp(buildVarRefExp(vindex_name, new_block),
                                  pntr2->get_lhs_operand(), buildIntVal(4));
  }
...
//
SgAssignInitializer *intel_write_gather(SgBinaryOp *op,
                                        SgOmpSimdStatement *target,
                                        SgBasicBlock *new_block) {
  SgExpression *lval = op->get_lhs_operand();
  SgExpression *rval = op->get_rhs_operand();
  ...
  std::string vindex_name = intel_gen_vindex();
  SgType *mask_type =
      intel_simd_type(mask_pntr->get_type(), target->get_scope());
  ...
  SgType *kmask_type;
  if (simd_len == 16)
    kmask_type = buildOpaqueType("__mmask16", new_block);
  else
    kmask_type = buildOpaqueType("__mmask8", new_block);

  SgVariableDeclaration *mask1_vd =
      buildVariableDeclaration(mask1, kmask_type, NULL, new_block);
  SgVariableDeclaration *mask2_vd =
      buildVariableDeclaration(mask2, kmask_type, NULL, new_block);
  ...
  SgVariableDeclaration *kmask_vd =
      buildVariableDeclaration(kmask, kmask_type, local_init, new_block);
  ...
  SgVariableDeclaration *zero_vd =
      buildVariableDeclaration(zero_name, vector_type, local_init, new_block);
...
//
void intel_write_store(SgBinaryOp *op, SgOmpSimdStatement *target,
                       SgBasicBlock *new_block) {
  SgExpression *lval = op->get_lhs_operand();
  SgExpression *rval = op->get_rhs_operand();
...
//
void intel_write_scatter(SgBinaryOp *op, SgOmpSimdStatement *target,
                         SgBasicBlock *new_block) {
  SgExpression *lval = op->get_lhs_operand();
  SgExpression *rval = op->get_rhs_operand();
...
//
SgAssignInitializer *intel_write_partial_store(SgBinaryOp *op,
                                               SgOmpSimdStatement *target,
                                               SgBasicBlock *new_block) {
  ...
  if (std::find(partial_broadcasts.begin(), partial_broadcasts.end(), name) !=
      partial_broadcasts.end()) {
    // Found
  } else {
    ...
    SgExpression *ld =
        buildFunctionCallExp(func_name, vector_type, NULL, target->get_scope());
    SgAssignInitializer *local_init = buildAssignInitializer(ld);

    SgVariableDeclaration *vd =
        buildVariableDeclaration(name, vector_type, local_init, new_block);
...
//
void intel_write_scalar_store(SgBinaryOp *op, SgOmpSimdStatement *target,
                              SgBasicBlock *new_block) {
  SgExpression *lval = op->get_lhs_operand();
  SgExpression *rval = op->get_rhs_operand();
  ...
  // Create the types
  SgType *vector_type =
      intel_simd_type(scalar->get_type(), target->get_scope(), true);
  ...
  if (simd_len == 16) {
    ...
    SgVariableDeclaration *vd1 =
        buildVariableDeclaration(vec1, vector_type, local_init, new_block);
    ...
    SgVariableDeclaration *vd2 =
        buildVariableDeclaration(vec2, vector_type, local_init, new_block);
    ...
    SgVariableDeclaration *vd2 = buildVariableDeclaration(
        vec2, vector_type, local_init, target->get_scope());
    to_insert.push_back(vd2);
  }
  ...
  SgExpression *fc4 = buildFunctionCallExp(func_name, vector_type, hadd_params,
                                           target->get_scope());
...
//
SgAssignInitializer *intel_write_math(SgBinaryOp *op,
                                      SgOmpSimdStatement *target,
                                      SgBasicBlock *new_block,
                                      VariantT math_type) {
  SgExpression *lval = op->get_lhs_operand();
...
void omp_simd_write_intel(SgOmpSimdStatement *target, SgForStatement *for_loop,
  ...
  for (Rose_STL_Container<SgNode *>::iterator i = ir_block->begin();
    ...
    if ((*i)->variantT() !=
      ...
      if (isSgVarRefExp(lval)) {
        ...
        SgType *vector_type =
            intel_simd_type(var->get_type(), target->get_scope());
  ...
  std::vector<SgVariableDeclaration *> loop_statements =
      SageInterface::querySubTree<SgVariableDeclaration>(
          target->get_scope(), V_SgVariableDeclaration);
  for (size_t i = 0; i < loop_statements.size(); i++) {
    ...
    if (var_name->get_name() == "_lt_var_inc") {
      SgAssignInitializer *init =
          buildAssignInitializer(buildIntVal(loop_increment));
  ...
  SgMultiplyOp *mul =
      buildMultiplyOp(inc->get_rhs_operand(), buildIntVal(loop_increment));
