...
int xomp_get_num_devices(void) {
  ...
  if (xomp_num_devices != -1) {
    //    printf ("DEBUG: xomp_get_num_devices() returns the existing value
    //    %d\n", xomp_num_devices);
  ...
  if (env_var_str != NULL) {
    ...
    if (env_var_val > xomp_max_num_devices) {
      printf("OMP_NUM_DEVICES %d is too big, set to max number of devices %d "
             "instead\n",
...
// this can be called multiple times. But the xomp_cuda_prop variable will only
// be set once
...
void xomp_print_gpu_info(int devID) {
  ...
  printf(
      ...
      \tglobal mem bytes =%d, \n\tshared mem bytes =%d, \n\tregs per block = %d\n",
      max_threads_per_block, max_blocks_per_grid_x, global_memory_size,
      shared_memory_size, registers_per_block);
}
// A helper function to probe physical limits based on GPU Compute Capability
// numbers Reference:
...
// The goal is to maximize GPU occupancy for each multiprocessor : physical max
// warps Reference:
// http://developer.download.nvidia.com/compute/cuda/CUDA_Occupancy_calculator.xls
//
...
// So for 1-D block, max threads per block = maxThreadsPerMultiProcessor /
// maxBlocks per multiprocessor
size_t xomp_get_maxThreadsPerBlock(int devID) {
  // this often causes oversubscription to the cores supported by GPU SM
  // processors
  // return xomp_getCudaDeviceProp()->maxThreadsPerBlock;
  // return 128;
  // 2.0: 1536/8= 192 threads per block
  // 3.5 2048/16 = 128
  return xomp_getCudaDeviceProp(devID)->maxThreadsPerMultiProcessor /
         xomp_get_maxThreadBlocksPerMultiprocessor(devID);
}

/*
 * In order to ensure best performance, we setup max_block limitation here, so
 * that each core in the GPU works on only one threads. Use
 * XOMP_accelerator_loop_default() runtime to support input data size that
 * exceeds max_block*xomp_get_maxThreadsPerBlock().
 */
size_t xomp_get_max1DBlock(int devID, size_t s) {
  ...
  size_t max_block = xomp_getCudaDeviceProp(devID)->multiProcessorCount *
                     xomp_get_maxThreadBlocksPerMultiprocessor(devID);
  ...
  // return
  // xomp_getCudaDeviceProp()->multiProcessorCount*(xomp_getCudaDeviceProp()->maxThreadsPerMultiProcessor
  // /xomp_get_maxThreadsPerBlock()) ; return
  // xomp_getCudaDeviceProp()->maxThreadsPerMultiProcessor
  // /xomp_get_maxThreadsPerBlock() ;
}

// Get the max number threads for one dimension (x or y) of a 2D block
// Two factors are considered: the total number of threads within the 2D block
// must<= total threads per block
...
//  maxThreadsDim[0] happens to be equal to  maxThreadsDim[1] so we use a single
//  function to calculate max segments for both dimensions
size_t xomp_get_max_threads_per_dimesion_2D(int devID) {
  ...
  assert(max_threads_per_2d_dimension * max_threads_per_2d_dimension <=
         max_threads_per_block);

  // our assumption is that dim[0] == dim[1] so we handle x and y in one
  // function
  assert(xomp_getCudaDeviceProp(devID)->maxThreadsDim[0] ==
         xomp_getCudaDeviceProp(devID)->maxThreadsDim[1]);
  assert(max_threads_per_2d_dimension <=
         xomp_getCudaDeviceProp(devID)->maxThreadsDim[0]);
  return max_threads_per_2d_dimension;
}

// return the max number of segments for a dimension (either x or y) of a 2D
// block we define the number of segments to be
// SIZE_of_Dimension_x/max_threads_x_dimension
size_t xomp_get_maxSegmentsPerDimensionOf2DBlock(int devID,
  ...
  // e.g. max threads per block is 1024, then max number of tiles per dimension
  // in a 2D block is 1024^0.5 = 32 threads
  size_t max_threads_per_2d_dimension =
      xomp_get_max_threads_per_dimesion_2D(devID);
...
void *xomp_hostMalloc(size_t size) {
  ...
  if (hostPtr == NULL) {
    fprintf(stderr,
            "Error: malloc() failed to allocate the requested %zu bytes!\n",
            size);
    assert(hostPtr != NULL);
  }
  return hostPtr;
...
// memory copy from src to dest, return the pointer to dest. NULL pointer if
// anything is wrong
...
void *xomp_memcpyDeviceToHost(void *dest, const void *src, size_t n) {
  ...
  if (rt == cudaSuccess)
    ...
    fprintf(stderr,
            "Error: cudaMemcpy() failed to copy memory from Device %p to Host "
            "%p, for %zu bytes!\n",
            src, dest, n);
...
// copy a dynamically allocated host source array to a linear dest address on a
// GPU device. The dimension information of the source array is given by: int
// dimensions[dimension_size], with known element size. bytes_copied reports the
// total bytes copied by this function. Liao 4/25/2012
void *xomp_memcpyDynamicHostToDevice(void *dest, const void *src,
                                     int *dimensions, size_t dimension_size,
                                     size_t element_size,
                                     size_t *bytes_copied) {
  assert(dest != NULL);
  assert(src != NULL);
  if (dimension_size == 1) // down to the final, inner-most dimension
  {
    ...
    *bytes_copied =
        element_size * dimensions[0]; // number of elements * element_size
    ...
    size_t sub_array_count =
        dimensions[0]; // top dimension, how many sub-arrays
    ...
    for (i = 0; i < sub_array_count; i++) // for each sub-arrays
    {
      ...
      void *sub_array_src =
          (void *)(array2[i]); // get start address for each sub-array
      assert(sub_array_src != NULL);
      // recursively call to copy each sub-array
      xomp_memcpyDynamicHostToDevice(new_dest, (void *)sub_array_src,
                                     dimensions, dimension_size, element_size,
                                     &subarray_bytes_copied);

      total_subarray_bytes_copied +=
          subarray_bytes_copied; // recursively accumulate the bytes copied,
...
// copy linear src memory to dynamically allocated destination, with dimension
// information given by int dimensions[dimension_size] the source memory has
// total n continuous memory, with known size for each element the total bytes
// copied by this function is reported by bytes_copied test code:
// mallocArray-xomp.cu
void *xomp_memcpyDynamicDeviceToHost(void *dest, int *dimensions,
                                     size_t dimension_size, const void *src,
                                     size_t element_size,
                                     size_t *bytes_copied) {
  ...
  if (dimension_size == 1) // down to the final dimension
  {
    ...
    *bytes_copied =
    ...
    size_t sub_array_count =
        dimensions[0]; // current dimension, how many sub-arrays
    ...
    for (i = 0; i < sub_array_count; i++) // for each sub-arrays
    {
      ...
      xomp_memcpyDynamicDeviceToHost((void *)sub_array_dest, dimensions,
                                     dimension_size, new_src, element_size,
                                     &subarray_bytes_copied);
...
void *xomp_memcpyHostToHost(void *dest, const void *src,
                            size_t n) // same as memcpy??
{
...
//------------------------------------------------------
// free the device memory pointed by a pointer, return false in case of failure,
...
// free the host memory pointed by a pointer, return false in case of failure,
// otherwise return true
...
void *xomp_mallocArray(int *dimensions, size_t dimension_num, size_t esize) {
  ...
  //  if (xomp_verbose)
  //    printf("xomp_xomp_mallocArray(): dimensions=%p dimension =%d element
  ...
  } else // two and more dimensions to be allocated: reduce it to be a 2-step
         // allocation
  {
    ...
    size_t prev_dim_size =
        dimensions[0]; // number of elements of the first dimension

    // 2nd step: allocate the remaining N -1 dimension arrays, each is an
    // element of the first array peel off the 1st(previous) dimension, focus on
    // the rest dimensions
    dimensions++;
...
void xomp_freeArrayPointer(void *array, int *dimensions, size_t dimension_num) {
  ...
  if (dimension_num == 1) {
    free(array);
  } else { // 2-D or more, iterate through higher dimension and try to free
           // inner arrays
...
#define XOMP_BEYOND_BLOCK_REDUCTION_DEF(dtype)                                 \
  dtype xomp_beyond_block_reduction_##dtype(dtype *per_block_results,          \
                                            int numBlocks, int reduction_op) { \
    dtype result;                                                              \
    dtype *per_block_results_cpu =                                             \
        (dtype *)xomp_hostMalloc(numBlocks * sizeof(dtype));                   \
    xomp_memcpyDeviceToHost(per_block_results_cpu, per_block_results,          \
                            sizeof(dtype) * numBlocks);                        \
    int r_i;                                                                   \
...
void copy_mapped_variable(struct XOMP_mapped_variable *desc,
  ...
  // we do not want to inherit the copy directions or map-type of parent DDE's
  // variable OpenMP 4.0 has the reuse enclosing data and discard map-type rule.
  // desc->copyFrom= src ->copyFrom;
}

// create a new DDE-data node and
// append it to the end of the tracking list, and
// copy all variables from its parent node to be into the set of inherited
// variable set.
void xomp_deviceDataEnvironmentEnter(int devID) {
  ...
  data->new_variables = (struct XOMP_mapped_variable *)malloc(
      XOMP_MAX_MAPPED_VARS * sizeof(struct XOMP_mapped_variable));
  data->inherited_variables = (struct XOMP_mapped_variable *)malloc(
      XOMP_MAX_MAPPED_VARS * sizeof(struct XOMP_mapped_variable));
  ...
  // Both new and inherited variables of the parent node become inherited for
  // the current node
  data->inherited_variable_count =
      data->parent->new_variable_count + data->parent->inherited_variable_count;
  data->inherited_variables = (struct XOMP_mapped_variable *)malloc(
      data->inherited_variable_count * sizeof(struct XOMP_mapped_variable));
  assert(data->inherited_variables != NULL);

  int i;
  int offset = 0;
  for (i = 0; i < data->parent->new_variable_count; i++) {
    struct XOMP_mapped_variable *dest_element =
        data->inherited_variables + offset;
    DDE *p = data->parent;
  ...
  for (i = 0; i < data->parent->inherited_variable_count; i++) {
    // copy_mapped_variable(&((data->inherited_variables)[offset]), &(
    // (data->parent->inherited_variables)[i]));
    copy_mapped_variable(
        (struct XOMP_mapped_variable *)(data->inherited_variables + offset),
        (struct XOMP_mapped_variable *)(data->parent->inherited_variables + i));
    offset++;
  }
  assert(offset == data->inherited_variable_count);
}

// Check if an original  variable is already mapped in enclosing data
// environment, return its device variable's address if yes. return NULL if not
void *xomp_deviceDataEnvironmentGetInheritedVariable(int devID, void *orig_var,
                                                     int typeSize, int *size) {
  void *dev_address = NULL;
  assert(orig_var != NULL);
  int i;
  // At this point, DDE list should not be empty
  // At least a call to XOMP_Device_Data_Environment_Enter() should have
  // finished before
  assert(DDE_tail[devID] != NULL);
  for (i = 0; i < DDE_tail[devID]->inherited_variable_count; i++) {
    struct XOMP_mapped_variable *cur_var =
        DDE_tail[devID]->inherited_variables + i;
...
void xomp_deviceDataEnvironmentAddVariable(int devID, void *var_addr,
                                           int *var_size, int *var_offset,
                                           int *var_dim, int nDim, int typeSize,
                                           void *dev_addr, bool copyTo,
                                           bool copyFrom) {
  // TODO: sanity check to avoid add duplicated variable or inheritable variable
  assert(DDE_tail[devID] != NULL);
  struct XOMP_mapped_variable *mapped_var =
      DDE_tail[devID]->new_variables + DDE_tail[devID]->new_variable_count;
...
void xomp_memGatherDeviceToHost(void *dest, void *src, int *vsize, int *voffset,
                                int *vDimSize, int ndim, int typeSize) {
  int offset_src;
  int offset_dest;
  assert(ndim <= 3);
  if (ndim == 1) {
    xomp_memcpyDeviceToHost((char *)dest + voffset[0] * typeSize, (char *)src,
                            vsize[0] * typeSize);
  } else if (ndim == 2) {
    ...
    for (j = 0; j < vsize[0]; ++j) {
      ...
      xomp_memcpyDeviceToHost((char *)dest + offset_dest * typeSize,
                              (char *)src + offset_src * typeSize,
                              vsize[1] * typeSize);
    }
  } else if (ndim == 3) {
    int i, j;
    for (j = 0; j < vsize[2]; ++j) {
      offset_dest =
          voffset[0] +
          vDimSize[0] * (voffset[1] + vDimSize[1] * (j + voffset[2])) -
          vDimSize[0];
...
void xomp_memScatterHostToDevice(void *dest, void *src, int *vsize,
                                 int *voffset, int *vDimSize, int ndim,
                                 int typeSize) {
  int offset_src;
  int offset_dest;
  assert(ndim <= 3);
  if (ndim == 1) {
    xomp_memcpyHostToDevice((char *)dest, (char *)src + voffset[0] * typeSize,
                            vsize[0] * typeSize);
  } else if (ndim == 2) {
    int j;
    for (j = 0; j < vsize[0]; ++j) {
      ...
      xomp_memcpyHostToDevice((char *)dest + offset_dest * typeSize,
                              (char *)src + offset_src * typeSize,
                              vsize[1] * typeSize);
    }
  } else if (ndim == 3) {
    int i, j;
    for (j = 0; j < vsize[2]; ++j) {
      // offset_src = voffset[0] + vDimSize[0]*( voffset[1] + vDimSize[1] * (j +
      // voffset[2]) -1);
      offset_src = (j + voffset[2]) * vDimSize[0] * vDimSize[1] +
                   voffset[1] * vDimSize[0] + voffset[0] - vDimSize[0];
...
void *xomp_deviceDataEnvironmentPrepareVariable(
    int devID, void *original_variable_address, int nDim, int typeSize,
    int *vsize, int *voffset, int *vDimSize, bool copy_into, bool copy_back) {
  // currently only handle one dimension
  void *dev_var_address = NULL;
  dev_var_address = xomp_deviceDataEnvironmentGetInheritedVariable(
      devID, original_variable_address, typeSize, vsize);
  if (dev_var_address == NULL) {
    ...
    xomp_deviceDataEnvironmentAddVariable(
        devID, original_variable_address, vsize, voffset, vDimSize, nDim,
        typeSize, dev_var_address, copy_into, copy_back);
    // The spec says : reuse enclosing data and discard map-type rule.
    // So map-type only matters when no-reuse happens
    if (copy_into) {
      xomp_memScatterHostToDevice(dev_var_address, original_variable_address,
                                  vsize, voffset, vDimSize, nDim, typeSize);
      //  xomp_memcpyHostToDevice(dev_var_address, original_variable_address,
      //  vsize[0]);
    }
  }
...
// Exit current DDE: copy back values if specified, deallocate memory, delete
// the DDE-data node from the end of the tracking list
void xomp_deviceDataEnvironmentExit(int devID) {
  ...
  for (i = 0; i < DDE_tail[devID]->new_variable_count; i++) {
    struct XOMP_mapped_variable *mapped_var =
        DDE_tail[devID]->new_variables + i;
    void *dev_address = mapped_var->dev_address;
    if (mapped_var->copyFrom) {
      xomp_memGatherDeviceToHost(((void *)((char *)mapped_var->address)),
                                 ((void *)((char *)mapped_var->dev_address)),
                                 mapped_var->size, mapped_var->offset,
                                 mapped_var->DimSize, mapped_var->nDim,
                                 mapped_var->typeSize);
      // xomp_memcpyDeviceToHost(((void
      // *)((char*)mapped_var->address+mapped_var->offset[0])),((const void
      // *)mapped_var->dev_address), mapped_var->size[0]);
    }
    // free after copy back!!
    xomp_freeDevice(
        dev_address); // TODO Will this work without type info? Looks so!
  }
