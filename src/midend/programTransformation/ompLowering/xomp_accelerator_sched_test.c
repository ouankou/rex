...
(loop_chunk_size)
*/
int XOMP_static_sched_next(int *loop_sched_index, int loop_end, int loop_stride,
...
void OUT__2__10550__(int n, int *_dev_u) {
  ...
  while (XOMP_static_sched_next(
      &_dev_loop_sched_index, orig_end, _dev_loop_stride, _dev_loop_chunk_size,
...
int main() {
  ...
  for (i = 0; i < SIZE; i++)
    a[i] =
        SIZE -
        i; // reverse order to make sure no default values are messing up things
