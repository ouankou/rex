...
SgType *arm_get_type(SgType *input, SgBasicBlock *new_block) {
  switch (input->variantT()) {
  case V_SgTypeInt:
    return buildOpaqueType("svint32_t", new_block);
  case V_SgTypeFloat:
    return buildOpaqueType("svfloat32_t", new_block);
  case V_SgTypeDouble:
    return buildOpaqueType("svfloat64_t", new_block);
...
void omp_simd_write_arm(SgOmpSimdStatement *target, SgForStatement *for_loop,
  ...
  SgExprStatement *test_stmt =
      static_cast<SgExprStatement *>(for_loop->get_test());
  ...
  SgExpression *predicate = buildFunctionCallExp(
      pred_func_name, pred_type, parameters, target->get_scope());
  SgAssignInitializer *init = buildAssignInitializer(predicate);

  SgVariableDeclaration *vd =
      buildVariableDeclaration(pg_name, pred_type, init, new_block);
  Rose_STL_Container<SgNode *> bodyList = NodeQuery::querySubTree(
      getEnclosingScope(target), V_SgVariableDeclaration);
  ...
  SgExpression *inc_fc = buildFunctionCallExp(pred_count_name, buildIntType(),
                                              NULL, target->get_scope());
  ...
  for (Rose_STL_Container<SgNode *>::iterator i = ir_block->begin();
    ...
    case V_SgSIMDGather: {
      ...
      SgExpression *ld = buildFunctionCallExp("svld1", mask_type, parameters,
                                              target->get_scope());
    ...
    case V_SgSIMDStore: {
      ...
      SgExprStatement *str = buildFunctionCallStmt(
          "svst1", buildVoidType(), parameters, target->get_scope());
    ...
    case V_SgSIMDPartialStore: {
      ...
      if (std::find(arm_partial_broadcasts.begin(),
        ...
        SgExpression *ld = buildFunctionCallExp(
            func_name, vector_type, parameters, target->get_scope());
        SgAssignInitializer *local_init = buildAssignInitializer(ld);
    ...
    case V_SgSIMDScalarStore: {
      ...
      SgFunctionCallExp *reductionCall = buildFunctionCallExp(
          "svaddv", scalar->get_type(), parameters, target->get_scope());
