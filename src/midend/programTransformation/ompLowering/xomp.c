...
char *current_time_to_str(void) {
  ...
  assert(tm != NULL);
  snprintf(timestamp, TIMESTAMP_SIZE, "%04d_%02d_%02d_%02d_%02d_%02d",
           tm->tm_year + 1900, tm->tm_mon + 1, // month starts from 0
...
void xomp_terminate(int *exitcode) { XOMP_terminate(*exitcode); }
...
void xomp_parallel_start(void (*func)(void *), unsigned *ifClauseValue,
                         unsigned *numThread, int *argcount, ...);
// Wrapper functions to support linking with Fortran programs
//
//  OFP does not support extensions like %VAL, so we use XOMP to compensate the
//  difference between pass-by-reference vs. pass-by-value.
// void xomp_parallel_start (void (*func) (void *), void *data, unsigned
// numThread)
#pragma weak xomp_parallel_start_ = xomp_parallel_start
// void xomp_parallel_start (void (*func) (void *), void *data, unsigned*
// numThread)
//
//  We expect the outlined function from a Fortran task will have multiple
//  parameters passed by references This wrapper function will pack those
//  parameters into a single one to be compatible with gomp's parallel_start
//  function A glue part from Fortran's multiple parameters --> XOMP's single
//  parameter
void xomp_parallel_start(void (*func)(void *), unsigned *ifClauseValue,
                         unsigned *numThread, int *argcount, ...) {
  int x;
  va_list v1;
  int p_offset = 1; // the first position is reserved to the function pointer to
                    // the outlined Fortran subroutine
  ...
  //  {
  //    printf("xomp.c xomp_parallel_start(), malloc failed for %d
  //    parameters.\n", parameter_count); exit (3);
  //  }
...
void XOMP_parallel_start(void (*func)(void *), void *data,
  ...
  //  It is only used to indicate a combination of the NUM_THREADS clause and
  //  the IF clause. GOMP has an internal function (gomp_resolve_num_threads())
  //  to determine the actual number of threads to be used later on.
  if (!ifClauseValue)
    numThread = 1;
  else
...
// C/C++ support
/* Initialize sections and return the next section id (starting from 0) to be
...
/* Return the next section id (starting from 1) to be executed by the current
 * thread, value 0 means no sections left */
int xomp_sections_next(void);
#pragma weak xomp_sections_next_ = xomp_sections_next
int xomp_sections_next(void) { return XOMP_sections_next(); }
int XOMP_sections_next(void) {
  int result = -1;
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
...
void xomp_sections_end(void) { XOMP_sections_end(); }
/* Called after the current thread is told that all sections are executed. It
 * synchronizes all threads also. */
void XOMP_sections_end(void) {
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  GOMP_sections_end();
#else
...
void xomp_sections_end_nowait(void) { XOMP_sections_end_nowait(); }
/* Called after the current thread is told that all sections are executed. It
 * does not synchronizes all threads. */
void XOMP_sections_end_nowait(void) {
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
...
#define MAX_XOMP_TASK_NUMBER                                                   \
  99999 // cannot be too large, otherwise exceed static allocation limit
...
// Main tasks:
//   Convert Fortran parameters to the right C ones so XOMP_task() can handle
//   them properly
...
//     Especially, the values/addresses must be copied into task data when tasks
//     are created. Since a task's parent function stack may not exist when a
//     task is being executed.
//
//  Solution:
//   Compiler provides all information about the nature (value vs. reference) of
//   parameter, size, and where to get the parameter. The XOMP layer encodes all
//   information into a single data segment, which will be copied into each task
//   by gomp_task() etc. The XOMP layer also provides function (run_me_task_ ())
//   to decode the data segment, and call the outlined Fortran subroutine.
//
//   Wrap function pointer to a outlined function routine and all its parameters
//   into one single parameter gp_parameter[] The single data segment has the
//   following layout
...
//          1 byte for a boolean value to indicate if the parameter should be
//          passed by value 4 bytes for the size of the parameter, size should
//          be equal to sizeof (void*) if not pass-by-value ( pointer type
//          instead) x bytes for value or address, x will be the size provide by
//          the 2nd element of the triplet.
//  For example, if we have a outlined function with two parameters:
//     an integer index suitable for pass-by-value, This is very typical for a
//     task's firstprivate variables. an array pass-by-reference,
//  The data segment to be created will contain (assuming 64-bit target):
//      8 bytes (function pointer),
//      1 byte(byValue=true),  4 bytes (int value),           8 bytes(address of
//      the index) 1 byte(byValue=false), 8 bytes (address of the array), 8
//      bytes(address of the array)
//  Total   38 bytes for the data segment
//
//  Finally, we can call the real xomp_task () with the bridge run_me_task_#()
//  function
void xomp_task(void (*func)(void *), void (*cpyfn)(void *, void *),
               int *arg_size, int *arg_align, int *if_clause, int *untied,
               int *argcount, ...) {
  ...
  // The 0 integer generated by the lowering process will be passed by its
  // address, which is not NULL. So I set it to 0 here since this is the only
  // reasonable value for the translation we use.
  // TODO find a better way to handle cpyfn pointer when it is NULL, memcpy?
  cpyfn = 0;
  long largcount =
      (long)(*argcount) /
      3; // divide by 3 to get real number since we pass each parameter with a
         // triplet form(by-value, value-size, address)
  ...
  // simplest case where no shared variables to be passed.
  if (largcount == 0) {
    // void XOMP_task (void (*fn) (void *), void *data, void (*cpyfn) (void *,
    // void *), long arg_size, long arg_align, bool if_clause, unsigned untied)
    //    printf("xomp.c xomp_task(), 0 variables to be passed. func = %p data =
    //    %p, cpyfn = %p\n", func, NULL, cpyfn );
    XOMP_task(func, NULL, cpyfn, larg_size, larg_align, bif_clause, uuntied);
    return;
  }

  // Common case where there are one or more private/shared variables to be
  // passed Adjust parameter count and size: a function pointer + its original
  // parameters
  int parameter_count = largcount + 1; // todo REMOVE this variable

  // TODO: iteration here to calculate real size, or let compiler calculate it
  // and pass it here assume 512 bytes for now
  larg_size = 512; // 40+ 4+ 8 = 52, larg_size + sizeof(void*);  // variable
                   // size + function_pointer size
  // We need to use dedicated private g_parameter for each task.
  // Single the parent function of tasks may die out before children tasks begin
  // execution: untied case
  char *pg_parameter =
      (char *)malloc(larg_size); // allocate 8+4 =12 bytes for pg_parameter,
                                 // this is the data to be coped into each task!
  ...
  if (pg_parameter == (void *)0) {
    ...
    //    printf("Debug: xomp_task(), allocated pg_parameter at address %p ,
    //    void* address %p, for func %p \n", pg_parameter, (void*)pg_parameter,
    //    func);
  }

  // first parameter ----------------
  // Fill in the Fortran subroutine pointer, which will be size(void*)
  memcpy(pg_parameter, &func,
         sizeof(void *)); // copy function pointer to pg_parameter
  p_offset +=
      sizeof(void *); // the two position (8 bytes) is reserved to the function
                      // pointer to the outlined Fortran subroutine
  ...
  for (x = 0; x < largcount; x++) {
    ...
    memcpy(&(pg_parameter[p_offset]), &bValue,
           sizeof(bool)); // we use 1 byte to store the by-value flag
    p_offset += sizeof(bool);

    // grab 2nd of the triplet: size of the value/address
    int v_size = *(int *)(va_arg(v1, void *));
    if (!by_value) { // verify size of address
      assert(v_size == sizeof(void *));
    }
    memcpy(&(pg_parameter[p_offset]), &v_size,
           sizeof(int)); // we use 4 byte to store the size
    p_offset += sizeof(int);

    // 3rd of the triplet: address (of the value) : store value or address as
    // required
    if (by_value) {
      char *i = (char *)(va_arg(v1, void *)); // every parameter is a pointer to
                                              // something from Fortran to xomp
      // in this case, the pointer points to a value, we just copy the pointed
      // value bytes
  ...
  switch (largcount) {
    //   XOMP_task(run_me_task_?, (void*) pg_parameter, cpyfn, *arg_size,
    //   *arg_align, *if_clause, *untied);
...
void XOMP_task(void (*fn)(void *), void *data, void (*cpyfn)(void *, void *),
  ...
  // It is fine to have older gcc to compile this, just remember to link with
  // gcc 4.4 and beyond
  // #if __GNUC__ > 4 || \  //
...
#define CHECK_SIGNED_INT_RANGE(x)                                              \
  assert((x >= MIN_SIGNED_INT) && (x <= MAX_SIGNED_INT))

// -------default scheduling ----------------------
// Accommodate Fortran issues: underscore, small case, pass-by-reference
void xomp_loop_default(int *lower, int *upper, int *stride, int *n_lower,
                       int *n_upper);
#pragma weak xomp_loop_default_ = xomp_loop_default
void xomp_loop_default(
    int *lower, int *upper, int *stride, int *n_lower,
    int *n_upper) { // deal with mismatch between int and long int.
...
extern void XOMP_loop_default(unsigned int lower, unsigned int upper,
  ...
  int _p_upper;
  int _p_chunk_size;
  int addOne; // adjustment to input and output upper bounds, depending on if
              // they are inclusive or non-inclusive
  ...
  if (isDecremental == 1) {
    ...
    // stride = 0 - stride;  // n
    if (stride > 0) {
      printf("Error: in XOMP_loop_default() of xomp.c: stride must be negative "
             "for decremental iteration. stride = %d \n ",
             stride);
      ROSE_ABORT();
    }
  } else // incremental
  {
    ...
    if (stride < 0) {
      printf("Error: in XOMP_loop_default() of xomp.c: stride must be positive "
             "for incremental iteration. stride = %d \n ",
             stride);
      ROSE_ABORT();
    }
  }
...
void xomp_loop_static_init(int *lower, int *upper, int *stride,
                           int *chunk_size);
...
#else
  // adjust inclusive upper bounds of XOMP to non inclusive bounds of GOMP and
  // OMNI
  if (stride > 0)
...
//---------- dynamic--------------
// Glue to support Fortran
void xomp_loop_dynamic_init(int *lower, int *upper, int *stride,
                            int *chunk_size);
...
//---------- guided--------------
// Glue to support Fortran
void xomp_loop_guided_init(int *lower, int *upper, int *stride,
                           int *chunk_size);
...
//---------------- init ordered -----------------------------------------
//----------- static
void xomp_loop_ordered_static_init(int *lower, int *upper, int *stride,
                                   int *chunk_size);
...
void XOMP_loop_ordered_static_init(int lower, int upper, int stride,
  ...
  if (chunk_size == 0)
    chunk_size = 1; // TODO calculate the default static chunk size here
                    //  Omni actually has another simpler way to support ordered
                    //  default scheduling we use ordered static scheduling to
                    //  have a uniform translation as the one supporting GOMP
  _ompc_static_sched_init(lower, upper, stride, chunk_size);
#endif
}

//-----------dynamic
void xomp_loop_ordered_dynamic_init(int *lower, int *upper, int *stride,
                                    int *chunk_size);
...
//-----------guided
void xomp_loop_ordered_guided_init(int *lower, int *upper, int *stride,
                                   int *chunk_size);
...
//----------------------  start---------------------------------
// -----------  static
bool xomp_loop_static_start(int *start, int *end, int *incr, int *chunk_size,
                            int *istart, int *iend);
...
bool xomp_loop_static_start(int *start, int *end, int *incr, int *chunk_size,
  ...
  bool rt;
  rt = XOMP_loop_static_start(*start, *end, *incr, *chunk_size, &l_istart,
                              &l_iend);
  if (rt) {
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_static_start() start=%ld , end=%ld, rt =%d size
  //  is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_static_start(long start, long end, long incr, long chunk_size,
                            long *istart, long *iend) {
  bool rt;
  long lend;

  // convert inclusive bounds of XOMP to non-inclusive upper bound from
  // GOMP/OMNI
  if (incr > 0)
    end++;
  else
  ...
  // convert non-inclusive upper bound from GOMP/OMNI to inclusive bounds of
  // XOMP
  if (incr > 0)
    *iend = lend - 1;
  else
...
// -----------  dynamic
bool xomp_loop_dynamic_start(int *start, int *end, int *incr, int *chunk_size,
                             int *istart, int *iend);
...
bool xomp_loop_dynamic_start(int *start, int *end, int *incr, int *chunk_size,
  ...
  long l_istart, l_iend;
  bool rt;
  rt = XOMP_loop_dynamic_start(*start, *end, *incr, *chunk_size, &l_istart,
                               &l_iend);
  if (rt) {
    CHECK_SIGNED_INT_RANGE(l_istart);
    CHECK_SIGNED_INT_RANGE(l_iend);
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_dynamic_start() start=%ld , end=%ld, rt =%d size
  //  is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_dynamic_start(long start, long end, long incr, long chunk_size,
                             long *istart, long *iend) {
  bool rt;
  long lend;
...
// -----------  guided
bool xomp_loop_guided_start(int *start, int *end, int *incr, int *chunk_size,
                            int *istart, int *iend);
...
bool xomp_loop_guided_start(int *start, int *end, int *incr, int *chunk_size,
  ...
  long l_istart, l_iend;
  bool rt;
  rt = XOMP_loop_guided_start(*start, *end, *incr, *chunk_size, &l_istart,
                              &l_iend);
  if (rt) {
    CHECK_SIGNED_INT_RANGE(l_istart);
    CHECK_SIGNED_INT_RANGE(l_iend);
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_guided_start() start=%ld , end=%ld, rt =%d size
  //  is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_guided_start(long start, long end, long incr, long chunk_size,
                            long *istart, long *iend) {
  bool rt;
  long lend;
...
// -----------  runtime
bool xomp_loop_runtime_start(int *start, int *end, int *incr, int *istart,
                             int *iend);
...
bool xomp_loop_runtime_start(int *start, int *end, int *incr, int *istart,
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_runtime_start() start=%ld , end=%ld, rt =%d size
  //  is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_runtime_start(long start, long end, long incr, long *istart,
                             long *iend) {
  bool rt;
  long lend;
...
//----------------------------------ordered start --------------------------
// -----------  ordered_static
bool xomp_loop_ordered_static_start(int *start, int *end, int *incr,
...
bool xomp_loop_ordered_static_start(int *start, int *end, int *incr,
  ...
  long l_istart, l_iend;
  bool rt;
  rt = XOMP_loop_ordered_static_start(*start, *end, *incr, *chunk_size,
                                      &l_istart, &l_iend);
  if (rt) {
    CHECK_SIGNED_INT_RANGE(l_istart);
    CHECK_SIGNED_INT_RANGE(l_iend);
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_ordered_static_start() start=%ld , end=%ld, rt
  //  =%d size is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_ordered_static_start(long start, long end, long incr,
...
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  rt = GOMP_loop_ordered_static_start(start, end, incr, chunk_size, istart,
                                      &lend);
#else
  rt = _ompc_static_sched_next((int *)istart, (int *)(&lend));
#endif
...
// -----------  ordered_dynamic
bool xomp_loop_ordered_dynamic_start(int *start, int *end, int *incr,
                                     int *chunk_size, int *istart, int *iend);
...
bool xomp_loop_ordered_dynamic_start(int *start, int *end, int *incr,
  ...
  long l_istart, l_iend;
  bool rt;
  rt = XOMP_loop_ordered_dynamic_start(*start, *end, *incr, *chunk_size,
                                       &l_istart, &l_iend);
  if (rt) {
    CHECK_SIGNED_INT_RANGE(l_istart);
    CHECK_SIGNED_INT_RANGE(l_iend);
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_ordered_dynamic_start() start=%ld , end=%ld, rt
  //  =%d size is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_ordered_dynamic_start(long start, long end, long incr,
...
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  rt = GOMP_loop_ordered_dynamic_start(start, end, incr, chunk_size, istart,
                                       &lend);
#else
  rt = _ompc_dynamic_sched_next((int *)istart, (int *)(&lend));
#endif
...
// -----------  ordered_guided
bool xomp_loop_ordered_guided_start(int *start, int *end, int *incr,
                                    int *chunk_size, int *istart, int *iend);
...
bool xomp_loop_ordered_guided_start(int *start, int *end, int *incr,
  ...
  long l_istart, l_iend;
  bool rt;
  rt = XOMP_loop_ordered_guided_start(*start, *end, *incr, *chunk_size,
                                      &l_istart, &l_iend);
  if (rt) {
    CHECK_SIGNED_INT_RANGE(l_istart);
    CHECK_SIGNED_INT_RANGE(l_iend);
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_ordered_guided_start() start=%ld , end=%ld, rt
  //  =%d size is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}
bool XOMP_loop_ordered_guided_start(long start, long end, long incr,
                                    long chunk_size, long *istart, long *iend) {
  bool rt;
  long lend;
...
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  rt = GOMP_loop_ordered_guided_start(start, end, incr, chunk_size, istart,
                                      &lend);
#else
  rt = _ompc_guided_sched_next((int *)istart, (int *)(&lend));
#endif
...
// -----------  ordered_runtime
bool xomp_loop_ordered_runtime_start(int *start, int *end, int *incr,
                                     int *istart, int *iend);
...
bool xomp_loop_ordered_runtime_start(int *start, int *end, int *incr,
  ...
  *istart = l_istart;
  *iend = l_iend;
  //  printf ("debug: xomp_loop_ordered_runtime_start() start=%ld , end=%ld, rt
  //  =%d size is=%lu \n", l_istart, l_iend, rt,sizeof(rt));
  return rt;
}

bool XOMP_loop_ordered_runtime_start(long start, long end, long incr,
                                     long *istart, long *iend) {
  // DQ (12/10/2016): We should discuss this since it is not initialized when
  // USE_ROSE_GOMP_OPENMP_LIBRARY is not defined. bool rt ;
  bool rt = false;

  // DQ (12/10/2016): Eliminating a warning that we want to be an error:
  // -Werror=maybe-uninitialized. long lend;
  long lend = 0;
...
bool xomp_loop_static_next(int *l, int *u) {
  ...
  if (rt) { // avoid assertion on garbage values
...
extern void XOMP_static_even_divide(long start, long orig_size, int chunk_count,
  ...
  if (chunk_id >= remainder)
    offset +=
        remainder; // all extra elements on top of base line offset size*tid
  else
...
void xomp_loop_end(void) { XOMP_loop_end(); }
void XOMP_loop_end(void) {
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  GOMP_loop_end();
#else
...
void xomp_loop_end_nowait(void) { XOMP_loop_end_nowait(); }
...
void xomp_barrier(void) { XOMP_barrier(); }
void XOMP_barrier(void) {
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  GOMP_barrier();
#else
...
// We generate a default name for it and use the named critical support function
// instead to be consistent with OMNI
...
bool xomp_single(void) { return XOMP_single(); }
...
bool xomp_master(void) { return XOMP_master(); }
...
void xomp_atomic_start(void) { XOMP_atomic_start(); }
void XOMP_atomic_start(void) {
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  GOMP_atomic_start();
#else
...
void xomp_atomic_end(void) { XOMP_atomic_end(); }
void XOMP_atomic_end(void) {
#ifdef USE_ROSE_GOMP_OPENMP_LIBRARY
  GOMP_atomic_end();
#else
