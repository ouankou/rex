// Liao 8/30/2013
// A dedicated self-contained file to test a scheduler using round-robin method
// across multiple threads v1: using exclusive upper bounds V2: using inclusive
// upper bounds
//
...
*/
void XOMP_static_sched_init(int lb, int up, int step, int orig_chunk_size,
                            int _p_num_threads, int _p_thread_id,
                            int *loop_chunk_size, int *loop_sched_index,
                            int *loop_stride) {
...
(loop_chunk_size)
*/
int XOMP_static_sched_next(int *loop_sched_index, int loop_end, int orig_step,
                           int loop_stride, int loop_chunk_size,
                           int _p_num_threads, int _p_thread_id, int *lb,
                           int *ub) {
  int b, e;
  b = *loop_sched_index;
...
void OUT__2__10550__(int n, int *_dev_u) {
  ...
  // variables for adjusted loop info considering both original chunk size and
  // step(strip)
  ...
  while (XOMP_static_sched_next(&_dev_loop_sched_index, orig_end, orig_step,
                                _dev_loop_stride, _dev_loop_chunk_size,
                                _dev_thread_num, _dev_thread_id, &_dev_lower,
                                &_dev_upper)) {
    printf("Thread ID: %d Allocated lower = %d upper = %d\n", _dev_thread_id,
           _dev_lower, _dev_upper);
    for (ij = _dev_lower; ij <= _dev_upper;
         ij++) { // using inclusive bound here
      _dev_u[ij] += (n - ij);
    }
  }
}
...
int main() {
  ...
  for (i = 0; i <= SIZE - 1; i++) {
    ...
    b[i] = 0;
    c[i] = 0;
    a[i] += (SIZE - i); // reverse order to make sure no default values are
                        // messing up things futher using += to catch duplicated
                        // execution of one iteartion!
  }
