...
TODO: extend to support 3-D mapping case, which should be trivial based on 2-D
implementation
...
/*  reduction minus is handled the same way as reduction plus since we just
   replace the reduction variable with its local copy for each thread The
   associated statement is intact except for the variable replacement : e.g.
   a-=5 becomes local_a -= 5; in the end of each thread accumulates thread local
   negative values. At the block level, we just simply add them all to be the
   block level negative values
*/
/* we have to encode the type into function name since C function signature does
 * not include parameter list! */
/* grid_level_results: across blocks, each block has a result inside this array
 * of size = gridDim.x * gridDim.y */
#define XOMP_INNER_BLOCK_REDUCTION_DEF(dtype)                                  \
  __device__ void xomp_inner_block_reduction_##dtype(                          \
      dtype local_value, dtype *grid_level_results, int reduction_op) {        \
    /* To speedup reduction, we transfer local_value to a shared data within   \
     * the block */                                                            \
    /* __shared__ float* sdata[blockDim.x * blockDim.y * blockDim.z]; not      \
     * compilable */                                                           \
    /* block size of data, size is specified by the kernel launch parameter    \
     * (3rd one) */                                                            \
    /* shared data has to have different names for different types. Cannot     \
     * reuse name across types. */                                             \
    ...
    for (int offset = nn / 2; offset > 0; /* folding and add */                \
    ...
    {                                                                          \
      if (ii < offset) {                                                       \
        ...
        default: {                                                             \
          ...
          /* fprintf (stderr, "Error. xomp_inner_block_reduction() unhandled   \
           * reduction operation:%d\n",reduction_op); */                       \
          /* ROSE_ABORT(); */                                                  \
        }                                                                      \
        } /* end switch */                                                     \
      }                                                                        \
      ...
      /* MUST wait until all threads in the block have updated their partial   \
       * sums */                                                               \
      __syncthreads(); /* sync after each folding */                           \
    }                                                                          \
    /* thread (0,0,0) writes the partial sum of this thread block to grid      \
     * level results (linearized also)*/                                       \
    if (ii == 0) {                                                             \
      grid_level_results[blockIdx.x * gridDim.y + blockIdx.y] =                \
          sdata_##dtype[0];                                                    \
    }                                                                          \
  }
...
//  It turns out that evenly dividing up iteration space for GPU threads is not
//  optimal. Each thread may touch a large range (chunk) of data elements ,
//  which has low memory access efficiency (cannot be coalesced) A better
//  solution is to use round -robin scheduling, similar to the static scheduling
//  with chunk size 1 in regular OpenMP CPU threads loops
__device__ void XOMP_cuda_loop_default_internal(int lower, int upper,
                                                int stride, int _p_num_threads,
                                                int _p_thread_id, long *n_lower,
                                                long *n_upper) {
  int _p_lower;
  int _p_upper;
  int _p_chunk_size;
  ...
  if (isDecremental == 1) {
    ...
    if (stride > 0) {
      /*  TODO: assertion and error code
            printf("Error: in XOMP_loop_default() of xomp.c: stride must be
         negative for decremental iteration. stride = %d \n ", stride);
            ROSE_ABORT();
      */
    }
  } else // incremental
  {
    ...
    if (stride < 0) {
      /* TODO assertion and error code
            printf("Error: in XOMP_loop_default() of xomp.c: stride must be
         positive for incremental iteration. stride = %d \n ", stride);
            ROSE_ABORT();
      */
    }
  }
...
/* This is the top level function which decides on the lower and upper bounds
  for the current thread id and current block id It internally calls
  XOMP_cuda_loop_default_internal twice to figure this out: once for the block's
  bounds, the other for the thread's bounds
*/
__device__ void XOMP_accelerator_loop_default(int lower, int upper, int stride,
                                              long *n_lower, long *n_upper) {
  ...
  XOMP_cuda_loop_default_internal(lower, upper, stride, gridDim.x, blockIdx.x,
                                  &lower_for_block, &upper_for_block);

  /* now focus on the bounds of the current thread of the current block */
  XOMP_cuda_loop_default_internal(lower_for_block, upper_for_block, stride,
                                  blockDim.x, threadIdx.x, n_lower, n_upper);
}

/*
_p_num_threads: number of threads of the thread team participating the
scheduling _p_thread_id: the current thread's id within the current team
...
*/
__device__ void XOMP_static_sched_init(int lb, int up, int step,
                                       int orig_chunk_size, int _p_num_threads,
                                       int _p_thread_id, int *loop_chunk_size,
...
/*
Using current thread ID (_p_thread_id) and team size (_p_num_threads), calculate
lb and ub for the current thread for the round robin scheduling with lower
(loop_sched_index), upper (loop_end) , stride (loop_stride), and chunk size
(loop_chunk_size)
*/
__device__ int XOMP_static_sched_next(int *loop_sched_index, int loop_end,
                                      int orig_step, int loop_stride,
                                      int loop_chunk_size, int _p_num_threads,
                                      int _p_thread_id, int *lb, int *ub) {
  int b, e;
  b = *loop_sched_index;
  // The code logic is original for exclusive upper bound!!
  //  But in ROSE, we normalize all loops to be inclusive bounds. So we have to
  //  ajust them in the functions, instead of during transformation.
  //
  //  1. adjust the original loop end from inclusive to be exclusive.
  if (orig_step > 0)
    loop_end++; // expect the user code will use the upper bound as an inclusive
                // one, so minus one in advance
  else
    loop_end--;
  ...
  if (_p_num_threads == 1) { /* not in parallel */
    ...
    *loop_sched_index = e;
    if (orig_step > 0)
      *ub--; // expect the user code will use the upper bound as an inclusive
             // one, so minus one in advance
    else
      *ub++;
    return 1;
  ...
  *loop_sched_index += loop_stride;
  ...
  e = b + loop_chunk_size;
  if (orig_step > 0)
    e--; // expect the user code will use the upper bound as an inclusive one,
         // so minus one in advance
  else
    e++;
...
// since there are three dimensions x, y, z. we use dimension_no to indicate
// which dimension is requested. dimension_no start from 1 to 3, corresponding
// to x, y, z dimensions.
__device__ int getLoopIndexFromCUDAVariables(int dimension_no) {
  if (dimension_no == 1)
    ...
    // printf("getLoopIndexFromCUDAVariables() accept a parameter of range from
    // 1 to 3 only\n"); ROSE_ABORT();
  }
  return -1;
}

// A wrapper function for gridDim.x * blockDim.x, to hide CUDA variables
// gridDim.x and blockDim.x.
__device__ int getCUDABlockThreadCount(int dimension_no) {
  if (dimension_no == 1)
    ...
    // printf("getCUDABlockThreadCount() accept a parameter of range from 1 to 3
    // only\n"); ROSE_ABORT();
  }
  return -1;
}
