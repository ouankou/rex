...
#  echo "  func =(FUNC_P) (((void**)data)[0]);"
  ...
  echo "     if (v_size <= 0) {"
  echo "       fprintf (stderr, \"Fatal error in xomp, run_me_task_(). Invalid value size %d.\\n\", v_size);"
  echo "       ROSE_ABORT();"
  echo "     }"
  echo "     "
  ...
  echo "       if (v_size != (int)sizeof (void*)) {"
  echo "         fprintf (stderr, \"Fatal error in xomp, run_me_task_(). Invalid reference size %d.\\n\", v_size);"
  echo "         ROSE_ABORT();"
  echo "       }"
  echo "       memcpy (&(p[i]), &( ((char*)data)[offset]), v_size);"
  echo "     }"
