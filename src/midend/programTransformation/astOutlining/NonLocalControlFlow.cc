 ...
 */
static SgVariableDeclaration *createIntDecl(const string &name, int val = 0,
 ...
 *      JUMP[end-1];
 */
static SgIfStmt *createJumpTable(SgVariableSymbol *jump_var,
  ...
  // Assemble final 'if'
  SgIfStmt *new_jump =
      SageBuilder::buildIfStmt(cond_stmt, true_body, false_body);
 ...
 *    }
 */
static SgBasicBlock *createJumpReplacementGoto(SgVariableSymbol *jump_var,
                                               size_t jump_id,
 ...
 */
static void convertJumpsToGotos(SgVariableSymbol *jump_var,
                                ASTtools::JumpMap_t &jumps,
                                SgLabelStatement *local_target) {
  for (ASTtools::JumpMap_t::iterator i = jumps.begin(); i != jumps.end(); ++i) {
    SgStatement *jump_stmt = const_cast<SgStatement *>(i->first);
    ROSE_ASSERT(jump_stmt);
    size_t jump_id = i->second;

    SgBasicBlock *new_block =
        createJumpReplacementGoto(jump_var, jump_id, local_target);
...
/*
Transformation to replace a local variable used by return with a variable with
higher scope.
...
TODO: what if it is not?  We should not outline code blocks with such return
statements.
...
public:
  ...
  int process() {
    ...
    for (size_t i = 0; i < stmts.size(); i++)
      if (normalizeReturnStatement(stmts[i], _scope))
        count++;
...
private:
  ...
  bool normalizeReturnStatement(SgReturnStmt *stmt, SgScopeStatement *scope) {
    ...
    // iterate all nodes within RHS expression, find scopes lower than the
    // reference scope
    ...
    for (RoseAst::iterator i = ast.begin(); i != ast.end(); ++i) {
      if (SgVarRefExp *var_ref = isSgVarRefExp(*i)) {
        ...
        // if equal, we don't need to introduce the normalization, scope here
        // mostly is the BB to be outlined. and preprocessed to support
        // non-local jumps.
        if (isAncestor(
                scope,
                iname_scope)) // strict ancestor, equal scopes are not
                              // considered strict ancestor-descendant relation
    ...
    if (needNormalization) {
      ...
      string temp_name =
          "__rt_temp_" + Rose::StringUtility::numberToString(temp_count);
      ...
      SgExprStatement *assign_stmt = buildAssignStatement(
          buildVarRefExp(temp_decl), deepCopy(stmt->get_expression()));
...
Outliner::Preprocess::transformNonLocalControlFlow(SgBasicBlock *b_orig) {
  ...
  SgStatement *null_stmt =
      SageBuilder::buildNullStatement(); // Liao 12/18/2012, needed to unparse ;
