...
void CPreproc::IfDirectiveExtractor::visitTopDown(SgNode *n) { visit(n, true); }

void CPreproc::IfDirectiveExtractor::visitBottomUp(SgNode *n) {
  visit(n, false);
}
...
CPreproc::If::Case *CPreproc::IfDirectiveExtractor::getCurrentContext(void) {
  return pending_.empty() ? 0 : pending_.top()->lastCase();
}

void CPreproc::IfDirectiveExtractor::validateNonEmptyStack(
    const PreprocessingInfo *info, const SgLocatedNode *node) const {
...
void CPreproc::IfDirectiveExtractor::visit(SgNode *n, bool isTopDown) {
...
void CPreproc::IfDirectiveExtractor::handleCase(PreprocessingInfo *info,
                                                SgLocatedNode *node) {
...
void CPreproc::IfDirectiveExtractor::newIf(PreprocessingInfo *info,
                                           SgLocatedNode *node) {
  // Create the new '#if' representation.
  If *new_if = new If;
  ROSE_ASSERT(new_if);
  // Liao, 3/11/2008. eliminate the side effect of assertion, fix a bug caused
  // by NDEBUG
...
void CPreproc::IfDirectiveExtractor::addCase(PreprocessingInfo *info,
                                             SgLocatedNode *node) {
...
void CPreproc::IfDirectiveExtractor::checkLastCaseNotEndif(const If *i) {
  if (i) {
