...
CPreproc::IfDirectiveContextFinder::IfDirectiveContextFinder(
    CPreproc::Ifs_t &ifs, const SgLocatedNode *target)
    : IfDirectiveExtractor(ifs), target_(target), top_(0), bottom_(0) {}

void CPreproc::IfDirectiveContextFinder::visitTopDown(SgNode *n) {
  IfDirectiveExtractor::visitTopDown(n);
  if (isSgLocatedNode(n) == target_)
    top_ = getCurrentContext();
}

void CPreproc::IfDirectiveContextFinder::visitBottomUp(SgNode *n) {
  if (isSgLocatedNode(n) == target_)
    bottom_ = getCurrentContext();
  IfDirectiveExtractor::visitBottomUp(n);
}

CPreproc::If::Case *CPreproc::IfDirectiveContextFinder::getContextTop(void) {
  return top_;
}
...
CPreproc::If::Case *CPreproc::IfDirectiveContextFinder::getContextBottom(void) {
  return bottom_;
}
