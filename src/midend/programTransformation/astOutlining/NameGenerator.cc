...
NameGenerator::NameGenerator(const string &pre, size_t first_id,
                             const string &suf)
...
size_t NameGenerator::id(std::string full_file_name) {
...
string NameGenerator::prefix(void) const { return prefix_; }

string NameGenerator::suffix(void) const { return suffix_; }

string NameGenerator::current(std::string full_file_name) {
  stringstream s;
  s << prefix() << id(full_file_name) << suffix();
  return s.str();
}

string NameGenerator::next(std::string full_file_name) {
  // First time to get id?
  if (cur_id_.count(full_file_name) == 0)
    cur_id_[full_file_name] = first_id_;
