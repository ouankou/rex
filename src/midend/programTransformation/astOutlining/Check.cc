...
// =====================================================================
static bool checkType(const SgStatement *s) {
  if (s)
    switch (s->variantT()) {
...
bool isHiddenType(const SgType *type) {
  if (!type)
    return false;
 ...
 */
static bool doesCall__builtin_va_start(const SgStatement *s) {
  typedef Rose_STL_Container<SgNode *> NodeList_t;
  NodeList_t refs =
      NodeQuery::querySubTree(const_cast<SgStatement *>(s), V_SgFunctionRefExp);
  for (NodeList_t::const_iterator i = refs.begin(); i != refs.end(); ++i) {
 ...
 *  obsolete.
 */
static bool hasVAStart(const SgStatement *s) {
  typedef Rose_STL_Container<SgNode *> NodeList_t;
  NodeList_t L =
      NodeQuery::querySubTree(const_cast<SgStatement *>(s), V_SgVarArgStartOp);
  return !L.empty();
}
 ...
 */
static bool isForInitOrTest(const SgStatement *s) {
  if (!s)
    return false;
 ...
 */
bool Outliner::isOutlineable(const SgStatement *s, bool verbose) {
  if (verbose) {
    cerr << "=== Checking Outliner preconditions for " << (const void *)s;
    if (s) {
      cerr << ':' << s->class_name() << ' ' << ASTtools::toStringFileLoc(s);
    }
    cerr << "... ===" << endl;
  }
  ...
  if (!decl) {
    if (verbose)
      cerr << "*** Statement must appear in an enclosing function declaration. "
              "***"
           << endl;
    return false;
  }
  ...
  if (isSgDeclarationStatement(s) && !isSgVariableDeclaration(s)) {
    if (verbose) {
      cerr << "*** Statement must not be a declaration statement, unless it is "
              "a variable declaration. ***"
           << endl
           << "    (Statement: " << (const void *)s << ":<" << s->class_name()
           << ">)" << " " << ASTtools::toStringFileLoc(s) << endl;
    }
    return false;
  }
  ...
  if (isSgBasicBlock(s) && (!isSgBasicBlock(s->get_parent()) &&
                            !isSgOmpBodyStatement(s->get_parent()))) {
    if (verbose)
      cerr << "*** Basic blocks must be secondary (nested) or body of "
              "SgOmpBodyStatement. ***"
           << endl;
    return false;
  }
  ...
  if (ASTtools::isIfCond(s) || ASTtools::isWhileCond(s) ||
      ASTtools::isSwitchCond(s)) {
    if (verbose)
      cerr << "*** Can't outline the condition of an 'if', 'while', or "
              "'switch' statement. ***"
           << endl;
    return false;
  }
  ...
  if (isSgExprStatement(s)) {
    ...
    if ((assign1) && (isSgBasicBlock(s->get_parent()))) {
      ...
      if ((isSgWhereStatement(grandpa)) || (isSgElseWhereStatement(grandpa))) {
        if (verbose)
          cerr << "*** Can't outline array assignment statement within a WHERE "
                  "block***"
               << endl;
        return false;
      }
  ...
  if (isSgWhereStatement(s)) {
    ...
    if (isSgBasicBlock(where1->get_parent()))
      if (isSgWhereStatement(where1->get_parent()->get_parent())) {
        if (verbose)
          cerr << "*** Can't outline WHERE stmt nested within another WHERE "
                  "stmt***"
               << endl;
        return false;
      }
