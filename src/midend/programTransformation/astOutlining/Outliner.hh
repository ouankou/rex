...
// * all variables are passed by references whenever possible by default
// (pointer types, and pointer dereferencing)
// * function parameter packing is needed to transfer the value of parameter to
// a local declaration

// default behavior + variable cloning
// Using variable cloning(temp variable) to reduce the uses of pointer
// dereferencing in computation kernels.
// Details are in the paper: Chunhua Liao, Daniel J. Quinlan, Richard Vuduc, and
// Thomas Panas. 2009. Effective source-to-source outlining to support whole
// program empirical optimization. In Proceedings of the 22nd international
// conference on Languages and Compilers for Parallel Computing (LCPC'09).
ROSE_DLL_API extern bool
    temp_variable; // Use temporary variables to reduce the uses of pointer
                   // dereferencing. Activated by -rose:outline:temp_variable

// -----------------------------------
// Method 1: classic outlining behavior:
// Each parameter represents a single variable being passed in/out the outlined
// function. No parameter wrapping Use parameters of the outlined function
// directly in the function,  no local declaration/copying of parameter is
// needed . Side effect analysis is used for deciding on pass-by-value
// (readOnly) and pass-by-ref,
ROSE_DLL_API extern bool enable_classic;
// -----------------------------------
// Method 2: using a wrapper (array of pointers vs. structure of flexible typed
// members) use a wrapper for all variables or one parameter for a variable or a
// wrapper for all variables
ROSE_DLL_API extern bool
    useParameterWrapper; // use an array of pointers wrapper for parameters of
                         // the outlined function. all things are passed by
                         // pointers (addressOf) by default
ROSE_DLL_API extern bool
    useStructureWrapper; // use a structure-type wrapper for parameters of the
                         // outlined function, this is a sub option for
                         // useParameterWrapper. false means using array, true
                         // means using structure (the same as
                         // useParameterWrapper).
// turned on by command line option:   -rose:outline:parameter_wrapper

//---------------------------------others ---------------------
ROSE_DLL_API extern bool
    preproc_only_; // preprocessing only, -rose:outline:preproc-only
ROSE_DLL_API extern bool
    useNewFile; // Generate the outlined function into a separated new source
                // file -rose:outline:new_file
ROSE_DLL_API extern bool
    copy_origFile; // when generating the new file to store outlined function,
                   // copy entire original file to it.
ROSE_DLL_API extern std::vector<int>
    lines; // line positions of outlining targets, given by command line option
           // -rose:outline:line for each
ROSE_DLL_API extern bool enable_debug; // output debug information for outliner
ROSE_DLL_API extern bool exclude_headers; // exclude headers from the new file
                                          // containing outlined functions
ROSE_DLL_API extern bool
    enable_liveness; // enable liveness analysis to reduce restoring statements
                     // when temp variables are used
ROSE_DLL_API extern bool
    use_dlopen; // Outlining the target to a separated file and calling it using
                // a dlopen() scheme. It turns on useNewFile.
...
ROSE_DLL_API extern bool
    use_dlopen_simple; // Outlining the target to a separated file and calling
                       // it using a dlopen() scheme. It turns on useNewFile.
                       // Additionally using a simple call convention through
                       // findAndCallFunctionUsingDlopen

ROSE_DLL_API extern bool
    enable_template; // Enabling outlining code blocks inside template functions
ROSE_DLL_API extern bool
    select_omp_loop; // Find OpenMP for loops and outline them. This is used for
                     // testing purposes.

ROSE_DLL_API extern std::string
    output_path; // where to save the new file containing the outlined function
...
// files.
ROSE_DLL_API extern bool
    suppress_autotuning_header; // when generating the new file to store
                                // outlined function, suppress output of
...
const std::string AUTOTUNING_LIB_HEADER =
    "outlining_lib.h"; // we moved the lib to be inside of rose/src and renamed
                       // it.
...
// If this variable has non empty value, we use it as the name for a single .so
// file including all outlined functions from multiple files An example value of
// this variable can be "master_shared_library.so" output_path specifies the
// path to this file.
...
struct Result {
  ...
  // improvement). DeferredTransformation deferredTransformation;
  ...
  // SageInterface namespace to support more general usage. DQ (8/15/2019):
  // Adding support to defere the transformations in header files (a performance
  // improvement). Result (SgFunctionDeclaration *, SgStatement *, SgFile*
  // file=NULL); ROSE_DLL_API Result (SgFunctionDeclaration *, SgStatement *,
  // SgFile* file, DeferredTransformation deferredTransformation);
  ROSE_DLL_API
  Result(SgFunctionDeclaration *, SgStatement *, SgFile *file,
         SageInterface::DeferredTransformation deferredTransformation);
 ...
 *
 *  This function should be called after changing analysis settings (directly or
 * via command-line parsing) to ensure they are all in a consistent state before
 * any analysis is performed.
 *
 *  @todo Ideally this should happen automatically by those functions that
 * perform analysis so the user doesn't have to remember this, but I'm not
 * familiar enough which the code to know which functions those are. Originally,
 * the outliner only validated its settings when they were changed by its @ref
 * commandLineProcessing function, which still happes. [Robb P Matzke
 * 2016-09-11] */
ROSE_DLL_API void validateSettings();

//! Accept a set of command line options to adjust internal behaviors
// Please use this function before calling the frontend() to set the internal
// flags
...
/*!
 *  \brief Create a unique outlined-function's wrapper argument name for the
 * specified statement.
 *
 *  The generated name will be "unique" within the current
 *  translation unit, and is likely (but not guaranteed) to be
...
//! Outline to a new function with the specified name, calling preprocessing
//! internally
 ...
 *  passed to the outlined routine (semantically equivalent to shared variables
 * in OpenMP)
 *
 *  Note: user codes should handle non-generic variable handling on their own.
 *  Outliner only handles generic operations.
 *  For example, in OpenMP translation calling outliner: OpenMP specific
 * variables such as private, firstprivate, reduction variables are handled by
 * OmpSupport::transOmpVariables() before it calls outliner. So the outliner can
 * focus on generic operations while the caller should handle their special
 * variables in advance.
 */
void collectVars(const SgStatement *s, ASTtools::VarSymSet_t &syms);
// void collectVars (const SgStatement* s, ASTtools::VarSymSet_t& syms,
// ASTtools::VarSymSet_t& private_syms);
 ...
 */
ROSE_DLL_API SgSourceFile *generateNewSourceFile(SgBasicBlock *target,
...
/*!\brief Generate a struct declaration to wrap all variables to be passed to
 * the outlined function There are two ways to wrap a variable: Using its value
 * vs. Using its address (pointer type)
 *
 * This function will also insert the declaration inside the global scope point
 * right before the outlining target If the scope of the outlined function is
 * different, a declaration will also be inserted there.
 */
ROSE_DLL_API
SgClassDeclaration *generateParameterStructureDeclaration(
    ...
    ASTtools::VarSymSet_t
        &pdSyms, // variables must use pointer types (pointer dereferencing:
                 // pdf). The rest variables use pass-by-value
 ...
 *
 * // pSyms are OpenMP private variables, or dead variables (neither livein nor
 * liveout)
 *
 *  Note: private, firstprivate, reduction variables are handled by
 * OmpSupport::transOmpVariables() They are not in actual use anymore.
 *
...
// const parameter. SgFunctionDeclaration* generateFunction (const SgBasicBlock*
// s,const std::string& func_name_str,const ASTtools::VarSymSet_t&
// syms,SgScopeStatement* scope);
ROSE_DLL_API
SgFunctionDeclaration *generateFunction(
    SgBasicBlock *s, const std::string &func_name_str,
    const ASTtools::VarSymSet_t &syms,
    const ASTtools::VarSymSet_t &
        pdSyms, // variables using their addresses as parameters of the outlined
                // functions, compared to use their values (pass-by-value)
    // const std::set<SgInitializedName*>& readOnlyVars, // optional
    // readOnlyVariables to optimize type of parameter when no wrapper is used.
    // const std::set< SgInitializedName *>& liveOuts, // optional live out
    // variables to optimize away the copy-back statements in variable cloning
...
generatePackingStatements(SgStatement *target, ASTtools::VarSymSet_t &syms,
...
// support to defer the transformations to header files. ROSE_DLL_API void
// insert (SgFunctionDeclaration* func, SgGlobal* scope, SgBasicBlock*
// target_outlined_code ) ROSE_DLL_API DeferredTransformation insert
 ...
 */
void appendIndividualFunctionCallArgs(
    const ASTtools::VarSymSet_t &syms, // all symbols for parameters
    const std::set<SgInitializedName *>
        varUsingOriginalType, // symbols which should be passed using their
                              // original types
