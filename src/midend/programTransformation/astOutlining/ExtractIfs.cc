...
void CPreproc::extractIfDirectives(SgNode *root, CPreproc::Ifs_t &ifs) {
...
void CPreproc::findIfDirectiveContext(const SgLocatedNode *target,
                                      CPreproc::Ifs_t &ifs,
...
void CPreproc::dump(CPreproc::Ifs_t::const_iterator b,
                    CPreproc::Ifs_t::const_iterator e, size_t level) {
  size_t count = 0;
  for (CPreproc::Ifs_t::const_iterator i = b; i != e; ++i) {
    ...
    for (c = (*i)->begin(); c != (*i)->end(); ++c) {
      ...
      cerr << indent(level) << (*c)->getDirective() << " : "
           << "'" << (*c)->getCondition() << "'" << ' '
           << ASTtools::toStringFileLoc(n) << endl;
      dump((*c)->begin(), (*c)->end(), level + 1);
    }
  }
}
void CPreproc::dump(const Ifs_t ifs) {
