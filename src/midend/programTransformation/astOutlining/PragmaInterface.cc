 ...
 *  Liao, 12/19/2008
 */
static SgStatement *processFortranComment(SgLocatedNode *node) {
  ...
  for (i = comments->begin(); i != comments->end(); i++) {
    if ((*i)->getTypeOfDirective() == PreprocessingInfo::FortranStyleComment) {
      ...
      if ((commentString == "!$" + PRAGMA_OUTLINE) ||
        ...
        if (target == NULL) {
          cerr << "Unhandled case when a Fortran statement is attached to a "
                  "non-statement node!!"
               << endl;
          ROSE_ABORT();
        }
  ...
  for (std::vector<PreprocessingInfo *>::iterator j = removeList.begin();
    ...
    // DQ (2/27/2009): Don't mix free with C++ code (malloc/free for C and
    // new/delete for C++) free memory also free(*j);
    delete (*j);
  }
...
Outliner::Result Outliner::outline(SgPragmaDeclaration *decl) {
  ...
  if (!preproc_only_) {
    ...
    // DQ (2/26/2009): If this was sucessful, then delete the input block? (this
    // may be a problem for moreTest1.c). SageInterface::deleteAST(s);
...
SgBasicBlock *Outliner::preprocess(SgPragmaDeclaration *decl) {
  SgStatement *s = processPragma(decl);
  if (s)
    return preprocess(s);
  else
...
static size_t collectPragmas(SgProject *proj, PragmaList_t &pragmas) {
  ...
  size_t count = 0;
  for (NodeList_t::reverse_iterator i = raw_list.rbegin(); i != raw_list.rend();
       ++i) {
...
static size_t collectFortranTarget(SgProject *proj, TargetList_t &targets) {
  typedef Rose_STL_Container<SgNode *> NodeList_t;
...
// The AST has normalized #pragma omp parallel for also (split into separated
// paralle and for AST nodes).
static size_t
collectOmpLoops(SgProject *proj,
...
//! Collect outlining targets specified using source code positions, e.g.
//! line:col
// save them into targetList
static size_t collectTargetFromLines(SgProject *proj, TargetList_t &targets,
...
//-------------------top level drivers----------------------------------------
size_t Outliner::outlineAll(SgProject *project) {
  ...
  if (Outliner::lines.size() > 0) {
    ...
    for (TargetList_t::iterator i = targets.begin(); i != targets.end(); ++i)
      if (outline(*i).isValid())
        ++num_outlined;

  } else if (SageInterface::is_Fortran_language()) { // Search for the special
                                                     // source comments for
                                                     // Fortran input
  ...
  } else // Search for pragmas for C/C++
  {
    ...
    if (collectPragmas(project, pragmas)) {
      for (PragmaList_t::iterator i = pragmas.begin(); i != pragmas.end();
           ++i) {
        if (outline(*i).isValid()) {
          ++num_outlined;
        }
    ...
    // New interface : OpenMP for loops, for testing purpose, only for C/C++
    // code
    if (select_omp_loop) {
      Rose_STL_Container<SgOmpForStatement *> ompfor_stmts;
      if (collectOmpLoops(project, ompfor_stmts)) {
        for (Rose_STL_Container<SgOmpForStatement *>::iterator i =
                 ompfor_stmts.begin();
             i != ompfor_stmts.end(); ++i) {
...
size_t Outliner::preprocessAll(SgProject *proj) {
  size_t count = 0;
  if (SageInterface::is_Fortran_language()) { // Search for the special source
                                              // comments for Fortran input
    TargetList_t targets;
