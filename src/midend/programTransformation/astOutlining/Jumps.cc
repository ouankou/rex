...
struct MostRecentScope {
  ...
  MostRecentScope(const MostRecentScope &x)
...
public:
  ...
  virtual MostRecentScope evaluateInheritedAttribute(SgNode *n,
                                                     MostRecentScope s) {
...
void ASTtools::collectNonLocalGotos(const SgStatement *root, JumpMap_t &jumps) {
  ...
  for (NodeList_t::const_iterator i = labels.begin(); i != labels.end(); ++i)
