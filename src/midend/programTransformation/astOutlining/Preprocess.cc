...
void Outliner::Preprocess::checkAndPatchUpOptions() {
  ...
  if (useStructureWrapper && !useParameterWrapper) {
    cout << "Warning: Outliner::useParameterWrapper is automatically set to be "
            "true since useStructureWrapper is set."
         << endl;
...
SgBasicBlock *Outliner::Preprocess::preprocessOutlineTarget(SgStatement *s) {
  ...
  switch (s->variantT()) {
    // Liao, 4/14/2009
    // This branch does not make sense (will never reached) since
    // Outliner::isOutlineable() will exclude single variable declaration
  ...
  // TODO better solution is to add Fortran support and report illegal non-local
  // jumps Liao, 1/10/2011
  if (!SageInterface::getEnclosingFileNode(s)->get_openmp_lowering())
  ...
  // Static const member variables in a class: if int or enum type, replace
...
*/
static SgExpression *
isStaticConstIntOrEnumWithInitializer(SgVarRefExp *varRef) {
      ...
      if (varInit) {
        ...
        if (parentNode) {
          SgClassDefinition *classDef =
              isSgClassDefinition(parentNode->get_parent());
          ...
          if (classDef) {
            ...
            SgType *base_type = varInit->get_type()->stripType(
                SgType::STRIP_MODIFIER_TYPE | SgType::STRIP_TYPEDEF_TYPE);
            // Check for static, constant, integer or enumeration type, and
            // initialized
            if (isStatic(var_decl) && isConstType(varInit->get_type()) &&
...
void Outliner::Preprocess::propagateStaticConstMembers(SgBasicBlock *b) {
  ...
  for (RoseAst::iterator it = ast.begin(); it != ast.end(); ++it) {
    ...
    if (varRef) {
      ...
      if (SgExpression *initor =
              isStaticConstIntOrEnumWithInitializer(varRef)) {
