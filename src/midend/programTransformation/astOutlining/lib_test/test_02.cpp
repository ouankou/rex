...
void foov1(int x) {
  if (rose_dynamic_library_exists("./master_shared_library.so")) {
    ...
    __out_argv1__11178__[0] =
        ((void *)(&abxyz)); // global variable passed into the outlined function

    OUT_1_test_02_cpp_0p = findFunctionUsingDlopen(
        "OUT_1_test_02_cpp_0", "./master_shared_library.so");
...
void foov2(int x) {
  ...
  findAndCallFunctionUsingDlopen(
      3, "OUT_1_test_02_cpp_0", "./master_shared_library.so", (void *)(&abxyz));
}
