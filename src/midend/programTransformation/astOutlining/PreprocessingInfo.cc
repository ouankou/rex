...
void ASTtools::attachComment(const string &comment, SgStatement *s) {
  if (s) {
    ...
    PreprocessingInfo *comment_info = new PreprocessingInfo(
        PreprocessingInfo::CplusplusStyleComment, "// " + comment,
...
bool ASTtools::insertHeader(const string &filename, SgProject *project) {
  ...
  for (NodeList_t::iterator i = globalScopeList.begin();
    ...
    for (SgDeclarationStatementPtrList::iterator j = stmtList.begin();
      ...
      if (((*j)->get_file_info())->isSameFile(globalScope->get_file_info())) {
        ...
        // Note that this function could directly call
        // SageInterface::attachComment(SgLocatedNode*,std::string);
        (*j)->addToAttachedPreprocessingInfo(new PreprocessingInfo(
            PreprocessingInfo::CpreprocessorIncludeDeclaration,
            string("#include \"" + filename + "\" \n"),
            "Compiler-generated by Outliner", 0, 0, 0,
...
void ASTtools::cutPreprocInfo(SgBasicBlock *b,
  ...
  ROSE_ASSERT(info);
  remove_copy_if(
      info->begin(), info->end(), back_inserter(save_buf),
  ...
  // Liao (10/3/2007), implement list::remove_if for vector, which lacks sth.
  // like erase_if
  AttachedPreprocessingInfoType::iterator new_end = remove_if(
      info->begin(), info->end(),
      [pos](const PreprocessingInfo *info) { return isRelPos(info, pos); });
...
void ASTtools::moveBeforePreprocInfo(SgStatement *src, SgStatement *dest) {
  ...
  // DQ (9/26/2007): Commented out as part of move from std::list to std::vector
  //   printf ("Commented out remove_copy_if() as part of move from std::list to
  //   std::vector \n");
  // remove_copy_if (s_info->rbegin (), s_info->rend (), front_inserter
  // (*d_info), bind2nd (ptr_fun (isNotRelPos), PreprocessingInfo::before));
  // Liao (10/3/2007), workaround for inserting at the beginning of a vector
  for (AttachedPreprocessingInfoType::reverse_iterator i = s_info->rbegin();
       i != s_info->rend(); i++)
    if (!isNotRelPos(*i, PreprocessingInfo::before))
      d_info->insert(d_info->begin(), *i);

  // DQ (9/26/2007): Commented out as part of move from std::list to std::vector
  //   printf ("Commented out s_info->remove_if() as part of move from std::list
  //   to std::vector \n");
  // s_info->remove_if (bind2nd (ptr_fun (isRelPos),
  // PreprocessingInfo::before)); Liao (10/3/2007), vectors do not support
  // remove_if
  AttachedPreprocessingInfoType::iterator new_end = remove_if(
      s_info->begin(), s_info->end(), [](const PreprocessingInfo *info) {
...
void ASTtools::moveInsidePreprocInfo(SgBasicBlock *src, SgBasicBlock *dest) {
  ...
  AttachedPreprocessingInfoType::iterator i = find_if(
      d_info->begin(), d_info->end(), [](const PreprocessingInfo *info) {
        return isRelPos(info, PreprocessingInfo::inside);
      });
  ...
  //   to std::vector \n");
  // s_info->remove_if (bind2nd (ptr_fun (isRelPos),
  // PreprocessingInfo::inside)); Liao (10/3/2007), vectors do not support
  // remove_if
  AttachedPreprocessingInfoType::iterator new_end = remove_if(
      s_info->begin(), s_info->end(), [](const PreprocessingInfo *info) {
        return isRelPos(info, PreprocessingInfo::inside);
...
void ASTtools::moveAfterPreprocInfo(SgStatement *src, SgStatement *dest) {
  ...
  remove_copy_if(s_info->begin(), s_info->end(), back_inserter(*d_info),
                 [](const PreprocessingInfo *info) {
                   return isNotRelPos(info, PreprocessingInfo::after);
                 });
