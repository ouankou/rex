...
bool isGoodCtorInit(SgConstructorInitializer *ci) {
...
void addStmtVarsIfAny(in_list &objs, SgStatement *stmt) {
  if (SgVariableDeclaration *ds = isSgVariableDeclaration(stmt)) {
    ...
    for (Rose_STL_Container<SgInitializedName *>::iterator i = vars.begin();
         i != vars.end(); ++i) {
      if (SgConstructorInitializer *ci =
...
std::ostream &operator<<(std::ostream &os, const Rose_STL_Container<T> &list) {
  os << "[";
  for (typename Rose_STL_Container<T>::const_iterator i = list.begin();
       i != list.end(); ++i) {
    if (i != list.begin()) {
      os << ",";
    }
...
private:
  bool isGood(SgNode *n) {
    if (isSgAssignOp(n->get_parent()) ||
        isSgAssignInitializer(n->get_parent())) {
      return false;
    }
  ...
  void visit(SgNode *node) {
    ...
    if (SgConstructorInitializer *ci = isSgConstructorInitializer(node)) {
      ...
      if (SgClassType *retClsType = isSgClassType(retType)) {
        SgClassDeclaration *retClsDecl =
            isSgClassDeclaration(retClsType->get_declaration());
...
in_list blockObjectsAllocated(SgStatement *block, SgStatement *stop) {
  in_list objs;
  ROSE_ASSERT(block);
  ...
  case V_SgBasicBlock: {
    ...
    for (SgStatementPtrList::iterator i = stmts.begin(); i != stmts.end();
         ++i) {
      if (*i == stop)
        break;
...
/*! Returns the list of objects allocated at the given statement, not including
  ...
  i.e. order of destruction.  Does not proceed any further than a node
  satisfying stopCond. Itf omitInit true, omit objects allocated at the start of
  a loop which will not be
 */
/* implementation technique: startOfCurrent helps ensure items inserted in
 * reverse order for the current block, while blocks, visited from inner to
 * outer are in the correct order  */
template <class StopCond>
in_list objectsAllocated(SgStatement *statement, StopCond stopCond) {
 ...
 */
in_list auxObjectsAllocated(SgStatement *stmt) {
  in_list objs;
...
struct IsBreakPoint {
  using result_type = bool;
  result_type operator()(SgNode *n) {
    return isSgForStatement(n) || isSgWhileStmt(n) || isSgDoWhileStmt(n) ||
           isSgSwitchStatement(n) || isSgFunctionDefinition(n);
  }
};

struct ParentIsBreakPoint {
  using result_type = bool;
  result_type operator()(SgNode *n) { return IsBreakPoint()(n->get_parent()); }
};
...
public:
  void visit(SgNode *node) {
    if (SgStatement *stmt = isSgStatement(node)) {
      ...
      cout << "objectsAllocated = " << objectsAllocated(stmt, IsFunctionDef())
           << endl;
    }
  }
};
...
inline T &GetAttribute(SgNode *n, const std::string &name) {
  if (n->attributeExists(name)) {
    return static_cast<GenericAttr<T> *>(n->getAttribute(name))->attr;
  } else {
...
#define ATTR_ACCESSOR(type, name)                                              \
  type &name(SgNode *n) { return GetAttribute<type>(n, #name); }
...
template <typename NodeType> NodeType *&findReference(NodeType *node) {
...
SgMemberFunctionRefExp *buildDtorRefExpr(SgClassDeclaration *decl) {
  ...
  string dtorName = "~";
  if (SgTemplateInstantiationDecl *tmplDecl =
  ...
  SgMemberFunctionSymbol *dtor =
      isSgMemberFunctionSymbol(def->lookup_function_symbol(dtorName));
  cout << "dtorName = " << dtorName
       << ", dtor = " << (void *)dtor->get_declaration() << endl;
  // ROSE_ASSERT(dtor);
  if (!dtor) {
  ...
  SgMemberFunctionRefExp *ref = new SgMemberFunctionRefExp(
      SgNULL_FILE, dtor, false, dtor->get_declaration()->get_type(), false);
  return ref;
}
...
SgDotExp *buildDtorDotExpr(SgInitializedName *in) {
  ...
  SgMemberFunctionRefExp *dtorRef = buildDtorRefExpr(
      isSgConstructorInitializer(in->get_initializer())->get_class_decl());
  SgDotExp *dtorDot =
      new SgDotExp(SgNULL_FILE, vref, dtorRef, SgTypeVoid::createType());
...
SgExpression *buildDestructorExpr(SgNode *n) {
  ...
  SgFunctionCallExp *dtorCall = new SgFunctionCallExp(
      SgNULL_FILE, ref, dtorParams, SgTypeVoid::createType());
...
void destroyTemporariesInFollowingPositions(NodeType *n, SgNode *tempSrc) {
  ...
  if (poss.empty() && !temps.empty()) {
    ...
    if (parentStmt != NULL) {
      if (SgForInitStatement *forInitStmt =
              isSgForInitStatement(parentStmt->get_parent())) {
        ...
        // SgStatementPtrList::iterator parentStmtPos =
        // findIterator(forInitStmtList, parentStmt);

        // DQ (9/25/2007): Moved from std::list to std::vector
        // forInitStmtList.remove(parentStmt);
        forInitStmtList.erase(
            find(forInitStmtList.begin(), forInitStmtList.end(), parentStmt));
    ...
    if (poss.empty()) {
      cout << "WARNING: temporary created in statement after which it is"
           << endl
           << "impossible to call destructor, and I could not correct the"
           << endl
...
public:
  void visit(SgNode *node) {
    if (SgBasicBlock *bb = isSgBasicBlock(node)) {
    ...
    } else if (node->variantT() == V_SgForStatement ||
               node->variantT() == V_SgWhileStmt) {
    ...
    } else if (node->variantT() == V_SgBreakStmt ||
               node->variantT() == V_SgContinueStmt ||
               node->variantT() == V_SgGotoStatement) {
      SgStatement *stmt = isSgStatement(node);
      in_list &ctors = ObjectsAllocated(stmt);
      ...
      cout << "before: " << isSgBasicBlock(stmt->get_parent())->get_statements()
           << endl;
      newBlock->set_parent(stmt->get_parent());
      newBlock->append_statement(stmt);
      stmtRef = newBlock;
      // cout << "after: " <<
      // isSgBasicBlock(stmt->get_parent())->get_statements() << endl;
...
bool customFilter::operator()(pair<SgNode *, std::string> &x) {
  ...
  if (locatedNode != NULL) {
    ...
    std::string filename(
        Rose::utility_stripPathFromFileName(fileInfo->get_filename()));
...
void moveForInitializers(SgForStatement *forStmt) {
  ...
  for (SgStatementPtrList::iterator i = initStmts.begin(); i != initStmts.end();
       ++i) {
...
void destructorCallAnnotator(SgProject *prj) {
  Rose_STL_Container<SgNode *> forStmts =
      NodeQuery::querySubTree(prj, V_SgForStatement);
  ...
  Rose_STL_Container<SgNode *> defs =
      NodeQuery::querySubTree(prj, V_SgFunctionDefinition);

  for (Rose_STL_Container<SgNode *>::iterator i = defs.begin(); i != defs.end();
       ++i) {
    Annotator a;
