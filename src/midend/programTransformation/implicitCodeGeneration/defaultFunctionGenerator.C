...
// when run. Use this to suppress it.
...
//-----------------------------------------------------------------------------
//  generateDefaultFunctionDeclaration() will call
//  generateDefaultFunctionDefinition() when necessary
...
private:
  ...
  SgClassDefinition *
  get_ClassDeclarationFromNewOp(SgConstructorInitializer *ctorInitNode);
  SgMemberFunctionDeclaration *
  generateDefaultFunctionDeclaration(defaultEnumFunctionType enumFunctionType,
                                     SgClassDefinition *classDef1);
  void
  generateDefaultFunctionDefinition(defaultEnumFunctionType enumFunctionType,
                                    SgMemberFunctionDeclaration *func);
  SgMemberFunctionDeclaration *
  findDefaultFunctionDeclaration(defaultEnumFunctionType enumFunctionType,
                                 SgClassDefinition *classDef1);
  SgName generateDefaultFunctionName(defaultEnumFunctionType enumFunctionType,
                                     SgClassDefinition *parentClassDef);
  void generateDefaultFunctionCall(SgMemberFunctionDeclaration *func,
                                   SgClassDefinition *parentClassDef,
                                   SgExpression *receiver, SgExpression *arg,
                                   SgBasicBlock *basicBlock);
  void generateDefaultConstructorCall(
      SgMemberFunctionDeclaration *func, SgClassDefinition *parentClassDef,
      SgExpression *arg, SgCtorInitializerList *ctorInitializerList,
  ...
  normalizeAssignInitializer(SgAssignInitializer *assignInit);
  SgFunctionCallExp *
  translateAssignmentToOperatorEqCall(SgExpression *lhs, SgExpression *rhs,
                                      SgClassDefinition *clsDef);
  SgMemberFunctionDeclaration *findOrGenerateDefaultFunctionDeclaration(
      defaultEnumFunctionType enumFunctionType, SgClassDefinition *classDef1);
};
...
* create default member functions in the form of AST subtree
    refer to a full AST with a constructor etc. to see what nodes and edges are
needed SgMemberFunctionType, SgFunctionDefinition,SgFunctionParameterList Must
set special function modifier to isConstructor

** generated both declaration and definition when applicable(copy
 ...
 *
 ...
 { std::cout << "C" << std::endl;
      ...
      // This might be an alternative form of specification of the constructor
 calls
...
#define COMPILERGENERATED_FILE_INFO                                            \
  (generateCompilerGeneratedNodes                                              \
       ? Sg_File_Info::generateDefaultFileInfoForCompilerGeneratedNode()       \
       : Sg_File_Info::generateDefaultFileInfoForTransformationNode())
...
static bool isCompilerGenerated(SgNode *node) {
  ...
  if (!compilerGenerated) {
    ...
    if (functionDeclaration != NULL) {
      ...
      compilerGenerated = (functionDefinition == NULL
                               ? true
...
canonicalClassDeclaration(SgClassDeclaration *declaration) {
  ASSERT_not_null(declaration);
  if (SgClassDeclaration *defining =
          isSgClassDeclaration(declaration->get_definingDeclaration())) {
    return defining;
  }
  if (SgClassDeclaration *first_nondefining = isSgClassDeclaration(
 ...
 *  To do:  move this to a generally-accessible utilities file.
 */
static SgNode *lookThroughCastExpAndAssignInitializer(SgNode *node) {
  ROSE_ASSERT(node != NULL);
  ...
  case V_SgAssignInitializer: {
    ...
    ret = lookThroughCastExpAndAssignInitializer(
        assignInitializer->get_operand_i());
 ...
 *  To do:  move this to a generally-accessible utilities file.
 */
static SgClassDeclaration *getClassDeclaration(SgType *type) {
  ...
  case V_SgTypedefType: {
    ...
    if (isSgTypedefType(baseType) || isSgReferenceType(baseType) ||
        isSgModifierType(baseType)) {
      // Recursive case:  base type of typedef is also a typedef type or a
      // reference type.
      ...
      SgDeclarationStatement *innerDecl = namedType->get_declaration();
 ...
 */
static bool
isConstructor(SgMemberFunctionDeclaration *memberFunctionDeclaration) {
 ...
 */
static bool
isDefaultConstructor(SgMemberFunctionDeclaration *memberFunctionDeclaration) {
  ROSE_ASSERT(memberFunctionDeclaration != NULL);
 ...
 */
static bool
isCopyConstructor(SgMemberFunctionDeclaration *memberFunctionDeclaration) {
  ROSE_ASSERT(memberFunctionDeclaration != NULL);
  ...
  for (; p != params.end(); ++p) {
    ...
    if (firstParam) {
      ...
      SgClassType *classType = isSgClassType(getBaseType(referenceType));
      if (classType == NULL)
        return false;
      ...
      SgDeclarationStatement *methodDeclaration =
      ...
      if (xdecl != NULL) {
        if (classDeclaration->get_type()->get_name() !=
            xdecl->get_type()->get_name())
          return false;
      } else if (nrdecl != NULL) {
        if (classDeclaration->get_type()->get_name() !=
            nrdecl->get_type()->get_name())
          return false;
      } else {
        ROSE_ABORT();
      }
...
static bool isCopyAssignmentTypeEquivalent(SgType *type, SgType *typeToMatch) {
  ...
  typeToMatch = typeToMatch->stripType(SgType::STRIP_REFERENCE_TYPE |
  ...
  // Not sure if we could use strip type: are we removing only ine level of
  // reference?
 ...
 */
static bool
isOperatorEquals(SgMemberFunctionDeclaration *memberFunctionDeclaration) {
  ROSE_ASSERT(memberFunctionDeclaration != NULL);
  ...
  if (memberFnType->get_arguments().size() !=
      1) // I can't imagine when this would be false
  {
    return false;
  }
  ...
  if (xdecl != NULL) {
    SgClassDeclaration *firstNondefining =
        isSgClassDeclaration(xdecl->get_firstNondefiningDeclaration());
    if (firstNondefining == NULL ||
        firstNondefining->get_firstNondefiningDeclaration() !=
            firstNondefining ||
        xdecl->get_scope() != firstNondefining->get_scope() ||
        xdecl->get_definingDeclaration() !=
            firstNondefining->get_definingDeclaration()) {
      std::cerr << "REX_AST_INVARIANT[default-function-class-owner]: "
                   "associated class declaration has no exact canonical "
                   "first/defining chain and scope\n";
      ROSE_ABORT();
    }
    SgClassType *desiredClassType = SgClassType::createType(firstNondefining);
    return isCopyAssignmentTypeEquivalent(
        desiredClassType, memberFnType->get_arguments().front());
  } else if (nrdecl != NULL) {
    return isCopyAssignmentTypeEquivalent(
        nrdecl->get_type(), memberFnType->get_arguments().front());
  } else {
    ROSE_ABORT();
  }
 ...
 */
static defaultEnumFunctionType
declarationToEnumType(SgMemberFunctionDeclaration *func) {
 ...
 */
static void findClassesInvokedViaConstructors(
    SgNode *subtree, std::set<SgClassDeclaration *> &invokedClasses) {
  ...
  Rose_STL_Container<SgNode *> nodes =
      NodeQuery::querySubTree(subtree, V_SgInitializedName);
...
static SgClassSymbol *lookupClassSymbol(SgClassDeclaration *classDecl) {
  ...
  SgClassSymbol *classSymbol =
      isSgClassSymbol(classDecl->get_symbol_from_symbol_table());
  if (!classSymbol) {
    if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
      cerr << "Warning: getThisExp doing lookup by name on "
           << classDecl->get_name().getString() << endl;
    }
    // DQ (5/21/2013): We want to restrict direct use of the symbol table.
    // classSymbol =
    // classDecl->get_scope()->get_symbol_table()->find_class(classDecl->get_name());
    classSymbol =
        classDecl->get_scope()->lookup_class_symbol(classDecl->get_name());
    if (!classSymbol) {
      cerr << "Class symbol not found " << classDecl->get_name().getString()
           << endl;
      // classDecl->get_scope()->get_symbol_table()->print("debug");
      classDecl->get_scope()->print_symboltable("debug");
      cerr << "get_symbol_from_symbol_table(): "
           << (classDecl->get_symbol_from_symbol_table() ? "Not null" : "Null")
           << endl;
      // cerr << "find_class on OptionDeclaration: " <<
      // (classDecl->get_scope()->get_symbol_table()->find_class("OptionDeclaration")
      // ? "Not null" : "Null") << endl; cerr << "find_class on name: " <<
      // (classDecl->get_scope()->get_symbol_table()->find_class(classDecl->get_name())
      // ? "Not null" : "Null") << endl; cerr << "find_any on OptionDeclaration:
      // " <<
      // (classDecl->get_scope()->get_symbol_table()->find_any("OptionDeclaration")
      // ? "Not null" : "Null") << endl;
    }
  }
 ...
 */
static SgType *
buildExactThisPointerType(SgClassSymbol *class_symbol,
                          const SgMemberFunctionDeclaration *member_function) {
  SgMemberFunctionType *member_type =
      member_function != nullptr
          ? isSgMemberFunctionType(member_function->get_type())
          : nullptr;
  SgType *base_type =
      class_symbol != nullptr ? class_symbol->get_type() : nullptr;
  if (member_type == nullptr || base_type == nullptr) {
    cerr << "REX_AST_INVARIANT[default-function-this-type]: generated this "
            "expression requires exact class and member-function types"
         << endl;
    ROSE_ABORT();
  }
...
static SgThisExp *
getThisExp(SgClassDeclaration *classDecl,
           const SgMemberFunctionDeclaration *member_function) {
  SgClassSymbol *class_symbol = lookupClassSymbol(classDecl);
  return buildThisExp(class_symbol,
                      buildExactThisPointerType(class_symbol, member_function));
}
 ...
 */
static SgNode *getMethodInvocationLhs(SgFunctionCallExp *functionCall) {
 ...
 */
static SgVarRefExp *getVarRefForFormal(SgFunctionDefinition *functionDefinition,
                                       int formalNum) {
  SgFunctionDeclaration *functionDeclaration =
  ...
  SgVariableSymbol *symbol =
      isSgVariableSymbol(specifiedInitName->get_symbol_from_symbol_table());
 ...
 *       via delete.
 */
static void findClassesInvokedViaDestructors(
    SgNode *subtree, SgThisExp *,
    std::set<SgClassDeclaration *> &explicitlyInvokedClasses) {
  // Find any function callsites within subtree.
  Rose_STL_Container<SgNode *> nodes =
      NodeQuery::querySubTree(subtree, V_SgFunctionCallExp);
 ...
 *                          union'ed with invokedClasses.
 */
static void findClassesInvokedViaOperatorEquals(
    SgNode *subtree, SgThisExp *,
    std::set<SgClassDeclaration *> &explicitlyInvokedClasses) {
  // Find any function callsites within subtree.
  Rose_STL_Container<SgNode *> nodes =
...
DefaultFunctionGenerator::getMethodInClass(SgMemberFunctionDeclaration *method,
  ...
  if (method == NULL) {
    ...
    if (targetFunctionDeclaration == NULL) {
      ...
      generateDefaultFunctionDeclaration(enumFunctionType, classDefinition);
      targetFunctionDeclaration =
          lookupDefaultConstructorInClass(classDefinition);
    }
  } else {
    defaultEnumFunctionType enumFunctionType = declarationToEnumType(method);
    targetFunctionDeclaration = lookupMethodInClass(method, classDefinition);
    if (targetFunctionDeclaration == NULL) {
      if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
 ...
 */
SgMemberFunctionDeclaration *
DefaultFunctionGenerator::findDefaultFunctionDeclaration(
    defaultEnumFunctionType enumFunctionType, SgClassDefinition *classDef1) {
  //       SgClassDefinition * classDef1 =
  //       get_ClassDeclarationFromNewOp(ctorIniter);
  //      cout<<"The name of class missing constructor is:
  //      "<<classDef1->get_qualified_name().str()<<endl;

  // lookup all member functions for a constructor
  SgDeclarationStatementPtrList memList = classDef1->get_members();
  typedef Rose_STL_Container<SgDeclarationStatement *>::iterator
      memListIterator;
  SgMemberFunctionDeclaration *func = NULL;

  for (memListIterator listElement = memList.begin();
       listElement != memList.end(); ++listElement) {
    SgMemberFunctionDeclaration *mfDec1 =
        isSgMemberFunctionDeclaration(*listElement);
    if (mfDec1 != NULL) {
 ...
 */
static void appendGeneratedCtorInitializer(
    SgCtorInitializerList *initializerList, SgInitializedName *initializer,
    SgInitializedName::preinitialization_enum preinitialization) {
  ROSE_ASSERT(initializerList != NULL);
  ROSE_ASSERT(initializer != NULL);

  SgMemberFunctionDeclaration *owner =
      isSgMemberFunctionDeclaration(initializerList->get_parent());
  ROSE_ASSERT(owner != NULL);
  ROSE_ASSERT(owner->get_CtorInitializerList() == initializerList);
  SgClassDefinition *classScope = isSgClassDefinition(owner->get_scope());
  ROSE_ASSERT(classScope != NULL);

  ROSE_ASSERT(initializer->get_parent() == NULL);
  ROSE_ASSERT(initializer->get_declptr() == NULL);
  ROSE_ASSERT(initializer->get_scope() == NULL);
  switch (preinitialization) {
  ...
  default:
    fprintf(stderr,
            "REX_AST_INVARIANT[generated-constructor-initializer-role]: "
            "initializer=%p name=%s has invalid explicit role=%d\n",
            static_cast<void *>(initializer), initializer->get_name().str(),
            static_cast<int>(preinitialization));
    ROSE_ABORT();
  }
  const SgInitializedNamePtrList &initializers = initializerList->get_ctors();
  ROSE_ASSERT(std::find(initializers.begin(), initializers.end(),
                        initializer) == initializers.end());

  initializer->set_preinitialization(preinitialization);
  SageInterface::publishGeneratedSubtreeOutputOwner(initializer,
                                                    initializerList);
  initializerList->append_ctor_initializer(initializer);
  initializer->set_parent(initializerList);
  initializer->set_declptr(owner);
  initializer->set_scope(classScope);

  ROSE_ASSERT(initializerList->get_ctors().back() == initializer);
  ROSE_ASSERT(initializer->get_parent() == initializerList);
  ROSE_ASSERT(initializer->get_declptr() == owner);
  ROSE_ASSERT(initializer->get_scope() == classScope);
  ROSE_ASSERT(initializer->get_preinitialization() == preinitialization);
}
...
void DefaultFunctionGenerator::generateDefaultFunctionDefinition(
  ...
  SgType *this_pointer_type =
      buildExactThisPointerType(lookupClassSymbol(classDeclaration), func);
  ...
  SgInitializedName *parameter1 =
      *(argList.begin()); // only 1 parameter for copy constructor
  ...
  if (enumFunctionType == e_assignment_operator) {
    ...
    // return *this
    SgThisExp *this_for_return = buildThisExp(classSymbol1, this_pointer_type);
    SgPointerDerefExp *derefExp1 = buildPointerDerefExp(
        this_for_return,
        SageInterface::getElementType(this_for_return->get_type()));
    ...
    // SgStatementPtrList falseList =
    // ifStatement->get_false_body()->getStatementList();
    SgBasicBlock *falseBody1 = buildBasicBlock();
    SgVariableSymbol *symbolPar1 =
        isSgVariableSymbol(parameter1->get_symbol_from_symbol_table());
    ROSE_ASSERT(symbolPar1 != NULL);
    SgVarRefExp *parameter_ref = buildVarRefExp(symbolPar1);
    SgType *addressed_parameter_type = parameter_ref->get_type()->stripType(
        SgType::STRIP_REFERENCE_TYPE | SgType::STRIP_RVALUE_REFERENCE_TYPE);
    ROSE_ASSERT(addressed_parameter_type != nullptr);
    SgType *equality_type = SageInterface::is_C_language()
                                ? static_cast<SgType *>(buildIntType())
                                : static_cast<SgType *>(buildBoolType());
    SgEqualityOp *equalityOp1 = buildEqualityOp(
        buildThisExp(classSymbol1, this_pointer_type),
        buildAddressOfOp(parameter_ref,
                         buildPointerType(addressed_parameter_type)),
        equality_type);
    ...
    SageInterface::publishGeneratedSubtreeOutputOwner(ifStmt1, bBlock1);
  ...
  typedef Rose_STL_Container<SgDeclarationStatement *>::iterator
      declareListIterator;
  for (declareListIterator listElement = declareList.begin();
       listElement != declareList.end(); ++listElement) {
    SgVariableDeclaration *varDec1 = isSgVariableDeclaration(*(listElement));
    if (varDec1 != NULL) {
      SgInitializedNamePtrList &nameList = varDec1->get_variables();
      typedef Rose_STL_Container<SgInitializedName *>::iterator
          variableListIterator;
      for (variableListIterator listVariable = nameList.begin();
           listVariable != nameList.end(); ++listVariable) {

        SgName varName = (*listVariable)->get_name();
        SgType *varType = (*listVariable)->get_type();
        // cout<<"name is "<<varName.str()<<" of type
        // "<<(varType->get_mangled()).str()<<endl;

        SgVariableSymbol *varSymbol1 =
            isSgVariableSymbol(parameter1->get_symbol_from_symbol_table());
        SgVariableSymbol *varSymbol2 =
            isSgVariableSymbol((*listVariable)->get_symbol_from_symbol_table());
        ...
        SgDotExp *dotExp1 =
            buildDotExp(lhsOperator, rhsOperator, rhsOperator->get_type());
        //"_member(rhs._member)" for copy constructor
        if (enumFunctionType == e_copy_constructor) {
          SgAssignInitializer *assignInit1 =
              buildAssignInitializer(dotExp1, varType);
          SgInitializedName *initName1 =
              buildInitializedName(varName, varType, assignInit1);
          appendGeneratedCtorInitializer(func->get_CtorInitializerList(),
                                         initName1,
        ...
        if (enumFunctionType == e_assignment_operator) {
          ...
          SgArrowExp *arrowExp1 =
              buildArrowExp(buildThisExp(classSymbol1, this_pointer_type),
                            rhsOperator2, rhsOperator2->get_type());
          SgAssignOp *assignOp1 =
              buildAssignOp(arrowExp1, dotExp1, arrowExp1->get_type());
          SgExprStatement *myExpStmt1 = buildExprStatement(assignOp1);
          SageInterface::publishGeneratedSubtreeOutputOwner(myExpStmt1,
                                                            bBlock1);
  ...
  if (enumFunctionType == e_assignment_operator) {
    ...
    SgPointerDerefExp *derefExp1 = buildPointerDerefExp(
        ...
        SageInterface::getElementType(this_for_return->get_type()));
    SgReturnStmt *myReturn1 = buildReturnStmt(derefExp1);
    SageInterface::publishGeneratedSubtreeOutputOwner(myReturn1, bBlock1);
 ...
 *            by ctorInitNode.
 */
SgClassDefinition *DefaultFunctionGenerator::get_ClassDeclarationFromNewOp(
    SgConstructorInitializer *ctorInitNode) {
  SgConstructorInitializer *ctorIniter =
      isSgConstructorInitializer(ctorInitNode);
  ROSE_ASSERT(ctorIniter != NULL);
  // get the corresponding class definition node from a call site, a long road
  // to go
  SgClassDefinition *classDef1 =
      isSgClassDeclaration(
          isSgClassType(
              isSgNewExp(ctorIniter->get_parent())->get_specified_type())
              ->get_declaration())
          ->get_definition();
  return classDef1;
}
 ...
 *            specified class.
 */
SgName DefaultFunctionGenerator::generateDefaultFunctionName(
    defaultEnumFunctionType enumFunctionType,
    SgClassDefinition *parentClassDef) {
  SgName func_name;

  if (SgTemplateInstantiationDefn *tempDef =
          isSgTemplateInstantiationDefn(parentClassDef)) {
    SgTemplateInstantiationDecl *tempDecl =
        isSgTemplateInstantiationDecl(tempDef->get_declaration());
 ...
 */
SgMemberFunctionDeclaration *
DefaultFunctionGenerator::generateDefaultFunctionDeclaration(
    defaultEnumFunctionType enumFunctionType,
    SgClassDefinition *parentClassDef1) {
  ROSE_ASSERT(parentClassDef1 != nullptr);

  // debug
  if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
    if (enumFunctionType == e_constructor)
      cout << "Generating constructor for "
           << parentClassDef1->get_qualified_name().str() << endl;
    else if (enumFunctionType == e_destructor)
      cout << "Generating destructor for "
           << parentClassDef1->get_qualified_name().str() << endl;
    else if (enumFunctionType == e_copy_constructor)
      cout << "Generating copy constructor for "
           << parentClassDef1->get_qualified_name().str() << endl;
  }
  ...
  SgType *base_type1 = nullptr;
  if (enumFunctionType == e_assignment_operator ||
      enumFunctionType == e_copy_constructor) {
    SgClassDeclaration *classDeclaration =
        isSgClassDeclaration(parentClassDef1->get_declaration());
    SgClassDeclaration *canonicalDeclaration =
        classDeclaration != nullptr
            ? isSgClassDeclaration(
                  classDeclaration->get_firstNondefiningDeclaration())
            : nullptr;
    if (canonicalDeclaration == nullptr ||
        canonicalDeclaration->get_type() == nullptr) {
      fprintf(stderr,
              "REX_AST_INVARIANT[default-function-class-identity]: "
              "class-definition=%p declaration=%p has no exact canonical "
              "class type for special-member kind=%d\n",
              static_cast<void *>(parentClassDef1),
              static_cast<void *>(classDeclaration),
              static_cast<int>(enumFunctionType));
      ROSE_ABORT();
    }
    base_type1 = canonicalDeclaration->get_type();
  }

  SgType *func_return_type = nullptr;
  if (enumFunctionType == e_assignment_operator) {
    func_return_type = buildReferenceType(base_type1);
  } else {
    func_return_type = SgTypeVoid::createType();
  }
  ROSE_ASSERT(func_return_type != nullptr);

  SgName func_name =
      generateDefaultFunctionName(enumFunctionType, parentClassDef1);

  if (isSgTemplateInstantiationDefn(parentClassDef1)) {
    SgTemplateInstantiationDecl *classDecl =
        isSgTemplateInstantiationDecl(parentClassDef1->get_declaration());
    ROSE_ASSERT(classDecl);

    // DQ (12/22/2011): This is required by the new design to support
    // SgTemplateClassDeclaration derived from SgClassDeclaration. A better
    // solution might be to fix this to use the SageBuilder API directly. Make
  ...
  if (enumFunctionType == e_copy_constructor ||
      enumFunctionType == e_assignment_operator) {
    SgInitializedName *rhs =
        buildInitializedName("rhs", buildReferenceType(base_type1));
    parameterList = buildFunctionParameterList(rhs);
  } else {
  ...
  special_member_kind specialKind = special_member_kind::none;
  if ((enumFunctionType == e_constructor) ||
      (enumFunctionType == e_copy_constructor)) {
    specialKind = special_member_kind::constructor;
  } else if (enumFunctionType == e_destructor) {
    specialKind = special_member_kind::destructor;
  } else if (enumFunctionType == e_assignment_operator) {
    specialKind = special_member_kind::overloaded_operator;
  } else {
    fprintf(stderr,
            "REX_AST_INVARIANT[default-function-special-member-kind]: "
            "unsupported default-function kind=%d\n",
            static_cast<int>(enumFunctionType));
    ROSE_ABORT();
  }
  ...
  const SageBuilder::function_declaration_ownership ownership =
      SageBuilder::function_declaration_ownership::sourceLexicalIn(
          parentClassDef1)
          .withSpecialMemberKind(specialKind);
  SgMemberFunctionDeclaration *func = buildDefiningMemberFunctionDeclaration(
      ownership, func_name, func_return_type, parameterList, parentClassDef1);
  if (func == nullptr || func->get_definition() == nullptr ||
      func->get_definition()->get_body() == nullptr ||
      func->get_parameterList() != parameterList ||
      func->get_parent() != parentClassDef1) {
    fprintf(stderr,
            "REX_AST_INVARIANT[default-function-builder-transaction]: "
            "class=%p function=%p name=%s did not publish one exact defining "
            "special-member subtree\n",
            static_cast<void *>(parentClassDef1), static_cast<void *>(func),
            func_name.str());
    ROSE_ABORT();
  }
  if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
 ...
 *                      prepended.
 */
void DefaultFunctionGenerator::generateDefaultFunctionCall(
    SgMemberFunctionDeclaration *methodFunc, SgClassDefinition *parentClassDef,
    SgExpression *receiver, SgExpression *arg, SgBasicBlock *basicBlock) {
  ...
  ROSE_ASSERT(enumFunctionType == e_destructor ||
              enumFunctionType == e_assignment_operator);
  ...
  if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
    if (enumFunctionType == e_constructor)
      cout << "Generating constructor call for "
           << parentClassDef->get_qualified_name().str() << endl;
    else if (enumFunctionType == e_destructor)
      cout << "Generating destructor call for "
           << parentClassDef->get_qualified_name().str() << endl;
    else if (enumFunctionType == e_copy_constructor)
      cout << "Generating copy constructor call for "
           << parentClassDef->get_qualified_name().str() << endl;
  }
  ...
  SgMemberFunctionSymbol *functionSymbol =
      isSgMemberFunctionSymbol(func->get_symbol_from_symbol_table());
  ...
  SgMemberFunctionRefExp *functionRefExp =
      buildMemberFunctionRefExp(functionSymbol, virtual_call, needs_qualifier);
  ...
  if (isSgPointerType(receiverType)) {
    ...
    function =
        buildArrowExp(receiver, functionRefExp, functionRefExp->get_type());
    ROSE_ASSERT(receiver != NULL);
  } else {
    // The receiver is not a pointer; create a dot expression.
    function =
        buildDotExp(receiver, functionRefExp, functionRefExp->get_type());
    ROSE_ASSERT(receiver != NULL);
  }
  ...
  SgExprStatement *exprStatement = buildExprStatement(functionCallExp);

  // Prepend the expression statement to the basic block.
  // NB:  appending might put this after a return statement!
  SageInterface::publishGeneratedSubtreeOutputOwner(exprStatement, basicBlock);
 ...
 *                               will be appended.
 */
void DefaultFunctionGenerator::generateDefaultConstructorCall(
    SgMemberFunctionDeclaration *methodFunc, SgClassDefinition *parentClassDef,
    SgExpression *arg, SgCtorInitializerList *ctorInitializerList,
    SgInitializedName::preinitialization_enum preinitialization) {
  SgSpecialFunctionModifier sfMod = methodFunc->get_specialFunctionModifier();
  ...
  ROSE_ASSERT(enumFunctionType == e_constructor ||
              enumFunctionType == e_copy_constructor);
  ...
  if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
    if (enumFunctionType == e_constructor)
      cout << "Generating constructor call for "
           << parentClassDef->get_qualified_name().str() << " within method: "
           << ctorInitializerList->get_parent()->unparseToString() << endl;
    else if (enumFunctionType == e_destructor)
      cout << "Generating destructor call for "
           << parentClassDef->get_qualified_name().str() << endl;
    else if (enumFunctionType == e_copy_constructor)
  ...
  */
  ...
  SgConstructorInitializer *ctorInitializer = buildConstructorInitializer(
      func, args, classDeclaration->get_type(), need_name, need_qualifier,
  ...
  SgInitializedName *initializedName =
      buildInitializedName(className, classType, ctorInitializer);
  appendGeneratedCtorInitializer(ctorInitializerList, initializedName,
                                 preinitialization);
}
 ...
 *  base class is virtual.
 */
void DefaultFunctionGenerator::generateImplicitInvocations(
    SgFunctionDefinition *functionDefinition) {
  ...
  if (isConstructor(memberFunctionDeclaration)) {
    ...
    SgThisExp *thisExp =
        getThisExp(classDeclaration, memberFunctionDeclaration);
    findClassesInvokedViaDestructors(functionDefinition, thisExp,
                                     invokedBaseClasses);
    delete thisExp;
    ...
    SgThisExp *thisExp =
        getThisExp(classDeclaration, memberFunctionDeclaration);
    findClassesInvokedViaOperatorEquals(functionDefinition, thisExp,
                                        invokedBaseClasses);
  ...
  // isSgClassDeclaration(targetClassDefinition->get_declaration());
  ...
  std::set<SgClassDeclaration *> invokedBaseClassDeclarations;
  for (std::set<SgClassDeclaration *>::iterator it = invokedBaseClasses.begin();
    ...
    baseClassDecl = canonicalClassDeclaration(baseClassDecl);
    invokedBaseClassDeclarations.insert(baseClassDecl);

    if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
      cout << "invoked base class: " << baseClassDecl->get_qualified_name()
           << endl;
    }
  }
  ...
  for (SgBaseClassPtrList::iterator i = baseClassList.begin();
    ...
    ROSE_ASSERT(baseClassModifier != nullptr);
    const SgInitializedName::preinitialization_enum basePreinitialization =
        baseClassModifier->isVirtual()
    ...
    SgClassDeclaration *canonicalBaseClassDeclaration =
        canonicalClassDeclaration(baseClassDeclaration);
    SgClassType *baseClassType = baseClassDeclaration->get_type();
    ASSERT_not_null(baseClassType);
    if (invokedBaseClassDeclarations.find(canonicalBaseClassDeclaration) ==
        invokedBaseClassDeclarations.end()) {

      invokedBaseClassDeclarations.insert(canonicalBaseClassDeclaration);

      if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
        cout << "uninvoked base class: "
             << canonicalBaseClassDeclaration->get_qualified_name() << endl;
      }
      ...
      if (isCopyConstructor(memberFunctionDeclaration)) {
        if (compilerGenerated) {
          ...
          generateDefaultConstructorCall(
              targetFunctionDeclaration, baseClassDefn, arg,
              ctorInitializerList, basePreinitialization);
        } else {
          ...
          // The invocation should be appended to the copy constructor's
          // constructor initializer list.
          targetFunctionDeclaration = getMethodInClass(NULL, baseClassDefn);
          generateDefaultConstructorCall(
              targetFunctionDeclaration, baseClassDefn, arg,
              ctorInitializerList, basePreinitialization);
        }
        ...
        // initializer list.
        targetFunctionDeclaration = getMethodInClass(NULL, baseClassDefn);
        generateDefaultConstructorCall(targetFunctionDeclaration, baseClassDefn,
                                       arg, ctorInitializerList,
                                       basePreinitialization);
      } else if (isDestructor(memberFunctionDeclaration)) {
        ...
        SgThisExp *thisExp =
            getThisExp(classDeclaration, memberFunctionDeclaration);
        SgPointerType *pointerType = buildPointerType(baseClassType);

        SgCastExp *lhs = buildCastExp(thisExp, pointerType);
        targetFunctionDeclaration =
            getMethodInClass(memberFunctionDeclaration, baseClassDefn);
        generateDefaultFunctionCall(targetFunctionDeclaration, baseClassDefn,
                                    lhs, arg, basicBlock);
      } else if (isOperatorEquals(memberFunctionDeclaration)) {
        if (compilerGenerated) {
          ...
          /**
              ...
              Unfortunately, this will result in infinite recursion.  The
             operator= I introduced is actually Foo::operator=, where Foo is the
             base class of FooBar.  How can I force the unparser to qualify
             operator=:
             ...
             f, 0, functype, 0); fr->set_need_qualifier(false);

              i.e., the last need_qualifier arg to the constructor is false.  I
             then explicitly set need_qualifier to false.  Presumably, this is
             redundant. If I set these to true, I get:

                FooBar &operator=(const class FooBar &rhs)
              {
                  =rhs;

              I suppose I could cast rhs to be of type Foo, so that I wouldn't
             need it to be qualified.  i.e.,
          ...
          */
          ...
          /**
              ...
              Subject: Re: constructor normalization: introducing name
             qualifiers
          ...
          */
          ...
          SgReferenceType *referenceType = buildReferenceType(baseClassType);

          SgCastExp *rhs = buildCastExp(arg, referenceType);
...
SgClassDefinition *getClassDefinition(SgType *objType) {
  ...
  SgDeclarationStatement *objClsDefDeclStmt =
      objClsDecl->get_definingDeclaration();
  ...
  // It is legal to instantiate a templated class with an undefined parameter
  // with a function that has as one of its parameters or local variables an
  // object of this type.  If we encounter a case such as this, we can assume
  // the function is never called and thus has no bearing on any analysis.
  if (objClsDef == NULL) {
    return NULL;
  }
...
DefaultFunctionGenerator::findOrGenerateDefaultFunctionDeclaration(
    defaultEnumFunctionType enumFunctionType,
    SgClassDefinition *parentClassDef1) {
  SgMemberFunctionDeclaration *fnDecl =
      findDefaultFunctionDeclaration(enumFunctionType, parentClassDef1);
  if (fnDecl) {
    return fnDecl;
  } else {
    return generateDefaultFunctionDeclaration(enumFunctionType,
                                              parentClassDef1);
  }
}

void DefaultFunctionGenerator::normalizeCtorInitializer(
  ...
  if (objClsDef->get_declaration()->get_name() ==
...
SgConstructorInitializer *DefaultFunctionGenerator::normalizeAssignInitializer(
  ...
  SgConstructorInitializer *newCtorInit = buildConstructorInitializer(
      copyCtorDecl, rhsArgList, exp_type, need_name, need_qualifier,
      need_parenthesis_after_name, associated_class_unknown);
...
SgInitializer *
DefaultFunctionGenerator::normalizeInitializer(SgInitializer *origInit,
                                               SgType *objType) {
  ...
  if (origInit == NULL) {
    ...
    SgConstructorInitializer *newCtorInit = buildConstructorInitializer(
        defaultCtorDecl, emptyArgList, exp_type, need_name, need_qualifier,
        need_parenthesis_after_name, associated_class_unknown);
  ...
  if (SgAssignInitializer *origAssignInit = isSgAssignInitializer(origInit)) {
    SgConstructorInitializer *newCtorInit =
        normalizeAssignInitializer(origAssignInit);
    if (newCtorInit != NULL) {
      return newCtorInit;
    } else {
  ...
  if (SgConstructorInitializer *origCtorInit =
          isSgConstructorInitializer(origInit)) {
    if (origCtorInit->get_class_decl() == NULL) {
      ...
      // AssignInitializer.
      SgExpressionPtrList &origCtorInitParams =
          origCtorInit->get_args()->get_expressions();
      ROSE_ASSERT(origCtorInitParams.size() == 1);

      SgTreeCopy tc;
      SgExpression *origCtorInitParamCopy =
          isSgExpression(origCtorInitParams.front()->copy(tc));
      SgAssignInitializer *newAssignInit = buildAssignInitializer(
          origCtorInitParamCopy, origCtorInit->get_type());
  ...
  if (SgAggregateInitializer *origAggrInit =
          isSgAggregateInitializer(origInit)) {
    return origAggrInit;
  }

  cout
      << "DefaultFunctionGenerator::normalizeInitializer: unknown case (type = "
      << origInit->class_name() << ")" << endl;
  ROSE_ABORT();
...
SgFunctionCallExp *
DefaultFunctionGenerator::translateAssignmentToOperatorEqCall(
    SgExpression *lhs, SgExpression *rhs, SgClassDefinition *clsDef) {
  SgMemberFunctionDeclaration *operatorEqDecl =
      findOrGenerateDefaultFunctionDeclaration(e_assignment_operator, clsDef);
  if (operatorEqDecl == NULL) {
    cout << clsDef->unparseToString();
  }
  ROSE_ASSERT(operatorEqDecl != NULL);

  //     SgMemberFunctionSymbol *operatorEqSym =
  //     isSgMemberFunctionSymbol(clsDef->lookup_function_symbol("operator="));
  //     // XXX: : won't work if the class defines an operator= with parameter
  //     of different type.  The same also applies to
  //     findDefaultFuncitonDeclaration... needs to be fixed!
  SgMemberFunctionSymbol *operatorEqSym =
      isSgMemberFunctionSymbol(operatorEqDecl->get_symbol_from_symbol_table());
  ROSE_ASSERT(operatorEqSym != NULL);

  bool virtual_call = false;   //! TODO correct?
  bool need_qualifier = false; //! TODO correct?
  SgMemberFunctionRefExp *operatorEqRef =
      buildMemberFunctionRefExp(operatorEqSym, virtual_call, need_qualifier);
  ...
  SgDotExp *dotExp =
      buildDotExp(lhsCopy, operatorEqRef, operatorEqRef->get_type());
  ...
  SgFunctionType *operatorEqType = operatorEqDecl->get_type();
  ROSE_ASSERT(operatorEqType != nullptr);
  ROSE_ASSERT(operatorEqType->get_return_type() != nullptr);
  SgFunctionCallExp *operatorEqCall = buildFunctionCallExp(
      dotExp, operatorEqType->get_return_type(), paramList);
  ...
  if (pass == 1) {
    ...
    case V_SgFunctionDefinition: {
      ...
      if (memberFunctionDeclaration != NULL) {
        ...
        if (DEBUG_DEFAULT_FUNCTION_GENERATOR) {
          std::cout << "Visiting memberFunctionDecl: "
                    << memberFunctionDeclaration->get_name().getString()
                    << std::endl;
        }
    ...
    case V_SgTemplateInstantiationDefn: {
      ...
      if (findDefaultFunctionDeclaration(e_constructor, classDefinition) ==
          NULL)
      ...
      if (findDefaultFunctionDeclaration(e_copy_constructor, classDefinition) ==
          NULL)
        generateDefaultFunctionDeclaration(e_copy_constructor, classDefinition);
      if (findDefaultFunctionDeclaration(e_assignment_operator,
                                         classDefinition) == NULL)
        generateDefaultFunctionDeclaration(e_assignment_operator,
                                           classDefinition);
  ...
  if (pass == 2) {
    ...
    case V_SgInitializedName: {
      ...
      if (SgVariableDeclaration *varDecl =
        ...
        // if (varDecl->get_linkage() != NULL ||
        // varDecl->get_declarationModifier().get_storageModifier().isExtern())
        if (varDecl->get_linkage().empty() == false ||
      ...
      // driscoll6 (01/26/2011) SgInitializedNames in SgFunctionParameterLists
      // should not be normalized.
      if (isSgFunctionParameterList(initName->get_parent()) != NULL) {
        break;
      }

      SgInitializer *newInit = normalizeInitializer(initName->get_initializer(),
                                                    initName->get_type());
    ...
    case V_SgNewExp: {
      ...
      if (SgAssignInitializer *builtinArgsAssignInit =
              isSgAssignInitializer(builtinArgs)) {
        ...
        SgInitializer *newCtorArgs = normalizeInitializer(ctorArgs, t);
    ...
    case V_SgAssignInitializer: {
      ...
      if (SgConstructorInitializer *ctorInit =
        ...
        SgConstructorInitializer *newCtorInit =
            normalizeAssignInitializer(assignInit);
        if (newCtorInit != NULL) {
          SgExpression *assignInitParent =
              isSgExpression(assignInit->get_parent());
    ...
    case V_SgAssignOp: {
      ...
      SgFunctionCallExp *fnCall = translateAssignmentToOperatorEqCall(
          lhs, assignOp->get_rhs_operand(), clsDef);
