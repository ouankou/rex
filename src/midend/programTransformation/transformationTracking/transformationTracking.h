 ...
 *
 *  To store transformation info., we use unique ID based on a pre-order
 * traversal order to identify a node. It is most for convenience and
 * persistence across different executions. Memory addresses can change from one
 * run to another. But the Logic ID based on traversal should be the same even
 * with different
 *
 *  TODO: Moving is tricky to model: it introduces the same input and output
 * nodes In this case, the node itself cannot be directly used since the node
 * can be moved around several times during transformation process. Each time it
 * should have a new ID. This complicate implementation. We don't track this for
 * now
 *
 ...
 *
 *  A challenge is the granularity of tracking: statement level vs. expression
 * level Liao 3/24/2014
 */
...
//  Markus reminded me that this will increase the size of AST for any
//  situation. He suggested a more flexible way is to use separated data
//  structures (map, vector, etc.) to maintain the IDs for each SgNode.
//
...
//
// This should be called after the transformation is completed and all IDs are
// assigned.
...
// This is also the same as the total number of IDs + 1. It can be used to
// control loop iteration over all IDs
