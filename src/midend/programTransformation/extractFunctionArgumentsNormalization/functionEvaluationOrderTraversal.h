...
// sage3basic.h. #include "rose.h" rose.h and sage3basic.h should not be
...
struct FunctionCallInheritedAttribute {
  ...
  /** Is the current node inside a for loop or conditional expresion structure
   * (not the body). */
  enum {
    INSIDE_FOR_INIT,
    INSIDE_FOR_TEST,
  ...
  FunctionCallInheritedAttribute()
      : currentScope(NULL), lastStatement(NULL), scopeStatus(IN_SAFE_PLACE) {}
};

/** Stores a function call expression, along with associated information about
 * its context. */
struct FunctionCallInfo {
  /** The function call expression. */
  SgFunctionCallExp *functionCall;

  /** When a variable is created to replace one of the arguments of this
   * function, where should it be inserted? The declaration of the variable will
   * occur right before this statement. */
  ...
  FunctionCallInfo(SgFunctionCallExp *function)
      : functionCall(function), tempVarDeclarationLocation(NULL),
        tempVarDeclarationInsertionMode(INVALID) {}
};

//! Traverses a given AST and finds all function calls in the order in which
//! they're evaluated Also, for each function we find where to put declarations
//! of temporary variables so that they're accessible at the function call

class FunctionEvaluationOrderTraversal
    : public AstTopDownBottomUpProcessing<FunctionCallInheritedAttribute,
                                          bool> {
public:
  /** Traverses the subtree of the given AST node and finds all function calls
   * in function-evaluation order. */
  static std::pair<std::vector<FunctionCallInfo>, std::vector<FunctionCallInfo>>
...
private:
  //! Private constructor. Use the static method to access the functionality of
  //! this class.
