...
FunctionEvaluationOrderTraversal::evaluateInheritedAttribute(
    SgNode *astNode, FunctionCallInheritedAttribute parentAttribute) {
  FunctionCallInheritedAttribute result = parentAttribute;
  SgForStatement *parentForLoop =
      isSgForStatement(parentAttribute.currentScope);
  SgWhileStmt *parentWhileLoop = isSgWhileStmt(parentAttribute.currentScope);
  SgDoWhileStmt *parentDoWhileLoop =
      isSgDoWhileStmt(parentAttribute.currentScope);
  ...
  SgOrOp *parentOrOp =
      astNode->get_parent() ? isSgOrOp(astNode->get_parent()) : nullptr;
  ...
  //     result.currentScope = isSgOrOp(astNode);
    ...
    ASSERT_require(result.scopeStatus ==
                   FunctionCallInheritedAttribute::IN_SAFE_PLACE);
  ...
  } else if (parentForLoop != nullptr &&
             parentForLoop->get_increment() == astNode) {
    ASSERT_require(result.scopeStatus ==
                   FunctionCallInheritedAttribute::IN_SAFE_PLACE);
    result.scopeStatus = FunctionCallInheritedAttribute::INSIDE_FOR_INCREMENT;
  } else if (parentWhileLoop != nullptr &&
             parentWhileLoop->get_condition() == astNode) {
    ASSERT_require(result.scopeStatus ==
                   FunctionCallInheritedAttribute::IN_SAFE_PLACE);
    result.scopeStatus = FunctionCallInheritedAttribute::INSIDE_WHILE_CONDITION;
  } else if (parentDoWhileLoop != nullptr &&
             parentDoWhileLoop->get_condition() == astNode) {
    ASSERT_require(result.scopeStatus ==
                   FunctionCallInheritedAttribute::IN_SAFE_PLACE);
    result.scopeStatus =
        FunctionCallInheritedAttribute::INSIDE_DO_WHILE_CONDITION;
  } else if (parentSgConditionalExp != nullptr &&
             parentSgConditionalExp->get_true_exp() == astNode) {
    // if the scope status was safe, turn it into unsafe
    if (IsStatusSafe(result.scopeStatus))
      result.scopeStatus =
          FunctionCallInheritedAttribute::INSIDE_CONDITIONAL_EXP_TRUE_ARM;
  } else if (parentSgConditionalExp != nullptr &&
             parentSgConditionalExp->get_false_exp() == astNode) {
    // if the scope status was safe, turn it into unsafe
    if (IsStatusSafe(result.scopeStatus))
      result.scopeStatus =
          FunctionCallInheritedAttribute::INSIDE_CONDITIONAL_EXP_FALSE_ARM;
  } else if (parentOrOp != nullptr &&
             parentOrOp->get_rhs_operand() == astNode) {
    // if the scope status was safe, turn it into unsafe
    if (IsStatusSafe(result.scopeStatus))
      result.scopeStatus =
          FunctionCallInheritedAttribute::INSIDE_SHORT_CIRCUIT_EXP_RHS;
  } else if (parentAndOp != nullptr &&
             parentAndOp->get_rhs_operand() == astNode) {
    // if the scope status was safe, turn it into unsafe
    if (IsStatusSafe(result.scopeStatus))
      result.scopeStatus =
          FunctionCallInheritedAttribute::INSIDE_SHORT_CIRCUIT_EXP_RHS;
  ...
  // We can't insert variables before an expression statement that appears
  // inside if(), switch, throw, etc.
  if (isSgExprStatement(astNode) && !isSgBasicBlock(astNode->get_parent())) {
    // We can't insert a variable declaration at these locations. Use the parent
    // statement
...
bool FunctionEvaluationOrderTraversal::IsFunctionCallSideEffectFree(
    SgFunctionCallExp * /*functionCall*/) {
  return false;
}

/** Visits AST nodes in post-order. This is function-evaluation order. */
bool FunctionEvaluationOrderTraversal::evaluateSynthesizedAttribute(
    SgNode *astNode, FunctionCallInheritedAttribute parentAttribute,
  ...
  // An alternative is to use comma operators and use assignemnt op as done by
  // the original author. for(;foo(bar());) ==> T i; for(;i=bar();foo(i);) But
  // using assignement op is not always safe and it requires us to always have a
  // default constructor There is also an issue when the return type is a
  // reference and we'll have to use & op to get a pointer but if & op is
  // overloaded we may not get the pointer. Taking all these in view, I am
  // simpling not lifting such expressions.

  if (parentAttribute.scopeStatus ==
          FunctionCallInheritedAttribute::INSIDE_FOR_TEST ||
      parentAttribute.scopeStatus ==
          FunctionCallInheritedAttribute::INSIDE_FOR_INCREMENT ||
      parentAttribute.scopeStatus ==
          FunctionCallInheritedAttribute::INSIDE_WHILE_CONDITION ||
      parentAttribute.scopeStatus ==
          FunctionCallInheritedAttribute::INSIDE_DO_WHILE_CONDITION) {
    ...
    functionCallInfo.tempVarDeclarationLocation =
        isSgStatement(parentAttribute.currentScope);
  ...
  // In future, if the function call is assured to be side effect free, then we
  // can lift it from short circuit and conditional expressions
  if (parentAttribute.scopeStatus ==
          FunctionCallInheritedAttribute::INSIDE_CONDITIONAL_EXP_TRUE_ARM ||
      parentAttribute.scopeStatus ==
          FunctionCallInheritedAttribute::INSIDE_CONDITIONAL_EXP_FALSE_ARM ||
  ...
  // Handle for loops (being inside the body of a for loop doesn't need special
  // handling)
  if (parentAttribute.scopeStatus ==
      FunctionCallInheritedAttribute::INSIDE_FOR_INIT) {
    ...
    functionCallInfo.tempVarDeclarationInsertionMode =
        FunctionCallInfo::INSERT_BEFORE;
  } else if (parentAttribute.scopeStatus ==
             FunctionCallInheritedAttribute::IN_SAFE_PLACE) {
    // Assume we're in a basic block. Then just insert right before the current
    // statement
    ASSERT_require(parentAttribute.scopeStatus =
                       FunctionCallInheritedAttribute::IN_SAFE_PLACE);
