...
//! This normalization makes sure each function call argument is a side-effect
//! free expression of only one variable. To accomplish this, temporary
//! variables are declared and the arguments of a function are evaluated before
//! the function itself. Note: Normalization is not performed if it is unsafe.
//! E,g.: while(Foo(Bar()) {}  is not equal to t = Bar(); while(Foo(t) {} Note
//! that only SgFunctionCall arguments are normalized. For example the arguments
//! of the constructor initializer MyObject o(foo(), bar()) will not be
//! extracted.
class ExtractFunctionArguments {
public:
  /** Performs the function argument extraction on all function calls in the
   * given subtree of the AST. */
  /** It does not do transofrmations in places where it is not safe. If you pass
   * doUnsafeNormalization= true, we will normalize all call sites ignoring the
  ...
  /*
   ...
   function call is a either trivial arguemnt or is present in a SAFE place from
...
private:
  ...
  /** Given the expression which is the argument to a function call, returns
   * true if that expression should be pulled out into a temporary variable on a
  ...
  /** Returns true if any of the arguments of the given function call will need
   * to be extracted. */
  ...
  /** Given the information about a function call (obtained through a
   * traversal), extract its arguments into temporary variables where it is
   * necessary. */
   ...
   */
  void InsertStatement(SgStatement *newStatement, SgStatement *location,
  ...
  /* Given a vector of function call sites returns true if every argument of
   every function call is a trivial expression (IsFunctionArgumentTrivial). Such
