...
void BreakupStatement::cmdline_configure(
    const std::vector<std::string> &argv,
    std::vector<std::string> *unknown_args) {
  unsigned index = 0;
  if (!cmdline_find(argv, index, "-bs", unknown_args))
    return;
  ++index;
  if (index < argv.size())
...
bool BreakupStatement::operator()(AstInterface &fa, const AstNodePtr &s,
  ...
  if (fa.IsAssignment(s, &lhs, &rhs)) {
    ...
    while (fa.IsBinaryOp(exp, &opr, &opd1, &opd2) &&
      ...
      if (refList.size() >= breaksize) {
        bool alias = false;
        for (SinglyLinkedListWrap<AstNodePtr>::Iterator p(refList);
             !p.ReachEnd(); ++p) {
        ...
        AstNodePtr newvar =
            (alias) ? fa.CreateVarRef(fa.NewVar(lhstype)) : fa.CopyAstTree(lhs);
    ...
    for (std::list<AstNodePtr>::reverse_iterator p = stmtlist.rbegin();
         p != stmtlist.rend(); ++p) {
      AstNodePtr cur = *p;
      fa.InsertStmt(s, cur);
    }
