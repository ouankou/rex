...
protected:
  virtual CompSliceStmt *CreateSliceStmtNode(LoopTreeNode *n,
                                             CompSliceStmt *_that = 0) {
    CondSliceStmt *that =
        (_that == 0 || _that->CompSliceGetClassName() != "CondSliceStmt")
            ? 0
            : static_cast<CondSliceStmt *>(_that);
    size_t gi =
        (that == 0) ? groupIndex : groupIndex + that->GetSliceGroupIndex();
    if (gi > groupNum)
      groupNum = gi;
...
unsigned
DynamicCompSlice::QuerySliceStmtGroupIndex(const LoopTreeNode *n) const {
  CondSliceStmt *sliceStmt =
      static_cast<CondSliceStmt *>(GetImpl()->QuerySliceStmt(n));
...
LoopTreeNode *DynamicSlicing::Transform(LoopTreeDepComp &c,
                                        const CompSlice *_slice,
                                        LoopTreeNode *root) {
  ...
  if (num > 1) {
    std::string groupVar = fa.NewVar(fa.GetType("int")),
                groupVarN = groupVar + "N";
    fa.NewVar(fa.GetType("int"), groupVarN);
    LoopTreeCreate *tc = c.GetLoopTreeCreate();
    nr = tc->CreateLoopNode(SymbolicVar(groupVar, AST_NULL), 1,
                            SymbolicVar(groupVarN, AST_NULL), 1);
    ...
    int id = 0;
    AstNodePtr config = LoopTransformInterface::CreateDynamicFusionConfig(
        fa.CreateVarRef(groupVarN), args, id);
