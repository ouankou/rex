...
CompSlice ::CompSlice(CompSliceImpl *_impl) : impl(_impl) {
...
void CompSlice::SetSliceLoop(LoopTreeNode *s, LoopTreeNode *l,
...
CompSlice::SliceStmtInfo
CompSlice ::QuerySliceStmtInfo(const LoopTreeNode *n) const {
  CompSliceStmt *stmnt = impl->QuerySliceStmt(n);
  assert(stmnt != NULL);
  return stmnt->GetSliceInfo();
}

CompSlice::SliceLoopInfo
CompSlice ::QuerySliceLoopInfo(const LoopTreeNode *n) const {
...
int CompSlice ::QuerySliceLevel() const { return impl->QuerySliceLevel(); }
...
bool CompSlice ::SliceLoopReversible() const {
  for (ConstLoopIterator iter = GetConstLoopIterator(); iter.Current() != 0;
       iter.Advance()) {
...
class CompSliceNest::ObserveImpl : public ObserveObject<CompSliceNestObserver> {
};
...
SymbolicBound SliceLoopRange(const CompSlice *slice, LoopTreeNode *root) {
  ...
  for (LoopTreeNode *stmt; (stmt = stmtIter.Current()); stmtIter++) {
    ...
    SymbolicBound b = LoopTreeGetVarConstBound(info.loop, root->Parent())
                          .GetConstBound(l->GetVar());
    b.lb = b.lb - info.align;
    b.ub = b.ub - info.align;
    result.Union(b, &bf);
  }
  return result;
}
...
SymbolicVar SliceLoopIvar(AstInterface &fa, const CompSlice *slice) {
  ...
  for (LoopTreeNode *loop; (loop = loopIter.Current()); loopIter.Advance()) {
    ...
    for (LoopTreeNode *stmt; (stmt = stmtIter.Current()); stmtIter.Advance()) {
      for (LoopTreeNode *l = GetEnclosingLoop(stmt, looptreeInterface); l != 0;
           l = GetEnclosingLoop(l, looptreeInterface)) {
        if (l == loop)
          continue;
  ...
  for (nameset::iterator p = sliceVars.begin(); p != sliceVars.end(); ++p) {
...
CompSlice::ConstLoopIterator::ConstLoopIterator(LoopTreeNode *root)
...
LoopTreeNode *CompSlice::ConstStmtIterator::Current() const {
  ...
  return (cur == 0) ? 0 : static_cast<CompSliceStmt *>(cur)->GetSliceStmt();
}
...
LoopTreeNode *CompSlice::ConstLoopIterator::Current() const {
  ...
  return (cur == 0) ? 0 : static_cast<CompSliceLoop *>(cur)->GetSliceLoop();
}
...
bool CompSlice::ConstLoopIterator::CurrentLoopReversible() const {
  LoopTreeNode *cur = LoopTreeTraverseSelectLoop::Current();
...
void CompSlice::UpdateStmtIterator::UpdateSplitStmt(const SplitStmtInfo &info) {
...
void CompSlice::UpdateStmtIterator::UpdateDeleteNode(const LoopTreeNode *n) {
...
void CompSlice::UpdateLoopIterator::UpdateDistNode(const DistNodeInfo &info) {
  const LoopTreeNode *o = info.GetObserveNode();
...
void CompSlice::UpdateLoopIterator::UpdateDeleteNode(const LoopTreeNode *n) {
  CompSlice::ConstLoopIterator::Advance();
  Attach();
}

CompSlice::ConstStmtIterator
CompSlice::ConstLoopIterator::GetConstStmtIterator() const {
  LoopTreeNode *cur = LoopTreeTraverseSelectLoop::Current();
  return CompSlice::ConstStmtIterator(cur);
}

CompSlice::UpdateStmtIterator
CompSlice::UpdateLoopIterator::GetUpdateStmtIterator() const {
  LoopTreeNode *cur = LoopTreeTraverseSelectLoop::Current();
  return CompSlice::UpdateStmtIterator(cur);
}

/* QY: can be used find common root of two nodes under parent but currently not
being used class FindCommonInterface { LoopTreeNode* parent; public:
