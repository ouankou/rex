...
public:
  CompSliceLoop(LoopTreeNode *l, CompSliceImpl *impl, bool r = true);
...
protected:
  ...
  virtual void UpdateSplitStmt(const SplitStmtInfo &info) {
    ...
    SplitStmtInfo info1(this, that, info.GetLoop1(), info.GetLoop2(),
                        info.GetRel());
    Notify(info1);
  }
  virtual void UpdateDeleteNode(const LoopTreeNode *n) { delete this; }
...
public:
  ...
  CompSlice::SliceStmtInfo GetSliceInfo() const {
    ...
    return CompSlice::SliceStmtInfo(l->GetSliceLoop(), l->LoopReversible(),
                                    align);
  }
...
inline CompSlice::SliceLoopInfo CompSliceLoop::GetSliceInfo() const {
  ...
  for (CompSliceStmt *s = static_cast<CompSliceStmt *>(FirstChild()); s != 0;
       s = static_cast<CompSliceStmt *>(s->NextSibling())) {
...
protected:
  ...
  virtual CompSliceLoop *CreateSliceLoopNode(LoopTreeNode *n,
                                             CompSliceLoop *that = 0) {
    bool r = (that == 0) ? true : that->LoopReversible();
    return new CompSliceLoop(n, this, r);
  }
  virtual CompSliceStmt *CreateSliceStmtNode(LoopTreeNode *n,
                                             CompSliceStmt *that = 0) {
    return new CompSliceStmt(n, this);
  }
...
public:
  ...
  CompSliceStmt *QuerySliceStmt(const LoopTreeNode *n) const {
    return (IsSimpleStmt(n)) ? static_cast<CompSliceStmt *>(QueryShadowNode(n))
                             : 0;
  }
  CompSliceLoop *QuerySliceLoop(const LoopTreeNode *n) const {
    return n->IncreaseLoopLevel()
               ? static_cast<CompSliceLoop *>(QueryShadowNode(n))
               : 0;
  }
...
inline CompSliceLoop::CompSliceLoop(LoopTreeNode *l, CompSliceImpl *_slice,
                                    bool r)
    : LoopTreeShadowNode(l, _slice), reversible(r) {
  l->AttachObserver(*this);
}

inline CompSliceStmt::CompSliceStmt(LoopTreeNode *s, CompSliceImpl *_slice,
                                    int a)
