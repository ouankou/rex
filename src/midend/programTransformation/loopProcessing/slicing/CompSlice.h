...
public:
  ...
  public:
    ConstStmtIterator(LoopTreeNode *r) : LoopTreeTraverseSelectStmt(r) {}
  ...
  public:
    UpdateStmtIterator(LoopTreeNode *root) : ConstStmtIterator(root) {
      Attach();
    }
    ...
    void Reset() {
      Detach();
      ConstStmtIterator::Reset();
      Attach();
    }
    void Advance() {
      Detach();
      ConstStmtIterator::Advance();
      Attach();
    }
  ...
  public:
    UpdateLoopIterator(LoopTreeNode *root) : ConstLoopIterator(root) {
      Attach();
    }
    ...
    void Reset() {
      Detach();
      ConstLoopIterator::Reset();
      Attach();
    }
    void Advance() {
      Detach();
      ConstLoopIterator::Advance();
      Attach();
    }
    void operator++() { Advance(); }
    void operator++(int) { Advance(); }
...
public:
  ...
  virtual LoopTreeNode *Transform(LoopTreeDepComp &c, const CompSlice *slice,
                                  LoopTreeNode *root);
...
public:
  ...
  /* QY: 6/2009: if saveAsInner, the slice entry is removed but its storage is
   * not deleted*/
