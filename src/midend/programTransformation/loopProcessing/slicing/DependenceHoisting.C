...
struct SliceInfo {
  ...
  SliceInfo(LoopTreeNode *s = 0, LoopTreeNode *l = 0, int level = -1,
      ...
      : loop(l), stmt(s), looplevel(level), reversible(r),
        alignInfo(mina, maxa) {}
  void SetLoop(LoopTreeNode *l, int level) {
    loop = l;
    looplevel = level;
  }
...
public:
  ...
  int FuseInfoOffset(int i1, int i2) const { return i1 * maxsize / 2 + i2 - 1; }
...
void GetLoopTreeSize(LoopTreeNode *root, int &stmtnum, int &stmtlevel) {
  stmtnum = 0;
  stmtlevel = 0;
  for (LoopTreeTraverseSelectStmt stmtIter(root); !stmtIter.ReachEnd();
       ++stmtIter) {
    ++stmtnum;
...
void DependenceHoisting ::Analyze(LoopTreeDepComp &comp,
                                  LoopTreeTransDepGraphCreate *tg,
  ...
  LoopAlignInfo *buf1 = new LoopAlignInfo[slicesize],
                *buf2 = new LoopAlignInfo[slicesize];
  for (int i = 0; i < size; ++i)
  ...
  for (int sliceindex = 0; sliceindex < size; ++sliceindex) {
    ...
    for (stmtIter++; (stmt = stmtIter.Current()); stmtIter++) {
      ...
      for (; index >= rootlevel;
           loop = GetEnclosingLoop(loop, looptreeInterface)) {
...
void DependenceHoisting ::Analyze(LoopTreeDepComp &comp,
  ...
  typedef PerfectLoopReversible<DepInfoEdge, LoopTreeDepGraph> LoopReversible;
  ...
  for (LoopTreeNode *n = root; n;
       n = (n->ChildCount() == 1) ? n->FirstChild() : 0) {
    if (!n->IncreaseLoopLevel())
      continue;
    index++;
    if (!LoopSlicable()(dg, index)) {
      DebugSlice([&index]() {
        return "Loop " + std::to_string(index) + " is not sliceable.";
      });
      continue;
    }
    CompSlice *slice = CreateCompSlice(rootlevel);
...
LoopTreeNode *DependenceHoisting::Transform(LoopTreeDepComp &comp,
  ...
  SymbolicVar ivar =
      SliceLoopIvar(LoopTransformInterface::getAstInterface(), slice);
  SymbolicVal step = SliceLoopStep(slice).step;
  SymbolicBound bound = SliceLoopRange(slice, h1);
  LoopTreeNode *h2 = (step > 0) ? comp.GetLoopTreeCreate()->CreateLoopNode(
                                      ivar, bound.lb, bound.ub, step)
                                : comp.GetLoopTreeCreate()->CreateLoopNode(
                                      ivar, bound.ub, bound.lb, step);
    ...
    if (loopIter.CurrentInfo().stmtcount < CountEnclosedStmts(loop)) {
      ...
      for (LoopTreeNode *s; (s = inStmts.Current());) {
        ...
        if (info1.loop != loop) {
          ...
          DepRel r1 = comp.GetDomain(s).Entry(info1.loop->LoopLevel(),
                                              loop->LoopLevel());
          DepRel r2 = r & r1;
          if (r2 != r1 && !r2.IsTop())
