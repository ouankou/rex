...
CompSliceDepGraphCreate ::CompSliceDepGraphCreate(
  ...
  DebugSlice([&comp]() {
    return "Creating CompSliceDepGraph from " + comp.TreeToString();
  });
  for (LoopTreeNode *n = root->FirstChild(); n != 0; n = n->NextSibling()) {
    ...
    if (tc == 0 || !n->ContainLoop() || n->IsPerfectLoopNest()) {
      ...
      DebugSlice(
          [&sliceNode]() { return "new slice node:" + sliceNode->toString(); });
    } else {
      DebugSlice([&n]() {
        return "construct transitive dep. graph for " + n->TreeToString();
      });
  ...
  for (GraphEdgeIterator<LoopTreeDepGraph> depIter(depGraph);
    ...
    CompSliceNest *nest1 = src->GetInfo().GetNest(),
                  *nest2 = snk->GetInfo().GetNest();
    ...
    for (int i = 0; i < dim1; i++) {
      ...
      for (int j = 0; j < dim2; j++) {
        CompSlice::SliceStmtInfo info2 =
            nest2->Entry(j)->QuerySliceStmtInfo(s2);
...
CompSliceDepGraphNode ::CompSliceDepGraphNode(MultiGraphCreate *g,
                                              LoopTreeDepComp &c,
...
void CompSliceDepGraphNode::UpdateFusion(const CompSliceNestFusionInfo &info) {
  ...
  if (that->impl->innerNest != 0 || impl->innerNest != 0) {
    ...
    if (p == 0) {
      ...
      for (CompSlice::ConstStmtIterator stmts = g1[0]->GetConstStmtIterator();
...
void CompSliceDepGraphNode::UpdateDeleteEntry(
    const CompSliceNestDeleteEntryInfo &info) {
  ...
  if (impl->innerNest == 0) {
    impl->innerNest =
        new FullNestInfo(const_cast<CompSlice *>(sliceVec[index]), size);
  } else {
...
void CompSliceDepGraphNode::UpdateDuplicateEntry(
    const CompSliceNestDuplicateEntryInfo &info) {
...
void CompSliceDepGraphNode::UpdateAlignEntry(
    const CompSliceNestAlignEntryInfo &info) {
  CompSliceDepGraphCreate *graph =
      static_cast<CompSliceDepGraphCreate *>(GetGraphCreate());
  DepGraphNodeAlignLoop(graph, this, info.GetIndex(), info.GetAlign());
}

void CompSliceDepGraphCreate::MoveEdgeEndPoint(DepInfoEdge *e,
                                               EdgeDirection dir,
...
DepInfoEdge *CompSliceDepGraphCreate::CreateEdge(CompSliceDepGraphNode *n1,
                                                 CompSliceDepGraphNode *n2,
                                                 const DepInfo &info) {
...
LoopTreeNode *
CompSliceDepGraphNode::FullNestInfo::GenXformRoot(LoopTreeNode *top) const {
  ...
  LoopTreeNode *res =
      LoopTreeDistributeNode()(top, sel, LoopTreeDistributeNode::BEFORE);
  assert(res != 0);
  return res;
}

LoopTreeNode *
CompSliceDepGraphNode::EmptyNestInfo::GenXformRoot(LoopTreeNode *top) const {
