...
public:
  ...
  apply(const CompSliceDepGraphNode::FullNestInfo &nestInfo,
        LoopTreeDepComp &comp, DependenceHoisting &op, LoopTreeNode *&top) = 0;

  // DQ (12/6/2016): Added explicit virtual destructor to eliminate warning we
  // want to be an error: -Wdelete-non-virtual-dtor.
  virtual ~LoopPar() {}
};
...
class ParallelizeBlocking : public LoopPar {
  ...
  virtual LoopTransformOptions::OptType GetOptimizationType() {
    return LoopTransformOptions::PAR_LOOP_OPT;
  }
