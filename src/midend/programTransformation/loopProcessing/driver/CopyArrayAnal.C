...
void CopyArrayOperator::operator()(LoopTreeLocalityAnal &tc,
                                   LoopTreeNode *root) {
...
int CopyArrayOperator::OutmostReuseLevel(
  ...
  for (DepCompCopyArrayCollect::CopyArrayUnit::NodeSet::const_iterator p1 =
    ...
    for (int level = copylevel; level < cur->GetInfo().stmt->LoopLevel();
         ++level) {
  ...
  if (DebugCopyRoot()) {
    std::cerr << "outmost reuse level for "
              << IteratorToString2(unit.refs.begin()) << " is " << res
              << std::endl;
  }
  return res;
}

int CopyArrayOperator::EnforceCopyDimension(
    DepCompCopyArrayCollect::CopyArrayUnit &unit,
  ...
  for (DepCompCopyArrayCollect::CopyArrayUnit::NodeSet::const_iterator p1 =
    ...
    const DepCompAstRefGraphNode *cur = *p1;
    int curdim = 0;
    for (int level = cur->GetInfo().stmt->LoopLevel() - 1; level >= copylevel;
         --level) {
      if (!refDep.SelfReuseLevel(cur, level)) {
        ++curdim;
        if (DebugCopyRoot())
          std::cerr << "processing node " << cur << ":  loop at level " << level
                    << "do not carry reuse \n";
      }
...
bool CopyArrayOperator::IsRedundantCopy(
    DepCompCopyArrayCollect::CopyArrayUnit &unit, int copydim) {
  int copylevel = unit.copylevel();
  int dimdiff = 0;
  DepCompCopyArrayCollect::CopyArrayUnit::NodeSet::const_iterator p =
      unit.refs.begin();
  for (; !p.ReachEnd(); ++p) {
  ...
  /*
     for (p.Reset(); !p.ReachEnd(); ++p) {
        ...
        for (int i = 0; i < copydim && loop != unit.root; loop =
     loop->EnclosingLoop(), ++i) if (!ReferenceDimension(ref,
     loop->GetLoopInfo()->GetLoopIndexVar().GetVarName(),i)) return false;
     }
     return true;
  */
  return false;
...
bool CopyArrayOperator::SplitDisconnectedUnit(
...
void CopyArrayUnderSizeLimit::ModifyCopyArrayCollect(
  ...
  for (DepCompCopyArrayCollect::iterator arrays = collect.begin();
    ...
    if (DebugCopySplit() || DebugCopyRoot())
      std::cerr << " modifying copy unit: "
                << IteratorToString2(unit.refs.begin()) << " with root = "
                << ((unit.root == 0) ? "null" : unit.root->toString())
                << std::endl;
    ...
    while (true) {
      ...
      if (cuts.size() == unit.refs.size()) {
        ...
        LoopTreeNode *rootloop =
            (unit.root->GetLoopInfo() == 0) ? 0 : unit.root;
        while (n != rootloop && p != rootloop) {
          n = p;
          p = GetEnclosingLoop(p, looptreeInterface);
        }
        if (DebugCopyRoot())
          std::cerr << "resetting copy root to be " << n->toString()
                    << std::endl;
        unit.root = n;
      ...
      if (DebugCopySplit())
        std::cerr << "Enforce copy dimension by removing "
                  << IteratorToString2(cuts.begin()) << std::endl;
      collect.AddCopyArray() = DepCompCopyArrayCollect::CopyArrayUnit(
          cuts, collect.ComputeCommonRoot(cuts));
      unit.refs -= cuts;
      unit.root = origroot = collect.ComputeCommonRoot(unit.refs);
      unit.root =
          collect.OutmostCopyRoot(unit, refDep, collect.get_tree_root());
    }
    DepCompCopyArrayCollect::CopyArrayUnit::NodeSet cuts;
    if (SplitDisconnectedUnit(collect, unit, refDep, cuts)) {
      ...
      if (DebugCopySplit() || DebugCopyRoot())
        std::cerr << " Spliting disconnected refs: removing "
                  << IteratorToString2(cuts.begin()) << std::endl;
      left -= cuts;
      collect.AddCopyArray() = DepCompCopyArrayCollect::CopyArrayUnit(
          left, collect.ComputeCommonRoot(left));
      unit.refs = cuts;
    ...
    if (origroot != 0 && unit.root != origroot) {
      ...
      if (reuselevel > copylevel) {
        ...
        for (int curlevel = origroot->LoopLevel(); reuselevel <= curlevel;
          ...
          ;
        if (DebugCopyRoot())
          std::cerr << "After reuse anal, resetting copy root to be "
                    << cur->toString() << std::endl;
        unit.root = cur;
        curdim -= (reuselevel - copylevel + 1);
      } else if (DebugCopyRoot())
        std::cerr << "do not reset copy root because copylevel = " << copylevel
                  << " and copy root = " << unit.root->toString() << std::endl;
    }
    DepCompCopyArrayCollect::iterator tmp = arrays;
    ++arrays;
    if (IsRedundantCopy(unit, curdim)) {
      if (DebugCopyRemove()) {
        std::cerr << "remove redundant copy "
                  << IteratorToString2(unit.refs.begin())
                  << " with root = " << unit.root->toString() << std::endl;
      }
...
void ParameterizeCopyArray::ApplyXform(
    DepCompCopyArrayCollect::CopyArrayUnit &curarray, CopyArrayConfig &config,
    LoopTreeNode *repl, LoopTreeNode *init, LoopTreeNode *save) {
...
bool ParameterizeCopyArray::CanBeBlocked(
    DepCompCopyArrayCollect::CopyArrayUnit &unit, int copydim) {
  if (copydim == 0)
    return false;
  ...
  for (int i = 0; i < copydim; ++i) {
    ...
    for (DepCompCopyArrayCollect::CopyArrayUnit::NodeSet::const_iterator p =
      ...
      for (LoopTreeNode *loop = (*p)->GetInfo().stmt->EnclosingLoop();
        ...
        AstNodePtr ref = (*p)->GetInfo().orig;
        if (!ReferenceDimension(ref, loop->GetLoopInfo()->GetVar().GetVarName(),
                                i)) {
          ++j;
          sweeping = true;
          break;
      ...
      if (j > 1)
        return false; /*QY: multiple loops sweeping a dimension*/
    }
    if (sweeping)
      ++k;
  }
  if (k == 0)
    return false; /* QY: no sweeping dimensions*/
  return true;
}

void ParameterizeCopyArray::ModifyCopyArrayCollect(
    DepCompCopyArrayCollect &collect, DepCompAstRefGraphCreate &refDep) {
