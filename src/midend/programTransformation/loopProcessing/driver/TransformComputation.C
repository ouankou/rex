...
void LoopTransformation(LoopTreeDepComp &comp, DependenceHoisting &op,
                        LoopTreeLocalityAnal &anal, int optlevel);
...
int ApplyOptLevel() {
  ...
  if (level < 0) {
    ...
    vector<string>::const_iterator p =
        CmdOptions::GetInstance()->GetOptionPosition("-opt");
...
private:
  ...
  bool ProcessFunctionDefinition(AstInterface &fa, const AstNodePtr &s,
    ...
    DebugOpt([]() {
      return "Seeing function definition inside, setting optimizability to "
             "falses";
    });
    succ = -1;
    return false;
  ...
  bool ProcessDecls(AstInterface &fa, const AstNodePtr &s) {
    if (fa.IsVariableDecl(s) && fa.GetParent(s) != top) {
      DebugOpt([&s]() {
        return "Not optimizable b/c local variable declaration " +
  ...
  bool ProcessLoop(AstInterface &fa, const AstNodePtr &s,
    ...
    if (!fa.IsFortranLoop(s)) {
      DebugOpt([&s]() {
        return "Not Optimizable due to non-fortran loop: " +
               AstInterface::AstToString(s);
      });
      succ = -1;
      return false;
    ...
    if (t == AstInterface::PreVisit) {
      if (loop == 0)
        ...
        DebugOpt([]() {
          return "Not optimizable b/c no optimization is specified";
        });
        succ = -1;
        return false;
      }
  ...
  bool ProcessGoto(AstInterface &fa, const AstNodePtr &s,
  ...
  bool ProcessIf(AstInterface &fa, const AstNodePtr &s, const AstNodePtr &cond,
                 const AstNodePtr &truebody, const AstNodePtr &falsebody,
                 AstInterface::TraversalVisitType t) {
    if (succ < 0)
      return false;
    if (s == top)
...
public:
  ...
  bool operator()(const AstNodePtr &head) {
    ...
    if (optType & LoopTransformOptions::LOOP_DATA_OPT) {
      ProcessAstTree<AstNodePtr>::operator()(
          LoopTransformInterface::getAstInterface(), head);
      if (succ <= 0) {
...
bool LoopTransformation(const AstNodePtr &head, AstNodePtr &result) {
  ...
  bool depOnly =
      outputdep || CmdOptions::GetInstance()->HasOption("-depAnalOnly");
  ...
  DebugOpt([&head]() {
    return "Try applying loop transformation to " +
           AstInterface::AstToString(head);
  });
  if (reportPhaseTiming)
    GetWallTime();
  LoopTreeDepCompCreate comp(head);
  if (reportPhaseTiming)
    std::cerr << "dependence analysis time: " << GetWallTime() << "\n";
  ...
  if (!depOnly && sel.PerformTransformation()) {
    ...
    DebugOpt([&loopAnal]() {
      return "LoopTree input dep graph: " +
             GraphToString(*loopAnal.GetInputGraph());
    });
...
void LoopTransformation(LoopTreeDepComp &comp, DependenceHoisting &op,
                        LoopTreeLocalityAnal &loopAnal, int optlevel) {
  LoopTransformOptions *lopt = LoopTransformOptions::GetInstance();
  ...
  if (!(lopt->GetOptimizationType() & LoopTransformOptions::LOOP_OPT)) {
    ...
    if (cp != 0)
      (*cp)(loopAnal, comp.GetLoopTreeRoot());
    return;
  }
  ...
  if (reportPhaseTimings)
    GetWallTime();
  LoopTreeTransDepGraphCreate tc(comp.GetDepGraph(),
                                 lopt->GetTransAnalSplitLimit());
  Preprocess(comp);
  CompSliceDepGraphCreate sliceGraph(comp, op, &tc);
  if (reportPhaseTimings)
    std::cerr << "slicing analysis timing:" << GetWallTime() << "\n";

  CompSliceLocalityRegistry sliceAnal(loopAnal, lopt->GetCacheLineSize(),
                                      lopt->GetReuseDistance());
  RearrangeCompSliceGraph(comp, sliceGraph, sliceAnal);
  if (reportPhaseTimings)
    std::cerr << "transformation analysis timing:" << GetWallTime() << "\n";

  sliceGraph.TopoSort();
  DebugOpt(
      [&comp]() { return "*******before xform*****" + comp.TreeToString(); });
  LoopTreeNode *top = comp.GetLoopTreeRoot();
  top = LoopTreeTransform().InsertHandle(top, 1);
  for (CompSliceDepGraphCreate::NodeIterator iter =
           sliceGraph.GetNodeIterator();
       !iter.ReachEnd(); ++iter) {
    ...
    if (loopTree_handle == top) {
      top = 0;
    }

    CompSliceDepGraphNode::FullNestInfo *info =
        dynamic_cast<CompSliceDepGraphNode::FullNestInfo *>(&nest);
    if (info == 0)
      continue;
    ...
    DebugOpt(
        [&comp]() { return "*******after xform*****" + comp.TreeToString(); });
    DebugOpt(
        [&comp]() { return "*******after xform*****" + comp.DepToString(); });
    if (optlevel-- > 0) {
    ...
    if (cp != 0) {
      ...
      DebugOpt([&loopTree_handle]() {
        return "Applying array copy to " + loopTree_handle->TreeToString();
      });
  ...
  Postprocess(comp);
  if (reportPhaseTimings)
    std::cerr << "slicing transformation timing:" << GetWallTime() << "\n";
}
