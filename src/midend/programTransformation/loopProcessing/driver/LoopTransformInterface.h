...
public:
  virtual bool IsArrayAccess(AstInterface &fa, const AstNodePtr &s,
                             AstNodePtr *array = 0,
                             AstInterface::AstNodeList *index = 0) = 0;
  virtual bool GetArrayBound(AstInterface &fa, const AstNodePtr &array, int dim,
                             int &lb, int &ub) = 0;
  ...
  virtual SymbolicVal CreateArrayAccess(const SymbolicVal &arr,
                                        const SymbolicVal &index) = 0;
  //{ /*QY: need to be defined by a derived class*/ assert(0); }
  virtual ~ArrayAbstractionInterface() {}
};

class ArrayUseAccessFunction : public ArrayAbstractionInterface,
...
public:
  ArrayUseAccessFunction(const std::string &fn,
                         ArrayAbstractionInterface *n = 0,
...
public:
  ...
  static AstInterface &getAstInterface() {
    assert(fa != 0);
    return *fa;
  }
  ...
  static bool IsLoop(const AstNodePtr &s, SymbolicVal *init = 0,
                     SymbolicVal *cond = 0, SymbolicVal *incr = 0,
                     AstNodePtr *body = 0);
  ...
  static bool GetArrayBound(const AstNodePtr &array, int dim, int &lb,
                            int &ub) {
    assert(fa != 0);
  ...
  static bool IsDynamicFusionConfig(const AstNodePtr &n,
                                    AstNodePtr *configvar = 0,
                                    int *configID = 0,
                                    AstInterface::AstNodeList *params = 0);
  static bool IsDynamicFusionEnd(const AstNodePtr &n);

  /*************
   QY: apply transformations to head; return the transformation result
  ************/
  static AstNodePtr TransformTraverse(AstInterfaceImpl &scope,
