...
inline unsigned ReadUnsignedInt(LoopTransformOptions &opt,
...
class BlockParameterizeOpt : public LoopTransformOptions::OptRegistryType {
  virtual void operator()(LoopTransformOptions &opt, unsigned &index,
                          const std::vector<std::string> &argv) {
    opt.SetDefaultBlockSize(
        ReadUnsignedInt(opt, argv, index, "block size", 16));
    opt.SetBlockSel(new ParameterizeBlocking());
  }

public:
  BlockParameterizeOpt()
      : OptRegistryType(
            "-bk_poet",
            " <blocksize> : parameterize the blocking transformation") {}
};
...
class POETParallelizeOpt : public LoopTransformOptions::OptRegistryType {
  virtual void operator()(LoopTransformOptions &opt, unsigned &index,
                          const std::vector<std::string> &argv) {
    opt.SetParBlockSize(
        ReadUnsignedInt(opt, argv, index, "parallel block size", 256));
...
public:
  POETParallelizeOpt()
      : OptRegistryType(
            "-par_poet",
            " <blocksize> : paralleization transformation using POET") {}
};
...
public:
  BlockOuterLoopOpt()
      : OptRegistryType("-bk1", " <blocksize> :block outer loops") {}
};
...
public:
  BlockInnerLoopOpt()
      : OptRegistryType("-bk2", " <blocksize> :block inner loops") {}
};
...
public:
  BlockAllLoopOpt()
      : OptRegistryType("-bk3", " <blocksize> :block all loops") {}
};
...
public:
  CopyArrayDimensionOpt()
      : OptRegistryType(
            "-cp",
            " <copydim> :copy array regions with dimensions <= <copydim>") {}
};
...
public:
  ParameterizeCopyArrayOpt()
      : OptRegistryType("-cp_poet",
                        "<copydim> :parameterize array copy array regions; to "
                        "be applied together with blocking.") {}
};
...
public:
  ReuseInterchangeOpt()
      : OptRegistryType("-ic1", " :loop interchange for more reuses") {}
};
...
public:
  FissionOpt()
      : OptRegistryType("-fs0", " : maximum distribution at all loops") {}
};
...
public:
  InnerFissionOpt()
      : OptRegistryType("-fs01",
                        " : maximum distribution at inner-most loops") {}
};
...
public:
  SplitLimitOpt()
      : OptRegistryType("-ta",
                        " <int> :split limit for transitive dep. analysis") {}
};
...
public:
  CacheLineSizeOpt()
      : OptRegistryType("-clsize", " <int> :set cache line size") {}
};
...
public:
  ReuseDistOpt()
      : OptRegistryType("-reuse_dist", " <int> :set reuse distance") {}
};
...
LoopTransformOptions::OptType LoopTransformOptions::GetOptimizationType() {
  int t = (fsOp->GetOptimizationType() |
           ((bkOp == 0) ? NO_OPT : bkOp->GetOptimizationType()) |
...
void LoopTransformOptions ::PrintUsage(std::ostream &stream) const {
  for (SinglyLinkedListWrap<OptRegistryType *>::Iterator p(reg); !p.ReachEnd();
       ++p) {
...
void LoopTransformOptions ::SetOptions(const std::vector<std::string> &argvList,
