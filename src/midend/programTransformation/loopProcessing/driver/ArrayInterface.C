...
void ArrayInterface::initialize(AstInterface &fa, const AstNodePtr &h) {
  aliasCollect.analyze(fa, h);
  valueCollect.build(fa, h, *this, ArrayAnnotation::get_inst());
}

void ArrayInterface::analyze(AstInterface &fa, const AstNodePtr &h) {
  aliasCollect.analyze(fa, h);
}
...
void ArrayInterface::ObserveCopyAst(AstInterfaceImpl &fa,
                                    const AstNodePtr &orig,
                                    const AstNodePtr &copy) {
  std::map<AstNodePtr, int>::const_iterator p1 = dimmap.find(orig);
  if (p1 != dimmap.end())
    dimmap[copy] = (*p1).second;
  std::map<AstNodePtr, SymbolicFunctionDeclarationGroup>::const_iterator p2 =
      lenmap.find(orig);
  if (p2 != lenmap.end())
    lenmap[copy] = (*p2).second;
}

void ArrayInterface ::stop_observe(AstInterface &fa) {
  fa.DetachObserver(&valueCollect.get_value_map());
  fa.DetachObserver(this);
}

bool ArrayInterface ::may_alias(AstInterface &_fa, const AstNodePtr &r1,
                                const AstNodePtr &r2) {
...
bool ArrayInterface::get_array_opt(CPPAstInterface &fa, const AstNodePtr &array,
                                   ArrayOptDescriptor &r) {
  std::string name;
  if (!fa.IsVarRef(array, 0, &name))
    return false;

  std::map<std::string, ArrayOptDescriptor>::const_iterator p =
      optmap.find(name);
  if (p != optmap.end()) {
    r = (*p).second;
    return true;
...
void ArrayInterface ::set_array_dimension(const AstNodePtr &arrayexp, int dim) {
...
bool ArrayInterface ::is_array_exp(CPPAstInterface &fa, const AstNodePtr &array,
                                   int *dimp,
                                   SymbolicFunctionDeclarationGroup *lenp,
                                   bool *changep) {
  ArrayDefineDescriptor desc;
  if (!ArrayAnnotation::get_inst()->known_array(fa, array, &desc))
    return false;
  if (lenp != 0) {
    std::map<AstNodePtr, SymbolicFunctionDeclarationGroup>::const_iterator p =
        lenmap.find(array);
    if (p != lenmap.end()) {
      *lenp = (*p).second;
      lenp = 0;
...
bool ArrayInterface::is_array_mod_op(CPPAstInterface &fa,
                                     const AstNodePtr &arrayExp,
                                     AstNodePtr *arrayp, int *dimp,
                                     SymbolicFunctionDeclarationGroup *len,
                                     SymbolicFunctionDeclarationGroup *elem,
                                     bool *reshape) {
  AstNodePtr array;
  ArrayDescriptor desc;
  if (!ArrayAnnotation::get_inst()->is_array_mod_op(fa, arrayExp, &array, &desc,
                                                    reshape))
    return false;
...
bool ArrayInterface::is_array_construct_op(
    CPPAstInterface &fa, const AstNodePtr &arrayExp,
    AstInterface::AstNodeList *alias, int *dimp,
    SymbolicFunctionDeclarationGroup *len,
    SymbolicFunctionDeclarationGroup *elem) {
  ArrayDescriptor desc;
  if (ArrayAnnotation::get_inst()->is_array_construct_op(fa, arrayExp, alias,
                                                         &desc)) {
    HasValueMapReplace repl(fa, valueCollect.get_value_map(), true);
    desc.replace_val(repl);
...
AstNodePtr ArrayInterface::impl_array_opt_init(CPPAstInterface &fa,
...
AstNodePtr
ArrayInterface::impl_reshape_array(CPPAstInterface &fa, const AstNodePtr &array,
...
AstNodePtr ArrayInterface::impl_access_array_elem(
...
AstNodePtr ArrayInterface::impl_access_array_length(CPPAstInterface &fa,
                                                    const AstNodePtr &array,
                                                    int dim, int plus) {
  SymbolicVal rval;
  ArrayOptDescriptor desc;
...
bool ArrayInterface ::IsArray(CPPAstInterface &fa, const AstNodePtr &s) {
  if (ArrayAnnotation::get_inst()->known_array(fa, s))
    return true;
  return false;
}

bool ArrayInterface::IsUniqueArray(AstInterface &_fa, const AstNodePtr &s) {
  if (DebugArrayAnnot()) {
...
bool ArrayInterface ::IsArrayRef(CPPAstInterface &fa, const AstNodePtr &t) {
  if (ArrayAnnotation::get_inst()->known_array_type(fa,
                                                    fa.GetExpressionType(t)))
    return true;
  return false;
}
bool ArrayInterface ::IsArrayAccess(AstInterface &_fa, const AstNodePtr &s,
                                    AstNodePtr *arrayp,
...
AstNodePtr ArrayInterface::CreateArrayAccess(AstInterface &_fa,
...
bool ArrayInterface ::GetArrayBound(AstInterface &_fa, const AstNodePtr &array,
...
SymbolicVal ArrayInterface::CreateArrayAccess(const SymbolicVal &v1,
                                              const SymbolicVal &v2) {
  return new SymbolicFunction(AstInterface::OP_ARRAY_ACCESS, "ArrayAccess", v1,
                              v2);
}

ArrayInterface *ArrayInterface::get_inst(ArrayAnnotation &a, AstInterface &fa,
