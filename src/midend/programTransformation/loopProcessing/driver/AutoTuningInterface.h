...
public:
  ...
  static LocalVar *GenTraceCommand(POETProgram &poet,
                                   const std::vector<LocalVar *> &handles,
...
public:
  ...
  typedef enum {
    ...
    OPT_PAR_LEVEL = 1,
    OPT_CACHE_LEVEL = 2,
    OPT_PAR_CACHE_LEVEL = 3,
    OPT_REG_LEVEL = 4,
    OPT_CACHE_REG_LEVEL = 6,
    OPT_PROC_LEVEL = 8,
    OPT_POST_PAR_LEVEL = 16,
    OPT_CLEANUP_LEVEL = 32,
    OPT_CACHE_CLEANUP_LEVEL = 34,
    OPT_CACHE_REG_CLEANUP_LEVEL = 38,
    OPT_CACHE_PROC_CLEANUP_LEVEL = 42,
    OPT_ALL = 63,
    OPT_LEVEL_MAX = 32
  } OptLevel;
...
public:
  ...
  static POETCode *CreateArrayRef(POETProgram &poet, POETCode *arr,
  ...
  void BlockLoops(LoopTreeNode *outerLoop, LoopTreeNode *innerLoop,
                  LoopBlocking *config,
                  const std::vector<FuseLoopInfo> *nonperfect = 0);
...
public:
  ...
  /*QY: return a unique name that save relevant info. for a loop unroll
   * optimization*/
...
public:
  BlockSpec(HandleMap & /*handleMap*/, LocalVar *outerHandle,
            LoopTreeNode *innerLoop, LoopBlocking *config,
            const std::vector<FuseLoopInfo> *nonperfect = 0);
...
protected:
  ...
  POETCode *gen_copyInvoke(POETProgram &poet, POETCode *cphandle, LocalVar *top,
                           POETCode *cpblock, bool scalar, POETCode *traceMod,
                           int &lineNo);
  ...
  CopyArraySpec(POETCode *input, const std::string &name,
      ...
      : OptSpec(input, name), sel(config.get_arr()), opt(config.get_opt()),
        permute(0) {}

public:
  CopyArraySpec(HandleMap &handleMap, POETCode *input, const std::string &name,
  ...
  /*QY: insert xform declaration; append traceMod with variables that need to be
     kept track of; return the new line number */
  virtual void insert_xformDecl(POETProgram &poet, LocalVar *top,
                                POETCode *&traceMod, int &lineNo);
  virtual POETCode *gen_xformEval(POETProgram &poet, LocalVar *top,
                                  POETCode *traceMod, OptLevel optLevel,
  ...
  POETCode *gen_copyInvoke(POETProgram &poet, POETCode *handle, LocalVar *top,
                           const std::string &arrname, POETCode *arrelemType,
                           CopyArrayOpt opt, POETCode *cpDim, POETCode *cpblock,
