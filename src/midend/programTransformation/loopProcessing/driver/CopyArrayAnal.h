...
public:
  ...
  /* QY: Modifies (unit.root) to Ensure at most (copydim) loop dimensions are
   * copied for the given (unit) */
  int EnforceCopyDimension(
  ...
  bool IsRedundantCopy(DepCompCopyArrayCollect::CopyArrayUnit &unit,
                       int copydim);
