...
public:
  struct CopyArrayUnit {
    ...
    public:
      ...
      bool ContainEdge(const DepInfoEdge *e) const {
        return nodesel.IsMember(
                   impl->GetEdgeEndPoint(e, GraphAccess::EdgeOut)) &&
               nodesel.IsMember(
                   impl->GetEdgeEndPoint(e, GraphAccess::EdgeIn)) &&
    ...
    bool carrybyroot;
    int copylevel() const {
      return (!root->IncreaseLoopLevel()) ? root->LoopLevel()
                                          : root->LoopLevel() + 1;
    }
  ...
  CopyArrayUnit &AddCopyArray() {
    DoublyLinkedEntryWrap<CopyArrayUnit> *r =
        arraycopy.AppendLast(CopyArrayUnit());
    return r->GetEntry();
  }
  void RemoveCopyArray(iterator &cur) { arraycopy.Delete(cur.CurrentPtr()); }
