...
void save_value(AstInterface &fa, SymbolicVal &v, const std::string &name,
                int i, AstNodePtr &prep) {
...
void LoopTreeTransform::InsertNode(LoopTreeNode *node, LoopTreeNode *pos,
                                   int opt) {
  if (opt > 0) {
    UnlinkNode(node);
...
LoopTreeNode *LoopTreeTransform ::InsertLoop(LoopTreeNode *loop,
                                             LoopTreeNode *pos, int opt) {
...
LoopTreeNode *LoopTreeDistributeNode ::Distribute(LoopTreeNode *n,
                                                  SelectLoopTreeNode sel,
                                                  int pos, ObserveTransform &ob,
                                                  LoopTreeNode **loc) {
  if (sel(n)) {
    if (DebugLoopDist())
...
LoopTreeNode *LoopTreeDistributeNode ::DistributeBefore(LoopTreeNode *parent,
                                                        LoopTreeNode *loc) {
  assert(parent != 0);
...
LoopTreeNode *LoopTreeDistributeNode ::operator()(LoopTreeNode *n,
                                                  SelectLoopTreeNode stmts,
                                                  Location config) {
  ObserveTransform ob;
...
LoopTreeNode *LoopTreeSplitStmt ::operator()(LoopTreeNode *stmt,
                                             LoopTreeNode *restr1,
                                             LoopTreeNode *restr2) {
...
LoopTreeNode *LoopTreeSplitStmt ::operator()(LoopTreeNode *stmt,
                                             LoopTreeNode *l1, LoopTreeNode *l2,
                                             DepRel rel) {
  LoopTreeNode *n = operator()(stmt, CreateSplitRestr(l1, l2, rel),
                               CreateSplitRestr(l1, l2, -rel));
...
LoopTreeNode *LoopTreeSplitStmt ::operator()(LoopTreeNode *stmt,
                                             LoopTreeNode *loop,
                                             const SymbolicVal &split) {
  LoopTreeNode *n = operator()(
      stmt, new LoopTreeRestrLoopRange(loop, 0, SymbolicVal(), split),
...
void LoopTreeMergeLoop ::operator()(LoopTreeNode *ances, LoopTreeNode *desc,
                                    int align) {
...
void LoopTreeMergeStmtLoop ::operator()(LoopTreeNode *ancs, LoopTreeNode *desc,
...
LoopTreeNode *LoopTreeBlockLoop ::operator()(LoopTreeNode *n, SymbolicVar bvar,
                                             SymbolicVal bsize) {
  LoopInfo *info = n->GetLoopInfo();

  SymbolicVal step = info->GetStep() * bsize;
  LoopTreeBlockLoopNode *n1 = new LoopTreeBlockLoopNode(
      bvar, info->GetBound().lb, info->GetBound().ub, step);
  InsertNode(n1, n, -1);
...
void LoopTreeTransform::ReplaceChildren(LoopTreeNode *on, LoopTreeNode *nn,
                                        int pos) {
...
void LoopTreeSwapNodePos ::operator()(LoopTreeNode *parent,
                                      LoopTreeNode *child) {
  UnlinkNode(child);
...
void LoopTreeEmbedStmt ::operator()(LoopTreeNode *loop, LoopTreeNode *stmt,
                                    SymbolicVal selIter) {
...
void OptimizeLoopTree(LoopTreeNode *r) { OptimizeLoopTreeImpl()(r); }
    ...
    const SymbolicVal &startdiff) /*QY: difference in starting address to copy*/
{
  ...
  while (p != selinfo.end()) {
    ...
    if (cursel.get_arrDim() < arrDim) /*QY:do nothing; keep going */
      ...
      break;
    } else if (cursel.get_incr() ==
               selincr) { /* QY:extend this dimension if necessary*/
      ...
      ROSE_ABORT(); /* QY: cannot handle this yet*/
    else {
      ...
      if (cursel.incr == 1 || (rel & REL_LE)) {
        ...
        selinfo.insert(p,
                       ArrayDim(arrDim, selincr, selsize, minLevel, maxLevel));
        break;
      }
...
bool SelectArray::select(LoopTreeNode *stmt, LoopTreeNode *cproot,
  ...
  for (LoopTreeNode *c = stmt; c != cproot; c = c->Parent()) {
    ...
    if (info != 0) {
      ...
      if (b.lb == b.ub)
        repl.push_back(
            std::pair<SymbolicVar, SymbolicVal>(vinfo.GetVar(), b.lb));
    }
  }
  ...
  for (int i = 0; indexp != index.end(); ++i, ++indexp) {
    ...
    for (size_t j = 0; j < ivars.size(); ++j) {
      /*std::cerr << "j = " << j << "; ivar[j] = " << ivars[j] << "; curdim[j]="
       * << curdim.second[j] << "\n";
       */
    ...
    do {
      ...
      for (size_t j1 = 0; j1 < ivars.size(); ++j1) {
        ...
        if (cut != 0) {
          if (cut != 1) {
            ...
            if (SplitEquation(curdim.second, cut, bounds, context,
                              split.second))
      ...
      mod = insert_selinfo(context, i, incr, selsize, loopnum - maxindex - 1,
                           loopnum - minindex - 1, startdiff);
    } while (coeffs.size());
  }
  return mod;
}
...
std::string SelectArray::toString() const {
  ...
  for (size_t i = 0; i < selstart.size(); ++i) {
    ...
    for (; selp != selinfo.end() && size_t((*selp).get_arrDim()) == i; ++selp)
      r = r + ":" + (*selp).get_incr().toString() + ":" +
          (*selp).get_size().toString() + ":";
    r = r + "; ";
  }
...
CopyArrayConfig::CopyArrayConfig(AstInterface &fa, const SelectArray &_sel,
  ...
  if (shift != 0) {
    ...
    for (size_t j = 0; j < sel.arr_dim(); ++j) {
      ...
      SymbolicVal n =
          ReplaceVal(cur, shift->GetVar(), shift->GetVar() + shift->GetStep());
...
AstNodePtr SelectArray::buf_codegen(AstInterface &fa, int offsetval) const {
  if (offsetval == 0)
...
AstNodePtr SelectArray::buf_codegen(AstInterface &fa,
                                    const SymbolicVal &bufoffset) const {
  int offsetval = -1;
...
SymbolicVal SelectArray::buf_offset(AstInterface &fa,
                                    std::vector<SymbolicVal> &arrindex) const {
...
AstNodePtr SelectArray::buf_codegen(AstInterface &fa,
  ...
  if (scalar_repl()) {
    int offsetval = -1;
    if (!offset.isConstInt(offsetval))
      ROSE_ABORT();
    return buf_codegen(fa, offsetval);
  } else
...
public:
  ...
  virtual void loopcopy_codegen(SelectArray::const_iterator selp1, int curdim,
                                const SymbolicVar &ivar,
                                const SymbolicVal &cursize,
                                const SymbolicVal &curincr, AstNodePtr &res) {
    SymbolicVal ub = (cursize - 1) * curincr;
    AstNodePtr body = fa.CreateBlock();
    AstNodePtr curcopy =
        fa.CreateLoop(ivar.CodeGen(fa), fa.CreateConstInt(0), ub.CodeGen(fa),
  ...
  void operator()(AstNodePtr &res) { apply(sel.begin(), res); }
};
...
protected:
  void base_impl(AstNodePtr &res, const AstNodePtr &init) {
    ...
    fa.BlockAppendStmt(
        res, fa.CreateAssignment(bufoffset.CodeGen(fa),
                                 fa.CreateBinaryOP(AstInterface::BOP_PLUS,
                                                   bufoffset.CodeGen(fa),
                                                   fa.CreateConstInt(1))));
  }
  virtual void basecopy_codegen(AstNodePtr &res) {
    AstNodePtr lhs = sel.buf_codegen(fa, bufoffset);
    AstNodePtr rhs =
        LoopTransformInterface::CreateArrayAccess(sel.arr_name(), arrindex);
...
public:
  ...
  void operator()(AstNodePtr &res) {
    fa.BlockAppendStmt(
        res, fa.CreateAssignment(bufoffset.CodeGen(fa), fa.CreateConstInt(0)));
    apply(sel.begin(), res);
  }
};
...
public:
  LoopCopySaveCodeGen(const SelectArray &_sel,
                      const std::vector<SymbolicVal> &_arrindex)
...
class LoopCopyShiftCodeGen : public LoopCopyInitCodeGen {
  ...
  virtual void scalarcopy_codegen(SelectArray::const_iterator selp1, int curdim,
    ...
    if (curshift.isConstInt(shiftnum) && shiftnum != 0 && curshift > curincr) {
      LoopCopyInitCodeGen::scalarcopy_codegen(selp1, curdim, copynum - shiftnum,
                                              curincr, res);
      copynum = shiftnum;
    }
  ...
  virtual void loopcopy_codegen(SelectArray::const_iterator selp1, int curdim,
    ...
    if (curshift != 0 && curshift > curincr) {
      LoopCopyInitCodeGen::loopcopy_codegen(selp1, curdim, ivar,
                                            cursize - curshift, curincr, res);
    } else
      curshift = cursize;
    LoopCopyInitCodeGen initgen(sel, arrindex, bufoffset);
...
public:
  LoopCopyShiftCodeGen(const SelectArray &_sel,
                       const std::vector<SymbolicVal> &_arrindex,
                       const SymbolicVal &_bufshift,
                       const std::vector<SymbolicVal> &_arrshift)
      : LoopCopyInitCodeGen(_sel, _arrindex), bufshift(_bufshift),
        arrshift(_arrshift) {}
};
...
public:
  ScalarCopyInitCodeGen(const SelectArray &_sel,
                        const std::vector<SymbolicVal> &_arrindex)
...
public:
  ScalarCopySaveCodeGen(const SelectArray &_sel,
                        const std::vector<SymbolicVal> &_arrindex)
...
class ScalarCopyShiftCodeGen : public ScalarCopyInitCodeGen {
  ...
  virtual void basecopy_codegen(AstNodePtr &res) {
    ...
    if (offset < sel.buf_size())
      rhs = sel.buf_codegen(fa, offset);
    else
      rhs = LoopTransformInterface::CreateArrayAccess(sel.arr_name(), arrindex);
    fa.BlockAppendStmt(res, fa.CreateAssignment(lhs, rhs));
  }

public:
  ScalarCopyShiftCodeGen(const SelectArray &_sel,
                         const std::vector<SymbolicVal> &_arrindex,
                         const SymbolicVal &_bufshift)
      : ScalarCopyInitCodeGen(_sel, _arrindex), bufshift(_bufshift) {}
};

AstNodePtr SelectArray::initcopy_codegen(LoopInfo *shift) const {
  AstInterface &fa = LoopTransformInterface::getAstInterface();
  AstNodePtr r = fa.CreateBlock();
  ...
  if (shift != 0) {
    for (size_t i = 0; i < arr_dim(); ++i) {
      SymbolicVal curindex =
          ReplaceVal(sel_start(i), shift->GetVar(), shift->GetBound().lb);
  ...
  if (scalar_repl())
    ScalarCopyInitCodeGen(*this, arrindex)(r);
  else
    LoopCopyInitCodeGen(*this, arrindex)(r);
  return r;
}

AstNodePtr SelectArray::savecopy_codegen() const {
  AstInterface &fa = LoopTransformInterface::getAstInterface();
  AstNodePtr r = fa.CreateBlock();
  ...
  if (scalar_repl())
    ScalarCopySaveCodeGen(*this, arrindex)(r);
  else
    LoopCopySaveCodeGen(*this, arrindex)(r);
  return r;
}

AstNodePtr
SelectArray::shiftcopy_codegen(LoopInfo &shift,
  ...
  if (scalar_repl())
    ...
    LoopCopyShiftCodeGen(*this, arrindex, bufshift, arrshift)(r);
  AstNodePtr cond =
      fa.CreateBinaryOP(AstInterface::BOP_LT, shift.GetVar().CodeGen(fa),
...
void LoopTreeCopyArrayToBuffer::operator()(LoopTreeNode *repl,
                                           LoopTreeNode *init,
                                           LoopTreeNode *save,
                                           CopyArrayConfig &c) {
  ...
  CopyArrayOpt opt2 =
      (CopyArrayOpt)(opt & (SAVE_COPY | SHIFT_COPY | DELETE_COPY));
  if (opt1 != 0) {
    c.set_opt(opt1);
    LoopTreeNode *r = new LoopTreeCopyArray(c);
    assert(init != NULL);
...
LoopTreeNode *LoopTreeReplaceAst ::operator()(LoopTreeNode *h,
                                              const AstNodePtr &orig,
                                              const AstNodePtr &repl) {
...
class ApplyLoopSplittingImpl {
  ...
  void SplitLoop(LoopTreeNode *loop, SplitList::const_iterator cur,
                 SplitList::const_iterator end) {
    if (cur == end) {
      return;
    }
    PtrSetWrap<LoopTreeNode> stmtlist;
    for (LoopTreeTraverseSelectStmt stmts(loop); !stmts.ReachEnd();
         stmts.Advance()) {
...
void ApplyLoopSplitting(LoopTreeNode *r) { ApplyLoopSplittingImpl()(r); }
