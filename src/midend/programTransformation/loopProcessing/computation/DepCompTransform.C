...
void DepLinkedNodes(LoopTreeDepComp &comp, LoopTreeNodeIterator stmts,
  ...
  for (; !stmts.ReachEnd(); stmts++) {
    ...
    GraphGetNodeReachable<LoopTreeDepGraph, CollectTreeNodes>()(depGraph, n,
                                                                dir, collect);
  }
}

DepCompDistributeLoop::Result
DepCompDistributeLoop ::operator()(LoopTreeDepComp &tc, LoopTreeNode *l,
                                   LoopTreeNodeIterator stmts) {
...
DepCompDistributeLoop ::operator()(LoopTreeDepComp &tc, LoopTreeNode *l) {
  ...
  if (DebugLoopDist()) {
    std::cerr
        << "**Loop distribution based on SCC analysis From sub tree *******\n";
    loopComp.DumpTree();
    loopComp.DumpDep();
    std::cerr << "***************end sub tree *******\n";
  ...
  for (GroupGraphCreate::NodeIterator sccIter = sccGraph.GetNodeIterator();
    ...
    for (GroupGraphNode::const_iterator iter = scc->begin(); !iter.ReachEnd();
         iter++) {
...
public:
  DepCompAstRefDAG(const DepCompAstRefAnal &stmtorder,
    ...
    for (nodes.Reset(); !nodes.ReachEnd(); nodes.Advance()) {
      ...
      for (DepInfoEdgeIterator edges =
               g->GetNodeEdgeIterator(n, GraphAccess::EdgeOut);
...
class AstRefTypedFusionOperator : public TypedFusionOperator {
  ...
  std::map<DepCompAstRefGraphNode *, FuseNodeInfo,
           std::less<DepCompAstRefGraphNode *>>
      nodeMap;
  int size;
  DepCompCopyArrayCollect &collect;

  void AddNodeType(const DepCompAstRefGraphCreate *g,
                   DepCompAstRefGraphNode *n) {
    for (GraphAccess::EdgeDirection d = GraphAccess::EdgeOut;
         d != GraphAccess::BiEdge;
         d = (d == GraphAccess::EdgeOut) ? GraphAccess::EdgeIn
                                         : GraphAccess::BiEdge) {
      DepCompAstRefGraphCreate::EdgeIterator edges =
          g->GetNodeEdgeIterator(n, d);
      for (; !edges.ReachEnd(); ++edges) {
        DepInfoEdge *e = (*edges);
        DepCompAstRefGraphNode *n1 =
            g->GetEdgeEndPoint(e, GraphAccess::Reverse(d));
        if (nodeMap.find(n1) != nodeMap.end())
          continue;
        if (DebugRefFuse())
          std::cerr << "mapping node " << n1->toString() << " to " << size
                    << std::endl;
...
public:
  AstRefTypedFusionOperator(DepCompCopyArrayCollect &c,
    ...
    for (DepCompAstRefGraphCreate::NodeIterator nodes = g->GetNodeIterator();
      ...
      if (LoopTransformInterface::IsArrayAccess(r, &arr) && fa.IsVarRef(arr)) {
        if (DebugRefFuse())
          std::cerr << "mapping node " << n->toString() << " to " << size
                    << std::endl;
        nodeMap[n] = size;
        AddNodeType(g, n);
        ++size;
  ...
  virtual void MarkFuseNodes(GraphAccessInterface::Node *gn1,
    ...
    if (DebugRefFuse())
      std::cerr << "fusing refs: " << n1->toString() << " with "
                << n2->toString() << "\n";
  ...
  virtual bool PreventFusion(GraphAccessInterface::Node *src,
    ...
    if (DebugRefFuse())
      std::cerr << "checking fusion between " << n1->toString() << " and "
                << n2->toString();
...
LoopTreeNode *
DepCompCopyArrayCollect::OutmostCopyRoot(CopyArrayUnit &unit,
                                         DepCompAstRefGraphCreate &refDep,
                                         LoopTreeNode *treeroot) {
  LoopTreeNode *origroot = unit.root;
  while (unit.root != 0) {
    DepCompCopyArrayCollect::CopyArrayUnit::CrossGraph crossgraph(&refDep,
                                                                  unit);
    GraphEdgeIterator<DepCompCopyArrayCollect::CopyArrayUnit::CrossGraph> edges(
        &crossgraph);
    if (!edges.ReachEnd())
      break;
    DepCompCopyArrayCollect::CopyArrayUnit::InsideGraph insidegraph(&refDep,
                                                                    unit);
    GraphEdgeIterator<DepCompCopyArrayCollect::CopyArrayUnit::InsideGraph> ep(
        &insidegraph);
    for (; !ep.ReachEnd(); ++ep) {
  ...
  if (res == 0)
    res = treeroot;
  else if (res->LoopLevel() + 1 == origroot->LoopLevel() &&
           origroot->Parent()->ChildCount() == 1) {
    res = origroot->Parent();
  }
  unit.root = origroot;
  return res;
...
LoopTreeNode *
DepCompCopyArrayCollect::ComputeCommonRoot(CopyArrayUnit::NodeSet &refs) {
  LoopTreeInterface looptreeInterface;
  CopyArrayUnit::NodeSet::const_iterator rp = refs.begin();

  LoopTreeNode *curroot =
      GetEnclosingLoop((*rp)->GetInfo().stmt, looptreeInterface);
...
bool EnforceCopyRootRemove(
    NodeIter nodes, const DepCompAstRefGraphNode *outnode, int copylevel,
...
void DepCompCopyArrayToBuffer::EnforceCopyRoot(
    DepCompCopyArrayCollect::CopyArrayUnit &curunit,
    const DepCompAstRefGraphCreate &refDep,
    const DepCompAstRefGraphNode *outnode,
    DepCompCopyArrayCollect::CopyArrayUnit::NodeSet &cuts) {
  int copylevel = curunit.copylevel();

  DepCompCopyArrayCollect::CopyArrayUnit::CrossGraphOut crossout(&refDep,
                                                                 curunit);
  ...
  GraphNodeSuccessorIterator<
      DepCompCopyArrayCollect::CopyArrayUnit::CrossGraphIn>
...
void DepCompCopyArrayToBuffer::EnforceCopyRoot(
  ...
  if (DebugCopySplit()) {
    std::cerr << IteratorToString2(curunit.refs.begin())
              << " ; \n with root = " << curunit.root->TreeToString()
              << std::endl;
  }
  ...
  DepCompCopyArrayCollect::CopyArrayUnit::CrossGraphOut crossout(&refDep,
                                                                 curunit);
  for (GraphEdgeIterator<DepCompCopyArrayCollect::CopyArrayUnit::CrossGraphOut>
           outedges(&crossout);
       !outedges.ReachEnd(); ++outedges) {
    DepCompAstRefGraphNode *cur =
        refDep.GetEdgeEndPoint(*outedges, GraphAccess::EdgeIn);
    outnodes.insert(cur);
  }
  DepCompCopyArrayCollect::CopyArrayUnit::CrossGraphIn crossin(&refDep,
                                                               curunit);
  for (GraphEdgeIterator<DepCompCopyArrayCollect::CopyArrayUnit::CrossGraphIn>
           inedges(&crossin);
       !inedges.ReachEnd(); ++inedges) {
    DepCompAstRefGraphNode *cur =
        refDep.GetEdgeEndPoint(*inedges, GraphAccess::EdgeOut);
    outnodes.insert(cur);
  }

  for (DepCompCopyArrayCollect::CopyArrayUnit::NodeSet::const_iterator p1 =
           outnodes.begin();
       !p1.ReachEnd(); ++p1) {
...
void DepCompCopyArrayToBuffer::CollectCopyArray(
    DepCompCopyArrayCollect &collect, const DepCompAstRefGraphCreate &refDep) {
...
ComputeCopyConfig(const DepCompAstRefAnal &stmtorder,
  ...
  bool readlhs = false;
  bool is_init =
      ai.IsAssignment(initInfo.stmt->GetOrigStmt(), &lhs, 0, &readlhs) &&
  ...
  if (!LoopTransformInterface::IsArrayAccess(initInfo.orig, &arr, &initIndex) ||
      !ai.IsVarRef(arr, 0, &arrname))
    ROSE_ABORT();
...
void ComputeCutBoundary(const DepCompAstRefGraphCreate &refDep,
  ...
  for (DepCompCopyArrayCollect::CopyArrayUnit::NodeSet::const_iterator p =
    ...
    const DepCompAstRefGraphNode *curref = *p;
    if (cut1 == 0 ||
        stmtorder.CompareAstRef(cut1->GetInfo(), curref->GetInfo()) > 0)
      cut1 = curref;
    if (cut2 == 0 ||
        stmtorder.CompareAstRef(cut2->GetInfo(), curref->GetInfo()) < 0)
      cut2 = curref;
  }
  assert(cut1 != 0 && cut2 != 0);
}

void DepCompCopyArrayToBuffer::ApplyCopyArray(
    DepCompCopyArrayCollect &collect, const DepCompAstRefGraphCreate &refDep) {
  ...
  for (DepCompCopyArrayCollect::iterator arrays = collect.begin();
    ...
    CopyArrayConfig curconfig = ComputeCopyConfig(stmtorder, curarray, initcut);
    ...
    if (init != 0) {
      if (opt & SHIFT_COPY)
        ...
        for (; init->Parent() != curarray.root; init = init->Parent())
          ;
    }
    if ((opt & SHIFT_COPY) && (init != 0 || save != 0))
      save = curarray.root->LastChild();
    else if (save != 0)
      for (; save->Parent() != curarray.root; save = save->Parent())
        ;
...
void DepCompCopyArrayToBuffer::ApplyXform(
    ...
    CopyArrayConfig &curconfig, LoopTreeNode *replRoot, LoopTreeNode *initstmt,
    LoopTreeNode *savestmt) {
  AstInterface &fa = LoopTransformInterface::getAstInterface();
