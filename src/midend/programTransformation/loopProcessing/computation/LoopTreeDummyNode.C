...
LoopTreeRestrLoopRange::~LoopTreeRestrLoopRange() { DetachObserver(*this); }
...
AstNodePtr LoopTreeRestrLoopRange ::CodeGen(const AstNodePtr &c) const {
  ...
  AstNodePtr cond1;
  if (b.lb == b.ub) {
    cond1 = fa.CreateBinaryOP(AstInterface::BOP_EQ, ivar.CodeGen(fa),
                              lval.CodeGen(fa));
  } else {
    SymbolicVal uval = align ? b.ub - align : b.ub;
    cond1 = lval.IsNIL()
                ? AST_NULL
                : fa.CreateBinaryOP(AstInterface::BOP_GE, ivar.CodeGen(fa),
                                    lval.CodeGen(fa));
    AstNodePtr cond2 =
        uval.IsNIL() ? AST_NULL
                     : fa.CreateBinaryOP(AstInterface::BOP_LE, ivar.CodeGen(fa),
                                         uval.CodeGen(fa));
...
LoopTreeNode *ApproachAncesLoop(LoopTreeNode *start,
                                RoseSelectObject<LoopTreeNode *> &sel) {
  LoopTreeNode *l = 0;
  for (l = start->Parent(); l != 0 && l->ChildCount() == 1; l = l->Parent()) {
    if (sel(l))
      return l;
...
LoopTreeNode *FindSibling(LoopTreeNode *&t, int opt) {
  for (LoopTreeNode *l = t->Parent(); l && l->ChildCount() == 1;
       t = l, l = l->Parent())
    ;
...
bool LoopTreeRestrLoopRange ::MergeSibling(int opt) {
  ...
  for (LoopTreeNode *that = p2; that != 0;
    ...
    if (n->GetRestrLoop() == GetRestrLoop()) {
      ...
      if (lbr.IsNIL() && ubr.IsNIL())
        continue;
      LoopTreeRestrLoopRange *nr =
          new LoopTreeRestrLoopRange(GetRestrLoop(), 0, lbr, ubr);
...
AstNodePtr LoopTreeRelateLoopIvar ::CodeGen(const AstNodePtr &c) const {
  AstInterface &fa = LoopTransformInterface::getAstInterface();
  AstNodePtr cond =
      fa.CreateBinaryOP(AstInterface::BOP_EQ, GetIvar1().CodeGen(fa),
...
std::string LoopTreeCopyArray::toString() const { return config.toString(); }

std::string LoopTreeReplAst::toString() const {
  return "replace " + AstInterface::AstToString(orig) + " -> " +
         AstInterface::AstToString(repl);
}
...
AstNodePtr LoopTreeCopyArray::CodeGen(const AstNodePtr &c) const {
