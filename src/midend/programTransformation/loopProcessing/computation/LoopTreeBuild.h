...
protected:
  bool ProcessLoop(AstInterface &fa, const AstNodePtr &loop,
    ...
    if (t == AstInterface::PreVisit) {
      // Liao, we want to allow a loop with nested non-fortran loop in the loop
      // tree.
      if (fa.IsFortranLoop(loop) || supportNonFortranLoop) {
        ...
        std::cerr
            << "Warning: treating non-Fortran loop as a single statement\n";
        ProcessStmt(fa, loop);
      }
    } else {
      if (cur != root)
    ...
    return ProcessAstTree<AstNodePtr>::ProcessLoop(fa, loop, body, t);
  }
  bool ProcessIf(AstInterface &fa, const AstNodePtr &s, const AstNodePtr &cond,
    ...
    return ProcessAstTree<AstNodePtr>::ProcessIf(fa, s, cond, body1, body2, t);
  }
  ...
  bool ProcessStmt(AstInterface &fa, const AstNodePtr &start) {
    ...
    return ProcessAstTree<AstNodePtr>::ProcessStmt(fa, start);
  }

public:
  LoopTreeBuild(bool nonfortran = 0) : supportNonFortranLoop(nonfortran) {}
  bool operator()(AstInterface &fa, const AstNodePtr &top, LoopTreeCreate *ltc,
                  LoopTreeNode *root = 0) {
    lt = ltc;
