...
struct DepCompAstRef {
  AstNodePtr orig;
  LoopTreeNode *stmt;
  DepCompAstRef(const AstNodePtr &o, LoopTreeNode *s) : orig(o), stmt(s) {}
...
public:
  DepCompAstRefAnal(LoopTreeNode *_root) : root(_root) { Append(root); }

  LoopTreeNode *get_tree_root() const { return root; }
  int get_stmt_size() const { return stmtmap.size(); }
  bool has_stmt(LoopTreeNode *s) const {
    return stmtmap.find(s) != stmtmap.end();
  }
...
class LoopTreeLocalityAnal {
  ...
  void ComputeInputDep(LoopTreeDepGraph::NodeIterator iter,
...
public:
  ...
  int TemporaryReuseRefs(LoopTreeNode *s1, int loop1, LoopTreeNode *s2,
                         int loop2, AstNodeSet &refSet, int reuseDist);
};

class DepCompAstRefGraphCreate
    : public DepInfoGraphCreate<DepCompAstRefGraphNode> {
  typedef std::map<DepCompAstRef, DepCompAstRefGraphNode *,
                   std::less<DepCompAstRef>>
      AstRefNodeMap;
  AstRefNodeMap refNodeMap;

public:
  DepCompAstRefGraphNode *CreateNode(const AstNodePtr &r, LoopTreeNode *stmt);
  DepInfoEdge *CreateEdge(DepCompAstRefGraphNode *src,
                          DepCompAstRefGraphNode *snk, const DepInfo &dep);
