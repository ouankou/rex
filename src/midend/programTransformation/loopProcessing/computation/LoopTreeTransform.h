...
public:
  LoopTreeNode *InsertHandle(LoopTreeNode *pos,
                             int opt); // opt == -1 as parent; 1 : as child
...
public:
  ...
  LoopTreeNode *operator()(LoopTreeNode *n, SelectLoopTreeNode sel,
                           Location config = BEFORE);
};

class LoopTreeSplitStmt : public LoopTreeTransform {
  LoopTreeNode *operator()(LoopTreeNode *stmt, LoopTreeNode *restr1,
                           LoopTreeNode *restr2);

public:
  LoopTreeNode *operator()(LoopTreeNode *stmt, LoopTreeNode *loop1,
                           LoopTreeNode *loop2, DepRel r);
  LoopTreeNode *operator()(LoopTreeNode *stmt, LoopTreeNode *loop,
                           const SymbolicVal &split);
};

class LoopTreeMergeLoop : public LoopTreeTransform {
public:
  void operator()(LoopTreeNode *ancs, LoopTreeNode *desc, int align);
};

class LoopTreeMergeStmtLoop : public LoopTreeTransform {
public:
  void operator()(LoopTreeNode *ancs, LoopTreeNode *desc, LoopTreeNode *stmt,
                  int align);
};
...
public:
  void operator()(LoopTreeNode *n1, LoopTreeNode *n2);
};

class LoopTreeEmbedStmt : public LoopTreeTransform {
public:
  void operator()(LoopTreeNode *loop, LoopTreeNode *stmt, SymbolicVal selIter);
};
...
typedef enum {
  ...
  ALLOC_INIT_COPY = 5,
  ALLOC_SAVE_COPY = 6,
  ALLOC_INIT_SAVE_COPY = 7,
  DELETE_COPY = 8,
  ALLOC_DELETE_COPY = 12,
  ALLOC_INIT_DELETE_COPY = 13,
  ALLOC_SAVE_DELETE_COPY = 14,
  ALLOC_INIT_SAVE_DELETE_COPY = 15,
  SHIFT_COPY = 16,
  COPY_INSIDE = 32
} CopyArrayOpt;
...
public:
  ...
  public:
    ArrayDim(int _dim, const SymbolicVal &_incr, const SymbolicVal &_size,
        ...
        : incr(_incr), size(_size), dim(_dim),
          accLoops(minLoopLevel, maxLoopLevel) {}
...
public:
  SelectArray(const AstNodeType &base, const std::string &arr, int arrdim)
    ...
    for (int i = 0; i < arrdim; ++i)
      selstart.push_back(SymbolicVal());
  }
  std::string toString() const;

  /*QY: insert new select info (selincr,selsize,startdiff) to selinfo.
         return whether the internal is modified */
  bool insert_selinfo(LoopTreeGetVarBound &context, int arrDim,
                      SymbolicVal &selincr, SymbolicVal &selsize, int minLevel,
                      int maxLevel, const SymbolicVal &startdiff);
  ...
  SymbolicVal buf_offset(AstInterface &fa,
                         std::vector<SymbolicVal> &arroffset) const;
  ...
  AstNodePtr buf_codegen(AstInterface &fa,
  ...
  public:
    ...
    bool operator==(const iterator &that) const { return impl == that.impl; }
    bool operator!=(const iterator &that) const { return impl != that.impl; }
    void operator++() { ++impl; }
    void operator++(int) { impl++; }
...
public:
  CopyArrayConfig(AstInterface &fa, const SelectArray &sel, CopyArrayOpt opt,
...
public:
  void operator()(LoopTreeNode *repl, LoopTreeNode *init, LoopTreeNode *save,
                  CopyArrayConfig &c);
};

class LoopTreeReplaceAst : public LoopTreeTransform {
public:
  LoopTreeNode *operator()(LoopTreeNode *h, const AstNodePtr &orig,
