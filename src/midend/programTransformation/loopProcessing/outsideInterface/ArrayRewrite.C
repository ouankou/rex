...
bool RecognizeArrayOp(CPPAstInterface &fa, ArrayInterface &anal,
  ...
  if (anal.is_array_mod_op(fa, orig)) {
    ...
    std::cerr << "recognized array construct op: "
...
public:
  RewriteModArrayAccess(CPPAstInterface &_ai, ArrayInterface &a,
                        ...
                        const AstNodePtr &declloc)
      : CreateTmpArray(_varmap, _newstmts, declloc), stmt(_stmt), lhs(_lhs),
        size(0), ai(_ai), depAnal(_ai), anal(a) {
    AstInterface::AstNodeList subs;
    if (!ArrayAnnotation::get_inst()->is_access_array_elem(ai, lhs, &modarray,
                                                           &subs)) {
      std::cerr << "unrecognized array reference: "
                << AstInterface::AstToString(lhs) << "\n";
      ROSE_ABORT();
    }
    size = subs.size();
  }
  bool operator()(AstInterface &_fa, const AstNodePtr &orig,
                  AstNodePtr &result) {
    AstNodePtr array;
    AstInterface::AstNodeList subs;
    if (!ArrayAnnotation::get_inst()->is_access_array_elem(ai, orig, &array,
                                                           &subs) ||
        subs.size() == 0)
      return false;
    HasDependence test;
    if (ai.IsSameVarRef(array, modarray)) {
      ...
      DepInfoAnal::StmtRefDep ref =
          depAnal.GetStmtRefDep(stmt, orig, stmt, lhs);
    ...
    if (test.get_result()) {
      ...
      result = ArrayAnnotation::get_inst()->create_access_array_elem(ai, result,
                                                                     subscopy);
      return true;
    }
    return false;
...
bool RewriteConstructArrayAccess::rewritable(const SymbolicVal &orig) {
  ...
  if (ArrayAnnotation::get_inst()->is_access_array_elem(fa, orig, &arrayExp) ||
      ArrayAnnotation::get_inst()->is_access_array_length(fa, orig,
                                                          &arrayExp)) {
...
AstNodePtr CreateTmpArray::create_tmp_array(AstInterface &fa,
  ...
  if (split == 0) {
    ...
    std::string splitname =
        fa.NewVar(fa.GetType(tname), name, true, false, get_decl_scope());
    if (model == 0) {
...
SymbolicVal RewriteConstructArrayAccess ::operator()(const SymbolicVal &orig) {
  SymbolicVal result;
  SymbolicVal dim;
  ...
  if (ArrayAnnotation::get_inst()->is_access_array_elem(fa, orig, &arrayExp,
    ...
    if (anal.is_array_construct_op(fa, arrayExp, 0, 0, 0, &elem)) {
      ...
      result =
          ArrayAnnotation::get_inst()->create_access_array_elem(split, args);
    }
  } else if (ArrayAnnotation::get_inst()->is_access_array_length(
                 fa, orig, &arrayExp, &dim)) {
    ...
    if (anal.is_array_construct_op(fa, arrayExp, 0, 0, &len)) {
      ...
      AstNodePtr split = create_tmp_array(fa, arrayExp, "array");
      result =
          ArrayAnnotation::get_inst()->create_access_array_length(split, dim);
    }
  }
  return result;
}
...
bool CollectArrayRef(CPPAstInterface &fa, ArrayInterface &anal,
  ...
  if (!anal.is_array_construct_op(fa, array, &cur))
    return false;
  for (AstInterface::AstNodeList::iterator p = cur.begin(); p != cur.end();
       ++p) {
...
bool RewriteArrayModOp::operator()(AstInterface &_fa, const AstNodePtr &orig,
                                   AstNodePtr &result) {
  ...
  bool reshape = false;
  if (!anal.is_array_mod_op(fa, orig, &modArray, &dimension, &len, &elem,
                            &reshape))
    return false;
  ...
  SymbolicVal lhs =
      ArrayAnnotation::get_inst()->create_access_array_elem(modArray, ivarList);
  ...
  RewriteModArrayAccess modArrayRewrite(fa, anal, stmt, lhsast, varmap,
                                        newStmts, result);
  TransformAstTraverse(fa, rhsast, modArrayRewrite);

  for (std::list<AstNodePtr>::iterator p = newStmts.begin();
       p != newStmts.end(); ++p) {
    AstNodePtr cur = (*p);
    AstNodePtr ncur = cur;
  ...
  if (reshape) {
    ...
    AstNodePtr reshapeStmt = ArrayAnnotation::get_inst()->create_reshape_array(
        fa, modArray, argList);
    fa.BlockAppendStmt(result, reshapeStmt);
  }
  fa.BlockAppendStmt(result, body);
  std::cerr << "modarray rewrite: result = "
            << AstInterface::AstToString(result) << "\n";
  return true;
}

bool RewriteToArrayAst::operator()(AstInterface &fa, const AstNodePtr &orig,
                                   AstNodePtr &result) {
...
bool RewriteFromArrayAst::operator()(AstInterface &_fa, const AstNodePtr &orig,
                                     AstNodePtr &result) {
  AstNodePtr array;
  ...
  if (fa.IsVariableDecl(orig, &vars) ||
    ...
    return true;
  } else if (ArrayAnnotation::get_inst()->is_access_array_elem(fa, orig, &array,
                                                               &args)) {
    result = anal.impl_access_array_elem(fa, array, args);
    return true;
  } else if (ArrayAnnotation::get_inst()->is_reshape_array(fa, orig, &array,
                                                           &args)) {
