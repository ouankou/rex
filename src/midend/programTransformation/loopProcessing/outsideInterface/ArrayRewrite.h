...
public:
  CreateTmpArray(std::map<std::string, AstNodePtr> &_varmap,
                 std::list<AstNodePtr> &_newstmts, const AstNodePtr &declscope)
      : varmap(_varmap), newStmts(_newstmts), scope(declscope), model() {}
  AstNodePtr create_tmp_array(AstInterface &fa, const AstNodePtr &arrayExp,
                              const std::string name);
  void set_model_array(const AstNodePtr &mod) { model = mod; }
  const AstNodePtr &get_decl_scope() { return scope; }
};

class RewriteConstructArrayAccess : public CreateTmpArray,
