...
public:
  ...
  struct StmtRefInfo {
    AstNodePtr stmt, ref;
    StmtRefInfo(const AstNodePtr &s, const AstNodePtr &r) : stmt(s), ref(r) {}
  };
  struct StmtRefDep {
    StmtRefInfo r1, r2;
    AstNodePtr commLoop;
  ...
  void ComputePrivateScalarDep(const StmtRefDep &ref, DepInfoCollect &outDeps,
                               DepInfoCollect &inDeps);
  void ComputeGlobalScalarDep(const StmtRefDep &ref, DepInfoCollect &outDeps,
                              DepInfoCollect &inDeps);
  void ComputeArrayDep(const StmtRefDep &ref, DepType deptype,
...
public:
  DepInfo ComputeArrayDep(DepInfoAnal &anal, const DepInfoAnal::StmtRefDep &ref,
                          DepType deptype);
};
...
public:
  SetDepEntry(DomainCond &_d1, DomainCond &_d2, DepInfo *_d, int _l1,
              bool &_succ)
...
public:
  ...
  typedef std::map<std::string, ReverseRec, std::less<std::string>>
      ReverseRecMap;
...
public:
  MakeUniqueVarGetBound(ReverseRecMap &r1, DepInfoAnal &a)
      : SymbolicConstBoundAnalysis<AstNodePtr, DepInfoAnalInterface>(
            DepInfoAnalInterface(a), AST_NULL, AST_NULL),
        reverse(r1) {}
  SymbolicBound GetBound(const SymbolicVar &var) {
    ...
    SymbolicBound r =
        SymbolicConstBoundAnalysis<AstNodePtr, DepInfoAnalInterface>::
            GetConstBound(SymbolicVar(entry.first, var.GetVarScope()));
    return r;
  }
};
