...
SymbolicVal DecomposeAffineExpression(const SymbolicVal &exp,
                                      const VarVec &vars, CoeffVec &vec,
                                      int size) {
  // AstInterface& fa = la;
  SymbolicVal val = exp;
  // int coeff;
  for (int i = 0; i < size; ++i) {
    ...
    SymbolicVal coeff =
        UnwrapVarCond(SymbolicCond(REL_LE, val, 0), ivar, ivarbound);
    if (coeff.IsNIL())
      return SymbolicVal();
    if (!(coeff == 0)) {
...
// Coefficient vector, current coefficient, bounds vector, GetBoundOperator,
// Split coefficient vector
template <class CoeffVec, class BoundVec, class BoundOp>
bool SplitEquation(CoeffVec &cur, const SymbolicVal &cut,
                   const BoundVec &bounds, BoundOp &boundop, CoeffVec &split) {
  DebugLog DebugDep("-debugdep");
  int dim = cur.size() - 1;
  SymbolicVal leftval =
      cur[dim]; // obtain the last coefficient, which is right side terms
                // without using loop index variable
  if (leftval != 0) {
    CompareRel r1 = CompareVal(leftval, -cut, &boundop);
    CompareRel r2 = CompareVal(leftval, cut, &boundop);
    bool lt =
        ((r1 & REL_GT) && (r2 & REL_LT)) || ((r1 & REL_LT) && (r2 & REL_GT));
    if (!lt) { // relation of r1 and r2 must be reversed pair, or error
      DebugDep([&cut, &leftval]() {
        return "unable to split because " + leftval.toString() + " ? " +
               cut.toString();
      });
      return false;
    }
  ...
  if (succ) {
    ...
    for (j = 0; j < dim; ++j) {
      if (split[j] == 0)
        ...
        case REL_LE:
          left = left + cur[j] * bounds[j].lb;
          break;
        case REL_GE:
          left = left + cur[j] * bounds[j].ub;
          break;
        default:
          break;
        }
    ...
    if (j == dim && (left == 0 || (CompareVal(left, cut) & REL_LT))) {
      ...
      DebugDep([&cut, &left]() {
        return "unable to decide left " + left.toString() + " ? " +
               cut.toString();
      });
    else
      DebugDep([&j]() {
        return "unable to decide cur[" + std::to_string(j) + "] ? 0";
      });
  }
  split.clear();
  return false;
...
template <class Mat> bool NormalizeMatrix(Mat &analMatrix, int rows, int cols) {
  int k;
...
int SetDepDirection(DepInfo &edd, int commLevel, Collect &result) {
  ...
  for (i = 0; i < commLevel; i++) {
    ...
    DebugDep([i, &e1]() {
      return "at common loop level " + std::to_string(i) + ":" + e1.toString();
    });
    if (!e1.IsTop()) {
      DepInfo edd1(edd);
      edd1.Entry(i, i) = e1;
...
bool AnalyzeEquation(const CoeffVec &vec, const BoundVec &bounds,
  ...
  for (int index = 0; index < dim; ++index) {
    ...
    if (b.lb >= 0) {
      if (cb.lb >= 0)
        ...
        DebugDep([index]() {
          return "unable to decide sign of coeff when lb >=0 for ivar[" +
                 std::to_string(index);
        });
        signs.push_back(2);
      }
      ...
      if (cb.lb >= 0)
        ...
        if (DebugDep())
          std::cerr << "unable to decide sign of coeff when ub <=0 for ivar["
                    << index << "]\n";
        signs.push_back(2);
      }
    } else {
      DebugDep([index]() {
        return "unable to decide sign of ivar[" + std::to_string(index);
      });
      signs.push_back(2);
    }
  }
  if (vec[dim] == 0)
    ...
    if (leftval.IsNIL()) {
      DebugDep([&leftval]() {
        return "unable to decide sign of leftval: " + leftval.toString();
      });
      return false;
    }
    ...
    for (j = i + 1; j < dim; ++j) {
      ...
      if (hasdiff) {
        ...
        if (signs[dim] * signs[i] > 0) {
          // DepStats.AddAdhocDV(DepStats.RoseToPlatoDV(DepRel(DEPDIR_GE,
          // diff)));
          result[i][j] = rel * DepRel(DEPDIR_GE, diff);
        } else {
          // DepStats.AddAdhocDV(DepStats.RoseToPlatoDV(DepRel(DEPDIR_LE,
          // diff)));
