...
DomainCondConstIterator DomainInfo::GetConstIterator() const {
  return new IteratorImplTemplate<DomainCond, DomainInfoIterator>(
      DomainInfoIterator(ConstRef()));
}

DomainCondUpdateIterator DomainInfo::GetUpdateIterator() {
  return new IteratorImplTemplate<DomainCond &, DomainInfoIterator>(
      DomainInfoIterator(UpdateRef()));
}
...
bool DomainInfo::AddCond(const DomainCond &c) { return UpdateRef().AddElem(c); }
...
std::string DomainInfo::toString() const {
  ...
  for (LatticeElemList<DomainCond>::iterator iter(list); !iter.ReachEnd();
       iter++) {
...
DomainInfo operator&(const DomainInfo &dm1, const DomainInfo &dm2) {
  DomainInfo result;
  for (DomainCondConstIterator iter1 = dm1.GetConstIterator();
       !iter1.ReachEnd(); iter1++) {
    DomainCond c1 = iter1.Current();
    for (DomainCondConstIterator iter2 = dm2.GetConstIterator();
...
DomainInfo operator-(const DomainInfo &info1, const DomainInfo &info2) {
  ...
  for (DomainCondConstIterator iter2 = info2.GetConstIterator();
       !iter2.ReachEnd(); iter2++) {
    DomainInfo tmp;
    for (DomainCondConstIterator iter1 = result.GetConstIterator();
         !iter1.ReachEnd(); iter1++) {
