...
DepInfoConstIterator DepInfoSet ::GetConstIterator() const {
  if (ConstPtr() == 0)
    ...
    return new IteratorImplTemplate<DepInfo, DepInfoSetIterator>(
        DepInfoSetIterator(ConstRef()));
}
...
DepInfoUpdateIterator DepInfoSet ::GetUpdateIterator() {
  if (ConstPtr() == 0)
    ...
    return new IteratorImplTemplate<DepInfo &, DepInfoSetIterator>(
        DepInfoSetIterator(UpdateRef()));
}

DepInfoSet::DepInfoSet()
    : CountRefHandle<DepInfoSetImpl>(new DepInfoSetImpl()) {}
...
DepInfoSet DepInfoSetGenerator::GetTopInfoSet() { return DepInfoSet(); }

DepInfoSet DepInfoSetGenerator::GetBottomInfoSet(int nr, int nc,
                                                 int commLevel) {
  DepInfoSet result;
...
DepInfoSet Closure(DepInfoSet t) {
  DepInfoSet result(DepInfoSetGenerator::GetTopInfoSet());
  for (DepInfoConstIterator iter = t.GetConstIterator(); !iter.ReachEnd();
       iter++) {
  ...
  while (change) {
    ...
    for (DepInfoConstIterator iter1 = result.GetConstIterator();
         !iter1.ReachEnd(); iter1++) {
      for (DepInfoConstIterator iter2 = result.GetConstIterator();
           !iter2.ReachEnd(); iter2++) {
...
DepInfoSet operator*(const DepInfoSet &t1, const DepInfoSet &t2) {
  DepInfoSet result(DepInfoSetGenerator::GetTopInfoSet());
  for (DepInfoConstIterator iter1 = t1.GetConstIterator(); !iter1.ReachEnd();
       iter1++) {
    for (DepInfoConstIterator iter2 = t2.GetConstIterator(); !iter2.ReachEnd();
         iter2++) {
      DepInfo d = iter1.Current() * iter2.Current();
      result.AddDep(d);
    }
