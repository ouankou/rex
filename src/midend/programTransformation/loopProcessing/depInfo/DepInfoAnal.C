...
bool AnalyzeStmtRefs(AstInterface &fa, const AstNodePtr &n,
  ...
  std::function<bool(AstNodePtr, AstNodePtr)> colw =
      [&wRefs](AstNodePtr mod_first, AstNodePtr mod_second) {
        return wRefs(mod_first);
      };
  std::function<bool(AstNodePtr, AstNodePtr)> colr =
      [&rRefs](AstNodePtr read_first, AstNodePtr read_second) {
        return rRefs(read_first);
      };
  return StmtSideEffectCollect<AstNodePtr>(
      fa, LoopTransformInterface::getSideEffectInterface())(n, &colw, &colr);
}
...
DepInfo ComputePrivateDep(DepInfoAnal &anal, const DepInfoAnal::StmtRefDep &ref,
                          DepType t, int pos) {
  ...
  DepInfo result = DepInfoGenerator::GetDepInfo(
      dim1, dim2, t, ref.r1.ref, ref.r2.ref, false, ref.commLevel);
  int i;
...
DepInfo ComputeGlobalDep(DepInfoAnal &anal, const DepInfoAnal::StmtRefDep &ref,
                         DepType t, int pos) {
  const DepInfoAnal::LoopDepInfo &info1 = anal.GetStmtInfo(ref.r1.stmt);
  const DepInfoAnal::LoopDepInfo &info2 = anal.GetStmtInfo(ref.r2.stmt);
...
const DepInfoAnal::LoopDepInfo &DepInfoAnal::GetStmtInfo(const AstNodePtr &_s) {
...
const DepInfoAnal::LoopDepInfo &DepInfoAnal::GetLoopInfo(const AstNodePtr &s) {
  AstInterface &ai = get_astInterface();
  SymbolicVar ivar;
  SymbolicVal lb, ub;
  if (s == AST_NULL ||
      !SymbolicValGenerator::IsFortranLoop(ai, s, &ivar, &lb, &ub))
  ...
  if (info.IsTop()) {
    ...
    SymbolicConstBoundAnalysis<AstNodePtr, DepInfoAnalInterface> boundop(
        DepInfoAnalInterface(*this), s, AST_NULL);
    ...
    if (!AnalyzeEquation(lbvec, info.ivarbounds, boundop, op,
                         DepRel(DEPDIR_LE, 0)))
      if (DebugDep())
        std::cerr << "unable to analyze equation: " << toString(lbvec)
                  << std::endl;
    SymbolicVal ubleft =
        DecomposeAffineExpression(ub, info1.ivars, ubvec, dim1);
    ubvec.push_back(-1);
    ubvec.push_back(-ubleft);
    if (!AnalyzeEquation(ubvec, info.ivarbounds, boundop, op,
                         DepRel(DEPDIR_GE, 0)))
      if (DebugDep())
        std::cerr << "unable to analyze equation: " << toString(ubvec)
                  << std::endl;
    info.domain = op.get_domain1();
    info.domain.ClosureCond();
    if (DebugDep())
      std::cerr << "domain of statement " << AstInterface::AstToString(s)
                << " is : " << info.domain.toString() << std::endl;
  }
  assert(!info.IsTop());
  return info;
}

void DepInfoAnal ::ComputePrivateScalarDep(const StmtRefDep &ref,
                                           DepInfoCollect &outDeps,
...
void DepInfoAnal ::ComputeGlobalScalarDep(const StmtRefDep &ref,
                                          DepInfoCollect &outDeps,
                                          DepInfoCollect &inDeps) {
  DepType t1 = DEPTYPE_SCALAR, t2 = DEPTYPE_BACKSCALAR;
  if (ref.commLevel > 0 || ref.r1.ref != ref.r2.ref) {
...
DepInfoAnal ::DepInfoAnal(AstInterface &fa, DependenceTesting &h)
    : handle(h), varmodInfo(fa, SelectLoop(),
                            LoopTransformInterface::getSideEffectInterface()) {
  AstNodePtr root = fa.GetRoot();
  varmodInfo.Collect(root);
}

void DepInfoAnal ::ComputeArrayDep(const StmtRefDep &ref, DepType deptype,
                                   DepInfoCollect &outDeps,
                                   DepInfoCollect &inDeps) {
  DepInfo d;
...
DepInfo AdhocDependenceTesting::ComputeArrayDep(
    DepInfoAnal &anal, const DepInfoAnal::StmtRefDep &ref, DepType deptype) {
  if (DebugDep())
    std::cerr << "compute array dep between "
              << AstInterface::AstToString(ref.r1.ref) << " and "
              << AstInterface::AstToString(ref.r2.ref) << std::endl;
  ...
  for (; iter1 != sub1.end() && iter2 != sub2.end(); ++iter1, ++iter2) {
    ...
    if (val1 == val2 && fa.IsArrayAccess(s1, &s2) &&
        LoopTransformInterface::IsUniqueArray(s2)) {
    ...
    SymbolicVal left2 =
        DecomposeAffineExpression(-val2, info2.ivars, cur, dim2);
    ...
    if (DebugDep()) {
      std::cerr << "analyzing array subscripts: " << val1.toString() << " .vs. "
                << val2.toString() << "\n";
      std::cerr << "remaining value assumed to be loop invarient: "
                << left1.toString() << " + " << left2.toString() << "\n";
    }
    ...
    if (DebugDep()) {
      std::cerr << "coefficients for induction variables (" << dim1 << " + "
                << dim2 << "+ 1)\n";
  ...
  if (DebugDep())
    std::cerr << "after normalization, relation matrix = \n"
              << toString(analMatrix) << std::endl;
  DepInfo result = DepInfoGenerator::GetDepInfo(
      dim1, dim2, deptype, ref.r1.ref, ref.r2.ref, false, ref.commLevel);
  ...
  for (size_t k = 0; setdep && k < analMatrix.size(); ++k) {
    ...
    if (j == dim + 1) // equation has only 0
      continue;
    if (j == dim && analMatrix[k][j].GetValType() == VAL_CONST &&
        analMatrix[k][j] != 0)
      return DepInfo();
    if (!AnalyzeEquation(analMatrix[k], bounds, boundop, setdep,
                         DepRel(DEPDIR_EQ, 0))) {
      precise = false;
      if (DebugDep())
  ...
  if (ref.commLevel > 0) {
    ...
    // buffer << "Prob\t" << adhocProbNum << " between " << lineNo1 << " and "
    // << lineNo2 << "\tAdhoc\t" << DepType2String(result.GetDepType()) <<
    // "\tTime\t" << adhocTime << std::endl;
    // buffer << "Prob\t" << adhocProbNum << "\tAdhoc\t" <<
    // DepType2String(result.GetDepType()) << "\tDV\t" << adhocDV << "\tTime\t"
    // << adhocTime <<  std::endl; PrintResults(buffer.str());
  }
#endif
...
DepInfoAnal::StmtRefDep DepInfoAnal::GetStmtRefDep(const AstNodePtr &s1,
                                                   const AstNodePtr &r1,
                                                   const AstNodePtr &s2,
                                                   const AstNodePtr &r2) {
  StmtRefDep r(StmtRefInfo(s1, r1), StmtRefInfo(s2, r2), AST_NULL, 0);
  AstInterface &fa = get_astInterface();
  r.commLoop = GetCommonLoop(fa, s1, GetStmtInfo(s1).domain.NumOfLoops(), s2,
                             GetStmtInfo(s2).domain.NumOfLoops(), &r.commLevel);
  return r;
}

void DepInfoAnal ::ComputeIODep(const AstNodePtr &s1, const AstNodePtr &s2,
...
void DepInfoAnal::ComputeCtrlDep(const AstNodePtr &s1, const AstNodePtr &s2,
                                 DepInfoCollect &outDeps,
                                 DepInfoCollect &inDeps, DepType t) {
  StmtRefDep ref = GetStmtRefDep(s1, s1, s2, s2);
  if (ref.commLevel > 0 || s1 != s2) {
...
void ComputeRefSetDep(DepInfoAnal &anal, DepInfoAnal::StmtRefDep &ref,
                      DoublyLinkedListWrap<AstNodePtr> *rs1,
                      DoublyLinkedListWrap<AstNodePtr> *rs2, DepType t,
                      CollectObject<DepInfo> &outDeps,
  ...
  for (DoublyLinkedListWrap<AstNodePtr>::iterator iter1 = rs1->begin();
    ...
    for (DoublyLinkedListWrap<AstNodePtr>::iterator iter2 =
      ...
      ref.r2.ref = r2;
      if (CmdOptions::GetInstance()->HasOption("-very_conserv") &&
...
void DepInfoAnal ::ComputeDataDep(const AstNodePtr &s1, const AstNodePtr &s2,
                                  DepInfoCollect &outDeps,
                                  DepInfoCollect &inDeps, int t) {
  AstInterface &fa = get_astInterface();
  DoublyLinkedListWrap<AstNodePtr> rRef1, wRef1, rRef2, wRef2;
  CollectDoublyLinkedList<AstNodePtr> crRef1(rRef1), cwRef1(wRef1),
      crRef2(rRef2), cwRef2(wRef2);
  if (!AnalyzeStmtRefs(fa, s1, cwRef1, crRef1) ||
      (s1 != s2 && !AnalyzeStmtRefs(fa, s2, cwRef2, crRef2))) {
    if (DebugDep())
      std::cerr << "cannot determine side effects of statements: "
                << AstInterface::AstToString(s1) << "; or "
                << AstInterface::AstToString(s2) << std::endl;
  ...
  if (s1 == s2) {
    ...
    if (t & DEPTYPE_OUTPUT)
      ComputeRefSetDep(*this, ref, &wRef1, &wRef1, DEPTYPE_OUTPUT, outDeps,
                       inDeps);
    if (t & DEPTYPE_TRUE || t & DEPTYPE_ANTI)
      ComputeRefSetDep(*this, ref, &rRef1, &wRef1, DEPTYPE_ANTI, outDeps,
                       inDeps);
    if (t & DEPTYPE_INPUT)
      ComputeRefSetDep(*this, ref, &rRef1, &rRef1, DEPTYPE_INPUT, outDeps,
                       inDeps);
  } else {
    RemoveIvars(fa, rRef1, GetStmtInfo(s1).ivars);
    ...
    if (t & DEPTYPE_OUTPUT)
      ComputeRefSetDep(*this, ref, &wRef1, &wRef2, DEPTYPE_OUTPUT, outDeps,
                       inDeps);
    if (t & DEPTYPE_TRUE)
      ComputeRefSetDep(*this, ref, &wRef1, &rRef2, DEPTYPE_TRUE, outDeps,
                       inDeps);
    if (t & DEPTYPE_ANTI)
      ComputeRefSetDep(*this, ref, &rRef1, &wRef2, DEPTYPE_ANTI, outDeps,
                       inDeps);
    if (t & DEPTYPE_INPUT)
      ComputeRefSetDep(*this, ref, &rRef1, &rRef2, DEPTYPE_INPUT, outDeps,
                       inDeps);
  }
}
