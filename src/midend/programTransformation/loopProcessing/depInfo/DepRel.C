...
int AlignRangeSize(int entryType) { return entryType >> 3; }
...
bool DepRel ::IsTop() const { return GetRelType(entryType) == DEPDIR_NONE; }

bool DepRel ::IsBottom() const { return GetRelType(entryType) == DEPDIR_ALL; }
...
bool DepRel::IntersectUpdate(DepRel &e1, const DepRel &e2) {
  ...
  return t1 != e1.GetDirType() || a1 != e1.GetMinAlign() ||
         b1 != e1.GetMaxAlign();
}
