...
typedef enum {
  ...
  DEPTYPE_OUTPUT = 4,
  DEPTYPE_SCALAR = 8,
  DEPTYPE_BACKSCALAR = 16,
  DEPTYPE_INPUT = 32,
  DEPTYPE_ARRAY = 39,
  DEPTYPE_IO = 64,
  DEPTYPE_DATA = 95,
  DEPTYPE_CTRL = 128,
  DEPTYPE_BACKCTRL = 256,
  DEPTYPE_ALL = 479,
  DEPTYPE_TRANS = 512
} DepType;
...
protected:
  ...
  DepInfoImpl(int dim1, int dim2, bool p, int cl) // p: precise or not
      : edd(dim1, dim2, (dim1 == 0 || dim2 == 0) ? 0 : 1), precise(p),
        commonlevel(cl) {}

  DepInfoImpl(const DepInfoImpl &that)
      : edd(that.edd), precise(that.precise), commonlevel(that.commonlevel) {}
...
public:
  ...
  virtual DepInfoImpl *Clone() const { return new DepInfoImpl(*this); }
...
public:
  ...
  DepInfo &operator=(const DepInfo &that) {
    CountRefHandle<DepInfoImpl>::operator=(that);
    return *this;
  }
  ...
  AstNodePtr SrcRef() const {
    return (ConstPtr() == 0) ? AST_NULL : ConstRef().SrcRef();
  }
  AstNodePtr SnkRef() const {
    return (ConstPtr() == 0) ? AST_NULL : ConstRef().SnkRef();
  }
  int CommonLevel() const {
    return (ConstPtr() == 0) ? -1 : ConstRef().CommonLevel();
  }
  ...
  bool is_precise() const {
    return (ConstPtr() == 0) ? true : ConstRef().is_precise();
  }
...
typedef IteratorWrap<DepInfo &, DepInfoUpdateIteratorImpl>
...
public:
  ...
  static DepInfo GetDepInfo(int nr, int nc, DepType t, bool precise,
                            int commLevel);
  static DepInfo GetDepInfo(int nr, int nc, DepType t, const AstNodePtr &srcRef,
                            const AstNodePtr &snkRef, bool precise,
                            int commLevel);
};
