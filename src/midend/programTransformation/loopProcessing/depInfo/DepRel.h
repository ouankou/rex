...
public:
  DepRel(DepDirType t = DEPDIR_ALL, int a = 0) { Reset(t, a, a); }
  DepRel(DepDirType t, int min, int max) { Reset(t, min, max); }
  ...
  bool operator!=(const DepRel &that) const { return !operator==(that); }
