...
typedef struct VAR_U {
  OMEGA_FREE_VAR *_omega_free_var;
  PLATO_FREE_VAR *_plato_free_var;
} FREE_VAR_U;
typedef struct VAR_ID_U {
  OMEGA_STD_VAR _omega_std_var;
  PLATO_STD_VAR *_plato_std_var;
} STD_VAR_U;
...
private:
  ...
  public:
    PlatoRelation(int loop_nest_size, bool aFlag = false)
        : _loop_nest_size(loop_nest_size), RationalVITestProblem(aFlag) {};
    virtual ~PlatoRelation() {};
    virtual bool testDV(const plato::AbstractDirectionVector &dv,
                        bool &accurate);
...
#define added(a, b) (a->count(b) > 0)
  bool IsModified(AstNodePtr loop, std::string name) {
    return _anal->GetModifyVariableInfo().Modify(loop, name);
  }

  /** Dependence testing functions **/
  void AddConstraintsImposedByDV(Relation &relation, F_And &constraints,
                                 const plato::dddir &dv);
  Relation GetNewRelationFromDV(const plato::dddir &dv);
  void doHierarchy(SetDep &setdep, DepInfo &result, plato::dddir dv, int level,
                   DepRel &rel);
...
inline FreeVar::FreeVar(const FreeVar &other) {
  if (UseOmega()) {
    ...
    _free_var_u->_omega_free_var =
        new OMEGA_FREE_VAR(*(other._free_var_u->_omega_free_var));
    assert(_free_var_u->_omega_free_var);
  } else {
    assert(other._free_var_u->_plato_free_var);
    _free_var_u->_plato_free_var =
        new PLATO_FREE_VAR(*(other._free_var_u->_plato_free_var));
    assert(_free_var_u->_plato_free_var);
  }
}
