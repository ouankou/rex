...
int debug_determine_function_arity = 0, debug_GetStdVarU = 0,
    debug_RenameDupedVars = 0, debug_AddSubscripts = 0,
    debug_AddCommonLoopIndices = 0, debug_AddLocalLoopIndices = 0,
    debug_AddLoopBounds = 0, debug_ComputeArrayDep = 0, debug_doHierarchy = 0,
    debug_VisitVar = 0, debug_VisitExpr = 0, debug_VisitConst = 0,
    debug_ParseVal = 0, debug_GetRationalExpression = 0,
    debug_GetNewRelationFromDV = 0, debug_AddConstraintsImposedByDV = 0;
...
static char *str_dir_vec(plato::dddir dv, int dvLevel) {
  ...
  for (i = 0; i < dvLevel; i++) {
    index = dv & 0xf;
    sprintf(dirstr, (i < dvLevel - 1) ? "%s%s," : "%s%s", dirstr,
            StrDir[index]);
    dv = dv >> 4;
  }
...
bool PlatoOmegaDepTesting::PlatoRelation::testDV(
    const plato::AbstractDirectionVector &dv, bool &accurate) {
  bool accCond, coupling, feasible, ifIgnored;
  feasible =
      plato::RationalVITestProblem::testDV(dv, accCond, coupling, ifIgnored);
  accurate = accCond && !coupling && !ifIgnored;
  return feasible;
}

plato::RationalExpression PlatoOmegaDepTesting ::GetRationalExpression(
    AstInterface &fa, const AstNodePtr &exp, bool is_input) {
  ...
  if (fa.IsVarRef(exp, 0, &name, &scope)) {
    ...
    print_debug("GetRationalExpression", "exiting",
                debug_GetRationalExpression);
    return *tempVar;
...
int PlatoOmegaDepTesting::ReturnVarianceLevel(const std::string name) {
  ...
  print_debug("_determine_function_arity", "entering",
              debug_determine_function_arity);
  ...
  print_debug("_determine_function_arity", "exiting",
              debug_determine_function_arity);
  return arity;
}
  ...
  while (iter != _omega_input_vars->end()) {
    ...
    case Input_Var: {
      std::cerr << " Input_Var, position "
                << iter->second->ReturnOmegaStdVar()->get_position()
                << std::endl;
    } break;
    case Output_Var: {
...
/**         Returns the STD_VAR_U union which contains either an omega
 *Variable_ID or a plato Variable.
 **/

STD_VAR_U *PlatoOmegaDepTesting::GetStdVarU(const std::string name,
                                            bool is_input) {
  ...
  if (UseOmega()) {
    if (_omega_input_vars->count(name) > 0 && is_input) {
      ...
      if (arity == 0) {
        print_debug("GetStdVarU", "arity 0 creating omega free var " + name,
                    debug_GetStdVarU);
        ...
        assert(std_var_u);
        std_var_u->_omega_std_var =
            _omega_relation->get_local(free_var->ReturnOmegaFreeVar());
        ...
        print_debug("GetStdVarU", "finished creating omega free var " + name,
                    debug_GetStdVarU);
      } else {
        print_debug("GetStdVarU", "arity > 0 creating omega free var " + name,
                    debug_GetStdVarU);
        FREE_VAR_U *free_var_u = new FREE_VAR_U();
        free_var_u->_omega_free_var =
            new OMEGA_FREE_VAR(goofy_omega_string, arity);
        FreeVar *free_var = new FreeVar(free_var_u);
        _free_vars->insert(make_pair(name, free_var));

        STD_VAR_U *std_var_u = new STD_VAR_U();
        std_var_u->_omega_std_var = _omega_relation->get_local(
            free_var->ReturnOmegaFreeVar(), Input_Tuple);
        ...
        std_var_u->_omega_std_var = _omega_relation->get_local(
            free_var->ReturnOmegaFreeVar(), Output_Tuple);
      ...
      case SYMOP_MULTIPLY: {
        if (operandsCount == 2 && varCount != 2) {
          if (UseOmega()) {
            ...
            plato::RationalExpression *v1 =
                new plato::RationalExpression(*tempID);
            plato::RationalExpression *v2 =
                new plato::RationalExpression(value);
            *_re = *_re + *v1 * *v2;
          }
          operandsCount = 0;
        } else if (varCount == 2) {
    ...
    case VAL_CONST: {
      ...
      case SYMOP_MULTIPLY: {
        if (operandsCount == 2) {
          if (UseOmega()) {
            if (_omega_negate_expr) {
              _constraint_handle->update_coef(
                  GetOmegaStdVar(name, _is_input_expr), -value);
            } else {
              _constraint_handle->update_coef(
                  GetOmegaStdVar(name, _is_input_expr), value);
            }
  ...
  if (UseOmega()) {
    ...
    if (_plato_is_bound) {
      if (_plato_is_lower_bound) {
        plato::RationalExpression *v1 =
            new plato::RationalExpression(MINUS_INF);
        *_re = *v1;
      } else {
...
plato::RationalExpression *
PlatoOmegaDepTesting::ParseVal(const SymbolicVal &v, bool negate, bool is_input,
                               bool is_bound, bool is_low_bound) {
...
/**        Adds all the index variables in the common (to both array refs)
 *loops. This function makes a big assumption in that both the _info1->ivars and
 *_info2->ivars have exactly the same entries up to the least dim - either _dim1
 *or _dim2. Then the loop stops and further entries are picked up in the
 *AddLocalLoopIndices.
 **/
...
/**        AddLocalLoopIndices adds all the loop indicies that surround the
 *common loops, and have not been added yet. The function makes the assumption
 *                that if it hasn't been added yet, then that dimension is
 *larger than the other and a free variable will be made and it will be added to
 *the proper map.
 **/

void PlatoOmegaDepTesting::AddLocalLoopIndices(
    void /*DepInfoAnal::LoopDepInfo info, bool is_input*/) {
  // int i = 1;
  StandardVar *var_ref = 0;
    ...
    while (iter != info->ivars.end()) {
      ...
      if (UseOmega()) {
        if ((!added(_omega_input_vars, index.toString()) && is_input) ||
          ...
          assert(free_var_u);
          free_var_u->_omega_free_var =
              new OMEGA_FREE_VAR(StringToChar(index.toString()));
          assert(free_var_u->_omega_free_var);
          FreeVar *free_var = new FreeVar(free_var_u);
          assert(free_var);
      ...
      /**        -------------- PLATO ONLY --------------
       *                 Here we are assuming that all of the common loop
       *indices have been added already, so any other enclosing loop indices
       ...
       **/
        ...
        FREE_VAR_U *free_var_u = new FREE_VAR_U();
        assert(free_var_u);
        free_var_u->_plato_free_var =
            new PLATO_FREE_VAR(StringToChar(index.toString()));
        ...
        if (is_input) {
          print_debug("AddLocalLoopIndices",
                      "creating plato input loop indice " + index.toString(),
                      debug_AddLocalLoopIndices);
...
void PlatoOmegaDepTesting::AddLoopBounds(
    void /*DepInfoAnal::LoopDepInfo info, bool is_input*/) {
  int stride = 1;
  int val;
  ...
  for (int h = 0; h < 2; h++) // input then output
  {
    ...
    for (int i = 0; i < dim; i++) {
      ...
      if (UseOmega()) {
        ...
        print_debug("AddLoopBounds",
                    "creating lower bound constraints for " +
                        info->ivars[i].GetVarName(),
                    debug_AddLoopBounds);
        ...
        print_debug("AddLoopBounds", "finished with upper bound constraints",
                    debug_AddLoopBounds);
      } else {
        print_debug("AddLoopBounds",
                    "creating bound constraints for " +
                        info->ivars[i].GetVarName(),
                    debug_AddLoopBounds);
        PLATO_STD_VAR *index;
        ...
        {
                print_debug("AddLoopBounds","about to addCommonLoopVariable " +
        info->ivars[i].GetVarName(), debug_AddLoopBounds);
        ...
        */
        ...
        {
          print_debug("AddLoopBounds",
                      "about to add plato common loop variable " +
                          info->ivars[i].GetVarName(),
                      debug_AddLoopBounds);
          _plato_relation->addCommonLoopVariable(*index, *lowBound, *upBound,
                                                 1);
        } else if (is_input && i >= _dim2) {
          print_debug("AddLoopBounds",
                      "about to add plato local input loop variable " +
                          info->ivars[i].GetVarName(),
                      debug_AddLoopBounds);
          _plato_relation->addNest1LoopVariable(*index, *lowBound, *upBound, 1);
        } else if (!is_input && i >= _dim1) {
          print_debug("AddLoopBounds",
                      "about to add plato local output loop variable " +
                          info->ivars[i].GetVarName(),
                      debug_AddLoopBounds);
...
void PlatoOmegaDepTesting::AddSubscripts(void) {
  ...
  for (; iter1 != sub1.rend() && iter2 != sub2.rend(); iter1++, iter2++) {
    ...
    if (UseOmega()) {
      ...
      print_debug("AddSubscripts", "about to add plato subscripts",
                  debug_AddSubscripts);
...
void PlatoOmegaDepTesting::AddConstraintsImposedByDV(Relation &relation,
  ...
  print_debug("AddConstraintsImposedByDV", "entering",
              debug_AddConstraintsImposedByDV);
  for (loop_depth = 0; loop_depth < _commLevel; loop_depth++) {
    ...
    */
    ...
    /*
    case _NEQ: {
            EQ_Handle neq_constraint =
    constraints.add_not()->add_and()->add_EQ();
  ...
  print_debug("AddConstraintsImposedByDV", "exiting",
              debug_AddConstraintsImposedByDV);
}

/**        ----------------OMEGA ONLY--------------
 *                Returns a relation with the same number of input and output
 *variables as my relation and with constraints imposed upon these variables by
...
Relation PlatoOmegaDepTesting::GetNewRelationFromDV(const plato::dddir &dv) {
  ...
  while (iter1 != _info1->ivars.end() && iter2 != _info2->ivars.end()) {
    const SymbolicVal &index = *iter1;
    // std::cerr << "creating name " << index.toString() << " index " << i <<
    // std::endl;
...
void PlatoOmegaDepTesting::doHierarchy(
    SetDep &setdep, DepInfo &result, plato::dddir dv, int level,
    DepRel &rel) { // Recursive data dependence test
  ...
  } else /** Finished constructing, now test **/
  {
    if (DebugOmegaDep() || debug_doHierarchy)
      std::cerr << "entering doHierarchy Testing Direction Vector "
                << str_dir_vec(dv, sharedLevel) << " level " << level
                << std::endl;
      ...
      */
      if (dv_omega_relation.is_upper_bound_satisfiable()) {
        ...
        */
        ...
        if (lower_bound == 0 && upper_bound == 0 && level > sharedLevel) {
          ...
          */
          ...
          if (upper_bound != posInfinity) {
            ...
            if (DebugOmegaDep() || debug_doHierarchy)
              std::cerr << "i " << i << " " << " _commLevel " << _commLevel
                        << " assigned DEPDIR_EQ " << lower_bound << " "
                        << upper_bound << std::endl;
            precise = true;
          } else {
            if (DebugOmegaDep() || debug_doHierarchy)
              std::cerr << "i " << i << " " << " _commLevel " << _commLevel
                        << " assigned DEPDIR_GE " << std::endl;
        ...
        if (DebugOmegaDep() || debug_doHierarchy)
          std::cerr << "i " << i << " " << " _commLevel " << _commLevel
                    << " has DV of " << str_dir_vec(_saved_dv, sharedLevel)
                    << std::endl;
      } else {
        if (DebugOmegaDep() || debug_doHierarchy)
          std::cerr << "i " << i << " " << " _commLevel " << _commLevel
                    << " assigned DEPDIR_NONE " << std::endl;
...
void PlatoOmegaDepTesting::RenameDupedVars(void) {
  ...
  for (; iter1 != sub1.rend() && iter2 != sub2.rend(); iter1++, iter2++) {
    ...
    SymbolicVal left1 =
        DecomposeAffineExpression(fa, val1, _info1->ivars, cur, _dim1);
    SymbolicVal left2 =
        DecomposeAffineExpression(fa, -val2, _info2->ivars, cur, _dim2);
    for (i = 0; i < _dim1; ++i) {
      ...
      print_debug("RenameDupedVars", "finished varoping",
                  debug_RenameDupedVars);
    }
    ...
    // if (DebugOmegaDep())
    {
      ...
      // std::cerr << "coefficients for induction variables (" << _dim1 << " + "
      // << _dim2 << "+ 1)\n"; for (int i = 0; i < _dim1 + _dim2; ++i) std::cerr
      // << cur[i].toString() << _bounds[i]->toString() << " " ;
    ...
    print_debug("RenameDupedVars", "finisishing loop body",
                debug_RenameDupedVars);
  }
...
DepInfo PlatoOmegaDepTesting::ComputeArrayDep(
    DepInfoAnal &anal, const DepInfoAnal::StmtRefDep &ref, DepType deptype) {
  if ((DebugOmegaDep() || debug_ComputeArrayDep) && UseOmega()) {
    std::cerr << "OmegaDependenceTesting compute array dep between "
              << AstToString(ref.r1.ref) << " and " << AstToString(ref.r2.ref)
              << std::endl;
  } else if ((DebugOmegaDep() || debug_ComputeArrayDep) && !UseOmega()) {
    std::cerr << "PlatoDependenceTesting compute array dep between "
              << AstToString(ref.r1.ref) << " and " << AstToString(ref.r2.ref)
              << std::endl;
  }

  /**        info1 and info2 contains both a vector of the induction variables
   *of the surrounding loops and a vector of the bounds of the induction
   *variables See definition for LoopDepInfo in DepInfoAnal.h
   **/
   ...
   *                that model the relations of the induction variables
   *surrounding each stmt. _dim1 and _dim2 are the numbers of loops surrounding
  ...
  /**        'precise' is set to be true to indicate a 'yes' answer from
   *testing. should be reset to false if the testing result is 'maybe'
   **/

  int precise = true;

  /**        result contains a new DepInfo object that will remember the
   *dependence relations between r1 and r2
   **/
  DepInfo result = DepInfoGenerator::GetDepInfo(
      _dim1, _dim2, deptype, ref.r1.ref, ref.r2.ref, false, ref.commLevel);
  ...
  if (_commLevel > 0) {
    ...
    if (UseOmega()) {
      ...
      assert(_omega_relation && _omega_constraints && _omega_input_vars &&
             _omega_output_vars && _free_vars);
    } else {
      ...
      assert(_plato_relation && _plato_driver && _plato_input_vars &&
             _plato_input_vars && _free_vars);
    }
    ...
    /*
                    if (DebugOmegaDep() && !_nonLinear) {
                            ...
                            if (_omega_relation->is_upper_bound_satisfiable()) {
                                    std::cerr << "is upper bound satisfiable" <<
       std::endl;
                            }
                            else {
                                    std::cerr << "is not upper bound
    ...
    */
    ...
    if ((DebugOmegaDep() || debug_ComputeArrayDep) && UseOmega())
      std::cerr << "Omega Returned Direction Vector "
                << str_dir_vec(_saved_dv, ref.commLevel) << std::endl;
    else if ((DebugOmegaDep() || debug_ComputeArrayDep) && !UseOmega())
      std::cerr << "Plato Returned Direction Vector "
                << str_dir_vec(_saved_dv, ref.commLevel) << std::endl;
  ...
  /**        the finalize function is called to impose as much restrictions as
   *possible to the DepInfo result
   **/
  setdep.finalize();
