...
public:
  DepEDDTypeInfo(DepType _t, int dim1, int dim2, bool p,
                 int cl) // p: precise or not,  cl: common level
      : DepInfoImpl(dim1, dim2, p, cl) {
    t = _t;
  }
  DepEDDTypeInfo(const DepEDDTypeInfo &that) : DepInfoImpl(that) { t = that.t; }
...
public:
  ...
  DepEDDRefInfo(DepType _t, int dim1, int dim2, AstNodePtr _src,
                const AstNodePtr &_snk, bool p,
                int cl) // dim1, dim2 are rows and cols for the matrix, p cl is
                        // common level
      : DepEDDTypeInfo(_t, dim1, dim2, p, cl), src(_src), snk(_snk) {}
  DepEDDRefInfo(const DepEDDRefInfo &that)
      : DepEDDTypeInfo(that), src(that.SrcRef()), snk(that.SnkRef()) {}
  virtual ~DepEDDRefInfo() {}
  virtual DepInfoImpl *Clone() const { return new DepEDDRefInfo(*this); }
};

DepInfo DepInfoGenerator::GetTopDepInfo() { return DepInfo(); }
...
DepInfo DepInfoGenerator::GetDepInfo(int nr, int nc, DepType t, bool p,
                                     int commLevel) {
  return (t == DEPTYPE_NONE)
             ? DepInfo(new DepInfoImpl(nr, nc, p, commLevel))
             : DepInfo(new DepEDDTypeInfo(t, nr, nc, p, commLevel));
}

DepInfo DepInfoGenerator::GetDepInfo(int nr, int nc, DepType t,
                                     const AstNodePtr &srcRef,
                                     const AstNodePtr &snkRef, bool p,
                                     int commLevel) {
  return ((t & DEPTYPE_DATA) || (t & DEPTYPE_INPUT))
             ? DepInfo(
                   new DepEDDRefInfo(t, nr, nc, srcRef, snkRef, p, commLevel))
             : DepInfo(new DepEDDTypeInfo(t, nr, nc, p, commLevel));
}
...
std::string DepType2String(DepType t) {
  ...
  case DEPTYPE_SCALAR:
    return "SCALAR_DEP;";
  case DEPTYPE_BACKSCALAR:
    return "SCALAR_BACK_DEP;";
  case DEPTYPE_IO:
    return "IO_DEP;";
  case DEPTYPE_DATA:
    return "DATA_DEP;";
  case DEPTYPE_BACKCTRL:
    return "BACKCTRL_DEP;";
  case DEPTYPE_TRANS:
    return "TRANS_DEP;";
  case DEPTYPE_NONE:
...
std::string DepInfo ::toString() const {
  ...
  if (is_precise())
    ...
    out << " Not precise ";
  out << AstInterface::AstToString(SrcRef())
      << AstInterface::getAstLocation(SrcRef()) << "->"
      << AstInterface::AstToString(SnkRef())
      << AstInterface::getAstLocation(SnkRef()) << "\n";
...
DepInfo Reverse(const DepInfo &info) {
  ...
  DepInfo result = DepInfoGenerator::GetDepInfo(
      info.cols(), info.rows(), t, info.SnkRef(), info.SrcRef(),
      info.is_precise(), info.CommonLevel());
