...
public:
  DepGraphEdgeCreate(AstTreeDepGraphBuildImpl *g,
                     GraphAccessInterface::Node *_n1,
                     GraphAccessInterface::Node *_n2)
...
void AstTreeDepGraphAnal ::ComputeDataDep(const StmtNodeInfo &n1,
...
void AstTreeDepGraphAnal ::ComputeStmtDep(const StmtNodeInfo &n1,
...
void AstTreeDepGraphAnal::ComputeCtrlDep(const StmtNodeInfo &nc,
                                         const StmtNodeInfo &ns, DepType t) {
  AstNodePtr c = nc.start, s = ns.start;
  // bool equal  = (c == s);
  DepGraphEdgeCreate deps1(graph, nc.node, ns.node),
      deps2(graph, ns.node, nc.node);
  impl.ComputeCtrlDep(c, s, deps1, deps2, t);
}

bool BuildAstTreeDepGraph::ProcessIf(AstInterface &fa, const AstNodePtr &l,
                                     const AstNodePtr &cond,
                                     const AstNodePtr &truebody,
  ...
  return ProcessAstTree<AstNodePtr>::ProcessIf(fa, l, cond, truebody, falsebody,
                                               t);
}

bool BuildAstTreeDepGraph::ProcessLoop(AstInterface &fa, const AstNodePtr &l,
  ...
  return ProcessAstTree<AstNodePtr>::ProcessLoop(fa, l, body, t);
}

bool BuildAstTreeDepGraph ::ProcessGoto(AstInterface &fa, const AstNodePtr &s,
...
bool BuildAstTreeDepGraph ::ProcessStmt(AstInterface &fa, const AstNodePtr &s) {
  GraphAccessInterface::Node *n = graph->CreateNodeImpl(s, GetStmtDomain(s));
  StmtNodeInfo info(n, s);
...
void BuildAstTreeDepGraph ::TranslateCtrlDeps() {
  ...
  for (StmtStackType::Iterator p(stmtNodes); !p.ReachEnd(); ++p) {
    ...
    for (GraphAccessInterface::EdgeIterator p1 =
             g->GetNodeEdgeIterator(n1, GraphAccess::EdgeIn);
         !p1.ReachEnd(); ++p1) {
      ...
      if (depIter1.ReachEnd())
        continue;
      GraphAccessInterface::Node *ctrl =
          g->GetEdgeEndPoint(e1, GraphAccess::EdgeOut);
      DepInfo cd = depIter1.Current();
      DepInfo cd1 = Reverse(cd);
      GraphAccessInterface::EdgeIterator p2 =
          g->GetNodeEdgeIterator(ctrl, GraphAccess::EdgeOut);
      for (; !p2.ReachEnd(); ++p2) {
        GraphAccessInterface::Edge *e2 = p2.Current();
        GraphAccessInterface::Node *n2 =
            g->GetEdgeEndPoint(e2, GraphAccess::EdgeIn);
      ...
      for (p2 = g->GetNodeEdgeIterator(ctrl, GraphAccess::EdgeIn);
        ...
        GraphAccessInterface::Edge *e2 = p2.Current();
        GraphAccessInterface::Node *n2 =
            g->GetEdgeEndPoint(e2, GraphAccess::EdgeOut);
        for (DepInfoConstIterator depIter2 =
                 graph->GetDepInfoIteratorImpl(e2, DEPTYPE_DATA);
             !depIter2.ReachEnd(); depIter2++) {
