...
public:
  DepInfoSet GetTopInfo(const GraphAccessInterface::Node *n1,
                        const GraphAccessInterface::Node *n2) {
    return DepInfoSetGenerator::GetTopInfoSet();
  }
  DepInfoSet GetBottomInfo(const GraphAccessInterface::Node *n1,
                           const GraphAccessInterface::Node *n2) {
  ...
  void UnionWith(DepInfoSet &e1, DepInfoSet e2) { e1 |= e2; }
  DepInfoSet Composite(DepInfoSet e1, DepInfoSet e2) { return e1 * e2; }
  DepInfoSet Closure(DepInfoSet e) { return ::Closure(e); }
};

template <class Node>
bool TransDepGraphCreate<Node>::TransInfoComputed(
    const GraphAccessInterface::Node *_src,
    const GraphAccessInterface::Node *_snk) {
  const Node *src = static_cast<const Node *>(_src);
  const Node *snk = static_cast<const Node *>(_snk);
  return !GraphCrossEdgeIterator<TransDepGraphCreate<Node>>(this, src, snk)
              .ReachEnd();
}

template <class Node>
DepInfoSet TransDepGraphCreate<Node>::GetTransInfo(
    const GraphAccessInterface::Node *_src,
    const GraphAccessInterface::Node *_snk) {
  const Node *src = static_cast<const Node *>(_src);
  const Node *snk = static_cast<const Node *>(_snk);
  DepInfoSetEdge *edge =
      GraphCrossEdgeIterator<TransDepGraphCreate<Node>>(this, src, snk)
          .Current();
  return edge->GetInfo();
}

template <class Node>
void TransDepGraphCreate<Node>::SetTransInfo(GraphAccessInterface::Node *src1,
                                             GraphAccessInterface::Node *snk1,
                                             DepInfoSet info) {
...
template <class Node>
DepInfoSetEdge *TransDepGraphCreate<Node>::GetTransDep(Node *src, Node *snk) {
  DepInfoSetEdge *edge = 0;
  GraphCrossEdgeIterator<TransDepGraphCreate<Node>> cross(this, src, snk);
  if (cross.ReachEnd() && anal.ComputeTransInfo(this, src, snk)) {
    edge = (GraphCrossEdgeIterator<TransDepGraphCreate<Node>>(this, src, snk)
                .Current());
    assert(edge != 0);
  } else
    edge = cross.Current();
...
template <class Node>
TransDepGraphCreate<Node>::TransDepGraphCreate(
    GraphAccessTemplate<Node, DepInfoEdge> *graph, int splitlimit,
    BaseGraphCreate *bc)
    : VirtualGraphCreateTemplate<Node, DepInfoSetEdge>(bc), access(graph),
