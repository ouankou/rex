...
template <class Graph, class DepInfoUpdate> class UpdateDepEdgeInfo {
  DepInfoUpdate T;
  Graph *graph;
...
void CopySplitNodeEdge(Graph *graph, Node *orig, Node *split,
...
template <class Graph, class Node>
void DepGraphNodeRestrictDomain(Graph *graph, Node *node,
...
template <class Graph, class Node>
void DepGraphNodeSwapLoop(Graph *graph, Node *node, int level1, int level2) {
...
template <class Graph, class Node>
void DepGraphNodeMergeLoop(Graph *graph, Node *node, int desc, int src) {
  typedef UpdateDepEdgeInfo<Graph, DepInfoMergeLoop> InfoOp;
  UpdateNodeEdge(graph, node, InfoOp(graph, DepInfoMergeLoop(desc, src)));
}

template <class Graph, class Node>
void DepGraphNodeAlignLoop(Graph *graph, Node *node, int level, int align) {
  if (align != 0) {
