...
template <class Node> struct LoopAnalInfo {
  Node *n;
  int index;
  LoopAnalInfo(Node *_n, int _level) {
    n = _n;
    index = _level;
  }
};
...
inline LoopAlignInfo operator+(const LoopAlignInfo &info1,
                               const LoopAlignInfo &info2) {
...
template <class Node> class TransLoopFusible {
public:
...
template <class Node> class TransLoopSlicable {
public:
  bool operator()(TransDepGraphCreate<Node> *graph,
                  const LoopAnalInfo<Node> &l);
};

template <class Edge, class GraphCreate> class PerfectLoopSlicable {
public:
  bool operator()(GraphCreate *g, int level);
};

template <class Node> class TransLoopDistributable {
public:
  bool operator()(TransDepGraphCreate<Node> *graph,
                  const LoopAnalInfo<Node> &l1, const LoopAnalInfo<Node> &l2);
};

template <class Node> class TransLoopReversible {
public:
  bool operator()(TransDepGraphCreate<Node> *graph,
                  const LoopAnalInfo<Node> &l);
};

template <class Edge, class GraphCreate> class PerfectLoopReversible {
public:
  bool operator()(GraphCreate *g, int level);
};
