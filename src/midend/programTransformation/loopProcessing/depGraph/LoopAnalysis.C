...
template <class Node>
LoopAlignInfo
TransLoopFusible<Node>::operator()(TransDepGraphCreate<Node> *graph,
...
template <class Node>
bool TransLoopSlicable<Node>::operator()(TransDepGraphCreate<Node> *graph,
...
bool PerfectLoopSlicable<Edge, GraphCreate>::operator()(GraphCreate *g,
                                                        int level) {
  DebugLog DebugSlice("-debugslice");
  DepDirType cur = DEPDIR_EQ;
  for (GraphEdgeIterator<GraphCreate> edgeIter(g); !edgeIter.ReachEnd();
       ++edgeIter) {
    for (DepInfoConstIterator depIter = edgeIter.Current()->get_depIterator();
      ...
      if (cur == DEPDIR_EQ)
        ...
        DebugSlice([&r]() {
          return "Loop not slideable b/c of conflicting dependence "
                 "direction: " +
                 r.toString();
        });
        return false;
      ...
      if ((dir == DEPDIR_LE && a2 <= 0) || (dir == DEPDIR_GE && a1 >= 0) ||
        ...
        continue;
      DebugSlice([&r]() {
        return "Loop not slideable b/c of dependence direction: " +
               r.toString();
      });
      return false;
    }
...
template <class Node>
bool TransLoopReversible<Node>::operator()(TransDepGraphCreate<Node> *graph,
                                           const LoopAnalInfo<Node> &l) {
  bool result = false;
...
template <class Edge, class GraphCreate>
bool PerfectLoopReversible<Edge, GraphCreate>::operator()(GraphCreate *g,
                                                          int level) {
  for (GraphEdgeIterator<GraphCreate> edgeIter(g); !edgeIter.ReachEnd();
       ++edgeIter) {
    for (DepInfoConstIterator depIter = edgeIter.Current()->get_depIterator();
...
template <class Node>
bool TransLoopDistributable<Node>::operator()(TransDepGraphCreate<Node> *graph,
                                              const LoopAnalInfo<Node> &l1,
                                              const LoopAnalInfo<Node> &l2) {
  bool result = false;
  ;
