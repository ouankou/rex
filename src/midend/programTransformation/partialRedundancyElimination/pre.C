...
#define FIXPOINT_OUTPUT_BEGIN(pmap, cfg) bool old_##pmap = pmap[v]
...
public:
  ReplaceExpressionWithVarrefVisitor(SgExpression *expr, SgVarRefExp *vr)
      : expr(expr), vr(vr) {}
...
// 48 (2003) 1--20).
void PRE::partialRedundancyEliminationOne(SgExpression *expr,
                                          SgBasicBlock *root,
  ...
  // cerr << "Trying to do PRE using expression " << expr->unparseToString() <<
  // " whose type is " << expr->sage_class_name() << endl;

  VertexIter i = cfg.graph.vertices().begin(), end = cfg.graph.vertices().end();
  ...
  vector<bool> transp(cfg.graph.vertices().size()),
      comp(cfg.graph.vertices().size()), antloc(cfg.graph.vertices().size());
  ...
  for (i = cfg.graph.vertices().begin(); i != end; ++i) {
    ...
    for (unsigned int j = 0; j < stmts.size(); ++j) {
      ...
      expressionComputedInStatement[j] =
          countComputationsOfExpressionIn(expr, stmts[j]);
    }
    ...
    // cout << "In node " << *i << endl;
    for (unsigned int j = 0; j < stmts.size(); ++j) {
      // cout << "In stmt " << j << ", expressionComputedInStatement = " <<
      // expressionComputedInStatement[j]
      //      << ", argumentsModifiedInStatement = " <<
      //      argumentsModifiedInStatement[j] << endl;
      if (expressionComputedInStatement[j] &&
          !argumentsModifiedInStatement[j] && comp[*i] /* from last iter */) {
    ...
    for (unsigned int j = 0; j < stmts.size(); ++j) {
      if (expressionComputedInStatement[j] &&
          !argumentsModifiedInStatement[j]) {
        antloc[*i] = true;
...
// #define PRINT_PROPERTY(p) // for (i = cfg.graph.vertices().begin(); i != end;
// ++i) if (p[*i]) cerr << #p ": " << *i << endl;
#define PRINT_PROPERTY(p) // for (i = cfg.graph.vertices().begin(); i != end;
                          // ++i) if (p[*i]) cerr << #p ": " << *i << endl;
  ...
  avin[v] = accumulate_neighbors(cfg, v, avout, cfg.graph.in_edges(v),
                                 &simpleGraph::source, logical_and<bool>(),
                                 true, false);
  ...
  antout[v] =
      accumulate_neighbors(cfg, v, antin, cfg.graph.out_edges(v), target_ptr,
  ...
  vector<bool> safein(cfg.graph.vertices().size()),
      safeout(cfg.graph.vertices().size());
  ...
  spavin[v] = safein[v] && accumulate_neighbors(
                               cfg, v, spavout, cfg.graph.in_edges(v),
                               source_ptr, logical_or<bool>(), false, false);
  ...
  spantout[v] = safeout[v] && accumulate_neighbors(
                                  cfg, v, spantin, cfg.graph.out_edges(v),
                                  target_ptr, logical_or<bool>(), false, false);
...
#undef PRINT_PROPERTY
  ...
  for (i = cfg.graph.vertices().begin(); i != end; ++i) {
    ...
    if (node_insert[*i]) {
      ...
      // cerr << "Insert computation of " << expr->unparseToString() << " just
      // before last computation in " << *i << endl;
    }
  ...
  for (; j != jend; ++j) {
    edge_insert[*j] = !spavout[cfg.graph.source(*j)] &&
                      spavin[cfg.graph.target(*j)] &&
                      spantin[cfg.graph.target(*j)];

    // printf ("edge_insert[*j] = %s \n",edge_insert[*j] ? "true" : "false");
    if (edge_insert[*j]) {
      needToMakeCachevar = true;
      // cerr << "Insert computation of " << expr->unparseToString() << " on
      // edge from "
  ...
  for (j = cfg.graph.edges().begin(); j != jend; ++j) {
    ...
    if (edge_insert[*j]) {
      ...
      if (insert_point.first) {
        ...
        // DQ (3/16/2006): This is a visited when we fixup the NULL pointer to
        // the initializer in a SgForStatment.
        cerr << "Warning: no insertion point found" << endl; // FIXME was assert
        printf("Need to figure out what to do here! "
               "cfg.edge_insertion_point[*j] = %p \n",
  ...
  // \n",insertions.size());
  for (vector<pair<SgNode *, bool>>::iterator i = insertions.begin();
       i != insertions.end(); ++i) {
    SgTreeCopy tc1, tc2;
    ...
    if (isSgExpression(i->first) && !i->second) {
      ...
      if (isSgForStatement(ifp)) {
        ...
        isSgBinaryOp(ifp)->set_lhs_operand(comma);
                 ...
                 isSgBinaryOp(ifp)->get_rhs_operand() == i->first) {
        isSgBinaryOp(ifp)->set_rhs_operand(comma);
      } else if (isSgUnaryOp(ifp) &&
                 isSgUnaryOp(ifp)->get_operand() == i->first) {
      ...
      if (isSgBasicBlock(i->first) && i->second) {
        ...
        if (forStatement != NULL) {
          // Make sure that both pointers are not equal because they are both
          // NULL!
          ...
          SgForInitStatement *forInitStatement =
              isSgForInitStatement(i->first->get_parent());
          if (forInitStatement != NULL) {
            // printf ("Found the SgForInitStatement \n");
            SgVariableDeclaration *possibleVariable =
                isSgVariableDeclaration(i->first);
            ...
            // avoid interator invalidation
            if (addToForInitList == true) {
              // Add the the_computation statment to the list in the
              // SgForInitStatement. Later if we abandon the SgForInitStatement
              // then we would have to add it to the list of SgInitializedName
              // objects in the SgVariableDeclaration OR modify the expression
              // list to handle the extra expression (but I think this case in
              // handled above).
...
void PRE::partialRedundancyEliminationFunction(SgFunctionDefinition *n) {
  ...
  // DQ (4/8/2006): Call the new constant folding (not fully implemented except
  // that it works well to eliminate the stored constant expression trees in the
  // AST which are redundant with the values on SgValueExp IR nodes.  The
  // storage of the constant expression trees from which constant folded values
  // are generated is a new development within ROSE and required to represent
  // the complete structure of the source code (and handle details like marcos,
  // floating literals as strings, etc.).  The storage, and traversal of these
  // redundant constant expressions is a problem for PRE, and since I could not
  // fix it easily, I implemented the constant folding in ROSE (not finished) so
  // that we could remove the constant expression trees which interfere with PRE
  // and which are philosophically a problem for other forms of program analysis
  // as well. So calling this here is as if we have constant folding as a phase
  // before PRE.
  ...
  for (vector<SgExpression *>::iterator i = vis.exprs.begin();
       i != vis.exprs.end(); ++i) {
    if (std::find_if(exprs_done.begin(), exprs_done.end(),
                     ExpressionTreeEqualTo(*i)) != exprs_done.end())
      continue;
...
public:
  virtual void visit(SgNode *n) {
    if (isSgFunctionDefinition(n)) {
      ...
      // \n",n->class_name().c_str()); n->get_file_info()->display("location of
...
void PRE::partialRedundancyElimination(SgNode *n) {
