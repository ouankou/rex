...
public:
  ...
  Vertices vertices() const { return all_vertices; }

  Edges edges() const { return all_edges; }

  Edges out_edges(int vertex) const { return raw_out_edges[vertex]; }

  Edges in_edges(int vertex) const { return raw_in_edges[vertex]; }

  int source(int edge) const { return sources[edge]; }

  int target(int edge) const { return targets[edge]; }
};
...
void printCfgAsDot(std::ostream &dotfile, const ControlFlowGraph &controlflow);
