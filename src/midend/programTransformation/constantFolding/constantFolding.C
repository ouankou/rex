...
// been done within legacy frontend on the part of the AST generated by the
// front-end.
...
void ConstantFolding::constantFoldingOptimization(
    SgNode *n, bool internalTestingAgainstFrontend) {
  // This is the main function interface for constant folding
  ConstantFoldingTraversal t;
  ConstantFoldingInheritedAttribute ih;

  // Set internal ability to do error checking against any constant expression
  // trees stored in the AST.
...
SgValueExp *ConstantFolding::returnConstantFoldedValueExpression(
    SgNode *n, bool internalTestingAgainstFrontend) {
  // This is the main function interface for constant folding
  ConstantFoldingTraversal t;
  ConstantFoldingInheritedAttribute ih;
...
void ConstantFolding::constantUnFoldingTest(SgNode *n) {
  // This is the main function interface for constant un-folding tests.
  // Since the constant folding we have only checks the results computed by
  // legacy frontend where there are no transformation, we have to generate a
  // transformation to have something to constant fold.  This transformation
  // generates constant expressions from constants to use in testing the
  // constant folding where legacy frontend would not have generated the
  // constant folding.
  ...
  // Test this by turning constants into rather long and ugly constant
  // expressions!
  t.traverse(n);
...
ConstantFoldingTraversal::evaluateInheritedAttribute(
    SgNode *astNode, ConstantFoldingInheritedAttribute inheritedAttribute) {
  // Compute the inheritedAttribute.  Since the modification of the AST must be
  // done bottom up, this is the most we can do here (since the evaluation of
  // inherited attributes happens top down).
  if (inheritedAttribute.isConstantFoldedValue == true) {
    ...
    if (originalExpression != NULL &&
        originalExpression->get_originalExpressionTree() != NULL) {
      if (inheritedAttribute.internalTestingAgainstFrontend == true) {
        // If we are testing the constant folding then refold the
        // values constant expression trees stored in the AST and check
        // the result against what legacy frontend computed.

        // To test the constant folding skip marking the constant as folded and
        // test if we fold the constant expression correctly
        inheritedAttribute.isConstantFoldedValue = false;
      } else {
        // This is a resulting value generated from constant folding
        // in the front-end (legacy frontend)
        inheritedAttribute.isConstantFoldedValue = true;

        // We can just remove a subtree that we have not yet traversed, is this
        // perfectly safe? I think it is safe since it is a child and not a
        // sibling! However, this does not delete the existing constant folded
        // trees, it just disconnects them from the AST.
        SgExpression *original_tree =
            originalExpression->get_originalExpressionTree();
#if PRINT_DEVELOPER_WARNINGS
        printf("Deleting the original expression tree within "
               "ConstantFoldingTraversal::evaluateInheritedAttribute.\n");
#endif
...
void ConstantFoldingTraversal::replaceExpressionWithDeferredDelete(
    SgExpression *oldExp, SgExpression *newExp) {
...
static int cf_get_int_value(SgValueExp *sg_value_exp) {
  ...
  if (isSgBoolValExp(sg_value_exp))
    ...
    cerr << "error: wrong value exp type for cf_get_int_value():"
         << sg_value_exp->class_name() << endl;
    ROSE_ABORT();
  }
  return rtval;
}

//! Evaluate a binary expression  a binOp b, return a value exp if both operands
//! are constants
static SgValueExp *evaluateBinaryOp(SgBinaryOp *binaryOperator) {
  ...
  if (lhsOperand == NULL || rhsOperand == NULL)
    return NULL;
  // Check if these are value expressions (constants appropriate for constant
  // folding)
  ...
  if (lhsValue != NULL && rhsValue != NULL) {
    ...
    case V_SgBoolValExp: { // special handling for bool type, since intVal is
                           // built as a result
    ...
    case V_SgCharVal: // mostly provide a pair of SgValueExp class type and its
                      // internal value type for SgCharVal, the internal value
                      // type is also char
    {
      result = buildResultValueExp_t<SgCharVal, char>(binaryOperator, lhsValue,
                                                      rhsValue);
      break;
    }
      // each calls a subset (bitvec again?) of binary calculation according to
      // data types
    case V_SgDoubleVal: {
      result = buildResultValueExp_float_t<SgDoubleVal, double>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
      // enumerate value is special, we build an integer equivalent here for
      // simplicity
    case V_SgEnumVal: {
      result = buildResultValueExp_t<SgIntVal, int>(binaryOperator, lhsValue,
                                                    rhsValue);
      break;
    }
    case V_SgFloatVal: {
      // Liao, 9/22/2009, folding float to float may decrease the accuracy.
      // promoting it to double
      // TODO, need an option to skip folding float point operations
      result = buildResultValueExp_float_t<SgFloatVal, float>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgIntVal: {
    ...
    case V_SgLongDoubleVal: {
      result = buildResultValueExp_float_t<SgLongDoubleVal, long double>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgLongIntVal: {
      result = buildResultValueExp_t<SgLongIntVal, long int>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgLongLongIntVal: {
      result = buildResultValueExp_t<SgLongLongIntVal, long long int>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgShortVal: {
      result = buildResultValueExp_t<SgShortVal, short>(binaryOperator,
                                                        lhsValue, rhsValue);
      break;
    }
    // special handling for string val due to its unique constructor
    case V_SgStringVal: {
      std::string lhsInteger =
          cf_get_value_t<std::string>(isSgStringVal(lhsValue));
      std::string rhsInteger =
          cf_get_value_t<std::string>(isSgStringVal(rhsValue));
      std::string foldedValue =
          calculate_string_t(binaryOperator, lhsInteger, rhsInteger);
      result = SageBuilder::buildStringVal(foldedValue);
      break;
    }
    case V_SgUnsignedCharVal: {
      result = buildResultValueExp_t<SgUnsignedCharVal, unsigned char>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgSignedCharVal: {
      result = buildResultValueExp_t<SgSignedCharVal, signed char>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgUnsignedIntVal: {
      result = buildResultValueExp_t<SgUnsignedIntVal, unsigned int>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgUnsignedLongLongIntVal: {
      result = buildResultValueExp_t<SgUnsignedLongLongIntVal,
    ...
    case V_SgUnsignedLongVal: {
      result = buildResultValueExp_t<SgUnsignedLongVal, unsigned long>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    case V_SgUnsignedShortVal: {
      result = buildResultValueExp_t<SgUnsignedShortVal, unsigned short>(
          binaryOperator, lhsValue, rhsValue);
      break;
    }
    default: {
      cout << "evaluateBinaryOp(): " << "Unhandled SgValueExp case for:"
           << lhsValue->class_name() << endl;
      // Not a handled type
    }
    }
...
static SgValueExp *evaluateUnaryOp(SgUnaryOp *unaryOperator) {
  ...
  if (theValue != NULL) {
    ...
    case V_SgDoubleVal: {
      result =
          buildResultValueExp_u_t<SgDoubleVal, double>(unaryOperator, theValue);
      break;
    }
    case V_SgEnumVal: {
    ...
    case V_SgFloatVal: {
      // folding float to float may decrease the accuracy. promoting it to
      // double
      result =
          buildResultValueExp_u_t<SgFloatVal, float>(unaryOperator, theValue);
      break;
    }
    case V_SgIntVal: {
      result = buildResultValueExp_u_t<SgIntVal, int>(unaryOperator, theValue);
    ...
    case V_SgLongDoubleVal: {
      result = buildResultValueExp_u_t<SgLongDoubleVal, long double>(
          unaryOperator, theValue);
      break;
    }
    case V_SgLongIntVal: {
      result = buildResultValueExp_u_t<SgLongIntVal, long int>(unaryOperator,
                                                               theValue);
      break;
    }
    case V_SgLongLongIntVal: {
      result = buildResultValueExp_u_t<SgLongLongIntVal, long long int>(
          unaryOperator, theValue);
      break;
    }
    case V_SgShortVal: {
      result =
          buildResultValueExp_u_t<SgShortVal, short>(unaryOperator, theValue);
      break;
    }
    case V_SgUnsignedIntVal: {
      result = buildResultValueExp_u_t<SgUnsignedIntVal, unsigned int>(
          unaryOperator, theValue);
      break;
    }
    case V_SgUnsignedLongLongIntVal: {
      result = buildResultValueExp_u_t<SgUnsignedLongLongIntVal,
                                       unsigned long long int>(unaryOperator,
                                                               theValue);
      break;
    }

    case V_SgUnsignedLongVal: {
      result = buildResultValueExp_u_t<SgUnsignedLongVal, unsigned long>(
          unaryOperator, theValue);
      break;
    }
    case V_SgUnsignedShortVal: {
      result = buildResultValueExp_u_t<SgUnsignedShortVal, unsigned short>(
          unaryOperator, theValue);
      break;
    }
    default: {
      cout << "evaluateUnaryOp(): " << "Unhandled SgValueExp case for:"
           << theValue->class_name() << endl;
    }
    }
  } // operand is constant
  return result;
...
static SgValueExp *evaluateConditionalExp(SgConditionalExp *cond_exp) {
  ...
  if (value_exp) {
    ...
    // TODO first operand can be scalar type: arithmetic (integer + floating
    // point) + pointer
    //   case V_SgCharVal:
    //   {
    ...
    default: {
      cout << "evaluateConditionalExp(): unhandled value expression type of a "
              "conditional exp:"
           << value_exp->class_name() << endl;
    }
    } // end switch
...
ConstantFoldingTraversal::evaluateSynthesizedAttribute(
  ...
  if (inheritedAttribute.isConstantFoldedValue == false) {
    // Here is where the front-end mechanism leaves off and we pickup the job to
    // do the constant folding of anything not handled by the front-end.  This
    // applies to constant expression trees built as part of transformations.
    // This should help cleanup a lot of automatically generated code.
    ...
    if (expr != NULL) {
      if (inheritedAttribute.internalTestingAgainstFrontend == true) {
        // If this is a SgValueExp it might be that a const expression tree is
        // hidden inside of it. This would have been done in the front-end
        // processing. To check our constant folding we have to look in the
        // SgValueExp for the originalExpressionTree and copy the generated
        // SgIntVal from there (actually where we are just checking, so we only
        // verify that we get the same value).
        SgValueExp *valueExpression = isSgValueExp(expr);
        if (valueExpression != NULL) {
          // If we have a value expression then there could be many types of
          // value IR nodes
          switch (valueExpression->variantT()) {
          case V_SgIntVal: {
            printf("Warning: originalExpressionTree is no longer a part of the "
                   "AST traversal (must be accessed explicitly). code in "
      ...
      // Liao, 6/26/2009, add more cases, this is used by
      // SageInterface::loopUnrolling() For each type of expressions, do the
      // following: step 1: replace children with their synthesized attribute
      // values , if any step 1: calculate the current node 's synthesized
      // attribute value if children' values are constants Binary expressions
      if (isSgBinaryOp(
              expr)) // we don't use 'switch (expr->variantT())' here to avoid
                     // enumerating all variantT values for binary operations
      {
        ...
        // DQ (2/9/2011): This is now 3 with the addition of the original
        // expression tree. ROSE_ASSERT(synthesizedAttributeList.size() == 2);
        // ROSE_ASSERT(synthesizedAttributeList.size() == 3);

        // step 1: replace children with their synthesized attribute values
        //
        // Process any SubTreeSynthesizedAttributes so any values there can be
        // reused (propagated up in the AST) If we are not testing against
        // values from the constant expression trees saved by ROSE in the AST,
        // then this is where we would expect to find the values propagated up
        ...
        if (lhsSynthesizedValue != NULL) {
          replaceExpressionWithDeferredDelete(binaryOperator->get_lhs_operand(),
                                              lhsSynthesizedValue);
        }
        if (rhsSynthesizedValue != NULL) {
          replaceExpressionWithDeferredDelete(binaryOperator->get_rhs_operand(),
                                              rhsSynthesizedValue);
        }
        ...
        // Constant folding makes sense on unary operators, but this case is not
        // implemented yet!
        ...
        if (value != NULL) {
          ...
          default: {
            // There are a lot of expressions where constant folding does not
            // apply
          }
          }
        }
        ...
        ROSE_ASSERT(synthesizedAttributeList.size() ==
                    exprList->get_expressions().size());
        for (size_t i = 0; i < exprList->get_expressions().size(); ++i) {
          SgExpression *synthesizedValue =
              synthesizedAttributeList[i].newValueExp;
          if (synthesizedValue != NULL) {
            replaceExpressionWithDeferredDelete(exprList->get_expressions()[i],
                                                synthesizedValue);
          }
        }
      } else if (isSgConditionalExp(expr)) // a ? b: c
      {
        ...
        if (end_exp_value != NULL)
          replaceExpressionWithDeferredDelete(r_exp->get_end(), end_exp_value);
      ...
      //
      //  ignore expressions without any constant valued children expressions,
      //  they are not supposed to be handled here
      else if (!isSgValueExp(expr) && !isSgVarRefExp(expr) &&
               !isSgFunctionRefExp(expr) &&
               // DQ (6/14/2015): Added case to ignore.  SgNullExpression does
               // not make since in the context of constant folding.
               !isSgNullExpression(expr) &&
               // DQ (6/14/2015): Added case to ignore.  SgSizeOfOp is not clear
               // if it should be constant folded, since it might be dependent
               // on something that might be transformed.
               !isSgSizeOfOp(expr) && !isSgFunctionCallExp(expr)) {
        if (SgProject::get_verbose() > 0)
          cout << "constant folding: unhandled expression type:"
...
ConstantUnFoldingTraversal::evaluateSynthesizedAttribute(
  ...
  // This code will expand out a constant into a constant expression (this is
  // done to test the constant folding)
  SgValueExp *valueExp = isSgValueExp(astNode);
  if (valueExp != NULL) {
    SgType *type = valueExp->get_type();
    switch (type->variantT()) {
    case V_SgTypeInt: {
      // Since a number of value expressions could evaluate to int we don't want
      // to assume that this is always a SgIntVal, so we have to test.
      ...
      if (intValueExp != NULL) {
        ...
        printf("Found an integer type value expression which was not a "
               "SgIntVal: valueExp = %p = %s \n",
    ...
    default: {
      // There are a lot of expressions where constant folding does not get
      // tested using this test
    }
    }
  }

  // This will turn binary operators containing value expressions into binary
  // operators with binary operators as subexpressions.
  SgBinaryOp *binaryOperator = isSgBinaryOp(astNode);
  if (binaryOperator != NULL) {
    // Process any SubTreeSynthesizedAttributes so any values there can be
    // reused (propagated up in the AST) If we are not testing against values
    // from the constant expression trees saved by ROSE in the AST, then this is
    ...
    // traversal
    if (lhsSynthesizedValue != NULL)
      SageInterface::replaceExpression(binaryOperator->get_lhs_operand(),
                                       lhsSynthesizedValue);
    if (rhsSynthesizedValue != NULL)
      SageInterface::replaceExpression(binaryOperator->get_rhs_operand(),
                                       rhsSynthesizedValue);
  }
