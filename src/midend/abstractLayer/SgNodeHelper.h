...
//! returns true if the declaration has an initializer list (e.g.
//! char[2]={'a',0};
bool isAggregateDeclarationWithInitializerList(SgVariableDeclaration *decl);

//! returns the initializer expression of a variable declaration. If no
//! initializer exists it returns 0.
SgExpression *
getInitializerExpressionOfVariableDeclaration(SgVariableDeclaration *decl);

//! returns the initialized name object of a variable declaration. Otherwise it
//! throws an exception.
SgInitializedName *
...
//! returns the function declaration statement found for a given function
//! symbol.
SgFunctionDeclaration *findFunctionDeclarationWithFunctionSymbol(SgNode *node);

//! returns filename+line+column information of AST fragment in format
//! "filename:line:column". Used for generating readable output
std::string sourceFilenameLineColumnToString(SgNode *node);

//! returns a std::pair of line and column number. If no file info exists at
//! this node it returns  (-1,-1).
...
//! returns filename followed by line:column in one string. Used for generating
//! readable output.
std::string sourceLineColumnToString(SgNode *node);
//! returns filename followed by line, separator, and column in one string. Used
//! for generating readable output.
...
std::string abbreviatedLocationToString(SgNode *node,
                                        size_t maxFileNameLength = 30);

//! returns location (consisting of filename, line, column) and source code of
//! unparsed node in one string. Abbreviates fileName and unparsed source if too
//! long (defaults are 30,20)
std::string locationAndSourceCodeToString(SgNode *node,
                                          size_t maxFileNameLength = 30,
                                          size_t maxSourceLength = 20);

//! determines all VarRefExp in the subtree of 'node'. The order in the vector
//! corresponds to the traversal order on the AST.
std::vector<SgVarRefExp *> determineVariablesInSubtree(SgNode *node);

/*! computes a list representing the nesting structure of classes (including
  structs and unions). It traverses the AST upwards and collects
  SgClassDeclaration(s) only. This covers nested classes, nested structs, and
  nested unions, and combinations of those.
*/
std::list<SgClassDeclaration *>
classDeclarationNestingSequence(SgDeclarationStatement *);

/*! computes for a given node at which scope nesting level this node is in its
 * AST */
...
//! determines whether a node is the root node of an AST representing the
//! inc-expr in a SgForStatement. This function is helpful to deal with this
//! special case in the ROSE AST where an expression does not have a root node
//! which can be easily determined to be a root node of an expression (i.e. here
//! it can be *any* binary or unary node in constrast to all other expressions
//! in the ROSE AST which are either a SgExprStatement or have a
//! SgExpressionRoot node.
bool isForIncExpr(SgNode *node);

//! returns the root node representing the AST of the condition of If, While,
//! DoWhile, For, CondOperator, switch.
...
//! returns the root node representing the AST of the true branch of If,
//! CondOperator.
SgNode *getTrueBranch(SgNode *node);

//! returns the root node representing the AST of the false branch of If,
//! CondOperator.
SgNode *getFalseBranch(SgNode *node);

//! returns the root node representing the AST of the loop body of While,
//! DoWhile, For.
SgNode *getLoopBody(SgNode *node);

//! returns the first Statement of SgBasicBlock (throws exception if
//! numChildren==0)
SgNode *getFirstOfBlock(SgNode *node);

//! returns the last Statement of SgBasicBlock (throws exception if
//! numChildren==0)
...
//! returns function name of SgFunctionDefinition, SgFunctionDeclaration,
//! SgFunctionCall.
...
/*! this function should only be called for a node in the subtree of
   ...
   SgFunctionDefinition.  Returns 0 if no SgFunctionDefinition is found (e.g.
   global scope).
*/
SgFunctionDefinition *correspondingSgFunctionDefinition(SgNode *node);

//! checks whether the node 'node' is the root node of the AST by using the
//! get_parent function.
bool isAstRoot(SgNode *node);

//! is true if 'node' is the root node of the AST representing a Loop construct
//! (While, DoWhile, For).
bool isLoopStmt(SgNode *node);

//! is true if 'node' is the root node of the AST representing the condition of
//! a Loop construct (While, DoWhile, For).
bool isLoopCond(SgNode *node);

//! is true if 'node' is the root node of the AST representing If, While,
//! DoWhile, For, switch, CondExp.
...
//! is true if 'node' is the root node of the AST representing the condition of
//! If, While, DoWhile, For, switch, CondExp.
bool isCond(SgNode *node);

//! sets 'cond' as the root node of the AST representing the condition in
//! statements if, while, dowhile, for, switch.
...
/*! \brief Creates a long unique variable name for a given node of type
   SgVariableDeclaration or SgVarRefExp
   ...
   In case of global scope functionName is empty, giving a string:
   $$scopeLevel$varName Note: this function only considers C-functions. Classes
   are recognized.
*/
...
/*! \brief returns a set of SgNode where each node is a break node, but
   ...
   @param [in] node can point directly to the AST construct (e.g. SgIfStatement)
   or a basic block of the respective loop construct.
...
//! returns the first child of an arbitrary AST node (throws exception if
//! numChildren==0)
...
//! returns the child of SgExprStatement (which is guaranteed to be unique and
//! to exist)
SgNode *getExprStmtChild(SgNode *node);

//! returns the child of SgExpressionRoot (which is guaranteed to be unique and
//! to exist)
SgNode *getExprRootChild(SgNode *node);

//! returns the child of a SgUnaryExp (which is guaranteed to be unique and to
//! exist)
SgNode *getUnaryOpChild(SgNode *node);

/*! returns the number of children as int (intentionally not as t_size)
   ensures that the number of children fits into an int, otherwise throws
   exception.
*/
int numChildren(SgNode *node);

/*! computes a new string from s1 where each doublequote is replaced with a
 * backslash followed by the doublequote. This is helpful when printing unparsed
 * program fragments which contain doublequoted strings to a dot file (used by
 * nodeToString). This function also replaces <,<=,>=,> with the corresponding
 * HTML codes.
 */
std::string doubleQuotedEscapedString(std::string s1);

/*!
  Same as doubleQuotedEscapedString but also replaces <,<=,>=,> with the
  corresponding HTML codes. This is required when printing program code inside
  HTML tables of a dot file.
*/
...
//! checks whether a SgVariableSymbol is representing a function parameter (this
//! does not apply for forward declarations)
...
//! return lhs of a binary node (if it is not a binary node it throws an
//! exception)
SgNode *getLhs(SgNode *node);

//! return rhs of a binary node (if it is not a binary node it throws an
//! exception)
SgNode *getRhs(SgNode *node);

/*! returns the parent of a node. Essentially a wrapper function of the ROSE
   get_parent() function, but throws an exception if no parent exists. For
   SgProject node  no exception is thrown if no parent exists because it is the
   root node of a ROSE AST.
*/
...
/*!
  checks whether the expression 'node' represents an assignment to an array's
  element considers all assignment operators and arrays of any size
*/
...
//
// Returns the underlying (without typedefs and mofifiers) rvalue OR lvalue
...
isTypeEligibleForFunctionToPointerConversion(const SgType *type);
...
// returns the list of initializers of an array or struct (e.g. for int
// a[]={1,2,3} it return the list 1,2,3)
std::vector<SgExpression *> &
getInitializerListOfAggregateDeclaration(SgVariableDeclaration *decl);
...
//! checks whether 'elem' is the last child (in traversal order) of node
//! 'parent'.
bool isLastChildOf(SgNode *elem, SgNode *parent);

/*! Returns for a given class/struct/union a list with the variable declarations
  of the member variables. Note this is a filtered list returned by the
  SgType::returnDataMemberPointers function which also returns pointers to
  methods
 */
...
//! Provides functions which match a certain AST pattern and return a pointer to
//! a node of interest inside that pattern.
namespace Pattern {
//! tests several patterns and returns pointer to FunctionCallExp inside that
//! matched pattern, otherwise 0.
SgFunctionCallExp *matchFunctionCall(SgNode *);
//! tests pattern SgReturnStmt(FunctionCallExp) and returns pointer to
//! FunctionCallExp, otherwise 0.
SgFunctionCallExp *matchReturnStmtFunctionCallExp(SgNode *);

//! tests pattern SgExprStatement(FunctionCallExp) and returns pointer to
//! FunctionCallExp, otherwise 0.
SgFunctionCallExp *matchExprStmtFunctionCallExp(SgNode *);

//! tests pattern SgExprStatement(SgAssignOp(VarRefExp,FunctionCallExp)) and
//! returns pointer to FunctionCallExp otherwise 0.
SgFunctionCallExp *matchExprStmtAssignOpVarRefExpFunctionCallExp(SgNode *);

//! tests pattern for function call in variable declaration and returns pointer
//! to FunctionCallExp otherwise 0.
SgFunctionCallExp *matchFunctionCallExpInVariableDeclaration(SgNode *node);

//! checks variable declaration with function call, returns variable
//! declaration. Otherwise 0. e.g. int x=f();
SgVariableDeclaration *matchVariableDeclarationWithFunctionCall(SgNode *node);
//! checks variable declaration with function call, returns both in a pair, or a
//! with (0,0).
...
matchExprStmtAssignOpVarRefExpFunctionCallExp2(SgNode *node);

//! tests pattern for an assert
bool matchAssertExpr(SgNode *node);

//! tests pattern SgFunctionCall(...) where the name of the function is scanf
//! with 2 params
SgVarRefExp *matchSingleVarScanf(SgNode *node);
//! tests pattern SgFunctionCall(...) where the name of the function is printf
//! with 2 params
SgVarRefExp *matchSingleVarPrintf(SgNode *node);
//! tests pattern SgFunctionCall(...) where the name of the function is fprintf
//! with 3 params
...
/// Result structure of extended C++ function call matching @ref
/// matchExtendedNormalizedCall
/// \details
///   extended matches function, constructor (on stack and w/new), and
///   destructor calls.
struct ExtendedCallInfo {
  /// no-match constructor
  ExtendedCallInfo() : rep(NULL) {}

  /// @param callnode Node containing some function call.
  ExtendedCallInfo(SgLocatedNode &callnode) : rep(&callnode) {}
...
ExtendedCallInfo matchExtendedNormalizedCall(SgNode *,
