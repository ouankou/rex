...
SgVariableSymbol *
SgNodeHelper::isFunctionParameterVariableSymbol(SgNode *node) {
  if (node) {
    if (SgVariableSymbol *varsym = isSgVariableSymbol(node))
      if (SgInitializedName *initname = varsym->get_declaration())
        if (SgDeclarationStatement *declstmt = initname->get_declaration())
          if (SgFunctionParameterList *fpl =
                  isSgFunctionParameterList(declstmt))
            if (fpl &&
                !SgNodeHelper::isForwardFunctionDeclaration(fpl->get_parent()))
              return varsym;
  }
  return 0;
}

bool SgNodeHelper::isVariableSymbolInFunctionForwardDeclaration(
    SgNode *varsym) {
  if (!(varsym && isSgVariableSymbol(varsym)))
    return false;
  if (SgInitializedName *initname =
          isSgVariableSymbol(varsym)->get_declaration())
    if (SgDeclarationStatement *declstmt = initname->get_declaration())
...
SgNodeHelper::findVariableDeclarationWithVariableSymbol(SgNode *node) {
  if (SgVariableSymbol *varsym = isSgVariableSymbol(node)) {
    // if(SgNodeHelper::isVariableSymbolInFunctionForwardDeclaration(node)) {
    //  in this case no Ast node representing a declaration exists in the ROSE
    //  AST.
    //  return 0;
    //}
    if (SgInitializedName *initname = varsym->get_declaration()) {
    ...
    throw std::runtime_error(
        "SgNodeHelper::getSgVariableDeclarationOfSgVariableSymbol : parameter "
        "not a SgVariableSymbol");
  }
  return 0; // non-reachable
}

SgFunctionDeclaration *
SgNodeHelper::findFunctionDeclarationWithFunctionSymbol(SgNode *node) {
  if (SgFunctionSymbol *funcsym = isSgFunctionSymbol(node)) {
    ...
    throw std::runtime_error(
        "SgNodeHelper::getSgFunctionDeclarationOfSgFunctionSymbol : parameter "
        "not a SgFunctionSymbol");
  }
}
...
std::string
SgNodeHelper::abbreviatedLocationToString(SgNode *node,
...
std::string SgNodeHelper::locationAndSourceCodeToString(
    SgNode *node, size_t maxFileNameLength, size_t maxSourceLength) {
  string location =
      SgNodeHelper::abbreviatedLocationToString(node, maxFileNameLength);
  // truncate unparsed source
  string sourceLine = node->unparseToString();
  if (sourceLine.size() > maxSourceLength) {
    sourceLine = sourceLine.substr(0, maxSourceLength) + "... (" +
                 std::to_string(sourceLine.size()) + " chars)";
  }
...
std::string SgNodeHelper::sourceLocationAndNodeToString(SgNode *node) {
  return SgNodeHelper::locationAndSourceCodeToString(
      node); // uses default values for maxFileLength and maxSourceLength
}
...
std::string SgNodeHelper::lineColumnAndSourceCodeToString(SgNode *node) {
  return SgNodeHelper::sourceLineColumnToString(node) + ": " +
         SgNodeHelper::nodeToString(node);
}
...
bool SgNodeHelper::Pattern::OutputTarget::isKnown() {
  return outType != SgNodeHelper::Pattern::OutputTarget::UNKNOWNOPERATION &&
         outType != SgNodeHelper::Pattern::OutputTarget::UNKNOWNPRINTF;
}
...
SgVarRefExp *SgNodeHelper::Pattern::matchSingleVarScanf(SgNode *node) {
  ...
  if (SgFunctionCallExp *funCall =
          SgNodeHelper::Pattern::matchFunctionCall(nextNodeToAnalyze1)) {
    string fName = SgNodeHelper::getFunctionName(funCall);
    SgExpressionPtrList &actualParams =
        SgNodeHelper::getFunctionCallActualParameterList(funCall);
    if (fName == "scanf") {
      if (actualParams.size() == 2) {
        ...
        SgVarRefExp *varRefExp =
            isSgVarRefExp(SgNodeHelper::getFirstChild(addressOp));
        if (!varRefExp) {
...
SgNodeHelper::Pattern::OutputTarget
SgNodeHelper::Pattern::matchSingleVarOrValuePrintf(SgNode *node) {
  ...
  if (SgFunctionCallExp *funCall =
    ...
    if (fName == "fprintf") {
      if (actualParams.size() == 3) {
        ...
        if (!varRefExp) {
          if (showWarnings) {
            cerr << "WARNING: unsupported fprint argument #3 (no variable "
                    "found). Required form of fprintf(stream,\"...%d...\",v)."
                 << endl;
          }
          return 0;
        }
        ...
        if (showWarnings) {
          cerr << "WARNING: unsupported number of fprintf arguments. Required "
                  "form of fprintf(stream,\"...%d...\",v)."
               << endl;
        }
        return 0;
      }
...
SgExpression *SgNodeHelper::getInitializerExpressionOfVariableDeclaration(
    SgVariableDeclaration *decl) {
  SgInitializedName *initName =
      SgNodeHelper::getInitializedNameOfVariableDeclaration(decl);
  SgInitializer *initializer = initName->get_initializer();
  // check if it is a declaration without initializer
  if (!initializer)
    return 0;
  if (SgAssignInitializer *assignInitializer =
          isSgAssignInitializer(initializer)) {
    SgExpression *expr = assignInitializer->get_operand_i();
    return expr;
  } else if (SgAggregateInitializer *aggregateInit =
...
SgInitializedName *SgNodeHelper::getInitializedNameOfVariableDeclaration(
    SgVariableDeclaration *decl) {
  ROSE_ASSERT(decl);
...
list<SgGlobal *> SgNodeHelper::listOfSgGlobal(SgProject *project) {
  ...
  for (int i = 0; i < numFiles; ++i) {
    ...
    if (SgSourceFile *sourceFile = isSgSourceFile(file)) {
      ...
      throw std::runtime_error(
          "Error: Ast structure failure: file is not a source file.");
    }
  }
  return globalList;
}
...
list<SgVariableDeclaration *>
SgNodeHelper::listOfGlobalVars(SgProject *project) {
  list<SgVariableDeclaration *> globalVarDeclList;
  list<SgGlobal *> globalList = SgNodeHelper::listOfSgGlobal(project);
  for (list<SgGlobal *>::iterator i = globalList.begin(); i != globalList.end();
       ++i) {
    list<SgVariableDeclaration *> varDeclList =
        SgNodeHelper::listOfGlobalVars(*i);
    globalVarDeclList.splice(
        globalVarDeclList.end(),
        varDeclList); // we are *moving* objects (not copying)
  }
...
list<SgVariableDeclaration *>
SgNodeHelper::listOfGlobalFields(SgProject *project) {
  list<SgVariableDeclaration *> globalFieldDeclList;
  list<SgGlobal *> globalList = SgNodeHelper::listOfSgGlobal(project);
  for (list<SgGlobal *>::iterator i = globalList.begin(); i != globalList.end();
       ++i) {
    list<SgVariableDeclaration *> varDeclList =
        SgNodeHelper::listOfGlobalFields(*i);
    globalFieldDeclList.splice(
        globalFieldDeclList.end(),
        varDeclList); // we are *moving* objects (not copying)
  }
...
list<SgFunctionDefinition *>
SgNodeHelper::listOfFunctionDefinitions(SgNode *node) {
...
SgNodeHelper::listOfFunctionDeclarations(SgNode *node) {
...
list<SgVarRefExp *>
SgNodeHelper::listOfUsedVarsInFunctions(SgProject *project) {
  list<SgVarRefExp *> varRefExpList;
  list<SgFunctionDefinition *> funDefList =
      SgNodeHelper::listOfFunctionDefinitions(project);
  for (list<SgFunctionDefinition *>::iterator i = funDefList.begin();
       i != funDefList.end(); ++i) {
    RoseAst ast(*i);
...
list<SgVariableDeclaration *> SgNodeHelper::listOfGlobalVars(SgGlobal *global) {
  ...
  for (SgDeclarationStatementPtrList::iterator i = declStmtList.begin();
       i != declStmtList.end(); ++i) {
...
#if __cplusplus > 199711L
list<SgVariableDeclaration *>
SgNodeHelper::listOfGlobalFields(SgGlobal *global) {
...
SgFunctionSymbol *
SgNodeHelper::getSymbolOfFunctionDeclaration(SgFunctionDeclaration *decl) {
...
/*!
  * \brief Returns a unique UniqueVariableSymbol (SgSymbol*) for a variale in a
  variable declaration (can be used as ID)
...
SgFunctionSymbol *
SgNodeHelper::getSymbolOfFunction(SgFunctionRefExp *funcRefExp) {
...
SgNodeHelper::getSymbolOfInitializedName(SgInitializedName *initName) {
  // if(initName->get_prev_decl_item()==0 &&
  // initName->get_symbol_from_symbol_table()==0) { cerr<<"WARNING:
  // SgInitializedName: symbol-look-up would fail:
  // get_name()=="<<initName->get_name()<< " .. skipping."<<endl; return 0;
  // }
  // schroder3 (2016-07-22): If the init name is in a constructor initializer
  // list then is does not have a valid symbol.
  if (SgCtorInitializerList *ctorInitializerList =
          isSgCtorInitializerList(initName->get_parent())) {
    ROSE_ASSERT(initName->search_for_symbol_from_symbol_table() == 0);
    //  Find the declaration of the corresponding member and use the
    //  declaration's init name instead:
    SgMemberFunctionDeclaration *parentConstructorDecl =
        isSgMemberFunctionDeclaration(ctorInitializerList->get_parent());
    ROSE_ASSERT(parentConstructorDecl);
    SgClassDefinition *correspondingClass =
        isSgClassDefinition(parentConstructorDecl->get_class_scope());
    ...
    if (isSgConstructorInitializer(initName->get_initializer())) {
      // Init name "is" a call of a (base) class constructor. Finding the
      ...
      SgDeclarationStatementPtrList &members =
          correspondingClass->get_members();
      bool declFound = false;
      for (SgDeclarationStatementPtrList::const_iterator i = members.begin();
           i != members.end(); ++i) {
        if (SgVariableDeclaration *varDecl = isSgVariableDeclaration(*i)) {
          SgInitializedName *currMemberInitName =
              getInitializedNameOfVariableDeclaration(varDecl);
          ROSE_ASSERT(currMemberInitName);
          if (currMemberInitName->get_name() == initName->get_name()) {
            // Found the corresponding member declaration. Use the member
            // declaration's init name to get
            //  the symbol:
      ...
      if (!declFound) {
        throw std::runtime_error(
            "Error: Unable to find declaration of member \"" +
...
list<SgClassDeclaration *>
SgNodeHelper::classDeclarationNestingSequence(SgDeclarationStatement *node) {
...
// MS: TODO: this implementation is complicated and needs to be structured
// better
string SgNodeHelper::uniqueLongVariableName(SgNode *node) {
  if (!(isSgVarRefExp(node) || isSgVariableDeclaration(node) ||
        isSgVariableSymbol(node))) {
    string s = "Error: uniqueVariableName: unsupported node type: " +
               node->class_name();
  ...
  if (SgVariableSymbol *varsym =
          SgNodeHelper::isFunctionParameterVariableSymbol(node)) {
    ...
    if (SgVariableSymbol *varsym = isSgVariableSymbol(node)) {
      ...
      // schroder3 (2016-08-23): Commented out assertion because lambda closure
      // variables do
    ...
    if (SgVarRefExp *varRef = isSgVarRefExp(node)) {
      SgVariableSymbol *varsym =
          isSgVariableSymbol(SgNodeHelper::getSymbolOfVariable(varRef));
      ROSE_ASSERT(varsym);
    ...
    if (SgVariableDeclaration *varDecl = isSgVariableDeclaration(node)) {
      ...
      sym = isSgVariableSymbol(
          SgNodeHelper::getSymbolOfVariableDeclaration(varDecl));
      ROSE_ASSERT(sym);
      found = true;
      ...
      // class nesting name
      classnestingname = "[";
      list<SgClassDeclaration *> typenestingsequence =
          SgNodeHelper::classDeclarationNestingSequence(varDecl);
      for (list<SgClassDeclaration *>::iterator i = typenestingsequence.begin();
           i != typenestingsequence.end(); ++i) {
  ...
  if (found) {
    ...
    // NOTE: in case of a function parameter varDecl is represented by the
    // function declaration

    // we search from the SgSymbol (which is somewhere found in the AST). Even
    // if it is in the symbol table we will still find the right function!
    SgFunctionDefinition *funDef =
        SgNodeHelper::correspondingSgFunctionDefinition(sym);
    string funName; // intentionally empty when we are in global scope and no
                    // funDef is found.
    if (funDef)
    ...
    string longName = string("$") + filename + string("$") + funName + "$" +
                      scopeLevel + "/" + scopesequencenumber + "$" +
                      classnestingname + "$" + name;
    return longName;
  } else {
    throw std::runtime_error(
        "SgNodeHelper::uniqueVariableName: improper node operation (" +
        node->class_name());
  }
}

SgFunctionDefinition *
SgNodeHelper::correspondingSgFunctionDefinition(SgNode *node) {
  if (node == 0)
    return 0;
...
int SgNodeHelper::scopeSequenceNumber(SgNode *node) {
  /* this is quite an expensive function (searches the AST upwards and within
     one container). However, the purpose is to provide a meaningful (=human
     readable) scope-sequence number. Alternatively it could also be provided
     for each scope as precomputed value attached as ast-attribute. 1) find
     scope we are in 2) determine scope number from parent container scopes are
     numbered 0 .. n returns the scope-number of the provided node's scope
     within the outer scope returns -1 if the provided node has no outer scope
  */
  size_t cnum = -1;
  // ad1)
  ...
  throw std::runtime_error(
      "SgNodeHelper::scopeSequenceNumber: improper node operation.");
}
...
SgFunctionDefinition *
SgNodeHelper::determineFunctionDefinition(SgFunctionCallExp *funCall) {
  if (SgFunctionDeclaration *funDecl =
          funCall->getAssociatedFunctionDeclaration()) {
    if (SgDeclarationStatement *defFunDecl =
            funDecl->get_definingDeclaration()) {
      if (SgFunctionDeclaration *funDecl2 =
              isSgFunctionDeclaration(defFunDecl)) {
...
string SgNodeHelper::getLabelName(SgNode *node) {
  if (!isSgLabelStatement(node))
    throw std::runtime_error(
        "SgNodeHelper::getLabelName: improper node operation.");
...
SgExpressionPtrList &
SgNodeHelper::getFunctionCallActualParameterList(SgNode *node) {
  if (!isSgFunctionCallExp(node))
    throw std::runtime_error("SgNodeHelper::getFunctionCallActualParameterList:"
                             " improper node operation.");
  return isSgExprListExp(node->get_traversalSuccessorByIndex(1))
      ->get_expressions();
}
...
// schroder3 (2016-06-24): Returns the function type of the callee of the given
// call expression
SgFunctionType *
SgNodeHelper::getCalleeFunctionType(/*const*/ SgFunctionCallExp *call) {
...
SgNodeHelper::getFunctionDefinitionFormalParameterList(SgNode *node) {
  SgFunctionDefinition *funDef = isSgFunctionDefinition(node);
  if (!funDef)
    throw std::runtime_error(
        "SgNodeHelper::getFunctionDefinitionFormalParameterList: improper node "
        "operation.");
...
SgType *SgNodeHelper::getFunctionReturnType(SgNode *node) {
  ...
  if (!funDef)
    throw std::runtime_error(
        "SgNodeHelper::getFunctionReturnType: improper node operation.");
  SgFunctionDeclaration *funDecl = funDef->get_declaration();
  return funDecl->get_orig_return_type();
}

set<SgVariableDeclaration *> SgNodeHelper::localVariableDeclarationsOfFunction(
    SgFunctionDefinition *funDef) {
...
//! schroder3 (2016-07-22): Returns the closest function definition that
...
// schroder3 (2016-07-22): Modified version of SageInterface::isPointerType(...)
// that
...
// schroder3 (2016-08-17): Added support for rvalue reference types. See
// isLvalueReferenceType(...)
//  if only lvalue references are desired.
// schroder3 (2016-07-22): Modified version of
// SageInterface::isReferenceType(...) that
//  returns the underlying reference type.
//
// Returns the underlying (without typedefs and modifiers) rvalue OR lvalue
...
const SgType *SgNodeHelper::isReferenceType(const SgType *t) {
  if (isSgReferenceType(t) /* <- tests for lvalue reference */ ||
      isSgRvalueReferenceType(t)) {
    return t;
  } else if (isSgTypedefType(t)) {
...
// schroder3 (2016-08-22): Modified version of
// SageInterface::isReferenceType(...) that
...
const SgReferenceType *SgNodeHelper::isLvalueReferenceType(const SgType *t) {
  if (const SgType *underlyingRvalueOrLvalueRef = isReferenceType(t)) {
    return isSgReferenceType(
        underlyingRvalueOrLvalueRef); // isSgReferenceType tests for lvalue
                                      // reference
  } else {
    return 0;
  }
...
// schroder3 (2016-08-22): Returns the underlying RVALUE reference type if the
// given type is a
//  rvalue reference type. Returns 0 otherwise.
const SgRvalueReferenceType *
SgNodeHelper::isRvalueReferenceType(const SgType *t) {
  if (const SgType *underlyingRvalueOrLvalueRef = isReferenceType(t)) {
...
// schroder3 (2016-08-22): Wrapper around SgReferenceType::get_base_type(...)
// and
//  SgRvalueReferenceType::get_base_type(...) that works for both reference
//  types. (This is a workaround for the missing mutual base class of
//  SgReferenceType and SgRvalueReferenceType.)
SgType *SgNodeHelper::getReferenceBaseType(const SgType *t) {
  ...
  } else if (const SgRvalueReferenceType *rvalueReferenceType =
                 isRvalueReferenceType(t)) {
    return rvalueReferenceType->get_base_type();
  } else {
    throw std::runtime_error(
        "SgNodeHelper::getReferenceBaseType(...): Parameter is not a"
...
// schroder3 (2016-07-26): Returns the (underlying) function type of the given
// type if the given
//  type is eligible for function-to-pointer conversion. Returns 0 otherwise.
const SgFunctionType *
SgNodeHelper::isTypeEligibleForFunctionToPointerConversion(const SgType *type) {
  // Only lvalues of function (reference) type are eligible for
  // function-to-pointer
  //  conversion.
...
// schroder3 (2016-07-27): Returns the underlying function type of the given
// expression if it
//  is callable. Returns 0 otherwise.
//
//  A special handling of expressions of class type where the corresponding
//  class has a call
//   operator ("operator()(...)") is not necessary because ROSE normalizes calls
//   like "A a; a(1);" to "A a; a.operator()(1);". This also applies to calls of
//   lambdas.
SgFunctionType *
SgNodeHelper::isCallableExpression(/*const*/ SgExpression *expr) {
  return isCallableType(expr->get_type());
}

// schroder3 (2016-07-27): Returns the underlying function type if the given
// type
//  is callable i.e. a expression of this type could be called. Returns 0
//  otherwise.
...
SgFunctionCallExp *SgNodeHelper::Pattern::matchFunctionCall(SgNode *node) {
  if (SgFunctionCallExp *fce =
          SgNodeHelper::Pattern::matchReturnStmtFunctionCallExp(node))
    return fce;
  if (SgFunctionCallExp *fce =
          SgNodeHelper::Pattern::matchExprStmtFunctionCallExp(node))
    return fce;
  if (SgFunctionCallExp *fce =
...
SgFunctionCallExp *
SgNodeHelper::Pattern::matchExprStmtFunctionCallExp(SgNode *node) {
  if (SgNode *sexp = isSgExprStatement(node))
    if (SgFunctionCallExp *fcp =
            isSgFunctionCallExp(SgNodeHelper::getExprStmtChild(sexp)))
      return fcp;
  return 0;
}

SgFunctionCallExp *
SgNodeHelper::Pattern::matchReturnStmtFunctionCallExp(SgNode *node) {
  if (SgNode *rexp = isSgReturnStmt(node))
    if (SgFunctionCallExp *fcp =
            isSgFunctionCallExp(SgNodeHelper::getFirstChild(rexp)))
      return fcp;
  return 0;
}
...
SgNodeHelper::Pattern::matchExprStmtAssignOpVarRefExpFunctionCallExp(
    SgNode *node) {
  std::pair<SgVarRefExp *, SgFunctionCallExp *> p =
      SgNodeHelper::Pattern::matchExprStmtAssignOpVarRefExpFunctionCallExp2(
          node);
  return p.second;
}

std::pair<SgVarRefExp *, SgFunctionCallExp *>
SgNodeHelper::Pattern::matchExprStmtAssignOpVarRefExpFunctionCallExp2(
    SgNode *node) {
  if (SgNode *sexp = isSgExprStatement(node)) {
...
SgNodeHelper::Pattern::matchVariableDeclarationWithFunctionCall(SgNode *node) {
  std::pair<SgVariableDeclaration *, SgFunctionCallExp *> pair =
      SgNodeHelper::Pattern::matchVariableDeclarationWithFunctionCall2(node);
  return pair.first;
}

SgFunctionCallExp *
SgNodeHelper::Pattern::matchFunctionCallExpInVariableDeclaration(SgNode *node) {
  std::pair<SgVariableDeclaration *, SgFunctionCallExp *> pair =
      SgNodeHelper::Pattern::matchVariableDeclarationWithFunctionCall2(node);
  return pair.second;
}
...
SgNodeHelper::Pattern::matchVariableDeclarationWithFunctionCall2(SgNode *node) {
  if (SgVariableDeclaration *varDecl = isSgVariableDeclaration(node)) {
    SgExpression *initializer =
        SgNodeHelper::getInitializerExpressionOfVariableDeclaration(varDecl);
...
set<SgNode *> SgNodeHelper::loopRelevantBreakStmtNodes(SgNode *node) {
  ...
  while (i != ast.end()) {
    if (isSgBreakStmt(*i))
      breakNodes.insert(*i);
    if (isSgForStatement(*i) || isSgWhileStmt(*i) || isSgDoWhileStmt(*i) ||
        isSgSwitchStatement(*i))
...
set<SgContinueStmt *>
SgNodeHelper::loopRelevantContinueStmtNodes(SgNode *node) {
...
set<SgCaseOptionStmt *>
SgNodeHelper::switchRelevantCaseStmtNodes(SgNode *node) {
...
bool SgNodeHelper::isAstRoot(SgNode *node) { return !node->get_parent(); }
...
SgNode *SgNodeHelper::getParent(SgNode *node) {
  ...
  if (node == 0 && !isSgProject(origNode)) {
    throw std::runtime_error(
        "SgNodeHelper::getParent: improper node operation (@" +
        origNode->class_name() + ")");
  }
  return node;
}
...
bool SgNodeHelper::isCondStmt(SgNode *node) {
  return isSgIfStmt(node) || isSgWhileStmt(node) || isSgDoWhileStmt(node) ||
         isSgForStatement(node) || isSgSwitchStatement(node);
}
...
SgStatementPtrList &SgNodeHelper::getForInitList(SgNode *node) {
  ...
  // SgForInitStatement
  throw std::runtime_error(
      "SgNodeHelper::getForInitList: improper node operation.");
}
...
SgExpression *SgNodeHelper::getForIncExpr(SgNode *node) {
  ...
  // SgForInitStatement
  throw std::runtime_error(
      "SgNodeHelper::getForIncExpr: improper node operation.");
}
...
void SgNodeHelper::setCond(SgStatement *stmt, SgNode *cond) {
  if (SgStatement *stmtCond = isSgStatement(cond)) {
    if (SgIfStmt *ifstmt = isSgIfStmt(stmt)) {
      ...
      throw std::runtime_error("SgNodeHelper::setCond: improper node operation "
                               "(unknown branching construct).");
    }
  } else {
...
string SgNodeHelper::unparseCond(SgNode *cond) {
  if (SgNodeHelper::isCond(cond)) {
    ...
    throw std::runtime_error(
        "SgNodeHelper::unparseCond: improper node operation.");
  }
}
...
SgNode *SgNodeHelper::getFalseBranch(SgNode *node) {
  ...
  throw std::runtime_error(
      "SgNodeHelper::getFalseBranch: improper node operation.");
}
...
SgNode *SgNodeHelper::getLoopBody(SgNode *node) {
  ...
  throw std::runtime_error(
      "SgNodeHelper::getLoopBody: improper node operation.");
}
...
SgNode *SgNodeHelper::getFirstOfBlock(SgNode *node) {
  ...
  // MS: note, the child could be 0 as well. Therefore we do not return 0, but
  // throw an runtime_error.
  throw std::runtime_error(
      "SgNodeHelper::getFirstBlock: improper node operation.");
}
...
SgNode *SgNodeHelper::getLastOfBlock(SgNode *node) {
  ...
  // throw an runtime_error.
  throw std::runtime_error(
      "SgNodeHelper::getLastOfBlock: improper node operation.");
}
...
string SgNodeHelper::doubleQuotedEscapedHTMLString(string s1) {
  ...
  // not clear yet how to get these special characters to display in labels
  // inside tables in dot.
...
string SgNodeHelper::getFunctionName(SgNode *node) {
  ...
  throw std::runtime_error(
      "SgNodeHelper::getFunctionName: improper node operation.");
}

SgNode *SgNodeHelper::getExprStmtChild(SgNode *node) {
  if (!isSgExprStatement(node)) {
    throw std::runtime_error("SgNodeHelper::getExprStmtChild: improper type (" +
                             node->class_name() + ")");
  }
...
SgNode *SgNodeHelper::getExprRootChild(SgNode *node) {
  if (!isSgExpressionRoot(node)) {
    throw std::runtime_error("SgNodeHelper::getExprRootChild: improper type (" +
                             node->class_name() + ")");
  }
  return SgNodeHelper::getFirstChild(node);
}
...
SgNode *SgNodeHelper::getUnaryOpChild(SgNode *node) {
  if (!dynamic_cast<SgUnaryOp *>(node)) {
    throw std::runtime_error("Error: improper type in getUnaryOpChild (" +
                             node->class_name() + ")");
  }
  return SgNodeHelper::getFirstChild(node);
}
...
SgNode *SgNodeHelper::getFirstChild(SgNode *node) {
  ...
  if (len > 0)
    ...
    // throw an runtime_error.
    throw std::runtime_error(
        "SgNodeHelper::getFirstChild: improper node operation.");
  }
}
...
int SgNodeHelper::numChildren(SgNode *node) {
  if (node) {
    ...
    if (len <= (size_t)INT_MAX)
      ...
      throw std::runtime_error(
          "SgNodeHelper::numChildren: number of children beyond max int.");
  } else {
    return 0; // if node==0 we return 0 as number of children
  }
}

void SgNodeHelper::replaceExpression(SgExpression *e1, SgExpression *e2,
                                     bool mode) {
  ...
  if (pe && mode == false) {
    ...
    SageInterface::replaceExpression(
        e1, e2, mode); // this function is more general but very slow
  }
}
...
bool SgNodeHelper::hasAssignInitializer(SgVariableDeclaration *decl) {
  ROSE_ASSERT(decl);
  SgNode *initName0 =
      decl->get_traversalSuccessorByIndex(1); // get-InitializedName
...
bool SgNodeHelper::isAggregateDeclarationWithInitializerList(
    SgVariableDeclaration *decl) {
  SgNode *initName0 =
      decl->get_traversalSuccessorByIndex(1); // get-InitializedName
  ROSE_ASSERT(initName0);
...
SgExpressionPtrList &SgNodeHelper::getInitializerListOfAggregateDeclaration(
    SgVariableDeclaration *decl) {
  ...
  if (SgInitializedName *initName = isSgInitializedName(initName0)) {
    // array initializer
    SgInitializer *initializer = initName->get_initializer();
    if (SgAggregateInitializer *arrayInit =
            isSgAggregateInitializer(initializer)) {
  ...
  throw std::runtime_error(
      "SgNodeHelper::getInitializerListOfAggregateDeclaration: "
      "getInitializerListOfArrayVariable failed.");
}

SgNodeHelper::PragmaList SgNodeHelper::collectPragmaLines(string pragmaName,
                                                          SgNode *root) {
  PragmaList l;
  RoseAst ast(root);
  for (RoseAst::iterator i = ast.begin(); i != ast.end(); ++i) {
    ...
    if (i != ast.end()) {
      for (std::list<SgPragmaDeclaration *>::iterator p = pragmaNodes.begin();
           p != pragmaNodes.end(); ++p) {
        ...
        if (SgNodeHelper::isPrefix(pragmaName, str)) {
          ...
          if (!(isLastChildOf(lastPragmaDecl, lastPragmaDecl->get_parent()))) {
            if (SgStatement *assocStmt = isSgStatement(*i)) {
              ...
              // cout<<"PRAGMA REVERSE: "<<str<<" :
              // "<<(assocStmt)->unparseToString()<<endl;
              l.push_back(make_pair(str, assocStmt));
            } else {
              throw std::runtime_error(
                  "SgNodeHelper::collectPragmaLines " +
                  SgNodeHelper::sourceLineColumnToString(*p) +
                  ": pragma not associated with a method or statement.");
            }
          } else {
            throw std::runtime_error(
                "SgNodeHelper::collectPragmaLines " +
                SgNodeHelper::sourceLineColumnToString(*p) +
                ": pragma at end of block. This is not allowed.");
          }
        }
      }
    }
...
std::string
SgNodeHelper::getPragmaDeclarationString(SgPragmaDeclaration *pragmaDecl) {
  SgPragma *pragma = pragmaDecl->get_pragma();
  ROSE_ASSERT(pragma);
  return pragma->get_pragma();
}

void SgNodeHelper::replaceString(std::string &str, const std::string &from,
                                 const std::string &to) {
  if (from.empty())
    return;
  size_t start_pos = 0;
...
bool SgNodeHelper::isPrefix(const std::string &prefix, const std::string &s) {
  return std::equal(prefix.begin(), prefix.begin() + prefix.size(), s.begin());
}
...
SgNodeHelper::OmpSectionList
SgNodeHelper::getOmpSectionList(SgOmpSectionsStatement *sectionsStmt) {
...
template <typename N> bool node_can_be_changed(N *node);

template <> bool node_can_be_changed<SgLocatedNode>(SgLocatedNode *lnode) {
...
template <> bool node_can_be_changed<SgStatement>(SgStatement *stmt) {
...
bool node_can_be_changed<SgFunctionDeclaration>(SgFunctionDeclaration *fdecl) {
  ...
  return fname.find("__builtin_") != 0 &&
         SgNodeHelper::node_can_be_changed<SgDeclarationStatement>(fdecl);
}
...
struct FunctionPointer : sg::DispatchHandler<SgExpression *> {
  ...
  static SgExpression *eval(SgExpression *ex);
};
...
SgNodeHelper::ExtendedCallInfo::ctorInitializer() const {
  return isSgConstructorInitializer(rep);
}

SgExpression *SgNodeHelper::ExtendedCallInfo::functionPointer() const {
...
namespace {
SgInitializedName &onlyDecl(const SgVariableDeclaration &n) {
...
struct ExtendedCallMatcherInner
    : sg::DispatchHandler<SgNodeHelper::ExtendedCallInfo> {
  void handle(SgNode &n) { SG_UNEXPECTED_NODE(n); }
  ...
  static ReturnType eval(SgNode *n);

  static ReturnType eval_nullcheck(SgNode *n);
};

ExtendedCallMatcherInner::ReturnType ExtendedCallMatcherInner::eval(SgNode *n) {
...
struct ExtendedCallMatcherOuter
    : sg::DispatchHandler<SgNodeHelper::ExtendedCallInfo> {
  void handle(SgNode &n) { SG_UNEXPECTED_NODE(n); }
  ...
  static ReturnType eval(SgExpression *n);

  static ReturnType eval_nullcheck(SgExpression *n);
};
...
SgNodeHelper::matchExtendedNormalizedCall(SgNode *n, bool matchExtended) {
  ...
  if (matchExtended) {
    SgNodeHelper::ExtendedCallInfo res =
        sg::dispatch(ExtendedCallMatcherOuter(), n);
...
SgNodeHelper::memberVariableDeclarationsList(SgClassType *sgType) {
  std::list<SgVariableDeclaration *> declVarList;
  typedef std::vector<std::pair<SgNode *, std::string>> DataMemberPointers;
  DataMemberPointers dataMemPtrs =
      isSgClassType(sgType)->returnDataMemberPointers();
  // returnDataMemberPointers includes all declarations (methods need to be
  // filtered)
  for (DataMemberPointers::iterator i = dataMemPtrs.begin();
       i != dataMemPtrs.end(); ++i) {
    SgNode *node = (*i).first;
    if (SgVariableDeclaration *varDecl = isSgVariableDeclaration(node)) {
