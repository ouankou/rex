...
public:
  ...
  virtual void Visit(SymbolicVisitor *op) const {
    return op->VisitUnknown(*this);
  }
...
public:
  SymbolicVar(std::string name, const AstNodePtr &sc,
              const AstNodePtr exp = AST_NULL)
      : varname(name), scope(sc), exp_(exp) {}
  SymbolicVar() : varname(""), scope(AST_NULL), exp_(AST_NULL) {}
  SymbolicVar(const SymbolicVar &that)
      : varname(that.varname), scope(that.scope), exp_(that.exp_) {}
  ~SymbolicVar() {}
  virtual std::string toString() const;
...
public:
  ...
  bool operator==(const SymbolicAstWrap &that) const { return ast == that.ast; }
...
public:
  ...
  SymbolicVal &operator=(const SymbolicVal &that) {
    CountRefHandle<SymbolicValImpl>::operator=(that);
    return *this;
  }
  virtual ~SymbolicVal() {}
  ...
  bool IsSame(const SymbolicValImpl &impl) const { return ConstPtr() == &impl; }
  ...
  bool isAstWrap(AstNodePtr &r) const {
    if (ConstPtr() != 0 && ConstRef().GetValType() == VAL_AST) {
      const SymbolicAstWrap &c =
          static_cast<const SymbolicAstWrap &>(ConstRef());
      r = c.get_ast();
      return true;
    }
    return false;
...
public:
  ...
  SymbolicFunction(AstInterface::OperatorEnum _t, const std::string &_op,
      ...
      : op(new SymbolicConst(_op, "function")), t(_t) {
    args.push_back(v1);
    args.push_back(v2);
  }
  SymbolicFunction(AstInterface::OperatorEnum _t, const std::string &_op,
...
public:
  ...
  SymbolicPow(const SymbolicPow &that) : SymbolicFunction(that) {}
...
public:
  ...
  static bool IsFortranLoop(AstInterface &, const AstNodePtr &s,
                            SymbolicVar *ivar = 0, SymbolicVal *lb = 0,
...
bool HasFraction(const SymbolicVal &v, SymbolicVal *integral = 0,
                 SymbolicVal *frac = 0);
...
SymbolicBound &operator|=(SymbolicBound &b1, const SymbolicBound &b2);
SymbolicBound GetValBound(SymbolicVal val,
                          MapObject<SymbolicVal, SymbolicBound> &f);
SymbolicVal GetValLB(const SymbolicVal &val,
                     MapObject<SymbolicVal, SymbolicBound> &f);
SymbolicVal GetValUB(const SymbolicVal &val,
                     MapObject<SymbolicVal, SymbolicBound> &f);
...
SymbolicVal Min(const SymbolicVal &v1, const SymbolicVal &v2,
                MapObject<SymbolicVal, SymbolicBound> *f = 0);
SymbolicVal
UnwrapVarCond(const SymbolicCond &valCond, const SymbolicVar &pivot,
              SymbolicBound &pivotBound); // return pivot coefficient

bool FindVal(const SymbolicVal &v, const SymbolicVal &sub);
SymbolicVal ReplaceVal(const SymbolicVal &v, const SymbolicVal &sub,
                       const SymbolicVal &newval);
SymbolicVal ReplaceVal(const SymbolicVal &v,
