...
class UnwrapCond : public SymbolicVisitor {
  ...
  void VisitExpr(const SymbolicExpr &v) {
    ...
    while (
        !iter.ReachEnd()) { // find the first term using pivot variable like 2*i
    ...
    if (iter.ReachEnd())
      return;
    SymbolicTerm p =
        iter.Current(); // look ahead for next term without using pivot
    switch (v.GetOpType()) {
    case SYMOP_MULTIPLY: // expression is multiplication of terms
      ...
      if (!iter.ReachEnd())
        return; // still found later terms using pivot? return
      left = 1; // Now we know the begin and end offset of all terms using
                // privot, multiply them together.
...
public:
  ...
  VarRestr
  operator()(const SymbolicCond &cond) // normalize condition to be
                                       // Left_is_func(x) Relation const_exp
  {
    ...
    if (!b1 && !b2)
      ...
      SymbolicVal tmp = left; // need to create a temporary because left may be
                              // re-assigned, causing memory error
      tmp.Visit(this);
      if (!succ)
        rel = REL_UNKNOWN;
    }
...
// normalize and extract condition expression: left-Relation-right,  right side
// does not contain pivot variable Depending on the relation type, save result
// into a bound
SymbolicVal UnwrapVarCond(const SymbolicCond &cond, const SymbolicVar &pivot,
                          SymbolicBound &pivotBound) {
  VarRestr res = UnwrapCond(pivot)(cond);
  SymbolicVal coeff = res.coeff;
  switch (res.rel) {
  case REL_EQ:
    pivotBound.Intersect(SymbolicBound(res.result, res.result));
    break;
  case REL_LT:
    pivotBound.Intersect(SymbolicBound(SymbolicVal(), res.result - 1));
    break;
  case REL_GT:
    pivotBound.Intersect(SymbolicBound(res.result + 1, SymbolicVal()));
    break;
  case REL_LE:
    pivotBound.Intersect(SymbolicBound(SymbolicVal(), res.result));
    break;
  case REL_GE:
    pivotBound.Intersect(SymbolicBound(res.result, SymbolicVal()));
    break;
  case REL_NONE:
    return 0;
  default:
