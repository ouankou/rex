...
public:
  ...
  bool IsTop(const SymbolicTerm &v) { return op.IsTop(v); }
...
SymbolicTerm &SymbolicExpr ::FirstOpd() const { return ConstRef().First(); }
...
bool SymbolicExpr::operator==(const SymbolicExpr &that) const {
  if (GetOpType() == that.GetOpType() && NumOfOpds() == that.NumOfOpds()) {
...
SymbolicVal GetExprVal(SymbolicExpr *r) {
  ...
  if (num == 1) {
    ...
    case SYMOP_MULTIPLY:
      return 1;
    default:
      std::cerr << "non-recognized expression type: " << r->toString()
                << std::endl;
      ROSE_ABORT();
    }
  }
...
public:
  ...
  SymbolicVal ApplyOP(const SymbolicVal &v) {
    v2 = v;
    return OPHelpVisitor::ApplyOP(v);
  }
};
...
class DistributeVisitor : private OPHelpVisitor {
  ...
  void Default() { result = TermVisitor(v2, op).ApplyOP(that); }
  void VisitConst(const SymbolicConst &v) {
    int valu, vald;
    if (v.GetIntVal(valu, vald))
      result = IntVisitor(v2, valu, vald, op).ApplyOP(that);
...
public:
  DistributeVisitor(const SymbolicVal &v, const SymbolicExpr &_exp,
                    OPApplicator &_op)
      : OPHelpVisitor(_op, v), exp(_exp) {}
  SymbolicVal ApplyOP(const SymbolicVal &_v2) {
    v2 = _v2;
    return OPHelpVisitor::ApplyOP(_v2);
  }
};
...
class OPVisitor : public SymbolicVisitor {
  ...
  OPApplicator &op;
  void Default() { result = TermVisitor(v1, op).ApplyOP(v2); }
  void VisitConst(const SymbolicConst &v) {
    int valu, vald;
    if (v.GetIntVal(valu, vald)) {
...
SymbolicVal ApplyBinOP(OPApplicator &op, const SymbolicVal &v1,
