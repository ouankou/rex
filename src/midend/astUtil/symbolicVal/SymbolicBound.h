...
public:
  SingleValBound(const SymbolicVal &v, const SymbolicVal &lb,
...
class MapVarBound : public MapObject<SymbolicVal, SymbolicBound>,
  ...
  void VisitVar(const SymbolicVar &var) {
    MapBound::const_iterator p =
        bmap.find(Key(var.GetVarName(), var.GetVarScope()));
    if (p != bmap.end())
      result = (*p).second;
  }
...
public:
  ...
  VarInfo(const SymbolicVar &_var, const SymbolicVal &_lb,
          const SymbolicVal &_ub)
      : var(_var), b(_lb, _ub) {}
  VarInfo(const SymbolicVar &_var, const SymbolicBound &_b)
      : var(_var), b(_b) {}
  std::string toString() const { return var.toString() + " : " + b.toString(); }
  SymbolicBound operator()(const SymbolicVal &v) {
...
class SymbolicBoundAnalysis : public MapObject<SymbolicVal, SymbolicBound>,
                              private SymbolicVisitor {
protected:
  Interface iface;
  SymbolicBound result;
  Stmt node, ances;
  void VisitVar(const SymbolicVar &var) { result = GetBound(var); }
...
template <class Stmt, class Interface>
class SymbolicConstBoundAnalysis
    : public SymbolicBoundAnalysis<Stmt, Interface> {
protected:
...
private:
  void VisitVar(const SymbolicVar &var) { result = GetConstBound(var); }
...
public:
  ...
  SymbolicBound GetConstBound(const SymbolicVar &var) {
    Stmt n;
    SymbolicBound tmp =
        SymbolicBoundAnalysis<Stmt, Interface>::GetBound(var, &n);
    if (n != ances) {
      SymbolicBoundAnalysis<Stmt, Interface> next(iface, iface.GetParent(n),
                                                  ances);
      if (tmp.lb.IsNIL())
        tmp.lb = var;
      if (tmp.ub.IsNIL())
