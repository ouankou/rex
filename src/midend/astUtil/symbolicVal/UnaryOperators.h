...
public:
  ...
  std::string toString() const override {
    ...
    for (OpdIterator iter = GetOpdIterator(); !iter.ReachEnd();
         iter.Advance()) {
  ...
  virtual AstNodePtr CodeGenUnaryOP(AstInterface &fa,
                                    const AstNodePtr &operand) const = 0;

  AstNodePtr CodeGenOP(AstInterface & /*fa*/, const AstNodePtr & /*a1*/,
                       const AstNodePtr & /*a2*/) const override {
    ROSE_ABORT();
  }

  SymbolicExpr *DistributeExpr(SymOpType /*op*/,
                               const SymbolicVal & /*that*/) const override {
    return CloneExpr();
  }
};
...
public:
  ...
  AstNodePtr CodeGenUnaryOP(AstInterface &fa,
                            const AstNodePtr &operand) const override {
...
public:
  bool MergeConstInt(int vu1, int vd1, int /*vu2*/, int /*vd2*/, int &r1,
                     int &r2) override final {
    return MergeConstIntUnary(vu1, vd1, r1, r2);
  }

  bool MergeElem(const SymbolicTerm &t1, const SymbolicTerm & /*t2*/,
                 SymbolicTerm &result) override final {
    return MergeElemUnary(t1, result);
  }

  virtual bool MergeConstIntUnary(int /*vu*/, int /*vd*/, int & /*r1*/,
                                  int & /*r2*/) = 0;
  virtual bool MergeElemUnary(const SymbolicTerm & /*t1*/,
                              SymbolicTerm & /*result*/) = 0;
};
...
public:
  ...
  bool IsTop(const SymbolicTerm &t) override { return t.IsTop(); }
