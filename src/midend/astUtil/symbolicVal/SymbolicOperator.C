...
SymbolicVal ApplyBinOP(SymOpType t, const SymbolicVal &v1,
  ...
  case SYMOP_PLUS: {
    ...
    if (DebugOp())
      std::cerr << v1.toString() << " + " << v2.toString() << " = "
                << r.toString() << std::endl;
    return r;
  }
  case SYMOP_MULTIPLY: {
  ...
  case SYMOP_MIN:
    r = Min(v1, v2);
    if (DebugOp())
      std::cerr << "Min( " << v1.toString() << " , " << v2.toString()
                << ") = " << r.toString() << std::endl;
    return r;
  case SYMOP_MAX:
    return Max(v1, v2);
    r = Max(v1, v2);
    if (DebugOp())
      std::cerr << "Max( " << v1.toString() << " , " << v2.toString()
                << ") = " << r.toString() << std::endl;
    return r;
  case SYMOP_POW: {
    int val2;
    ...
    if (DebugOp())
      std::cerr << "Pow( " << v1.toString() << " , " << v2.toString()
                << ") = " << r.toString() << std::endl;
    return r;
  }
  case SYMOP_AND: {
  ...
  // DQ (12/11/2016): It appears that printf() is not declared here on some
  // versions of Linux (even RH).
  std::cerr << "Exiting at a previously undefined function return location. "
            << std::endl;
  ROSE_ABORT();
}
...
int CountGT(CompareRel r) { return (r == REL_GT) ? 1 : 0; }
int CountGE(CompareRel r) {
  return (r == REL_GE || r == REL_GT || r == REL_EQ) ? 1 : 0;
}
int CountLT(CompareRel r) { return (r == REL_LT) ? 1 : 0; }
int CountLE(CompareRel r) {
  return (r == REL_LE || r == REL_LT || r == REL_EQ) ? 1 : 0;
}
int CountEQ(CompareRel r) { return (r == REL_EQ) ? 1 : 0; }
...
class ExprEQ : public EQOperator {
  ...
  void VisitExpr(const SymbolicExpr &v) { result = (exp == v); }
...
protected:
  ...
  void Default0(const SymbolicVal &v1, const SymbolicVal &v2) {
    ...
    if (result == REL_UNKNOWN) {
      ...
      if (diff.GetValType() == VAL_CONST) {
        ...
        comparetime = tmp;
        CompareRel ge1 =
            (b1.lb != v1) ? CompareValHelp(b1.lb, v2, func) : REL_UNKNOWN;
        CompareRel le2 =
            (b2.ub != v2) ? CompareValHelp(b2.ub, v1, func) : REL_UNKNOWN;
        CompareRel le1 =
            (b1.ub != v1) ? CompareValHelp(b1.ub, v2, func) : REL_UNKNOWN;
        CompareRel ge2 =
            (b2.lb != v2) ? CompareValHelp(b2.lb, v1, func) : REL_UNKNOWN;
  ...
  CompareOperator(MapObject<SymbolicVal, SymbolicBound> *_func = 0)
      : func(_func) {}
};
...
void ExprTermCompare(const SymbolicExpr &e1, const SymbolicVal &v2,
  ...
  int i = 0;
  for (SymbolicExpr::OpdIterator p1 = e1.GetOpdIterator(); !p1.ReachEnd();
       ++p1, ++i) {
...
void ExprTermCompare(const SymbolicExpr &e1, const SymbolicExpr &e2,
  ...
  for (SymbolicExpr::OpdIterator p1 = e1.GetOpdIterator(); !p1.ReachEnd();
    ...
    SymbolicVal v1 = e1.Term2Val(p1.Current());
    int j = 0;
    for (SymbolicExpr::OpdIterator p2 = e2.GetOpdIterator(); !p2.ReachEnd();
         ++p2, ++j) {
...
public:
  ValCompare(MapObject<SymbolicVal, SymbolicBound> *_func = 0)
      : CompareOperator(_func) {}
...
CompareRel CompareVal(const SymbolicVal &v1, const SymbolicVal &v2,
  ...
  if (v1.IsNIL() && v2.IsNIL())
    return REL_UNKNOWN;
  if (DebugCompareVal())
    std::cerr << "comparing " << v1.toString() << " with " << v2.toString()
              << " under " << f << std::endl;
  comparetime = 0;
...
CompareRel CompareValHelp(const SymbolicVal &v1, const SymbolicVal &v2,
  ...
  if (DebugCompareVal())
    std::cerr << v1.toString() << RelToString(r) << v2.toString() << " under "
              << f << std::endl;
  return r;
}
  ...
  virtual void VisitExpr(const SymbolicExpr &v) {
    ...
    if (opds.ReachEnd()) {
      ...
      if (inp == 0 && frp == 0)
        return;
      SymbolicExpr *inv =
          (inp == 0) ? 0 : v.DistributeExpr(SYMOP_NIL, SymbolicVal());
      SymbolicExpr *frv =
          (frp == 0) ? 0 : v.DistributeExpr(SYMOP_NIL, SymbolicVal());
...
// extract fraction from v, save integral/rac if the last two pointers are not
// NULL
