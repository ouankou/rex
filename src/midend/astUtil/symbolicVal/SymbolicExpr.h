...
public:
  ...
  bool IsConst() const { return v.IsNIL(); }
...
public:
  ...
  SymbolicTerm &First() const {
    assert(opds.First() != NULL);
    return opds.First()->GetEntry();
  }
  std::string toString() const;
...
public:
  ...
  virtual SymbolicExpr *DistributeExpr(SymOpType t,
                                       const SymbolicVal &that) const = 0;
  ...
  virtual AstNodePtr CodeGenOP(AstInterface &fa, const AstNodePtr &a1,
                               const AstNodePtr &a2) const = 0;
...
class ValTermVisitor : public SymbolicVisitor {
  ...
  void VisitFunction(const SymbolicFunction &v) {
    int val1, val2;
    if (v.GetOpType() == expOP && v.GetConstOpd(val1, val2)) {
...
public:
  ...
  virtual bool MergeConstInt(int vu1, int vd1, int vu2, int vd2, int &r1,
                             int &r2) = 0;
  virtual bool IsTop(const SymbolicTerm &v) { return v.IsTop(); }
...
inline SymbolicTerm SymbolicExpr::Val2Term(const SymbolicVal &v) const {
  return ValTermVisitor(GetTermOP())(v);
}

SymbolicVal ApplyBinOP(OPApplicator &op, const SymbolicVal &_v1,
                       const SymbolicVal &_v2);
SymbolicVal GetExprVal(SymbolicExpr *exp);
// delete exp if return value if different from exp
SymbolicVal ApplyUnaryOP(OPApplicator &op, const SymbolicVal &_v);
#endif
