...
public:
  AstNodePtr(BaseType *_repr = 0)
      : repr_(_repr), nodetype_(SpecialAstType::SG_AST) {
    if (_repr == 0)
      nodetype_ = SpecialAstType::NULL_AST;
  }
  AstNodePtr(const AstNodePtr &that)
      : repr_(that.repr_), nodetype_(that.nodetype_), sig_(that.sig_) {}
  AstNodePtr(const std::string &sig)
      : repr_(0), nodetype_(SpecialAstType::GLOBAL_SIGNATURE), sig_(sig) {}
  AstNodePtr(SpecialAstType t, BaseType *_repr = 0)
      : repr_(_repr), nodetype_(t) {
    switch (t) {
  ...
  bool is_unknown_reference() const {
    return nodetype_ == SpecialAstType::UNKNOWN_PTR_REF;
  }
  void set_is_unknown_function_call() {
    nodetype_ = SpecialAstType::UNKNOWN_FUNCTION_CALL;
  }
  void set_is_unknown_reference() {
    nodetype_ = SpecialAstType::UNKNOWN_PTR_REF;
  }
...
#define AST_UNKNOWN_CALL(x)                                                    \
  AstNodePtr(AstNodePtr::SpecialAstType::UNKNOWN_FUNCTION_CALL, x)
...
public:
  ...
  AstNodeType(BaseType *repr)
      : repr_(repr), nodetype_(SpecialAstType::SG_TYPE) {
    if (repr_ == 0)
  ...
  AstNodeType(const AstNodeType &that)
      : repr_(that.repr_), nodetype_(that.nodetype_) {}
...
class AstObserver {
public:
  virtual void ObserveCopyAst(AstInterfaceImpl &fa, const AstNodePtr &orig,
                              const AstNodePtr &n) = 0;
};
...
public:
  CopyAstRecord(AstInterfaceImpl &_fa, const AstNodePtr &o,
                const AstNodePtr &_n)
      : orig(o), n(_n), fa(_fa) {}
...
public:
  ...
  bool get_fileInfo(const AstNodePtr &n, std::string *fname = 0,
                    int *lineno = 0);
  ...
  bool IsVariableDecl(const AstNodePtr &exp, AstList *vars = 0,
                      AstList *inits = 0);
  //! Check if exp declares a set of variables to be aliased to non-local
  //! storages, represented by the returned global_signatures.
  ...
  static bool IsLoop(const AstNodePtr &s, AstNodePtr *init = 0,
                     AstNodePtr *cond = 0, AstNodePtr *incr = 0,
                     AstNodePtr *body = 0);
  bool IsPostTestLoop(const AstNodePtr &s);

  //! Check whether $s$ is a Fortran-style loop in the form: for
  //! (ivar=lb;ivar<=ub;ivar+=step)
  ...
  AstNodePtr CreateIf(const AstNodePtr &__cond, const AstNodePtr &__if_stmt,
                      const AstNodePtr &__else_stmt = AST_NULL) const;
  ...
  AstNodePtr CreateFunctionCall(const std::string &func,
                                AstList::const_iterator args_begin,
  ...
  static AstNodePtr
  GetFunctionDefinitionFromDeclaration(const AstNodePtr &decl);
  ...
  AstNodePtr CreateMethodRef(std::string classname, std::string fieldname,
  ...
  static bool IsMemoryAllocation(const AstNodePtr &s,
                                 ...
                                 AstNodePtr *init = 0);
  static bool IsMemoryFree(const AstNodePtr &s, AstNodeType *freedType = 0,
                           AstNodePtr *freedVariable = 0);
  static bool IsExpression(const AstNodePtr &s, AstNodeType *exptype = 0,
                           AstNodePtr *strip_exp = 0);
  ...
  bool IsConstant(const AstNodePtr &exp, std::string *valtype = 0,
                  std::string *value = 0);
  //! Create AST for constant values of  types int, bool, string, float, etc. as
  //! well as names of variable and function references. e.g:
  ...
  //! Check whether $exp$ is a variable reference; If yes, return type, name,
   ...
   * var declarations are removed from their original block*/
  ...
  static bool IsArrayType(const AstNodeType &t, int *dim = 0,
                          AstNodeType *base = 0, std::string *annotation = 0);
  ...
  AstNodePtr CreateAllocateArray(const AstNodePtr &arr,
                                 const AstNodeType &elemtype,
  ...
  AstNodePtr GetParentBlock(const AstNodePtr &__n) {
    if (__n == AST_NULL)
      return AST_NULL;
  ...
  AstNodePtr GetParentStatement(const AstNodePtr &__n) {
    if (__n == AST_NULL)
      return AST_NULL;
...
//! Interface class for processing each AstNode from within the ReadAstTraverse
//! function.
template <class AstNodePtr> class ProcessAstNode {
public:
...
//! Traverse an entire AST, where $op$ is invoked on each AST node to gather
//! information.
bool ReadAstTraverse(
    AstInterface &fa, const AstNodePtr &root, ProcessAstNode<AstNodePtr> &op,
    AstInterface::TraversalOrderType t = AstInterface::PreOrder);

//! Interface class for processing each AstNode from within the
//! TransformAstTraverse function.
...
//! Traverse and transform an entire AST, where $op$ is invoked to transform
//! each sub-Tree.
AstNodePtr TransformAstTraverse(
    AstInterface &fa, const AstNodePtr &r,
    bool (*op)(AstInterface &fa, const AstNodePtr &head, AstNodePtr &result),
    AstInterface::TraversalVisitType t = AstInterface::PreVisit);
AstNodePtr TransformAstTraverse(
    AstInterface &fa, const AstNodePtr &r, TransformAstTree &op,
    AstInterface::TraversalVisitType t = AstInterface::PreVisit);
