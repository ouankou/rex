...
template <class Descriptor> class CPPTypeCollection {
...
public:
  ...
  //! Check if an expression is a member access or member function call
  //! expression of a known array type
  // (Only check the array type, not the further data/function member name!!)
  // If yes, store the object of the member function, argument list , and the
  // collected descriptor for this class
  ...
  std::string is_known_member_function(CPPAstInterface &fa,
                                       ...
                                       Descriptor *desc = 0);
  SymbolicVal
  create_known_member_function(const AstNodePtr &obj,
                               const std::string &memname,
                               const SymbolicFunction::Arguments &args);
  AstNodePtr
  create_known_member_function(CPPAstInterface &fa, const AstNodePtr &obj,
                               const std::string &memname,
...
std::string CPPTypeCollection<Descriptor>::is_known_member_function(
    CPPAstInterface &fa, const SymbolicVal &exp, AstNodePtr *objp,
...
// Check if an expression is an member access/member function call of a known
// array type.
template <class Descriptor>
std::string CPPTypeCollection<Descriptor>::is_known_member_function(
    CPPAstInterface &fa, const AstNodePtr &exp, AstNodePtr *objp,
    AstInterface::AstNodeList *args, Descriptor *desc) {
  AstNodePtr obj;
  std::string fname;
...
template <class Descriptor>
SymbolicVal CPPTypeCollection<Descriptor>::create_known_member_function(
    const AstNodePtr &obj, const std::string &memname,
    const SymbolicFunction::Arguments &_args) {
  SymbolicDotExp func(new SymbolicAstWrap(obj),
                      new SymbolicConst(memname, "memberfunction"));
...
template <class Descriptor>
AstNodePtr CPPTypeCollection<Descriptor>::create_known_member_function(
    CPPAstInterface &fa, const AstNodePtr &obj, const std::string &memname,
    const AstInterface::AstNodeList &args) {
  SymbolicDotExp op(new SymbolicAstWrap(obj),
                    new SymbolicConst(memname, "memberfunction"));
