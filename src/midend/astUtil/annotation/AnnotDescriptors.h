...
public:
  //! Write the elements inside a container to string format, with delimitor
  //! 'seq'
...
public:
  ...
  CollectPair() {}
  CollectPair(const std::pair<First, Second> &that)
      : std::pair<First, Second>(that) {}
...
//! Similar to CollectPair, but the second item may start with 'sel' without
//! 'sep' in front of it.
...
public:
  ...
  SelectPair() {}
  SelectPair(const std::pair<First, Second> &that)
      : std::pair<First, Second>(that) {}
  bool read(std::istream &in);
  void write(std::ostream &out) const;
  void Dump() const { write(std::cerr); }
...
public:
  ...
  void push_back(const Member &n) { this->insert(n); }
  bool is_member(const Member &n) { return find(n) != end(); }
  ...
  bool read(std::istream &in) {
    return ReadContainer<SetDescriptor<Member, sep, left, right>, Member, sep,
                         left, right>::read(*this, in);
  }
};
...
public:
  ...
  bool read(std::istream &in) {
    return ReadContainer<MapDescriptor<Key, Member, sep, left, right>,
                         CollectPair<Key, Member, 0>, sep, left,
                         right>::read(*this, in);
  }

  void write(std::ostream &out) const {
    WriteContainer<MapDescriptor<Key, Member, sep, left, right>, sep, left,
                   right>::write(*this, out);
  }
  void Dump() const { write(std::cerr); }
  std::string ToString() const {
    std::stringstream out;
...
//! A descriptor with begin(left) and end(right) characters, such as {x}, (x),
//! etc
