...
bool OperatorInlineAnnotation::get_inline(AstInterface &fa, const AstNodePtr &h,
                                          SymbolicVal *val) {
  return known_operator(fa, h, val);
}

bool OperatorSideEffectAnnotation::get_modify(
...
bool OperatorSideEffectAnnotation::get_read(
    AstInterface &fa, const AstNodePtr &fc,
    CollectObject<AstNodePtr> *collect) {
  AstInterface::AstNodeList args;
...
bool OperatorAliasAnnotation::may_alias(
    AstInterface &fa, const AstNodePtr &fc, const AstNodePtr &result,
    CollectObject<pair<AstNodePtr, int>> &collectalias) {
  return AliasAnnotAnal(fa, aliasInfo, fc, result, collectalias);
}

bool OperatorAliasAnnotation::allow_alias(
    AstInterface &fa, const AstNodePtr &fc,
    CollectObject<pair<AstNodePtr, int>> &collectalias) {
...
// function will be available (required for g++ 4.0.2) This is reported as a
// duplicate template instantiation directive if the previousl on before the
// AnnotDescriptors.C is include is not commented out.
template class ReadContainer<ParameterDeclaration,
                             CollectPair<TypeDescriptor, NameDescriptor, 0>,
                             ',', '(', ')'>;
