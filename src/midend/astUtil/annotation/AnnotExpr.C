...
void SymbolicValDescriptor::replace_val(
...
AstNodePtr SymbolicExtendVar ::CodeGen(AstInterface &fa) const {
  string name;
  assert(first_arg().GetValType() == VAL_VAR &&
         last_arg().GetValType() == VAL_CONST);
  SymbolicVar var(first_arg().toString() + "_" + last_arg().toString(),
                  AST_NULL);
...
SymbolicVal ReadSymbolicTerm(istream &in) {
  ...
  if (c == '(') {
    ...
    if (id == "Min" || id == "min") {
      ...
      if (c != '(') {
        ...
        c = peek_ch(in);
        if (c != ')') {
          ReadContainerWrap<SymbolicValDescriptor, SymbolicFunction::Arguments>
              accu(args);
          read_list(in, accu, ',');
        }
        read_ch(in, ')');
  ...
  if (c == '(') {
    ...
    read_ch(in, ']');
    result =
        new SymbolicFunction(AstInterface::OP_ARRAY_ACCESS, "[]", result, r2);
  }
...
void ExtendibleParamDescriptor ::replace_var(const string &varname,
                                             const SymbolicVal &_val) {
  get_ext_lb().replace_var(varname, _val);
  get_ext_ub().replace_var(varname, _val);
}

void ExtendibleParamDescriptor ::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
...
void SymbolicParamListDescriptor::replace_var(const string &varname,
...
void SymbolicParamListDescriptor::Dump() const { write(cerr); }

void SymbolicFunctionDeclaration::replace_var(const string &varname,
                                              const SymbolicVal &repl) {
  first.replace_var(varname, repl);
  second.replace_var(varname, repl);
}

void SymbolicFunctionDeclaration::Dump() const { write(cerr); }

void SymbolicFunctionDeclaration::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
...
class ReplaceExtendibleParam : public SymbolicVisitor {
  ...
  void Default() { result.push_back(orig); }
  ...
  void VisitFunction(const SymbolicFunction &u) {
    ...
    if (buf == "$" && u.first_arg().toString() == basename) {
      ...
      for (int j = lb; j <= ub; ++j) {
        SymbolicVal tmp =
            ReplaceVal(cur, SymbolicVar(buf, AST_NULL), SymbolicConst(j));
        orig = tmp;
        orig.Visit(this);
      }
    } else {
...
public:
  ReplaceExtendibleParam(const string &bn, const string &en, int lb, int ub,
      ...
      : basename(bn), extname(en), extlb(lb), extub(ub), parstart(start),
        parList(l) {}
...
bool SymbolicFunctionDeclarationGroup::get_val(
    AstInterface &fa, const AstInterface::AstNodeList &argList,
    AstNodePtr &r) const {
...
void SymbolicFunctionDeclarationGroup::Dump() const { write(cerr); }

//! store the function name in r ??  Store the result in 'r' ?
bool SymbolicFunctionDeclaration::get_val(
    const std::vector<SymbolicVal> &parList, SymbolicVal &r) const {
  ...
  if ((size_t)j < parList.size()) {
    cerr << "Error: given more than needed arguments *********" << j << "<"
         << parList.size() << endl;
    ROSE_ABORT();
  }
  return true;
...
void SymbolicFunctionDeclarationGroup::replace_var(const string &varname,
                                                   const SymbolicVal &repl)
...
void SymbolicFunctionDeclarationGroup::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
  for (iterator p = begin(); p != end(); ++p) {
    SymbolicFunctionDeclaration &cur = *p;
...
void DefineVariableDescriptor::replace_var(const string &varname,
                                           const SymbolicVal &repl)
...
void DefineVariableDescriptor::replace_val(
    MapObject<SymbolicVal, SymbolicVal> &repl) {
  first.second.replace_val(repl);
  second.replace_val(repl);
}

void DefineVariableDescriptor::Dump() const { write(cerr); }
