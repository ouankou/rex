                ...
                Map2Object<AstInterface*,AstNodePtr,AstNodePtr>* codegen)
{
  ...
  for (AstInterface::AstNodeList::const_iterator p1 = args.begin();
    ...
    DebugOperatorDescriptor([&]() { return "Operator parameter " + curpar + "->" + curarg.toString(); });
  }
  if (decl.get_params().size() > args.size()) {
    std::cerr << "Error: mismatching numbers of parameters and arguments in annotation."
              << decl.get_params().size() << " vs " << args.size() << "\n";
...
SymbolicAstWrap ReplaceParams:: find( const string& varname)
{
  ...
  DebugOperatorDescriptor([&]() { return "Cannot find argument for parameter: " + varname + ". Returning empty!"; });
...
std::string OperatorDeclaration::operator_signature(AstInterface& fa, const AstNodePtr& exp,
                                 AstInterface::AstNodeList* argp,
                                 AstInterface::AstTypeList* paramp) {
    DebugOperatorDescriptor([&]() { return "Creating operator signature:" + AstInterface::AstToString(exp); });
    std::string fname;
    AstNodePtr f;
    AstNodeType t;
    AstTypeList params;
    if (paramp == 0) paramp = &params;
    if (AstInterface::IsVarRef(exp,&t,&fname, 0, 0, /*use_globl_name=*/true) && fa.IsFunctionType(t, paramp)) {
       if (argp != 0 && paramp->size() == 1) {
          argp->push_back(exp.get_ptr());
       }  
    }
    else if ((fa.IsFunctionCall(exp, &f, argp, 0, paramp) &&
              AstInterface::IsVarRef(f, 0, &fname, 0, 0, /*use_globl_name=*/true)) ||
             AstInterface::IsFunctionDefinition(exp, &fname, argp, 0, 0, paramp, 0,
                                                /*use_globl_name=*/true)) {
    } else {
      DebugOperatorDescriptor([&]() {
        return "Unexpected operator: not recognized:" + AstInterface::AstToString(exp) + ". Return empty name.";
      });
      return "";
    }
...
OperatorDeclaration:: OperatorDeclaration(AstInterface& fa, AstNodePtr op_ast,
    ...
    if (TypeDescriptor::get_name() == "") {
        DebugOperatorDescriptor([&]() {
          return "Error: Unknown/inconsistent operation: " + AstInterface::AstToString(op_ast) + ". Generating empty declaration.";
        });
        return;
    }
    ...
    while (p2 != params.end() && p1 != argp->end()) {
       DebugOperatorDescriptor([&]() { return "Adding operator parameter:" + AstInterface::AstToString(*p1); });
