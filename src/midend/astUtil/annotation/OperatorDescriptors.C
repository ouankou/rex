...
ReplaceParams::ReplaceParams(
    const ParamDescriptor &decl, AstInterface::AstNodeList &args,
    Map2Object<AstInterface *, AstNodePtr, AstNodePtr> *codegen) {
  assert(decl.size() == args.size());
  int index = 0;
  for (AstInterface::AstNodeList::iterator p1 = args.begin(); p1 != args.end();
       ++p1, ++index) {
...
string
OperatorDeclaration::get_signiture(AstInterface &fa, const std::string &fname,
...
bool OperatorSideEffectDescriptor::read(istream &in,
                                        const OperatorDeclaration &op) {
  param_num = 0;
...
void OperatorSideEffectDescriptor::get_side_effect(
    AstInterface &fa, AstInterface::AstNodeList &args,
    CollectObject<AstNodePtr> &collect) {
  ReplaceParams paramMap(get_param_decl().get_params(), args);
  for (OperatorSideEffectDescriptor::const_iterator p = begin(); p != end();
       ++p) {
    string varname = *p;
...
// DQ (1/8/2006): force instantiation of this template so that the "read" member
// function will be available (required for g++ 4.0.2) template class
// ReadContainer<ParameterDeclaration, CollectPair<TypeDescriptor,
// NameDescriptor,0>, ',', '(', ')'>;

// template bool ReadContainer<ParameterDeclaration, CollectPair<TypeDescriptor,
// NameDescriptor, (char)0>, (char)44, (char)40,
// (char)41>::read(ParameterDeclaration&, std::basic_istream<char,
