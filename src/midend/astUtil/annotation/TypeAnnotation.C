...
public:
  ...
  bool read(istream &in) {
    ...
    for (typename ReadAnnotCollection<TargetInfo, sep1, sep2, end>::iterator p =
             op.begin();
         p != op.end(); ++p) {
      cur = *p;
...
//! Read a collection of annotations of type TargeInfo (a typedescriptor or an
...
template <class Descriptor> void TypeCollection<Descriptor>::Dump() const {
  write(std::cerr);
}

template <class Descriptor>
Descriptor *TypeCollection<Descriptor>::known_type(const TypeDescriptor &name) {
...
template <class Descriptor>
Descriptor *TypeCollection<Descriptor>::known_type(AstInterface &fa,
...
template <class Descriptor>
void TypeAnnotCollection<Descriptor>::read_descriptor(
    const TypeDescriptor &name, const string & /*annotName*/, istream &in) {
  Descriptor d;
  d.read(in);
  add_annot(name, d);
}
