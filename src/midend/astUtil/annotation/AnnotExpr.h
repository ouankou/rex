...
public:
  ...
  SymbolicValDescriptor(const SymbolicVal &that, std::string varname) {
    set_val(that);
    varname_ = varname;
  }
  ...
  static SymbolicVal get_bottom() {
    return SymbolicValGenerator::get_unknown();
  }
...
public:
  ...
  SymbolicDotExp(const SymbolicVal &obj, const std::string &fieldname)
      : SymbolicFunction(AstInterface::BOP_DOT_ACCESS, ".", obj,
                         SymbolicConst(fieldname, "field")) {}
  SymbolicDotExp(const SymbolicDotExp &that) : SymbolicFunction(that) {}
...
public:
  ...
  SymbolicExtendVar(const std::string &varname, int index)
      : SymbolicFunction(AstInterface::OP_NONE, "$",
                         SymbolicVar(varname, AST_NULL), index) {}
  SymbolicExtendVar(const SymbolicExtendVar &that) : SymbolicFunction(that) {}

  virtual AstNodePtr CodeGen(AstInterface &fa) const;
  static std::string get_varname(std::string var, int index);
};

//!  a parameter with a named range of values.
//!  parameter:range_name:lower_bound:upper_bound
// e.g:  _size:dim:1:dimension
class ExtendibleParamDescriptor
    : private SelectPair<
          SymbolicValDescriptor, // parameter itself
          CollectPair<
              NameDescriptor, // a range's name followed by its lower and upper
                              // bound
              CollectPair<SymbolicValDescriptor, SymbolicValDescriptor, ':'>,
              ':'>,
          ':', 0> {
  typedef SelectPair<
  ...
  const SymbolicValDescriptor &get_ext_lb() const {
    return second.second.first;
  }
  SymbolicValDescriptor &get_ext_ub() { return second.second.second; }
  const SymbolicValDescriptor &get_ext_ub() const {
    return second.second.second;
  }

public:
  ExtendibleParamDescriptor() {}
  ExtendibleParamDescriptor(const SymbolicValDescriptor &p) { first = p; }
  ...
  std::string get_param_name() const {
    assert(first.get_val().GetValType() == VAL_VAR);
    return first.get_val().toString();
  }
...
// a parameter list 'SymbolicParamListDescriptor' and a name (??)
// 'SymbolicValDescriptor', separated by ':' in their string format Note the
// reversed order for function name and parameter list
class SymbolicFunctionDeclaration
    : private CollectPair<SymbolicParamListDescriptor, SymbolicValDescriptor,
                          ':'> {
  typedef CollectPair<SymbolicParamListDescriptor, SymbolicValDescriptor, ':'>
      BaseClass;
...
class DefineVariableDescriptor
    : public CollectPair<
          CollectPair<TypeDescriptor, ExtendibleParamDescriptor, 0>,
          SymbolicValDescriptor, '='> {
...
class ReadSymbolicFunctionDeclaration
    : public CollectPair<
          SelectPair<StringDescriptor, SymbolicParamListDescriptor, 0, '('>,
          SymbolicValDescriptor, '='> {
public:
