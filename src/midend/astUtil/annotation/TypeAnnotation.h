...
template <class TargetInfo> class AnnotCollectionBase {
public:
  //! Check if annotName matches the current annotation type name (TargetInfo)
  // The name is one of the following:
  //    array(is-array?), construct_array(new-array), modify_array,
  //    array_optimize, has_value, alias/allow_alias, inline, modify, read,
  //    restrict_value, ??(inheritable)
...
//! An interface to read and store a collection of annotations for a type or an
//! operator
// TargetInfo is result's type, targetsep is begin char, annotend is the end
// char descriptorsep is the delimitor
// e.g.
//         ReadAnnotCollection<TypeDescriptor,      '{', ';','}'>
//         ReadTypeCollection;
...
public:
  typedef
      typename std::vector<AnnotCollectionBase<TargetInfo> *>::const_iterator
          const_iterator;
  typedef typename std::vector<AnnotCollectionBase<TargetInfo> *>::iterator
      iterator;
...
//! A map storing types and their descriptors, the descriptor could be any
//! instantiated types as a template class.
// Descriptor could be either of has_value, array, array_opt annotation types
template <class Descriptor> class TypeCollection {
protected:
...
public:
  ...
  public:
    const_iterator(
        const typename std::map<std::string, Descriptor>::const_iterator &that)
        : std::map<std::string, Descriptor>::const_iterator(that) {}

    TypeDescriptor get_type() const {
      return std::map<std::string, Descriptor>::const_iterator::operator*()
          .first;
    }
  ...
  // Check if a named type 'name' is a type with annotation. Returns null if
  // false.
...
class TypeAnnotCollection
    : public AnnotCollectionBase<TypeDescriptor>, //== TypeCollectionBase
      public TypeCollection<Descriptor> { // Derived from TypeCollectionBase
