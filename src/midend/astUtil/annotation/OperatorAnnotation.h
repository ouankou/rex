...
class OperatorAnnotCollection : public AnnotCollectionBase<OperatorDeclaration>,
...
public:
  ...
  bool known_operator(
    ...
    if (original_descriptor == 0) {
      debugAnnot(
          [&op]() { return "Did not recognize operation:" + op.ToString(); });
      return false;
    }
    if (argp != 0)
      *argp = args;
...
public:
  static OperatorInlineAnnotation *get_inst() {
    if (inst == 0)
      inst = new OperatorInlineAnnotation();
    return inst;
  }
...
public:
  typedef OperatorAnnotCollection<OperatorSideEffectDescriptor>::const_iterator
      const_iterator;

  static OperatorSideEffectAnnotation *get_inst() {
    if (inst == 0)
      inst = new OperatorSideEffectAnnotation();
    return inst;
  }
  static void release_inst();
  void register_annot() {
  ...
  get_modify_descriptor(AstInterface &fa, const AstNodePtr &fc,
                        bool insert_if_missing = false) {
    return modInfo.get_annot_descriptor(OperatorDeclaration(fa, fc),
                                        insert_if_missing);
  }
  OperatorSideEffectDescriptor *
  get_modify_descriptor(const std::string &signature,
                        bool insert_if_missing = false) {
    return modInfo.get_annot_descriptor(OperatorDeclaration(signature),
                                        insert_if_missing);
  }
  bool get_modify(AstInterface &fa, const AstNodePtr &fc,
                  CollectObject<AstNodePtr> *collect = 0) override {
    return modInfo.CollectAnnotation(fa, fc, collect);
  }

  OperatorSideEffectDescriptor *
  get_read_descriptor(AstInterface &fa, const AstNodePtr &fc,
                      bool insert_if_missing = false) {
    return readInfo.get_annot_descriptor(OperatorDeclaration(fa, fc),
                                         insert_if_missing);
  }
  OperatorSideEffectDescriptor *
  get_read_descriptor(const std::string &signature,
                      bool insert_if_missing = false) {
    return readInfo.get_annot_descriptor(OperatorDeclaration(signature),
                                         insert_if_missing);
  }
  ...
  OperatorSideEffectDescriptor *
  get_call_descriptor(AstInterface &fa, const AstNodePtr &fc,
                      bool insert_if_missing = false) {
    return callInfo.get_annot_descriptor(OperatorDeclaration(fa, fc),
                                         insert_if_missing);
  }
  bool get_call(AstInterface &fa, const AstNodePtr &fc,
                CollectObject<AstNodePtr> *collect = 0) override {
    return callInfo.CollectAnnotation(fa, fc, collect);
  }

  OperatorSideEffectDescriptor *
  get_kill_descriptor(AstInterface &fa, const AstNodePtr &fc,
                      bool insert_if_missing = false) {
    return killInfo.get_annot_descriptor(OperatorDeclaration(fa, fc),
                                         insert_if_missing);
  }
  virtual bool get_kill(AstInterface &fa, const AstNodePtr &fc,
                        CollectObject<AstNodePtr> *collect = 0) {
    return killInfo.CollectAnnotation(fa, fc, collect);
  }

  void add_modify(const OperatorDeclaration &op,
                  OperatorSideEffectDescriptor &d) {
    modInfo.add_annot(op, d);
  }
  void add_read(const OperatorDeclaration &op,
                OperatorSideEffectDescriptor &d) {
    readInfo.add_annot(op, d);
  }
  void add_call(const OperatorDeclaration &op,
                OperatorSideEffectDescriptor &d) {
    callInfo.add_annot(op, d);
  }
  void add_kill(const OperatorDeclaration &op,
                OperatorSideEffectDescriptor &d) {
...
public:
  static OperatorAliasAnnotation *get_inst() {
    if (inst == 0)
      inst = new OperatorAliasAnnotation();
    return inst;
  }
  static void release_inst();
  //! Inherited from FunctionAliasInterface
  bool may_alias(AstInterface &fa, const AstNodePtr &fc,
...
//! Replace an operator with another equivalent operation, which is specified by
//! "inline" annotation
