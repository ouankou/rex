...
void StmtInfoCollect::AppendFuncCallArguments(AstInterface &fa,
  ...
  std::string fname;
  if (AstInterface::IsFunctionDefinition(fa.GetFunctionDefinition(p_callee),
                                         &fname, &params)) {
    DebugLocalInfoCollect(
        [&fname]() { return "static Function call to : " + fname; });
  }
  ...
  for (AstInterface::AstNodeList::const_iterator p1 = args.begin();
    ...
    DebugLocalInfoCollect([&c]() {
      return "Function Argument: " + AstInterface::AstToString(c);
    });
    operator()(fa, c);
    if (p2 != params.end()) {
...
bool StmtInfoCollect ::ProcessTree(AstInterface &fa,
                                   const AstInterface::AstNodePtr &s,
  ...
  if (t == AstInterface::PreVisit) {
    ...
    if (fa.IsFunctionDefinition(s, 0, &vars, &args, &body)) {
      ...
      for (AstInterface::AstNodePtr param : args) {
        if (fa.IsAssignment(param, &lhs, &rhs, /*read_lhs=*/&additional)) {
          ...
          AppendModLoc(fa, AstNodePtrImpl(lhs).get_ptr(),
                       AstNodePtrImpl(rhs).get_ptr());
        }
      }
      SkipUntil(body);
      return true;
      ...
      DebugLocalInfoCollect([&s]() {
        return "previsiting cur statement " + AstInterface::AstToString(s);
      });
      curstmt = AstNodePtrImpl(s).get_ptr();
    } else {
      DebugLocalInfoCollect([&s]() {
        return "previsiting cur node " + AstInterface::AstToString(s);
      });
      if (curstmt == 0) {
    ...
    if (fa.IsAssignment(s, &lhs, &rhs, /*read_lhs=*/&additional)) {
      ...
      DebugLocalInfoCollect([&s]() {
        return " append function call " + AstInterface::AstToString(s);
      });
    ...
    // to behave the same as a memory access, although it isn't one exactly
      ...
      if (!fa.IsSameVarRef(
              s,
              fa.GetParent(
                  s))) { /*QY: skip s if it refers to the same thing as parent*/
        ModMap *mp = modstack.size() ? &modstack.back().modmap : 0;
    ...
    DebugLocalInfoCollect([&s]() {
      return "postvisiting cur node " + AstInterface::AstToString(s);
    });
...
public:
  ...
  bool operator()(const AstInterface::AstNodePtr &ref) {
    ...
    if (fa.IsArrayAccess(ref, 0, &args)) {
      for (AstNodeList::const_iterator p = args.begin(); p != args.end(); ++p) {
        operator()(*p);
      }
    }
  ...
  bool operator()(const AstInterface::AstNodePtr &ref) {
    ...
    if (fa.IsFunctionCall(ref, &callee)) {
      ...
      DebugLocalInfoCollect([&ref]() {
        return "Error: Expecting a function call but getting:" +
...
public:
  CollectModRefWrap(AstInterface &_fa, FunctionSideEffectInterface *f,
                    const AstNodePtr &_stmt, CollectObject *read,
                    CollectObject *m)
...
void StmtSideEffectCollect::AppendAliasDecl(AstInterface & /* fa */,
...
void StmtSideEffectCollect::AppendVariableDecl(AstInterface & /* fa */,
                                               const AstNodePtr &variable,
                                               const AstNodePtr &var_init) {
  DebugLocalInfoCollect([&variable, &var_init]() {
    return "appending var decl " + AstInterface::AstToString(variable) + " = " +
...
void StmtSideEffectCollect::AppendModLoc(AstInterface &, const AstNodePtr &mod,
                                         const AstNodePtr &rhs) {
  DebugLocalInfoCollect([&mod, &rhs]() {
    return "appending modifying " + AstInterface::AstToString(mod) + " = " +
           AstInterface::AstToString(rhs);
  });
  if (curstmt == 0)
    return;
...
void StmtSideEffectCollect::AppendReadLoc(AstInterface & /*fa*/,
  ...
  DebugLocalInfoCollect([&read]() {
    return "appending reading " + AstInterface::AstToString(read);
  });
  if (readcollect != 0) {
    if (lhs == 0) {
...
void StmtSideEffectCollect::AppendFuncCall(AstInterface &fa,
                                           const AstNodePtr &fc) {
  CollectReadRefWrap read(fa, funcanal, curstmt, readcollect);
  DebugLocalInfoCollect([&fc]() {
    return "Checking function info: " + AstInterface::AstToString(fc);
  });
...
void StmtSideEffectCollect::AppendMemoryAllocate(AstInterface & /* fa */,
                                                 const AstNodePtr &s) {
  AstNodePtr init;
...
void StmtSideEffectCollect::AppendMemoryFree(AstInterface & /* fa */,
                                             const AstNodePtr &s) {
  if (free_collect != 0) {
...
public:
  ...
  bool operator()(const std::pair<AstNodePtr, int> &cur) {
    ...
    if (cur.first == 0 || !fa.IsVarRef(cur.first, 0, &varname, &scope, 0,
                                       /*use_global_unique_name=*/true))
      return false;
    if (cur.second == index) {
...
std::string Ast2StringMap ::get_string(const AstNodePtr &s) const {
  ...
  MapType::const_iterator p = astmap.find(s);
  if (p == astmap.end()) {
    std::cerr << "Error: cannot find ast: " << AstInterface::AstToString(s)
              << std::endl;
    for (p = astmap.begin(); p != astmap.end(); ++p) {
      std::cerr << " : " << AstInterface::AstToString((*p).first) << ":"
                << (*p).second << std::endl;
    }
    ROSE_ABORT();
  }
...
std::string
InterProcVariableUniqueRepr::get_unique_name(AstInterface &fa,
                                             const AstNodePtr &exp) {
  std::string expname = Ast2StringMap::inst()->get_string(exp);
  return get_unique_name(fa, exp, expname);
}

std::string InterProcVariableUniqueRepr::get_unique_name(
    AstInterface &fa, const AstNodePtr &_scope, const std::string &varname) {
  if (_scope == 0)
    return varname;
  std::string fname;
...
UF_elem *
StmtVarAliasCollect::VarAliasMap::get_alias_map(const std::string &varname,
...
void StmtVarAliasCollect::AppendVariableDecl(AstInterface &fa,
                                             const AstNodePtr &var,
                                             const AstNodePtr &init) {
  return AppendModLoc(fa, var, init);
}

void StmtVarAliasCollect::AppendAliasDecl(AstInterface &fa,
                                          const AstNodePtr &var,
                                          const AstNodePtr &init) {
  return AppendModLoc(fa, var, init);
}

void StmtVarAliasCollect::AppendModLoc(AstInterface &fa, const AstNodePtr &mod,
                                       const AstNodePtr &rhs) {
  DebugLog DebugAliasAnal("-debugaliasanal");
  std::string modname;
  AstNodeType modtype;
  AstNodePtr modscope;
  if (rhs == 0 || !fa.IsVarRef(mod, &modtype, &modname, &modscope) ||
      fa.IsScalarType(modtype))
    return;
  AstInterface::AstNodeList args;
  if (fa.IsFunctionCall(rhs, 0, &args)) {
    ...
    hasunknown = true;
    DebugAliasAnal([&rhs]() {
      return "unknown alias info for function call : " +
             AstInterface::AstToString(rhs);
    });
    DebugAliasAnal([&mod]() {
      return "aliasing all parameters with " + AstInterface::AstToString(mod);
    });
    collect.reset();
...
void StmtVarAliasCollect::AppendFuncCall(AstInterface &fa,
                                         const AstNodePtr &fc) {
  DebugLog DebugAliasAnal("-debugaliasanal");
  ModifyAliasMap collect(fa, aliasmap);
  if (funcanal == 0 || !funcanal->may_alias(fa, fc, 0, collect)) {
    ...
    DebugAliasAnal([&fc]() {
      return "unknown alias info. for function call : " +
             AstInterface::AstToString(fc) + "; Aliasing all parameters";
    });
    collect.reset();
    AstInterface::AstNodeList args;
...
bool StmtVarAliasCollect::may_alias(AstInterface &fa, const AstNodePtr &r1,
                                    const AstNodePtr &r2) {
  DebugLog DebugAliasAnal("-debugaliasanal");
  ...
  if (!fa.IsExpression(r1, &t1) || !fa.IsExpression(r2, &t2))
    ROSE_ABORT();
  if (!hasresult) {
    DebugAliasAnal([&r1, &r2]() {
      return "No alias analysis performed. Has alias between " +
             AstInterface::AstToString(r1) + " and " +
             AstInterface::AstToString(r2);
    });
    return true;
  }
  ...
  if (!fa.IsVarRef(r1, 0, &varname1, &scope1, &global1) ||
    ...
    DebugAliasAnal([&r1, &r2]() {
      return "no alias analysis between non_variables performed. Has alias "
             "between " +
             AstInterface::AstToString(r1) + " and " +
             AstInterface::AstToString(r2);
    });
  ...
  if (global1 && global2 && hasunknown) {
    if (global1) {
      DebugAliasAnal([&varname1, &varname2]() {
        return "INFO:" + varname1 + " and " + varname2 + " are global";
      });
      DebugAliasAnal([&r1, &r2]() {
        return "INFO: Has alias between " + AstInterface::AstToString(r1) +
               " and " + AstInterface::AstToString(r2);
      });
    }
    return true;
  }
  if (aliasmap.get_alias_map(varname1, scope1)
          ->in_same_group(aliasmap.get_alias_map(varname2, scope2))) {
    DebugAliasAnal([&r1, &r2]() {
      return "INFO: Alias analysis performed. Has alias between " +
             AstInterface::AstToString(r1) + " and " +
             AstInterface::AstToString(r2);
    });
    return true;
...
void StmtVarAliasCollect::operator()(AstInterface &fa,
                                     const AstNodePtr &funcdef) {
  analyze(fa, funcdef);
  return;
}

void StmtVarAliasCollect::analyze(AstInterface &fa, const AstNodePtr &funcdef) {
  assert(!hasresult);
  hasresult = true;
  AstNodePtr body;
  AstInterface::AstNodeList params;
  if (!fa.IsFunctionDefinition(funcdef, 0, &params, 0, &body)) {
    std::cerr << "Error: alias analysis requires function definition as input "
                 "instead of "
              << AstInterface::AstToString(funcdef) << std::endl;
    ROSE_ABORT();
  }
