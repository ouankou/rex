...
protected:
  ...
  virtual void AppendVariableDecl(AstInterface &fa, const AstNodePtr &variable,
                                  const AstNodePtr &var_init) = 0;
  virtual void AppendAliasDecl(AstInterface &fa, const AstNodePtr &variable,
                               const AstNodePtr &var_init) = 0;
  ...
  virtual void AppendMemoryAllocate(AstInterface & /* fa */,
                                    const AstNodePtr & /* s */) {}
  virtual void AppendMemoryFree(AstInterface & /* fa */,
                                const AstNodePtr & /* s */) {}

  // Analyzes call arguments to determine what are read. Returns callee if
  // requested.
...
class StmtSideEffectCollect : public StmtInfoCollect,
                              public SideEffectAnalysisInterface {
  using SideEffectAnalysisInterface::alias_collect;
...
public:
  ...
  virtual bool get_side_effect(AstInterface & /*fa*/,
                               const AstNodePtr &h) override {
    return operator()(h);
  }
...
protected:
  ...
  virtual void AppendVariableDecl(AstInterface & /* fa */,
                                  const AstNodePtr &variable,
                                  const AstNodePtr &var_init) override;
  virtual void AppendAliasDecl(AstInterface & /*fa*/,
                               const AstNodePtr &variable,
                               const AstNodePtr &var_init) override;
  virtual void AppendModLoc(AstInterface &fa, const AstNodePtr &mod,
...
public:
  ...
  static std::string get_unique_name(const std::string &fname,
                                     const AstNodePtr &scope,
                                     const std::string &varname) {
    return fname + "-" +
           (scope == 0 ? "" : Ast2StringMap::inst()->get_string(scope)) + "-" +
           varname;
  }

  static std::string get_unique_name(AstInterface &fa, const AstNodePtr &exp);

  static std::string get_unique_name(AstInterface &fa, const AstNodePtr &scope,
                                     const std::string &varname);
};

class FunctionAliasInterface;
// flow insensitive alias analysis for named variables only
class StmtVarAliasCollect : public StmtInfoCollect,
                            public AliasAnalysisInterface {
public:
  class VarAliasMap { // name -> disjoint-set ID, we only need to check if two
                      // variables belong to the same group
    std::map<std::string, UF_elem *, std::less<std::string>> aliasmap;
    static Ast2StringMap scopemap;

  public:
    ~VarAliasMap() {
      for (std::map<std::string, UF_elem *,
                    std::less<std::string>>::const_iterator p =
               aliasmap.begin();
           p != aliasmap.end(); ++p) {
        delete (*p).second;
      }
...
public:
  ...
  virtual void analyze(AstInterface &fa,
                       const AstNodePtr &funcdefinition) override;
  void operator()(AstInterface &fa, const AstNodePtr &funcdefinition);
  bool may_alias(AstInterface &fa, const AstNodePtr &r1,
                 const AstNodePtr &r2) override;
};

/* Support tracking modification of variables, by building a map from each
 * variable name to all the AST nodes where the variable is modified */
template <class Select> class ModifyVariableMap : public StmtSideEffectCollect {
