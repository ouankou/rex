...
template <class AstNodePtr>
class StmtSideEffectCollect : public StmtInfoCollect<AstNodePtr>,
                              public SideEffectAnalysisInterface<AstNodePtr> {
public:
  StmtSideEffectCollect(AstInterface &fa, FunctionSideEffectInterface *a = 0)
      : fa_(fa), modunknown(false), readunknown(false), funcanal(a),
        modcollect(0), readcollect(0), killcollect(0) {}
  ...
  bool operator()(const AstNodePtr &h, CollectObject *mod,
                  CollectObject *read = 0, CollectObject *kill = 0) {
    modcollect = mod;
    readcollect = read;
    killcollect = kill;
...
class StmtVarAliasCollect : public StmtInfoCollect<AstNodePtr>,
