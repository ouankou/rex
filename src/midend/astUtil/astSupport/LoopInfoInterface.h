...
int GetLoopLevel(Stmt s, Interface &ast_interface) {
  ...
  for (; s != ast_interface.getNULL();
       ++r, s = GetEnclosingLoop(s, ast_interface))
    ;
  return r;
}

template <class Stmt, class Interface>
Stmt GetCommonLoop(Interface &ast_interface, Stmt b1, int dim1, Stmt b2,
                   int dim2, int *level = 0) {
  while (dim1 > dim2) {
...
Stmt GetCommonLoop(Interface &ast_interface, Stmt b1, Stmt b2, int *level = 0) {
  int dim1 = GetLoopLevel(b1, ast_interface),
      dim2 = GetLoopLevel(b2, ast_interface);
