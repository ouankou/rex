...
public:
  DependenceEntry(const std::string &first, const std::string &second,
                  const std::string &deptype, const std::string &attr)
  ...
  bool operator==(const DependenceEntry &e2) const {
    return first_ == e2.first_ && second_ == e2.second_ &&
           deptype_ == e2.deptype_;
  }
  bool operator<(const DependenceEntry &e2) const {
    return first_ < e2.first_ ||
           (first_ == e2.first_ && second_ < e2.second_) ||
           (first_ == e2.first_ && second_ == e2.second_ &&
            deptype_ < e2.deptype_);
  }
  std::string to_string() const {
    return first_ + " : " + " [ " + deptype_ + " ] " + second_ + " = " + attr_;
  }
...
// Read a table that records the dependences among all the components of a
// software. The syntax for specifying a dependence is x1 : y1 y2 ... ym; which
...
protected:
  virtual void save_dependence(const DependenceEntry &e) {
    if ((tag_first_.empty() ||
         tag_first_.find(e.first_entry()) != tag_first_.end()) &&
        (tag_second_.empty() ||
         tag_second_.find(e.second_entry()) != tag_second_.end()) &&
        (tag_type_.empty() ||
         tag_type_.find(e.type_entry()) != tag_type_.end())) {
      result.push_back(e);
    }
  }

public:
  void add_selection(const std::string &_tag_first,
                     const std::string &_tag_second = "",
                     const std::string &_tag_type = "") {
    if (!_tag_first.empty()) {
...
public:
  ...
  // Collect all the components that depends on input downstream, by
  // transitively traversing the dependence table. The result should be
  // initialized to empty before invocation. The what_to_do parameter, if given,
  // can skip computing some entries by returning false.
  void
  ...
  void
  Compute(const std::string &input, std::set<std::string> &result,
          const std::function<bool(const DependenceEntry &)> *what_to_do = 0);

  void
  Output(std::ostream &output, std::set<std::string> *select = 0,
         const std::function<bool(const DependenceEntry &)> *what_to_do = 0) {
  ...
  // Collect transitive dependences from input, invoke what to do for each
  // entry.
private:
  // Store downstream dependences from a component x to all the other components
  // depending on x.
  ...
  std::string next_start(const DependenceEntry &e) const {
    return (direction_ == Direction::CollectBackward) ? e.first_entry()
                                                      : e.second_entry();
  }
  std::string current_start(const DependenceEntry &e) const {
    return (direction_ == Direction::CollectBackward) ? e.second_entry()
                                                      : e.first_entry();
  }
...
public:
  ...
  //
  //! The operator op accesses the given memory reference in nature of the given
  //! relation.
