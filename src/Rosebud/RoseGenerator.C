...
RoseGenerator::adjustParser(Sawyer::CommandLine::Parser &parser) {
    ...
    sg.insert(Switch("generated")
              .argument("directory", anyParser(generatedDirName))
...
RoseGenerator::generate(const Ast::Project::Ptr &project) {
    ...
    if (!generatedDirName.empty())
        generatedDir = Rose::FileSystem::Path(generatedDirName);
...
RoseGenerator::genHeaderIncludes(std::ostream &header, const Ast::Class::Ptr &c, const Hierarchy &h) {
    ...
    for (const auto &base: c->inheritance) {
        const Rose::FileSystem::Path include = h.findVertexKey(base.second) == h.vertices().end() ?
...
RoseGenerator::genClass(const Ast::Class::Ptr &c, const Hierarchy &h) {
    ...
    const Rose::FileSystem::Path headerName = generatedDir / toPath(c->name, ".h");
    const Rose::FileSystem::Path implName = generatedDir / toPath(c->name, ".C");
    std::ofstream header(headerName.string().c_str());
    std::ofstream impl(implName.string().c_str());
    if (!header) {
