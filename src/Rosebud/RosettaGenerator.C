...
RosettaGenerator::purpose() const {
    return "Generate code that's backward compatible with ROSETTA.";
}
...
RosettaGenerator::adjustParser(Sawyer::CommandLine::Parser &parser) {
    ...
    SwitchGroup sg("ROSETTA backend for binary analysis IR/AST nodes (--backend=" + name() + ")");
    sg.name(name());
...
RosettaGenerator::shortName(const Ast::Class::Ptr &c) {
    ...
    if (c->name.size() >= 3 && startsWith(c->name, "Sg")) {
...
RosettaGenerator::fileWrtRose(const Rose::FileSystem::Path &fileName) {
    Rose::FileSystem::Path retval = relativeToRoseSource(fileName);
    if (retval.empty()) {
...
RosettaGenerator::genRosettaFileBegin(std::ostream &rosetta) {
    ...
    rosetta <<"#include \"ROSETTA_macros.h\"\n"
            <<"#include \"grammar.h\"\n"
            ...
            <<"#endif\n"
            <<"\n"
            <<"#define IS_SERIALIZABLE(CLASS_WITHOUT_Sg) /*void*/\n"
            <<"\n";
}
...
RosettaGenerator::rosettaBaseClass(const Ast::Class::Ptr &c) {
    ASSERT_not_null(c);
    for (const auto &super: c->inheritance) {
        if (super.second.size() >= 3 && startsWith(super.second, "Sg"))
            return super.second;
    }
    return "";
}
...
RosettaGenerator::genPropertyDataMember(std::ostream &rosetta, std::ostream &header, const Ast::Property::Ptr &p) {
    ...
    //     as Rosebud::cloneptr is present.
...
RosettaGenerator::genPropertyMutators(std::ostream &header, std::ostream &impl, const Ast::Property::Ptr &p) {
    ...
    for (const std::string &mutatorName: propertyMutatorNames(p)) {
        ...
        if (strictParents && p->findAttribute("Rosebud::traverse")) {
            impl <<"    changeChildPointer(this->" <<propertyDataMemberName(p) <<", const_cast<" <<type <<"&>(x));\n";
        } else {
...
void
RosettaGenerator::genRosettaPragmas(std::ostream &rosetta, const std::vector<std::string> &pragmas,
                                    const Ast::Class::Ptr &c) {
    for (const std::string &pragma: pragmas) {
        std::string s = Rose::StringUtility::replaceAllCopy(pragma, "\\\n", "\n");
...
RosettaGenerator::genClassDefinition(std::ostream &rosetta, const Ast::Class::Ptr &c, const Hierarchy &h) {
    ...
    if (nPragmas > 0 && settings.showingWarnings) {
        message(WARN, c->findAncestor<Ast::File>(), c->nameToken,
                "class " + c->name + " contains " + std::to_string(nPragmas) + " pragma " +
...
RosettaGenerator::genTupFile(const std::vector<std::string> &implFileNames) {
    if (!implDirectoryName.empty()) {
        const Rose::FileSystem::Path tupFileName = implDirectoryName / "Tupfile";
        std::ofstream out(tupFileName.string());
        if (!out) {
...
RosettaGenerator::genMakeFile(const std::vector<std::string> &implFileNames) {
    if (!implDirectoryName.empty()) {
        const Rose::FileSystem::Path tupFileName = implDirectoryName / "Makefile.am";
        std::ofstream out(tupFileName.string());
        if (!out) {
            message(ERROR, "cannot create file: \"" + tupFileName.string() + "\"");
            return;
...
RosettaGenerator::genCmakeFile(const std::vector<std::string> &implFileNames) {
    if (!implDirectoryName.empty()) {
        const Rose::FileSystem::Path cmakeFileName = implDirectoryName / "CMakeLists.txt";
        std::ofstream out(cmakeFileName.string());
        if (!out) {
        ...
        if (implFileNames.empty()) {
            std::ofstream stub((implDirectoryName / "stub.C").string());
...
RosettaGenerator::generate(const Ast::Project::Ptr &project) {
    ...
    const std::string rosettaFileNameStr = rosettaFileName.string();
    if (rosettaFileNameStr != "-")
        rosettaFile.open(rosettaFileName.empty() ? "/dev/null" : rosettaFileNameStr);
    std::ostream &rosetta = rosettaFileNameStr == "-" ? std::cout : rosettaFile;
    if (!rosetta) {
