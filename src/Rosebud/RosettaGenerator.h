...
protected:
    ...
    Rose::FileSystem::Path rosettaFileName;             // optional name of file into which ROSETTA source code is written
    Rose::FileSystem::Path implDirectoryName;           // optional name of directory where node implementation files are created
    Rose::FileSystem::Path nodeListFileName;            // optional name of file that contains list of all node types
...
private:
    Rose::FileSystem::Path fileWrtRose(const Rose::FileSystem::Path&);
    ...
    void genRosettaPragmas(std::ostream &rosetta, const std::vector<std::string>&, const Ast::ClassPtr&);
