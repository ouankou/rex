...
static int get_next_char(std::istream &stream) { return stream.get(); }

template <class Stream>
ssize_t rose_getline_impl(char **lineptr, size_t *n, Stream &stream) {
  assert(lineptr);
  assert(n);
  assert(stream);
...
ssize_t rose_getline(char **lineptr, size_t *n, std::istream &stream) {
  return rose_getline_impl(lineptr, n, stream);
}

std::string rose_getline(FILE *stream) {
  struct Resources {
    char *buffer;
    size_t bufsz;

    Resources() : buffer(NULL), bufsz(0) {}
...
std::string rose_getline(std::istream &stream) {
  struct Resources {
    char *buffer;
    size_t bufsz;
