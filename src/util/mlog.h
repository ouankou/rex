 ...
 *
 * Optional logging are those msgs that provide information of program
 * execution. It is controlled by the mlogLevel varible that control which level
 * of information will be spit out. The levels of details incrementally include:
 * MLOG_LEVEL_KEY
 * MLOG_LEVEL_INFO
 * MLOG_LEVEL_MARCH
 * MLOG_LEVEL_TRACE
 * The default mlogLevel is MLOG_LEVEL_NONE, i.e. no information of these four
 * levels will spit out.
 *
 * MLOG_LEVEL_DEBUG is a mlogging level that can be controlled by users, it
 * should be used for debugging purpose only.
 *
 * There are two sets of APIs to use, C-style (printf) and C++-style (<<), and
 * these APIs are are simply macros that are used as functions. API names carry
 * the level information, and each mlogging API call should also be proivde a
 * subject to indicate the compoenents or module of this mlogging. MLogging msgs
 * include logging level, subject, file:line, function name, and user-specified
 * msgs. User msgs are either C-style printf format or C++-style << format
 * depending on which style to use. C-style APIs and C++-style APIs have _C and
 * _CXX suffix respectively.
 *
 * The mlog.h/C also include the implementation for ASSERT_*, ROSE_ASSERT,
 * ROSE_ABORT that are used througout the source tree.
 */
...
extern int mlogLevel; // The current mlog level setting to control which level
                      // (KEY/INFO/MARCH/TRACE) info is spit out
extern void mlog_C(MLOG_LEVEL_t level, const char *subject, const char *file,
                   int line, const char *funcname, const char *format, ...);
#if __cplusplus >= 201103L
[[noreturn]]
#endif
extern void mlogAssertFail_C(const char *subject, const char *expr,
                             const char *file, int line, const char *funcname,
                             const char *format, ...);
#if __cplusplus >= 201103L
inline void mlogAssertFail_C(const char *subject, const char *expr,
                             const char *file, int line, const char *funcname,
...
#define MLOG_FATAL_C(SUBJECT, ...)                                             \
  mlog_C(MLOG_LEVEL_FATAL, SUBJECT, WHEREARG, __VA_ARGS__)
#define MLOG_ERROR_C(SUBJECT, ...)                                             \
  mlog_C(MLOG_LEVEL_ERROR, SUBJECT, WHEREARG, __VA_ARGS__)
#define MLOG_WARN_C(SUBJECT, ...)                                              \
  mlog_C(MLOG_LEVEL_WARN, SUBJECT, WHEREARG, __VA_ARGS__)
#define MLOG_DEBUG_C(SUBJECT, ...)                                             \
  mlog_C(MLOG_LEVEL_DEBUG, SUBJECT, WHEREARG, __VA_ARGS__)
...
#define MLOG_LEVEL_C(level, subject, ...)                                      \
  (level > mlogLevel) ? static_cast<void>(0)                                   \
                      : mlog_C(level, subject, WHEREARG, __VA_ARGS__)
#define MLOG_LEVEL_MORE_C(level, ...)                                          \
  (level > mlogLevel) ? static_cast<void>(0) : mlogMore_C(__VA_ARGS__)

// C-style API, i.e. printf style for optional key/info/march/trace mlogging
// controlled by mlogLevel
#define MLOG_KEY_C(SUBJECT, ...)                                               \
  MLOG_LEVEL_C(MLOG_LEVEL_KEY, SUBJECT, __VA_ARGS__)
#define MLOG_INFO_C(SUBJECT, ...)                                              \
  MLOG_LEVEL_C(MLOG_LEVEL_INFO, SUBJECT, __VA_ARGS__)
#define MLOG_MARCH_C(SUBJECT, ...)                                             \
  MLOG_LEVEL_C(MLOG_LEVEL_MARCH, SUBJECT, __VA_ARGS__)
#define MLOG_TRACE_C(SUBJECT, ...)                                             \
  MLOG_LEVEL_C(MLOG_LEVEL_TRACE, SUBJECT, __VA_ARGS__)
...
// C-style API to use for fatal/error/warning that is conditioned by the
// provided expression
 ...
 * ***************************************/
#define MLOG_LEVEL_HEADER_CXX(level, subject)                                  \
...
// C++-style API for optional key/info/march/trace mlogging controlled by
// mlogLevel
...
#define ASSERT_require2(expr, ...)                                             \
  (expr)                                                                       \
      ? static_cast<void>(0)                                                   \
      : mlogAssertFail_C("ASSERTION:require:", #expr, WHEREARG, __VA_ARGS__)
...
#define ASSERT_forbid2(expr, ...)                                              \
  (!(expr))                                                                    \
      ? static_cast<void>(0)                                                   \
      : mlogAssertFail_C("ASSERTION:forbidden:", #expr, WHEREARG, __VA_ARGS__)

#define ASSERT_not_null(expr) ASSERT_not_null2(expr, "")
#define ASSERT_not_null2(expr, ...)                                            \
  ((expr) != NULL) ? static_cast<void>(0)                                      \
                   : mlogAssertFail_C("ASSERTION:NULL pointer:", #expr,        \
                                      WHEREARG, __VA_ARGS__)

// Convenience for member functions that want to assert on "this".
#define ASSERT_this() ASSERT_not_null(this)

// A more informative approach for assert and abort than the standard
// assert/abort
#define ROSE_ASSERT ASSERT_require
#define ROSE_ABORT() mlogAssertFail_C("ABORT:", "", WHEREARG, "")

#define ASSERT_not_reachable(...)                                              \
  mlogAssertFail_C("reached impossible state:", "", WHEREARG, __VA_ARGS__)
#define ASSERT_not_implemented(...)                                            \
  mlogAssertFail_C("not implemented yet:", "", WHEREARG, __VA_ARGS__)
