...
public:
  SelectIterator(Iterator it, const Select &s) : iter(it), sel(s) {
    SetIterator();
  }
...
public:
  MultiCrossIterator(Iterator1 it, Cross c) : iter1(it), cross(c) {
    SetIterator();
  }
...
template <class T, class TU, class Iterator> class MultiIterator {
  Iterator iter, next;
