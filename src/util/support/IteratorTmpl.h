...
template <class T> class SingleIterator : public IteratorImpl<T> {
  T item;
  bool end;
...
public:
  ...
  virtual IteratorImpl<T> *Clone() const { return new SingleIterator(*this); }
};
...
public:
  IteratorWrap<T, Impl> &operator=(const IteratorWrap<T, Impl> &that) {
    CountRefHandle<Impl>::operator=(that);
    return *this;
  }
  IteratorWrap(const IteratorWrap<T, Impl> &that)
      : CountRefHandle<Impl>(that) {}
  IteratorWrap() {}
  IteratorWrap(Impl *_impl) : CountRefHandle<Impl>(_impl) {}
  IteratorWrap(const Impl &_impl) : CountRefHandle<Impl>(_impl) {}
  ~IteratorWrap() {}

  T Current() const { return ConstRef().Current(); }
  T operator*() const { return Current(); }
  void Advance() {
    Impl *impl = UpdatePtr();
    if (impl)
...
public:
  MultiIterator(Iterator impl1, Iterator impl2) : iter(impl1), next(impl2) {}
  MultiIterator(const MultiIterator &that) : iter(that.iter), next(that.next) {}
  ~MultiIterator() {}
...
template <class Iter, class Update> void UpdateEach(Iter iter, Update T) {
  for (; !iter.ReachEnd(); iter++) {
    T(iter.Current());
  }
}

template <class Iter> unsigned CountIteratorSize(Iter iter) {
  unsigned size = 0;
...
template <class Iter> std::string IteratorToString1(Iter iter) {
  std::string res;
...
template <class Iter> std::string IteratorToString2(Iter iter) {
  std::string res;
  for (Iter p = iter; !p.ReachEnd(); ++p)
    res = res + (*p)->toString();
  return res;
...
template <class Iter, class Update> bool FindFirst(Iter &iter, Update T) {
  for (; !iter.ReachEnd(); iter++) {
    if (T(iter.Current()))
      return true;
  }
