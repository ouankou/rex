...
template <class T> class PtrSetWrap {
  VoidPtrSet impl;

public:
  PtrSetWrap() {}
  PtrSetWrap(const PtrSetWrap<T> &that) : impl(that.impl) {}
  PtrSetWrap<T> &operator=(const PtrSetWrap<T> &that) {
    impl = that.impl;
    return *this;
  }
  ~PtrSetWrap() {}
  ...
  class Iterator {
    ...
    Iterator(const VoidPtrSet *_impl) : impl(_impl) { Reset(); }
  ...
  public:
    ~Iterator() {}
    Iterator(const Iterator &that) : impl(that.impl), p(that.p) {}
  ...
  const_iterator find(const T *t) const {
    return Iterator(&impl, impl.find((void *)t));
  }
  ...
  bool IsMember(const T *t) const { return impl.find((void *)t) != impl.end(); }
...
template <class T> class AppendSTLSet : public CollectObject<T> {
...
template <class T> class AppendPtrSet : public CollectObject<T *> {
  PtrSetWrap<T> &res;
...
template <class T> class SelectSTLSet : public RoseSelectObject<T> {
  std::set<T> res;

public:
  SelectSTLSet(const std::set<T> r) : res(r) {}
  bool operator()(const T &cur) const { return (res.find(cur) != res.end()); }
};
template <class T> class SelectPtrSet : public RoseSelectObject<T *> {
  PtrSetWrap<T> res;
...
public:
  ...
  bool operator()(T *const &cur) const { return (res.IsMember(cur)); }
};
template <class T> class NotSelectPtrSet : public RoseSelectObject<T *> {
  PtrSetWrap<T> res;
