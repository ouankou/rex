...
private:
  ...
  public:
    ...
    int operator()(Node *const &n) {
      return (reverse) ? n->GetGraph()->NumberOfNodes() - n->TopoOrderIndex()
                       : n->TopoOrderIndex() - 1;
    }
  };
...
public:
  ...
  void SortNodes(MapObject<Node *, int> &f) {
    DirectedGraph<Node, Edge>::SortNodes(f);
    sort = NON_SORT;
  }
...
DAGNode<Node, Edge>::DAGNode(DAG<Node, Edge> *dag)
  ...
  visitNo = 0;
  ordNo = dag->NumberOfNodes();
}

template <class Node, class Edge> DAGNode<Node, Edge>::~DAGNode() {
...
template <class Node, class Edge>
DAG<Node, Edge> *DAGNode<Node, Edge>::GetGraph() const {
...
template <class Node, class Edge>
void DAGEdge<Node, Edge>::MoveEndPoint(Node *n, EdgeDirection dir) {
...
template <class Node, class Edge> bool DAGEdge<Node, Edge>::IsBackEdge() const {
#ifndef NDEBUG
