...
void write_graph_node_edge(Graph &g, const Node *curnode, std::ostream &out,
                           const std::string &hint,
                           GraphAccess::EdgeDirection dir) {
  typename Graph::EdgeIterator edges = g.GetNodeEdgeIterator(curnode, dir);
  for (; !edges.ReachEnd(); ++edges) {
    typename Graph::Edge *e = (*edges);
    typename Graph::Node *n = g.GetEdgeEndPoint(e, GraphAccess::Reverse(dir));
...
template <class Graph> std::string GraphToString(const Graph &g) {
