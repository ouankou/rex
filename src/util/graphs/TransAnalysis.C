...
public:
  ShadowGraphNode(MultiGraphCreate *c, GraphAccessInterface::Node *o)
      : MultiGraphElem(c) {
    orig = o;
  }
...
template <class T> class ShadowBaseInfoGraph : public TransInfoGraph<T> {
...
public:
  ...
  bool TransInfoComputed(const GraphAccessInterface::Node *src,
                         const GraphAccessInterface::Node *snk) {
  ...
  T GetTransInfo(const GraphAccessInterface::Node *src,
                 const GraphAccessInterface::Node *snk) {
    const DAGBaseNodeImpl *b1 = static_cast<const DAGBaseNodeImpl *>(src),
                          *b2 = static_cast<const DAGBaseNodeImpl *>(snk);
  ...
  void SetTransInfo(GraphAccessInterface::Node *src,
                    GraphAccessInterface::Node *snk, T e) {
    const DAGBaseNodeImpl *b1 = static_cast<const DAGBaseNodeImpl *>(src),
                          *b2 = static_cast<const DAGBaseNodeImpl *>(snk);
...
template <class T> class ShadowBaseOP : public TransInfoOP<T> {
  CloneShadowGraphCreate *cloneCreate;
...
public:
  ...
  T GetBottomInfo(const GraphAccessInterface::Node *n1,
                  const GraphAccessInterface::Node *n2) {
  ...
  T GetTopInfo(const GraphAccessInterface::Node *n1,
               const GraphAccessInterface::Node *n2) {
    const DAGBaseNodeImpl *b1 = static_cast<const DAGBaseNodeImpl *>(n1),
                          *b2 = static_cast<const DAGBaseNodeImpl *>(n2);
...
public:
  ...
  public:
    EdgeImpl(MultiGraphCreate *c, const T &t) : MultiGraphElem(c), info(t) {}
    ~EdgeImpl() {}
    T &Info() { return info; }
  };
  ...
  bool TransInfoComputed(const GraphAccessInterface::Node *src,
    ...
    return ContainNode(src) && ContainNode(snk) &&
           !GraphCrossEdgeIterator<TransInfoCreate<T>>(this, src, snk)
                .ReachEnd();
  }
  T GetTransInfo(const GraphAccessInterface::Node *n1,
                 const GraphAccessInterface::Node *n2) {
  ...
  void SetTransInfo(GraphAccessInterface::Node *n1,
                    GraphAccessInterface::Node *n2, T info) {
    if (!ContainNode(n1))
      AddNode(n1);
    if (!ContainNode(n2))
      AddNode(n2);
...
public:
  ...
  void ComputeTransInfoOnDAG(DAGBaseGraphImpl *impl, DAGBaseNodeImpl *node,
    ...
    for (DAGBaseGraphImpl::NodeIterator baseIter = impl->GetNodeIterator();
      ...
      for (DAGBaseGraphImpl::EdgeIterator edgeIter =
               impl->GetNodeEdgeIterator(n1, dir);
        ...
        r = (dir == GraphAccess::EdgeOut) ? baseOp.Composite(r, r2)
                                          : baseOp.Composite(r2, r);
  ...
  void SetTransInfo(DAGBaseNodeImpl *src, DAGBaseNodeImpl *snk, const T e,
                    GraphAccess::EdgeDirection dir = GraphAccess::EdgeOut) {
    if (dir == GraphAccess::EdgeOut)
      infoCreate.SetTransInfo(src, snk, e);
    else
      infoCreate.SetTransInfo(snk, src, e);
  }
  void ClearTransInfo(DAGBaseNodeImpl *n) { infoCreate.DeleteNode(n); }
  TransInfoGraph<T> *GetTransInfoGraph() { return &infoCreate; }
  T GetTransInfo(const DAGBaseNodeImpl *src, const DAGBaseNodeImpl *snk,
                 GraphAccess::EdgeDirection dir = GraphAccess::EdgeOut) {
    if (src == snk)
...
template <class T> class TransAnalSCCGraphNode : public GroupGraphNode {
  typedef struct TwinNodeStruct {
    DAGBaseNodeImpl *orig, *split;
    TwinNodeStruct() : orig(0), split(0) {}
    TwinNodeStruct(const TwinNodeStruct &that)
        : orig(that.orig), split(that.split) {}
  ...
  void Preprocess(TransInfoOP<T> *analOp, GraphAccessInterface *orig,
                  int limit);
  DAGBaseNodeImpl *ComputeInitTransInfo(GraphAccessInterface::Node *n,
                                        GraphAccess::EdgeDirection dir,
                                        TransInfoOP<T> *analOp,
                                        GraphAccessInterface *orig,
                                        TransInfoGraph<T> *recOp);
  void CloseTransInfo(DAGBaseNodeImpl *n, GraphAccess::EdgeDirection dir,
                      int num, TransInfoOP<T> *analOp,
                      TransInfoGraph<T> *recOp);

public:
  TransAnalSCCGraphNode(GroupGraphCreate *g) : GroupGraphNode(g) {
    cloneBase = 0;
    cloneCreate = 0;
    dagOp = 0;
    bottom = false;
  ...
  bool ComputeTransInfo(GraphAccessInterface::Node *n,
                        GraphAccess::EdgeDirection dir, TransInfoOP<T> *analOp,
                        GraphAccessInterface *g, int limit,
                        TransInfoGraph<T> *recOp);
  bool ReachBottom() const { return bottom; }
};

template <class T> class TransAnalSCCOperator : public SCCGroupGraphOperator {
public:
  TransAnalSCCOperator(GroupGraphCreate *g) : SCCGroupGraphOperator(g) {}
  void CreateSCC() {
    SetCurSCC(new TransAnalSCCGraphNode<T>(GetGroupGraphCreate()));
  }
  TransAnalSCCGraphNode<T> *
  GetOrigSCCNode(const GraphAccessInterface::Node *n) {
    return (TransAnalSCCGraphNode<T> *)(GetSCCNode(
        const_cast<GraphAccessInterface::Node *>(n)));
  }
};

template <class T> class TransAnalSCCCreate : public SCCGraphCreate {
  GraphAccessInterface *orig;
  TransInfoOP<T> *analOp;
...
public:
  TransAnalSCCCreate(GraphAccessInterface *g, TransInfoOP<T> *op, int split)
      : SCCGraphCreate(g, sccOp = new TransAnalSCCOperator<T>(this)), orig(g),
        analOp(op), limit(split) {}
  ...
  bool ComputeTransInfo(GraphAccessInterface::Node *n1,
                        GraphAccessInterface::Node *n2,
                        TransInfoGraph<T> *recOp);
  friend class TransAnalSCCGraphNode<T>;
};

template <class T>
bool TransAnalSCCGraphNode<T>::ComputeTransInfo(GraphAccessInterface::Node *n,
                                                GraphAccess::EdgeDirection dir,
                                                TransInfoOP<T> *analOp,
                                                GraphAccessInterface *orig,
                                                int limit,
  ...
  if (!bottom || !ContainNode(n)) {
    DAGBaseNodeImpl *base =
        static_cast<DAGBaseNodeImpl *>(cloneCreate->GetOrigBaseNode(n));
    if (base == 0) {
...
template <class T>
void TransAnalSCCGraphNode<T>::Preprocess(TransInfoOP<T> *analOp,
                                          GraphAccessInterface *orig,
                                          int limit) {
  if (cloneCreate != 0)
    return;
  ...
  GraphAccessWrapTemplate<GraphAccessInterface::Node,
                          GraphAccessInterface::Node, GroupNodeSelect>
      scc(&sccImpl);
  cloneCreate->AddGraph(&scc);
  cloneBase->TopoSort();
  for (DAGBaseGraphImpl::NodeIterator nodeIter = cloneBase->GetNodeIterator();
  ...
  if (!bottom) {
    ...
    for (int index = 0; index < num; index++) {
      ...
      dagOp->ComputeTransInfoOnDAG(cloneBase, twin->split,
                                   GraphAccess::EdgeOut);
...
template <class T>
DAGBaseNodeImpl *TransAnalSCCGraphNode<T>::ComputeInitTransInfo(
    GraphAccessInterface::Node *n, GraphAccess::EdgeDirection dir,
    TransInfoOP<T> *analOp, GraphAccessInterface *g, TransInfoGraph<T> *recOp) {
  ...
  for (const_iterator nodeIter = begin(); !(nodeIter == end()); ++nodeIter) {
    ...
    T result = (dir == GraphAccess::EdgeOut) ? analOp->GetTopInfo(n1, n)
                                             : analOp->GetTopInfo(n, n1);
    for (GraphAccessInterface::EdgeIterator edgeIter =
    ...
    if (!analOp->IsTop(result)) {
      if (base == 0)
        base = static_cast<DAGBaseNodeImpl *>(cloneCreate->AddNode(n));
      DAGBaseNodeImpl *base1 =
          static_cast<DAGBaseNodeImpl *>(cloneCreate->GetOrigBaseNode(n1));
...
template <class T>
void TransAnalSCCGraphNode<T>::CloseTransInfo(DAGBaseNodeImpl *n,
                                              GraphAccess::EdgeDirection dir,
                                              int num, TransInfoOP<T> *analOp,
                                              TransInfoGraph<T> *recOp) {
  for (DAGBaseGraphImpl::NodeIterator nodeIter = cloneBase->GetNodeIterator();
...
template <class T>
bool TransAnalSCCCreate<T>::ComputeTransInfo(GraphAccessInterface::Node *n1,
                                             GraphAccessInterface::Node *n2,
                                             TransInfoGraph<T> *recOp) {
  bool succ = false;
  ...
  if (sccNode1 == sccNode2) {
    succ = sccNode1->ComputeTransInfo(n2, GraphAccess::EdgeOut, analOp, orig,
                                      limit, recOp);
    ...
    if (base1->TopoOrderIndex() > base2->TopoOrderIndex())
      return false;
    NodeIterator iter;
    for (iter = GetNodeIterator(); !iter.ReachEnd(); ++iter) {
      TransAnalSCCGraphNode<T> *scc =
          static_cast<TransAnalSCCGraphNode<T> *>(iter.Current());
      if (scc == sccNode2)
        break;
    }
    for (; !iter.ReachEnd(); ++iter) {
      TransAnalSCCGraphNode<T> *scc =
          static_cast<TransAnalSCCGraphNode<T> *>(iter.Current());
      if (scc->ComputeTransInfo(n2, GraphAccess::EdgeOut, analOp, orig, limit,
                                recOp))
        succ = true;
      if (scc == sccNode1)
        break;
...
template <class T>
GraphTransAnalysis<T>::GraphTransAnalysis(GraphAccessInterface *g,
                                          TransInfoOP<T> *_op, int splitlimit) {
  impl = new TransAnalSCCCreate<T>(g, _op, splitlimit);
  impl->TopoSort(true);
}

template <class T> GraphTransAnalysis<T>::~GraphTransAnalysis() { delete impl; }

template <class T>
bool GraphTransAnalysis<T>::ComputeTransInfo(TransInfoGraph<T> *tg,
                                             GraphAccessInterface::Node *n1,
                                             GraphAccessInterface::Node *n2) {
