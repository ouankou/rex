...
class BaseGraphCreate
    : public MultiGraphCreate,
      public GraphAccessTemplate<BaseGraphNode, BaseGraphEdge> {
protected:
...
public:
  ...
  virtual BaseGraphEdge *CreateEdge(BaseGraphNode *src, BaseGraphNode *snk,
                                    EdgeContent _id) = 0;
  virtual void MoveEdgeEndPoint(BaseGraphEdge *e, BaseGraphNode *n,
                                EdgeDirection dir) = 0;
...
template <class GraphImpl> class BaseGraphCreateWrap : public BaseGraphCreate {
public:
  ...
  virtual BaseGraphEdge *CreateEdge(BaseGraphNode *src, BaseGraphNode *snk,
    ...
    return new typename GraphImpl::Edge(
        this, static_cast<typename GraphImpl::Node *>(src),
        static_cast<typename GraphImpl::Node *>(snk), _id);
  }
  virtual void MoveEdgeEndPoint(BaseGraphEdge *e, BaseGraphNode *n,
                                GraphAccess::EdgeDirection dir) {
    assert(ContainEdge(e) && ContainNode(n));
    static_cast<typename GraphImpl::Edge *>(e)->MoveEndPoint(
        static_cast<typename GraphImpl::Node *>(n), TranslateDirection(dir));
  }
  ...
  EdgeIterator GetNodeEdgeIterator(const Node *n,
    ...
    return new IteratorImplTemplate<Edge *, typename GraphImpl::EdgeIterator>(
        impl->GetNodeEdgeIterator(
            static_cast<const typename GraphImpl::Node *>(n),
            TranslateDirection(dir)));
  }
  Node *GetEdgeEndPoint(const BaseGraphEdge *e,
                        GraphAccess::EdgeDirection dir) const {
    return impl->GetEdgeEndPoint(
        static_cast<const typename GraphImpl::Edge *>(e),
        TranslateDirection(dir));
  }
