...
private:
  ...
  std::map<GraphAccessInterface::Node *, SCCNodeStatus,
           std::less<GraphAccessInterface::Node *>>
      status;
...
public:
  ...
  GraphAccessInterface::Node *Top() { return nodeStack.First()->GetEntry(); }
...
void SCCAnalHelp(const GraphAccessInterface *dg, GraphAccessInterface::Node *v,
  ...
  for (GraphAccessInterface::EdgeIterator outEdges =
           dg->GetNodeEdgeIterator(v, GraphAccess::EdgeOut);
...
SCCGraphCreate::SCCGraphCreate(const GraphAccessInterface *orig,
                               SCCGroupGraphOperator *op)
  ...
  for (GroupGraphCreate::NodeIterator sccNodes = GetNodeIterator();
    ...
    for (GroupGraphNode::const_iterator nodes = scc->begin(); !nodes.ReachEnd();
         ++nodes) {
      ...
      for (GraphAccessInterface::EdgeIterator edges =
        ...
        GraphAccessInterface::Node *snk =
            orig->GetEdgeEndPoint(e, GraphAccess::EdgeIn);
