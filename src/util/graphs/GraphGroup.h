...
public:
  ...
  virtual void AddNode(const Node *n) { nodeSet.insert(const_cast<Node *>(n)); }
...
public:
  void AddGroupNode(GroupGraphNode *n) { AddNode(n); }
  void AddGroupEdge(GroupGraphNode *src, GroupGraphNode *snk,
                    GroupGraphEdge *e) {
    AddEdge(src, snk, e);
  }
