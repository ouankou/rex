...
template <class Graph>
class GraphNodePredecessorIterator
    : public IteratorImpl<typename Graph::Node *> {
  const Graph *graph;
  typename Graph::EdgeIterator edgep;
...
template <class Graph, class Collect> class GraphGetNodeReachable {
public:
  void operator()(const Graph *g, const typename Graph::Node *n,
                  typename Graph::EdgeDirection dir, Collect &collect) {
