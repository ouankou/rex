...
void TypedFusion ::operator()(GraphAccessInterface *dg,
                              TypedFusionOperator &fuseOp, int fusetype) {
  int lastnum = 0;
  ...
  for (GraphAccessInterface::NodeIterator nodeIter = dg->GetNodeIterator();
    ...
    for (GraphAccessInterface::EdgeIterator edgeIter =
             dg->GetNodeEdgeIterator(n, GraphAccess::EdgeIn);
  ...
  while (workStack.size() > 0) {
    ...
    for (GraphAccessInterface::EdgeIterator edgeIter =
             dg->GetNodeEdgeIterator(node, GraphAccess::EdgeOut);
         !edgeIter.ReachEnd(); ++edgeIter) {
      GraphAccessInterface::Edge *edge = edgeIter.Current();
      GraphAccessInterface::Node *m =
          dg->GetEdgeEndPoint(edge, GraphAccess::EdgeIn);
