...
*/
template <class T> class TransInfoOP {
public:
  ...
  virtual T GetBottomInfo(const GraphAccessInterface::Node *n1,
                          const GraphAccessInterface::Node *n2) = 0;
  ...
  virtual T GetTopInfo(const GraphAccessInterface::Node *n1,
                       const GraphAccessInterface::Node *n2) = 0;
  ...
  // Closure(info) = info union Composite(info,info) union
  // Composite(info,info,info)...
...
/****************************************************
 ...
 in the input graph
*/
template <class T> class TransInfoGraph {
public:
  ...
  // already computed)
  virtual T GetTransInfo(const GraphAccessInterface::Node *src,
                         const GraphAccessInterface::Node *snk) = 0;

  // record the transitive information e computed from src to snk
  virtual void SetTransInfo(GraphAccessInterface::Node *src,
                            GraphAccessInterface::Node *snk, T e) = 0;
...
*/
template <class T> class GraphTransAnalysis {
  TransAnalSCCCreate<T> *impl;

public:
  // constructor of GraphTransAnalysis operator. The GraphAccessInterface object
  // is the input graph to perform transitive analysis on. The TransInfoOP
  ...
  bool ComputeTransInfo(TransInfoGraph<T> *tg, GraphAccessInterface::Node *n1,
