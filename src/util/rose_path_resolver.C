...
#ifdef HAVE_DLADDR
  Dl_info info{};
  if (dladdr(reinterpret_cast<void *>(&resolveRosePaths), &info) == 0) {
    return std::nullopt;
  }
...
bool is_within_tree(const path &root, const path &candidate) {
  ...
  if (ec) {
    std::fprintf(stderr,
                 "ROSE fatal: weakly_canonical failed for root path '%s': %s\n",
  ...
  if (ec) {
    std::fprintf(
        stderr,
        "ROSE fatal: weakly_canonical failed for candidate path '%s': %s\n",
...
bool is_same_path(const path &left, const path &right) {
  ...
  if (ec) {
    std::fprintf(stderr,
                 "ROSE fatal: weakly_canonical failed for path '%s': %s\n",
...
RosePathRoots make_build_tree_roots(const path &build_root) {
  RosePathRoots roots;
  roots.in_install_tree = false;
  roots.build_root = build_root.string();
...
RosePathRoots make_install_tree_roots(const path &install_prefix) {
  RosePathRoots roots;
  roots.in_install_tree = true;
  roots.install_prefix = install_prefix.string();
  const path rose_root =
...
RosePathRoots resolveRosePaths(const char *argv0) {
  ...
  return RosePathRoots();
}

bool rosePathIsWithinTree(const std::string &root,
