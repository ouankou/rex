...
std::list<std::string> tokenize(const std::string &s, char delim) {
...
std::string join(char separator, char *strings[], size_t nstrings) {
  return join_range(std::string(1, separator), strings, strings + nstrings);
}

std::string join(char separator, const char *strings[], size_t nstrings) {
  return join_range(std::string(1, separator), strings, strings + nstrings);
}

std::string joinEnglish(const std::vector<std::string> &phrases,
                        const std::string &separator,
                        const std::string &finalIntro) {
  if (phrases.empty())
    return "";
  if (phrases.size() == 1)
    return phrases[0];
...
std::string listToString(const std::list<std::string> &container,
                         bool separateStrings) {
  std::string separator = separateStrings ? " \n" : " ";
  std::string retval =
      join_range(separator, container.begin(), container.end());
  if (!container.empty())
    retval += separator;
  return retval;
}

std::string listToString(const std::vector<std::string> &container,
                         bool separateStrings) {
  std::string separator = separateStrings ? " \n" : " ";
  std::string retval =
      join_range(separator, container.begin(), container.end());
...
std::string listToString(const std::list<int> &container,
                         bool separateStrings) {
  std::string separator = separateStrings ? " \n" : " ";
  std::string retval =
      join_range(separator, container.begin(), container.end());
...
std::list<std::string> stringToList(const std::string &input) {
...
// This function was written by Bobby Philip in support of the newer approach
// toward handling a broader number of back-end C++ compilers.
