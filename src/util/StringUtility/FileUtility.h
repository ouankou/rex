...
// [Robb P Matzke 2016-06-16]: These file utilities were all originally part of
// StringUtility. I'm leaving them here for now just so I don't need to make so
// many changes to projects that use these, but you should expect these to be
// moved sometime in the future.
namespace StringUtility {

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// This part of the StringUtility API deals with file names and should be moved
// to some other name space. In particular, it provides no definitions for
// "path", "filename", "extension", etc. and many of these functions won't work
// properly on a non-POSIX system. Therefore, consider using Rose::FileSystem,
// which is mostly a thin wrapper around std::filesystem. The std::filesystem
// documentation has good definitions for what the various terms should mean and
// works on non-POSIX file systems.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 * and a line number. Line numbers are one-origin; the first line of a file is
 * numbered 1, not 0.
 *
 *  @todo What does it mean to have a non-positive line number? What line number
 * should be used for generated code when the `filename` member is empty? */
 ...
 * writes the string `outputString` into the file. The name of the file is
 * constructed by concatenating `directoryName` and `fileNameString` without any
 * intervening component separator (e.g., no "/").
 *
...
ROSE_UTIL_API void writeFile(const std::string &outputString,
 ...
 * The message reads "File not found" regardless of the actual error condition.
 *
 ...
 * strings with location information, closes the file, and returns the vector.
 * The file names in the returned value are absolute names. The strings in the
 * return value have their final line-feeds removed.
 *
 *  If the file cannot be opened then an std::string error message is thrown.
 * The message reads "File not found" regardless of the actual error condition.
 *
 ...
 * `dir` argument. Will segfault if this environment variable is not set.
 *
 ...
 * consider using std::filesystem instead.
 *
 ...
 * includes a path and returns only the path. Make it safe to input a filename
 * without a path name (return the filename).
 *
 *  Terms are loosely defined and this function possibly doesn't work for
 * non-POSIX file systems; consider using std::filesystem instead.
 *
 * @param fileNameWithPath Path-like string.
 ...
 * ".", ".foo", "..", ".foo.bar", "/.", etc. Consider using std::filesystem
 * instead.
 *
 ...
 * non-POSIX systems. Consider using std::filesystem instead.
 *
 * @param relativePath Relative path string.
 * @param printErrorIfAny Whether to print errors.
 * @return Absolute path string. */
ROSE_UTIL_API std::string
getAbsolutePathFromRelativePath(const std::string &relativePath,
 ...
 * found in the input fileName, the function just returns the original fileName.
 *
 *  Terms are loosely defined and this function is not likely to work correctly
 * in some situations, such as when the "." is not in the last component of the
 * file name.  Consider using std::filesystem instead.
 *
...
//   1. It appears to not be used anywhere in ROSE, projects, tests, or
//   documentation.
//   2. The name is spelled wrong (should be "findFile")
//   3. File operations should not be in StringUtility since they have nothing
//   to do with string manipulation
//   4. Rose::FileSystem::findNames does something similar.

/** Find file names non-recursively.
 *
 *  Scans the directory named `pathString` and returns a list of files in that
 * directory which have `patternString` as a substring of their name. Note that
 * `patternString` is not a glob or regular expression.  The return value
 * strings are formed by concatenating the `pathString` and the file name with
 * an intervening slash.
 *
 *  This function does not work for non-POSIX systems. Consider using
 * std::filesystem instead, which has a directory iterator that works for
 * non-POSIX systems also. */
// ROSE_UTIL_API std::list<std::string> findfile(std::string patternString,
// std::string pathString)

/* File name location.
 *
 * Files can be classified as being in one of three locations: We don't know if
 * it's user or system It is a user (application) file It is a system library
...
public:
   ...
   * during the call.  The distance is defined as the number of cd's that only
   * move up or down one directory that it would take to move from the directory
   * of the filename to the directory that was given by appPath.  This is
   * intended as a heuristic to gage whether or not one believes that the
   * filename is related to the source (appPath) directory.  Examples:
   *
   ...
   * will be 0.  The idea being that this filename is "in" the appPath somewhere
   * and thus part of the application. */
 ...
 * code directory, return a FileNameClassification indicating whether the
 * fileName is part of the source code or some system library and automatically
 * determine the operating system from the host uname.
 *
 * @param fileName File to classify.
 * @param appPath Application source root.
 * @return Classification result. */
ROSE_UTIL_API FileNameClassification
classifyFileName(const std::string &fileName, const std::string &appPath);

/** Determine whether a file is source code or system library.
 *
 *  Given a fileName and an appPath that is a path to some application's source
 * code directory, return a FileNameClassification indicating whether the
 * fileName is part of the source code or some system library.
 *
 * @param fileName File to classify.
 * @param appPath Application source root.
 * @param os Operating system type to assume.
 * @return Classification result. */
ROSE_UTIL_API FileNameClassification classifyFileName(
    const std::string &fileName, const std::string &appPath, OSType os);

/** Determine whether a file is source code or system library.
 *
 *  Given a fileName and an appPath that is a path to some application's source
 * code directory, and a collection of library paths, return a
 * FileNameClassification indicating whether the fileName is part of the source
 * code or some system library and automatically determine the operating system
 * from the host uname.
 *
 * @param fileName File to classify.
 * @param appPath Application source root.
 ...
 * between two directories. Returns the "distance" between left and right. The
 * distance is defined as the number of cd's that only move up or down one
 * directory that it would take to move from the directory of the filename to
 * the directory that was given by appPath.  This is intended as a heuristic to
 * gage whether or not one believes that the left is related to the right
 * directory.  Examples:
 *
 *  Between /a/b/c/file.h and /a/b/d/e/ the distance is 3 because one must cd
 * ..; cd d; cd e
 ...
 * then an error message is printed to standard output (not error) and the
 ...
 * terminated abnormally, input buffer too small, etc), then an error is printed
 * to standard error (with or without line termination) and false is returned.
 * When an error occurs the pipes that were opened to communicate with the
 * subcommand might not be closed.
 *
 ...
 * strings with intervening C preprocessor #line directives. For strings that
 * have no source location, use the specified `filename` argument as the name of
 * the file. The `physicalLine` is the line number to assume for the first line
 * of output (the return value). The #line directives are not emitted into the
 * return value when they're not needed (e.g., two `strings` are at successive
 * line numbers of the same source file) or if ROSE has been configured to not
 * emit #line directivies (i.e., the
 *  `SKIP_HASH_LINE_NUMBER_DECLARATIONS_IN_GENERATED_FILES` preprocessor symbol
 * is defined). The `strings` should not have trailing linefeeds or else the
 * output will contain extra blank lines.
 *
...
ROSE_UTIL_API std::string toString(const FileWithLineNumbers &strings,
 ...
 *
 *  Appends `str` to the last string in the vector of strings with location
 * information. If the vector is empty then a new element is created.
 *
 *  The new code is marked as either generated (empty file name) or not
 * generated (non-empty name) based on whether the vector was initially empty or
 * whether the final element of the vector was generated.  Adding a string to an
 * empty vector makes it always be a generated string; adding it to a non-empty
 * vector makes it generated or not generated depending on whether the last
 * element of the vector is generated or not generated.
 *
 ...
 *
 *  Finds all occurrences of the `oldToken` string in `inputString` and replaces
 * them each with `newToken,` returning the result.
 *
 ...
 *
 * @{ */
ROSE_UTIL_API std::string copyEdit(const std::string &inputString,
                                   const std::string &oldToken,
                                   const std::string &newToken);
ROSE_UTIL_API FileWithLineNumbers
copyEdit(const FileWithLineNumbers &inputString, const std::string &oldToken,
         const std::string &newToken);
ROSE_UTIL_API FileWithLineNumbers
copyEdit(const FileWithLineNumbers &inputString, const std::string &oldToken,
