 ...
 *
 *  Replaces special characters in the input so that it is suitable for the contents of a C string literal (if `context` is a
 *  double quote character) or the contents of a C character constant (if `context` is a single quote). That is, things
 ...
 *  `std::string{"hello\\nworld\\00hidden"}` that contains 20 characters including two backslashes, the function replaces
 *  the first backslash+"n" pair with a line feed, and the second backslash+"0"+"0" with a NUL to result in
 *  `std::string{"hello\nworld\0hidden"}` (18 characters including the LF and NUL). Unicode escapes are not supported
