...
flags. Copyright (C) 1998,1999 Brian T. N. Gunney
...
SLA_FCN_BEGIN(sla_str, std::string) {
...
#endif
#endif
  // Rasmussen-2019.01.25: Removed register as it is deprecated and incompatible
  // with C++17
...
#ifdef __cplusplus
  if (debug >= 16)
    std::cout << "Forming search string." << std::endl;
#endif
#endif
...
#ifdef __cplusplus
  if (debug >= 2)
    std::cout << "flind expression: '" << flind_exp << "'" << std::endl;
#endif
#endif
  /* compile regular expression flind_re for flind */
  // printf(" flind_re: %d %s    address: %d \n",flind_re.re_magic, flind_re,
  // &flind_re); printf(" flind_exp: %s  \n",flind_exp.c_str()); printf("
  // REG_EXTENDED: %d \n",REG_EXTENDED);
  //  errf != 0;
  ...
  std::string pname_exp =
      "^" + pname.substr((!pname.empty() && pname[0] == '*'));
#ifdef SLA_DEBUG
#ifdef __cplusplus
  if (debug >= 2)
    std::cout << "pname expression: '" << pname_exp << "'" << std::endl;
#endif
#endif
  /* compile regular expression flind_re for flind */
...
#ifdef __cplusplus
  if (debug >= 2)
    std::cout << "assop expression: '" << assop_exp << "'" << std::endl;
#endif
#endif
  /* compile regular expression flind_re for flind */
...
#ifdef __cplusplus
  if (debug >= 2)
    std::cout << "sup_assop expression: '" << sup_assop_exp << "'" << std::endl;
#endif
#endif
  /* compile regular expression flind_re for flind */
  ...
  */
  if (argd == 0) { /* Determine automatically. */
    if (assop == "$")
      argd = 1; /* Delete any found. */
    else
      argd = value != NULL ? 1 : -1; /* Delete as assigned to value. */
  }
#ifdef SLA_DEBUG
#ifdef __cplusplus
  if (debug >= 16) {
  ...
  if (debug >= 4) {
    std::cout << "Found assignment instructions will "
              << (argd > 0 ? "" : "NOT ") << "be deleted\n";
  }
#endif
#endif
  ...
  for (i = 0; i < argv.size(); i++) {
...
#ifdef __cplusplus
      std::cerr << "**** Out of memory in regexec: " << __LINE__ << " of "
                << __FILE__;
#endif
      return -1;
    }
...
#ifdef __cplusplus
    if (debug >= 4)
      std::cout << "assop match length is " << assopmatchlength << std::endl;
#endif
#endif
    ...
    for (k = 0; k < assopmatchlength; k++)
      tstr[k] = *(argv[i].c_str() + offset + k);
    tstr[k] = 'x';
    tstr[k + 1] = '\0';
    pm.rm_so = pm.rm_eo = -1;
...
#ifdef __cplusplus
    if (debug >= 4)
      std::cout << "assop ends str is " << assop_ends_str << std::endl;
#endif
#endif
    DEL_MEMORY(tstr);
...
#ifdef __cplusplus
    if (debug >= 4)
      std::cout << "matched with " << argv[i] << std::endl;
#endif
#endif
    ...
    if (debug >= 4) {
      if (!assop_ends_str) { /* attached or no value */
        std::cout << "using ATtached value '" << attachedargbegin << "'"
                  << std::endl;
      } else { /* detached value */
        std::cout << "using absent or DEtached value '" << argv[i + 1] << "'"
                  << std::endl;
      }
    }
#endif
    ...
    */
    ...
    if (value) {
      if (assop_ends_str) {  /* either detached value or no value */
        ...
        value[(!pname.empty() && pname[0] == '*') ? nvalue : 0] =
            attachedargbegin;
      }
    }
...
#ifdef __cplusplus
          if (debug >= 4)
            std::cout << "Deleting " << i << ':' << argv[i] << " and " << i + 1
                      << ':' << argv[i + 1] << " from argv" << std::endl;
#endif
#endif
          argv.erase(argv.begin() + i, argv.begin() + i + 2);
          i -= 2; /* Cause the new argv[i] to be processed. */
        } else
          i++; /* skip next argv because it is an value. */
      } else { /* no value */
        if (argd >= 0) {
...
#ifdef __cplusplus
          if (debug >= 4)
            std::cout << "Deleting " << i << ':' << argv[i] << " from argv"
                      << std::endl;
#endif
#endif
...
/* Note: this function is just a matter of convenience, not truly
   ...
   */
int sla_none(std::vector<std::string> &argv, const std::string &flind,
             const std::string &assop, const std::string &pname, int argd) {
 ...
 */
#include <stdlib.h>
SLA_FCN_BEGIN(sla_float, float){SLA_CNV((float)atof)} SLA_FCN_BEGIN(sla_double,
                                                                    double){
    SLA_CNV(atof)} SLA_FCN_BEGIN(sla_int, int){
    SLA_CNV(atoi)} SLA_FCN_BEGIN(sla_short, short){
    SLA_CNV((short)atoi)} SLA_FCN_BEGIN(sla_long, long){
    SLA_CNV((long)atoi)} SLA_FCN_BEGIN(sla_uint, unsigned int){
    SLA_CNV((unsigned int)atoi)} SLA_FCN_BEGIN(sla_ushort, unsigned short){
    SLA_CNV((unsigned short)atoi)} SLA_FCN_BEGIN(sla_ulong, unsigned long){
    SLA_CNV((unsigned long)atoi)} SLA_FCN_BEGIN(sla_char, char) {
  SLA_CNV(*)
}
