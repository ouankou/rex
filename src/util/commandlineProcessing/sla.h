...
/*
$Header:
/nfs/casc/overture/ROSE/ROSE2_Repository/ROSE/src/util/commandlineProcessing/sla.h,v
...
#ifdef __cplusplus
ROSE_UTIL_API int sla_none(std::vector<std::string> &argv,
                           const std::string &flind, const std::string &assop,
                           const std::string &pname, int argd = 0);
#else
ROSE_UTIL_API int sla_none(int *argc, ARGVCONST char **argv, const char *flind,
                           const char *assop, const char *pname, int argd);
#endif
...
#define SLA_FCN_PROTO(N, T)                                                    \
  ROSE_UTIL_API int N(std::vector<std::string> &argv,                          \
                      const std::string &flind, const std::string &assop,      \
                      const std::string &pname, T *value,                      \
                      int argd = 0 /* default argument */                      \
  )
#else
#define SLA_FCN_PROTO(N, T)                                                    \
  ROSE_UTIL_API int N(int *argc, ARGVCONST char **argv, const char *flind,     \
                      const char *assop, const char *pname, T *value,          \
...
#define SLA_FCN_BEGIN(N, T)                                                    \
  ROSE_UTIL_API int N(std::vector<std::string> &argv,                          \
                      const std::string &flind, const std::string &assop,      \
                      const std::string &pname, T *value, int argd)
#else
...
#define SLA_CNV(C)                                                             \
  ...
  if (nvalue > 0 && value != NULL) {                                           \
    if (!pname.empty() && pname[0] == '*') {                                   \
      for (i = 0; i < nvalue; i++)                                             \
        value[i] = C(rr[i].c_str());                                           \
    } else {                                                                   \
      for (i = 0; i < nvalue; i++)                                             \
        value[0] = C(rr[i].c_str());                                           \
    }                                                                          \
  }                                                                            \
  return nvalue;
#else
