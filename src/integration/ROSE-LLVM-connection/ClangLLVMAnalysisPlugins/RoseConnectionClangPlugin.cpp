...
namespace {
llvm::json::Object &getOrCreateObject(llvm::json::Object &obj,
                                      llvm::StringRef key) {
  if (auto *existing = obj.getObject(key)) {
    return *existing;
  }
  std::string key_string = key.str();
  obj[key_string] = llvm::json::Object{};
  return *obj.getObject(key_string);
}

void setJsonValue(llvm::json::Object &root, llvm::StringRef key,
                  llvm::json::Value value) {
  llvm::json::Object *current = &root;
  while (true) {
    auto parts = key.split('.');
    llvm::StringRef head = parts.first;
    llvm::StringRef tail = parts.second;
    if (tail.empty()) {
      (*current)[head.str()] = std::move(value);
      return;
    }
    current = &getOrCreateObject(*current, head);
    key = tail;
  }
}
} // namespace
...
//    llvm::outs() << "Constructing in " << srcFile << "\n";
  ...
  void dumpLocation(std::string prefix,bool isbegin, SourceLocation Loc) {
    ...
    if (srcFile.compare(PLoc.getFilename()) == 0) {
      ...
      setJsonValue(pt, jsonval + "Line",
                   static_cast<int64_t>(PLoc.getLine()));
      setJsonValue(pt, jsonval + "Column",
                   static_cast<int64_t>(PLoc.getColumn()));
    }
  }
...
//    llvm::outs() << "Processing in " << srcFile << "\n";
    ...
    if(srcFile.compare(filename.str()) == 0)
    {
       ...
       setJsonValue(pt, jsonval, "Decl");
       jsonval = address+".ASTname";
       setJsonValue(pt, jsonval, d->getDeclKindName());
       jsonval = address+".filename";
       setJsonValue(pt, jsonval, filename.str());
    ...
    if(isa<DeclaratorDecl>(d))
    {
      ...
      {
        jsonval = address+".Type";
        setJsonValue(pt, jsonval, "Type");
        jsonval = address+".ASTname";
        setJsonValue(pt, jsonval, type->getTypeClassName());
        jsonval = address+".filename";
        setJsonValue(pt, jsonval, filename.str());
  ...
  bool VisitStmt(Stmt *s) {
    ...
    {
       jsonval = address+".Type";
       setJsonValue(pt, jsonval, "Stmt");
       jsonval = address+".ASTname";
       setJsonValue(pt, jsonval, s->getStmtClassName());
       jsonval = address+".filename";
       setJsonValue(pt, jsonval, filename.str());
...
*/
 ...
 virtual void HandleTranslationUnit(clang::ASTContext &Context) override{
   ...
   llvm::raw_os_ostream os(jfile);
   os << llvm::formatv("{0:2}", llvm::json::Value(pt));
   os.flush();
 }
