...
1. Building ROSE
  - Configure REX with Clang and Clang++
  - Example config command: ${ROSE_SRC}/configure --prefix=${ROSE_INSTALL_TREE} --enable-languages=c,c++ --with-CXX_DEBUG=-g --with-C_OPTIMIZE=-O0 --with-CXX_OPTIMIZE=-O0 CC=${PATH_TO_CLANG} CXX=${PATH_TO_CLANG++}
2. Building Plugins 
  - mkdir build; cd build
  - cmake -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DROSE_ROOT=${ROSE_INSTALL_TREE} ${PATH_TO_ROSE-LLVM-CONNECTION-SRC}
  - make
