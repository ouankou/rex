...
void SgCompoundInitializer::append_initializer(SgExpression *what) {
  assert(this != NULL);
...
SgExpression *SgCompoundInitializer::get_next(int &n) const {
  if (n == 0) {
    n++;
...
int SgCompoundInitializer::replace_expression(SgExpression *o,
                                              SgExpression *n) {
...
SgType *SgCompoundInitializer::get_type() const {
