...
SgType *SgFunctionCallExp::get_type() const {
  ...
  if (functionType == NULL) {
    ...
    // Milind Chabbi, after discussing with DQ (7/29/2013), we need to strip
    // all. See test: CompileTests/Cxx_tests/test2013_milind_01.C
    likelyFunctionType = likelyFunctionType->stripType(
