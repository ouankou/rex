...
bool SgCastExp::cast_looses_precision() {
  // DQ (10/24/2014): Added support to determine when cast should be allowable.
  // This is used in the SageInterace support for evaluation of constant
  // expressions.

  bool result = false;
  if (isSgTypeDouble(p_expression_type) != NULL &&
      isSgTypeFloat(p_expression_type) != NULL) {
    ...
    if (isSgTypeInt(operantType) != NULL || isSgTypeLong(operantType) != NULL ||
        isSgTypeShort(operantType) != NULL ||
        isSgTypeChar(operantType) != NULL ||
        isSgTypeUnsignedInt(operantType) != NULL ||
        isSgTypeUnsignedLong(operantType) != NULL ||
        isSgTypeUnsignedShort(operantType) != NULL ||
        isSgTypeUnsignedChar(operantType) != NULL ||
        isSgTypeSignedInt(operantType) != NULL ||
        isSgTypeSignedLong(operantType) != NULL ||
        isSgTypeSignedShort(operantType) != NULL ||
        isSgTypeSignedChar(operantType) != NULL) {
      result = true;
    }
  }
