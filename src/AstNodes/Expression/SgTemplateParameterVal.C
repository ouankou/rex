...
SgName SgTemplateParameterVal::get_template_parameter_name() const {
  ...
  // DQ (7/25/2012): This reflects the new design of how templates are supported
  // in ROSE.
  SgDeclarationStatement *templateDeclaration =
      SageInterface::getTemplateDeclaration(this);
  if (templateDeclaration == NULL) {
    printf("Error: In SgTemplateParameterVal::get_template_parameter_name(): "
           "templateDeclaration == NULL: this = %p = %s \n",
           this, this->class_name().c_str());

    // DQ (6/6/2013): If the template parameter is used as a template argument,
    // then we just return a simple name. Nowever, maybe the constant should
    // have been shared so that it would be correctly associated with a template
    // declaration.
  ...
  case V_SgTemplateDeclaration:
    templateParameters = &(
        isSgTemplateDeclaration(templateDeclaration)->get_templateParameters());
    break;
  case V_SgTemplateClassDeclaration:
    templateParameters = &(isSgTemplateClassDeclaration(templateDeclaration)
                               ->get_templateParameters());
    break;

    // DQ (2/1/2019): Bug fix, likely copy-paste error, casting to wrong type.
    // case V_SgTemplateFunctionDeclaration:       templateParameters =
    // &(isSgTemplateMemberFunctionDeclaration(templateDeclaration)->get_templateParameters());
    // break;
  case V_SgTemplateFunctionDeclaration:
    templateParameters = &(isSgTemplateFunctionDeclaration(templateDeclaration)
                               ->get_templateParameters());
    break;

  case V_SgTemplateMemberFunctionDeclaration:
    templateParameters =
        &(isSgTemplateMemberFunctionDeclaration(templateDeclaration)
              ->get_templateParameters());
    break;
  case V_SgTemplateVariableDeclaration:
    templateParameters = &(isSgTemplateVariableDeclaration(templateDeclaration)
                               ->get_templateParameters());
    break;

  default: {
    printf("Error: default reached in "
           "SgTemplateParameterVal::get_template_parameter_name(): "
           "templateDeclaration = %p = %s \n",
  ...
  // DQ (12/13/2011): this is a problem demonstrated by test2011_186.C, so we
  // have to disable the template parameter position error checking.
  ...
  if (getParameterNameFromTemplateParameterList == true) {
    ...
    if (template_parameter_position >= 0) {
      ...
      ROSE_ASSERT(templateParameter->get_parameterType() ==
    ...
    // This is not the favored case since i think it can be problematic to use
