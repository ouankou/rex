...
SgType *SgPntrArrRefExp::get_type() const {
  ...
  // array being referenced (lhs->get_type()). DQ (1/14/2006): p_expression_type
  ...
  } else if (isArraySubRange == false && lhsBase == nullptr &&
             rhsBase != nullptr) {
...
keepStripping:
  ...
  case V_SgArrayType: {
    returnType =
        isArraySubRange ? someType : isSgArrayType(someType)->get_base_type();
    break;
  }
  ...
  case V_SgTypeFloat: {
    ...
    // DQ (10/3/2006): Added case of array reference of string type (assume it
    // returns a char). Once we have a wcharString type then we have to add that
    // case to this list as well!
  case V_SgTypeString: {
