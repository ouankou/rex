...
SgType *SgPointerDerefExp::get_type() const {
  ...
  // appropriate type (IR specific code) DQ (7/28/2006): This code can't be used
  // for both the SgPointerDerefExp and the SgPntrArrRefExp IR nodes because it
  // is a bit different (one node is a unary operator and the other is a binary
  // operator). This function returns the base type of the type returned from
  // get_operand()->get_type() (which should be a SgPointerType).
  ...
  // This code should be shared between the SgPntrArrRefExp and the
  // SgPointerDerefExp IR nodes A better idea would be to have a function that
  // strips off types based on a set of flags that would control stripping of
  // pointer references, array references, C++ references, modifiers, and
  // typedefs.
...
keepStripping:
  ...
  switch (someType->variantT()) {
    // These cases all have base types and the pointer dereferencing returns the
    // base type But since p_base_type is placed at each of the different SgType
    // IR nodes we have to handle each case separately.
  case V_SgPointerType: {
    SgPointerType *pointerType = isSgPointerType(someType);
    returnType = pointerType->get_base_type();

    // DQ (6/11/2014): We also need to take any associated SgModifierType node
    // chains used to modify the SgPointerType (e.g. defining const).
  ...
  case V_SgRvalueReferenceType: {
    SgRvalueReferenceType *rvalueReferenceType =
        isSgRvalueReferenceType(someType);
    // returnType = rvalueReferenceType;
    someType = rvalueReferenceType->get_base_type();
    goto keepStripping;
  }

    // DQ (9/27/2012): Added case to avoid falling through the default case
    // (more template support).
  case V_SgTemplateType: {
    // DQ (9/27/2012): I think that this is the correct handling of the return
    // value generation.
    returnType = someType;
    break;
  }

    // DQ (9/27/2012): The dereference of a function type is a function call so
    // the type is it's return type (I think)
