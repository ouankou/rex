...
SgType *SgStatementExpression::get_type() const {
  ...
  // The rules are: The last thing in the compound statement should be an
  // expression followed by a semicolon; the value of this subexpression serves
  // as the value of the entire construct. (If you use some other kind of
  // statement last within the braces, the construct has type void, and thus
  // effectively no value.).

  // printf ("SgStatementExpression::get_type() for GNU Statement Expression
  // extension not implemented p_statement = %p = %s
  // \n",p_statement,p_statement->class_name().c_str());

  // DQ (8/6/2006): The rules for the type of a statement expression are that it
  // is the type of the last expression in the statement list or void if the
  // last statement is not an expression (expression statement,
  ...
  if (statementList.empty() == true) {
    // DQ (4/8/2015): After discusion, this might be better setup as a
    // SgTypeUnknown instead of SgTypeVoid.
    ...
    if (expressionStatement != NULL) {
      ...
      // This is the default type if last statement in block is not an
      // expression statement (C++ standard)
      returnType = SgTypeVoid::createType();
    }
  }
