...
SgType *SgBinaryOp::get_type() const {
  ...
  if (p_expression_type != NULL) {
    ...
    printf("Found an case where SgBinaryOp::p_expression_type != NULL on %s "
           "expression_type_name = %s \n",
...
int SgBinaryOp::replace_expression(SgExpression *o, SgExpression *n) {
  ...
  if (get_lhs_operand() == o) {
    ...
    printf("Warning: inside of SgBinaryOp::replace_expression original "
