...
SgType *SgArrowStarOp::get_type() const {
  // DQ (1/14/2006): p_expression_type has been removed, we have to compute the
  // appropriate type (IR specific code) For the SgArrowStarOp, the type is the
  // type of the rhs operand (e.g. "A->*x" where "x" is a member function
  // pointer of the pointer to the class "A").
...
keepStripping:
  ...
  default: {
    ...
    // DQ (1/19/2017): I think we want to fix this similarly to the case of
    // SgDotStarExp::get_type() (above)
    returnType = someType;
#else
