...
* The `Rosebud::tag` attribute takes a single argument which is a
  ...
  tag is the type name without the leading "Sg" and with a "Tag"
...
7. Rosebud is intended to be modular rather than monolithic. The core
   tool can emit a machine-readable form of the C++-like input that
...
8. Rosebud does various style checks such as checking for the presence
   ...
   that operate on the machine-readable output.
