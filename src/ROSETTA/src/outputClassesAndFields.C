...
string
// Grammar::outputClassesAndFields ( GrammarTreeNode & node, fstream &
// outputFile )
Grammar::outputClassesAndFields(AstNodeClass &node) {
  ...
  // returnString =
  // GrammarString::copyEdit(returnString.c_str(),"$CLASSNAME",node.getName());
  const string target = "$CLASSNAME";
  const string name = node.getName();
  // returnString = GrammarString::copyEdit(returnString,target,name);
  // returnString =
  // GrammarString::copyEdit(string(returnString),"$CLASSNAME",string(node.getName()));
...
string AstNodeClass::outputClassesAndFields() {
  string returnString;
  ...
  for (stringListIterator = localList.begin();
    ...
    string tempString =
        "-- data member -- " + typeName + " p_" + variableNameString + "\n";
...
string AstNodeClass::outputFields() {
  string returnString;
  vector<GrammarString *> localList;
  vector<GrammarString *> localExcludeList;
  vector<GrammarString *>::iterator stringListIterator;
