...
public:
  ...
  std::string getFunctionNameStringTestAgainstExclusions(
      AstNodeClass &node, std::vector<GrammarString *> &,
  ...
  // DQ (9/24/2005): Build the copy mechanism's source code to set the parents
  // of child nodes DQ (9/28/2022): Fixing compiler warning for argument not
  // used. std::string buildCopyMemberFunctionSetParentSource ( std::string
  // copyString );
  ...
  static std::string copyEdit(const std::string &inputString,
                              const std::string &oldToken,
                              const std::string &newToken);
  static inline Rose::StringUtility::FileWithLineNumbers
  copyEdit(const Rose::StringUtility::FileWithLineNumbers &inputString,
           const std::string &oldToken, const std::string &newToken) {
    return Rose::StringUtility::copyEdit(inputString, oldToken, newToken);
  }
  static inline Rose::StringUtility::FileWithLineNumbers
  copyEdit(const Rose::StringUtility::FileWithLineNumbers &inputString,
           const std::string &oldToken,
           const Rose::StringUtility::FileWithLineNumbers &newToken) {
    return Rose::StringUtility::copyEdit(inputString, oldToken, newToken);
  }
  static bool isContainedIn(const std::string &longString,
                            const std::string &shortString);
  // A smarter string concatination function (corrects for space in the target
  // automatically)
  static std::string stringConcatenate(const std::string &target,
                                       const std::string &endingString);

  // DQ (3/22/2017): Added to support use of override keyword to remove warnings
  // from Clang.
