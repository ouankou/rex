...
class AstNodeClass {
  // Later we will template this class to permit it to refer to an arbitrary
  // lower level grammar

  // Terminals for the basis for the definition of a grammar that defines a
  // language This class represents a AstNodeClass (the NonTerminal class is
  // derived from the AstNodeClass class)
...
public:
  ...
  std::string
      name; // The name we assign to the token (e.g. "BinaryOperatorEquals")

  // where the name of the AstNodeClass is saved unmodified (with no grammar
  // suffix)
  std::string baseName;

  // AstNodeClass/nonterminal for the base class of this AstNodeClass (set in
  // the NonTerminal constructor)
  ...
  // The lexeme is used to build up the character string as the different
  // operators are evaluated.  The when the operator| is called the lexeme is
  // placed into the name (effectively naming the nonterminal).
  std::string lexeme; // The actual character string (e.g. "operator=")

  // Tag used within enum declaration (to that each AstNodeClass can be assigned
  // an ID
  std::string tag;

  // Relationship in lower level grammar (value == NULL only if a part of the
  // C++ grammar) This needs to be recorded into the classes that are build as
  // well (I think) We want to automatically set these (not sure if we can!)
  ...
  // Header file class definition function prototypes (local and subtree
  // definitions) Use an array of lists since every string has a local
  // node/subtree representation and a local node/subtree exclude list
  std::vector<GrammarString *> memberFunctionPrototypeList[2][2];

  // Header file class definition data declarations (local and subtree
  // definitions)
  ...
  // String representation of '#if' that surrounds the class. If empty, then
  // emits "#if 1"
  std::string cppCondition;

  // Content that should appear before the start of the class definition, such
  // as CPP include directives
  std::string preDefinitionText;

  // A derived grammar node can narrow the traversal cardinality of an inherited
  // scalar edge without redeclaring the C++ data member.  This is part of the
  // typed schema: it is not a code-generation exception keyed on a node name.
  std::map<std::string, TraversalCardinalityEnum> traversalCardinalityOverrides;

  enum locationInTree { LOCAL_LIST = 0, SUBTREE_LIST = 1 };

  enum includeExcludeMechanism { INCLUDE_LIST = 0, EXCLUDE_LIST = 1 };

  // Access function for list objects
  std::vector<GrammarString *> &getMemberFunctionPrototypeList(int i,
                                                               int j) const;
  ...
  // Add additional derived class. The overload that takes a pointer had better
  // not be a reference to a local variable! This is par for the ROSETTA code
  // quality--there are other places that do similar things.
  void insertDerivedClass(AstNodeClass *);
  void insertDerivedClass(std::vector<AstNodeClass *> &terminalList,
  ...
  void setFunctionPrototype(const std::string &markerString,
                            const std::string &filename,
                            bool pureVirtual = false);

  void setSubTreeFunctionPrototype(const GrammarString &memberFunction);
  void setSubTreeFunctionPrototype(const std::string &markerString,
                                   const std::string &filename,
                                   bool pureVirtual = false);

  void setFunctionSource(const std::string &markerString,
                         const std::string &filename, bool pureVirtual = false);
  void setSubTreeFunctionSource(const std::string &markerString,
                                const std::string &filename,
                                bool pureVirtual = false);
  ...
  void excludeFunctionPrototype(const std::string &markerString,
                                const std::string &filename,
                                bool pureVirtual = false);
  void excludeSubTreeFunctionPrototype(const GrammarString &memberFunction);
  void excludeSubTreeFunctionPrototype(const std::string &markerString,
                                       const std::string &filename,
                                       bool pureVirtual = false);

  void excludeFunctionSource(const std::string &markerString,
                             const std::string &filename,
                             bool pureVirtual = false);
  void excludeSubTreeFunctionSource(const std::string &markerString,
                                    const std::string &filename,
                                    bool pureVirtual = false);
...
protected:
  AstNodeClass();
  std::unique_ptr<GrammarString> setupMarkerStrings(std::string markerString,
                                                    std::string fileName,
                                                    bool pureVirtual);

public:
  AstNodeClass(const std::string &lexeme, Grammar &X,
               const std::string &stringVar, const std::string &tagString,
               bool canHaveInstances = true,
               const SubclassListBuilder &childList = SubclassListBuilder());
  ...
  void setName(const std::string &label = "defaultName",
  ...
  // std::string buildConstructorBody( bool withInitializers, ConstructParamEnum
  // config );
  ...
  void editSubstitute(const std::string &oldString,
                      const std::string &newString);
  void editSubstituteSubTree(const std::string &oldString,
                             const std::string &newString);
  void editSubstituteExclude(const std::string &oldString,
                             const std::string &newString);
  void editSubstituteExcludeSubTree(const std::string &oldString,
                                    const std::string &newString);
  ...
  void setPredeclarationString(const std::string &markerString,
                               const std::string &filename);
  void setPostdeclarationString(const std::string &markerString,
                                const std::string &filename);
  ...
  // Access function for automaticGenerationOfDestructor and
  // automaticGenerationOfConstructor
  ...
  // AS (2/18/06): Added support for building code to return data member
  // pointers to IR nodes
  std::string buildReturnDataMemberPointers();
  std::string buildListIteratorString(std::string typeName,
                                      std::string variableName,
                                      std::string classNameString);

  // DQ (4/30/2006): Added similar function to buildReturnDataMemberPointers()
  // so that we could return references to the pointers to IR nodes (required
  // low level support for AST Merge).
  std::string buildProcessDataMemberReferenceToPointers();
  // DQ (9/28/2022): Fixing compiler warning for argument not used.
  // std::string buildListIteratorStringForReferenceToPointers(std::string
  // typeName, std::string variableName, std::string classNameString, bool
  // traverse);
  std::string buildListIteratorStringForReferenceToPointers(
      std::string typeName, std::string variableName, bool traverse);
  ...
  /* JH (21/01/20005): Since in every method for building the source code for
     the StorageClases we need to build specific code for the data members of
     IRNode classes, we list all cases that we have to separate. Some of them
     can be removed, when ROSE does not longer contain pointers to stl vectors
     or lists. They are marked below, but may cause no trouble if they remain.
  */
  ...
  // JH (21/01/20005): method that evaluates the type string of a data member
  // and returns the corresponding TypeEvaluation type DQ (4/6/2006): Modified
  // the function parameter as per Jochen's version
  TypeEvaluation evaluateType(std::string &varTypeString);

  // DQ (10/12/2014): output the name associated with the TypeEvaluation enum
  // values.
  ...
  /* JH (10/28/2005): declaration of the source building methods for the storage
     classes concenrning the ast file IO. More about them one can find in the
     AstNodeClass.C file. They are called while the code generation of ROSETTA
     by the method 'buildStringForStorageClassSource' in the file grammar.C
  */
