...
Grammar::setUpExpressions () {
  ...
  // SIMD operators
    ...
    // These are required to support builtin type-trait functions used by newer compilers and some STL code. These builtin
    // functions take types as parameters and sometimes return types as well. They require an implementation in ROSE to
    // support analysis.
    ...
    NEW_NONTERMINAL_MACRO (ValueExp,
     ...
     NEW_NONTERMINAL_MACRO (Expression,
                            ...
                            SuperExp                 | TypeIdOp                 | ConditionalExp          | NewExp              | DeleteExp                |
                            ThisExp                  | RefExp                   | Initializer             | VarArgStartOp       | VarArgOp                 |
                            VarArgEndOp              | VarArgCopyOp             | VarArgStartOneOperandOp | NullExpression      | VariantExpression        |
                            SubscriptExpression      | ColonShapeExp            | AsteriskShapeExp        | /*UseOnlyExpression |*/ ImpliedDo               |
                            IOItemExpression         | /* UseRenameExpression   | */ StatementExpression | AsmOp                |
                            LabelRefExp              | ActualArgumentExpression | UnknownArrayOrFunctionReference               | PseudoDestructorRefExp |
                            CAFCoExpression          | CudaKernelExecConfig     | /* TV (04/22/2010): CUDA support */
                            LambdaRefExp             | TemplateFunctionRefExp   | TemplateMemberFunctionRefExp | AlignOfOp            | RangeExp                 |
                            TypeTraitBuiltinOperator | CompoundLiteralExp       | TypeExpression          | ClassExp                 |
                            FunctionParameterRefExp  | LambdaExp                | NoexceptOp              | NonrealRefExp             |
