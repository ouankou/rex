...
string localOutputVisitorSupport(string name) {
  ...
  // DXN (08/27/2010): instead of do nothing, call visitDefault() which does
  // nothing.
...
string localOutputVisitorParentSupport(string name) {
  ...
  s += ") { visit(static_cast<" + name + "::base_node_type *>(variable_" +
       name + ")); }\n";
...
string Grammar::buildVisitorBaseClass() {
  ...
  s += "/** Strategy pattern applied to visitors: serves as a context to a "
       "strategy,\n";
  s += " * which itself is a visitor; delegates all requests to the current "
       "strategy;\n";
  s += " * capable of switching strategy at will.  Derived classes may declare "
       "the\n";
  s += " * known strategies as friend to facilitate communication between the "
       "strategies\n";
  s += " * and their context.\n";
  ...
  s += "    void setStrategy(ROSE_VisitorPattern* strategy)  {_strategy = "
       "strategy; }\n\n";
  for (size_t i = 0; i < terminalList.size(); i++) {
    string name = terminalList[i]->name;
