...
// MS 2015: rewrote ROSETTA to use enums instead of wrapper classes around
// enums. enums: ConstructParamEnum, BuildAccessEnum, CopyConfigEnum
enum ConstructParamEnum { NO_CONSTRUCTOR_PARAMETER, CONSTRUCTOR_PARAMETER };
...
enum CopyConfigEnum {
  // Note that CLONE_TREE is the default if nothing is specified in the
  // setDataPrototype() member function.
  ...
  */
  ...
  /* This builds a new object dependent on the use of the
     SgCopyHelp input object (deep or shallow) using the object's copy member
     function
  */
  CLONE_TREE
};
...
AstNodeClass *lookupTerminal(const std::vector<AstNodeClass *> &tl,
                             const std::string &name);

class Grammar {
  // This class defines a language grammar and pemits it to be input in a form
  // similar to BNF notation. Later we will template this class to permit it to
  // refer to an arbitrary lower level grammar.
public:
  // Pointer to member function
  // typedef List<GrammarString> & (AstNodeClass::*FunctionPointerType)(int
  // i,int j) const;
  typedef std::vector<GrammarString *> &(AstNodeClass::*FunctionPointerType)(
      int i, int j) const;

  // Pointer to member function
  typedef std::string (
      GrammarString::*StringGeneratorFunctionPointerType)() const;

  Grammar(const Grammar &X);

  /**! The constructor builds a grammar.  The inputs are strings:
       inputGrammarName          -- The name of the grammar (used in the
     construction of filenames etc.) inputPrefixName           -- The name used
     for the prefix to all classes generated to implement the grammar.  This
     permits each class to have a unique name.  The prefix for the base level
     grammar should be "Sg" to match SAGE! inputGrammarNameBaseClass -- The name
     of the class representing the grammar!  There can be many grammars defined
     within a preprocessor. inputParentGrammar        -- A pointer to the parent
     grammar (object) representing the base grammar.
   */
  Grammar(const std::string &inputGrammarName,
          const std::string &inputPrefixName,
          const std::string &inputGrammarNameBaseClass,
          const Grammar *parentGrammar, const std::string &t_directory,
...
public:
  ...
  // This is a pointer to the lower level grammar (every grammar has a lower
  // level grammar except the C++ grammar)
  const Grammar *parentGrammar;

  // filename of file containing declaration and source code for a grammar's
  // supporting classes
  ...
  void
  generateStringListsFromSubtreeLists(AstNodeClass &node,
                                      std::vector<GrammarString *> &,
                                      std::vector<GrammarString *> &excludeList,
                                      FunctionPointerType listFunction);

  void
  generateStringListsFromLocalLists(AstNodeClass &node,
                                    std::vector<GrammarString *> &,
                                    std::vector<GrammarString *> &excludeList,
                                    FunctionPointerType listFunction);

  void
  generateStringListsFromAllLists(AstNodeClass &node,
...
public:
  ...
  // names are different for the "$GRAMMAR_PREFIX_" and the
  // "$GRAMMAR_TAG_PREFIX_")
  ...
  void buildGrammarClassDeclaration(
      Rose::StringUtility::FileWithLineNumbers &outputFile);

  void addGrammarElement(AstNodeClass &X);

  // Check if the AstNodeClass or nonterminal (identified by a string) is a
  // AstNodeClass or nonterminal
  bool isTerminal(const std::string &terminalName) const;

  // This is the mechanism for adding new types to an existing grammar. This
  // function will more generally show how new terminals are added to an
  // existing grammar.  It will be generalized later to the addition of
  // nonterminals (implying the addition of subtrees of grammar being added to
  // an existing grammar).  The later use of this will be important for the
  // addition of the array type system (and then Overture and Kull)
  //   -->  (realArray:doubleArray,floatArray; intArray, Indexing: Index,Range).
  // The SgProject contains the declarations associated with the AstNodeClass.
  // In the case where this this is a class, member functions of the class will
  // define additional terminals to be added to the grammar.
...
public:
  ...
  buildHeaderStringAfterMarker(const std::string &marker,
                               const std::string &fileName);
  Rose::StringUtility::FileWithLineNumbers
  buildHeaderStringBeforeMarker(const std::string &marker,
                                const std::string &fileName);

  static Rose::StringUtility::FileWithLineNumbers
  readFileWithPos(const std::string &inputFileName);

  void writeFile(const Rose::StringUtility::FileWithLineNumbers &outputString,
                 const std::string &directoryName, const std::string &className,
                 const std::string &fileExtension);
  // DQ (12/31/2009): Added mechanism to append generated text to files.
  void appendFile(const Rose::StringUtility::FileWithLineNumbers &outputString,
  ...
  void buildRTIFile(AstNodeClass *rootNode,
                    Rose::StringUtility::FileWithLineNumbers &rttiFile);
  ...
  void buildVariantsStringPrototype(
      Rose::StringUtility::FileWithLineNumbers &outputFile);
  void buildVariantsStringDataBase(
      Rose::StringUtility::FileWithLineNumbers &outputFile);

  void buildClassDefinition(AstNodeClass &,
                            Rose::StringUtility::FileWithLineNumbers &);
  void buildHeaderFiles(AstNodeClass &node,
                        Rose::StringUtility::FileWithLineNumbers &outputFile);

  std::string buildStringForPrototypes(AstNodeClass &node);
  Rose::StringUtility::FileWithLineNumbers
  buildStringForDataDeclaration(AstNodeClass &node);
  Rose::StringUtility::FileWithLineNumbers
  buildStringForDataAccessFunctionDeclaration(AstNodeClass &node);
  Rose::StringUtility::FileWithLineNumbers
  buildStringForSource(AstNodeClass &node);

  // Builds the "CLASSNAME::variant()" member function
  // DQ (9/28/2022): Fixing compiler warning for argument not used.
  // Rose::StringUtility::FileWithLineNumbers
  // buildStringForVariantFunctionSource               ( AstNodeClass & node );
  Rose::StringUtility::FileWithLineNumbers
  ...
  supportForBuildStringForIsClassNameFunctionSource(
      AstNodeClass &node,
      const Rose::StringUtility::FileWithLineNumbers &accumulationString);
  Rose::StringUtility::FileWithLineNumbers
  buildStringForIsClassNameFunctionSource(AstNodeClass &node);

  // DQ (9/21/2005): Added support for memory pools
  Rose::StringUtility::FileWithLineNumbers
  buildStringForNewAndDeleteOperatorSource(AstNodeClass &node);

  // DQ (12/24/2005): Support for memory pool traversal
  Rose::StringUtility::FileWithLineNumbers
  buildStringForTraverseMemoryPoolSource(AstNodeClass &node);

  // DQ & JH (1/17/2006): Added support for building code to check pointers to
  // IR nodes
  Rose::StringUtility::FileWithLineNumbers
  buildStringForCheckingIfDataMembersAreInMemoryPoolSource(AstNodeClass &node);
  Rose::StringUtility::FileWithLineNumbers
  buildStringToTestPointerForContainmentInMemoryPoolSource(AstNodeClass &node);

  // AS (2/14/06): Added support for building code to return data member
  // pointers to IR nodes
  Rose::StringUtility::FileWithLineNumbers
  buildStringForReturnDataMemberPointersSource(AstNodeClass &node);
  ...
  bool buildConstructorParameterList(
      AstNodeClass &node,
      std::vector<GrammarString *> &constructorParameterList);

  // DQ (9/28/2022): Fixing compiler warning for argument not used.
  // std::string buildConstructorParameterListString ( AstNodeClass & node, bool
  // withInitializers, bool withTypes, ConstructParamEnum config, bool *complete
  // = 0 );
  std::string buildConstructorParameterListString(AstNodeClass &node,
                                                  bool withInitializers,
                                                  bool withTypes,
                                                  bool *complete = 0);
  std::string buildConstructorParameterListStringForEssentialDataMembers(
      AstNodeClass &node, bool withInitializers);

  // DQ 11/6/2006): Support for building newer from of constructors (withouth
  // source position information).
  void
  markNodeForConstructorWithoutSourcePositionInformation(AstNodeClass &node);
  void markNodeForConstructorWithoutSourcePositionInformationSupport(
      AstNodeClass &node);
  Rose::StringUtility::FileWithLineNumbers
  buildConstructorWithoutSourcePositionInformation(AstNodeClass &node);
  void buildConstructorWithoutSourcePositionInformationSupport(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);
  void
  constructorLoopBody(const ConstructParamEnum &config, bool &complete,
                      const Rose::StringUtility::FileWithLineNumbers
                          &constructorSourceCodeTemplate,
                      AstNodeClass &node,
                      Rose::StringUtility::FileWithLineNumbers &returnString);

  // DQ (11/7/2006): Get a specific node from the tree.
  AstNodeClass *getNamedNode(AstNodeClass &node, const std::string &name);
  // DQ (9/28/2022): Fixing compiler warning for argument not used.
  // GrammarString* getNamedDataMember ( AstNodeClass & node, const std::string
  // & name );
  ...
  // access functions at the top.  This permist us to present the generated
  // documentation better. string
  // buildDataPrototypesAndAccessFunctionPrototypesAndConstuctorPrototype (
  // GrammarTreeNode & node );
  Rose::StringUtility::FileWithLineNumbers
  buildDataMemberVariableDeclarations(AstNodeClass &node);
  Rose::StringUtility::FileWithLineNumbers
  ...
  Rose::StringUtility::FileWithLineNumbers
  buildConstructorForEssentialDataMembers(AstNodeClass &node);

  Rose::StringUtility::FileWithLineNumbers
  buildCopyMemberFunctionSource(AstNodeClass &node);

  void buildSourceFiles(AstNodeClass &node,
                        Rose::StringUtility::FileWithLineNumbers &outputFile);

  // DQ (12/23/2005): Added to support moving the new and elete operators to a
  // different file (mostly for clarity to support users who read the source
  // code).
  void buildNewAndDeleteOperators(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);

  // DQ (12/24/2005): Support for memory pool traversal
  void buildTraverseMemoryPoolSupport(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);

  // DQ & JH (1/17/2006): Added support for building code to check pointers to
  // IR nodes
  void buildStringForCheckingIfDataMembersAreInMemoryPoolSupport(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);

  // AS(2/14/2006): Added support for building code to return pointers to IR
  // nodes
  void buildStringForReturnDataMemberPointersSupport(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);

  // DQ (4/30/2006): Support for
  // buildStringForReturnDataMemberReferenceToPointersSource JJW (11/1/2008):
  // Changed to process rather than return the references
  void buildStringForProcessDataMemberReferenceToPointersSupport(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);
  ...
  // substitutions
  Rose::StringUtility::FileWithLineNumbers
  editSubstitution(AstNodeClass &node,
                   const Rose::StringUtility::FileWithLineNumbers &editString);

  // This calls the functions to build the parsers (one for each child of the
  // root node)
  void buildGrammarClassSourceCode(
      Rose::StringUtility::FileWithLineNumbers &outputFile);

  // DQ (9/28/2022): Fixing compiler warning for argument not used.
  // static Rose::StringUtility::FileWithLineNumbers extractStringFromFile (
  // const std::string& startMarker, const std::string& endMarker,
  //                                            const std::string& filename,
  //                                            const std::string& directory );
  static Rose::StringUtility::FileWithLineNumbers
  extractStringFromFile(const std::string &startMarker,
  ...
  AstNodeClass &nonTerminalConstructor(const std::string &lexeme, Grammar &X,
                                       const std::string &stringVar,
                                       const std::string &tagString,
                                       const SubclassListBuilder &builder,
                                       bool canHaveInstances = false);
  AstNodeClass &terminalConstructor(const std::string &lexeme, Grammar &X,
                                    const std::string &stringVar = "",
                                    const std::string &tagString = "");
  ...
  // a more general way to traverse the Grammar and build strings for source
  // code generation
  ...
  typedef std::string (Grammar::*evaluateStringAttributeFunctionType)(
      AstNodeClass &, std::string);
  ...
  BottomUpProcessing(AstNodeClass *node,
  ...
  void buildAbstractTreeGrammarFile(AstNodeClass *rootNode,
                                    std::ostream &AbstractTreeGrammarFile);
  void buildSDFTreeGrammarFile(AstNodeClass *rootNode,
                               std::ostream &RoseTreeGrammarFile);

  // MS: generate source for implementation of the RTI interface
  GrammarSynthesizedAttribute
  generateRTIImplementation(AstNodeClass *grammarnode,
                            std::vector<GrammarSynthesizedAttribute> v);
  ...
  // MS: auxiliary function to make tests on GrammarNodes more compact. (should
  // be made a set of member
  //     functions of GrammarNode at some point (requires many other functions
  //     to be moved as well)
  GrammarNodeInfo getGrammarNodeInfo(AstNodeClass *grammarnode);

  // MS: generates the code to implement the creation of the
  // treeTraversalSuccessorContainer in Sage
  void buildTreeTraversalFunctions(
      AstNodeClass &node, Rose::StringUtility::FileWithLineNumbers &outputFile);

  // MS: create source code for enums used in traversal functions to access
  ...
  void buildClassHierarchyCastTable(
      AstNodeClass *astNodeClass,
      std::vector<AstNodeClass *> &myParentsDescendents);
  ...
  // AS: build the funtion to automatically generate the memory pool based
  // traversal on VariantVectors
  std::string buildMemoryPoolBasedVariantVectorTraversalSupport();

  // MS: build VariantEnumnames
  std::string buildVariantEnumNames();

  // MS: creates Include list (used by buildImplementationForTerminal and
  // travMemberAccessEnums)
  std::vector<GrammarString *> classMemberIncludeList(AstNodeClass &node);

  // MK: This member function is used by the member function
  // buildTreeTraversalFunctions() (see above) in order to determine if the
...
private:
  ...
  std::string restrictedTypeStringOfGrammarString(
      GrammarString *gs, AstNodeClass *grammarnode,
