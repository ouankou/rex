...
// Support for ROSE tree traversal type traversal (simpler then classic visitor
// pattern)
...
string Grammar::buildMemoryPoolBasedTraversalSupport() {
  ...
  string s = string("\n\nvoid traverseMemoryPoolVisitorPattern ( "
                    "ROSE_VisitorPattern & visitor )\n   {\n");
  ...
  // Using this traversal only static member functions of the IR nodes may be
  // called (or any global function).  We don't traverse all the instances of
  // the IR nodes.
  s += string("\n\nvoid traverseRepresentativeNodes ( ROSE_VisitTraversal & "
              "visit )\n   {\n");
