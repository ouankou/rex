...
string replaceString(string patternInInput, string replacePattern,
                     string input) {
  ...
  if (grammarnode->isInnerNode()) {
    ...
    for (vector<GrammarString *>::iterator stringListIterator =
      ...
      if ((*stringListIterator)->getToBeTraversed() == DEF_TRAVERSAL) {
        ...
        s += string(grammarnode->getName()) + " -> " + type + " [label=\"" +
             (*stringListIterator)->getVariableNameString() + "\"];\n";
      }
    }
    // approximation: if the node has at least one successor that is a
    // non-AstNodeClass then
    //                create successor information
    // unfortunately GrammarString does not contain this information
    bool createSuccessorInfo = false;
    for (vector<AstNodeClass *>::iterator succiter =
             grammarnode->subclasses.begin();
         succiter != grammarnode->subclasses.end(); succiter++) {
      if ((*succiter)->isInnerNode())
        createSuccessorInfo = true;
    }

    // inheritance hierarchy information
    // create data for current node (edges to subtree nodes) (only edges for
    // inner nodes)
    if (createSuccessorInfo) {
      for (vector<GrammarSynthesizedAttribute>::iterator viter = v.begin();
           viter != v.end(); viter++) {
        s += string(grammarnode->getName()) + " -> " +
             (*viter).grammarnode->getName() + " [color=\"blue\"];\n";
      }
    }
  }
...
bool Grammar::isAbstractTreeGrammarSymbol(AstNodeClass *t) {
  return t->getCanHaveInstances();
}

bool Grammar::isAbstractTreeGrammarSymbol(string s) {
  ...
  // hack: exclude symbols explicitely which are not traversed but are included
...
// MS: 2003
// MS: We compute the set of traversed terminals to restrict the abstract
// grammar to traversed nodes only. DQ (9/28/2022): Fixing compiler warning for
// argument not used.
Grammar::GrammarSynthesizedAttribute
Grammar::CreateMinimalTraversedGrammarSymbolsSet(
  ...
  if (grammarnode->isLeafNode()) {
    ...
    for (vector<GrammarString *>::iterator stringListIterator =
             includeList.begin();
         stringListIterator !=
         includeList.end(); // ", " only between the elements of the list
         stringListIterator++) {
      if ((*stringListIterator)->getToBeTraversed() == DEF_TRAVERSAL) {
        traversedTerminals.insert(restrictedTypeStringOfGrammarString(
            *stringListIterator, grammarnode, "", ""));
      }
    }
  }
...
// MS: 2002, 2003, 2014
Grammar::GrammarSynthesizedAttribute Grammar::CreateAbstractTreeGrammarString(
    AstNodeClass *grammarnode, vector<GrammarSynthesizedAttribute> v) {
  ...
  for (vector<GrammarString *>::iterator stringListIterator =
           includeList.begin();
       stringListIterator != includeList.end(); stringListIterator++) {
    string type = restrictedTypeStringOfGrammarString(
        *stringListIterator, grammarnode, grammarSymListOpPrefix,
        grammarSymListOpPostfix);

    // ignores static types
    if (type == "IGNORE")
      continue;
    if (type == "SgToken::ROSE_Fortran_Keywords")
      type = "<int>";
  ...
  synAttr.nodetext = string(grammarnode->getName()) + " " + grammarSymTreeLB +
                     rhsTerminalSuccessors + grammarSymTreeRB;
  ...
  string grammarRule;
  grammarRule = grammarnode->getName() + " -> " + grammarnode->getName() +
                grammarSymTreeLB + rhsTerminalSuccessors + grammarSymTreeRB;
  ...
  for (vector<GrammarSynthesizedAttribute>::iterator viter = v.begin();
       viter != v.end(); viter++) {
    if ((*viter).nodetext !=
        "" /*&& isAbstractTreeGrammarSymbol(string(grammarnode->getName()))*/) {
      if (generateSDFTreeGrammar) {
        if (false && (*viter).isTerminal) {
          // nothing to do
        } else {
          grammarRule += string(grammarnode->getName()) + " -> " +
                         (*viter).terminalname + "\n";
        }
      } else {
        if (first) {
          grammarRule += string(grammarnode->getName()) + grammarSymArrow +
                         (*viter).nodetext + "\n";
          first = false;
        } else {
  ...
  // union non-AstNodeClass, AstNodeClass, and problematic nodes data of subtree
  // nodes
  for (vector<GrammarSynthesizedAttribute>::iterator viter = v.begin();
       viter != v.end(); viter++) {
    // union subtrees
...
void Grammar::buildGrammarDotFile(AstNodeClass *rootNode,
                                  ostream &GrammarDotFile) {
  GrammarSynthesizedAttribute a =
      BottomUpProcessing(rootNode, &Grammar::CreateGrammarDotString);
...
// MS:2002,2014
void Grammar::buildAbstractTreeGrammarFile(AstNodeClass *rootNode,
                                           ostream &AbstractTreeGrammarFile) {
  generateSDFTreeGrammar = false;
  // DQ (9/28/2022): Fixing compiler warning for argument not used.
  GrammarSynthesizedAttribute dummy = BottomUpProcessing(
      rootNode, &Grammar::CreateMinimalTraversedGrammarSymbolsSet);
  GrammarSynthesizedAttribute a =
      BottomUpProcessing(rootNode, &Grammar::CreateAbstractTreeGrammarString);
  AbstractTreeGrammarFile << "//  Abstract Tree Grammar" << endl << endl;
  // AbstractTreeGrammarFile << "Grammar G=<NonTerminals, Terminals, Rules,
  // SgNode>\n\n";
  AbstractTreeGrammarFile
      << "/*  Problematic nodes (fixed number of children and container(s)):\n"
      << a.problematicnodes << "*/" << endl
      << endl;
...
// MS:2014
void Grammar::buildSDFTreeGrammarFile(AstNodeClass *rootNode,
                                      ostream &SDFTreeGrammarFile) {
  ...
  // SDFTreeGrammarFile <<
  // "#CONTAINERTYPES:"<<sdfTreeGrammarContainerTypes.size()<<endl;
  //  generate rtg rules for container types
  for (set<string>::iterator i = sdfTreeGrammarContainerTypes.begin();
       i != sdfTreeGrammarContainerTypes.end(); ++i) {
    string listName = *i;
    string elemName = replaceString("ListStarOf", "", listName);
    SDFTreeGrammarFile << listName << " -> <nil>()" << endl;
    SDFTreeGrammarFile << listName << " -> <cons>(" << elemName << ","
                       << listName << ")" << endl;
  }
}
