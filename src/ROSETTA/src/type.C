...
void Grammar::setUpTypes() {
  ...
  NEW_TERMINAL_MACRO(TypeUnsignedShort, "TypeUnsignedShort",
                     "T_UNSIGNED_SHORT");
  ...
  NEW_TERMINAL_MACRO(TypeSignedLongLong, "TypeSignedLongLong",
                     "T_SIGNED_LONG_LONG");
  NEW_TERMINAL_MACRO(TypeUnsignedLongLong, "TypeUnsignedLongLong",
  ...
  // DQ (7/29/2014): Added nullptr type (I think we required this for C++11
  // support..
  NEW_TERMINAL_MACRO(RvalueReferenceType, "RvalueReferenceType",
                     "T_RVALUE_REFERENCE");

  // DQ (8/2/2014): Adding support for C++11 decltype() (which should be an new
  // SgType in the IR).
  NEW_TERMINAL_MACRO(DeclType, "DeclType", "T_DECLTYPE");

  // DQ (3/27/2015): Adding support for GNU C language extension "typeof"
  // operator (works similar to decltype in C++11).
  ...
  NEW_TERMINAL_MACRO(TypeUnsigned128bitInteger, "TypeUnsigned128bitInteger",
                     "T_UNSIGNED_128BIT_INTEGER");
  NEW_TERMINAL_MACRO(TypeSigned128bitInteger, "TypeSigned128bitInteger",
  ...
  // [DT] 5/11/2000 -- Added TemplateType.  Should it be called
  // TemplateInstantiationType
  //      to maintain symmetry with TemplateInstantiationDecl and
  //      TemplateInstantiationSymbol?
  //
  //      Later, 5/11 -- Changed it to TemplateInstantiationType.
  //

  // DQ (5/7/2004): There is no need for a TemplateInstantiationType since once
  // the template is intantiated it actually has a more specific C++ type
  // (class, function, member function, etc.) But we do need a type for the
  // TemplateDeclarationStatement since every sysmbol should have a type as a
  // general rule.  So cancle TemplateInstantiationType and build TemplateType
  // instead. NEW_TERMINAL_MACRO ( TemplateInstantiationType,
  // "TemplateInstantiationType", "T_TEMPLATE_INSTANTIATION" );
  ...
  NEW_TERMINAL_MACRO(PartialFunctionModifierType, "PartialFunctionModifierType",
  ...
  // to specify product rules with more than one AstNodeClass or nonTerminal
  // object (under some circomstances at least).
#define ROSETTA_BUG false
#if ROSETTA_BUG
  // Dummy AstNodeClass so that we can add avoid the NonTerminal copy
  // constructor (this may be a bug) I think that this is not a problem now
  // (Terminals can be reused in multiple BNF statements) though except as a
  // work around this makes little sense.
  ...
  NEW_NONTERMINAL_MACRO(PointerType, PointerMemberType, "PointerType",
                        "T_POINTER", true);

  // Note that TemplateInstantiationType can't be both a child of the NamedType
  // and ClassType This is an error that ROSETTA currently does not catch and
  // which I need to discuss with Danny Thorne For the moment it seems that
  // ROSETTA is by default ignoring this connection! printf ("WARNING:
  // TemplateInstantiationType specificed as a child of both NamedType and
  // ClassType! \n");

  // NEW_NONTERMINAL_MACRO (NamedType,
  //                        ClassType | TemplateInstantiationType | EnumType |
  //                        TypedefType, "NamedType","T_NAME");
  // DQ (2/10/2014): Original code
  NEW_TERMINAL_MACRO(ClassType, "ClassType", "T_CLASS");
  NEW_NONTERMINAL_MACRO(NamedType,
                        ClassType | EnumType | TypedefType | NonrealType,
                        "NamedType", "T_NAME", false);
  // DQ (5/11/2011): This is no longer used, and has not be used since the 3rd
  // rewite of the name qualification support in 2007.  We are now working on
  // the 4rh rewrite of this horrible subject and it is not clear if it should
  // be revived.  I would rather place the name qualification information into
  // the constructs the reference the type, instead of having the type re
  // wrapped as if it were a SgPointerType, SgReferenceType, etc. DQ
  // (12/21/2005): Support for qualified named types (wraps SgType (always a
  // SgNamedType) with SgQualifiedName)
  ...
  // PL (10/01/2025): Break out integral and floating-point types into
  // nonterminals.
  NEW_NONTERMINAL_MACRO(SignedIntegralType,
                        TypeInt | TypeLong | TypeLongLong | TypeShort |
                            TypeSigned128bitInteger | TypeSignedChar |
                            TypeSignedInt | TypeSignedLong |
                            TypeSignedLongLong | TypeSignedShort,
                        "SignedIntegralType", "SignedIntegralTypeTag", false);

  NEW_NONTERMINAL_MACRO(
      UnsignedIntegralType,
      TypeUnsigned128bitInteger | TypeUnsignedChar | TypeUnsignedInt |
          TypeUnsignedLong | TypeUnsignedLongLong | TypeUnsignedShort,
  ...
  NEW_NONTERMINAL_MACRO(FloatingPointType,
                        TypeFloat | TypeFloat128 | TypeFloat80 | TypeDouble |
                            TypeLongDouble | TypeFloat16 | TypeBFloat16 |
                            TypeFp16 | TypeFloat32x | TypeFloat64x |
                            TypeFloat32 | TypeFloat64,
                        "FloatingPointType", "FloatingPointTypeTag", false);

  // DQ (5/7/2004): Added TemplateType to be derived from SgType (this leaves
  // room later to build more specific types for template classes ? template
  // function, etc. unless they should be derived from there non-template
  // associated types as is done for the template instantiations; this is a
  // judgement call).  For now we can define a TemplateDeclarationStatement to
  // have a TemplateType and since all templates are the same (until they are
  // instatiated, likely there should only be a single TemplateType).
  NEW_NONTERMINAL_MACRO(
      Type,
  ...
  // Generate these function declarations in the classes derived from Type (but
  // not on Type) Type.setSubTreeFunctionPrototype( "HEADER",
  // "../Grammar/Common.code" ); Type.excludeFunctionPrototype   ( "HEADER",
  // "../Grammar/Common.code" );
  ...
  Type.setSubTreeFunctionPrototype("HEADER_GET_MANGLED",
                                   "../Grammar/Type.code");
  Type.excludeFunctionPrototype("HEADER_GET_MANGLED", "../Grammar/Type.code");

  // FMZ (2/9/2009): Added a flag for CoArray
  Type.setDataPrototype("bool", "isCoArray", "= 0", NO_CONSTRUCTOR_PARAMETER,
  ...
  Type.setDataPrototype("SgModifierNodes*", "modifiers", "= NULL",
                        NO_CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                        NO_TRAVERSAL, NO_DELETE);
  // Reference to typedef type (ensure mapping is fully resolved in the
  // frontend).
  ...
  Type.setDataPrototype("AstAttributeMechanism*", "attributeMechanism",
                        "= NULL", NO_CONSTRUCTOR_PARAMETER,
                        BUILD_FLAG_ACCESS_FUNCTIONS, NO_TRAVERSAL, DEF_DELETE,
                        CLONE_PTR);
  Type.setFunctionPrototype("HEADER_ATTRIBUTE_SUPPORT",
  ...
  PartialFunctionType.excludeFunctionPrototype("HEADER_GET_MANGLED",
                                               "../Grammar/Type.code");
  NamedType.excludeFunctionPrototype("HEADER_GET_MANGLED",
                                     "../Grammar/Type.code");
  ...
  // Use simple "static CLASSNAME builtin_type;" on most classed derived from
  // Type
  Type.setSubTreeFunctionPrototype("HEADER_COMMON_CREATE_TYPE",
                                   "../Grammar/Type.code");
  Type.excludeFunctionPrototype("HEADER_COMMON_CREATE_TYPE",
                                "../Grammar/Type.code");

  Type.setSubTreeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                "../Grammar/Type.code");
  Type.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                             "../Grammar/Type.code");

  FloatingPointType.setSubTreeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                             "../Grammar/Type.code");
  FloatingPointType.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                          "../Grammar/Type.code");

  IntegralType.setSubTreeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                        "../Grammar/Type.code");
  IntegralType.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                     "../Grammar/Type.code");

  SignedIntegralType.setSubTreeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                              "../Grammar/Type.code");
  SignedIntegralType.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                           "../Grammar/Type.code");

  UnsignedIntegralType.setSubTreeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                                "../Grammar/Type.code");
  UnsignedIntegralType.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                             "../Grammar/Type.code");

  TypeComplex.excludeFunctionPrototype("HEADER_COMMON_CREATE_TYPE",
                                       "../Grammar/Type.code");
  TypeComplex.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",
                                    "../Grammar/Type.code");

  TypeImaginary.excludeFunctionPrototype("HEADER_COMMON_CREATE_TYPE",
                                         "../Grammar/Type.code");
  ...
  // node.
  TypeString.excludeFunctionPrototype("HEADER_CREATE_TYPE_WITH_PARAMETER",
                                      "../Grammar/Type.code");
  ...
  // TypeString.setDataPrototype           ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);

  // DQ (10/5/2010): Removed the lengthScalar and definedUsingScalarLength data
  // members (no longer required). DQ (8/6/2010): Add string length to type
  // (this type is used only in Fortran support, as I recall, but might be used
  // for string literals in all langauges). A fixed length string is the most
  // common form, but fortran also permits a variable length string so we have
  ...
  // "definedUsingScalarLength", "= false", NO_CONSTRUCTOR_PARAMETER,
  // BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);
  ...
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  // ReferenceType.setDataPrototype        ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  ...
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  // FunctionType.setDataPrototype         ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  // ArrayType.setDataPrototype            ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  ...
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  // QualifiedNameType.setDataPrototype    ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  // UnknownMemberFunctionType.setDataPrototype   ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  // PartialFunctionType.setDataPrototype         ("static
  // $CLASSNAME*","builtin_type","",NO_CONSTRUCTOR_PARAMETER,
  // NO_ACCESS_FUNCTIONS, DEF_TRAVERSAL, NO_DELETE, NO_COPY_DATA);
  ...
  // DQ (8/25/2006): We can't specify an initializer if this is a static pointer
  // type (since this triggers the output of the initialization code in the
  // constructor). DQ (8/10/2006): Added support for different kinds of complex
  // types (float,double, and long double) JJW (11/22/2008): Changed to a static
  // variable in a function for the builtin complex and imaginary type caches

#define CUSTOM_CREATE_TYPE_MACRO(typeObjectName, sourceCodeName,               \
                                 parameterString)                              \
  typeObjectName.excludeFunctionPrototype("HEADER_COMMON_CREATE_TYPE",         \
                                          "../Grammar/Type.code");             \
  typeObjectName.excludeFunctionSource("SOURCE_COMMON_CREATE_TYPE",            \
                                       "../Grammar/Type.code");                \
  typeObjectName.setFunctionPrototype("HEADER_CREATE_TYPE_WITH_PARAMETER",     \
                                      "../Grammar/Type.code");                 \
  ...
  CUSTOM_CREATE_TYPE_MACRO(TypeInt, "SOURCE_CREATE_TYPE_FOR_TYPE_INT_TYPE",
                           "int sz = 0");
  TypeInt.setFunctionPrototype(
      "HEADER_BUILTIN_FOR_CREATE_TYPE_WITH_PARAMETER_TYPE_ARRAY",
      "../Grammar/Type.code");

  CUSTOM_CREATE_TYPE_MACRO(PointerType, "SOURCE_CREATE_TYPE_FOR_POINTER_TYPE",
                           "SgType* type = NULL");
  ...
  // DQ (8/2/2014): Adding support for C++11 decltype().
  CUSTOM_CREATE_TYPE_MACRO(DeclType, "SOURCE_CREATE_TYPE_FOR_DECL_TYPE",
                           "SgExpression* expr = NULL");

  // DQ (3/27/2015): Adding support for GNU C language extension "typeof"
  // operator (works similar to decltype in C++11).
  CUSTOM_CREATE_TYPE_MACRO(TypeOfType, "SOURCE_CREATE_TYPE_FOR_TYPEOF_TYPE",
                           "SgExpression* expr = NULL");

  // DQ (12/4/2011): Adding support for template declarations into the AST.
  // CUSTOM_CREATE_TYPE_MACRO(ClassType,"SOURCE_CREATE_TYPE_FOR_CLASS_TYPE","SgClassDeclaration*
  // decl = NULL");
  CUSTOM_CREATE_TYPE_MACRO(ClassType, "SOURCE_CREATE_TYPE_FOR_CLASS_TYPE",
                           "SgDeclarationStatement* decl = NULL");

  // DQ (11/28/2011): Make this more like the NamedType internal support.
  // CUSTOM_CREATE_TYPE_MACRO(TemplateType,"SOURCE_CREATE_TYPE_FOR_TEMPLATE_TYPE","SgTemplateInstantiationDecl*
  // decl = NULL");
  CUSTOM_CREATE_TYPE_MACRO(TemplateType, "SOURCE_CREATE_TYPE_FOR_TEMPLATE_TYPE",
                           "SgTemplateDeclaration* decl = NULL");

  CUSTOM_CREATE_TYPE_MACRO(NonrealType, "SOURCE_CREATE_TYPE_FOR_NONREAL_TYPE",
                           "SgNonrealDecl* decl = NULL");
  CUSTOM_CREATE_TYPE_MACRO(AutoType, "SOURCE_CREATE_TYPE_FOR_AUTO_TYPE",
                           "SgNode* node = NULL");

  CUSTOM_CREATE_TYPE_MACRO(TemplateType, "SOURCE_CREATE_TYPE_FOR_TEMPLATE_TYPE",
                           "SgTemplateInstantiationDecl* decl = NULL");
  CUSTOM_CREATE_TYPE_MACRO(EnumType, "SOURCE_CREATE_TYPE_FOR_ENUM_TYPE",
                           "SgEnumDeclaration* decl = NULL");
  CUSTOM_CREATE_TYPE_MACRO(TypedefType, "SOURCE_CREATE_TYPE_FOR_TYPEDEF_TYPE",
                           "SgTypedefDeclaration* decl = NULL");
  CUSTOM_CREATE_TYPE_MACRO(ModifierType, "SOURCE_CREATE_TYPE_FOR_MODIFIER_TYPE",
                           "SgType* type = NULL, unsigned int x = 0");
  CUSTOM_CREATE_TYPE_MACRO(QualifiedNameType,
                           "SOURCE_CREATE_TYPE_FOR_QUALIFIED_NAME_TYPE",
                           "SgType* type = NULL, unsigned int x = 0");
  CUSTOM_CREATE_TYPE_MACRO(FunctionType, "SOURCE_CREATE_TYPE_FOR_FUNCTION_TYPE",
  ...
  CUSTOM_CREATE_TYPE_MACRO(PartialFunctionType,
                           ...
                           "SgType* type = NULL");
  CUSTOM_CREATE_TYPE_MACRO(ArrayType, "SOURCE_CREATE_TYPE_FOR_ARRAY_TYPE",
                           "SgType* type = NULL, SgExpression* expr = NULL");

  // DQ (8/17/2010): Added support for create function for StringType (Fortran
  // specific)
  CUSTOM_CREATE_TYPE_MACRO(TypeString, "SOURCE_CREATE_TYPE_FOR_STRING_TYPE",
                           "SgExpression* expr = NULL, size_t length = 0");

  // TypeComplex.excludeFunctionSource ("SOURCE_CREATE_TYPE_FOR_COMPLEX_TYPE",
  // "../Grammar/Type.code" );
  TypeComplex.setFunctionSource("SOURCE_CREATE_TYPE_FOR_COMPLEX_TYPE",
                                "../Grammar/Type.code");
  // TypeImaginary.excludeFunctionSource ("SOURCE_CREATE_TYPE_FOR_COMPLEX_TYPE",
  // "../Grammar/Type.code" );
  TypeImaginary.setFunctionSource("SOURCE_CREATE_TYPE_FOR_COMPLEX_TYPE",
                                  "../Grammar/Type.code");

  TypeInt.setFunctionPrototype("HEADER_TYPE_INT_TYPE", "../Grammar/Type.code");
  // TypeInt.excludeFunctionPrototype ( "HEADER_EXTRA_FUNCTIONS",
  // "../Grammar/Type.code" ); TypeInt.setFunctionPrototype     (
  // "HEADER_CREATE_TYPE" , "../Grammar/Type.code" ); TypeInt.setFunctionSource
  // ( "SOURCE_BUILTIN_TYPE_ARRAY" , "../Grammar/Type.code" );

  // These classes have data fields
  TypeInt.setDataPrototype("int", "field_size", "= 0", CONSTRUCTOR_PARAMETER,
                           BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);

  // DQ (4/23/2014): I think this has to be defined as DEF_TRAVERSAL so that we
  // can traverse the nested type. This is required to support type
  // transformations fo the shared memory DSL work. Likely also required for
  ...
  RvalueReferenceType.setDataPrototype(
  ...
  // we can translate the code into a parameter.  Then we need only make sure
  // that the expression type (if available) is consistant with the associated
  // stored type. If the expresssion's type is available, then the expression
  // weill be stored, but if the expression's type is unavailable (either the
  // expression is unavailable or the expression is a SgFunctionParameterRefExp
  // then the type is stored).  This is the more complex handling of stored
  // type, but is hidden within the SgDeclType and the
  // SgFunctionParameterRefExp, as required to present a simple API).
  ...
  TypeOfType.setDataPrototype("SgType*", "base_type", "= NULL",
                              CONSTRUCTOR_PARAMETER, NO_ACCESS_FUNCTIONS,
                              NO_TRAVERSAL, NO_DELETE);
  TypeOfType.setFunctionPrototype("HEADER_GET_BASE_TYPE",
                                  "../Grammar/Type.code");

  PointerMemberType.setFunctionPrototype("HEADER_POINTER_MEMBER_TYPE",
                                         "../Grammar/Type.code");
  ...
  // the SgPointerMemberType which will allow the name qualification to be
  // computed for a set of types in a specific context (location in the code,
  // e.g. statement) and then use the mechanism to generate a string for the
  // type (as is used for template instantiations) and then put the generated
  // string into a map used for name qualificaiton of types.  The reason we
  // can't just use a limited number of fields on each IR node that can
  // reference a type is that there can be an abitrary number of points in the
  // nested representation of SgPointerMemberType(s) where name qualificaiton is
  // required. To this extent it is similar to the name qualification complexity
  // of the template instantiations which can have arbitrary numbers of template
  // arguments with additional name qualification.

  // To support each context generating the name qualification for every part of
  // type tree, we need to put support for the name qualificaiton directly into
  // the SgPointerMemberType.

  NamedType.setFunctionPrototype("HEADER_VIRTUAL_GET_NAME",
                                 "../Grammar/Type.code");
  NamedType.setFunctionPrototype("HEADER_GET_QUALIFIED_NAME",
                                 "../Grammar/Type.code");
  ...
  // NamedType.setDataPrototype     ("SgDeclarationStatement*", "declaration","=
  // NULL",
  ...
  NamedType.setDataPrototype("bool", "autonomous_declaration", "= true",
                             NO_CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                             NO_TRAVERSAL, NO_DELETE);
  // NamedType.setDataPrototype ( "static SgQualifiedNamePtrList",
  // "defaultQualifiedNamePtrList", "",
  //           NO_CONSTRUCTOR_PARAMETER, NO_ACCESS_FUNCTIONS, NO_TRAVERSAL,
  //           NO_DELETE);
  // NamedType.setDataPrototype ( "SgQualifiedNamePtrList", "qualifiedNameList",
  // "= p_defaultQualifiedNamePtrList",
  //           NO_CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL,
  //           NO_DELETE);

  // DQ (7/16/2017): We need to mark that a named type is associated with a
  // template parameter.
  ...
  ClassType.setDataPrototype("bool", "packed", "= false",
                             NO_CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                             NO_TRAVERSAL, NO_DELETE);

  NonrealType.setFunctionPrototype("HEADER_NONREAL_TYPE",
                                   "../Grammar/Type.code");
  NonrealType.setFunctionPrototype("HEADER_GET_NAME", "../Grammar/Type.code");
  //   NonrealType.excludeFunctionPrototype ( "HEADER_GET_NAME",
  //   "../Grammar/Type.code" ); NonrealType.excludeFunctionPrototype (
  ...
  // TemplateInstantiationType.setFunctionPrototype
  // ("HEADER_TEMPLATE_INSTANTIATION_TYPE", "../Grammar/Type.code" );
  TemplateType.setFunctionPrototype("HEADER_TEMPLATE_TYPE",
                                    "../Grammar/Type.code");
  // TemplateInstantiationType.setFunctionPrototype ("HEADER_GET_NAME",
  // "../Grammar/Type.code" );

  // DQ (8/25/2012): Added support for name of template type. Note that this is
  // the name "T1" in instantiation of a template using a template parameter.
  // This should likely be mapped back to the template parameter by position in
  // the sequence of template parameters and the template declaration (OR just
  // the template paramters only; I have not decided).
  TemplateType.setDataPrototype("SgName", "name", "= \"\"",
                                CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                                NO_TRAVERSAL, NO_DELETE);
  TemplateType.setDataPrototype(
      "int", "template_parameter_position", "= -1", NO_CONSTRUCTOR_PARAMETER,
      BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);
  TemplateType.setDataPrototype(
  ...
  TemplateType.setDataPrototype(
      ...
      NO_DELETE);
  TemplateType.setDataPrototype(
      "SgTemplateArgumentPtrList", "part_spec_tpl_args",
      "= SgTemplateArgumentPtrList()", NO_CONSTRUCTOR_PARAMETER,
      BUILD_LIST_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);

  // DQ (3/18/2017): We need to support a flag to indicate packing in template
  // parameters.
  TemplateType.setDataPrototype(
      "bool", "packed", "= false", NO_CONSTRUCTOR_PARAMETER,
      BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);
  ...
  if (isRootGrammar() == false) {
    ...
    NamedType.excludeFunctionSource("SOURCE_PARSER",
  ...
  // DQ (6/30/2005): Added member function for stiping away typedefs and
  // modifiers TypedefType.setFunctionPrototype ("HEADER_TYPEDEF_TYPE",
  // "../Grammar/Type.code" );

  TypedefType.setFunctionPrototype("HEADER_GET_NAME", "../Grammar/Type.code");

  // This get_base_type function is not associated with a base_type data filed
  // in this object
  TypedefType.setFunctionPrototype("HEADER_GET_BASE_TYPE",
                                   "../Grammar/Type.code");
  TypedefType.setFunctionPrototype("HEADER_TYPEDEF_TYPE",
                                   "../Grammar/Type.code");
  TypedefType.setDataPrototype("SgSymbol*", "parent_scope", "= NULL",
                               CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                               NO_TRAVERSAL, NO_DELETE);

  // Exclude the get_mangled function since we include it in the
  // HEADER_MODIFIER_TYPE string
  ModifierType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                     "../Grammar/Type.code");
  ModifierType.setFunctionPrototype("HEADER_MODIFIER_TYPE",
                                    "../Grammar/Type.code");
  ...
  ModifierType.setDataPrototype("SgTypeModifier", "typeModifier", "",
                                NO_CONSTRUCTOR_PARAMETER, NO_ACCESS_FUNCTIONS,
                                NO_TRAVERSAL, NO_DELETE);
  // DQ (8/12/2012): This is used as a wrapper to support type references that
  // will be fixed up after the AST is build and all types exist. It is part of
  // the new C++ support and required for types hidden in template
  // instantiations that have not yet been processed yet.
  ModifierType.setDataPrototype(
      "char*", "frontend_type_reference", "= NULL", NO_CONSTRUCTOR_PARAMETER,
      BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);

  // DQ (10/10/2006): The idea here is that these would be wrappers for existing
  // types, but I think this was ultimately a problem to make it really work
  // (because it could only replace SgType and not SgNamedType, for example; so
  // it is not used as an IR node at present. DQ (12/21/2005): Global
  // qualification and qualified name handling are now represented explicitly in
  // the AST Exclude the get_mangled function since we include it in the
  // HEADER_MODIFIER_TYPE string
  QualifiedNameType.setFunctionPrototype("HEADER_QUALIFIED_NAME_TYPE",
                                         "../Grammar/Type.code");
  QualifiedNameType.setDataPrototype(
      "SgType*", "base_type", "= NULL", CONSTRUCTOR_PARAMETER,
      BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);
  QualifiedNameType.setDataPrototype(
  ...
  FunctionType.setFunctionPrototype("HEADER_FUNCTION_TYPE",
                                    "../Grammar/Type.code");
  ...
  FunctionType.setDataPrototype("bool", "has_ellipses", "= true",
                                CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                                NO_TRAVERSAL, NO_DELETE);
  // DQ (7/20/2004): Modified to NOT traverse this object (later: likely OK to
  // traverse) FunctionType.setDataPrototype("SgFunctionParameterTypeList*",
  // "arguments", "= NULL",
  ...
  MemberFunctionType.setDataPrototype(
      ...
      BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);
  MemberFunctionType.setDataPrototype(
      "unsigned int", "mfunc_specifier", "= 0", CONSTRUCTOR_PARAMETER,
      BUILD_ACCESS_FUNCTIONS, NO_TRAVERSAL, NO_DELETE);
  ...
  // DQ (3/22/2017): I think it is a mistake to include this this way. This is
  // for the virtual function support and it is not a virtual function in
  // SgArrayType. ArrayType.setFunctionPrototype ("HEADER_GET_NAME",
  // "../Grammar/Type.code" );
  ArrayType.setFunctionPrototype("HEADER_ARRAY_TYPE", "../Grammar/Type.code");

  ArrayType.setFunctionPrototype("HEADER_GET_QUALIFIED_NAME",
                                 "../Grammar/Type.code");
  ...
  // DQ (2/7/2007): I think that the delete operator should call delete for the
  // index expression. ArrayType.setDataPrototype ("SgExpression*", "index"    ,
  // "= NULL",
  ...
  TypeComplex.setFunctionPrototype("HEADER_TYPE_COMPLEX_TYPE",
                                   "../Grammar/Type.code");
  TypeComplex.setDataPrototype("SgType*", "base_type", "= NULL",
                               CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                               NO_TRAVERSAL, NO_DELETE);

  // DQ (8/27/2006): Use the code from the complex class to define the imaginary
  // class!
  TypeImaginary.setFunctionPrototype("HEADER_TYPE_COMPLEX_TYPE",
                                     "../Grammar/Type.code");
  TypeImaginary.setDataPrototype("SgType*", "base_type", "= NULL",
                                 CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                                 NO_TRAVERSAL, NO_DELETE);

  // DQ (8/17/2010): Added support for string types for Fortran (in C/C++ they
  // are just arrays of char).
  TypeString.setFunctionPrototype("HEADER_TYPE_STRING_TYPE",
                                  "../Grammar/Type.code");

  // DQ (12/26/2010): Added mechanism to store names of types that can't be
  // identified in initial parsing (applicable only to Fortran).
  TypeDefault.setFunctionPrototype("HEADER_TYPE_DEFAULT_TYPE",
                                   "../Grammar/Type.code");
  ...
  TypeLabel.setFunctionPrototype("HEADER_TYPE_LABEL_TYPE",
                                 "../Grammar/Type.code");
  TypeLabel.setDataPrototype("SgName", "name", "= \"\"",
                             NO_CONSTRUCTOR_PARAMETER, BUILD_ACCESS_FUNCTIONS,
                             NO_TRAVERSAL, NO_DELETE);
  ...
  Type.editSubstituteSubTree("CONSTRUCTOR_BODY", "SOURCE_CONSTRUCTOR_BODY",
                             "../Grammar/Type.code");
  ...
  // Use a different get_mangled member function for the PointerType,
  // ReferenceType, and TypeComplex objects
  PointerType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                    "../Grammar/Type.code");
  PointerType.setFunctionSource("SOURCE_GET_MANGLED_BASE_TYPE",
                                "../Grammar/Type.code");

  ReferenceType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                      "../Grammar/Type.code");
  ReferenceType.setFunctionSource("SOURCE_GET_MANGLED_BASE_TYPE",
                                  "../Grammar/Type.code");

  RvalueReferenceType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                            "../Grammar/Type.code");
  RvalueReferenceType.setFunctionSource("SOURCE_GET_MANGLED_BASE_TYPE",
                                        "../Grammar/Type.code");

  TypeComplex.excludeFunctionSource("SOURCE_GET_MANGLED",
                                    "../Grammar/Type.code");
  TypeComplex.setFunctionSource("SOURCE_GET_MANGLED_BASE_TYPE",
                                "../Grammar/Type.code");

  // DQ (8/2/2014): Adding support for C++11 decltype().
  DeclType.excludeFunctionSource("SOURCE_GET_MANGLED", "../Grammar/Type.code");
  // TV (02/21/2019): We need a specific version of this function for the
  // decltype() operator. DeclType.setFunctionSource     (
  // "SOURCE_GET_MANGLED_BASE_TYPE", "../Grammar/Type.code");

  // DQ (3/27/2015): Adding support for GNU C language extension "typeof"
  // operator (works similar to decltype in C++11).
  TypeOfType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                   "../Grammar/Type.code");
  // DQ (4/8/2015): We need a specific version of this function for the typeof()
  // operator. TypeOfType.setFunctionSource     (
  // "SOURCE_GET_MANGLED_BASE_TYPE", "../Grammar/Type.code");

  ArrayType.excludeFunctionSource("SOURCE_GET_MANGLED", "../Grammar/Type.code");
  // ArrayType.setFunctionSource ( "SOURCE_GET_MANGLED_BASE_TYPE",
  // "../Grammar/Type.code");

  // We require a special function here which is included directly
  FunctionType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                     "../Grammar/Type.code");

  TypedefType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                    "../Grammar/Type.code");
  NamedType.excludeFunctionSource("SOURCE_GET_MANGLED", "../Grammar/Type.code");

  MemberFunctionType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                           "../Grammar/Type.code");
  PartialFunctionType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                            "../Grammar/Type.code");
  ModifierType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                     "../Grammar/Type.code");

  QualifiedNameType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                          "../Grammar/Type.code");

  ClassType.excludeFunctionSource("SOURCE_GET_MANGLED", "../Grammar/Type.code");

  //   NonrealType.excludeFunctionSource    ( "SOURCE_GET_NAME",
  //   "../Grammar/Type.code");
  NonrealType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                    "../Grammar/Type.code");
  NonrealType.setFunctionSource("SOURCE_NONREAL_TYPE", "../Grammar/Type.code");

  //   AutoType.excludeFunctionSource    ( "SOURCE_GET_NAME",
  //   "../Grammar/Type.code");
  ...
  // TemplateInstantiationType.excludeFunctionSource ( "SOURCE_GET_MANGLED",
  // "../Grammar/Type.code");
  EnumType.excludeFunctionSource("SOURCE_GET_MANGLED", "../Grammar/Type.code");

  PointerMemberType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                          "../Grammar/Type.code");
  PointerMemberType.setFunctionSource("SOURCE_POINTER_MEMBER_GET_MANGLED",
                                      "../Grammar/Type.code");

  // DQ (8/25/2012): Avoid the default generated get_mangled_name() function for
  // this IR node.
  TemplateType.excludeFunctionSource("SOURCE_GET_MANGLED",
                                     "../Grammar/Type.code");
  ...
  // DQ (8/25/2012): Avoid the default generated get_mangled_name() function for
  // this IR node. DQ (5/11/2012): We need to define this.
  // TemplateType.editSubstitute( "MANGLED_ID_STRING", "TemplateType" );

  // DQ (5/11/2012): This should not be used, bu we should define this for all
  // types.
  PartialFunctionModifierType.editSubstitute("MANGLED_ID_STRING",
                                             "PartialFunctionType");

  PartialFunctionType.setFunctionSource("SOURCE_PARTIAL_FUNCTION_TYPE",
                                        "../Grammar/Type.code");
  TypedefType.setFunctionSource("SOURCE_TYPEDEF_TYPE", "../Grammar/Type.code");
  MemberFunctionType.setFunctionSource("SOURCE_MEMBER_FUNCTION_TYPE",
                                       "../Grammar/Type.code");
  PointerMemberType.setFunctionSource("SOURCE_POINTER_MEMBER_TYPE",
                                      "../Grammar/Type.code");
  ...
  FunctionType.setFunctionSource("SOURCE_FUNCTION_TYPE",
                                 "../Grammar/Type.code");

  ClassType.setFunctionSource("SOURCE_CLASS_TYPE", "../Grammar/Type.code");

  TemplateType.setFunctionSource("SOURCE_TEMPLATE_TYPE",
                                 "../Grammar/Type.code");

  PointerType.setFunctionSource("SOURCE_POINTER_TYPE", "../Grammar/Type.code");
  ReferenceType.setFunctionSource("SOURCE_REFERENCE_TYPE",
                                  "../Grammar/Type.code");
  RvalueReferenceType.setFunctionSource("SOURCE_RVALUE_REFERENCE_TYPE",
                                        "../Grammar/Type.code");
  ...
  ModifierType.setFunctionSource("SOURCE_MODIFIER_TYPE",
                                 "../Grammar/Type.code");
  QualifiedNameType.setFunctionSource("SOURCE_QUALIFIED_NAME_TYPE",
                                      "../Grammar/Type.code");
  TypeCrayPointer.editSubstitute("MANGLED_ID_STRING", "CrayPointer");

  // DQ (8/17/2010): Added support for SgTypeString (used in Fortran).
  TypeString.setFunctionSource("SOURCE_TYPE_STRING_TYPE",
                               "../Grammar/Type.code");
  // DQ (8/17/2010): Added support for SgTypeString name mangling.
  TypeString.excludeFunctionSource("SOURCE_GET_MANGLED",
                                   "../Grammar/Type.code");
  TypeString.setFunctionSource("SOURCE_GET_MANGLED_STRING_TYPE",
                               "../Grammar/Type.code");

  // DQ (12/26/2010): Added support for names in TypeDefault (used to store
  // parser information until proper type can be constructed, e.g. after using
  // statement is seen for function return types).
  TypeDefault.excludeFunctionSource("SOURCE_GET_MANGLED",
                                    "../Grammar/Type.code");
