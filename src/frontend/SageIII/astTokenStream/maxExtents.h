...
// Support to the evaluation of source positions of subtrees to propagate source
// positions start and end positions to compiler generated SgStatement IR nodes
// (that don't have a source position) that contain valid subtrees.  Where the
// source position map to a single character position the subtree is either:
//   1) A macro expansion
//   2) A single character statement (e.g. ";", or SgValueExpression convertedt
//   to a SgExprStatement).
...
public:
  ...
  InheritedAttribute(SgSourceFile *input_sourceFile, int input_start_line,
                     int input_start_column, int input_end_line,
                     int input_end_column, bool processed);
...
public:
  ...
  SynthesizedAttribute(SgNode *n, int start_line, int start_column,
...
public:
  ...
  // virtual function must be defined
  SynthesizedAttribute
  evaluateSynthesizedAttribute(SgNode *n, InheritedAttribute inheritedAttribute,
                               SubTreeSynthesizedAttributes childAttributes);
};

void computeMaxSourceExtents(SgSourceFile *sourceFile, SgNode *n,
