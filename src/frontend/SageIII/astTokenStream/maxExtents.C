...
MaxSourceExtents::InheritedAttribute::InheritedAttribute(
    SgSourceFile *input_sourceFile, int input_start_line,
    int input_start_column, int input_end_line, int input_end_column,
    bool processed)
    : processChildNodes(processed), sourceFile(input_sourceFile) {
...
MaxSourceExtents::InheritedAttribute::InheritedAttribute(
    ...
    : processChildNodes(X.processChildNodes), sourceFile(X.sourceFile) {
...
MaxSourceExtents::SynthesizedAttribute::SynthesizedAttribute(
    SgNode *n, int input_start_line, int input_start_column, int input_end_line,
...
MaxSourceExtents::SourceExtentsTraversal::evaluateInheritedAttribute(
    SgNode * /*n*/, InheritedAttribute inheritedAttribute) {
  return inheritedAttribute;
}

MaxSourceExtents::SynthesizedAttribute
MaxSourceExtents::SourceExtentsTraversal::evaluateSynthesizedAttribute(
    SgNode *n, InheritedAttribute inheritedAttribute,
    SubTreeSynthesizedAttributes childAttributes) {
#define DEBUG_evaluateSynthesizedAttribute 0

#if DEBUG_evaluateSynthesizedAttribute
  printf("\n\nIn SourceExtentsTraversal::evaluateSynthesizedAttribute(): n = "
         "%p = %s \n",
  ...
  bool isInSameFile = this_locatedNode->get_startOfConstruct()->isSameFile(
  ...
  // Set these to the default values if this is a compiler generate node or a
  // node from a different file.
  int this_start_line =
      (isCompilerGenerated == false && isInSameFile == true)
          ? this_locatedNode->get_startOfConstruct()->get_line()
          : max_value;
  int this_start_column =
      (isCompilerGenerated == false && isInSameFile == true)
          ? this_locatedNode->get_startOfConstruct()->get_col()
          : max_value;
  int this_end_line = (isCompilerGenerated == false && isInSameFile == true)
                          ? this_locatedNode->get_endOfConstruct()->get_line()
                          : 0;
  int this_end_column = (isCompilerGenerated == false && isInSameFile == true)
                            ? this_locatedNode->get_endOfConstruct()->get_col()
                            : 0;

#if DEBUG_evaluateSynthesizedAttribute
  printf("   --- isCompilerGenerated = %s this_start_line = %d "
         "this_start_column = %d this_end_line = %d this_end_column = %d \n",
         isCompilerGenerated ? "true" : "false", this_start_line,
         this_start_column, this_end_line, this_end_column);
#endif

  int local_start_line = (this_start_line > 0) ? this_start_line : max_value;
  int local_start_column =
      (this_start_column > 0) ? this_start_column : max_value;
  int local_end_line = (this_end_line > 0) ? this_end_line : 0;
  int local_end_column = (this_end_column > 0) ? this_end_column : 0;

#if DEBUG_evaluateSynthesizedAttribute
  printf("   --- (initial) local_start_line = %d local_start_column = %d "
         "local_end_line = %d local_end_column = %d \n",
         local_start_line, local_start_column, local_end_line,
         local_end_column);
  ...
  for (size_t i = 0; i < childAttributes.size(); i++) {
    ...
    if (childAttributes[i].node != nullptr) {
      ...
      printf("   ---   --- child = %p = %s = %s \n", childAttributes[i].node,
             child_name.c_str(), childAttributes[i].node->class_name().c_str());
#endif
      ASSERT_require(isSgLocatedNode(childAttributes[i].node));

      // Get the child values from the child attribute (not the asociated IR
      // node.
...
#if DEBUG_evaluateSynthesizedAttribute
      printf("   ---   ---   --- child_start_line = %d child_start_column = %d "
             "child_end_line = %d child_end_column = %d \n",
             child_start_line, child_start_column, child_end_line,
             child_end_column);
#endif
...
#if DEBUG_evaluateSynthesizedAttribute
  printf("   --- (final) local_start_line = %d local_start_column = %d "
         "local_end_line = %d local_end_column = %d \n",
         local_start_line, local_start_column, local_end_line,
         local_end_column);
#endif

  SynthesizedAttribute return_attribute(n, local_start_line, local_start_column,
                                        local_end_line, local_end_column);

  return return_attribute;
}

void MaxSourceExtents::computeMaxSourceExtents(SgSourceFile *sourceFile,
                                               SgNode *n, int &start_line,
  ...
  InheritedAttribute inheritedAttribute(sourceFile, start_line, start_column,
                                        end_line, end_column, processed);
  SourceExtentsTraversal traversal;

  SynthesizedAttribute return_attribute =
      traversal.traverse(n, inheritedAttribute);
