...
#define DEBUG_INHERIT 0
  ...
  // Unparse the middle of the AST from the AST and the outer 25% from the token
  // stream.
  ...
  if (statement != nullptr) {
    ...
    printf("*** In ArtificialFrontierTraversal::evaluateInheritedAttribute(): "
           "n = %p = %s filename = %s \n",
  ...
  if (locatedNode != nullptr) {
    ...
    // be a statement. The evaluateSynthesizedAttribute() function will set the
    // parent scopes as containing a transformation.
    if (statement != nullptr) {
...
#define DEBUG_SYNTH 0
  ...
  printf("### In ArtificialFrontierTraversal::evaluateSynthesizedAttribute(): "
         "TOP n = %p = %s \n",
...
int ArtificialFrontierTraversal::numberOfNodesInSubtree(
    SgSourceFile *sourceFile) {
  int value = 0;
  TimingPerformance timer(
      "AST ArtificialFrontierTraversal::numberOfNodesInSubtree():");
