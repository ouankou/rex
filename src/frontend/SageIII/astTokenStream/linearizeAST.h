...
private:
  // The idea is to create an inorder ordering of the nodes using a
  // SgTopDownBottomUp traversal. The idea is that every node can be viewed as a
  // leaf node if you diregard it's subtree. Using 4 nodes we can describe any
  // such operation
  //                                  A
  //                                  |
  //                                  B
  ...
  //
  //  PS! This operation will only be applied on expression. For all other
  //  constructs the ordering of the preorder traversal is used.
...
public:
  ...
  std::vector<SgNode *> get_linearization();
