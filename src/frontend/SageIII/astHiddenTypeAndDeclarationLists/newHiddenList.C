...
//      makes it more clear that it is associated with Name Qualification
//      support.
//   2) The filename for this source code should also be changed.

void newBuildHiddenTypeAndDeclarationLists(
  ...
  TimingPerformance timer(
      "New hidden type and hidden declaration list generation:");
  ...
  // HiddenListTraversal
  // t(SgNode::get_globalQualifiedNameMapForNames(),SgNode::get_globalQualifiedNameMapForTypes(),SgNode::get_globalTypeNameMap(),SgNode::get_globalReferencedNameSet());
  HiddenListTraversal t(SgNode::get_globalQualifiedNameMapForNames(),
                        SgNode::get_globalQualifiedNameMapForTypes(),
                        SgNode::get_globalTypeNameMap(), referencedNameSet);
...
HiddenListInheritedAttribute::HiddenListInheritedAttribute(
    const HiddenListInheritedAttribute &X) {
  // Copy constructor.
}
...
HiddenListSynthesizedAttribute::HiddenListSynthesizedAttribute(
    const HiddenListSynthesizedAttribute &X) {
  // Copy constructor.
}
...
HiddenListTraversal::HiddenListTraversal(
    SgUnorderedMapNodeToString &input_qualifiedNameMapForNames,
...
HiddenListTraversal::associatedDeclaration(SgScopeStatement *scope) {
  ...
  default: {
    printf("Default reached in HiddenListTraversal::associatedDeclaration() "
           "scope = %s \n",
...
HiddenListTraversal::associatedDeclaration(SgType *type) {
  ...
  case V_SgMemberFunctionType: {
    // Not clear if I have to resolve declarations associated with function
    // types.
    printf("In HiddenListTraversal::associatedDeclaration(): Case of "
           "SgFunctionType not implemented strippedType = %s \n",
...
void HiddenListTraversal::evaluateTemplateInstantiationDeclaration(
  ...
  case V_SgTemplateInstantiationDecl: {
    ...
    evaluateNameQualificationForTemplateArgumentList(
        templateInstantiationDeclaration->get_templateArguments(), currentScope,
        positionStatement);
    break;
  }
...
int HiddenListTraversal::nameQualificationDepthOfParent(
    SgDeclarationStatement *declaration, SgScopeStatement *currentScope,
    SgStatement *positionStatement) {
  ...
  printf("***** Inside of "
         "HiddenListTraversal::nameQualificationDepthOfParent() ***** \n");
...
bool HiddenListTraversal::requiresTypeElaboration(SgSymbol *symbol) {
  ...
  default: {
    printf("Default reached in HiddenListTraversal::requiresTypeElaboration(): "
           "symbol = %p = %s \n",
...
// int HiddenListTraversal::nameQualificationDepth ( SgScopeStatement*
// classOrNamespaceDefinition )
int HiddenListTraversal::nameQualificationDepth(
    SgDeclarationStatement *declaration, SgScopeStatement *currentScope,
  ...
  // Also this function may have to be able to take a SgInitializedName, but at
  // the moment it takes a SgVariable declaration to handle the equivalent case.
  // Later when we support multiple SgInitializedName IR nodes in a single
  // SgVariableDeclaration this will have to be changed.
  ...
  printf(
      "##### Inside of HiddenListTraversal::nameQualificationDepth() ##### \n");
  ...
  // template declaration (SgTemplateInstantiationDecl, I think). printf
  // ("declaration  = %p = %s = %s = %s
  ...
  if (name.is_null() == true) {
    // This name is empty so we need to keep going to evaluate the qualified
    // name (see test2006_121.C).
    printf(
        "CASE OF EMPTY NAME: declaration = %p = %s but has no valid name (it "
        "has and empty name), thus we have to recurse to the next level \n",
...

  {
    ...
    if (symbol != NULL) {
      ...
      // \n",symbol,symbol->class_name().c_str());
      printf("Lookup symbol based on name only (via parents starting at "
      ...
      case V_SgClassDeclaration: {
        ...
        if (classSymbol == NULL) {
          ...
          // (see test2011_37.C). printf ("Type elaboration is required:
          // declaration = %s symbol = %s
          ...
          if (symbol != NULL) {
            ...
            printf("In HiddenListTraversal::nameQualificationDepth(): "
                   "classSymbol == NULL \n");
          }
        } else {
          printf("Symbol matches the class declaration "
      ...
      case V_SgFunctionDeclaration: {
        ...
        if (functionSymbol == NULL) {
          ...
          printf("WARNING: Present implementation of symbol table will not "
                 "find alias sysmbols of SgFunctionSymbol \n");
      ...
      case V_SgTypedefDeclaration: {
        ...
        if (typedefSymbol == NULL) {
          ...
          // printf ("WARNING: Present implementation of symbol table will not
          // find alias sysmbols of SgTypedefSymbol \n");
          ...
          if (symbol != NULL) {
            ...
            symbol = SageInterface::lookupTemplateSymbolInParentScopes(
                name, currentScope);
            ROSE_ASSERT(symbol != NULL);
          }
          ROSE_ASSERT(symbol != NULL);
      ...
      case V_SgTemplateDeclaration: {
        ...
        // DQ (6/1/2011): Not clear if we need a special case for the case of
        // SgTemplateInstantiationMemberFunctionDecl. I think we need to call:
        // evaluateNameQualificationForTemplateArgumentList() to evaluate
        // template arguments for both SgTemplateInstantiationFunctionDecl and
        // SgTemplateInstantiationMemberFunctionDecl.
      ...
      case V_SgEnumDeclaration: {
        ...
        if (enumSymbol == NULL) {
          ...
          if (symbol != NULL) {
            ...
            printf("Detected no enum symbol in a parent scope (ignoring this "
                   "case for now) \n");
          }
        }
      ...
      case V_SgTemplateInstantiationDecl: {
        ...
        if (classSymbol == NULL) {
          ...
          if (nestedTemplateDeclaration != NULL) {
            ...
            // DQ (6/1/2011): This is now done below; for all IR nodes
            // contianing template parameters.
            // evaluateNameQualificationForTemplateArgumentList
            // (nestedTemplateDeclaration->get_templateArguments(),currentScope,positionStatement);
          }
        }
      ...
      printf("SageInterface::lookupSymbolInParentScopes(name,currentScope) "
             "returned NULL \n");
    }
    ...
    if (symbol != NULL) {
      ...
      if (numberOfSymbols > 1000 || forceMoreNameQualification == true) {
        ...
        printf(
            "Found a case of ambiguity of declaration = %s in the currentScope "
            "= %p = %s = %s (trigger additional name qualifier). \n",
            declaration->class_name().c_str(), currentScope,
            currentScope->class_name().c_str(),
            SageInterface::get_name(currentScope).c_str());
        switch (declaration->variantT()) {
        case V_SgFunctionDeclaration: {
          // See test2011_48.C for an example of this (the overloaded functions
          // cause numberOfSymbols > 1.
          printf("Error: Skipping name qualification for SgFunctionDeclaration "
        ...
        case V_SgEnumDeclaration: {
          ...
          // this point!
          printf("Error: Skipping name qualification for enum types (sorry, "
                 "not implemented) \n");
        ...
        // sysmbols scope is want might have to be qualified (not the scope of
        ...
        case V_SgClassSymbol: {
          ...
          if (associatedClassDeclaration->get_firstNondefiningDeclaration() ==
              classDeclaration->get_firstNondefiningDeclaration()) {
            ...
            printf("This class is NOT visible from where it is referenced "
        ...
        case V_SgNamespaceSymbol: {
          ...
          if (associatedNamespaceDeclaration
            ...
            // The name does not match, so the associatedClassDeclaration is
            // hidding the base class declaration.
            printf("This namespace is NOT visible from where it is referenced "
                   "(declaration with same name does not match) \n");
        ...
        case V_SgVariableSymbol: {
          ...
          if (associatedInitializedName ==
            ...
            // The name does not match, so the associatedClassDeclaration is
            // hidding the base class declaration.
            printf("This variable is NOT visible from where it is referenced "
                   "(declaration with same name does not match) \n");
        ...
        case V_SgFunctionSymbol: {
          ...
          printf("Case of SgFunctionSymbol or SgMemberFunctionSymbol: "
                 "functionSymbol = %p = %s \n",
          ...
          printf("associatedFunctionDeclarationFromSymbol->get_"
          ...
          if (associatedFunctionDeclarationFromSymbol ==
            ...
            printf("This function or member function IS visible from where it "
                   "is referenced (but there could still be ambiguity if this "
                   "was just the first of several symbols found in the current "
                   "scope) \n");
        ...
        case V_SgTypedefSymbol: {
          ...
          printf(
              ...
              "= %p \n",
              associatedTypedefDeclaration->get_firstNondefiningDeclaration());
          printf("typedefDeclaration->get_firstNondefiningDeclaration()        "
                 "   = %p \n",
          ...
          if (associatedTypedefDeclaration->get_firstNondefiningDeclaration() ==
            ...
            // The name does not match, so the associatedFunctionDeclaration is
            // hidding the base class declaration.
            printf("This typedef is NOT visible from where it is referenced "
                   "(declaration with same name does not match) \n");
        ...
        case V_SgEnumSymbol: {
          ...
          if (associatedEnumDeclaration->get_firstNondefiningDeclaration() ==
            ...
            // The name does not match, so the associatedFunctionDeclaration is
            // hidding the base class declaration.
            printf("This enum is NOT visible from where it is referenced "
                   "(declaration with same name does not match) \n");
      ...
      // This class is visible from where it is referenced.
      printf("This class is NOT visible from where it is referenced (no "
  ...
  printf("In HiddenListTraversal::nameQualificationDepth(): Report type "
         "elaboration: typeElaborationIsRequired = %s \n",
...
HiddenListTraversal::getDeclarationAssociatedWithType(SgType *type) {
  ...
  // ROSE_ASSERT(declaration != NULL);
  if (declaration == NULL) {
    printf("In getDeclarationAssociatedWithType(): declaration == NULL type = "
           "%s \n",
           type->class_name().c_str());
  } else {
    printf("In getDeclarationAssociatedWithType(): declaration = %p \n",
           declaration);
    printf("In getDeclarationAssociatedWithType(): "
...
void HiddenListTraversal::evaluateNameQualificationForTemplateArgumentList(
    SgTemplateArgumentPtrList &templateArgumentList,
  ...
  printf(
      "Inside of "
      "HiddenListTraversal::evaluateNameQualificationForTemplateArgumentList():"
    ...
    if (type != NULL) {
      ...
      if (namedType != NULL) {
        ...
        if (templateArgumentTypeDeclaration != NULL) {
          ...
          // nested types). DQ (5/15/2011): Added recursive handling of template
          // arguments which can require name qualification.
          printf("xxxxxx --- Making a RECURSIVE call to "
                 "nameQualificationDepth() on the template argument \n");
          int amountOfNameQualificationRequiredForTemplateArgument =
              nameQualificationDepth(namedType, currentScope,
                                     positionStatement);
          printf("xxxxxx --- "
                 "amountOfNameQualificationRequiredForTemplateArgument = %d \n",
                 amountOfNameQualificationRequiredForTemplateArgument);

          printf("xxxxxx --- Must call a function to set the name "
      ...
      // DQ (6/20/2011): This might be a variable reference requiring name
      // qualification (see test2011_87.C and test2011_88.C).
      ...
      if (expression != NULL) {
        ...
        // qualification.  If we we have to traverse this expression
        // recursively.
        ...
        printf("Recursive call to newBuildHiddenTypeAndDeclarationLists() with "
               "expression = %p = %s \n",
               expression, expression->class_name().c_str());
        newBuildHiddenTypeAndDeclarationLists(expression, referencedNameSet);
        printf(
            "DONE: Recursive call to newBuildHiddenTypeAndDeclarationLists() "
            "with expression = %p = %s \n",
            expression, expression->class_name().c_str());
      }
    }
...
int HiddenListTraversal::nameQualificationDepth(
  ...
  if (declaration != NULL) {
    ...
    // This is where the template argument hide, they must be evaluated for name
    // qualification requirements.
    SgTemplateInstantiationDecl *templateInstatiationDecl =
        isSgTemplateInstantiationDecl(declaration);
...
// int HiddenListTraversal::nameQualificationDepthForType ( SgInitializedName*
// initializedName, SgStatement* positionStatement )
int HiddenListTraversal::nameQualificationDepthForType(
  ...
  // return
  // nameQualificationDepth(initializedName->get_type(),initializedName->get_scope(),positionStatement);
  return nameQualificationDepth(initializedName->get_type(), currentScope,
                                positionStatement);
}
...
int HiddenListTraversal::nameQualificationDepth(
  ...
  printf("In HiddenListTraversal::nameQualificationDepth(SgInitializedName* = "
         "%p): symbol = %p \n",
         initializedName, symbol);
  if (symbol != NULL) {
    printf("Lookup symbol based on name only (via parents starting at "
           "currentScope = %p = %s: name = %s symbol = %p = %s) \n",
           currentScope, currentScope->class_name().c_str(), name.str(), symbol,
           symbol->class_name().c_str());
    ...
    if (variableSymbol == NULL) {
      ...
      if (variableSymbol != NULL) {
        ...
        printf("In "
               "HiddenListTraversal::nameQualificationDepth(SgInitializedName*,"
               "SgScopeStatement*,SgStatement*): variableSymbol == NULL \n");
      }
      ...
      // DQ (6/4/2011): Make sure we have the correct symbol, else we have
      // detected a collision which will require name qualification to resolve.
      if (variableSymbol == targetInitializedNameSymbol) {
        ...
        // match.
        printf("These symbols only match based on name and is not the "
    ...
    if (declaration != NULL) {
      ...
      printf(
          "In HiddenListTraversal::nameQualificationDepth(SgInitializedName*): "
          "declaration == NULL, why is this? initializedName->get_scope() = %p "
          "= %s \n",
...
void HiddenListTraversal::addToNameMap(SgNode *nodeReferenceToType,
                                       string typeNameString) {
  // DQ (6/21/2011): This is refactored code used in traverseType() and
  // traverseTemplatedFunction().
  bool isTemplateName = (typeNameString.find('<') != string::npos) &&
                        (typeNameString.find("::") != string::npos);
  if (isTemplateName == true) {
    if (typeNameMap.find(nodeReferenceToType) == typeNameMap.end()) {
      printf("============== Inserting qualifier for name = %s into list at IR "
             "node = %p = %s \n",
             typeNameString.c_str(), nodeReferenceToType,
             nodeReferenceToType->class_name().c_str());
      typeNameMap.insert(std::pair<SgNode *, std::string>(nodeReferenceToType,
                                                          typeNameString));
    } else {
      // If it already existes then overwrite the existing information.
      SgUnorderedMapNodeToString::iterator i =
          typeNameMap.find(nodeReferenceToType);
      ROSE_ASSERT(i != typeNameMap.end());

      string previousTypeName = i->second.c_str();
      printf("WARNING: replacing previousTypeName = %s with new typeNameString "
             "= %s for nodeReferenceToType = %p = %s \n",
             previousTypeName.c_str(), typeNameString.c_str(),
      ...
      if (i->second != typeNameString) {
        ...
        printf("Error: name in qualifiedNameMapForNames already exists and is "
               "different... nodeReferenceToType = %p = %s \n",
               nodeReferenceToType, nodeReferenceToType->class_name().c_str());
        ROSE_ABORT();
      }
    }
...
void HiddenListTraversal::traverseType(SgType *type,
                                       SgNode *nodeReferenceToType,
                                       SgScopeStatement *currentScope,
                                       SgStatement *positionStatement) {
  // The type can contain subtypes (e.g. template arguments) and when the
  // subtypes need to be name qualificed the name of the encompassing type has a
  ...
  if (skipThisType == false) {
    ...
    // This is symptematic of an error which causes the whole class to be
    // included with the class definition.  This was fixed by calling
    // unparseInfoPointer->set_SkipClassDefinition() above.
    if (typeNameString.length() > 2000) {
      printf("Error: type names should not be this long... "
...
void HiddenListTraversal::traverseTemplatedFunction(
    SgFunctionRefExp *functionRefExp, SgNode *nodeReferenceToType,
    SgScopeStatement *currentScope, SgStatement *positionStatement) {
  // Called using
  ...
  // printf ("Inside of traverseTemplatedFunction functionRefExp = %p
  // currentScope = %p = %s
  // \n",functionRefExp,currentScope,currentScope->class_name().c_str());

  printf("<<<<< Starting traversal of traverseTemplatedFunction functionRefExp "
         "= %p currentScope = %p = %s \n",
         functionRefExp, currentScope, currentScope->class_name().c_str());
  ...
  if (skipThisFunction == false) {
    ...
    if (functionNameString.length() > 2000) {
      printf("Error: function names should not be this long... "
  ...
  printf("<<<<< Ending traversal of traverseTemplatedFunction functionRefExp = "
         "%p currentScope = %p = %s \n",
         functionRefExp, currentScope, currentScope->class_name().c_str());
}

bool HiddenListTraversal::
  ...
  if (referencedNameSet.find(declarationToSearchForInReferencedNameSet) ==
    ...
    printf("$$$$$$$$$$ NOT Found: declaration %p = %s in referencedNameSet "
           "referencedNameSet.size() = %" PRIuPTR " \n",
           declaration, declaration->class_name().c_str(),
...
HiddenListInheritedAttribute HiddenListTraversal::evaluateInheritedAttribute(
    SgNode *n, HiddenListInheritedAttribute inheritedAttribute) {
  ROSE_ASSERT(n != NULL);

  // printf ("Inside of HiddenListTraversal::evaluateInheritedAttribute(): node
  ...
  if (templateClassDefinition != NULL) {
    printf("Name qualification of SgTemplateClassDefinition = %p not "
           "implemented \n",
  ...
  if (classDefinition != NULL) {
    ...
    printf("!!!!! Evaluate the derived classes: are they visible --- "
    ...
    while (i != baseClassList.end()) {
      ...
      printf(
          "Calling nameQualificationDepth() for classDeclaration name = %s \n",
          classDeclaration->get_name().str());

      int amountOfNameQualificationRequired = nameQualificationDepth(
          classDeclaration, currentScope, classDefinition);

      printf("amountOfNameQualificationRequired = %d \n",
             amountOfNameQualificationRequired);

      setNameQualification(baseClass, classDeclaration,
                           amountOfNameQualificationRequired);
  ...
  if (classDeclaration != NULL) {
    ...
    if (currentScope != NULL) {
      ...
      if (currentScope != classDeclaration->get_scope()) {
        ...
        printf("WARNING: SgClassDeclaration -- currentScope is not available "
               "through predicate (currentScope != "
               "classDeclaration->get_scope()), not clear why! \n");

        ROSE_ASSERT(classDeclaration->get_parent() ==
                    classDeclaration->get_scope());
        printf("name qualification for classDeclaration->get_scope()  = %p = "
               "%s \n",
               classDeclaration->get_scope(),
               classDeclaration->get_scope()->class_name().c_str());
        printf("classDeclaration->get_parent()                        = %p = "
               "%s \n",
  ...
  if (variableDeclaration != NULL) {
    ...
    // SgScopeStatement* currentScope =
    // isSgScopeStatement(variableDeclaration->get_parent());
    ROSE_ASSERT(currentScope != NULL);

    printf("================ Calling nameQualificationDepthForType to evaluate "
           "the type \n");
    ...
    // nameQualificationDepthForType(initializedName,variableDeclaration);
    int amountOfNameQualificationRequiredForType =
        nameQualificationDepthForType(initializedName, currentScope,
                                      variableDeclaration);
    printf("SgVariableDeclaration's type: "
           "amountOfNameQualificationRequiredForType = %d \n",
           amountOfNameQualificationRequiredForType);

    // Not all types have an associated declaration, but some do: examples
    // include classes, typedefs, and enums.
    SgDeclarationStatement *declaration =
        getDeclarationAssociatedWithType(initializedName->get_type());
    if (declaration != NULL) {
      printf("Putting the name qualification for the type into the "
             "SgInitializedName = %p = %s \n",
             initializedName, initializedName->get_name().str());
      setNameQualification(initializedName, declaration,
                           amountOfNameQualificationRequiredForType);
    } else {
      printf("declaration == NULL: could not put name qualification for the "
             "type into the SgInitializedName = %p = %s \n",
             initializedName, initializedName->get_name().str());
    }

    printf("\n++++++++++++++++ Calling nameQualificationDepth to evaluate the "
           "name currentScope = %p = %s \n",
           currentScope, currentScope->class_name().c_str());
    int amountOfNameQualificationRequiredForName = nameQualificationDepth(
        initializedName, currentScope, variableDeclaration);
    printf("SgVariableDeclaration's variable name: "
           "amountOfNameQualificationRequiredForName = %d \n",
           amountOfNameQualificationRequiredForName);

    // DQ (6/5/2011): Debugging test2011_75.C ...

    // If there is a previous declaration then that is the original declaration
    // (see test2011_30.C for an example of this case).
    SgInitializedName *originalInitializedName =
        initializedName->get_prev_decl_item();
    ...
    if (originalInitializedName != NULL) {
      ...
      // DQ (6/5/2011): Handle case where originalInitializedName->get_parent()
      // == NULL ROSE_ASSERT(originalInitializedName->get_parent() != NULL);
      if (originalInitializedName->get_parent() != NULL) {
        SgVariableDeclaration *originalVariableDeclaration =
            isSgVariableDeclaration(originalInitializedName->get_parent());
        ...
        if (originalVariableDeclaration != NULL) {
          // Accumulate names of scopes starting at the
          // originalVariableDeclaration
          // (amountOfNameQualificationRequiredForName times).
          setNameQualification(variableDeclaration, originalVariableDeclaration,
                               amountOfNameQualificationRequiredForName);
        } else {
          // DQ (6/4/2011): test2005_68.C is an example that fails this test
          // (also test2011_70.C).  If the SgInitializedName is referenced
          // before it is declared then it can appear associated via the symbol
          // table through a non variable declaration (SgVariableDeclaration) IR
          // node.
          printf("WARNING: variableDeclaration case of name qualification "
                 "originalVariableDeclaration == NULL (building "
                 "alternativeDecaration) \n");
          SgFunctionDeclaration *alternativeDecaration =
              SageInterface::getEnclosingFunctionDeclaration(
                  originalInitializedName->get_parent(), true);
          ROSE_ASSERT(alternativeDecaration != NULL);
          setNameQualification(variableDeclaration, alternativeDecaration,
                               amountOfNameQualificationRequiredForName);
        }
      } else {
        // This is demonstrated by test2011_72.C (and maybe test2005_103.C).  I
        // can't really fix this in the AST until I can get all of the test
        // codes back to passing with the new name qualification support, so
        // this will have to wait.  We output an error message for now.
        printf("Error: originalInitializedName->get_parent() == NULL (skipping "
               "any required name qualification, I think this is a bug in the "
               "AST) \n");
      }
    }
    printf("++++++++++++++++ DONE: Calling nameQualificationDepth to evaluate "
           "the name \n\n");
  }
  ...
  if (initializedName != NULL) {
    // We want to handle types from every where a SgInitializedName might be
    // used.
    SgDeclarationStatement *declaration =
        getDeclarationAssociatedWithType(initializedName->get_type());
    if (declaration != NULL) {
      ...
      int amountOfNameQualificationRequiredForType =
          nameQualificationDepthForType(initializedName, currentScope,
                                        currentStatement);
      printf("SgInitializedName's (%s) type: "
             "amountOfNameQualificationRequiredForType = %d \n",
             initializedName->get_name().str(),
             amountOfNameQualificationRequiredForType);
      ...
      // Traverse the type to set any possible template arguments (or other
      // subtypes?) that require name qualification.
  ...
  if (functionDeclaration != NULL && isSgMemberFunctionDeclaration(n) == NULL) {
    ...
    // SgScopeStatement* currentScope =
    // TransformationSupport::getScope(functionDeclaration); SgScopeStatement*
    ...
    if (currentScope != NULL) {
      ...
      if (functionDeclaration->get_specialFunctionModifier().isOperator() ==
        ...
        // allow it to be handled with greater precission. Never use name
        // qualification for friend functions or operators. I am more sure of
        // the case of friend functions than operators. Friend functions will
        // have a global scope (though this might change in the future; google
        // "friend global scope injection"). printf ("Detected a friend or
        // operator function, these are not provided with name qualification.
        // \n");
        printf("Detected a operator function, these are not provided with name "
               "qualification. \n");
      } else {
        // Only use name qualification where the scopes of the declaration's use
        ...
        printf("I would like to not have to have this SgFunctionDeclaration "
               "logic, we should get the name qualification correct more "
               "directly. \n");
        if (currentScope != functionDeclaration->get_scope()) {
          int amountOfNameQualificationRequired = nameQualificationDepth(
              functionDeclaration, currentScope, functionDeclaration);
          printf("SgFunctionDeclaration: amountOfNameQualificationRequired = "
                 "%d \n",
                 amountOfNameQualificationRequired);
          // DQ (21/2011): test2011_89.C demonstrates a case where name
          // wualification of a functionRef expression is required. DQ
          ...
          // where it could be expected to be defined (e.g. not using a forward
          // declaration in a SgBasicBlock, since the function definition could
          // not live in the SgBasicBlock.
      ...
      // foobar (T x){ }".
      printf("WARNING: SgFunctionDeclaration -- currentScope is not available, "
             "not clear why! \n");
      // ROSE_ABORT();
    }
  }
  ...
  if (memberFunctionDeclaration != NULL) {
    ...
    if (currentScope != NULL) {
      ...
      traverseType(returnType, memberFunctionDeclaration, currentScope,
                   memberFunctionDeclaration);
      printf("Don't forget possible covariant return types for "
             "SgMemberFunctionDeclaration IR nodes \n");

      // Only use name qualification where the scopes of the declaration's use
      // (currentScope) is not the same as the scope of the function
      // declaration.  However, the analysis should work and determin that the
      // required name qualification length is zero.
      printf("I would like to not have to have this "
      ...
      if (currentScope != memberFunctionDeclaration->get_scope()) {
        ...
        printf("SgMemberFunctionDeclaration: amountOfNameQualificationRequired "
               "= %d \n",
               amountOfNameQualificationRequired);
        ...
        printf("WARNING: SgMemberFunctionDeclaration -- currentScope is not "
      ...
      if (currentScope != NULL) {
        ...
        printf("WARNING: SgMemberFunctionDeclaration -- currentScope is not "
               "available through parent SgDeclarationStatement, not clear "
               "why! \n");
        ROSE_ABORT();
      }
  ...
  if (typedefDeclaration != NULL) {
    ...
    printf("typedefDeclaration->get_typedefBaseTypeContainsDefiningDeclaration("
           ") = %s \n",
           typedefDeclaration->get_typedefBaseTypeContainsDefiningDeclaration()
               ? "true"
               : "false");

    // This is NULL if the base type is not associated with a declaration (e.g.
    // not a SgNamedType). ROSE_ASSERT(baseTypeDeclaration != NULL); if
    ...
    if ((baseTypeDeclaration != NULL) &&
      ...
      setNameQualification(typedefDeclaration, baseTypeDeclaration,
                           amountOfNameQualificationRequiredForBaseType);
    }
  ...
  if (usingDirective != NULL) {
    ...
    printf("SgUsingDirectiveStatement's SgNamespaceDeclarationStatement: "
  ...
  if (usingDeclaration != NULL) {
    ...
    printf("In case for SgUsingDeclarationStatement: associatedDeclaration = "
           "%p associatedInitializedName = %p \n",
    ...
    int amountOfNameQualificationRequired = 0;
    if (associatedDeclaration != NULL) {
      // DQ (6/22/2011): The declaration in a SgUsingDeclarationStatement must
      // have some qualification else it is not required and will be an error.
      // To avoid it being confused with a name in the current scope we have to
      // start the process assuming the scope of the current scope. This is a
      // special case in the name qualification handling. But we also do not
      // need the forceMoreNameQualification mechanism (I think). DQ
      // (5/14/2011): For the case of test2001_46.C I think we need to force
      // name qualification. In general any reference using a
      // SgUsingDeclarationStatement should likely be qualified (at least one
      // level, via the parent of the associatedDeclaration).
      // amountOfNameQualificationRequired =
      // nameQualificationDepth(associatedDeclaration,currentScope,usingDeclaration);
      // amountOfNameQualificationRequired =
      // nameQualificationDepth(associatedDeclaration,currentScope,usingDeclaration,
      // /* forceMoreNameQualification = */ true);
      amountOfNameQualificationRequired = nameQualificationDepth(
          associatedDeclaration, currentScope->get_scope(), usingDeclaration,
          /* forceMoreNameQualification = */ false);

      // DQ (6/22/2011): If the amountOfNameQualificationRequired is zero then
      // add one to force at least global qualification.
      if (isSgGlobal(currentScope->get_scope()) != NULL &&
    ...
    printf("SgUsingDeclarationStatement's SgVarRefExp: "
           "amountOfNameQualificationRequired = %d \n",
           amountOfNameQualificationRequired);
  }
  ...
  if (functionRefExp != NULL) {
    ...
    if (functionDeclaration != NULL) {
      ...
      printf("SgFunctionCallExp's function name: "
             "amountOfNameQualificationRequired = %d \n",
             amountOfNameQualificationRequired);

      // DQ (6/9/2011): Support for test2011_78.C (we only qualify function call
      // references where the function has been declared in a scope where it
      // could be expected to be defined (e.g. not using a forward declaration
      // in a SgBasicBlock, since the function definition could not live in the
      // SgBasicBlock.
      bool skipNameQualification =
          skipNameQualificationIfNotProperlyDeclaredWhereDeclarationIsDefinable(
              functionDeclaration);
      ...
      printf("WARNING: functionDeclaration == NULL in SgFunctionCallExp for "
             "name qualification support! \n");
    }

    // If this is a templated function then we have to save the name becuase its
  ...
  if (memberFunctionRefExp != NULL) {
    ...
    if (memberFunctionDeclaration != NULL) {
      ...
      printf("SgMemberFunctionCallExp's member function name: "
             "amountOfNameQualificationRequired = %d \n",
             amountOfNameQualificationRequired);

      // DQ (6/5/2011): test2005_112.C demonstrates a case where this special
      // case applies. If this had to be more qualified,
      // (amountOfNameQualificationRequired > 0), then it should be sufficently
      // qualified and not need special case handling.
      if (amountOfNameQualificationRequired == 0) {
        // GNU reports that: "ISO C++ forbids taking the address of an
        // unqualified or parenthesized non-static member function to form a
        // pointer to member function.  Say '&A::f1'"
        ROSE_ASSERT(memberFunctionRefExp->get_parent() != NULL);
        SgAddressOfOp *addressOfOp =
            isSgAddressOfOp(memberFunctionRefExp->get_parent());
        if (addressOfOp != NULL) {
          // I think that setting this to 1 is enough, but there could be a case
          // where it must be more qualified than just to include it's qualified
          // class name.
          amountOfNameQualificationRequired = 1;
        }
      }
      ...
      printf("WARNING: memberFunctionDeclaration == NULL in "
  ...
  if (constructorInitializer != NULL) {
    ...
    if (memberFunctionDeclaration != NULL) {
      int amountOfNameQualificationRequired = nameQualificationDepth(
          memberFunctionDeclaration, currentScope, currentStatement);
      printf("SgConstructorInitializer's constructor member function name: "
             "amountOfNameQualificationRequired = %d \n",
             amountOfNameQualificationRequired);
      ...
      // DQ (6/1/2011): This happens when there is not constructor that can be
      // used to build a class, in this case the class name must be used to
      // define a default constructor. This is a problem for test2004_130.C (at
      // line 165 col = 14 file under the build-tree compiler header staging
      // area, e.g., g++_HEADERS/hdrs3/bits/stl_iterator_base_types.h).
      ...
      if (classDeclaration != NULL) {
        ...
        printf("WARNING: In SgConstructorInitializer name qualification "
               "support: neither memberFunctionDeclaration or classDeclaration "
               "are valid pointers. \n");
      }
    }
  }
  ...
  if (varRefExp != NULL) {
    ...
    // DQ (5/30/2011): Handle the case of test2011_58.C (index declaration in
    // for loop construct). SgScopeStatement* currentScope =
    ...
    if (variableDeclaration == NULL) {
      ...
      if (initializedName->get_name() == "__PRETTY_FUNCTION__" ||
        ...
        // If this is a SgInitializedName from a function parameter list then it
        ...
        if (functionParameterList != NULL) {
          printf("Names from function parameter list can not be name "
                 "qualified: name = %s \n",
                 initializedName->get_name().str());
        } else {
          printf("varRefExp's initialized name = %s is not associated with a "
          ...
          SgStatement *currentStatement = SageInterface::getEnclosingStatement(
          ...
          printf("SgVarRefExp case (no associated variableDeclaration): "
                 "currentStatement = %p = %s \n",
                 currentStatement, currentStatement->class_name().c_str());
      ...
      printf("SgVarRefExp's SgDeclarationStatement: "
             "amountOfNameQualificationRequired = %d \n",
             amountOfNameQualificationRequired);
  ...
  if (enumVal != NULL) {
    ...
    int amountOfNameQualificationRequired =
        nameQualificationDepth(enumDeclaration, currentScope, currentStatement);
  ...
  if (newExp != NULL || sizeOfOp != NULL || castExp != NULL ||
    ...
    case V_SgSizeOfOp: {
      ...
      if (qualifiedType == NULL) {
        // This is the case of a value, which need not be qualified. Excpet that
    ...
    if (skipQualification == false) {
      ...
      if (associatedTypeDeclaration != NULL) {
        ...
        printf("SgExpression (name = %s) type: "
               "amountOfNameQualificationRequiredForType = %d \n",
        ...
        printf("Note: associatedTypeDeclaration == NULL in SgExpression for "
               "name qualification support! referenceToType = %s \n",
               referenceToType->class_name().c_str());
      }
    }
  }
  ...
  if (expression != NULL) {
    ...
    if (originalExpressionTree != NULL) {
      ...
      // test2011_89.C).
      printf("@@@@@@@@@@@@@ Recursive call to the originalExpressionTree = %p "
             "= %s \n",
             originalExpressionTree,
             originalExpressionTree->class_name().c_str());
      newBuildHiddenTypeAndDeclarationLists(originalExpressionTree,
                                            referencedNameSet);
      printf("@@@@@@@@@@@@@ DONE: Recursive call to the originalExpressionTree "
             "= %p = %s \n",
             originalExpressionTree,
             originalExpressionTree->class_name().c_str());
    }
  ...
  // ******************************************************************************
  // Now that this declaration is pocessed, mark it as being seen (place into
  // set).
  // ******************************************************************************
  ...
  if (declaration != NULL) {
    ...
    if (declarationForReferencedNameSet == NULL) {
      ...
      // classes and some onther sorts of declarations).
    ...
    // pointer to function, etc.) ROSE_ASSERT(scopeOfNondefiningDeclaration !=
    // NULL);
    ...
    if (acceptableDeclarationScope == true &&
        referencedNameSet.find(declarationForReferencedNameSet) ==
            referencedNameSet.end()) {
      // printf ("Adding firstNondefiningDeclaration = %p = %s to set of visited
      // declarations
      ...
      printf("Adding declarationForReferencedNameSet = %p = %s to set of "
      ...
      // printf ("firstNondefiningDeclaration = %p NOT added to
      // referencedNameSet \n",firstNondefiningDeclaration);
      printf("declarationForReferencedNameSet = %p NOT added to "
             "referencedNameSet \n",
             declarationForReferencedNameSet);
    }
  }
...
HiddenListTraversal::evaluateSynthesizedAttribute(
    SgNode *n, HiddenListInheritedAttribute inheritedAttribute,
...
void HiddenListTraversal::setNameQualification(
  ...
  string qualifier = setNameQualificationSupport(
      variableDeclaration->get_scope(), amountOfNameQualificationRequired,
  ...
  if (qualifiedNameMapForNames.find(varRefExp) ==
    ...
    printf(
        "WARNING: replacing previousQualifier = %s with new qualifier = %s \n",
...
void HiddenListTraversal::setNameQualification(
  ...
  if (qualifiedNameMapForNames.find(functionRefExp) ==
      qualifiedNameMapForNames.end()) {
    printf("Inserting qualifier for (SgFunctionRefExp) name = %s into list at "
           "IR node = %p = %s \n",
...
// HiddenListTraversal::setNameQualification(SgConstructorInitializer*
...
void HiddenListTraversal::setNameQualification(
  ...
  string qualifier = setNameQualificationSupport(
      enumDeclaration->get_scope(), amountOfNameQualificationRequired,
      outputNameQualificationLength, outputGlobalQualification,
      outputTypeEvaluation);
...
void HiddenListTraversal::setNameQualification(
  ...
  string qualifier = setNameQualificationSupport(
      classDeclaration->get_scope(), amountOfNameQualificationRequired,
      outputNameQualificationLength, outputGlobalQualification,
      outputTypeEvaluation);
...
void HiddenListTraversal::setNameQualification(
  ...
  printf("In HiddenListTraversal::setNameQualification(): "
         ...
         functionDeclaration->get_type_elaboration_required() ? "true"
                                                              : "false");
  printf("In HiddenListTraversal::setNameQualification(): "
  ...
  printf("In HiddenListTraversal::setNameQualification(): functionDeclaration "
         "= %p firstNondefiningDeclaration() = %p \n",
         functionDeclaration,
         functionDeclaration->get_firstNondefiningDeclaration());
  printf("In HiddenListTraversal::setNameQualification(): functionDeclaration "
         "= %p definingDeclaration()         = %p \n",
...
// void HiddenListTraversal::setNameQualificationReturnType (
...
void HiddenListTraversal::setNameQualificationReturnType(
  ...
  // setNameQualificationSupport(functionDeclaration->get_scope(),amountOfNameQualificationRequired,
  // outputNameQualificationLength, outputGlobalQualification,
  // outputTypeEvaluation);
  string qualifier = setNameQualificationSupport(
      declaration->get_scope(), amountOfNameQualificationRequired,
  ...
  printf("In HiddenListTraversal::setNameQualification(): "
         ...
         functionDeclaration->get_type_elaboration_required_for_return_type()
             ...
             : "false");
  printf(
      "In HiddenListTraversal::setNameQualification(): "
...
// void HiddenListTraversal::setNameQualification(SgInitializedName*
...
void HiddenListTraversal::setNameQualification(
    ...
    int amountOfNameQualificationRequired) {
  // This is used to set the name qualification on the return type referenced by
  // the SgInitializedName, and not on the SgInitializedName IR node itself.
  ...
  if (qualifiedNameMapForTypes.find(initializedName) ==
      qualifiedNameMapForTypes.end()) {
    printf("Inserting qualifier for type = %s into list at SgInitializedName "
           "IR node = %p = %s \n",
    ...
    if (i->second != qualifier) {
      ...
      printf("WARNING: name in qualifiedNameMapForTypes already exists and is "
             "different... \n");
      // ROSE_ABORT();
    }
  }
...
void HiddenListTraversal::setNameQualification(
  ...
  printf("In HiddenListTraversal::setNameQualification(): "
         ...
         variableDeclaration->get_type_elaboration_required() ? "true"
                                                              : "false");
  printf("In HiddenListTraversal::setNameQualification(): "
  ...
  if (qualifiedNameMapForNames.find(variableDeclaration) ==
      qualifiedNameMapForNames.end()) {
    printf("Inserting qualifier for name = %s into list at "
...
void HiddenListTraversal::setNameQualification(
  ...
  // recall.
  ROSE_ASSERT(outputTypeEvaluation == false);
  printf("In HiddenListTraversal::setNameQualification(): templateArgument     "
         "                                 = %p \n",
         templateArgument);
  printf("In HiddenListTraversal::setNameQualification(): "
  ...
  if (qualifiedNameMapForTypes.find(templateArgument) ==
      qualifiedNameMapForTypes.end()) {
    printf("Inserting qualifier for name or type = %s into list at IR node = "
           "%p = %s \n",
...
// void HiddenListTraversal::setNameQualification(SgCastExp* castExp,
...
void HiddenListTraversal::setNameQualification(
  ...
  string qualifier = setNameQualificationSupport(
      typeDeclaration->get_scope(), amountOfNameQualificationRequired,
      outputNameQualificationLength, outputGlobalQualification,
      outputTypeEvaluation);
...
void HiddenListTraversal::setNameQualification(
  ...
  printf("In HiddenListTraversal::setNameQualification(): "
         ...
         classDeclaration->get_global_qualification_required() ? "true"
                                                               : "false");

  if (qualifiedNameMapForNames.find(classDeclaration) ==
      qualifiedNameMapForNames.end()) {
    printf("Inserting qualifier for name = %s into list at SgClassDeclaration "
           "IR node = %p = %s \n",
...
string HiddenListTraversal::setNameQualificationSupport(
  ...
  printf("In HiddenListTraversal::setNameQualificationSupport(): scope = %p = "
         "%s = %s inputNameQualificationLength = %d \n",
         scope, scope->class_name().c_str(),
  ...
  for (int i = 0; i < inputNameQualificationLength; i++) {
    printf("   --- In loop: scope = %p = %s = %s \n", scope,
           scope->class_name().c_str(), SageInterface::get_name(scope).c_str());
    ...
    if (templateClassDefinition != NULL) {
      ...
      ROSE_ASSERT(unparseInfoPointer != NULL);
      unparseInfoPointer->set_outputCompilerGeneratedStatements();
      templateClassDeclaration->get_file_info()->display(
          "SgTemplateInstantiationDecl trying to generate the qualified name: "
          "debug");
      ...
      while (i != templateArgumentList.end()) {
        ...
        string template_argument_name =
            globalUnparseToString(templateArgument, unparseInfoPointer);
    ...
    if (globalScope != NULL) {
      ...
      printf("!!!!! We have iterated beyond the number of nested scopes: "
    ...
    if (outputGlobalQualification == true) {
      ...
      if (scope_name.length() == 0) {
        // Nothing to do for this case of an empty string for a scope name (see
        // test2006_121.C).
      } else {
        qualifierString = scope_name + "::" + qualifierString;
      }
  ...
  printf(
      "In HiddenListTraversal::setNameQualificationSupport(): "
