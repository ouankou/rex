...
class HiddenListTraversal
    : public AstTopDownBottomUpProcessing<HiddenListInheritedAttribute,
                                          HiddenListSynthesizedAttribute> {
private:
  ...
  // could refernece the versions in SgNode directly. Initially in the design
  ...
  // nexted types in templates since the unparser uses the SgNode static members
  // directly.  so the switch to make this a reference fixes this problem.
...
public:
  ...
  // class.
  virtual HiddenListInheritedAttribute
  evaluateInheritedAttribute(SgNode *n,
                             HiddenListInheritedAttribute inheritedAttribute);

  virtual HiddenListSynthesizedAttribute evaluateSynthesizedAttribute(
  ...
  // DQ (6/5/2011): Added to support case where SgInitializedName in SgVarRefExp
  // can't be traced back to a SgVariableDeclaration (see test2011_75.C).
  void setNameQualification(SgVarRefExp *varRefExp,
  ...
  void setNameQualification(SgInitializedName *initializedName,
                            SgDeclarationStatement *declaration,
                            int amountOfNameQualificationRequired);
  void setNameQualification(SgVariableDeclaration *variableDeclaration,
                            SgDeclarationStatement *declaration,
                            int amountOfNameQualificationRequired);
  void setNameQualification(SgTypedefDeclaration *typedefDeclaration,
                            SgDeclarationStatement *declaration,
                            int amountOfNameQualificationRequired);
  ...
  // DQ (6/21/2011): Added support to generate function names containging
  // template arguments.
