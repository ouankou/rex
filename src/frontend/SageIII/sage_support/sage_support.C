                      ...
                      else
                       {
                         if (CommandlineProcessing::isCFileNameSuffix(
                           ...
                           // DQ (11/25/2020): Add support to set this as a specific language kind file (there is at least one language kind file processed by ROSE).
                              ...
                              // Default C standard is handled in cmdline.C; avoid forcing GNU here.
               ...
               for (SgFile* file : get_fileList())
                  {
                    ...
                    std::string normalizedPath =
                        normalizePathIfPossible(sourceFile->getFileName());
                    ...
                    it != includedFilesMap.end(); ++it)
                  {
                    ...
                    for (set<string>::const_iterator incIt = includedSet.begin();
                         incIt != includedSet.end(); ++incIt)
                       {
                         ...
                         std::map<std::string, SgSourceFile*>::const_iterator
                             headerIt = sourceFilesByPath.find(includedPath);
                         SgSourceFile* headerFile =
                             headerIt != sourceFilesByPath.end()
                                 ? headerIt->second
                                 : NULL;
                         if (headerFile == NULL)
                            {
                         ...
                         if (headerFile == NULL)
                            {
                              ...
                              if (headerFile != NULL)
                                 {
                                   tokenMapFilesByPath[includedPath] =
                                       headerFile;
                                 }
                            }
                         if (headerFile == NULL)
  ...
  // compiled together on the same command line.
     ...
     if (get_experimental_flang_frontend() == true)
       ...
       // Rasmussen (3/12/2018): Modified call to include the source file.
          ...
          if (frontendErrorLevel == 0)
             {
               if (SgProject::get_verbose() > 1) MLOG_INFO_CXX(MLOG_FRONTEND) << "SUCCESS with call to experimental_FortranParser_main()\n";
             }
            else
             {
               MLOG_FATAL_CXX(MLOG_FRONTEND) << "Error returned from call to experimental_FortranParser_main(): frontendErrorLevel = "
                                             << frontendErrorLevel << "\n";
               ROSE_ABORT();
             }
        }
...
#define DEBUG_PROJECT_COMPILE_COMMAND_LINE_WITH_ARGS 0
  ...
  // Build the commandline to hand off to the C++/C compiler
     ...
     // If this is C-only but the suffix indicates C++, force C mode in the backend.
