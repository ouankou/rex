...
// This is a header file (even though it has a ".C" extension) which is
// #included in rose.h in order to define templates. Therefore it should include
// neither rose.h nor sage3basic.h. Instead, use the normal ROSE policy for
// header files: include <rose.h> or <sage3basic.h> before including
// <utility_functionsImpl.C>. If you don't believe this is really a header file,
// just look through the various makefiles and you'll find it lumped into lists
// of header files, but never in lists of source files.
// #include "rose.h"
...
struct generateDOTExtended {
  // generateDOTExtended ( const SgProject & project, std::string
  // filenamePostfix = "", ExtraNodeInfo_t eni =
  ...
  generateDOTExtended(const SgProject &project,
                      std::string filenamePostfix = "",
                      ExtraNodeInfo_t eni = ExtraNodeInfo_t(),
                      ExtraNodeOptions_t eno = ExtraNodeOptions_t(),
                      ExtraEdgeInfo_t eei = ExtraEdgeInfo_t(),
                      ExtraEdgeOptions_t eeo = ExtraEdgeOptions_t()) {
    ...
    AstDOTGenerationExtended<ExtraNodeInfo_t, ExtraNodeOptions_t,
                             ExtraEdgeInfo_t, ExtraEdgeOptions_t>
        astdotgen(eni, eno, eei, eeo);
    ...
    astdotgen.generateInputFiles(&nonconstProject,
                                 DOTGeneration<SgNode *>::TOPDOWNBOTTOMUP,
                                 filenamePostfix);
  }
};
...
struct generateDOTExtendedforMultipleFile {
  generateDOTExtendedforMultipleFile(
    ...
    // This is the best way to handle generation of DOT files where multiple
    // files are specified on the command line.  Later we may be able to filter
    // out the include files (but this is a bit difficult until
    // generateInputFiles() can be implemetned to call the evaluation of
    // inherited and synchizied attributes.
    generateDOTExtended_withIncludes<ExtraNodeInfo_t, ExtraNodeOptions_t,
                                     ExtraEdgeInfo_t, ExtraEdgeOptions_t>(
        project, filenamePostfix, eni, eno, eei, eeo);
  }
};
