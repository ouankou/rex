...
// "version_message()" functions. returns a string containing the current
// version message (includes the version number).
...
// agreed that we want to support multiple SgProject IR nodes.  So in addition
// to the SgProject* frontend() function we will in the future support:
//   1) A SgProject member function "frontend()" that can be used to generate a
//   new SgFile in
//      an existing SgProject.
//   2) A SgBuilder function for building an empty SgProject (without files).
//   3) We will remove the use of the SageInterface::getProject() (which asserts
//   that there
//      is only one SgProject).

// This builds a shell of a frontend SgProject with associated SgFile objects
// (but with empty SgGlobal objects) supporting only commandline processing and
// requiring the frontend to be called explicitly for each SgFile object.  See
// tutorial/selectedFileTranslation.C for example.
SgProject *frontendShell(int argc, char **argv);
ROSE_DLL_API SgProject *frontendShell(const std::vector<std::string> &argv);

// DQ (3/18/2006): Modified backend function interface to permit handling of
// user specified objects to control the formatting of code generation and the
...
// use on projects that target Compass or any other analysis only tool using
// ROSE. Called in tests/nonsmoke/functional/testAnalysis.C for example.
ROSE_DLL_API int
backendCompilesUsingOriginalInputFile(SgProject *project,
                                      bool compile_with_USE_ROSE_macro = false);

// DQ (2/6/2010): This backend forces all code to be generated but still uses
// the backend vendor compiler to compile the original code.  This is a step
// between backendUsingOriginalInputFile(), which does not generate code; and
// backend() which generated code and compiles it.  The use of this backend
// permits an intermediate test of robustness where the code that we generate
// might be generated incorrectly (usually with missing name qualification as
...
// globalBackendErrorCode); SgProject & frontend ( int argc, char * argv[] );

ROSE_DLL_API void generatePDF(const SgProject &project);

// DQ (12/20/2018): Added option to exclude template instantiations which make
// the graphs smaller and tractable t manage for larger C++ applications.
ROSE_DLL_API void generateDOT(const SgProject &project,
                              std::string filenamePostfix = "",
                              bool excludeTemplateInstantiations = false);

// DQ (9/22/2017): Adding support that is can work with any IR node, so that we
// can generated DOT files on untyped ASTs. I prefer the API that takes a SgNode
// pointer. ROSE_DLL_API void generateDOT ( SgNode* node, std::string
// baseFilename, std::string filenamePostfix = "" );
ROSE_DLL_API void generateDOT(SgNode *node, std::string filename);

// DQ (9/1/2008): Added function to generate the compete AST when specificed
// with multiple files on the command line.  This is the older default behavior
// of generateDOT (from last year, or so).
...
// DQ (6/14/2007): Support for whole AST graphs output with attributes (types,
// symbols, all edges, etc.) We define a default value for the maximum graph
// size (number of nodes).
ROSE_DLL_API void generateAstGraph(const SgProject *project, int maxSize = 2000,
                                   std::string filenameSuffix = "");
// void generateAstGraph ( const SgProject* project, int maxSize, std::string
// filenameSuffix = "", CustomMemoryPoolDOTGeneration::s_Filter_Flags*
// filter_flags = NULL)
...
template <typename ExtraNodeInfo_t =
              AstDOTGenerationExtended_Defaults::DefaultExtraNodeInfo,
          typename ExtraNodeOptions_t =
              AstDOTGenerationExtended_Defaults::DefaultExtraNodeOptions,
          typename ExtraEdgeInfo_t =
              AstDOTGenerationExtended_Defaults::DefaultExtraEdgeInfo,
          typename ExtraEdgeOptions_t =
              AstDOTGenerationExtended_Defaults::DefaultExtraEdgeOptions>
struct generateDOTExtended;
// void
// generateDOTExtended ( const SgProject & project, std::string filenamePostfix
// = "", ExtraNodeInfo_t eni =
// AstDOTGenerationExtended_Defaults::DefaultExtraNodeInfo(), ExtraNodeOptions_t
// eno = AstDOTGenerationExtended_Defaults::DefaultExtraNodeOptions(),
// ExtraEdgeInfo_t eei =
// AstDOTGenerationExtended_Defaults::DefaultExtraEdgeInfo(), ExtraEdgeOptions_t
// eeo = AstDOTGenerationExtended_Defaults::DefaultExtraEdgeOptions() );
...
namespace Rose {
// This class serves as a catch all location for functions of general use within
// ROSE we have added variables that are set using command line parameters to
// avoid the use of global variables.

// DQ (3/6/2017): Adding ROSE options data structure to support frontend and
// backend options such as:
//    1) output of warnings from the frontend.
//    2) output of notes from the frontend.
//    3) output of warnings from the backend compiler.
// The reason for this options clas is to support tools that would want to
// suppress warnings from ROSE, and still also permit compilers that would be
// implemented using ROSE to have the full range of output from the frontend to
// control notes and warnings.
class Options {
private:
  bool frontend_notes;
...
// Global variable (in this rose namespace) to permit multiple parts of ROSE to
// access consistant information on options.
ROSE_DLL_API extern Options global_options;

// DQ (8/10/2004): This was moved to the SgFile a long time ago and should not
// be used any more) DQ (8/11/2004): Need to put this back so that there is a
// global concept of verbosity for all of ROSE. static int roseVerboseLevel;
...
int containsString(const std::string &masterString,
...
// DQ (5/25/2005): Removed from ROSE class (since they are redundant with other
// uses)
std::string
getFileName(SgLocatedNode *locatedNodePointer) ROSE_DEPRECATED_FUNCTION;
...
ROSE_DLL_API std::string getSourceDirectory(
    std::string fileNameWithPath); //! get the sourceDirectory directory

std::string getFileNameWithoutPath(SgStatement *statementPointer);
ROSE_DLL_API std::string utility_stripPathFromFileName(
    const std::string
        &fileNameWithPath); //! get the filename from the full filename

ROSE_DLL_API std::string getPathFromFileName(
    std::string fileNameWithPath); //! get the path from the full filename

// DQ (9/8/2008): This is removed since it is redundant with the version in
// StringUtility. std::string stripFileSuffixFromFileName ( const std::string&
// fileNameWithSuffix ); //! get the name without the ".C"
...
void filterInputFile(const std::string inputFileName,
...
// node. extern std::map<SgNode*,TokenStreamSequenceToNodeMapping*>
// tokenSubsequenceMap;
extern std::map<int, std::map<SgNode *, TokenStreamSequenceToNodeMapping *> *>
    tokenSubsequenceMapOfMaps;

// DQ (1/19/2021): This is part of moving to a new map that uses the
// SgSourceFile pointer instead of the fild_id.
extern std::map<SgSourceFile *,
...
// representativeWhitespaceStatementMap;
extern std::map<int, std::map<SgScopeStatement *, SgStatement *> *>
    representativeWhitespaceStatementMapOfMaps;

// DQ (11/30/2015): Provide a statement to use as a key in the macro expansion
// map to get info about macro expansions. extern
// std::map<SgStatement*,MacroExpansion*> macroExpansionMap;
extern std::map<int, std::map<SgStatement *, MacroExpansion *> *>
    macroExpansionMapOfMaps;
