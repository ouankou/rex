...
bool Rose::KeepGoing::Backend::UseOriginalInputFile(SgFile *file) {
...
// Move many helper functions here to simplify writing a translator with
// keep-going logging support, Liao 4/12/2017

//! Processing command line options
void Rose::KeepGoing::commandLineProcessing(
    int argc, char *argv[], std::vector<std::string> &rose_cmdline) {
    ...
    for (int ii = 1; ii < argc; ++ii) {
      ...
      // --report=<filename>
        ...
        if (arg.empty()) {
          std::cerr << "[ERROR] "
                    << "[" << program_name << "] "
                    << "--report requires an argument, see --help" << std::endl;
          exit(1);
        } else {
        ...
        if (arg.empty()) {
          ...
          if (!std::filesystem::exists(expectations_filename__fail)) {
            std::cerr << "[FATAL] "
                      ...
                      << expectations_filename__fail << std::endl;
            exit(1);
          }
        }
      }
        ...
        if (arg.empty()) {
          ...
          if (!std::filesystem::exists(expectations_filename__pass)) {
            std::cerr << "[FATAL] "
                      ...
                      << expectations_filename__pass << std::endl;
            exit(1);
          }
        }
      }
...
void Rose::KeepGoing::generate_reports(
    SgProject *project, std::vector<std::string> orig_rose_cmdline) {
  // --------------------------------------------------------------
  ...
  // add original command line into the log file so users can easily reproduce
  // the errors.
  if (files_with_errors.size() > 0 && report_filename__fail.size() > 0) {
    ...
    for (SgFile *file : files_with_errors) {
      ...
      std::stringstream ss;
      ss << "Processed File: With Errors " << filename
         << endl; // Help diagnosis , output full file name, even command lines
                  // may have partial file paths
      if (verbose) {
        std::cout << "[ERROR] "
                  << "ROSE encountered an error while processing this file: "
                  << "'" << path_prefix << filename << "'" << std::endl;
      }

      // <file> <frontend> <unparser> <backend>
      // Keep all info. of one file into one line. Users can easily count the
      // total failures.
      ...
      if (file->get_backendCompilerErrorCode())
        ss << "\t Backend Compiler Error Code: "
           << file->get_backendCompilerErrorCode();
      if (file->get_unparsedFileFailedCompilation())
        ss << "\t Unparsed File Failed Compilation Code: "
           << file->get_unparsedFileFailedCompilation();
      ss << "\n";
      AppendToFile(report_filename__fail, ss.str());

      // Sometimes even for files failed on backend stage, some analysis results
      // are generated. we still want to output such results.
  ...
  if (files_without_errors.size() > 0 && report_filename__pass.size() > 0) {
    ...
    for (SgFile *file : files_without_errors) {
      ...
      AppendToFile(report_filename__pass,
                   "Processed File: Without Errors:" + full_filename + "\n");
      if (verbose) {
        std::cout << "[INFO] "
  ...
  // Sometimes even for files failed on backend stage, some analysis results are
  // generated. we still want to output such results.
  //  If exists, output the analysis results associated with each file
  //  Only merge the results into the pass log file if the fail log file is not
  //  specified!!
  if (files_with_errors.size() > 0 && report_filename__pass.size() > 0 &&
      report_filename__fail.empty()) {
    bool runonce = false;
    // add time stamp in the beginning
    std::ostringstream ostamp;
    ostamp << GetTimestamp() << " " << getpid() << std::endl;
    ...
    for (SgFile *file : files_with_errors) {
      ...
      if (oss.str().size() > 0) {
        if (!runonce) {
          AppendToFile(report_filename__pass,
                       "------Analysis results for files with backend "
                       "errors----------------------\n");
          AppendToFile(report_filename__pass, ostamp.str());
          runonce = true;
        }
        ...
        AppendToFile(report_filename__pass,
                     "Processed File: With Backend Errors:" + filename + "\n");
        AppendToFile(report_filename__pass, oss.str());
      }
    }
  }
  ...
  if (!expectations_filename__fail.empty()) {
    ...
    // TOO1 (3/5/2014): Only works if all files are processed by this single
    // commandline.
...
void Rose::KeepGoing::ShowUsage(std::string program_name) {
  std::cerr
      << "Usage: " << program_name << " [--help] [ROSE Commandline]\n"
      << "Options:\n"
      << "  --list-filenames                Prints the filename(s), computed "
         "by the ROSE commandline handling,\n"
      << "                                  onto standard out. Each filename "
      ...
      << "  --expected-failures=<filename>  File containing filenames that are "
         "expected to fail\n"
      << "  --expected-passes=<filename>    File containing filenames that are "
         "expected to pass\n"
      << "  --strip-path-prefix=<filename>  Normalize filenames by stripping "
         "this path prefix from them\n"
      << "\n"
      << "  --enable-ast-tests              Enables the internal ROSE AST "
         "consistency tests\n"
      << "\n"
      << "  -h,--help                       Show this help message\n"
      << "  --verbose                       Enables debugging output, e.g. "
         "outputs successful files\n"
      << "  --silent                        Disables debugging output\n"
      << std::endl;
}

std::string Rose::KeepGoing::StripPrefix(const std::string &prefix,
...
std::string Rose::KeepGoing::GetTimestamp(const std::string &format) {
  // Get the current time
  auto now = std::chrono::system_clock::now();
  std::time_t now_c = std::chrono::system_clock::to_time_t(now);
  std::tm local_tm =
      *std::localtime(&now_c); // Use local time for user-friendliness
...
void Rose::KeepGoing::touch(const std::string &pathname) {
  int fd =
      open(pathname.c_str(), O_WRONLY | O_CREAT | O_NOCTTY | O_NONBLOCK, 0666);
  ...
  if (rc) {
    std::cerr << __PRETTY_FUNCTION__ << ": Couldn't utime() path \"" << pathname
              << "\"\n";
    return;
  }
  if (verbose) {
...
void Rose::KeepGoing::AppendToFile(const std::string &filename,
  ...
  while (std::filesystem::exists(lock_file_name))
    ;                    // busy wait until it acquires lock
  ...
  std::filesystem::remove(
      lock_file_name); // release the lock by deleting the file
}
 ...
 */
std::vector<std::string> Rose::KeepGoing::GetSourceFilenamesFromCommandline(
