...
enum omp_construct_enum {
  ...
  // Liao, 1/15/2013, experimental implementation for the draft OpenMP
  // Accelerator Model technical report
  e_target,
  e_target_declare,
  e_target_data,
  e_target_update,
  e_map, // map clauses
  e_device,
  e_begin, // 10/29/2015, experimental begin/end directives for SPMD code
           // blocks, without changing variable scopes
  e_end,
  ...
  // experimental dist_data clause dist_data(dim1_policy, dim2_policy,
  // dim3_policy) A policy can be block(n), cyclic(n), or duplicate
  e_dist_data,
  e_duplicate,
  e_block,
  e_cyclic,
...
//! Check if the construct is a Fortran directive which can (optionally) have a
...
// reduction([reduction_modifier,]reduction_identifier:list). We call this kind
// of clause as ComplexClause, compared with other clauses which just take one
// parameter or a list of variable/expression.
class ComplexClause {
...
public:
  ...
  ComplexClause(omp_construct_enum first = e_unknown,
                omp_construct_enum second = e_unknown,
                omp_construct_enum third = e_unknown)
      : first_parameter(first), second_parameter(second),
        third_parameter(third) {};
};
