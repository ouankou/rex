...
string toString(omp_construct_enum omp_type) {
  ...
  case e_target_declare:
    result = "target declare";
    break;
  case e_target_data:
    result = "target data";
    break;
  case e_target_update:
  ...
  case e_end_parallel_do:
    result = "end parallel do";
    break;
  case e_end_parallel_sections:
    result = "end parallel sections";
    break;
  case e_end_parallel_workshare:
    result = "end parallel workshare";
    break;
  case e_end_parallel:
    result = "end parallel_support";
    break;
  case e_end_sections:
    result = "end sections";
    break;
  ...
  case e_reduction_and:
    result = ".and.";
    break;
  case e_reduction_or:
    result = ".or.";
    break;
  case e_reduction_eqv:
    result = ".eqv.";
    break;

  case e_reduction_neqv:
    result = ".neqv.";
    break;
  ...
  case e_not_omp:
    ...
    break;
  default: {
    cerr << "OmpSupport::toString(): unrecognized enumerate value:" << omp_type
         << endl;
    ROSE_ABORT();
  }
  }
...
bool isDirective(omp_construct_enum omp_type) {
  bool result = false;
  switch (omp_type) { // 16 directives as OpenMP 3.0
  //+2 for Fortran
  case e_parallel:
  case e_for:
  case e_for_simd:
...
omp_construct_enum getEndOmpConstructEnum(omp_construct_enum begin_enum) {
  ...
  default: {
    cerr << "In getEndOmpConstructEnum(): illegal begin enum is found:"
         << toString(begin_enum) << endl;
    ROSE_ABORT();
  }
  } // end switch
  return rt;
...
bool isClause(omp_construct_enum clause_type) {
  ...
  switch (clause_type) { // total 15 possible clauses as OpenMP 3.0
  case e_default:
  case e_shared:
  case e_private:
  case e_firstprivate:
