...
#define FixupCopyDataMemberMacro(IR_node_copy, IR_node_type,                   \
  ...
  if (IR_node_copy->get_accessFunctionName() ==                                \
    ...
    printf("SgCopyHelp::copiedNodeMapTypeIterator i != "                       \
           "help.get_copiedNodeMap().end() = %s (using %s()) \n",              \
    ...
    if (i != help.get_copiedNodeMap().end()) {                                 \
      ...
      printf("Resetting %s=%p->%s(local_copy=%p=%s), this=%p \n",              \
             #IR_node_copy, (void *)IR_node_copy, #set_accessFunctionName,     \
             (void *)local_copy, local_copy->class_name().c_str(),             \
             (void *)this);                                                    \
    ...
    printf(                                                                    \
        "%s->%s() = %p != this->%s() = %p (so %s is already set for %p) \n",   \
        #IR_node_copy, #get_accessFunctionName, IR_node_copy,                  \
