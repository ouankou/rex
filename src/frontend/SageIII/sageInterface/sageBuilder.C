...
// 4. fortran ?
  ...
  // ROSE_ASSERT(SageInterface::hasTemplateSyntax(XXX_name) == false);
     if (SageInterface::hasTemplateSyntax(nameWithoutTemplateArguments) ==
         true) {
     }
  ...
  // DQ (3/8/2012): Added assertion.
     ...
     ROSE_ASSERT(func_symbol != NULL);
     ROSE_ASSERT(func_symbol->get_symbol_basis() == func->get_firstNondefiningDeclaration());
...
//! Build a prototype for an existing function declaration (defining or nondefining )
  ...
  SgFunctionDeclaration* returnFunction = buildNondefiningFunctionDeclaration(name,return_type,paralist,scope);
     ...
     SgFunctionDeclaration *returnFunction =
         buildNondefiningFunctionDeclaration(name, return_type, paralist, scope,
                                             false, NULL);
   ...
   SgFunctionDeclaration *SageBuilder::buildNondefiningFunctionDeclaration(
       ...
       SgFunctionParameterList *paralist, SgScopeStatement *scope,
     ...
     if ((SageInterface::is_Fortran_language() == true) && (getEnclosingFileNode(scope)->get_outputLanguage() == SgFile::e_Fortran_language))
       ...
       result =
           buildNondefiningFunctionDeclaration_T<SgProcedureHeaderStatement>(
               name, return_type, paralist, /* isMemberFunction = */ false,
               scope, 0, NULL, NULL, sm);
        }
       else
        {
#if 0
          ...
          if (buildTemplateInstantiation == true)
             {
               if (isMemberFunction)
                 ...
                 result = buildNondefiningFunctionDeclaration_T<
                     ...
                     name, return_type, paralist, /* isMemberFunction = */ true,
                     scope, 0, templateArgumentsList, NULL, sm);
                  }
                 else
                  {
                    result = buildNondefiningFunctionDeclaration_T<
            ...
            else
             {
               if (isMemberFunction)
                  {
                 result = buildNondefiningFunctionDeclaration_T<
                     SgMemberFunctionDeclaration>(name, return_type, paralist,
                  ...
                  {
                    result = buildNondefiningFunctionDeclaration_T<
                        SgFunctionDeclaration>(name, return_type, paralist,
...
SageBuilder::buildDefiningMemberFunctionDeclaration(const SgName& name, SgType* return_type, SgFunctionParameterList* paralist, SgScopeStatement* scope)
  ...
  // each defining member function decl must have a non-defining counter part now. 11/27/2012, Liao
          nondefining_decl = buildNondefiningMemberFunctionDeclaration (name, return_type, paralist, scope, memberFunctionModifiers, false, NULL);
        }
     ...
     if (buildTemplateInstantiation == true)
       ...
       result =
           buildNondefiningFunctionDeclaration_T<SgMemberFunctionDeclaration>(
               name, return_type, paralist, /* isMemberFunction = */ true,
               scope, functionConstVolatileFlags, NULL, NULL,
               SgStorageModifier::e_default);
        }
     ROSE_ASSERT(result != NULL);
       ...
       // DQ (1/26/2013): Added test failing in buildDefiningFunctionDeclaration_T().
            ...
            // ROSE_ASSERT(func_symbol != NULL);
             ...
             result = buildDefiningFunctionDeclaration_T<
                 SgTemplateInstantiationMemberFunctionDecl>(
                 name, return_type, paralist, /* isMemberFunction = */ true,
                 scope, functionConstVolatileFlags,
                 templateInstantiationMemberFunctionDecl,
                 templateArgumentsList);
#if 0
    printf("In SageBuilder::buildDefiningMemberFunctionDeclaration(): "
           "isSgTemplateInstantiationMemberFunctionDecl(result)->get_templateName() = %s \n",
           isSgTemplateInstantiationMemberFunctionDecl(result)
               ->get_templateName()
               .str());
    ROSE_ASSERT(isSgTemplateInstantiationMemberFunctionDecl(result) != NULL);
    ROSE_ASSERT(isSgTemplateInstantiationMemberFunctionDecl(result)
                    ->get_templateName()
                    .is_null() == false);
#endif
  } else {
     ...
     if (buildTemplateInstantiation == true)
          ...
          if (isMemberFunction)
             {
               ...
               func = buildDefiningFunctionDeclaration_T<
                   SgTemplateInstantiationMemberFunctionDecl>(
                   name, return_type, paralist, /* isMemberFunction = */ true,
                   scope, 0, templateInstantiationMemberFunctionDecl,
                   templateArgumentsList);
            ...
            else
             {
               ...
               func = buildDefiningFunctionDeclaration_T<
                   SgTemplateInstantiationFunctionDecl>(
                   name, return_type, paralist, /* isMemberFunction = */ false,
                   scope, 0, templateInstantiationFunctionDecl,
                   templateArgumentsList);
       ...
       else
        {
          if (isMemberFunction)
             {
            func =
                buildDefiningFunctionDeclaration_T<SgMemberFunctionDeclaration>(
                    name, return_type, paralist, /* isMemberFunction = */ true,
                    scope, 0,
            ...
            ROSE_ASSERT(isSgMemberFunctionDeclaration(func) != NULL);
            ...
            else
             {
               func = buildDefiningFunctionDeclaration_T<SgFunctionDeclaration>(
                   name, return_type, paralist, /* isMemberFunction = */ false,
                   scope, 0, first_nondefining_declaration, NULL);
...
SageBuilder::buildDefiningFunctionDeclaration(const SgName& name, SgType* return_type, SgFunctionParameterList* parameter_list, SgScopeStatement* scope, bool forceFreeFunctionScope)
  ...
  // DQ (8/23/2013): Added assertions.
     ROSE_ASSERT(nondefiningDeclaration != NULL);
     ROSE_ASSERT(nondefiningDeclaration->get_firstNondefiningDeclaration() != NULL);
     ...
     ROSE_ASSERT(nondefiningDeclaration->get_firstNondefiningDeclaration() == nondefiningDeclaration);

     return buildDefiningFunctionDeclaration(
         name, return_type, parameter_list, scope, false,
...
// DQ (8/28/2012): This preserves the original API with a simpler function (however for C++ at least, it is frequently not sufficent).
// We need to decide if the SageBuilder API should include these sorts of functions.
// CR (10/7/2020): May not be appropriate for C++ but improves capability for Fortran.
SgProcedureHeaderStatement* SageBuilder::
     ...
     if (kind == SgProcedureHeaderStatement::e_subroutine_subprogram_kind)
       ...
       else
        {
          if (kind != SgProcedureHeaderStatement::e_function_subprogram_kind)
             {
        	   MLOG_ERROR_CXX("Rose::SageBuilder") << "unhandled subprogram kind for Fortran function declaration:"
        	                          << kind << endl;
               ROSE_ABORT();
             }
        }
...
SageBuilder::buildFunctionRefExp(const SgFunctionDeclaration* func_decl)
{
  ...
  {
    std::cerr<<"Fatal error: SageBuilder::buildFunctionRefExp():defining and nondefining declarations for a function cannot be both NULL"<<std::endl;
    ROSE_ABORT ();
  }
  ROSE_ASSERT( symbol != NULL);
...
//! Based on the contribution from Pradeep Srinivasa@ LANL
//Liao, 8/27/2008
   SgForStatement *SageBuilder::buildForStatement_nfi(
       SgStatement *initialize_stmt, SgStatement *test, SgExpression *increment,
       SgStatement *loop_body) {
     SgForStatement * result = new SgForStatement(test, increment, loop_body);
     ROSE_ASSERT(result);
  ...
  // if (symbol_table_case_insensitive_semantics == true)
   ...
   SgForStatement *SageBuilder::buildForStatement_nfi(
       SgForInitStatement *init_stmt, SgStatement *test,
...
// CR (3/6/2020): Added builder functions for type size (kind) expressions for Fortran
...
#error "DEAD CODE!"
            ...
            // if (hasTemplateSyntax(isSgTemplateInstantiationDecl(nondefdecl)->get_templateName()) == false);
               if (hasTemplateSyntax(isSgTemplateInstantiationDecl(nondefdecl)
                                         ->get_templateName()) == false) {
               }
...
SgTemplateArgument *buildTemplateArgument(SgType *t) {
  ...
  SgTemplateArgument *r =
      new SgTemplateArgument(SgTemplateArgument::type_argument, false, t,
                             nullptr, nullptr);
  return r;
}
...
SgExpression *instantiateNonrealRefExps(
  ...
  if (!expr) {
...
#endif
        if (tpl_param->get_initializedName()->get_name() ==
            iname->get_name()) {
    ...
    cond->set_false_exp(instantiateNonrealRefExps(
        cond->get_false_exp(), tpl_params, tpl_args));
    return cond;
    ...
    bop->set_lhs_operand_i(instantiateNonrealRefExps(
        bop->get_lhs_operand_i(), tpl_params, tpl_args));
    bop->set_rhs_operand_i(instantiateNonrealRefExps(
        bop->get_rhs_operand_i(), tpl_params, tpl_args));
    return bop;
  } else {
...
SgType *instantiateNonrealTypes(
  ...
  if (!type) {
    ...
    if (tpldecl != nullptr) {
      ...
      if (xtpldecl != nullptr) {
#if DEBUG_Rose_Builder_Templates_instantiateNonrealTypes
        std::cout << "    xtpldecl->get_name() = "
                  << xtpldecl->get_name() << std::endl;
#endif
        ...
        SgTemplateInstantiationDecl *inst_decl =
            isSgTemplateInstantiationDecl(SageBuilder::buildClassDeclaration_nfi(
        ...
        ((SgTemplateInstantiationDecl *)(inst_decl->get_firstNondefiningDeclaration()))
...
#if DEBUG_Rose_Builder_Templates_instantiateNonrealTypes
      std::cout << "    ->get_template_parameter_position() = "
                << std::dec << nrdecl->get_template_parameter_position()
                << std::endl;
