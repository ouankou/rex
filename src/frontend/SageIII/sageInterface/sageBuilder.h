...
enum SourcePositionClassification {
  ...
  e_sourcePositionNullPointers,      //! Set pointers to Sg_File_Info objects to
                                     //! NULL.
...
//! DQ (7/27/2012): changed semantics from removing the template arguments in
//! names to adding the template arguments to names.
ROSE_DLL_API SgName appendTemplateArgumentsToName(
    const SgName &name, const SgTemplateArgumentPtrList &templateArgumentsList);
...
buildFunctionType(SgType *return_type,
...
//! Must provide scope here.
/*! Some types are not known during translation but nevertheless are needed. For
 * example, some internal types from a runtime library.  To work around this
 * problem: this function prepends a hidden typedef declaration into scope
 * 'typedef int OpaqueTypeName;'
 * The translation-generated code is expected to include the runtime library's
 * headers to have the real type declarations.
 */
ROSE_DLL_API SgType *buildOpaqueType(std::string const type_name,
...
// DQ (3/20/2017): This function has never existed (inputs must be SgValueExp
// pointers). ROSE_DLL_API SgComplexVal* buildComplexVal(long double real_value
// = 0.0, long double imaginary_value = 0.0 );
ROSE_DLL_API SgComplexVal *buildComplexVal(SgValueExp *real_value,
                                           SgValueExp *imaginary_value);
ROSE_DLL_API SgComplexVal *buildComplexVal_nfi(SgValueExp *real_value,
                                               SgValueExp *imaginary_value,
                                               const std::string &str);
...
ROSE_DLL_API SgComplexVal *buildImaginaryVal_nfi(SgValueExp *imaginary_value,
                                                 const std::string &str);
...
//! Build a declaration of a non-real class or class-member representing
//! template parameters and their members
...
buildCastExp(SgExpression *operand_i = NULL, SgType *expression_type = NULL,
...
#define BUILD_BINARY_PROTO(suffix)                                             \
  ROSE_DLL_API Sg##suffix *build##suffix(SgExpression *lhs = NULL,             \
                                         SgExpression *rhs = NULL);            \
  ROSE_DLL_API Sg##suffix *build##suffix##_nfi(SgExpression *lhs,              \
                                               SgExpression *rhs);
...
ROSE_DLL_API SgConditionalExp *buildConditionalExp(SgExpression *test = NULL,
                                                   SgExpression *a = NULL,
                                                   SgExpression *b = NULL);
...
//! Build SgVarRefExp based on a variable's Sage name. It will lookup the name
//! in the symbol table internally starting from scope. A variable name is
//! unique so type can be inferred (double check this).

/*!
It is possible to build a reference to a variable with known name before the
variable is declared, especially during bottomup construction of AST. In this
case, SgTypeUnknown is used to indicate the variable reference needing
postprocessing fix using fixVariableReferences() once the AST is complete and
all variable declarations exist. But the side effect is that some get_type()
operations may not recognize the unknown type before the fix. So far, I extended
SgPointerDerefExp::get_type() and SgPntrArrRefExp::get_type() for SgTypeUnknown.
There may be others needing the same extension.
*/
 ...
 */
ROSE_DLL_API SgVarRefExp *buildOpaqueVarRefExp(const std::string &varName,
                                               SgScopeStatement *scope = NULL);
...
ROSE_DLL_API SgFunctionCallExp *
buildFunctionCallExp(SgExpression *f, SgExprListExp *parameters = NULL);
...
buildAssignInitializer(SgExpression *operand_i = NULL,
                       SgType *expression_type = NULL);
ROSE_DLL_API SgAssignInitializer *
buildAssignInitializer_nfi(SgExpression *operand_i = NULL,
                           SgType *expression_type = NULL);

//! Build an aggregate initializer
ROSE_DLL_API SgAggregateInitializer *
buildAggregateInitializer(SgExprListExp *initializers = NULL,
                          SgType *type = NULL);
ROSE_DLL_API SgAggregateInitializer *
buildAggregateInitializer_nfi(SgExprListExp *initializers, SgType *type = NULL);

//! Build a compound initializer, for vector type initialization
ROSE_DLL_API SgCompoundInitializer *
buildCompoundInitializer(SgExprListExp *initializers = NULL,
                         SgType *type = NULL);
ROSE_DLL_API SgCompoundInitializer *
buildCompoundInitializer_nfi(SgExprListExp *initializers, SgType *type = NULL);
...
buildBracedInitializer(SgExprListExp *initializers = NULL,
                       SgType *expression_type = NULL);
ROSE_DLL_API SgBracedInitializer *
buildBracedInitializer_nfi(SgExprListExp *initializers = NULL,
                           SgType *expression_type = NULL);
...
ROSE_DLL_API SgVariableDeclaration *
buildVariableDeclaration(const char *name, SgType *type,
                         SgInitializer *varInit = NULL,
                         SgScopeStatement *scope = NULL);

// DQ (6/25/2019): Added support to force building the varialbe declaration from
// the variable use (e.g. within a class) instead of from a declaration.
// Variable declaration built from a use are reused when the declaration is seen
// (later in processing of the class members). ROSE_DLL_API
// SgVariableDeclaration* buildVariableDeclaration_nfi(const SgName & name,
// SgType *type, SgInitializer *varInit, SgScopeStatement* scope);
ROSE_DLL_API SgVariableDeclaration *buildVariableDeclaration_nfi(
    const SgName &name, SgType *type, SgInitializer *varInit,
    SgScopeStatement *scope, bool builtFromUseOnly = false,
...
buildTemplateVariableDeclaration_nfi(const SgName &name, SgType *type,
                                     SgInitializer *varInit,
                                     SgScopeStatement *scope);

//! Build template variable declarations
ROSE_DLL_API SgTemplateVariableDeclaration *
buildTemplateVariableDeclaration(const SgName &name, SgType *type,
                                 SgInitializer *varInit,
                                 SgScopeStatement *scope);
...
buildTemplateTypedefDeclaration_nfi(const SgName &name, SgType *base_type,
                                    SgScopeStatement *scope = NULL,
                                    bool has_defining_base = false);
...
ROSE_DLL_API SgFunctionDeclaration *buildNondefiningFunctionDeclaration(
    ...
    SgScopeStatement *scope = NULL, bool buildTemplateInstantiation = false,
...
buildNondefiningFunctionDeclaration(const SgFunctionDeclaration *funcdecl,
                                    SgScopeStatement *scope = NULL);
...
buildDefaultConstructor(SgClassType *classType);
...
buildNondefiningMemberFunctionDeclaration(
    ...
    SgFunctionParameterList *parameter_list, SgScopeStatement *scope = NULL);

// DQ (8/28/2012): This preserves the original API with a simpler function
// (however for C++ at least, it is frequently not sufficent). We need to decide
// if the SageBuilder API should include these sorts of functions.
ROSE_DLL_API SgMemberFunctionDeclaration *
buildDefiningMemberFunctionDeclaration(const SgName &name, SgType *return_type,
...
ROSE_DLL_API SgFunctionDeclaration *
buildDefiningFunctionDeclaration(const SgName &name, SgType *return_type,
                                 SgFunctionParameterList *parameter_list,
                                 SgScopeStatement *scope = NULL,
                                 bool forceFreeFunctionScope = false);
...
buildProcedureHeaderStatement(const SgName &name, SgType *return_type,
                              SgFunctionParameterList *parameter_list,
...
ROSE_DLL_API SgProcedureHeaderStatement *buildProcedureHeaderStatement(
    const char *name, SgType *return_type, SgFunctionParameterList *parlist,
    SgProcedureHeaderStatement::subprogram_kind_enum, SgScopeStatement *scope,
...
buildLabelStatement(const SgName &name, SgStatement *stmt = NULL,
                    SgScopeStatement *scope = NULL);
...
buildStatementExpression_nfi(SgStatement *exp);
...
//! Build an empty declaration (useful for adding precission to comments and CPP
//! handling under token-based unparsing).
...
ROSE_DLL_API bool setNonrealDeclarationScope(SgDeclarationStatement *owner,
                                             SgDeclarationScope *scope);
...
// DQ (6/6/2012): Added support to get the template arguments into place before
// computing the type. SgClassDeclaration*
// buildNondefiningClassDeclaration_nfi(const SgName& name,
// SgClassDeclaration::class_types kind, SgScopeStatement* scope);
// SgClassDeclaration* buildNondefiningClassDeclaration_nfi(const SgName& name,
// SgClassDeclaration::class_types kind, SgScopeStatement* scope, bool
...
buildNondefiningClassDeclaration(SgName name, SgScopeStatement *scope);
ROSE_DLL_API SgClassDeclaration *
buildDefiningClassDeclaration(SgName name, SgScopeStatement *scope);

//! DQ (11/7/2009): Added function to build C++ class (builds both the
//! non-defining and defining declarations; in that order).
ROSE_DLL_API SgClassDeclaration *buildClassDeclaration(SgName name,
                                                       SgScopeStatement *scope);

//! Build an enum first nondefining declaration, without file info
ROSE_DLL_API SgEnumDeclaration *
buildNondefiningEnumDeclaration_nfi(const SgName &name,
                                    SgScopeStatement *scope);

//! Build a structure, It is also a declaration statement in SAGE III
ROSE_DLL_API SgClassDeclaration *
buildStructDeclaration(const SgName &name, SgScopeStatement *scope = NULL);
ROSE_DLL_API SgClassDeclaration *
buildStructDeclaration(const std::string &name, SgScopeStatement *scope = NULL);
ROSE_DLL_API SgClassDeclaration *
buildStructDeclaration(const char *name, SgScopeStatement *scope = NULL);
...
ROSE_DLL_API SgNamespaceDeclarationStatement *
buildNamespaceDeclaration_nfi(const SgName &name, bool unnamednamespace,
                              SgScopeStatement *scope);
...
buildDerivedTypeStatement(const SgName &name, SgScopeStatement *scope = NULL);

//! Build a Fortran module declaration.
ROSE_DLL_API SgModuleStatement *
buildModuleStatement(const SgName &name, SgScopeStatement *scope /*=NULL*/);
...
ROSE_DLL_API SgEnumDeclaration *
buildEnumDeclaration(const SgName &name, SgScopeStatement *scope = NULL);

//! Build an enum, It is also a declaration statement in SAGE III
ROSE_DLL_API SgEnumDeclaration *
buildEnumDeclaration_nfi(const SgName &name, SgScopeStatement *scope = NULL);
...
buildNonrealBaseClass(SgNonrealDecl *classDeclaration,
...
//! Build a SgFile node and attach it to SgProject
/*! The file will be build with an empty global scope to support declarations
 * being added.
 */
ROSE_DLL_API SgSourceFile *
buildSourceFile(const std::string &outputFileName, SgProject *project = NULL,
                bool clear_globalScopeAcrossFiles = false);
 ...
 */
ROSE_DLL_API SgSourceFile *
buildSourceFile(const std::string &inputFileName,
                const std::string &outputFileName, SgProject *project,
                bool clear_globalScopeAcrossFiles = false);

// DQ (11/8/2019): Support function for the new file (to support changing the
// file names in the source position info objects of each AST subtree node.
//! Change the source file associated with the source position information in
//! the AST.
// ROSE_DLL_API void fixupSourcePositionFileSpecification(SgNode* subtreeRoot,
// const std::string& newFileName);
ROSE_DLL_API void
fixupSourcePositionFileSpecification(SgNode *subtreeRoot,
                                     const std::string &newFileName);
...
//! Build and attach a comment, comment style is inferred from the language type
//! of the target node if not provided. It is indeed a wrapper of
...
//! actually insert it
ROSE_DLL_API PreprocessingInfo *
buildHeader(const std::string &header_filename,
            PreprocessingInfo::RelativePositionType position =
                PreprocessingInfo::before,
