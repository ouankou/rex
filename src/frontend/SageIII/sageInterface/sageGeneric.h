...
// note: the comments are right aligned to support code blocks in the doc
// generator :)
...
#define SG_ASSERT_TYPE(SAGENODE, N)                                            \
  (sg::assert_sage_type<SAGENODE>(N, __FILE__, __LINE__))
#define SG_ERROR_IF(COND, MSG)                                                 \
...
///        unused parameter.
template <class T> static inline void unused(const T &) {}

/// \brief projects the constness of T1 on T2
template <class T1, class T2> struct ConstLike {
  typedef T2 type;
};

template <class T1, class T2> struct ConstLike<const T1, T2> {
...
template <class T, class E> static inline T conv(const E &el) {
  T res;
#if !defined(NDEBUG)
  std::stringstream s;
...
[[noreturn]]
void unexpected_node(const SgNode &n, const char *file = nullptr,
                     size_t ln = 0);
/// \}

static inline void report_error_if(bool iserror, const std::string &desc,
                                   const char *file = nullptr, size_t ln = 0) {
  if (!iserror)
    return;
...
template <class T> T &deref(T *ptr, const char *file = 0, size_t ln = 0) {
 ...
 **/
template <class _ReturnType> struct DispatchHandler {
  typedef _ReturnType ReturnType;
  typedef DispatchHandler<ReturnType> Base;

  DispatchHandler() : res() {}

  explicit DispatchHandler(const ReturnType &defaultval) : res(defaultval) {}
...
template <class SageNode> static inline SageNode &assume_sage_type(SgNode &n) {
...
static inline const SageNode &assume_sage_type(const SgNode &n) {
...
struct VisitDispatcher : ROSE_VisitorPatternDefaultBase {
  ...
  GEN_VISIT(SgOmpWorkdistributeStatement)
  ...
  GEN_VISIT(SgOmpTargetTeamsWorkdistributeStatement)
  ...
  GEN_VISIT(SgOmpMaskedStatement)
  GEN_VISIT(SgOmpMaskedTaskloopStatement)
  GEN_VISIT(SgOmpMaskedTaskloopSimdStatement)
  ...
  GEN_VISIT(SgOmpNothingStatement)
  ...
  GEN_VISIT(SgOmpDirectiveKindClause)
  ...
  GEN_VISIT(SgFriendTypeDeclaration)
  ...
  GEN_VISIT(SgTypeChar8)
  GEN_VISIT(SgTypeChar16)
  GEN_VISIT(SgTypeChar32)
  GEN_VISIT(SgTypeComplex)
  GEN_VISIT(SgTypeDefault)
  GEN_VISIT(SgTypeFortranAssumed)
  GEN_VISIT(SgTypeFortranUnlimitedPolymorphic)
  ...
  GEN_VISIT(SgCAFImageSelectorExp)
...
template <class RoseVisitor>
inline typename std::remove_const<
    typename std::remove_reference<RoseVisitor>::type>::type
...
///           "handle." The handle function with the most suitable SgNode type
///           will get invoked.
 ...
 **/
template <class SageNode> struct DefaultHandler {
...
struct AncestorTypeFinder : DefaultHandler<const SgProject> {
  ...
  AncestorTypeFinder() : Base(), res(NULL, NULL) {}
...
AncestorNode *_ancestor(QualSgNode &n) {
  ...
  while (res.second != NULL) {
    res = (typename AncestorFinder::Pair)sg::dispatch(AncestorFinder(),
                                                      res.second);
  }
...
///          - SgNode&       -> AncestorNode&       ( assert(false) when an
///          ancestor of
///                                                   the specified type cannot
///                                                   be found )
///          - const SgNode& -> const AncestorNode& ( assert(false) when an
///          ancestor of
///                                                   the specified type cannot
///                                                   be found )
/// @code
...
/// \overload
template <class AncestorNode> const AncestorNode *ancestor(const SgNode *n) {
  if (n == NULL)
    return NULL;
...
/// \private
template <class SageNode> struct TypeRecoveryHandler {
...
template <class SageNode>
const SageNode *assert_sage_type(const SgNode *n, const char *f = 0,
                                 size_t ln = 0) {
...
/// \note  internal use
static inline void swap_parent(SgNode *lhs, SgNode *rhs) {
...
///       e.g., Rose_Containers
static inline void swap_parent(void *, void *) {}

/// \brief  swaps children (of equal kind) between two ancestor nodes of the
/// same type
...
void swap_child(SageNode &lhs, SageNode &rhs,
                SageChild *(SageNode::*getter)() const,
...
template <class SageNode> struct TraversalFunction {
  typedef void (*TransformHandlerFn)(SageNode *);

  explicit TraversalFunction(TransformHandlerFn fun) : fn(fun) {}
...
/// \details internal use
template <class SageNode>
static inline TraversalFunction<SageNode>
...
template <class GVisitor> struct TraversalClass : AstSimpleProcessing {
  explicit TraversalClass(GVisitor gv)
      : gvisitor(gv)
  //~ : gvisitor(std::move(gv))
  {}

  void visit(SgNode *n) { gvisitor = sg::dispatch(gvisitor, n); }
...
template <class F>
static inline F forAllNodes(F fn, SgNode *root,
...
template <class SageNode>
static inline void forAllNodes(void (*fn)(SageNode *), SgNode *root,
                               AstSimpleProcessing::Order order = postorder) {
  forAllNodes(createTransformExecutor(fn), root, order);
}

#if !defined(NDEBUG)
static inline std::string nodeType(const SgNode &n) { return typeid(n).name(); }

static inline std::string nodeType(const SgNode *n) {
  if (n == NULL)
    return "<null>";
...
template <class GVisitor> struct DispatchHelper {
  explicit DispatchHelper(GVisitor gv, SgNode *p)
  ...
  void operator()(SgNode *n) {
    ...
    if (n != NULL)
      gvisitor = sg::dispatch(std::move(gvisitor), n);
  }
...
void linkParentChild(SageParent &parent, SageChild &child,
...
/// \{
template <class SageNode>
typename SageNode::base_node_type &asBaseType(SageNode &n) {
  return n;
}

template <class SageNode>
const typename SageNode::base_node_type &asBaseType(const SageNode &n) {
  return n;
}
