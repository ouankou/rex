...
#if 0 // FMZ(07/07/2010): the argument "nextErrorCode" should be
      // call-by-reference
...
#define DEBUG_GET_ENCLOSING_NODE 0
  ...
  if (foundCycle == false) {
    ...
    while ((parent != nullptr) && (!dynamic_cast<const NodeType *>(parent))) {
      ...
      ROSE_ASSERT(parent != previouslySeenParent);
#else
      printf("WARNING::WARNING::WARNING commented out assertion for parent != "
             "previouslySeenParent \n");
      if (parent == previouslySeenParent)
        break;
#endif
...
#else  /* TOP_LEVEL_IF */
  // DQ (10/20/2012): Using Robb's newer version with my modification to use the
  // definingDeclaration rather than firstNondefiningDeclaration (below).

  // Find the parent of specified type, but watch out for cycles in the ancestry
  // (which would cause an infinite loop). Cast away const because isSg*
  // functions aren't defined for const node pointers; and our return is not
  // const.
  SgNode *node = const_cast<SgNode *>(
      !astNode || includingSelf ? astNode : astNode->get_parent());
  std::set<const SgNode *> seen; // nodes we've seen, in order to detect cycles
  while (node) {
    if (NodeType *found = dynamic_cast<NodeType *>(node))
      return found;

    // FIXME: Cycle detection could be moved elsewhere so we don't need to do it
    // on every call. [RPM 2012-10-09] DQ (12/30/2019): Provide more detail in
    // error message.
    if (seen.insert(node).second == false) {
      printf("Error: node is already in set and defines a cycle: node = %p = "
             "%s \n",
    ...
    if (SgDeclarationStatement *declarationStatement =
            isSgDeclarationStatement(node)) {
      SgDeclarationStatement *definingDeclaration =
          declarationStatement->get_definingDeclaration();
      SgDeclarationStatement *firstNondefiningDeclaration =
          declarationStatement->get_firstNondefiningDeclaration();
      if (definingDeclaration && firstNondefiningDeclaration &&
          declarationStatement != firstNondefiningDeclaration) {
        // DQ (10/19/2012): Use the defining declaration instead.
...
//! Special purpose function for deleting AST expression tress containing valid
//! original expression trees in constant folded expressions (for internal use
//! only).
ROSE_DLL_API void
...
//! appending statements, e.g. preprocessing info, defining/nondefining pointers
//! etc.
...
//! Append a copy ('decl') of a function ('original_statement') into a 'scope',
//! include any referenced declarations required if the scope is within a
//! compiler generated file. All referenced declarations, including those from
//! headers, are inserted if excludeHeaderFiles is set to true (the new file
//! will not have any headers).
...
//! Insert a statement before or after the target statement within the target's
//! scope. Move around preprocessing info automatically
...
//! Insert a statement after a target statement, Move around preprocessing info
...
//! Insert a statement before the first non-declaration statement in a scope. If
//! the scope has no non-declaration statements
...
//! Insert statements before the first non-declaration statement in a scope.  If
//! the scope has no non-declaration statements
...
//! Remove a statement from its attach point of the AST. Automatically keep its
//! associated preprocessing information at the original place after the
//! removal. The statement is still in memory and it is up to the users to
//! decide if the removed one will be inserted somewhere else or released from
//! memory (deleteAST()).
...
//! node. Users must take care of any dangling pointers, symbols or types that
//! result. This is identical to deleteAST()
...
//! SgVariantExpression. All SgVariantExpression in the pattern will be replaced
//! with copies of the anchor node.
 ...
 * needs to be evaluated. NOTE: This handles reference types correctly by using
 * pointer types for the temporary.
 ...
 * @param reEvaluate an assignment op to reevaluate the expression. Leave NULL
 * if not needed
...
/*! We recommend to build SgFunctionParameterList before building a function
 declaration However, it is still allowed to append new arguments for existing
