...
void setStatementDetails(SgStatement *stmt, SgNode *parent,
                         SgScopeStatement *scope) {
  // This function sets a few of the required details of IR nodes.

  // The scope sometimes has to be explicit set (mostly for
...
void setLocatedNodeDetails(SgLocatedNode *locatedNode, SgNode *parent) {
  ...
  // The position of any SgLocatedNode must be set (in this case the IR nodes is
  // classified as being part of a transformation and to be output in the code
  // generation. Since the High Level Interface to the AST is all about
  // construction of new AST sub-trees, there is not position in the original
  // source code so we mark all new IR nodes as being part of a transformation
  // and to be output in the generated code.
...
void buildInitializedName(const SgName &name, SgType *type,
                          SgScopeStatment *scope = NULL) {
