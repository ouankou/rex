...
template <class SageNode> static inline SageNode *cloneNode(const SageNode *n) {
  if (!n)
    return 0;
...
/// \note  internal use
static inline void _append(SgExprListExp &container, SgExpression *elem) {
  SageInterface::appendExpression(&container, elem);
}

/// \overload
static inline void _append(SgFunctionParameterList &container,
                           SgInitializedName *elem) {
  SageInterface::appendArg(&container, elem);
}

/// \brief Functor setting the scope of a sage node to a specified (at Functor
/// construction time) scope
struct ScopeSetter {
  explicit ScopeSetter(SgScopeStatement &the_scope) : scope(the_scope) {}

  template <class ScopedSageNode> void handle(ScopedSageNode *scopeElem) const {
...
struct VarRefBuilder {
  explicit VarRefBuilder(SgScopeStatement &the_scope) : scope(the_scope) {}
...
private:
  // DQ (3/25/2017): Remove to avoid Clang warning about unused private
  // variable. SgDeclarationStatement& decl;
  SgScopeStatement *scope;
};

/// \brief   Generic inserter for sage containers
/// \tparam  SageSequenceContainer, a sage container that supports appending an
/// element
/// \details forwards actual insert to function family _append
template <class SageSequenceContainer> struct SageInserter {
  ...
  explicit SageInserter(Container &cont) : container(cont) {}

  // \todo SageElem should be derived form the container type
  template <class SageElem> SageInserter &operator=(SageElem *elem) {
