...
void *parseOpenMPExprCallback(OpenMPDirectiveKind directive_kind,
  ...
  if (parse_mode == OMP_EXPR_PARSE_expression) {
    ...
    if (clause_kind == OMPC_depend) {
      if (isArraySectionDesignator(parsed->text)) {
        parseAndStoreArraySection(parsed->text, parsed.get(),
                                  context->pragma_declaration,
                                  context->directive, clause_kind);
      } else {
        parseAndStoreExpression(parsed->text, parsed.get(),
                                context->pragma_declaration, context->directive,
                                clause_kind);
...
collectObjectLikeMacrosUntilLine(const std::string &filename,
  ...
  while (current_line < max_line_inclusive && std::getline(stream, raw_line)) {
    ...
    while (current_line < max_line_inclusive) {
      std::string::size_type end = logical_line.find_last_not_of(" \t\r");
      if (end == std::string::npos || logical_line[end] != '\\') {
        break;
      }
    ...
    if (stripped.rfind("#define", 0) == 0 &&
        (stripped.size() == 7 ||
         std::isspace(static_cast<unsigned char>(stripped[7])))) {
      std::string::size_type pos = 7;
      while (pos < stripped.size() &&
             std::isspace(static_cast<unsigned char>(stripped[pos]))) {
        ++pos;
      }
      if (pos >= stripped.size() || !isIdentifierStart(stripped[pos])) {
        continue;
      }
      const std::string::size_type name_begin = pos;
    ...
    if (stripped.rfind("#undef", 0) == 0 &&
        (stripped.size() == 6 ||
         std::isspace(static_cast<unsigned char>(stripped[6])))) {
      std::string::size_type pos = 6;
      while (pos < stripped.size() &&
             std::isspace(static_cast<unsigned char>(stripped[pos]))) {
        ++pos;
      }
...
std::string expandObjectLikeMacros(
    const std::string &text,
    const std::unordered_map<std::string, std::string> &object_macros) {
  if (text.empty() || object_macros.empty()) {
    return text;
  }
...
std::string expandOpenMPPragmaObjectLikeMacros(
    const std::string &pragma_text,
    const SgPragmaDeclaration *pragma_declaration) {
  if (pragma_text.empty() || pragma_declaration == nullptr) {
    return pragma_text;
  }

  const Sg_File_Info *file_info = pragma_declaration->get_file_info();
  if (file_info == nullptr) {
    return pragma_text;
  }

  const std::string filename = file_info->get_filenameString();
  const int pragma_line = file_info->get_line();
  if (filename.empty() || pragma_line <= 1) {
    return pragma_text;
  }

  const auto object_macros = collectObjectLikeMacrosUntilLine(
      filename, static_cast<unsigned int>(pragma_line - 1));
  return expandObjectLikeMacros(pragma_text, object_macros);
}
...
void processOpenMP(SgSourceFile *sageFilePtr) {
  ...
  if (isFortran) { // use ompparser to process Fortran.
    ...
    for (iter = all_pragmas.begin(); iter != all_pragmas.end(); iter++) {
      ...
      pragmaString =
          expandOpenMPPragmaObjectLikeMacros(pragmaString, pragmaDeclaration);
