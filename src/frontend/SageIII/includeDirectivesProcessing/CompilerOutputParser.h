...
private:
  ...
  // This map keeps track of what files are included in a file. Note that all
  // included files are combined toghether (e.g. the same file may include
  // different files in the same compilation session depending on preprocessor
  // values), so no particular order is available.
  map<string, set<string>> includedFilesMap;

  list<string> quotedIncludesSearchPaths;
  list<string> bracketedIncludesSearchPaths;

  // The following two members are needed for recursive parsing of the compiler
  ...
  void addIncludedFilesToMap(const string &includingFileName,
                             const set<string> &includedFiles);
  void parseIncludedFilesFromCompilerOutput(const string &includingFileName,
                                            const string &parsePrefix);
