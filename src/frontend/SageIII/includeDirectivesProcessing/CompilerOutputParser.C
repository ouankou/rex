...
// This method is a copy of the ROSE's method popenReadFromVector in file
// processSupport.C. The only two changes are adding an additional option to the
// compiler arguments, and replacing stdout with stderr in the connector pipe
// (since both -v and -H options output goes to stderr).

FILE *CompilerOutputParser::getCompilerOutput(const vector<string> &argv,
                                              bool isVerbose) {
  ...
  if (pid == 0) { // Child
    ...
    // Is either verbose or outputs included files (i.e. either -v or -H
    // option). Its ok to duplicate the option if it is already present.
    if (isVerbose) {
...
void CompilerOutputParser::addIncludedFilesToMap(
    const string &includingFileName, const set<string> &includedFiles) {
  map<string, set<string>>::iterator mapEntry =
      includedFilesMap.find(includingFileName);
  if (mapEntry != includedFilesMap.end()) {
    ...
    includedFilesMap.insert(
        pair<string, set<string>>(includingFileName, includedFilesSet));
  }
  //        //If the list of included files for this including file already
  //        exists, then check that it is the same as the newly collected list.
  ...
  //            ROSE_ASSERT((*existingListIterator).compare(*newListIterator) ==
  //            0); existingListIterator++; newListIterator++;
  //        }
  //    } else {
  //        includedFilesMap.insert(pair<string, list<string>
  //        >(includingFileName, includedFiles));
  //    }
}

void CompilerOutputParser::parseIncludedFilesFromCompilerOutput(
    const string &includingFileName, const string &parsePrefix) {
  ROSE_ASSERT(
      FileHelper::fileExists(includingFileName)); // should be an existing file
  ...
  while (compilerOutputReader->hasLine()) {
    ...
    if (currentLine.compare(0, parsePrefixSize, parsePrefix) == 0) {
      ...
      lastIncludedFile = FileHelper::makeAbsoluteNormalizedPath(
          currentLine.substr(parsePrefixSize), workingDirectory);
      includedFiles.insert(lastIncludedFile);
      compilerOutputReader->readNextLine(); // read the next line, because the
                                            // current line was processed
    } else {
      if (currentLine.compare(0, oneLevelDeeperParsePrefix.size(),
                              oneLevelDeeperParsePrefix) == 0) {
        // The current line belongs to the last processed included file.
        parseIncludedFilesFromCompilerOutput(lastIncludedFile,
                                             oneLevelDeeperParsePrefix);
      } else {
        // The current line is neither of this including file nor of the last
        // processed included file, so stop and let the caller process it (and
        // the following lines, if any), unless this is the top level.
        if (parsePrefix.compare(topLevelParsePrefix) != 0) {
          break;
        } else {
          compilerOutputReader->readNextLine(); // read the next line, because
                                                // the current line was skipped
        }
      }
    }
  }
...
void CompilerOutputParser::parseIncludedFilesSearchPathsFromCompilerOutput() {
  ...
  while (compilerOutputReader->hasLine()) {
    ...
    if (isInsideQuotedIncludes || isInsideBracketedIncludes) {
      string path = FileHelper::makeAbsoluteNormalizedPath(
          /*the first character is a white space, so ignore it*/ currentLine
              .substr(1),
          workingDirectory);
...
void CompilerOutputParser::processFile(SgFile *inputFile, bool isVerbose) {
  // This code duplicates parts of methods int SgFile::compileOutput ( int
  // fileNameIndex ) from file Cxx_Grammar.C and int SgFile::compileOutput (
  // vector<string>& argv, int fileNameIndex ) from file sageSupport.C
  ...
  if (inputFile->get_C_only() == true || inputFile->get_C99_only() == true ||
      inputFile->get_Cxx_only() == true) {

  } else {
    if (inputFile->get_Fortran_only() == true) {
      // DQ (4/2/2011): some Fortran tests pass in
      // tests/nonsmoke/functional/roseTests/astInterfaceTests pass in
      // legacy frontend options, we might want to correct this (for now
      // we will clean up the command line as part of command line
      // handling).
  ...
  string originalUnparseOutputFileName =
      inputFile->get_unparse_output_filename();

  inputFile->set_skip_unparse(true);
  inputFile->set_unparse_output_filename(
      inputFile->get_sourceFileNameWithPath());

  // Collect include trees and search paths from compiler output (-H/-v).
  vector<string> compilerNameString =
      inputFile->buildCompilerCommandLineOptions(argv, 0, compilerName);
  ...
  if (isVerbose) {
    ...
    parseIncludedFilesFromCompilerOutput(
        FileHelper::normalizePath(inputFile->getFileName()),
        topLevelParsePrefix);
  }
...
map<string, set<string>> CompilerOutputParser::collectIncludedFilesMap() {
  ...
  for (SgFilePtrList::const_iterator it = inputFileList.begin();
...
CompilerOutputParser::collectIncludedFilesSearchPaths() {
  processFile(
      *(projectNode->get_fileList().begin()),
      true); // it is sufficient to get verbose output for a single input file
  return pair<list<string>, list<string>>(quotedIncludesSearchPaths,
