...
void FixupStorageAccessOfForwardTemplateDeclarations::visit(SgNode *node) {
  ROSE_ASSERT(node != NULL);

  // These are output a static functions to avoid being defined using multiple
  // symbols at link time. This is a temporary fix until we get the better
  // prelinker written.
  ...
  if (functionDeclaration != NULL) {
    ...
    if (definingDeclaration != NULL) {
      bool isStatic = definingDeclaration->get_declarationModifier()
                          .get_storageModifier()
                          .isStatic();
      if (isStatic == true) {
        // This is a static function so make sure that all declarations
        // associated with it (e.g. forward declarations) are marked the same.
        bool currentDeclarationIsStatic =
        ...
        if (currentDeclarationIsStatic == false) {
          if (SgProject::get_verbose() >= AST_FIXES_VERBOSE_LEVEL)
            printf("AST fixup: Changing non-defining declaration (%p) to match "
                   "defining declaration (static)\n",
                   functionDeclaration);
          functionDeclaration->get_declarationModifier()
              .get_storageModifier()
              .setStatic();
