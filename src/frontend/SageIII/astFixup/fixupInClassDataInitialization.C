...
void FixupInClassDataInitialization::visit(SgNode *node) {
  ...
  case V_SgVariableDeclaration: {
    ...
    SgClassDefinition *classDefinition =
        isSgClassDefinition(variableDeclaration->get_parent());
    if (classDefinition != NULL) {
      // Check if it has an initializer (there should not be many of these!)
      SgInitializedNamePtrList::iterator variable =
          variableDeclaration->get_variables().begin();
      if ((*variable)->get_initializer() != NULL) {
        // Check the type (looking for float or double) (should be uniform for
        // all types) Check that it is non-static and const and modify to make
        // it static const
        ...
        if (modifierType == NULL) {
          ...
          // DEBUG_SAGE_ACCESS_FUNCTIONS and
          // DEBUG_SAGE_ACCESS_FUNCTIONS_ASSERTION in sage3.h indicate this is
          // required.
          if ((*variable)->get_typeptr() != NULL) {
            printf("Note: overwriting (*variable)->get_typeptr() = %p with "
                   "NULL before assignment to modifierType = %p \n",
        ...
        if (variableDeclaration->get_declarationModifier()
          ...
          // printf ("Modify non-static const in-class initialization to be
          // static \n");
          variableDeclaration->get_declarationModifier()
              .get_storageModifier()
              .setStatic();
        } else {
          // printf ("Found static const in-class initialization (no
  ...
  case V_SgDoubleVal: {
    ...
    if (dotExpParentOfValueExp != NULL) {
      // This is an illegal code construction such as x.3.14159 instead of
      // just 3.14159 get the parent and substitute the floating point constant
      // value in its place
      ...
      SgAssignInitializer *initializer =
          isSgAssignInitializer(parentOfProblemExpression);
      ...
      if (initializer->get_operand() != NULL) {
        printf("Note: overwriting initializer->get_operand() = %p with NULL "
               "before assignment to value = %p \n",
    ...
    if (arrowExpParentOfValueExp != NULL) {
      ...
      // just 3.14159 get the parent and substitute the floating point constant
      // value in its place
      SgNode *parentOfProblemExpression =
          arrowExpParentOfValueExp->get_parent();
      ROSE_ASSERT(parentOfProblemExpression != NULL);
      // printf ("parentOfProblemExpression = %s
      // \n",parentOfProblemExpression->sage_class_name());
      ...
      case V_SgAssignOp: {
        SgAssignOp *assignmentOperator =
            isSgAssignOp(parentOfProblemExpression);
