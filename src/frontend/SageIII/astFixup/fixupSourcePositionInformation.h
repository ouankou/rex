...
public:
  ...
  FixupSourcePositionInformationSynthesizedAttribute(
      const FixupSourcePositionInformationSynthesizedAttribute &);
  FixupSourcePositionInformationSynthesizedAttribute &operator=(
      const FixupSourcePositionInformationSynthesizedAttribute &) = default;
...
public:
  ...
  FixupSourcePositionInformationInheritedAttribute evaluateInheritedAttribute(
      SgNode *node, FixupSourcePositionInformationInheritedAttribute ia);
...
private:
  void processChildSynthesizedAttributes(
  ...
  void fixupSourcePositionUnavailableInFrontendToBeUniformlyMarked(
      SgLocatedNode *locatedNode);
};

/*! \brief This traversal uses the Memory Pool traversal to fixup the source
   position constructs.

    This allows us to make sure that there is alwasy and ending position
   construct to match the starting position construct.
 */
