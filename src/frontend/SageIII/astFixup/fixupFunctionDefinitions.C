...
void FixupFunctionDefinitions::visit(SgNode *node) {
  ...
  case V_SgFunctionDefinition: {
    ...
    if (functionDefinition->get_body() == NULL) {
      ...
      printf("AST Fixup: function definition violates the rule that every "
             "function definition has a function body, building an empty "
             "SgBasicBlock \n");
      ...
      body->set_endOfConstruct(
          Sg_File_Info::generateDefaultFileInfoForTransformationNode());
      functionDefinition->set_body(body);
      // this is required since the access function don't have side-effects on
      // their parameters
