...
//! For all nodes in the memory pool, it looks for the edges in the replacement
//! map. If a match is found the edge is updated.
void edgePointerReplacement(replacement_map_t const &);

//! Traverse the AST `root` looking for the edges in the replacement map. If a
//! match is found the edge is updated.
