...
void fixupEnumValues() {
  ...
  TimingPerformance timer(
      "Fixup reference to enum declarations in enum values:");
...
void FixupEnumValues::visit(SgNode *node) {
  ...
  if (enumVal->get_declaration() == NULL) {
    ...
    // printf ("Found an enum value with a NULL declaration, fixup the
    // declaration! \n"); enumVal->get_startOfConstruct()->display("Found an
    // enum value with a NULL declaration");

    SgClassDefinition *enclosingClassDefinition =
        SageInterface::getEnclosingClassDefinition(enumVal, true);
    ROSE_ASSERT(enclosingClassDefinition != NULL);

    // Now search for the enumVal name in the symbol table
    SgEnumFieldSymbol *enumFieldSymbol =
        enclosingClassDefinition->lookup_enum_field_symbol(enumVal->get_name());
    ROSE_ASSERT(enumFieldSymbol != NULL);
    if (enumFieldSymbol != NULL) {
      // We have now found the enum symbol in the scope of the class where the
      // enum value was used. It could have been elsewhere and we don't search
      // there, it could be in a namespace that was included using a using
      // directive, so this could be very complex if it is not in the enclosing
      // class.  To handle the case of where we can't find it, we ALLOW the
      // declaration in the enumVal to be NULL, but we try to set it where we
      ...
      // printf ("Fixup the NULL declaration in enumVal = %p = %s with valid
      // enum declaration = %p = %s
