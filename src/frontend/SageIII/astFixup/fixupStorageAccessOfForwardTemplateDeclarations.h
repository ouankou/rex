...
/*! \brief Fixup the storage access of all compiler-generated forward
   ...
   declaration.
    ...
    This fix is required because legacy frontend builds forward declarations for
   template instantations but does not mark the forward declaration as static
   when using the instantiation mode "local" ("--instantiation local"). The
   defining declaration is correctly marked as static.  This fixup only applies
   to compiler generated forward declarations (placed after the template
   declaration) and having a valid defining declaration.

    \internal This is a bug in legacy frontend, which I suppose we should
