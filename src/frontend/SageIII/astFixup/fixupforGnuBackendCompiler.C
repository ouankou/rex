...
void FixupforGnuBackendCompiler::visit(SgNode *node) {
  // Convert:
  ...
  case V_SgExprListExp: {
    ...
    // Transformation only applies for single argument copy constructors (I
    // think) and only to those inside of constructors (so leave one level of
    // constructor!)
    SgConstructorInitializer *outerConstructorInitializer =
        isSgConstructorInitializer(expressionListExp->get_parent());
    if ((outerConstructorInitializer != NULL) &&
        (expressionListExp->get_expressions().size() == 1)) {
      SgExpression *argument = *(expressionListExp->get_expressions().begin());
      ROSE_ASSERT(argument != NULL);

      SgConstructorInitializer *innerConstructorInitializer =
      ...
      if (innerConstructorInitializer != NULL) {
        ...
        // bool constructorTypesMatch = outerConstructorInitializer->get_type()
        ...
        printf("Warning: calling "
               "\"outerConstructorInitializer->get_declaration()->"
               "unparseToString()\" fails in ASTFixup \n");
        // This causes and error: attachPreprocessingInfo.C:262: void
        // AttachPreprocessingInfoTreeTrav::
        //      setupPointerToPreviousNode(SgLocatedNode*): Assertion
        //      `parentStatement != __null' failed.
        // string unparsedCode =
        // outerConstructorInitializer->get_declaration()->unparseToString();
        // This is OK
        // string unparsedCode =
        // outerConstructorInitializer->get_type()->unparseToString();
#endif

        if (constructorMatch == true) {
#if PRINT_DEVELOPER_WARNINGS
          if (outerConstructorInitializer->get_type() ==
              innerConstructorInitializer->get_type()) {
            // DQ (3/25/2005): This is something to investigate more deeply at
            // some point!
            printf("Warning in AST Fixup: constructors don't match but types "
                   "are reported as being the same! \n");
          }
#endif
          SgExprListExp *innerExpressionListExp =
              isSgExprListExp(innerConstructorInitializer->get_args());
          if ((innerExpressionListExp != NULL) &&
              (innerExpressionListExp->get_expressions().size() == 1)) {
            SgExpression *targetArgument =
                *(innerExpressionListExp->get_expressions().begin());
            ...
            // SgExpression* elementToDelete =
            // expressionListExp->get_expressions().front(); delete
            // elementToDelete;
            ...
            // DQ (9/25/2007): Moved uniformally from std::list to std::vector
            // in ROSE, also since the list is length == 1, pop_front() can be
            // replaced with pop_back().
          ...
          // printf ("This is a required constructor call (can't be optimized
          // away) \n");
        }
      }
    }
