...
void fixupSourcePositionInformation(SgNode *node) {
  ...
  // While we debug this and other code we need to be able to optionally turn
  // this on and off.
...
FixupSourcePositionInformationSynthesizedAttribute::
    FixupSourcePositionInformationSynthesizedAttribute(
        const FixupSourcePositionInformationSynthesizedAttribute &X)
    : startingPosition(X.startingPosition), endingPosition(X.endingPosition),
      associatedNode(X.associatedNode) {
  // Copy Constructor
  // printf ("Calling FixupSourcePositionInformationSynthesizedAttribute copy
  // constructor (this = %p, X = %p) \n",this,&X);
}
...
FixupSourcePositionInformationSynthesizedAttribute::
  ...
  // Default Constructor
  // printf ("Calling FixupSourcePositionInformationSynthesizedAttribute default
  // constructor (this = %p) \n",this);
}
...
void FixupSourcePositionInformation::
  ...
  if (locatedNode->get_startOfConstruct()->isCompilerGenerated() == true) {
    if (locatedNode->get_endOfConstruct()->isCompilerGenerated() == false) {
      printf(
          "Warning: expression->get_endOfConstruct()->isCompilerGenerated() == "
          "false (IR node not uniformally marked as compiler generated) \n");
      locatedNode->get_endOfConstruct()->setCompilerGenerated();
    }

    // If this is an expression then make sure that the operatorPosition is
    // uniformally marked.
    if (expression != NULL) {
      // ROSE_ASSERT(expression->get_operatorPosition() != NULL);
      if (expression->get_operatorPosition() != NULL &&
          expression->get_operatorPosition()->isCompilerGenerated() == false) {
        printf(
            "Warning: "
            "expression->get_operatorPosition()->isCompilerGenerated() == "
            "false (IR node not uniformally marked as compiler generated) \n");
    ...
    ROSE_ASSERT(locatedNode->get_startOfConstruct()->isCompilerGenerated() ==
                false);
    ROSE_ASSERT(locatedNode->get_endOfConstruct()->isCompilerGenerated() ==
                false);
    ...
    if (expression != NULL) {
      if (expression->get_operatorPosition() != NULL) {
        ROSE_ASSERT(expression->get_operatorPosition()->isCompilerGenerated() ==
                    false);
      } else {
        // This is allowed since it does not make sense for all expressions!
        // printf ("In fixupCompilerGeneratedNodesToBeUniformlyMarked():
        // expression = %p = %s has no operatorPosition information
...
void FixupSourcePositionInformation::
    fixupSourcePositionUnavailableInFrontendToBeUniformlyMarked(
        SgLocatedNode *locatedNode) {
  // Note that the isSourcePositionUnavailableInFrontend information can be set
  // uniformally, but is not always true uniformally. However, if the starting
  // position is set so that isSourcePositionUnavailableInFrontend == true then
  // the ending positon should be marked similarly.  There are however, case
  ...
  if (locatedNode->get_startOfConstruct()
    ...
    if (locatedNode->get_endOfConstruct()
      ...
      printf("Warning: expression->get_endOfConstruct()->isCompilerGenerated() "
             "== false (IR node not uniformally marked as "
             "SourcePositionUnavailableInFrontend) \n");
    ...
    // Make sure that these are what we expect
    ROSE_ASSERT(locatedNode->get_startOfConstruct()
                    ->isSourcePositionUnavailableInFrontend() == false);
    // ROSE_ASSERT(locatedNode->get_endOfConstruct()->isSourcePositionUnavailableInFrontend()
    // == false);
    ...
    if (expression != NULL) {
      if (expression->get_operatorPosition() != NULL) {
        ROSE_ASSERT(expression->get_operatorPosition()
                        ->isSourcePositionUnavailableInFrontend() == false);
      } else {
        // This is allowed since it does not make sense for all expressions!
        // printf ("In fixupCompilerGeneratedNodesToBeUniformlyMarked():
...
FixupSourcePositionInformation::evaluateSynthesizedAttribute(
    SgNode *node, FixupSourcePositionInformationInheritedAttribute,
  ...
  SgLocatedNode *locatedNode = isSgLocatedNode(node);
  if (locatedNode != NULL) {
    // DQ (12/23/2006): This can not be an error since transformations commonly
    // don't test an endOfConstruct Sg_File_Info object.  Thus we can either fix
    // it up or ignore it, it is easier to fix it up for now, thought this may
    // be a performance issue at some point.
    if (locatedNode->get_endOfConstruct() == NULL) {
#if PRINT_DEVELOPER_WARNINGS
      printf("Note: locatedNode->get_endOfConstruct() == NULL (locatedNode = "
             "%p = %s), generating new Sg_File_Info object ... \n",
      ...
      Sg_File_Info *newFileInfo =
          new Sg_File_Info(*(locatedNode->get_startOfConstruct()));
    ...
    if (isSgGlobal(node) == NULL && isSgFile(node) == NULL) {
      if (locatedNode->get_startOfConstruct()->ok() == false ||
          locatedNode->get_endOfConstruct()->ok() == false) {
    ...
    // Make sure that the starting and ending source positions are marked
    // uniformally (testing compiler generated flag).
    fixupCompilerGeneratedNodesToBeUniformlyMarked(locatedNode);

    // Make sure that isSourcePositionUnavailableInFrontend is marked
    // uniformally.
    ...
    // This case applies to class definitions and should work for templated
    // classes as well.
    SgClassDefinition *classDefinition = isSgClassDefinition(node);
    if (classDefinition != NULL) {
      // Set the end of the class definition to match the associated class
      // declaration (access it through the parent). printf ("Resetting special
      // case of ending position of SgClassDefinition to match the parent
      // SgClassDeclaration \n");
      SgClassDeclaration *classDeclaration =
          isSgClassDeclaration(classDefinition->get_parent());
      ROSE_ASSERT(classDeclaration != NULL);
      ROSE_ASSERT(classDefinition->get_startOfConstruct() !=
      ...
      *(classDefinition->get_endOfConstruct()) =
      ...
      // classDefinition->get_endOfConstruct()->display("Resetting special case
      // of ending position of SgClassDefinition to match the parent
      // SgClassDeclaration");
    }
    ...
    if (memberFunctionDeclaration != NULL) {
      SgCtorInitializerList *ctorInitializationList = isSgCtorInitializerList(
          memberFunctionDeclaration->get_CtorInitializerList());
      if (ctorInitializationList != NULL) {
        // Sg_File_Info* startingFileInfo =
        // ctorInitializationList->get_startOfConstruct();
        if (ctorInitializationList->get_ctors().empty() == true) {
          // As a compiler generate IR nodes the source position is not defined.
          // printf ("Marking SgCtorInitializerList in
          // SgMemberFunctionDeclaration to be compiler generated (since it is
          // empty()) \n"); startingFileInfo->setCompilerGenerated();
          ctorInitializationList->setCompilerGenerated();
        }
      } else {
        printf("ctorInitializationList == NULL for memberFunctionDeclaration = "
               "%p \n",
  ...
  // Handle the case of a SgInitializedName, which is not currently a
  // SgLocatedNode
  ...
  // This test will fail for SgGlobal (we should fix that!)
  if (isSgGlobal(node) == NULL && isSgFile(node) == NULL) {
    // DQ (12/22/2006): This test fails for Qing's loopProcessor (and the
    // translators tests).
    ROSE_ASSERT(returnAttribute.startingPosition->ok() == true);
    ROSE_ASSERT(returnAttribute.endingPosition->ok() == true);
  }

  // DQ (12/21/2006): Only test for "<=" if these are valid positions from the
  // front-end.
  if (returnAttribute.startingPosition
              ->isSourcePositionUnavailableInFrontend() == false &&
      returnAttribute.endingPosition->isSourcePositionUnavailableInFrontend() ==
          false) {
    // This will fail where the end of an expression or statement is in a
    // different file than the start. ROSE_ASSERT(
    // *(returnAttribute.startingPosition) <=
    // *(returnAttribute.endingPosition));
    if ((*(returnAttribute.startingPosition) <=
         *(returnAttribute.endingPosition)) == false) {
      printf("Error: (*(returnAttribute.startingPosition) <= "
             "*(returnAttribute.endingPosition)) == false \n");
      ...
      returnAttribute.startingPosition->display(
          "Leaving evaluateSynthesizedAttribute(): "
          "returnAttribute.startingPosition");
      returnAttribute.endingPosition->display(
          "Leaving evaluateSynthesizedAttribute(): "
          "returnAttribute.endingPosition");
    }
    ROSE_ASSERT(*(returnAttribute.startingPosition) <=
                *(returnAttribute.endingPosition));
  }

  printf(
      "Leaving FixupSourcePositionInformation::evaluateSynthesizedAttribute(): "
      "node = %p = %s (%d,%d) -> (%d,%d) \n",
      node, node->sage_class_name(), node->get_startOfConstruct()->get_line(),
      node->get_startOfConstruct()->get_col(),
      node->get_endOfConstruct()->get_line(),
      node->get_endOfConstruct()->get_col());

  return returnAttribute;
}

void FixupSourcePositionInformation::processChildSynthesizedAttributes(
    FixupSourcePositionInformationSynthesizedAttribute &returnAttribute,
    SynthesizedAttributesList &l) {
  // This function handles the details of evaluating the list of child
  // attributes and updating the returnAttribute with the correct information.
  // We need this function because not all IR nodes that contain source file
  // position information are derived from SgLocatedNode (though the IR should
  // be changed to reflect this, I think).
  ROSE_ASSERT(returnAttribute.associatedNode != NULL);
  ...
  if (block != NULL) {
    // printf ("Leave SgBasicBlock IR nodes with unmodified source positions to
    // help contain possible errors to the local scope where they occur. \n");
    return;
  }
  ...
  for (SynthesizedAttributesList::iterator i = l.begin(); i != l.end(); i++) {
    ...
    // DQ (11/1/2006): This should not be required, but a bad synthesized
    // attribute is being built (using the default constructor) and so
    // associatedNode field is NULL.
    if ((*i).associatedNode != NULL) {
      // printf ("Processing synthesized attribute from (*i).associatedNode = %p
      // = %s
      ...
      if (subTreeStartingPosition == NULL) {
        ...
        // This code uses the relational operators defined for the Sg_File_Info
        // class (these are not defined where the source files are different).
        subTreeStartingPosition =
            (*((*i).startingPosition) < *subTreeStartingPosition)
                ? (*i).startingPosition
                : subTreeStartingPosition;
      }
      ...
      if (subTreeEndingPosition == NULL) {
        ...
        // This code uses the relational operators defined for the Sg_File_Info
        // class (these are not defined where the source files are different).
        subTreeEndingPosition =
            (*((*i).endingPosition) > *subTreeEndingPosition)
                ? (*i).endingPosition
                : subTreeEndingPosition;
      }
    } else {
      // It appears that this occurs where there are function declarations
      // withouth a valid pointer to the function definition and so a default
      // synthesized attribute is constructed in the array of child synthesized
      // attributes. printf ("Warning, this unknown synthesized attribute should
      // never have been built ... \n");
    }
  }

  // These variables are created to make the code more clear, we could just
  // update returnAttribute.startingPosition and returnAttribute.endingPosition
  // directly.
  ...
  // Let these be the default values and then update these if the subtree has a
  // better value. returnAttribute.startingPosition = currentStartingPosition;
  // returnAttribute.endingPosition   = currentEndingPosition;

  // Now fixup the returnAttribute's source position information using that from
  // the subtree (child, synthizied, attributes).
  if (subTreeStartingPosition != NULL) {
    // Note that > is not the same as !(<=) since the file names must match for
    // any relational operator to return the value of "true".
    // returnAttribute.startingPosition = (*currentStartingPosition <
    // *subTreeStartingPosition) ? currentStartingPosition :
    // subTreeStartingPosition;
    if (*currentStartingPosition < *subTreeStartingPosition) {
      // printf ("*currentStartingPosition < *subTreeStartingPosition \n");
      returnAttribute.startingPosition = currentStartingPosition;
    } else {
      // This does not really imply that *currentStartingPosition >=
      // *subTreeStartingPosition printf ("NOT *currentStartingPosition <
      // *subTreeStartingPosition \n");
  ...
  if (subTreeEndingPosition != NULL) {
    ...
    // any relational operator to return the value of "true".
    // returnAttribute.endingPosition   = (*currentEndingPosition   >
    // *subTreeEndingPosition)   ? currentEndingPosition   :
    // subTreeEndingPosition;
    if (*currentEndingPosition > *subTreeEndingPosition) {
      // printf ("*currentEndingPosition > *subTreeEndingPosition \n");
      returnAttribute.endingPosition = currentEndingPosition;
    } else {
      // This does not really imply that *currentEndingPosition <=
      // *subTreeEndingPosition printf ("NOT *currentEndingPosition >
      // *subTreeEndingPosition \n"); ROSE_ASSERT (*currentEndingPosition <
      // *subTreeEndingPosition);
      returnAttribute.endingPosition = subTreeEndingPosition;
    }
  }

  // Now adjust the source position information if required for make it correct
  // (we only modify the line number and column numbers).
  if (subTreeStartingPosition != NULL &&
      *currentStartingPosition > *subTreeStartingPosition) {
    // DQ (12/22/2006): This may be a source of errors when the update changes
    // the flags that classify the IR node (e.g. compiler generated, output in
    // code generation, etc.). printf ("Skipping reset of Sg_File_Info object
