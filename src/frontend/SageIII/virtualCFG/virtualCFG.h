...
enum EdgeConditionKind {
  ...
  eckComputedGotoCaseLabel,   //! Case in computed goto -- number needs to be
                              //! computed separately
  eckArithmeticIfLess, //! Edge for the arithmetic if expression being less than
                       //! zero
  eckArithmeticIfEqual, //! Edge for the arithmetic if expression being equal to
                        //! zero
  eckArithmeticIfGreater, //! Edge for the arithmetic if expression being
                          //! greater than zero
...
class ROSE_DLL_API CFGNode {
  ...
  SgNode *node; // Must be either a SgStatement, SgExpression, or
                // SgInitializedName (FIXME: change this to just SgLocatedNode
                // if SgInitializedName becomes a subclass of that)
...
public:
  ...
  //! The expression of the computed goto represented by the eckArithmeticIf*
  //! conditions
  unsigned int computedGotoCaseIndex() const;
  //! The test or case key that is tested as a condition of this control flow
  //! edge
  ...
  //! operator<() has an arbitrary ordering, but allows these objects to be used
  //! with std::set and std::map
  bool operator<(const CFGEdge &o) const {
...
public:
  ...
  // (in compiling this file using ROSE) see test2006_124.C for a smaller
  // example.
  ...
  //! Returns the edges in the path, starting at the source and ending at the
  //! target
...
//! execute)
inline CFGNode cfgBeginningOfConstruct(SgNode *c) { return CFGNode(c, 0); }
...
inline CFGNode makeCfg(SgNode *start) { return cfgBeginningOfConstruct(start); }
