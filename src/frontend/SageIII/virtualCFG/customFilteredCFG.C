...
void ROSE_DLL_API CustomFilteredCFG<_filter>::buildFilteredCFG() {
  ...
  if (SgProject *project = isSgProject(start_)) {
    ...
    for (Rose_STL_Container<SgNode *>::const_iterator i = functions.begin();
      ...
      if (proc) {
        buildTemplatedCFG<VirtualCFG::FilteredCFGNode<_filter>,
                          VirtualCFG::FilteredCFGEdge<_filter>>(
...
void CustomFilteredCFG<_filter>::buildTemplatedCFG(
    NodeT n, std::map<NodeT, SgGraphNode *> &all_nodes,
