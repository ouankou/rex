...
/*! \brief Mark an AST subtree to be unparsed using an alternative file (for
   multi-file unparsing).
    \internal The fileIDsToUnparse set in Sg_File_Info is used to indicate what
...
public:
  ...
  MarkSharedDeclarationsInheritedAttribute(
      const MarkSharedDeclarationsInheritedAttribute &);
  MarkSharedDeclarationsInheritedAttribute &
  operator=(const MarkSharedDeclarationsInheritedAttribute &) =
      default; // defeat warning
};
...
class MarkSharedDeclarationsForOutputInCodeGeneration
    : public SgTopDownProcessing<MarkSharedDeclarationsInheritedAttribute> {
public:
  ...
  MarkSharedDeclarationsInheritedAttribute evaluateInheritedAttribute(
      SgNode *node,
