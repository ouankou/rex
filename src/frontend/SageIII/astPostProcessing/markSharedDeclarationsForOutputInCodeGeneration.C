...
void markSharedDeclarationsForOutputInCodeGeneration(SgNode *node) {
  ...
  // TimingPerformance timer ("(mark as compiler generated code to be unparsed)
  // time (sec) = ");
...
MarkSharedDeclarationsInheritedAttribute::
    MarkSharedDeclarationsInheritedAttribute(
        const MarkSharedDeclarationsInheritedAttribute &X) {
...
MarkSharedDeclarationsForOutputInCodeGeneration::evaluateInheritedAttribute(
    SgNode *node, MarkSharedDeclarationsInheritedAttribute inheritedAttribute) {
  MarkSharedDeclarationsInheritedAttribute return_inheritedAttribute(
      inheritedAttribute);
  ...
  if (statement != nullptr) {
    ...
    if (statement->get_file_info()->get_fileIDsToUnparse().empty() == false) {
      return_inheritedAttribute.unparseFileIdList =
          statement->get_file_info()->get_fileIDsToUnparse();
    } else {
      if (inheritedAttribute.unparseFileIdList.empty() == false) {
        statement->get_file_info()->get_fileIDsToUnparse() =
            inheritedAttribute.unparseFileIdList;
      }
    }
  }
