...
void addMacro(SgStatement *associatedStatement, std::string macroString,
              PreprocessingInfo::DirectiveType directiveType) {
  // DQ (11/5/2012): Fixup for test2012_17.c and test2012_163.c
  // This function handled the details of adding a macro associated with the
  // input string to the specfied SgStatement.
  ...
  PreprocessingInfo *macro = new PreprocessingInfo(
      directiveType, macroString, filenameString, line_no, col_no, nol, relPos);
...
void FixupSelfReferentialMacrosInAST::visit(SgNode *node) {
  ...
  case V_SgInitializedName: {
    ...
    if (classType != NULL) {
      ...
      if (targetClassDeclaration != NULL) {
        ...
        // printf ("In FixupSelfReferentialMacrosInAST::visit(): Found a class
        // declaration name = %s \n",className.str());

        // For sudo_exec_pty.c also look for siginfo
        if (className == "sigaction" || className == "siginfo") {
          // printf ("In FixupSelfReferentialMacrosInAST::visit(): Found a
          // sigaction type \n");

          // Note we could also check that the declaration came from a known
          // header file.
          SgStatement *associatedStatement =
              isSgStatement(initializedName->get_parent());
          if (associatedStatement != NULL) {
            // Add a macro to undefine the "#define sa_handler
            // __sigaction_handler.sa_handler" macro. printf ("In
            // FixupSelfReferentialMacrosInAST::visit(): Add a macro to undefine
            // the macro #define sa_handler __sigaction_handler.sa_handler \n");

            // PreprocessingInfo* macro = new PreprocessingInfo(DirectiveType,
            // const std::string & inputString,const std::string &
            // filenameString, int line_no , int col_no,int nol,
            // RelativePositionType relPos );

            PreprocessingInfo::DirectiveType directiveType =
                PreprocessingInfo::CpreprocessorUndefDeclaration;

            // We are puting out all macros anytime we see either type.  This
            // might be too much...
        ...
        // DQ (7/18/2020): Output a warning message here
        // (Cxx11_tests/test2020_69.C).
        printf("Warning: In FixupSelfReferentialMacrosInAST::visit(): "
