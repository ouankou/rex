...
void markTransformationsForOutput(SgNode *node) {
  ...
  // or IR nodes marked explicitly for output (typically compiler generated IR
  // nodes).
...
MarkTransformationsForOutput::evaluateInheritedAttribute(
  ...
  MarkTransformationsForOutputInheritedAttribute returnAttribute =
      inheritedAttribute;
  ...
  if (fileInfo != NULL) {
    if (returnAttribute.insideTransformationToOutput == true) {
      // Liao 4/32/2011
      // We want to keep the original file info for PragmaDeclaration of the
      // following code: for () // similarly for if-stmt, etc
      //   #pragma omp
      //   single-statement;
      // the frontend will fix the for loop's body with a basic block, which has
      // transformation generated file info

      bool bkeep = false;
      if (isSgPragmaDeclaration(node))
        if (isSgBasicBlock(node->get_parent())) {
          SgLocatedNode *grand_parent =
              isSgLocatedNode(node->get_parent()->get_parent());
      ...
      if (fileInfo->isTransformation() == false && !bkeep) {
        ...
        // DQ (3/20/2015): Mark this as a transformation uniformally for both
        // the startOfConstruct and endOfConstruct separately. Note: there is
        // not uniform API at SgLocatedNode for marking the startOfConstruct and
        // endOfConstruct together. fileInfo->setTransformation();
        ...
        // printf ("In MarkTransformationsForOutput: Also mark transformation
      ...
      if (fileInfo->isTransformation() == true) {
        ...
        // DQ (5/16/2017): Test if this is the cause of the fortran code
        // generation issue (for the outliner).  It IS! DQ (5/17/2017): Liao
        // helped with a better fix, which was to not build a prototype in the
        // AST where the outliner is being used on Frotran code. DQ (5/9/2017):
        // We also have to mark the root of the subtree.
        SgLocatedNode *locatedNode = isSgLocatedNode(node);
        if (locatedNode != NULL) {
    ...
    if (SgProject::get_verbose() > 3) {
      if (fileInfo->isTransformation() == true) {
        printf(
            "MarkTransformationsForOutput (isTransformation() == true): node = "
            "%p = %s = %s fileinfo = %p insideTransformationToOutput = %s \n",
            node, node->class_name().c_str(),
            SageInterface::get_name(node).c_str(), fileInfo,
            returnAttribute.insideTransformationToOutput ? "true" : "false");
      }

      if (fileInfo->isCompilerGenerated() == true) {
        printf("MarkTransformationsForOutput (isCompilerGenerated() == true): "
               "node = %p = %s = %s fileinfo = %p \n",
               node, node->class_name().c_str(),
               SageInterface::get_name(node).c_str(), fileInfo);
      }

      if (fileInfo->isFrontendSpecific() == true) {
        printf("MarkTransformationsForOutput (isFrontendSpecific() == true): "
               "node = %p = %s = %s fileinfo = %p \n",
               node, node->class_name().c_str(),
               SageInterface::get_name(node).c_str(), fileInfo);
      }
      ...
      if (fileInfo->isCompilerGenerated() == false &&
        ...
        printf("MarkTransformationsForOutput (normal): node = %p = %s = %s "
               "fileinfo = %p \n",
               node, node->class_name().c_str(),
               SageInterface::get_name(node).c_str(), fileInfo);
      }
