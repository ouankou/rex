...
size_t checkIsFrontendSpecificFlag(SgNode *ast) {
  struct T1 : public AstPrePostProcessing {
    ...
    // Start marking nodes as frontend-specific once we enter an AST that's
    // frontend-specific.
    void preOrderVisit(SgNode *node) {
      SgLocatedNode *located = isSgLocatedNode(node);
      if (located) {
        bool in_fes_ast =
            fes_ast != NULL || is_frontend_specific(located->get_file_info()) ||
    ...
    // Criteria for deciding whether we're entering the top of an AST that's
    // frontend-specific.
    bool is_frontend_specific(Sg_File_Info *finfo) {
      static const char *header_name = "/rose_required_macros_and_functions.h";
      return finfo && std::string::npos !=
                          finfo->get_filenameString().rfind(header_name);
    }

    // Mark node as frontend-specific and emit a warning if it wasn't already so
    // marked.
