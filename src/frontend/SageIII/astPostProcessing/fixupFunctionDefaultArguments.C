...
// DQ (7/20/2025): Added performance tracing using Matt's tool (built on top of
// the default performance analysis in ROSE).
#define USING_PERFORMANCE_TRACING 0

bool containsLambdaSupportForFixupFunctionDefaultArguments(SgExpression *node) {
  // This function takes the initializer for any default initialization.
  // I need it becasue the SageInterface::deleteAST is not robust enough to
  // support the rather complicated case of deleting a lambda expression and
  // allof the associated generated classes and member functions.

#if USING_PERFORMANCE_TRACING || 0
  TimingPerformance timer(
      "Fixup function default arguments: "
      "containsLambdaSupportForFixupFunctionDefaultArguments");
#endif
...
size_t numberOfNodeInAST(SgExpression *node) {
  // DQ (7/20/2025): This function is for debugging information, it counts the
  // number of nodes in the subtrees represented by the input expression.

#if USING_PERFORMANCE_TRACING
  TimingPerformance timer(
      "Fixup function default arguments: numberOfNodeInAST");
#endif
  ...
  public:
    ...
    void visit(SgNode * /*node*/) { nodeCount++; }
  };
...
void fixupFunctionDefaultArguments(SgNode *node) {
  ...
  if (node->get_parent() == nullptr) {
...
#if USING_PERFORMANCE_TRACING
      TimingPerformance timer(
          "Fixup function default arguments: processing the SgProject");
#endif
    ...
    // When processing templates we need to get the SgFile so that we can check
    // the command line options.
    file = SageInterface::getEnclosingSourceFile(node);
    if (file != nullptr) {
#if USING_PERFORMANCE_TRACING
      TimingPerformance timer(
          "Fixup function default arguments: processing the SgSourceFile");
#endif
      // This simplifies how the traversal is called!
      FixupFunctionDefaultArguments declarationFixupTraversal(file);

      // This inherited attribute is used for all traversals (within the
      // iterative approach we define)
      FixupFunctionDefaultArgumentsInheritedAttribute inheritedAttribute;

      {
#if USING_PERFORMANCE_TRACING
        TimingPerformance timer("Fixup function default arguments: traversal "
                                "to generate map of sets");
#endif
        // This will be called iteratively so that we can do a fixed point
        // iteration
...
#if USING_PERFORMANCE_TRACING
        TimingPerformance timer(
            "Fixup function default arguments: processing the map of sets");
#endif
        // Now we have assembled the global data structure to represent the
        // function declarations using default arguments and we have to use this
        // data to eliminate the redundant default entries.
        std::map<SgFunctionDeclaration *,
                 FixupFunctionDefaultArguments::SetStructure *>::iterator i =
            declarationFixupTraversal.mapOfSetsOfFunctionDeclarations.begin();
        while (
            i !=
            declarationFixupTraversal.mapOfSetsOfFunctionDeclarations.end()) {
          ...
          SgFunctionDeclaration *bestFunctionDeclarationForDefaultArguments =
              setStructure->associatedFunctionDeclaration;
          std::set<SgFunctionDeclaration *> &setOfFunctionDeclarations =
              setStructure->setOfFunctionDeclarations;
          std::set<SgFunctionDeclaration *>::iterator j =
              setOfFunctionDeclarations.begin();
          while (j != setOfFunctionDeclarations.end()) {
            ...
            if (functionDeclarationFromSet !=
              ...
              SgInitializedNamePtrList &argList =
                  functionDeclarationWithRedundantDefaultArguments->get_args();
              ...
              while (k != argList.end()) {
                ...
                if (defaultArgument != nullptr) {
                  // DQ (1/27/2019): Test this for a Lambda Expression: see
                  // Cxx11_tests/test2019_38.C. This sort of test would not be
                  // suffient, comment out the call to delete instead.
                  bool foundLambda =
                      containsLambdaSupportForFixupFunctionDefaultArguments(
                          defaultArgument);
...
#if USING_PERFORMANCE_TRACING
                    TimingPerformance timer(
                        "Fixup function default arguments: traversal to "
                        "process sets: deleteAST (now skipping deleteAST)");
#endif
                    // DQ (7/20/2025): After discussion with Tristan, we will
                    // skip the call to deleteAST() because it is expensive
                    // (after the performance bug with symbol table fixup, it
                    // now takes 1/3 of the time to just call deleteAST() (on a
                    // input program including rose.h header file as a
                    // benchmark). Note that for this input program, there are
                    // 20M IR nodes, but only 1700 need to be deleted, so this
                    // is an insignificant memory issue. A plan is to at a later
                    // date use a function that Tristan wrote to identify and
                    // delete all of the IR nodes that are disconected from the
                    // AST (as defined with a root at SgProject).
                    defaultArgument->set_parent(NULL);
                    arg->set_initializer(NULL);
                  } else {
                    printf(
                        "NOTE: Skipping call to SageInterface::deleteAST() for "
                        "default arguments containing lambda expressions \n");
                  }
...
FixupFunctionDefaultArguments::evaluateInheritedAttribute(
  ...
  // I think that our design does not need inherited attributes, but this might
  // change.
  ...
  if (functionDeclaration != nullptr) {
    ...
    while (functionNotProcessed == false && i != initializedNameList.end()) {
      if ((*i)->get_initializer() != nullptr) {
        ...
        std::map<SgFunctionDeclaration *, SetStructure *>::iterator
            existingSetIterator = mapOfSetsOfFunctionDeclarations.find(
        ...
        // DQ (6/24/2013): This test fails for ROSE compiling ROSE (largest
        // input codes yet tested). Issue a warning and commented out the
        // assertion for now.  Need to investigate this further. This might be
        // OK to have be negative in the case of some template or whare one of
        // the other of the source sequence points are unknown (compiler
        // generated).
        if (d < 0) {
          printf("WARNING: negative distance generated between source sequence "
                 "numbers: d = %d (commented out assertion) \n",
                 d);
        }
        // Because of the way we construct the AST initally, we should only have
        // positive values or zero as distance. ROSE_ASSERT(d >= 0);
        ...
        if (existingSetIterator != mapOfSetsOfFunctionDeclarations.end()) {
          ...
          mapOfSetsOfFunctionDeclarations[firstNondefiningFunctionDeclaration] =
              newSet;
        ...
        // As soon as we have processed a single default argument for the
        // function we have processed the function and need not worry about
        // other default arguments.  When we process the AST (later) to remove
        // the redundant default arguments we will process all of the default
