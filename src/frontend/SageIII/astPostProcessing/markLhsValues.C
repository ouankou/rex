...
void markLhsValues(SgNode *node) {
  // Liao, 2/19/2009, turn this off since it is called hundreds times and
  // pollute the timing report
...
void MarkLhsValues::visit(SgNode *node) {
  // DQ (1/19/2008): Fixup the get_lvalue() member function which is common on
  // expressions. printf ("In TestLValueExpressions::visit(): node = %s
  // \n",node->class_name().c_str());

  ROSE_ASSERT(node != NULL);

  // This function most often sets the SgVarRefExp which appears as an lhs
  // operand in a limited set of binary operators.
  ...
  if (expression != NULL) {
    ...
    if (binaryOperator != NULL) {
      ...
      case V_SgXorAssignOp: {
        ...
        // DQ (12/16/2020): Added debugging info (was a problem for preliminary
        // transformation for the code segregation).
        if (rhs == NULL) {
      ...
      case V_SgArrowExp: {
        ...
        // DQ (10/9/2008): For the Fortran user defined operator, the lhs is not
        // an L-value. This represents my understanding, because assignment is
      ...
      // SgExpression* rhs = binaryOperator->get_rhs_operand();
      //  Liao 3/14/2011. This function is called by builders for binary
      //  expressions. These builders can accept empty right hand operands.
      //  ROSE_ASSERT(rhs != NULL);
      ...
      if (rhs != NULL)
        if (rhs->get_lvalue() == true) {
          printf("Error for rhs = %p = %s = %s in binary expression = %s \n",
                 ...
                 SageInterface::get_name(rhs).c_str(),
                 expression->class_name().c_str());
          binaryOperator->get_startOfConstruct()->display(
    ...
    if (unaryOperator != NULL) {
      ...
      case V_SgPlusPlusOp: {
        ...
        if (operand->get_lvalue() == false) {
          printf("Error for operand = %p = %s = %s in unary expression "
                 "(SgMinusMinusOp or SgPlusPlusOp) = %s \n",
      ...
      case V_SgThrowOp: {
        ...
        // DQ (10/9/2008): For the Fortran user defined operator, the operand is
        // not an L-value. This represents my understanding, because assignment
        // is explicitly handled separately.
