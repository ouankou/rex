...
void FixupFortranReferences::visit(SgNode *node) {
  // DQ (11/24/2007): Output the current IR node for debugging the traversal of
  // the Fortran AST. printf ("node = %s \n",node->class_name().c_str());

  SgUnknownArrayOrFunctionReference *unknownArrayOrFunctionReference =
      isSgUnknownArrayOrFunctionReference(node);
  if (unknownArrayOrFunctionReference != NULL) {
    // printf ("Found the SgUnknownArrayOrFunctionReference \n");

    // SgStatement* currentStatement =
    // SageInterface::getEnclosingStatement(unknownArrayOrFunctionReference);
    ROSE_ASSERT(unknownArrayOrFunctionReference->get_parent() != NULL);
    SgExprStatement *currentExpressionStatement =
        isSgExprStatement(unknownArrayOrFunctionReference->get_parent());
    ...
    if (currentExpressionStatement != NULL) {
      ...
      SgExprListExp *functionArguments =
          unknownArrayOrFunctionReference->get_expression_list();
      ...
      // Remove the SgUnknownArrayOrFunctionReference IR node (add this to a
      // delete list so we don't confuse the AST traversal). delete
  ...
  if (arithmeticIf != NULL) {
    // Fixup the label references.  This is required because the labeled
    // statements were not available (had not been visited) when the
    // SgArithmeticIfStatement IR node was built.

    // DQ (12/30/2007): This is now modified due to the introduction of the
    // SgLabelRefExp SgLabelSymbol* labelArray[3] = {
    // arithmeticIf->get_less_label(),
    //                                  arithmeticIf->get_equal_label(),
    //                                  arithmeticIf->get_greater_label() };
    SgLabelSymbol *labelArray[3] = {
        arithmeticIf->get_less_label()->get_symbol(),
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (labelSymbol != NULL) {
        // Check if this is a SgLabelSymbol pointing to a SgNullStatement
        // (usually, if not always, because the reference to the label appeared
        // before the statement containing the label) in the AST construction
        // form OFP, I build SgNullStatements just to hold the numeric label.
        // This is better than having the IR node carry around scratch space for
        // storing such values that would later not be relavant.
        ...
        if (isSgNullStatement(fortranStatement) != NULL) {
          ...
          printf("Looking up SgLabelSymbol from symbol table for numeric label "
                 "= %d \n",
                 numericLabel);

          // fortran labels have a scope in the function where they are defined,
          // I think.
          SgFunctionDeclaration *functionDeclaration =
              SageInterface::getEnclosingFunctionDeclaration(fortranStatement,
                                                             true);
          ...
          SgLabelSymbol *labelSymbolFromSymbolTable =
              functionDefinition->lookup_label_symbol(name);
          if (labelSymbolFromSymbolTable == NULL) {
            printf("Error: labelSymbolFromSymbolTable == NULL \n");
            // functionDefinition->get_symbol_table()->print("Error:
            // labelSymbolFromSymbolTable == NULL",V_SgLabelSymbol);
            functionDefinition->print_symboltable(
                "Error: labelSymbolFromSymbolTable == NULL", V_SgLabelSymbol);
          }
        ...
        printf("The SgLabelSymbol should have been built (if only so that the "
               "numeric label value could be communicated via a "
               "SgNullStatement) \n");
        ROSE_ABORT();
      }
    }
