...
void processTemplateHandlingOptions(SgNode *node) {
  ...
  else // if (file != NULL)
  {
    ...
    bool buildImplicitTemplates =
        (file != NULL) && (file->get_no_implicit_templates() == false);
    bool buildImplicitInlineTemplates =
        (file != NULL) && (file->get_no_implicit_inline_templates() == false);
    // This simplifies how the traversal is called!
    ProcessTemplateHandlingOptions astFixupTraversal(
        file, buildImplicitTemplates, buildImplicitInlineTemplates);

    // I think the default should be preorder so that the interfaces would be
    // more uniform
    astFixupTraversal.traverse(node, preorder);
  }
}

ProcessTemplateHandlingOptions::ProcessTemplateHandlingOptions(
    SgFile * /*file*/, bool buildImplicitTemplates,
    bool buildImplicitInlineTemplates)
    : buildImplicitTemplates(buildImplicitTemplates),
      buildImplicitInlineTemplates(buildImplicitInlineTemplates) {
  // Constructor
}

void ProcessTemplateHandlingOptions::visit(SgNode *node) {
  // DQ (8/19/2005): Unmark implicit template instantiations that would be
  // output
  ...
  case V_SgTemplateInstantiationDecl: {
    ...
    if (buildImplicitTemplates == false) {
      ...
      if (fInfo->isOutputInCodeGeneration() == true &&
          s->isSpecialization() == false) {
        printf("AST Fixup: This template class will not be output within code "
               "generation \n");
        fInfo->unsetOutputInCodeGeneration();
      }
      ROSE_ASSERT(fInfo->isOutputInCodeGeneration() == false ||
                  s->isSpecialization() == true);
    }
    break;
  }
  ...
  case V_SgTemplateInstantiationFunctionDecl: {
    ...
    if (buildImplicitTemplates == false) {
      ...
      // instantiations to NOT be output in code generation
      ROSE_ASSERT(s != NULL);
      if (s->get_file_info()->isOutputInCodeGeneration() == true &&
          s->isSpecialization() == false) {
        printf("AST Fixup: This template function will not be output within "
               "code generation \n");
        s->get_file_info()->unsetOutputInCodeGeneration();
      }
      ROSE_ASSERT(s->get_file_info()->isOutputInCodeGeneration() == false ||
                  s->isSpecialization() == true);
    }
    break;
  }
  ...
  case V_SgTemplateInstantiationMemberFunctionDecl: {
    ...
    if (buildImplicitTemplates == false &&
        buildImplicitInlineTemplates == false) {
      // Mark all template member function instantiations except specialized
      // template instantiations to NOT be output in code generation
      if (s->get_file_info()->isOutputInCodeGeneration() == true &&
          s->isSpecialization() == false) {
        printf("AST Fixup: This template member function will not be output "
