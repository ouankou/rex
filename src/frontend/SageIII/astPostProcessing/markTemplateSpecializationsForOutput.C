  ...
  if (node->get_parent() == NULL) {
    ...
    if (project != NULL) {
      ...
      for (fIterator = files.begin(); fIterator != files.end(); ++fIterator) {
        ...
        // printf ("Calling markTemplateInstantiationsForOutput() for file = %p
        // = %s \n",file,file->class_name().c_str());
        markTemplateInstantiationsForOutput(file);
        // printf ("DONE: Calling markTemplateInstantiationsForOutput() for file
        // = %p = %s \n",file,file->class_name().c_str());
      }
    }
  } else {
    ...
    // the command line options. ROSE_ASSERT(file != NULL);

    // printf ("current file is: %s \n",(file != NULL) ? file->getFileName() :
    // "NULL SgFile pointer");

    // Note: since the currentFile must be valid, any template specializations
    // in code fragements unattached to the primary AST will not be marked for
    // output.  Later post processing, once the fragment is attached to the
    // primary AST, will catch these cases.
    if (file != NULL) {
...
MarkTemplateSpecializationsForOutput::MarkTemplateSpecializationsForOutput(
    SgSourceFile *inputFile) {
  // Save the file in the traversal class so that we can access the backend
  // specific template instatiation control flags easily. ROSE_ASSERT(inputFile
  // != NULL);
...
MarkTemplateSpecializationsForOutput::evaluateInheritedAttribute(
    SgNode *node,
    MarkTemplateSpecializationsForOutputInheritedAttribute inheritedAttribute) {
  MarkTemplateSpecializationsForOutputInheritedAttribute returnAttribute =
      inheritedAttribute;
  ...
  // Note: since the currentFile must be valid, any template specializations in
  // code fragements unattached to the primary AST will not be marked for
  // output.  Later post processing, once the fragment is attached to the
  // primary AST, will catch these cases.
