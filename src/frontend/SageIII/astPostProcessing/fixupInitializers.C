...
void fixupInitializersUsingIncludeFiles(SgProject *node) {
  ...
  // uniform DQ (8/27/2020): We will only want to address files that we will
  // unparse.
...
FixupInitializersUsingIncludeFilesInheritedAttribute::
    FixupInitializersUsingIncludeFilesInheritedAttribute(
        const FixupInitializersUsingIncludeFilesInheritedAttribute &X) {
...
FixupInitializersUsingIncludeFilesSynthesizedAttribute::
    FixupInitializersUsingIncludeFilesSynthesizedAttribute(
        const FixupInitializersUsingIncludeFilesSynthesizedAttribute &) {}
...
void FixupInitializersUsingIncludeFilesTraversal::findAndRemoveMatchingInclude(
  ...
  if (comments != nullptr) {
    // DQ (8/27/2020): Save the CPP directive and delete after the processing
    // via the loop to avoid iterator invalidation.
    // std::vector<AttachedPreprocessingInfoType::iterator>
    // includeDirectiveToRemove;
    AttachedPreprocessingInfoType includeDirectiveToRemove;
    ...
    for (AttachedPreprocessingInfoType::iterator i = comments->begin();
      ...
      if ((*i)->getRelativePosition() == location_to_search) {
        ...
        if ((*i)->getTypeOfDirective() ==
          ...
          std::string filenameFromIncludeDirective =
              (*i)->get_filename_from_include_directive();
          string expressionFilename =
              expression->get_file_info()->get_filename();
          ...
          if (expressionFilename == "compilerGenerated") {
            ...
            if (valueExpression == nullptr) {
              printf("Error: valueExpression == NULL: expression = %p = %s "
                     "name = %s \n",
                     expression, expression->class_name().c_str(),
                     SageInterface::get_name(expression).c_str());
              printf(" --- statement = %p = %s \n", statement,
                     statement->class_name().c_str());
              statement->get_file_info()->display(
                  "Error: valueExpression == NULL");
            }
            ...
            if (knownSourcePositionUnavailable == true) {
              ...
              SgSourceFile *sourceFile = SageInterface::getEnclosingSourceFile(
                  statement, includingSelf);
              ASSERT_not_null(sourceFile);

              std::string source_directory = sourceFile->getSourceDirectory();
              std::string expectedSourceFileLocation =
                  source_directory + "/" + filenameFromIncludeDirective;
              ifstream infile;
              ...
              if (infile.is_open()) {
                ...
                file_as_string.assign((std::istreambuf_iterator<char>(infile)),
                ...
                printf("Could not file include file at: "
                       "filenameFromIncludeDirective = %s \n",
                       filenameFromIncludeDirective.c_str());
                // list the possible places to search:
                // ROSE_DLL_API SgSourceFile* getEnclosingSourceFile(SgNode* n,
                // const bool includingSelf=false);
              }
            } else {
              printf("Note: Source position information is unavailable for "
                     "this expression = %p = %s name = %s \n",
                     expression, expression->class_name().c_str(),
                     SageInterface::get_name(expression).c_str());
            }
          }

          // DQ (8/27/2020): This is a bit more complex than I expected it to
          // be. printf ("This logic is not quite correct yet \n"); This logic
          // is not quite correct yet, we need to search for the
          // filenameFromIncludeDirective in the expressionFilename (and even
          // that might be problematic). Cases such as: #include "../header.h"
          // are not addressed bu this logic. and there are more complex cases.
          // ultimately we would have to implement the header file lookup rules
          // using the compiler's logic.
          size_t includeFilenameLocationInExpressionFilename =
              expressionFilename.find(filenameFromIncludeDirective);

          // bool files_match = (includeFilenameLocationInExpressionFilename !=
          // string::npos);
          bool files_match =
              (includeFilenameLocationInExpressionFilename != string::npos) ||
              (foundCorrectHeaderFileToRemove == true);
    ...
    for (AttachedPreprocessingInfoType::iterator i =
             includeDirectiveToRemove.begin();
         i != includeDirectiveToRemove.end(); i++) {
      // AttachedPreprocessingInfoType::iterator location = comments.find(*i);
      // comments.erase(location);
      // comments.erase(std::remove(comments.begin(), comments.end(), *i),
      // comments.end()); std::remove(comments.begin(), comments.end(), *i);
      AttachedPreprocessingInfoType::iterator position =
...
FixupInitializersUsingIncludeFilesInheritedAttribute
FixupInitializersUsingIncludeFilesTraversal::evaluateInheritedAttribute(
    SgNode *node,
    FixupInitializersUsingIncludeFilesInheritedAttribute inheritedAttribute) {

  FixupInitializersUsingIncludeFilesInheritedAttribute return_attribute(
      inheritedAttribute);
  ...
  if (inheritedAttribute.isInsideInitializer == true) {
    ...
    // doesn't tell us anything about the location of what could come from a
    // header file.
    ...
    bool ignoreSomeExpressions =
        (exprListExp != nullptr) || (castExp != nullptr);

    if ((expression != nullptr) && (initializer == nullptr) &&
        (ignoreSomeExpressions == false)) {
      string expressionFilename = expression->get_file_info()->get_filename();
      ASSERT_not_null(inheritedAttribute.variableDeclaration);

      // string variableDeclarationFilename =
      // inheritedAttribute.variableDeclaration->get_file_info()->get_filename();
      string variableDeclarationFilename =
          inheritedAttribute.variableDeclaration->get_file_info()
              ->get_physical_filename();
      // Note that SgStringVal will not be assigned a source position (it will
      // be marked as: "compilerGenerated". if (expressionFilename !=
      // variableDeclarationFilename || expressionFilename ==
      // "compilerGenerated")
      if (expressionFilename != variableDeclarationFilename) {
        ...
        if (comments != nullptr) {
          ...
          findAndRemoveMatchingInclude(inheritedAttribute.variableDeclaration,
                                       expression, PreprocessingInfo::after);
        } else {
        }
        ...
        // DQ (8/27/2020): Some scopes don't work for the
        // SageInterface::getNextStatement() function.
        ...
        if (ignoreThisScope == false) {
          ...
          if (next_statement == nullptr) {
            ...
            findAndRemoveMatchingInclude(next_statement, expression,
