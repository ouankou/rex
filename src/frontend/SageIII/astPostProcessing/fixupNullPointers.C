...
void FixupNullPointersInAST::visit(SgNode *node) {
  ...
  switch (node->variantT()) {
    // This fixup was implemented because the inliner demonstrated how Qing's
    // AST interface could be used to build an SgForStatement where4 many parts
    // of the loop were left as NULL pointers.  All added IR nodes are marked as
    // compiler generated.
  case V_SgForStatement: {
    // Note that the for loop's initializer is set internally by default, it is
    // a list; and is allowed to be an empty list.  Thus no Null pointer value
    // is require to indicate that there are no variable present.
    ...
    if (forStatement->get_for_init_stmt() == NULL) {
      printf("In FixupNullPointersInAST::visit(): "
             "forStatement->get_for_init_stmt() == NULL: Building "
             "SgNullExpression with source position from "
             "generateDefaultFileInfoForCompilerGeneratedNode() \n");
      SgForInitStatement *forInitStatement = new SgForInitStatement(
    ...
    // Avoid leaving a null pointer for the for loop's test statement (note the
    // test is not an expression in C or C++).
    if (forStatement->get_test() == NULL) {
      printf("In FixupNullPointersInAST::visit(): forStatement->get_test() == "
             "NULL: Building SgNullExpression with source position from "
             "generateDefaultFileInfoForCompilerGeneratedNode() \n");
    ...
    if (forStatement->get_increment() == NULL) {
      ...
      //      new
      //      SgExpressionRoot(Sg_File_Info::generateDefaultFileInfoForCompilerGeneratedNode(),
      //                           new
      //                           SgNullExpression(Sg_File_Info::generateDefaultFileInfoForCompilerGeneratedNode()));
      // ROSE_ASSERT(expressionRoot != NULL);
      // forStatement->set_increment_expr_root(expressionRoot);
      // expressionRoot->set_parent(forStatement);
      printf(
          "In FixupNullPointersInAST::visit(): forStatement->get_increment() "
          "== NULL: Building SgNullExpression with source position from "
          "generateDefaultFileInfoForCompilerGeneratedNode() \n");
      SgExpression *expression = new SgNullExpression(
          Sg_File_Info::generateDefaultFileInfoForCompilerGeneratedNode());
    ...
    // this code to evolve as a way to tighten up the design of the AST over
    // time.
  default: {
  ...
  if (locatedNode != NULL) {
    if (locatedNode->get_startOfConstruct() == NULL)
      printf("Warning: startOfConstruct not set: Testing end of construct "
             "Sg_File_Info objects for locatedNode = %p = %s \n",
             locatedNode, locatedNode->class_name().c_str());

    ROSE_ASSERT(locatedNode->get_startOfConstruct() != NULL);
    // DQ (10/27/2007): Make this a warning for now since it appears to be a
    // common problem
    if (locatedNode->get_endOfConstruct() == NULL)
      printf("Warning: endOfConstruct not set: Testing end of construct "
             "Sg_File_Info objects for locatedNode = %p = %s \n",
             locatedNode, locatedNode->class_name().c_str());
  }
}
