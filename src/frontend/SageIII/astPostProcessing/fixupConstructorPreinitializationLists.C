...
void ResetContructorInitilizerLists::visit(SgNode *node) {
  ...
  while (i != ctorList.end()) {
    ...
    // This is likely a base class, but even if it is a data member
    // initialization of a class type we still want to check it Actually I think
    // we need to distinguish between these to case (as they appear in "B(int
    // i): A<int>(i), x(7),a(99) {};") But we could just cheat for now and look
    // for "____L" substrings in the name
    ...
    if (classType != NULL) {
      ...
      if (name.find("____L") != string::npos) {
        ...
        printf("Error: found a base class name that needs to be updated for "
               "initializedName = %p name = %s resetting using %s \n",
