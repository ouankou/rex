...
// DQ (5/18/2017):
/*! \brief Fixups transformations that have added template instantiations or
   caused template instantiations to be output so that there is a proper
   prototype declaration before the template instantation is called.

    All prototypes if built, should be output into the source file.

   \internal This post processing needs to be run before unparsing, and mostly
...
//! Inherited attribute required for AddPrototypesForTemplateInstantiations
//! class.
...
class CollectTemplateInstantiationsMarkedForOutput : public SgSimpleProcessing {
public:
  // Data structure to accumulate list of defining template instatiations marked
  // to be output in source code.
