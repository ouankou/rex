...
/*! \brief Fixup Fortran array vs. function references.
    ...
    this analysis, we build a special IR node
   (SgUnknownArrayOrFunctionReference) when a function call can not be resolved
   to a function symbol in the symbol table.  There may in the future be more
   complex senarios where we build the SgUnknownArrayOrFunctionReference IR
   nodes and defer the evaluation of a reference (as an array or a function)
