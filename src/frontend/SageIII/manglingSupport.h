...
// We need to keep a set of visit template declarations so that in the
// processing of mangled names for each template instnatiation we can detect the
// use recursion that would represent a cycle in the type system represented by
 ...
 */
std::string
mangleFunctionNameToString(const std::string &s,
 ...
 *
 *  \param[in] templ_name   Unmangled base name of the template. This
 * std::string should not contain the template arguments (i.e., should be 'foo'
 * and not 'foo < int >').
 ...
 *
 *  \param[in] templ_name   Unmangled base name of the function. This
 * std::string should not contain the template arguments (i.e., should be 'foo'
 * and not 'foo < int >').
 *  \param[in] templ_args   Container of template arguments.
 ...
 *  \returns Mangled name, including all parameter information.
 */
std::string mangleTemplateFunctionToString(
    const std::string &templ_name, const SgTemplateArgumentPtrList &templ_args,
 ...
 *
 *  \param[in] templ_name   Unmangled base name of the template. This
 * std::string should not contain the template parameter (i.e., should be 'foo'
 * and not 'foo < typename T >').
 ...
 */
std::string
mangleTemplateToString(const std::string &templ_name,
...
// DQ (10/29/2017): Added support for testing for a specific cycle in the type
// system for template arguments. It is not clear how readily this cycle
// detection can be easily generalized, so this is a test for a specific cycle
// at present.
void testForCycleInTemplateArgumentsOfTemplateDeclaration(
    const SgTemplateInstantiationDefn *templateInstantiationDefinition);
