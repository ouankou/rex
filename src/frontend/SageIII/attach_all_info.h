...
struct wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t {
  ...
  wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t(
      SgLocatedNode *node = NULL, ROSEAttributesList *preprocInfo = NULL,
      int len = 0);
};

class AttachAllPreprocessingInfoTreeTrav
    : public AttachPreprocessingInfoTreeTrav {
private:
  std::string src_file_name;
  SgFile *sage_file;

  /* map: key = filename, value = a wrapper for the data used in
   * AttachPreprocessingInfoTreeTrav. */
  std::map<int, wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t>
      map_of_all_attributes;

  /* map: key = filename, value = the first node in AST from the file (could be
   * NULL) */
  ...
  // within the handling of a large application that include system header
  // files. Basically we want to avoid processing comments in system header
  // files because they are not relevant. Holds paths to exclude when getting
  // all commments and directives
  ...
  std::pair<SgNode *, PreprocessingInfo::RelativePositionType>
...
private:
  std::istream &dependancies; /* use cin for now, better to run ROSE internally
                                 to get the list. TODO. */
public:
