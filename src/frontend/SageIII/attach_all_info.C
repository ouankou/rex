...
// DQ (12/3/2008): This is the data that we have to save (even though the
// currentListOfAttributes has been moved to the inherited attribute)
wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t::
    wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t(
        SgLocatedNode *node, ROSEAttributesList *preprocInfo, int len)
...
static const char *OPTION_EXCLUDEDIRECTIVESFILENAME =
    "*excludeCommentsAndDirectivesFrom";
static const char *OPTION_INCLUDEDIRECTIVESFILENAME =
    "*includeCommentsAndDirectivesFrom";
...
int getRoseOptionValuesFromCommandline(vector<string> &argv,
                                       const string &opt_name,
                                       std::vector<std::string> &values) {
  int num_matches = sla_str(argv, OPTION_PREFIX_ROSE, OPTION_VALUE_SEPARATOR,
                            opt_name, (std::string *)NULL);
  if (num_matches > 0) {
    vector<string> raw_values(num_matches);
    sla_str(argv, OPTION_PREFIX_ROSE, OPTION_VALUE_SEPARATOR, opt_name,
            &raw_values[0]);
...
AttachAllPreprocessingInfoTreeTrav::AttachAllPreprocessingInfoTreeTrav(
    SgFile *sagep)
    : AttachPreprocessingInfoTreeTrav(), sage_file(sagep),
      map_of_all_attributes(), nFiles(0), map_of_file_order(),
      array_of_first_nodes(16, (SgNode *)NULL), dependancies(cin) {
  ...
  printf("AttachAllPreprocessingInfoTreeTrav constructor: This code is now "
         "redundant with the include/exclude support for paths and filename in "
         "SgProject \n");

  // Read option to exclude directives
  getRoseOptionValuesFromCommandline(argv, OPTION_EXCLUDEDIRECTIVES,
                                     raw_conf_filename);
  ...
  getRoseOptionValuesFromCommandline(argv, OPTION_INCLUDEDIRECTIVES,
                                     raw_conf_filename);
  ...
  raw_conf_filename.clear();
  getRoseOptionValuesFromCommandline(argv, OPTION_INCLUDEDIRECTIVESFILENAME,
                                     raw_conf_filename);
  ...
  raw_conf_filename.clear();
  getRoseOptionValuesFromCommandline(argv, OPTION_EXCLUDEDIRECTIVESFILENAME,
                                     raw_conf_filename);
...
AttachAllPreprocessingInfoTreeTrav::evaluateInheritedAttribute(
  ...
  // DQ (12/1/2008): I think this code can be simplified now, but I am not clear
  ...
  if (currentFilePtr != NULL) {
    ...
    if (currentLocNodePtr != NULL) {
      ...
      // isCompilerGenerated =
      // file_info_ptr->isCompilerGeneratedNodeToBeUnparsed();
  ...
  map<int, wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t>::iterator
      attr_list_itr = map_of_all_attributes.find(file_name_id);

  // If the name is not in the map then ... it is a new file and we have to
  // process it
  if (attr_list_itr == map_of_all_attributes.end()) {
    ...
    if ((excludePath == false) && (includePath == true)) {
      ...
      printf("AttachAllPreprocessingInfoTreeTrav::evaluateInheritedAttribute():"
             " Calling getPreprocessorDirectives for file = %s \n",
             file_name_str.c_str());

      // DQ (11/30/2008): Moved currentListOfAttributes to inherited attribute
      // save_data.currentListOfAttributes =
      // getPreprocessorDirectives(file_name_str);
      ...
      printf("AttachAllPreprocessingInfoTreeTrav::evaluateInheritedAttribute():"
             " Building an empty list to represent the data from file = %s \n",
             file_name_str.c_str());
    ...
    // DQ (11/30/2008): Moved currentListOfAttributes to inherited attribute
    // save_data.sizeOfCurrentListOfAttributes =
    // save_data.currentListOfAttributes->getLength();
  ...
  AttachPreprocessingInfoTreeTraversalInheritedAttrribute inh_rsl =
      AttachPreprocessingInfoTreeTrav::evaluateInheritedAttribute(n, inh);
  save_data.previousLocNodePtr = previousLocNodePtr;
  ...
  if (dynamic_cast<SgStatement *>(n) != NULL) {
    // Nodes that should not have comments attached (since they are not
    // unparsed) Conditions obtained from
    // src/ROSETTA/Grammar/LocatedNode.code:251
    switch (n->variantT()) {
    ...
    default: {
      // 5. if the first node for a file doesn't exist, then add it; otherwise
      // do nothing:
      map<int, int>::iterator first_node_itr =
          map_of_file_order.find(currentFileNameId);
...
AttachAllPreprocessingInfoTreeTrav::evaluateSynthesizedAttribute(
  ...
  if ((currentFilePtr != NULL) || (currentLocNodePtr != NULL)) {
    syn = AttachPreprocessingInfoTreeTrav::evaluateSynthesizedAttribute(n, inh,
                                                                        st);
    save_data.previousLocNodePtr = previousLocNodePtr;
...
bool AttachAllPreprocessingInfoTreeTrav::add_first_node_for_file(const int fn,
                                                                 SgNode *n,
                                                                 int pos) {
  bool flag = false;
...
AttachAllPreprocessingInfoTreeTrav::get_first_node_for_file(const int fn,
                                                            int hintfororder) {
  ...
  for (int i = pos_itr; i < nFiles; i++) {
    if (array_of_first_nodes[i] != NULL)
      return pair<SgNode *, PreprocessingInfo::RelativePositionType>(
          array_of_first_nodes[i], loc);
  }
...
void AttachAllPreprocessingInfoTreeTrav::display(
  ...
  std::map<int,
           wrap_data_used_by_AttachPreprocessingInfoTreeTrav_t>::const_iterator
      i = map_of_all_attributes.begin();
  while (i != map_of_all_attributes.end()) {
    ...
    // i->second.currentListOfAttributes; int sizeOfCurrentListOfAttributes =
    // i->second.sizeOfCurrentListOfAttributes;

    printf(" file_id                       = %d \n", file_id);
    printf(" previousLocNodePtr            = %p \n", previousLocNodePtr);
    // printf (" currentListOfAttributes       = %p
    // \n",currentListOfAttributes); printf (" sizeOfCurrentListOfAttributes =
