...
void finishSageAST(ClangToSageTranslator &translator) {
  ...
  */
  ...
  if (SgProject::get_verbose() > 0 && global_scope != nullptr) {
    auto decl_output = [](SgDeclarationStatement *decl) -> int {
      if (decl == nullptr || decl->get_file_info() == nullptr) {
        return -1;
      }
      return decl->get_file_info()->isOutputInCodeGeneration() ? 1 : 0;
    };
    auto decl_line = [](SgDeclarationStatement *decl) -> int {
      if (decl == nullptr) {
        return -1;
      }
    ...
    auto decl_sort_line = [&](SgDeclarationStatement *decl) -> int {
      if (decl == nullptr) {
        return -1;
      }
      if (Sg_File_Info *fi = decl->get_startOfConstruct()) {
      ...
      if (SgClassDeclaration *class_decl = isSgClassDeclaration(decl)) {
        ...
        if (SgClassDefinition *class_def = def_decl->get_definition()) {
          if (Sg_File_Info *fi = class_def->get_startOfConstruct()) {
            if (fi->get_line() > 0) {
              return fi->get_line();
            }
          }
          ...
          if (SgClassDeclaration *definition_decl =
                  isSgClassDeclaration(class_def->get_declaration())) {
            if (Sg_File_Info *fi = definition_decl->get_startOfConstruct()) {
              if (fi->get_line() > 0) {
                return fi->get_line();
              }
            }
            if (Sg_File_Info *fi = definition_decl->get_file_info()) {
              if (fi->get_line() > 0) {
                return fi->get_line();
              }
            }
          }
          auto line_from_matching_decl =
              [&](const SgDeclarationStatementPtrList &list) -> int {
            for (SgDeclarationStatement *member : list) {
              SgClassDeclaration *member_decl = isSgClassDeclaration(member);
              if (member_decl == nullptr || member_decl == class_decl ||
                  member_decl == def_decl ||
                  member_decl->get_name() != def_decl->get_name()) {
                continue;
              }
              if (Sg_File_Info *fi = member_decl->get_startOfConstruct()) {
                if (fi->get_line() > 0) {
                  return fi->get_line();
                }
              }
              if (Sg_File_Info *fi = member_decl->get_file_info()) {
                if (fi->get_line() > 0) {
                  return fi->get_line();
                }
              }
          ...
          if (int member_line =
                  line_from_matching_decl(class_def->get_members());
              member_line > 0) {
            return member_line;
          }
          if (int member_line =
                  line_from_matching_decl(class_def->getDeclarationList());
              member_line > 0) {
            return member_line;
          }
          auto line_from_first_source_backed =
              [&](const SgDeclarationStatementPtrList &list) -> int {
            for (SgDeclarationStatement *member : list) {
              if (SgLocatedNode *located = isSgLocatedNode(member)) {
          ...
          if (int member_line =
                  line_from_first_source_backed(class_def->get_members());
              member_line > 0) {
            return member_line;
          }
          if (int member_line = line_from_first_source_backed(
                  class_def->getDeclarationList());
              member_line > 0) {
            return member_line;
          }
          ...
          for (SgNode *node : nested_class_nodes) {
            SgClassDeclaration *member_decl = isSgClassDeclaration(node);
            if (member_decl == nullptr || member_decl == class_decl ||
                member_decl == def_decl ||
                member_decl->get_name() != def_decl->get_name()) {
              continue;
    ...
    auto located_line = [](SgLocatedNode *node) -> int {
      if (node == nullptr) {
        return -1;
      }
      if (Sg_File_Info *fi = node->get_startOfConstruct()) {
    ...
    for (SgNode *node : class_nodes) {
      ...
      std::string name =
          decl != nullptr ? decl->get_name().getString() : std::string();
      if (decl == nullptr || (name != "ios_base" && name != "codecvt_base")) {
        continue;
      }
      ...
      SgClassDefinition *def = def_decl != nullptr ? def_decl->get_definition()
                                                   : decl->get_definition();
      std::cerr
          << "TRACE finishSageAST " << name << " decl=" << decl
          << " output=" << decl_output(decl) << " line=" << decl_line(decl)
          << " sortLine=" << decl_sort_line(decl)
          << " forward=" << (decl->isForward() ? 1 : 0) << " parent="
          << (decl->get_parent() != nullptr ? decl->get_parent()->class_name()
                                            : std::string("<null>"))
          << " scope="
          << (decl->get_scope() != nullptr ? decl->get_scope()->class_name()
                                           : std::string("<null>"))
          << " first=" << first_nondef << " defDecl=" << def_decl
          << " defLine=" << located_line(def) << " defMembers="
          << (def != nullptr ? static_cast<long long>(def->get_members().size())
                             : -1)
          << std::endl;
    }
    ...
    Rose_STL_Container<SgNode *> enum_nodes =
        NodeQuery::querySubTree(global_scope, V_SgEnumDeclaration);
    for (SgNode *node : enum_nodes) {
      SgEnumDeclaration *decl = isSgEnumDeclaration(node);
      if (decl == nullptr || decl->get_name().getString() != "_Ios_Fmtflags") {
        continue;
      }
      std::cerr << "TRACE finishSageAST _Ios_Fmtflags decl=" << decl
                << " output=" << decl_output(decl)
                << " line=" << decl_line(decl) << " parent="
    ...
    Rose_STL_Container<SgNode *> namespace_nodes =
        NodeQuery::querySubTree(global_scope, V_SgNamespaceDefinitionStatement);
    for (SgNode *node : namespace_nodes) {
      SgNamespaceDefinitionStatement *ns =
          isSgNamespaceDefinitionStatement(node);
      if (ns == nullptr || ns->get_namespaceDeclaration() == nullptr ||
          ns->get_namespaceDeclaration()->get_name().getString() != "std") {
        continue;
      }

      SgDeclarationStatementPtrList &decls = ns->get_declarations();
      for (size_t i = 0; i < decls.size(); ++i) {
        ...
        std::string name = SageInterface::get_name(decl);
        bool interesting = name == "ios_base" || name == "codecvt_base" ||
                           name == "boolalpha" || name == "scientific" ||
                           name == "_Ios_Seekdir";
        if (!interesting) {
          continue;
        }

        std::cerr << "TRACE finishSageAST std-order index="
                  << static_cast<unsigned long long>(i) << " decl=" << decl
                  << " class=" << decl->class_name() << " name=" << name
                  << " line=" << decl_line(decl)
                  << " sortLine=" << decl_sort_line(decl)
                  << " output=" << decl_output(decl) << std::endl;
      }
    }
    ...
    for (SgNode *node : located_nodes) {
      ...
      for (PreprocessingInfo *info : *attached) {
        ...
        if (text.find("#pragma GCC visibility") == std::string::npos) {
          continue;
        }
        ...
        Sg_File_Info *info_fi = info->get_file_info();
        std::cerr << "TRACE visibility-pragma owner=" << located
                  << " class=" << located->class_name() << " ownerLine="
                  << (owner_fi != nullptr ? owner_fi->get_line() : -1)
                  << " ownerStart="
                  << (owner_start != nullptr ? owner_start->get_line() : -1)
                  << " ppLine="
                  << (info_fi != nullptr ? info_fi->get_line() : -1) << " rel="
