...
ClangToSageTranslator::get_sgAsmOperandModifier(std::string modifier) {
...
ClangToSageTranslator::get_sgAsmOperandConstraint(std::string constraint) {
...
ClangToSageTranslator::get_sgAsmRegister(std::string reg) {
  SgInitializedName::asm_register_name_enum result =
...
std::string ClangToSageTranslator::generate_source_position_string(
  ...
  //  return_string = string("0x") + StringUtility::numberToString(file_id) +
  //  "_" + StringUtility::numberToString(line_number) + "_" +
  //  StringUtility::numberToString(column_number);
  return_string = std::string("0x") + std::to_string(fEntry->getUID()) + "_" +
                  std::to_string(SM.getSpellingLineNumber(srcLoc)) + "_" +
                  std::to_string(SM.getSpellingColumnNumber(srcLoc));
...
std::string
ClangToSageTranslator::generate_name_for_type(clang::TypeSourceInfo *typeInfo) {
  clang::SourceManager &SM = p_compiler_instance->getSourceManager();
  clang::TypeLoc typeLoc = typeInfo->getTypeLoc();
  return "__anonymous_" +
         generate_source_position_string(typeLoc.getBeginLoc());
}
