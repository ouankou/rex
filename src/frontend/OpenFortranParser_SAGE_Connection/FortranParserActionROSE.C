...
void build_implicit_program_statement_if_required() {
  ...
  // If we will not be seeing a program-stmt, function-stmt, or subroutine-stmt
  // then we need to generate a program-stmt as a compiler generated construct.
  ...
  if (currentScope->variantT() == V_SgGlobal) {
    ...
    // DQ (1/13/2011): Where this is in a module or type declaration other types
    // may be referenced which are constructed as SgTypeDefault and thus must be
    // fixed up as soon as possible (i.e. before references to them are possible
    // in executable statements).  This function fixes up these references.
...
void c_action_generic_name_list__begin() {
  // Nothing to do here since we keep a stack and handle items pushed on the the
  // stack later. ROSE_ASSERT(astNameListStack.empty() == true); Output
  // debugging information about saved state (stack) information.
...
void c_action_generic_name_list_part(Token_t *ident) {
  // These generic_name_list functions appear to be used for function parameters
  // (where as subroutines use the dummy-arg-list functions, I don't know why
...
#endif
{
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 0
    printf("In c_action_specification_part: numUseStmts = %d numImportStmts = "
           "%d numImplStmts = %d numDeclConstructs = %d \n",
           numUseStmts, numImportStmts, numImplStmts, numDeclConstructs);
#else
    printf("In c_action_specification_part: numUseStmts = %d numImportStmts = "
           "%d numDeclConstructs = %d \n",
           numUseStmts, numImportStmts, numDeclConstructs);
#endif
  }

  // We have got to this point and not had to build a containing main function
  // then we will not likely be any further before we process an action
  // statement (not declaration statement).
  ...
  // DQ (11/24/2007): It might be that we could output a "contains" statement
  // here (but it would be better if OFP implemented R1237).
  ...
  // If this is the specification part of a function, and if the result name
  // does not exist, create implicit result_name for function here. Look at the
  // scope stack to determine whether or not this the specification part of a
  // function.
  SgFunctionDefinition *funcDef =
      isSgFunctionDefinition(astScopeStack.front()->get_scope());
  SgProcedureHeaderStatement *funcDecl =
      funcDef ? isSgProcedureHeaderStatement(funcDef->get_declaration()) : NULL;
  if (funcDecl && !funcDecl->get_result_name()) {
    SgInitializedName *resultName = new SgInitializedName(
        funcDecl->get_name(), funcDecl->get_type()->get_return_type(), NULL,
        NULL, NULL);
    ...
    astScopeStack.front()->insert_symbol(resultName->get_name(),
                                         variableSymbol);
  }
}
...
void c_action_executable_construct() {
  // This function enforces new rules about internal stack handling.  A number
  // of stacks should be empty at this point where a statement should have been
  // completely processed.

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_executable_construct(): astExpressionStack = %" PRIuPTR
           " astNodeStack = %" PRIuPTR " \n",
           astExpressionStack.size(), astNodeStack.size());

  // DQ (12/4/2010): This works well as a test (after fixing a lot of bugs where
  // it failed).
  if (astExpressionStack.empty() == false) {
    printf("Error: There appear to be unused expressions on the expression "
           "stack \n");
  }
  ...
  if (astLabelSymbolStack.empty() == false) {
    printf("Error: There appear to be unused SgLabelSymbol IR nodes on the "
           "astLabelSymbolStack \n");
  }
...
void c_action_action_stmt() {
  // DQ (11/15/2007): At the start of every new statement (or at the end?) we
  // should have not remaining AST nodes on the stack! This occures because the
  // where statement processing will sometime need the predicate pushed on the
  // stack. But if we don't see the where statement then we didn't need it.  It
  // is not clear if this is required handling or the result of something not
  // done well.
  ...
  if (astNodeStack.empty() == false) {
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("Clearing the astNodeStack of IR nodes that were pushed so that a "
             "possible where statement could use them \n");
 ...
 *
 * @param id The identifier representing the named constant if present,
 ...
 *
 * @param definedOp The operator (either a defined-unary-op, defined-binary-op,
 * or extended-intrinsic-op).
 * @param isExtended True if the token is an extended-intrinsic-op, otherwise is
 * a defined operator.
 */
void c_action_defined_operator(Token_t *definedOp,
                               ofp_bool isExtendedIntrinsicOp) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_defined_operator(): definedOp = %p = %s "
           "isExtendedIntrinsicOp = %s \n",
  ...
  if (astNameStack.empty() == false) {
    printf("astNameStack is non-empty (skip pushing another copy on the stack: "
           "astNameStack.front() = %s \n",
           astNameStack.front()->text);
  }

  // DQ (10/8/2008): this rule causes the token to sometime appear on the stack
  // a 2nd time. printf ("Should we push the token on the stack? (YES) \n");

  // DQ (10/9/2008): To be uniform in the handling of operators (used defined
  // and intrinsic, we ALWAYS push the token onto the stack!
...
void c_action_label(Token_t *lbl) {
  ...
  // This can return a NULL symbol if the label is attached to statement which
  // can not be associated with a function scope.
  if (lbl->text != NULL) {
    // DQ (1/25/2011): This is checking for a bug found and reported to Craig
    // already. This is an advanced warning of a bug caught in
    // buildNumericLabelSymbol().
    if (lbl->line == 0) {
    ...
    SgLabelSymbol *labelSymbol =
        buildNumericLabelSymbol(lbl); // returns an extant label if any

    if (labelSymbol != NULL) {
      // printf ("Adding a labelSymbol %p for label = %s to
      // astLabelSymbolStack.size() = %" PRIuPTR "
 ...
 *   astTypeParameterStack.front() is the type param value: an expression or an
 * asterisk
 */
void c_action_type_param_value(ofp_bool hasExpr, ofp_bool hasAsterisk,
                               ofp_bool hasColon) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_type_param_value(): hasExpr = %s hasAsterisk = %s "
           "hasColon = %s \n",
           hasExpr ? "true" : "false", hasAsterisk ? "true" : "false",
           hasColon ? "true" : "false");

  // DQ (9/28/2010): This function is no longer called for each type parameter,
  // and is only called for the length parameter.
...
void c_action_intrinsic_type_spec(Token_t *keyword1, Token_t *keyword2,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_intrinsic_type_spec(): keyword1 = %p = %s keyword2 = "
           "%p = %s type = %d, hasKindSelector = %s \n",
  ...
  if (hasKindSelector == true) {
    ...
    // DQ (8/21/2010): The length expression (kind, length, or tyep-spec) for
    // the type is what we are trying to identify.  Depending on the version of
    // Fortran and the syntax used to specify the kind.

    // Note that for CHARACTER types the length of the string is on the
    // astExpressionStack, but for where it is a kind parameter it is on the
    // astTypeKindStack.
    if (astExpressionStack.empty() == false) {
      ...
      if (astTypeKindStack.empty() == false) {
        // Note that it is (I think) possible for a string to be declared
        // "character*n" where "n" is an integer expression.  I need a test code
        // for this before I can fix it. lengthExpression =
        // isSgIntVal(astTypeKindStack.front());
    ...
    // Base on the base type, we want to inteprete the type parameters, kind
    // parameters, etc. and generate the correct mapping to the type IR node in
    // the ROSE and use that in the AST.
    ...
    case V_SgTypeInt: {
      // Note that this does not have to be an integer value and can be another
      // variable or "c_int" (for example) DQ (10/4/2010): Moved to new
      // (improved) design of type_kind data member in SgType.
    ...
    case V_SgTypeBool: {
      // DQ (10/4/2010): Logical types are now represented with the associated
      // kind.
    ...
    // DQ (8/14/2010): Not clear if this is a problem so output a message for
    // now to support debugging. (NOT A PROBLEM)
  }
 ...
 *      |       T_ASTERISK T_DIGIT_STRING               // Nonstandard
 * extension: source common practice
 *                                                                        //
 * e.g., COMPLEX*16
 *
...
void c_action_kind_selector(Token_t *token1, Token_t *token2,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_kind_selector(): token1 = %p = %s token2 = %p = %s "
           "hasExpression = %s \n",
  ...
  // For the case of "real*8" this is the only rule called before seeing the
  // base type ("real")
  if (token2 != NULL && atoi(token2->text) != atoi("=")) {
    // Note that token1 == "*" if token2 is valid, I think.
    // This is the case of <type>*n (e.g. "real*8").  Should we convert this to
    // an expression? astNameStack.push_front(token2);
    int value = atoi(token2->text);
    SgIntVal *integerValue = new SgIntVal(value, token2->text);
    ROSE_ASSERT(integerValue != NULL);

    // This sets the start and end source position to the beginning of the
    // number's text string)
...
void c_action_int_literal_constant(Token_t *digitString, Token_t *kindParam) {
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_int_literal_constant(): digitString = %p = %d "
           "kindParam = %p \n",
           digitString, (digitString != NULL) ? atoi(digitString->text) : -1,
           kindParam);
  ...
  string constant_text =
      digitString->text + (kindParam ? string("_") + kindParam->text : "");
  ...
  // printf ("Leaving c_action_int_literal_constant(): astExpressionStack.size()
...
void c_action_real_literal_constant(Token_t *realConstant, Token_t *kindParam) {
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_real_literal_constant(): realConstant = %p = %s "
           "kindParam = %p = %s \n",
  ...
  // preserve kind parameter if any
  string constant_text =
      realConstant->text + (kindParam ? string("_") + kindParam->text : "");
  SgValueExp *pValueExp =
      new SgFloatVal(atof(realConstant->text), constant_text);
...
void c_action_complex_literal_constant() {
  ...
  SgValueExp *complexValue =
      new SgComplexVal(realValue, imaginaryValue, realValue->get_type(), "");
...
void c_action_real_part(ofp_bool hasIntConstant, ofp_bool hasRealConstant,
                        Token_t *id) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_real_part(): hasIntConstant = %s hasRealConstant = %s "
           "id = %s \n",
           hasIntConstant ? "true" : "false",
           hasRealConstant ? "true" : "false", id != NULL ? id->text : "NULL");
}
...
void c_action_imag_part(ofp_bool hasIntConstant, ofp_bool hasRealConstant,
                        Token_t *id) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_imag_part(): hasIntConstant = %s hasRealConstant = %s "
           "id = %s \n",
           hasIntConstant ? "true" : "false",
           hasRealConstant ? "true" : "false", id != NULL ? id->text : "NULL");
}
 ...
 * T_RPAREN |       T_LPAREN type_param_value ( T_COMMA (T_KIND T_EQUALS)? expr
 * )? T_RPAREN
 *
 * @param kindOrLen1 Specifies whether the first kind or len type-param-value is
 * present
 * @param kindOrLen2 Specifies whether the second kind or len type-param-value
 * is present
...
void c_action_char_selector(Token_t *type_keyword, Token_t *optional_keyword,
                            int kindOrLen1, int kindOrLen2,
                            ofp_bool hasAsterisk) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf(
        "In c_action_char_selector(): type_keyword = %p = %s optional_keyword "
        "= %p = %s kindOrLen1 = %d kindOrLen2 = %d hasAsterisk = %s \n",
        type_keyword, type_keyword != NULL ? type_keyword->text : "NULL",
        optional_keyword,
        optional_keyword != NULL ? optional_keyword->text : "NULL", kindOrLen1,
        kindOrLen2, hasAsterisk ? "true" : "false");

  // DQ (9/27/2010): New for OFP 0.8.2 is that the c_action_type_param_value()
  // is only called for the length type parameter and not for the kind
  // parameter. It used to be called for each parameter.

  // DQ (1/25/2011): This is a great example of where we should cut loose older
  // versions of OFP to reduce the complexity of the Fortran support. DQ
  // (9/28/2010): This function used to be called twice for
  // "CHARACTER(kind=1,len=2) C" In this version R404 is now not called as well
  // (a previous OFP bug).  This changes what is on the stack and thus how it is
  // processed.
  if (kindOrLen1 == KindLenParam_none) {
    if (hasAsterisk == true) {
      // See test2007_196.f for an example of this case
      // printf ("In c_action_char_selector(): astTypeParameterStack.empty() =
      ...
      if (astTypeParameterStack.empty() == false) {
        ...
        // Not sure what to do here, I think we have to push an Asterisk onto
        // the astExpressionStack and a KindLenParam_len onto either the
        // astTypeParameterStack of the astTypeKindStack. printf ("In
        // c_action_char_selector(): R424 astTypeParameterStack.empty() ==
        // true), but hasAsterisk == true, so this is character*(*) \n");
      }
    } else {
      ROSE_ASSERT(astTypeParameterStack.empty() == true);
      ROSE_ASSERT(astTypeKindStack.empty() == true);
    ...
    if (kindOrLen1 == KindLenParam_kind) {
      // This needs to transfered from the astExpressionStack to the
      // astTypeKindStack
...
void c_action_length_selector(Token_t *len_keyword, int kindOrLen,
                              ofp_bool hasAsterisk) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_length_selector(): len_keyword = %p = %s kindOrLen = "
           "%d hasAsterisk = %s \n",
 ...
 *
 * @param hasTypeParamValue True if a type-param-value is specified, otherwise
 * is a scalar-int-literal-constant
 *
 */
 ...
 * @param id Optional identifier representing the kind parameter variable AND
 * the '_'
...
void c_action_char_literal_constant(Token_t *digitString, Token_t *id,
                                    Token_t *str) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_char_literal_constant(): digitString = %s id = %s str "
           "= %s \n",
           digitString ? digitString->text : "NULL", id ? id->text : "NULL",
           str ? str->text : "NULL");
  ...
  // Handle ethe case of a regular int (the kind parameter will allow for short,
  // and long sizes)
  string value = str->text;

  // This is the Fortran default, but we need to test this since double quotes
  // are an alterrnative
  ...
  // This set the start and end source position to the beginning of the number's
  // text string) setSourcePosition(stringValue);
...
void c_action_derived_type_def() {
  // I think this is the end of a derived type, but is not meaningful as far as
  // I can tell.
}
 ...
 */
void c_action_derived_type_stmt(Token_t *label, Token_t *keyword, Token_t *id,
                                Token_t * /*eos*/,
                                ofp_bool /*hasTypeAttrSpecList*/,
                                ofp_bool /*hasGenericNameList*/) {
  // Build a SgClassDeclaration to hold the Fortran Type (maybe it should be a
  // SgFortranType derived from a SgClassDeclaration?)
  ...
  // Pei-Hung (07/17/2019) Derived type can only be PUBLIC in the specification
  // part of a module SgAccessModifier has default as public and will cause
  // issue in test2019_accessibility_attribute.f90 Changed the access attribute
  // to unknown to avoid unparser generating "PUBLIC" attribute
  derivedTypeStatement->get_declarationModifier()
      .get_accessModifier()
      .setUndefined();
  ...
  derivedTypeStatement->get_firstNondefiningDeclaration()
      ->get_declarationModifier()
      .get_accessModifier()
      .set_modifier(derivedTypeStatement->get_declarationModifier()
                        .get_accessModifier()
                        .get_modifier());
 ...
 * @param specType "Enum"  on type: access_spec, extnds, abstrct, or bind.
 * (Weird spelling avoids overriding reserved keywords in generator code.)
 */
void c_action_type_attr_spec(Token_t *keyword, Token_t *id, int specType) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_type_attr_spec(): keyword = %p = %s id = %p = %s "
           "specType = %d \n",
  ...
  case TypeAttrSpec_extends: {
    std::cerr
        << "In c_action_type_attr_spec: TypeAttrSpec_extends not implemented"
        << std::endl;
    break;
  }
  ...
  case TypeAttrSpec_abstract: {
    std::cerr
        << "In c_action_type_attr_spec: TypeAttrSpec_abstract not implemented"
        << std::endl;
    break;
  }
  ...
  default: {
    std::cerr << "In c_action_type_attr_spec: found unknown TypeAttrSpec "
              << specType << std::endl;
    ROSE_ABORT();
  }
  }
...
void c_action_end_type_stmt(Token_t *label, Token_t *endKeyword,
                            Token_t * /*typeKeyword*/, Token_t *id,
                            Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
#if !SKIP_C_ACTION_IMPLEMENTATION
  // Pop the class definition (SgClassDefinition) used to hold Fortran derived
  // types
  ...
  ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct()->get_line() !=
  ...
  // Only checking the function return type since this is the only location
  // where a fixup can be required except for the more general type rules inside
  // of modules which are handled separately (and are a bit more complex).
...
void c_action_sequence_stmt(Token_t * /*label*/, Token_t * /*sequenceKeyword*/,
                            Token_t * /*eos*/) {
  // The sequence statement is really a type attribute, so we implement it as
  // such in ROSE.
  SgClassDefinition *classDefinition =
      isSgClassDefinition(astScopeStack.front());
...
void c_action_component_def_stmt(int type) {
  ...
  // as a procedure. So I don't know if it is of much use in building the ROSE
  // AST.
...
void c_action_data_component_def_stmt(Token_t *label, Token_t *eos,
                                      ofp_bool hasSpec) {
  // This is where we build the declaration using whatever was pushed onto the
  // stack (nameStack)
  ...
  SgVariableDeclaration *varDecl =
      isSgVariableDeclaration(DeclAttributes.getDeclaration());
  if (!varDecl) {
  ...
  ROSE_ASSERT(firstInitializedNameForSourcePosition->get_startOfConstruct() !=
              NULL);
  ROSE_ASSERT(lastInitializedNameForSourcePosition->get_startOfConstruct() !=
              NULL);
 ...
 * |  T_LEN
 *
 * @param specType  Type of spec in enum form: pointer, dimension_paren,
 * dimension_bracket, allocable, access_spec, kind, or len.
 */
void c_action_component_attr_spec(Token_t *attrKeyword, int specType) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_component_attr_spec(): attrKeyword = %p = %s specType "
           "= %d \n",
...
void c_action_component_attr_spec_list__begin() {
  // I think this is the correct assertion, but the rest of this rule is not
  // implemented!
}

void c_action_component_attr_spec_list(int count) {
  // This function R441 is similar to R504 R503-F2008 but is used for
  // declarations in types.

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_component_attr_spec_list(): count = %d (building the "
           "SgArrayType) astAttributeSpecStack.size() = %" PRIuPTR " \n",
           count, astAttributeSpecStack.size());

  // printf ("astBaseTypeStack.empty() = %s \n",astBaseTypeStack.empty() ?
  // "true" : "false");

  // DQ (12/27/2007): parameters associated with variable declaration type
  // attributes (e.g. dimention attribute) should not be used yet. But I don't
  // know how to avoid it! if (astBaseTypeStack.empty() == true ||
  // astAttributeSpecStack.empty() == false)

  // printf ("In R441 c_action_component_attr_spec_list(): calling
...
void c_action_component_decl(Token_t *id, ofp_bool hasComponentArraySpec,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_component_decl() id = %p hasComponentArraySpec = %s "
           "hasCoArraySpec = %s hasCharLength = %s hasComponentInitialization "
           "= %s \n",
  ...
  c_action_entity_decl(id, hasComponentArraySpec, hasCoArraySpec, hasCharLength,
...
void c_action_component_decl_list__begin() {
  ...
  // Need to do this here because parsing component copointer attribute calls
  // c_ation_attr_spec instead of c_action_component_attr_spec. After this
  // inconsistency is fixed, this code can be moved to
  // c_action_component_attr_spec_list, leaving out
  // DeclAttributes.setDeclAttrSpecs().
  // Each component attribute parsed will be processed directly in
  // c_action_component_attr_spec instead.
...
void c_action_component_decl_list(int count) {
  ...
  ROSE_ASSERT(
      DeclAttributes.getDeclaration()->get_file_info()->isCompilerGenerated() ==
      false);
 ...
 *
 * @param isExplicit True if this is an explicit shape spec list. false if it is
...
void c_action_deferred_shape_spec_list(int count) {
  ...
  // push the number of array spec elements on the astExpressionStack to be
  // processed later in attr_spec DIMENSION  and in entity_decl
...
void c_action_proc_component_def_stmt(Token_t * /*label*/,
                                      Token_t * /*procedureKeyword*/,
                                      Token_t * /*eos*/,
                                      ofp_bool /*hasInterface*/) {}
 ...
 * @param specType  "Enum" to specify type of spec: pointer, pass, nopass, or
 * access_spec
 */
void c_action_proc_component_attr_spec(Token_t * /*attrSpecKeyword*/,
                                       Token_t * /*id*/, int /*specType*/) {}
...
void c_action_private_components_stmt(Token_t * /*label*/,
  ...
  // The private statement (in this context) is really a type attribute, so we
  // implement it as such in ROSE.
  SgClassDefinition *classDefinition =
      isSgClassDefinition(astScopeStack.front());
...
void c_action_type_bound_procedure_part(int count,
  ...
  // printf ("In c_action_type_bound_procedure_part(): containsKeyword = %p = %s
  // hasBindingPrivateStmt = %s \n",containsKeyword,containsKeyword != NULL ?
  // containsKeyword->text : "NULL",hasBindingPrivateStmt ? "true" : "false");

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_type_bound_procedure_part(): count = %d "
           "hasBindingPrivateStmt = %s \n",
...
void c_action_derived_type_spec(Token_t *typeName,
  ...
  // If a type is being specified and it is a derived type, the name is seen
  // here. The type should have already been constructed within the AST, if it
  // has been declared, ans so we only have to find it in the declaration which
  // we can find by searching the symbol tables.  The current implementation of
  // this does not handle USE statements. printf ("In
  // c_action_derived_type_spec(): typeName = %p hasTypeParamSpecList = %s
  // \n",typeName,hasTypeParamSpecList ? "true" : "false");
  ROSE_ASSERT(typeName != NULL);
  ROSE_ASSERT(typeName->text != NULL);

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_derived_type_spec(): typeName->text = %s "
           "hasTypeParamSpecList = %s \n",
  ...
  // DQ (8/29/2010): Added support for where the referenced type has not been
  // seen yet and must be fixed up later (see R1106:
  // c_action_end_module_stmt()).
  SgType *derivedType = NULL;
  if (derivedTypeSymbol == NULL) {
    // Case of where we are referencing a type that we have not seen yet in the
    // source sequence. We will have to make up something (perhaps using
    // SgDefaultType) and then fix it up at the end of the module.
    if (SgProject::get_verbose() > 0)
      printf("Warning: type referenced has not been seen in the source "
             "sequence yet, using SgTypeDefault (will be fixed up at the end "
             "of the module processing). \n");

    // DQ (12/26/2010): Use SgTypeDefault to trigger that we don't know the type
    // and that it will have to be fixed up later (e.g. function return types
    // when the type is in the use statement defines the scope where the type is
    // declarted). DQ (9/6/2010): Fix this later! derivedType =
    // SgTypeDefault::createType(); derivedType = SgTypeInt::createType();
  ...
  // Save the type on the type stack!  We will use it in the construction of the
...
void c_action_type_param_spec_list(int count) {
  ...
  // DQ (1/24/2011): Delete an entry from the stack to avoid unset parent in
  // testGraphGeneration. This is a temp fix for test2011_26.f03 since we have
  // no full implementation for type kind handling in data member initializers.
  // This is part of an outstanding question to Craig about the OFP handling in
  // this case.
  if (astTypeParameterStack.empty() == false) {
...
void c_action_enum_def_stmt(Token_t * /*label*/, Token_t * /*enumKeyword*/,
                            Token_t * /*bindKeyword*/, Token_t * /*id*/,
                            Token_t * /*eos*/) {}
...
void c_action_array_constructor() {
  ...
  while (astExpressionStack.empty() == false &&
         isSgExprListExp(astExpressionStack.front()) != NULL) {
  ...
  if (expressionList->get_expressions().empty() == false) {
    SgAggregateInitializer *initializer =
        new SgAggregateInitializer(expressionList, NULL);
...
void c_action_ac_value() {
  ...
  // DQ (21/4/2008): Move the value from the astExpressionStack to the
  // astInitializerStack
  ROSE_ASSERT(astExpressionStack.empty() == false);
}
...
void c_action_ac_value_list__begin() {
  ...
  // DQ (1/17/2011): If we get there and there is an expression on the stack
  // then we should use it to convert the type into an array type. printf
  // ("MAYBE THIS IS WHERE THE TYPE SHOULD BE CONSTRUCTED AS A LAST RESORT
  // (should be reading the attribute stack!) \n");

  // DQ (1/17/2011): If there is an expression on the astExpressionStack then it
  // is used to convert the type in astTypeStack into an array.  See
  // test2007_94.f90 for where this is required! Note that an alternative might
  // be to check if the entry on the top of the astExpressionStack is a
  // SgExprListExp... Error: The type should have been built in R504 R503-F2008
  // c_action_entity_decl(), since this is too late to build the type. if
  // (astExpressionStack.empty() == false)
  if ((astExpressionStack.empty() == false) &&
      (isSgExprListExp(astExpressionStack.front()) != NULL)) {
    // DQ (1/22/2011): This is not required for executable statements (where the
    // astAttributeSpecStack will be empty.  See test2010_49.f90 for an example
    // (e.g. "localCount = (/ 2, 3, 5, 7 /)"). Verify that the attribute stack
    // has an entry to tell us what to do (but build an array for now!
    // ROSE_ASSERT(astAttributeSpecStack.empty() == false);
    if (astAttributeSpecStack.empty() == false) {
      // We don't know the count yet, not clear where we get that (assume 1 for
      // initial test). Note that this parameter is no longer used, so it should
      // be removed! printf ("REMOVE USE OF count PARAMETER IN
      // convertTypeOnStackToArrayType() \n");

      // Note that this is a different count than then one in the function
      ...
      // DQ (1/23/2011): Put the new type onto the astTypeStack instead of the
      // astBaseTypeStack. ROSE_ASSERT(astTypeStack.empty() == true);
      ...
      ROSE_ASSERT(astAttributeSpecStack.front() == AttrSpec_DIMENSION ||
                  astAttributeSpecStack.front() == ComponentAttrSpec_dimension);
      astAttributeSpecStack.pop_front();
    }

    // Output debugging information about saved state (stack) information.
    outputState("After converion of type to array type in R469 (list__begin) "
...
void c_action_ac_value_list(int count) {
  ...
  // DQ (1/17/2011): I now think that MAYBE this should be handling the
  // initialization instead of the R504 parser action!

  // Build a SgExprListExp and put the elements on the astInitializerStack into
  // the stack and then put the SgExprListExp on the stack. DQ (5/4/2008):
  // switching back from using astInitializerStack to using astExpressionStack.
  if (count >= 1) // DXN (02/10/2011): create an SgExprListExp when there is at
                  // least one ac value.
  {
    SgExprListExp *expressionList = new SgExprListExp();
    setSourcePosition(expressionList);
...
void c_action_ac_implied_do() {
  ...
  // DQ (9/26/2010): Within the statement: real, dimension (3) :: b =
  // (/(j+i,j=1,3)/)
  ...
  // A current problem is that OFP is not providing the ac-do-variable (this is
  // getting fixed). This similar problem happens for the io-implied-do rule
  // (both are being fixed uniformally in OFP). When this if fixed in OFP:
  //    1) What is now the doLoopVarExp, will be the first element of the
  //    object_list. 2) We can remove the doLoopVarExp from the IR and fix the
  //    unparser.

  // Output debugging information about saved state (stack) information.
  outputState("At TOP of R470 list c_action_ac_implied_do()");

  // Since we might have an increment, and that information is only available in
  // R471, we have to process the loop control information in R471 and pass it
  // on the stack to R470.
  ROSE_ASSERT(astExpressionStack.empty() == false);
  ...
  // We used a SgExprList as a container of SgExpression objects, not delete the
  // container.
  delete loopControl;
  loopControl = NULL;
  ...
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVar,lowerBound,upperBound,increment,objectList);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(variableReference,doLoopVar,lowerBound,upperBound,increment,objectList);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(variableReference,doLoopVar,lowerBound,upperBound,increment,objectList);
  SgScopeStatement *implied_do_scope = NULL; // new SgBasicBlock();
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVarExp,lowerBound,upperBound,increment,objectList);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVarExp,lowerBound,upperBound,increment,objectList,implied_do_scope);
  SgImpliedDo *impliedDo = new SgImpliedDo(doLoopVarExp, upperBound, increment,
                                           objectList, implied_do_scope);
  ...
  // DQ (4/21/2008): We want to use the astInitializerStack for initialization
  // purposes, actually the R469 ac-value will be called and it should transfer
  // the implicit do-loop from the astExpressionStack to the
  // astInitializerStack.
...
void c_action_ac_implied_do_control(ofp_bool hasStride) {
  ...
  // Pull off 2 elements from the astExpressionStack (and a third if hasStride
  // == true)
  ...
  // We have to form the implied do loop variable initialization. Note that this
  // is not a variable declaration, since if implicit none is used, the variable
  // must have already been declared.
  ...
  SgExpression *doVariableInitialization =
...
void c_action_type_declaration_stmt(Token_t *label, int numAttributes,
  ...
  // This is a variable declaration (build the SgVariableDeclaration and
  // populate it using data saved on the stack).
...
void c_action_declaration_type_spec(Token_t *udtKeyword, int type) {
  // The type value makes it clear what the type will be for the variable
  // declaration being defined in the next setof rules. We need to save this
  // information so that we can know the type of the variable declaration when
  // it is later built.

  // So we need to build the SgType and push it onto the type stack!

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R502 c_action_declaration_type_spec() udtKeyword = %p = %s type "
           "= %d \n",
  ...
  case DeclarationTypeSpec_INTRINSIC: {
    ...
    // This marks the type in a declaration to be one of the primative types,
    // not really used in ROSE. printf ("In c_action_declaration_type_spec(type
    // = %d = DeclarationTypeSpec_INTRINSIC) \n",type); printf ("Sorry, not
    // implemented:
    // c_action_declaration_type_spec(DeclarationTypeSpec_INTRINSIC) \n");
    // ROSE_ABORT();
    ...
    if (!astTypeStack.empty()) {
      ...
      // printf ("In c_action_declaration_type_spec(): astExpressionStack.size()
      // = %" PRIuPTR " \n",astExpressionStack.size());
      ...
      if (astExpressionStack.empty() == false) {
        ...
        // DQ (9/30/2007): Set the parent (if not this is caught in AST whole
        // graph generation)
        kindExpression->set_parent(typeNode);

        printf("After setting the kind, the expression stack size = %" PRIuPTR
               " \n",
               astExpressionStack.size());
        astExpressionStack.pop_front();
      }
  ...
  case DeclarationTypeSpec_TYPE: {
    ...
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("In c_action_declaration_type_spec(type = %d = "
             "DeclarationTypeSpec_TYPE) \n",
             type);
    break;
  }
  ...
  case DeclarationTypeSpec_CLASS: {
    // Rasmussen (4/11/2019):
    // This is a CLASS declaration-type-spec. It is effectively the same as
    // TYPE.
    // TODO: It would be helpful to mark the derived type as a class declaration
    // rather
    //       than a struct but I don't think one can get the variable
    //       declaration here. This is not super critical as it can be handled
    //       by token-based unparsing.
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("In c_action_declaration_type_spec(type = %d = "
             "DeclarationTypeSpec_CLASS) \n",
             type);
    break;
  }
  ...
  case DeclarationTypeSpec_unlimited: {
    printf("Sorry, not implemented: "
           "c_action_declaration_type_spec(DeclarationTypeSpec_unlimited) \n");
    ROSE_ABORT();
  }
  ...
  if (astTypeStack.empty() == false) {
    ...
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("No type found on astTypeStack (make sure there is a base type on "
             "the astBaseTypeStack) \n");
    ROSE_ASSERT(astBaseTypeStack.empty() == false);
  }
}
...
void c_action_attr_spec(Token_t *attrKeyword, int attr) {
  ...
  case AttrSpec_access: {
    // DQ (5/20/2008): This is a redundant specifier, it appears to only be used
    // with AttrSpec_PUBLIC or AttrSpec_PRIVATE
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
  ...
  case AttrSpec_DIMENSION: {
    // DQ (1/16/2011): Just as with the case of AttrSpec_POINTER, we have to
    // process this to generate an array. This should be done here so get the
    // base type onto the astBaseTypeStack as soon as possible. Alternatively,
    // we could reverse the stack entries so that we processed the
    // astAttributeSpecStack in the correct (original) order. This is only a
    // problem when there are 2 or more entries such as "real, dimension(:),
    // pointer :: h".
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
  ...
  case AttrSpec_INTENT: {
    ...
    /* TODO
     *  DeclAttributes.intent = astIntentSpecStack.back(); // see
     * c_action_intent_spec
     */
    break;
  }
...
void c_action_entity_decl(Token_t *id, ofp_bool hasArraySpec,
  ...
  // DQ (1/28/2009): We need to have the global scope and the function scope in
  // place before building variable declarations.  See test2007_09.f90.
  // initialize_global_scope_if_required(); not required!
  build_implicit_program_statement_if_required();

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    // printf ("In R504 R503-F2008 c_action_entity_decl(): save variableName =
    // %s \n",id->text);
    string current_filename = getCurrentFilename();
    printf("In R504 R503-F2008 c_action_entity_decl(): save variableName = %s "
           "file = %s hasArraySpec = %s hasCoarraySpec = %s hasCharLength = %s "
           "hasInitialization = %s \n",
...
#if !SKIP_C_ACTION_IMPLEMENTATION
  // Merge the entity attributes with the declaration attributes in order to
  // build the correct type for the entity-decl
  AttributeRec entityAttr;
  entityAttr = DeclAttributes;
  SgExpression *initialization = NULL;
  if (hasInitialization) {
    ROSE_ASSERT(
        !astExpressionStack.empty()); // astExpressionStack.front() contains
  ...
  if (hasCharLength) {
    ROSE_ASSERT(
        !astExpressionStack
             .empty()); // astExpressionStack() contains char_length expression
  ...
  if (hasInitialization) {
    if (isSgClassType(entityType)) {
      // if entityType is a derived type then the initialzer must be a
      ...
      // For example: type(c_ptr) = c_null_ptr, in which case c_null_ptr is an
      // SgVarRefExp (I believe) I removed assertion and changed logic to allow
      // initializer not to be a constructor initializer
  ...
  // This entity decl may be part of a type declaration that defines the return
  // type of a function, in which case, fix up the return type of the function
  // in question; however if this entity decl is a compoent decl then it has
  // nothing to do with the functions in enclosing scopes and so do not fix up
  // the return type of any function. If the entity_decl is inside of a
  // function, check to see if it matches the function's name; If matched, then
  // need to update the function type.
  SgFunctionDefinition *funcDef =
      isSgFunctionDefinition(currentScope->get_scope());
  bool needFuncTypeUpdate =
      functionSymbol &&
      functionSymbol->get_declaration()->get_definition() == funcDef &&
      !classDef;
  ...
  if (variableSymbol != NULL) {
    ...
    // Pei-Hung (07/19/2019) a variable inside a derived type can have the same
    // name as other varible in other scope.  If the case happens, we should
    // build the initialized name for the variable inside the derived type
    initializedName = variableSymbol->get_declaration();
    ROSE_ASSERT(initializedName != NULL);

    SgType *variableType = initializedName->get_type();

    if (isSgDerivedTypeStatement(currentScope->get_parent()) != NULL &&
        currentScope != initializedName->get_scope()) {
      initializedName =
          buildInitializedNameAndPutOntoStack(name, entityType, initializer);
    } else {
      initializedName = variableSymbol->get_declaration();
      ROSE_ASSERT(initializedName != NULL);
      ROSE_ASSERT(initializedName->get_scope() != NULL);
      SgFunctionDefinition *functionDefinition =
          isSgFunctionDefinition(initializedName->get_scope());
      if (functionDefinition != NULL) {
        SgProcedureHeaderStatement *functionDeclaration =
            isSgProcedureHeaderStatement(functionDefinition->get_parent());
        ...
        // array dimension info can be specified in common block before the type
        // specification
        ...
        if (currentScope != initializedName->get_scope())
          initializedName = buildInitializedNameAndPutOntoStack(
              name, entityType, initializer);
        else {
          // Pei-Hung (07/19/2019) In test2019_common_block_name.f90,
          // array dimension info can be specified in common block before the
          // type specification
          if (isSgArrayType(variableType) != NULL) {
            SgArrayType *arrayType = isSgArrayType(variableType);
            ROSE_ASSERT(arrayType != NULL);
    ...
    if (needFuncTypeUpdate) { // only applies to non component entities.
      ROSE_ASSERT(initializedName == NULL);
      initializedName =
          new SgInitializedName(name, entityType, initializer, NULL, NULL);
      SgProcedureHeaderStatement *functionDeclaration =
  ...
  // Now that we have the initialized name corresponding to the entiy-decl, add
  // it to the current variable declaration:
  SgVariableDeclaration *varDecl =
      isSgVariableDeclaration(DeclAttributes.getDeclaration());
  if (!varDecl) {
  ...
  // Make sure that the variable name corresponding to the initialized name is
  // in the appropriate scope with appropriate symbol table information: (see
  // buildVariableDeclaration in fortran_support.C)
  SgFunctionDefinition *functionDefinition =
      isSgFunctionDefinition(astScopeStack.front()->get_parent());
  variableSymbol = NULL;
  SgName variableName = initializedName->get_name();
  if (functionDefinition != NULL) {
    variableSymbol = functionDefinition->lookup_variable_symbol(variableName);
    if (variableSymbol != NULL) {
      // This variable symbol has already been placed into the function
      // definition's symbol table Link the SgInitializedName in the variable
      // declaration with its entry in the function parameter list.
      initializedName->set_prev_decl_item(variableSymbol->get_declaration());
      // Set the referenced type in the function parameter to be the same as
      // that in the declaration being processed.
      variableSymbol->get_declaration()->set_type(initializedName->get_type());
      // Function parameters are in the scope of the function definition (same
      // for C/C++)
      initializedName->set_scope(functionDefinition);
    }
  }
  // If not just set above then this is not a function parameter, but it could
  // have been built in a common block
  if (variableSymbol == NULL) {
...
void c_action_entity_decl_list__begin() {
  // The use of the astNameListStack has been discontinued, we just use a stack
  // of names (tokens) now! This make for a simpler implementation and I don't
  // think we require the additional complexity.

  // DQ (1/20/2011): We have to save the astAttributeSpecStack so that we can
  // use the same attribute stack for processing all variables in the
  // declaration uniformally.  See test2007_248.f90 for an example.  We will
  // also have to test this support in R441 as well (similar parser action, but
  // for variable declarations inside of types. printf ("Save the
  // astAttributeSpecStack stack to reuse it for each variable in the
  // declaration. \n");

  // OR just don't delete the attributes until we process
  // "c_action_entity_decl_list(int count)"!!!

  // printf ("Build the base type for the variable declaration. \n");

  // DQ (12/8/2007): Modified to reflect use of new astBaseTypeStack (see
  // test2007_148.f) DQ (12/7/2007): Added assertion.
...
void c_action_entity_decl_list(int count) {
  // This function R504 R503-F2008 is similar to R441 but is used for
  // declarations in NON-types.

  // Since we already have the elements in the list we don't have to do anything
  // here. In general it is a tradeoff as to if we should build the list in the
  // <rule>_list__begin() function and then add the elements to the list in the
  // element-specific functions or if we should push the elements onto the stack
  // in the element-specific functions and assemble the lists in the
  // <rule>_list() functions.  Except that we want to be consistent it is not
  // clear if there is a best answer to this question.

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R504 c_action_entity_decl_list: (number of variable "
           "declarations) count = %d \n",
           count);
...
void c_action_initialization(ofp_bool hasExpr, ofp_bool hasNullInit) {
  ...
  //    1) For a implied-do loop we need to identify the array length and push
  //    it onto the stack. 2)
  ...
  if (hasExpr == true) {
    if (astExpressionStack.empty() == false) {
      ...
      // printf ("This is the case were we should process the initializer (see
      // test2008_21.f90) \n");

      // We should have an SgVarRefExp on the astExpressionStack, but we are
      // going to ignore it since this is for initialization.  So clear the
      // astExpressionStack.  Later we might need to have saved this, but for
      // now it appears to be redundant with the type specification which we can
      // derive in the unparser.
      ...
      // This is the case for test2007_181.f90 ("integer :: n = 1"). Note that
      // ac_value() is not called here!

      // ROSE_ASSERT(astExpressionStack.empty() == false);
      // ROSE_ASSERT(isSgExprListExp(astExpressionStack.front()) == NULL);

      // DQ (1/23/2011): Added test to explain why nothing is done in this code
      // below.
      ROSE_ASSERT(astExpressionStack.empty() == true);
      ...
      if (astExpressionStack.empty() == false) {
        while (astExpressionStack.empty() == false) {
          // Transfer the IR node from the astExpressionStack to the
          // astNodeStack.
          SgExpression *initializer = astExpressionStack.front();
          astExpressionStack.pop_front();
        ...
        printf("Could not find an initializer on the stack (either the "
               "astInitializerStack nor the astExpressionStack) \n");
        ROSE_ABORT();
      }
    }
  }

  // DQ (5/4/2008): Now that we don't use the astInitializerStack, we can't
  // assert this. ROSE_ASSERT(astExpressionStack.empty() == true);
...
void c_action_null_init(Token_t *id) {
  ...
  // DQ (1/23/2011): We don't have a NULL() IR node, so I will have to add one.
  // For the moment (debugging) use a value!.
...
void c_action_access_spec(Token_t *keyword, int type) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_access_spec(): keyword = %p = %s type = %d \n", keyword,
           keyword != NULL ? keyword->text : "NULL", type);
...
/** R509
 * language_binding_spec
 *      :       T_BIND T_LPAREN T_IDENT // 'C' // (T_COMMA name T_EQUALS expr)?
 * T_RPAREN
 *
 * @param keyword The BIND keyword token.
 * @param id The identifier representing the language binding, must be 'C' or
 * 'c'.
...
void c_action_language_binding_spec(Token_t *keyword, Token_t *id,
                                    ofp_bool hasName) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_language_binding_spec(): keyword = %p = %s id = %p = "
           "%s hasName = %s \n",
           keyword, keyword != NULL ? keyword->text : "NULL", id,
...
void c_action_array_spec(int count) {
  ...
  // The stack size here is the number of types used to represent the array (a
  // single SgArrayType), and the count is the number of dimensions of the
  // array.  This could be used to set the rank, but likely the rank should not
  // be stored explicitly since it is redundant with the array dimension
  // expression list.

  // ROSE_ASSERT(astTypeStack.size() == (size_t)count);

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R510 c_action_array_spec(): count = %d (building the "
           "multi-dimensional shape for the future SgArrayType) "
           "astAttributeSpecStack.size() = %" PRIuPTR " \n",
           count, astAttributeSpecStack.size());
...
void c_action_array_spec_element(int type) {
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R510 c_action_array_spec_element(): (value pushed onto "
           "astAttributeSpecStack) type = %d \n",
           type);

  // We don't really need to push these integers onto the stack, as long as each
  // one traslates (one-to-one) into expressions on the astExpressionStack.
  // astAttributeSpecStack.push_back(type);

  // If this was a value expression (type == ArraySpecElement_expr), then it has
  // be processed an placed onto the stack. Else we have to process it here.
  ...
  if (type != ArraySpecElement_expr) {
    ...
    case ArraySpecElement_expr: {
      printf("Error: This sort of case ArraySpecElement_expr should have been "
             "processed already \n");
      ROSE_ABORT();
    }

    case ArraySpecElement_expr_colon: {
      // DQ (1/23/2009): Fix suggested by one of Craig's students (need name of
      // student). I think that example test code: test2009_03.f90 demonstrates
      // this case.
    ...
    case ArraySpecElement_expr_colon_expr: {
      ...
      // DQ (11/14/2008): A colon expression at this point can only have a base
      // and bound (no stride).
    ...
    case ArraySpecElement_expr_colon_asterisk: {
      ...
      // DQ (1/26/2009): Build a "*" and push it onto the stack so that we can
      // form the subscript expression "<expression>:*"
    ...
    case ArraySpecElement_colon: {
      // DQ (1/23/2009): Fix suggested by one of Craig's students (need name of
      // student). I think that example test code: test2009_03.f90 demonstrates
      // this case. CER (9/24/2008): I think SgSubscriptExpression is the
...
void c_action_explicit_shape_spec(ofp_bool hasUpperBound) {
  ...
  // DQ (1/13/2011): See test2011_01.f90 for an example of where this might be a
  // fix. int type = (hasUpperBound == true) ? ArraySpecElement_expr_colon_expr
  // : ArraySpecElement_expr_colon;
...
void c_action_intent_spec(Token_t *intentKeyword1, Token_t *intentKeyword2,
                          int intent) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_intent_spec() intentKeyword1 = %p = %s intentKeyword2 "
           "= %p = %s intent = %d \n",
           intentKeyword1,
...
void c_action_access_stmt(Token_t *label, Token_t *eos, ofp_bool hasList) {
  ...
  // (see test2007_147.f, the original Fortran I code from the IBM 704 Fortran
  // Manual).
  build_implicit_program_statement_if_required();

  if (DeclAttributes.getAccessType() == AttrSpec_PRIVATE) {
    buildAttributeSpecificationStatement(
        SgAttributeSpecificationStatement::e_accessStatement_private, label,
        eos);
  } else // (DeclAttributes.accessType == AttrSpec_PUBLIC)
  {
    buildAttributeSpecificationStatement(
        SgAttributeSpecificationStatement::e_accessStatement_public, label,
        eos);
  }
  DeclAttributes.reset();
}
 ...
 */
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 0
void c_action_allocatable_stmt(Token_t *label, Token_t *keyword,
                               Token_t * /*eos*/)
#else
void c_action_allocatable_stmt(Token_t *label, Token_t *keyword,
                               Token_t * /*eos*/, int count)
#endif
{
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 0
    ...
    printf("In c_action_allocatable_stmt() label = %p = %s keyword = %p = %s "
           "count = %d \n",
           label, label != NULL ? label->text : "NULL", keyword,
           keyword != NULL ? keyword->text : "NULL", count);
#endif
  }
...
void c_action_allocatable_decl(Token_t *id, ofp_bool hasArraySpec,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_allocatable_decl() id = %p = %s hasArraySpec = %s "
           "hasCoArraySpec = %s \n",
           id, id != NULL ? id->text : "NULL", hasArraySpec ? "true" : "false",
  ...
  if (hasArraySpec) {
    ...
    // Mark this variable has having had its shape deferred (not specified in
    // its declaration)
...
/** R522
 * bind_stmt
 *      :       (label)? language_binding_spec (T_COLON_COLON)? bind_entity_list
 * T_EOS
 *
 * @param label Optional statement label
 * @param eos End of statement token.
...
void c_action_bind_stmt(Token_t *label, Token_t *eos) {
  ...
  // Since the keyword is missing form the API, until it is available use the
  // eos.
  Token_t *keyword = eos;
...
void c_action_bind_entity(Token_t *entity, ofp_bool isCommonBlockName) {
  ...
  if (isCommonBlockName == true) {
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("Case of a common block (common blocks map to SgClassDeclaration "
             "IR node) \n");
    ...
    if (classSymbol == NULL) {
      SgDerivedTypeStatement *derivedTypeStatement =
          buildDerivedTypeStatementAndDefinition(name, astScopeStack.front());

      // The source position is unknown, but later we can likely find the
      // original declaration through the symbol table.
      setSourcePosition(derivedTypeStatement);
    }
  } else {
    // Handle this case using implicit typing rules
    // Push a name onto the astNameStack so that we can use c_action_data_ref to
    // convert it into a variable on the astExpressionStack.
    astNameStack.push_front(entity);

    // This takes a name off of the name stack and puts a variable reference
    // onto the astExpressionStack (building a declaration if required) types
    // are computed using implicit type rules.
    c_action_data_ref(0);
...
void c_action_bind_entity_list(int count) {
  ...
  // This is all we know since there are additional entries on the astNameStack
  // (bind_language and binding_label might be present for example) printf
  // ("astNameStack.size() = %" PRIuPTR " \n",astNameStack.size());
  ROSE_ASSERT(astNameStack.size() >= (size_t)count);
...
void c_action_data_stmt_object_list(int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    // Pei-Hung (07/16/2019) prepend variables in stack to keep the order in a
    // data block
...
void c_action_data_implied_do(Token_t *id, ofp_bool hasThirdExpr) {
  ...
  // There should be a base, bound and optional increment, so either 2 or 3
  // entries
  ROSE_ASSERT(numberOfImplicitDoLoopControlValues == 2 ||
              numberOfImplicitDoLoopControlValues == 3);
  ...
  trace_back_through_parent_scopes_lookup_variable_symbol_but_do_not_build_variable(
      doLoopVarName, currentScope, variableSymbol, functionSymbol, classSymbol);
  ...
  // SgImpliedDo(doLoopVar,lowerBound,upperBound,increment,objectList);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVar,lowerBound,upperBound,increment,objectList,implied_do_scope);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVar,upperBound,increment,objectList,implied_do_scope);
  SgImpliedDo *impliedDo =
      new SgImpliedDo(doLoopVariableWithInitialization, upperBound, increment,
                      objectList, implied_do_scope);
  setSourcePosition(impliedDo);
...
void c_action_data_i_do_object_list(int count) {
  ...
  // The code implemented here is different from the code in R918, because we
  // have a
...
void c_action_data_stmt_value(Token_t *asterisk) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_data_stmt_value(): asterisk = %p = %s \n", asterisk,
           asterisk != NULL ? asterisk->text : "NULL");
  ...
  if (astExpressionStack.size() == 0) {
    ...
    if (astExpressionStack.size() == 1) {
      if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL) {
        printf("Case of SgDataStatementValue::e_explict_list (expression, "
               "typically a simple value)\n");
      }
      ...
      if (astExpressionStack.size() == 2) {
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL) {
          printf("Case of SgDataStatementValue::e_implicit_list (repeat * "
                 "value)\n");
        }
        dataValue->set_data_initialization_format(
        ...
        printf("This might be an implied do loop (need an example to debug "
               "this case!)\n");
        ROSE_ABORT();
      }
    }
  ...
  default: {
    printf("Error: default reached dataValue->get_data_initialization_format() "
           "= %d \n",
 ...
 */
void c_action_dimension_stmt(Token_t *label, Token_t *keyword,
                             Token_t * /*eos*/, int count) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  // This statement's semantics is that it should change the type of the
  // declared variables
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_dimensionStatement, label, keyword);
}
...
#else
void c_action_dimension_decl(Token_t *id, ofp_bool hasArraySpec,
                             ofp_bool hasCoArraySpec)
#endif
{
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 0
    // printf ("In R544-F2008 (extracted from R535-F2003)
    // c_action_dimension_decl(): id = %p = %s lbracket = %p = %s rbracket = %p
    // = %s \n",
    //          id,id != NULL ? id->text : "NULL",lbracket,lbracket != NULL ?
    //          lbracket->text : "NULL",rbracket,rbracket != NULL ?
    //          rbracket->text : "NULL");
    printf("In R544-F2008 (extracted from R535-F2003) "
           "c_action_dimension_decl(): id = %p = %s \n",
           id, id != NULL ? id->text : "NULL");
#else
    printf(
        "In R544-F2008 (extracted from R535-F2003) c_action_dimension_decl(): "
        "id = %p = %s hasArraySpec = %s hasCoArraySpec = %s \n",
        id, id != NULL ? id->text : "NULL", hasArraySpec ? "true" : "false",
        hasCoArraySpec ? "true" : "false");
#endif
  }
  ...
  // printf ("We need to turn the named array into an array declaration at this
  // point (variable = %s) \n",id->text);

  SgName arrayName = id->text;

  // DQ (1/18/2011): This detects where we have used the semantics of implicitly
  // building symbols for implicit variables. printf ("WARNING: This use of
  // trace_back_through_parent_scopes_lookup_variable_symbol() used the side
  // effect of building a symbol if the reference is not found! \n");
  // ROSE_ABORT();
  ...
  // Now we have a scalar variable declaration, and we can process the type into
  // an array type.
  ...
  SgTreeCopy copyHelp;
  SgNode *expressionListNode =
      arrayVariableType->get_dim_info()->copy(copyHelp);
  ...
  SgPntrArrRefExp *arrayRefExpression =
      new SgPntrArrRefExp(variableReference, expressionList, NULL);
...
void c_action_intent_stmt(Token_t *label, Token_t *keyword, Token_t * /*eos*/) {
  ...
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_intentStatement, label, keyword);
}
 ...
 * @param eos End of statement token.
 */
void c_action_optional_stmt(Token_t *label, Token_t *keyword,
                            Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
 ...
 * @param eos End of statement token.
 */
void c_action_parameter_stmt(Token_t *label, Token_t *keyword,
                             Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_parameterStatement, label, keyword);
}
...
void c_action_named_constant_def(Token_t *id) {
  ...
  // An AttributeSpecification statement can be the first statement in a program
  // (see test2007_213.f, if so then this can be one of the first c_action
  // statements called.
  build_implicit_program_statement_if_required();
  ...
  SgAssignOp *assignmentExpression =
      new SgAssignOp(variableReference, rhs, NULL);
  ROSE_ASSERT(assignmentExpression != NULL);

  // DQ (1/23/2008): Set the source position to the only availalbe reference
  // point! setSourcePosition(assignmentExpression);
  setSourcePosition(assignmentExpression, id);

  // After we push this we have a pair of expressions on the stack (we could
  // process it here of later).
  astExpressionStack.push_front(assignmentExpression);

  // Build the expression "id" = "current expression on astExpressionStack,
  // attach it to the SgAttributeSpecificationStatement
}
 ...
 * @param eos End of statement token.
 */
void c_action_pointer_stmt(Token_t *label, Token_t *keyword,
                           Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  ROSE_ASSERT(keyword != NULL);
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_pointerStatement, label, keyword);
}
...
void c_action_pointer_decl(Token_t *id, ofp_bool hasSpecList) {
  // I think this is part of the Cray Fortran pointer support (or it uses the
  // same rule).

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_pointer_decl(): id = %p = %s hasSpecList = %s \n", id,
           id != NULL ? id->text : "NULL", hasSpecList ? "true" : "false");
 ...
 */
void c_action_cray_pointer_stmt(Token_t * /*labelTok*/,
                                Token_t * /*pointerTok*/,
                                Token_t * /*eosTok*/) {}
...
void c_action_cray_pointer_assoc(Token_t *crayPtrId, Token_t *targetId) {
  ...
  SgVariableDeclaration *pCrayVarDecl = SageBuilder::buildVariableDeclaration(
 ...
 * @param eos End of statement token.
 */
void c_action_protected_stmt(Token_t *label, Token_t *keyword,
                             Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  ROSE_ASSERT(keyword != NULL);
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_protectedStatement, label, keyword);
...
#endif
{
  ...
  ROSE_ASSERT(keyword != NULL);
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_targetStatement, label, keyword);
}
...
void c_action_target_decl(Token_t *id, ofp_bool hasArraySpec,
                          ofp_bool hasCoArraySpec) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_target_decl(): id = %p = %s hasArraySpec = %s "
           "hasCoArraySpec = %s \n",
           id, id != NULL ? id->text : "NULL", hasArraySpec ? "true" : "false",
           hasCoArraySpec ? "true" : "false");
 ...
 * @param eos End of statement token.
 */
void c_action_volatile_stmt(Token_t *label, Token_t *keyword,
                            Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  ROSE_ASSERT(keyword != NULL);
  buildAttributeSpecificationStatement(
      SgAttributeSpecificationStatement::e_volatileStatement, label, keyword);
}
...
void c_action_implicit_stmt(Token_t *label, Token_t *implicitKeyword,
                            Token_t * /*noneKeyword*/, Token_t * /*eos*/,
                            ofp_bool hasImplicitSpecList) {
  ...
  SgImplicitStatement *implicitStatement =
      new SgImplicitStatement(isImplicitNone);

  // There is nothing to use to set the position of the SgImplicitStatement
  // Send email about this interesting example where no source position
  // information is available (once I get my email back up!).
  ...
  // Output debugging information about saved state (stack) information.
  outputState("Before clearing stack for unimplemented implicit statement in "
              "R549 c_action_implicit_stmt()");

  // DQ (12/2/2010): Now that we have cleaned up the stack we can proceed (but
  // check the stack).
  ROSE_ASSERT(astBaseTypeStack.empty() == true);

  if (astNodeStack.empty() == false) {
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("WARNING: unfinished implicit statement (for not implicit none "
             "case) base types being cleared from stack! (implicit) \n");
    SgInitializedNamePtrList tempStack;
    while (astNodeStack.empty() == false) {
      SgInitializedName *implicitTypeName =
          isSgInitializedName(astNodeStack.front());
    ...
    while (tempStack.empty() == false) {
      SgInitializedName *implicitTypeName =
          isSgInitializedName(tempStack.back());
      ROSE_ASSERT(implicitTypeName != NULL);
...
void c_action_letter_spec(Token_t *id1, Token_t *id2) {
  ...
  // We will use a SgInitializedName to bind a type with a name (using
  // implicitCharacerRangePtr as the name).
  SgInitializedName *implicitTypeName =
      SageBuilder::buildInitializedName(implicitCharacerRange, implicitType);
  ROSE_ASSERT(implicitTypeName != NULL);

  // SgInitializedName IR nodes carry a scope which should always be
  // initialized.
...
void c_action_letter_spec_list(int count) {
  ...
  // The base type can be used across multiple implicitCharacerRange, so we need
  // only clean it up once.
  ROSE_ASSERT(astBaseTypeStack.empty() == false);
  astBaseTypeStack.pop_front();
 ...
 */
void c_action_namelist_stmt(Token_t * /*label*/, Token_t *keyword,
                            Token_t * /*eos*/, int count) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
void c_action_namelist_group_object_list(int count) {
  ...
  // Gather the names off the name stack and use the last name as the group name
  // and build a SgNameGroup object and put it onto the astNodeStack.
 ...
 */
void c_action_common_stmt(Token_t * /*label*/, Token_t *commonKeyword,
                          Token_t * /*eos*/, int numBlocks) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  while (astNodeStack.empty() == false) {
    SgCommonBlockObject *commonBlockObject =
        isSgCommonBlockObject(astNodeStack.front());
...
void c_action_common_block_name(Token_t *id) {
  ...
  // DQ (12/14/2007): Make sure that we have added the implicit program function
  // to the scope (see test2007_161.f90)
  build_implicit_program_statement_if_required();
...
void c_action_common_block_object_list(int count) {
  ...
  // Liao 12/9/2010
  // We have to do this after moving SgCommonBlockObject to be under
  // SgLocatedNodeSupport
  // TODO: ask Dan to fill in real file info for both SgCommonBlockObject and
  // the contained varRefList.
...
void c_action_common_block_object(Token_t *id, ofp_bool hasShapeSpecList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_common_block_object(): id = %p = %s hasShapeSpecList = "
           "%s \n",
           id, id != NULL ? id->text : "NULL",
  ...
  if (variableSymbol != NULL) {
    if (hasShapeSpecList == true) {
      // There must be a scalar variable declaration for this symbol because we
      // found a symbol and there is a shape-spec-list (the array specification
      // can't be in two places).  Thus the common-block-object acts like a
      // dimension statement and the variable type should be converted into an
      // array type (see c_action_dimension_decl) [Rasmussen, 2019.06.19].
...
void c_action_variable() {
  // I think that this just defines the previously built SgVarRefExp as an
  // l-value and that the creation of the SgVarRefExp should be moved to R612.
 ...
 *      |       array-section              // R617 is data-ref [
 *(substring-range) ] |       structure-component      // R614 is data-ref |
 *substring
 *
 *@param hasSubstringRange True if substring-range is present.
 */
void c_action_designator(ofp_bool hasSubstringRange) {
  // This is a part of a variable reference (any likely used many other places
  // as well)
  ...
  if (hasSubstringRange == true) {
    ...
    ROSE_ASSERT(astExpressionStack.empty() == false);
    SgSubscriptExpression *subscriptRange =
        isSgSubscriptExpression(astExpressionStack.front());
    ...
    if (isSgPntrArrRefExp(simpleExpression)) {
      ...
      // (arrayReference,subscriptRange,arrayReference->get_type());
...
void c_action_designator_or_func_ref() {
  // DQ (5/4/2008): I am unclear what to do with this rule! (called by
  // test2008_22.f90)
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
 ...
 * T_RPAREN, so required T_LPAREN substring_range T_RPAREN made optional ERR_CHK
 * 609 ensure final () is (substring-range)
 *
 * @param hasSubstringRange True if substring-range is present, otherwise it
 * must be extracted from the data-ref.
 */
 ...
 *
 * @param hasLowerBound True if lower bound is present in a substring-range
 * (lower_bound:upper_bound).
 * @param hasUpperBound True if upper bound is present in a substring-range
 * (lower_bound:upper_bound).
 */
void c_action_substring_range(ofp_bool hasLowerBound, ofp_bool hasUpperBound) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R611 c_action_substring_range() hasLowerBound = %s "
           "hasUpperBound = %s \n",
           hasLowerBound ? "true" : "false", hasUpperBound ? "true" : "false");

  SgExpression *subscriptRange = buildSubscriptExpression(
      hasLowerBound, hasUpperBound, /* hasStride = */ false,
      /* isAmbiguous = */ false);
  ROSE_ASSERT(subscriptRange != NULL);
...
void c_action_data_ref(int numPartRef) {
  ...
  // DQ (12/29/2010): Form the full name as a vector of strcutures specific to
  // suporting R612 and R613.
  std::vector<MultipartReferenceType> qualifiedNameList(numPartRef);

  // Iterate backwards from the front of the list for the first numPartRef
  // entries.
  ...
  // Get the token (and the name in side of the token) previously saved on the
  // stack
  ...
  SgVariableSymbol *variableSymbol =
      variableSymbolList.empty() ? NULL
                                 : isSgVariableSymbol(variableSymbolList[0]);

  if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL) {
    printf("After "
           "trace_back_through_parent_scopes_lookup_member_variable_symbol(%s,"
           "astScopeStack) variableSymbol = %p = %s \n",
           variableName.str(), variableSymbol,
           variableSymbol ? SageInterface::get_name(variableSymbol).c_str()
                          : "NULL");
    cout << "variableSymbolList.size() = " << variableSymbolList.size() << endl;
  }

  // We should have found no more symbols than numPartRef (.e.g if this is an
  // implicit variable then variableSymbolList would be empty.
  ROSE_ASSERT(variableSymbolList.size() <= (size_t)numPartRef);

  // DQ (12/29/2010): Moved outside of conditional so that it could be tested
  // afteward.
  SgFunctionSymbol *functionSymbol = NULL;

  if (variableSymbol == NULL) {
    // And there was no list of variables returned...(see test2010_176.f90 for
    // an example that fails this test; return value initialization for derived
    // type). ROSE_ASSERT(variableSymbolList.empty() == true);

    // This is a function call and the 'coExpr' detected above, if any, will be
    // ignored in this routine. Preserve it for future use in
    // 'c_action_rice_spawn_stmt'.
    // TODO: check that coExpr is present here only if in the context of a Rice
    // CAF2 'spawn' statement.
    if (qualifiedNameList[0].hasImageSelector) {
      rice_dataref_coexpr = isSgCAFCoExpression(astExpressionStack.front());
      astExpressionStack.pop_front();
      qualifiedNameList[0].hasImageSelector =
          false; // avoid processing the image selector twice
    }

    // DQ (12/21/2010): For the variable to not have existed, this must be the
    // simple case of a reference with a single part. (see test2010_176.f90 for
    // an example that fails this test; return value initialization for derived
    // type).
    ROSE_ASSERT(numPartRef <= 1 ||
                ((variableSymbolList.empty() == false) &&
                 (isSgFunctionSymbol(variableSymbolList[0]) != NULL)));

    // This is a variable that has not been previously declared (Fortran allows
    // this inplicit declaration), but first check to make sure it is not an
    // implicit function.
    bool isAnIntrinsicFunction =
        matchAgainstIntrinsicFunctionList(nameToken->text);

    if ((variableSymbolList.empty() == false) &&
        (isSgFunctionSymbol(variableSymbolList[0]) != NULL)) {
      // This is the case of an initialization of a function return type for a
      // derived type (see test2010_176.f90 for an example). functionSymbol =
      // isSgFunctionSymbol(variableSymbolList[0]); ROSE_ASSERT(functionSymbol
      // != NULL);
      ...
      // DQ (12/29/2010): I think I can assert this...works for all our basic
      // test codes in Fortran_tests/*.f,f90,f03...
      ROSE_ASSERT(numPartRef == 1);

      // Note that this function call would not make sense if numPartRef > 1
      // since the input scope would be wrong.
      functionSymbol = trace_back_through_parent_scopes_lookup_function_symbol(
          variableName, getTopOfScopeStack());
    }
    // DQ (4/29/2008): Added support for detecting derived types
    SgClassSymbol *classSymbol = NULL;
    if (numPartRef == 1) {
      // This function call only makes since if numPartRef == 1 since only then
      // is the scope reasonable.
      classSymbol = trace_back_through_parent_scopes_lookup_derived_type_symbol(
          variableName, getTopOfScopeStack());
    }

    bool isANonIntrinsicFunction =
        (functionSymbol != NULL); // DXN: && !isAnIntrinsicFunction
    ...
    if ((isAnIntrinsicFunction == true) || (isANonIntrinsicFunction == true)) {
      // DQ (4/29/2008): If it is to be handled as a function then is must not
      // be a derived type.
      ROSE_ASSERT(isADerivedType == false);
      if (isAnIntrinsicFunction == true) {
        // This is a function call and not an array reference (this case is an
        // implicit function).
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
          printf("Found an implicit function call: not implemented yet! name = "
                 "%s \n",
                 nameToken->text);

        SgName functionName = nameToken->text;

        // DQ (11/26/2010): Fixing bug as represented in test2010_111.f90 (use
        // of implicit function name as a variable). If this is an implicit
        // function that has function arguements, then we should see an
        // expression list on the astExpressionStack.  What other information
        // could be used to trigger this?  We would like to not have behavior
        // within the ROFP/ROE translation that would be dependent on our parser
        // state (stack sizes).
        if (astExpressionStack.empty() == true) {
          bool isAnImplicitNoneScope = isImplicitNoneScope();
          if (isAnImplicitNoneScope == false) {
            // This is an implicitly defined variable declaration that matches
            // the name of an implicit function (so requires some special
            // handling).
            buildImplicitVariableDeclaration(variableName);
            ...
            // This does not build a variable, but it does build a
            // SgVariableSymbol. printf ("Building a SgVariableSymbol, though
            // not building a SgVarRefExp \n");
            trace_back_through_parent_scopes_lookup_variable_symbol_but_do_not_build_variable(
                variableName, currentScope, variableSymbol, functionSymbol,
                classSymbol);
            ROSE_ASSERT(variableSymbol != NULL);

            // Add the new variableSymbol to the list so that the processing of
            // the list can be properly triggered.
            variableSymbolList.push_back(variableSymbol);
            if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
              cout << "Case ExpressionStack is empty and no implicit none: "
                      "after buildImplicitVariableDeclaration, "
                      "variableSymbolList.size() = "
                   << variableSymbolList.size() << endl;
          } else {
            // This the case of calling the implicit function without the "()"
            // as in "call random_seed" instead of "call random_seed()" (See
            // test2010_169.f90). printf ("Note: This the case of calling the
            // implicit function without the \"()\" as in \"call random_seed\"
            // instead of \"call random_seed()\"\n");

            // DQ (12/29/2010): Fixed to use the new return type and push the
            // function symbol. generateFunctionCall(nameToken);
            functionSymbol = generateFunctionCall(nameToken);
            variableSymbolList.push_back(functionSymbol);
            if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
              cout << "Case ExpressionStack is empty and with implicit none: "
                      "after generateFunctionCall, variableSymbolList.size() = "
                   << variableSymbolList.size() << endl;
            // printf ("Test #1 variableSymbolList.size() = %" PRIuPTR "
            // \n",variableSymbolList.size());

            // printf ("Error: This is an implicit variable that has a name
            // matching an implicit function, but isImplicitNoneScope() == true
            // (so this is an inconsistancy).\n"); ROSE_ABORT();
          }
          // ROSE_ABORT();
        } else {
          // DQ (12/29/2010): Fixed to use the new return type and push the
          // function symbol. DQ (12/29/2010): This is the case in evaluating an
          // implicit function call for the first time (see test2007_57.f90 for
          // function "sign"). generateFunctionCall(nameToken);
          functionSymbol = generateFunctionCall(nameToken);

          // variableSymbolList.push_back(functionSymbol);
          if (find(variableSymbolList.begin(), variableSymbolList.end(),
                   functionSymbol) == variableSymbolList.end()) {
            // printf ("Adding functionSymbol to variableSymbolList since it was
            // not present. \n");
            variableSymbolList.push_back(functionSymbol);
          }
          if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
            cout << "Case ExpressionStack is not empty: after "
                    "generateFunctionCall, variableSymbolList.size() = "
                 << variableSymbolList.size() << endl;
        ...
        if (isANonIntrinsicFunction == true) {
          // This is a function call and not an array reference (this case is
          // NOT an implicit function).
          ROSE_ASSERT(functionSymbol != NULL);
          SgFunctionRefExp *functionRefExp =
              new SgFunctionRefExp(functionSymbol, NULL);
          setSourcePosition(functionRefExp, nameToken);
          // Original code!
          if (astExpressionStack.empty() == true) {
            // This should be the SgFunctionSymbol for the current function
            // where the return value is set (fortran syntax is <function-name>
            // = <expression>)

            // DQ (1/20/2008): If this is associated with a function call
            // lacking the "()" syntax then the expressionStack will be empty.
            // printf ("Skipping tests of functionDeclarationFoundViaScopes vs.
            // functionDeclarationFoundViaSymbol \n");
            // ROSE_ASSERT(functionDeclarationFoundViaScopes->get_definingDeclaration()
            // == functionDeclarationFoundViaSymbol->get_definingDeclaration());
            // ROSE_ASSERT(functionDeclarationFoundViaScopes->get_firstNondefiningDeclaration()
            // ==
            // functionDeclarationFoundViaSymbol->get_firstNondefiningDeclaration());

            // Push the function reference expression onto the stack (this is
            // going to be strange to C/C++ analysis) Fortran pretends that the
            // function reference is assigned to as part of the return.  It is
            // unclear if we should handle this is any specific uniform way
            // within the AST.  For now I will reproduce what the Fortran does.
            // But we might want to generate a "return <expression>" statement
            // instead so that the analysis between C/C++ and Fortran will be
            // more uniform. Then we could handle the syntax difference within
            // the code generation (unparser).
            ...
            if (functionArguments != NULL) {
              SgFunctionCallExp *functionCallExp = new SgFunctionCallExp(
                  functionRefExp, functionArguments, NULL);
          ...
          // DQ (12/29/2010): Add to the variableSymbolList to try to enforce
          // one-to-one mapping to input number of parts in multi-part
          // references.
          ROSE_ASSERT(functionSymbol != NULL);
          // variableSymbolList.push_back(functionSymbol);
          if (find(variableSymbolList.begin(), variableSymbolList.end(),
          ...
          // printf ("Found an function call (but NOT an implicit function): not
          // implemented yet! \n"); ROSE_ABORT();
        }
      }
    } else {
      // This branch is the case that both isAnIntrinsicFunction and
      // isANonIntrinsicFunction are false.
      ROSE_ASSERT((isAnIntrinsicFunction == false) &&
                  (isANonIntrinsicFunction == false));
      ...
      if (isADerivedType == true) {
        ...
        // DQ (5/14/2008): The top of astExpressionStack should be a
        // SgExprListExp IR node
        ...
        SgMemberFunctionDeclaration *memberfuncdecl =
            new SgMemberFunctionDeclaration(functionName, functionType, NULL);
        ...
        SgConstructorInitializer *constructorInitializer =
            new SgConstructorInitializer(memberfuncdecl, exprList, classType,
        ...
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
          printf("WARNING: Still need to add SgMemberFunctionSymbol to "
                 "typeDeclarationScope = %p \n",
        ...
        // DQ (1/18/2011): In general everything is a function unless there is a
        // hint that it is an array. DQ (1/17/2011): Check if this is supposed
        // to be a function or a variable.
        ...
        if (isSgVariableSymbol(symbol) == NULL) {
          if (isSgFunctionSymbol(symbol) != NULL) {
            ...
            // If this is not clearly a variable and it has parameters then it
            // is to be interpreted to be a function.
            ...
            if (hasSelectionSubscriptList == true) {
              ...
              // No parameter list, so this is variable (neither a function nor
              // an array).
    ...
    // These don't work because the SageInterface versions fail to handle
    // SgAlaisSymbols and non-SgVariableSymbols correctly. SgVariableSymbol*
    // tmp_variableSymbol =
    // SageInterface::lookupVariableSymbolInParentScopes("x",astScopeStack.front());
    // ROSE_ASSERT(tmp_variableSymbol != NULL);

    // SgVariableSymbol* local_variableSymbol =
    // SageInterface::lookupVariableSymbolInParentScopes(initializedName->get_name(),astScopeStack.front());
    // ROSE_ASSERT(local_variableSymbol != NULL);
    ROSE_ASSERT(initializedName->get_scope()->get_symbol_table() != NULL);
    ROSE_ASSERT(initializedName->get_scope()->lookup_variable_symbol(
                    initializedName->get_name()) != NULL);
    ROSE_ASSERT(initializedName->get_scope()->get_symbol_table()->find(
                    initializedName) != NULL);

    ROSE_ASSERT(initializedName->get_symbol_from_symbol_table() != NULL);

    // Pei-Hung (06/20/2019)  Take care of statement funciton case (see
    // test2007_179.f90) The previous code in R613 (c_action_part_ref) is moved
    // here to handle statement function Previously a symbol is removed from
    // symbol table for statement function, but will cause issue in other test
    // (see test2019_derived_type_name_conflict.f90) when a variable has the
    // same name as a member variable in a derived type. If a variable symbol
    // with selectSubScriptList is available but it is not an array or a
    // function, then we treat it as a statement function. The symbol is removed
    // from the symbol table, and generateFunctionCall is called to create a new
    // function symbol for the statement funciton support.

    bool hasSelectionSubscriptList =
        qualifiedNameList[0].hasSelectionSubscriptList;
    if (hasSelectionSubscriptList == true) {
      SgType *variableType = initializedName->get_type();
      ...
      // DQ (8/21/2010): Added support for string type so we have to eliminate
      // SgTypeString as a posability before we conclude that we should build a
      // function.
      ...
      // If this is either an array or a string type or a function type, don't
      // convert it to a function. if (arrayType != NULL || stringType != NULL)
      if (arrayType != NULL || stringType != NULL || functionType != NULL) {
        if (arrayType != NULL) {
          if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
            printf("This is an array type so it is OK for it to be indexed \n");

          // DQ (12/14/2010): Removed the support for recursive handling of R612
          // by NOT pushing the class type's scope onto the astScopeStack.
        }

        // DQ (1/24/2011): Added support for procedure pointer variables.
        if (functionType != NULL) {
          if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
            printf("This is an function type so it is OK for it to be called "
                   "with parameters (procedure pointer variable) \n");
        }
      } else {
        // This case is visited in the handling of Fortran statement functions
        // (see test2007_179.f90). See also test2011_30.f90 for procedure
        // pointers when used to call the functions to which they are pointed.
        if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
          printf("This is NOT an array type so it must be converted to a "
                 "function call with argument (if arguments are required)\n");
        ROSE_ASSERT(matchingName(initializedName->get_name().str(),
                                 nameToken->text) == true);
        // Remove the associated variable symbol so this will not be confused
        // later
        ...
        // Pei-Hung In the case that variable name is same as the derived type
        // member, we should not remove the symbol
        //  Note that we might want to clean up more than just removing the
  ...
  // DQ (12/29/2010): This fails for test2007_57.f90 (but after a lot of work we
  // can enforce this uniromally for all of out simple test codes at least).
  // This is important since now we can use information about the index number
  // of the symbol in variableSymbolList to map back to the
  // hasSelectionSubscriptList data field in the vector of
  // MultipartReferenceType object build in R612 for each part of the multi-part
  // reference. printf ("numPartRef = %d variableSymbolList.size() = %" PRIuPTR
  // " \n",numPartRef,variableSymbolList.size());
  ROSE_ASSERT((size_t)numPartRef == variableSymbolList.size());

  // Build a list of intermedate expressions so that we don't disturb the
  // astExpressionStack.
  ...
  for (size_t i = 0; i < variableSymbolList.size(); i++) {
    ...
    // DQ (12/27/2010): Handle case of function return value initialization for
    // derived type (see test2010_176.f90).
    if (isSgVariableSymbol(tempSymbol) != NULL) {
      // This is the typical case.
    } else {
      // DQ (12/27/2010): We have a function reference expresion on the stack,
      // as part of building a function but this is not to be a function call...
      ROSE_ASSERT(astExpressionStack.empty() == false);
    }
    ...
    // Rice CAF: handle a possible copointer dereference, but only for a leading
    ...
    case V_SgTypeString: {
      // DQ (12/29/2010): A huge amount of work has gone into supporting a clean
      // way to have this information from R613 be available in R613. Many
      // issues have been forced to be made uniform and rewritten internally.
      // This support could not have been moved to R613 for reasons made more
      // clear in the commencts at the top of R613.
      if (hasSelectionSubscriptList == true) {
        SgExpression *arrayVariable = variable;

        // THIS IS A COPY OF THE CODE IN CASE V_SgArrayType

        // See test2007_36.f90 for an example of where this can be false (data
        // statement) ROSE_ASSERT(astExpressionStack.empty() == false);
        if (astExpressionStack.empty() == false) {
          SgExpression *indexExpression = astExpressionStack.front();
          // printf ("#################### variable name = %s
          // indexExpression->get_type() = %s stackHoldsAnIndexExpression = %s
          // \n",variableSymbol->get_name().str(),indexExpression->get_type()->class_name().c_str(),stackHoldsAnIndexExpression
          // ? "true" : "false");
          ...
          if (arrayType == NULL && stackHoldsAnIndexExpression == true) {
            astExpressionStack.pop_front();
            variable =
                new SgPntrArrRefExp(arrayVariable, indexExpression, NULL);
          } else {
            // This is the case of an array expression (array language
            // expression)
    ...
    case V_SgFunctionType: {
      ...
      if (functionSymbol != NULL) {
        ...
        SgFunctionType *functionType =
            isSgFunctionType(functionSymbol->get_declaration()->get_type());
        ROSE_ASSERT(functionType != NULL);
        // Pei-Hung (06/28/19) The following code only generates an unused AST
        // node comment them out to clean up the AST. SgExpression*
        // functionReference = new SgFunctionRefExp(functionSymbol,
        // functionType); ROSE_ASSERT(functionReference != NULL);
        ...
        // DQ (1/24/2011): Added case to support variables with SgFunctionType
        // to support procedure pointers. See test2011_28.f90 for an example.
        ...
        if (stackHoldsAnIndexExpression == true) {
          ...
          // DQ (1/24/2011): Default name for procedure to use as declaration
          // for symbol needed for function reference...(where required)...
          ...
          ROSE_ASSERT(functionType != NULL);
          SgExpression *functionReference =
              new SgFunctionRefExp(functionSymbol, functionType);
          ...
          SgExprListExp *argumentList =
              isSgExprListExp(astExpressionStack.front());
          ROSE_ASSERT(argumentList != NULL);
          astExpressionStack.pop_front();
          SgFunctionCallExp *functionCall = new SgFunctionCallExp(
              functionReference, argumentList, SgTypeVoid::createType());
  ...
  // Transfer the expressions from the intermediateExpresionList to the
  // astExpressionStack printf ("In R612: intermediateExpresionList.size() = %"
  ...
  for (int i = 0; i < numPartRef; i++) {
    ...
    // DQ (12/29/2010): These should be kept in sync (or at least there shuld be
    // one astNameStack entry for each astHasSelectionSubscriptStack entry.  But
    ...
    // DQ (12/29/2010): Alternative structure to using astNameStack and
    // astHasSelectionSubscriptStack.
  ...
  for (int i = 1; i < numPartRef; i++) {
    ...
    // printf ("Check the type of the LHS and if it is an array then assume
    // there is an index expression on the astExpressionStack! \n");
    ROSE_ASSERT(lhs != NULL);
    ROSE_ASSERT(lhs->get_type() != NULL);
    SgArrayType *arrayType = isSgArrayType(lhs->get_type());
    if (arrayType != NULL) {
      // printf ("Need to factor in indexing expression on stack to this array
      // expression \n");
      ROSE_ASSERT(astExpressionStack.empty() == false);

      // Note that the index expression is under the list of expressions given
      // by the size == numPartRef!!! Maybe this should be handled in R613 more
      // directly!!!
    ...
    // printf ("Check the type of the RHS and if it is an array then assume
    // there is an index expression on the astExpressionStack! \n");
    ...
    // DQ (2/26/2013): The source position was just marked with default values
    // and compiler generated is not required here. Note that the SageInterface
    // support in now used for default source position marking.
    // setSourcePositionCompilerGenerated(recordReference);
    // lhs->get_startOfConstruct()->display("Set position of recordReference
    // using lhs: debug");
 ...
 * @param hasSelectionSubscriptList True if a selection-subscript-list is
 * present
...
void c_action_part_ref(Token_t *id, ofp_bool hasSelectionSubscriptList,
  ...
  // This rule works with R612 to support references to names that have multiple
  // parts (e.g. struct%field, etc.) However, it is difficult to support but
  // R612 and R613, so a lot of work has gone into these functions. This
  // function (R613) does not modify state (except for the handling of a
  // statement function where any previous symbol of that name is removed in
  // preparation for R612 to build a proper statement function IR node (as I
  // recall).  Mostly this function pushes the name onto the astNameStack, but
  // it is a problem that the value of hasSelectionSubscriptList is lost and not
  // saved since it is needed to correctly handle the interpretation of the
  // names and know which are associated with index expression (SgExprList IR
  // nodes already saved onto the stack.  So we add another auxilery stack to
  // save the information about the value of the hasSelectionSubscriptList.

  // A possible alternative to saving the value of the hasSelectionSubscriptList
  // might be to do more processing in this R613 rule. However, the order of
  // calls to R613 does not permit a stack to be used to save the scope to which
  // a named reference can be interpreted.  All names must be evaluated once the
  // full list of multiple parts are available (i.e. in R612).  I don't think
  // there is any option here.  so most processing is done in R612 and very
  // little can be done in R612.

  // DQ (12/14/2007): Make sure that we have added the implicit program function
  // to the scope (see test2007_17.f90)

  // Pei-Hung (11/15/2019) implicit program should be built at statement, not
  // expression build_implicit_program_statement_if_required();

  ROSE_ASSERT(id != NULL);

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R613 c_action_part_ref() id = %s on line %d "
           "hasSelectionSubscriptList = %s hasImageSelector = %s \n",
           id->text, id->line, hasSelectionSubscriptList ? "true" : "false",
           hasImageSelector ? "true" : "false");

  // Pei-Hung (06/20/2019) the previous code here is moved to R612
  // (c_action_data_ref()) Only id is pushed to astNameStack and have everything
  // else resolved in R612.
  ...
  // DQ (12/29/2010): Stand up redundant stack to replace use of astNameStack
  // and astHasSelectionSubscriptStack in R612 and R613 handling.
  SgName id_name = id->text;
  astMultipartReferenceStack.push_front(MultipartReferenceType(
      id_name, hasSelectionSubscriptList, hasImageSelector));
}
 ...
 *      |       T_COLON_COLON expr
 *      |       T_IDENT T_EQUALS (expr | T_ASTERISK label ) // could be an
 * actual-arg-spec, see R1220 |       T_ASTERISK label // could be an
 * actual-arg-spec, see R1220 |       { // empty could be an actual-arg, see
 * R1220 // }
 *
 ...
 *
 * @param hasLowerBound True if lower bound is present in a section-subscript
 * (lower_bound:upper_bound:stride).
 * @param hasUpperBound True if upper bound is present in a section-subscript
 * (lower_bound:upper_bound:stride).
 * @param hasStride True if stride is present in a section-subscript
 * (lower_bound:upper_bound:stride).
...
void c_action_section_subscript(ofp_bool hasLowerBound, ofp_bool hasUpperBound,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R619 c_action_section_subscript() hasLowerBound = %s "
           "hasUpperBound = %s hasStride = %s isAmbiguous = %s \n",
           hasLowerBound ? "true" : "false", hasUpperBound ? "true" : "false",
           hasStride ? "true" : "false", isAmbiguous ? "true" : "false");

  // DQ (4/26/2008): The problem here is that this rule can be called in the
  // evaluation of initializers for types in structures (fortran types).  This
  // might be an error in the parser, since it allows that there is another rule
  // that should be called but it is never called by OFP.  Discussions with
  // Craig have concluded that we should handle type initialization using this
  // rule and just get over it :-). The only clue that this rule is being used
  // for initialization is that there are no arguments on the astExpressionStack
  // and that all the arguments are on the astInitializationStack (after a
  // recent rewrite of the general initialization support in the Fortran support
  // of ROSE).

  // DQ (1/31/2011): This rule is also called as part of a function call (e.g.
  // "call g(k=0, *100)", see test2011_37.f90). So this rule is for far more
  // than just array substricpts.

  // Output debugging information about saved state (stack) information.
  outputState("At TOP of R619 c_action_section_subscript()");

  // This is required for the processing of "a(:,:,:,n)", however going to be a
  // problem for the type initialization!
  SgExpression *subscript = buildSubscriptExpression(
      hasLowerBound, hasUpperBound, hasStride, isAmbiguous);
...
void c_action_section_subscript_list(int count) {
  // This is either a subscript or a function parameter list, or an implicit do
  // loop.  If it is an implicit function then treat it as a function call
  // directly, later some array references may have be be fixed up to be
  // function calls but this will be a post processing step.
  ...
  // DQ (4/28/2008): If the astExpressionStack is empty then this is being
  // called as part of the rules for initializers (which I think mistakenly use
  // the subscript support).
  //
  // FMZ (5/10/2010): When function call doesn't have argument, we have to
  // generate SgExpListExp:
  //  example:
  //       -if (foo() .eq. 1) then ....end if
  //         the express stack is empty in this case.
  //       -if (1 .eq. foo()) then ... end if
  //         the expression stack is not empty, but it is "SgIntVal" for "1" not
  //         for the subscript list

  // DQ (2/1/2011): This is the older version of the code that is better suited
  // to use with the newer version of R1220 from Zung's fix. But I am not sure
  // that this predicit is required... see test2010_162.f90 and recheck...
  if (astExpressionStack.empty() == false || count == 0) {
    ...
    // printf ("In c_action_section_subscript_list():
    // astActualArgumentNameStack.size() = %" PRIuPTR "
    ...
    for (int i = 0; i < count; i++) {
      ...
      // DQ (2/2/2011): Why are we rebuilding the SgActualArgumentExpression
      // objects?

      // DQ (11/30/2007): Actual arguments have associated names which have to
      // be recorded on to a separate stack. test2007_162.h demonstrates this
      // problems (and test2007_184.f). Built astActualArgumentNameStack to
      // support this. Actual argument specification permits function arguments
      // to be specified in any order. Examples: sum(array,dim=1)
      if (astActualArgumentNameStack.empty() == false) {
        ...
        // See test2007_162.f for an example of where this restriction is
        // required.  It might be that we need a special stack for this! if
        // (name != "defaultString")
        {
          // DQ (2/2/2011): Doesn't this just nest a SgActualArgumentExpression
          // into another SgActualArgumentExpression.
    ...
    // printf ("In c_action_section_subscript_list(): AFTER astExpressionStack
    // processing \n");
    astExpressionStack.push_front(expressionList);
  }
 ...
 * T_COLON_COLON allocation_list (T_COMMA alloc_opt_list)? T_RPAREN T_EOS |
 * (label)? T_ALLOCATE T_LPAREN allocation_list (T_COMMA alloc_opt_list)?
 * T_RPAREN T_EOS
 *
 * @param label Optional statement label
...
void c_action_allocate_stmt(Token_t *label, Token_t *allocateKeyword,
                            Token_t * /*eos*/, ofp_bool hasTypeSpec,
                            ofp_bool hasAllocOptList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R623 c_action_allocate_stmt() label = %p = %s allocateKeyword = "
           "%p = %s hasTypeSpec = %s hasAllocOptList = %s \n",
...
/** R628, R631-F2008
 * allocation
 *              ( T_LPAREN allocate_shape_spec_list
 * {hasAllocateShapeSpecList=true;} T_RPAREN )? ( T_LBRACKET
 * allocate_co_array_spec {hasAllocateCoArraySpec=true;} T_RBRACKET )?
 *
 * NOTE: In current parser, hasAllocateShapeSpecList is always false, appears as
 * R619 section-subscript-list.  In a section-subscript, the stride shall not be
 * present and if hasUpperBound is false, hasLowerBound shall be present and
 * must be interpreted as an upper-bound-expr.
 *
...
void c_action_allocation(ofp_bool hasAllocateShapeSpecList,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R628 c_action_allocation() hasAllocateShapeSpecList = %s "
           "hasAllocateCoArraySpec = %s \n",
...
void c_action_allocate_object_list(int count) {
  ...
  // c_action_allocate_stmt().  I don't know why it is named
  // "c_action_allocate_object_list()"
...
void c_action_allocate_shape_spec(ofp_bool hasLowerBound,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R630 c_action_allocate_shape_spec(): hasLowerBound = %s "
           "hasUpperBound = %s \n",
           hasLowerBound ? "true" : "false", hasUpperBound ? "true" : "false");
}
...
void c_action_nullify_stmt(Token_t *label, Token_t *nullifyKeyword,
                           Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_nullify_stmt(): label = %p = %s nullifyKeyword = %p = "
           "%s \n",
...
void c_action_deallocate_stmt(Token_t *label, Token_t *deallocateKeyword,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_deallocate_stmt(): label = %p = %s nullifyKeyword = %p "
           "= %s hasDeallocOptList = %s \n",
...
void c_action_level_1_expr(Token_t *definedUnaryOp) {
  ...
  if (definedUnaryOp != NULL) {
    ...
    SgExpression *result = createUnaryOperator(
        tmp, definedUnaryOp->text, /* is_user_defined_operator */
        true);
...
void c_action_defined_unary_op(Token_t *definedOp) {
  // This action is called with the parser sees the unary operator, but the
  // operand for the unary operator has not been processed yet (will be done in
  // R702).  So this is too early a point to process the unary operator!
...
void c_action_power_operand(ofp_bool hasPowerOperand) {
...
#if !SKIP_C_ACTION_IMPLEMENTATION
  // This appears to be called a number of times, but we only want to process
  // entries on the stack to generate a SgExponentiationOp when hasPowerOperand
  // == true.
  if (hasPowerOperand == true) {
    ...
    // Note that the type provided is NULL, since ROSE will internally
    // (dynamically) evaluate the type as required.  This avoids state in the
    // AST and better supports transformations on the AST.
...
void c_action_mult_operand__mult_op(Token_t *multOp) {
  ...
  // Note that the type provided is NULL, since ROSE will internally
  // (dynamically) evaluate the type as required.  This avoids state in the AST
  // and better supports transformations on the AST.
...
#if ROSE_OFP_MINOR_VERSION_NUMBER >= 8 & ROSE_OFP_PATCH_VERSION_NUMBER >= 2
  // printf ("WARNING: In OFP versions 0.8.2 and greater it is not clear what to
  // do in c_action_add_operand() \n");
...
void c_action_signed_operand(Token_t *addOp) {
  // DQ (9/26/2010): Moved the support for selecting unary +/- to fix Rice
  // fortran bug (test2010_??.f90).
...
void c_action_add_operand__add_op(Token_t *addOp) {
  // Unclear if I should be using c_action_add_operand__add_op or
  // c_action_add_operand. However, since we want to set the source position of
  // the addOp in the generated expression I have elected to build the
  // expression here.
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
void c_action_level_2_expr(int numConcatOps) {
  ...
  if (numConcatOps > 0) {
    ...
    result = createBinaryOperator(lhs, rhs,
                                  relOp->text, /* is_user_defined_operator */
                                  false);
    ...
    for (int i = 0; i < numConcatOps - 1; i++) {
      ...
      result = createBinaryOperator(lhs, accumulator_rhs,
                                    relOp->text, /* is_user_defined_operator */
                                    false);
      setSourcePosition(result, relOp);
...
void c_action_level_3_expr(Token_t *relOp) {
  ...
  if (relOp != NULL) {
    ...
    // DQ (10/7/2008): There should uniformally be a toke on the stack for the
    // ".<operator>." name.
    ROSE_ASSERT(astNameStack.empty() == false);
    astNameStack.pop_front();
  }
...
void c_action_rel_op(Token_t *relOp) {
  ...
  // DQ (10/7/2008): Added support for relational operators to be handled
  // symetricly with other operators.
  ROSE_ASSERT(relOp != NULL);
...
void c_action_and_operand(ofp_bool hasNotOp, int numAndOps) {
  ...
  if (processRelationalOperator == true) {
    ...
    // ROSE_ASSERT( (hasNotOp == true && numAndOps == 0) || (hasNotOp == false
    // && numAndOps > 0) );
    ...
    for (int i = 0; i < numAndOps; i++) {
      ...
      // This applies only to the first operand (which is a special case, and
      // last on the stack since it was pushed first)
      if (i == numAndOps - 1 && hasNotOp == true) {
        // Then this is a UNARY operator (.NOT.)
        Token_t *local_relOp = astNameStack.front();
        // printf ("local_relOp->text = %s (should be
        // .NOT.)\n",local_relOp->text);
        ROSE_ASSERT(local_relOp != NULL);
        lhs = createUnaryOperator(
            lhs, local_relOp->text, /* is_user_defined_operator */
            false);
      ...
      ROSE_ASSERT(relOp != NULL);
      result = createBinaryOperator(lhs, tempExpression,
                                    relOp->text, /* is_user_defined_operator */
                                    false);
      setSourcePosition(result, relOp);
    ...
    // This applies only to the first operand (which is a special case, and last
    // on the stack since it was pushed first)
    if (numAndOps == 0 && hasNotOp == true) {
      // Then this is a UNARY operator (.NOT.)
      ROSE_ASSERT(relOp != NULL);
      // printf ("relOp->text = %s (should be .NOT.)\n",relOp->text);
      result =
          createUnaryOperator(rhs, relOp->text, /* is_user_defined_operator */
                              false);
      setSourcePosition(result, relOp);
...
void c_action_and_operand__not_op(ofp_bool hasNotOp) {
  // This rule applies only to each of child operands except the first one in a
  // chain (of ".and." operators).
  ...
  if (hasNotOp == true) {
    ...
    ROSE_ASSERT(local_relOp != NULL);
    astNameStack.pop_front();
    // printf ("c_action_and_operand__not_op() (hasNotOp == true):
    // local_relOp->text = %s \n",local_relOp->text);
    ROSE_ASSERT(astExpressionStack.empty() == false);
    SgExpression *tmp = astExpressionStack.front();
    astExpressionStack.pop_front();
    ...
    SgExpression *result = createUnaryOperator(
        tmp, local_relOp->text, /* is_user_defined_operator */
        false);
...
void c_action_equiv_operand__equiv_op(Token_t *equivOp) {
  ...
  if (equivOp != NULL) {
    ...
    // printf ("In c_action_equiv_operand__equiv_op() (level 5 expr):
    // equivOp->text = %s \n",equivOp->text);
    ...
    result = createBinaryOperator(lhs, rhs,
                                  equivOp->text, /* is_user_defined_operator */
                                  false);
...
void c_action_level_5_expr__defined_binary_op(Token_t *definedBinaryOp) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_level_5_expr__defined_binary_op(): definedBinaryOp = "
           "%p = %s \n",
           definedBinaryOp,
  ...
  if (definedBinaryOp != NULL) {
    ...
    result = createBinaryOperator(
        lhs, rhs, definedBinaryOp->text, /* is_user_defined_operator */
        true);
    ...
    // DQ (10/7/2008): There should uniformally be a token on the stack for the
    // ".<operator>." name. ROSE_ASSERT(astNameStack.empty() == false);
    // astNameStack.pop_front();
...
void c_action_defined_binary_op(Token_t *binaryOp) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_defined_binary_op(): binaryOp = %p = %s \n", binaryOp,
           binaryOp != NULL ? binaryOp->text : "NULL");
}
...
void c_action_assignment_stmt(Token_t *label, Token_t *) {
  ...
  // DQ (1/31/2009): Refactored code to support us in R735 pointer assignment
  // statement.
...
void c_action_pointer_assignment_stmt(Token_t *label, Token_t * /*eos*/,
                                      ofp_bool hasBoundsSpecList,
                                      ofp_bool hasBRList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R735 c_action_pointer_assignment_stmt() label = %p "
           "hasBoundsSpecList = %s hasBRList = %s \n",
           label, hasBoundsSpecList ? "true" : "false",
           hasBRList ? "true" : "false");

  // We map the pointer assignment operator "=>" to the regular assignment
  // expression. But in the unparser we detect that the l-value is a pointer
  // type and use "=>" in the generated code instead of "=".
  ...
  // DQ (1/31/2009): I need an example of this code before I can handle these
  // cases.
...
void c_action_where_stmt__begin() {
  ...
  // DQ (1/23/2008): This will be set later when we have more information about
  // its source position. setSourcePosition(whereStatement);
...
void c_action_where_stmt(Token_t *label, Token_t *whereKeyword) {
  ...
  // DQ (11/15/2007): Pop the SgBasicBlock that was previously pushed on the
  // stack for the body!
  astScopeStack.pop_front();
  ...
  // DQ (11/15/2007): Check if the whereStatement has already been added
  // previously!
  // ROSE_ASSERT(find(currentBlockScope->get_statements().begin(),currentBlockScope->get_statements().end(),whereStatement)
  // == currentBlockScope->get_statements().end());
  // currentBlockScope->append_statement(whereStatement);
  ROSE_ASSERT(find(currentBlockScope->get_statements().begin(),
                   currentBlockScope->get_statements().end(), whereStatement) !=
              currentBlockScope->get_statements().end());
  ...
  // Add any saved statements on the stack to the body (there should only be a
  // single statement, I think).
  ROSE_ASSERT(astNodeStack.empty() == false);
  SgStatement *statement = isSgStatement(astNodeStack.front());
  ROSE_ASSERT(statement != NULL);

  // The c_action_where_stmt() is called when we don't have an end where
  // statement
  whereStatement->set_has_end_statement(false);

  // DQ (12/26/2007): Strip the label from any statement used in a where
  // statement (the label belongs to the where statement itself)
  if (statement->get_numeric_label() != NULL) {
    // printf ("Warning: Strip the label from any statement used in a where
    // statement \n");
    statement->set_numeric_label(NULL);
  }

  ROSE_ASSERT(find(body->get_statements().begin(), body->get_statements().end(),
                   statement) != body->get_statements().end());
  ...
  // DQ (11/15/2007): At this point we should have the body of the function or
  // the SgBasicBlock of another scope which can has statements.
  ROSE_ASSERT(astScopeStack.empty() == false);
 ...
 *              ( elsewhere_stmt ( where_body_construct {numElsewhereConstructs
 * += 1;} )* )? end_where_stmt
 *
...
void c_action_where_construct(int numConstructs, ofp_bool hasMaskedElsewhere,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R744 c_action_where_construct() numConstructs = %d "
           "hasMaskedElsewhere = %s hasElsewhere = %s \n",
 ...
 *      :       (T_IDENT T_COLON)? T_WHERE_CONSTRUCT_STMT T_WHERE T_LPAREN expr
 * T_RPAREN T_EOS
 *
 * @param id Optional name for the loop. If you use this up front, you have to
 * use it in the end, and vice versa.
 */
void c_action_where_construct_stmt(Token_t *id, Token_t *whereKeyword,
  ...
  SgWhereStatement *whereStatement =
      new SgWhereStatement(condition, body, NULL);
  ...
  // The c_action_construct_where_stmt() is called when we do have an end where
  // statement
...
void c_action_masked_elsewhere_stmt(Token_t *label, Token_t *elseKeyword,
                                    Token_t * /*whereKeyword*/, Token_t *id,
                                    Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf(
  ...
  // Get the associated where statement and push the false body onto the scope
  // stack.
  ROSE_ASSERT(astScopeStack.empty() == false);
  SgBasicBlock *current_where_body = isSgBasicBlock(astScopeStack.front());
  SgWhereStatement *parentWhereStmt =
      isSgWhereStatement(current_where_body->get_parent());
  SgElseWhereStatement *parentElseWhereStmt =
      isSgElseWhereStatement(current_where_body->get_parent());
  ...
  SgElseWhereStatement *elseWhereStatement =
      new SgElseWhereStatement(condition, body, NULL);
 ...
 *
 * @param numBodyConstructs The number of where-body-constructs in the
 * elsewhere-stmt (called from R744)
 */

void c_action_masked_elsewhere_stmt__end(int numBodyConstructs) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R749 c_action_masked_elsewhere_stmt__end() numBodyConstructs = "
           "%d \n",
           numBodyConstructs);
 ...
 * @param label The label
 * @param id Optional name for the forall loop. If you use this up front, you
 * have to use it in the end, and vice versa.
 */
void c_action_forall_construct_stmt(Token_t *label, Token_t *id,
                                    Token_t *forallKeyword, Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R753 c_action_forall_construct_stmt() label = %p = %s id = %p = "
           "%s forallKeyword = %p = %s \n",
...
void c_action_end_forall_stmt(Token_t *label, Token_t *endKeyword,
                              Token_t *forallKeyword, Token_t *id,
                              Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R753 c_action_end_forall_stmt() label = %p = %s endKeyword = %p "
           "= %s forallKeyword = %p = %s id = %p = %s \n",
...
void c_action_forall_stmt__begin() {
  ...
  // treating it as a scope will allow it to be consistent across C,C++, and
  // Fortran.
...
void c_action_forall_stmt(Token_t *label, Token_t *forallKeyword) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R753 c_action_forall_stmt() label = %p = %s forallKeyword = %p "
           "= %s \n",
  ...
  SgForAllStatement *forAllStatement =
      isSgForAllStatement(astScopeStack.front());
...
void c_action_block() {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_block() (popping the current scope from a case "
           "statement) \n");
  ...
  if (astScopeStack.front()->get_endOfConstruct()->get_line() ==
      astScopeStack.front()->get_startOfConstruct()->get_line()) {
    if (lastStatement != NULL)
      printf("Error: (endOfConstruct == startOfConstruct) "
             "lastStatement->get_endOfConstruct()->get_line() = %d \n",
             lastStatement->get_endOfConstruct()->get_line());

    printf("Error: (endOfConstruct == startOfConstruct) "
           "astScopeStack.front()->get_endOfConstruct()->get_line() = %d \n",
...
void c_action_if_then_stmt(Token_t *label, Token_t *id, Token_t *ifKeyword,
  ...
  SgExprStatement *conditionalStatement =
      new SgExprStatement(conditionalExpression);
  SgBasicBlock *true_block = new SgBasicBlock();

  // Rasmussen(7/23/2018): Moved to C style AST construction (false block is
  // NULL if present) This was done to allow analysis and transformations to
  // work the same for C and Fortran. SgBasicBlock* false_block = new
  // SgBasicBlock();
  SgBasicBlock *false_block = NULL;

  SgIfStmt *ifStatement =
      new SgIfStmt(conditionalStatement, true_block, false_block);
  ...
  // DQ (12/7/2010): Let the unparser know that this came from R803 sio that we
  // know to output the "then" keyword.
  ifStatement->set_use_then_keyword(true);

  // Save the if-stmt that might be the start the a chain of
  // if-then-else-if-else-endif
  astIfStatementStack.push_front(ifStatement);
  // printf ("Pushed ifStatement = %p onto astIfStatementStack (size = %"
  ...
  SgBasicBlock *currentBlockScope = isSgBasicBlock(currentScope);
  ROSE_ASSERT(currentBlockScope != NULL);
  // printf ("Appending ifStatement = %p to scope = %p
  // currentBlockScope->get_statements().size() = %" PRIuPTR "
  ...
  // Set or clear the astLabelSymbolStack (since it is redundant with the label
  // being passed in) There are two mechanisms used to set labels, and we never
  // know which will be used by OFP.
  ...
  // DQ (11/26/2007): This should be empty now. Check to make sure that we have
  // not left track on the stack.
  ROSE_ASSERT(astExpressionStack.empty() == true);
}
...
void c_action_else_if_stmt(Token_t *label, Token_t *elseKeyword,
                           Token_t *ifKeyword, Token_t *thenKeyword,
                           Token_t *id, Token_t *eos) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_else_if_stmt() label = %p = %s elseKeyword = %p = %s "
           "ifKeyword = %p = %s caseKeyword = %p = %s id = %p = %s \n",
  ...
  // Rasmussen(7/23/2018): Moved to C style AST construction (false block is
  // NULL if present) This was done to allow analysis and transformations to
  // work the same for C and Fortran.
  //
  ...
  // Rasmussen (7/22/2018): set is_else_if_statement correctly (now used by
  // unparser)
  ...
  // After calling c_action_if_then_stmt() to reuse code there, pop the if-stmt
  // that was saved, since it is NOT the beginning of a chain of
  // if-then-else-if-else-endif statements. printf ("Pop ifStatement = %p off of
  // astIfStatementStack (pushed on by R804 calling R803) (size = %" PRIuPTR ")
  // \n",ifStatement,astIfStatementStack.size());
...
void c_action_else_stmt(Token_t *label, Token_t *elseKeyword, Token_t *id,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_else_stmt(): label = %p = %s elseKeyword = %p = %s id "
           "= %p = %s \n",
  ...
  // DQ (12/12/2007): After implimenting the support for the select and case
  // statements it made more sense to pop the scope in R801 instead of here. pop
  // off the true block! astScopeStack.pop_front();
  ...
  // DQ (12/16/2007): Added support for specification of label on the else part
  // of the SgIfStmt
...
void c_action_end_if_stmt(Token_t *label, Token_t *endKeyword,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In R1230 c_action_end_function_stmt(): label = %p = %s endKeyword "
           "= %p = %s ifKeyword = %p = %s id = %p = %s \n",
           label, label != NULL ? label->text : "NULL", endKeyword,
  ...
  // DQ (12/12/2007): After implimenting the support for the select and case
  // statements it made more sense to pop the scope in R801 instead of here. Pop
  // off the true block! ROSE_ASSERT(astScopeStack.empty() == false);
  // astScopeStack.pop_front();

  // Remove the accumulated scopes on the stack back to the original starting
  // SgIfStmt object
  ...
  while (astScopeStack.empty() == false &&
         astScopeStack.front() != startingIfStatement) {
    // Unwind through all the accumulated IF ... ELSE using a single END IF.
    // printf ("LOOP: (unwinding through if ... else statements)
    // astScopeStack.front() = %p = %s
  ...
  // Output debugging information about saved state (stack) information.
  outputState("After cleaning up stack of if-stmt and block pairs in R806 "
              "c_action_end_if_stmt()");

  SgIfStmt *ifStatement = isSgIfStmt(astScopeStack.front());
  ROSE_ASSERT(ifStatement != NULL);

  // DQ (12/26/2007): record that the if statement has an associated endif
  // statement
...
void c_action_if_stmt__begin() {
  ...
  SgIfStmt *ifStatement =
      new SgIfStmt((SgStatement *)NULL, true_block, false_block);
...
void c_action_if_stmt(Token_t *label, Token_t *ifKeyword) {
  ...
  // This rule is for statements of the sort: "if (a) b = 0" without an
  ...
  while (i != statementList.end()) {
    // Clear the numeric labels in the block of an if-statement (should have
    // only a single statement) If that had been R803, we would not clear the
    // labels since that if construct has a block where the statements in the
    // block can have labels.
  ...
  // DQ (10/10/2010): See example test2007_17.f90 of if statment on a single
  // line for were we can't enforce this.
  // ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct()->get_line() !=
  // astScopeStack.front()->get_startOfConstruct()->get_line());
...
void c_action_select_case_stmt(Token_t *label, Token_t *id,
                               Token_t *selectKeyword, Token_t *caseKeyword,
                               Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_select_case_stmt() label = %p = %s id = %p = %s "
           "selectKeyword = %p = %s caseKeyword = %p = %s \n",
           label, label ? label->text : "NULL", id, id ? id->text : "NULL",
           selectKeyword, selectKeyword ? selectKeyword->text : "NULL",
  ...
  astExpressionStack.pop_front();
  SgExprStatement *itemSelectorStatement =
      new SgExprStatement(itemSelectorExpression);
  ...
  SgSwitchStatement *switchStatement =
      new SgSwitchStatement(itemSelectorStatement, body);
...
void c_action_case_stmt(Token_t *label, Token_t *caseKeyword, Token_t *id,
                        Token_t * /*eos*/) {
  MLOG_TRACE_CXX("OpenFortranParser_SAGE_CONNECTION")
      << "In c_action_case_stmt()"
      << " label = " << label << " = " << (label ? label->text : "NULL")
      << " caseKeyword = " << caseKeyword << " = "
      << (caseKeyword ? caseKeyword->text : "NULL") << " id = " << id << " = "
      << (id ? id->text : "NULL") << endl;

  SgStatement *caseOrDefaultStatement = NULL;
  SgBasicBlock *body = SageBuilder::buildBasicBlock_nfi();

  if (getCaseStmt()->isDefault) {
    SgDefaultOptionStmt *defaultOptionStmt =
        SageBuilder::buildDefaultOptionStmt_nfi(body);
    if (id != NULL)
      defaultOptionStmt->set_default_construct_name(id->text);

    caseOrDefaultStatement = defaultOptionStmt;
  } else {
    SgExprListExp *key = getCaseStmt()->key;
    SgCaseOptionStmt *caseOptionStmt =
        SageBuilder::buildCaseOptionStmt_nfi(key, body);

    if (id != NULL)
      caseOptionStmt->set_case_construct_name(id->text);
  ...
  SgWhereStatement *whereStatement =
      isSgWhereStatement(astScopeStack.front()->get_parent());
  SgIfStmt *ifStatement = isSgIfStmt(astScopeStack.front()->get_parent());
  if (whereStatement != NULL || ifStatement != NULL) {
    // If in a where statement produced with R 619:section-subscript-list__begin
    // then the condition is on the stack, else if it was produced with
    // R744:where-construct-stmt then the condition was used directly and
    // already cleared from the stack.
    ROSE_ASSERT(astExpressionStack.size() <= 1);
  } else {
...
void c_action_end_select_stmt(Token_t *label, Token_t *endKeyword,
                              Token_t *selectKeyword, Token_t *id,
                              Token_t * /*eos*/) {
  MLOG_TRACE_CXX("FortranParser")
      << "In c_action_end_select_stmt()"
      << " label = " << label << " = " << (label ? label->text : "NULL")
      << " endKeyword = " << endKeyword << " = "
      << (endKeyword ? endKeyword->text : "NULL")
      << " selectKeyword = " << selectKeyword << " = "
      << (selectKeyword ? selectKeyword->text : "NULL") << " id = " << id
      << " = " << (id ? id->text : "NULL") << endl;
  ...
  // DQ (10/10/2010): Moved the poping of the stack to this function so that we
  // could set the source end position of the scope.
  ROSE_ASSERT(endKeyword != NULL);
  resetEndingSourcePosition(astScopeStack.front(), endKeyword);
...
void c_action_case_selector(Token_t *defaultToken) {
  MLOG_TRACE_CXX("FortranParser")
      << "In c_action_case_selector()"
      << " defaultToken = " << defaultToken << " = "
      << (defaultToken ? defaultToken->text : "NULL") << endl;
...
void c_action_case_value_range(ofp_bool hasColon) {
  MLOG_TRACE_CXX("FortranParser")
      << "In c_action_case_value_range(): hasColon = " << hasColon << "\n";
  ;
  ...
  if (hasColon == false) {
    ...
    } else if (caseValueRange->hasSuffix &&
               (caseValueRange->hasSuffixExpr == false)) {
      ROSE_ASSERT(caseValueRange->numValues == 1);
    ...
    range = SageBuilder::buildSubscriptExpression_nfi(lowerBound, upperBound,
                                                      stride);
  }
...
void c_action_case_value_range_list__begin() {
  MLOG_TRACE_CXX("FortranParser")
      << "In c_action_case_value_range_list__begin() \n";

  // An SgExprListExp (the key) is needed for the Fortran grammar
...
void c_action_case_value_range_list(int count) {
  MLOG_TRACE_CXX("FortranParser")
      << "In c_action_case_value_range_list(): count = " << count << endl;
}
...
void c_action_case_value() {
  ...
  caseValueRange->values[caseValueRange->numValues++] =
      astExpressionStack.front();
  astExpressionStack.pop_front();
}
...
void c_action_associate_stmt(Token_t *label, Token_t *id,
  ...
  SgAssociateStatement *associateStatement =
      isSgAssociateStatement(scopeParent);
  ...
  do {
    ...
    variableDeclaration->prepend_variable(initializedName,
    ...
    astScopeStack.front()->insert_symbol(initializedName->get_name(),
                                         variableSymbol);
    ROSE_ASSERT(initializedName->get_symbol_from_symbol_table() != NULL);
...
void c_action_association_list(int count) {
  ...
  // This will be come the entry in the SgAssociateStatemen's
  // variable_declaration_list. Note that we require a list of variable
  // declarations because the types can be different.

  // DQ (11/30/2007): The current implementation only handles the case of a
  // single associate variable. Later we need to extend this to handle a list of
  // variable declarations.

  // Pei-Hung (07/24/2019) Change implementation to handle multiple associate
  // variables ROSE_ASSERT(count == 1);
...
void c_action_association(Token_t *id) {
  ...
  // Transfer the expression on the astExpressionStack to the
  // astInitializerStack (for the upcoming variable declaration)
  ROSE_ASSERT(astExpressionStack.empty() == false);

  SgAssignInitializer *initializer =
      new SgAssignInitializer(astExpressionStack.front(), NULL);
  setSourcePosition(initializer);
  ...
  SgInitializedName *associateVariable =
      new SgInitializedName(variableName, variableType, initializer);
  ...
  // Set the position of the declaration to be the first use of the implicitly
  // defined variable. setSourcePosition(implicitVariable,getTopOfNameStack());
  ...
  // // printf ("DONE: Calling buildVariableDeclaration in
  // c_action_association() \n");
  // ROSE_ASSERT(variableDeclaration->get_file_info()->isCompilerGenerated() ==
  // false);
  //
  ...
  // This takes a name off of the name stack and puts a variable reference onto
  // the astExpressionStack (building a declaration if required) types are
  // computed using implicit type rules. c_action_data_ref(0);
...
void c_action_end_associate_stmt(Token_t *label, Token_t *endKeyword,
                                 Token_t * /*associateKeyword*/, Token_t *id,
                                 Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  // Pop off the SgAssociateStatement (which is a scope, derived from
  // SgScopeStatement)
  astScopeStack.pop_front();
}
...
void c_action_select_type_stmt(Token_t * /*label*/,
                               Token_t * /*selectConstructName*/,
 ...
 */
void c_action_end_select_type_stmt(Token_t * /*label*/,
                                   Token_t * /*endKeyword*/,
                                   Token_t * /*selectKeyword*/,
                                   Token_t * /*id*/, Token_t * /*eos*/) {
  ROSE_ASSERT(astScopeStack.empty() == false);
  ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct() != NULL);
  ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct()->get_line() !=
...
void c_action_do_stmt(Token_t *label, Token_t *id, Token_t *doKeyword,
                      Token_t *digitString, Token_t *eos,
                      ofp_bool hasLoopControl) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
  ...
  // This has to work for "DO i=1,100", "DO WHILE", and "DO
  // CONCURRENT(i=1:100:2)

  // At this point for "DO I = 1, 100" the expressions for "I", "1", and "100"
  // are on the astExpressionStack.
  ...
  if (hasLoopControl == true) {
    // We have a "DO 123 I=1, 100, 4" style loop, where the digit string 123 and
    // stride 4 are optional.
    ...
    if (astExpressionStack.size() == 2) {
      // There are ending and starting indices on astExpressionStack and the
      // index variable on astNameStack
      ...
      // DQ (1/18/2011): This detects where we have used the semantics of
      // implicitly building symbols for implicit variables. printf ("WARNING:
      // This use of trace_back_through_parent_scopes_lookup_variable_symbol()
      // used the side effect of building a symbol if the reference is not
      // found! \n"); ROSE_ABORT();
      ...
      // The index expression is the "I=1" part (which is built so that we can
      // hand it to the SgFortranDo constructor)
      ...
      if (astExpressionStack.size() == 1) {
        // This is the case of a "DO WHILE" loop, with the condition on the
        // astExpressionStack...
        buildWhileLoop = true;
        predicate = astExpressionStack.front();
        astExpressionStack.pop_front();
      } else if (astExpressionStack.size() == 0) {
        // This is the case of a "DO CONCURRENT" loop, with the expression list
        // astNodeStack... Note that this may also apply to a "FORALL" loop but
        // it has been make obsolete in the standard so ignoring for now
        // [Rasmussen 2019.08.23]
        ...
        printf("Error: astExpressionStack.size() incorrect for do_stmt "
               "astExpressionStack.size() = %" PRIuPTR " \n",
               astExpressionStack.size());
        ROSE_ABORT();
      }
    }
    ...
    // We have a simpler "DO" style loop, there are no expressions on the stack
    // to get
    ROSE_ASSERT(astExpressionStack.empty() == true);
  ...
  if (buildWhileLoop == true) {
    ...
    // Use valid pointers to SgNullExpression when nothing was present in the
    // source code.
    if (stride == NULL) {
      stride = new SgNullExpression();
      setSourcePosition(stride);
    ...
    SgFortranDo *fortranDo =
        new SgFortranDo(indexExpression, endingIndex, stride, body);

    // DQ (11/28/2010): Added specification of case insensitivity for Fortran.
    fortranDo->setCaseInsensitive(true);

    // DQ (12/26/2007): This field is depricated in favor of the
    // has_end_statement boolean field used uniformally in several IR nodes). DQ
    // (12/24/2007): Default to true and then if we see the c_action_end_do_stmt
    // we can set it to false. fortranDo->set_old_style(true);
  ...
  // Liao 2/1/2011, set the body's endOfConstruct to be the same as the loop
  // statement's endOfConstruct We should not use the endOfConstruct of fortran
  // loop here since c_action_end_do_stmt() will call
  // resetEndingSourcePosition() to change it later on.
  //   body->set_endOfConstruct (new
  //   Sg_File_Info(*(loopStatement->get_endOfConstruct())));
  ...
  // DQ (12/25/2007): Made the FortranDo a SgScopeStatement to be uniform with
  // other loop constructs. So now we want to push the loopStatement onto the
  // astScopeStack.
 ...
 * @param hasLoopControl True if there is a loop control.
 */
void c_action_label_do_stmt(Token_t * /*label*/, Token_t * /*id*/,
                            Token_t * /*doKeyword*/, Token_t * /*digitString*/,
                            Token_t * /*eos*/, ofp_bool hasLoopControl) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
void c_action_loop_control(Token_t *whileKeyword, int doConstructType,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In c_action_loop_control(): whileKeyword = %p = %s doConstructType "
           "= %d hasOptExpr = %s \n",
           whileKeyword, whileKeyword != NULL ? whileKeyword->text : "NULL",
...
void c_action_do_variable(Token_t *id) {
  // This identifies the do loop index variable, but at this point it is on the
  // astExpressionStack already.
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_do_variable() id = %s \n",
           (id != NULL) ? id->text : "NULL");

  // DQ (9/27/2010): Push this so that we can have it on the stack and determine
  // the name of the variable in the implied-do-control loop. This is also used
  // in the c_action_do_stmt.
  astNameStack.push_front(id);
}
...
void c_action_end_do() {
  ...
  // DQ (12/12/2007): After implimenting the support for the select and case
  // statements it made more sense to pop the scope in R801 instead of here.

  // Pop the stack here, since the c_action_end_do_stmt() will only be called
  // for "DO" statement that uses an "END DO" and not a "CONTINUE" statement.
  // There are at least two ways to close of a Fortran "DO" loop. This function
  // is called for each of the two ways which close off the Fortran "DO" loop.
  // astScopeStack.pop_front();
  ...
  // Pop off the loop construct's scope (it is a scope in C/C++, even if not in
  // Fortran) treating it as a scope will allow it to be consistent across
  // C,C++, and Fortran.
  ROSE_ASSERT(astScopeStack.empty() == false);

  // printf ("NOTE: Moved call to pop astScopeStack from R833 c_action_end_do()
  // both R834 c_action_end_do_stmt() and R838 c_action_do_term_action_stmt()
  // \n"); astScopeStack.pop_front();
}
...
void c_action_end_do_stmt(Token_t *label, Token_t *endKeyword,
  ...
  if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL) {
    // Note that if doKeyword == NULL then the endKeyword containes the shorted
    // form "ENDDO" Also eos contains the "\n" at the end of the line
    printf("   label      = %p = %s \n", label,
           label != NULL ? label->text : "NULL");
  ...
  // printf ("In c_action_end_do_stmt(): We should pop the astScopeStack here
  // instead of in c_action_end_do() \n");
  ...
  // Pei-Hung (06/25/2019) test2019_doconstruct.f90 shows that a do constrcut
  // can be in a single line. Therefore the following assert is unnecessary.
  // ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct()->get_line() !=
  // astScopeStack.front()->get_startOfConstruct()->get_line());

  setStatementNumericLabel(astScopeStack.front(), label);

  // DQ (12/7/2010): We left a lable on the astLabelSymbolStack, see
  // test2007_76.f90 (with R213 stack empty rule enforced).
...
void c_action_do_term_action_stmt(Token_t *label, Token_t *endKeyword,
                                  Token_t * /*doKeyword*/, Token_t * /*id*/,
                                  Token_t * /*eos*/, ofp_bool inserted) {
  // Output debugging information about saved state (stack) information.
  outputState("At TOP of R838 c_action_do_term_action_stmt()");

  // This rule can be called even where the end-do does not exist (in this case,
  // the endKeyword == NULL).
  if (endKeyword != NULL) {
    markDoLoopAsUsingEndDo();
    resetEndingSourcePosition(astScopeStack.front(), endKeyword);
  ...
  if (inserted) {
    // this is an artificial do-terminator inserted by the scanner and 'label'
...
void c_action_goto_stmt(Token_t *label, Token_t *goKeyword, Token_t *toKeyword,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_goto_stmt() target_label = %p = %s goKeyword = %p = %s "
           "toKeyword = %p = %s \n",
           goKeyword, goKeyword != NULL ? goKeyword->text : "NULL", toKeyword,
           toKeyword != NULL ? toKeyword->text : "NULL", target_label,
  ...
  // This takes a SgStatement as a label, but that is being replaced to take a
  // SgLabelSymbol.
  ...
  // When this statement can handle a numericl label (on the statement itself)
  // then uncomment this line.  This is an OFP bug that was reported 12/20/2007.
...
/** R846
 * computed_goto_stmt
 *      :       (label)? t_go_to T_LPAREN label_list T_RPAREN (T_COMMA)? expr
 * T_EOS
 *
...
void c_action_computed_goto_stmt(Token_t *label, Token_t *goKeyword,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_computed_goto_stmt() label = %p = %s goKeyword = %p = "
           "%s toKeyword = %p = %s \n",
  ...
  // the label list is now on top of the astExpressionStack via
  // c_action_label_list
  ROSE_ASSERT(astExpressionStack.empty() == false);
  ...
  SgComputedGotoStatement *computedGoto =
      new SgComputedGotoStatement(labelList, index_expression);
...
void c_action_assign_stmt(Token_t *label1, Token_t *assignKeyword,
                          Token_t *label2, Token_t *toKeyword, Token_t *name,
                          Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_assign_stmt(): label1 = %p = %s goKeyword = %p = %s "
           "label2 = %p = %s toKeyword = %p = %s name = %p = %s \n",
...
void c_action_assigned_goto_stmt(Token_t *label, Token_t *goKeyword,
                                 Token_t *toKeyword, Token_t *name,
                                 Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_assigned_goto_stmt(): label = %p = %s goKeyword = %p = "
           "%s toKeyword = %p = %s name = %p = %s \n",
           label, label != NULL ? label->text : "NULL", goKeyword,
           goKeyword != NULL ? goKeyword->text : "NULL", toKeyword,
           toKeyword != NULL ? toKeyword->text : "NULL", name,
           name != NULL ? name->text : "NULL");
...
void c_action_pause_stmt(Token_t *label, Token_t *pauseKeyword,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_pause_stmt(): label = %p = %s pauseKeyword = %p = %s "
           "constant = %p = %s \n",
  ...
  SgProcessControlStatement *pauseStatement =
      new SgProcessControlStatement(pauseExpression);

  pauseStatement->set_control_kind(SgProcessControlStatement::e_pause);

  if (constant != NULL) {
    // Note that this constant is also mistakenly interpreted as a label by OFP
    // and so we end up with a label matching the constant.  This should be
    // fixed in OFP. See test2010_134.f90 (with error: Duplicate statement label
    // 10 at (1) and (2)).
...
void c_action_arithmetic_if_stmt(Token_t *label, Token_t *ifKeyword,
                                 Token_t *label1, Token_t *label2,
                                 Token_t *label3, Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_arithmetic_if_stmt() label = %p = %s label = %p = %s "
           "label = %p = %s label = %p = %s label = %p = %s \n",
...
void c_action_continue_stmt(Token_t *label, Token_t *continueKeyword,
                            Token_t * /*eos*/) {
  // A Fortran "CONTINUE" statement is mapped to a C "label" statement, so we
  // use the existing SgLabelStatement IR node.

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_continue_stmt(): label = %p = %s continueKeyword = %p "
           "= %s \n",
  ...
  if (label != NULL) {
    ...
    if (astLabelSymbolStack.empty() == false) {
      ...
      // Mark this as a regular numerical label (in columns 2-6) and not
      // associated with an else statement or an end statement.
      labelSymbolFromStack->set_label_type(SgLabelSymbol::e_start_label_type);
      ...
      // Reset the statement referenced by this label (was previously referenced
      // to a statement which referenced it but was not that statement's
      // numerical_label).
      labelSymbolFromStack->set_fortran_statement(labelStatement);
    } else {
      // printf ("Warning: No label found in argument to
      // c_action_continue_stmt() and astLabelSymbolStack.empty() == true \n");
    }
  }
  ...
  SgFunctionDefinition *currentFunctionScope =
      SageInterface::getEnclosingFunctionDefinition(astScopeStack.front(),
                                                    true);
  ...
  if (continueKeyword->line == 0) {
    // DXN: FIXME - workaround OFP bug where label->line = 0 as in the following
    // example:
    /*       subroutine FOURT()
     *       DO 125 I1=1, 2
     *       DO 125 I3= 3, 4
     ...
     *       END
     *
     *  When label is parsed the second time around, label->line = 0 and
     * label->col = -1
     */
  ...
  // DQ (12/16/2007): A label had been pushed onto the stack, but it is
  // redundant with the label provided as a token.
  if (label != NULL) {
    // outputState("Before astNameStack.pop_front() in R848
    // c_action_continue_stmt()");
    ...
    if (astLabelSymbolStack.empty() == false) {
      if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
        printf("Warning: found a LabelSymbol on the stack, but it was not "
               "passed into the c_action_continue_stmt() \n");

      astLabelSymbolStack.pop_front();
    }
  }

  // DQ (10/10/2010): Mark the end of the do loop scope using the
...
void c_action_stop_stmt(Token_t *label, Token_t *stopKeyword, Token_t * /*eos*/,
  ...
  if (hasStopCode) // DXN (04/03/2011): the stop code is at the top of the
                   // astExpressionStack
  {
    // ROSE_ASSERT(astExpressionStack.empty() == false);
  ...
  SgProcessControlStatement *stopStatement =
      new SgProcessControlStatement(stopExpression);
...
void c_action_allstop_stmt(Token_t * /*label*/, Token_t * /*allKeyword*/,
                           Token_t * /*stopKeyword*/, Token_t * /*eos*/,
                           ofp_bool /*hasStopCode*/) {
  printf(
...
void c_action_sync_all_stmt(Token_t * /*label*/, Token_t * /*syncKeyword*/,
                            Token_t * /*allKeyword*/, Token_t * /*eos*/,
                            ofp_bool /*hasStopCode*/) {
  printf(
...
void c_action_sync_stat_list__begin() {
  printf("In c_action_sync_stat_list__begin() - this function needs to be "
         "implemented.\n");
}
...
void c_action_sync_stat_list(int /*count*/) {
  printf("In c_action_sync_stat_list() - this function needs to be "
         "implemented.\n");
}
...
void c_action_file_unit_number() {
  ...
  if (astExpressionStack.empty() == true) {
    printf("In c_action_file_unit_number(): Unclear what to do in this case "
           "(empty astExpressionStack)! \n");
    ROSE_ABORT();
  } else {
    // Note that if this is a scalar-logical-expression, then the expression is
    // likely from a if-stmt and not a part of a format statement.
    SgExpression *expressionOnStack = astExpressionStack.front();
    // Using a switch statement allows us to more cleanly handle a growing
    // number of special cases. printf ("expressionOnStack = %p = %s
    ...
    // Handle case of format string (passed as a variable):  CHARACTER(len=*) ::
    // fmtstr;  PRINT fmtstr, str (see test2010_125.f90).
    case V_SgVarRefExp: {
      ...
      if (intType != NULL) {
        ...
        printf("In c_action_file_unit_number(): Unclear what to do in this "
               "case! \n");
        ROSE_ABORT();
      }
    ...
    default: {
      // Add not "fmt" token.
      printf("Non integer scalars can NOT be used in format statements so "
             "ignore this (no fmt pushed onto astNameStack). \n");
...
void c_action_open_stmt(Token_t *label, Token_t *openKeyword, Token_t *) {
  ...
  // DQ (12/11/2010): There can be more expressions on the astExpressionStack
  // than names on the astNameStack, but only the number of names on the
  // astNameStack count.  See test2010_143.f90. while
  // (astExpressionStack.empty() == false)
  while (astNameStack.empty() == false) {
    ROSE_ASSERT(astNameStack.empty() == false);
    ...
    // We don't need the current_IO_Control_Spec data structure in the code
    // below.

    // The "unit=" string is optional, if it was not present then a toekn was
    // pushed onto the stack with the text value "defaultString"
    if (strncasecmp(name->text, "unit", 4) == 0 ||
...
void c_action_connect_spec(Token_t *id) {
  ...
  if (id != NULL) {
    ...
    // This is the case of an option not being specified, as in "open(1)"
    // instead of "open(UNIT=1)" To make the astExpressionStack match the
    // astNameStack we have to push a default token onto the astNameStack.
    Token_t *defaultToken = createSyntheticToken(0, 0, 0, "defaultString");
...
void c_action_close_stmt(Token_t *label, Token_t *closeKeyword, Token_t *) {
  ...
  // DQ (9/6/2010): Switch to using the astNameStack since there can be an
  // additional expression on the astExpressionStack due to an if stmt predicate
  // (see test2010_40.f90). printf ("Warning: Ignoring all but the 'unit' in the
  // OpenStatement \n"); while (astExpressionStack.empty() == false)
  while (astNameStack.empty() == false) {
    ROSE_ASSERT(astNameStack.empty() == false);
    ROSE_ASSERT(astExpressionStack.empty() == false);
...
void c_action_close_spec(Token_t *closeSpec) {
  ...
  if (closeSpec != NULL) {
    ...
    // This is the case of an option not being specified, as in "close(1)"
    // instead of "close(UNIT=1)" To make the astExpressionStack match the
    // astNameStack we have to push a default token onto the astNameStack.
    Token_t *defaultToken = createSyntheticToken(0, 0, 0, "defaultString");
    ROSE_ASSERT(defaultToken != NULL);
    astNameStack.push_front(defaultToken);
...
void c_action_read_stmt(Token_t *label, Token_t *readKeyword, Token_t * /*eos*/,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_read_stmt(): label = %p = %s readKeyword = %p = %s "
           "hasInputItemList = %s \n",
  ...
  // DQ (12/19/2007): This is a more uniform handling of the SgExprListExp
  // (computed in R915) SgExprListExp* exprListExp =
  // isSgExprListExp(astExpressionStack.front());
  ...
  if (astExpressionStack.empty() == false) {
    ...
    // DQ (9/6/2010): Handle the case of empty input-item-list (see
    // test2010_40.f90) astExpressionStack.pop_front(); ROSE_ASSERT(exprListExp
    // != NULL);
    if (exprListExp == NULL) {
    ...
    // DQ (12/30/2007): This is the trivial case where no input items have been
    // specified (e.g. "read *")
    exprListExp = new SgExprListExp();
    setSourcePosition(exprListExp);
  }
  ...
  // If this list is empty then there was nothing specified, as reported by OFP,
  // but this is the case of "read *, ..." so we will interpret this as the case
  // of "read fmt=*. ..." or "read *, ..."
  bool availableFormatSpecifier = astNameStack.empty();
  if (availableFormatSpecifier == true) {
    // There was no option recorded by OFP, so this means that "*" was used (OFP
    // bug, I think). So push the asterisk, and record it as the fmt option.
    SgAsteriskShapeExp *asterisk = new SgAsteriskShapeExp();
    setSourcePosition(asterisk);

    astExpressionStack.push_front(asterisk);

    // This is the case of an option not being specified, as in "read(1)"
    // instead of "read(UNIT=1)" To make the astExpressionStack match the
    // astNameStack we have to push a default token onto the astNameStack.
  ...
  while (astNameStack.empty() == false) {
    ...
    if (astNameStack.empty() == false) {
      // This is required for cases such as: "read (1,asynchronous='YES')
      // array", see test2007_123.f03.
      lookAheadName = astNameStack.front();
    }
    // printf ("In c_action_read_stmt(): processing token = %s with expression =
    // %p = %s lookAheadName = %p
    ...
    // The "unit=" string is optional, if it was not present then a token was
    // pushed onto the stack with the text value "defaultString" if (
    // (strncasecmp(name->text,"fmt",3) == 0) ||
    // (strncmp(name->text,"defaultString",13) == 0) &&
    // (readStatement->get_format() == NULL) ) if (
    // (strncasecmp(name->text,"fmt",3) == 0) || (
    // (strncmp(name->text,"defaultString",13) == 0) &&
    // (readStatement->get_format() == NULL) && (initialStackDepth >= 2) ) )
 ...
 */
void c_action_write_stmt(Token_t *label, Token_t *writeKeyword,
                         Token_t * /*eos*/, ofp_bool hasOutputItemList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_write_stmt(): label = %p = %s writeKeyword = %p = %s "
           "hasOutputItemList = %s \n",
  ...
  // Rasmussen (05/30/2019): Extensive rewrite to fix error in processing the
  ...
  if (exprListExp == NULL) {
    ...
    // If we got the SgExprListExp from the stack then we pop the stack (but
    // only then)
    astExpressionStack.pop_front();
  }
  ...
  // will be sorted out last after processing the named ones (not
  // "defaultString"). Regarding order:
  //   1. If unit is not named it shall be the first item in the list (and shall
  //   be present).
  //   2. If nml is not named and appears in the list (as a namelist) it shall
  //   be the second item after unit.
  //   3. If fmt is not named it shall be the second item in the list following
  //   unit.
  ...
  while (astNameStack.empty() == false) {
    ...
    // Handle the unnamed case (with name "defaultString") by storing the
    ...
    if (strncmp(spec_name->text, "defaultString", 13) == 0) {
      ...
      if (isSgIntVal(spec_expr) != NULL) {
        ...
        labelRefExp->set_parent(writeStatement);
                 ...
                 isSgFunctionCallExp(spec_expr) !=
                     NULL || // can occur when trim is used for a character
                             // string
                 isSgAsteriskShapeExp(spec_expr) != NULL) {
        writeStatement->set_format(spec_expr);
      } else {
        fprintf(stderr,
                "In c_action_write_stmt(): ERROR processing token = %s with "
                "spec_expr of class = %s \n",
    ...
    // Always set the parent of the spec_expr so we won't have to do it later in
    // processing the unnamed_spec_list
    spec_expr->set_parent(writeStatement);
  }

  // Now process the unnamed spec items (they are in reverse order because they
  // came from a stack)
  //
  ...
  if (has_unit_spec)
    ROSE_ASSERT(num_unnamed <= 1);

  // Get spec expressions in reverse order (recall they came from a stack)
  SgExpression *spec_expr_1 =
      (num_unnamed >= 1) ? unnamed_spec_list[num_unnamed - 1] : NULL;
  ...
  //   (if has_unit_spec=true it is has been named with "unit=" and is not on
  //   the unnamed list)
  SgExpression *spec_expr_other = (has_unit_spec) ? spec_expr_1 : spec_expr_2;

  // Handle the unit spec first as it must be present and first on the list (if
  ...
  // Now check for a namelist as it can only be a name (SgVarRefExp) and thus
  // different from a format spec
  SgVarRefExp *nml_expr =
      (has_nml_spec == false) ? isSgVarRefExp(spec_expr_other) : NULL;
  if (nml_expr != NULL) {
  ...
  SgIntVal *fmt_label_expr =
      (has_format_spec == false) ? isSgIntVal(spec_expr_other) : NULL;
  ...
  // Check for a format spec as a string, variable, function call (e.g., trim),
  // or '*'
  ...
  if (has_format_spec == false) {
    if (isSgStringVal(spec_expr_other) != NULL ||
        ...
        isSgFunctionCallExp(spec_expr_other) !=
            NULL || // can occur when trim is used for character string
  ...
  if (spec_expr_other != NULL) {
    ...
    printf("  assuming this is a name-list-spec gone wrong, examine and try "
           "test pr17285.f90 \n");
    // test pr17285.f90 has a namelist that appears as a SgFunctionRefExp
    // TODO - fix this - but for now let it be a name-list

    printf(
        "--- recovering by processing expression class name %s as nml spec \n",
        spec_expr_other->class_name().c_str());
  ...
  // Error checking: there may still be something on the stack from an unhandled
  // implied do loop when we have a correctly handled implied do loop (there are
  // three flavors) this should not be required.
  if (astExpressionStack.empty() == false) {
    // DQ (12/12/2010): Make this a warning only output within verbose mode.
    if (SgProject::get_verbose() > 0) {
      printf("WARNING: unfinished implied do loop expressions may be left on "
             "stack (or from a c_action_if_stmt()) (write)\n");
    }
  }
}
 ...
 * @param hasOutputItemList True if output-item-list is present
 */
void c_action_print_stmt(Token_t *label, Token_t *printKeyword,
                         Token_t * /*eos*/, ofp_bool hasOutputItemList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_print_stmt(): label = %p = %s printKeyword = %p = %s "
           "hasOutputItemList = %s \n",
  ...
  // DQ (1/26/2009): test2009_07.f demonstrates that the astExpressionStack can
  // have the conditional expression from another part of the statement if this
  // is an "if" statement.
  ...
  // DQ (1/26/2009): Avoid getting things from the stack that are at least wrong
  // to be used in a print format.
  if (astExpressionStack.empty() == false) {
    SgExpression *formatLabel = astExpressionStack.front();

    // DQ (1/26/2009): This should not be a binary operator (else it is likely
    // an error). Can be a string concatination operator which is a binary op
    // (see test2010_124.f90). ROSE_ASSERT(isSgBinaryOp(formatLabel) == NULL);
    ...
    if (astNameStack.empty() == false) {
      ...
      // (strncasecmp(name->text,"fmt",3) == 0) ||
      // (strncmp(name->text,"defaultString",13) == 0) &&
      // (writeStatement->get_format() == NULL) && numberOfDefaultOptions == 2)
      // if (strncasecmp(name->text,"fmt",3) == 0)
      // printf ("Processing token = %s as format spec \n",name->text);
  ...
  // DQ (1/26/2009): test2009_07.f shows the use of "if (7==8) print *, 'proc
  // coords: ', 1, 2, 3" for this case we can't clear the stack, let's see what
  // other test codes fail if we skip clearing the stack after and implied do in
  // a print statement.
  if (astExpressionStack.empty() == false) {
    printf("WARNING: astExpressionStack still has %" PRIuPTR
           " entries (Ok if used in a statement for example) (print) \n",
           astExpressionStack.size());
  }
...
void c_action_io_control_spec(ofp_bool hasExpression, Token_t *keyword,
                              ofp_bool hasAsterisk) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In R913 c_action_io_control_spec(): hasExpression = %s keyword = "
           "%p = %s hasAsterisk = %s \n",
           hasExpression ? "true" : "false", keyword,
           keyword != NULL ? keyword->text : "NULL",
           hasAsterisk ? "true" : "false");

  // This is the case of an option not being specified, as in "read(1)" instead
  // of "read(UNIT=1)" To make the astExpressionStack match the astNameStack we
  // have to push a default token onto the astNameStack.
  if (keyword == NULL) {
    Token_t *defaultToken = createSyntheticToken(0, 0, 0, "defaultString");
    ROSE_ASSERT(defaultToken != NULL);
    astNameStack.push_front(defaultToken);
  ...
  if (hasAsterisk) {
    ...
    cout << "c_action_io_control_spec(): case hasExpression == false and "
            "hasAsterisk == false NOT possible from the grammar."
         << endl;
    ROSE_ABORT();
  }

  // DQ (9/6/2010): Added error checking. We can't assert equalify since an if
  // stmt can have a predicate on the stack.
...
void c_action_io_control_spec_list(int count) {
  ...
  // Note that we need to associate each expression with names on the
  // astNameStack.
}
...
void c_action_format() {
  ...
  // DQ (12/3/2010): I think this is the problem with test2007_211.f.  Namely we
  // have to push a name ("fmt") on the the astNamestack!

  // Output debugging information about saved state (stack) information.
  outputState("At TOP of R914 c_action_format()");

  if (astExpressionStack.empty() == true) {
    // If there was nothing pushed to the stack, then push the
    // SgAsteriskShapeExp ??? I think we are just borrowing the fact that this
    // expression will output a "*" and perhaps we should have a specialized
    // expression more specific to format semantics.
    SgExpression *asteriskExpression = new SgAsteriskShapeExp();
    setSourcePosition(asteriskExpression);
    ...
    // Handle case of format string: print "(a, i8)", "a = ", N (see
    // test2010_123.f90).
    case V_SgStringVal:

    // Handle case of format string (formed from string concatination): print
    // "(a, "||" i8)", "a = ", N (see test2010_124.f90).
...
void c_action_input_item_list(int count) {
  // This takes the number of expression on the stack (given by 'count'). clears
  // them from the stack, and puts them into an expression list and puts that on
  // the stack.
...
void c_action_output_item_list(int count) {
  ...
  // Convert the list of expressions into an expression list (and put it onto
  // the stack) ROSE_ASSERT(astExpressionStack.size() == (unsigned)count);
...
void c_action_io_implied_do() {
  ...
  // DQ (1/31/2011): The typical value is two unless the stride is included (in
  // which case it is three). DQ (10/9/2010): This improved design uses a single
  // expression to hold the do loop variable initialization.
  // ROSE_ASSERT(implied_do_control->get_expressions().size() == 2);
  ROSE_ASSERT(implied_do_control->get_expressions().size() == 2 ||
              implied_do_control->get_expressions().size() == 3);
  ...
  // DQ (1/31/2011): Added the case of where a stride is explicitly specified
  // (see test2010_160.f90).
  SgExpression *stride = NULL;
  ...
  SgExpression *increment = NULL;
  if (stride != NULL) {
    // printf ("Setting the io_implied do loop intrement using the stride = %p =
    // %s taken from the stack. \n",stride,stride->class_name().c_str());
    increment = stride;
  } else {
    increment = new SgNullExpression();
    ROSE_ASSERT(increment != NULL);
  ...
  SgScopeStatement *implied_do_scope = NULL; // new SgBasicBlock();
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVar,assignment,upperBound,increment,objectList);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(variableReference,doLoopVar,lowerBound,upperBound,increment,objectList);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVar,lowerBound,upperBound,increment,objectList);
  ...
  // SgImpliedDo(doLoopVar,lowerBound,upperBound,increment,objectList,implied_do_scope);
  // SgImpliedDo* impliedDo = new
  // SgImpliedDo(doLoopVarInitialization,lowerBound,upperBound,increment,objectList,implied_do_scope);
  SgImpliedDo *impliedDo =
      new SgImpliedDo(doLoopVarInitialization, upperBound, increment,
                      objectList, implied_do_scope);
...
void c_action_io_implied_do_object() {
  // This rule is called for each object so we have to accumulate the results
  // into a SgExprListExp which we store on the stack.
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  // Accumulate the entries into a SgExprListExp that we replace at the top of
  // the list.
  ROSE_ASSERT(astExpressionStack.empty() == false);
  SgExprListExp *implied_do_object_list =
      isSgExprListExp(astExpressionStack.front());
...
#endif
{
  ...
  // If this was an implicit variable then variableSymbol would be NULL, but the
  // variable would have been built as a side-effect of calling
  // trace_back_through_parent_scopes_lookup_variable_symbol(). So now we have
  // to call it again.

  // Note that the location of the scope of the definition of the implicit
  // implied do index will later be a special scope stored in the implied do
  // loop IR node.
  if (variableSymbol == NULL) {
    // DQ (1/19/2011): Build the implicit variable
 ...
 * @param label The label.
 */
void c_action_wait_stmt(Token_t * /*label*/, Token_t *waitKeyword,
                        Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_wait_stmt(): waitKeyword = %p = %s \n", waitKeyword,
           waitKeyword != NULL ? waitKeyword->text : "NULL");
...
void c_action_wait_spec(Token_t *id) {
  ...
  if (id != NULL) {
    ...
    // This is the case of an option not being specified, as in "wait(1)"
    // instead of "wait(UNIT=1)" To make the astExpressionStack match the
    // astNameStack we have to push a default token onto the astNameStack.
    Token_t *defaultToken = createSyntheticToken(0, 0, 0, "defaultString");
    ROSE_ASSERT(defaultToken != NULL);
    astNameStack.push_front(defaultToken);
 ...
 *
 * @param label The label.
 * @param hasPositionSpecList True if there is a position spec list. False is
 there is a file unit number.
 */
...
void c_action_endfile_stmt(Token_t * /*label*/, Token_t *endKeyword,
                           Token_t *fileKeyword, Token_t * /*eos*/,
                           ofp_bool hasPositionSpecList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_wait_stmt(): endKeyword = %p = %s fileKeyword = %p = "
           "%s hasPositionSpecList = %s \n",
 ...
 * @param hasPositionSpecList True if there is a position spec list. False is
 there is a file unit number.
 */
void c_action_rewind_stmt(Token_t * /*label*/, Token_t *rewindKeyword,
                          Token_t * /*eos*/, ofp_bool hasPositionSpecList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_rewind_stmt(): rewindKeyword = %p = %s "
           "hasPositionSpecList = %s \n",
           rewindKeyword, rewindKeyword != NULL ? rewindKeyword->text : "NULL",
           hasPositionSpecList ? "true" : "false");
  ...
  // If this was "rewind 1" then we would now have a "1" on the stack, and the
  // astNameStack would have been empty.
  if (astExpressionStack.empty() == false) {
    SgExpression *formatLabel = astExpressionStack.front();
...
void c_action_position_spec(Token_t *id) {
  ...
  if (id != NULL) {
    ...
    // This is the case of an option not being specified, as in "flush(1)"
    // instead of "flush(UNIT=1)" To make the astExpressionStack match the
    // astNameStack we have to push a default token onto the astNameStack.
    Token_t *defaultToken = createSyntheticToken(0, 0, 0, "defaultString");
    ROSE_ASSERT(defaultToken != NULL);
    astNameStack.push_front(defaultToken);
 ...
 *
 * @param label The label.
 * @param hasFlushSpecList True if there is a flush spec list. False is there is
 * a file unit number.
 */
void c_action_flush_stmt(Token_t *label, Token_t *flushKeyword,
                         Token_t * /*eos*/, ofp_bool hasFlushSpecList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_flush_stmt(): label = %p = %s flushKeyword = %p = %s "
           "hasFlushSpecList = %s \n",
...
// used to communicate # inquire specs from 'c_action_inquire_spec_list' to
// 'c_action_inquire_stmt'
static int inquireSpecListCount;

void c_action_inquire_stmt(Token_t * /*label*/, Token_t *inquireKeyword,
                           Token_t *id, Token_t *, ofp_bool isType2) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_inquire_stmt(): inquireKeyword = %p = %s id = %s "
           "isType2 = %s \n",
           inquireKeyword,
  ...
  if (isType2 == true) {
    ...
    SgVarRefExp *scalarIntegerExpression =
        isSgVarRefExp(astExpressionStack.front());
...
void c_action_format_specification(ofp_bool hasFormatItemList) {
  ...
  if (hasFormatItemList == true) {
    ...
    // this is the case of "format()", we need to push something onto some stack
    // so that R1001 can know that it has something (even if it is nothing).
 ...
 *
 * @param descOrDigit Edit descriptor, unless has a format item list, then
 * either the optinal digit string or null.
 * @param hasFormatItemList True if has a format item list.
 */
void c_action_format_item(Token_t *descOrDigit, ofp_bool hasFormatItemList) {
  ...
  // DQ (12/2/2010): I think this is the case of an inner list as an entry in
  // the outer list (see test2010_115.f90).
  ...
  if (isNumber == true) {
    ...
    // ROSE_ASSERT(astNodeStack.empty() == false);
    if (astNodeStack.empty() == false) {
      // This is the case of "format (10/)" which processes "10" and "/"
      // seperately (I think this is a bug, see test2007_241.f).
      SgFormatItemList *formatItemList =
          isSgFormatItemList(astNodeStack.front());
      astNodeStack.pop_front();
    ...
    // Set both usesSingleQuotes and usesDoubleQuotes so that we can distinguish
    // un-quoted strings.
    stringValue->set_usesSingleQuotes(usingSingleQuotes);
...
void c_action_format_item_list(int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (formatItem == NULL) {
      SgFormatItemList *nestedFormatItemList =
          isSgFormatItemList(astNodeStack.front());
...
void c_action_v_list_part(Token_t *plus_minus, Token_t *digitString) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_v_list_part(): plus_minus = %p = %s digitString = %p = "
           "%s \n",
...
void c_action_main_program__begin() {
  ...
  // DQ (8/19/2007): This is the new code to use the SgFile built by ROSE (by
  // translator calling the Open Fortran Parser).

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_main_program__begin(): "
...
void c_action_main_program(ofp_bool /*hasProgramStmt*/,
                           ofp_bool /*hasExecutionPart*/,
                           ofp_bool /*hasInternalSubprogramPart*/) {
  // Now can we setup the endOfConstruct in the global scope! But we have no
  // token to use to do this!
 ...
 */
void c_action_program_stmt(Token_t *label, Token_t *programKeyword, Token_t *id,
                           Token_t *) {
  // If this is called then we want to mark the SgProgramHeaderStatement
  // (derived from a SgFunctionDeclaration) as explicit in the source code
  // (using the tokens to communicate the source position information) Also need
  // to make the SgFunctionDefinition and the SgBasicBlock (function body) as
  // being explicit in the source code.
  ...
  // Inconsistency to fix: notice that we are pusing tokens to the back of the
  // tokenList and scopes to the front of the astScopeStack.  This is however a
  ...
  SgFunctionType *functionType =
      new SgFunctionType(SgTypeVoid::createType(), false);
  ...
  // Add a symbol to the symbol table in global scope
  // SgFunctionTypeSymbol* symbol = new
  // SgFunctionTypeSymbol(programDeclaration,functionType);
  // globalScope->insert_symbol(programName, symbol);
  ...
  SgFunctionDefinition *programDefinition =
      new SgFunctionDefinition(programDeclaration, programBody);
  ...
  // DQ (12/9/2007): Moved so that the label's symbol can be put into the
  // function definition's scope.  See test2007_01.f90.
  setStatementNumericLabel(programDeclaration, label);
  ...
  if (programKeyword != NULL) {
    ...
    // These will be marked as isSourcePositionUnavailableInFrontend = true and
    // isOutputInCodeGeneration = true

    // DQ (12/18/2008): These need to make marked with a valid file id (not
    // NULL_FILE, internally), so that any attached comments and CPP directives
  ...
  // Unclear if we should use the same token list for resetting the source
  // position in all three IR nodes.
  ...
  ROSE_ASSERT(programDeclaration->get_firstNondefiningDeclaration() !=
              programDeclaration);
...
void c_action_end_program_stmt(Token_t *label, Token_t *endKeyword,
                               Token_t * /*programKeyword*/, Token_t *id,
                               Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  // SgFunctionDeclaration* programDeclaration =
  // programDefinition->get_declaration();
  SgProgramHeaderStatement *programDeclaration =
      isSgProgramHeaderStatement(programDefinition->get_declaration());
  ...
  // ROSE_ASSERT(functionDeclaration->get_name().getString() ==
  // id->get_lexeme_string());
  if (id != NULL) {
    // printf("%s == %s\n", programDeclaration->get_name().getString().c_str(),
    // id->text); ROSE_ASSERT(programDeclaration->get_name().getString() ==
    // string(id->text));
...
void c_action_module_stmt(Token_t *label, Token_t *moduleKeyword, Token_t *id,
                          Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_module_stmt(): label = %p = %s moduleKeyword = %p = %s "
           "id = %p = %s \n",
  ...
  SgModuleStatement *moduleStatement =
      buildModuleStatementAndDefinition(name, astScopeStack.front());
...
void c_action_end_module_stmt(Token_t *label, Token_t *endKeyword,
                              Token_t * /*moduleKeyword*/, Token_t *id,
                              Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
  ...
  SgModuleStatement *moduleStatement =
      isSgModuleStatement(astScopeStack.front()->get_parent());
  ...
  // DQ (5/21/2008): This should be cleared (see test2008_35.f90, where "public"
  ...
  for (size_t i = 0; i < interfaceList.size(); i++) {
    ...
    // Note that there can be many function declarations (interface bodies) in
    // each interface statement in a module.
    std::vector<SgInterfaceBody *> interfaceBodyList =
        interfaceStatement->get_interface_body_list();

    // Accumulate the new SgRenameSymbol nodes into a list and then add them
    // after we are finished traversing the symbols in the current scope!
    ...
    for (size_t j = 0; j < interfaceBodyList.size(); j++) {
      ...
      if (interfaceBody->get_functionDeclaration() == NULL) {
        ...
        // DQ (9/29/2008): inject all symbols from the module's symbol table
        // into symbol table at current scope.
        SgSymbol *symbol = moduleDefinition->first_any_symbol();
        while (symbol != NULL) {
          SgFunctionSymbol *functionSymbol = isSgFunctionSymbol(symbol);

          // I don't think that the mapped function in the interface has to be
          // public. if (isPubliclyAccessible(symbol) == true)

          if (functionSymbol != NULL) {
            // By definition we assume that the interface name is different from
            // the function name bool isRenamed = true;
            ...
            if (functionDeclaration->get_name() == functionName) {
              // Set the previously NULL function declaration stored in the
              // SgInterfaceBody
              interfaceBody->set_functionDeclaration(functionDeclaration);

              // DQ (10/8/2008): Build a symbol to map the function name to the
              // name defined by the interface. We might want to have a
              // different sort of symbol for this purpose since the
              // functionSymbol and the aliasSymbol are in the same scope (where
              // as previously the SgAliasSymbol has been used to map symbols
              // between scopes). SgAliasSymbol* aliasSymbol = new
              // SgAliasSymbol(functionSymbol,/* isRenamed = true */
              // true,interfaceName); SgRenameSymbol* renameSymbol = new
              // SgRenameSymbol(functionSymbol,interfaceName);
              SgRenameSymbol *renameSymbol =
                  new SgRenameSymbol(functionSymbol->get_declaration(),
                                     functionSymbol, interfaceName);
              if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
                printf("R1106 Insert aliased symbol name = %s (renamed from "
                       "functionName = %s )\n",
                       interfaceName.str(), functionName.str());
              // Accumulate the list of required SgRenameSymbol IR nodes, and
              // add them after we finish the traversal over the current scops
              // symbols currentScope->insert_symbol(interfaceName,aliasSymbol);
  ...
  // DQ (8/28/2010): A function call in a module can appear before it's
  // declaration. This is supported by:
  //    1) building the function declaration
  //    2) Building a function symbol
  //    3) Put the function symbol into the global scope (since it is not clear
  //    where to put it) 4) Fixup function symbols of referenced function calls
  //    in the module
  // printf ("Now process function references in the module to match against
  // functions defined in the module scope! \n"); printf ("Current scope = %p =
 ...
 * @param hasOnly True if has an only statement, regardless of whether a list is
 * present.
 */
void c_action_use_stmt(Token_t *label, Token_t *useKeyword, Token_t *id,
                       Token_t *onlyKeyword, Token_t * /*eos*/,
                       ofp_bool hasModuleNature, ofp_bool hasRenameList,
                       ofp_bool hasOnly) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In c_action_use_stmt(): label = %p = %s useKeyword = %p = %s id = "
           "%p = %s onlyKeyword = %p = %s hasModuleNature = %s hasRenameList = "
           "%s hasOnly = %s \n",
           ...
           hasModuleNature ? "true" : "false", hasRenameList ? "true" : "false",
           hasOnly ? "true" : "false");
  }
  ...
  // SgUseStatement* useStatement = new
  // SgUseStatement(name,nameList,useOnlyExpression);
  ...
  // If moduleSymbol is still null then this was not an intrinsic module and we
  // have to search a bit.
  if (moduleSymbol == NULL) {
    trace_back_through_parent_scopes_searching_for_module(name, currentScope,
                                                          moduleSymbol);
  }

  // If moduleSymbol is still null then we have to go and look for the *.rmod
  // file and read in the module directly.
  ...
  if (moduleSymbol == NULL) {
    ...
    if (moduleStatement == NULL) {
      cout << "Error : cannot find the module (module should have been seen in "
              "a previously generated file) : "
           << modName << endl;
      ROSE_ABORT();
    }
    ...
    // Need to use the defining declaration to get the module (class)
    // definition.
    ...
    // printf ("Found cached (or local) module declaration in AST for module =
    // %s \n",name.str());
  ...
  // DQ (11/12/2008): Now set the reference to the module statement from the use
  // statement (to help support general analysis).  Note that we still do all
  // the symbol table support to alias symbols in the module statement.
  useStatement->set_module(moduleStatement);

  // DQ (9/14/2010): This this should clarify that the current scope was set to
  // be where the use statement is located and that the moduleStatement is the
  // module that is extracted from an rmod file or exists in the current file
  // and was visited previously.
  ROSE_ASSERT(currentScope != moduleStatement->get_definition());

  // Found the module, now read the module's symbols for public members...
  // printf ("Found the module, now read the symbols from the module's symbol
  // table for all public members...\n");

  // This appears to always be false, and I think it is a bug in OFP.
  // Actually, the c_action_only steals the result, so this does work when not
  // using the "only" option. ROSE_ASSERT(hasRenameList == false);
...
#define RICE_S3D_WORKAROUND                                                    \
  1 // Rice's workaround the USE module with rename-list bug
  // Only supporting hasOnly == false in initial work.
  if (hasOnly == false) {
    // This case can only include optional rename entries (and it must be on the
    // astNodeList).
    ...
    if (astNodeStack.empty() == true) {
      ...
      // DQ (9/29/2008): inject all symbols from the module's symbol table into
      ...
      while (symbol != NULL) {
        ...
        if (isPubliclyAccessible(symbol) == true) {
          ...
          // DQ (9/13/2010): Check if this symbol is already in the symbol table
          // and avoid adding it redundently. This is (I think) related to a bug
          // causing an exponential number of symbols to be used. if
          // (classDefinition->symbol_exists(symbolName) == false)
          if (currentScope->symbol_exists(symbolName) == false) {
            SgAliasSymbol *aliasSymbol =
                new SgAliasSymbol(symbol, /* isRenamed */
                                  false);

            if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
              printf("R1109 (hasOnly == false && empty astNodeStack) Insert "
                     "aliased symbol name = %s \n",
                     symbolName.str());

            currentScope->insert_symbol(symbolName, aliasSymbol);
          } else {
            // printf ("R1109 (hasOnly == false && astNodeStack.empty() == true)
            // This symbol already exists (at least using this name = %s) in the
            // symbol table. \n",symbolName.str());
          }
        }
      ...
      // We handle these first and then all the other symbols (not-renamed) from
      ...
      while (astNodeStack.empty() == false) {
        ...
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
          printf("renamePair->get_local_name() = %s renamePair->get_use_name() "
                 "= %s \n",
                 renamePair->get_local_name().str(),
                 renamePair->get_use_name().str());

        // DQ (9/29/2008): inject symbols from module into symbol table at
        // current scope.
        SgSymbol *symbol = classDefinition->first_any_symbol();
        while (symbol != NULL) {
          // Assume for now that there is no renaming of declarations
          if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
            printf("renamePair->get_use_name() = %s symbol->get_name() = %s "
                   "renamePair->get_local_name() = %s \n",
                   renamePair->get_use_name().str(), symbol->get_name().str(),
                   renamePair->get_local_name().str());
          ...
          if (renamePair->get_use_name() == symbol->get_name() &&
            ...
            setOfRenamedSymbols.insert(symbol);
              ...
              SgAliasSymbol *aliasSymbol = new SgAliasSymbol(
                  symbol, /* isRenamed = true */ true, declarationName);
              if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
                printf("R1109 (non-empty astNodeStack)Insert aliased symbol "
                       "name = %s (renamed = %s)\n",
      ...
      // New add the non-renamed symbols from the module's symbol table,
      // retraverse the module's symbol table to do this.
      SgSymbol *symbol = classDefinition->first_any_symbol();
      while (symbol != NULL) {
        bool isRenamedSymbol =
            (setOfRenamedSymbols.find(symbol) != setOfRenamedSymbols.end());
    ...
    // For the case of hasOnly == true, we don't have to check if the current
    // symbol table has already has symbols from the module already (semantics
    // assumes the union of all use statements), I think.
    ...
    // SgRenamePair IR nodes are on the stack (even if there is no renaming
    // done), this provides a uniform interface.
    while (astNodeStack.empty() == false) {
      SgRenamePair *renamePair = isSgRenamePair(astNodeStack.front());
      astNodeStack.pop_front();
      ...
      // DQ (9/13/2010): I think this may be the cause of some inefficency in
      // the symbol table handling, but it is only for the short number of
      // entries on the stack, so not likely significant. DQ (9/29/2008): inject
      // symbols from module's symbol table into local symbol table at current
      // scope.
      SgSymbol *symbol = classDefinition->first_any_symbol();
      while (symbol != NULL) {
        // Assume for now that there is no renaming of declarations
        ...
        if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
          printf("Test: local_name = %s use_name = %s symbol = %p = %s symbol "
                 "name = %s \n",
        ...
        if (use_name == symbol->get_name()) {
          ...
          // DQ (9/13/2010): I think this is true by definition, so we need a
          // better predicate in the conditional below!
          ROSE_ASSERT(classDefinition->symbol_exists(symbolName) == true);

          // DQ (9/13/2010): Check that the symbol using the "local_name" does
          // not already exist in the symbol table. if
          // (classDefinition->symbol_exists(local_name) == false)
          if (currentScope->symbol_exists(local_name) == false) {
            ...
            if (varSymbol) {
              ...
              aliasSymbol = isRenamed
                                ? new SgAliasSymbol(symbol, true, local_name)
          ...
          if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
            printf("The use name is different from the symbol's name (in the "
                   "declaration) (%s vs. %s) \n",
  ...
  // DQ (12/26/2010): Factor out the fixup required for where use statements are
  // used (e.g. function return types).
  use_statement_fixup();

  // astScopeStack.front()->print_symboltable("Output from R1109
  // c_action_use_stmt()");
}
...
void c_action_rename(Token_t *id1, Token_t *id2, Token_t *op1, Token_t *defOp1,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_rename(): id1 = %p = %s id2 = %p = %s op1 = %p = %s "
           "defOp1 = %p = %s op2 = %p = %s defOp2 = %p = %s \n",
  ...
  if (id1 != NULL && id2 != NULL) {
    ...
    // Since there is more than one token used to define a rename, the source
    // position is not accurately set using just "id1".
    setSourcePosition(renamePair, id1);
  } else {
    // This is the case of an operator renaming, note that op1 and op2 will
    // contain the token "operator" if this is an operator renaming.  These
    // should maybe be folded into the names that are input into the
    // SgRenamePair.
    ...
    if (op1 != NULL && op2 != NULL) {
      ...
      // If they are not both valid pointers, make sure that they are both NULL,
      // else we may have missed some corner case for which I don't have a test
      // code yet.
      ROSE_ASSERT(op1 == NULL && op2 == NULL);
    }
  ...
  // Note that any variable reference to the name id1->text will use the alias
  // symbol that will be built in later steps (R1109)
}
...
void c_action_rename_list(int count) {
  ...
  // Since the SgRenamePair IR nodes was built in R1111 (above), I don't think
...
void c_action_only_list(int count) {
  ...
  // they are already processed into SgRenamePair IR nodes and on the
  // astNodeStack.
  if (hasRenameList == false) {
    // If there was not renaming, then build the SgRenamePair using empty names
    // for the local name to signal that there was no renaming. This permits a
    // consistant interface when they are processed by R1109. Pei-Hung
    // (06/18/2019) renameList and generic name can co-exist in the same list
    // Since no renameList will exist in astNameStack, it should use the stack
    // size here, not the count from parser.
    int astNameStackSize = astNameStack.size();
    for (int i = 0; i < astNameStackSize; i++) {
      // Construct the name pair for the case of the "only" clause, where there
      // is no renaming.
      ROSE_ASSERT(astNameStack.empty() == false);
      SgName name = astNameStack.front()->text;

      // printf ("In c_action_only_list(): Building SgRenamePair for name = %s
      // (not renamed) \n",name.str());
      ...
      // Use the rename pir IR node to provide a uniform interface to the
      // construction of the SgUseStatement, but set the local-name to be "".
    ...
    if (astNameStack.empty() == false) {
      printf("Error: rename list with only clause not supported yet, bug in "
             "OFP. \n");
      ROSE_ABORT();
    }
    ROSE_ASSERT(astNameStack.empty() == true);
...
void c_action_block_data_stmt(Token_t *label, Token_t *blockKeyword,
                              Token_t * /*dataKeyword*/, Token_t *id,
                              Token_t * /*eos*/) {
  // The block data statement is implemented to build a function (which
  // initializes data) Note that it can be declared with the "EXTERNAL"
  // statement and as such it works much the same as any other procedure.
...
#if !SKIP_C_ACTION_IMPLEMENTATION
  // This could be the first function or subroutine in the program, so setup the
  // global scope!
  initialize_global_scope_if_required();
  // build_implicit_program_statement_if_required();
  ...
  // Note that a ProcedureHeaderStatement is derived from a
  // SgFunctionDeclaration (and is Fortran specific). The
  // SgProcedureHeaderStatement can be used for a Fortran function, subroutine,
  // or block data declaration.

  SgProcedureHeaderStatement *blockDataDeclaration =
      new SgProcedureHeaderStatement(name, functionType, NULL);
  // DQ (1/21/2008): Set the source position to avoid it being set without
  // accurate token position information
  ...
  // This does not do everything required to build a function or subroutine, but
  // it does as much as possible (factors out code so that it can be called for
  // R1117, R1224, and R1232.
...
void c_action_interface_stmt(Token_t * /*label*/, Token_t *abstractToken,
                             Token_t *keyword, Token_t * /*eos*/,
                             ofp_bool hasGenericSpec) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_interface_stmt(): kw1 = %p = %s kw2 = %p = %s "
           "hasPrefix = %s \n",
  ...
  if (matchingName(interfaceNameString, "OPERATOR") == true) {
    ...
    // DQ (10/10/2008): We want to use the actual operator name if it is a .xxx.
    // form
  ...
  // DQ (10/1/2008): Changed interface to support a list of interface blocks
  // (now a SgDeclarationStatementPtrList).
  SgInterfaceStatement *interfaceStatement =
      new SgInterfaceStatement(interfaceName, generic_spec_kind);
  ...
  // DQ (10/2/2008): Push the new SgInterfaceStatement onto the
  // astInterfaceStack.
  astInterfaceStack.push_front(interfaceStatement);

  // DQ (10/8/2008): Fixup astNameStack to clear any remaining entries (see
  // test2008_55.f90)
  if (astNameStack.empty() == false) {
    if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
 ...
 * @param hasGenericSpec True if has a generic spec.
 */
void c_action_end_interface_stmt(Token_t * /*label*/, Token_t *kw1,
                                 Token_t *kw2, Token_t * /*eos*/,
                                 ofp_bool hasGenericSpec) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_end_interface_stmt(): kw1 = %p = %s kw2 = %p = %s "
           "hasPrefix = %s \n",
           kw1, kw1 != NULL ? kw1->text : "NULL", kw2,
           kw2 != NULL ? kw2->text : "NULL", hasGenericSpec ? "true" : "false");

  // astScopeStack.pop_front();

  // DQ (10/1/2008): Fixup astNameStack to clear any remaining entries (see
  // test2008_42.f90)
  if (astNameStack.empty() == false) {
    if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
      printf("WARNING: astNameStack not empty in c_action_end_interface_stmt() "
...
void c_action_procedure_stmt(Token_t * /*label*/, Token_t *module,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_procedure_stmt(): module = %p = %s procedureKeyword = "
           "%p = %s \n",
           module, module != NULL ? module->text : "NULL", procedureKeyword,
           procedureKeyword != NULL ? procedureKeyword->text : "NULL");

  // This function is called when processing function prototypes in interface
  // blocks. At this point we may not have seen the function definition yet, so
  // there may not be a valid function declaration. Except for the case of an
  // assignment interface there should be enough information to construct the
  // non-defining declaration. For the case of an assignment interface it is not
  // clear that we can do more than just save the name and fixup the interace
  // statement to have a proper function declaration when we later see the
  // function definition.
  ...
  if (astInterfaceStack.empty() == false) {
    ...
    // DQ (9/7/2010): A procedure interface can have a list of names, so we have
    // to process the list saved on the astNameStack. See test2010_42.f90 for an
    ...
    while (astNameStack.empty() == false) {
      ...
      SgInterfaceBody *interfaceBody = new SgInterfaceBody(
          name, nullFunctionDeclaration, /*use_function_name*/ true);
...
void c_action_generic_spec(Token_t *keyword, Token_t *name, int type) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_generic_spec(): keyword = %p = %s name = %p = %s type "
           "= %d \n",
  ...
  case GenericSpec_ASSIGNMENT: {
    // printf ("Operator definition not yet implemented type =
    // GenericSpec_ASSIGNMENT \n"); ROSE_ABORT();
    ROSE_ASSERT(keyword != NULL);
    astNameStack.push_front(keyword);
    break;
  }
...
void c_action_dtio_generic_spec(Token_t *rw, Token_t *format, int type) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_dtio_generic_spec():  rw = %p = %s format = %p = %s "
           "type = %d \n",
 ...
 */
void c_action_import_stmt(Token_t * /*label*/, Token_t *importKeyword,
                          Token_t * /*eos*/, ofp_bool hasGenericNameList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In c_action_import_stmt(): importKeyword = %p = %s "
           "hasGenericNameList = %s \n",
           importKeyword, importKeyword != NULL ? importKeyword->text : "NULL",
...
void c_action_external_stmt(Token_t *label, Token_t *externalKeyword,
  ...
  // DQ (12/11/2010): I think this is always NULL, since labels can't be used
  ...
  while (astNameStack.empty() == false) {
    ...
    // Since we have changed this from a variable to a function, remove the
    // variable symbol
    SgVariableSymbol *variableSymbol =
...
void c_action_procedure_declaration_stmt(Token_t *label,
                                         Token_t *procedureKeyword,
                                         Token_t *eos,
                                         ofp_bool hasProcInterface, int count) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_procedure_declaration_stmt(): label = %p = %s "
           "procedureKeyword = %p = %s eos = %p hasProcInterface = %s count = "
           "%d \n",
           label, label != NULL ? label->text : "NULL", procedureKeyword,
           procedureKeyword != NULL ? procedureKeyword->text : "NULL", eos,
           hasProcInterface ? "true" : "false", count);
  ...
  // DQ (1/24/2011): Test2011_25.f90 demonstrates a use of a procedure attribure
  // spec. printf ("Sorry, procedure declaration statements are not implemented
  // yet. \n");
}
...
void c_action_proc_attr_spec(Token_t *attrKeyword, Token_t *id, int spec) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_proc_attr_spec(): attrKeyword = %p = %s id = %p = %s "
           "spec = %d \n",
           attrKeyword, attrKeyword != NULL ? attrKeyword->text : "NULL", id,
           id != NULL ? id->text : "NULL", spec);
...
void c_action_proc_decl(Token_t *id, ofp_bool hasNullInit) {
  ...
  // DQ (1/25/2011): This could be the first statement in a program (see
  // test2011_35.f90).
  build_implicit_program_statement_if_required();

  // DQ (1/24/2011): In this function we build a variable as a procedure and
  // define it as a function type AND we build it as a variable with a symbol.
  // Not clear if this is how I really want to support fortran procedure
  // pointers.
  ROSE_ASSERT(id != NULL);
  astNameStack.push_front(id);
  ...
  // DQ (1/24/2011): Add an assocciated function "procedure()" to the global
  // scope so that fortran can build procedure pointers.
  SgScopeStatement *topOfStack = *(astScopeStack.rbegin());
  ROSE_ASSERT(topOfStack != NULL);
  ...
  ROSE_ASSERT(topOfStack->variantT() == V_SgGlobal);
  SgName programName = "procedure";
  // SgProgramHeaderStatement* programDeclaration = new
  // SgProgramHeaderStatement(programName,functionType,NULL);
  ...
  if (globalScope->symbol_exists(programName) == false) {
    ...
    // SgVariableSymbol* variableSymbol = new
    // SgVariableSymbol(variableInitilizedName);
  ...
  // DQ (1/25/2011): We can't use c_action_entity_decl() since it tests for the
  // existence of the referenced name in an outer scope this causes problems
  // when the name was previously used in a outer scope (see test2011_32.f90 as
  // an example).
  ...
  // DQ (1/25/2011): This support used the fortran support function:
  // buildVariableDeclaration() in R1214 c_action_proc_decl_list(int count).
  SgName variableName = id->text;
  ROSE_ASSERT(astBaseTypeStack.empty() == false);
  SgType *type = astBaseTypeStack.front();

  // DQ (1/25/2011): We need this on the stack for when
  // buildVariableDeclarationAndCleanupTypeStack() is called in R1214
  // c_action_proc_decl_list(int count). astBaseTypeStack.pop_front();

  SgInitializedName *initializedName =
      new SgInitializedName(variableName, type, NULL, NULL, NULL);
  setSourcePosition(initializedName);

  // This will be filled in by buildVariableDeclaration (fortran support), but
  // we set it here since it is tested below.
  initializedName->set_scope(astScopeStack.front());

  // The semantics of the call to buildVariableDeclaration (fortran support)
  // required the SgInitializedName objects on the stack.
  ...
  // DQ (1/25/2011): Until the SgVariableDeclaration is built, we can't assert
  // this! ROSE_ASSERT(initializedName->get_symbol_from_symbol_table() != NULL);
  ...
  ROSE_ASSERT(initializedName->get_scope()->get_symbol_table() != NULL);
  // initializedName->get_scope()->get_symbol_table()->print("Output symbol
  // table from R612");

  if (hasNullInit == true) {
    // We could use the stack value to build the SgInitializedName more
    // directly. See test2011_25.f90 for an example of this (procedure pointer).
...
void c_action_proc_decl_list__begin() {
  // I don't think we have to do anything here (unless we have to complete the
  // definition of the base type as we do for regular variable declarations).
}
...
void c_action_call_stmt(Token_t *label, Token_t *callKeyword, Token_t * /*eos*/,
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_call_stmt(): label = %p = %s callKeyword = %p = %s "
           "hasActualArgSpecList = %s \n",
  ...
  SgFunctionCallExp *functionCallExp =
      isSgFunctionCallExp(astExpressionStack.front());

  if (functionCallExp == NULL) {
    // If the syntax didn't include "()" then we get a SgFunctionRefExp on the
    // stack and we have to build the SgFunctionCallExp.
  ...
  // Since this is used in a subroutine call, we know that the function is a
  // subroutine, so mark it as such so that the unparser will generate the
  // correct code.
  SgFunctionRefExp *functionRefExp =
      isSgFunctionRefExp(functionCallExp->get_function());
  ...
  // printf ("In R1218 c_action_call_stmt(): functionDeclaration = %p = %s name
  // = %s
  // \n",functionDeclaration,functionDeclaration->class_name().c_str(),functionDeclaration->get_name().str());

  // SgProgramHeaderStatement* programHeaderStatement =
  // isSgProgramHeaderStatement(functionDeclaration);
  ...
  // subroutineDeclaration->get_symbol_from_symbol_table(); SgFunctionSymbol*
  // functionSymbol =
  // trace_back_through_parent_scopes_lookup_function_symbol(procedureDeclaration->get_name(),currentScopeOfFunctionDeclaration);
  // ROSE_ASSERT(functionSymbol != NULL);
}
...
void c_action_procedure_designator() {
  ...
  // This means that the item on the astExpressionStack should have been a
  // function call expression. printf ("This means that the item on the
  // astExpressionStack should have been a function call expression \n"); DQ
  // (5/15/2008): Temp code!

  // Note that convertExpressionOnStackToFunctionCallExp() will also inforce
  // that "astExpressionStack.empty() == false"
  ROSE_ASSERT(astExpressionStack.empty() == false);
  if (astExpressionStack.empty() == false) {
...
void c_action_actual_arg_spec(Token_t *keyword) {
  ...
  // associated with expressions will not cause separate stacks to be out of
  // sync.
  if (keyword != NULL) {
    ROSE_ASSERT(astExpressionStack.empty() == false);
    SgExpression *expression = astExpressionStack.front();
 ...
 *
 * @param count The number of items in the list.  If count is less than zero,
 * clean up the effects of list begin (as if it had not been called).
 */
void c_action_actual_arg_spec_list__begin() {
  ...
  // printf ("I don't think that this is ever called!
...
void c_action_actual_arg(ofp_bool hasExpr, Token_t *label) {
  ...
  // DXN (01/06/2011):
  // concatenate '*' and the label and push it on the astExpressionStack as the
  // actual argument.
  if (label != NULL) {
    // string asteriskArg = "*" + string(label->text);
    string asteriskArg = label->text;
    // SgVarRefExp* p_actualArg = SageBuilder::buildOpaqueVarRefExp(asteriskArg,
    // getTopOfScopeStack());
    ...
    if (!astLabelSymbolStack.empty()) {
      ...
      if (SgProject::get_verbose() > 0)
        printf("WARNING: astLabelSymbolStack is empty: c_action_actual_arg: "
               "Line %d Col %d\n",
...
void c_action_function_subprogram(ofp_bool hasExePart, ofp_bool hasIntSubProg) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_function_subprogram() hasExePart = %s hasIntSubProg = "
           "%s \n",
...
void c_action_function_stmt(Token_t *label, Token_t *keyword, Token_t *name,
                            ...
                            ofp_bool hasSuffix) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_function_stmt(): label = %p (function name) name = %s "
           "hasGenericNameList = %s hasSuffix = %s \n",
           label, name ? name->text : "NULL",
           hasGenericNameList ? "true" : "false", hasSuffix ? "true" : "false");

#if !SKIP_C_ACTION_IMPLEMENTATION
  // *** Note that function and subroutine code is nearly the same ***

  // *** This setup is the same for functions, subroutines, and the program
  // statement ***
  ...
  // We will only know the name when we get to the end? No, this appears to be a
  // bug in OFP. SgName tempName = "temp_function_name";
  ROSE_ASSERT(name != NULL);
  ...
  if (astBaseTypeStack.empty() == false) {
    ...
    // Note that this type might have to be replaced if the older stype syntax
    // is seen that defines the function type (in the next line) FMZ(6/9/2010):
    // this cause error: when a function doesn't have type, with implicit none
    // we need to issue error message. p.s.  This is not ROSE bug! This is
    // caused by our (Rice folks) using "skip_syntax_check".
    returnType = SgTypeVoid::createType();
  }

  ROSE_ASSERT(returnType != NULL);

  // Note that since each function builds a new function type, we will have to
  // in a post processing step coalesce the function types used in the program.
  // The advantage of not sharing function types is that the return type is each
  // to change as we parse the function specification statement section (before
  // the executable statment section).
  SgFunctionType *functionType = new SgFunctionType(returnType, false);

  // Note that a ProcedureHeaderStatement is derived from a
  // SgFunctionDeclaration (and is Fortran specific).
  SgProcedureHeaderStatement *functionDeclaration =
  ...
  // Rasmussen (1/6/2019): Added so that language binding is processed for
  ...
  buildProcedureSupport(functionDeclaration,
                        hasDummyArgList); // DXN: use the flag hasSuffix and
                                          // build the result name here instead.
#endif

  // DQ (11/30/2007): This should be true here. We have just build a function
  // and so all the working stacks should be empty (except astScopeStack)!
  if (astTypeStack.empty() == false) {
    printf("WARNING: clearing the astTypeStack in R1224 "
           "c_action_function_stmt() \n");
    astTypeStack.clear();
  }
  ROSE_ASSERT(astTypeStack.empty() == true);
}
...
void c_action_suffix(Token_t *resultKeyword, ofp_bool hasProcLangBindSpec) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_suffix(): resultKeyword = %p = %s hasProcLangBindSpec "
           "= %s \n",
           resultKeyword, resultKeyword != NULL ? resultKeyword->text : "NULL",
...
void c_action_result_name() {
  ...
  // printf ("Warning: type for return parameter to function assumed to be
  // integer (arg_name = %s) \n",arg_name.str());
  // DQ (10/11/2014): Added extra parameter to resolve ambiguity as a result of
  // changing which are marked as constructor parameter in ROSETTA.
...
void c_action_end_function_stmt(Token_t *label, Token_t *keyword1,
                                Token_t *keyword2, Token_t *name,
                                Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In R1230 c_action_end_function_stmt(): label = %p = %s keyword1 = "
           "%p = %s keyword2 = %p = %s name = %p = %s \n",
  ...
  ROSE_ASSERT(astScopeStack.empty() == false);
  SgFunctionDefinition *functionDefinition =
      isSgFunctionDefinition(getTopOfScopeStack());

  // FMZ(6/9/2010): with "implicit none" presented, a function must have type
  // explicitly declared
  ROSE_ASSERT(functionDefinition != NULL);
  SgProcedureHeaderStatement *func_decl =
      isSgProcedureHeaderStatement(functionDefinition->get_declaration());
  ...
  if (func_decl != NULL && func_decl->isFunction() == true) {
    ...
    if (isSgTypeVoid(func_return_type) != NULL) {
      if (isAnImplicitScope == true) {
        cout << "Error: '" << func_name << "' has not been explicitly declared."
             << endl;
      } else {
        ...
        // c_action_specification_part creates an implicit result nane if
        // necessary; as a result, we need to reset the type of the result name
        // accordingly.
        func_decl->get_result_name()->set_type(
            implicit_type->get_return_type());
      }
    }
  }
 ...
 * @param hasArgSpecifier True if has anything between parentheses following
 * statement.
 */
void c_action_subroutine_stmt(Token_t *label, Token_t *keyword, Token_t *name,
                              Token_t * /*eos*/, ofp_bool hasPrefix,
                              ofp_bool hasDummyArgList, ofp_bool hasBindingSpec,
                              ofp_bool hasArgSpecifier) {
  // Support for subroutines maps to functions with void return type in the ROSE
  // AST.

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_subroutine_stmt(): label = %p (routine name) name = %s "
           "hasPrefix = %s hasDummyArgList = %s hasBindingSpec = %s "
           "hasArgSpecifier = %s \n",
           label, name ? name->text : "empty", hasPrefix ? "true" : "false",
  ...
  // We will only know the name when we get to the end? No, this appears to be a
  // bug in OFP. SgName tempName = "temp_subroutine_name";
  ROSE_ASSERT(name != NULL);
  ROSE_ASSERT(name->text != NULL);
  SgName tempName = name->text;

  // Building a void return type since a subroutine returns type void by
  // definition. However, once we see the function parameters and their type we
  // will have to update the function type!
  SgFunctionType *functionType =
      new SgFunctionType(SgTypeVoid::createType(), false);
  ...
  // This has to be done before the buildProcedureSupport() function is called
  // (must use values on the stack in a specific order).

  // hasBindingSpec should be processed before hasDummyArgList
  // Rasmussen (1/6/2019): Modified to be similar to processing of function
  // declarations
  ...
  // DQ (8/25/2010): Check if there is a proper symbol in the symbol table.
  // SgSymbol* local_symbol =
  // subroutineDeclaration->get_symbol_from_symbol_table();
...
void c_action_end_subroutine_stmt(Token_t *label, Token_t *keyword1,
                                  ...
                                  Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In R1234 c_action_end_subroutine_stmt(): label = %p = %s keyword1 "
           "= %p = %s keyword2 = %p = %s name = %p = %s \n",
           label, label != NULL ? label->text : "NULL", keyword1,
           keyword1 != NULL ? keyword1->text : "NULL", keyword2,
           keyword2 != NULL ? keyword2->text : "NULL", name,
  ...
  // DQ (11/21/2007): This is not required, and not true for subroutines in an
  // interface block. printf ("In R1234 c_action_end_subroutine_stmt():
  // topOfStack = %p = %s \n",topOfStack,topOfStack->class_name().c_str());
  // ROSE_ASSERT(topOfStack->variantT() == V_SgGlobal);
  ...
  if (astNodeStack.empty() == false) {
    // In possible anticipation of where statements we sometime leave some
    // statements on the stack, but since where statements can't span function
    // boundaries we know that we can clear them at this point.
    if (SgProject::get_verbose() > DEBUG_COMMENT_LEVEL)
      printf("In R1234 c_action_end_subroutine_stmt(): trash left on stack, "
             "clearing it astNodeStack.size() = %" PRIuPTR " \n",
             astNodeStack.size());
    astNodeStack.clear();
  }
...
void c_action_entry_stmt(Token_t *label, Token_t *keyword, Token_t *id,
                         Token_t * /*eos*/, ofp_bool hasDummyArgList,
                         ofp_bool hasSuffix) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In c_action_entry_stmt(): label = %p = %s keyword = %p = %s id = "
           "%p = %s hasDummyArgList = %s hasSuffix = %s \n",
           label, label != NULL ? label->text : "NULL", keyword,
           keyword != NULL ? keyword->text : "NULL", id,
           id != NULL ? id->text : "NULL", hasDummyArgList ? "true" : "false",
  ...
  // Unclear how we should handle the entry, since it shares only a part of the
  // function definition!
  ...
  // Reuse the current functions scope (this makes for a strange design, but
  // entry statements are a primative concept). SgFunctionDefinition*
  // functionDefinition = NULL;
  SgFunctionDefinition *functionDefinition =
      getFunctionDefinitionFromScopeStack();

  // It is an error to have the entryFunctionDefinition be defined to be the
  // functionDefinition. It is conceptually wrong, but also results in a
  // seg-fault internally in ROSE immediately after returning from OFP.  I am
  // not clear on how to define this concept of entry function definition.
  SgFunctionDefinition *entryFunctionDefinition = NULL;

  // SgEntryStatement* entryStatement = new
  // SgEntryStatement(entryFunctionName,functionType,functionDefinition);
  SgEntryStatement *entryStatement = new SgEntryStatement(
      entryFunctionName, functionType, entryFunctionDefinition);
  // Set the scope of the entryStatement (the scope is stroed explicit since
  // SgEntryStatement is derived from a SgFunctionDeclaration
  entryStatement->set_scope(functionDefinition);

  // Get the function parameter list and set its position (this might only be
  // required when the hasDummyArgList is false).
  ...
  if (hasDummyArgList == true) {
    // DQ (12/12/2010): This code is copied from buildProcedureSupport in
    // fortran_support.C
    ...
    while (astNameStack.empty() == false) {
      ...
      // Build a SgInitializedName with a SgTypeDefault and fixup the type later
      // when we see the declaration inside the procedure.
      SgInitializedName *initializedName =
          new SgInitializedName(arg_name, generateImplicitType(arg_name.str()),
                                NULL, entryStatement, NULL);
      ...
      // Now build associated SgVariableSymbol and put it into the current scope
      // (function definition scope) SgVariableSymbol* variableSymbol = new
      // SgVariableSymbol(initializedName);
  ...
  if (hasSuffix == true) {
    ...
    if (astNodeStack.empty() == false) {
      ...
      // Note that for a SgEntryStatement, since it shares a scope with an
      // existing function the symbol might already exist. Not clear how to
      // handle this case!
...
void c_action_return_stmt(Token_t *label, Token_t *keyword, Token_t * /*eos*/,
  ...
  // -Werror=unused-but-set-variable. bool
  // returningFromFunctionUsingAlternativeReturnArguments = false;
  ...
  if (hasScalarIntExpr == true) {
    ...
    // DQ (2/2/2011): Saving the return support for alternative return handling
    // until tomorrow.
    if (integerValue != NULL) {
      // This might be a function with alternative return arguments (keep
      // checking).
      ...
      if (alternativeReturnValue < 1 || alternativeReturnValue > args.size()) {
        printf("Error: alternativeReturnValue = %" PRIuPTR
               " is out of range for this function's parameter list \n",
      ...
      if (argumentInitializedName != NULL) {
        ...
        // Before we build the label reference expression delete the SgIntVal IR
        // node.
    ...
    // The source position has to be set for this case only, since it is already
    // set for where the value is on the stack (hasScalarIntExpr == true).
    setSourcePosition(returnValue);
  }

  // DQ (1/31/2010): This can't be set for both branches since it is already set
  // for any expression on the stack. setSourcePosition(returnValue);
 ...
 * @param eos End of statement token.
 */
void c_action_contains_stmt(Token_t *label, Token_t *keyword,
                            Token_t * /*eos*/) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
  ...
  // Added keyword for position information (from hirotaki, pull request #8)
  // [Rasmussen 2019.05.08]
...
void c_action_stmt_function_stmt(Token_t *label, Token_t *functionName,
                                 ...
                                 ofp_bool hasGenericNameList) {
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_stmt_function_stmt() label = %p functionName = %s "
           "hasGenericNameList = %s \n",
...
void c_action_end_of_stmt(Token_t *eos) {
  ...
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
    printf("In c_action_end_of_stmt() astNodeStack size = %" PRIuPTR
           " astExpressionStack size = %" PRIuPTR " \n",
...
#endif
{
  ...
  // DXN: We create a SgFortranIncludeLine node only when the current file is
  // not a top level file and is not an rmod file.  When parsing a top level
  // file, the astIncludeStack should be empty.
  if (!astIncludeStack.empty() && !isARoseModuleFile(filepath)) {
    // After the first time, ever call to this function is significant
    // (represents use of the Fortran include mechanism; not formally a part of
    ...
    if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
      printf("Note: Need a token to represent the filename so that we can get "
             "the position of the include statement\n");
    ...
    int lineNumberOfLastStatement =
        (astScopeStack.empty() == false)
            ? lastStatement->get_file_info()->get_line()
            : 0;

    // Increment to next line since an include must be on it's own line (as I
    // understand the Fortran standard).
    lineNumberOfLastStatement++;

    int columnNumber = 0;

    // The filename where the include is located is not the filename specified
    // with the include!
    string filenameOfIncludeLocation = getCurrentFilename();

    Sg_File_Info *fileInfo = new Sg_File_Info(
        filenameOfIncludeLocation, lineNumberOfLastStatement, columnNumber);
    ...
    // DQ (11/20/2010): Also set the end of the construct for this statement
    // (end of column position is not correct).
    ...
    // DQ (1/27/2009): Make sure that the filenames match (make sure this is a
    // STL string equality test!). This is a test for a recursive file
    // inclusion. This fails for: module_A_file.f90 or module_B_file.f90.  Also
    // for these files the call to c_action_start_of_file() is triggered by a
    // symantic handling of the use statement, not the existance of the Fortran
    // include so it should be a mistake to insert an Fortran include statement!
    // ROSE_ASSERT(includeLine->get_file_info()->get_filenameString() !=
    // string(filepath));
    ROSE_ASSERT(astScopeStack.empty() == false);
...
void c_action_end_of_file(const char * /*filename*/,
  ...
  // DQ (11/11/2010): The header file can have only a single statement and if
  // the original code has only an include then the starting line number is
  // equal to the ending line number. See test2010_81.f90 and test2010_81.h for
  // an example of this.
  // ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct()->get_line() !=
  // astScopeStack.front()->get_startOfConstruct()->get_line());
  ROSE_ASSERT(astScopeStack.front()->get_endOfConstruct()->get_line() >=
  ...
  // DQ (12/3/2010): I think that the astExpressionStack should be empty at the
  // end of a file. If this is not true then maybe it should at least be true
  // for the last file processed. For now this is just a status warning.
  if (astExpressionStack.empty() == false) {
    printf("WARNING: astExpressionStack non-empty at "
           "c_action_end_of_file(filenameString = %s) \n",
           filenameString.c_str());
  }
...
void c_action_cleanUp() {
  // This function permits memory to be cleaned up, not required for use in
  // ROSE.
  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL)
...
void c_action_coarray_spec(int count) {
  ...
  if (count != 1 || !isSgAsteriskShapeExp(astExpressionStack.front())) {
    cout << "ERROR (Rice CoArray Fortran 2.0): the co-rank specification must "
            "be an asterisk."
         << endl;
  }
...
void c_action_rice_image_selector(Token_t *team_id) {
  ...
  SgCAFCoExpression *coExpr =
      new SgCAFCoExpression(teamIdReference, rankExpr, dataExpr);
...
void c_action_rice_allocate_coarray_spec(int flag, Token_t *team_id) {
  ...
  SgVariableSymbol *teamSymbol =
      trace_back_through_parent_scopes_lookup_variable_symbol(
          teamName, astScopeStack.front());
  if (!teamSymbol) {
    if (teamName == "team_world" || teamName == "team_default")
      teamSymbol = add_external_team_decl(teamName);
    else
      cout << "FATAL ERROR: undeclared team variable '" << teamName
           << "' in allocate statement" << endl;
  }
  ...
  // -- team_id may not be available, dataExpr was not known earlier, 'reset...'
  // requires prior 'set...'
  if (teamVarRef) {
    setSourcePosition(teamVarRef);
    resetSourcePosition(teamVarRef, dataExpr);
  }
...
void c_action_rice_co_with_team_stmt(Token_t * /*label*/, Token_t *team_id) {
  ...
  SgBasicBlock *body =
      new SgBasicBlock(Sg_File_Info::generateDefaultFileInfo());
  ROSE_ASSERT(body != NULL);
  body->setCaseInsensitive(true);
  ...
  SgVariableSymbol *teamId =
      trace_back_through_parent_scopes_lookup_variable_symbol(
          team_id->text, astScopeStack.front());
  if (teamId == NULL) {
    printf("ERROR(CAF): The team selector:\"%s\" must be declared with "
           "\"Team\" statement.\n",
           team_id->text);
    ROSE_ABORT();
  }
  ...
  SgCAFWithTeamStatement *withTeam =
      new SgCAFWithTeamStatement(teamIdReference, body);
 ...
 */
void c_action_rice_end_with_team_stmt(Token_t * /*label*/,
                                      Token_t * /*team_id*/, Token_t *eos) {
  ASSERT_not_null(eos);
...
void c_action_rice_finish_stmt(Token_t *label, Token_t *teamToken,
                               Token_t *eos) {
 ...
 * @param hasWhere True if a 'processor@team' is present; if so, a processor
 * number is on expr stack.
 */
void c_action_rice_spawn_stmt(Token_t * /*label*/, Token_t *spawn,
                              Token_t * /*eos*/, ofp_bool hasEvent) {
  ...
  ROSE_ASSERT(astExpressionStack.empty() == false);
  SgFunctionCallExp *originalCallExpr =
      isSgFunctionCallExp(astExpressionStack.front());
  ROSE_ASSERT(originalCallExpr !=
              NULL); // TODO: can we get a SgFunctionRefExp instead, if no
                     // actual param list given?
  astExpressionStack.pop_front();

  SgFunctionRefExp *functionRef =
      isSgFunctionRefExp(originalCallExpr->get_function());
...
// DQ (10/1/2009): Added these functions because the NMI platforms complain
// about it being undefined. It appears that it is strict.
...
void c_action_specific_binding(Token_t *, Token_t *, Token_t *, Token_t *,
                               ofp_bool) {}
...
void c_action_next_token(Token_t *token) {
  ...
  Sg_File_Info *starting_fileInfo =
      new Sg_File_Info(currentFilename, line_number, column_number);

  // Building the token as unclassified tokens (tokens can be classified by
  // language, language construct, etc.)
  SgToken *roseToken = new SgToken(starting_fileInfo, text, 0);

  // Currently all SgLocatedNode objects are required to have a valid source
  // code position for the start and end. However this might be a bit redundant
  // for the case of SgToken objects.  So we might want to handle this
  // differently in the future.
  Sg_File_Info *ending_fileInfo = new Sg_File_Info(
      currentFilename, line_number, column_number + text.length());
  roseToken->set_endOfConstruct(ending_fileInfo);

  if (SgProject::get_verbose() > DEBUG_RULE_COMMENT_LEVEL) {
    printf("In c_action_next_token = %s file = %s line = %d column = %d \n",
           SageInterface::get_name(roseToken).c_str(), currentFilename.c_str(),
           line_number, column_number);
  }

  // Verify that we have a propoer start and end source code position for the
  // token.
