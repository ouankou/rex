...
// DQ (12/16/2007): This is associated with a bug that cause lables for the
// computed goto to be processed twice with R313
...
// Global token stack (used to hold state between the <fortran_rule>__begin()
// and the <fortran_rule>()
...
// DQ (12/8/2007): Global stack of SgType IR nodes used to hold the base type
// seperately from the the constructed types build from the base type.  This is
// designed to handle the case of "integer i(5),j" and "character*100 k,l" (see
// test2007_148.f)
extern std::list<SgType *> astBaseTypeStack;

// DQ (4/4/2008): I think that type initialization shows that we can't have a
// separate astInitializerStack See comment in fortran_support.C file.  For now
// use a macro to test this idea. Global stack of expressions used for
// initialization of variables in declarations. extern std::list<SgExpression*>
// astInitializerStack;

// DQ (11/30/2007): Function attributes are held as tokens and not not defined
// as integer value codes (like other attributes)
...
// Global stack for SgIfStmt objects (allows scopes pushed onto stack to be
// clean off back to the initial SgIfStmt)
extern std::list<SgIfStmt *> astIfStatementStack;

// DQ (11/30/2007): Actual arguments have associated names which have to be
// recorded on to a separate stack. test2007_162.h demonstrates this problems
// (and test2007_184.f)
...
// and function prototypes of function not defined in the interface we need
// attach declarations and names to the SgInterfaceStatement as they are seen.
// Since this is nt always just the last statement, it is easier to support this
// using a stack.
...
// DQ (12/29/2010): This handles a technical problem in the implementation or
// R612 and R613. Namely I need the information in R613 to be available when
// processing the fully resolved multi-part reference in R612.  I can't move the
// processing to R613, since the scope information is unavailable except from
// the list of references in the fully resolved multi-part reference. An
// alternative to this stack might be a stack of separate data-structures
// specific to the support of the R612 and R613 implementation.
...
public:
  ...
  MultipartReferenceType(
      const SgName &input_name = "",
      const bool &input_hasSelectionSubscriptList = false,
      const bool &input_hasImageSelector = false,
      const bool &input_hasCo_deref =
          false) // DXN (04/22/2011): add default values to avoid writing a
  ...
  // We need a copy constructor and the operator== to support use of this class
  // in STL.
...
// DQ (10/6/2008): the need for this is eliminated by moving some nodes that
// have source position from SgSupport to teh new SgLocatedNodeSupport subtree
// of SgLocatedNode. void setSourcePosition  ( SgRenamePair* namePair, Token_t*
// token ); void setSourcePosition  ( SgSupport* namePair, Token_t* token );
...
// SgVariableDeclaration* buildVariableDeclaration (Token_t * label, bool
// buildingImplicitVariable = false);
...
// SgVariableSymbol*
// trace_back_through_parent_scopes_lookup_variable_symbol(const
...
// void
// trace_back_through_parent_scopes_lookup_variable_symbol_but_do_not_build_variable(const
// SgName & variableName, SgScopeStatement* currentScope, SgVariableSymbol* &
// variableSymbol, SgFunctionSymbol* & functionSymbol );
...
buildDerivedTypeStatementAndDefinition(std::string name,
                                       SgScopeStatement *scope);
SgModuleStatement *buildModuleStatementAndDefinition(std::string name,
                                                     SgScopeStatement *scope);
...
extern SgAttributeSpecificationStatement *buildAttributeSpecificationStatement(
    SgAttributeSpecificationStatement::attribute_spec_enum kind, Token_t *label,
    Token_t *sourcePositionToken);

// DQ (8/28/2010): This is now generalized to support any SgDeclarationStatement
// (fix for test2010_34.f90; required for type declarations) void
// setDeclarationAttributeSpec ( SgVariableDeclaration* variableDeclaration, int
// astAttributeSpec );
...
void processBindingAttributeSupport(std::string &bind_language,
...
// DQ (1/28/2009): Added fix to avoid output of SgFortranInclude IR nodes when
// we switch file streams to read a ROSE generated *.rmod file.
...
//! Support for building implicit variables (works even when the name matches an
//! inplicit function).
...
//! Support to setting on statement (labels are found on the
//! astLabelSymbolStack).
void processLabelOnStack(SgStatement *statement);

//! This support is required to work around a bug in OFP (label == NULL in
//! R807).
...
//! This function isolates some of the support for the R612 and R613
//! implementation.
std::string generateQualifiedName(
    const std::vector<MultipartReferenceType> &qualifiedNameList);

//! Fixup for types in declarations where they could reference undeclared types
//! which will be resolved at this point.
void fixup_forward_type_declarations();

//! Generate correct type using attributes specs (operates exclusively on AST
//! stacks).
void processAttributeSpecStack(bool hasArraySpec, bool hasInitialization);

//! Common handling of multidimensional subscripts in array declarations (called
//! by R510 and R443).
