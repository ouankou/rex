...
void FortranParserState::clearStacks() {
...
bool emptyFortranStateStack() { return FortranParserState::empty(); }

// Rasmussen (7/17/2019): Added function to see if there are entries in scope
// stack
bool emptyFortranScopeStack() { return astScopeStack.empty(); }

void AttributeRec::setDeclAttrSpecs() {
  // this sets public/private because parser grammar rules bundles
  // public/private into access
  if (hasAccessSpec)
    setDeclarationAttributeSpec(declaration, accessType);

  if (isPublic)
    setDeclarationAttributeSpec(declaration, publicAttr);
  if (isPrivate)
    setDeclarationAttributeSpec(declaration, privateAttr);
  if (isAllocatable)
    setDeclarationAttributeSpec(declaration, allocatableAttr);
  if (isAsynchronous)
    setDeclarationAttributeSpec(declaration, asyncAttr);
  if (hasCodimension)
    setDeclarationAttributeSpec(declaration, codimAttr);
  if (isContiguous)
    setDeclarationAttributeSpec(declaration, contiguousAttr);
  if (hasDimension)
    setDeclarationAttributeSpec(declaration, dimAttr);
  if (isExternal)
    setDeclarationAttributeSpec(declaration, externalAttr);
  if (hasIntent)
    setDeclarationAttributeSpec(declaration, intentAttr);
  //    if (isIn) setDeclarationAttributeSpec(varDeclaration, inAttr);
  //    if (isOut) setDeclarationAttributeSpec(varDeclaration, outAttr);
  if (isIntrinsic)
    setDeclarationAttributeSpec(declaration, intrinsicAttr);
  if (hasLangBinding)
    setDeclarationAttributeSpec(declaration, bindingAttr);
  if (hasBindC)
    setDeclarationAttributeSpec(declaration, bindCAttr);
  if (isOptional)
    setDeclarationAttributeSpec(declaration, optionalAttr);
  if (hasParameter)
    setDeclarationAttributeSpec(declaration, parameterAttr);
  if (isPointer)
    setDeclarationAttributeSpec(declaration, pointerAttr);
  if (isCopointer)
    setDeclarationAttributeSpec(declaration, copointerAttr);
  if (isProtected)
    setDeclarationAttributeSpec(declaration, protectedAttr);
  if (isSave)
    setDeclarationAttributeSpec(declaration, saveAttr);
  if (isTarget)
    setDeclarationAttributeSpec(declaration, targetAttr);
  if (isCotarget)
    setDeclarationAttributeSpec(declaration, cotargetAttr);
  if (isValue)
    setDeclarationAttributeSpec(declaration, valueAttr);
  if (isVolatile)
    setDeclarationAttributeSpec(declaration, volatileAttr);
  if (isPass)
    setDeclarationAttributeSpec(declaration, passAttr);
  if (isNonOverridable)
    setDeclarationAttributeSpec(declaration, nonOverrideAttr);
  if (isDeferred)
    setDeclarationAttributeSpec(declaration, deferredAttr);
}

// The entity type follows the following ordering: pointer < copointer <
// codimension < dimension
SgType *AttributeRec::computeEntityType() {
  if (charLenExpr) {
    // convert entityType to string type with char length and the existing type
    // kind
...
SgArrayType *AttributeRec::buildArrayType(SgExprListExp *dimInfo) {
  SgExpression *sizeExpression =
      new SgNullExpression(); // this is the so-called index
...
void AttributeRec::makeBaseTypeCoArray() {
  if (!isSgArrayType(entityType)) { // build an array of rank 0 to represent a
                                    // covariable of a non-array type.
...
void AttributeRec::setDeclaration(SgDeclarationStatement *decl) {
  declaration = decl;
}
...
void AttributeRec::setHasAccessSpec(bool hasAccSpec) {
  hasAccessSpec = hasAccSpec;
}
...
// -Werror=return-type. bool AttributeRec::getIsAllocatable() { isAllocatable; }
...
void AttributeRec::setAllocatableAttr(int allocAttr) {
  allocatableAttr = allocAttr;
}
...
void AttributeRec::setHasCodimension(int hasCodim) {
  hasCodimension = hasCodim;
}
...
void AttributeRec::setExternalAttr(int externAttr) {
  externalAttr = externAttr;
}
...
void AttributeRec::setIsCotarget(bool cotargetFlag) {
  isCotarget = cotargetFlag;
}
...
void AttributeRec::setIsVolatile(bool volatileFlag) {
  isVolatile = volatileFlag;
}
...
void AttributeRec::setIsNonOverridable(bool nonOverrideFlag) {
  isNonOverridable = nonOverrideFlag;
}
...
SgExprListExp *AttributeRec::buildDimensionInfo() {
  // transfer the array/coarray spec from the astExpressionStack to this
  // AttrSpec record.
  SgIntVal *intVal = isSgIntVal(astExpressionStack.front());
  ROSE_ASSERT(intVal); // astExpressionStack.front() contains the number of
                       // array spec elements for the dimension info
  int count = intVal->get_value();
  delete intVal; // must remove it from AST
  astExpressionStack.pop_front();
  processMultidimensionalSubscriptsIntoExpressionList(
      count); // the dimension info is now on top of astExpressionStack
