 ...
 * are made available under the terms of the EPL v1.0
...
public abstract interface IFortranParserAction {
   ...
   * @param kindOrLen Specifies whether a kind or len type-param-value is
   *     present
   * @param hasAsterisk True if a '*' char-selector is specified
   */
   ...
   *   |   T_LEN
   *
   * @param attrKeyword T_POINTER, T_DIMENSION, T_ALLOCATABLE, T_KIND, or T_LEN
   *     if given; null otherwise.
   ...
   *   |   access_spec
   *
   * @param attrSpecKeyword T_POINTER, T_PASS, or T_NOPASS token if given; null
   *     otherwise.
   * @param id Identifier if present in pass.
   * @param specType "Enum" to specify type of spec: pointer, pass, nopass, or
   *     access_spec
   */
  public abstract void proc_component_attr_spec(Token attrSpecKeyword, Token id,
                                                int specType);
  ...
  public abstract void specific_binding(Token procedureKeyword,
                                        Token interfaceName, Token bindingName,
                                        Token procedureName,
   ...
   *   |   access_spec
   *
   * @param bindingAttr T_PASS, T_NOPASS, T_NON_OVERRIDABLE, T_DEFERRED, or
   *     null.
   ...
   * @param eos T_EOS token.
   */
  public abstract void enumerator_def_stmt(Token label, Token enumeratorKeyword,
                                           Token eos);
   ...
   *
   * @param hasStride True if is of the form a = b,c,d where d is optional
   *     stride
   */
   ...
   */
  public abstract void common_stmt(Token label, Token commonKeyword, Token eos,
                                   int numBlocks);
   ...
   *
   * @param hasAllocateShapeSpecList True if allocate-shape-spec-list is
   *     present.
   ...
   * R803
   * if_then_stmt
   *   :   (label)? ( T_IDENT T_COLON )? T_IF T_LPAREN expr T_RPAREN T_THEN
   * T_EOS
   *
   * @param label The label.
  ...
  public abstract void select_case_stmt(Token label, Token id,
                                        Token selectKeyword, Token caseKeyword,
                                        Token eos);
   ...
   * @param eos T_EOS token.
   */
  public abstract void select_type_stmt(Token label, Token selectConstructName,
   ...
   * @param digitString The value of the digit string, if there. Otherwise,
   *     null.
   * @param eos T_EOS token.
   ...
   * @param hasOptExpr Flag specifying if optional expression was given (if type
   *     is variable)
   */
   ...
   * @param eos T_EOS token.
   */
  public abstract void goto_stmt(Token label, Token goKeyword, Token toKeyword,
   ...
   * @param eos T_EOS token.
   * @param hasSyncStatList True if the sync_stat_list is present, false
   *     otherwise
   */
  public abstract void sync_all_stmt(Token label, Token syncKeyword,
                                     Token allKeyword, Token eos,
   ...
   */
  public abstract void unlock_stmt(Token label, Token unlockKeyword, Token eos,
                                   boolean hasSyncStatList);
   ...
   *                 position_spec_list T_RPAREN T_EOS
   *   |   ((label)? T_END T_FILE)=> (label)? T_END T_FILE file_unit_number
   ...
   * list. False is there is a file unit number.
   */
  public abstract void rewind_stmt(Token label, Token rewindKeyword, Token eos,
                                   boolean hasPositionSpecList);
   ...
   */
  public abstract void inquire_stmt(Token label, Token inquireKeyword, Token id,
   ...
   * @param eos T_EOS token.
   */
  public abstract void program_stmt(Token label, Token programKeyword, Token id,
                                    Token eos);
  ...
  public abstract void end_submodule_stmt(Token label, Token endKeyword,
                                          Token submoduleKeyword, Token name,
                                          Token eos);
   ...
   */
  public abstract void import_stmt(Token label, Token importKeyword, Token eos,
                                   boolean hasGenericNameList);
   ...
   */
  public abstract void
  procedure_declaration_stmt(Token label, Token procedureKeyword, Token eos,
                             boolean hasProcInterface, int count);
   ...
   *        end-mp-subprogram
   *
   * @param hasExecutionPart True if there is an execution-part, false
   *     otherwise.
   * @param hasInternalSubprogramPart True if there is an
   *     internal-subprogram-part.
   */
  public abstract void
  separate_module_subprogram(boolean hasExecutionPart,
                             boolean hasInternalSubprogramPart);
  ...
  public abstract void mp_subprogram_stmt(Token label, Token moduleKeyword,
                                          Token procedureKeyword, Token name,
                                          Token eos);
  ...
  /**
    ...
    * spawn_stmt
    *   :   (label)?
    *       T_SPAWN procedure_designator // includes actual parameter list and
    * cosubscript, sigh
   ...
   /**
    * next_token
    * This action is not part of the Fortran grammar.  It is used to output the
    * token stream.
    *
