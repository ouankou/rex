...
// DQ (10/11/2010): We only require this include to support the type:
// AstNameListType.
...
#define astAttributeSpecStack                                                  \
  (*(FortranParserState::getCurrentAttributeSpecStack()))
#define astInitializerStack                                                    \
  (*(FortranParserState::getCurrentInitializerStack()))
#define astTypeKindStack (*(FortranParserState::getCurrentTypeKindStack()))
#define astTypeParameterStack                                                  \
  (*(FortranParserState::getCurrentTypeParameterStack()))
#define astLabelSymbolStack                                                    \
  (*(FortranParserState::getCurrentLabelSymbolStack()))
#define astIfStatementStack                                                    \
  (*(FortranParserState::getCurrentIfStatementStack()))
#define astActualArgumentNameStack                                             \
  (*(FortranParserState::getCurrentActualArgumentNameStack()))
#define astFunctionAttributeStack                                              \
  (*(FortranParserState::getCurrentFunctionAttributeStack()))

// DQ (12/29/2010): Note that this stack is not cleared in the
// FortranParserState::clearStacks() function (is this an error).
...
#define astUnresolvedFunctionsList                                             \
  (*(FortranParserState::getCurrentUnresolvedFunctionsList()))

// DQ (12/29/2010): Added support for R612 & R613 handling.
#define astHasSelectionSubscriptStack                                          \
  (*(FortranParserState::getCurrentHasSelectionSubscriptStack()))
#define astMultipartReferenceStack                                             \
  (*(FortranParserState::getCurrentMultipartReferenceStack()))
...
// Class to record Fortran attributes in a type declaration statement or in a
...
private:
  ...
  bool hasCodimension;
  int codimAttr;
  SgExprListExp *codimExp;
  bool isContiguous;
  int contiguousAttr;
  bool hasDimension;
  int dimAttr;
  SgExprListExp *dimExp;
  bool isExternal;
  int externalAttr;
  bool hasIntent;
  int intentAttr;
...
public:
  AttributeRec()
      : declaration(NULL), entityType(NULL), charLenExpr(NULL),
        hasAccessSpec(false), accessAttr(-1), accessType(-1), isPublic(false),
        publicAttr(-1), isPrivate(false), privateAttr(-1), isAllocatable(false),
        allocatableAttr(-1), isAsynchronous(false), asyncAttr(-1),
        hasCodimension(false), codimAttr(-1), codimExp(NULL),
        isContiguous(false), contiguousAttr(-1), hasDimension(false),
        dimAttr(-1), dimExp(NULL), isExternal(false), externalAttr(-1),
        hasIntent(false), intentAttr(-1), intent(-1), isIntrinsic(false),
        intrinsicAttr(-1), hasLangBinding(false), bindingAttr(-1),
        hasBindC(false), bindCAttr(-1), isOptional(false), optionalAttr(-1),
        hasParameter(false), parameterAttr(-1), isPointer(false),
        pointerAttr(-1), isCopointer(false), copointerAttr(-1),
        ...
        isNonOverridable(false), nonOverrideAttr(-1), isDeferred(false),
        deferredAttr(-1) {}
  ...
  // Builds an SgArrayType object with the current entityType as the base type
  // and with the given dim info Pre-condition: dimInfo != NULL
  ...
  // Computes the type of the entity_decl with the computed based type and
  // accumulated attributes
  SgType *computeEntityType();
};

class FortranParserState {
  // This class supports a stack of the collections of stacks required to
  // represent the state within the translation of the OFP actions to build the
  // ROSE AST.

private:
  static stack<FortranParserState *> statesStack;

  // The declared attributes on the left-hand-side of the "::" in a type
  // declaration statement or in a derived type statement
...
public:
  static AttributeRec &getDeclAttrSpec() { return *DeclAttrSpec; }

  // DQ (7/30/2010): Added empty function to if there are entries in the stack
  static bool empty() { return statesStack.empty(); }
