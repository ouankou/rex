...
class UnparseSageVisitor {
public:
  UnparseSageVisitor(llvm::raw_ostream &out,
                     const common::LangOptions &langOpts, int indentationAmount,
      ...
      : out_{out}, langOpts_{langOpts}, indentationAmount_{indentationAmount},
        ...
        asFortran_{nullptr}, cooked_{cooked} {}
  ...
  void Unparse(/*const*/ CharSelector::LengthAndKind &x) { // R721
    Put('('), Word("KIND="), Walk(std::get<1>(x.t));
    Walk(", LEN=", std::get<0>(x.t)), Put(')');
  }
  ...
  void Unparse(/*const*/ DataComponentDefStmt &x) { // R737
    ...
    if (!attrs.empty() ||
        (!std::holds_alternative<DeclarationTypeSpec::Record>(dts.u) &&
         std::none_of(decls.begin(), decls.end(), [](const ComponentOrFill &c) {
           return common::visit(
               common::visitors{
  ...
  void Unparse(/*const*/ LanguageBindingSpec &x) { // R808 & R1528
    Word("BIND(C");
    Walk(", NAME=",
         std::get<std::optional<ScalarDefaultCharConstantExpr>>(x.t));
    if (std::get<bool>(x.t)) {
      Word(", CDEFINED");
    }
    Put(')');
  ...
  void Unparse(/*const*/ SavedEntity &x) { // R857, R858
    bool isCommon{std::get<SavedEntity::Kind>(x.t) ==
  ...
  void Unparse(/*const*/ ImportStmt &x) { // R867
    const auto importKind = std::get<common::ImportKind>(x.t);
  ...
  bool Pre(const IoControlSpec &x) { // R1213
    return common::visit(common::visitors{
                             ...
                             [&](const ErrorRecovery &) { return false; },
                         },
                         x.u);
  }
  ...
  void
  Unparse(/*const*/ format::FormatSpecification &x) { // R1302, R1303, R1305
  ...
  void
  Unparse(const format::IntrinsicTypeDataEditDesc &x) { // R1307(1/2) - R1311
    switch (x.kind) {
#define FMT(x)                                                                 \
...
#undef FMT
  ...
  void
  Unparse(/*const*/ format::DerivedTypeDataEditDesc &x) { // R1307(2/2), R1312
    Word("DT");
    if (!x.type.empty()) {
      Put('"'), Put(x.type), Put('"');
    }
    ...
    common::visit(
        common::visitors{
            ...
            [&](const CompilerDirective::VectorLength &vlength) {
              Word("!DIR$ VECTORLENGTH(");
              Walk(std::get<std::uint64_t>(vlength.t));
              switch (
                  std::get<CompilerDirective::VectorLength::Kind>(vlength.t)) {
              case CompilerDirective::VectorLength::Kind::Auto:
                break;
              case CompilerDirective::VectorLength::Kind::Fixed:
                Put(", FIXED");
                break;
              case CompilerDirective::VectorLength::Kind::Scalable:
                Put(", SCALABLE");
                break;
              }
              Put(')');
            ...
            [&](const CompilerDirective::UnrollAndJam &unroll) {
...
#include <llvm/Frontend/OpenACC/ACC.inc>
  ...
  void Unparse(const OmpDirectiveName &x) { Unparse(x.v); }
  void Unparse(const OmpArgument &x) {
    common::visit(common::visitors{
                      [&](const OmpLocator &y) { Walk(y); },
                      [&](const OmpBaseVariantNames &y) { Walk(y); },
                      [&](const OmpMapperSpecifier &y) { Walk(y); },
                      [&](const OmpReductionSpecifier &y) { Walk(y); },
                  },
                  x.u);
  }
  ...
  void Unparse(const OmpDirectiveSpecification &x) {
    Walk(x.DirName());
    if (const auto &args = std::get<std::optional<OmpArgumentList>>(x.t)) {
      Put("(");
      Walk(*args);
      Put(")");
    }
  ...
  void Unparse(const OmpTraitPropertyExtension &x) {
    common::visit(
        common::visitors{
            [&](const OmpTraitPropertyName &name) { Walk(name); },
            [&](const ScalarExpr &expr) { Walk(expr); },
            [&](const OmpTraitPropertyExtension::Complex &complex) {
              Walk(std::get<OmpTraitPropertyName>(complex.t));
              Put("(");
              Walk(std::get<std::list<
  ...
  void Unparse(const OmpObject &x) {
    common::visit(common::visitors{
                      [&](const Designator &y) { Walk(y); },
                      [&](const Name &y) { Put("/"), Walk(y), Put("/"); },
                      [&](const OmpObject::Invalid &) { Put("//"); },
                  },
                  x.u);
  }
  void Unparse(const OmpLocator &x) {
    common::visit(common::visitors{
                      [&](const OmpObject &y) { Walk(y); },
                      [&](const FunctionReference &y) { Walk(y); },
                  },
                  x.u);
  }
  ...
  void Unparse(const OmpTypeName &x) {
    common::visit(common::visitors{
                      [&](const TypeSpec &y) { Walk(y); },
                      [&](const DeclarationTypeSpec &y) { Walk(y); },
                  },
                  x.u);
  }
  ...
  void Unparse(const OmpMapperSpecifier &x) {
    const auto &mapperId = std::get<std::string>(x.t);
    if (!mapperId.empty()) {
      Word(mapperId);
      Put(":");
    }
  ...
  void Unparse(const OmpCombinerExpression &x) { Put(x.source.ToString()); }
  void Unparse(const OmpInitializerExpression &x) { Put(x.source.ToString()); }
  void Unparse(const OmpReductionSpecifier &x) {
    Walk(std::get<OmpReductionIdentifier>(x.t));
    Put(" : ");
    Walk(std::get<OmpTypeNameList>(x.t));
    if (const auto &combiner =
            std::get<std::optional<OmpCombinerExpression>>(x.t)) {
      Put(" : ");
      Walk(*combiner);
    }
  }
...
#include "llvm/Frontend/OpenMP/OMP.inc"
  ...
  void Unparse(const OpenMPSectionsConstruct &x) {
    ...
    Put("\n");
    EndOpenMP();
    for (const auto &section : std::get<std::list<OpenMPConstruct>>(x.t)) {
      Walk(section);
    }
    if (const auto &end = x.EndDir()) {
      BeginOpenMP();
      Word("!$OMP END ");
      Walk(*end);
      Put("\n");
...
#define WALK_NESTED_ENUM(CLASS, ENUM)                                          \
  ...
  WALK_NESTED_ENUM(OmpDeviceTypeClause,
                   DeviceTypeDescription)       // OMP device_type
...
#undef WALK_NESTED_ENUM
  void Unparse(const OmpTaskDependenceType::Value &x) {
    Word(common::EnumToString(x));
  }
...
private:
  ...
  template <typename A>
  void Walk(const char *prefix, const std::optional<A> &x,
            const char *suffix = "") {
    if (x) {
...
void UnparseSageVisitor::Put(char ch) {
  int sav = indent_;
...
void UnparseSageVisitor::Put(const char *str) {
  for (; *str != '\0'; ++str) {
    Put(*str);
  }
}

void UnparseSageVisitor::Put(const std::string &str) {
  for (char ch : str) {
    Put(ch);
  }
}

void UnparseSageVisitor::PutNormalized(const std::string &str) {
...
void UnparseSageVisitor::PutKeywordLetter(char ch) {
...
void UnparseSageVisitor::Word(const char *str) {
  for (; *str != '\0'; ++str) {
    PutKeywordLetter(*str);
  }
}

void UnparseSageVisitor::Word(const std::string &str) { Word(str.c_str()); }

void UnparseSageVisitor::Word(const std::string_view &str) {
...
void Unparse(llvm::raw_ostream &out, /*const*/ A &root,
             ...
             preStatementType *preStatement, parser::AllCookedSources *cooked) {
  UnparseSageVisitor visitor{
      out,          langOpts, 1, encoding, capitalizeKeywords, backslashEscapes,
      preStatement, cooked};
  Walk(root, visitor);
  visitor.Done();
}

// explicit instantiations of function template 'Unparse'
//
template void Unparse<Program>(llvm::raw_ostream &, /*const*/ Program &,
                               const common::LangOptions &, Encoding, bool,
                               bool, preStatementType *,
                               parser::AllCookedSources *);
template void Unparse<Expr>(llvm::raw_ostream &, /*const*/ Expr &,
                            const common::LangOptions &, Encoding, bool, bool,
                            preStatementType *, parser::AllCookedSources *);
