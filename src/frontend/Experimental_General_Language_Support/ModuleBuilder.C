    ...
    for (SgNode* node : namespace_symbols->get_symbols()) {
      ...
      if (!file_scope->symbol_exists(symbol_name)) {
        if (import_names.size() == 0) {
          ...
          // Only import names from the list, use lower case for comparison in case-insensitive languages.
...
void ModuleBuilder::loadSymbol(SgClassSymbol* symbol, SgSymbolTable* symbol_table, SgGlobal* file_scope)
{
  ...
  if (decl) {
    if (SgClassDeclaration* defining_decl = isSgClassDeclaration(decl->get_definingDeclaration())) {
...
SgSourceFile* ModuleBuilder::createSgSourceFile(const std::string &module_name)
{
  ...
  // current directory
  std::string module_filename = StringUtility::convertToLowerCase(module_name) + getModuleFileSuffix();
