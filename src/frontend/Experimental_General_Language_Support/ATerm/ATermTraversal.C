...
PosInfo ATermTraversal::getLocation(ATerm term) {
  PosInfo pinfo;
...
void ATermTraversal::setSourcePositions(ATerm term, rb::SourcePosition &start,
                                        rb::SourcePosition &end) {
...
void ATermTraversal::setSourcePosition(SgLocatedNode *node, PosInfo &pos) {
  ...
  node->set_startOfConstruct(
      new Sg_File_Info(filename, pos.getStartLine(), pos.getStartCol()));
  node->get_startOfConstruct()->set_parent(node);

  node->set_endOfConstruct(
      new Sg_File_Info(filename, pos.getEndLine(), pos.getEndCol()));
  node->get_endOfConstruct()->set_parent(node);

  SageInterface::setSourcePosition(node);
}

void ATermTraversal::setSourcePosition(SgLocatedNode *node,
...
void ATermTraversal::setSourcePositionFrom(SgLocatedNode *node,
                                           SgLocatedNode *fromNode) {
  PosInfo pos{};
...
void ATermTraversal::setSourcePositionFromEndOnly(SgLocatedNode *node,
                                                  SgLocatedNode *fromNode) {
  PosInfo pos{};
...
void ATermTraversal::setSourcePosition(SgLocatedNode *node, ATerm term) {
  PosInfo pos = getLocation(term);
  return setSourcePosition(node, pos);
}

void ATermTraversal::setSourcePositionExcludingTerm(SgLocatedNode *node,
                                                    ATerm startTerm,
                                                    ATerm endTerm) {
...
void ATermTraversal::setSourcePositionIncludingTerm(SgLocatedNode *node,
                                                    ATerm startTerm,
                                                    ATerm endTerm) {
  PosInfo pos = getLocation(startTerm);
  PosInfo end = getLocation(endTerm);
...
void ATermTraversal::setSourcePositionIncludingNode(SgLocatedNode *node,
                                                    ATerm startTerm,
                                                    SgLocatedNode *endNode) {
  PosInfo pos = getLocation(startTerm);
