...
static const char *description =
    "This command displays various configuration settings that are useful in "
    "user makefiles when compiling or linking a "
    "program that uses ROSE.  It does this by reading a configuration file "
    "named " CONFIG_NAME " from the directory that "
    "holdes the ROSE libraries, or from the file specified with the @s{config} "
    "switch.  This file contains blank lines, "
    "comments starting with '#' as the first non-white-space character of a "
    "line, or lines of the form \"KEY = VALUE\" "
    "where white-space around either end of the VALUE is stripped.\n\n"

    "The tool should be invoked with one positional argument: the name of the "
    "KEY whose value is to be reported. The "
    ...
    "@named{cppflags}{Shows the switches that should be passed to the C "
    "preprocessor as part of compile commands.}"

    "@named{cflags}{Shows the C compiler switches, excluding preprocessor "
    "switches, that should be used when compiling a "
    "program that uses ROSE.}"

    "@named{cxxflags}{Shows the C++ compiler switches, excluding preprocessor "
    "switches, that should be used when compiling a "
    "program that uses ROSE.}"

    "@named{ldflags}{Shows the compiler switches that should be used when "
    "linking a program that uses the ROSE library.}"

    "@named{libdirs}{Shows a colon-separated list of library directories. "
    "These are the directories that might contain shared "
    "libraries.}"

    "@named{prefix}{ROSE installation prefix. This is the name of the "
    "directory that includes \"lib\" and \"include\" "
...
struct Settings {
  ...
  // When compiling this program, LIBDIR C preprocessor symbol should be the
  // name of the installation path for libraries.
  Settings() {
    if (LIBDIR && *LIBDIR) {
      searchDirs = LIBDIR;
    }
...
static const std::vector<std::string> &requiredKeys() {
...
std::string toOldKeyFormat(std::string &key) {
  ...
  std::transform(key.begin() + 5, key.end(), std::back_inserter(oldFormat),
                 ::tolower);
  return oldFormat;
}

// Read a specific configuration file
static Configuration readConfigFile(const std::filesystem::path &configName) {
  struct Resources {
    FILE *file;
    char *line;
    size_t linesz;

    Resources() : file(NULL), line(NULL), linesz(0) {}
  ...
  if (NULL == (r.file = fopen(configName.string().c_str(), "r"))) {
    MLOG_FATAL_CXX("rose-config")
        << strerror(errno) << ": \"" << configName << "\"\n";
    exit(1);
  }
  ...
  while (ssize_t nchars = rose_getline(&r.line, &r.linesz, r.file)) {
    ...
    if (nchars < 0) {
      if (errno) {
        MLOG_FATAL_CXX("rose-config") << configName << ":" << lineNumber << ": "
                                      << strerror(errno) << "\n";
        exit(1);
      }
      break; // EOF
    ...
    std::string value =
        equal == std::string::npos ? std::string() : s.substr(equal + 1);
    ...
    if (!retval.insert(std::make_pair(key, value)).second) {
      MLOG_FATAL_CXX("rose-config")
          << configName << ":" << lineNumber << ": duplicate key \""
          << StringUtility::cEscape(key) << "\"\n";
      exit(1);
    }
  ...
  for (const std::string &key : requiredKeys()) {
    if (retval.find(key) == retval.end()) {
      MLOG_FATAL_CXX("rose-config")
          << configName << ":" << lineNumber << ": required key \"" << key
          << "\" is not defined\n";
      hadError = true;
    }
  }
...
static Configuration readConfigFile(const Settings &settings) {
  ...
  MLOG_FATAL_CXX("rose-config")
      << "cannot find file \"" << StringUtility::cEscape(CONFIG_NAME) << "\"\n";
  MLOG_FATAL_CXX("rose-config") << "searched in these directories:\n";
  for (const std::string &dir : dirs)
    MLOG_FATAL_CXX("rose-config")
        << "  \"" << StringUtility::cEscape(dir) << "\"\n";
  exit(1);
}
...
static std::string makeLibrarySearchPaths(const std::string &str) {
  ...
  std::vector<std::string> args; // A list of split out strings (from str)
  {
    ...
    const char *s =
        str.c_str(); // pointer into the current string being split out
    for (/*void*/; s && *s; ++s) { // While we haven't hit the end of str
      if ('\\' == *s &&
          s[1]) { // If we see an escaped backslash, just skip past it.
        arg += *++s;
      } else if ('\'' == *s || '"' == *s) { // If we see a quote
        if (!quoted) { // If we aren't in a quoted string, save the quote, but
                       // it doesn't go in arg
          quoted = *s;
        } else if (quoted == *s) { // If we we're in a quoted string, and the
                                   // quotes match, that's the end of the quoted
                                   // section.  Clear the quote
          quoted = '\0';
        } else { // We found a different quote than the string we're in, so just
                 // save it arg like any other char
          arg += *s;
        }
...
// FIXME[Robb P. Matzke 2015-04-14]: for now, just use C-style escaping, which
// might be good enough.
static std::string makefileEscape(const std::string &s) {
...
int main(int argc, char *argv[]) {
  ...
  // Errors
  MLOG_FATAL_CXX("rose-config")
      << "unknown key \"" << StringUtility::cEscape(key) << "\"\n";
  exit(1);
}
