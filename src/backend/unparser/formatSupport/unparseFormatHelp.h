...
public:
  ...
  // Base class for user-defined callbacks to be invoked immediately after the
  ...
  public:
    struct Args {
      ...
      Args(SgFile *fileNode, const Rose::FileSystem::Path &outputName)
          : fileNode(fileNode), outputName(outputName) {}
    };
