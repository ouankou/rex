...
void CopyUnparser::put(int c) { (*os).put(c); }
...
CopyUnparser::~CopyUnparser() { ifs.close(); }

void CopyUnparser ::Traverse(SgStatement *stmt) {
  Sg_File_Info *start = stmt->get_startOfConstruct(),
               *end = stmt->get_endOfConstruct();
  assert(start != 0);
  ...
  if (DebugCopyUnparse() && end == 0)
    cerr << "statement has no ending info " << start->get_line() << " : "
         << start->get_col() << " => ? " << endl;

  if (!is_modified(stmt) && start != 0 && end != 0) {
    if (DebugCopyUnparse())
      cerr << "statement is non-modified: " << start->get_line() << " : "
           << start->get_col() << " => " << end->get_line() << " : "
           << end->get_col() + 1 << endl;

    copylist[stmt] =
        CopyUnit(lineMap[start->get_line() - 1] + start->get_col() - 1,
    ...
    if (DebugCopyUnparse() && start != 0 && end != 0)
      cerr << "statement is modified: " << start->get_line() << " : "
           << start->get_col() << " => " << end->get_line() << " : "
           << end->get_col() << endl;
...
bool CopyUnparser::unparse_statement(SgStatement *stmt, SgUnparse_Info &info,
                                     UnparseFormat &out) {
  ...
  if (cp != copylist.end()) {
    ...
    if (DebugCopyUnparse())
      cerr << "copying from " << cur.get_start() << " until " << cur.get_end()
           << endl;
    ifs.clear();
