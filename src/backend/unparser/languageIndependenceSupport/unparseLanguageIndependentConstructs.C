...
statements_with_token_emitted_leading_preprocessing() {
...
void find_adjacent_statement_in_parent_sequence(SgStatement *target,
  ...
  if (scope->containsOnlyDeclarations()) {
    SgDeclarationStatement *decl = isSgDeclarationStatement(target);
    if (decl == nullptr) {
      return;
    }

    const SgDeclarationStatementPtrList &decl_list =
        scope->getDeclarationList();
    for (size_t idx = 0; idx < decl_list.size(); ++idx) {
      if (decl_list[idx] != decl) {
        continue;
      }

      previous_stmt = idx > 0 ? isSgStatement(decl_list[idx - 1]) : nullptr;
      next_stmt = idx + 1 < decl_list.size() ? isSgStatement(decl_list[idx + 1])
                                             : nullptr;
      return;
    }
  ...
  const SgStatementPtrList *statement_list = nullptr;
  if (SgBasicBlock *basic_block = isSgBasicBlock(scope)) {
    statement_list = &basic_block->getStatementList();
  } else if (SgForInitStatement *for_init_stmt = isSgForInitStatement(scope)) {
    statement_list = &for_init_stmt->get_init_stmt();
  }
  ...
  for (size_t idx = 0; idx < statement_list->size(); ++idx) {
    if ((*statement_list)[idx] != target) {
      continue;
    }

    previous_stmt =
        idx > 0 ? isSgStatement((*statement_list)[idx - 1]) : nullptr;
    next_stmt = idx + 1 < statement_list->size()
                    ? isSgStatement((*statement_list)[idx + 1])
                    : nullptr;
    return;
  }
}
...
bool declaration_requires_enclosing_scope_ast_fallback(
  ...
  if (isSgFunctionDeclaration(decl) != nullptr) {
    ...
    // unparsed canonically from the AST, not through a token-shell fallback
...
bool source_supports_inherited_token_surface_replay(
    const SgSourceFile *source_file) {
  return source_file != nullptr && source_file->get_unparse_tokens() &&
         source_file->get_use_token_stream_to_improve_source_position_info();
}
...
bool statement_prefers_source_span_canonical_token_mapping(
    const SgStatement *statement) {
  if (statement == nullptr) {
    return false;
  }
  ...
  if (isSgDeclarationStatement(const_cast<SgStatement *>(statement)) !=
      ...
      isSgNamespaceDefinitionStatement(const_cast<SgStatement *>(statement)) !=
          nullptr ||
      isSgClassDefinition(const_cast<SgStatement *>(statement)) != nullptr ||
      isSgFunctionDefinition(const_cast<SgStatement *>(statement)) != nullptr ||
      isSgBasicBlock(const_cast<SgStatement *>(statement)) != nullptr ||
      isSgTryStmt(const_cast<SgStatement *>(statement)) != nullptr ||
      isSgCatchOptionStmt(const_cast<SgStatement *>(statement)) != nullptr ||
...
TokenStreamSequenceToNodeMapping *lookup_statement_token_subsequence_mapping(
  ...
  auto lookup =
    ...
    extend_statement_token_interval_to_end_of_pragma_line(
        source_file, candidate, found->second);
    trim_statement_leading_boundary_tokens(source_file, candidate,
                                           found->second);
    return found->second;
  };
  auto synthesize_mapping_from_source_span =
    ...
    if (!statement_prefers_source_span_canonical_token_mapping(candidate)) {
      return nullptr;
    }
    ...
    if (SgStatement *candidate_stmt = isSgStatement(candidate)) {
      ...
      if (!is_linkage_marker) {
        ...
        auto trim_leading_preprocessing_tokens = [&]() {
          ...
          for (int idx = leading_whitespace_start;
            ...
            const int classification =
                token_vector[idx]->get_classification_code();
        ...
        auto trim_trailing_preprocessing_tokens = [&]() {
          while (trailing_whitespace_start != -1 &&
                 ...
                 token_vector[trailing_whitespace_end]
        ...
        auto trim_standalone_trailing_preprocessing_tokens = [&]() {
          ...
          for (int idx = trailing_whitespace_start;
               idx <= trailing_whitespace_end; ++idx) {
            SgToken *token = token_vector[idx];
            if (token == nullptr ||
        ...
        find_adjacent_statement_in_parent_sequence(candidate_stmt,
                                                   previous_stmt, next_stmt);
    ...
    extend_statement_token_interval_to_trailing_semicolon(source_file,
                                                          candidate, mapping);
    extend_statement_token_interval_to_end_of_pragma_line(source_file,
                                                          candidate, mapping);
    trim_statement_leading_boundary_tokens(source_file, candidate, mapping);
...
static bool isRexDebuggedProblemClassStatement(const SgStatement *statement) {
  const SgClassDeclaration *class_decl =
      isSgClassDeclaration(const_cast<SgStatement *>(statement));
  if (class_decl == nullptr) {
    return false;
  }

  const std::string name = class_decl->get_name().getString();
  return name == "sentry" || name == "_IO_FILE" || name == "__gconv_step_data";
}
...
bool inherited_partial_token_replay_requires_ast_boundary(
  ...
  const bool leading_boundary_requires_owner =
      relax_namespace_or_class_member_boundary
          ? range_contains_structural_declaration_boundary(
                token_subsequence->leading_whitespace_start,
                token_subsequence->leading_whitespace_end)
          : range_contains_nontrivial_boundary_owner(
                token_subsequence->leading_whitespace_start,
                token_subsequence->leading_whitespace_end);
...
// 'FortranCodeGeneration_locatedNode::curprint'. Factor this!
void UnparseLanguageIndependentConstructs::curprint(
  ...
  if (unp->currentFile != NULL && unp->currentFile->get_Fortran_only()) {
    ...
    bool is_free_format = unp->currentFile->get_outputFormat() ==
                          SgFile::e_free_form_output_format;
    int usable_cols =
        (is_fixed_format ? MAX_F90_LINE_LEN_FIXED
         : is_free_format
      ...
      if (is_fixed_format) {
        ...
        if (!(used_cols == 0 && str[0] != ' ')) {
          ...
          if (6 + str.size() > usable_cols)
            printf("Warning: can't wrap long line in Fortran fixed format "
                   "(continuation + text is longer than a line)\n");
        ...
        // emit free-format line continuation even if result will still be too
        // long
        unp->u_sage->curprint("&");
        unp->cur.insert_newline(1);
        unp->u_sage->curprint("&");
...
bool UnparseLanguageIndependentConstructs::statementFromFile(
  ...
  // was a "#line xx "filename"" macro that was used (to explain the
  // difference).

  ASSERT_not_null(stmt);

  bool statementInFile = false;
// Debug prints in statementFromFile are extremely verbose and slow; keep off by
// default.
#define DEBUG_STATEMENT_FROM_FILE 0
  ...
  if (includeDirectiveStatement != NULL) {
    printf("**************** Found a SgIncludeDirectiveStatement IR node: "
           "directive = %s \n",
           includeDirectiveStatement->get_directiveString().c_str());
    ROSE_ASSERT(includeDirectiveStatement->get_startOfConstruct() != NULL);
    // DQ (6/23/2020): These are no longer introduced, so they should be an
    // error.
  ...
  if (SgProject::get_unparseHeaderFilesDebug() >= 10) {
    ...
    printf(
        "   --- stmt->get_file_info()->get_fileIDsToUnparse().size() = %zu \n",
        stmt->get_file_info()->get_fileIDsToUnparse().size());
  }
  // #endif

  // FMZ (comment by DQ (11/14/2008)):
  // This is part of the support for module files in Fortran.  Modules seen in
  // the compilation of a Fortran program cause a "<module name>.rmod" file to
  // be generated. When we unparse the "*.rmod" we want to output all
  // statements, but since they came from the original fortran file (a better
  // translation would avoid this problem), the function would conclude that
  // they should not be unparsed (this fix forces the statements in a "*.rmod"
  // file to always be unparsed.  If the SgSourceFile built to represent the
  // "*.rmod" file had been constructed as a transformation then the file info
  // objects would have been marked as part of a transformation and this fix
  // would not have been required.  At some point this can be improved.  So this
  // is a fine temporary fix for now.
  if (StringUtility::fileNameSuffix(sourceFilename) == "rmod") {
    // If we are to unparse a module into the .rmod file this this is ALWAYS
    // true
    return true;
  }

  // DQ (8/7/2018): I think we can assert this (but check it). Definitely not,
  // there is at least one test of this feature in the C_tests directory.
  ...
  }
  {
...
#endif
    // DQ (10/25/2018): If we are using the unarsing of header files then we
    // require a more complex test to avoid transformations in header files
    // being unparsed in the output source (e.g. *.C) file. if
    // (isOutputInCodeGeneration == true && sourceFile->get_unparse_tokens() ==
    // true) if (isOutputInCodeGeneration == true &&
    // (sourceFile->get_unparse_tokens() == true ||
    // sourceFile->get_unparseHeaderFiles() == true)) if
    // ((isOutputInCodeGeneration == true || forceOutputOfGeneratedCode == true)
    // && (sourceFile->get_unparse_tokens() == true ||
    ...
    if ((isOutputInCodeGeneration == true ||
        ...
        (sourceFile->get_unparse_tokens() == true ||
         sourceFile->get_unparseHeaderFiles() == true)) {
      // We need to evaluate if this is a transformation which should be output,
      // since under the header file unparsing it would be output in every file
      // using the existing logic.  This is because ROSE was originally designed
      // to have only one file output and now has been modified to unparse
      // multiple files (the original input source file and posible many header
      // files as well).

      // The solution will be to match on the sourceFile name and the physical
      // file name associated with the transformation. This will mean that
      // transformations using the header file unparsing will have to explicitly
      // specify the physical file where they eill be unparsed, at least were it
      // can not be determined by surounding statement's associated physical
      // file). Note that the names are interpreted, so we need to use the
      // get_physical_file_id() functions instead.
#if DEBUG_STATEMENT_FROM_FILE
      printf("isOutputInCodeGeneration == true && "
             "sourceFile->get_unparse_tokens() == true: "
             "sourceFile->getFileName()                       = %s \n",
             sourceFile->getFileName().c_str());
      printf("isOutputInCodeGeneration == true && "
             "sourceFile->get_unparse_tokens() == true: "
             "stmt-->get_file_info()->get_physical_filename() = %s \n",
             stmt->get_file_info()->get_physical_filename().c_str());
      printf(
          "   --- sourceFile->get_file_info()->get_physical_file_id() = %d \n",
          sourceFile->get_file_info()->get_physical_file_id());
      printf(
          "   --- stmt->get_file_info()->get_physical_file_id()       = %d \n",
          stmt->get_file_info()->get_physical_file_id());
#endif

      // DQ (11/1/2018): If we are not unparsing header files then any/every
      // transformation can be output in the default file (input source file).
      // if (sourceFile->getFileName() !=
      // stmt->get_file_info()->get_physical_filename()) if
      // (sourceFile->get_file_info()->get_physical_file_id() !=
      // stmt->get_file_info()->get_physical_file_id()) if
      // (sourceFile->get_file_info()->get_physical_file_id() !=
      // stmt->get_file_info()->get_physical_file_id()) if
      // (sourceFile->get_file_info()->get_physical_file_id() !=
      // stmt->get_file_info()->get_physical_file_id() &&
      ...
      printf("sourceFile->get_unparseHeaderFiles()                = %s \n",
             sourceFile->get_unparseHeaderFiles() ? "true" : "false");
#endif
      if (sourceFile->get_file_info()->get_physical_file_id() !=
              stmt->get_file_info()->get_physical_file_id() &&
          sourceFile->get_unparseHeaderFiles() == true &&
          isCompilerGenerated == false) // isTransformation == false
      {
#if DEBUG_STATEMENT_FROM_FILE
        printf("########## Forcing isOutputInCodeGeneration == false and "
        ...
        // DQ (10/30/2018): Also for this to be false since it was required to
        // trigger the traversal of the outer most scope that are referenced by
        // header files.
        forceOutputOfGeneratedCode = false;
      }
    }
...
#if DEBUG_STATEMENT_FROM_FILE
    printf("In statementFromFile(): stmt = %p = %s isOutputInCodeGeneration   "
           "= %s \n",
           stmt, stmt->class_name().c_str(),
           isOutputInCodeGeneration ? "true" : "false");
    printf("In statementFromFile(): stmt = %p = %s forceOutputOfGeneratedCode "
           "= %s \n",
           stmt, stmt->class_name().c_str(),
           forceOutputOfGeneratedCode ? "true" : "false");
#endif
    // DQ (1/11/2006): OutputCodeGeneration is not set to be true where
    // transformations require it.  Transformation to include header files don't
    // set the OutputCodeGeneration flag. if (isOutputInCodeGeneration ||
    ...
    if ((isOutputInCodeGeneration == true ||
      ...
      // Sg_File_Info::file_id numbers so that we can remove the string
      // comparision operator. statementfilename = Rose::getFileName(stmt);

      // DQ (9/20/2013): We need to use the physical file name in checking which
      // statements to unparse. statementfilename =
...
#if DEBUG_STATEMENT_FROM_FILE
      printf("In statementFromFile(): statementfilename = %s sourceFilename = "
             "%s \n",
             statementfilename.c_str(), sourceFilename.c_str());
      printf("In statementFromFile(): "
             "stmt->get_file_info()->get_physical_filename() = %s \n",
             stmt->get_file_info()->get_physical_filename().c_str());
      printf(
          "   --- sourceFile->get_file_info()->get_physical_file_id() = %d \n",
      ...
      // DQ (12/12/2018): Adding testing for failing test in virtual function
      // analysis. ASSERT_not_null(sourceFile);
      // ASSERT_not_null(sourceFile->get_file_info());
      ASSERT_not_null(stmt);
      ASSERT_not_null(stmt->get_file_info());

      // DQ (10/31/2018): Because the filenames are interpreted, we have to use
      // the file_id values. DQ (9/20/2013): If this is a performance issue, an
      // optimization would be to use file_id's instead of strings (filenames).
...
#if DEBUG_STATEMENT_FROM_FILE
      printf(
          "In statementFromFile(): stmt->get_file_info()->isShared() = %s \n",
          stmt->get_file_info()->isShared() ? "true" : "false");
#endif
      // DQ (11/10/2019): Details apply when nodes are shared across multiple
      // files.
      if (stmt->get_file_info()->isShared() == true) {
        // Need to consult the fileIDsToUnparse to see if the current file id is
        // in the list.
        SgFileIdList &fileIdList =
            stmt->get_file_info()->get_fileIDsToUnparse();
        ROSE_ASSERT(
            fileIdList.size() ==
            stmt->get_file_info()->get_fileLineNumbersToUnparse().size());
...
#endif
        ASSERT_not_null(sourceFile);
        SgFileIdList::const_iterator pos =
            find(fileIdList.begin(), fileIdList.end(),
                 sourceFile->get_file_info()->get_physical_file_id());
        if (pos != fileIdList.end()) {
#if DEBUG_STATEMENT_FROM_FILE
          printf("In statementFromFile(): current file is sharing this IR "
                 "node: source file_id = %d stmt physical_file_id = %d \n",
                 sourceFile->get_file_info()->get_physical_file_id(),
                 stmt->get_file_info()->get_physical_file_id());
#endif
          statementInFile = true;
      ...
      // negara1 (08/15/2011): Make a special consideration for header file
      // bodies in include directive statements.
      // TODO: Change when SgIncludeDirectiveStatement is used instead of
      // attached PreprocessingInfo.
      SgIncludeDirectiveStatement *includeDirectiveStatement =
          isSgIncludeDirectiveStatement(stmt);
      if (includeDirectiveStatement != NULL) {
        // DQ (3/24/2019): The newest use of this IR nodes does not accomidate
        // the headerFileBody. if
        // (includeDirectiveStatement->get_headerFileBody()->get_file_info()->get_filenameString()
        // == sourceFilename) if
        // (includeDirectiveStatement->get_headerFileBody() != NULL &&
        // includeDirectiveStatement->get_headerFileBody()->get_file_info()->get_filenameString()
        // == sourceFilename)
        if (includeDirectiveStatement->get_headerFileBody() != NULL &&
            includeDirectiveStatement->get_headerFileBody()
                    ->get_file_info()
                    ->get_filenameString() == sourceFilename) {
          statementInFile = true;
        }
    ...
    // DQ (4/11/2021): The case of just using the unparseHeaderFiles without the
    // token needs to work as well. DQ (3/15/2021): When generating the token
    // sequence, some statements will not have a token sequence and if they are
    // not marked as transformations, then we want to skip their output. And
    // example of if ((sourceFile != NULL) && (sourceFile->get_unparse_tokens()
    // == true || sourceFile->get_unparseHeaderFiles() == true)) if ((sourceFile
    // != NULL) && (sourceFile->get_unparse_tokens() == true))
    if ((sourceFile != NULL) &&
        (sourceFile->get_unparse_tokens() == true ||
         sourceFile->get_unparseHeaderFiles() == true)) {
      // bool isOutputInCodeGeneration =
      ...
      if ((sourceFile != NULL) &&
        ...
        if (globalscope != NULL) {
#if DEBUG_STATEMENT_FROM_FILE
          printf("Found a reference to a SgGlobal while unparsing a header "
                 "file: set statementInFile = true \n");
#endif
          statementInFile = true;
        }
      }
    ...
    // DQ (12/22/2014): this is the most general way to supress the output of
    // normalized template declaration member and non-member functions. The
    // alternative approach is implemented in the
    // unparseTemplateDeclarationStatment_support() function.
    ...
    if (functionDeclaration != NULL &&
      ...
      // moveDeclarationTool/inputmoveDeclarationToInnermostScope_test2014_26.C
...
#if DEBUG_STATEMENT_FROM_FILE
    printf("In statementFromFile (statementInFile = %s output = %s stmt = %p = "
           "%s = %s in file = %s sourceFilename = %s ) \n",
           (statementInFile == true) ? "true" : "false",
           (isOutputInCodeGeneration == true) ? "true" : "false", stmt,
  ...
  if (SgProject::get_unparseHeaderFilesDebug() >= 9) {
    ...
    printf("Leaving statementFromFile(): stmt = %p = %s = %s statementInFile = "
           "%s sourceFilename = %s isDefiningDeclaration = %s \n",
           stmt, stmt->class_name().c_str(),
           SageInterface::get_name(stmt).c_str(),
           (statementInFile == true) ? "true" : "false", sourceFilename.c_str(),
  ...
  if (statementInFile == true) {
    ...
    printf("@@@@@@@@@@@@@@ Leaving statementFromFile(): stmt = %p = %s = %s "
           "statementInFile = %s \n",
           stmt, stmt->class_name().c_str(),
           SageInterface::get_name(stmt).c_str(),
           (statementInFile == true) ? "true" : "false");
  ...
  if (declarationStatement != NULL && statementInFile == false &&
      stmt->get_file_info()->isFrontendSpecific() == false) {
    curprint(
        string("\n/* Leaving of "
               "UnparseLanguageIndependentConstructs::statementFromFile (") +
        StringUtility::numberToString(stmt) + "): class_name() = " +
        stmt->class_name() + " (statementInFile == false) */ \n");
  } else {
    if (statementInFile == true) {
      curprint(
          string("\n/* Leaving of "
                 "UnparseLanguageIndependentConstructs::statementFromFile (") +
          StringUtility::numberToString(stmt) + "): class_name() = " +
          stmt->class_name() + " (statementInFile == true) */ \n");
    } else {
      curprint(
          string("\n/* Leaving of "
                 "UnparseLanguageIndependentConstructs::statementFromFile (") +
  ...
  // isSgIncludeDirectiveStatement(stmt);
  if (includeDirectiveStatement != NULL) {
    printf("BOTTOM: Found a SgIncludeDirectiveStatement IR node: directive = "
           "%s \n",
           includeDirectiveStatement->get_directiveString().c_str());
    ROSE_ASSERT(includeDirectiveStatement->get_startOfConstruct() != NULL);
...
string UnparseLanguageIndependentConstructs::getFileName(SgNode *stmt) {
  string returnString;
  ...
  if (stmt->get_file_info() != NULL) {
    ...
    // the fileinfo was not available, so check the stmt's parent for the
    // fileinfo
...
void UnparseLanguageIndependentConstructs::outputCompilerGeneratedStatements(
    SgUnparse_Info &info) {
  // DQ (3/28/2017): Eliminate warning about unused variable from Clang.
  // list<SgStatement*>::iterator i =
  // unp->compilerGeneratedStatementQueue.begin();

  // printf ("compilerGeneratedStatementQueue.size() = %" PRIuPTR "
  // \n",compilerGeneratedStatementQueue.size());
  while (info.outputCompilerGeneratedStatements() == false &&
         unp->compilerGeneratedStatementQueue.empty() == false) {
    // now output the compiler generated statements
    SgStatement *compilerGeneratedStatement =
        unp->compilerGeneratedStatementQueue.back();
...
void UnparseLanguageIndependentConstructs::unparseLineDirectives(
  ...
  if (unp->opt.get_linefile_opt() == true) {
    ...
    // There are a number of IR nodes that we don't want to have generate #line
    // directives.
    ...
    if (outputdirective == true) {
      ...
      string lineNumber = StringUtility::numberToString(
...
// functionality.  This is required to avoid premature saving of state regarding
// the static previouslyUnparsedTokenSubsequences which is required to support
// multiple statements (e.g. a variable declarations with containing multiple
// variables which translates (typically) to multiple variable declarations
// (each with one variable) within the AST). bool
// UnparseLanguageIndependentConstructs::redundantStatementMappingToTokenSequence(SgSourceFile*
// sourceFile, SgStatement* stmt)
bool UnparseLanguageIndependentConstructs::
    redundantStatementMappingToTokenSequence(SgSourceFile *sourceFile,
                                             SgStatement *stmt,
                                             SgUnparse_Info &info) {
  ASSERT_not_null(sourceFile);
  ASSERT_not_null(stmt);
  ...
  // DQ (4/18/2021): This needs to be able to support token sequences from
  // different files, currently it does not distinquish between different files
  // and they detects previously unparsed token sequences by mistake. DQ
  // (1/13/2015): Adding another mechanism to support supression of previously
  // unparsed token subsequences (required to support macros that map to
  // multiple statements). static std::set<TokenStreamSequenceToNodeMapping*>
  // previouslyUnparsedTokenSubsequences;
  static std::set<std::pair<SgSourceFile *, TokenStreamSequenceToNodeMapping *>>
      previouslyUnparsedTokenSubsequences;
...
#define DEBUG_REDUNDANT_STATEMENT_MAPPING 0
  ...
  // DQ (5/26/2021): Force returning false and debug the case of redundnat token
  // sequences afterward.
  printf("WARNING: redundantStatementMappingToTokenSequence always returns "
         "false \n");
#endif
  ...
  int counter_i = 0;
  for (std::set<SgStatement *>::iterator i = previouslySeenStatement.begin();
       i != previouslySeenStatement.end(); i++) {
    ...
    printf(" --- --- previouslySeenStatement[%d] = %p = %s name = %s \n",
           counter_i, statement, statement->class_name().c_str(),
...
#if DEBUG_REDUNDANT_STATEMENT_MAPPING
    printf("Note: In redundantStatementMappingToTokenSequence(): "
           "info.usedInUparseToStringFunction() == true: returning false \n");
#endif
    return false;
  }
  ...
  for (std::set<std::pair<SgSourceFile *, TokenStreamSequenceToNodeMapping *>>::
           iterator i = previouslyUnparsedTokenSubsequences.begin();
       i != previouslyUnparsedTokenSubsequences.end(); i++) {
    SgSourceFile *sourceFile = i->first;
    TokenStreamSequenceToNodeMapping *tokenStreamSequenceToNodeMapping =
        i->second;
    printf(" --- --- source file = %s previouslyUnparsedTokenSubsequences[%d] "
           "= %p \n",
           sourceFile->getFileName().c_str(), counter_j,
           tokenStreamSequenceToNodeMapping);

    tokenStreamSequenceToNodeMapping->display(
        "In redundantStatementMappingToTokenSequence(): token sequence");
  ...
  if (tokenSubsequence != NULL) {
    ...
    printf("Redundant statement list: lastTokenIndex          = %d \n",
           lastTokenIndex);
    printf("redundantTokenEndings.find(lastTokenIndex) != "
           "redundantTokenEndings.end() = %s \n",
           (redundantTokenEndings.find(lastTokenIndex) !=
            redundantTokenEndings.end())
               ? "true"
               : "false");
#endif

    if (redundantTokenEndings.find(lastTokenIndex) !=
        redundantTokenEndings.end()) {
#if DEBUG_REDUNDANT_STATEMENT_MAPPING || 0
      ...
      if (previouslySeenStatement.find(stmt) == previouslySeenStatement.end() &&
        ...
        printf("   ---   --- Detected first use of statment = %p = %s \n", stmt,
               stmt->class_name().c_str());
#endif
        // Add all of the redundnat statement to the previouslySeenStatement (so
        // that they will all trigger redundantStatement = true when used
        // (unparsed) later).
        while (local_iterator != last_iterator) {
          SgStatement *redundant_stmt = local_iterator->second;
#if DEBUG_REDUNDANT_STATEMENT_MAPPING
          printf("   --- redundant statement for lastTokenIndex = %d "
                 "redundant_stmt = %p = %s \n",
                 lastTokenIndex, redundant_stmt,
                 redundant_stmt->class_name().c_str());
#endif
      ...
      // DQ (4/18/2021): This needs to only be declared with the new form of the
      // static previouslyUnparsedTokenSubsequences data structure.
      std::pair<SgSourceFile *, TokenStreamSequenceToNodeMapping *>
          sourceFile_tokenSubsequence(sourceFile, tokenSubsequence);
#if DEBUG_REDUNDANT_STATEMENT_MAPPING || 0
      printf("Not found in redundantTokenEndings: lastTokenIndex = %d \n",
             lastTokenIndex);
      printf("previouslyUnparsedTokenSubsequences.find(sourceFile_"
             "tokenSubsequence) != previouslyUnparsedTokenSubsequences.end() = "
             "%s \n",
             (previouslyUnparsedTokenSubsequences.find(
                  sourceFile_tokenSubsequence) !=
              previouslyUnparsedTokenSubsequences.end())
                 ? "true"
                 : "false");
#endif
      // DQ (1/13/2015): We might need to output the last statement that has a
      // replicated token sequence, and not the first (I think).
      if (previouslyUnparsedTokenSubsequences.find(
              sourceFile_tokenSubsequence) !=
          previouslyUnparsedTokenSubsequences.end()) {
        // DQ (5/30/2021): This is the case where multiple variable declarations
        // from "int x,y;" are detected and onle one will be unparsed.
#if DEBUG_REDUNDANT_STATEMENT_MAPPING || 0
        printf("Return TRUE from redundantStatementMappingToTokenSequence(): "
               "tokenSubsequence = %p stmt = %p = %s \n",
               tokenSubsequence, stmt, stmt->class_name().c_str());
#endif
        return true;
      } else {
#if DEBUG_REDUNDANT_STATEMENT_MAPPING || 0
        printf("Record that this TokenStreamSequenceToNodeMapping data has "
               "been processed: tokenSubsequence = %p \n",
               tokenSubsequence);
#endif
...
#if DEBUG_REDUNDANT_STATEMENT_MAPPING
    printf("In redundantStatementMappingToTokenSequence(): not found in "
           "tokenStreamSequenceMap: stmt = %p = %s \n",
           stmt, stmt->class_name().c_str());
#endif
  }

#if DEBUG_REDUNDANT_STATEMENT_MAPPING
  printf("Leaving redundantStatementMappingToTokenSequence(): stmt = %p = %s "
         "redundantStatement = %s \n",
         stmt, stmt->class_name().c_str(),
...
int UnparseLanguageIndependentConstructs::
  ...
  // This function returns non-zero value if the input statement can be wholely
  // unparsed using the token stream (not partially and not from the AST).

  // TODO: it would be better to add this specific logic to the
  // canBeUnparsedFromTokenStream() function and eliminate this function.
  // TODO: Also this function is not clearly named under its revised semantics
  // (doe not actaully unparse any tokens).
...
#define DEBUG_UNPARSE_FROM_TOKENS_NODE_CONTAIN_TRANS 0
  ...
  printf("\n\nUTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-"
         "UTSNCT-UTSNCT-UTSNCT \n");
  printf("UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-UTSNCT-"
         "UTSNCT-UTSNCT-UTSNCT \n");
  ...
  if (SgProject::get_verbose() >= 1) {
    string s = "/* Unparse a partial token sequence: contains transformation: "
               "stmt = " +
               stmt->class_name() + " */ ";
    curprint(s);
    string s2 =
        string("/* --- lastStatementOfGlobalScopeUnparsedUsingTokenStream = ") +
  ...
  if (tokenSubsequence != NULL) {
    ...
    printf(
        "In unparseStatementFromTokenStreamForNodeContainingTransformation(): "
        "NO existing token sequence found for this statement \n");
  }
#endif
  ...
  printf("In unparseStatementFromTokenStreamForNodeContainingTransformation(): "
         "canBeUnparsedFromTokenStream(): unparseStatus = %s \n",
  ...
  if (unparseStatus == true) {
    // DQ (1/6/2021): Adding support to detect use of unparseToString()
    // functionality.  This is required to avoid premature saving of state
    // regarding the static previouslyUnparsedTokenSubsequences which is
    // required to support multiple statements (e.g. a variable declarations
    // with containing multiple variables which translates (typically) to
    // multiple variable declarations (each with one variable) within the AST).
    // Check if this is a previously processed statement (static map is located
    // in redundantStatementMappingToTokenSequence() function. Check if this is
    // a previously processed statement (static map is located in
    // redundantStatementMappingToTokenSequence() function. bool
    // redundantStatement =
    ...
    printf(
        "In unparseStatementFromTokenStreamForNodeContainingTransformation(): "
        "canBeUnparsedFromTokenStream(): redundantStatement = %s \n",
        redundantStatement ? "true" : "false");
    // printf
    // ("GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG
    // \n");
...
int UnparseLanguageIndependentConstructs::unparseStatementFromTokenStream(
  ...
  printf(
      " --- stmt->get_containsTransformationToSurroundingWhitespace() = %s \n",
      stmt->get_containsTransformationToSurroundingWhitespace() ? "true"
                                                                : "false");
#endif
#if DEBUG_USING_CURPRINT || 0
  curprint(
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING || 0
  printf("In unparseStatementFromTokenStream(): stmt = %p = %s unparseStatus = "
         "%s \n",
  ...
  if (unparseStatus == true) {
    ...
    // multiple variable declarations (each with one variable) within the AST).
    // Check if this is a previously processed statement (static map is located
    // in redundantStatementMappingToTokenSequence() function. bool
    // redundantStatement =
    // redundantStatementMappingToTokenSequence(sourceFile,stmt);
    bool redundantStatement =
        redundantStatementMappingToTokenSequence(sourceFile, stmt, info);
    // DQ (9/24/2018): These are always going to be redundant, since these are
    // inserted after the token stream mapping, and redundantly represent the
    // same tokens in the token stream.
    ...
    curprint(string("\n/* In unparseStatementFromTokenStream(): "
             ...
             (redundantStatement ? "true" : "false") + " */\n");
#endif
    // DQ (11/13/2015): Comment added: redundant statements are generated when
    // multiple statements in the AST are mapped to a single token sequence for
    // a single statement in the token stream.  This happens for normalizations,
    // generally, but really only in the case of a variable declaration with
    // multiple variables which in the AST is currently normalized to multiple
    // variable declaration statements, but in the token stream is the original
    // variable declaration with multiple variables.  I have worked out the fix
    // in the front-end to support mulple SgInitializedName objects in a single
    // SgVariableDeclStatement, but we have not taken the step to eliminate this
    // normalization because it would likely break code where people have grown
    // dependent on it.  So this de-normalization will have to be added as an
    // option in ROSE so that we can permit the evaluation of existing code, in
    // a staged release of a future version which will eliminate this
    // normalization.  For now we record the variable declarations that appear
    // after the first one as redundant in the token-based unparsing.
    if (redundantStatement == false) {
      ...
      printf("In unparseStatementFromTokenStream(): tokenSubsequence = %p "
             "(%d,%d) \n",
             tokenSubsequence, tokenSubsequence->token_subsequence_start,
             tokenSubsequence->token_subsequence_end);
      ...
      if (sourceFile->getFileName() ==
          "/home/quinlan1/ROSE/ROSE_GARDEN/codeSegregation/tests/sources/"
          "test_143_1.h") {
        ...
        if (SgProject::get_verbose() > -1) {
          printf(
              "In buildTokenStreamMapping(): Calling "
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING || 0
      printf("In unparseStatementFromTokenStream(): stmt = %p = %s "
             "previousAndNextFrontierDataMap.find(stmt) != "
             "previousAndNextFrontierDataMap.end() = %s \n",
             stmt, stmt->class_name().c_str(),
             previousAndNextFrontierDataMap.find(stmt) !=
                     previousAndNextFrontierDataMap.end()
                 ? "true"
      ...
      if (previousAndNextFrontierDataMap.find(mapped_statement) !=
        ...
        SgStatement *previousStatement =
            isSgStatement(previousAndNextFrontierData->previous);
        ASSERT_not_null(previousStatement);

        // This fails in the case where the whole AST is unparsed from the token
        // stream. ROSE_ASSERT(previousStatement != stmt);
        ...
        printf("previousStatement->get_file_info()->isOutputInCodeGeneration() "
               "= %s \n",
        ...
        printf("In unparseStatementFromTokenStream(): "
               "unparseStatus_previousStatement = %s \n",
               unparseStatus_previousStatement ? "true" : "false");
        printf("In unparseStatementFromTokenStream(): "
               "unparseLeadingTokenStream = %s \n",
               unparseLeadingTokenStream ? "true" : "false");
#endif
      } else {
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
        printf(
            "In unparseStatementFromTokenStream(): stmt not in "
            "previousAndNextFrontierDataMap: unparseLeadingTokenStream = %s \n",
            unparseLeadingTokenStream ? "true" : "false");
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING || 0
      printf("In unparseStatementFromTokenStream(): stmt = %p = %s "
             "unparseStatus_previousStatement = %s unparseLeadingTokenStream = "
             "%s \n",
             stmt, stmt->class_name().c_str(),
             unparseStatus_previousStatement ? "true" : "false",
             unparseLeadingTokenStream ? "true" : "false");
      ...
      if (unparseStatus_previousStatement == true ||
        ...
        printf("   --- tokenSubsequence->leading_whitespace_start = %d "
               "tokenSubsequence->leading_whitespace_end = %d \n",
               tokenSubsequence->leading_whitespace_start,
               tokenSubsequence->leading_whitespace_end);
#endif
        SgGlobal *globalScope = isSgGlobal(stmt);
        ...
        if (globalScope != NULL) {
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
          printf("Processing corner case of empty global scope unparsed using "
                 "the token stream \n");
#endif
#if DEBUG_USING_CURPRINT
          curprint(
          ...
          if (sourceFile->get_isHeaderFile() == true) {
            ...
            if (sourceFile->get_lastStatement() != NULL &&
                stmt == sourceFile->get_lastStatement()) {
              lastStatementOfGlobalScopeUnparsedUsingTokenStream = true;
#if DEBUG_USING_CURPRINT
              curprint(
                  "/* In unparseStatementFromTokenStream(): globalScope != "
          ...
          if (unparseLeadingTokenStream == true) {
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
            printf("tokenSubsequence->leading_whitespace_start != -1 && "
                   "tokenSubsequence->leading_whitespace_end != -1 = %s \n",
                   (tokenSubsequence->leading_whitespace_start != -1 &&
                    tokenSubsequence->leading_whitespace_end != -1)
                       ? "true"
                       : "false");
#endif
            if (tokenSubsequence->leading_whitespace_start != -1 &&
              ...
              for (int j = tokenSubsequence->leading_whitespace_start;
                ...
                // DQ (1/29/2014): Implementing better fedility in the unparsing
                // of tokens (avoid line ending interpretations in curprint()
                // function. Note that
                // "unp->get_output_stream().output_stream()" is of type:
                // "std::ostream*" type.
                // *(unp->get_output_stream().output_stream()) <<
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING || 0
            printf("Unparse the leading whitespace from the AST because it's "
                   "comments and/or CPP directives have been modified: stmt = "
                   "%p = %s \n",
                   stmt, stmt->class_name().c_str());
            curprint(
                "\n/* Unparse the leading whitespace from the AST because it's "
                "comments and/or CPP directives have been modified */ \n");
#endif
            unparseAttachedPreprocessingInfo(stmt, info,
                                             PreprocessingInfo::before);

#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
            curprint("\n/* DONE: Unparse the leading whitespace from the AST "
                     "because it's comments and/or CPP directives have been "
                     "modified */ \n");
#endif
          }
        }
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
        printf("In unparseStatementFromTokenStream(): DONE with leading "
               "whitespace: stmt = %p = %s \n",
               stmt, stmt->class_name().c_str());
        curprint(string("\n/* In "
                        "UnparseLanguageIndependentConstructs::"
                        "unparseStatementFromTokenStream(SgSourceFile*,,,): "
                        "DONE with leading whitespace: stmt = ") +
                 stmt->class_name().c_str() + " */\n");
#endif
#if DEBUG_USING_CURPRINT
        string s = "\n/* tokenSubsequence->token_subsequence_start = " +
        ...
        for (int j = tokenSubsequence->token_subsequence_start;
          ...
          // Note that "unp->get_output_stream().output_stream()" is of type:
          // "std::ostream*" type.
          // *(unp->get_output_stream().output_stream()) <<
          // tokenVector[j]->get_lexeme_string(); unp->get_output_stream() <<
          // tokenVector[j]->get_lexeme_string();
        ...
        // provide the precise token based output.
          ...
          if (pragma_prefix_continues) {
            ...
            *(unp->get_output_stream().output_stream()) << "\n";
            ...
            emitted_text_ends_with_newline = true;
            awaiting_pragma_suffix = false;
          } else if (token_contains_newline || !is_preprocessing_token) {
            awaiting_pragma_suffix = false;
          }
        }

#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
        printf("In unparseStatementFromTokenStream(): DONE with token output: "
               "stmt = %p = %s \n",
               stmt, stmt->class_name().c_str());
        curprint(string("\n/* In "
                        "UnparseLanguageIndependentConstructs::"
                        "unparseStatementFromTokenStream(SgSourceFile*,,,): "
                        "DONE with token output: stmt = ") +
                 stmt->class_name().c_str() + " */");
#endif
        ...
        // DQ (1/29/2014): The only consequence that I can see in not closing
        // off the trailing token stream is that we will (at least sometimes)
        // not output a trailing CR after the last line. DQ (12/1/2013): I am
        // not clear if there are cases where we need to output the associated
        // trailing tokens. None of these cases appear to be an issue in the C
        // regression tests.
...
#endif
        // DQ (5/3/2021): The settings of scope->get_containsTransformation()
        // will have to be supporting on a per source file basis to support
        // unparsing of header files using the token-based unparsing. This is
        // something to work on next, once the existing work passes the current
        // regression tests.

        // DQ (1/6/2014): Get the last statement in the global scope that has
        // valid token information. if (globalScope != NULL &&
        ...
        if (scope != NULL && scope->get_containsTransformation() == true) {
          ...
          SgStatement *lastStatement =
              SageInterface::lastStatementOfScopeWithTokenInfo(
                  scope, tokenStreamSequenceMap);

#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
          printf("computed lastStatement of scope = %p = %s stmt = %p = %s \n",
                 lastStatement, lastStatement->class_name().c_str(), stmt,
                 stmt->class_name().c_str());
#endif
        ...
        // bool unparseTrailingTokenStream =
        // unparseAttachedPreprocessingInfoUsingTokenStream(stmt,info,PreprocessingInfo::after);
      }
    } else {
    ...
    // statement. unp->get_output_stream().output_stream()->flush();
...
#if OUTPUT_TOKEN_STREAM_FOR_DEBUGGING
    printf("Leaving unparseStatementFromTokenStream(): "
...
#define DEBUG_UNPARSE_STATEMENT 0
    ...
    if (info.outputCompilerGeneratedStatements() == false &&
      ...
      // DQ (10/30/2013): This code is executed for C++ code (e.g. for
      // test2004_58.C -- template support).

      // push all compiler generated nodes onto the static stack and unparse
      // them after comments and directives of the next statement are output but
      // before the associated statement to which they are attached.
      unp->compilerGeneratedStatementQueue.push_front(stmt);

      // This return prevents this code from being trivially separated out into
      // function.
      return;
    }
    ...
    // Because there is a return in the code to accumulate the
    // compiler-generated statements the code (above) could not be refactored as
    // nicely.
...
#define OUTPUT_EMBEDDED_COLOR_CODES_FOR_STATEMENTS 0
    ...
    // DQ (7/20/2008): This mechanism is now extended to SgStatement and revised
    // to handle more cases than just replacement of the AST subtree with a
    // string.  Now we can add arbitrary text into different locations relative
    // to the specific IR node.  For now we are supporting before, replace, and
    // after.
    AstUnparseAttribute *unparseAttribute = dynamic_cast<AstUnparseAttribute *>(
        stmt->getAttribute(AstUnparseAttribute::markerName));
    ...
    // Only replace the unparsing of the IR node with a string if a string is
    // marked as AstUnparseAttribute::e_replace.
    if (unparseAttribute != nullptr &&
        unparseAttribute->replacementStringExists()) {
      string code = unparseAttribute->toString(AstUnparseAttribute::e_replace);
      curprint(code);
    } else {
#if DEBUG_USING_CURPRINT
      curprint(
          "/* In unparseStatement(): (unparseAttribute != NULL && "
          "unparseAttribute->replacementStringExists() == true) == false */");
#endif
      ...
      const bool partial_token_replay_enabled =
          ...
          (current_source_file->get_unparse_tokens() == true ||
           (isSgFunctionDeclaration(stmt) == nullptr &&
            isSgFunctionDefinition(stmt) == nullptr &&
            SageInterface::getEnclosingFunctionDefinition(stmt) == nullptr));
      const bool inherited_token_surface_replay_enabled =
          source_supports_inherited_token_surface_replay(current_source_file);
      if (partial_token_replay_enabled ||
          inherited_token_surface_replay_enabled ||
          info.unparsedPartiallyUsingTokenStream()) {
        ...
        printf("In UnparseLanguageIndependentConstructs::unparseStatement(): "
               ...
               frontier_nodes.size());
#endif
        std::map<SgStatement *, FrontierNode *>::iterator i =
        ...
        auto statement_requires_direct_partial_token_unparse =
          ...
          if (isSgDeclarationStatement(candidate) != NULL) {
            return declaration_requires_enclosing_scope_ast_fallback(
        ...
        auto function_declaration_body_contains_transformation =
          ...
          auto statement_requires_function_body_ast_replay =
            ...
            auto has_concrete_source_location =
                [](const SgStatement *candidate_stmt) -> bool {
              return candidate_stmt != NULL &&
                     candidate_stmt->get_startOfConstruct() != NULL &&
                     candidate_stmt->get_startOfConstruct()->get_line() > 0;
            };
            ...
            const bool synthetic_declaration_only =
                isSgDeclarationStatement(statement) != NULL &&
                has_concrete_source_location(statement) == false;

            if (statement->isTransformation()) {
              if (!synthetic_declaration_only) {
                return true;
              }
            }
        ...
        auto is_function_declaration_definition =
          ...
          return function_decl != NULL &&
                 function_decl->get_definition() != NULL;
        };
        ...
        auto statement_has_output_relevant_transformation =
          ...
          if (SgDeclarationStatement *decl =
            ...
            return declaration_requires_scope_ast_fallback(decl);
          }
        ...
        auto inherited_partial_token_replay_needs_leading_boundary_text =
          ...
          for (int idx = token_subsequence->leading_whitespace_start;
               idx <= token_subsequence->leading_whitespace_end; ++idx) {
            if (idx < 0 ||
                idx >= static_cast<int>(
        ...
        if (unparseViaTokenStream == true) {
...
#if DEBUG_UNPARSE_STATEMENT
          printf(
              "In unparseStatement(): DONE: unparse using FULL token stream ");
#endif
#if DEBUG_USING_CURPRINT
          ...
          if (partial_token_replay_enabled &&
            ...
            ROSE_ASSERT(partial_token_replay_enabled == false ||
            ...
            if (stmt->isTransformation() == true) {
              ...
              if (info.unparsedPartiallyUsingTokenStream()) {
                if (is_function_declaration_definition(stmt)) {
                  ...
                  // inherited partial-token state here or they fall back to a
                  // fully AST-normalized header.
                  statementUnparsedUsingTokenStream = true;
                } else {
      ...
      if (global_previous_unparsed_as == e_unparsed_as_token_stream ||
        ...
        if (global_unparsed_as == e_unparsed_as_AST) {
          ...
          if (parentStatementListBeingUnparsedUsingPartialTokenSequence ==
              true) {
            // ROSE_ABORT();
      ...
      if (outputStatementAsTokens == false) {
        ...
        if (skipBeforePreprocessingInfo == false) {
          ...
          if (cur_file != NULL && cur_file->get_unparse_tokens() == true &&
              unparseExtraNewLine == true && !info.SkipFormatting()) {
#if DEBUG_UNPARSE_STATEMENT
            // Add a new line.
            ...
            curprint("\n/* In unparseStatement(): unparseExtraNewLine == true "
                     "(unparse an extra CR) */ \n");
#endif
            // Adding a new line is more complex than it should be because the
            // the CR is interpreted and ignored if it is at the end of the
...
#if DEBUG_USING_CURPRINT
          curprint(
              "\n/* Test 2: This is where added test via AstUnparseAttribute "
              "using e_before_but_after_cpp_directives_and_comments */ \n");
#endif
          // DQ (10/4/2020): Part of new support for unparsing arbitrary strings
          // into the unparsed code.
          if (unparseAttribute != NULL) {
            string code = unparseAttribute->toString(
                AstUnparseAttribute::
                    e_before_but_after_cpp_directives_and_comments);
            curprint(code);
          }
#if DEBUG_UNPARSE_STATEMENT
      ...
      printf("   --- In unparseStatement(): outputStatementAsTokens = %s \n",
             outputStatementAsTokens == true ? "true" : "false");
      printf(
          "   --- global_previous_unparsed_as:                         %s \n",
          unparsed_as_kind(global_previous_unparsed_as).c_str());
      printf(
          "   --- global_unparsed_as:                                  %s \n",
          unparsed_as_kind(global_unparsed_as).c_str());
      printf(
          "   --- info.unparsedPartiallyUsingTokenStream()           = %s \n",
          info.unparsedPartiallyUsingTokenStream() ? "true" : "false");
      printf(
          "   --- lastStatementOfGlobalScopeUnparsedUsingTokenStream = %s \n",
          lastStatementOfGlobalScopeUnparsedUsingTokenStream ? "true"
      ...
      if (Rose::firstAndLastStatementsToUnparseInScopeMapBySourceFile.find(
        ...
        ROSE_ASSERT(
            Rose::firstAndLastStatementsToUnparseInScopeMapBySourceFile.find(
                sourceFile) !=
            Rose::firstAndLastStatementsToUnparseInScopeMapBySourceFile.end());
        ...
        // DQ (6/3/2021): Commented out because it is causing 187 failing tests
        // in the Cxx_tests directory. ROSE_ASSERT (cur_file != NULL &&
        // cur_file->get_unparse_tokens() == false);
      }
...
// Output the trailing whitespace.
...
#endif
        // DQ (2/18/2021): This is a bug in the token-based unparsing for
        // test_127.cpp in the code segregation tests. The details below are
        // copied from the top of that test code. This will need to be fixed
        // later.

        // The bug is that the #endif is not generated in the output.
        // The reason is because the statement before it is transformed
        // (literally replaced) This causes the mapping to the leading tokens
        // (before the transformed statement) to be unparsed, but the trailing
        // whitespace after the statement can't be looked up in the map because
        // the statement used as a key does not exist. Normally the leading
        // whitespace of the next statement would be used, but this transformed
        // statement is the last statement in the block.

        // Note that adding a statement after the transformed statement (so it
        // is not the last statement in the block) allows the leading whitespace
        // of the statement after the transformed statement to be found.

        // To fix this, it might be best to either not unparse the leading
        // whitespace before the transformed statement, or have a secondary key
        // that can be used to lookup the trailing whitespace of the statement
        // that was removed.  However, this is sufficiently obscure that we can
        // ignore it for the moment while I get ready to test the spnosor's code
        // in the morning.

        // Here we are either unparsing from the AST or partially from the token
        // stream (based on the AST). This is the only case were we have to
        // worry about unparsing the trailing tokens (since they case when
        // unparsing using the token stream is well handled, though not
        // refactored to be located here).
        SgScopeStatement *scope = isSgScopeStatement(stmt->get_parent());

        // ASSERT_not_null(scope);
        // Note that the parent of the global scope is not a scope, so we handle
        // this as a special case. Also the parent of a SgFunctionDefinition is
        // a SgFunctionDeclaration (also not a scope).
        SgGlobal *globalScope = isSgGlobal(stmt);
        ...
        // DQ (1/10/2015) We have added support to carry a pointer to the
        // SgSourceFile within the SgUnparse_Info. SgSourceFile* sourceFile =
        // info.get_current_source_file();

        // DQ (3/16/2021): Commented out since this fails test_16 in the
        // codeSegregation. I think this was new code, not required.
        // ROSE_ASSERT(sourceFile != NULL);
      ...
      // DQ (7/20/2008): Part of new support for unparsing arbitrary strings
      // into the unparsed code.
      if (unparseAttribute != NULL) {
    ...
    // DQ (6/5/2021): Save the previous statement that was just unparsed.
    SgUnparse_Info::set_previousStatementUnparsedFromTokenStream(
        statementUnparsedUsingTokenStream);

#if OUTPUT_DEBUGGING_FUNCTION_BOUNDARIES || 0
    printf(
        "Leaving unparse statement (%p): sage_class_name() = %s name = %s \n",
...
#define DEBUG_ROSE_2423 0
    ...
    if (expr->get_endOfConstruct() == NULL &&
        expr->get_file_info()->isTransformation() == false) {
      printf("Error in unparseExpression(): expr = %p = %s "
             "expr->get_endOfConstruct() == NULL \n",
             expr, expr->class_name().c_str());
...
#define OUTPUT_EMBEDDED_COLOR_CODES_FOR_EXPRESSIONS 0
    ...
    // if (expr->get_attribute() != NULL &&
    // expr->attribute().exists("_UnparserSourceReplacement"))

    // DQ (7/20/2008): This is now revised to handle more cases than just
    // replacement of the AST subtree with a string.  Now we can add arbitrary
    // text into different locations relative to the specific IR node.  For now
    // we are supporting before, replace, and after.

    // TV (04/22/11): More generic support of Original Expression Tree.
    // Introduce to support function pointer cast
    //     in function pointer initialization (test2006_160.C)
    SgExpression *expressionTree = NULL; // expr->get_originalExpressionTree();
    switch (expr->variantT()) {
    case V_SgVarRefExp: {
      expressionTree =
          isSgExpression(isSgVarRefExp(expr)->get_originalExpressionTree());
      break;
    }
    case V_SgCastExp: {
      expressionTree =
          isSgExpression(isSgCastExp(expr)->get_originalExpressionTree());
      break;
    }
    default: {
    ...
    AstUnparseAttribute *unparseAttribute = dynamic_cast<AstUnparseAttribute *>(
        expr->getAttribute(AstUnparseAttribute::markerName));
    if (unparseAttribute != NULL) {
      string code = unparseAttribute->toString(AstUnparseAttribute::e_before);
      curprint(code);
    }
    ...
    // marked as AstUnparseAttribute::e_replace.
    if (unparseAttribute != NULL &&
        unparseAttribute->replacementStringExists() == true) {
      string code = unparseAttribute->toString(AstUnparseAttribute::e_replace);
      curprint(code);
    } else if (expressionTree != NULL &&
               info.SkipConstantFoldedExpressions() == false) {
    ...
    // DQ (7/20/2008): Part of new support for unparsing arbitrary strings into
    // the unparsed code.
    if (unparseAttribute != NULL) {
      string code = unparseAttribute->toString(AstUnparseAttribute::e_after);
      curprint(code);
  ...
  UnparseLanguageIndependentConstructs::unparseStatementWithExternBraceTracking(
      SgStatement * stmt, SgUnparse_Info & info, size_t &extern_brace_depth,
  ...
  void UnparseLanguageIndependentConstructs::unparseGlobalStmt(
    ...
    if (sourceFile != NULL) {
      ...
      bool emitted_file_prefix_tokens = false;
      if (source_supports_partial_token_replay(sourceFile)) {
        ...
        if (first_statement != NULL) {
          const bool first_statement_starts_file_region =
              includeFile != NULL ? first_statement == firstStatement : true;
      ...
      if (sourceFile->get_unparse_tokens() == false &&
          emitted_file_prefix_tokens == false &&
          located_node_has_before_preprocessing_info(globalScope)) {
        unparseAttachedPreprocessingInfo(globalScope, info,
                                         PreprocessingInfo::before);
      }
      ...
      SgDeclarationStatementPtrList::iterator statementIterator =
          globalStatementList.begin();
      bool global_frontier_contains_ast_boundary = false;
      bool global_statement_contains_transformation = false;
      if (source_supports_inherited_token_surface_replay(sourceFile)) {
        std::map<SgStatement *, FrontierNode *> &frontier_nodes =
            sourceFile->get_token_unparse_frontier();
        for (const auto &entry : frontier_nodes) {
          FrontierNode *frontier = entry.second;
          if (frontier != NULL && frontier->unparseFromTheAST) {
            global_frontier_contains_ast_boundary = true;
            break;
          }
        }
        if (global_statement_contains_transformation == false) {
          Rose_STL_Container<SgNode *> located_nodes =
              NodeQuery::querySubTree(globalScope, V_SgLocatedNode);
          for (SgNode *node : located_nodes) {
            ...
            if (located != NULL &&
                (located->isTransformation() ||
                 ...
                 located
                     ->get_containsTransformationToSurroundingWhitespace())) {
              global_statement_contains_transformation = true;
              break;
            }
          }
        }
      ...
      const bool global_requires_partial_token_context =
          source_supports_inherited_token_surface_replay(sourceFile) &&
          (globalScope->isTransformation() ||
           ...
           global_statement_contains_transformation ||
           global_frontier_contains_ast_boundary);
      ...
      while (statementIterator != globalStatementList.end()) {
        ...
        if (sourceFile->get_unparse_tokens() == true) {
          // DQ (12/22/2014): The stl semantics are allowing NULL pointers to
          // get into the tokenStreamSequenceMap container. bool
          // found_token_data = (tokenStreamSequenceMap.find(currentStatement)
          // != tokenStreamSequenceMap.end());
          bool found_token_data =
              (tokenStreamSequenceMap.find(currentStatement) !=
               tokenStreamSequenceMap.end()) &&
          ...
          if (found_token_data == true) {
            // DQ (12/10/2016): Eliminating a warning that we want to be an
            // error: -Werror=unused-but-set-variable. last_statement =
            // currentStatement;
          }
        }
    ...
    // DQ (5/27/2005): Added support for compiler-generated statements that
    // might appear at the end of the applications printf ("At end of
    // unparseGlobalStmt \n"); outputCompilerGeneratedStatements(info);
  ...
  //--------------------------------------------------------------------------------
  int UnparseLanguageIndependentConstructs::num_stmt_in_block(SgBasicBlock *
                                                              basic_stmt) {
  ...
  bool UnparseLanguageIndependentConstructs::unparseLineReplacement(
    ...
    AttachedPreprocessingInfoType::iterator i;
    for (i = prepInfoPtr->begin(); i != prepInfoPtr->end(); ++i) {
      // i ist a pointer to the current prepInfo object, print current
      // preprocessing info Assert that i points to a valid preprocssingInfo
      // object
      ASSERT_not_null((*i));
      ROSE_ASSERT((*i)->getTypeOfDirective() !=
      ...
      // unparsed.
      infoSaysGoAhead =
          (infoSaysGoAhead == true) || (isSgExpression(stmt) != NULL);

      switch ((*i)->getTypeOfDirective()) {
      case PreprocessingInfo::LineReplacement: {
        if (isSgExpression(stmt) ==
            NULL) // Do this when line replacement matches a whole statement
  ...
  void UnparseLanguageIndependentConstructs::unparseAttachedPreprocessingInfo(
    ...
    auto normalize_preprocessing_comment = [&](const std::string &comment,
      ...
      SgStatement *statement = isSgStatement(stmt);
      const bool compact_comment_normalization_enabled =
          statement != nullptr &&
          source_requests_compact_declaration_normalization(info) &&
          is_within_transformed_declaration_scope(statement);
      std::string text = normalize_fortran_comment(comment);
      ...
      if (trimmed.rfind("//", 0) == 0) {
        ...
        if (compact_comment_normalization_enabled) {
    ...
    auto statement_preserves_original_layout = [&](SgStatement *statement) {
      if (statement == nullptr) {
        return true;
      }

      if (node_has_any_transformation_flags(statement)) {
        return false;
      }

      for (SgNode *cursor = statement->get_parent(); cursor != nullptr;
           cursor = cursor->get_parent()) {
        SgStatement *parent_statement = isSgStatement(cursor);
        SgScopeStatement *parent_scope = isSgScopeStatement(cursor);
        if (parent_statement == nullptr && parent_scope == nullptr) {
          continue;
        }

        SgNode *layout_owner = parent_statement != nullptr
                                   ? static_cast<SgNode *>(parent_statement)
                                   : static_cast<SgNode *>(parent_scope);
        if (layout_owner != statement &&
            node_has_any_transformation_flags(layout_owner)) {
          return false;
        }

        if (isSgFunctionDeclaration(cursor) != nullptr ||
            isSgFunctionDefinition(cursor) != nullptr ||
            isSgGlobal(cursor) != nullptr) {
          break;
        }
      }
    ...
    auto has_adjacent_clinkage_marker = [&](bool want_start) -> bool {
      ...
      SgStatement *previous_statement = nullptr;
      SgStatement *next_statement = nullptr;
      find_adjacent_statement_in_parent_sequence(statement, previous_statement,
                                                 next_statement);

      if (want_start) {
        if (isSgClinkageStartStatement(previous_statement) != NULL &&
            marker_output_eligible(previous_statement)) {
          return true;
        }
      } else {
        if ((isSgClinkageEndStatement(previous_statement) != NULL &&
             marker_output_eligible(previous_statement)) ||
            (isSgClinkageEndStatement(next_statement) != NULL &&
             marker_output_eligible(next_statement))) {
          return true;
        }
      }
    ...
    for (i = prepInfoPtr->begin(); i != prepInfoPtr->end(); ++i) {
      ...
      // DQ (2/27/2019): Added assertions for debugging, for
      // Cxx_tests/test2005_15.C (and many other files) this can be NULL.
      // ASSERT_not_null(info.get_current_source_file());
      bool isCommentFromCurrentFile = true;

      bool isSharedLocatedNode = (stmt->get_file_info()->isShared() == true);

      // DQ (3/12/2019): Only review the decission to reset infoSaysGoAhead if
      // it is true. if (info.get_current_source_file() != NULL) if
      // (infoSaysGoAhead == true && info.get_current_source_file() != NULL)
      if (isSharedLocatedNode == true && infoSaysGoAhead == true &&
          info.get_current_source_file() != NULL) {
        ASSERT_not_null(info.get_current_source_file()->get_file_info());

        // DQ (2/27/2019): If this is a comment from a different file (not
        // current file) then we can't unparse it here.
        isCommentFromCurrentFile =
            (info.get_current_source_file()->get_file_info()->get_file_id() ==
             (*i)->getFileId());

        if (isCommentFromCurrentFile == false) {
          // DQ (3/2/2019): so when this fails for generated comments, what does
          // the file info look like?
          // (*i)->get_file_info()->display("so when this fails for generated
          // comments, what does the file info look like");

          // DQ (3/2/2019): I will alow this for now, but it is an inappropriate
          // use of the Sg_File_Info object to define a file that does not
          // exist.
          if (((*i)->get_file_info()->get_filenameString() ==
               "Compiler-Generated in PRE") ||
              ((*i)->get_file_info()->get_filenameString() ==
               "Compiler-Generated in Finite Differencing") ||
              ((*i)->get_file_info()->isTransformation() == true)) {
            // Don't suppress the output of ROSE generated comments in this
            // case.
          } else {
      ...
      // properly, but is not sufficient for handling more complex
      // transformations. I am trying to recover the old way which
      // permitted all the test codes to work before I fix it to be
      // consistent with how it must work for the newer transformation
      // pipeline to work properly. This is work that was
        ...
        if (is_commented_pragma) {
          ...
          if (inline_block_comment) {
            ...
            const bool compact_leading_block_comment =
                compact_comment_normalization_enabled &&
                whereToUnparse == PreprocessingInfo::before &&
        ...
        if (unp->opt.get_unparse_includes_opt() == true) {
          ...
          default: {
            if ((*i)->getNumberOfLines() == 1) {
              // DQ (9/16/2013): Commented out single line CPP directives is
              // easy, so go ahead and do that. This used later style C comment
              // syntax.  This permits the user to see the original CPP
              // directives in a way that they will have no effect.
              curprint("// " + (*i)->getString());
            } else {
              // DQ (9/16/2013): Multi-line CPP directives are a bit more
              // complex so ignore them.
              curprint("/* multi-line CPP directive ignored (no robust way to "
                       "comment them out yet implemented) */\n");
            }
          }
          }
        } else {
          ...
          case PreprocessingInfo::CpreprocessorUndefDeclaration:
            if (!info.SkipCPPDirectives()) {
              if (unp->opt.get_unparse_includes_opt() == true) {
                ...
                // DQ (12/30/2013): Handle the case of a self referential macro
                // declaration e.g. "#define foo  X->foo".
                if ((*i)->getTypeOfDirective() ==
                    PreprocessingInfo::CpreprocessorDefineDeclaration) {
                  // We need to supress the output of self-referential macros
                  // since they will be expanded twice in the back-end
                  // compilation of the ROSE generated code.
                  bool isSelfReferential = (*i)->isSelfReferential();
                  if (isSelfReferential == true) {
                    // DQ (12/31/2013): We can't use /* */ to comment out the
                    // #define macro since it might also include a "/* ... */"
                    // substring. curprint(string("/* (previously processed:
                    // ignoring self-referential macro declaration) " ) +
                    // (*i)->getString() + " */\n"); curprint(string("//
                    // (previously processed: ignoring self-referential macro
                    // declaration) " ) + (*i)->getString() + " \n");
                    // DQ (1/21/2014): This has to be a C style comments for the
                    // options used in Valgrind compilation (not C99, I think).
                    curprint(string("/* (previously processed: ignoring "
                                    "self-referential macro declaration) macro "
                                    "name = ") +
                             (*i)->getMacroName() + " */ \n");
                  } else {
  ...
  void UnparseLanguageIndependentConstructs::unparseUnaryExpr(
    ...
    //
    // Flag to indicate whether the operand contains an overloaded arrow
    // operator
    //
    bool arrow_op = false;
    ...
    // Bugfix (2/26/2001) If this is for a function pointer then skip printing
    // out the operator name (for dereferencing operator)
  ...
  bool UnparseLanguageIndependentConstructs::isDotExprWithAnonymousUnion(
      SgExpression * expr) {
    // DQ (1/23/2014): This function support detecting when the supress the
    // output of the SgDotExp in the access of data members from un-named
    // unions.  Note that variables of type that are un-named unions are given a
    // unique generated name of the form "__anonymous_0x" as a prefix to the
    // pointer value of the declaration that defines the un-named union.  The
    // handling here is not specific to unions and handles any type where the
    // declaration is using a generated name of this specific form.
      ...
      if (binary_op != NULL) {
        ...
        if (varRefExp != NULL) {
          bool isAnonymousName =
              (string(varRefExp->get_symbol()->get_name()).substr(0, 14) ==
               "__anonymous_0x");
          returnValue = isAnonymousName;
        }
      } else {
        // The other case we have to handle is that the lhs is a SgVarRefExp to
        // an un-named variable.  See test2014_152.C.
...
#define DEBUG_IMPLICIT_ARROWEXP_WITHIN_LAMBDA 0
    ...
    printf("&&&&&&&&&&&&& In isImplicitArrowExpWithinLambdaFunction(): expr = "
           "%p = %s info.supressImplicitThisOperator = %s \n",
           expr, expr->class_name().c_str(),
           info.supressImplicitThisOperator() ? "true" : "false");
#endif
#if DEBUG_IMPLICIT_ARROWEXP_WITHIN_LAMBDA
    curprint(
        " /* &&&&&&&&&&&&& In isImplicitArrowExpWithinLambdaFunction() */ ");
#endif
    ...
    if (info.supressImplicitThisOperator() == true) {
      ...
      if (arrowExp != NULL) {
...
#if DEBUG_IMPLICIT_ARROWEXP_WITHIN_LAMBDA
            printf("In isImplicitArrowExpWithinLambdaFunction(): found "
                   "compiler generated this expression: thisExp = %p = %s \n",
        ...
        // DQ (11/20/2017): Added recursive step for chains of arrow operators
        // (see C++11 test2017_29.C).
        SgArrowExp *nested_arrowExp = isSgArrowExp(lhs);
        if (nested_arrowExp != NULL) {
#if DEBUG_IMPLICIT_ARROWEXP_WITHIN_LAMBDA
          printf("In isImplicitArrowExpWithinLambdaFunction(): detected nested "
                 "arrow expression: nested_arrowExp = %p = %s \n",
                 nested_arrowExp, nested_arrowExp->class_name().c_str());
#endif
          suppressOutputOfImplicitArrowExp =
              isImplicitArrowExpWithinLambdaFunction(nested_arrowExp, info);

          // DQ (2/12/2018): And if the rhs is not compiler generated then we
          // want to return false.
          SgExpression *nested_operator_rhs =
              nested_arrowExp->get_rhs_operand();
          if (nested_operator_rhs->get_file_info()->isCompilerGenerated() ==
              false) {
// #if DEBUG_ARROW_OPERATOR_CHAIN
#if DEBUG_IMPLICIT_ARROWEXP_WITHIN_LAMBDA
            printf("In isImplicitArrowExpWithinLambdaFunction(): detected "
                   "nested arrow expression: found NON compiler generated "
                   "expression: nested_operator_rhs = %p = %s \n",
                   nested_operator_rhs,
                   nested_operator_rhs->class_name().c_str());
#endif
...
#if DEBUG_IMPLICIT_ARROWEXP_WITHIN_LAMBDA
    printf("&&&&&&&&&&&&& Leaving isImplicitArrowExpWithinLambdaFunction(): "
           "expr = %p = %s suppressOutputOfImplicitArrowExp = %s \n",
           expr, expr->class_name().c_str(),
...
#define DEBUG_BINARY_OPERATORS 0
    ...
    // DQ (1/23/2014): Added better support for unparsing of data member access
    // of un-named class (structs and unions) typed variables.
    bool suppressOutputOfDotExp = isDotExprWithAnonymousUnion(expr);
    bool suppressOutputOfImplicitArrowExp =
        isImplicitArrowExpWithinLambdaFunction(expr, info);
    ...
    if (suppressOutputOfImplicitArrowExp == true && arrowExp != NULL) {
      ...
      if (memberFunctionRefExp != NULL) {
        // If this is a member function, then we will need to include that
        // reference to the calling class through the arrow operator.
        ...
        // DQ (7/24/2018): Instead of making this an exception, we should try to
        // handle this case better in test2018_85.C and test2018_130.C.
        // suppressOutputOfImplicitArrowExp = false;
      } else {
      }
    }
    ...
    printf("In unparseBinaryExpr(): suppressOutputOfDotExp                     "
           "  = %s \n",
           suppressOutputOfDotExp ? "true" : "false");
    ...
    if (possibleFunctionCall != NULL) {
      ...
      if (functionCallExp != NULL) {
        ...
        current_function_call_uses_operator_syntax =
            functionCallExp->get_uses_operator_syntax();
        current_function_call_is_compiler_generated =
            functionCallExp->isCompilerGenerated();
      }
    }
    ...
    printf("In unparseBinaryExpr(): parent_function_call_uses_operator_syntax  "
           ...
           parent_function_call_uses_operator_syntax == true ? "true"
                                                             : "false");
    printf("In unparseBinaryExpr(): parent_function_call_is_compiler_generated "
           "  = %s \n",
           parent_function_call_is_compiler_generated == true ? "true"
                                                              : "false");
    ...
    printf("In unparseBinaryExpr(): current_function_call_uses_operator_syntax "
           ...
           current_function_call_uses_operator_syntax == true ? "true"
                                                              : "false");
    printf("In unparseBinaryExpr(): "
           "current_function_call_is_compiler_generated  = %s \n",
           current_function_call_is_compiler_generated == true ? "true"
                                                               : "false");
#endif

    // DQ (2/7/2011): Unparser support for more general originalExpressionTree
    // handling.
...
#if DEBUG_BINARY_OPERATORS
      printf(
          "Found and expression tree representing a cast expression (unfolded "
          "constant expression requiring a cast) expressionTree = %p = %s \n",
    ...
    if (info.get_operator_name() == "[]" || isSgPntrArrRefExp(expr) != NULL) {
...
#if DEBUG_BINARY_OPERATORS
          printf(
              "In unparseBinaryExp(): Case 1.1.2 binary_op->get_rhs_operand() "
      ...
      // DQ (9/3/2014): Adding support to supress the output if this operators
      // in lambda functions. unparseExpression(binary_op->get_lhs_operand(),
      // info);
      if (suppressOutputOfImplicitArrowExp == false) {
        // DQ (7/12/2018): Check if this is a member function and if it is from
        // a compiler generated variable and then if "this" should be output.
        unparseExpression(binary_op->get_lhs_operand(), info);
      }
...
#if DEBUG_BINARY_OPERATORS
      printf(
          "In unparseBinaryExpr(): binary_op->get_rhs_operand()  = %p = %s \n",
          binary_op->get_rhs_operand(),
          binary_op->get_rhs_operand()->class_name().c_str());
      ...
      printf("In unparseBinaryExpr(): (after LHS) binary_op = %p = %s "
             ...
             binary_op, binary_op->class_name().c_str(),
             binary_op->isCompilerGenerated() == true ? "true" : "false");
      printf("In unparseBinaryExpr(): (after LHS) suppressOutputOfDotExp       "
             "                = %s \n",
      ...
      printf("In unparseBinaryExpr(): (after LHS) "
             ...
             parent_function_call_uses_operator_syntax == true ? "true"
                                                               : "false");
      printf("In unparseBinaryExpr(): (after LHS) "
             "parent_function_call_is_compiler_generated   = %s \n",
             parent_function_call_is_compiler_generated == true ? "true"
                                                                : "false");
      ...
      if (suppressOutputOfDotExp == false &&
        ...
        if (((current_function_call_uses_operator_syntax == false) &&
          ...
          if (is_currently_a_function_call == true) {
            // DQ (7/5/2014): Reset based on test2004_149.C (can't be set true
            // or false based on test2004_149.C).
            // ROSE_ASSERT(current_function_call_uses_operator_syntax == true);
            // ROSE_ASSERT(current_function_call_uses_operator_syntax == false);

            // DQ (7/5/2014): Reset based on test2004_142.C (can't be set true
            // or false based on test2004_149.C).
            // ROSE_ASSERT(current_function_call_is_compiler_generated ==
            // false); ROSE_ASSERT(current_function_call_is_compiler_generated
            // == true);
          }
        } else {
#if DEBUG_BINARY_OPERATORS
          ...
          // translation.
          if (is_currently_a_function_call == true) {
            // DQ (7/5/2014): Reset based on test2013_113.C (can't be set true
            // or false based on test2004_47.C).
            // ROSE_ASSERT(current_function_call_uses_operator_syntax == true);
            // ROSE_ASSERT(current_function_call_uses_operator_syntax == false);

            // DQ (7/5/2014): Reset based on test2010_04.C (can't be set true or
            // false based on test2004_47.C).
...
#if DEBUG_BINARY_OPERATORS
      printf(
          "In unparseBinaryExpr() -- before output of RHS: "
      ...
      printf("current_function_call_uses_operator_syntax   = %s \n",
             current_function_call_uses_operator_syntax ? "true" : "false");
#endif
      // DQ (7/4/2014): Consider this as a possible fix (put back so I can look
      // at this in the morning). Use
      // (current_function_call_uses_operator_syntax == false) only if the child
      // is a function call.
...
#define NEW_CASE 0
      ...
      // if (parent_function_call_uses_operator_syntax == true)
      // if (current_function_call_uses_operator_syntax == false &&
      // parent_function_call_uses_operator_syntax == true) if (
...
#endif
      {
        ...
        // DQ (7/6/2014): change this to be symetric with prefix operator
        // handling (switched back: see test2014_75.C).
        if (parent_function_call_uses_operator_syntax == true)
        // if (current_function_call_uses_operator_syntax == true)
        {
          // DQ (4/20/2013): I think we want to restrict this to the case of
          // overloaded operators for "->" (and maybe "."). test2013_114.C
          // demonstrates where just because we detect a cast we don't want to
          // skip the output of other operators. This is because the
          // introduction of the compiler generated case causes only a
          // restricted number of extra functions to be calls (e.g. SgArrowExp
          // or it's overloaded version).  I am unclear if the SgDotExp can be
          // caused to be generated by a cases (but I expect it can).

          // DQ (7/4/2014): I think that we need to check if the
          // current_function_call_uses_operator_syntax, if true then we don't
          // want to output the operator here.  This is the key point of the bug
          // associated with test2014_75.C.  This would avoid and elaborate
          // lookup from the rhs expression to see if there were to sequential
          // calls to output an overloaded operator (such as "->").

          ROSE_ASSERT(parent_function_call_uses_operator_syntax == true);
          // ROSE_ASSERT(current_function_call_uses_operator_syntax == true);

          bool isRelevantOverloadedOperator = false;
          // DQ (7/6/2014): I would like to eliminate this code to simplify the
          // handling of operator syntax.
          SgMemberFunctionRefExp *memberFunctionRefExp =
              isSgMemberFunctionRefExp(rhs);
          if (memberFunctionRefExp != NULL) {
            string functionName =
                memberFunctionRefExp->get_symbol()->get_name();
#if DEBUG_BINARY_OPERATORS
            printf("++++++++++++++++ functionName = %s \n",
                   functionName.c_str());
#endif
            if (functionName == "operator->") {
              // DQ (7/6/2014): Only allow this when it is not compiler
              // generated.
          ...
          printf("++++++++++++++++ binary_op = %p = %s "
                 "parent_function_call_uses_operator_syntax == true: search "
                 "for SgCastExp \n",
                 binary_op, binary_op->class_name().c_str());
          printf("++++++++++++++++ isRelevantOverloadedOperator = %s \n",
                 isRelevantOverloadedOperator ? "true" : "false");
#endif
          // If this is a consequence of a cast that was implicit (compiler
          // generated), then we don't want to output this operator.
          ASSERT_not_null(binary_op->get_parent());
          // TV (11/08/2018): that can be called when creating an array type
          // where the index is a call expression to a method...
          // ASSERT_not_null(binary_op->get_parent()->get_parent());

#if DEBUG_BINARY_OPERATORS
          printf("++++++++++++++++ binary_op->get_parent()->get_parent() = %p "
                 "= %s \n",
                 binary_op->get_parent()->get_parent(),
                 binary_op->get_parent()->get_parent()->class_name().c_str());
#endif
          SgCastExp *castExp =
              isSgCastExp(binary_op->get_parent()->get_parent());
          // if (castExp != NULL)
          if (castExp != NULL && isRelevantOverloadedOperator == true) {
#if DEBUG_BINARY_OPERATORS
            printf("++++++++++++++++ Found SgCastExp \n");
#endif
            if (castExp->get_startOfConstruct()->isCompilerGenerated() ==
                true) {
            ...
            if (is_currently_a_function_call == true) {
              // DQ (7/6/2014): Reset based on test2013_117.C (can't be set
              // based on test2013_117.C).
              // ROSE_ASSERT(current_function_call_uses_operator_syntax ==
              // false);

              // DQ (7/5/2014): Reset based on test2013_38.C (can't be set based
              // on test2010_04.C).
              // ROSE_ASSERT(current_function_call_is_compiler_generated ==
              // false); ROSE_ASSERT(current_function_call_is_compiler_generated
              // == true);
...
#if DEBUG_BINARY_OPERATORS
          printf("++++++++++++++++ output of the RHS 3 (non-operator syntax "
                 "used) \n");
          ...
          // translation.
          if (is_currently_a_function_call == true) {
            // DQ (7/5/2014): Reset based on test2004_149.C (can't be set
            // according to test2004_47.C).
            // ROSE_ASSERT(current_function_call_uses_operator_syntax == true);
            // ROSE_ASSERT(current_function_call_uses_operator_syntax == false);

            // DQ (7/5/2014): Reset based on test2014_75.C (can't be set
            // according to test2004_149.C).
            // ROSE_ASSERT(current_function_call_is_compiler_generated ==
            // false); ROSE_ASSERT(current_function_call_is_compiler_generated
            // == true);
          }
  ...
  bool UnparseLanguageIndependentConstructs::isRequiredOperator(
    ...
    // const
    SgType *stripped_lhs_type = lhs_type->stripType(
  ...
  void UnparseLanguageIndependentConstructs::unparseValue(
      SgExpression * expr, SgUnparse_Info & info) {
    ...
    if (expressionTree != NULL &&
      ...
      switch (valueExp->variantT()) {
        // Handle enums so that they will be unparsed as "enum name" instead of
        // as integers bool does not require special handling. case
        // V_SgBoolValExp:             { unparseBoolVal(expr, info); break; }
      case V_SgEnumVal: {
  ...
  void UnparseLanguageIndependentConstructs::unparseSCharVal(
...
#define DEBUG_UNPARSE_ENUM_VAL 0
    ...
    // todo: optimize this so that the qualified name is only printed when
    // necessary.
    if (info.inEnumDecl() == true) {
      // The enum value output in the enum declaration should be a value (it
      // could be an enum constant if it has already been output as a value
      // (repreated reference) but this is an obsure detail).
      ...
      if (enum_val->get_declaration() != NULL) {
        ...
        if (SageInterface::is_Cxx_language() == true) {
          ...
          if (enum_val->get_name().is_null() == true) {
            printf("If the enum name does not exist, then we can't qualify it "
                   "nameQualifier = %s (reset) \n",
                   (nameQualifier.is_null() == false) ? nameQualifier.str()
                                                      : "NULL");
            nameQualifier = "";
        ...
        printf("Warning in Unparser::unparseEnumVal(): no associated enum "
               "declaration specificed for enum value = %s \n",
      ...
      // DQ (11/4/2012): Detect if this is a generated name with an cast from an
      // un-named type (we can't output these names).
      ...
      // DQ (11/4/2012): Never output a generated name since it will cause link
      // failures (and frequently also interpretation as implicit function calls
      // in C). if (enum_val->get_name().is_null() == false)
...
#if DEBUG_UNPARSE_ENUM_VAL
    printf(
        "Leaving Unparse_ExprStmt::unparseEnumVal: info.inEnumDecl() = %s \n",
        info.inEnumDecl() ? "true" : "false");
#endif
  }

  void UnparseLanguageIndependentConstructs::unparseIntVal(
      SgExpression * expr, SgUnparse_Info & info) {
  ...
  void UnparseLanguageIndependentConstructs::unparseULongLongIntVal(
      SgExpression * expr, SgUnparse_Info & info) {
    SgUnsignedLongLongIntVal *ulonglongint_val =
  ...
  void UnparseLanguageIndependentConstructs::unparseFloatVal(
    ...
    if (float_value == std::numeric_limits<float>::infinity()) {
      // Because of Fortran kind (compiler dependent) the string literal may be
      // a double. Thus it makes more sense to print the original string literal
      // [Rasmussen 4/28/2019].
      if (SageInterface::is_Fortran_language() &&
          float_val->get_valueString().length() > 0) {
...
#define DEBUG__isAssociatedWithCxx11_initializationList 0
  static bool isAssociatedWithCxx11_initializationList(
      SgConstructorInitializer * con_init) {
...
#define DEBUG__unparseExprList 0
    ...
    if (i != expr_list->get_expressions().end()) {
      while (i != expr_list->get_expressions().end()) {
        ...
        printf("    argument_type = %p = %s \n", argument_type,
               argument_type->class_name().c_str());
#endif
        ...
        if (ctor_init != NULL) {
          ...
          bool this_constructor_initializer_is_using_Cxx11_initializer_list =
              isAssociatedWithCxx11_initializationList(ctor_init);
#if DEBUG__unparseExprList
          printf("    "
                 "this_constructor_initializer_is_using_Cxx11_initializer_list "
                 "= %s \n",
                 this_constructor_initializer_is_using_Cxx11_initializer_list
                     ? "true"
                     : "false");
...
#endif
          if (this_constructor_initializer_is_using_Cxx11_initializer_list)
            needParen = false;
  ...
  void UnparseLanguageIndependentConstructs::unparseIncludeDirectiveStatement(
      SgStatement * stmt, SgUnparse_Info & info) {
    SgIncludeDirectiveStatement *directive =
        isSgIncludeDirectiveStatement(stmt);
    ASSERT_not_null(directive);

    // negara1 (08/15/2011): Do not unparse the directive string as long as
    // there are PreprocessorInfos attached to nodes in order to avoid double
    // unparsing. Also, note that it might belong to a different file since the
    // code can reach here when the include directive statement's header file
    // body belongs to the unparsed file.
    // curprint(directive->get_directiveString());
    // unp->cur.insert_newline(1);

    ASSERT_not_null(info.get_current_source_file());
    bool usingTokenUnparsing =
        info.get_current_source_file()->get_unparse_tokens();

    printf("In unparseIncludeDirectiveStatement: usingTokenUnparsing = %s \n",
           usingTokenUnparsing ? "true" : "false");

    // DQ (12/1/2018): This code may be required when unparsing using the header
    // file unparsing support but not using the token unparsing.
    if (usingTokenUnparsing == false) {
      // DQ (11/13/2018): I want to try to turn this off as a tst for test9 in
      // UnparseHeader_tests. DQ (10/31/2018): This might be a better approach
      // that I first realized (since the header file can contain just another
      ...
      printf("In unparseIncludeDirectiveStatement(): headerFileBody = %p \n",
             headerFileBody);
      // DQ (3/24/2019): The newest use of this IR nodes does not accomidate the
      // headerFileBody. ASSERT_not_null(headerFileBody);
      if (headerFileBody != NULL) {
        if (headerFileBody->get_file_info()->get_filenameString() ==
            getFileName()) {
          unparseAttachedPreprocessingInfo(
              headerFileBody, info,
              PreprocessingInfo::after); // Its always "after" if attached to a
                                         // header file body.
        }
      } else {
        // DQ (3/24/2019): The newest use of this IR nodes does not accomidate
  ...
  void UnparseLanguageIndependentConstructs::unparseWarningDirectiveStatement(
      SgStatement * stmt, SgUnparse_Info & info) {
    SgWarningDirectiveStatement *directive =
        isSgWarningDirectiveStatement(stmt);
    ASSERT_not_null(directive);
    curprint("\n ");
  ...
  void
  UnparseLanguageIndependentConstructs::unparseIncludeNextDirectiveStatement(
      SgStatement * stmt, SgUnparse_Info & info) {
  ...
  void
  UnparseLanguageIndependentConstructs::unparseLinemarkerDirectiveStatement(
      SgStatement * stmt, SgUnparse_Info & info) {
  ...
  void UnparseLanguageIndependentConstructs::unparseOmpAllocatorClause(
    ...
    if (modifier != SgOmpClause::e_omp_allocator_modifier_unknown) {
      if (modifier == SgOmpClause::e_omp_allocator_user_defined_modifier) {
        ...
        unparseExpression(
            isSgOmpAllocatorClause(c)->get_user_defined_modifier(), new_info);
      } else {
        switch (modifier) {
        ...
        default: {
          cerr << "Error: "
                  "UnparseLanguageIndependentConstructs::"
                  "unparseOmpDefaultClause() meets unacceptable default option "
                  "value:"
               << endl;
          ROSE_ABORT();
        }
  ...
  void
  UnparseLanguageIndependentConstructs::unparseOmpAtomicDefaultMemOrderClause(
      SgOmpClause * clause, SgUnparse_Info & info) {
    ROSE_ASSERT(clause != NULL);
  ...
  static std::string reductionIdentifierToString(
      SgOmpClause::omp_reduction_identifier_enum ro) {
    string result;
    switch (ro) {
  ...
  static std::string taskReductionIdentifierToString(
    ...
    default: {
      cerr
          << "Error: unhandled operator type taskReductionIdentifierToString():"
          << ro << endl;
      ROSE_ABORT();
    }
    }
  ...
  static std::string reductionModifierToString(
      SgOmpClause::omp_reduction_modifier_enum rm) {
    string result;
    switch (rm) {
  ...
  static std::string lastprivateModifierToString(
      SgOmpClause::omp_lastprivate_modifier_enum rm) {
    string result = "";
    switch (rm) {
  ...
  static std::string scheduleKindToString(
      SgOmpClause::omp_schedule_kind_enum rm) {
    string result = "";
    switch (rm) {
  ...
  static std::string distScheduleKindToString(
      SgOmpClause::omp_dist_schedule_kind_enum rm) {
    string result = "";
    switch (rm) {
  ...
  static std::string defaultmapBehaviorToString(
      SgOmpClause::omp_defaultmap_behavior_enum rm) {
    string result = "";
    switch (rm) {
  ...
  static std::string defaultmapCategoryToString(
      SgOmpClause::omp_defaultmap_category_enum rm) {
    string result = "";
    switch (rm) {
  ...
  static std::string linearModifierToString(
      SgOmpClause::omp_linear_modifier_enum rm) {
    string result = "";
    switch (rm) {
  ...
  static std::string allocateModifierToString(
  ...
  static std::string usesAllocatorsAllocatorToString(
    ...
    default: {
      cerr
          << "Error: unhandled operator type usesAllocatorsAllocatorToString():"
          << allocator << endl;
      ROSE_ABORT();
    }
    }
  ...
  // TODO put into a better place and expose it to users.
  static std::string dependenceTypeToString(
      SgOmpClause::omp_dependence_type_enum ro) {
    string result;
    switch (ro) {
    ...
    default: {
      cerr << "Error: unhandled operator type" << __func__ << "():" << ro
           << endl;
      ROSE_ABORT();
    }
    }
  ...
  static std::string dependModifierToString(
      SgOmpClause::omp_depend_modifier_enum ro) {
    string result;
    switch (ro) {
  ...
  static std::string affinityModifierToString(
      SgOmpClause::omp_affinity_modifier_enum ro) {
    string result;
    switch (ro) {
  ...
  static std::string mapOperatorToString(
  ...
  static std::string distPolicyToString(
      SgOmpClause::omp_map_dist_data_enum ro) {
    string result;
    switch (ro) {
  ...
  void UnparseLanguageIndependentConstructs::unparseMapDistDataPoliciesToString(
    ...
    for (size_t i = 0; i < policies.size(); i++) {
      ...
      if (policies[i].second != NULL) {
        ...
        curprint(string(")"));
        //      cerr<<"Error: unhandled dist_data policy with expression option
        //      in mapDistDataPoliciesToString() of
        //      unparseLanguageIndependentConstructs.C"<<endl; ROSE_ABORT();
      }
  ...
  void UnparseLanguageIndependentConstructs::unparseOmpVariablesClause(
    ...
    case V_SgOmpReductionClause: {
      ...
      SgOmpClause::omp_reduction_modifier_enum modifier =
          isSgOmpReductionClause(c)->get_modifier();
    ...
    case V_SgOmpInReductionClause: {
      ...
      SgOmpClause::omp_in_reduction_identifier_enum identifier =
          isSgOmpInReductionClause(c)->get_identifier();
      if (identifier !=
        ...
        SgUnparse_Info new_info(info);
        unparseExpression(
            isSgOmpInReductionClause(c)->get_user_defined_identifier(),
            new_info);
      };
      curprint(string(" : "));
      break;
    ...
    case V_SgOmpTaskReductionClause: {
      ...
      SgOmpClause::omp_task_reduction_identifier_enum identifier =
          isSgOmpTaskReductionClause(c)->get_identifier();
      if (identifier !=
        ...
        SgUnparse_Info new_info(info);
        unparseExpression(
            isSgOmpTaskReductionClause(c)->get_user_defined_identifier(),
            new_info);
      };
      curprint(string(" : "));
      break;
    ...
    case V_SgOmpDependClause: {
      ...
      if (isSgOmpDependClause(c)->get_depend_modifier()) {
        curprint(dependModifierToString(
            isSgOmpDependClause(c)->get_depend_modifier()));
        curprint(string(" ( "));
        SgOmpDependClause *d_clause = isSgOmpDependClause(clause);
        std::list<std::list<SgExpression *>> depend_iterators_definition_class =
            d_clause->get_iterator();
        std::list<std::list<SgExpression *>>::iterator iter;
        SgUnparse_Info ninfo(info);
        for (iter = depend_iterators_definition_class.begin();
             iter != depend_iterators_definition_class.end(); iter++) {
          std::list<SgExpression *> depend_iterators_definition = (*iter);
          std::list<SgExpression *>::iterator iter1;
          int count = 0;
          if (iter != depend_iterators_definition_class.begin())
            curprint(string(" , "));
          for (iter1 = depend_iterators_definition.begin();
               iter1 != depend_iterators_definition.end(); iter1++) {
      ...
      curprint(dependenceTypeToString(
          isSgOmpDependClause(c)->get_dependence_type()));
    ...
    case V_SgOmpAffinityClause: {
      ...
      if (isSgOmpAffinityClause(c)->get_affinity_modifier()) {
        curprint(affinityModifierToString(
            isSgOmpAffinityClause(c)->get_affinity_modifier()));
        curprint(string(" ( "));
        SgOmpAffinityClause *d_clause = isSgOmpAffinityClause(clause);
        std::list<std::list<SgExpression *>>
            affinity_iterators_definition_class = d_clause->get_iterator();
        std::list<std::list<SgExpression *>>::iterator iter;
        SgUnparse_Info ninfo(info);
        for (iter = affinity_iterators_definition_class.begin();
             iter != affinity_iterators_definition_class.end(); iter++) {
          std::list<SgExpression *> affinity_iterators_definition = (*iter);
          std::list<SgExpression *>::iterator iter1;
          int count = 0;
          if (iter != affinity_iterators_definition_class.begin())
            curprint(string(" , "));
          for (iter1 = affinity_iterators_definition.begin();
               iter1 != affinity_iterators_definition.end(); iter1++) {
            SgExpression *tmp = (*iter1);
            if (count == 0 && tmp != NULL) {
              unparseExpression(tmp, ninfo);
              curprint(string(" "));
    ...
    if (is_from) {
      ...
      ROSE_ASSERT(m_clause != NULL);
      dims = m_clause->get_array_dimensions();
    } else if (is_depend) // task depend(A[i:BS][j:BS]) , is also stored as
                          // array section.
                          // TODO: long term, we need a dedicated array section
                          // AST node
    {
    ...
    if (isSgOmpDependClause(c) &&
      ...
      for (iter = vec_list.begin(); iter != vec_list.end(); iter++) {
        ...
        count_for_vec++;
        if (count_for_vec < vec_list.size())
          curprint(",");
      }
    }
  ...
  void UnparseLanguageIndependentConstructs::unparseOmpExpressionClause(
    ...
    if (isSgOmpSizesClause(c)) {
      SgExprListExp *list =
          static_cast<SgExprListExp *>(exp_clause->get_expression());
  ...
  // directive
  void
  UnparseLanguageIndependentConstructs::unparseOmpEndDirectivePrefixAndName(
      SgStatement * stmt, SgUnparse_Info & info) {
    ASSERT_not_null(stmt);
  ...
  PrecedenceSpecifier UnparseLanguageIndependentConstructs::getPrecedence(
    ...
    case V_SgCastExp: {
      ...
      if (castExp == NULL) {
        ...
        if (functionCallExp != NULL) {
          ...
          if (binaryOperator != NULL) {
            ...
            // This could be a member function from a class or a templated
            // class.
            ...
            if (mfunc_ref != NULL) {
              ...
              SgTemplateMemberFunctionRefExp *template_mfunc_ref =
                  isSgTemplateMemberFunctionRefExp(rhs);
            ...
            if (func_ref != NULL) {
              ...
              SgTemplateFunctionRefExp *template_func_ref =
                  isSgTemplateFunctionRefExp(function);
          ...
          printf("ERROR: case V_SgCastExp: If this is not a SgCastExp (or "
                 "overloaded cast operator) then what is it: expr = %p = %s = "
                 "%s \n",
                 expr, expr->class_name().c_str(),
                 SageInterface::get_name(expr).c_str());
          ROSE_ABORT();
        }
      } else {
      ...
      // Since both SgCastExp and overloaded cast operators may be compiler
      // generated we just want to know if this is compiler generated (so use
      // "expr" instead of "castExp" below).
      ASSERT_not_null(expr->get_startOfConstruct());
      if (expr->get_startOfConstruct()->isCompilerGenerated() == true) {
        // It is a problem to return zero since this causes the calling function
        // to return "true" for needing parenthesis. So I have returned the
        // value "1".
        // If this is compiler generated then we have to look at the precedence
        // of the unary operator's operand. return 1;
        if (castExp != NULL) {
          // return getPrecedence(castExp->get_operand());
          precedence_value = getPrecedence(castExp->get_operand());
        } else {
          // DQ (8/29/2014): Changed the precedence to avoid over parenthization
          // of user-defined conversion operators (see test2006_185.C).  If it
          // is compiler generated then we can argue that it has a higher
          // precedence and this avoids the redundant parenthization which is a
          // bug for GNU 4.4 and other versions of GNU.

          // If this is compiler generated then we have to look at the
          // precedence of the unary operator's operand. printf ("WARNING: case
          // of overloaded cast operator: If this is compiler generated then we
          // have to look at the precedence of the unary operator's operand
          // (returning 1) \n"); return 1;
          MLOG_WARN_C("languageIndependenceSupport",
                      "case of overloaded cast operator: If this is compiler "
                      "generated then we have to look at the precedence of the "
                      "unary operator's operand (returning 16) \n");
          // return 16;
          precedence_value = 16;
        }
      } else {
    ...
    case V_SgChooseExpression:
      ...
      // DQ (8/23/2020): Working on Cxx_tests/test2020_44.C (niehter setting
      // appear to make a difference here). case V_SgConstructorInitializer:
      // precedence_value = 0; break; case V_SgConstructorInitializer:
    ...
    default: {
      // We want this to be a printed warning (so we can catch these missing
      // cases), but it is not worthy of calling an error since the default
      // works fine.
      printf(
          "Warning: getPrecedence() in unparseLanguageIndependentConstructs.C: "
          "Undefined expression variant = %d = %s \n",
          variant, Cxx_GrammarTerminalNames[variant].name.c_str());
      // DQ (9/25/2013): Temporarily added assertion to get rid of warnings
      // (catching them and fixing them).
      printf(
          "Error: getPrecedence() in unparseLanguageIndependentConstructs.C: "
          "Undefined expression variant = %d = %s \n",
          variant, Cxx_GrammarTerminalNames[variant].name.c_str());
      ROSE_ABORT();
    }
  ...
  AssociativitySpecifier UnparseLanguageIndependentConstructs::getAssociativity(
      SgExpression * expr) {
    ...
    case V_SgMinusMinusOp: {
      ...
      if (unaryOp == NULL) {
        ...
        // The number of function parametes is what determins if this is a
        // prefix or postfix operator. For non-member functions: The prefix
        // operator has 1, and the postfix operator has 2. For member functions:
        // The prefix operator has 0, and the postfix operator has 1.
        SgMemberFunctionDeclaration *memberFunctionDeclaration =
    ...
    default: {
      // We want this to be a printed warning (so we can catch these missing
      // cases), but it is not worthy of calling an error since the default
      // works fine. The implementation of this function assumes unhandled cases
      // are not associative.
      printf("Warning: getAssociativity(): Undefined expression variant = %d = "
             "%s (returning e_assoc_none) \n",
             variant, Cxx_GrammarTerminalNames[variant].name.c_str());
    }
    }
...
#define DEBUG_PARENTHESIS_PLACEMENT 0
    ...
    printf("In requiresParentheses(): expr = %p = %s need_paren = %s \n", expr,
           expr->class_name().c_str(),
           expr->get_need_paren() ? "true" : "false");
    ...
    curprint(string("/* In requiresParentheses(): expr = ") +
             expr->class_name() + " */ \n");
    ...
    // TV (04/24/11): As compiler generated cast are not unparsed they don't
    // need additional parenthesis.
    if (isSgCastExp(expr) &&
        expr->get_startOfConstruct()->isCompilerGenerated()) {
#if DEBUG_PARENTHESIS_PLACEMENT
      printf("In requiresParentheses(): Case 3 (compiler generated SgCastExp): "
             "Output false \n");
      curprint("/* In requiresParentheses(): Case 3 (compiler generated "
...
#if DEBUG_PARENTHESIS_PLACEMENT
      printf(
          "parentExpr = %p = %s parentVariant = %d  parentPrecedence = %d \n",
...
#if DEBUG_PARENTHESIS_PLACEMENT
            printf("WARNING: In requiresParentheses(): assoc == e_assoc_none "
                   "(not clear what to return, returning true is required to "
                   "pass regression tests) \n");
#endif
            // DQ (7/23/2013): This is required to be true for test2012_104.c to
            // pass. return false;
            return true;
          }
        } else {
