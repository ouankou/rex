...
public:
  ...
  // DQ (2/20/2005): The overloaded versions of these operators have to be
  // recognized so that we can skip the output of their function arguments which
  // control the interpritation as prefix/postfix operators when expressed using
  // the function syntax.
  ...
  //! This function is used to get for the combination of
  //! "operator->()->operator[](i)" since in this case the unparsed code can not
  //! be rediced to "->[i]".
  ...
  // DQ (1/19/2014): Added support for prefixed attributes (it makes a
  // difference where they go...).
  void printPrefixAttributes(SgDeclarationStatement *decl_stmt,
                             SgUnparse_Info &info);

  // DQ (12/31/2013): Added support for missing attributes in unparsed code (on
  // types in variable declarations).
  void printAttributesForType(SgDeclarationStatement *decl_stmt,
                              SgUnparse_Info &info);

  void directives(SgLocatedNode *lnode);

  // DQ (8/15/2020): Add state to the SgUnparse_Info class to record when we are
  ...
  // DQ (10/25/2006): Support for color code to identify errors in source code
  // position, compiler generated code, and general properties of the code.
  //! Support for color codes in unparse output.
  void printColorCodes(SgNode *node, bool openState,
