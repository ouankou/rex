...
public:
  ...
  /**
   * @param printAttrs - true means print the type attributes such as dimension
   * or length on the left of :: for the given type; false means not. the
   * default is set to true so that all extant callers of this method are
   * unaffected.
   */
   ...
   * :: for the given type; false means not. the default is set to true so that
   * all extant callers of this method are unaffected.
   */
  ...
  // These are C++ specific code generation functions that are not likely
  // required for Fortran virtual void unparseMemberPointerType(SgType* type,
  // SgUnparse_Info& info); void unparseNameType(SgType* type, SgUnparse_Info&
  // info); virtual void unparseEnumType(SgType* type, SgUnparse_Info& info);
  ...
  // DQ (10/5/2010): Added unparse support for types with fortran kind
  // parameters.
  void unparseTypeKind(SgType *type, SgUnparse_Info &info);
  void unparseTypeLengthAndKind(SgType *type, SgExpression *lengthExpression,
                                SgUnparse_Info &info);
  void unparseBaseType(SgType *type, const std::string &nameOfType,
                       SgUnparse_Info &info);
};
