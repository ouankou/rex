...
class AstUnparseAttribute : public AstAttribute {
  ...
  // This class is supports a mechanism to have text be output within the
  // unparsing phase. This mechanism is to be used for code generation of text
  // that is not proper code. It is not intended to be a mechanism to do
  // significant code generation (or be a crutch for transformations). however
  // it is particularly useful for backend specific code generation such as:
  //    1) backend compiler directives (which by definition are not a part of
  //    the language grammar) 2) tool specific annotations 3)
  //
  // It does not replace the comment or CPP directive mechanism, but does permit
  // it to be more precise or user defined (if required in rare cases).  In many
  // cases either that mechanism or this mechanism could be used to accomplish
  // the same goal.  It is prefered that the comment or CPP directive mechanism
  // be used and that this mechanism be used only as a backup. It is expected
  // that the user will select the appropriate mechanism in each case.

  // Note: CPP directives will soon be formal IR nodes, but that will only
  // handle the common case of where they are used, not the more general problem
  // (corner cases).
...
public:
  ...
  // DQ (7/19/2008): I think we should not reuse the
  // PreprocessingInfo::RelativePositionType since it does not make sense to
  // "replace" an IR node with a comment or CPP directive, I think. typedef
  // PreprocessingInfo::RelativePositionType RelativeLocation;
  enum RelativePositionType {
    ...
    e_inside = 4, // Directive goes inside the correponding code segment (as in
                  // between "{" and "}" of an empty basic block)

    // DQ (7/19/2008): Added additional fields so that we could use this enum
    // type in the AstUnparseAttribute
    e_replace = 5, // Support for replacing the IR node in the unparsing of any
                   // associated subtree
  ...
  //! This reports if there exists a replacement string for the IR nodes where
  //! the AstUnparseAttribute is attached.
