...
class CopyUnparser : public UnparseDelegate {
  // This class is a delegate class.  The unparser was modified to date an
  // optional UnparseDelegate object, if the pointer to UnparseDelegate is
  // non-null then the unparser queries the UnparseDelegate object before
  // unparsing statements (only applied to SgStatement) look for a delegate
  // before proceeding to unparse.  In this way alternative unparsing can be
  // introduced on top of the original unparser.
...
public:
  ...
  // DQ (5/26/2005): Made destructor virtual to match use with virtual member
  // function
