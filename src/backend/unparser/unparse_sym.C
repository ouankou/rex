...
//-----------------------------------------------------------------------------------
void Unparse_Sym::unparseSymbol(SgSymbol *sym, SgUnparse_Info &info) {
  ASSERT_not_null(sym);

  switch (sym->variant()) {
  case VARIABLE_NAME: {
    unparseVarSymbol(sym, info);
    break;
  }
  case FUNCTION_NAME: {
    unparseFunctionSymbol(sym, info);
    break;
  }
  case FUNCTYPE_NAME: {
    unparseFuncTypeSymbol(sym, info);
    break;
  }
  case CLASS_NAME: {
    unparseClassSymbol(sym, info);
    break;
  }
  case ENUM_NAME: {
    unparseEnumSymbol(sym, info);
    break;
  }
  case FIELD_NAME: {
    unparseFieldSymbol(sym, info);
    break;
  }
  case TYPEDEF_NAME: {
    unparseTypedefSymbol(sym, info);
    break;
  }
  case MEMBER_FUNC_NAME: {
    unparseMFunctionSymbol(sym, info);
    break;
  }
  case LABEL_NAME: {
    unparseLabelSymbol(sym, info);
    break;
  }
  case TEMPLATE_NAME: {
    unparseTemplateSymbol(sym, info);
    break;
  }
  case NAMESPACE_NAME: {
    unparseNamespaceSymbol(sym, info);
    break;
  }
  default: {
    printf(
        "Default reached in Unparse_Sym::unparseSymbol(sym = %p = %s,info) \n",
