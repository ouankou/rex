...
void unparse_alignas(SgInitializedName* decl_item, Unparse_ExprStmt& unparser, SgUnparse_Info& info)
{
                                                 ...
                                                 bool inside_for_init_stmt)
{
  ...
  if (need_initializer) {
    ...
    if (ctor_init) {
      ...
      bool might_need_assign_op = ctor_init->get_need_name() || ctor_init->get_associated_class_unknown() ||
                                  use_copy_ctor_syntax || unparse_info.inConditional();

      if (might_need_assign_op && inside_for_init_stmt && ctor_init->get_need_name() &&
          ctor_init->get_is_explicit_cast()) {
        need_assign_op = true;
      } else if (might_need_assign_op && (ctor_init->get_need_name() && ctor_init->get_is_explicit_cast()) ||
                 use_copy_ctor_syntax) {
        bool suppressAssignmentSyntax =
          (ctor_args_empty && !ctor_init->get_is_explicit_cast()) || ctor_init->get_is_braced_initialized();
...
void Unparse_ExprStmt::unparseVarDeclStmt(SgStatement* stmt, SgUnparse_Info& info)
{
  ...
  if (classDefinition != nullptr &&
      classDefinition->get_declaration()->get_class_type() == SgClassDeclaration::e_class &&
  ...
  while (vdecl_iname_it != vardecl_stmt->get_variables().end()) {
    ...
    if (vdecl_iname_it != vardecl_stmt->get_variables().begin()) {
      ...
      if (vardecl_stmt->get_is_thread_local()) {
        SgSourceFile* file = SageInterface::getEnclosingSourceFile(vardecl_stmt, true);
      ...
      need_assign_and_initializer_unparsed(decl_item, need_assign, need_initializer, ninfo,
