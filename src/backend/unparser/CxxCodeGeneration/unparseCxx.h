...
public:
  ...
  // DQ (6/21/2011): Refactored support for unparing names with template
  // arguments.
  void unparseTemplateFunctionName(SgTemplateInstantiationFunctionDecl *
  ...
  // function.
  void unparseTemplateMemberFunctionName(
      SgTemplateInstantiationMemberFunctionDecl
          *templateInstantiationMemberFunctionDeclaration,
      SgUnparse_Info &info);

  // DQ (2/16/2004): Added to refactor code and add support for old-style K&R C
  void unparseFunctionArgs(SgFunctionDeclaration *funcdecl_stmt,
                           SgUnparse_Info &info);
  ...
  // DQ (12/21/2005): Added to support name qualification when explicitly stored
  // in the AST (rather than generated).

  //        virtual void unparseQualifiedNameList(const SgQualifiedNamePtrList &
  //        qualifiedNameList);

  // DQ (8/14/2007): This is where all the langauge specific statement unparing
  // is done
  ...
  // unparser. virtual void unparseExpression      (SgExpression* expr,
  // SgUnparse_Info& info);

  virtual void unparseUnaryOperator(SgExpression *expr, const char *op,
                                    SgUnparse_Info &info);
  virtual void unparseBinaryOperator(SgExpression *expr, const char *op,
                                     SgUnparse_Info &info);
  // virtual void unparseUnaryExpr               (SgExpression* expr,
  // SgUnparse_Info& info); virtual void unparseBinaryExpr (SgExpression* expr,
  // SgUnparse_Info& info);
  virtual void unparseAssnExpr(SgExpression *expr, SgUnparse_Info &info);
  //        virtual void unparseExprRoot                (SgExpression* expr,
  //        SgUnparse_Info& info); virtual void unparseExprList (SgExpression*
  //        expr, SgUnparse_Info& info);
  ...
  virtual void unparseTemplateMFuncRef(SgExpression *expr,
                                       SgUnparse_Info &info);

  // DQ (4/25/2012): Refactored support for different templated adn non-template
  // member an non-member function references into these two template functions.
  template <class T>
  ...
  virtual void unparseChooseExpression(SgExpression *expr,
                                       SgUnparse_Info &info);
  ...
  // DQ (6/20/2013): Added support for C/C++ alignment extension __alignof__
  // operator.
  ...
  virtual void unparseVarArgStartOneOperandOp(SgExpression *expr,
                                              SgUnparse_Info &info);
  ...
  //        virtual void unparseNullExpression          (SgExpression* expr,
  //        SgUnparse_Info& info);
  virtual void unparseDesignatedInitializer(SgExpression *expr,
                                            SgUnparse_Info &info);
  ...
  // DQ (8/11/2014): Added more general support for function parameter
  // expressions (required for C++11 support).
  virtual void unparseFunctionParameterRefExpression(SgExpression *expr,
                                                     SgUnparse_Info &info);

  // DQ (9/3/2014): Adding C++11 Lambda expression support.
  virtual void unparseLambdaExpression(SgExpression *expr,
                                       SgUnparse_Info &info);

  // DQ (11/21/2017): Adding support for GNU C extension for computed goto.
  virtual void unparseLabelRefExpression(SgExpression *expr,
                                         SgUnparse_Info &info);

  virtual void unparseNonrealRefExpression(SgExpression *expr,
                                           SgUnparse_Info &info);

  //! unparse statement functions implememted in unparse_stmt.C
  // DQ (4/25/2005): Made this virtual so that Gabriel could build a specialized
  // unparser. virtual void unparseStatement        (SgStatement* stmt,
  // SgUnparse_Info& info); virtual void unparseGlobalStmt       (SgStatement*
  // stmt, SgUnparse_Info& info); CI virtual void unparseDeclStmt (SgStatement*
  // stmt, SgUnparse_Info& info);
  //        virtual void unparseScopeStmt        (SgStatement* stmt,
  //        SgUnparse_Info& info); virtual void unparseFuncTblStmt (SgStatement*
  //        stmt, SgUnparse_Info& info);
  virtual void unparseBasicBlockStmt(SgStatement *stmt, SgUnparse_Info &info);
  // CI
  // virtual void unparseElseIfStmt       (SgStatement* stmt, SgUnparse_Info&
  // info);
  ...
  virtual void unparseRangeBasedForStmt(SgStatement *stmt,
                                        SgUnparse_Info &info);
  ...
  //        virtual void unparseSpawnStmt        (SgStatement* stmt,
  //        SgUnparse_Info& info);
  // CI
  // virtual void unparseParStmt          (SgStatement* stmt, SgUnparse_Info&
  // info); CI virtual void unparseParForStmt       (SgStatement* stmt,
  // SgUnparse_Info& info);
  ...
  virtual void unparseMicrosoftAttributeDeclaration(SgStatement *stmt,
                                                    SgUnparse_Info &info);

  // DQ (12/26/2011): New unparse functions for new template declaration IR
  // nodes (new design for template declarations).
  virtual void unparseTemplateClassDeclStmt(SgStatement *stmt,
                                            SgUnparse_Info &info);
  virtual void unparseTemplateClassDefnStmt(SgStatement *stmt,
                                            SgUnparse_Info &info);
  virtual void unparseTemplateFunctionDeclStmt(SgStatement *stmt,
                                               SgUnparse_Info &info);
  virtual void unparseTemplateMemberFunctionDeclStmt(SgStatement *stmt,
                                                     SgUnparse_Info &info);
  virtual void unparseTemplateVariableDeclStmt(SgStatement *stmt,
                                               SgUnparse_Info &info);
  virtual void unparseTemplateFunctionDefnStmt(SgStatement *stmt,
                                               SgUnparse_Info &info);

  // DQ 11/3/2014): Adding C++11 templated typedef declaration support.
  virtual void unparseTemplateTypedefDeclaration(SgStatement *stmt,
                                                 SgUnparse_Info &info);

  // DQ (9/7/2014): Refactored this code so we could call it from the template
  // member and non-member function declaration unparse function.
  virtual void unparseReturnType(SgFunctionDeclaration *funcdecl_stmt,
                                 SgType *&rtype, SgUnparse_Info &info);
  ...
  // TV (3/12/18): Unparsing template headers, works the same way for class,
  // function, methods (typedef and variable ???)
  template <class T> void unparseTemplateHeader(T *decl, SgUnparse_Info &info);

  // DQ (12/26/2011): Supporting function for all template declarations
  // (initially at least).
  template <class T>
  void unparseTemplateDeclarationStatment_support(SgStatement *stmt,
                                                  SgUnparse_Info &info);

  virtual void unparseNonrealDecl(SgStatement *stmt, SgUnparse_Info &info);

  // virtual void unparseNullStatement    (SgStatement* stmt, SgUnparse_Info&
  // info);

  // DQ (7/21/2006): Added support for GNU statement expression extension.
  virtual void unparseStatementExpression(SgExpression *expr,
                                          SgUnparse_Info &info);

  // DQ (7/22/2006): Added support for asm operands.
  virtual void unparseAsmOp(SgExpression *expr, SgUnparse_Info &info);

  // DQ (6/25/2011): Can we modify this to perm it to be compiler withouth
  // seeing the enum definition? virtual void
  // unparse_asm_operand_modifier(SgAsmOp::asm_operand_modifier_enum flags);
  virtual void
  unparse_asm_operand_modifier(SgAsmOp::asm_operand_modifier_enum flags);

  // DQ (2/29/2004): Added to support unparsing of template instantiations
  // (similar to class declarations)
  virtual void unparseTemplateInstantiationDeclStmt(SgStatement *stmt,
                                                    SgUnparse_Info &info);

  // DQ (3/24/2004): Added to support template functions and template member
  // functions
  virtual void
  unparseTemplateInstantiationFunctionDeclStmt(SgStatement *stmt,
                                               SgUnparse_Info &info);
  virtual void
  unparseTemplateInstantiationMemberFunctionDeclStmt(SgStatement *stmt,
                                                     SgUnparse_Info &info);

  // DQ (4/16/2005): Added support for explicit template directives
  virtual void unparseTemplateInstantiationDirectiveStmt(SgStatement *stmt,
                                                         SgUnparse_Info &info);
  ...
  virtual void unparseNamespaceDeclarationStatement(SgStatement *stmt,
                                                    SgUnparse_Info &info);
  virtual void unparseNamespaceDefinitionStatement(SgStatement *stmt,
                                                   SgUnparse_Info &info);
  virtual void unparseNamespaceAliasDeclarationStatement(SgStatement *stmt,
                                                         SgUnparse_Info &info);
  virtual void unparseUsingDirectiveStatement(SgStatement *stmt,
                                              SgUnparse_Info &info);
  virtual void unparseUsingDeclarationStatement(SgStatement *stmt,
                                                SgUnparse_Info &info);
  ...
  virtual void
  unparseExceptionSpecification(const SgTypePtrList &exceptionSpecifierList,
                                SgUnparse_Info &info);
  ...
  //       virtual void unparseOmpDefaultClause(SgOmpDefaultClause* clause,
  //       SgUnparse_Info& info); virtual void
  //       unparseOmpScheduleClause(SgOmpScheduleClause* clause, SgUnparse_Info&
  //       info); virtual void unparseOmpVariablesClause(SgOmpVariablesClause*
  //       clause, SgUnparse_Info& info); virtual void
  //       unparseOmpExpressionClause(SgOmpExpressionClause* clause,
  //       SgUnparse_Info& info); virtual void unparseOmpClause(SgOmpClause*
  //       clause, SgUnparse_Info& info); virtual void
  //       unparseOmpParallelStatement (SgStatement* stmt, SgUnparse_Info&
  //       info); virtual void unparseOmpBodyStatement(SgOmpBodyStatement* stmt,
  //       SgUnparse_Info& info); virtual void
  //       unparseOmpThreadprivateStatement(SgOmpThreadprivateStatement* stmt,
  //       SgUnparse_Info& info); virtual void
  //       unparseOmpFlushStatement(SgOmpFlushStatement* stmt, SgUnparse_Info&
  //       info); virtual void unparseOmpBarrierStatement(SgOmpBarrierStatement*
  ...
  void unparseTypeTraitBuiltinOperator(SgExpression *expr,
                                       SgUnparse_Info &info);

  // DQ (2/8/2014): The name of the constructor call for a
  // SgConstructorInitializer must be output differently for the GNU g++ 4.5
  // version compiler and later.
  SgName trimOutputOfFunctionNameForGNU_4_5_VersionAndLater(
      SgName nameQualifier, bool &skipOutputOfFunctionName);

  // DQ (1/21/2018): Added support for lambda function capture variables in
  // annonymous compiler generated classes passed as template arguments
  ...
  void unparseClassInheritanceList(SgClassDefinition *classdefn_stmt,
                                   SgUnparse_Info &ninfo);

  // DQ (9/23/2020): Output the SgPragma when it is associated with a
  // SgScopeStatement.
