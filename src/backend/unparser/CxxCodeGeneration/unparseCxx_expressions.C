...
#else
        // DQ (2/8/2019): The start_of_pack_expansion_argument can appear
        // anywhere in the list, so skip over it. skip pack expansion argument,
        // it will be NULL anyway
        if ((*i)->get_argumentType() ==
            SgTemplateArgument::start_of_pack_expansion_argument) {
          i++;
          // continue;
...
#endif
      // If this is a template then the name will include template arguments
      // which require name qualification and the name qualification will depend
      // on where the name is referenced in the code.  So we have generate the
      // non-canonical name with all possible qualifications and save it to be
      // reused by the unparser when it unparses the tempated function name.
      SgTemplateInstantiationFunctionDecl *templateInstantiationFunctionDecl =
          isSgTemplateInstantiationFunctionDecl(declaration);
      if (templateInstantiationFunctionDecl != NULL) {
#if DEBUG_FUNCTION_REFERENCE_SUPPORT
...
#endif
      // DQ (12/28/2005): Changed to check for more general overloaded operators
      // (e.g. operator[]) if (!unp->opt.get_overload_opt() &&
      // isIOStreamOperator(mfunc_ref)); if (unp->opt.get_overload_opt() ==
      // false && unp->u_sage->isOperator(mfunc_ref) == true)
...
Unparse_ExprStmt::unparseDoubleVal(SgExpression* expr, SgUnparse_Info& info)
  ...
  // infinity, quiet NaN, and signaling NaN values.
     ...
     if (double_value == std::numeric_limits<double>::infinity())
       ...
       // We detect C99 and C "__NAN__" and translate to backend specific builtin function.
...
#define DEBUG_FUNCTION_CALL 0
...
#if DEBUG_FUNCTION_CALL
  printf("In Unparse_ExprStmt::unparseFuncCall(): (before test for "
         "conversion operator) uses_operator_syntax = %s \n",
  ...
  if (dotExp != NULL) {
    ...
    if (memberFunctionRefExp != NULL) {
      ...
      // b.A::operator+(b);" instead of "B b; b+b;" For conversion operators
      // the form would be "B b; return b.operator A();" instead of "B b;
      // return A(b);"

      // DQ (8/28/2014): It is a bug in GNU 4.4.7 to use the operator syntax
      // of a user-defined conversion operator. So we have to detect such
      // operators and then detect if they are implicit then mark them to use
      // the operator syntax plus supress them from being output.  We might
      // alternatively go directly to supressing them from being output,
      // except that this is might be more complex for the operator syntax
      // unparsing (I think).
      ...
      // if (uses_operator_syntax == true)
      {
        if (functionDeclaration->get_specialFunctionModifier().isConversion() ==
          ...
          printf("In Unparse_ExprStmt::unparseFuncCall(): Detected a "
                 "conversion operator! \n");
#endif
          // DQ (8/28/2014): Force output of generated code using the
          // operator syntax, plus supress the output if
          // is_compiler_generated == true. uses_operator_syntax = false;
...
void Unparse_ExprStmt::unparsePointStOp(SgExpression *expr,
  ...
  curprint("\n/* In Unparse_ExprStmt::unparsePointStOp " +
           StringUtility::numberToString(expr) + " */ \n");

  unparseBinaryOperator(expr, "->", info);

  printf("Leaving Unparse_ExprStmt::unparsePointStOp = %p \n", expr);
  curprint("\n/* Leaving Unparse_ExprStmt::unparsePointStOp " +
           StringUtility::numberToString(expr) + " */ \n");
}
#endif
...
#if 1
      // DQ (3/15/2015): test2015_11.c demonstrates a case where I think this
      // should be not be set (un-named struct type). DQ (10/17/2012): Added new
      // code not present where this is handled for SgVariableDeclaration IR
      // nodes.
...
void Unparse_ExprStmt::unparseAlignOfOp(SgExpression *expr,
  ...
  if (sizeof_op->get_operand_expr() != NULL) {
    ...
    // newinfo); unp->u_type->unparseType(cast_op->get_type(), newinfo);
    unp->u_type->unparseType(sizeof_op->get_operand_type(), newinfo);
#endif
  }
  curprint(")");
...
#if 0
  // DQ (9/1/2013): Now that we have modified the frontend to support the source position information in constants in the
...
static bool sharesSameStatement(SgExpression *, SgType *expressionType) {
  ...
  if (namedType != NULL) {
    ...
    if (classDeclaration != NULL) {
      isDeclarationPartOfVariableDeclaration =
