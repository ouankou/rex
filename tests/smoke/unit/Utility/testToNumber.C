...
template <class T>
static std::string toString(const T &n_, size_t radix = 10,
...
template <>
std::string toString<signed char>(const signed char &n, size_t radix,
                                  size_t nBits) {
  return toString<int16_t>(n, radix, nBits);
}

template <>
std::string toString<unsigned char>(const unsigned char &n, size_t radix,
                                    size_t nBits) {
  return toString<uint16_t>(n, radix, nBits);
}

template <class T>
static void assert_equal(const std::string &s, T y, const char *file,
                         const int line) {
...
template <class T>
static void assert_exception(const std::string &s, const std::string &reStr,
  ...
  if (parsed) {
    std::cerr << "test failed at " << file << ":" << line << "\n"
              ...
              << "\"\n"
              << "  parsed as " << toString(*parsed) << "\n"
              << "  expected exception matching /"
              << Rose::StringUtility::cEscape(reStr) << "/\n";
    exit(1);
  } else {
    std::regex re(reStr);
    if (!std::regex_search(*parsed.error(), re)) {
      std::cerr << "test failed at " << file << ":" << line << "\n"
                ...
                << Rose::StringUtility::cEscape(reStr) << "/\n"
                << "  got exception \""
                << Rose::StringUtility::cEscape(*parsed.error()) << "\"\n";
      exit(1);
    }
  }
}
...
template <class T> static void test() {
  ...
  if constexpr (sizeof(T) < 8) {
    if constexpr (std::numeric_limits<T>::is_signed) {
      const std::string big_min_10 =
          toString((int64_t)std::numeric_limits<T>::min() - 1);
      assert_fail(big_min_10,
                  "overflow error: less than minimum value for type");
      const std::string big_max_10 =
          toString((int64_t)std::numeric_limits<T>::max() + 1);
      assert_fail(big_max_10,
                  "overflow error: greater than maximum value for type");
    } else {
      const std::string big_max_10 =
          toString((uint64_t)std::numeric_limits<T>::max() + 1);
      assert_fail(big_max_10,
                  "overflow error: greater than maximum value for type");
    }
  }
