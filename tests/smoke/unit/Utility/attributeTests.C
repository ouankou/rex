...
template <class T> class AllocationCounter {
public:
...
template <class T> int AllocationCounter<T>::nAllocated = 0;
...
static void test_allocation_counter() {
  std::ostringstream unused;

  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 0,
                         "initial allocation count is zero");
  {
    AllocTest o1;
    ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 0,
                           "'new' is not called by constructors");
    unused << o1.x;
  }
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 0,
                         "'delete' is not called by destructors");

  AllocTest *o2 = new AllocTest;
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 1,
                         "'new' should have incremented allocation count");

  AllocTest o3(*o2);
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 1,
                         "copy constructor should not have changed count");

  *o2 = o3;
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 1,
                         "assignment should not have changed count");
  o3 = *o2;
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 1,
                         "assignment should not have changed count");

  AllocTest *o4 = new AllocTest;
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 2,
                         "'new' should have incremented count");

  delete o4;
  ASSERT_always_require2(AllocationCounter<AllocTest>::nAllocated == 1,
                         "'delete' should have decremented count");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Verify that all operations compile and that non-modifying operations can be
// applied to const containers.

class Attr1 : public AstAttribute {
public:
  virtual AstAttribute *copy() const override { return new Attr1(*this); }

  virtual OwnershipPolicy getOwnershipPolicy() const override {
    return CONTAINER_OWNERSHIP;
  }

  virtual std::string attribute_class_name() const override { return "Attr1"; }
};

// This function is not intended to be called.
void compile_tests(const AstAttributeMechanism &c, AstAttributeMechanism &m) {
...
public:
  virtual AstAttribute *copy() const override { return new Attr2(*this); }

  virtual OwnershipPolicy getOwnershipPolicy() const override {
    return CONTAINER_OWNERSHIP;
  }

  virtual std::string attribute_class_name() const override { return "Attr2"; }
};

static void test_value_deletion() {
  AstAttributeMechanism a;

  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 0,
                         "initial count");
  ...
  v1 = new Attr2;
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 1,
                         "one value allocated for storage");

  a.set("x", v1);
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 1,
                         "ownership should have transfered to container");

  (void)a["x"];
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 1,
                         "container should retain ownership");
  ASSERT_always_require2(a["x"] == v1, "value should have been stored");

  Attr2 *v2 = new Attr2;
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 2,
                         "second value is allocated by user");
  bool wasAdded = a.add("x", v2);
  ASSERT_always_require2(
      wasAdded == false,
      "not inserted because value by that name already exists");
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 1,
                         "ownership should have transfered to container");
  ...
  ASSERT_always_require2(
      wasAdded == false,
      "not inserted because value by that name does not exist");
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 1,
                         "ownership should have transfered to container");
...
static void test_container_destruction() {
  {
    AstAttributeMechanism a;
    ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 0,
                           "initial count");
...
static void test_container_copying() {
  {
    AstAttributeMechanism a;
    ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 0,
                           "initial count");
    ...
    ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 2,
                           "container copy-constructed");

    AstAttributeMechanism c;
    ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 2,
                           "no change");
    c = a;
    ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 3,
                           "assignment to empty container");
    c = b;
    ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 3,
                           "old value deleted during assignment");
  }
  ASSERT_always_require2(AllocationCounter<Attr2>::nAllocated == 0,
                         "values deleted when containers destroyed");
}
...
public:
  ...
  virtual std::string attribute_class_name() const override { return "Attr3"; }
};

static void test_missing_copy() {
  {
    AstAttributeMechanism a;
    ASSERT_always_require2(AllocationCounter<Attr3>::nAllocated == 0,
                           "initial count");

    a.set("x", new Attr3);
    ASSERT_always_require2(AllocationCounter<Attr3>::nAllocated == 1,
                           "one value inserted");
    ...
    ASSERT_always_require2(
        !b.exists("x"),
        "value lacking copy constructor should not have been copied");
    ASSERT_always_require2(AllocationCounter<Attr3>::nAllocated == 1,
                           "one value inserted");
  }
  ASSERT_always_require2(AllocationCounter<Attr3>::nAllocated == 0,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Test what happens if a container's virtual copy constructor doesn't make a
// copy
...
public:
  ...
  virtual std::string attribute_class_name() const override { return "Attr4"; }
};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Test that assignment is exception-safe by having a virtual copy constructor
// throw an exception part way through copying the attribute container.

class Attr5 : public AstAttribute, public AllocationCounter<Attr5> {
public:
  bool shouldThrow;

  explicit Attr5(bool shouldThrow) : shouldThrow(shouldThrow) {}
  ...
  virtual std::string attribute_class_name() const override { return "Attr5"; }
};

static void test_exception_safety() {
  {
    AstAttributeMechanism a;
    ASSERT_always_require2(AllocationCounter<Attr5>::nAllocated == 0,
                           "initial count");

    a.set("x", new Attr5(false));
    a.set("y", new Attr5(true));
    a.set("z", new Attr5(false));
    ASSERT_always_require2(AllocationCounter<Attr5>::nAllocated == 3,
                           "three values inserted");

    AstAttributeMechanism b;
    Attr5 *v1 = new Attr5(false);
    b.set("v1", v1);
    ASSERT_always_require2(AllocationCounter<Attr5>::nAllocated == 4,
                           "inserted value into destination container");

    try {
      b = a;
      ASSERT_not_reachable(
          "y's copy constructor should have thrown an exception");
    } catch (const std::runtime_error &e) {
    }
    ASSERT_always_require2(
        b["v1"] == v1, "destination container should not have been modified");
    ASSERT_always_require2(
        b.size() == 1, "destination container should not have been modified");
    ASSERT_always_require2(a.size() == 3,
                           "source container should not have been modified");
    ASSERT_always_require2(AllocationCounter<Attr5>::nAllocated == 4,
                           "assignment cleaned up extra copies");
  }
  ASSERT_always_require2(AllocationCounter<Attr5>::nAllocated == 0,
                         "containers destroyed");
}
...
public:
  ...
  Attr6(const Attr6 &other) : nCopies_(other.nCopies_), isDeleted_(false) {
    ++nCopies_;
  }
...
static void test_ast_attributes() {
  ...
  // "Update" an existing attribute (i.e., erase existing attribute and insert
  ...
  // "Set" an attribute that exists (i.e., erase the old value and insert the
  // new one)
  size_t attr5_n = 0;
  ...
  ASSERT_always_require(node0->attributeExists("attr4"));
  ASSERT_always_require(node0->getAttribute("attr4") ==
                        attr5); // because an earlier test replaced it
  ...
  ASSERT_always_require(node0->get_attributeMechanism() !=
                        node1->get_attributeMechanism());
