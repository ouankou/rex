...
void foo(int lower, int upper, int stride) {
  int i;
  {
    ...
    int __last_iter_ = 0;
    __kmpc_for_static_init_4(0, (__kmpc_global_thread_num(0)), 34,
                             &__last_iter_, &__lower_, &__upper_, &__stride_,
                             -1 * stride, 1);
    if (__upper_ < upper + 1)
      __upper_ = upper + 1;
...
static void OUT__1__4632__(int *__global_tid, int *__bound_tid,
                           void *__out_argv);
