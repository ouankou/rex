...
void foo(int lower, int upper, int stride) {
  ...
  XOMP_barrier();
  {
    ...
    if (XOMP_loop_static_start(lower, upper - 1, stride, 5, &p_lower_,
                               &p_upper_)) {
      do {
        for (p_index_ = p_lower_; p_index_ <= p_upper_; p_index_ += stride) {
          a[p_index_] = p_index_ * 2;
  ...
  XOMP_barrier();
  {
    ...
    if (XOMP_loop_dynamic_start(lower, upper - 1, stride, 3, &p_lower_,
                                &p_upper_)) {
      do {
        for (p_index_ = p_lower_; p_index_ <= p_upper_; p_index_ += stride) {
          a[p_index_] = p_index_ * 2;
  ...
  XOMP_barrier();
  {
    ...
    if (XOMP_loop_guided_start(lower, upper - 1, stride, 1, &p_lower_,
                               &p_upper_)) {
      do {
        for (p_index_ = p_lower_; p_index_ <= p_upper_; p_index_ += stride) {
          a[p_index_] = p_index_ * 2;
  ...
  XOMP_barrier();
  {
    ...
    if (XOMP_loop_ordered_runtime_start(lower, upper - 1, stride, &p_lower_,
                                        &p_upper_)) {
      do {
        for (p_index_ = p_lower_; p_index_ <= p_upper_; p_index_ += stride) {
          a[p_index_] = p_index_ * 2;
...
int main(int argc, char **argv) {
  ...
  XOMP_parallel_start(OUT__1__9651__, 0, 1, 0,
                      "/home/awang15/Projects/rexdev/rex_src/tests/nonsmoke/"
                      "functional/CompileTests/OpenMP_tests/ompfor2.c",
                      81);
