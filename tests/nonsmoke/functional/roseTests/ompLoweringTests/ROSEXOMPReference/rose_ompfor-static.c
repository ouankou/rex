...
void foo(int lower, int upper, int stride) {
  int i;
  {
    ...
    if (XOMP_loop_static_start(lower, upper - 1, stride, 3, &p_lower_,
                               &p_upper_)) {
      do {
...
int main(int argc, char **argv) {
  int status = 0;
  XOMP_init(argc, argv);
  XOMP_parallel_start(OUT__1__10294__, 0, 1, 0,
                      "/home/awang15/Projects/rexdev/rex_src/tests/nonsmoke/"
                      "functional/CompileTests/OpenMP_tests/ompfor-static.c",
                      22);
