...
struct ProgramOptions {
  ProgramOptions(void)
      : preproc_only_(false), node_type_(""), num_outline_(0),
        make_pdfs_(false), emit_stages_(false) {}
...
static size_t outlineRandom(SgProject *proj, size_t max_outline,
                            bool emit_stages, const std::string &node_type);

//! Run just the outlining preprocessor phase on random statements.
static size_t preprocessRandom(SgProject *proj, size_t max_outline,
                               bool emit_stages, const std::string &node_type);
...
int main(int argc, char *argv[]) {
  ...
  if (!proj->get_skip_transformation()) {
    ...
    if (opts.preproc_only_) {
      ...
      count = preprocessRandom(proj, opts.num_outline_, opts.emit_stages_,
                               opts.node_type_);
    } else {
      if (SgProject::get_verbose() > 0)
        cerr << "[Outlining...]" << endl;
      count = outlineRandom(proj, opts.num_outline_, opts.emit_stages_,
                            opts.node_type_);
    }
    if (SgProject::get_verbose() > 0)
...
static bool getOptions(vector<string> &argvList, ProgramOptions &opts) {
  ...
  if (!CommandlineProcessing::isOptionWithParameter(
          argvList, "-rose:outline:", "random", num_outlines, true) ||
      num_outlines < 1) {
    cerr << endl
         << "usage: " << argvList[0] << " -rose:outline:random <k>"
  ...
  string node_type;
  if (CommandlineProcessing::isOptionWithParameter(
          argvList, "-rose:outline:", "node", node_type, true)) {
    if (SgProject::get_verbose() > 0)
 ...
 */
static void selectStatements(SgProject *proj, size_t max_outline,
  ...
  public:
    ReorderTraversal(RandomStmtSelector::StmtSet_t &initial_targets,
                     StmtList_t &final_targets)
        : initial_targets_(initial_targets), final_targets_(final_targets) {}
  ...
  RandomStmtSelector::selectOutlineable(proj, max_outline, initial_targets);
...
static size_t preprocessRandom(SgProject *proj, size_t max_outline,
                               bool emit_stages, const string &node_type) {
  StmtList_t targets;
