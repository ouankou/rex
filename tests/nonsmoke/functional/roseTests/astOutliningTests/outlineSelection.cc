...
struct ProgramOptions_t {
  ProgramOptions_t(void)
      : preproc_only_(false), make_pdfs_(false), emit_stages_(false),
        max_rand_(0) {}
...
static size_t outlineSelection(SgProject *proj, const ProgramOptions_t &opts);
...
int main(int argc, char *argv[]) {
  ...
  if (!proj->get_skip_transformation()) {
    ...
    if (Outliner::preproc_only_) // Liao, 2/24/2009, we moved this into
...
static void collectStmtsAtPos(SgProject *root, const string &filename,
                              size_t line, StmtSet_t &out_stmts);

//! Collect a sequence of statements.
static void collectStmtsSeq(SgProject *root, size_t max_num_outline,
                            size_t step, StmtSet_t &out_stmts);

//! Collect a random number of statements.
static void collectRandom(SgProject *root, size_t max_num_outline,
                          StmtSet_t &out_stmts);
...
protected:
  ...
  size_t numCollected(void) const { return out_stmts_.size() - init_size_; }
  ...
  bool collected(const SgStatement *s) const {
    return out_stmts_.find(const_cast<SgStatement *>(s)) != out_stmts_.end();
  }

  //! Insert a given statement into the output set.
  void collect(SgStatement *s) { out_stmts_.insert(s); }
...
static void collectRandom(SgProject *root, size_t max_num_outline,
  ...
  public:
    ...
    void commit(void) {
      for (StmtVec_t::const_iterator i = keep_.begin(); i != keep_.end(); ++i)
...
static bool getOptions(vector<string> &argvList, ProgramOptions_t &opts) {
  ...
  if (CommandlineProcessing::isOptionWithParameter(
          argvList, "-rose:", "verbose", verbosity_level, false) &&
      verbosity_level >= 2) {
    if (SgProject::get_verbose() > 0)
  ...
  if (CommandlineProcessing::isOption(argvList, "-rose:outline:", "emit-stages",
                                      true)) {
    if (SgProject::get_verbose() > 0)
  ...
  int max_rand = 0;
  while (CommandlineProcessing::isOptionWithParameter(
      argvList, "-rose:outline:", "random", max_rand, true))
    if (max_rand > 0)
  ...
  if (opts.max_rand_ > 0)
    if (SgProject::get_verbose() > 0)
      cerr << "==> Outlining " << opts.max_rand_
           << " randomly-selected statements." << endl;

  int step = 0;
  while (CommandlineProcessing::isOptionWithParameter(
      argvList, "-rose:outline:", "seq", step, true))
    if (step > 0) {
      if (SgProject::get_verbose() > 0)
      ...
      if (SgProject::get_verbose() > 0)
        cerr << "*** WARNING: Ignoring invalid uplimit size, " << step << " ***"
             << endl;
    }

  int lineno = 0;
  while (CommandlineProcessing::isOptionWithParameter(
      argvList, "-rose:outline:", "line", lineno, true))
    if (lineno > 0)
      opts.lines_.push_back(ProgramOptions_t::SourcePos_t(string(""), lineno));
    else {
      if (SgProject::get_verbose() > 0)
        cerr << "*** WARNING: Ignoring bad line number, " << lineno << " ***"
             << endl;
    }
...
void dump(StmtsConstIterator b, StmtsConstIterator e) {
...
static void unparseIt(SgFile *f) {
...
static SgFile *getRoot(SgNode *node) {
  if (isSgFile(node))
...
static void selectAllStmtsSeq(SgProject *proj,
...
static void selectAllStmtsPos(SgProject *proj,
...
static void selectAllStmtsRandom(SgProject *proj, size_t max_rand,
                                 StmtSet_t &targets) {
 ...
 */
static void selectStatements(SgProject *proj, const ProgramOptions_t &opts,
  ...
  public:
    ReorderTraversal(StmtSet_t &initial, StmtList_t &final)
        : initial_targets_(initial), final_targets_(final) {}
...
static void preprocess(SgStatement *s, bool emit = false) {
...
static void outline(SgStatement *s, bool emit = false) {
  static size_t count = 0;
