...
static string toString(const SgStatement *s, size_t id = 0) {
  ROSE_ASSERT(s);
...
static void collectFuncDefs(SgProject *proj, FuncDefList_t &funcs) {
...
static size_t collectOutlineableStatements(SgBasicBlock *b, StmtVec_t &S) {
  if (!b)
    return 0;
    ...
    virtual void visit(SgNode *n) {
      ...
      if (Outliner::isOutlineable(s)) {
        if (SgProject::get_verbose() >= 3)
          cerr << '<' << s->class_name() << '>' << ' '
               << ASTtools::toStringFileLoc(s) << " is outlineable." << endl;
...
static size_t collectOutlineableStatements(const FuncDefList_t &funcs,
                                           StmtVec_t &stmts) {
  size_t count = 0;
  for (FuncDefList_t::const_iterator i = funcs.begin(); i != funcs.end(); ++i)
...
void RandomStmtSelector::selectOutlineable(SgProject *proj, size_t max_stmts,
                                           StmtSet_t &stmts) {
  FuncDefList_t funcs;
  ...
  if (!max_outline) {
    ...
    cerr << "*** Warning: Only " << max_outline
         << " statements may be outlined. ***" << endl;
  ...
  if (!num_failures) {
    cerr << "*** WARNING: Failure limit (" << num_failures
