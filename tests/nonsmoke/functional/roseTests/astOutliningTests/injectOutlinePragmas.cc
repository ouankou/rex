...
static void insertOutlineDirectives(RandomStmtSelector::StmtSet_t &S) {
  size_t num_outlines = 0;
  for (RandomStmtSelector::StmtSet_t::iterator i = S.begin(); i != S.end();
       ++i) {
    SgStatement *s = *i;
...
int main(int argc, char *argv[]) {
  ...
  if (!CommandlineProcessing::isOptionWithParameter(
          argvList, "-rose:", "num-outline", num_outlines, true) ||
      num_outlines < 1) {
    cerr << endl
  ...
  cerr << "[Selecting " << num_outlines << " statements to outline...]" << endl;
