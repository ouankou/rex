...
double diff_ratio(double val, double ref, int significant_digits) {
  significant_digits >= 1 ? ((void)0)
                          : (__assert_fail("significant_digits>=1", "jacobi.c",
  ...
  diff_ratio < upper_limit
      ? ((void)0)
      : (__assert_fail("diff_ratio < upper_limit", "jacobi.c", 20,
                       __PRETTY_FUNCTION__));
  return diff_ratio;
}
...
int main() {
  float toler;
  /*
            printf("Input n,m (< %d) - grid dimension in x,y
     direction:\n",MSIZE); scanf ("%d",&n); scanf ("%d",&m); printf("Input tol -
...
void initialize() {
  ...
  OUT_3_jacobi_134p =
      findFunctionUsingDlopen("OUT_3_jacobi_134", "./rose_jacobi_lib.so");
...
void jacobi() {
  ...
  while (k <= mits && error > tol) {
    ...
    OUT_2_jacobi_199p =
        findFunctionUsingDlopen("OUT_2_jacobi_199", "./rose_jacobi_lib.so");
...
void error_check() {
  ...
  OUT_1_jacobi_242p =
      findFunctionUsingDlopen("OUT_1_jacobi_242", "./rose_jacobi_lib.so");
