...
TimingPerformance timer("ROSE Performance Tests: total execution time (sec) = ",
                        true);
...
int main() {
  ...
  foo();
  TimingPerformance nested_timer(
      "another timer in main: execution time (sec) = ");
  foobar();

  // Output any saved performance data (see
  // ROSE/src/astDiagnostics/AstPerformance.h) AstPerformance::generateReport();
}
