...
public:
  ...
  void visit(SgNode *node) {
    if (SgTypeTraitBuiltinOperator *expr = isSgTypeTraitBuiltinOperator(node)) {
      // if it is a SgTypeTraitBuiltinOperator, call our analysis passing it the
      // type
      ...
      // cout<<isSgExprStatement(ifStmt->get_conditional())->get_expression
      // ()->class_name();

      // if the condition in the If statement has a NOT operator expect false as
      // the return from calling SgTypeTraitBuiltinOperator. Else expect true.
