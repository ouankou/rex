...
evaluateAnalysisStates::evaluateAnalysisStates(
    ConstantPropagationAnalysis *div_, string indent_)
    : div(div_), indent(indent_), total_expectations(0) {
  expectations["testFunc1"]["a"] =
      ConstantPropagationLattice(ConstantPropagationLattice::constantValue, 1);
  expectations["testFunc1"]["b"] =
      ConstantPropagationLattice(ConstantPropagationLattice::constantValue, 4);
  expectations["testFunc1"]["c"] =
      ConstantPropagationLattice(ConstantPropagationLattice::constantValue, 5);

  for (map<string, map<SgName, ConstantPropagationLattice>>::iterator i =
           expectations.begin();
...
void evaluateAnalysisStates::visit(const Function &func, const DataflowNode &n,
                                   NodeState &state) {
  ...
  string funcName =
      fnCall->getAssociatedFunctionSymbol()->get_name().getString();
  if (funcName.find("testFunc") == string::npos)
    return;

  FiniteVarsExprsProductLattice *lat =
      dynamic_cast<FiniteVarsExprsProductLattice *>(
          state.getLatticeAbove(div)[0]);
  cout << indent << "Lattice before call to " << funcName << ": " << lat->str()
       << endl;
  ...
  for (set<varID>::iterator i = allVars.begin(); i != allVars.end(); ++i) {
    ...
    if (expectations[funcName][name] != got) {
      cout << "mismatched: " << got->str() << " was not the expected "
           << expectations[funcName][name].str();
      numFails++;
    } else {
      cout << "matched";
      numPass++;
...
int main(int argc, char *argv[]) {
  ...
  // liveness analysis is used to generate FiniteVarsExprsProductLattice only
  // for live variables at a CFG point &ldva can be set to NULL then all visible
  // variables and expressions will be used to generate
  ...
  // A better verification method using pragma strings embedded in the input
  // test code grab live-in information from a Pragma Declaration
  ...
  for (Rose_STL_Container<SgNode *>::iterator i = nodeList.begin();
    ...
    VarsExprsProductLattice *lattice = dynamic_cast<VarsExprsProductLattice *>(
        NodeState::getLatticeAbove(&cpA, pdecl, 0)[0]);
    ...
    if (lattice_str == pragma_str) {
      ...
      cout << "Analysis results are not identical to the reference results "
              "from the pragma !"
           << endl;
