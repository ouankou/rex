...
bool mFilter (CFGNode cfgn)
{
      ...
      {
          //    case V_SgBasicBlock:
           //    return cfgn == node->cfgForBeginning()  || cfgn ==
node->cfgForBeginning(); case V_SgAssignStatement: case V_SgAssignOp: case
...
class Traversal : public AstPreOrderTraversal
   {
     public:
          ...
          void preOrderVisit(SgNode* node){
            ...
            //std::cout<<" Attribute exists:" <<
node->attributeExists("VFA_attribute") <<endl;
          }
};
...
public:
    VirtualFunctionAnalysis(SgProject *_pro, pointerAliasAnalysis* _pl) :
project(_pro), pl(_pl) { classHierarchy = new ClassHierarchyWrapper(project);
    };
...
void VirtualFunctionAnalysis::ignoreVirtualFunctionCalls()
{
    ...
    callSites = SageInterface::querySubTree<SgExpression> (project,
V_SgFunctionCallExp); vector<SgExpression*> constrs =
SageInterface::querySubTree<SgExpression> (project, V_SgConstructorInitializer);
...
classHierarchy, funcs);
            ...
            if(isSgFunctionCallExp(callSites[index]) && funcs.size() > 1 ){
                std::cout<<"VFC
:"<<isSgFunctionCallExp(callSites[index])->unparseToString()<<endl;
...
*/
/*
void VirtualFunctionAnalysis::visit(const Function& func, const DataflowNode& n,
NodeState& state)
{
    FiniteVarsExprsProductLattice *lat =
dynamic_cast<FiniteVarsExprsProductLattice *>(state.getLatticeAbove(pl)[0]);
...
int main(int argc, char *argv[]) {
  ...
  for (Rose_STL_Container<SgNode *>::iterator i = nodeList.begin();
    ...
    VarsExprsProductLattice *lattice = dynamic_cast<VarsExprsProductLattice *>(
        NodeState::getLatticeAbove(&poal, pdecl, 0)[0]);
