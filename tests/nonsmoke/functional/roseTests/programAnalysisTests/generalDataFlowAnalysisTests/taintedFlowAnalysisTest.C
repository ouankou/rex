...
evaluateAnalysisStates::evaluateAnalysisStates(TaintedFlowAnalysis *div_,
                                               string indent_)
    : div(div_), indent(indent_), total_expectations(0) {
  expectations["testFunc1"]["a"] =
      TaintedFlowLattice(TaintedFlowLattice::taintedValue);
  expectations["testFunc1"]["b"] =
      TaintedFlowLattice(TaintedFlowLattice::taintedValue);
  expectations["testFunc1"]["c"] =
      TaintedFlowLattice(TaintedFlowLattice::taintedValue);

  for (map<string, map<SgName, TaintedFlowLattice>>::iterator i =
           expectations.begin();
