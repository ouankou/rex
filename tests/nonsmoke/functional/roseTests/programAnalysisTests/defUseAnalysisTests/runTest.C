...
void testOneFunction(
    std::string
        funcParamName, // qualified function name for the function being checked
    vector<string> argvList, bool debug,
    int nrOfNodes, // number of CFG nodes in the test input file, entry count of
                   // the map
    ...
    multimap<string, int> useresults) // uses node name and ID
{
  ...
  // std::list<SgNode*> vars = NodeQuery::querySubTree(project,
  // V_SgFunctionDefinition); std::list<SgNode*>::const_iterator i =
  // vars.begin();
  ...
  for (; i != vars.end(); ++i) {
    ...
    if (debug)
      cout << "\n------------------------\nchecking for " << name << " -- "
           << funcName << " -- " << nodeNr << endl;
    if (debug)
      cout << " Test has nodes:  " << nrOfNodes << endl;
    if (debug)
      cout
          << "\nChecking all definitions associated with a function definition:"
          << func << endl;
    // check nodes in multimap for the function definition
    std::vector<std::pair<SgInitializedName *, SgNode *>> map =
        defuse->getDefMultiMapFor(func);
    if (debug) {
      cout << funcParamName
           << "'s DefMultiMap (vector of pairs )'s size is:" << map.size()
           << endl;
      defuse->printDefMap();
    }
    if (map.size() > 0) {
      std::vector<std::pair<SgInitializedName *, SgNode *>>::const_iterator j =
          map.begin();
      unsigned int hit = 0;
      SgNode *defnode = NULL;
      string name = "";
      for (; j != map.end(); ++j) { // for each defined variable : a pair of
                                    // <SgInitializedName*, definingNode*>
        SgInitializedName *in_node = j->first;
        defnode = j->second;
        name = in_node->get_qualified_name().str();
        int tableNr = defuse->getIntForSgNode(defnode);
        if (debug)
          cout << " ... checking def within func " << funcParamName << " "
               << name << "--" << tableNr << endl;
      ...
      if (results.size() != 0) {
        cerr << " Error: Analysis generates 0 def variables. But references "
                "have "
             << results.size() << " variables for "
             << defuse->getIntForSgNode(func) << endl;
        abort();
      }
    }

    if (maxNodes != nrOfNodes) {
      cerr << " Error: Test should have " << nrOfNodes
           << " nodes. Found: " << maxNodes << endl;
      abort(); // This will be checked later, we should let it proceed to see
               // what goes wrong
    }
    ...
    if (debug) {
      cout << funcParamName
           << "'s UseMultiMap (vector of pairs )'s size is:" << map.size()
           << endl;
      defuse->printUseMap();
    }
    ...
    if (map.size() > 0) {
      ...
      for (; j != map.end(); ++j) {
        ...
        if (debug)
          cout << " ... checking use within func " << funcParamName << " "
               << name << "--" << tableNr << endl;
      ...
      if (hit != useresults.size() || map.size() != useresults.size()) {
        cerr
            << " hit count does not match reference results count for uses map "
            << endl;
        abort();
      }
    } // if
...
void runCurrentFile(vector<string> &argvList, bool debug, bool debug_map) {
  ...
  for (; i != vars.end(); ++i) {
    ...
    if (vec.size() > 0)
      if (debug)
        std::cout << "  DEF>> Vector entries for " << name << " ( " << initName
                  << " ) : " << vec.size() << std::endl;
  }
  ...
  for (; i != vars.end(); ++i) {
    ...
    for (; j != map.end(); ++j) {
      ...
      if (vec.size() > 0)
        std::cout << "  USE>> Vector entries for " << name << " ( " << ret
                  << " ) : " << vec.size() << std::endl;
    }
  }
...
void usage() {
  cout << " Usage: " << endl;
  cout << "   runTest all [startNr [stopNr]]   --- to test all prepared "
          "testcases (with builtin reference results) between ID startNr and "
          "ID stopNr, inclusive"
       << endl;
  cout << "   runTest [file]                   --- to test a new file (no "
          "validation, just show process and dump out results)"
       << endl;
  exit(1);
}
