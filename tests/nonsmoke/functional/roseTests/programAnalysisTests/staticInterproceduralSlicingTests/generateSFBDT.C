...
// SFBDT: Slice Filtered Back Dominance Tree
...
int main(int argc, char **argv) {
  ...
  // list < SgNode * >fd = NodeQuery::querySubTree(project,
  // V_SgFunctionDefinition);
  NodeQuerySynthesizedAttributeType fd =
  ...
  for (NodeQuerySynthesizedAttributeType::iterator i = fd.begin();
       i != fd.end(); i++) {
    ...
    filename = outputFileName + "." +
               (fDef->get_declaration()->get_name().getString()) + ".bDT.dot";
