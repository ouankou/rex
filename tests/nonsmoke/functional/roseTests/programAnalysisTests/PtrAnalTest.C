...
void PrintUsage(char *name) { std::cerr << name << "<program name>" << "\n"; }
...
private:
  ...
  virtual void contrl_flow(Stmt s1, Stmt s2, CFGConfig::EdgeType t) {
    std::cout << "ctrl: " << ((long)s1) << "->" << ((long)s2) << " "
              << CFGConfig::EdgeType2String(t) << "\n";
  }
  ...
  virtual Stmt x_eq_op_y(OpType op, const std::string &x,
    ...
    for (std::list<std::string>::const_iterator p = y.begin(); p != y.end();
         ++p) {
      std::cout << (*p) << " ";
    }
    std::cout << ");\n";
