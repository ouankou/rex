...
static bool is_type_node(SgNode *node) { return isSgType(node) != NULL; }

void testOneFunction(
    std::string funcParamName,                          // function in question
    vector<string> argvList, bool debug, int nrOfNodes, // number of CFG nodes
  ...
  for (; i != vars.end(); ++i) {
    ...
    if (abortme)
      break;
    if (funcName != funcParamName) { // skip function definition which does not
                                     // match the specified name
      if (debug)
        cerr << "    .. skipping live analysis check for func : " << funcName
             << endl;
      continue;
    }
    ...
    SgFunctionDeclaration *decl =
        isSgFunctionDeclaration(func->get_declaration());
    ROSE_ASSERT(decl);
    Rose_STL_Container<SgInitializedName *> args =
    ...
    if ((int)nodes.size() - 1 != nrOfNodes) {
      cerr << "Error :: Number of nodes = " << nodes.size() - 1
           << "  should be : " << nrOfNodes << endl;
      exit(1);
    } else {
      if (debug)
        cerr << "Investigating total number of nodes : " << nodes.size()
             << endl;
    }
    ...
    for (; nodesIt != nodes.end(); ++nodesIt) {
      ...
      ROSE_ASSERT(node);
      int curNodeId =
          defuse->getIntForSgNode(node); // the Unique ID for the current node
      ...
      for (; k != resultsIn.end(); ++k) {
        ...
        if (curNodeId == expectedId) {
          ...
          if (results == inName) {
            ...
            if (debug) {
              cerr << " >>>>>>>>>> Problem with contents for IN set of " << node
                   << " " << node->class_name() << " with ID " << curNodeId
                   << endl;
          ...
          if (debug)
            cout << " nodeNr: " << curNodeId
                 << ".  ResultSize IN should be:" << results.size()
                 << " -  resultSize is: " << in.size()
                 << "  foundMatches == : " << hitIn << "/" << resultsIn.size()
                 << endl;
        }
      }
      ...
      for (; k != resultsOut.end(); ++k) {
        ...
        if (curNodeId == expectedId) {
          ...
          if (results == outName) {
            ...
            if (debug) {
              cerr << " >>>>>>>>>> Problem with contents for OUT ! " << node
                   << " " << node->class_name() << " with ID " << curNodeId
                   << endl;
              cerr << " >>>>>>>>>> Analysis generated results... " << endl;
          ...
          if (debug)
            cout << " nodeNr: " << curNodeId
                 << ".  ResultSize OUT should be:" << results.size()
                 << " -  resultSize is: " << out.size()
                 << "  foundMatches == : " << hitOut << "/" << resultsOut.size()
                 << endl;
        }
      }
      if (hitIn == 0 && hitOut == 0) {
        if (debug)
          cout << " nodeNr: " << curNodeId << " IN: " << hitIn << "/"
               << resultsIn.size() << "        Out:" << hitOut << "/"
               << resultsOut.size() << endl;
      }
    }
    if (hitIn != (int)resultsIn.size() || hitOut != (int)resultsOut.size()) {
      cout << " Error: No hit! ... DFA values of node " << nrOfNodes
  ...
  for (; i != vars.end(); ++i) {
    ...
    string funcName = func->get_declaration()->get_qualified_name().str();
    if (debug)
      cerr << " .. running live analysis for func (fixupStatementsINOUT): "
           << funcName << endl;
    FilteredCFGNode<IsDFAFilter> rem_source = liv->run(func, abortme);
...
void thisUsage() {
  cout << " Usage: " << endl;
  cout << "   runTest all [startNr] [stopNr]  --- to test all buildin "
          "testcases, need SRCDIR env variable set to find them"
       << endl;
  ...
  if (allTests == true) {
    ...
    if (!srcdirVar) {
      cerr << "buildin tests need to know the full path to the test files. "
              "\nPlease set SRCDIR env variable to the path of buildin input "
              "tests files. \n for example export "
              "SRCDIR=/path/to/rose/tests/nonsmoke/functional/roseTests/"
              "programAnalysisTests/variableLivenessTests/tests"
           << endl;
    ...
    // -------------------------------------- use-def tests
    // Removed because they've already been tested by ../defUseAnalysisTests.
    // [Robb Matzke 2012-11-06]
    // -------------------------------------- use-def tests
