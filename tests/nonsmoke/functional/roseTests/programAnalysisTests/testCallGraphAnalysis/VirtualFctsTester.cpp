/**
 * TestCase generator for CallGraphAnalysis - resolution of virtual function
 * calls
 */
...
void printClass(ostream &os, int classId) {
  ...
  // include superclass
  if (classId != 0)
    os << "#include \"" << classNames[classId - 1] << ".h"
       << "\"" << endl
       << endl;
  ...
  for (size_t func = 0; func < table[classId].size(); func++) {
    ...
    os << "\t\t"
       << "virtual void " << functionNames[func] << "() {}" << endl;
  }
  os << "};" << endl;
...
void printTestFunc(ostream &os, int classId, int funcId) {
  os << "void test_" << classNames[classId] << "_" << functionNames[funcId]
     << "()" << endl
     << "{" << endl;
  os << "\t" << classNames[0] << " * p = new " << classNames[classId] << "();"
     << endl;
  os << "\t"
     << "p->" << functionNames[funcId] << "();" << endl;
  os << "\t"
     << "delete p;" << endl;
  os << "}" << endl << endl;
}
 ...
 *                the vector has exactly one entry: the id of the class, which
 * function gets called if correct is false: (mode for testing the CG-Generator,
 * it cannot determine the right function instead creates superset) all classes
 * that implement the function between 0 and <funcId>
 */
...
int main(int argc, char **argv) {
  ...
  if (argc != 5) {
    ...
    cerr << "Usage:" << endl;
    cerr << "./virtualTestCaseGen <classCount> <functionCount> <fillRate> "
            "<outputFolderName>"
         << endl;
    return 1;
  }
  ...
  mainfile << endl << "int main(int argc, char **argv)" << endl;
  mainfile << "{" << endl;
