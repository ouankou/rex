...
std::string edgeLabel(SgDirectedGraphEdge *edge) { return "some label"; }

std::string nodeLabel(SgGraphNode *node) { return "some label"; }
...
std::string getTrueFilePath(SgFunctionDeclaration *inFuncDecl) {
  // We need to figure out where the function originates, but that's different
  // if it's a template so handle both cases
  std::string filepathStr;
  SgTemplateInstantiationFunctionDecl *tempIDecl =
      isSgTemplateInstantiationFunctionDecl(inFuncDecl);
  SgTemplateInstantiationMemberFunctionDecl *tempIMDecl =
      isSgTemplateInstantiationMemberFunctionDecl(inFuncDecl);
  if (tempIDecl) { // is template
    SgTemplateFunctionDeclaration *tempDecl =
        isSgTemplateFunctionDeclaration(tempIDecl->get_templateDeclaration());
    SgFunctionDeclaration *defFuncDecl =
        isSgTemplateFunctionDeclaration(tempDecl->get_definingDeclaration());
    filepathStr = defFuncDecl->get_file_info()->get_filename();
  } else if (tempIMDecl) { // is template member
    SgTemplateMemberFunctionDeclaration *tempDecl =
        isSgTemplateMemberFunctionDeclaration(
            tempIMDecl->get_templateDeclaration());
    SgFunctionDeclaration *defFuncDecl = isSgTemplateMemberFunctionDeclaration(
        tempDecl->get_definingDeclaration());
    filepathStr = defFuncDecl->get_file_info()->get_filename();
  } else { // is NOT template
    SgFunctionDeclaration *defDecl =
        isSgFunctionDeclaration(inFuncDecl->get_definingDeclaration());
    if (defDecl == NULL) {
      return "";
    }
  ...
  std::error_code ec;
  fs::path filePath = fs::weakly_canonical(filepathStr, ec);
  if (ec) {
    return filepathStr;
  }
...
struct OnlyCurrentDirectory {
  bool operator()(SgFunctionDeclaration *node) const {
    ...
    string sourceFilenameSubstring =
        sourceFilename.substr(0, stringToFilter.size());
    string sourceFilenameSrcdirSubstring =
        sourceFilename.substr(0, srcDir.size());
    // #if 0
    ...
    // else
    // if (
    // string(node->get_file_info()->get_filename()).substr(0,srcDir.size()) ==
    // srcDir ) if (sourceFilenameSrcdirSubstring == srcDir) return true;
    else
      return false;
  }
};
...
SecurityVulnerabilityAttribute::SecurityVulnerabilityAttribute(
    SgDirectedGraphEdge *e)
    : edge(e), node(NULL) {}
...
SecurityVulnerabilityAttribute::additionalEdgeInfo() {
  ...
  if (edge != NULL) {
    ...
    string vulnerabilityOptions =
        " arrowsize=2.0 style=\"setlinewidth(7)\" constraint=false fillcolor=" +
        vulnerabilityColor + ",style=filled ";

    // AstAttribute::AttributeNodeInfo vulnerabilityNode ( (SgNode*)
    // vulnerabilityPointer, "SecurityVulnerabilityAttribute","
    // fillcolor=\"red\",style=filled ");
    AstAttribute::AttributeEdgeInfo vulnerabilityNode(
        edge->get_from(), edge->get_to(), vulnerabilityName,
...
SecurityVulnerabilityAttribute::additionalNodeInfo() {
  ...
  if (node != NULL) {
    ...
    // vulnerabilityPointer, "SecurityVulnerabilityAttribute","
    // fillcolor=\"red\",style=filled ");
    AstAttribute::AttributeNodeInfo vulnerabilityNode(node, vulnerabilityName,
                                                      vulnerabilityOptions);
    v.push_back(vulnerabilityNode);
  }
...
int main(int argc, char *argv[]) {
  ...
  // Generate a filename for this whole project (even if it has more than one
  // file)
  ...
  //  OutputDot::writeToDOTFile(newGraph, coloredFileName.c_str(),"Incidence
