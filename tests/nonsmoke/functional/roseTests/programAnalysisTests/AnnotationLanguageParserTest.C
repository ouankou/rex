...
int main(int argc, char **argv) {
  ...
  for (n = 1; n < argc; n++) {
    ...
    for (procedures_map_cp p = a->procedures().begin();
      ...
      cout << "  access { ";
      for (var_set_cp p = proc->uses().begin(); p != proc->uses().end(); ++p) {
        annVariable *decl = *p;
      ...
      cout << "  modify { ";
      for (var_set_cp p = proc->defs().begin(); p != proc->defs().end(); ++p) {
        annVariable *decl = *p;
        ...
        for (var_list_cp fp = proc->formal_params().begin();
             fp != proc->formal_params().end(); ++fp, ++pnum) {
