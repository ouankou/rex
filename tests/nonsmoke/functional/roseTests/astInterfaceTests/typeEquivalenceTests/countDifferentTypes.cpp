...
void TypeCounter::printDistinctTypes(std::ostream &outs) {
  for (std::vector<SgType *>::iterator i = distTypes.begin();
       i != distTypes.end(); ++i) {
...
size_t TypeCounter::getNumberOfDistinctTypes() {
  ...
  for (std::vector<SgType *>::iterator i = typesVisitedSoFar.begin();
       i != typesVisitedSoFar.end(); ++i) {
    bool flag = false;
    for (std::vector<SgType *>::iterator j = distTypes.begin();
         j != distTypes.end(); ++j) {
  ...
  if (isSgStatement(n)) {
    if (isSgStatement(n)->hasExplicitType()) {
      if (isSgTypedefDeclaration(n)) {
        ...
        std::cerr << "Documentation is wrong! Found: " << n->class_name()
                  << std::endl;
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  std::cout << "Distinct Types encountered: " << tc.getNumberOfDistinctTypes()
            << std::endl;
