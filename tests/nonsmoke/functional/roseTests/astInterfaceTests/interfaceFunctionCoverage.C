// A translator to call many SageInterface functions to improve code coverage
// reported by LCOV.
//
// Initially add analysis interface functions,
//
// Later to add transformation interface functions
//  SageInterface::initializeSwitchStatement(SgSwitchStatement*
//  switchStatement,SgStatement *item_selector,SgStatement *body)
//  SageInterface::appendStatement(SgStatement *stmt, SgForInitStatement*
//  for_init_stmt)
...
void RoseVisitor::visit(SgNode *node) {
  if (SgDeclarationStatement *decl = isSgDeclarationStatement(node)) {
    ...
    // cout<<"calling SageInterface::generateUniqueNameForUseAsIdentifier(
    // SgDeclarationStatement* declaration ) "<<endl;
  ...
  if (SgSwitchStatement *sw = isSgSwitchStatement(node)) {
    ...
    cout << "calling SageInterface::outputLocalSymbolTables ( SgNode* node ) "
         << endl;
  ...
  if (SgTemplateInstantiationMemberFunctionDecl *temp_decl =
          isSgTemplateInstantiationMemberFunctionDecl(node)) {
    cout << "calling functions operating on "
    ...
    // SgTemplateInstantiationMemberFunctionDecl* copy =
    // buildForwardFunctionDeclaration (temp_decl); prependStatement(copy,
    // temp_decl->get_scope());
  }
  ...
  if (SgMemberFunctionDeclaration *memfunc =
  ...
  // TODO: this never should work since we don't use
  // SgC_PreprocessorDirectiveStatement now.
  if (SgC_PreprocessorDirectiveStatement *pdecl =
  ...
  if (SgExpression *exp = isSgExpression(node)) {
    ...
    //    cout<<"calling SageInterface::getInitializerOfExpression(SgExpression*
    //    n) returns something "<<endl;
    ...
    if (isSgCharVal(exp) || isSgUnsignedCharVal(exp) || isSgShortVal(exp) ||
        ...
        isSgLongLongIntVal(exp) || isSgUnsignedLongLongIntVal(exp)) {
  ...
  if (SgFunctionCallExp *node2 = isSgFunctionCallExp(node)) {
    ...
    if (SgFunctionSymbol *fsym = node2->getAssociatedFunctionSymbol()) {
      if ((fsym->get_name().getString()) == "test_splitVariableDeclaration") {
        SgFunctionDeclaration *fdecl = findFunctionDeclaration(
            getGlobalScope(node2), "test_splitVariableDeclaration", NULL, true);
  ...
  if (SgScopeStatement *node2 = isSgScopeStatement(node)) {
    ...
    //  calling SageInterface::getEnclosingScope(SgNode* n, const bool
    //  includingSelf/* =false*/)
...
int main(int argc, char *argv[]) {
  ...
  //  calling SageInterface::computeUniqueNameForUseAsIdentifier( SgNode*
  //  astNode )
  ...
  SgVariableDeclaration *var_decl =
      isSgVariableDeclaration((body->get_statements())[0]);
