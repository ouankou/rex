...
int main(int argc, char *argv[]) {
  ...
  SgFunctionRefExp *func_ref1 = buildFunctionRefExp(
      "TEST2",
      buildFunctionType(buildVoidType(), buildFunctionParameterTypeList()),
      func_body);
  ...
  SgProcedureHeaderStatement *func2 = buildProcedureHeaderStatement(
      function_declaration_ownership::sourceLexical(), "TEST2", buildVoidType(),
      paraList2, buildSemanticFunctionParameterList(paraList2),
      SgProcedureHeaderStatement::e_subroutine_subprogram_kind,
      SgProcedureHeaderStatement::e_fortran_procedure_source_form_header,
      globalScope);
  ...
  SgAttributeSpecificationStatement *external_stmt =
      buildAttributeSpecificationStatement(
          SgAttributeSpecificationStatement::e_externalStatement);
  SgFunctionRefExp *func_ref = buildFunctionRefExp(
      "parallel_start",
      buildFunctionType(buildVoidType(), buildFunctionParameterTypeList()),
      func_body2);
