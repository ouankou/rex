...
int removeThisFunctionToTestAttachedInfoBeforeStatement_var =
    variable_hidden_in_header_file;

// Use this function to test removing statements to avoid an error in the next
// statement (which reuses the function name as a variable name).  If the
// function is not removed then the variable declaration will be legal code.  If
// the #include directive is removed then the variable declaration after the
// #include will not be a legal declaration.
int removeThisFunctionToTestAttachedInfoAfterStatement() { return 0; }

// Cause generation of: "int y = InsertStatementAfterThisFunction();" which is
// only legal after after declaration of InsertStatementAfterThisFunction(). The
// header file below will require this variable to exist when compiled using the
// backend compiler.
