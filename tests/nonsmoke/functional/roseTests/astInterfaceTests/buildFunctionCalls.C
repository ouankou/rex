 ...
 *   - build a function call statement without previous declaration, a header is
 * inserted first
 */
#include "rose.h"
...
int main(int argc, char *argv[]) {
 ...
 // using MiddleLevelRewrite to parse the content of the header, but NOT working!
  ...
  SgVarRefExp *arg1 =
      buildVarRefExp(SgName("p_sum")); // type is inferred from symbol table
  SgExprListExp *arg_list = buildExprListExp();
  appendExpression(arg_list, arg1);
  SgExprStatement *callStmt1 =
      buildFunctionCallStmt(SgName("foo"), return_type, arg_list);
  ...
  SgExprStatement *callStmt_2 =
      buildFunctionCallStmt(SgName("bar"), return_type_2, arg_list_2);
