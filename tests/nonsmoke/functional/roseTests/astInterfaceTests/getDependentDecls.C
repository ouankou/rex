/*
test code for C++
Any reference to class or template class from a namespace will treated as a
reference to the enclosing namespace: In this example, several classes are
dependent but their enclosing namespace will be reported as dependent
declarations. by Liao, 5/8/2009
*/
#include "rose.h"
...

{
  ...
  for (Rose_STL_Container<SgNode *>::iterator it = funcs.begin();
       it != funcs.end(); ++it) {
    ...
    if (fd) {
      ...
      cout << "  Function: " << fd->get_name().getString()
           ...
           << ", defining: "
           << (fd->get_definingDeclaration() == fd ? "YES" : "NO") << "\n";
    }
  }

  SgFunctionDeclaration *func = SageInterface::findMain(project);
  cout << "DEBUG: findMain returned: "
       << (func ? func->get_name().getString() : "NULL") << "\n";
