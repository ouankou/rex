...
int main(int argc, char *argv[]) {
  ...
  SgTypedefDeclaration *decl_1 = buildTypedefDeclaration(
      typedef_declaration_ownership::sourceLexical(),
      SgTypedefDeclaration::e_typedef, "MYINT", buildIntType(), globalScope);

  // typedef struct frame {int x;} frame

  // Build struct frame {} first
  SgClassDeclaration *struct_1 = buildStructDeclaration(
      declaration_ownership::embeddedDeclaratorChild(), "frame", globalScope);
  SgClassDefinition *def = struct_1->get_definition();
  SgVariableDeclaration *varDecl =
      buildVariableDeclaration(SgName("x"), buildIntType(), NULL, def);
  appendStatement(varDecl, def);

  // should not append struct here if we want it show up within the typedef
  // declaration otherwise duplicated definition for the same struct
  // appendStatement(struct_1, globalScope);

  // build typedef from struct frame then
  SgTypedefDeclaration *decl_2 = buildTypedefDeclarationWithEmbeddedTag(
      typedef_declaration_ownership::sourceLexical(),
      SgTypedefDeclaration::e_typedef, "frame", struct_1->get_type(),
      globalScope, struct_1);
