...
public:
  NodeCountSimple(enum VariantT variant) : variantCount(0), variant(variant) {}
  // setting this to false will use the old traversal successor container
  // mechanism
...
public:
  NodeCountPrePost(enum VariantT variant) : variantCount(0), variant(variant) {}
  unsigned long variantCount;
...
public:
  NodeCountTopDown(enum VariantT variant) : variantCount(0), variant(variant) {}
  unsigned long variantCount;

protected:
  virtual unsigned long *
  evaluateInheritedAttribute(SgNode *node, unsigned long *inhAttribute) {
    if (variant == node->variantT())
      ++*inhAttribute;
...
class NodeCountTopDownBottomUp
    : public AstTopDownBottomUpProcessing<unsigned long *, unsigned long *> {
public:
...
protected:
  virtual unsigned long *
  evaluateInheritedAttribute(SgNode *, unsigned long *inhAttribute) {
    return inhAttribute;
  }
  virtual unsigned long *
  evaluateSynthesizedAttribute(SgNode *node, unsigned long *inhAttribute,
  ...
  virtual unsigned long *
  defaultSynthesizedAttribute(unsigned long *inhAttribute) {
    return inhAttribute;
  }
  VariantT variant;
};
...
double timeDifference(const struct timeval &end, const struct timeval &begin) {
  return (end.tv_sec + end.tv_usec / 1.0e6) -
         (begin.tv_sec + begin.tv_usec / 1.0e6);
}
...
template <class T> std::vector<T *> *buildTraversalList() {
...
void runSequentialTests(SgProject *root,
  ...
  std::cout
      << "simple sequential using OLD traversal successor container mechanism"
      << std::endl;
  std::vector<NodeCountSimple *> *simpleList =
      buildTraversalList<NodeCountSimple>();
  ...
  std::cout << "simple sequential using NEW index based access mechanism"
            << std::endl;
  std::vector<NodeCountSimple *> *simpleList2 =
      buildTraversalList<NodeCountSimple>();
  ...
  std::vector<NodeCountPrePost *> *prePostList =
      buildTraversalList<NodeCountPrePost>();
  ...
  std::vector<NodeCountBottomUp *> *bottomUpList =
      buildTraversalList<NodeCountBottomUp>();
  ...
  std::vector<NodeCountTopDown *> *topDownList =
      buildTraversalList<NodeCountTopDown>();
  ...
  std::vector<NodeCountTopDownBottomUp *> *topDownBottomUpList =
      buildTraversalList<NodeCountTopDownBottomUp>();
  std::vector<NodeCountTopDownBottomUp *>::iterator tb;
  beginTime = getCPUTime();
  for (tb = topDownBottomUpList->begin(); tb != topDownBottomUpList->end();
       ++tb) {
...
void runCombinedTests(SgProject *root,
  ...
  beginTime = getCPUTime();
  std::vector<unsigned long *> *bottomUpResults =
      combinedBottomUp.traverse(root);
  endTime = getCPUTime();
  ...
  std::vector<NodeCountTopDownBottomUp *> *topDownBottomUpList =
      buildTraversalList<NodeCountTopDownBottomUp>();
  std::vector<NodeCountTopDownBottomUp *>::iterator tb;
  AstCombinedTopDownBottomUpProcessing<unsigned long *, unsigned long *>
      combinedTopDownBottomUp;
  ...
  beginTime = getCPUTime();
  std::vector<unsigned long *> *topDownBottomUpResults =
      combinedTopDownBottomUp.traverse(root, &variantCountPtrList2);
  endTime = getCPUTime();
  std::vector<unsigned long *>::const_iterator tbr;
  i = 0;
  for (tbr = topDownBottomUpResults->begin();
       tbr != topDownBottomUpResults->end(); ++tbr) {
#if OUTPUT_RESULTS
    std::cout << **tbr << ' ';
#endif
...
void runParallelTests(SgProject *root,
  ...
  beginTime = getCPUTime();
  std::vector<unsigned long *> *bottomUpResults =
      parallelBottomUp.traverseInParallel(root);
  endTime = getCPUTime();
  std::vector<unsigned long *>::const_iterator br;
  i = 0;
  for (br = bottomUpResults->begin(); br != bottomUpResults->end(); ++br) {
  ...
  std::vector<NodeCountTopDownBottomUp *> *topDownBottomUpList =
      buildTraversalList<NodeCountTopDownBottomUp>();
  std::vector<NodeCountTopDownBottomUp *>::iterator tb;
  AstSharedMemoryParallelTopDownBottomUpProcessing<unsigned long *,
                                                   unsigned long *>
      parallelTopDownBottomUp;
  std::vector<unsigned long *> variantCountPtrList2;
  for (tb = topDownBottomUpList->begin(); tb != topDownBottomUpList->end();
       ++tb) {
  ...
  beginTime = getCPUTime();
  std::vector<unsigned long *> *topDownBottomUpResults =
      parallelTopDownBottomUp.traverseInParallel(root, &variantCountPtrList2);
  endTime = getCPUTime();
  std::vector<unsigned long *>::const_iterator tbr;
  i = 0;
  for (tbr = topDownBottomUpResults->begin();
...
int main(int argc, char **argv) {
  ...
  endTime = getCPUTime();
  std::cout << "frontend ran for about " << timeDifference(endTime, beginTime)
            << " seconds on " << root->numberOfFiles() << " files" << std::endl;
