...
void visitorTraversal::analyzePath(vector<VertexID> &pathR) {
  // ROSE_ASSERT(find(pathstore.begin(), pathstore.end(), pth) ==
  // pathstore.end()); pathstore.push_back(pth); #pragma omp atomic paths++;
     ...
     if (funcDecl != NULL)
        {
          ...
          if ( (functionName.length() >= 4) && ((functionName.substr(0,4) ==
"get_") || (functionName.substr(0,4) == "set_")) ) returnNodeList.push_back
(astNode);
        }
...
*/
// Function printFunctionDeclarationList will print all function names in the
// list
void printFunctionDeclarationList(
  ...
  for (Rose_STL_Container<SgNode *>::iterator i =
    ...
    // Build a pointer to the current type so that we can call the get_name()
    // member function.
...
int main(int argc, char *argv[]) {
  ...
  // Build list using nested Queries (operating on return result of previous
  // query)
  //   Rose_STL_Container<SgNode*> accessFunctionsList;
  //   accessFunctionsList = NodeQuery::queryNodeList
  //   (functionDeclarationList,&querySolverAccessFunctions);
  //   printFunctionDeclarationList(accessFunctionsList);

  // Alternative form of same query building the list using a query of the whole
  // AST
  //  accessFunctionsList = NodeQuery::querySubTree
  //  (project,&querySolverAccessFunctions);
  //  printFunctionDeclarationList(accessFunctionsList);
  int counter = 0;
  for (Rose_STL_Container<SgNode *>::iterator i =
           functionDefinitionList.begin();
       i != functionDefinitionList.end(); i++) {
    ...
    string fileName =
        functionDeclaration->get_name()
            .str(); // StringUtility::stripPathFromFileName(mainDef->get_file_info()->get_filenameString());
    string dotFileName1;
  ...
  std::cout << "Number of class definitions in the memory pool is: "
            << NodeQuery::queryMemoryPool(vv1).size() << std::endl;

  // Another way to query for collections of multiple IR nodes.
  // VariantVector(V_SgType) is internally expanded to all IR nodes derived from
  // SgType.
  VariantVector vv2 =
      VariantVector(V_SgClassDefinition) + VariantVector(V_SgType);
  std::cout << "Number of class definitions AND types in the memory pool is: "
            << NodeQuery::queryMemoryPool(vv2).size() << std::endl;
