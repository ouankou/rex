...
void One_Dimensional_Goduov_Solver(int nriemann,double prsfac,double rhofloor,
                                   double prsfloor,double gamma,int pprieman,
                                   int rieiter,doubleArray & rhoremi,
                                   doubleArray & rhorepl,doubleArray & prsremi,
                                   doubleArray & prsrepl,doubleArray & vlxremi,
                                   doubleArray & vlxrepl,doubleArray & vlyremi,
                                   doubleArray & vlyrepl,doubleArray & vlzremi,
                                   doubleArray & vlzrepl,doubleArray & rhoflux,
                                   doubleArray & engflux,doubleArray & vlxflux,
                                   doubleArray & vlyflux,doubleArray & vlzflux,
                                   doubleArray & rho0,doubleArray & prs0,
                                   doubleArray & vlx0,doubleArray & vly0,
                                   doubleArray & vlz0,doubleArray & sdsp0)
...
// The name is special request from Dinshaw!
...
// Sync the C and C++ io systems!

     
// Turn on Index bounds checking!
...
intArray inraref(nriemann + 2); 
     ...
     Index Generic_Index(1,nriemann,1); 
     ...
     prsbar(Generic_Index)=((spdbarpl(Generic_Index)*prsremi(Generic_Index)+spdbarmi(Generic_Index)
     *prsrepl(Generic_Index))-(spdbarmi(Generic_Index)*spdbarpl(Generic_Index))*(vlxrepl(Generic_Index)
     -vlxremi(Generic_Index)))/(spdbarmi(Generic_Index)+spdbarpl(Generic_Index)); 
     prsbar(Generic_Index).replace(prsbar(Generic_Index)<=prsfloor,prsfloor); 
     if (
     pprieman == 1)
        { 
          for (
int ii=1; ii <= rieiter; ii++)
             { 
               tempaa.fill((gamma + 1.0) / (2.0 * gamma)); 
               spdbarpl(Generic_Index)=lsdrepl(Generic_Index)*sqrt((1.0+tempaa
               *(prsbar(Generic_Index)/prsrepl(Generic_Index)-1.0))); 
               spdbarmi(Generic_Index)=lsdremi(Generic_Index)*sqrt((1.0+tempaa
               *(prsbar(Generic_Index)/prsremi(Generic_Index)-1.0))); 
               tempzpl=(2.0*pow(spdbarpl(Generic_Index),3.0))/(pow(lsdrepl(Generic_Index),
               2.0)+pow(spdbarpl(Generic_Index),2.0)); 
               tempzmi=(2.0*pow(spdbarmi(Generic_Index),3.0))/(pow(lsdremi(Generic_Index),
               2.0)+pow(spdbarmi(Generic_Index),2.0)); 
               tempupl=vlxrepl(Generic_Index)+(prsbar(Generic_Index)-prsrepl(Generic_Index))
               /spdbarpl(Generic_Index); 
               tempumi=vlxremi(Generic_Index)-(prsbar(Generic_Index)-prsremi(Generic_Index))
               /spdbarmi(Generic_Index); 
               prsbar(Generic_Index)=prsbar(Generic_Index)-((tempzmi*tempzpl)*(tempupl
               -tempumi))/(tempzmi+tempzpl); 
               prsbar(Generic_Index).replace(prsbar(Generic_Index)<=prsfloor,prsfloor); 
          } 
          tempaa.fill((gamma + 1.0) / (2.0 * gamma)); 
          spdbarpl(Generic_Index)=lsdrepl(Generic_Index)*sqrt((1.0+tempaa*(prsbar(Generic_Index)
          /prsrepl(Generic_Index)-1.0))); 
          spdbarmi(Generic_Index)=lsdremi(Generic_Index)*sqrt((1.0+tempaa*(prsbar(Generic_Index)
          /prsremi(Generic_Index)-1.0))); 
          vlxbar(Generic_Index)=((spdbarmi(Generic_Index)*vlxremi(Generic_Index)+spdbarpl(Generic_Index)
          *vlxrepl(Generic_Index))-(prsrepl(Generic_Index)-prsremi(Generic_Index)))/(spdbarmi(Generic_Index)
          +spdbarpl(Generic_Index)); 
          rhobarpl(Generic_Index)=1.0/rhorepl(Generic_Index)-(vlxbar(Generic_Index)
          -vlxrepl(Generic_Index))/spdbarpl(Generic_Index); 
          rhobarpl(Generic_Index).replace(rhobarpl(Generic_Index)<=0.0,1.0
           / rhofloor); 
          rhobarmi(Generic_Index)=1.0/rhoremi(Generic_Index)+(vlxbar(Generic_Index)
          -vlxremi(Generic_Index))/spdbarmi(Generic_Index); 
          rhobarmi(Generic_Index).replace(rhobarmi(Generic_Index)<=0.0,1.0
           / rhofloor); 
     ...
     where (wavspd1(Generic_Index)>=0.0)
       ...
       elsewhere (wavspd2(Generic_Index)>=0.0)
        { 
          tempac=tempaa*((vlxremi(Generic_Index)*rhoremi(Generic_Index))/lsdremi(Generic_Index)
          +tempab); 
          ...
          prs0(Generic_Index)=(rho0(Generic_Index)*pow(sdsp0(Generic_Index),2.0))
          /gamma; 
       ...
       elsewhere (wavspd5(Generic_Index)>=0.0)
        { 
          tempac=tempaa*((-((vlxrepl(Generic_Index)*rhorepl(Generic_Index))/lsdrepl(Generic_Index)))
          +tempab); 
          tempac.replace(tempac>=0.0,tempac); 
 ...
 { 
      ...
      +0.5*((pow(vlx0(Generic_Index),2.0)+pow(vly0(Generic_Index),2.0))
...
int main(int argc,char * * argv)
   { 
     ...
     Optimization_Manager::Initialize_Virtual_Machine("/n/elf0/dquinlan/A++P++.4.1.3.experimental/APPLICATIONS/p++_riemann",
     ...
     int nriemann=SIZE - 2; 
     double prsfac=1.0; 
     double rhofloor=1e-08; 
     double prsfloor=1e-16; 
     double gamma=1.4; 
     int pprieman=1; 
     int rieiter=5; 
...
int i=0; i < 1; i++)
          { 
          One_Dimensional_Goduov_Solver(nriemann,prsfac,rhofloor,prsfloor,gamma,pprieman,
          rieiter,rhoremi,rhorepl,prsremi,prsrepl,vlxremi,vlxrepl,vlyremi,vlyrepl,vlzremi,
          vlzrepl,rhoflux,engflux,vlxflux,vlyflux,vlzflux,rho0,prs0,vlx0,vly0,vlz0,sdsp0); } 
     long End_Clock=clock(); 
     printf("Time for solve %ld \n",(End_Clock - Start_Clock) / 1000); 
     if (
     fabs((sdsp0(((sdsp0.getLocalBase)(0) + 1)) - 6.4949)) > 0.05)
        { 
