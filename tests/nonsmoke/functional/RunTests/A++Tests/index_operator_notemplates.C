    ...
    Index_Pointer_List ) const
{
     ...
     for (i=0; i < MAX_ARRAY_DIMENSION; i++)
        {
          if (Index_Pointer_List[i] != NULL)
             {
            // Legacy compilers will not allow us to take the address of
  ...
  // subscript(might want to make this a SerialArray function) ...
     ...
     // for the P++ code so that it can work properly with legacy toolchains
...
doubleArray doubleArray::operator() ( const Internal_Indirect_Addressing_Index & I ) const
{
   ...
   //  This may be less inefficient on some legacy systems and so we may have to
   //  modify this later. ...
