...
a combination of platform and communication paradigm. On Linux, set
ARCHDIR to one of the provided {\em linux*.gnu} files (for example,
{\em linux.gnu} or {\em linux\_alpha.gnu}) depending on whether you want
serial, MPI, OpenMP, or hybrid MPI+OpenMP settings. The {\em archdir.gnu} file contains
...
with a reference 20-step run on 4 processors captured from a Linux system.
The results from other Linux configurations should
