...
int main(int argc, char **argv) {
  ...
  SUMMARIZE_ERR;
  printf("*** Checking that HDF5 file can be read by netCDF-4, and adding an "
         "att...");
  {
    int ncid;
    ...
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdimid_in))
      ERR;
    if (ndims_in != 2 || nvars_in != 3 || natts_in != 0 || unlimdimid_in != -1)
      ERR;
    if (nc_inq_varid(ncid, PRES_NAME, &varid_in))
      ERR;
    if (nc_inq_var(ncid, varid_in, name_in, &xtype_in, &ndims_in, dimids_in,
                   &natts_in))
      ERR;
    if (strcmp(name_in, PRES_NAME) || xtype_in != NC_FLOAT || ndims_in != 2 ||
        natts_in != 0)
      ERR;
    if (nc_inq_dim(ncid, dimids_in[0], name_in, &len_in))
      ERR;
    if (len_in != LAT_LEN || strcmp(name_in, LAT_NAME))
      ERR;
    if (nc_inq_dim(ncid, dimids_in[1], name_in, &len_in))
      ERR;
    ...
    /* Change some data. */
    index[0] = index[1] = 0;
    if (nc_put_var1_float(ncid, varid_in, index, &new_float))
      ERR;

    /* Just for swank, add an attribute. Now we're talking
     * interoperabity, dude! */
    if (nc_put_att_text(ncid, NC_GLOBAL, ATT_NAME, strlen(song) + 1, song))
      ERR;
  ...
  SUMMARIZE_ERR;
  printf("*** Checking that one var, two dimscales, one att file can still be "
         "read by HDF5...");

  {
    ...
    /* Check it out. */
    if ((datasetid = H5Dopen1(grpid, PRES_NAME)) < 0)
      ERR;
    if ((spaceid = H5Dget_space(datasetid)) < 0)
      ERR;
    if (H5Dread(datasetid, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                pres_in) < 0)
      ERR;
    if (pres_in[0][0] != NEW_FLOAT)
      ERR;
    if ((attid = H5Aopen_name(grpid, ATT_NAME)) < 0)
      ERR;
    if ((typeid = H5Aget_type(attid)) < 0)
      ERR;
    if (H5Aread(attid, typeid, song_in) < 0)
      ERR;
    if (strcmp(song, song_in))
      ERR;

    /* Close up the shop. */
    if (H5Tclose(typeid) < 0 || H5Aclose(attid) < 0 || H5Sclose(spaceid) < 0 ||
        H5Dclose(datasetid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
  SUMMARIZE_ERR;
  printf("*** Creating a HDF5 file with one var and no dimension scales...");
  {
    ...
    if (nc_open(FILE_NAME, NC_NOWRITE, &ncid))
      ERR;
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdim_in))
      ERR;
    if (ndims_in != 2 || nvars_in != 1 || natts_in != 0 || unlimdim_in != -1)
      ERR;
    if (nc_inq_dim(ncid, 0, NULL, &len_in))
      ERR;
    if (len_in != LAT_LEN)
      ERR;
    if (nc_inq_dim(ncid, 1, NULL, &len_in))
      ERR;
    if (len_in != LON_LEN)
      ERR;
    if (nc_close(ncid))
  ...
  SUMMARIZE_ERR;
  printf("*** Creating a HDF5 file with two vars and no dimension scales, "
         "without creation ordering...");
  {
    ...
    dims[0] = LAT_LEN;
    dims[1] = LON_LEN;
    if ((spaceid = H5Screate_simple(DIMS_2, dims, dims)) < 0)
      ERR;
    ...
    /* Ring down the curtain. */
    if (H5Dclose(pres_datasetid) < 0 || H5Dclose(temp_datasetid) < 0 ||
    ...
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdim_in))
      ERR;
    if (ndims_in != 2 || nvars_in != 2 || natts_in != 0 || unlimdim_in != -1)
      ERR;
    if (nc_inq_dim(ncid, 0, NULL, &len_in))
      ERR;
    if (len_in != LAT_LEN)
...
#define MAX_LEN 6
    ...
    if ((typeid = H5Tcopy(H5T_C_S1)) < 0)
      ERR;
    if (H5Tset_size(typeid, MAX_LEN + 1) < 0)
      ERR;

    /* Write an attribute of this (string) type. */
    if ((spaceid = H5Screate_simple(1, dims, NULL)) < 0)
      ERR;
    if ((attid = H5Acreate(fileid, ATT_NAME2, typeid, spaceid, H5P_DEFAULT)) <
        0)
      ERR;
    if (H5Awrite(attid, typeid, data) < 0)
      ERR;
    ...
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDWR, H5P_DEFAULT)) < 0)
      ERR;
    if ((attid = H5Aopen_name(fileid, ATT_NAME2)) < 0)
      ERR;
    if ((typeid = H5Aget_type(attid)) < 0)
      ERR;
    ...
    if (H5Sget_simple_extent_dims(spaceid, dims, NULL) < 0)
      ERR;
    if (!(data_in = malloc(dims[0] * sizeof(char *) * (MAX_LEN + 1))))
      ERR;
    ...
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdim_in))
      ERR;
    if (ndims_in != 0 || nvars_in != 0 || natts_in != 1 || unlimdim_in != -1)
      ERR;
    if (nc_inq_att(ncid, NC_GLOBAL, ATT_NAME2, &type_in, &size_in))
      ERR;
    if (type_in != NC_STRING || size_in != ATT_LEN)
      ERR;
    if (nc_get_att_string(ncid, NC_GLOBAL, ATT_NAME2, data_in2))
      ERR;
    for (i = 0; i < size_in; i++)
      if (strcmp(data[i], data_in2[i]))
        ERR;
    if (nc_free_string(size_in, data_in2))
      ERR;
    ...
    if (H5Pset_fclose_degree(fapl_id, H5F_CLOSE_STRONG))
      ERR;
#endif
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, fapl_id)) < 0)
      ERR;
    if (H5Pclose(fapl_id) < 0)
      ERR;
    if (H5Fclose(fileid) < 0)
  ...
  SUMMARIZE_ERR;
  printf(
      "*** Checking a HDF5 file with scalar, fixed-length string dataset...");
  {
#define VAR_NAME "Gettysburg Address"
    ...
    if ((typeid = H5Tcopy(H5T_C_S1)) < 0)
      ERR;
    if (H5Tset_size(typeid, strlen(data) + 1) < 0)
      ERR;
    ...
    if ((plistid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
      ERR;
    if (H5Pset_fill_value(plistid, typeid, &empty) < 0)
      ERR;
    if ((datasetid = H5Dcreate1(grpid, VAR_NAME, typeid, spaceid, plistid)) < 0)
      ERR;
    if (H5Dwrite(datasetid, typeid, spaceid, spaceid, H5P_DEFAULT, data) < 0)
      ERR;
    ...
    /*       if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdim_in))
     * ERR; */
    /*       if (ndims_in != 0 || nvars_in != 1 || natts_in != 0 || unlimdim_in
     * != -1) ERR; */
    /*       if (nc_inq_att(ncid, NC_GLOBAL, ATT_NAME2, &type_in, &size_in))
     * ERR; */
   ...
   * &natts_in)) ERR; */
  /*       if (strcmp(var_name_in, D0_NAME) || xtype_in != NC_CHAR || ndims_in
   * != 2 || */
