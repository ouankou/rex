...
int nc4_reopen_dataset(NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var) {
  ...
  if (var->hdf_datasetid) {
    ...
    if (H5Dclose(var->hdf_datasetid) < 0)
      return NC_EHDFERR;
    if ((var->hdf_datasetid = H5Dopen2(grp->hdf_grpid, var->name, access_pid)) <
        0)
      return NC_EHDFERR;
...
int nc_get_var_chunk_cache_ints(int ncid, int varid, int *sizep, int *nelemsp,
  ...
  if ((ret = nc_get_var_chunk_cache(ncid, varid, &real_size, &real_nelems,
                                    &real_preemption)))
    return ret;
 ...
 * dimension). */
int nc4_find_default_chunksizes(NC_VAR_INFO_T *var) {
  int d;
  ...
  for (d = 0; d < var->ndims; d++)
    if (var->dim[d]->unlimited)
      ...
      var->chunksizes[d] =
          (pow((double)DEFAULT_CHUNK_SIZE / (num_values * type_size),
               1 / (double)(var->ndims - num_unlim)) *
               var->dim[d]->len +
           .5);
 ...
 * down! */
static int nc_def_var_nc4(int ncid, const char *name, nc_type xtype, int ndims,
  ...
  if (xtype > NC_STRING)
    ...
    if ((retval =
             nc4_get_hdf_typeid(h5, var->xtype, &var->type_info->hdf_typeid,
                                var->type_info->endianness)))
      return retval;
    if ((var->type_info->native_typeid = H5Tget_native_type(
             var->type_info->hdf_typeid, H5T_DIR_DEFAULT)) < 0)
      return NC_EHDFERR;
...
int nc_def_var(int ncid, const char *name, nc_type xtype, int ndims,
  ...
  LOG((2, "nc_def_var: ncid 0x%x name %s xtype %d ndims %d", ncid, name, xtype,
       ndims));
  ...
  if (nc->pnetcdf_file) {
    ...
    ret = ncmpi_def_var(nc->int_ncid, name, xtype, ndims, dimidsp, varidp);
  ...
  if (!nc->nc4_info)
    return nc3_def_var(nc->int_ncid, name, xtype, ndims, dimidsp, varidp);
 ...
 * the user. */
static int nc_inq_var_all(int ncid, int varid, char *name, nc_type *xtypep,
                          ...
                          void *fill_valuep, int *endiannessp,
  ...
  /* Take care of files created/opened with parallel-netcdf library. */
  if (nc->pnetcdf_file)
    return ncmpi_inq_var(nc->int_ncid, varid, name, xtypep, ndimsp, dimidsp,
                         nattsp);
#endif /* USE_PNETCDF */
  ...
  if (!h5) {
    ...
    return nc3_inq_var(nc->int_ncid, varid, name, xtypep, ndimsp, dimidsp,
                       nattsp);
  }
  assert(nc && grp && h5);
...
int nc_inq_var(int ncid, int varid, char *name, nc_type *xtypep, int *ndimsp,
               int *dimidsp, int *nattsp) {
  return nc_inq_var_all(ncid, varid, name, xtypep, ndimsp, dimidsp, nattsp,
...
static int nc_def_var_extra(int ncid, int varid, int *shuffle, int *deflate,
  ...
  if ((deflate && options_mask) || (deflate && !deflate_level) ||
  ...
  if (fill_value && !var->no_fill) {
    ...
    if ((retval =
             nc_put_att(ncid, varid, _FillValue, var->xtype, 1, fill_value)))
      return retval;
  }
...
int nc_def_var_deflate(int ncid, int varid, int shuffle, int deflate,
                       int deflate_level) {
  return nc_def_var_extra(ncid, varid, &shuffle, &deflate, &deflate_level, NULL,
                          0, NULL, NULL, NULL, NULL, NULL, NULL);
}

/* Find out compression settings of a var. */
int nc_inq_var_deflate(int ncid, int varid, int *shufflep, int *deflatep,
                       int *deflate_levelp) {
  return nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, shufflep,
                        deflatep, deflate_levelp, NULL, NULL, NULL, NULL, NULL,
                        NULL, NULL, NULL);
}
...
int nc_inq_var_szip(int ncid, int varid, int *options_maskp,
  ...
  return nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL, NULL, NULL, NULL, NULL, NULL, NULL, options_maskp,
                        pixels_per_blockp);
}
...
int nc_def_var_fletcher32(int ncid, int varid, int fletcher32) {
  return nc_def_var_extra(ncid, varid, NULL, NULL, NULL, &fletcher32, 0, NULL,
...
int nc_inq_var_fletcher32(int ncid, int varid, int *fletcher32p) {
  return nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL, fletcher32p, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL);
}
...
*/
int nc_def_var_chunking(int ncid, int varid, int contiguous,
                        const size_t *chunksizesp) {
  return nc_def_var_extra(ncid, varid, NULL, NULL, NULL, NULL, contiguous,
 ...
 * problems. */
int nc_inq_var_chunking_ints(int ncid, int varid, int *contiguousp,
                             int *chunksizesp) {
  NC_FILE_INFO_T *nc;
  NC_GRP_INFO_T *grp;
  NC_HDF5_FILE_INFO_T *h5;
  ...
  retval =
      nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                     NULL, NULL, contiguousp, cs, NULL, NULL, NULL, NULL, NULL);
 ...
 * this function. */
int nc_def_var_chunking_ints(int ncid, int varid, int contiguous,
                             int *chunksizesp) {
  NC_FILE_INFO_T *nc;
  NC_GRP_INFO_T *grp;
  NC_HDF5_FILE_INFO_T *h5;
  ...
  retval = nc_def_var_extra(ncid, varid, NULL, NULL, NULL, NULL, contiguous, cs,
                            NULL, NULL, NULL, NULL, NULL);
...
int nc_inq_var_chunking(int ncid, int varid, int *contiguousp,
                        size_t *chunksizesp) {
  return nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL, NULL, contiguousp, chunksizesp, NULL, NULL, NULL,
                        NULL, NULL);
}
...
int nc_def_var_fill(int ncid, int varid, int no_fill, const void *fill_value) {
  return nc_def_var_extra(ncid, varid, NULL, NULL, NULL, NULL, 0, NULL,
                          &no_fill, fill_value, NULL, NULL, NULL);
}

/* Inq fill value setting for a var. */
int nc_inq_var_fill(int ncid, int varid, int *no_fillp, void *fill_valuep) {
  return nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL, NULL, NULL, NULL, no_fillp, fill_valuep, NULL,
                        NULL, NULL);
}

/* Define the endianness of a variable. */
int nc_def_var_endian(int ncid, int varid, int endianness) {
  return nc_def_var_extra(ncid, varid, NULL, NULL, NULL, NULL, 0, NULL, NULL,
                          NULL, &endianness, NULL, NULL);
}

/* Learn about the endianness of a variable. */
int nc_inq_var_endian(int ncid, int varid, int *endiannessp) {
  return nc_inq_var_all(ncid, varid, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL, NULL, NULL, NULL, NULL, NULL, endiannessp, NULL,
                        NULL);
}
...
int nc_inq_vartype(int ncid, int varid, nc_type *xtypep) {
  return nc_inq_var(ncid, varid, NULL, xtypep, NULL, NULL, NULL);
}
...
int nc_rename_var(int ncid, int varid, const char *name) {
  ...
  LOG((2, "nc_rename_var: ncid 0x%x varid %d name %s", ncid, varid, name));
...
int nc_copy_var(int ncid_in, int varid_in, int ncid_out) {
  ...
  if ((retval =
           nc_inq_var(ncid_in, varid_in, name, &xtype, &ndims, dimids, &natts)))
    return retval;

  LOG((2, "nc_copy_var: ncid_in 0x%x varid_in %d ncid_out 0x%x", ncid_in,
       varid_in, ncid_out));
  ...
  if (retval && retval != NC_EINDEFINE)
    BAIL(retval);
  if ((retval = nc_def_var(ncid_out, name, xtype, ndims, dimids, &varid_out)))
    BAIL(retval);
  ...
  for (a = 0; a < natts; a++) {
    if ((retval = nc_inq_attname(ncid_in, varid_in, a, att_name)))
      BAIL(retval);
    if ((retval =
             nc_copy_att(ncid_in, varid_in, att_name, ncid_out, varid_out)))
      BAIL(retval);
  }
  ...
  for (start[0] = 0; !retval && start[0] < (size_t)dimlen[0]; start[0]++) {
    ...
    case NC_BYTE:
      ...
      break;
    case NC_CHAR:
      retval = nc_get_vara_text(ncid_in, varid_in, start, count, (char *)data);
      if (!retval)
        retval =
            nc_put_vara_text(ncid_out, varid_out, start, count, (char *)data);
      break;
    case NC_SHORT:
      retval =
          nc_get_vara_short(ncid_in, varid_in, start, count, (short *)data);
      if (!retval)
        retval =
            nc_put_vara_short(ncid_out, varid_out, start, count, (short *)data);
      break;
    case NC_INT:
      retval = nc_get_vara_int(ncid_in, varid_in, start, count, (int *)data);
      if (!retval)
        retval =
            nc_put_vara_int(ncid_out, varid_out, start, count, (int *)data);
      break;
    case NC_FLOAT:
      retval =
          nc_get_vara_float(ncid_in, varid_in, start, count, (float *)data);
      if (!retval)
        retval =
            nc_put_vara_float(ncid_out, varid_out, start, count, (float *)data);
      break;
    case NC_DOUBLE:
      retval =
          nc_get_vara_double(ncid_in, varid_in, start, count, (double *)data);
      if (!retval)
...
int nc_var_par_access(int ncid, int varid, int par_access) {
  ...
  LOG((1, "nc_var_par_access: ncid 0x%x varid %d par_access %d", ncid, varid,
       par_access));
...
static int nc4_put_var1_tc(int ncid, int varid, nc_type file_type,
                           nc_type mem_type, int mem_type_is_long,
                           const size_t *indexp, const void *op) {
  NC_FILE_INFO_T *nc;
  ...
  return (nc4_pg_var1(PUT, nc, ncid, varid, indexp, mem_type, mem_type_is_long,
                      (void *)op));
}

/* Get one little itsy-bitsy datum, with type conversion. */
static int nc4_get_var1_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *indexp,
                           void *ip) {
  NC_FILE_INFO_T *nc;
  ...
  /* Handle netcdf-4 cases. */
  return (nc4_pg_var1(GET, nc, ncid, varid, indexp, mem_type, mem_type_is_long,
                      ip));
}
...
static int nc4_put_vara_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *startp,
  ...
  if (nc->pnetcdf_file) {
    ...
    if (nc->pnetcdf_access_mode == NC_INDEPENDENT) {
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_put_vara_schar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    op);
      case NC_UBYTE:
        return ncmpi_put_vara_uchar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    op);
      case NC_CHAR:
        return ncmpi_put_vara_text(nc->int_ncid, varid, mpi_start, mpi_count,
                                   op);
      case NC_SHORT:
        return ncmpi_put_vara_short(nc->int_ncid, varid, mpi_start, mpi_count,
                                    op);
      case NC_INT:
        return ncmpi_put_vara_int(nc->int_ncid, varid, mpi_start, mpi_count,
                                  op);
      case NC_FLOAT:
        return ncmpi_put_vara_float(nc->int_ncid, varid, mpi_start, mpi_count,
                                    op);
      case NC_DOUBLE:
        return ncmpi_put_vara_double(nc->int_ncid, varid, mpi_start, mpi_count,
                                     op);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
      ...
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_put_vara_schar_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, op);
      case NC_UBYTE:
        return ncmpi_put_vara_uchar_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, op);
      case NC_CHAR:
        return ncmpi_put_vara_text_all(nc->int_ncid, varid, mpi_start,
                                       mpi_count, op);
      case NC_SHORT:
        return ncmpi_put_vara_short_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, op);
      case NC_INT:
        return ncmpi_put_vara_int_all(nc->int_ncid, varid, mpi_start, mpi_count,
                                      op);
      case NC_FLOAT:
        return ncmpi_put_vara_float_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, op);
      case NC_DOUBLE:
        return ncmpi_put_vara_double_all(nc->int_ncid, varid, mpi_start,
                                         mpi_count, op);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
...
int nc4_get_hdf4_vara(NC_FILE_INFO_T *nc, int ncid, int varid,
...
/* Get an array. */
static int nc4_get_vara_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *startp,
  ...
  if (nc->pnetcdf_file) {
    ...
    if (nc->pnetcdf_access_mode == NC_INDEPENDENT) {
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_get_vara_schar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    ip);
      case NC_UBYTE:
        return ncmpi_get_vara_uchar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    ip);
      case NC_CHAR:
        return ncmpi_get_vara_text(nc->int_ncid, varid, mpi_start, mpi_count,
                                   ip);
      case NC_SHORT:
        return ncmpi_get_vara_short(nc->int_ncid, varid, mpi_start, mpi_count,
                                    ip);
      case NC_INT:
        return ncmpi_get_vara_int(nc->int_ncid, varid, mpi_start, mpi_count,
                                  ip);
      case NC_FLOAT:
        return ncmpi_get_vara_float(nc->int_ncid, varid, mpi_start, mpi_count,
                                    ip);
      case NC_DOUBLE:
        return ncmpi_get_vara_double(nc->int_ncid, varid, mpi_start, mpi_count,
                                     ip);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
      ...
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_get_vara_schar_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, ip);
      case NC_UBYTE:
        return ncmpi_get_vara_uchar_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, ip);
      case NC_CHAR:
        return ncmpi_get_vara_text_all(nc->int_ncid, varid, mpi_start,
                                       mpi_count, ip);
      case NC_SHORT:
        return ncmpi_get_vara_short_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, ip);
      case NC_INT:
        return ncmpi_get_vara_int_all(nc->int_ncid, varid, mpi_start, mpi_count,
                                      ip);
      case NC_FLOAT:
        return ncmpi_get_vara_float_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, ip);
      case NC_DOUBLE:
        return ncmpi_get_vara_double_all(nc->int_ncid, varid, mpi_start,
                                         mpi_count, ip);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
...
static int nc4_put_vars_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *startp,
  ...
  if (nc->pnetcdf_file) {
    ...
    if (nc->pnetcdf_access_mode == NC_INDEPENDENT) {
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_put_vars_schar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, op);
      case NC_UBYTE:
        return ncmpi_put_vars_uchar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, op);
      case NC_CHAR:
        return ncmpi_put_vars_text(nc->int_ncid, varid, mpi_start, mpi_count,
                                   mpi_stride, op);
      case NC_SHORT:
        return ncmpi_put_vars_short(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, op);
      case NC_INT:
        return ncmpi_put_vars_int(nc->int_ncid, varid, mpi_start, mpi_count,
                                  mpi_stride, op);
      case NC_FLOAT:
        return ncmpi_put_vars_float(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, op);
      case NC_DOUBLE:
        return ncmpi_put_vars_double(nc->int_ncid, varid, mpi_start, mpi_count,
                                     mpi_stride, op);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
      ...
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_put_vars_schar_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, mpi_stride, op);
      case NC_UBYTE:
        return ncmpi_put_vars_uchar_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, mpi_stride, op);
      case NC_CHAR:
        return ncmpi_put_vars_text_all(nc->int_ncid, varid, mpi_start,
                                       mpi_count, mpi_stride, op);
      case NC_SHORT:
        return ncmpi_put_vars_short_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, mpi_stride, op);
      case NC_INT:
        return ncmpi_put_vars_int_all(nc->int_ncid, varid, mpi_start, mpi_count,
                                      mpi_stride, op);
      case NC_FLOAT:
        return ncmpi_put_vars_float_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, mpi_stride, op);
      case NC_DOUBLE:
        return ncmpi_put_vars_double_all(nc->int_ncid, varid, mpi_start,
                                         mpi_count, mpi_stride, op);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
  ...
  if (!nc->nc4_info) {
    ...
    case NC_BYTE:
      return nc3_put_vars_schar(nc->int_ncid, varid, startp, countp, stridep,
                                op);
    case NC_CHAR:
      return nc3_put_vars_text(nc->int_ncid, varid, startp, countp, stridep,
                               op);
    case NC_SHORT:
      return nc3_put_vars_short(nc->int_ncid, varid, startp, countp, stridep,
                                op);
    case NC_INT:
      if (mem_type_is_long)
        return nc3_put_vars_long(nc->int_ncid, varid, startp, countp, stridep,
                                 op);
      else
        return nc3_put_vars_int(nc->int_ncid, varid, startp, countp, stridep,
                                op);
    case NC_FLOAT:
      return nc3_put_vars_float(nc->int_ncid, varid, startp, countp, stridep,
                                op);
    case NC_DOUBLE:
      return nc3_put_vars_double(nc->int_ncid, varid, startp, countp, stridep,
                                 op);
    default:
      return NC_EBADTYPE;
    }
...
static int nc4_get_vars_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *startp,
                           const size_t *countp, const ptrdiff_t *stridep,
                           void *ip) {
  NC_FILE_INFO_T *nc;
  ...
  if (nc->pnetcdf_file) {
    ...
    if (nc->pnetcdf_access_mode == NC_INDEPENDENT) {
      switch (mem_type) {
      case NC_BYTE:
        return ncmpi_get_vars_schar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, ip);
      case NC_UBYTE:
        return ncmpi_get_vars_uchar(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, ip);
      case NC_CHAR:
        return ncmpi_get_vars_text(nc->int_ncid, varid, mpi_start, mpi_count,
                                   mpi_stride, ip);
      case NC_SHORT:
        return ncmpi_get_vars_short(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, ip);
      case NC_INT:
        return ncmpi_get_vars_int(nc->int_ncid, varid, mpi_start, mpi_count,
                                  mpi_stride, ip);
      case NC_FLOAT:
        return ncmpi_get_vars_float(nc->int_ncid, varid, mpi_start, mpi_count,
                                    mpi_stride, ip);
      case NC_DOUBLE:
        return ncmpi_get_vars_double(nc->int_ncid, varid, mpi_start, mpi_count,
                                     mpi_stride, ip);
      case NC_NAT:
      default:
        return NC_EBADTYPE;
      ...
      case NC_SHORT:
        return ncmpi_get_vars_short_all(nc->int_ncid, varid, mpi_start,
                                        mpi_count, mpi_stride, ip);
      case NC_INT:
        return ncmpi_get_vars_int_all(nc->int_ncid, varid, mpi_start, mpi_count,
                                      mpi_stride, ip);
      case NC_FLOAT:
  ...
  if (!nc->nc4_info) {
    ...
    case NC_BYTE:
      return nc3_get_vars_schar(nc->int_ncid, varid, startp, countp, stridep,
                                ip);
    case NC_UBYTE:
      return nc3_get_vars_uchar(nc->int_ncid, varid, startp, countp, stridep,
                                ip);
    case NC_CHAR:
      return nc3_get_vars_text(nc->int_ncid, varid, startp, countp, stridep,
                               ip);
    case NC_SHORT:
      return nc3_get_vars_short(nc->int_ncid, varid, startp, countp, stridep,
                                ip);
    case NC_INT:
      if (mem_type_is_long)
        return nc3_get_vars_long(nc->int_ncid, varid, startp, countp, stridep,
                                 ip);
      else
        return nc3_get_vars_int(nc->int_ncid, varid, startp, countp, stridep,
                                ip);
    case NC_FLOAT:
      return nc3_get_vars_float(nc->int_ncid, varid, startp, countp, stridep,
                                ip);
    case NC_DOUBLE:
      return nc3_get_vars_double(nc->int_ncid, varid, startp, countp, stridep,
                                 ip);
    default:
      return NC_EBADTYPE;
    }
...
int nc_put_vars(int ncid, int varid, const size_t *startp, const size_t *countp,
                const ptrdiff_t *stridep, const void *op) {
  return nc4_put_vars_tc(ncid, varid, NC_NAT, 0, startp, countp, stridep, op);
}

int nc_get_vars(int ncid, int varid, const size_t *startp, const size_t *countp,
                const ptrdiff_t *stridep, void *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_NAT, 0, startp, countp, stridep, ip);
}

int nc_put_vars_text(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep,
                     const char *op) {
  return nc4_put_vars_tc(ncid, varid, NC_CHAR, 0, startp, countp, stridep, op);
}

int nc_get_vars_text(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep, char *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_CHAR, 0, startp, countp, stridep, ip);
}

int nc_put_vars_uchar(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      const unsigned char *op) {
  return nc4_put_vars_tc(ncid, varid, NC_UBYTE, 0, startp, countp, stridep, op);
}

int nc_get_vars_uchar(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      unsigned char *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_UBYTE, 0, startp, countp, stridep, ip);
}

int nc_put_vars_schar(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      const signed char *op) {
  return nc4_put_vars_tc(ncid, varid, NC_BYTE, 0, startp, countp, stridep, op);
}

int nc_get_vars_schar(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      signed char *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_BYTE, 0, startp, countp, stridep, ip);
}

int nc_put_vars_short(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      const short *op) {
  return nc4_put_vars_tc(ncid, varid, NC_SHORT, 0, startp, countp, stridep, op);
}

int nc_get_vars_short(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      short *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_SHORT, 0, startp, countp, stridep, ip);
}

int nc_put_vars_int(int ncid, int varid, const size_t *startp,
                    const size_t *countp, const ptrdiff_t *stridep,
                    const int *op) {
  return nc4_put_vars_tc(ncid, varid, NC_INT, 0, startp, countp, stridep, op);
}

int nc_get_vars_int(int ncid, int varid, const size_t *startp,
                    const size_t *countp, const ptrdiff_t *stridep, int *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_INT, 0, startp, countp, stridep, ip);
}

int nc_put_vars_long(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep,
                     const long *op) {
  return nc4_put_vars_tc(ncid, varid, NC_INT, 1, startp, countp, stridep, op);
}

int nc_get_vars_long(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep, long *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_INT, 1, startp, countp, stridep, ip);
}

int nc_put_vars_float(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      const float *op) {
  return nc4_put_vars_tc(ncid, varid, NC_FLOAT, 0, startp, countp, stridep, op);
}

int nc_get_vars_float(int ncid, int varid, const size_t *startp,
                      const size_t *countp, const ptrdiff_t *stridep,
                      float *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_FLOAT, 0, startp, countp, stridep, ip);
}

int nc_put_vars_double(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       const double *op) {
  return nc4_put_vars_tc(ncid, varid, NC_DOUBLE, 0, startp, countp, stridep,
                         op);
}
...
int nc_get_vars_double(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       double *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_DOUBLE, 0, startp, countp, stridep,
                         ip);
}

static int nc4_put_varm_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *startp,
                           const size_t *countp, const ptrdiff_t *stridep,
  ...
  if (!nc->nc4_info) {
    ...
    switch (mem_type) {
    case NC_NAT:
      return nc3_put_varm(nc->int_ncid, varid, startp, countp, stridep, imapp,
                          op);
    case NC_BYTE:
      return nc3_put_varm_schar(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, op);
    case NC_CHAR:
      return nc3_put_varm_text(nc->int_ncid, varid, startp, countp, stridep,
                               imapp, op);
    case NC_SHORT:
      return nc3_put_varm_short(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, op);
    case NC_INT:
      if (mem_type_is_long)
        return nc3_put_varm_long(nc->int_ncid, varid, startp, countp, stridep,
                                 imapp, op);
      else
        return nc3_put_varm_int(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, op);
    case NC_FLOAT:
      return nc3_put_varm_float(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, op);
    case NC_DOUBLE:
      return nc3_put_varm_double(nc->int_ncid, varid, startp, countp, stridep,
                                 imapp, op);
    default:
      return NC_EBADTYPE;
    }
  ...
  return nc4_pg_varm(PUT, nc, ncid, varid, startp, countp, stridep, imapp,
                     mem_type, mem_type_is_long, (void *)op);
}

static int nc4_get_varm_tc(int ncid, int varid, nc_type mem_type,
                           int mem_type_is_long, const size_t *startp,
                           const size_t *countp, const ptrdiff_t *stridep,
  ...
  if (!nc->nc4_info) {
    switch (mem_type) {
    case NC_NAT:
      return nc3_get_varm(nc->int_ncid, varid, startp, countp, stridep, imapp,
                          ip);
    case NC_BYTE:
      return nc3_get_varm_schar(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, ip);
    case NC_UBYTE:
      return nc3_get_varm_uchar(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, ip);
    case NC_CHAR:
      return nc3_get_varm_text(nc->int_ncid, varid, startp, countp, stridep,
                               imapp, ip);
    case NC_SHORT:
      return nc3_get_varm_short(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, ip);
    case NC_INT:
      if (mem_type_is_long)
        return nc3_get_varm_long(nc->int_ncid, varid, startp, countp, stridep,
                                 imapp, ip);
      else
        return nc3_get_varm_int(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, ip);
    case NC_FLOAT:
      return nc3_get_varm_float(nc->int_ncid, varid, startp, countp, stridep,
                                imapp, ip);
    case NC_DOUBLE:
      return nc3_get_varm_double(nc->int_ncid, varid, startp, countp, stridep,
                                 imapp, ip);
    default:
      return NC_EBADTYPE;
    }
  ...
  return nc4_pg_varm(GET, nc, ncid, varid, startp, countp, stridep, imapp,
                     mem_type, mem_type_is_long, (void *)ip);
}

int nc_put_varm(int ncid, int varid, const size_t *startp, const size_t *countp,
                const ptrdiff_t *stridep, const ptrdiff_t *imapp,
                const void *op) {
  return nc4_put_varm_tc(ncid, varid, NC_NAT, 0, startp, countp, stridep, imapp,
                         op);
}

int nc_get_varm(int ncid, int varid, const size_t *startp, const size_t *countp,
                const ptrdiff_t *stridep, const ptrdiff_t *imapp, void *ip) {
  return nc4_get_varm_tc(ncid, varid, NC_NAT, 0, startp, countp, stridep, imapp,
                         ip);
}
...
int nc_put_varm_text(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_CHAR, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_text(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_CHAR, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_uchar(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_UBYTE, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_uchar(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_UBYTE, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_schar(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_BYTE, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_schar(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_BYTE, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_short(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_SHORT, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_short(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_SHORT, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_int(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_INT, 0, startp, countp, stridep, imapp,
                         op);
}
...
int nc_get_varm_int(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_INT, 0, startp, countp, stridep, imapp,
                         ip);
}
...
int nc_put_varm_long(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_INT, 1, startp, countp, stridep, imapp,
                         op);
}
...
int nc_get_varm_long(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_INT, 1, startp, countp, stridep, imapp,
                         ip);
}
...
int nc_put_varm_float(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_FLOAT, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_float(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_FLOAT, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_double(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_DOUBLE, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_double(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_DOUBLE, 0, startp, countp, stridep,
                         imapp, ip);
}

static int nc4_put_var_tc(int ncid, int varid, nc_type mem_type,
                          int mem_type_is_long, const void *op) {
  NC_FILE_INFO_T *nc;
  int ret;
  ...
  return pg_var(PUT, nc, ncid, varid, mem_type, mem_type_is_long, (void *)op);
}

static int nc4_get_var_tc(int ncid, int varid, nc_type mem_type,
...
int nc_put_vars_ushort(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       const unsigned short *op) {
  return nc4_put_vars_tc(ncid, varid, NC_USHORT, 0, startp, countp, stridep,
                         op);
}
...
int nc_get_vars_ushort(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       unsigned short *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_USHORT, 0, startp, countp, stridep,
                         ip);
}
...
int nc_put_vars_uint(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep,
                     const unsigned int *op) {
  return nc4_put_vars_tc(ncid, varid, NC_UINT, 0, startp, countp, stridep, op);
}

int nc_get_vars_uint(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep,
                     unsigned int *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_UINT, 0, startp, countp, stridep, ip);
}

int nc_put_vars_longlong(int ncid, int varid, const size_t *startp,
                         const size_t *countp, const ptrdiff_t *stridep,
                         const long long *op) {
  return nc4_put_vars_tc(ncid, varid, NC_INT64, 0, startp, countp, stridep, op);
}

int nc_get_vars_longlong(int ncid, int varid, const size_t *startp,
                         const size_t *countp, const ptrdiff_t *stridep,
                         long long *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_INT64, 0, startp, countp, stridep, ip);
}

int nc_put_vars_ulonglong(int ncid, int varid, const size_t *startp,
                          const size_t *countp, const ptrdiff_t *stridep,
                          const unsigned long long *op) {
  return nc4_put_vars_tc(ncid, varid, NC_UINT64, 0, startp, countp, stridep,
                         op);
}
...
int nc_get_vars_ulonglong(int ncid, int varid, const size_t *startp,
                          const size_t *countp, const ptrdiff_t *stridep,
                          unsigned long long *ip) {
  return nc4_get_vars_tc(ncid, varid, NC_UINT64, 0, startp, countp, stridep,
                         ip);
}
...
int nc_put_vars_string(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       const char **op) {
  return nc4_put_vars_tc(ncid, varid, NC_STRING, 0, startp, countp, stridep,
                         op);
}
...
int nc_get_vars_string(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       char **ip) {
  return nc4_get_vars_tc(ncid, varid, NC_STRING, 0, startp, countp, stridep,
                         ip);
}
...
int nc_put_varm_ushort(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_USHORT, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_ushort(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_USHORT, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_uint(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_UINT, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_uint(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_UINT, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_longlong(int ncid, int varid, const size_t *startp,
  ...
  return nc4_put_varm_tc(ncid, varid, NC_INT64, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_longlong(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_INT64, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_string(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       const ptrdiff_t *imapp, const char **op) {
  return nc4_put_varm_tc(ncid, varid, NC_STRING, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_string(int ncid, int varid, const size_t *startp,
                       const size_t *countp, const ptrdiff_t *stridep,
                       const ptrdiff_t *imapp, char **ip) {
  return nc4_get_varm_tc(ncid, varid, NC_STRING, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_varm_ulonglong(int ncid, int varid, const size_t *startp,
                          ...
                          const unsigned long long *op) {
  return nc4_put_varm_tc(ncid, varid, NC_UINT64, 0, startp, countp, stridep,
                         imapp, op);
}
...
int nc_get_varm_ulonglong(int ncid, int varid, const size_t *startp,
  ...
  return nc4_get_varm_tc(ncid, varid, NC_UINT64, 0, startp, countp, stridep,
                         imapp, ip);
}
...
int nc_put_vara(int ncid, int varid, const size_t *startp, const size_t *countp,
                const void *op) {
  return nc4_put_vara_tc(ncid, varid, NC_NAT, 0, startp, countp, op);
}

/* Read an array of values. */
int nc_get_vara(int ncid, int varid, const size_t *startp, const size_t *countp,
                void *ip) {
