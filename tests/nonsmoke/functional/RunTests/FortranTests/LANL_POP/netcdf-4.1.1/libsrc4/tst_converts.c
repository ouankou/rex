...
int main(int argc, char **argv) {
  ...
  for (format = 1; format < 5; format++) {
    printf("*** Testing conversion in netCDF %s files... ",
           format_name[format]);
...
void create_file(int format, unsigned char *uchar_out) {
  ...
  if (nc_create(FILE_NAME, cflags, &ncid))
    ERR;
  ...
  if ((format != NC_FORMAT_NETCDF4) && retval)
    ERR;
  if ((format == NC_FORMAT_NETCDF4) && (retval != NC_ERANGE))
    ERR;
  if (nc_close(ncid))
    ERR;
}
...
void check_file(int format, unsigned char *uchar_out) {
  ...
  if (strcmp(var_name, VAR1_NAME) || natts != 0 || ndims != 1 ||
      dimids_var[0] != 0 || var_type != NC_BYTE)
    ERR;
  ...
  if (nc_get_var_schar(ncid, 0, char_in))
    ERR;
  for (i = 0; i < DIM1_LEN; i++)
    if (char_in[i] != (signed char)uchar_out[i])
      ERR;

  if (nc_get_var_short(ncid, 0, short_in))
    ERR;
  for (i = 0; i < DIM1_LEN; i++)
    if (short_in[i] != (signed char)uchar_out[i])
      ERR;

  if (nc_get_var_int(ncid, 0, int_in))
    ERR;
  for (i = 0; i < DIM1_LEN; i++)
    if (int_in[i] != (signed char)uchar_out[i])
      ERR;
  ...
  if (format == NC_FORMAT_NETCDF4 || format == NC_FORMAT_NETCDF4_CLASSIC) {
     ...
     * as. */
    if (nc_get_var_ushort(ncid, 0, ushort_in) != NC_ERANGE)
      ERR;
    for (i = 0; i < DIM1_LEN; i++)
      if (ushort_in[i] != (unsigned short)(signed char)uchar_out[i])
        ERR;

    if (nc_get_var_uint(ncid, 0, uint_in) != NC_ERANGE)
      ERR;
    for (i = 0; i < DIM1_LEN; i++)
      if (uint_in[i] != (unsigned int)(signed char)uchar_out[i])
        ERR;

    if (nc_get_var_ulonglong(ncid, 0, uint64_in) != NC_ERANGE)
      ERR;
    for (i = 0; i < DIM1_LEN; i++)
      if (uint64_in[i] != (unsigned long long)(signed char)uchar_out[i])
        ERR;

    if (nc_get_var_longlong(ncid, 0, int64_in))
      ERR;
    for (i = 0; i < DIM1_LEN; i++)
      if (int64_in[i] != (signed char)uchar_out[i])
        ERR;
  }
