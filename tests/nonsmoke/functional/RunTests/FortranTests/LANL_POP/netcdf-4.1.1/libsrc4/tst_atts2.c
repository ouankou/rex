/* This is part of the netCDF package. Copyright 2005-2007 University
   ...
   $Id: tst_atts2.c,v 1.6 2010/04/01 14:05:47 ed Exp $
*/
...
int main(int argc, char **argv) {
  ...
  printf("*** testing sting attribute copy...");
  {
...
#define ATT_LEN 1
    ...
    if (nc_create(FILE_NAME1, NC_NETCDF4, &ncid1))
      ERR;
    if (nc_put_att_string(ncid1, NC_GLOBAL, ATT_NAME, ATT_LEN, mc))
      ERR;
    ...
    if (nc_open(FILE_NAME2, 0, &ncid2))
      ERR;
    if (nc_get_att_string(ncid2, NC_GLOBAL, ATT_NAME, &mc_in))
      ERR;
    if (strcmp(mc[0], mc_in))
      ERR;
    if (nc_close(ncid2))
      ERR;
    if (nc_free_string(ATT_LEN, &mc_in))
      ERR;
  }
  SUMMARIZE_ERR;
  printf("*** testing non-sting atomic attribute copy...");
  {
#define ATT_LEN 1
#define NUM_ATT 5
    int ncid1, ncid2;
    ...
    for (a = 0; a < NUM_ATT; a++) {
      ...
      nc_put_att(ncid1, NC_GLOBAL, name[a], a + 1, ATT_LEN, (void *)&data);
    }
    ...
    if (nc_create(FILE_NAME2, NC_NETCDF4, &ncid2))
      ERR;
    for (a = 0; a < NUM_ATT; a++)
      if (nc_copy_att(ncid1, NC_GLOBAL, name[a], ncid2, NC_GLOBAL))
        ERR;
...
#define NUM_FILES 2
    ...
    for (i = 0; i < NUM_FILES; i++) {
      if (nc_create(file_name[i], NC_NETCDF4, &ncid[i]))
        ERR;
      if (nc_def_compound(ncid[i], sizeof(struct s1), CMP_NAME, &typeid))
        ERR;
    ...
    if (nc_put_att(ncid[0], NC_GLOBAL, ATT_NAME2, typeid, DIM_LEN, data))
      ERR;

    if (nc_copy_att(ncid[0], NC_GLOBAL, ATT_NAME2, ncid[1], NC_GLOBAL))
      ERR;
    ...
    if (nc_open(FILE_NAME1, NC_WRITE, &ncid[1]))
      ERR;
    if (nc_get_att(ncid[1], NC_GLOBAL, ATT_NAME2, data_in))
      ERR;
  ...
  SUMMARIZE_ERR;
  printf(
      "*** testing compound attribute copy to different type of same name...");
  {
...
#define NUM_FILES 2
    ...
    /* Define s1 in file 1. */
    if (nc_def_compound(ncid[0], sizeof(struct s1), CMP_NAME1, &typeid))
      ERR;
    ...
    if (nc_def_compound(ncid[1], sizeof(struct s2), CMP_NAME1, &typeid))
      ERR;
    if (nc_insert_compound(ncid[1], typeid, I1_NAME,
                           NC_COMPOUND_OFFSET(struct s2, i1), NC_SHORT))
      ERR;
    if (nc_insert_compound(ncid[1], typeid, I2_NAME,
                           NC_COMPOUND_OFFSET(struct s2, i2), NC_INT))
      ERR;

    /* Write an att in one file. */
    if (nc_put_att(ncid[0], NC_GLOBAL, ATT_NAME3, typeid, DIM_LEN, data))
      ERR;

    /* Try to copy. It must fail, because the two types are not the
     * same. */
    if (nc_copy_att(ncid[0], NC_GLOBAL, ATT_NAME3, ncid[1], NC_GLOBAL) !=
        NC_EBADTYPE)
      ERR;
...
#define NUM_MEMBERS 4
    ...
    for (i = 0; i < NUM_FILES; i++) {
      if (nc_create(file_name[i], NC_NETCDF4, &ncid[i]))
        ERR;
      if (nc_def_enum(ncid[i], NC_INT, TYPE_NAME, &typeid))
        ERR;
      for (j = 0; j < NUM_MEMBERS; j++)
        if (nc_insert_enum(ncid[i], typeid, member_name[j], &member_value[j]))
          ERR;
    }

    /* Write an att in one file and copy it. */
    if (nc_put_att(ncid[0], NC_GLOBAL, ATT_NAME2, typeid, DIM_LEN_10, data))
      ERR;
