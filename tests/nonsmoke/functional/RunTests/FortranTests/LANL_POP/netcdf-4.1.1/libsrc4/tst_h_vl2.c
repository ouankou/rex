  ...
  printf("*** Reading reference file...");
  {
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      if (H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, i,
        ...
        ERR;
      printf("nc4_rec_read_metadata: encountered HDF5 object obj_class %d "
             "obj_name %s.\n",
             obj_class, obj_name);
      ...
      case H5O_TYPE_NAMED_DATATYPE:
        printf("type\n");
        if ((hdf_typeid = H5Topen2(grpid, obj_name, H5P_DEFAULT)) < 0)
          ERR;
        if ((class = H5Tget_class(hdf_typeid)) < 0)
          ERR;
        switch (class) {
        case H5T_VLEN:
          /* Find the base type of this vlen (i.e. what is this a * vlen of?)
           * and its size. */
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      if ((attid && H5Aclose(attid)))
        ERR;
    }

    /* Open the HDF5 attribute. */
    if ((attid = H5Aopen_name(grpid, obj_name)) < 0)
      ERR;

    /* Get type of attribute in file. */
    if ((file_typeid = H5Aget_type(attid)) < 0)
      ERR;
    if ((class = H5Tget_class(file_typeid)) < 0)
      ERR;
    if (class != H5T_VLEN)
      ERR;
    ...
    if ((native_typeid = H5Tget_native_type(file_typeid, H5T_DIR_DEFAULT)) < 0)
      ERR;
    if (!(vldata = malloc((unsigned int)(dims[0] * sizeof(hvl_t)))))
      ERR;
    if (H5Aread(attid, native_typeid, vldata) < 0)
      ERR;
    ...
    if (H5Aclose(attid) || H5Tclose(hdf_typeid) < 0 ||
        H5Tclose(base_hdf_typeid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
  }
