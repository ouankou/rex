...
int main() {
  ...
  if (H5Pset_fclose_degree(access_plist, H5F_CLOSE_STRONG))
    ERR;
  if ((fileid =
           H5Fcreate(FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT, access_plist)) < 0)
    ERR;
  ...
  if (H5Tinsert(typeid, "d", HOFFSET(struct s1, d), H5T_NATIVE_DOUBLE) < 0)
    ERR;
  if (H5Tcommit(fileid, COMPOUND_NAME, typeid) < 0)
    ERR;
  ...
  if ((datasetid = H5Dcreate(fileid, VAR_NAME, typeid, spaceid, H5P_DEFAULT)) <
      0)
    ERR;
