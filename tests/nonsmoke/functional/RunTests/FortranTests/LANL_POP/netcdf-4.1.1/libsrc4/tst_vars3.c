...
#define TEXT_LEN 15
    ...
    if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
      ERR;
    if (nc_def_compound(ncid, sizeof(struct billy_bob), COMPOUND_NAME, &typeid))
      ERR;
    if (nc_insert_compound(ncid, typeid, BILLY,
                           NC_COMPOUND_OFFSET(struct billy_bob, billy), NC_INT))
      ERR;
    if (nc_insert_compound(ncid, typeid, BOB,
                           NC_COMPOUND_OFFSET(struct billy_bob, bob), NC_INT))
      ERR;
    if (nc_def_var(ncid, VAR_NAME1, typeid, 0, NULL, &varid))
      ERR;
    ...
    if (nc_inq(ncid, &ndims, &nvars, &ngatts, &unlimdimid))
      ERR;
    if (nvars != 1 || ndims != 0 || ngatts != 0 || unlimdimid != -1)
      ERR;
    ...
    if (nc_inq_var(ncid, 0, var_name_in, &xtype_in, &ndims_in, dimids_in,
                   &natts_in))
      ERR;
    if (strcmp(var_name_in, VAR_NAME1) || xtype_in <= NC_STRING ||
        ndims_in != 0 || natts_in != 0)
      ERR;
    if (nc_close(ncid))
      ERR;
  }
  ...
  SUMMARIZE_ERR;
  printf(
      "**** testing that fixed vars with no filter end up being contiguous...");
...
#define D1_LEN 99
    ...
    if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
      ERR;
    if (nc_def_dim(ncid, D0_NAME1, D0_LEN, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, D1_NAME1, D1_LEN, &dimids[1]))
      ERR;
    if (nc_def_var(ncid, VAR_NAME2, NC_UINT64, NDIMS, dimids, &varid))
      ERR;
    if (nc_def_var_endian(ncid, varid, NC_ENDIAN_BIG))
      ERR;
    if (nc_close(ncid))
    ...
    if (nc_inq(ncid, &ndims, &nvars, &ngatts, &unlimdimid))
      ERR;
    if (nvars != 1 || ndims != 2 || ngatts != 0 || unlimdimid != -1)
      ERR;
    if (nc_inq_var(ncid, 0, var_name_in, &xtype_in, &ndims_in, dimids_in,
                   &natts_in))
      ERR;
    if (strcmp(var_name_in, VAR_NAME2) || xtype_in != NC_UINT64 ||
        ndims_in != 2 || natts_in != 0)
      ERR;
    if (nc_inq_var_chunking(ncid, varid, &contig, NULL))
      ERR;
    if (!contig)
      ERR;
    if (nc_close(ncid))
...
#define LEN 4
    ...
    /* define dimensions */
    if (nc_def_dim(ncid, "Time", NC_UNLIMITED, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, "X", 4, &dimids[2]))
      ERR;
    if (nc_def_dim(ncid, "Y", 3, &dimids[1]))
      ERR;

    /* define variables */
    if (nc_def_var(ncid, "Time", NC_DOUBLE, 1, dimids, &time_id))
      ERR;
    if (nc_def_var(ncid, "P", NC_FLOAT, RANK_P, dimids, &p_id))
      ERR;
    if (nc_enddef(ncid))
      ERR;

    /* Add one record in coordinate variable. */
    if (nc_put_vara(ncid, time_id, start, count, data))
      ERR;

    /* The other variable should show an increase in size, since it
     * uses the unlimited dimension. */
    if (nc_inq_var(ncid, 1, NULL, NULL, &ndims, dimids_in, NULL))
      ERR;
    if (ndims != 3 || dimids_in[0] != 0 || dimids_in[1] != 2 ||
        dimids_in[2] != 1)
      ERR;

    /* Read the record of non-existant data. */
    if (nc_get_vara(ncid, 1, cor, edg, P_data))
      ERR;
    for (i = 0; i < LEN; i++)
      if (P_data[i] != NC_FILL_FLOAT)
        ERR;
  ...
  /*       if (nc_def_var(ncid, V_SMALL, NC_INT64, NDIMS1, &small_dimid,
   * &small_varid)) ERR; */

  /*       if (nc_def_var(ncid, V_MEDIUM, NC_INT64, NDIMS1, &medium_dimid,
   * &medium_varid)) ERR; */
  /*       if (nc_def_var_szip(ncid, medium_varid, NC_SZIP_EC_OPTION_MASK, 32))
   * ERR; */

  /*       if (nc_def_var(ncid, V_LARGE, NC_INT64, NDIMS1, &large_dimid,
   * &large_varid)) ERR; */
  /*       if (nc_def_var_szip(ncid, large_varid, NC_SZIP_NN_OPTION_MASK, 16))
   * ERR; */
   ...
   */
  /*       if (nc_inq_var(ncid, 0, var_name_in, &xtype_in, &ndims_in,
   * &dimids_in, &natts_in)) ERR; */
  /*       if (strcmp(var_name_in, V_SMALL) || xtype_in != NC_INT64 || ndims_in
   * != 1 || */
  /* 	  natts_in != 0) ERR; */

  /*       /\* Make sure we have the szip settings we expect. *\/ */
  /*       if (nc_inq_var_szip(ncid, small_varid, &options_mask_in,
   * &bits_per_pixel_in)) ERR; */
  /*       if (options_mask_in != 0 || bits_per_pixel_in !=0) ERR; */
  /*       if (nc_inq_var_szip(ncid, medium_varid, &options_mask_in,
   * &bits_per_pixel_in)) ERR; */
  /*       if (!(options_mask_in & NC_SZIP_EC_OPTION_MASK) || bits_per_pixel_in
   * != 32) ERR; */
  /*       if (nc_inq_var_szip(ncid, large_varid, &options_mask_in,
   * &bits_per_pixel_in)) ERR; */
  /*       if (!(options_mask_in & NC_SZIP_NN_OPTION_MASK) || bits_per_pixel_in
   * != 16) ERR; */
