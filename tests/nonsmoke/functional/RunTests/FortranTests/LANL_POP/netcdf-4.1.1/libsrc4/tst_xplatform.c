...
int main(int argc, char **argv) {
  ...
  printf("*** testing define of struct s1 compound type...");
  {
    ...
    if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
      ERR;
    if (nc_def_compound(ncid, sizeof(struct s1), CMP_TYPE_NAME, &cmp_typeid))
      ERR;
    ...
    if (nc_open(FILE_NAME, NC_NOWRITE, &ncid))
      ERR;
    if (nc_inq_typeids(ncid, &ntypes_in, &cmp_typeid))
      ERR;
    if (ntypes_in != 1)
      ERR;
    if (nc_inq_compound(ncid, cmp_typeid, name_in, &size_in, &nfields_in))
      ERR;
    if (nfields_in != 2 || strcmp(name_in, CMP_TYPE_NAME) ||
        size_in != sizeof(struct s1))
      ERR;
    if (nc_inq_compound_field(ncid, cmp_typeid, 0, name_in, &offset_in,
                              &field_type_in, &ndims_in, NULL))
      ERR;
    if (strcmp(name_in, X_NAME) || offset_in != 0 ||
        field_type_in != NC_FLOAT || ndims_in)
      ERR;
  ...
  printf("*** reading ref_tst_compound3.nc...");
  {
    ...
    if (nc_open(file_in, NC_NOWRITE, &ncid))
      ERR;
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdimid))
      ERR;
    if (ndims_in != 1 || nvars_in != 1 || natts_in != 0 || unlimdimid != -1)
      ERR;
    if (nc_inq_var(ncid, 0, name_in, &cmp_typeid, &ndims_in, dimids_in,
                   &natts_in))
      ERR;
    if (strcmp(name_in, "var") || ndims_in != 1 || natts_in)
      ERR;
    ...
    if (nc_get_var(ncid, 0, data_in))
      ERR;
    if (data_in[0].x != 1.0 || data_in[0].y != -2.0)
      ERR;
    if (nc_close(ncid))
      ERR;
  }
...
#define S1_NAME "s1"
    ...
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdimid))
      ERR;
    if (ndims_in != 0 || nvars_in != 0 || natts_in != 1 || unlimdimid != -1)
      ERR;
    ...
    if (nc_inq_compound(ncid, cmp_typeid[0], name_in, &size_in, &nfields_in))
      ERR;
    if (nfields_in != 2 || strcmp(name_in, CMP_TYPE_NAME_C) ||
        size_in != sizeof(struct s1))
      ERR;
    ...
    if (nc_inq_compound(ncid, cmp_typeid[1], name_in, &size_in, &nfields_in))
      ERR;
    if (nfields_in != 1 || strcmp(name_in, CMP_TYPE_NAME_D) ||
        size_in != sizeof(struct s2))
      ERR;
    if (nc_inq_compound_field(ncid, cmp_typeid[1], 0, name_in, &offset_in,
                              &field_type_in, &ndims_in, NULL))
      ERR;
    /*      if (strcmp(name_in, S1_NAME) || offset_in != 0 || field_type_in !=
     * cmp_typeid[0] || ndims_in) ERR;*/

    /* Read the data. */
    if (nc_get_att(ncid, NC_GLOBAL, ATT_NAME, data_in))
      ERR;
    if (data_in[0].s1.x != 1.0 || data_in[0].s1.y != -2.0)
      ERR;
    if (nc_close(ncid))
      ERR;
  }
