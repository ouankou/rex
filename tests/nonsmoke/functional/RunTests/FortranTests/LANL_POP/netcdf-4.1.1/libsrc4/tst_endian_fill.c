...
int main(int argc, char **argv) {
  ...
  printf("*** testing simple case with int...");
  {
    ...
    if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
      ERR;
    if (nc_def_var(ncid, VAR_NAME, NC_INT, VAR_RANK, 0, &varid))
      ERR;
    if (nc_def_var_endian(ncid, varid, NC_ENDIAN_BIG))
      ERR;
    if (nc_def_var(ncid, VAR2_NAME, NC_INT, VAR_RANK, 0, &var2id))
      ERR;
    if (nc_def_var_endian(ncid, var2id, NC_ENDIAN_LITTLE))
      ERR;
    /* close without writing data, so vars 1 and 2 should contain
     * NC_FILL_INT */
    if (nc_def_var(ncid, VAR3_NAME, NC_INT, VAR_RANK, 0, &var3id))
      ERR;
    if (nc_def_var_endian(ncid, var3id, NC_ENDIAN_BIG))
      ERR;
    ...
    if (nc_open(FILE_NAME, NC_NOWRITE, &ncid))
      ERR;
    if (nc_inq_varid(ncid, VAR_NAME, &varid))
      ERR;
    if (nc_inq_varid(ncid, VAR2_NAME, &var2id))
      ERR;
    if (nc_get_var_int(ncid, varid, &data_in))
      ERR;
    if (data_in != NC_FILL_INT)
      ERR;
    if (nc_get_var_int(ncid, var2id, &data_in))
      ERR;
    if (data_in != NC_FILL_INT)
      ERR;
    if (nc_close(ncid))
...
#define NUM_TYPES_TO_CHECK 6
    ...
    char var_name[NUM_TYPES_TO_CHECK][NC_MAX_NAME + 1] = {
        "SHORT", "USHORT", "INT", "UINT", "INT64", "UINT64"};
    long long fill_value[NUM_TYPES_TO_CHECK] = {NC_FILL_SHORT, NC_FILL_USHORT,
                                                NC_FILL_INT,   NC_FILL_UINT,
                                                NC_FILL_INT64, NC_FILL_UINT64};
    long long data_in;
    int t;
    ...
    for (t = 0; t < NUM_TYPES_TO_CHECK; t++) {
      if (nc_def_var(ncid, var_name[t], check_type[t], 0, 0, &varid[t]))
        ERR;
      if (nc_def_var_endian(ncid, varid[t], NC_ENDIAN_BIG))
        ERR;
    }
    if (nc_close(ncid))
      ERR;
    ...
    if (nc_open(FILE_NAME, NC_NOWRITE, &ncid))
      ERR;
    for (t = 0; t < NUM_TYPES_TO_CHECK; t++) {
      if (nc_get_var_longlong(ncid, varid[t], &data_in))
        ERR;
