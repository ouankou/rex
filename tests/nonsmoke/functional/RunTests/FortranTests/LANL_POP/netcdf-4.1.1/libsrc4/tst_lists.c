...
int main(int argc, char **argv) {
  ...
  printf("*** testing group list code...");
  {
    ...
    if (nc4_file_list_add(0x00010000))
      ERR;
    if (!(nc = nc4_find_nc_file(0x00010000)))
      ERR;
    ...
    if (!(nc = nc4_find_nc_file(0x00010000)))
      ERR;
    if (nc4_nc4f_list_add(nc, NAME, NC_NETCDF4))
      ERR;
    if (!nc->nc4_info)
      ERR;
    if (strcmp(NAME, nc->nc4_info->path))
      ERR;
    ...
    if (nc4_nc4f_list_add(nc, NAME, NC_NETCDF4))
      ERR;
    if (nc4_grp_list_add(&nc->nc4_info->root_grp->children, 1,
                         nc->nc4_info->root_grp, nc, "hohoho", &g1))
      ERR;
    if (!(g = nc4_rec_find_grp(nc->nc4_info->root_grp, 1)))
      ERR;
    if (g->nc_grpid != 1)
      ERR;
    nc4_rec_grp_del(&nc->nc4_info->root_grp, nc->nc4_info->root_grp);
...

  {
    ...
    if (nc4_find_nc_file(0))
      ERR;
    if (nc4_file_list_add(0x00000000))
      ERR;
    if (nc4_file_list_add(0x00010000))
      ERR;
    if (nc4_file_list_add(0x00020000))
      ERR;
    if (!(nc = nc4_find_nc_file(0x00010000)))
      ERR;
    nc4_file_list_del(nc);
    if (nc4_find_nc_file(0x00010000))
      ERR;
    if (!(nc = nc4_find_nc_file(0x00020000)))
      ERR;
    nc4_file_list_del(nc);
    if (nc4_find_nc_file(0x00020000))
      ERR;
    if (!(nc = nc4_find_nc_file(0)))
      ERR;
    if (nc4_nc4f_list_add(nc, NAME, NC_NETCDF4))
      ERR;
    if (!nc->nc4_info)
      ERR;
    if (strncmp(NAME, nc->nc4_info->path, NC_MAX_NAME))
      ERR;
    if (!nc->nc4_info->root_grp)
      ERR;
    if (nc->nc4_info->root_grp->children)
      ERR;
    if (nc->nc4_info->root_grp->next)
      ERR;
    if (nc->nc4_info->root_grp->parent)
      ERR;
    if (!(nc = nc4_find_nc_file(0x00000000)))
      ERR;
    nc4_rec_grp_del(&nc->nc4_info->root_grp, nc->nc4_info->root_grp);
    free(nc->nc4_info);
    nc4_file_list_del(nc);
