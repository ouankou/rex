...
#define VAR_NAME1 "Amendments"
    ...
    if ((fileid =
      ...
      ERR;
    if ((grpid = H5Gcreate(fileid, GRP_NAME1, 0)) < 0)
      ERR;
    ...
    if ((plistid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
      ERR;
    if (H5Pset_fill_value(plistid, typeid, &f1) < 0)
      ERR;
    if ((datasetid = H5Dcreate(grpid, VAR_NAME1, typeid, spaceid, plistid)) < 0)
      ERR;
    if (H5Dwrite(datasetid, typeid, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) < 0)
      ERR;

    if (H5Dclose(datasetid) < 0 || H5Pclose(plistid) < 0 ||
