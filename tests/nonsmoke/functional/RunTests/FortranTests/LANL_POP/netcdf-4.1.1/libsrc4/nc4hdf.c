/*
  ...
  $Id: nc4hdf.c,v 1.267 2010/02/18 18:04:47 ed Exp $
*/
...
static int rec_reattach_scales(NC_GRP_INFO_T *grp, int dimid,
                               hid_t dimscaleid) {
  NC_VAR_INFO_T *var;
 ...
 * and then I need to go to every var in the file which uses that
 * dimension, and attach the new dimension scale. */
static int rec_detach_scales(NC_GRP_INFO_T *grp, int dimid, hid_t dimscaleid) {
  NC_VAR_INFO_T *var;
  NC_GRP_INFO_T *child_grp;
  int d;
  int retval;
...
int nc4_open_var_grp2(NC_GRP_INFO_T *grp, int varid, hid_t *dataset) {
  ...
  if (!var->hdf_datasetid)
    if ((var->hdf_datasetid =
             H5Dopen2(grp->hdf_grpid, var->name, H5P_DEFAULT)) < 0)
      return NC_ENOTVAR;
...
*/
int nc4_pg_var1(NC_PG_T pg, NC_FILE_INFO_T *nc, int ncid, int varid,
  ...
  if (pg == GET)
    return nc4_get_vara(nc, ncid, varid, start, count, xtype, is_long, ip);
  else
    return nc4_put_vara(nc, ncid, varid, start, count, xtype, is_long, ip);
}
...
static int get_fill_value(NC_HDF5_FILE_INFO_T *h5, NC_VAR_INFO_T *var,
                          void **fillp) {
  size_t size;
  int retval;
...
static int check_for_vara(nc_type *mem_nc_type, NC_VAR_INFO_T *var,
...
static void log_dim_info(NC_VAR_INFO_T *var, hsize_t *fdims, hsize_t *fmaxdims,
  ...
  for (d2 = 0; d2 < var->ndims; d2++) {
    LOG((4, "fdims[%d]=%Ld fmaxdims[%d]=%Ld", d2, fdims[d2], d2, fmaxdims[d2]));
...
#ifdef USE_PARALLEL
static int set_par_access(NC_HDF5_FILE_INFO_T *h5, NC_VAR_INFO_T *var,
                          hid_t xfer_plistid) {
  ...
  if (h5->parallel) {
    ...
    LOG((
        4,
        "hdf5_xfer_mode: %d H5FD_MPIO_COLLECTIVE: %d H5FD_MPIO_INDEPENDENT: %d",
...
int RENAME4(nc4_put_vara)(NC_FILE_INFO_T *nc, int ncid, int varid,
  ...
  LOG((3, "nc4_put_vara: var->name %s mem_nc_type %d is_long %d", var->name,
       mem_nc_type, is_long));
  ...
  if (H5Sget_simple_extent_type(file_spaceid) == H5S_SCALAR) {
    ...
    if (H5Sselect_hyperslab(file_spaceid, H5S_SELECT_SET, start, NULL, count,
                            NULL) < 0)
      BAIL(NC_EHDFERR);
  ...
  if ((mem_nc_type != var->xtype || (var->xtype == NC_INT && is_long)) &&
    ...
    if (var->ndims)
      for (d2 = 0; d2 < var->ndims; d2++)
        len *= countp[d2];
    LOG((4, "converting data for var %s type=%d len=%d", var->name, var->xtype,
         len));
  ...
  if (need_to_convert) {
    if ((retval = nc4_convert_type(data, bufr, mem_nc_type, var->xtype, len,
                                   &range_error, var->fill_value,
  ...
  LOG((4,
       "about to H5Dwrite datasetid 0x%x mem_spaceid 0x%x "
       "file_spaceid 0x%x",
       var->hdf_datasetid, mem_spaceid, file_spaceid));
  if (H5Dwrite(var->hdf_datasetid, var->type_info->hdf_typeid, mem_spaceid,
  ...
  if ((h5->cmode & NC_CLASSIC_MODEL) &&
      ...
      (mem_nc_type == NC_UBYTE || mem_nc_type == NC_BYTE) && range_error)
    range_error = 0;
...
int RENAME4(nc4_get_vara)(NC_FILE_INFO_T *nc, int ncid, int varid,
                          const size_t *startp, const size_t *countp,
                          nc_type mem_nc_type, int is_long, void *data) {
  NC_GRP_INFO_T *grp, *g;
  NC_HDF5_FILE_INFO_T *h5;
  ...
  int fill_value_size[NC_MAX_DIMS];
  int scalar = 0, retval = NC_NOERR, range_error = 0, i, d2;
  void *bufr = NULL;
  int break_it;
#ifndef HDF5_CONVERT
  ...
  LOG((3, "nc4_get_vara: var->name %s mem_nc_type %d is_long %d", var->name,
       mem_nc_type, is_long));
  ...
  if (!no_read) {
    ...
    if (H5Dread(var->hdf_datasetid, var->type_info->native_typeid, mem_spaceid,
                file_spaceid, xfer_plistid, bufr) < 0)
      BAIL(NC_EHDFERR);
    ...
    if (need_to_convert) {
      if ((retval = nc4_convert_type(
               bufr, data, var->xtype, mem_nc_type, len, &range_error,
               var->fill_value, (h5->cmode & NC_CLASSIC_MODEL), 0, is_long)))
        BAIL(retval);
...
static int write_coord_dimids(NC_VAR_INFO_T *var) {
  ...
  LOG((4,
       "A multidimensional coordinate variable can be "
       "a devilishly tricky little blighter! %d %d %d",
       var->dimids[0], var->dimids[1], var->dimids[2]));
  ...
  num_spaces++;
#endif
  if (!ret && (c_attid = H5Acreate(var->hdf_datasetid, COORDINATES,
                                   H5T_NATIVE_INT, c_spaceid, H5P_DEFAULT)) < 0)
    ret++;
  if (!ret && H5Awrite(c_attid, H5T_NATIVE_INT, var->dimids) < 0)
    ret++;
...
static int write_netcdf4_dimid(hid_t datasetid, int dimid) {
  ...
  if (!found_it)
    if ((dimid_attid = H5Acreate(datasetid, NC_DIMID_ATT_NAME, H5T_NATIVE_INT,
                                 dimid_spaceid, H5P_DEFAULT)) < 0)
      return NC_EHDFERR;
...
static int var_create_dataset(NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var,
                              int write_dimid) {
  NC_GRP_INFO_T *g;
  ...
  if (!var->no_fill) {
    ...
    if (fillp) {
      if (var->xtype == NC_STRING) {
        ...
        hid_t fill_typeid;
        if ((retval = nc4_get_hdf_typeid(grp->file->nc4_info, var->xtype,
                                         &fill_typeid, NC_ENDIAN_NATIVE)))
          BAIL(retval);
        ...
        if (var->type_info->nc_typeid == NC_STRING ||
            var->type_info->nc_typeid == NC_CHAR)
  ...
  if (var->ndims) {
    ...
    if (!(chunksize = malloc(var->ndims * sizeof(hsize_t))))
      BAIL(NC_ENOMEM);
    /*      for (d = 0; d < var->ndims; d++)
             dimsize[d] = var->dim[d]->unlimited ? NC_HDF5_UNLIMITED_DIMSIZE :
       var->dim[d]->len; maxdimsize[d] = var->dim[d]->unlimited ? H5S_UNLIMITED
       : (hsize_t)var->dim[d]->len; chunksize[d] = var->chunksizes[d];*/
  ...
  LOG((4, "var_create_dataset: about to H5Dcreate dataset %s of type 0x%x",
       name_to_use, typeid));
  if ((var->hdf_datasetid =
           H5Dcreate2(grp->hdf_grpid, name_to_use, typeid, spaceid, H5P_DEFAULT,
...
static int put_att_grpa(NC_GRP_INFO_T *grp, int varid, NC_ATT_INFO_T *att) {
  ...
  assert(att->name);
  LOG((3,
       "put_att_grpa: varid %d att->attnum %d att->name %s "
       "att->xtype %d att->len %d",
       varid, att->attnum, att->name, att->xtype, att->len));
  ...
  if ((attid = H5Acreate(locid, att->name, file_typeid, spaceid, H5P_DEFAULT)) <
      0)
    BAIL(NC_EATTMETA);
...
static int commit_type(NC_GRP_INFO_T *grp, NC_TYPE_INFO_T *type) {
  ...
  if (type->class == NC_COMPOUND) {
    ...
    for (field = type->field; field; field = field->next) {
      ...
      if (field->ndims) {
        ...
        if ((hdf_typeid = H5Tarray_create(hdf_base_typeid, field->ndims, dims,
                                          NULL)) < 0)
          return NC_EHDFERR;
      } else
      ...
      if (H5Tinsert(type->hdf_typeid, field->name, field->offset, hdf_typeid) <
          0)
        return NC_EHDFERR;
  ...
  if ((type->native_typeid =
           H5Tget_native_type(type->hdf_typeid, H5T_DIR_DEFAULT)) < 0)
    return NC_EHDFERR;
...
static int write_nc3_strict_att(hid_t hdf_grpid) {
  ...
  num_spaces++;
#endif
  if ((attid = H5Acreate(hdf_grpid, NC3_STRICT_ATT_NAME, H5T_NATIVE_INT,
...
static int create_group(NC_GRP_INFO_T *grp) {
  ...
  if (grp->parent) {
    ...
    if (H5Pset_link_creation_order(gcpl_id, H5P_CRT_ORDER_TRACKED |
                                                H5P_CRT_ORDER_INDEXED) < 0)
      BAIL(NC_EHDFERR);
    if (H5Pset_attr_creation_order(gcpl_id, H5P_CRT_ORDER_TRACKED |
                                                H5P_CRT_ORDER_INDEXED) < 0)
      BAIL(NC_EHDFERR);
    ...
    if ((grp->hdf_grpid =
             H5Gopen2(grp->file->nc4_info->hdfid, "/", H5P_DEFAULT)) < 0)
      BAIL(NC_EFILEMETA);
  }
  return NC_NOERR;
            ...
            for (g = grp; g && !var->dimscale_attached[d]; g = g->parent)
              for (dim1 = g->dim; dim1; dim1 = dim1->next)
                if (var->dimids[d] == dim1->dimid) {
                  LOG((2,
                       "attach_dimscales: attaching scale for dimid %d to var "
                       "%s",
                       var->dimids[d], var->name));
                  if (H5DSattach_scale(var->hdf_datasetid, dim1->hdf_dimscaleid,
                                       d) < 0)
                    BAIL(NC_EHDFERR);
                  var->dimscale_attached[d]++;
                  break;
...
static int write_attlist(NC_ATT_INFO_T *attlist, int varid,
                         NC_GRP_INFO_T *grp) {
  NC_ATT_INFO_T *att;
  int retval;
...
static int var_exists(hid_t grpid, char *name, int *exists) {
  hsize_t num_obj, i;
  H5O_info_t obj_info;
  int obj_class;
  ...
  *exists = 0;
  if (H5Gget_num_objs(grpid, &num_obj) < 0)
    return NC_EVARMETA;
  for (i = 0; i < num_obj; i++) {
    if (H5Oget_info_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, i,
                           &obj_info, H5P_DEFAULT) < 0)
      return NC_EHDFERR;
    obj_class = obj_info.type;
    if ((size = H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
                                   i, NULL, 0, H5P_DEFAULT)) < 0)
      return NC_EHDFERR;
    if (size > NC_MAX_NAME)
      return NC_EMAXNAME;
...
static int write_var(NC_VAR_INFO_T *var, NC_GRP_INFO_T *grp, int write_dimid) {
      ...
      for (d1 = grp->dim; d1; d1 = d1->next)
        if (!strcmp(d1->name, var->name)) {
          ...
          if (exists) {
            ...
            if ((retval =
                     rec_detach_scales(grp->file->nc4_info->root_grp,
      ...
      if (d1) {
        ...
        for (d = 0; d < var->ndims; d++)
          for (g = grp; g && !finished; g = g->parent)
            for (dim1 = g->dim; dim1; dim1 = dim1->next)
              if (var->dimids[d] == dim1->dimid) {
                if (H5DSdetach_scale(var->hdf_datasetid, dim1->hdf_dimscaleid,
                                     d) < 0)
                  BAIL(NC_EHDFERR);
                var->dimscale_attached[d] = 0;
...
static int write_dim(NC_DIM_INFO_T *dim, NC_GRP_INFO_T *grp, int write_dimid) {
  ...
  if (dim->dirty) {
    ...
    if (!dimscale_exists) {
      ...
      if (H5Pset_attr_creation_order(
              create_propid, H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED) < 0)
        BAIL(NC_EHDFERR);

      /* Create the dataset that will be the dimension scale. */
      LOG((4, "write_dim: about to H5Dcreate a dimscale dataset %s",
           dim->name));
      if ((dim->hdf_dimscaleid =
               H5Dcreate1(grp->hdf_grpid, dim->name, H5T_IEEE_F32BE, spaceid,
                          create_propid)) < 0)
        BAIL(NC_EHDFERR);
...
/* This reads/writes a whole var at a time. If the file has an
   ...
   here. */
int pg_var(NC_PG_T pg, NC_FILE_INFO_T *nc, int ncid, int varid, nc_type xtype,
           int is_long, void *ip) {
  NC_GRP_INFO_T *grp;
  NC_VAR_INFO_T *var;
  size_t start[NC_MAX_DIMS], count[NC_MAX_DIMS];
...
/* Write or read some mapped data. Yea, like I even understand what it
   ...
   Ed Hartnett, 9/43/03
*/
int nc4_pg_varm(NC_PG_T pg, NC_FILE_INFO_T *nc, int ncid, int varid,
                const size_t *start, const size_t *edges,
                const ptrdiff_t *stride, const ptrdiff_t *map, nc_type xtype,
                int is_long, void *data) {
  NC_GRP_INFO_T *grp;
  NC_HDF5_FILE_INFO_T *h5;
  NC_VAR_INFO_T *var;
  ...
  LOG((3, "nc4_pg_varm: ncid 0x%x varid %d xtype %d", ncid, varid, xtype));
  ...
  if ((h5->cmode & NC_CLASSIC_MODEL) &&
      (xtype == NC_CHAR || var->xtype == NC_CHAR) && (xtype != var->xtype))
    return NC_ECHAR;
  ...
  /* The variable is an array.  */
  {
    ...
    if (!(mystart =
              (size_t *)calloc((size_t)var->ndims * 7, sizeof(ptrdiff_t))))
      return NC_ENOMEM;
    ...
    for (idim = maxidim; idim >= 0; --idim) {
      ...
      LOG((4, "idim=%d mystart[idim]=%d myedge[idim]=%d dimlen=%d", idim,
           mystart[idim], myedges[idim], dimlen));
    ...
    for (;;) {
      ...
      if (pg == GET)
        ...
        lretval = nc4_put_vara(nc, ncid, varid, mystart, iocount, xtype,
                               is_long, data);
      if (lretval != NC_NOERR && (retval == NC_NOERR || lretval != NC_ERANGE))
        retval = lretval;
    ...
    carry:
      ...
      LOG((4, "mystart[%d]=%d length[%d]=%d", idim, (int)mystart[idim], idim,
           (int)length[idim]));
...
int nc4_convert_type(const void *src, void *dest, const nc_type src_type,
                     ...
                     int *range_error, const void *fill_value, int strict_nc3,
                     int src_long, int dest_long) {
  char *cp, *cp1;
  float *fp, *fp1;
  double *dp, *dp1;
  int *ip, *ip1;
  ...
  *range_error = 0;
  LOG((3,
       "nc4_convert_type: len %d src_type %d dest_type %d src_long %d"
       " dest_long %d",
       len, src_type, dest_type, src_long, dest_long));
  ...
  */
  ...
  case NC_BYTE:
    ...
    default:
      LOG((0,
           "nc4_convert_type: unexpected dest type. "
  ...
  case NC_UINT64:
    ...
    case NC_INT:
      if (dest_long)
        for (ulip = (unsigned long long *)src, lp = dest; count < len;
             count++) {
          if (*ulip > X_LONG_MAX)
            (*range_error)++;
          *lp++ = *ulip++;
        ...
        for (ulip = (unsigned long long *)src, ip = dest; count < len;
             count++) {
          if (*ulip > X_INT_MAX)
            (*range_error)++;
          *ip++ = *ulip++;
    ...
    case NC_INT64:
      ...
      break;
    case NC_UINT64:
      for (ulip = (unsigned long long *)src, ulip1 = dest; count < len;
           count++) {
  ...
  default:
    LOG((0,
         "nc4_convert_type: unexpected src type. "
         "src_type %d, dest_type %d",
         src_type, dest_type));
    return NC_EBADTYPE;
  }
...
int nc4_rec_match_dimscales(NC_GRP_INFO_T *grp) {
  ...
  for (var = grp->var; var; var = var->next) {
    ...
    if (var->dimscale_hdf5_objids) {
      for (d = 0; d < var->ndims; d++) {
        LOG((5, "nc4_rec_match_dimscales: var %s has dimscale info...",
             var->name));
        ...
        LOG((5, "nc4_rec_match_dimscales: dimid for this dimscale is %d",
             var->xtype));
      } /* next var->dim */
    } else {
      if (!var->dimscale) {
        ...
        for (d = 0; d < var->ndims; d++) {
          ...
          if (!dim) {
            LOG((3, "nc4_rec_match_dimscales: creating phony dim for var %s",
                 var->name));
...
int nc4_get_typelen_mem(NC_HDF5_FILE_INFO_T *h5, nc_type xtype, int is_long,
                        size_t *len) {
  NC_TYPE_INFO_T *type;
  int retval;
