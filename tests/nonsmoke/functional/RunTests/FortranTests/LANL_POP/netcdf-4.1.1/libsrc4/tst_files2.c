...
*/
void get_mem_used1(int *mem_used) {
...
#define MAX_DIMS 4
int create_sample_file(char *file_name, int ndims, int *dim_len, int num_vars,
                       int mode, int num_recs) {
  ...
  if (ndims != MAX_DIMS && ndims != MAX_DIMS - 1)
    ERR_RET;
  ...
  if (!(data_out = malloc(slab_nelems * sizeof(float))))
    ERR_RET;
  ...
  for (d = 0; d < ndims; d++) {
    ...
    if (nc_def_dim(ncid, dim_name, dim_len[d], &dimids[d]))
      ERR_RET;
  }

  /* Define num_vars variables. */
  if (!(varids = malloc(num_vars * sizeof(int))))
    ERR_RET;
  for (i = 0; i < num_vars; i++) {
    sprintf(varname, "a_%d", i);
    if (nc_def_var(ncid, varname, NC_FLOAT, ndims, dimids, &varids[i]))
      ERR_RET;
  }
  ...
  for (i = 0; i < num_vars; i++) {
    if (ndims == MAX_DIMS) {
      for (start[0] = 0; start[0] < (dim_len[0] ? dim_len[0] : num_recs);
           start[0]++)
        for (start[1] = 0; start[1] < dim_len[1]; start[1]++)
          if (nc_put_vara_float(ncid, varids[i], start, count, data_out))
            ERR_RET;
    } else {
      for (start[0] = 0; start[0] < (dim_len[0] ? dim_len[0] : num_recs);
           start[0]++)
...
int main(int argc, char **argv) {
  ...
  /* 	 if (timeval_subtract(&close_diff_time, &close_end_time,
   * &close_start_time)) ERR; */
  /* 	 close_us = ((int)close_diff_time.tv_sec * MILLION +
   * (int)close_diff_time.tv_usec); */
  ...
  /* 	 { */
  /* 	    if (ndims[t] == MAX_DIMS) */
  /* 	       sprintf(chunkstr, "%dx%dx%dx%d", (int)chunksize[0],
   * (int)chunksize[1],  */
  /* 		       (int)chunksize[2], (int)chunksize[3]); */
  /* 	    else */
  /* 	       sprintf(chunkstr, "%dx%dx%d", (int)chunksize[0],
   * (int)chunksize[1],  */
  /* 		       (int)chunksize[2]); */
  /* 	 } */
  /* 	 else */
  ...
  /* 		dimstr, chunkstr, mode_name[t], num_files[t],
   * (int)(cache_size[t]/1024),  */
...
#define NUM_TRIES 100
    ...
    for (i = 0; i < NUM_TRIES; i++) {
      ...
      if (mem_used2 - mem_used)
        printf("try %d - %d\t\t%d\t\t%d\t\t%d\t\t%d \n", i, mem_used, mem_used1,
               mem_used2, mem_used1 - mem_used, mem_used2 - mem_used);
    }
  }
  SUMMARIZE_ERR;
  FINAL_RESULTS;
