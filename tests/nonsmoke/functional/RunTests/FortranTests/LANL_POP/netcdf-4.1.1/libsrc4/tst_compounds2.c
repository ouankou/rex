...
#define ARRAY_LEN (NC_MAX_NAME + 1)
    ...
    if (nc_def_compound(ncid, sizeof(struct s1), "c", &type_id))
      ERR;
    if (nc_insert_array_compound(ncid, type_id, "x",
                                 NC_COMPOUND_OFFSET(struct s1, x), NC_UBYTE, 1,
                                 dim_sizes))
      ERR;
    if (nc_insert_compound(ncid, type_id, "y", NC_COMPOUND_OFFSET(struct s1, y),
                           NC_FLOAT))
      ERR;

    /* Write it as an attribute. */
    if (nc_put_att(ncid, NC_GLOBAL, "a1", type_id, DIM1_LEN, data_out))
      ERR;
    if (nc_close(ncid))
      ERR;
    ...
    if (nc_open(FILE_NAME, NC_WRITE, &ncid))
      ERR;
    if (nc_get_att(ncid, NC_GLOBAL, "a1", data_in))
      ERR;
    for (i = 0; i < DIM1_LEN; i++) {
    ...
    if (nc_inq_att(ncid, NC_GLOBAL, "a1", &xtype, &len))
      ERR;
    if (len != DIM1_LEN)
      ERR;
...
#define NAME2 "y"
    ...
    if (nc_get_att(ncid, NC_GLOBAL, "a1", data_in))
      ERR;
    for (i = 0; i < DIM1_LEN; i++)
      if (data_in[i].x != data_out[i].x || data_in[i].y != data_out[i].y)
        ERR;
    ...
    if (len != DIM1_LEN)
      ERR;
    if (nc_inq_compound_field(ncid, xtype, 0, field_name_in, &offset_in,
                              &field_typeid, &field_ndims, NULL))
      ERR;
    if (strcmp(field_name_in, NAME1) || field_typeid != NC_FLOAT || field_ndims)
      ERR;
    printf("offset x: %d  ", (int)offset_in);
    if (nc_inq_compound_field(ncid, xtype, 1, field_name_in, &offset_in,
                              &field_typeid, &field_ndims, NULL))
      ERR;
    if (strcmp(field_name_in, NAME2) || field_typeid != NC_DOUBLE ||
        field_ndims)
      ERR;
  ...
  SUMMARIZE_ERR;
  printf("*** testing compound var containing compound var, on different "
         "machines...");
  {
#define DIM1_LEN 1
#define S1_NAME_X "x"
...
#define ATT_NAME "a1"
    ...
    if (nc_def_compound(ncid, sizeof(struct s1), INNER_TYPE_NAME, &s1_typeid))
      ERR;
    ...
    if (nc_def_compound(ncid, sizeof(struct s2), OUTER_TYPE_NAME, &s2_typeid))
      ERR;
    if (nc_insert_compound(ncid, s2_typeid, S2_NAME_S1,
                           NC_COMPOUND_OFFSET(struct s2, s1), s1_typeid))
      ERR;

    /* Write it as an attribute. */
    if (nc_put_att(ncid, NC_GLOBAL, ATT_NAME, s2_typeid, DIM1_LEN, data_out))
      ERR;
    if (nc_close(ncid))
      ERR;
    ...
    if (nc_get_att(ncid, NC_GLOBAL, ATT_NAME, data_in))
      ERR;
    for (i = 0; i < DIM1_LEN; i++)
      if (data_in[i].s1.x != data_out[i].s1.x ||
          data_in[i].s1.y != data_out[i].s1.y)
        ERR;
    ...
    for (t = 0; t < NUM_TYPES; t++) {
      if (nc_inq_compound(ncid, typeids[t], name_in, &size_in, &nfields_in))
        ERR;
    ...
    if (nc_inq_att(ncid, NC_GLOBAL, ATT_NAME, &outer_typeid, &len))
      ERR;
    if (len != DIM1_LEN)
      ERR;
    if (nc_inq_compound_field(ncid, outer_typeid, 0, field_name_in, &offset_in,
                              &field_typeid, &field_ndims, NULL))
      ERR;
    if (strcmp(field_name_in, S2_NAME_S1) || field_typeid != inner_typeid ||
        field_ndims)
      ERR;
    if (nc_inq_compound_field(ncid, field_typeid, 1, field_name_in, &offset_in,
                              &field_typeid, &field_ndims, NULL))
      ERR;
    if (strcmp(field_name_in, S1_NAME_Y) || field_typeid != NC_DOUBLE ||
        field_ndims)
      ERR;
