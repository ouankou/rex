...
$Id: nc4internal.c,v 1.110 2010/01/31 19:00:45 ed Exp $
*/
...
/* Here is our linked list of open file info. */
...
static int find_var_shape_grp(NC_GRP_INFO_T *grp, int varid, int *ndims,
    ...
    if (!var->created) {
      ...
      if (H5Sget_simple_extent_type(spaceid) == H5S_SCALAR) {
        ...
        if (!(h5dimlenmax = malloc(dataset_ndims * sizeof(hsize_t))))
          BAIL(NC_ENOMEM);
        if ((dataset_ndims =
                 H5Sget_simple_extent_dims(spaceid, h5dimlen, h5dimlenmax)) < 0)
          BAIL(NC_EHDFERR);
        LOG((5, "find_var_shape_nc: varid %d len %d max: %d", varid,
             (int)h5dimlen[0], (int)h5dimlenmax[0]));
...
int nc4_nc4f_list_add(NC_FILE_INFO_T *nc, const char *path, int mode) {
  ...
  return nc4_grp_list_add(&(h5->root_grp), h5->next_nc_grpid++, NULL, nc,
                          NC_GROUP_NAME, &grp);
}
...
NC_FILE_INFO_T *nc4_find_nc_file(int ncid) {
  ...
  for (f = nc_file; f; f = f->next)
    if ((f->ext_ncid & FILE_ID_MASK) == (ncid & FILE_ID_MASK))
      return f;
...
int nc4_find_nc4_grp(int ncid, NC_GRP_INFO_T **grp) {
  ...
  for (f = nc_file; f; f = f->next) {
    /* Find the file from the file id part of ncid. */
    if (f->ext_ncid == (ncid & FILE_ID_MASK)) {
       ...
       * rules.*/
      if (f->nc4_info->cmode & NC_CLASSIC_MODEL)
        return NC_ESTRICTNC3;

      /* If we can't find it, the grp id part of ncid is bad. */
      if (!(*grp =
                nc4_rec_find_grp(f->nc4_info->root_grp, (ncid & GRP_ID_MASK))))
        return NC_EBADGRPID;
  ...
  for (f = nc_file; f; f = f->next) {
    ...
    if (f->ext_ncid == (ncid & FILE_ID_MASK)) {
      if (f->nc4_info) {
        ...
        /* If we can't find it, the grp id part of ncid is bad. */
        if (!(*grp = nc4_rec_find_grp(f->nc4_info->root_grp,
                                      (ncid & GRP_ID_MASK))))
          return NC_EBADGRPID;
...
NC_TYPE_INFO_T *nc4_rec_find_hdf_type(NC_GRP_INFO_T *start_grp,
  ...
  for (type = start_grp->type; type; type = type->next) {
    if ((equal = H5Tequal(type->native_typeid ? type->native_typeid
                                              : type->hdf_typeid,
                          target_hdf_typeid)) < 0)
      return NULL;
    if (equal)
...
int nc4_find_type(NC_HDF5_FILE_INFO_T *h5, nc_type typeid,
                  NC_TYPE_INFO_T **type) {
  if (typeid < 0 || !type)
    return NC_EINVAL;
  *type = NULL;
...
int nc4_find_dim_len(NC_GRP_INFO_T *grp, int dimid, size_t **len) {
  ...
  for (var = grp->var; var; var = var->next) {
    ...
    if ((retval = find_var_shape_grp(grp, var->varid, &ndims, dimids, dimlen)))
      return retval;
...
int nc4_find_nc_att(int ncid, int varid, const char *name, int attnum,
  ...
  LOG((4, "nc4_find_nc_att: ncid 0x%x varid %d name %s attnum %d", ncid, varid,
       name, attnum));
  ...
  /* Now find the attribute by name or number. If a name is provided, ignore the
   * attnum. */
  for (*att = attlist; *att; *att = (*att)->next)
    if ((name && !strcmp((*att)->name, name)) ||
        (!name && (*att)->attnum == attnum))
...
/* This will insert an NC_FILE_INFO_T at the beginning of the
   linked-list nc_file. */
...
int nc4_var_list_add(NC_VAR_INFO_T **list, NC_VAR_INFO_T **var) {
  ...
  (*var)->chunk_cache_size = MEGABYTE;
  (*var)->chunk_cache_nelems = MEGABYTE;
  (*var)->chunk_cache_preemption = 0.75;
...
/* Add to the beginning of a dim list. */
int nc4_dim_list_add2(NC_DIM_INFO_T **list, NC_DIM_INFO_T **new_dim) {
  NC_DIM_INFO_T *dim;
  if (!(dim = calloc(1, sizeof(NC_DIM_INFO_T))))
    return NC_ENOMEM;
  if (*list)
...
int nc4_grp_list_add(NC_GRP_INFO_T **list, int new_nc_grpid,
                     NC_GRP_INFO_T *parent_grp, NC_FILE_INFO_T *nc, char *name,
                     NC_GRP_INFO_T **grp) {
  NC_GRP_INFO_T *g;

  LOG((3, "grp_list_add: new_nc_grpid %d name %s ", new_nc_grpid, name));
...
int nc4_field_list_add(NC_FIELD_INFO_T **list, int fieldid, const char *name,
                       size_t offset, hid_t field_hdf_typeid,
                       hid_t native_typeid, nc_type xtype, int ndims,
...
static int var_list_del(NC_VAR_INFO_T **list, NC_VAR_INFO_T *var) {
  NC_ATT_INFO_T *a, *att;
  int ret;
...
int nc_set_log_level(int new_level) {
  ...
  if (new_level > NC_TURN_OFF_LOGGING && nc_log_level <= NC_TURN_OFF_LOGGING) {
...
#define MAX_NESTS 10
static int rec_print_metadata(NC_GRP_INFO_T *grp, int *tab_count) {
  NC_GRP_INFO_T *g;
  NC_ATT_INFO_T *att;
  NC_VAR_INFO_T *var;
  NC_DIM_INFO_T *dim;
  ...
  LOG((2, "%s GROUP - %s nc_grpid: %d nvars: %d natts: %d", tabs, grp->name,
       grp->nc_grpid, grp->nvars, grp->natts));

  for (att = grp->att; att; att = att->next)
    LOG((2, "%s GROUP ATTRIBUTE - attnum: %d name: %s type: %d len: %d", tabs,
  ...
  for (; dim; dim = dim->prev)
    LOG((2, "%s DIMENSION - dimid: %d name: %s len: %d unlimited: %d", tabs,
         dim->dimid, dim->name, dim->len, dim->unlimited));
  ...
  for (; var; var = var->prev) {
    ...
    LOG((2,
         "%s VARIABLE - varid: %d name: %s type: %d ndims: %d dimscale: %d "
         "dimids:%s",
         tabs, var->varid, var->name, var->xtype, var->ndims, var->dimscale,
         dims_string));
    for (att = var->att; att; att = att->next)
      LOG((2, "%s VAR ATTRIBUTE - attnum: %d name: %s type: %d len: %d", tabs,
           att->attnum, att->name, att->xtype, att->len));
  }
  ...
  for (type = grp->type; type; type = type->next) {
    LOG((2,
         ...
         "name: %s num_fields: %d base_nc_type: %d",
         tabs, type->nc_typeid, type->hdf_typeid, type->size, type->committed,
         type->name, type->num_fields, type->base_nc_type));
...
int log_metadata_nc(NC_FILE_INFO_T *nc) {
  ...
  LOG((2,
       ...
       "fill_mode: %d no_write: %d next_nc_grpid: %d",
       h5->hdfid, h5->path, h5->cmode, h5->parallel, h5->redef, h5->fill_mode,
       h5->no_write, h5->next_nc_grpid));
