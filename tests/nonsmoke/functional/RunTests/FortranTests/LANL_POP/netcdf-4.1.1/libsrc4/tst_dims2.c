/* This is part of the netCDF package. Copyright 2008 University
   ...
   $Id: tst_dims2.c,v 1.16 2009/12/30 12:03:35 ed Exp $
*/
...
#define NUM_NUTS 5
    ...
    if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
      ERR;
    if (nc_def_dim(ncid, CRAZY, NUM_CRAZY, &crazy_dimid))
      ERR;
    if (nc_def_dim(ncid, NUTS, NUM_NUTS, &nuts_dimid))
      ERR;
    if (nc_def_var(ncid, CRAZY, xtype, NDIMS1, &nuts_dimid, &varid))
      ERR;

    /* Check out the file. */
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdimid_in))
      ERR;
    if (ndims_in != 2 || nvars_in != 1 || natts_in != 0 || unlimdimid_in != -1)
      ERR;
    if (nc_inq_var(ncid, 0, name_in, &xtype_in, &ndims_in, &dimid_in,
                   &natts_in))
      ERR;
    if (strcmp(name_in, CRAZY) || xtype_in != xtype || ndims_in != NDIMS1 ||
        natts_in != 0 || dimid_in != nuts_dimid)
      ERR;
...
#define DEFLATE_LEVEL 5
    ...
    /*create dimensions*/
    if (nc_def_dim(ncid, "time", NC_UNLIMITED, &time_dim))
      ERR;
    if (nc_def_dim(ncid, "tl", TL, &tl_dim))
      ERR;

    /*create variables*/
    time_dimids[0] = time_dim;
    time_dimids[1] = tl_dim;
    if (nc_def_var(ncid, "time", NC_CHAR, NDIMS2, time_dimids, &time_id))
      ERR;
    ...
    if (nc_inq_varid(ncid, "time", &time_id))
      ERR;
    ...
    if (strcmp(name_in, "time") || xtype_in != NC_CHAR || ndims_in != NDIMS2 ||
      ...
      ERR;
    for (i = 0; i < NDIMS2; i++)
      if (time_dimids[i] != dimids_in[i])
        ERR;
    if (nc_get_vara_text(ncid, time_id, time_index, time_count, ttext_in))
      ERR;
    ttext_in[TL] = 0; /* Add a NULL so strcmp will work. */
    if (strcmp(ttext, ttext_in))
      ERR;
    if (nc_close(ncid))
      ERR;
  }
  SUMMARIZE_ERR;
  printf("*** Checking 2D coordinate variable with user-provided code some "
         "more...");
  {
    /* This test is from user Dipl.-Ing. Christian Schlamkow,
     * University of Rostock, Coastal Engineering Group. Thanks
...
#define NDIMS 2
    ...
    if (nc_def_dim(ncid, TIME, NC_UNLIMITED, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, LEN, 20, &dimids[1]))
      ERR;
    if (nc_def_var(ncid, TIME, xtype, 2, dimids, &varid))
      ERR;

    /* Check out the file. */
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdimid_in))
      ERR;
    if (ndims_in != 2 || nvars_in != 1 || natts_in != 0 || unlimdimid_in != 0)
      ERR;
    if (nc_inq_var(ncid, 0, name_in, &xtype_in, &ndims_in, dimids_in,
                   &natts_in))
      ERR;
...
#define MAX_VALUES 3
    ...
    size_t time_len, beam_len, start[NDIMS] = {0, 0},
                               count[NDIMS] = {1, MAX_VALUES};
    int i;
    ...
    if (nc_def_dim(ncid, "time", NC_UNLIMITED, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, "beam", NC_UNLIMITED, &dimids[1]))
      ERR;
    if (nc_def_var(ncid, "depth", NC_DOUBLE, NDIMS, dimids, &varid))
      ERR;

    /* Check the lengths of these dimensions. */
    if (nc_inq_dimlen(ncid, dimids[0], &time_len))
      ERR;
    if (time_len)
      ERR;
    if (nc_inq_dimlen(ncid, dimids[1], &beam_len))
      ERR;
    if (beam_len)
      ERR;

    /* Write some data, check dim lengths. */
    if (nc_put_vara_double(ncid, varid, start, count, value))
      ERR;
    if (nc_inq_dimlen(ncid, dimids[0], &time_len))
      ERR;
    if (time_len != 1)
  ...
  SUMMARIZE_ERR;
  printf("*** Checking multiple unlimited dimensions with more complex 2D "
         "test...");
  {
#define MAX(x, y) ((x) > (y) ? (x) : (y))
    int dimids[2];
    ...
    if (nc_def_dim(ncid, "time", NC_UNLIMITED, &timeDimID))
      ERR;
    if (nc_def_dim(ncid, "beam", NC_UNLIMITED, &beamDimID))
      ERR;

    dimids[0] = timeDimID;
    dimids[1] = beamDimID;

    if (nc_def_var(ncid, "depth", NC_DOUBLE, 2, dimids, &varid))
      ERR;
    ...
    for (j = 0; j < 100; j++) {
      ...
      if (nc_put_vara_int(ncid, varid, start, count, value))
        ERR;
      time_recs = MAX(time_recs, start[0] + count[0]);
      beam_recs = MAX(beam_recs, start[1] + count[1]);
      if (nc_inq_dimlen(ncid, timeDimID, &time_len))
        ERR;
      if (time_len != time_recs)
        ERR;
      if (nc_inq_dimlen(ncid, beamDimID, &beam_len))
        ERR;
      if (beam_len != beam_recs)
        ERR;
      start[0]++;
    }
...
#define TL 15
    ...
    if (nc_def_dim(ncid, TIME, NC_UNLIMITED, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, LEN, TL, &dimids[1]))
      ERR;
    if (nc_def_var(ncid, TIME, xtype, 2, dimids, &varid))
      ERR;
    ...
    if (nc_put_vara_text(ncid, varid, index, count, ttext))
      ERR;
    ...
    for (i = 0; i < NDIMS; i++)
      if (dimids[i] != dimids_in[i])
        ERR;
    if (nc_inq_dim(ncid, dimids[0], name_in, &len_in))
      ERR;
    if (strcmp(name_in, TIME) || len_in != 1)
      ERR;
    if (nc_close(ncid))
      ERR;
    ...
    if (nc_inq_dim(ncid, dimids[0], name_in, &len_in))
      ERR;
    if (strcmp(name_in, TIME) || len_in != NUM_RECS)
      ERR;
    if (nc_close(ncid))
      ERR;
  }
