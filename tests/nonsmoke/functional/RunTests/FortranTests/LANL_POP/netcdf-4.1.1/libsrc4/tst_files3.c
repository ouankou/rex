...
int dump_file2(const float *data, int docompression, int usedefdim) {
  ...
  if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
    ERR_RET;
  if (nc_def_dim(ncid, "time", X_LEN, &dimids[0]))
    ERR_RET;
  if (nc_def_dim(ncid, "lat", Y_LEN, &dimids[1]))
    ERR_RET;
  if (nc_def_dim(ncid, "lon", Z_LEN, &dimids[2]))
    ERR_RET;
  if (nc_def_var(ncid, "test", NC_FLOAT, NDIMS, dimids, &var))
    ERR_RET;
  if (docompression)
    if (nc_def_var_deflate(ncid, var, 1, 1, 1))
      ERR_RET;
  if (nc_enddef(ncid))
    ERR_RET;
  for (start[0] = 0; start[0] < X_LEN; start[0]++)
    for (start[1] = 0; start[1] < Y_LEN; start[1]++)
      if (nc_put_vara_float(ncid, var, start, count, data))
        ERR_RET;
  if (nc_close(ncid))
    ERR_RET;
...
int dump_file(const float *data, int docompression, int usedefdim) {
  ...
  if (nc_create(FILE_NAME, ncmode, &ncid))
    ERR_RET;
  if (nc_def_dim(ncid, "time", X_LEN, &dimids[0]))
    ERR_RET;
  if (nc_def_dim(ncid, "lat", Y_LEN, &dimids[1]))
  ...
  if (nc_enddef(ncid))
    ERR_RET;
  if (nc_put_vars_float(ncid, var, start, count, stride, data))
    ERR_RET;
  if (nc_close(ncid))
    ERR_RET;
...
int dump_hdf_file(const float *data, int docompression) {
  ...
  if (docompression) {
    if (H5Pset_layout(propid, H5D_CHUNKED) < 0)
      ERR;
    if (H5Pset_chunk(propid, NDIMS, dims) < 0)
      ERR;
    /*     values[0]=9; */
    /*     status = H5Pset_filter(propid, H5Z_FILTER_DEFLATE,0,1,&values[0]); */
    /*     printf("deflat estatus is: %i\n",status); */
    /* sets defalte level */
    if (H5Pset_deflate(propid, 1))
      ERR;
  }
  if ((file_spaceid = H5Screate_simple(NDIMS, dims, dims)) < 0)
    ERR;

  /* Set up the cache. */
  if ((access_plistid = H5Pcreate(H5P_DATASET_ACCESS)) < 0)
    ERR;
  if (H5Pset_chunk_cache(access_plistid, CHUNK_CACHE_NELEMS, CHUNK_CACHE_SIZE,
                         CHUNK_CACHE_PREEMPTION) < 0)
    ERR;

  /* Create the dataset. */
  if ((dataset_id = H5Dcreate2(file_id, "dset", H5T_NATIVE_FLOAT, file_spaceid,
                               H5P_DEFAULT, propid, access_plistid)) < 0)
    ERR;
  ...
  if (H5Pclose(propid) < 0)
    ERR;
  if (H5Pclose(access_plistid) < 0)
    ERR;
...
void get_mem_used2(int *mem_used) {
  char buf[30];
  FILE *pf;
...
int main(void) {
  ...
  printf("*** testing a scalar coordinate dimension...");
  {
    ...
    if (nc_create(FILE_NAME, NC_NETCDF4, &ncid))
      ERR_RET;
    if (nc_def_dim(ncid, "scalar", 0, &dimid))
      ERR_RET;
    if (nc_def_var(ncid, "scalar", NC_FLOAT, 0, &dimid, &varid))
      ERR_RET;
