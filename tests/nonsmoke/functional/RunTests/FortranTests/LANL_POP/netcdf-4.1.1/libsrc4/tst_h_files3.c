...
int main(int argc, char **argv) {
  ...
  /* 	 if ((fileid = H5Fopen(FILE_NAME2, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
   * ERR; */
  ...
  /* 	       for (a = 0; a < natts; a++) */
  /* 	       { */
  /* 		  if ((attid = H5Aopen_idx(datasetid[i], (unsigned int)a)) < 0)
   * ERR; */
  /* 		  if (H5Aget_name(attid, NC_MAX_HDF5_NAME, att_name) < 0) ERR;
   */
  /* 		  if (H5Aclose(attid) < 0) ERR; */
  /* 	       } */
  /* 	       if (H5DSget_scale_name(datasetid[i], dimscale_name_att,
   * NC_MAX_NAME) < 0) ERR; */
  /* 	    } */
  /* 	    else */
  /* 	    { */
