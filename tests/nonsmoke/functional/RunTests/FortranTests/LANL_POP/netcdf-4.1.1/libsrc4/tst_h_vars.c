/* This is part of the netCDF package.
   ...
   $Id: tst_h_vars.c,v 1.17 2010/01/12 18:11:27 ed Exp $
*/
#include "nc_tests.h"
...
int main() {
  ...
  if ((fileid = H5Fcreate(FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) <
      0)
    ERR;
  ...
  if ((spaceid = H5Screate_simple(1, dims, dims)) < 0)
    ERR;
  if ((datasetid = H5Dcreate(grpid, VAR_BOOL_NAME, H5T_NATIVE_HBOOL, spaceid,
                             H5P_DEFAULT)) < 0)
    ERR;
  if (H5Dwrite(datasetid, H5T_NATIVE_HBOOL, H5S_ALL, H5S_ALL, H5P_DEFAULT,
               bool_out) < 0)
    ERR;
  if (H5Dclose(datasetid) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 ||
      H5Fclose(fileid) < 0)
    ERR;
...
#define TEMP_NAME "Temperature"
    ...
    if ((fileid =
      ...
      ERR;
    if ((grpid = H5Gcreate(fileid, GRP2_NAME, 0)) < 0)
      ERR;
    ...
    if ((spaceid_in = H5Dget_space(pres_dsid)) < 0)
      ERR;
    if (H5Sget_simple_extent_dims(spaceid_in, dims_in, max_dims_in) < 0)
      ERR;
    if (dims_in[0] != 0 || dims_in[1] != LAT_LEN || dims_in[2] != LON_LEN)
      ERR;
    if (max_dims_in[0] != H5S_UNLIMITED || max_dims_in[1] != LAT_LEN ||
        max_dims_in[2] != LON_LEN)
      ERR;
    ...
    /* Get the spaceid and check various things. */
    if ((spaceid_in = H5Dget_space(temp_dsid)) < 0)
      ERR;
    if (H5Sget_simple_extent_dims(spaceid_in, dims_in, max_dims_in) < 0)
      ERR;
    if (dims_in[0] != 1 || dims_in[1] != LAT_LEN || dims_in[2] != LON_LEN)
      ERR;
    if (max_dims_in[0] != H5S_UNLIMITED || max_dims_in[1] != LAT_LEN ||
        max_dims_in[2] != LON_LEN)
      ERR;
    ...
    start[1] = 0;
    start[2] = 0;
    if (H5Sselect_hyperslab(write_spaceid, H5S_SELECT_SET, start, NULL, count,
                            NULL) < 0)
      ERR;
    ...
    if (H5Sget_simple_extent_dims(spaceid_in, dims_in, max_dims_in) < 0)
      ERR;
    if (dims_in[0] != 2 || dims_in[1] != LAT_LEN || dims_in[2] != LON_LEN)
      ERR;
    if (max_dims_in[0] != H5S_UNLIMITED || max_dims_in[1] != LAT_LEN ||
        max_dims_in[2] != LON_LEN)
      ERR;
...
#define DEFLATE_NAME "deflate"
  {
    ...
    dims[0] = DIM1_LEN;
    if ((propid = H5Pcreate(H5P_DATASET_CREATE)) < 0)
      ERR;
    if (H5Pset_layout(propid, H5D_CHUNKED))
      ERR;
    if (H5Pset_deflate(propid, DEFLATE_LEVEL))
      ERR;
    if ((spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    if ((datasetid = H5Dcreate(grpid, VAR_BOOL_NAME, H5T_NATIVE_HBOOL, spaceid,
    ...
    if (H5Dwrite(datasetid, H5T_NATIVE_HBOOL, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 bool_out) < 0)
      ERR;
    if (H5Dclose(datasetid) < 0 || H5Pclose(propid) < 0 ||
        H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((grpid = H5Gopen(fileid, GRP_NAME)) < 0)
      ERR;
    if ((datasetid = H5Dopen1(grpid, VAR_BOOL_NAME)) < 0)
      ERR;
  ...
  SUMMARIZE_ERR;
  printf("*** Checking HDF5 deflate, fletcher32, shuffle filter setting and "
         "getting...");
  {
    H5Z_filter_t filter;
    int num_filters;
    ...
    if (H5Pset_layout(propid, H5D_CHUNKED))
      ERR;
    if (H5Pset_shuffle(propid))
      ERR;
    if (H5Pset_deflate(propid, DEFLATE_LEVEL))
      ERR;
    if (H5Pset_fletcher32(propid))
      ERR;
    if ((spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    if ((datasetid = H5Dcreate(grpid, VAR_BOOL_NAME, H5T_NATIVE_HBOOL, spaceid,
    ...
    if (!found_fletcher32 || !found_deflate || !found_shuffle)
      ERR;
...
#define BE_VAR_NAME "be_var"
  {
    ...
    if ((spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    if ((native_did = H5Dcreate(grpid, NATIVE_VAR_NAME, H5T_NATIVE_INT, spaceid,
                                H5P_DEFAULT)) < 0)
      ERR;
    if ((le_did = H5Dcreate(grpid, LE_VAR_NAME, H5T_STD_I32LE, spaceid,
                            H5P_DEFAULT)) < 0)
      ERR;
    if ((be_did = H5Dcreate(grpid, BE_VAR_NAME, H5T_STD_I32BE, spaceid,
                            H5P_DEFAULT)) < 0)
      ERR;
    if (H5Dwrite(native_did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                 data) < 0)
      ERR;
    if (H5Dwrite(le_did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
        0)
      ERR;
    if (H5Dwrite(be_did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, data) <
        0)
      ERR;
    if (H5Dclose(native_did) < 0 || H5Dclose(le_did) < 0 ||
        H5Dclose(be_did) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((native_did = H5Dopen1(grpid, NATIVE_VAR_NAME)) < 0)
      ERR;
    if ((typeid = H5Dget_type(native_did)) < 0)
      ERR;
    if ((native_typeid = H5Tget_native_type(typeid, H5T_DIR_DESCEND)) < 0)
      ERR;
    if ((order = H5Tget_order(typeid)) < 0)
      ERR;
    if ((equal = H5Tequal(typeid, native_typeid)) < 0)
      ERR;
    if (!equal)
      ERR;
    ...
    if ((le_did = H5Dopen1(grpid, LE_VAR_NAME)) < 0)
      ERR;
    if ((typeid = H5Dget_type(le_did)) < 0)
      ERR;
    if ((order = H5Tget_order(typeid)) < 0)
      ERR;
    ...
    if ((be_did = H5Dopen1(grpid, BE_VAR_NAME)) < 0)
      ERR;
    if ((typeid = H5Dget_type(be_did)) < 0)
      ERR;
    if ((order = H5Tget_order(typeid)) < 0)
      ERR;
    ...
    if (H5Dclose(native_did) < 0 || H5Dclose(le_did) < 0 ||
        H5Dclose(be_did) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
