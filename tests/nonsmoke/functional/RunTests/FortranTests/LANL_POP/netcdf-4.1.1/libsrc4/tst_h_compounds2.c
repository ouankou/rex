/* This is part of the netCDF package.
   ...
   $Id: tst_h_compounds2.c,v 1.16 2010/01/21 00:23:42 ed Exp $
*/
#include "nc_tests.h"
...
int main() {
  ...
  printf("*** Checking HDF5 compound attribute which contains a simple "
         "compound type...");
  {
...
#define OUTER_TYPE_NAME "d"
    ...
    if ((attid = H5Acreate2(grpid, ATT_NAME1, typeid_outer, spaceid,
    ...
    if ((inner_type_native_typeid = H5Tget_member_type(att_native_typeid, 0)) <
        0)
      ERR;
    ...
    for (i_obj = 0; i_obj < num_obj; i_obj++) {
      if (H5Oget_info_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, i_obj,
                             &obj_info, H5P_DEFAULT) < 0)
        ERR;
      if (H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, i_obj,
                             obj_name, NC_MAX_HDF5_NAME + 1, H5P_DEFAULT) < 0)
        ERR;

      /* Deal with groups and datasets. */
      if (obj_info.type == H5O_TYPE_NAMED_DATATYPE) {
        /* Learn about the user-defined type. */
        if ((obj_hdf_typeid[i_obj] = H5Topen2(grpid, obj_name, H5P_DEFAULT)) <
            0)
          ERR;
        if ((obj_native_typeid[i_obj] = H5Tget_native_type(
                 obj_hdf_typeid[i_obj], H5T_DIR_DEFAULT)) < 0)
          ERR;
