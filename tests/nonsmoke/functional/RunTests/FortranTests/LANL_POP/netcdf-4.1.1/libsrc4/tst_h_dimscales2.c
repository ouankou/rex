/* This is part of the netCDF package.
   ...
   $Id: tst_h_dimscales2.c,v 1.18 2010/01/12 18:11:27 ed Exp $
*/
#include "nc_tests.h"
...
int main() {
...

  {
    ...
    if ((spaceid = H5Screate_simple(NDIMS, dims, maxdims)) < 0)
      ERR;
    ...
    if ((dimscaleid = H5Dcreate(fileid, DIMSCALE_NAME, H5T_NATIVE_INT, spaceid,
                                cparmsid)) < 0)
      ERR;
    ...
    if ((datasetid = H5Dcreate(fileid, VAR1_NAME, H5T_NATIVE_INT, spaceid,
                               cparmsid)) < 0)
      ERR;
    if (H5DSattach_scale(datasetid, dimscaleid, 0) < 0)
      ERR;
    if (H5DSset_label(datasetid, 0, DIMSCALE_LABEL) < 0)
      ERR;

    /* Fold up our tents. */
    if (H5Dclose(dimscaleid) < 0 || H5Dclose(datasetid) < 0 ||
        H5Sclose(spaceid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
...

  {
    ...
    if (H5Gget_num_objs(fileid, &num_obj) < 0)
      ERR;
    for (i = 0; i < num_obj; i++) {
      /* Get the type (i.e. group, dataset, etc.), and the name of
       * the object. */
      if ((obj_class = H5Gget_objtype_by_idx(fileid, i)) < 0)
        ERR;
      if (H5Gget_objname_by_idx(fileid, i, obj_name, NC_MAX_NAME) < 0)
        ERR;
      ...
      case H5G_DATASET:
        /* Open the dataset. */
        if ((datasetid = H5Dopen1(fileid, obj_name)) < 0)
          ERR;
        ...
        if (is_scale) {
          ...
          if (H5DSget_scale_name(datasetid, dimscale_name, NC_MAX_NAME) < 0)
            ERR;
          if (strcmp(dimscale_name, NAME_ATTRIBUTE))
            ERR;

          /* fileno and objno uniquely identify an object and a
           * HDF5 file. */
          if (H5Gget_objinfo(datasetid, ".", 1, &statbuf) < 0)
            ERR;
          ...
          if (H5DSiterate_scales(datasetid, 0, NULL, alien_visitor,
                                 &vars_dimscale_obj) < 0)
            ERR;
          /*printf("vars_dimscale_obj.fileno = %d vars_dimscale_obj.objno =
          ...
          if (vars_dimscale_obj.fileno[0] != dimscale_obj.fileno[0] ||
              ...
              vars_dimscale_obj.objno[1] != dimscale_obj.objno[1])
            ERR;
    ...
    /* Close up the shop. */
    if (H5Sclose(spaceid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
...
#define PRES_NAME "pres"
    ...
    dims[0] = LAT_LEN;
    if ((lat_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    dims[0] = LON_LEN;
    if ((lon_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;

    /* Create the space for the dataset. */
    dims[0] = LAT_LEN;
    dims[1] = LON_LEN;
    if ((pres_spaceid = H5Screate_simple(DIMS_2, dims, dims)) < 0)
      ERR;

    /* Create our dimension scales. */
    if ((lat_dimscaleid = H5Dcreate(fileid, LAT_NAME, H5T_NATIVE_INT,
                                    lat_spaceid, H5P_DEFAULT)) < 0)
      ERR;
    if (H5DSset_scale(lat_dimscaleid, NULL) < 0)
      ERR;
    if ((lon_dimscaleid = H5Dcreate(fileid, LON_NAME, H5T_NATIVE_INT,
                                    lon_spaceid, H5P_DEFAULT)) < 0)
      ERR;
    if (H5DSset_scale(lon_dimscaleid, NULL) < 0)
      ERR;

    /* Create a variable which uses these two dimscales. */
    if ((pres_datasetid = H5Dcreate(fileid, PRES_NAME, H5T_NATIVE_FLOAT,
                                    pres_spaceid, H5P_DEFAULT)) < 0)
      ERR;
    if (H5DSattach_scale(pres_datasetid, lat_dimscaleid, 0) < 0)
      ERR;
    if (H5DSattach_scale(pres_datasetid, lon_dimscaleid, 1) < 0)
      ERR;

    /* Fold up our tents. */
    if (H5Dclose(lat_dimscaleid) < 0 || H5Dclose(lon_dimscaleid) < 0 ||
        H5Dclose(pres_datasetid) < 0 || H5Sclose(lat_spaceid) < 0 ||
        H5Sclose(lon_spaceid) < 0 || H5Sclose(pres_spaceid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
  }
...
#define NDIMS2 2
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      case H5G_DATASET:
        ...
        if (is_scale) {
          ...
          /* 		  printf("dimscale_obj[%d].fileno = %d
           * dimscale_obj[%d].objno = %d\n", */
          /* 			 dimscale_cnt,
           * dimscale_obj[dimscale_cnt].fileno, dimscale_cnt,  */
          ...
          if (num_scales != 1)
            ERR;
          if ((num_scales = H5DSget_num_scales(datasetid, 1)) < 0)
            ERR;
          if (num_scales != 1)
            ERR;
          ...
          for (d = 0; d < ndims; d++) {
            ...
            if (vars_dimscale_obj[d].fileno[0] != dimscale_obj[d].fileno[0] ||
                vars_dimscale_obj[d].objno[0] != dimscale_obj[d].objno[0])
              ERR;
            if (vars_dimscale_obj[d].fileno[1] != dimscale_obj[d].fileno[1] ||
                vars_dimscale_obj[d].objno[1] != dimscale_obj[d].objno[1])
              ERR;
          }
        }
        if (H5Dclose(datasetid) < 0)
  ...
  SUMMARIZE_ERR;
  printf("*** Creating a file with one var with two unlimited dimension "
         "scales...");
  {
#define U1_LEN 3
#define U2_LEN 2
...
#define VNAME "v1"
    ...
    if ((lat_spaceid = H5Screate_simple(1, dims, maxdims)) < 0)
      ERR;
    if ((lon_spaceid = H5Screate_simple(1, dims, maxdims)) < 0)
      ERR;
    ...
    /* Create our dimension scales. */
    if ((lat_dimscaleid = H5Dcreate(grpid, U1_NAME, H5T_NATIVE_INT, lat_spaceid,
                                    plistid)) < 0)
      ERR;
    if (H5DSset_scale(lat_dimscaleid, NULL) < 0)
      ERR;
    if ((lon_dimscaleid = H5Dcreate(grpid, U2_NAME, H5T_NATIVE_INT, lon_spaceid,
                                    plistid)) < 0)
      ERR;
    if (H5DSset_scale(lon_dimscaleid, NULL) < 0)
      ERR;
    ...
    if ((plistid2 = H5Pcreate(H5P_DATASET_CREATE)) < 0)
      ERR;
    if (H5Pset_chunk(plistid2, DIMS2, chunksize) < 0)
      ERR;
    ...
    /* Create a variable which uses these two dimscales. */
    if ((pres_datasetid = H5Dcreate(grpid, VNAME, H5T_NATIVE_DOUBLE,
                                    pres_spaceid, plistid2)) < 0)
      ERR;
    if (H5DSattach_scale(pres_datasetid, lat_dimscaleid, 0) < 0)
      ERR;
    if (H5DSattach_scale(pres_datasetid, lon_dimscaleid, 1) < 0)
    ...
    if (H5Pclose(fapl_id) < 0 || H5Pclose(fcpl_id) < 0 ||
        H5Dclose(lat_dimscaleid) < 0 || H5Dclose(lon_dimscaleid) < 0 ||
        H5Dclose(pres_datasetid) < 0 || H5Sclose(lat_spaceid) < 0 ||
        H5Sclose(lon_spaceid) < 0 || H5Sclose(pres_spaceid) < 0 ||
        H5Pclose(plistid) < 0 || H5Pclose(plistid2) < 0 ||
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      case H5G_DATASET:
        /* Open the dataset. */
        if ((datasetid = H5Dopen1(grpid, obj_name)) < 0)
          ERR;
    ...
    /* Check the dimension lengths. */
    {
      ...
      /* Check U1. */
      if ((datasetid = H5Dopen1(grpid, U1_NAME)) < 0)
        ERR;
      if ((spaceid1 = H5Dget_space(datasetid)) < 0)
        ERR;
      if ((dataset_ndims =
               H5Sget_simple_extent_dims(spaceid1, h5dimlen, h5dimlenmax)) < 0)
        ERR;
      if (dataset_ndims != 1 || h5dimlen[0] != 0 ||
          h5dimlenmax[0] != H5S_UNLIMITED)
        ERR;
      if (H5Dclose(datasetid) || H5Sclose(spaceid1))
        ERR;

      /* Check U2. */
      if ((datasetid = H5Dopen1(grpid, U2_NAME)) < 0)
        ERR;
      if ((spaceid1 = H5Dget_space(datasetid)) < 0)
        ERR;
      if ((dataset_ndims =
      ...
      /* Check V1. */
      if ((datasetid = H5Dopen1(grpid, VNAME)) < 0)
        ERR;
      if ((spaceid1 = H5Dget_space(datasetid)) < 0)
        ERR;
      if ((dataset_ndims =
      ...
      if (dataset_ndims != 2 || h5dimlen[0] != 0 || h5dimlen[1] != 0 ||
          h5dimlenmax[0] != H5S_UNLIMITED || h5dimlenmax[1] != H5S_UNLIMITED)
        ERR;
...
#define NUM_VALS 3
      ...
      if (H5Sselect_hyperslab(file_spaceid, H5S_SELECT_SET, start, NULL, count,
                              NULL) < 0)
        ERR;
      if ((mem_spaceid = H5Screate_simple(DIMS2, count, NULL)) < 0)
        ERR;
      ...
      /* All done. */
      if (H5Dclose(datasetid) || H5Sclose(mem_spaceid) ||
...
#define LEN_NAME "u_len"
    ...
    hid_t fileid, lat_spaceid, lon_spaceid, time_spaceid, pres_spaceid,
        len_spaceid;
    hid_t pres_datasetid, lat_dimscaleid, lon_dimscaleid, time_dimscaleid,
        len_dimscaleid;
    ...
    H5G_stat_t statbuf;
    HDF5_OBJID_T dimscale_obj[NUM_DIMSCALES1],
        vars_dimscale_obj[NUM_DIMSCALES1];
    int dimscale_cnt = 0;
    int d, ndims;
    ...
    if ((lon_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    dims[0] = TIME_LEN;
    if ((time_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    dims[0] = LEN_LEN;
    if ((len_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    ...
    dims[1] = LON_LEN;
    dims[2] = TIME_LEN;
    if ((pres_spaceid = H5Screate_simple(DIMS_3, dims, dims)) < 0)
      ERR;
    ...
    if ((time_dimscaleid = H5Dcreate1(fileid, TIME_NAME, H5T_NATIVE_INT,
                                      time_spaceid, H5P_DEFAULT)) < 0)
      ERR;
    if (H5DSset_scale(time_dimscaleid, NULL) < 0)
      ERR;
    if ((len_dimscaleid = H5Dcreate1(fileid, LEN_NAME, H5T_NATIVE_INT,
                                     len_spaceid, H5P_DEFAULT)) < 0)
      ERR;
    if (H5DSset_scale(len_dimscaleid, NULL) < 0)
      ERR;
    ...
    if (H5DSattach_scale(pres_datasetid, lon_dimscaleid, 1) < 0)
      ERR;
    if (H5DSattach_scale(pres_datasetid, time_dimscaleid, 2) < 0)
      ERR;
    ...
    /* Fold up our tents. */
    if (H5Dclose(lat_dimscaleid) < 0 || H5Dclose(lon_dimscaleid) < 0 ||
        H5Dclose(time_dimscaleid) < 0 || H5Dclose(len_dimscaleid) < 0 ||
        H5Dclose(pres_datasetid) < 0 || H5Sclose(lat_spaceid) < 0 ||
        H5Sclose(lon_spaceid) < 0 || H5Sclose(time_spaceid) < 0 ||
        H5Sclose(pres_spaceid) < 0 || H5Sclose(len_spaceid) < 0 ||
        H5Pclose(fapl_id) < 0 || H5Pclose(fcpl_id) < 0 || H5Fclose(fileid) < 0)
      ERR;
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      case H5G_DATASET:
        ...
        if (is_scale) {
          ...
          dimscale_obj[dimscale_cnt].fileno[1] = statbuf.fileno[1];
          dimscale_obj[dimscale_cnt].objno[1] = statbuf.objno[1];
          /* printf("dimscale_obj[%d].fileno = %d dimscale_obj[%d].objno =
           * %d\n", */
          /* 			 dimscale_cnt,
           * dimscale_obj[dimscale_cnt].fileno, dimscale_cnt, */
          /* 			 dimscale_obj[dimscale_cnt].objno); */
  ...
  SUMMARIZE_ERR;
  printf("*** Checking cration ordering of datasets which are also dimension "
         "scales...");
...
#define LEN_NAME "u_len"
    ...
    hsize_t maxdims[DIMS_3];
    H5G_stat_t statbuf;
    HDF5_OBJID_T dimscale_obj[NUM_DIMSCALES2],
        vars_dimscale_obj[NUM_DIMSCALES2];
    int dimscale_cnt = 0;
    int d, ndims;
