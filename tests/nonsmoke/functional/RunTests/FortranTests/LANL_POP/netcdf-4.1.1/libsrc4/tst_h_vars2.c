/* This is part of the netCDF package. Copyright 2007 University
   ...
   $Id: tst_h_vars2.c,v 1.23 2010/01/12 18:11:27 ed Exp $
*/
#include "nc_tests.h"
...
#define ELEMENTS_NAME "Elements"
  {
    ...
    int i;
    H5O_info_t obj_info;
    char names[NUM_ELEMENTS][MAX_SYMBOL_LEN + 1] = {"H",  "He", "Li",
                                                    "Be", "B",  "C"};
    ...
    if (H5Pset_link_creation_order(gcpl_id, H5P_CRT_ORDER_TRACKED |
                                                H5P_CRT_ORDER_INDEXED) < 0)
      ERR;
    if ((grpid = H5Gcreate_anon(fileid, gcpl_id, H5P_DEFAULT)) < 0)
      ERR;
    if ((H5Olink(grpid, fileid, ELEMENTS_NAME, H5P_DEFAULT, H5P_DEFAULT)) < 0)
      ERR;
    ...
    for (i = 0; i < NUM_ELEMENTS; i++) {
      if ((did[i] = H5Dcreate(grpid, names[i], H5T_NATIVE_INT, spaceid,
                              H5P_DEFAULT)) < 0)
        ERR;
      if (H5Dclose(did[i]) < 0)
        ERR;
    ...
    if (H5Pclose(fapl_id) < 0 || H5Pclose(gcpl_id) < 0 ||
    ...
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, fapl_id)) < 0)
      ERR;
    if ((grpid = H5Gopen(fileid, ELEMENTS_NAME)) < 0)
      ERR;

    if (H5Gget_num_objs(grpid, &num_obj) < 0)
      ERR;
    if (num_obj != NUM_ELEMENTS)
      ERR;
    for (i = 0; i < num_obj; i++) {
      ...
      if (obj_info.type != H5O_TYPE_DATASET)
        ERR;
      if ((size = H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER,
                                     H5_ITER_INC, i, NULL, 0, H5P_DEFAULT)) < 0)
        ERR;
      H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, i, name,
                         size + 1, H5P_DEFAULT);
      if (strcmp(name, names[i]))
        ERR;
    }
    if (H5Pclose(fapl_id) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
  SUMMARIZE_ERR;
...
#define NAME_ATTRIBUTE "short"
  {
    ...
    if (H5Pclose(fapl_id) < 0 || H5Pclose(fcpl_id) < 0 ||
        H5Sclose(dimscale_spaceid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  ...
  SUMMARIZE_ERR;
  printf("*** Checking HDF5 variable ordering flags with redef-type "
         "situations...");
...
#define VAR_NAME "Sears_Zemansky_and_Young"
  {
    ...
    if ((did = H5Dcreate(grpid, VAR_NAME, H5T_NATIVE_INT, spaceid,
                         H5P_DEFAULT)) < 0)
      ERR;
    if (H5Dclose(did) < 0)
      ERR;
    ...
    if (num_obj != 1)
      ERR;
    if (H5Oget_info_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, 0,
                           &obj_info, H5P_DEFAULT) < 0)
      ERR;
    if (obj_info.type != H5O_TYPE_DATASET)
      ERR;
    if ((size = H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC,
      ...
      ERR;
    H5Lget_name_by_idx(grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, 0, name,
                       size + 1, H5P_DEFAULT);
...
#define SIMPLE_VAR_NAME "data"
  {
    ...
    if ((spaceid = H5Screate_simple(NDIMS, dimsize, maxdimsize)) < 0)
      ERR;
    ...
    /* Create the variable. */
    if ((datasetid = H5Dcreate(grpid, SIMPLE_VAR_NAME, H5T_NATIVE_INT, spaceid,
                               plistid)) < 0)
      ERR;
    ...
    if (H5Dclose(datasetid) < 0)
      ERR;
    if (H5Pclose(fapl_id) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((datasetid = H5Dopen1(grpid, SIMPLE_VAR_NAME)) < 0)
      ERR;
    if ((spaceid = H5Dget_space(datasetid)) < 0)
      if (H5Sget_simple_extent_dims(spaceid, fdims, fmaxdims) > 0)
        ERR;
    if (H5Dread(datasetid, H5T_NATIVE_INT, H5S_ALL, spaceid, H5P_DEFAULT,
                data_in) < 0)
      ERR;
    ...
    for (x = 0; x < NX; x++)
      for (y = 0; y < NY; y++)
        if (data_in[x][y] != data_out[x][y])
          ERR_RET;

    if (H5Pclose(fapl_id) < 0 || H5Dclose(datasetid) < 0 ||
        H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
  SUMMARIZE_ERR;
  ...
  /*       if ((fileid = H5Fcreate(FILE_NAME, H5F_ACC_TRUNC, fcpl_id, fapl_id))
   * < 0) ERR; */
...
#define ATT_NAME "_FillValue"
    ...
    if ((grpid = H5Gopen(fileid, "/")) < 0)
      ERR;
    if ((datasetid = H5Dopen1(grpid, VAR_NAME2)) < 0)
      ERR;
    ...
    if (!(type_size = H5Tget_size(native_typeid)))
      ERR;
    if (type_size != sizeof(obs_t))
      ERR;
    if (H5Pget_fill_value(propid, native_typeid, &f1) < 0)
      ERR;
    if (f1.day != m.day || f1.elev != m.elev || f1.count != m.count ||
        f1.relhum != m.relhum || f1.time != m.time)
      ERR;

    /* It's also in an attribute, netCDF style. */
    if ((attid = H5Aopen_name(datasetid, ATT_NAME)) < 0)
      ERR;
    if ((typeid = H5Aget_type(attid)) < 0)
      ERR;
    if ((native_typeid = H5Tget_native_type(typeid, H5T_DIR_DEFAULT)) < 0)
      ERR;
    if (H5Aread(attid, native_typeid, &f2) < 0)
      ERR;
    if (f2.day != m.day || f2.elev != m.elev || f2.count != m.count ||
        f2.relhum != m.relhum || f2.time != m.time)
      ERR;

    if (H5Dclose(datasetid) < 0 || H5Aclose(attid) < 0 ||
        H5Tclose(native_typeid) < 0 || H5Tclose(typeid) < 0 ||
        H5Pclose(propid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
  SUMMARIZE_ERR;
  FINAL_RESULTS;
