...
int nc4_get_att(int ncid, NC_FILE_INFO_T *nc, int varid, const char *name,
                nc_type *xtype, nc_type mem_type, size_t *lenp, int *attnum,
  ...
  LOG((3, "nc4_get_att: ncid 0x%x varid %d name %s attnum %d mem_type %d", ncid,
       varid, name, my_attnum, mem_type));
  ...
  if (data && att->len && mem_type != att->xtype && mem_type != NC_NAT &&
      !(mem_type == NC_CHAR &&
        (att->xtype == NC_UBYTE || att->xtype == NC_BYTE))) {
    need_to_convert++;
    if (!(bufr = malloc((size_t)(att->len * type_size))))
      return NC_ENOMEM;
    if ((retval = nc4_convert_type(att->data, bufr, att->xtype, mem_type,
                                   (size_t)att->len, &range_error, NULL,
    ...
    if ((h5->cmode & NC_CLASSIC_MODEL) &&
        (att->xtype == NC_UBYTE || att->xtype == NC_BYTE) &&
        (mem_type == NC_UBYTE || mem_type == NC_BYTE) && range_error)
      range_error = 0;
  } else {
    bufr = att->data;
  }
...
int nc4_put_att(int ncid, NC_FILE_INFO_T *nc, int varid, const char *name,
  ...
  LOG((1,
       "nc4_put_att: ncid 0x%x varid %d name %s "
       "file_type %d mem_type %d len %d",
       ncid, varid, name, file_type, mem_type, len));
  ...
  if (!att) {
    ...
    if (!(h5->flags & NC_INDEF) &&
        len * nc4typelen(file_type) >
            (size_t)att->len * nc4typelen(att->xtype)) {
      if (h5->cmode & NC_CLASSIC_MODEL)
        return NC_EINDEFINE;
  ...
  if (file_type != mem_type &&
      (file_type == NC_CHAR || mem_type == NC_CHAR || file_type == NC_STRING ||
       mem_type == NC_STRING))
    return NC_ECHAR;
  ...
  if (!strcmp(att->name, _FillValue) && varid != NC_GLOBAL) {
    ...
    if (type_info && type_info->class == NC_VLEN) {
      nc_vlen_t *in_vlen = (nc_vlen_t *)data,
                *fv_vlen = (nc_vlen_t *)(var->fill_value);
  ...
  if (type && type->class == NC_VLEN && data && att->len) {
    ...
    if (!(att->stdata = malloc(sizeof(char *) * att->len)))
      BAIL(NC_ENOMEM);
    for (i = 0; i < att->len; i++) {
      LOG((5, "copying string %d of size %d", i,
           strlen(((char **)data)[i]) + 1));
    ...
    if (att->len) {
      ...
      if (type) {
        /* Just copy the data... */
        if (type->class == NC_OPAQUE || type->class == NC_COMPOUND ||
            type->class == NC_ENUM)
        ...
        if ((retval = nc4_convert_type(
                 data, att->data, mem_type, file_type, len, &range_error, NULL,
...
int nc_inq_att(int ncid, int varid, const char *name, nc_type *xtypep,
  ...
  return nc4_get_att(ncid, nc, varid, name, xtypep, NC_UBYTE, lenp, NULL, 0,
                     NULL);
}
...
int nc_inq_attid(int ncid, int varid, const char *name, int *attnump) {
  ...
  /* Handle netcdf-4 files. */
  return nc4_get_att(ncid, nc, varid, name, NULL, NC_UBYTE, NULL, attnump, 0,
                     NULL);
}
...
int nc_inq_attname(int ncid, int varid, int attnum, char *name) {
  ...
  LOG((2, "nc_inq_attname: ncid 0x%x varid %d attnum %d", ncid, varid, attnum));
...
static int compare_nc_types(int ncid1, NC_TYPE_INFO_T *t1, int ncid2,
                            NC_TYPE_INFO_T *t2, int *equal) {
  ...
  *equal = 0;
  ...
  if (t1->nc_typeid <= NC_STRING) {
    ...
    /* Check the obvious. */
    if (t1->size != t2->size || t1->class != t2->class ||
    ...
    case NC_ENUM:
      if (t1->base_nc_type != t2->base_nc_type ||
          t1->num_enum_members != t2->num_enum_members)
        return NC_NOERR;
      for (m1 = t1->enum_member, m2 = t2->enum_member; m1;
           m1 = m1->next, m2 = m2->next)
        if (strcmp(m1->name, m2->name) ||
            memcmp(m1->value, m2->value, t1->size))
          return NC_NOERR;
      break;
    case NC_COMPOUND:
...
static int rec_find_nc_type(int ncid1, NC_TYPE_INFO_T *type1, int ncid2,
...
static int nc4_find_equal_type(int ncid1, nc_type xtype1, int ncid2,
                               nc_type *xtype2) {
...
int nc_copy_att(int ncid_in, int varid_in, const char *name, int ncid_out,
  ...
  LOG((2, "nc_copy_att: ncid_in 0x%x varid_in %d name %s", ncid_in, varid_in,
       name));
  ...
  if (xtype < NC_STRING) {
    ...
    if (!res)
      res = nc_put_att(ncid_out, varid_out, name, xtype, len, data);
    if (len)
      free(data);
    ...
    if ((res =
             nc_inq_user_type(ncid_in, xtype, NULL, &size, NULL, NULL, &class)))
      return res;
    if (class == NC_VLEN) /* VLENs are different... */
    {
      ...
      if ((res = nc_get_att(ncid_in, varid_in, name, vldata)))
        return res;
      if ((res = nc_put_att(ncid_out, varid_out, name, xtype_out, len, vldata)))
        return res;
      for (i = 0; i < len; i++)
      ...
      if ((res = nc_get_att(ncid_in, varid_in, name, data)))
        return res;
      if ((res = nc_put_att(ncid_out, varid_out, name, xtype_out, len, data)))
        return res;
      free(data);
    }
  }
...
int nc_rename_att(int ncid, int varid, const char *name, const char *newname) {
  ...
  LOG((2, "nc_rename_att: ncid 0x%x varid %d name %s newname %s", ncid, varid,
       name, newname));
...
int nc4_put_att_tc(int ncid, int varid, const char *name, nc_type file_type,
  ...
  LOG((3,
       "nc4_put_att_tc: ncid 0x%x varid %d name %s file_type %d "
       "mem_type %d len %d",
       ncid, varid, name, file_type, mem_type, len));
  ...
  if (nc->pnetcdf_file) {
    ...
    switch (mem_type) {
    case NC_BYTE:
      return ncmpi_put_att_schar(nc->int_ncid, varid, name, file_type, len, op);
    case NC_CHAR:
      return ncmpi_put_att_text(nc->int_ncid, varid, name, len, op);
    case NC_SHORT:
      return ncmpi_put_att_short(nc->int_ncid, varid, name, file_type, len, op);
    case NC_INT:
      if (mem_type_is_long)
        return ncmpi_put_att_long(nc->int_ncid, varid, name, file_type, len,
                                  op);
      else
        return ncmpi_put_att_int(nc->int_ncid, varid, name, file_type, len, op);
    case NC_FLOAT:
      return ncmpi_put_att_float(nc->int_ncid, varid, name, file_type, len, op);
    case NC_DOUBLE:
      return ncmpi_put_att_double(nc->int_ncid, varid, name, file_type, len,
                                  op);
    case NC_NAT:
    default:
      return NC_EBADTYPE;
  ...
  if (!nc->nc4_info) {
    ...
    switch (mem_type) {
    case NC_NAT:
      return nc3_put_att(nc->int_ncid, varid, name, file_type, len, op);
    case NC_BYTE:
      return nc3_put_att_schar(nc->int_ncid, varid, name, file_type, len, op);
    case NC_CHAR:
      return nc3_put_att_text(nc->int_ncid, varid, name, len, op);
    case NC_SHORT:
      return nc3_put_att_short(nc->int_ncid, varid, name, file_type, len, op);
    case NC_INT:
      if (mem_type_is_long)
        return nc3_put_att_long(nc->int_ncid, varid, name, file_type, len, op);
      else
        return nc3_put_att_int(nc->int_ncid, varid, name, file_type, len, op);
    case NC_FLOAT:
      return nc3_put_att_float(nc->int_ncid, varid, name, file_type, len, op);
    case NC_DOUBLE:
      return nc3_put_att_double(nc->int_ncid, varid, name, file_type, len, op);
    default:
      return NC_EBADTYPE;
    }
  }
...
int nc4_get_att_tc(int ncid, int varid, const char *name, nc_type mem_type,
                   int mem_type_is_long, void *ip) {
  NC_FILE_INFO_T *nc;

  LOG((3, "nc4_get_att_tc: ncid 0x%x varid %d name %s mem_type %d", ncid, varid,
       name, mem_type));
  ...
  return nc4_get_att(ncid, nc, varid, name, NULL, mem_type, NULL, NULL,
                     mem_type_is_long, ip);
}
...
int nc_put_att_text(int ncid, int varid, const char *name, size_t len,
                    const char *op) {
  return nc4_put_att_tc(ncid, varid, name, NC_CHAR, NC_CHAR, 0, len, op);
}
...
int nc_put_att_uchar(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_UBYTE, 0, len, op);
}
...
int nc_put_att_schar(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_BYTE, 0, len, op);
}
...
int nc_put_att_short(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_SHORT, 0, len, op);
}
...
int nc_put_att_int(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_INT, 0, len, op);
}
...
int nc_put_att_long(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_INT, 1, len, op);
}
...
int nc_put_att_float(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_FLOAT, 0, len, op);
}
...
int nc_put_att_double(int ncid, int varid, const char *name, nc_type xtype,
  ...
  if (xtype == NC_NAT)
    return NC_EBADTYPE;
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_DOUBLE, 0, len, op);
}
...
int nc_put_att_ubyte(int ncid, int varid, const char *name, nc_type xtype,
                     size_t len, const unsigned char *op) {
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_UBYTE, 0, len, (void *)op);
}
...
int nc_put_att_ushort(int ncid, int varid, const char *name, nc_type xtype,
  ...
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_USHORT, 0, len,
                        (void *)op);
}
...
int nc_put_att_uint(int ncid, int varid, const char *name, nc_type xtype,
  ...
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_UINT, 0, len, (void *)op);
}
...
int nc_put_att_longlong(int ncid, int varid, const char *name, nc_type xtype,
  ...
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_INT64, 0, len, (void *)op);
}
...
int nc_put_att_ulonglong(int ncid, int varid, const char *name, nc_type xtype,
  ...
  return nc4_put_att_tc(ncid, varid, name, xtype, NC_UINT64, 0, len,
                        (void *)op);
}
...
int nc_put_att_string(int ncid, int varid, const char *name, size_t len,
                      const char **op) {
  return nc4_put_att_tc(ncid, varid, name, NC_STRING, NC_STRING, 0, len,
                        (void *)op);
}
