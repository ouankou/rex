/* This is part of the netCDF package.  Copyright 2005 University
   ...
   $Id: tst_h_vl.c,v 1.10 2008/05/19 14:18:03 ed Exp $
*/
#include "nc_tests.h"
...
int main() {
  ...
  printf("*** Checking simple HDF5 variable length types...");
  {
    ...
    if ((fileid =
      ...
      ERR;
    if ((grpid = H5Gcreate(fileid, GROUP_NAME, 0)) < 0)
      ERR;

    /* Create VLEN type. */
    if ((typeid = H5Tvlen_create(H5T_NATIVE_INT)) < 0)
      ERR;

    /* Although it's a vlen of ints, the size is rouned up to 8. */
    if (!(size = H5Tget_size(typeid)))
      ERR;
    if (size < 8)
      ERR;
    ...
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
      ERR;
    if ((grpid = H5Gopen(fileid, GROUP_NAME)) < 0)
      ERR;
    ...
    if (H5Dvlen_reclaim(typeid, spaceid, H5P_DEFAULT, data_in) < 0)
      ERR;
  ...
  /*       if (H5Tinsert(compound_typeid, "temp_vl", HOFFSET(struct
   * sea_sounding, temp_vl),  */
  /* 		    vlen_typeid) < 0) ERR; */
  ...
  /*       if (H5Dwrite(datasetid, compound_typeid, H5S_ALL, H5S_ALL,
   * H5P_DEFAULT,  */
  /* 		   data) < 0) ERR; */
