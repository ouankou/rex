...
int write_atts(int ncid, int varid) {
  int a;
  char att_name[NC_MAX_NAME + 1];

  for (a = 0; a < NUM_ATTS; a++) {
    sprintf(att_name, "att_%d", a);
    if (nc_put_att_int(ncid, varid, att_name, NC_INT, ATT_LEN, att_data))
      ERR_RET;
  }
  return NC_NOERR;
}

int read_atts(int ncid, int varid) {
  int a, i;
  char att_name[NC_MAX_NAME + 1];
  ...
  for (a = 0; a < NUM_ATTS; a++) {
    sprintf(att_name, "att_%d", a);
    if (nc_get_att_int(ncid, varid, att_name, att_data_in))
      ERR_RET;
    for (i = 0; i < ATT_LEN; i++)
      if (att_data_in[i] != att_data[i])
        ERR_RET;
  }
  return NC_NOERR;
}

int write_vars(int ncid) {
  int v;
  ...
  if (nc_def_dim(ncid, DIM_NAME, VAR_LEN, NULL))
    ERR;
  for (v = 0; v < NUM_VARS; v++) {
    sprintf(var_name, "var_%d", v);
    if (nc_def_var(ncid, var_name, NC_INT, NUM_DIMS, dimid, NULL))
      ERR_RET;
    write_atts(ncid, v);
    if (nc_put_var_int(ncid, v, var_data))
      ERR;
  }
  return NC_NOERR;
}

int read_vars(int ncid) {
  int v, i;
  ...
  for (v = 0; v < NUM_VARS; v++) {
    if (nc_inq_var(ncid, v, var_name_in, &xtype_in, &ndims_in, NULL, &natts_in))
      ERR_RET;
    sprintf(var_name, "var_%d", v);
    if (strcmp(var_name, var_name_in) || xtype_in != NC_INT ||
        ndims_in != NUM_DIMS || natts_in != NUM_ATTS)
      ERR_RET;
    read_atts(ncid, v);
    if (nc_get_var_int(ncid, v, var_data_in))
      ERR;
    for (i = 0; i < VAR_LEN; i++)
      if (var_data_in[i] != var_data[i])
        ERR_RET;
  }
  return NC_NOERR;
}
...
int main(int argc, char **argv) {
  printf("\n*** Testing interoperability between big vs. little endian "
         "platforms.\n");
  printf("*** testing with file with lots of atts...");
  {
    ...
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdim_in))
      ERR;
    if (ndims_in != NUM_DIMS || nvars_in != NUM_VARS || natts_in != NUM_ATTS ||
        unlimdim_in != -1)
      ERR;
    if (read_atts(ncid, NC_GLOBAL))
      ERR;
    if (read_vars(ncid))
      ERR;
    if (nc_close(ncid))
