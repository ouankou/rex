/* This is part of the netCDF package.  Copyright 2005 University
   ...
   $Id: tst_h_strings.c,v 1.18 2010/03/15 12:01:43 ed Exp $
*/
#include "nc_tests.h"
...
int main() {
  ...
  printf("*** Checking scalar string attribute...");
  {
    ...
    if (!(type_size = H5Tget_size(typeid)))
      ERR;
    if ((is_str = H5Tis_variable_str(typeid)) < 0)
      ERR;
  ...
  printf("*** Checking simple HDF5 string types...");
  {
    ...
    if (H5Sget_simple_extent_dims(spaceid, dims, NULL) < 0)
      ERR;
    if (!(data_in = malloc(dims[0] * sizeof(char *))))
      ERR;
...
#define S2 "He proved so brave and daring..."
    ...
    if (H5Sget_simple_extent_dims(spaceid, dims_in, NULL) != 1)
      ERR;
    if (dims_in[0] != dims[0])
      ERR;
    ...
    if (!(data_in = malloc(dims_in[0] * sizeof(char *))))
      ERR;
