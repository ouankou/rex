...
int main(int argc, char **argv) {
  ...
  printf("**** testing simple fill value attribute creation...");
  {
    ...
    if (nc_set_fill(ncid, NC_NOFILL, NULL))
      ERR;
    if (nc_def_dim(ncid, "dim1", DIM1, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, "dim2", DIM2, &dimids[1]))
      ERR;
    if (nc_def_var(ncid, "var", NC_BYTE, NUMDIMS, dimids, &varid))
      ERR;
    if (nc_enddef(ncid))
      ERR;
    ...
    /*       if (char_val_in != 99)	/\* see above, the value written when
     * start[0]==0, j==0 *\/ */
    /* 	 ERR; */
