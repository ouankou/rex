/* This is part of the netCDF package.
   ...
   $Id: tst_interops2.c,v 1.8 2009/07/16 11:33:20 ed Exp $
*/
#include "config.h"
...
#define DIMS_2 2
    ...
    if (SDwritedata(sds_id, start, NULL, edge, (void *)data_out))
      ERR;
    ...
    if (nc_get_vara(ncid, 0, nstart, ncount, data_in))
      ERR;
    for (i = 0; i < LAT_LEN; i++)
      for (j = 0; j < LON_LEN; j++)
...
#define NUM_TYPES 8
    ...
    size_t len_in;
    nc_type type_in;
    int hdf4_type[NUM_TYPES] = {DFNT_FLOAT32, DFNT_FLOAT64, DFNT_INT8,
                                DFNT_UINT8,   DFNT_INT16,   DFNT_UINT16,
                                DFNT_INT32,   DFNT_UINT32};
    int netcdf_type[NUM_TYPES] = {NC_FLOAT, NC_DOUBLE, NC_BYTE, NC_UBYTE,
                                  NC_SHORT, NC_USHORT, NC_INT,  NC_UINT};
    ...
    for (t = 0; t < NUM_TYPES; t++) {
      ...
      if ((sds_id = SDcreate(sd_id, tmp_name, hdf4_type[t], DIMS_3,
                             dim_size)) == FAIL)
        ERR;
      ...
      for (d = 0; d < DIMS_3; d++) {
        int32 dimid;
        if ((dimid = SDgetdimid(sds_id, d)) == FAIL)
          ERR;
        if (SDsetdimname(dimid, dim_name[d]))
          ERR;
      }
      if (SDendaccess(sds_id))
        ERR;
    ...
    if (nc_inq(ncid, &ndims_in, &nvars_in, &natts_in, &unlimdim_in))
      ERR;
    if (ndims_in != DIMS_3 || nvars_in != NUM_TYPES || natts_in != 0 ||
        unlimdim_in != -1)
      ERR;
    if (nc_inq_dim(ncid, 0, NULL, &len_in))
      ERR;
    ...
    if (len_in != Y_LEN)
      ERR;
    if (nc_inq_dim(ncid, 2, NULL, &len_in))
      ERR;
    if (len_in != X_LEN)
      ERR;
    for (t = 0; t < NUM_TYPES; t++) {
      if (nc_inq_var(ncid, t, name_in, &type_in, &ndims_in, dimids_in,
                     &natts_in))
        ERR;
