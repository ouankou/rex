...
int main() {
  ...
  {
    float val = 99;
    char emp[NUM_EMPS][MAX_LEN + 1] = {"Augustus", "Tiberius", "Caligula",
                                       "Claudius", "Ne_r_o",   "V.esp.asi.an"};
    ...
    if ((fileid =
      ...
      ERR;
    if ((grpid = H5Gcreate(fileid, EMP_GRP, 0)) < 0)
      ERR;
    ...
    for (e = 0; e < NUM_EMPS; e++) {
      if ((attid = H5Acreate(grpid, emp[e], H5T_NATIVE_FLOAT, spaceid,
                             H5P_DEFAULT)) < 0)
        ERR;
      if (H5Awrite(attid, H5T_NATIVE_FLOAT, &val) < 0)
        ERR;
      if (H5Aclose(attid) < 0)
        ERR;
    }
    ...
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDWR, H5P_DEFAULT)) < 0)
      ERR;
    if ((grpid = H5Gopen(fileid, EMP_GRP)) < 0)
      ERR;

    /* How many attributes are there? */
    if ((num_obj = H5Aget_num_attrs(grpid)) != NUM_EMPS)
      ERR;

    /* Make sure the names are in the correct order. */
    for (i = 0; i < num_obj; i++) {
      if ((attid = H5Aopen_idx(grpid, (unsigned int)i)) < 0)
        ERR;
      if (H5Aget_name(attid, MAX_LEN + 1, obj_name) < 0)
        ERR;
      if (H5Aclose(attid) < 0)
        ERR;
      if (strcmp(obj_name, emp[i]))
        ERR;
    }
  ...
  printf("*** Checking HDF5 attribute deletes...");
  {
    ...
    if ((typeid = H5Tcopy(H5T_C_S1)) < 0)
      ERR;
    if (H5Tset_size(typeid, strlen(txt) + 1) < 0)
      ERR;
    if ((attid = H5Acreate(grpid, ATT1_NAME, typeid, spaceid, H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid, typeid, txt) < 0)
      ERR;

    /* Delete the attribute. */
    if (H5Aclose(attid) < 0)
      ERR;
    if (H5Adelete(grpid, ATT1_NAME) < 0)
      ERR;
    ...
    /* Close everything. */
    if (H5Aclose(attid) < 0 || H5Sclose(spaceid) < 0 || H5Tclose(typeid) < 0 ||
  ...
  printf("*** Checking HDF5 attributes in a group...");
  {
    ...
    if ((grpid = H5Gopen(fileid, GRP_NAME)) < 0)
      ERR;
    if ((attid = H5Aopen_name(grpid, ATT1_NAME)) < 0)
      ERR;
    if ((typeid = H5Aget_type(attid)) < 0)
      ERR;

    /* Check the size of the string in the attribute. */
    if (H5Tget_class(typeid) != H5T_STRING)
      ERR;
    if (!(txt_size = H5Tget_size(typeid)))
      ERR;
    if (txt_size != strlen(txt) + 1)
      ERR;

    /* Now read the attribute. But if I don't malloc the memory first,
     * I get zapped with a seg-fault. Aren't strings supposed to be
     * different? */
    if (!(txt_in = malloc(txt_size + 1)))
      ERR;
    if (H5Aread(attid, typeid, txt_in) < 0)
      ERR;
    if (strcmp(txt_in, txt))
      ERR;
    if (strlen(txt_in) != strlen(txt))
      ERR;
    ...
    if ((spaceid = H5Aget_space(attid)) < 0)
      ERR;
    if ((size = H5Sget_simple_extent_npoints(spaceid)) < 0)
      ERR;
    if (size != 1)
      ERR;
    ...
    if (ndims != 0)
      ERR;
    if (!(type_size = H5Tget_size(typeid)))
      ERR;
    if (type_size != strlen(txt) + 1)
      ERR;
    free(txt_in);
...

  {
    ...
    if ((typeid = H5Tcopy(H5T_C_S1)) < 0)
      ERR;
    if (H5Tset_size(typeid, 1) < 0)
      ERR;
    if ((attid1 = H5Acreate(grpid, ATT2_NAME, typeid, spaceid, H5P_DEFAULT)) <
        0)
      ERR;
    if (H5Awrite(attid1, H5T_NATIVE_FLOAT, &val) < 0)
      ERR;
    if (H5Sclose(spaceid) < 0 || H5Aclose(attid) < 0 || H5Aclose(attid1) < 0 ||
        H5Tclose(typeid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((grpid = H5Gopen(fileid, GRP_NAME)) < 0)
      ERR;
    if ((attid = H5Aopen_name(grpid, ATT2_NAME)) < 0)
      ERR;
    if ((spaceid = H5Aget_space(attid)) < 0)
      ERR;
    if ((size = H5Sget_simple_extent_npoints(spaceid)) < 0)
    ...
    if (ndims != 0)
      ERR;
    if (H5Sclose(spaceid) < 0 || H5Aclose(attid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
  }

  SUMMARIZE_ERR;
  printf("*** Checking how HDF5 handles a delete and recreation of dataset "
         "with atts...");
...
#define MAX_SPEECH_LEN 4095
  {
    ...
    char speech[NUM_SPEECHES][MAX_SPEECH_LEN + 1] = {
        ...
        "Speak the speech, I pray you, as I pronounc'd it to you, trippingly "
        "on\n"
        ...
        "offends me to the soul to hear a robustious periwig-pated fellow "
        "tear\n"
    ...
    char speech_name[NUM_SPEECHES][MAX_LEN + 1] = {
        "Act_1_Scene_2", "Act_3_Scene_2", "Act_5_Scene_2"};
    char obj_name[MAX_LEN + 1];
    int i;
    ...
    /* Create a dataset. */
    dims[0] = DIM1_LEN;
    if ((var_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    if ((datasetid = H5Dcreate(grpid, VAR_NAME, H5T_NATIVE_HBOOL, var_spaceid,
                               H5P_DEFAULT)) < 0)
      ERR;
    ...
    for (i = 0; i < NUM_SPEECHES; i++) {
      if (H5Tset_size(typeid, strlen(speech[i]) + 1) < 0)
        ERR;
      if ((attid = H5Acreate(datasetid, speech_name[i], typeid, spaceid,
                             H5P_DEFAULT)) < 0)
        ERR;
      if (H5Awrite(attid, typeid, speech[i]) < 0)
        ERR;
      if (H5Aclose(attid) < 0)
        ERR;
    }

    if (H5Dclose(datasetid) < 0 || H5Sclose(var_spaceid) < 0 ||
        H5Sclose(spaceid) < 0 || H5Tclose(typeid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((grpid = H5Gopen(fileid, "/")) < 0)
      ERR;
    if ((datasetid = H5Dopen1(grpid, VAR_NAME)) < 0)
      ERR;

    /* How many attributes are there? */
    if ((num_obj = H5Aget_num_attrs(datasetid)) != NUM_SPEECHES)
      ERR;

    /* Make sure the names are in the correct order. */
    for (i = 0; i < num_obj; i++) {
      if ((attid = H5Aopen_idx(datasetid, (unsigned int)i)) < 0)
        ERR;
      if (H5Aget_name(attid, MAX_LEN + 1, obj_name) < 0)
        ERR;
      if (H5Aclose(attid) < 0)
        ERR;
      if (strcmp(obj_name, speech_name[i]))
        ERR;
    }
  ...
  printf("*** Checking some more simple atts...");
  {
    ...
    if ((hdfid = H5Fcreate(FILE_NAME, H5F_ACC_TRUNC, fcpl_id, fapl_id)) < 0)
      ERR;
    ...
    if ((spaceid = H5Screate(H5S_SCALAR)) < 0)
      ERR;
    if ((attid = H5Acreate2(grpid, NC3_STRICT_ATT_NAME, H5T_NATIVE_INT, spaceid,
    ...
    if ((spaceid = H5Screate_simple(1, dims, NULL)) < 0)
      ERR;
    if ((attid1 = H5Acreate2(grpid, "l", H5T_NATIVE_INT, spaceid, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid1, H5T_NATIVE_INT, &one) < 0)
      ERR;
    if (H5Aclose(attid1) < 0)
      ERR;
    if ((attid1 = H5Acreate2(grpid, "y", H5T_NATIVE_INT, spaceid, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid1, H5T_NATIVE_INT, &one) < 0)
      ERR;
    if (H5Aclose(attid1) < 0)
      ERR;
    if ((attid1 = H5Acreate2(grpid, "c", H5T_NATIVE_INT, spaceid, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid1, H5T_NATIVE_INT, &one) < 0)
      ERR;
    ...
    if ((hdfid = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, fapl_id)) < 0)
      ERR;
    if ((grpid = H5Gopen2(hdfid, "/", H5P_DEFAULT)) < 0)
  ...
  printf("*** Checking some more simple atts even more...");
  {
    ...
    if ((spaceid = H5Screate(H5S_SCALAR)) < 0)
      ERR;
    if ((attid = H5Acreate(grpid, NC3_STRICT_ATT_NAME, H5T_NATIVE_INT, spaceid,
                           H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid, H5T_NATIVE_INT, &one) < 0)
      ERR;
    ...
    if ((spaceid = H5Screate_simple(1, dims, NULL)) < 0)
      ERR;
    if ((attid1 = H5Acreate2(grpid, "z", H5T_NATIVE_INT, spaceid, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid1, H5T_NATIVE_INT, &one) < 0)
      ERR;
...
#define DIM2_LEN 2
    ...
    dims[0] = DIM2_LEN;
    if ((att_spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    if ((attid = H5Acreate(grpid, att_name[0], H5T_NATIVE_UCHAR, att_spaceid,
                           H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid, H5T_NATIVE_UCHAR, b) < 0)
      ERR;
    if (H5Aclose(attid) < 0)
      ERR;
    if ((attid = H5Acreate(grpid, att_name[1], H5T_NATIVE_UCHAR, att_spaceid,
                           H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid, H5T_NATIVE_UCHAR, b) < 0)
      ERR;
    ...
    if ((spaceid = H5Screate_simple(1, dims, dims)) < 0)
      ERR;
    if ((dimscaleid =
             H5Dcreate(grpid, "D1", H5T_IEEE_F32BE, spaceid, H5P_DEFAULT)) < 0)
      ERR;

    /* Indicate that this is a scale. */
    if (H5DSset_scale(dimscaleid, NULL) < 0)
      ERR;

    /* Add another attribute to the group. Call it "third". */
    if ((attid = H5Acreate(grpid, att_name[2], H5T_NATIVE_UCHAR, att_spaceid,
                           H5P_DEFAULT)) < 0)
      ERR;
    if (H5Awrite(attid, H5T_NATIVE_UCHAR, b) < 0)
      ERR;
    ...
    if (H5Dclose(dimscaleid) < 0 || H5Sclose(spaceid) < 0 ||
        H5Sclose(att_spaceid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
    ...
    if (H5Pset_libver_bounds(fapl_id, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST) < 0)
      ERR;
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDWR, fapl_id)) < 0)
      ERR;
    if ((grpid = H5Gopen(fileid, "/")) < 0)
      ERR;

    /* How many attributes are there? */
    if ((num_obj = H5Aget_num_attrs(grpid)) != 3)
      ERR;
  ...
  SUMMARIZE_ERR;
  printf("*** Checking HDF5 attribute ordering with 9 attributes...(skipping "
...
#define ATT_MAX_NAME 2
  {
    ...
    if ((att_spaceid = H5Screate(H5S_NULL)) < 0)
      ERR;

    for (i = 0; i < NUM_SIMPLE_ATTS; i++) {
      if ((attid = H5Acreate(grpid, name[i], H5T_NATIVE_INT, att_spaceid,
                             H5P_DEFAULT)) < 0)
        ERR;
      if (H5Aclose(attid) < 0)
        ERR;
    ...
    if (H5Sclose(att_spaceid) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
    ...
    /* How many attributes are there? */
    if ((num_obj = H5Aget_num_attrs(grpid)) != NUM_SIMPLE_ATTS)
      ERR;
