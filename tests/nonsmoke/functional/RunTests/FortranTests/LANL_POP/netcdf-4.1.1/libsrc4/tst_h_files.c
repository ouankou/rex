...
#define DIM_LEN 3
  {
    ...
    if ((access_plist = H5Pcreate(H5P_FILE_ACCESS)) < 0)
      ERR;
    if (H5Pset_fclose_degree(access_plist, H5F_CLOSE_SEMI))
      ERR;
    ...
    if ((fileid = H5Fcreate(FILE_NAME, H5F_ACC_TRUNC, H5P_DEFAULT,
      ...
      ERR;
    /* Add an opaque type. */
    if ((typeid = H5Tcreate(H5T_OPAQUE, OPAQUE_SIZE)) < 0)
      ERR;
    if (H5Tcommit(fileid, OPAQUE_NAME, typeid) < 0)
      ERR;
    ...
    if ((spaceid = H5Screate_simple(1, dims, NULL)) < 0)
      ERR;
    if ((attid = H5Acreate(fileid, ATT_NAME, typeid, spaceid, H5P_DEFAULT)) < 0)
      ERR;
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      if (H5Gget_objname_by_idx(grpid, i, obj_name, NC_MAX_NAME) < 0)
        ERR;
      if (obj_class != H5G_TYPE)
        ERR;
      if ((typeid = H5Topen(grpid, obj_name)) < 0)
        ERR;
    ...
    if (H5Pset_fclose_degree(fapl_id, H5F_CLOSE_SEMI))
      ERR;
    if ((fileid = H5Fopen(FILE_NAME, 0, fapl_id)) < 0)
      ERR;
    if (H5Fclose(fileid) < 0)
      ERR;
  }
  ...
  printf("*** Checking HDF5 file creates and opens some more...");
  {
    ...
    if ((objs = H5Fget_obj_count(fileid, H5F_OBJ_ALL)) < 0)
      ERR;
    if (objs != 2)
      ERR;
    if ((objs = H5Fget_obj_count(fileid, H5F_OBJ_GROUP)) < 0)
      ERR;
    if (objs != 1)
      ERR;
    ...
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
      ERR;
    if ((fileid2 = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
      ERR;
    if (H5Fclose(fileid) < 0)
      ERR;
    if (H5Fclose(fileid2) < 0)
      ERR;
    ...
    if ((fileid = H5Fopen(FILE_NAME, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
      ERR;
    if ((fileid2 = H5Fopen(FILE_NAME, H5F_ACC_RDWR, H5P_DEFAULT)) >= 0)
      ERR;
    if (H5Fclose(fileid) < 0)
      ERR;
...
#define MILLION 1000000
    ...
    /* Create dataset. */
    if ((datasetid = H5Dcreate1(fileid, VAR_NAME, H5T_NATIVE_INT, write_spaceid,
                                H5P_DEFAULT)) < 0)
      ERR;
    ...
    /* Close. */
    free(data);
    if (H5Dclose(datasetid) < 0 || H5Sclose(write_spaceid) < 0 ||
        H5Sclose(mem_spaceid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
  SUMMARIZE_ERR;
  ...
  printf("*** Creating HDF5 file in the canonical netCDF-4 way...");
  {
    ...
    /* Close up. */
    if (H5Pclose(fapl_id) < 0 || H5Pclose(fcpl_id) < 0 || H5Gclose(grpid) < 0 ||
        H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((fileid2 = H5Fopen(FILE_NAME, H5F_ACC_RDWR, fapl_id)) < 0)
      ERR;
...
#define VAR_NAME2 "var"
  ...
  printf("*** large file test for HDF5...");
  {
    ...
    if (H5Pset_attr_creation_order(plistid, H5P_CRT_ORDER_TRACKED |
      ...
      ERR;
    if ((dim1_dimscaleid =
             H5Dcreate(grpid, "dim1", H5T_IEEE_F32BE, spaceid, plistid)) < 0)
      ERR;
    if (H5Sclose(spaceid) < 0)
      ERR;
    ...
    if (H5DSset_scale(dim1_dimscaleid, dimscale_wo_var) < 0)
      ERR;
    ...
    if (H5Pset_attr_creation_order(plistid, H5P_CRT_ORDER_TRACKED |
      ...
      ERR;
    if ((dim2_dimscaleid =
             H5Dcreate(grpid, "dim2", H5T_IEEE_F32BE, spaceid, plistid)) < 0)
      ERR;
    if (H5Sclose(spaceid) < 0)
      ERR;
    if (H5Pclose(plistid) < 0)
    ...
    if (H5DSset_scale(dim2_dimscaleid, dimscale_wo_var) < 0)
      ERR;
    ...
    *(signed char *)fillp = NC_FILL_BYTE;
    if (H5Pset_fill_value(plistid, H5T_NATIVE_SCHAR, fillp) < 0)
      ERR;
    if (!(chunksize = malloc(NDIMS2 * sizeof(hsize_t))))
      ERR;
    chunksize[0] = 1024;
    chunksize[1] = 1048576;
    if (H5Pset_chunk(plistid, NDIMS2, chunksize) < 0)
      ERR;
    if (!(dimsize = malloc(NDIMS2 * sizeof(hsize_t))))
      ERR;
    if (!(maxdimsize = malloc(NDIMS2 * sizeof(hsize_t))))
      ERR;
    dimsize[0] = 2048;
    dimsize[1] = 2097153;
    maxdimsize[0] = 2048;
    maxdimsize[1] = 2097153;
    if ((spaceid = H5Screate_simple(NDIMS2, dimsize, maxdimsize)) < 0)
      ERR;
    if (H5Pset_attr_creation_order(plistid, H5P_CRT_ORDER_TRACKED |
                                                H5P_CRT_ORDER_INDEXED) < 0)
      ERR;
    if ((datasetid = H5Dcreate(grpid, VAR_NAME2, H5T_NATIVE_SCHAR, spaceid,
                               plistid)) < 0)
    ...
    if (H5DSattach_scale(datasetid, dim1_dimscaleid, 0) < 0)
      ERR;
    if (H5DSattach_scale(datasetid, dim2_dimscaleid, 1) < 0)
      ERR;
    ...
    if (H5Sselect_hyperslab(file_spaceid, H5S_SELECT_SET, start, NULL, count,
                            NULL) < 0)
      ERR;
    if ((mem_spaceid = H5Screate_simple(NDIMS2, count, NULL)) < 0)
      ERR;
    if ((xfer_plistid = H5Pcreate(H5P_DATASET_XFER)) < 0)
      ERR;
    if ((native_typeid =
             H5Tget_native_type(H5T_NATIVE_SCHAR, H5T_DIR_DEFAULT)) < 0)
      ERR;
    if (!(bufr = malloc(DIM2)))
      ERR;
    ...
    if (H5Pclose(fcpl_id) < 0)
      ERR;
    if (H5Dclose(dim1_dimscaleid) < 0)
      ERR;
    if (H5Dclose(dim2_dimscaleid) < 0)
      ERR;
