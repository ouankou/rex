...
static int nc_check_for_hdf(const char *path, int use_parallel, MPI_Comm comm,
                            MPI_Info info, int *hdf_file) {
 ...
 * for parallel, use the MPI functions instead. */
  ...
  {
    FILE *fp;
    if (!(fp = fopen(path, "r")) || fread(blob, MAGIC_NUMBER_LEN, 1, fp) != 1)
      return errno;
    fclose(fp);
  }
...
static int nc4_create_file(const char *path, int cmode, MPI_Comm comm,
                           MPI_Info info, NC_FILE_INFO_T *nc) {
  hid_t fcpl_id, fapl_id;
  unsigned flags = (cmode & NC_NOCLOBBER) ? H5F_ACC_EXCL : H5F_ACC_TRUNC;
  FILE *fp;
  int retval = NC_NOERR;
  ...
  if (H5Pset_cache(fapl_id, 0, chunk_cache_nelems, chunk_cache_size,
    ...
    BAIL(NC_EHDFERR);
  LOG((4,
       "nc4_create_file: set HDF raw chunk cache to size %d nelems %d "
       "preemption %f",
  ...
  /* Open the root group. */
  if ((nc->nc4_info->root_grp->hdf_grpid =
           H5Gopen2(nc->nc4_info->hdfid, "/", H5P_DEFAULT)) < 0)
    BAIL(NC_EFILEMETA);
...
exit:
  if (nc->nc4_info->hdfid > 0)
    H5Fclose(nc->nc4_info->hdfid);
  return retval;
}

EXPORT4 int RENAME4(nc_create_file)(const char *path, int cmode,
                                    size_t initialsz, int basepe,
                                    size_t *chunksizehintp, MPI_Comm comm,
                                    MPI_Info info, int *ncidp) {
  int res;

  assert(ncidp);
  assert(path);
  LOG((1, "nc_create_file: path %s cmode 0x%x comm %d info %d", path, cmode,
       comm, info));
  ...
  if (cmode & NC_NETCDF4) {
    ...
    res = nc3__create_mp(path, cmode, initialsz, basepe, chunksizehintp,
                         &(nc_file->int_ncid));
  }
...
int nc__create(const char *path, int cmode, size_t initialsz,
  ...
  return nc_create_file(path, cmode, initialsz, 0, chunksizehintp, 0, 0, ncidp);
}

int nc__create_mp(const char *path, int cmode, size_t initialsz, int basepe,
                  size_t *chunksizehintp, int *ncidp) {
  return nc_create_file(path, cmode, initialsz, basepe, chunksizehintp, 0, 0,
                        ncidp);
}

/*#ifdef USE_PARALLEL*/
int nc_create_par(const char *path, int cmode, MPI_Comm comm, MPI_Info info,
                  int *ncidp) {
...
int nc_create_par_fortran(const char *path, int cmode, int comm, int info,
                          int *ncidp) {
  MPI_Comm comm_c = 0;
  MPI_Info info_c = 0;
#ifdef USE_PARALLEL
...
static int read_scale(NC_GRP_INFO_T *grp, hid_t datasetid, char *obj_name,
  ...
  if (H5DSget_scale_name(datasetid, dimscale_name_att, NC_MAX_NAME) >= 0) {
    if (!strncmp(dimscale_name_att, DIM_WITHOUT_VARIABLE,
      ...
      /*          else */
      /*          { */
      /*             start_of_len = dimscale_name_att +
       * strlen(DIM_WITHOUT_VARIABLE); */
      /* #if (SIZEOF_SIZE_T < 8) */
...
static int read_coord_dimids(NC_VAR_INFO_T *var) {
  ...
  if ((coord_attid = H5Aopen_name(var->hdf_datasetid, COORDINATES)) < 0)
    ret++;
  if (!ret && (coord_att_typeid = H5Aget_type(coord_attid)) < 0)
    ret++;
  if (!ret && H5Aread(coord_attid, coord_att_typeid, var->dimids) < 0)
    ret++;
  ...
  num_spaces++;
#endif
  if ((coord_array_size = H5Sget_simple_extent_npoints(spaceid)) < 0)
    ret++;
...
static herr_t dimscale_visitor(hid_t did, unsigned dim, hid_t dsid,
...
static int get_netcdf_type(NC_HDF5_FILE_INFO_T *h5, hid_t native_typeid,
                           nc_type *xtype) {
  NC_TYPE_INFO_T *type;
  hid_t class;
 ...
 * one. */
static int get_type_info2(NC_HDF5_FILE_INFO_T *h5, hid_t datasetid,
  ...
  H5T_order_t order;
  int endianness;
  nc_type nc_type_constant[NUM_TYPES] = {
      NC_CHAR,  NC_BYTE,   NC_SHORT, NC_INT,   NC_FLOAT,  NC_DOUBLE,
      NC_UBYTE, NC_USHORT, NC_UINT,  NC_INT64, NC_UINT64, NC_STRING};
  char type_name[NUM_TYPES][NC_MAX_NAME + 1] = {
      "char",  "byte",   "short", "int",   "float",  "double",
      "ubyte", "ushort", "uint",  "int64", "uint64", "string"};
  int type_size[NUM_TYPES] = {sizeof(char),
                              sizeof(char),
                              sizeof(short),
                              sizeof(int),
                              sizeof(float),
...
static int read_hdf5_att(NC_GRP_INFO_T *grp, hid_t attid, NC_ATT_INFO_T *att) {
  ...
  LOG((5,
       ...
       att->attnum, att->name, att->xtype, att->len));

  /* Get type of attribute in file. */
  if ((file_typeid = H5Aget_type(attid)) < 0)
    return NC_EATTMETA;
  if ((att->native_typeid = H5Tget_native_type(file_typeid, H5T_DIR_DEFAULT)) <
      0)
    BAIL(NC_EHDFERR);
  ...
  if ((retval = get_netcdf_type(grp->file->nc4_info, att->native_typeid,
                                &(att->xtype))))
    BAIL(retval);
...
static int read_type(NC_GRP_INFO_T *grp, char *type_name) {
  NC_TYPE_INFO_T *type;
  H5T_class_t class;
  ...
  if (ud_type_type == NC_COMPOUND) {
    ...
    for (m = 0; m < nmembers; m++) {
      ...
      if ((member_hdf_typeid = H5Tget_member_type(type->native_typeid, m)) < 0)
        return NC_EHDFERR;
      if ((member_native_typeid =
               H5Tget_native_type(member_hdf_typeid, H5T_DIR_DEFAULT)) < 0)
        return NC_EHDFERR;
      ...
      if (mem_class == H5T_ARRAY) {
        ...
        if ((retval = get_netcdf_type(grp->file->nc4_info,
                                      H5Tget_super(member_hdf_typeid),
                                      &member_xtype)))
          return retval;
      } else {
        /* What is the netCDF typeid of this member? */
      ...
      if (ndims) {
        ...
        if ((retval = nc4_field_list_add(
                 &type->field, type->num_fields++, member_name, member_offset,
                 member_hdf_typeid, member_native_typeid, member_xtype, 0,
                 NULL)))
          return retval;
      }
...
static int read_var(NC_GRP_INFO_T *grp, hid_t datasetid, char *obj_name,
...
#define CD_NELEMS_SZIP 4
  ...
  for (f = 0; f < num_filters; f++) {
    if ((filter = H5Pget_filter2(propid, f, NULL, &cd_nelems, cd_values, 0,
                                 NULL, NULL)) < 0)
      BAIL(NC_EHDFERR);
    switch (filter) {
  ...
  if ((retval = get_type_info2(grp->file->nc4_info, datasetid, &var->xtype,
                               &var->type_info)))
    BAIL(retval);
  ...
  if (!is_scale && num_scales) {
    ...
    if (!(var->dimscale_hdf5_objids =
              malloc(ndims * sizeof(struct hdf5_objid))))
      BAIL(NC_ENOMEM);
    for (d = 0; d < var->ndims; d++) {
  ...
  for (a = 0; a < natts; a++) {
    ...
    if (strcmp(att_name, REFERENCE_LIST) && strcmp(att_name, CLASS) &&
        strcmp(att_name, DIMENSION_LIST) && strcmp(att_name, NAME) &&
...
static int read_dataset(NC_GRP_INFO_T *grp, char *obj_name) {
  hid_t datasetid = 0;
  ...
  if (is_scale) {
    ...
    if ((retval = read_scale(grp, datasetid, obj_name, dims[0], max_dims[0],
                             &dim_without_var)))
      BAIL(retval);
  } else {
  ...
  if (!dim_without_var)
    if ((retval = read_var(grp, datasetid, obj_name, ndims, is_scale,
                           num_scales, access_pid)))
      BAIL(retval);
...
static int get_name_by_idx(NC_HDF5_FILE_INFO_T *h5, hid_t hdf_grpid, int i,
                           int *obj_class, char *obj_name) {
  H5O_info_t obj_info;
  H5_index_t idx_field = H5_INDEX_CRT_ORDER;
  ssize_t size;

  if (H5Oget_info_by_idx(hdf_grpid, ".", H5_INDEX_CRT_ORDER, H5_ITER_INC, i,
                         &obj_info, H5P_DEFAULT) < 0) {
    if (H5Oget_info_by_idx(hdf_grpid, ".", H5_INDEX_NAME, H5_ITER_INC, i,
                           &obj_info, H5P_DEFAULT) < 0)
      return NC_EHDFERR;
    if (!h5->no_write)
  ...
  *obj_class = obj_info.type;
  ...
  if (size > NC_MAX_NAME)
    return NC_EMAXNAME;
  if (H5Lget_name_by_idx(hdf_grpid, ".", idx_field, H5_ITER_INC, i, obj_name,
                         size + 1, H5P_DEFAULT) < 0)
    return NC_EHDFERR;
...
int nc4_rec_read_types(NC_GRP_INFO_T *grp) {
  hsize_t num_obj, i;
  int obj_class;
  char obj_name[NC_MAX_HDF5_NAME + 1];
  ...
  if (!grp->hdf_grpid) {
    if (grp->parent) {
      if ((grp->hdf_grpid =
               H5Gopen2(grp->parent->hdf_grpid, grp->name, H5P_DEFAULT)) < 0)
        return NC_EHDFERR;
    } else {
      if ((grp->hdf_grpid =
  ...
  for (i = 0; i < num_obj; i++) {
    ...
    if (obj_class == H5O_TYPE_GROUP) {
      ...
      if ((retval = nc4_grp_list_add(&(grp->children), h5->next_nc_grpid++, grp,
                                     grp->file, obj_name, &child_grp)))
        return retval;
...
/* This function recursively reads all the var and attribute metadata
   ...
   metadata structure. */
int nc4_rec_read_vars(NC_GRP_INFO_T *grp) {
  hsize_t num_obj, i;
  int obj_class;
  char obj_name[NC_MAX_HDF5_NAME + 1];
  NC_HDF5_FILE_INFO_T *h5 = grp->file->nc4_info;
  ...
  for (i = 0; i < num_obj; i++) {
    ...
    case H5G_LINK:
      ...
      break;
    default:
      LOG((0, "Unknown object class %d in nc4_rec_read_vars!", obj_class));
    }
  }
 ...
 * is handled. */
static int nc4_open_file(const char *path, int mode, MPI_Comm comm,
                         MPI_Info info, NC_FILE_INFO_T *nc) {
  hid_t fapl_id = H5P_DEFAULT;
  unsigned flags = (mode & NC_WRITE) ? H5F_ACC_RDWR : H5F_ACC_RDONLY;
  int retval;
  ...
  if (H5Pset_cache(fapl_id, 0, chunk_cache_nelems, chunk_cache_size,
    ...
    BAIL(NC_EHDFERR);
  LOG((4,
       "nc4_open_file: set HDF raw chunk cache to size %d nelems %d preemption "
       "%f",
       chunk_cache_size, chunk_cache_nelems, chunk_cache_preemption));
...
#ifdef USE_HDF4
static int get_netcdf_type_from_hdf4(NC_HDF5_FILE_INFO_T *h5, int32 hdf4_typeid,
                                     nc_type *xtype) {
...
static int nc4_open_hdf4_file(const char *path, int mode, NC_FILE_INFO_T *nc) {
#ifdef USE_HDF4
  ...
  for (a = 0; a < num_gatts; a++) {
    ...
    if (SDattrinfo(h5->sdid, a, att->name, &att_data_type, &att_count))
      return NC_EATTMETA;
    if ((retval = get_netcdf_type_from_hdf4(h5, att_data_type, &att->xtype)))
      return retval;
    att->len = att_count;
  ...
  for (v = 0; v < num_datasets; v++) {
    ...
    if ((retval = get_netcdf_type_from_hdf4(h5, data_type, &var->xtype)))
      return retval;
    LOG((3, "reading HDF4 dataset %s, rank %d netCDF type %d", var->name, rank,
         var->xtype));
    ...
    for (d = 0; d < var->ndims; d++) {
      ...
      if (!dim) {
        LOG((4, "adding dimension %s for HDF4 dataset %s", dim_name,
             var->name));
        if ((retval = nc4_dim_list_add(&grp->dim)))
          return retval;
        grp->ndims++;
...
EXPORT4 int RENAME4(nc_open_file)(const char *path, int mode, int basepe,
  ...
  assert(path && ncidp);
  LOG((1, "nc_open_file: path %s mode %d comm %d info %d", path, mode, comm,
       info));
   ...
   * selected at once. */
  if (mode & ~(NC_WRITE | NC_SHARE | NC_MPIIO | NC_MPIPOSIX | NC_PNETCDF |
               NC_NOCLOBBER) ||
  ...
  {
    res =
        nc3__open_mp(path, mode, basepe, chunksizehintp, &(nc_file->int_ncid));
  }
...
int nc__open_mp(const char *path, int mode, int basepe, size_t *chunksizehintp,
                int *ncidp) {
  return nc_open_file(path, mode, basepe, chunksizehintp, 0, 0, 0, ncidp);
}

/*#ifdef USE_PARALLEL*/
int nc_open_par(const char *path, int mode, MPI_Comm comm, MPI_Info info,
                int *ncidp) {
  /* Must use either MPIIO or MPIPOSIX. Default to the former. Also
   * default to use NC_PNETCDF if necessary and available.  */
...
int nc_open_par_fortran(const char *path, int mode, int comm, int info,
                        int *ncidp) {
  MPI_Comm comm_c = 0;
  MPI_Info info_c = 0;
#ifdef USE_PARALLEL
...
static int close_netcdf4_file(NC_HDF5_FILE_INFO_T *h5, int abort) {
  ...
  LOG((3, "close_netcdf4_file: h5->path %s abort %d", h5->path, abort));
...
#ifdef USE_DAP
short drno_delta_file_id(short delta) {
  last_file_id += delta;
  return last_file_id;
}

int drno_delta_numfiles(short delta) {
  numfiles += delta;
  return numfiles;
}
