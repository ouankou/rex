...
extern void NF_STRERROR(char *AS, unsigned D0, const int *A1) {
  ...
  memcpy(AS, A0,
         (D0 < (A0 == ((void *)0) ? 0 : strlen(A0))
              ? D0
              : (A0 == ((void *)0) ? 0 : strlen(A0))));
  D0 > (A0 == ((void *)0) ? 0 : strlen(A0))
      ? memset(AS + (A0 == ((void *)0) ? 0 : strlen(A0)), ' ',
               D0 - (A0 == ((void *)0) ? 0 : strlen(A0)))
      : 0;
  return;
}
#else
FCALLSCFUN1(STRING, (char *)nc_strerror, NF_STRERROR, nf_strerror, FINT2CINT)
#endif

static int nc_issyserr(int errcode) { return errcode > 0; }

/*
 * Indicate whether or not an error-code refers to a system error
 * rather than a netCDF error
 */
FCALLSCFUN1(LOGICAL, nc_issyserr, NF_ISSYSERR, nf_issyserr, FINT2CINT)
