 ...
 */
NF_INTEGER *c2f_dimids(int ncid, int varid, const int *cdimids,
                       NF_INTEGER *fdimids) {
  int i;
  int ndims;
...
/* Convert a C dimension-ID vector into a FORTRAN dimension-ID vector. */
NF_INTEGER *c2f_chunksizes(int ncid, int varid, const int *cchunksizes,
...
/* Convert a FORTRAN dimension-ID vector into a C dimension-ID vector. */
int *f2c_chunksizes(int ncid, int varid, const NF_INTEGER *fchunksizes,
                    int *cchunksizes) {
  int i;
  int ndims;
 ...
 */
size_t *f2c_coords(int ncid, int varid, const NF_INTEGER *fcoords,
                   size_t *ccoords) {
  int i;
  int ndims;
 ...
 */
size_t *f2c_counts(int ncid, int varid, const NF_INTEGER *fcounts,
                   size_t *ccounts) {
  int i;
  int ndims;
 ...
 * Helper function.
 */
ptrdiff_t *f2c_strides(int ncid, int varid, const NF_INTEGER *fstrides,
                       ptrdiff_t *cstrides) {
  int i;
  int ndims;
 ...
 */
ptrdiff_t *f2c_maps(int ncid, int varid, const NF_INTEGER *fmaps,
                    ptrdiff_t *cmaps) {
...
int nc_inq_compound_field_f(int ncid, nc_type xtype, int fieldid, char *name,
  ...
  if ((ret = nc_inq_compound_field(ncid, xtype, fieldid, NULL, NULL, NULL,
                                   &ndims, NULL)))
    return ret;
