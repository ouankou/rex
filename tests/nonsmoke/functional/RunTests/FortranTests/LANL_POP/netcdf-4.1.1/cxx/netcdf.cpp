...
NcFile::~NcFile(void) { (void)close(); }

NcBool NcFile::is_valid(void) const { return the_id != ncBad; }

int NcFile::num_dims(void) const {
  int num = 0;
  if (is_valid())
    NcError::set_err(nc_inq_ndims(the_id, &num));
  return num;
}
...
int NcFile::num_vars(void) const {
  int num = 0;
  if (is_valid())
    NcError::set_err(nc_inq_nvars(the_id, &num));
  return num;
}
...
int NcFile::num_atts(void) const {
  int num = 0;
  if (is_valid())
    NcError::set_err(nc_inq_natts(the_id, &num));
  return num;
}

NcDim *NcFile::get_dim(NcToken name) const {
  int dimid;
  if (NcError::set_err(nc_inq_dimid(the_id, name, &dimid)) != NC_NOERR)
    return 0;
...
NcVar *NcFile::get_var(NcToken name) const {
  int varid;
  if (NcError::set_err(nc_inq_varid(the_id, name, &varid)) != NC_NOERR)
    return 0;
...
NcDim *NcFile::rec_dim() const {
  if (!is_valid())
    return 0;
  int recdim;
  if (NcError::set_err(nc_inq_unlimdim(the_id, &recdim)) != NC_NOERR)
    return 0;
...
NcDim *NcFile::add_dim(NcToken name) { return add_dim(name, NC_UNLIMITED); }
...
NcVar *NcFile::add_var(NcToken name, NcType type, // scalar to 5D var
                       const NcDim *dim0, const NcDim *dim1, const NcDim *dim2,
                       const NcDim *dim3, const NcDim *dim4) {
  if (!is_valid() || !define_mode())
    return 0;
  int dims[5];
  int ndims = 0;
  ...
  int n = num_vars();
  int varid;
  if (NcError::set_err(nc_def_var(the_id, name, (nc_type)type, ndims, dims,
                                  &varid)) != NC_NOERR)
    return 0;
...
NcVar *NcFile::add_var(NcToken name, NcType type, int ndims,
  ...
  int n = num_vars();
  int varid;
  if (NcError::set_err(nc_def_var(the_id, name, (nc_type)type, ndims, dimids,
                                  &varid)) != NC_NOERR)
    return 0;
  NcVar *varp = new NcVar(this, varid);
  variables[n] = varp;
...
NcFile_add_scalar_att(char) NcFile_add_scalar_att(ncbyte)
    NcFile_add_scalar_att(short) NcFile_add_scalar_att(int)
        NcFile_add_scalar_att(long) NcFile_add_scalar_att(float)
            NcFile_add_scalar_att(double) NcFile_add_scalar_att(const char *)
...
#define NcFile_add_vector_att(TYPE)                                            \
  NcBool NcFile::add_att(NcToken aname, int n, const TYPE *val) {              \
    return globalv->add_att(aname, n, val);                                    \
                ...
                NcFile_add_vector_att(char) NcFile_add_vector_att(ncbyte)
                    NcFile_add_vector_att(short) NcFile_add_vector_att(int)
                        NcFile_add_vector_att(long) NcFile_add_vector_att(float)
                            NcFile_add_vector_att(double)

                                NcBool NcFile::set_fill(FillMode a_mode) {
  int prev_mode;
  if (NcError::set_err(nc_set_fill(the_id, a_mode, &prev_mode)) == NC_NOERR) {
...
NcFile::FillMode NcFile::get_fill(void) const { return the_fill_mode; }

NcFile::FileFormat NcFile::get_format(void) const {
  int the_format;
  NcError::set_err(nc_inq_format(the_id, &the_format));
  switch (the_format) {
...
NcBool NcFile::sync(void) {
  if (!data_mode())
    return 0;
  if (NcError::set_err(nc_sync(the_id)) != NC_NOERR)
    return 0;
  int i;
...
NcBool NcFile::close(void) {
  ...
  int old_id = the_id;
  the_id = ncBad;
  return NcError::set_err(nc_close(old_id)) == NC_NOERR;
}

NcBool NcFile::abort(void) {
  return NcError::set_err(nc_abort(the_id)) == NC_NOERR;
}
...
int NcFile::id(void) const { return the_id; }

NcFile::NcFile(const char *path, FileMode fmode, size_t *bufrsizeptr,
  ...
  case ReadOnly:
    ...
    status = NcError::set_err(nc__open(path, mode, bufrsizeptr, &the_id));
    if (status != NC_NOERR) {
...
NcToken NcDim::name(void) const { return the_name; }

long NcDim::size(void) const {
  size_t sz = 0;
  if (the_file)
    NcError::set_err(nc_inq_dimlen(the_file->id(), the_id, &sz));
  return sz;
}
...
NcBool NcDim::is_unlimited(void) const {
  if (!the_file)
    return FALSE;
  int recdim;
  NcError::set_err(nc_inq_unlimdim(the_file->id(), &recdim));
...
NcBool NcDim::rename(NcToken newname) {
  ...
  NcBool ret = NcError::set_err(
                   nc_rename_dim(the_file->id(), the_id, newname)) == NC_NOERR;
  if (ret) {
    delete[] the_name;
...
int NcDim::id(void) const { return the_id; }
...
NcBool NcDim::sync(void) {
  ...
  if (the_file && NcError::set_err(nc_inq_dimname(the_file->id(), the_id,
                                                  nam)) == NC_NOERR) {
...
NcDim::NcDim(NcFile *nc, int id) : the_file(nc), the_id(id) {
  char nam[NC_MAX_NAME];
  if (the_file && NcError::set_err(nc_inq_dimname(the_file->id(), the_id,
                                                  nam)) == NC_NOERR) {
    the_name = new char[strlen(nam) + 1];
...
NcDim::NcDim(NcFile *nc, NcToken name, long sz) : the_file(nc) {
  size_t dimlen = sz;
  if (NcError::set_err(nc_def_dim(the_file->id(), name, dimlen, &the_id)) ==
      NC_NOERR) {
...
NcDim::~NcDim(void) { delete[] the_name; }
...
NcTypedComponent_as(ncbyte) NcTypedComponent_as(char) NcTypedComponent_as(short)
    NcTypedComponent_as(int) NcTypedComponent_as(nclong)
        NcTypedComponent_as(long) NcTypedComponent_as(float)
...
NcTypedComponent::NcTypedComponent(NcFile *nc) : the_file(nc) {}
...
NcToken NcVar::name(void) const { return the_name; }

NcType NcVar::type(void) const {
  nc_type typ;
  NcError::set_err(nc_inq_vartype(the_file->id(), the_id, &typ));
  return (NcType)typ;
}
...
int NcVar::num_dims(void) const {
  int ndim;
  NcError::set_err(nc_inq_varndims(the_file->id(), the_id, &ndim));
  return ndim;
}
...
NcDim *NcVar::get_dim(int i) const {
  ...
  if (NcError::set_err(nc_inq_var(the_file->id(), the_id, 0, 0, &ndim, dims,
                                  0)) != NC_NOERR ||
      i < 0 || i >= ndim)
    return 0;
...
NcValues *NcVar::values(void) const {
  ...
  switch (type()) {
  case ncFloat:
    status = NcError::set_err(nc_get_vara_float(the_file->id(), the_id, crnr,
                                                edgs, (float *)valp->base()));
    break;
  case ncDouble:
    status = NcError::set_err(nc_get_vara_double(the_file->id(), the_id, crnr,
                                                 edgs, (double *)valp->base()));
    break;
  case ncInt:
    status = NcError::set_err(nc_get_vara_int(the_file->id(), the_id, crnr,
                                              edgs, (int *)valp->base()));
    break;
  case ncShort:
    status = NcError::set_err(nc_get_vara_short(the_file->id(), the_id, crnr,
                                                edgs, (short *)valp->base()));
    break;
  case ncByte:
    status = NcError::set_err(nc_get_vara_schar(
        the_file->id(), the_id, crnr, edgs, (signed char *)valp->base()));
    break;
  case ncChar:
    status = NcError::set_err(nc_get_vara_text(the_file->id(), the_id, crnr,
                                               edgs, (char *)valp->base()));
    break;
  case ncNoType:
  default:
...
NcValues *NcVar::get_rec(void) { return get_rec(get_dim(0), cur_rec[0]); }

NcValues *NcVar::get_rec(long rec) { return get_rec(get_dim(0), rec); }
...
NcValues *NcVar::get_rec(NcDim *rdim, long slice) {
  ...
  switch (type()) {
  case ncFloat:
    status = NcError::set_err(nc_get_vara_float(the_file->id(), the_id, start,
                                                edge, (float *)valp->base()));
    break;
  case ncDouble:
    status = NcError::set_err(nc_get_vara_double(the_file->id(), the_id, start,
                                                 edge, (double *)valp->base()));
    break;
  case ncInt:
    status = NcError::set_err(nc_get_vara_int(the_file->id(), the_id, start,
                                              edge, (int *)valp->base()));
    break;
  case ncShort:
    status = NcError::set_err(nc_get_vara_short(the_file->id(), the_id, start,
                                                edge, (short *)valp->base()));
    break;
  case ncByte:
    status = NcError::set_err(nc_get_vara_schar(
        the_file->id(), the_id, start, edge, (signed char *)valp->base()));
    break;
  case ncChar:
    status = NcError::set_err(nc_get_vara_text(the_file->id(), the_id, start,
                                               edge, (char *)valp->base()));
    break;
  case ncNoType:
  default:
...
NcVar_put_rec(ncbyte) NcVar_put_rec(char) NcVar_put_rec(short)
    NcVar_put_rec(int) NcVar_put_rec(long) NcVar_put_rec(float)
  ...
  long NcVar::get_index(NcDim *rdim, const TYPE *key) {                        \
    ...
    for (long j = 0; j < maxrec; j++) {                                        \
      ...
      for (validx = 0; validx < maxvals; validx++) {                           \
        if (key[validx] != val->as_##TYPE(validx))                             \
          break;                                                               \
      }                                                                        \
      delete val;                                                              \
      if (validx == maxvals)                                                   \
...
NcVar_get_index(ncbyte) NcVar_get_index(char) NcVar_get_index(short)
    NcVar_get_index(nclong) NcVar_get_index(long) NcVar_get_index(float)
...
#define NcVar_put_array(TYPE)                                                  \
  NcBool NcVar::put(const TYPE *vals, long edge0, long edge1, long edge2,      \
                    long edge3, long edge4) {                                  \
    ...
    return NcError::set_err(makename2(nc_put_vara_, TYPE)(                     \
               the_file->id(), the_id, start, count, vals)) == NC_NOERR;       \
  }

            NcBool NcVar::put(const ncbyte *vals, long edge0, long edge1,
                              long edge2, long edge3, long edge4) {
  /* no need to check type() vs. TYPE, invoked C function will do that */
  if (!the_file->data_mode())
    return FALSE;
  size_t count[5];
  ...
  return NcError::set_err(nc_put_vara_schar(the_file->id(), the_id, start,
                                            count, vals)) == NC_NOERR;
}

NcBool NcVar::put(const char *vals, long edge0, long edge1, long edge2,
                  long edge3, long edge4) {
  /* no need to check type() vs. TYPE, invoked C function will do that */
  if (!the_file->data_mode())
    return FALSE;
  ...
  return NcError::set_err(nc_put_vara_text(the_file->id(), the_id, start, count,
                                           vals)) == NC_NOERR;
}

NcVar_put_array(short) NcVar_put_array(int) NcVar_put_array(long)
    NcVar_put_array(float) NcVar_put_array(double)
...
#define NcVar_put_nd_array(TYPE)                                               \
  NcBool NcVar::put(const TYPE *vals, const long *count) {                     \
    ...
    for (int i = 0; i < num_dims(); i++)                                       \
      start[i] = the_cur[i];                                                   \
    return NcError::set_err(makename2(nc_put_vara_, TYPE)(                     \
               the_file->id(), the_id, start, (const size_t *)count, vals)) == \
           NC_NOERR;                                                           \
  }
...
NcBool NcVar::put(const char *vals, const long *count) {
  ...
  for (int i = 0; i < num_dims(); i++)
    start[i] = the_cur[i];
  return NcError::set_err(nc_put_vara_text(the_file->id(), the_id, start,
                                           (const size_t *)count, vals)) ==
         NC_NOERR;
}

NcVar_put_nd_array(short) NcVar_put_nd_array(int) NcVar_put_nd_array(long)
    NcVar_put_nd_array(float) NcVar_put_nd_array(double)

#define NcVar_get_array(TYPE)                                                  \
  NcBool NcVar::get(TYPE *vals, long edge0, long edge1, long edge2,            \
                    long edge3, long edge4) const {                            \
    if (!the_file->data_mode())                                                \
      return FALSE;                                                            \
    size_t count[5];                                                           \
    count[0] = edge0;                                                          \
    ...
    return NcError::set_err(makename2(nc_get_vara_, TYPE)(                     \
               the_file->id(), the_id, start, count, vals)) == NC_NOERR;       \
  }

        NcBool NcVar::get(ncbyte *vals, long edge0, long edge1, long edge2,
                          long edge3, long edge4) const {
  if (!the_file->data_mode())
    return FALSE;
  size_t count[5];
  ...
  return NcError::set_err(nc_get_vara_schar(the_file->id(), the_id, start,
                                            count, vals)) == NC_NOERR;
}

NcBool NcVar::get(char *vals, long edge0, long edge1, long edge2, long edge3,
                  long edge4) const {
  if (!the_file->data_mode())
    return FALSE;
  size_t count[5];
  ...
  return NcError::set_err(nc_get_vara_text(the_file->id(), the_id, start, count,
                                           vals)) == NC_NOERR;
}

NcVar_get_array(short) NcVar_get_array(int) NcVar_get_array(long)
    NcVar_get_array(float) NcVar_get_array(double)
...
#define NcVar_get_nd_array(TYPE)                                               \
  ...
  for (int i = 0; i < num_dims(); i++)
    start[i] = the_cur[i];
  return nc_get_vara_schar(the_file->id(), the_id, start, (const size_t *)count,
                           vals) == NC_NOERR;
}
...
NcBool NcVar::get(char *vals, const long *count) const {
  ...
  for (int i = 0; i < num_dims(); i++)
    start[i] = the_cur[i];
  return nc_get_vara_text(the_file->id(), the_id, start, (const size_t *)count,
                          vals) == NC_NOERR;
}

NcVar_get_nd_array(short) NcVar_get_nd_array(int) NcVar_get_nd_array(long)
    NcVar_get_nd_array(float) NcVar_get_nd_array(double)

    // If no args, set cursor to all zeros.	 Else set initial elements of
    // cursor to args provided, rest to zeros.
...
#define NcVar_add_scalar_att(TYPE)                                             \
  NcBool NcVar::add_att(NcToken aname, TYPE val) {                             \
    if (!the_file->define_mode())                                              \
      return FALSE;                                                            \
    if (NcError::set_err(makename2(nc_put_att_, TYPE)(                         \
            the_file->id(), the_id, aname, (nc_type)NcTypeEnum(TYPE), 1,       \
            &val)) != NC_NOERR)                                                \
      return FALSE;                                                            \
    return TRUE;                                                               \
  }
...
NcBool NcVar::add_att(NcToken aname, ncbyte val) {
  if (!the_file->define_mode())
    return FALSE;
  if (nc_put_att_schar(the_file->id(), the_id, aname,
                       (nc_type)NcTypeEnum(ncbyte), 1, &val) != NC_NOERR)
    return FALSE;
  return TRUE;
}
...
NcVar_add_scalar_att(short) NcVar_add_scalar_att(int) NcVar_add_scalar_att(long)
  ...
  if (nc_put_att_float(the_file->id(), the_id, aname, (nc_type)ncFloat, 1,
                       &fval) != NC_NOERR)
    return FALSE;
  return TRUE;
}
...
NcBool NcVar::add_att(NcToken aname, const char *val) {
  if (!the_file->define_mode())
    return FALSE;
  if (nc_put_att_text(the_file->id(), the_id, aname, strlen(val), val) !=
      NC_NOERR)
    return FALSE;
  return TRUE;
}
...
#define NcVar_add_vector_att(TYPE)                                             \
  NcBool NcVar::add_att(NcToken aname, int len, const TYPE *vals) {            \
    if (!the_file->define_mode())                                              \
      return FALSE;                                                            \
    if (NcError::set_err(makename2(nc_put_att_, TYPE)(                         \
            the_file->id(), the_id, aname, (nc_type)NcTypeEnum(TYPE), len,     \
            vals)) != NC_NOERR)                                                \
      return FALSE;                                                            \
    return TRUE;                                                               \
  }
...
NcBool NcVar::add_att(NcToken aname, int len, const ncbyte *vals) {
  if (!the_file->define_mode())
    return FALSE;
  if (NcError::set_err(nc_put_att_schar(the_file->id(), the_id, aname,
                                        (nc_type)NcTypeEnum(ncbyte), len,
                                        vals)) != NC_NOERR)
    return FALSE;
  return TRUE;
}
...
NcBool NcVar::add_att(NcToken aname, int len, const char *vals) {
  if (!the_file->define_mode())
    return FALSE;
  if (NcError::set_err(nc_put_att_text(the_file->id(), the_id, aname, len,
                                       vals)) != NC_NOERR)
    return FALSE;
  return TRUE;
}

NcVar_add_vector_att(short) NcVar_add_vector_att(int) NcVar_add_vector_att(long)
    NcVar_add_vector_att(float) NcVar_add_vector_att(double)
  ...
  NcBool ret = NcError::set_err(
                   nc_rename_var(the_file->id(), the_id, newname)) == NC_NOERR;
  if (ret) {
    delete[] the_name;
    the_name = new char[1 + strlen(newname)];
    strcpy(the_name, newname);
...
int NcVar::id(void) const { return the_id; }
...
NcBool NcVar::sync(void) {
  ...
  char nam[NC_MAX_NAME];
  if (the_file && NcError::set_err(nc_inq_varname(the_file->id(), the_id,
                                                  nam)) == NC_NOERR) {
...
NcVar::NcVar(NcFile *nc, int id) : NcTypedComponent(nc), the_id(id) {
  char nam[NC_MAX_NAME];
  if (the_file && NcError::set_err(nc_inq_varname(the_file->id(), the_id,
                                                  nam)) == NC_NOERR) {
    the_name = new char[1 + strlen(nam)];
...
int NcVar::attnum(NcToken attrname) const {
  ...
  for (num = 0; num < num_atts(); num++) {
    ...
    NcError::set_err(nc_inq_attname(the_file->id(), the_id, num, aname));
...
NcToken NcVar::attname(int attnum) const // caller must delete[]
{
  if (attnum < 0 || attnum >= num_atts())
    return 0;
  char aname[NC_MAX_NAME];
  if (NcError::set_err(nc_inq_attname(the_file->id(), the_id, attnum, aname)) !=
      NC_NOERR)
    return 0;
...
NcAtt::~NcAtt(void) { delete[] the_name; }

NcToken NcAtt::name(void) const { return the_name; }
...
NcBool NcAtt::is_valid(void) const {
  ...
  return the_file->is_valid() &&
         ...
         NcError::set_err(nc_inq_attid(the_file->id(), the_variable->id(),
                                       the_name, &num)) == NC_NOERR;
}
...
NcValues *NcAtt::values(void) const {
  ...
  switch (type()) {
  case ncFloat:
    status = NcError::set_err(nc_get_att_float(
        the_file->id(), the_variable->id(), the_name, (float *)valp->base()));
    break;
  case ncDouble:
    status = NcError::set_err(nc_get_att_double(
        the_file->id(), the_variable->id(), the_name, (double *)valp->base()));
    break;
  case ncInt:
    status = NcError::set_err(nc_get_att_int(the_file->id(), the_variable->id(),
                                             the_name, (int *)valp->base()));
    break;
  case ncShort:
    status = NcError::set_err(nc_get_att_short(
        the_file->id(), the_variable->id(), the_name, (short *)valp->base()));
    break;
  case ncByte:
    status = NcError::set_err(nc_get_att_schar(the_file->id(),
                                               the_variable->id(), the_name,
                                               (signed char *)valp->base()));
    break;
  case ncChar:
    status = NcError::set_err(nc_get_att_text(
        the_file->id(), the_variable->id(), the_name, (char *)valp->base()));
    break;
  case ncNoType:
  default:
    return 0;
...
NcBool NcAtt::rename(NcToken newname) {
  ...
  return NcError::set_err(nc_rename_att(the_file->id(), the_variable->id(),
                                        the_name, newname)) == NC_NOERR;
}

NcBool NcAtt::remove(void) {
  if (!the_file->define_mode())
    return FALSE;
  return NcError::set_err(nc_del_att(the_file->id(), the_variable->id(),
                                     the_name)) == NC_NOERR;
}
