...
#define NcValuesimplement(TYPE)                                                \
  ...
  NcVal(TYPE)::NcVal(TYPE)(long num)                                           \
      : NcValues(NcTypeEnum(TYPE), num), the_values(new TYPE[num]) {}          \
                                                                               \
  NcVal(TYPE)::NcVal(TYPE)(const NcVal(TYPE) & v) : NcValues(v) {              \
    delete[] the_values;                                                       \
  ...
  NcVal(TYPE) & NcVal(TYPE)::operator=(const NcVal(TYPE) & v) {                \
    ...
    return *this;                                                              \
                                                                               ...
                                                                               \
  void *NcVal(TYPE)::base(void) const { return the_values; }                   \
                                                                               \
  NcVal(TYPE)::~NcVal(TYPE)(void) { delete[] the_values; }                     \
                                                                               \
  int NcVal(TYPE)::invalid(void) const {                                       \
    for (int i = 0; i < the_number; i++)                                       \
      if (the_values[i] == makename2(ncBad_, TYPE))                            \
        return 1;                                                              \
    return 0;                                                                  \
  }

#define Ncbytes_for_one_implement(TYPE)                                        \
  int NcVal(TYPE)::bytes_for_one(void) const { return sizeof(TYPE); }
...
declare(NcValues, ncbyte) declare(NcValues, char) declare(NcValues, short)
    declare(NcValues, int) declare(NcValues, nclong) declare(NcValues, long)
        declare(NcValues, float) declare(NcValues, double)
