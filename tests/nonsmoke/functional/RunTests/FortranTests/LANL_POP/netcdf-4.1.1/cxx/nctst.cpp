...
int check_string_att(NcAtt *att, const char *theName, const char *value) {
...
int check_u_ln_atts(NcVar *var, const char *units, const char *long_name) {
  NcAtt *att;
...
int read(const char *path, NcFile::FileFormat format) {
  ...
  if (nc.num_dims() != 4 || nc.num_vars() != 6 || nc.num_atts() != 2)
    return NC_ERR;
  ...
  if (!(frtimeDim = nc.get_dim(FRTIME)))
    return NC_ERR;
  if (!frtimeDim->is_valid() ||
      strncmp(frtimeDim->name(), FRTIME, strlen(FRTIME)) ||
      frtimeDim->size() != 2 || !frtimeDim->is_unlimited())
    return NC_ERR;

  if (!(timeLenDim = nc.get_dim(TIMELEN1)))
    return NC_ERR;
  if (!timeLenDim->is_valid() ||
      strncmp(timeLenDim->name(), TIMELEN1, strlen(TIMELEN1)) ||
  ...
  if (!(att = pVar->get_att(VALID_RANGE)))
    return NC_ERR;
  if (!att->is_valid() ||
      strncmp(att->name(), VALID_RANGE, strlen(VALID_RANGE)) ||
  ...
  if (!(att = pVar->get_att(FILL_VALUE)))
    return NC_ERR;
  if (!att->is_valid() ||
      strncmp(att->name(), FILL_VALUE, strlen(FILL_VALUE)) ||
  ...
  if (!(att = scalarVar->get_att(SCALAR_ATT)))
    return NC_ERR;
  if (!att->is_valid() ||
      strncmp(att->name(), SCALAR_ATT, strlen(SCALAR_ATT)) ||
...
int gen(const char *path, NcFile::FileFormat format) // Generate a netCDF file
{
  ...
  NcFile nc(path, NcFile::Replace, NULL, 0,
            format); // Create, leave in define mode
  ...
  // P->edges()), but instead we write one record at a time, to show use of
  // setcur().
 ...
 */
static char *cdl_name(const char *path) {
...
void DumpableNcFile::dumpdims(void) {
  ...
  for (int n = 0; n < num_dims(); n++) {
    ...
    cout << "\t" << dim->name() << " = ";
    if (dim->is_unlimited())
      cout << "UNLIMITED" << " ;\t " << "// " << dim->size() << " currently\n";
    else
      cout << dim->size() << " ;\n";
  }
}
...
void DumpableNcFile::dumpvars(void) {
  int n;
  static const char *types[] = {"",     "byte",  "char",  "short",
                                "long", "float", "double"};
  NcVar *vp;
...
void dump(const char *path) {
  ...
  cout << "netcdf " << cdl_name(path) << " {" << endl << "dimensions:" << endl;
...
int main(void) // test new netCDF interface
{
  ...
  NcFile::FileFormat format[NUM_FORMATS] = {
      NcFile::Classic, NcFile::Offset64Bits
#ifdef USE_NETCDF4
      ,
  ...
  for (int i = 0; i < NUM_FORMATS; i++) {
    if (gen(file_name[i], format[i]) || read(file_name[i], format[i])) {
