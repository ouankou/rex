...
int utInit(const char *path) {
  ...
  if (unitSystem == NULL) {
    status = ut_get_status() == UT_PARSE ? UT_ESYNTAX : UT_EIO;
  } else {
 ...
 */
int utIsInit() { return unitSystem != NULL; }

static int compare(const void *key1, const void *key2) {
  return key1 < key2 ? -1 : key1 == key2 ? 0 : 1;
}

void utFree(utUnit *const unit) {
...
static void freeIfAllocated(utUnit *const unit) { utFree(unit); }

static int setUnit(utUnit *const unit, ut_unit *const ut_unit) {
  int status;
 ...
 */
int utScan(const char *spec, utUnit *unit) {
  int status;
  ...
  if (spec == NULL || unit == NULL) {
    ...
    if (ut_unit != NULL) {
      ...
      if (status == UT_BAD_ARG) {
        status = unitSystem == NULL ? UT_ENOINIT : UT_EINVALID;
 ...
 */
int utCalendar(double value, const utUnit *unit, int *year, int *month,
 ...
 */
int utInvCalendar(int year, int month, int day, int hour, int minute,
                  double second, const utUnit *unit, double *value) {
  int status = 0; /* success */
  ...
  if (converter == NULL) {
    ...
    double encodedTime = ut_encode_time(year, month, day, hour, minute, second);
...
static ut_status isTimeVisitBasic(const ut_unit *unit, void *arg) {
  return (ut_status)ut_are_convertible(unit, second);
}

static ut_status isTimeVisitProduct(const ut_unit *unit, int count,
...
static ut_status isTimeVisitGalilean(const ut_unit *unit, double scale,
                                     const ut_unit *underlyingUnit,
                                     double origin, void *arg) {
  return (ut_status)0;
}

static ut_status isTimeVisitTimestamp(const ut_unit *unit,
                                      const ut_unit *timeUnit, double origin,
                                      void *arg) {
  return (ut_status)1;
}

static ut_status isTimeVisitLogarithmic(const ut_unit *unit, double base,
...
static ut_status hasOriginVisitBasic(const ut_unit *unit, void *arg) {
  return (ut_status)0;
}

static ut_status hasOriginVisitProduct(const ut_unit *unit, int count,
                                       const ut_unit *const *basicUnits,
                                       const int *powers, void *arg) {
  return (ut_status)0;
}

static ut_status hasOriginVisitGalilean(const ut_unit *unit, double scale,
                                        const ut_unit *underlyingUnit,
                                        double origin, void *arg) {
  return (ut_status)1;
}

static ut_status hasOriginVisitTimestamp(const ut_unit *unit,
                                         const ut_unit *timeUnit, double origin,
                                         void *arg) {
  return (ut_status)1;
}

static ut_status hasOriginVisitLogarithmic(const ut_unit *unit, double base,
 ...
 */
int utHasOrigin(const utUnit *up) {
  ut_visitor visitor;
...
static utUnit *resultingUnit(utUnit *result, ut_unit *const unit) {
  if (unit == NULL) {
    result = NULL;
  } else if (result != NULL) {
 ...
 */
utUnit *utCopy(const utUnit *source, utUnit *dest) {
 ...
 */
utUnit *utMultiply(const utUnit *term1, const utUnit *term2, utUnit *result) {
  return term1 == NULL || term2 == NULL
             ? NULL
             : resultingUnit(result,
                             ut_multiply(term1->ut_unit, term2->ut_unit));
}

/*
 * Divide one unit-structure by another.
 */
utUnit *utDivide(const utUnit *numer, const utUnit *denom, utUnit *result) {
 ...
 */
utUnit *utInvert(const utUnit *unit, utUnit *result) {
 ...
 */
utUnit *utRaise(const utUnit *unit, int power, utUnit *result) {
 ...
 */
utUnit *utShift(const utUnit *unit, double amount, utUnit *result) {
  return unit == NULL ? NULL
 ...
 */
utUnit *utScale(const utUnit *unit, double factor, utUnit *result) {
  return unit == NULL ? NULL
 ...
 */
int utConvert(const utUnit *from, const utUnit *to, double *slope,
              double *intercept) {
  int status;
 ...
 */
int utPrint(const utUnit *unit, char **buf) {
  int status;
 ...
 */
int utAdd(char *name, int hasPlural, const utUnit *unit) {
  ...
  return status == UT_SUCCESS ? 0 : status == UT_EXISTS ? UT_DUP : UT_EALLOC;
}
