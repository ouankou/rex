...
typedef struct {
  ...
  ut_status (*acceptVisitor)(const ut_unit *, const ut_visitor *, void *);
} UnitOps;

typedef enum { BASIC, PRODUCT, GALILEAN, LOG, TIMESTAMP } UnitType;
...
#define GET_PRODUCT(unit) ((unit)->common.ops->getProduct(unit))
#define CLONE(unit) ((unit)->common.ops->clone(unit))
#define MULTIPLY(unit1, unit2) ((unit1)->common.ops->multiply(unit1, unit2))
#define RAISE(unit, power) ((unit)->common.ops->raise(unit, power))
#define ROOT(unit, root) ((unit)->common.ops->root(unit, root))
#define FREE(unit) ((unit)->common.ops->free(unit))
#define COMPARE(unit1, unit2) ((unit1)->common.ops->compare(unit1, unit2))
 ...
 */
static ProductUnit *productNew(ut_system *const system,
                               const short *const indexes,
                               const short *const powers, const int count);
static void productFree(ut_unit *const unit);
static ut_unit *productMultiply(const ut_unit *const unit1,
                                const ut_unit *const unit2);
static ut_unit *productRaise(const ut_unit *const unit, const int power);
static ut_unit *productRoot(const ut_unit *const unit, const int root);
 ...
 */
void julianDayToGregorianDate(
    julday, year, month, day) long julday; /* Julian day number to convert */
 ...
 */
long gregorianDateToJulianDay(year, month, day)
 ...
 */
static void decompose(value, uncer, nbasis, basis, count) double value;
double uncer; /* >= 0 */
int nbasis;
...
double ut_encode_date(int year, int month, int day) {
  ...
  return 86400.0 * (gregorianDateToJulianDay(year, month, day) - juldayOrigin);
}
 ...
 *	 0	Success.
 */
static int commonInit(Common *const common, const UnitOps *const ops,
                      const ut_system *const system, const UnitType type) {
  assert(system != NULL);
  assert(common != NULL);
  assert(ops != NULL);
 ...
 */
static BasicUnit *basicNew(ut_system *const system, const int isDimensionless,
                           const int index) {
  ...
  if (product == NULL) {
    ut_set_status(UT_OS);
    ut_handle_error_message("basicNew(): Couldn't create new product-unit");
  } else {
    ...
    if (basicUnit == NULL) {
      ...
      ut_handle_error_message(
          ...
          sizeof(BasicUnit));
    } else if (commonInit(&basicUnit->common, &basicOps, system, BASIC) == 0) {
...
static ProductUnit *basicGetProduct(const ut_unit *const unit) {
  assert(IS_BASIC(unit));

  return unit->basic.product;
}

static ut_unit *basicClone(const ut_unit *const unit) {
  assert(IS_BASIC(unit));
...
static void basicFree(ut_unit *const unit) {
  if (unit != NULL) {
    assert(IS_BASIC(unit));
...
static int basicCompare(const ut_unit *const unit1,
                        const ut_unit *const unit2) {
  int cmp;
 ...
 */
static ut_unit *basicMultiply(const ut_unit *const unit1,
                              const ut_unit *const unit2) {
  assert(unit1 != NULL);
  assert(unit2 != NULL);
  assert(IS_BASIC(unit1));
 ...
 *	else	The resulting unit.
 */
static ut_unit *basicRaise(const ut_unit *const unit, const int power) {
  assert(unit != NULL);
  assert(IS_BASIC(unit));
  assert(power != 0);
  assert(power != 1);
 ...
 *	else	The resulting unit.
 */
static ut_unit *basicRoot(const ut_unit *const unit, const int root) {
  assert(unit != NULL);
  assert(IS_BASIC(unit));
  assert(root > 1);
 ...
 *	 0	Success.
 */
static int basicInitConverterToProduct(ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_BASIC(unit));
 ...
 *	 0	Success.
 */
static int basicInitConverterFromProduct(ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_BASIC(unit));
...
static ut_status basicAcceptVisitor(const ut_unit *const unit,
...
static UnitOps basicOps = {basicGetProduct,
                           basicClone,
                           basicFree,
                           basicCompare,
                           basicMultiply,
...
static ProductUnit *productNew(ut_system *const system,
  ...
  if (productUnit == NULL) {
    ...
    if (commonInit(&productUnit->common, &productOps, system, PRODUCT) == 0) {
      if (count == 0) {
        ...
        if (count > 0 && newIndexes == NULL) {
          ...
          ut_handle_error_message("productNew(): "
                                  "Couldn't allocate %d-element index array",
                                  count);
        } else {
...
static ProductUnit *productGetProduct(const ut_unit *const unit) {
  assert(unit != NULL);
...
static ut_unit *productClone(const ut_unit *const unit) {
  ut_unit *clone;
  ...
  if (unit == unit->common.system->one) {
    ...
    clone = (ut_unit *)productNew(unit->common.system, unit->product.indexes,
...
static int productCompare(const ut_unit *const unit1,
                          const ut_unit *const unit2) {
  int cmp;
...
static void productReallyFree(ut_unit *const unit) {
  if (unit != NULL) {
    assert(IS_PRODUCT(unit));
    ...
    if (sumCount == 0) {
      ...
      if (indexes == NULL) {
        ...
        if (powers == NULL) {
          ...
          result = (ut_unit *)productNew(unit1->common.system, indexes, powers,
                                         count);
        } /* "powers" re-allocated */
  ...
  if (count == 0) {
    ...
    if (newPowers == NULL) {
      ...
      ut_handle_error_message("productRaise(): "
                              "Couldn't allocate %d-element powers-buffer",
                              count);
    } else {
      ...
      result = (ut_unit *)productNew(unit->common.system, product->indexes,
                                     newPowers, count);
 ...
 *	 0	Success.
 */
static int productInitConverter(cv_converter **const converter) {
 ...
 *	 0	Success.
 */
static int productInitConverterToProduct(ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_PRODUCT(unit));
 ...
 *	 0	Success.
 */
static int productInitConverterFromProduct(ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_PRODUCT(unit));
 ...
 *				unconvertible.
 */
static ProductRelationship productRelationship(const ProductUnit *const unit1,
...
static ut_status productAcceptVisitor(const ut_unit *const unit,
                                      const ut_visitor *const visitor,
                                      void *const arg) {
  ...
  if (count != 0 && basicUnits == NULL) {
    ...
    if (count != 0 && powers == NULL) {
      ...
      ut_set_status(visitor->visit_product(
          unit, count, (const ut_unit **)basicUnits, powers, arg));
...
static UnitOps productOps = {productGetProduct,
                             productClone,
                             productFree,
                             productCompare,
                             productMultiply,
 ...
 */
static int productIsDimensionless(const ProductUnit *const unit) {
 ...
 */
static ut_unit *galileanNew(double scale, const ut_unit *unit, double offset) {
  ...
  if (scale == 1 && offset == 0) {
    ...
    if (galileanUnit == NULL) {
      ...
      if (commonInit(&galileanUnit->common, &galileanOps, unit->common.system,
                     GALILEAN) == 0) {
...
static ProductUnit *galileanGetProduct(const ut_unit *const unit) {
  assert(unit != NULL);
...
static ut_unit *galileanClone(const ut_unit *const unit) {
...
static int galileanCompare(const ut_unit *const unit1,
                           const ut_unit *const unit2) {
  int cmp;
  ...
  if (!IS_GALILEAN(unit2)) {
    ...
    cmp = galilean1->offset < galilean2->offset    ? -1
          : galilean1->offset == galilean2->offset ? 0
                                                   : -1;

    if (cmp == 0) {
      cmp = galilean1->scale < galilean2->scale    ? -1
            : galilean1->scale == galilean2->scale ? 0
                                                   : -1;
...
static void galileanFree(ut_unit *const unit) {
  if (unit != NULL) {
    assert(IS_GALILEAN(unit));
 ...
 *	else	The resulting unit.
 */
static ut_unit *galileanMultiply(const ut_unit *const unit1,
                                 const ut_unit *const unit2) {
  ut_unit *result = NULL; /* failure */
  ...
  if (IS_PRODUCT(unit2)) {
    ...
    const GalileanUnit *const galilean2 = &unit2->galilean;
    ut_unit *tmp = MULTIPLY(galilean1->unit, galilean2->unit);
 ...
 *	else	The resulting unit.
 */
static ut_unit *galileanRaise(const ut_unit *const unit, const int power) {
 ...
 *	else	The resulting unit.
 */
static ut_unit *galileanRoot(const ut_unit *const unit, const int root) {
  const GalileanUnit *galilean;
  ut_unit *tmp;
  ut_unit *result = NULL; /* failure */
 ...
 *	0	Success.
 */
static int galileanInitConverterToProduct(ut_unit *const unit) {
  ...
  toUnderlying = cv_get_galilean(unit->galilean.scale,
                                 unit->galilean.offset * unit->galilean.scale);
  ...
  if (toUnderlying == NULL) {
    ...
    if (ENSURE_CONVERTER_TO_PRODUCT(unit->galilean.unit)) {
      ...
      unit->common.toProduct =
          cv_combine(toUnderlying, unit->galilean.unit->common.toProduct);
 ...
 *	0	Success.
 */
static int galileanInitConverterFromProduct(ut_unit *const unit) {
  int retCode = -1; /* failure */
  ...
  fromUnderlying =
      cv_get_galilean(1.0 / unit->galilean.scale, -unit->galilean.offset);
  ...
  if (fromUnderlying == NULL) {
    ...
    if (ENSURE_CONVERTER_FROM_PRODUCT(unit->galilean.unit)) {
      ...
      unit->common.fromProduct =
          cv_combine(unit->galilean.unit->common.fromProduct, fromUnderlying);
...
static ut_status galileanAcceptVisitor(const ut_unit *const unit,
                                       const ut_visitor *const visitor,
                                       void *const arg) {
  assert(unit != NULL);
  assert(IS_GALILEAN(unit));
  ...
  return visitor->visit_galilean(unit, unit->galilean.scale,
                                 unit->galilean.unit, unit->galilean.offset,
                                 arg);
}

static UnitOps galileanOps = {galileanGetProduct,
                              galileanClone,
                              galileanFree,
                              galileanCompare,
                              galileanMultiply,
 ...
 */
static ut_unit *timestampNewOrigin(const ut_unit *unit, const double origin) {
  ut_unit *newUnit = NULL; /* failure */
  ut_unit *secondUnit;
  ...
  if (secondUnit == NULL) {
    ...
    if (timestampUnit == NULL) {
      ...
      if (commonInit(&timestampUnit->common, &timestampOps, unit->common.system,
                     TIMESTAMP) == 0) {
 ...
 *	else	The newly-allocated, timestamp-unit.
 */
static ut_unit *timestampNew(ut_unit *unit, const int year, const int month,
...
static ProductUnit *timestampGetProduct(const ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_TIMESTAMP(unit));
...
static ut_unit *timestampClone(const ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_TIMESTAMP(unit));
...
static int timestampCompare(const ut_unit *const unit1,
                            const ut_unit *const unit2) {
  int cmp;
  ...
  if (!IS_TIMESTAMP(unit2)) {
    ...
    cmp = timestamp1->origin < timestamp2->origin    ? -1
          : timestamp1->origin == timestamp2->origin ? 0
                                                     : -1;
...
static void timestampFree(ut_unit *const unit) {
  if (unit != NULL) {
    assert(IS_TIMESTAMP(unit));
 ...
 *	else	The resulting unit.
 */
static ut_unit *timestampMultiply(const ut_unit *const unit1,
                                  const ut_unit *const unit2) {
  assert(unit1 != NULL);
  assert(IS_TIMESTAMP(unit1));
  assert(unit2 != NULL);
 ...
 *	else	The resulting unit.
 */
static ut_unit *timestampRaise(const ut_unit *const unit, const int power) {
  assert(unit != NULL);
  assert(IS_TIMESTAMP(unit));
  assert(power != 0);
  assert(power != 1);
 ...
 *	else	The resulting unit.
 */
static ut_unit *timestampRoot(const ut_unit *const unit, const int root) {
  assert(unit != NULL);
  assert(IS_TIMESTAMP(unit));
 ...
 *	-1	Failure.
 */
static int timestampInitConverterToProduct(ut_unit *const unit) {
  /*
 ...
 *	-1	Failure.
 */
static int timestampInitConverterFromProduct(ut_unit *const unit) {
  /*
   * This function is never called.
   */
  assert(0);
...
static ut_status timestampAcceptVisitor(const ut_unit *const unit,
                                        const ut_visitor *const visitor,
                                        void *const arg) {
  assert(unit != NULL);
  assert(IS_TIMESTAMP(unit));
...
static UnitOps timestampOps = {timestampGetProduct,
                               timestampClone,
                               timestampFree,
                               timestampCompare,
                               timestampMultiply,
 ...
 */
static ut_unit *logNew(const double base, const ut_unit *const reference) {
  LogUnit *logUnit;
  ...
  if (logUnit == NULL) {
    ...
    if (commonInit(&logUnit->common, &logOps, reference->common.system, LOG) !=
        0) {
      free(logUnit);
    } else {
...
static ProductUnit *logGetProduct(const ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_LOG(unit));
...
static ut_unit *logClone(const ut_unit *const unit) {
  assert(unit != NULL);
  assert(IS_LOG(unit));
...
static int logCompare(const ut_unit *const unit1, const ut_unit *const unit2) {
  int cmp;
  ...
  if (!IS_LOG(unit2)) {
    ...
    if (cmp == 0)
      cmp = u1->base < u2->base ? -1 : u1->base == u2->base ? 0 : 1;
  }

  return cmp;
}

static void logFree(ut_unit *const unit) {
  if (unit != NULL) {
    assert(IS_LOG(unit));
 ...
 *	else	The resulting unit.
 */
static ut_unit *logMultiply(const ut_unit *const unit1,
                            const ut_unit *const unit2) {
  ut_unit *result = NULL; /* failure */
 ...
 *	else	The resulting unit.
 */
static ut_unit *logRaise(const ut_unit *const unit, const int power) {
  assert(unit != NULL);
  assert(IS_LOG(unit));
  assert(power != 0);
  assert(power != 1);
 ...
 *	else	The resulting unit.
 */
static ut_unit *logRoot(const ut_unit *const unit, const int root) {
  assert(unit != NULL);
  assert(IS_LOG(unit));
  assert(root > 1 && root < 256);
 ...
 *	0	Success.
 */
static int logInitConverterToProduct(ut_unit *const unit) {
  int retCode = -1; /* failure */
  cv_converter *toUnderlying;
  ...
  if (toUnderlying == NULL) {
    ...
    if (ENSURE_CONVERTER_TO_PRODUCT(unit->log.reference)) {
      ...
      unit->common.toProduct =
          cv_combine(toUnderlying, unit->log.reference->common.toProduct);
 ...
 *	0	Success.
 */
static int logInitConverterFromProduct(ut_unit *const unit) {
  int retCode = -1; /* failure */
  cv_converter *fromUnderlying;
  ...
  if (fromUnderlying == NULL) {
    ...
    if (ENSURE_CONVERTER_FROM_PRODUCT(unit->log.reference)) {
      ...
      unit->common.fromProduct =
          cv_combine(unit->log.reference->common.fromProduct, fromUnderlying);
...
static ut_status logAcceptVisitor(const ut_unit *const unit,
                                  const ut_visitor *const visitor,
                                  void *const arg) {
  assert(unit != NULL);
  assert(IS_LOG(unit));
...
static UnitOps logOps = {logGetProduct,
                         logClone,
                         logFree,
                         logCompare,
                         logMultiply,
 ...
 */
void coreFreeSystem(ut_system *system) {
  if (system != NULL) {
    int i;
 ...
 */
static BasicUnit *newBasicUnit(ut_system *const system,
  ...
  if (system == NULL) {
    ...
    if (basicUnit != NULL) {
      ...
      if (save == NULL) {
        ut_set_status(UT_OS);
        ut_handle_error_message(strerror(errno));
        ut_handle_error_message("newBasicUnit(): Couldn't clone basic-unit");
      } else {
        BasicUnit **basicUnits = realloc(
            system->basicUnits, (system->basicCount + 1) * sizeof(BasicUnit *));
 ...
 * the first call to "ut_offset_by_time()". ut_read_xml() calls this function if
 *the resulting unit-system contains a unit named "second".
 *
 * Arguments:
...
ut_unit *ut_scale(const double factor, const ut_unit *const unit) {
  ...
  if (unit == NULL) {
    ...
    if (factor == 0) {
      ...
      result = factor == 1 ? CLONE(unit) : galileanNew(factor, unit, 0.0);
    }
  }
...
ut_unit *ut_offset(const ut_unit *const unit, const double offset) {
  ...
  if (unit == NULL) {
    ...
    result = offset == 0 ? CLONE(unit) : galileanNew(1.0, unit, offset);
  }
 ...
 */
ut_unit *ut_invert(const ut_unit *const unit) { return ut_raise(unit, -1); }
...
ut_unit *ut_raise(const ut_unit *const unit, const int power) {
  ...
  if (unit == NULL) {
    ...
    result = power == 0   ? unit->common.system->one
             : power == 1 ? CLONE(unit)
                          : RAISE(unit, power);
  }
...
ut_unit *ut_root(const ut_unit *const unit, const int root) {
  ...
  if (unit == NULL) {
    ...
    result = root == 1 ? CLONE(unit) : ROOT(unit, root);
  }
  ...
  if (from == NULL || to == NULL) {
    ...
    if (!IS_TIMESTAMP(from) && !IS_TIMESTAMP(to)) {
      ...
      if (relationship == PRODUCT_UNCONVERTIBLE) {
        ...
        if (relationship == PRODUCT_EQUAL) {
          converter =
              cv_combine(from->common.toProduct, to->common.fromProduct);
        } else {
          /*
          ...
          if (invert != NULL) {
            cv_converter *phase1 = cv_combine(from->common.toProduct, invert);

            if (phase1 != NULL) {
              converter = cv_combine(phase1, to->common.fromProduct);
        ...
        if (converter == NULL) {
          ut_set_status(UT_OS);
          ut_handle_error_message(strerror(errno));
          ut_handle_error_message("ut_get_converter(): Couldn't get converter");
        }
      } /* got necessary product converters */
    } /* neither unit is a timestamp */
    else {
      cv_converter *toSeconds =
          ut_get_converter(from->timestamp.unit, from->common.system->second);
      ...
      if (toSeconds == NULL) {
        ...
        if (shiftOrigin == NULL) {
          ...
          cv_converter *toToUnit = cv_combine(toSeconds, shiftOrigin);
          ...
          if (toToUnit == NULL) {
            ...
            cv_converter *fromSeconds =
                ut_get_converter(to->common.system->second, to->timestamp.unit);
...
int ut_is_dimensionless(const ut_unit *const unit) {
  ...
  if (unit == NULL) {
     ...
     */
    isDimensionless =
        IS_LOG(unit) ? 1 : productIsDimensionless(GET_PRODUCT(unit));
  }
 ...
 *		client.
 */
ut_unit *ut_clone(const ut_unit *const unit) {
  ...
  if (unit == NULL) {
    ...
    clone = unit == unit->common.system->one ? (ut_unit *)unit : CLONE(unit);
  }
