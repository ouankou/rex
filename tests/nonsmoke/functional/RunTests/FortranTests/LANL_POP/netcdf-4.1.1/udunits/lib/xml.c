...
#define IGNORE_TEXT XML_SetCharacterDataHandler(currFile->parser, NULL)
...
static ut_status readXml(const char *const path);
 ...
 */
const char *ut_form_plural(const char *singular) {
      ...
      if (length == 1) {
        ...
        if (lastChar == 'y') {
          ...
          if (lastChar == 's' || lastChar == 'x' || lastChar == 'z' ||
              (length >= 2 && (strcmp(singular + length - 2, "ch") == 0 ||
 ...
 *      else            Success.
 */
static int substitute(const char *const inString, const char *str,
                      char *const outString, const char *repl,
                      const size_t size) {
 ...
 *      1               Success.
 */
static int latin1_to_utf8(const char *inString, char *outString, size_t size) {
  size_t numSpecial = 0;
 ...
 *      1               Success.
 */
static int utf8_to_latin1(const char *inString, char *outString, size_t size) {
  size_t numSpecial = 0;
  const unsigned char *in;
  unsigned char *out;
 ...
 */
static void make_latin1_forms(const char *latin1, char *nonNbsp, char *nbsp) {
  if (strchr(latin1, '_')) {
 ...
 *                      underscores replaced with non-breaking spaces.
 */
static int make_utf8_forms(const char *utf8, char *nonNbsp, char *nbsp) {
  int success;
 ...
 */
static int makeDerivatives(const char *const id, const ut_encoding encoding,
                           Identifiers *ids) {
  int success = 1;
  ...
  if (strlen(id) > NAME_SIZE - 1) {
    ...
    if (encoding == UT_ASCII) {
      ...
      if (strchr(id, '_')) {
        (void)substitute(id, "_", ids->latin1Nbsp, LATIN1_NBSP, NAME_SIZE);
        success = latin1_to_utf8(ids->latin1Nbsp, ids->utf8Nbsp, NAME_SIZE);
      }
 ...
 *      else            Success.
 */
static int mapUnitToId(ut_unit *const unit, const char *const id,
                       ut_encoding encoding, int isName) {
 ...
 *      else            Success.
 */
static int mapUnitToIds(ut_unit *const unit, const char *const id,
                        ut_encoding encoding, int isName) {
 ...
 *      else            Success.
 */
static int mapUnitToNames(ut_unit *const unit, const char *const name,
 ...
 *      else            Success.
 */
static int mapUnitToSymbols(ut_unit *const unit, const char *const symbol,
                            ut_encoding encoding) {
 ...
 *      else            Success.
 */
static int mapIdToUnit(const char *id, const ut_encoding encoding,
                       ut_unit *unit, const int isName) {
  int success = 0; /* failure */
  ...
  if (prev != NULL) {
    char buf[128];
    int nchar =
        ut_format(prev, buf, sizeof(buf), UT_ASCII | UT_DEFINITION | UT_NAMES);

    ut_set_status(UT_PARSE);
    ut_handle_error_message("Duplicate definition for \"%s\" at \"%s\":%d", id,
                            currFile->path,
    ...
    if ((isName ? ut_map_name_to_unit(id, encoding, unit)
                : ut_map_symbol_to_unit(id, encoding, unit)) != UT_SUCCESS) {
      ut_set_status(UT_PARSE);
      ...
      if (prev != NULL) {
        ...
        if (nchar < 0 || nchar >= sizeof(buf)) {
          ...
          ut_handle_error_message("Definition of \"%s\" in \"%s\", "
                                  "line %d, overrides prefixed-unit",
                                  id, currFile->path,
                                  XML_GetCurrentLineNumber(currFile->parser));
        } else {
          buf[nchar] = 0;
 ...
 *      else            Success.
 */
static int mapIdsToUnit(const char *const id, const ut_encoding encoding,
                        ut_unit *const unit, const int isName) {
  Identifiers ids;
  int success = 1;
 ...
 *      else            Success.
 */
static int mapNamesToUnit(const char *const name, const ut_encoding encoding,
                          ut_unit *const unit) {
 ...
 *      else            Success.
 */
static int mapSymbolsToUnit(const char *const symbol,
                            const ut_encoding encoding, ut_unit *const unit) {
 ...
 *      else            Success.
 */
static int mapUnitAndName(ut_unit *const unit, const char *const name,
                          ut_encoding encoding) {
  return mapNamesToUnit(name, encoding, unit) &&
 ...
 *      else            Success.
 */
static int mapUnitAndSymbol(ut_unit *const unit, const char *const symbol,
                            ut_encoding encoding) {
  return mapSymbolsToUnit(symbol, encoding, unit) &&
 ...
 */
static void fileInit(File *const file) {
  file->context = START;
  file->skipDepth = 0;
  file->value = 0;
 ...
 */
static void accumulateText(void *data,
 ...
 */
static void startUnitSystem(void *data, const char **atts) {
 ...
 */
static void endUnitSystem(void *data) {}

/*
 * Handles the start of a <prefix> element.
 */
static void startPrefix(void *data, const char *const *atts) {
 ...
 */
static void endPrefix(void *data) {
 ...
 */
static void startUnit(void *data, const char **atts) {
  if (currFile->context != UNIT_SYSTEM) {
    ut_set_status(UT_PARSE);
 ...
 */
static void startBase(void *data, const char **atts) {
 ...
 */
static void startDimensionless(void *data, const char **atts) {
  if (currFile->context != UNIT) {
    ut_set_status(UT_PARSE);
 ...
 */
static void endDimensionless(void *data) {
 ...
 */
static void startDef(void *data, const char **atts) {
  if (currFile->context != UNIT) {
    ut_set_status(UT_PARSE);
    ...
    ut_set_status(UT_PARSE);
    ut_handle_error_message("<dimensionless> and <def> are mutually exclusive");
    XML_StopParser(currFile->parser, 0);
  } else if (currFile->unit != NULL) {
    ut_set_status(UT_PARSE);
...
static void endDef(void *data) {
  if (nbytes == 0) {
    ...
    if (currFile->unit == NULL) {
      ut_set_status(UT_PARSE);
      ut_handle_error_message("Couldn't parse unit specification \"%s\"", text);
      XML_StopParser(currFile->parser, 0);
    }
  }
}
 ...
 */
static void startName(void *data, const char **atts) {
  if (currFile->context == PREFIX) {
    ...
    if (currFile->unit == NULL) {
      ...
      currFile->singular[0] = 0;
      currFile->plural[0] = 0;
      currFile->context = currFile->context == UNIT ? UNIT_NAME : ALIAS_NAME;
    }
  } else {
    ut_set_status(UT_PARSE);
...
static void endName(void *data) {
  if (currFile->context == PREFIX) {
    if (!currFile->haveValue) {
      ...
      if (ut_add_name_prefix(unitSystem, text, currFile->value) != UT_SUCCESS) {
        ut_set_status(UT_PARSE);
        ut_handle_error_message("Couldn't map name-prefix \"%s\" to value %g",
                                text, currFile->value);
        XML_StopParser(currFile->parser, 0);
      } else {
    ...
    if (currFile->singular[0] == 0) {
      ...
      if (!currFile->noPLural) {
        ...
        if (plural != NULL) {
          if (!mapNamesToUnit(plural, currFile->textEncoding, currFile->unit))
            XML_StopParser(currFile->parser, 0);
        }
      } /* <noplural> not specified */
    } /* singular name specified */
 ...
 */
static void startSingular(void *data, const char **atts) {
 ...
 */
static void endSingular(void *data) {
  if (nbytes >= NAME_SIZE) {
    ut_set_status(UT_PARSE);
 ...
 */
static void startPlural(void *data, const char **atts) {
  if (currFile->context != UNIT_NAME && currFile->context != ALIAS_NAME) {
    ut_set_status(UT_PARSE);
 ...
 */
static void endPlural(void *data) {
  if (nbytes == 0) {
    ut_set_status(UT_PARSE);
 ...
 */
static void startNoPlural(void *data, const char **atts) {
  if (currFile->context != UNIT_NAME || currFile->context != ALIAS_NAME) {
    ut_set_status(UT_PARSE);
 ...
 */
static void endNoPlural(void *data) { currFile->noPLural = 1; }

/*
 * Handles the start of a <symbol> element.
 */
static void startSymbol(void *data, const char **atts) {
  if (currFile->context == PREFIX) {
    if (!currFile->haveValue) {
      ut_set_status(UT_PARSE);
      ut_handle_error_message("No previous <value> element");
...
static void endSymbol(void *data) {
  if (currFile->context == PREFIX) {
    if (ut_add_symbol_prefix(unitSystem, text, currFile->value) != UT_SUCCESS) {
      ut_set_status(UT_PARSE);
      ut_handle_error_message("Couldn't map symbol-prefix \"%s\" to value %g",
                              text, currFile->value);
      XML_StopParser(currFile->parser, 0);
    } else {
      currFile->prefixAdded = 1;
 ...
 */
static void startValue(void *data, const char **atts) {
  if (currFile->context != PREFIX) {
    ut_set_status(UT_PARSE);
 ...
 */
static void endValue(void *data) {
  char *endPtr;
 ...
 */
static void startAliases(void *data, const char **atts) {
  if (currFile->context != UNIT) {
    ut_set_status(UT_PARSE);
 ...
 */
static void endAliases(void *data) { currFile->context = UNIT; }

/*
 * Handles the start of an <import> element.
 */
static void startImport(void *data, const char **atts) {
  if (currFile->context != UNIT_SYSTEM) {
    ut_set_status(UT_PARSE);
 ...
 */
static void endImport(void *data) {
  ...
  if (text[0] == '/') {
    ...
    (void)snprintf(buf, sizeof(buf), "%s/%s", XML_GetBase(currFile->parser),
                   text);
 ...
 */
static void declareXml(void *data, const char *version, const char *encoding,
                       int standalone) {
  ...
  if (file.fd == -1) {
    ...
    do {
      ...
      if (nbytes < 0) {
        ...
        if (XML_Parse(file.parser, buf, nbytes, nbytes == 0) != XML_STATUS_OK) {
          status = UT_PARSE;
          ut_set_status(status);
          ut_handle_error_message(
    ...
    if (status != UT_SUCCESS) {
       ...
       */
      ut_handle_error_message("File \"%s\", line %d, column %d", path,
                              XML_GetCurrentLineNumber(file.parser),
