...
static PrefixSearchEntry *pseNew(int character, size_t position) {
...
static void pseFree(PrefixSearchEntry *const entry) { free(entry); }

static int pseSensitiveCompare(const void *const entry1,
...
static int pseInsensitiveCompare(const void *const entry1,
                                 const void *const entry2) {
...
static PrefixToValueMap *ptvmNew(int (*compare)(const void *, const void *)) {
  PrefixToValueMap *map = (PrefixToValueMap *)malloc(sizeof(PrefixToValueMap));
...
ptvmSearch(PrefixToValueMap *map, const char *const id, const double value) {
 ...
 *			beginning of "string".
 */
static const PrefixSearchEntry *ptvmFind(PrefixToValueMap *const map,
 ...
 */
static ut_status addPrefix(ut_system *const system, const char *const prefix,
                           const double value, SystemMap **const systemMap,
                           int (*compare)(const void *, const void *)) {
  ut_status status;
    ...
    if (*systemMap != NULL) {
      ...
      if (prefixToValue == NULL) {
        ...
        if (*prefixToValue != NULL) {
          ...
          status = entry == NULL             ? UT_OS
                   : (entry->value == value) ? UT_SUCCESS
                                             : UT_EXISTS;
        }
 ...
 *	UT_OS		Operating-system failure.  See "errno".
 */
ut_status ut_add_name_prefix(ut_system *const system, const char *const name,
                             const double value) {
 ...
 *	UT_OS		Operating-system failure.  See "errno".
 */
ut_status ut_add_symbol_prefix(ut_system *const system,
                               const char *const symbol, const double value) {
 ...
 *			with "system".
 */
static ut_status findPrefix(ut_system *const system, SystemMap *const systemMap,
                            const char *const string, double *const value,
                            size_t *const len) {
  ut_status status;
  ...
  if (system == NULL) {
    ...
    if (prefixToValue == NULL) {
      ...
      const PrefixSearchEntry *entry = ptvmFind(*prefixToValue, string);
 ...
 */
ut_status utGetPrefixByName(ut_system *const system, const char *const string,
                            double *const value, size_t *const len) {
  return string == NULL
             ? UT_BAD_ARG
 ...
 *	UT_SUCCESS	Success.  "*value" and "*len" will be set if non-NULL.
 */
ut_status utGetPrefixBySymbol(ut_system *const system, const char *const string,
                              double *const value, size_t *const len) {
  return string == NULL
             ? UT_BAD_ARG
