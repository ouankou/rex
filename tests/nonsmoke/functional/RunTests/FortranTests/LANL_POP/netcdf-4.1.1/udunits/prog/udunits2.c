...
static int decodeCommandLine(int argc, char *const *argv) {
  ...
  if (c == -1) {
    _xmlPath = optind < argc ? argv[optind] : NULL;
...
static int ensureXmlPathSet(void) {
  if (_xmlPath == NULL)
...
static void setEncoding(char *value) {
  if (value != NULL) {
    typedef struct {
      const char *pattern;
    ...
    if (!initialized) {
      ...
      for (i = 0; i < entryCount; i++) {
        ...
        status = regcomp(reg, entries[i].pattern,
...
static int readXmlDatabase(void) {
  int success = 0;
  ...
  if (_unitSystem != NULL) {
    ...
    (void)fprintf(stderr,
                  "%s: Couldn't initialize unit-system from "
                  "database \"%s\"\n",
                  _progname, _xmlPath);
  }
 ...
 * Get a specification.
 */
static int getSpec(const char *const prompt, char *const spec,
                   const size_t size) {
...
static int getOutputRequest(void) {
  int success = 0;
    ...
    if (getOutputRequest()) {
      if (_wantDefinition) {
        ...
        if (scaledUnit == NULL) {
          ...
          nbytes = ut_format(scaledUnit, buf, sizeof(buf), _formattingOptions);
        ...
        if (conv == NULL) {
          ...
          char whiteSpace[] = " \t\n\r\f\v\xa0";
          int needsParens = strpbrk(_wantSpec, whiteSpace) != NULL;
          int n;

          (void)printf(needsParens ? "    %g %s = %g (%s)\n"
                                   : "    %g %s = %g %s\n",
                       _haveNumber, _haveUnitSpec,
          ...
          if (n >= 0)
            (void)printf(strpbrk(_wantSpec, whiteSpace) ||
                                 strpbrk(_wantSpec, "/")
                             ? "    x/(%s) = %*s\n"
                             : "    x/%s = %*s\n",
                         _wantSpec, n, exp);
