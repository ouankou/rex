 ...
 */
static char *latin1ToUtf8(const char *const latin1String) {
  int nchar;
  const char *inp;
  char *outp;
  char *utf8String;
 ...
 */
static int adjustEncoding(ut_encoding *const encoding, const char *string) {
...
static int compareUnits(const void *const entry1, const void *const entry2) {
 ...
 */
static void **selectTree(UnitToIdMap *const unitToIdMap,
                         const ut_encoding encoding) {
  return encoding == UT_ASCII    ? &unitToIdMap->ascii
         : encoding == UT_LATIN1 ? &unitToIdMap->latin1
                                 : &unitToIdMap->utf8;
}
 ...
 */
static UnitToIdMap *utimNew(void) {
 ...
 */
static void utimFree(UnitToIdMap *map) {
  if (map != NULL) {
 ...
 *	UT_SUCCESS	Success.
 */
static ut_status utimAdd(UnitToIdMap *const map, const ut_unit *unit,
                         const char *const id, ut_encoding encoding) {
  ut_status status;
 ...
 *	UT_SUCCESS	Success.
 */
static ut_status utimRemove(UnitToIdMap *const map, const ut_unit *unit,
 ...
 *		in ASCII.
 */
static UnitAndId *utimFindAsciiByUnit(UnitToIdMap *const map,
 ...
 */
static UnitAndId *utimFindLatin1ByUnit(UnitToIdMap *const map,
                                       const ut_unit *const unit) {
  UnitAndId targetEntry;
  UnitAndId **treeEntry;
 ...
 */
static UnitAndId *utimFindUtf8ByUnit(UnitToIdMap *const map,
                                     const ut_unit *const unit) {
  UnitAndId targetEntry;
 ...
 *	UT_SUCCESS	Success.
 */
static ut_status mapUnitToId(SystemMap **const systemMap,
                             const ut_unit *const unit, const char *const id,
                             ut_encoding encoding) {
  ut_status status;
 ...
 *	UT_SUCCESS	Success.
 */
static ut_status unmapUnitToId(SystemMap *const systemMap,
                               const ut_unit *const unit,
                               ut_encoding encoding) {
  ut_status status;
  ...
  if (systemMap == NULL || unit == NULL) {
    ...
    status = (unitToIdMap == NULL || *unitToIdMap == NULL)
                 ? UT_SUCCESS
 ...
 */
static const char *getId(SystemMap *const systemMap, const ut_unit *const unit,
                         const ut_encoding encoding) {
  ...
  if (unit == NULL) {
    ...
    if (unitToId != NULL) {
      UnitAndId *mapEntry =
          encoding == UT_LATIN1 ? utimFindLatin1ByUnit(*unitToId, unit)
          : encoding == UT_UTF8 ? utimFindUtf8ByUnit(*unitToId, unit)
 ...
 *	UT_SUCCESS	Success.
 */
ut_status ut_unmap_unit_to_name(const ut_unit *const unit,
                                ut_encoding encoding) {
 ...
 *	UT_BAD_ARG	"unit" is NULL.
 */
ut_status ut_unmap_unit_to_symbol(const ut_unit *const unit,
                                  ut_encoding encoding) {
 ...
 *			"unit" maps.
 */
const char *ut_get_symbol(const ut_unit *const unit,
                          const ut_encoding encoding) {
  ut_set_status(UT_SUCCESS);
 ...
 *			resources freed.
 */
void utimFreeSystem(ut_system *system) {
  if (system != NULL) {
