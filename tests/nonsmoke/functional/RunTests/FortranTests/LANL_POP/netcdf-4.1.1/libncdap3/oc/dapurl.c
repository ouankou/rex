...
static char *legalprotocols[] = {
    "file:", "http:", "https:", "ftp:", NULL /* NULL terminate*/
};

/* Do a simple url parse*/
int dapurlparse(const char *url0, DAPURL *dapurl) {
  char *url;
  char **pp;
  char *p;
  char *p1;
  ...
  if (*p == LBRACKET) {
    ...
    if (*p == 0)
      goto fail; /* malformed client params*/
  ...
  if (ocdebug) {
    fprintf(stderr,
            "dapurl: params=|%s| base=|%s| projection=|%s| selection=|%s|\n",
            dapurl->params, dapurl->base, dapurl->projection,
            dapurl->selection);
  }
  free(url);
  return 1;
...
void dapurlclear(DAPURL *dapurl) {
  if (dapurl->url != NULL) {
    free(dapurl->url);
  }
  if (dapurl->base != NULL) {
  ...
  if (dapurl->protocol != NULL) {
    free(dapurl->protocol);
  }
  if (dapurl->projection != NULL) {
    free(dapurl->projection);
  }
  if (dapurl->selection != NULL) {
    free(dapurl->selection);
  }
...
void dapurlsetconstraints(DAPURL *durl, const char *constraints) {
  char *proj = NULL;
  char *select = NULL;
  const char *p;

  if (durl->projection != NULL)
    ocfree(durl->projection);
  if (durl->selection != NULL)
    ocfree(durl->selection);
  durl->projection = NULL;
  durl->selection = NULL;

  if (constraints == NULL || strlen(constraints) == 0)
    return;
...
*/
char *dapurlgeturl(DAPURL *durl, const char *prefix, const char *suffix,
                   int withconstraints) {
  size_t len = 0;
  char *newurl;
  ...
  if (withconstraints &&
    ...
    len += 1; /* leading '?' */
    if (durl->projection != NULL)
      len += strlen(durl->projection);
    if (durl->selection != NULL)
      len += strlen(durl->selection);
  }
  ...
  if (withconstraints &&
    ...
    strcat(newurl, "?");
    if (durl->projection != NULL)
      strcat(newurl, durl->projection);
    if (durl->selection != NULL)
      strcat(newurl, durl->selection);
  }
  return newurl;
}
