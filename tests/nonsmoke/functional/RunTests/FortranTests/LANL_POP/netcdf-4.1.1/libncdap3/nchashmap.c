 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/nchashmap.c,v 1.4 2009/09/23
 *22:26:08 dmh Exp $
 *********************************************************************/
#include "nchashmap.h"
...
int nchashfree(NChashmap *hm) {
  if (hm) {
    int i;
    for (i = 0; i < hm->alloc; i++) {
      if (hm->table[i] != NULL)
        nclistfree(hm->table[i]);
    }
    free(hm->table);
    free(hm);
  }
...
int nchashinsert(NChashmap *hm, nchashid hash, ncelem value) {
  int i, offset, len;
  NClist *seq;
  ncelem *list;
...
int nchashreplace(NChashmap *hm, nchashid hash, ncelem value) {
  int i, offset, len;
  NClist *seq;
  ncelem *list;
...
int nchashremove(NChashmap *hm, nchashid hash) {
  int i, offset, len;
  NClist *seq;
  ncelem *list;
  ...
  for (i = 0; i < len; i += 2, list += 2) {
    if (*list == hash) {
      ...
      nclistremove(seq, i);
      hm->size--;
      if (nclistlength(seq) == 0) {
        nclistfree(seq);
        hm->table[offset] = NULL;
      }
      return TRUE;
    }
  }
...
ncelem nchashget(NChashmap *hm, nchashid hash) {
  ncelem value;
  if (!nchashlookup(hm, hash, &value))
    return ncDATANULL;
  return value;
}

int nchashlookup(NChashmap *hm, nchashid hash, ncelem *valuep) {
  int i, offset, len;
  NClist *seq;
  ncelem *list;
...
/* Can be expensive*/
int nchashith(NChashmap *hm, int index, nchashid *hashp, ncelem *elemp) {
  int i;
  if (hm == NULL)
    return FALSE;
  for (i = 0; i < hm->alloc; i++) {
    ...
    if ((index - len) < 0) {
      if (hashp)
        *hashp = (nchashid)nclistget(seq, index * 2);
      if (elemp)
        *elemp = nclistget(seq, (index * 2) + 1);
      return TRUE;
    }
    index -= len;
  }
...
/* Can be expensive*/
int nchashkeys(NChashmap *hm, nchashid **keylist) {
  int i, j, index;
  nchashid *keys;
  if (hm == NULL)
    return FALSE;
