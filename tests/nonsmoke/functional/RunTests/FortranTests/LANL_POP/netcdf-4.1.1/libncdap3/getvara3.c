...
static NCerror movetor(NCDRNO *, OCdata currentcontent, NClist *path, int depth,
                       Getvara *, int dimindex, struct NCMEMORY *,
                       NClist *segments);

static int findfield(CDFnode *node, CDFnode *subnode);
static NCerror slicestring(OCconnection, OCdata, size_t, NCslice *,
                           struct NCMEMORY *);
static int wholeslicepoint(Dapodometer *odom);
static void removepseudodims3(NCprojection *clone);

NCerror nc3d_getvarx(int ncid, int varid, const size_t *startp,
                     const size_t *countp, const ptrdiff_t *stridep, void *data,
                     nc_type dsttype0) {
  ...
  for (i = 0; i < nclistlength(varnodes); i++) {
    ...
    if (node->array.basevar == NULL && node->nctype == NC_Primitive &&
        node->ncid == varid) {
      cdfvar = node;
      break;
    }
...
#ifdef DEBUG
  {
    ...
    if (nclistlength(dims) > 0) {
      ...
      for (i = 0; i < nclistlength(dims); i++)
        fprintf(stderr, "[%lu:%lu:%lu]", (unsigned long)startp[i],
                (unsigned long)countp[i], (unsigned long)stridep[i]);
      fprintf(stderr, " -> ");
      for (i = 0; i < nclistlength(dims); i++)
        if (stridep[i] == 1)
  ...
  ncstat = buildvaraprojection3(drno, varainfo, startp, countp, stridep,
                                &varaprojection);
  if (ncstat != NC_NOERR) {
    THROWCHK(ncstat);
    goto fail;
...
static NCerror moveto(NCDRNO *drno, Getvara *xgetvar, CDFnode *xrootnode,
                      void *memory) {
  OCerror ocstat = OC_NOERR;
  NCerror ncstat = NC_NOERR;
  ...
  ncstat = movetor(drno, xrootcontent, path, 0, xgetvar, 0, &memstate,
...
movetor(NCDRNO *drno, OCdata currentcontent, NClist *path,
  ...
  if (xnode->etype == NC_STRING || xnode->etype == NC_URL)
    hasstringdim = 1;
...
#define CASE(nc1, nc2) (nc1 * 1024 + nc2)
  ...
  default:
    PANIC2("Illegal combination: nctype=%d mode=%d", (int)xnode->nctype,
           (int)currentmode);
    break;
  ...
  case CASE(NC_Sequence, OCFIELDMODE):
    ...
    */
    ...
    ncstat = movetor(drno, fieldcontent, path, newdepth, xgetvar, dimindex,
                     memory, segments);
    break;
  ...
  case CASE(NC_Structure, OCARRAYMODE):
    ...
    while (dapodometermore(odom)) {
      ...
      ncstat = movetor(drno, dimcontent, path, depth, xgetvar, dimindex + rank,
                       memory, segments);
  ...
  case CASE(NC_Sequence, OCRECORDMODE): {
    ...
    */
    ...
    for (i = uslice->first; i < uslice->stop; i += uslice->stride) {
      ...
      ncstat = movetor(drno, reccontent, path, depth, xgetvar, dimindex + 1,
                       memory, segments);
      if (ncstat != OC_NOERR) {
        THROWCHK(ncstat);
        goto fail;
  ...
  case CASE(NC_Primitive, OCSCALARMODE):
    ...
    if (hasstringdim) {
      ...
      ncstat =
          dapconvert3(xnode->etype, xgetvar->dsttype, memory->next, value, 1);
      if (ncstat != NC_NOERR) {
        THROWCHK(ncstat);
        goto fail;
      }
  ...
  case CASE(NC_Primitive, OCARRAYMODE): {
    ...
    */
    ...
    if (wholepoint == 0 && !hasstringdim) { /* whole variable */
      ...
      if (localmemory == NULL) {
        ocstat = oc_data_get(conn, currentcontent, memory->next, internlen, 0,
                             odomsubsize);
        if (ocstat != OC_NOERR) {
          THROWCHK(ocstat);
          goto fail;
        ...
        ocstat = oc_data_get(conn, currentcontent, localmemory, internlen, 0,
                             odomsubsize);
        if (ocstat != OC_NOERR) {
          THROWCHK(ocstat);
          goto fail;
        ...
        /* do conversion */
        ncstat = dapconvert3(xnode->etype, xgetvar->dsttype, memory->next,
                             localmemory, odomsubsize);
        if (ncstat != NC_NOERR) {
          THROWCHK(ncstat);
          goto fail;
        }
      ...
      while (dapodometermore(odom)) {
        ...
        if (localmemory == NULL) {
          ocstat = oc_data_get(conn, currentcontent, memory->next, internlen,
                               dimoffset, odomsubsize);
          if (ocstat != OC_NOERR) {
            THROWCHK(ocstat);
            goto fail;
          }
        } else {
          ocstat = oc_data_get(conn, currentcontent, localmemory, internlen,
                               dimoffset, odomsubsize);
          if (ocstat != OC_NOERR) {
            THROWCHK(ocstat);
            goto fail;
      ...
      while (dapodometermore(odom)) {
        ...
        if (hasstringdim) {
          ncstat = slicestring(conn, currentcontent, dimoffset, &stringslice,
                               memory);
          if (ncstat != NC_NOERR) {
            THROWCHK(ncstat);
            goto fail;
...
static NCerror slicestring(OCconnection conn, OCdata content, size_t dimoffset,
  ...
  ocstat =
      oc_data_get(conn, content, &stringmem, sizeof(stringmem), dimoffset, 1);
  if (ocstat != OC_NOERR) {
    THROWCHK(ocstat);
    goto fail;
  }
...
#ifdef UNUSED
static int samevarinfo(Getvara *v1, Getvara *v2) {
  unsigned int i;
  NCslice *s1, *s2;
  if (v1 == NULL || v2 == NULL)
    return 0;
...
static int findfield(CDFnode *node, CDFnode *field) {
  size_t i;
...
static void removepseudodims3(NCprojection *clone) {
  ...
  for (i = 0; i < nsegs; i++) {
    ...
    if (seg->node->nctype != NC_Sequence)
  ...
  if (seg->node->nctype == NC_Primitive && seg->node->array.stringdim != NULL) {
    /* Remove the string dimension projection from the segment */
    seg->slicerank--; /* always the last */
  }
}

int nc3d_getvarmx(int ncid, int varid, const size_t *start, const size_t *edges,
                  const ptrdiff_t *stride, const ptrdiff_t *map, void *data,
                  nc_type dsttype0) {
  NCerror ncstat = NC_NOERR;
  int i;
  NC *ncp;
  ...
  */
  ...
  case NC_INT:
    ncstat =
        nc3d_get_vars_int(ncid, varid, start, edges, stride, (int *)localcopy);
    break;
  case NC_FLOAT:
