...
int nclistsetalloc(NClist *l, unsigned int sz) {
  ncelem *newcontent;
  if (l == NULL)
    return FALSE;
  if (sz <= 0) {
    sz = (l->length ? 2 * l->length : DEFAULTALLOC);
  }
  if (l->alloc >= sz) {
    return TRUE;
  }
...
int nclistsetlength(NClist *l, unsigned int sz) {
  if (l == NULL)
    return FALSE;
  if (sz > l->alloc && !nclistsetalloc(l, sz))
    return FALSE;
  l->length = sz;
  return TRUE;
}

ncelem nclistget(NClist *l, unsigned int index) {
...
int nclistset(NClist *l, unsigned int index, ncelem elem) {
  if (l == NULL)
    return FALSE;
  if (index >= l->length)
    return FALSE;
...
int nclistinsert(NClist *l, unsigned int index, ncelem elem) {
...
int nclistpush(NClist *l, ncelem elem) {
  if (l == NULL)
    return FALSE;
  if (l->length >= l->alloc)
    nclistsetalloc(l, 0);
...
ncelem nclisttop(NClist *l) {
  if (l == NULL || l->length == 0)
    return ncDATANULL;
  return l->content[l->length - 1];
}

ncelem nclistremove(NClist *l, unsigned int i) {
  unsigned int len;
  ncelem elem;
  if (l == NULL || (len = l->length) == 0)
    return ncDATANULL;
  if (i >= len)
    return ncDATANULL;
  elem = l->content[i];
  for (i += 1; i < len; i++)
    l->content[i - 1] = l->content[i];
  l->length--;
  return elem;
}

/* Duplicate and return the content (null terminate) */
ncelem *nclistdup(NClist *l) {
...
int nclistcontains(NClist *list, ncelem elem) {
  unsigned int i;
  for (i = 0; i < nclistlength(list); i++) {
    if (elem == nclistget(list, i))
      return 1;
  }
  return 0;
}
...
NClist *nclistclone(NClist *list) {
