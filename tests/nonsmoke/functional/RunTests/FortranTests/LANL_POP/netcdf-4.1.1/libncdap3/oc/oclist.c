...
int oclistsetalloc(OClist *l, unsigned int sz) {
  ocelem *newcontent;
  if (l == NULL)
    return FALSE;
  if (sz <= 0) {
...
int oclistsetlength(OClist *l, unsigned int sz) {
  if (l == NULL)
    return FALSE;
  if (sz > l->alloc && !oclistsetalloc(l, sz))
    return FALSE;
  l->length = sz;
  return TRUE;
}

ocelem oclistget(OClist *l, unsigned int index) {
...
int oclistset(OClist *l, unsigned int index, ocelem elem) {
  if (l == NULL)
    return FALSE;
  if (index >= l->length)
    return FALSE;
...
int oclistinsert(OClist *l, unsigned int index, ocelem elem) {
  unsigned int i;
  if (l == NULL)
    return FALSE;
  if (index > l->length)
...
int oclistpush(OClist *l, ocelem elem) {
  if (l == NULL)
    return FALSE;
  if (l->length >= l->alloc)
    oclistsetalloc(l, 0);
...
ocelem oclisttop(OClist *l) {
  if (l == NULL || l->length == 0)
    return ocDATANULL;
  return l->content[l->length - 1];
}

ocelem oclistremove(OClist *l, unsigned int i) {
  unsigned int len;
  ocelem elem;
...
int oclistcontains(OClist *list, ocelem elem) {
  unsigned int i;
  for (i = 0; i < oclistlength(list); i++) {
    if (elem == oclistget(list, i))
      return 1;
  }
  return 0;
}
