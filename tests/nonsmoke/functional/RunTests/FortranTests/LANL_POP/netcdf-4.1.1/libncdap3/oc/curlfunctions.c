 ...
 */
int set_user_password(CURL *curl, const char *userC, const char *passwordC) {
  CURLcode cstat;
...
int set_proxy(CURL *curl, struct OCproxy *pstructProxy) {
  CURLcode cstat;
  ...
  if (pstructProxy->user) {
    int userPassSize =
        strlen(pstructProxy->user) + strlen(pstructProxy->password) + 2;
...
int set_cookies(CURL *curl, const char *cook) {
  CURLcode cstat;
