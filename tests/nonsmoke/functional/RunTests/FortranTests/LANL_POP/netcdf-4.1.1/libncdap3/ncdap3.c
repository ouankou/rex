 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/ncdap3.c,v 1.82 2010/04/02 23:18:41
 *dmh Exp $
 *********************************************************************/
#include "config.h"
...
struct NCTMODEL nctmodels[] = {{3, NULL, (NCF_NC3 | NCF_NCDAP)},
...
static NCerror buildattribute3a(NCDRNO *, NCattribute *att, nc_type, int varid,
                                int ncid);
...
int nc3d_open(const char *path, int mode, int *ncidp) {
  NC *ncp = NULL;
  ...
  */
  ...
  if (nclistlength(drno->cdf.seqnodes) == 0 && drno->cdf.recorddim != NULL) {
...
NCerror freeNCDRNO3(NCDRNO *drno) {
#ifdef IGNORE
...
static void collectdims3(NClist *path, NClist *dimset) {
  int i, j;
  nclistclear(dimset);
...
static NCerror addstringdims(NCDRNO *drno) {
  ...
  */
  ...
  for (i = 0; i < nclistlength(varnodes); i++) {
    ...
    if (var->etype != NC_STRING && var->etype != NC_URL)
      continue;
    ...
    sdim =
        makecdfnode34(drno, dimname, OC_Dimension, OCNULL, drno->cdf.ddsroot);
    if (sdim == NULL)
...
static NCerror defrecorddim3(NCDRNO *drno) {
  ...
  for (i = 0; i < nclistlength(alldims); i++) {
    ...
    if (dim->nctype != NC_Dimension)
      continue;
    if (dim->dim.basedim != NULL)
      continue; /* not the controlling dim */
    if (strcmp(dim->name, drno->cdf.recorddim) != 0)
      continue;
    if (DIMFLAG(dim, CDFDIMCLONE))
      PANIC("cloned record dim");
    if (drno->cdf.unlimited != NULL)
      PANIC("Multiple unlimited");
...
static NCerror defseqdims(NCDRNO *drno) {
  ...
  if (!FLAGSET(drno, NCF_NOUNLIM)) {
    unlimited = makecdfnode34(drno, "unlimited", OC_Dimension, OCNULL,
                              drno->cdf.ddsroot);
...
static NCerror getseqdimsize(NCDRNO *drno, CDFnode *seq, size_t *sizep) {
  ...
  if (FLAGSET(drno, NCF_UNCONSTRAINABLE))
    ...
    ocstat = dap_oc_fetch(drno, conn, ncbytescontents(minconstraints),
                          OCDATADDS, &ocroot);
  if (ocstat)
    goto fail;
  ...
  if (ncstat != NC_NOERR) {
    ...
    if (code != NULL) {
      oc_log(OCLOGERR, "oc_fetch_datadds failed: %s %s %l", code, msg, httperr);
    }
    ocstat = OC_NOERR;
  }
  if (sizep)
...
#ifdef IGNORE
static NCerror addseqdims(NCDRNO *drno) {
  unsigned int i;
  NCerror ncstat = NC_NOERR;
  NClist *varnodes = drno->cdf.varnodes;
  for (i = 0; i < nclistlength(varnodes); i++) {
...
static int seqpartition(CDFnode *var) {
  unsigned int i;
...
static NCerror makeseqdim(NCDRNO *drno, CDFnode *seq, size_t count,
                          CDFnode **sqdimp) {
  CDFnode *sqdim;
...
static NCerror countsequence(NCDRNO *drno, CDFnode *xseq, size_t *sizep) {
  unsigned int i;
...
static NCerror buildncstructures(NCDRNO *drno)
...
static NCerror builddims(NCDRNO *drno) {
  int i;
  NCerror ncstat = NC_NOERR;
  int dimid;
  NC *ncp = (NC *)drno->controller;
  ...
  if (defunlimited && drno->cdf.unlimited != NULL) {
    ...
    size_t unlimsize;
    ncstat =
        nc3d_def_dim(ncid, unlimited->name, NC_UNLIMITED, &unlimited->ncid);
    if (ncstat != NC_NOERR)
      goto fail;
...
static NCerror buildvars(NCDRNO *drno) {
  int i, j, dimindex;
  NCerror ncstat = NC_NOERR;
  int varid;
  ...
  for (i = 0; i < nclistlength(varnodes); i++) {
    ...
    ncstat = nc3d_def_var(ncid, var->ncfullname, var->externaltype, ncrank,
                          (ncrank == 0 ? NULL : dimids), &varid);
    if (ncstat != NC_NOERR) {
      THROWCHK(ncstat);
      goto fail;
    }
...
static NCerror buildglobalattrs3(NCDRNO *drno, int ncid, CDFnode *root) {
  int i;
  NCerror ncstat = NC_NOERR;
  const char *txt;
  char *nltxt, *p;
...
static NCerror showprojection3(NCDRNO *drno, CDFnode *var) {
  ...
  ncstat =
      nc3d_put_att_text(getncid(drno->controller), var->ncid, "_projection",
                        ncbyteslength(projection), ncbytescontents(projection));
  return ncstat;
}

static NCerror buildattribute3a(NCDRNO *drno, NCattribute *att, nc_type vartype,
                                int varid, int ncid) {
  int i;
  NCerror ncstat = NC_NOERR;
  ...
  */
  if (att->etype == NC_STRING || att->etype == NC_URL) {
    ...
    if (newstring[0] == '\0')
      ...
      ncstat =
          nc3d_put_att_text(ncid, varid, cname, strlen(newstring), newstring);
    free(newstring);
  } else {
    nc_type atype;
...
#ifdef IGNORE
NCerror detachdatadds3(NCDRNO *drno) {
  int i;
...
NCerror attachdatadds3(NCDRNO *drno) {
  int i;
...
*/
static NCerror computeminconstraints3(NCDRNO *drno, CDFnode *seq,
                                      NCbytes *minconstraints) {
  NClist *path = nclistnew();
  CDFnode *var;
  CDFnode *candidate;
  ...
  for (candidate = NULL, var = NULL, i = 0;
       i < nclistlength(drno->cdf.varnodes); i++) {
...
static unsigned long cdftotalsize3(NClist *dimensions) {
  unsigned int i;
...
NCerror fetchtemplatemetadata3(NCDRNO *drno) {
  NCerror ncstat = NC_NOERR;
  OCerror ocstat = OC_NOERR;
...
NCerror fetchconstrainedmetadata3(NCDRNO *drno) {
  NCerror ncstat = NC_NOERR;
  OCerror ocstat = OC_NOERR;
  OCobject ocroot;
  ...
  if (ce == NULL || strlen(ce) == 0) {
    ...
    if (!FLAGSET(drno, NCF_UNCONSTRAINABLE)) {
      ...
      ncstat =
          regrid3(ddsroot, drno->cdf.ddsroot, drno->dap.constraint.projections);
      if (ncstat)
        goto fail;
    }
  ...
  while (found) {
    ...
    for (i = 0; i < nclistlength(drno->cdf.varnodes); i++) {
      ...
      for (j = 0; j < nclistlength(path); j++) {
        ...
        if (node->nctype == NC_Sequence && !node->usesequence) {
