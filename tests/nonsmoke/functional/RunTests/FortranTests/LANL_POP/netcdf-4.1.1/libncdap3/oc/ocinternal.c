...
static int ocinitialize(void) {
  ...
  /* Determine if this version of curl supports "file://..." urls.*/
  {
    ...
    for (proto = curldata->protocols; *proto; proto++) {
      if (strcmp("file", *proto) == 0) {
        oc_curl_file_supported = 1;
        break;
      }
    }
    if (ocdebug > 0) {
...
/**************************************************/
OCerror ocopen(OCstate **statep, const char *url) {
  int stat = OC_NOERR;
  OCstate *state = NULL;
  DAPURL tmpurl;
...
OCerror ocfetch(OCstate *state, const char *constraint, OCdxd kind,
                OCnode **rootp) {
  OCtree *tree = NULL;
  OCnode *root = NULL;
  ...
  if (stat != OC_NOERR) {
    ...
    if (state->error.httpcode >= 400) {
      oc_log(LOGWARN, "oc_open: Could not read url; http error = %l",
             state->error.httpcode);
    } else {
  ...
  if (stat == OC_EDAPSVC && state->error.code != NULL) {
    oc_log(
        LOGERR, "oc_open: server error retrieving url: code=%s message=\"%s\"",
        state->error.code, (state->error.message ? state->error.message : ""));
  }
  if (stat) {
    THROWCHK(stat);
    goto unwind;
  ...
  case OCDAS:
    if (root->octype != OC_Attributeset) {
      THROWCHK(stat = OC_EDAS);
      goto unwind;
    }
    break;
  case OCDDS:
    if (root->octype != OC_Dataset) {
      THROWCHK(stat = OC_EDDS);
      goto unwind;
    }
    break;
  case OCDATADDS:
    if (root->octype != OC_Dataset) {
      THROWCHK(stat = OC_EDATADDS);
      goto unwind;
    }
...
#else
    xdrmem_create(tree->data.xdrs, tree->data.xdrdata, tree->data.datasize,
                  XDR_DECODE);
#endif
    if (!xdr_setpos(tree->data.xdrs, tree->data.bod))
      return xdrerror();
  }
...
void occlose(OCstate *state) {
  ...
  if (state->curl != NULL)
    occurlclose(state->curl);
  if (state->clientparams != NULL)
    ocparamfree(state->clientparams);
  ocfree(state);
}

static OCerror ocextractdds(OCstate *state, OCtree *tree) {
  OCerror stat = OC_NOERR;
  ...
  if (tree->data.bod % sizeof(unsigned long long) != 0) {
    ...
    for (i = 0; i < count; i++)
      dst[i] = src[i]; /* avoid memcpy overlap */
...
#ifdef OC_DISK_STORAGE
static OCerror createtempfile(OCstate *state, OCtree *tree) {
...
OCerror ocupdatelastmodifieddata(OCstate *state) {
