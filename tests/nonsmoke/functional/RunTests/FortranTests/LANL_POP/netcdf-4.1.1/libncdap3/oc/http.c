...
int ocfetchurl_file(CURL *curl, char *url, FILE *stream, unsigned long *sizep,
                    long *filetime) {
  int stat = OC_NOERR;
  CURLcode cstat = CURLE_OK;
  struct Fetchdata fetchdata;

  if ((stat = ocsetcurlproperties(curl, url)) != OC_NOERR)
    goto fail;
...
int ocfetchurl(CURL *curl, char *url, OCbytes *buf, long *filetime) {
  int stat = OC_NOERR;
  CURLcode cstat = CURLE_OK;
  size_t len;
  ...
  if (cstat == CURLE_PARTIAL_FILE) {
    ...
    oc_log(LOGWARN, "curl error: %s; ignored", curl_easy_strerror(cstat));
    cstat = CURLE_OK;
  }
  if (cstat != CURLE_OK)
    goto fail;
...
static int ocsetcurlproperties(CURL *curl, const char *url) {
  CURLcode cstat = CURLE_OK;
...
int ocfetchlastmodified(CURL *curl, char *url, long *filetime) {
  int stat = OC_NOERR;
  CURLcode cstat = CURLE_OK;
  size_t len;
