...
static int ocverify(unsigned long object) {
  unsigned int i;
...
static OCobject ocassign(void *object) {
#ifdef TRACK
...
static void ocassignall(OClist *list) {
  unsigned int i;
  if (list != NULL)
...
OCerror oc_open(const char *url, OCconnection *connp) {
  OCerror ocerr;
  OCstate *state;
  if (ocmap == NULL) {
    ocmap = oclistnew();
...
OCerror oc_close(OCconnection conn) {
  OCstate *state;
...
OCerror oc_root_free(OCconnection conn, OCobject root0) {
  OCstate *state;
  OCnode *root;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
unsigned int oc_inq_nobjects(OCconnection conn, OCobject root0) {
  OCstate *state;
  OCnode *root;
  OClist *nodes;
...
OCobject *oc_inq_objects(OCconnection conn, OCobject root0) {
  unsigned int i;
  OCstate *state;
  OCnode *root;
  OClist *nodes;
...
const char *oc_inq_text(OCconnection conn, OCobject root0) {
  OCstate *state;
  OCnode *root;
  OCVERIFYX(OCstate *, state, conn, OCNULL);
  OCDEREF(OCstate *, state, conn);
...
OCerror
oc_inq_object(OCconnection conn, OCobject node0, char **namep,
              OCtype *objecttypep,
              OCtype *primitivetypep, /* if objecttype == OC_Primitive */
              OCobject *containerp, unsigned int *rankp,
              unsigned int *subnodesp, unsigned int *nattrp) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
  ...
  if (namep)
    *namep = (node->name == NULL ? NULL : strdup(node->name));
  if (objecttypep)
    *objecttypep = node->octype;
  if (primitivetypep)
    *primitivetypep = node->etype;
  if (rankp)
    *rankp = node->array.rank;
  if (containerp)
    *containerp = (OCobject)node->container;
  if (subnodesp)
    *subnodesp = oclistlength(node->subnodes);
  if (nattrp) {
...
OCerror oc_inq_name(OCconnection conn, OCobject node0, char **namep) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
  ...
  if (state == NULL || node == NULL)
    return OC_EINVAL;
  if (namep)
    *namep = nulldup(node->name);
  return OC_NOERR;
}

OCerror oc_inq_nsubnodes(OCconnection conn, OCobject node0,
  ...
  if (nsubnodesp)
    *nsubnodesp = oclistlength(node->subnodes);
  return OC_NOERR;
}

OCerror oc_inq_primtype(OCconnection conn, OCobject node0, OCtype *typep) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_inq_type(OCconnection conn, OCobject node0, OCtype *typep) {
  return oc_inq_class(conn, node0, typep);
}

OCerror oc_inq_class(OCconnection conn, OCobject node0, OCtype *typep) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_inq_rank(OCconnection conn, OCobject node0, unsigned int *rankp) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_inq_nattr(OCconnection conn, OCobject node0, unsigned int *nattrp) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_inq_root(OCconnection conn, OCobject node0, OCobject *rootp) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
  ...
  if (rootp)
    *rootp = (OCobject)node->root;
  return OC_NOERR;
}

OCerror oc_inq_container(OCconnection conn, OCobject node0,
                         OCobject *containerp) {
  OCstate *state;
  OCnode *node;
  OCVERIFY(OCstate *, state, conn);
...
OCerror oc_inq_subnodes(OCconnection conn, OCobject node0,
                        OCobject **subnodesp) {
  OCstate *state;
  OCnode *node;
...
OCerror oc_inq_ith(OCconnection conn, OCobject node0, unsigned int index,
                   OCobject *subnodeidp) {
  OCstate *state;
  OCnode *node;
  ...
  if (nsubnodes >= 0 && index < nsubnodes) {
    ...
    return OC_EINVAL;
  if (subnodeidp)
    *subnodeidp = subnodeid;
  return OC_NOERR;
}

/* Return the dimension objects, if any, for a given object */
/* Caller must free returned dimids */
OCerror oc_inq_dimset(OCconnection conn, OCobject node0, OCobject **dimids) {
  OCstate *state;
  OCnode *node;
  OCobject *dims = NULL;
  OCVERIFY(OCstate *, state, conn);
...
OCerror oc_inq_ithdim(OCconnection conn, OCobject node0, unsigned int index,
                      OCobject *dimidp) {
  OCstate *state;
  OCnode *node;
...
OCerror oc_inq_dim(OCconnection conn, OCobject node0, size_t *sizep,
                   char **namep) {
  OCstate *state;
  OCnode *dim;
  OCVERIFY(OCstate *, state, conn);
  ...
  if (dim->octype != OC_Dimension)
    return OC_EINVAL;
  if (sizep)
    *sizep = dim->dim.declsize;
  if (namep)
    *namep = (dim->name == NULL ? NULL : strdup(dim->name));
  return OC_NOERR;
}
...
OCerror oc_inq_attrstrings(OCconnection conn, OCobject node0, unsigned int i,
  ...
  if (namep)
    *namep = strdup(attr->name);
  if (octypep)
    *octypep = attr->etype;
  if (nvaluesp)
    *nvaluesp = attr->nvalues;
  if (stringsp) {
...
OCerror oc_inq_attr(OCconnection conn, OCobject node0, unsigned int i,
                    char **namep, OCtype *octypep, unsigned int *nvaluesp,
                    void **valuesp) {
  OCstate *state;
  OCnode *node;
  OCattribute *attr;
...
void oc_attr_reclaim(OCtype etype, unsigned int nvalues, void *values) {
  if (nvalues == 0 || values == NULL)
    return;
...
OCerror oc_inq_dasattr_nvalues(OCconnection conn, OCobject node0,
  ...
  if (attr->octype != OC_Attribute)
    return OC_EINVAL;
  if (nvaluesp)
    *nvaluesp = oclistlength(attr->att.values);
  return OC_NOERR;
}

OCerror oc_inq_dasattr(OCconnection conn, OCobject node0, unsigned int i,
  ...
  if (i >= nvalues)
    return OC_EINVAL;
  if (valuep)
    *valuep = nulldup((char *)oclistget(attr->att.values, i));
  if (primtypep)
    *primtypep = attr->etype;
  return OC_NOERR;
}
...
*/
OCerror oc_fetch(OCconnection conn, const char *constraint, OCdxd dxdkind,
                 OCobject *rootp) {
  OCstate *state;
  ...
  if (dxdkind == OCDATADDS) {
    ...
    compile = 1;
#else
    if (oc_clientparam_get(conn, "compile") != NULL)
      compile = 1;
#endif
    if (compile) {
  ...
  if (rootp)
    *rootp = (OCobject)ocassign(root);
  return ocerr;
}

OCerror oc_data_root(OCconnection conn, OCobject root0, OCdata content0) {
  OCstate *state;
  OCnode *root;
  OCcontent *content;
  OCerror ocerr = OC_NOERR;
...
OCdata oc_data_new(OCconnection conn) {
  OCstate *state;
  OCVERIFYX(OCstate *, state, conn, OCNULL);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_data_free(OCconnection conn, OCdata content0) {
  OCstate *state;
  OCcontent *content;
  if (content0 == OCNULL)
    return OC_NOERR;
...
OCerror oc_data_ith(OCconnection conn, OCdata parentdata, size_t index,
                    OCdata subdata) {
  OCstate *state;
  OCcontent *parent;
  OCcontent *child;
...
OCerror oc_data_get(OCconnection conn, OCdata currentcontent, void *memory,
  ...
  if (ocerr == OC_EDATADDS)
    ocdataddsmsg(state, current->tree);

  return ocerr;
}

OCerror oc_data_count(OCconnection conn, OCdata content0, size_t *sizep) {
  OCstate *state;
  OCcontent *current;
  size_t count = 0;
  OCerror ocerr = OC_NOERR;
  ...
  case OCARRAYMODE:
    ...
    break;
  case OCRECORDMODE:
    count = ocrecordcount(state, current);
    break;
  case OCFIELDMODE:
...
OCerror oc_data_index(OCconnection conn, OCdata content0, size_t *sizep) {
  OCstate *state;
  OCcontent *current;
  OCerror ocerr = OC_NOERR;
  OCVERIFY(OCstate *, state, conn);
...
OCerror oc_data_object(OCconnection conn, OCdata content0, OCobject *op) {
  OCstate *state;
  OCcontent *current;
  OCerror ocerr = OC_NOERR;
  OCVERIFY(OCstate *, state, conn);
  ...
  if (op)
    *op = (OCobject)current->node;
  return ocerr;
}

OCerror oc_data_mode(OCconnection conn, OCdata content0, OCmode *modep) {
  OCstate *state;
  OCcontent *current;
  OCerror ocerr = OC_NOERR;
  OCVERIFY(OCstate *, state, conn);
...
/**************************************************/
/* OCtype management */
size_t oc_typesize(OCtype etype) { return octypesize(etype); }

char *oc_typetostring(OCtype octype) { return octypetoddsstring(octype); }

OCerror oc_typeprint(OCtype etype, char *buf, size_t bufsize, void *value) {
...
char *oc_errstring(int err) { return ocerrstring(err); }

OCerror oc_compile(OCconnection conn, OCobject root0) {
  OCstate *state;
  OCnode *root;
...
const char *oc_clientparam_get(OCconnection conn, const char *param) {
  OCstate *state;
...
*/
OCerror oc_clientparam_delete(OCconnection conn, const char *param) {
  OCstate *state;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
*/
OCerror oc_clientparam_insert(OCconnection conn, const char *param,
                              const char *value) {
  OCstate *state;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
/* Replace client parameter
   return value:
        ...
        OC_EINVAL => not already defined
*/
OCerror oc_clientparam_replace(OCconnection conn, const char *param,
                               const char *value) {
  OCstate *state;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_dd(OCconnection conn, OCobject root0) {
  OCstate *state;
  OCnode *root;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
OCerror oc_attach_das(OCconnection conn, OCobject dasroot, OCobject ddsroot) {
  OCstate *state;
  OCnode *das;
  OCnode *dds;
  OCVERIFY(OCstate *, state, conn);
...
OCerror oc_raw_xdrsize(OCconnection conn, OCobject root0, size_t *sizep) {
  OCstate *state;
  OCnode *root;
  OCerror ocerr = OC_NOERR;
  OCVERIFY(OCstate *, state, conn);
...
OCerror oc_update_lastmodified_data(OCconnection conn) {
  OCstate *state;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
...
long oc_get_lastmodified_data(OCconnection conn) {
  OCstate *state;
  OCVERIFY(OCstate *, state, conn);
  OCDEREF(OCstate *, state, conn);
