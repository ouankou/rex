 ...
 */
int credentials_in_url(const char *url) {
...
int extract_credentials(const char *url, char **name, char **pw,
                        char **result_url) {
  char *pos;
  char *end;
  char *middle;
...
int set_credentials(const char *name, const char *pw) {
  if (!(name && pw)) {
...
/*Allows for a .dodsrc file to be read in and parsed in order to get
 * authentication information*/
int read_dodsrc(char *in_file_name) {
  char *p;
  char more[1024];
  char *v;
  FILE *in_file;
      ...
      *p = '\0';
      ...
      if (strcmp(more, "USE_CACHE") == 0) {
        ...
        if (credentials_in_url(v)) {
          ...
          extract_credentials(v, &pstructProxy->user, &pstructProxy->password,
                              &result_url);
          v = result_url;
        } else {
...
/*Allows for a .dodsrc file to be created if one does not currently exist for
 * default authentication values*/
int write_dodsrc(char *out_file_name) {
  char *authent[] = {
