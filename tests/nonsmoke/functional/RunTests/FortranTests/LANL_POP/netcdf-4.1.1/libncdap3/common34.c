 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/common34.c,v 1.28 2009/12/03
 *03:42:37 dmh Exp $
 *********************************************************************/
#include "config.h"
...
static NCerror buildcdftree34r(NCDRNO *, OCobject, CDFnode *, CDFtree *,
                               CDFnode **);
...
static NCerror fix1node34(NCDRNO *drno, CDFnode *node) {
  if (node->nctype == NC_Dimension && node->name == NULL)
    return NC_NOERR;
...
NCerror fixnodes34(NCDRNO *drno, NClist *cdfnodes) {
  int i;
...
#ifdef IGNORE
NCerror computecdfinfo34(NCDRNO *drno, NClist *cdfnodes) {
  int i;
  /* Ensure every node has an initial base name defined and fullname */
  /* Exceptions: anonymous dimensions. */
  for (i = 0; i < nclistlength(cdfnodes); i++) {
    ...
    efree(node->ncfullname);
    node->ncfullname = makecdfpathstring3(node, drno->cdf.separator);
    if (node == v4node && node->ncfullname[0] != 'Q')
      dappanic("");
  }
  for (i = 0; i < nclistlength(cdfnodes); i++) {
    CDFnode *node = (CDFnode *)nclistget(cdfnodes, i);
...
NCerror fixgrid34(NCDRNO *drno, CDFnode *grid) {
  unsigned int i, glen;
  CDFnode *array;
  ...
  if ((glen - 1) != nclistlength(array->array.dimensions))
    goto invalid;
  for (i = 1; i < glen; i++) {
    ...
    if (nclistlength(map->array.dimensions) != 1)
      goto invalid;
    ...
    if (FLAGSET(drno, (NCF_NCDAP | NCF_NC3))) {
      ...
      snprintf(tmp, sizeof(tmp), "%s%s%s", map->container->ncbasename,
               drno->cdf.separator, map->ncbasename);
...
*/
static int computedimindex3(CDFnode *var, CDFnode *dim) {
  int i, index;
  NClist *vardims = var->array.dimensions;
  for (index = -1, i = 0; i < nclistlength(vardims); i++) {
    if (dim == (CDFnode *)nclistget(vardims, i)) {
      index = i;
      break;
    }
  }
...
static CDFnode *clonedim(NCDRNO *drno, CDFnode *dim, CDFnode *var) {
  CDFnode *clone;
  clone = makecdfnode34(drno, dim->name, OC_Dimension, OCNULL, dim->container);
...
NCerror clonecdfdims34(NCDRNO *drno) {
  int i, j;
  ...
  for (i = 0; i < nclistlength(vars); i++) {
    ...
    if (node->array.dimensions != NULL) {
      ...
      for (j = 0; j < rank; j++) {
        ...
        CDFnode *clone = dim;
        if (j < ninherit)
          clone = clonedim(drno, dim, node);
        nclistpush(clonedims, (ncelem)clone);
      }
      nclistfree(dims);
      if (node->array.stringdim != NULL) {
        nclistpush(clonedims,
                   (ncelem)clonedim(drno, node->array.stringdim, node));
      }
...
static int getcompletedimset3(CDFnode *var, NClist *dimset) {
  int i, j;
...
NCerror computecdfdimnames34(NCDRNO *drno) {
  int i, j;
  ...
  for (i = 0; i < nclistlength(vars); i++) {
    ...
    if (nclistlength(var->array.dimensions) == 0)
      continue;
    ...
    for (j = 0; j < nclistlength(dims); j++) {
      ...
      if (DIMFLAG(dim, CDFDIMANON)) {
        ...
        snprintf(tmp, sizeof(tmp), "%s_%d", var->ncbasename, index);
        efree(dim->ncbasename);
        dim->ncbasename = cdflegalname3(tmp);
        snprintf(tmp, sizeof(tmp), "%s_%d", var->ncfullname, index);
  ...
  */
  ...
  if (drno->cdf.unlimited != NULL &&
      DIMFLAG(drno->cdf.unlimited, CDFDIMRECORD)) {
  ...
  for (i = 0; i < nclistlength(alldims); i++) {
    ...
    for (j = i + 1; j < nclistlength(alldims); j++) {
      ...
      if (dim->dim.basedim != NULL)
        continue; /* ignore*/
      if (strcmp(dim->ncfullname, basedim->ncfullname) != 0)
        continue;
      if (dim->dim.declsize == basedim->dim.declsize)
        continue;
  ...
  for (i = 0; i < nclistlength(alldims); i++) {
    ...
    if (dim1->dim.basedim != NULL)
      continue;
    for (j = 0; j < i; j++) {
      CDFnode *dim2 = (CDFnode *)nclistget(alldims, j);
      if (dim2->dim.basedim != NULL)
        continue;
...
NCerror makegetvar34(NCDRNO *drno, CDFnode *var, void *data, nc_type dsttype,
                     Getvara **getvarp) {
  Getvara *getvar;
...
int constrainable34(DAPURL *durl) {
...
CDFnode *makecdfnode34(NCDRNO *drno, char *name, OCtype octype,
...
*/
NCerror buildcdftree34(NCDRNO *drno, OCobject ocroot, OCdxd occlass,
                       CDFnode **cdfrootp) {
  CDFnode *root = NULL;
...
static NCerror buildcdftree34r(NCDRNO *drno, OCobject ocnode,
                               CDFnode *container, CDFtree *tree,
                               CDFnode **cdfnodep) {
  ...
  case OC_Dimension:
  default:
    PANIC1("buildcdftree: unexpect OC node type: %d", (int)octype);
  }
  /* cross link */
  cdfnode->root = tree->root;

  if (ocrank > 0)
    dupdimensions(ocnode, cdfnode, drno, tree);
...
static void dupdimensions(OCobject ocnode, CDFnode *cdfnode, NCDRNO *drno,
                          CDFtree *tree) {
  ...
  for (i = 0; i < ocrank; i++) {
    ...
    cdfdim =
        makecdfnode34(drno, ocname, OC_Dimension, ocdim, cdfnode->container);
    if (ocname == NULL)
      DIMFLAGSET(cdfdim, CDFDIMANON);
    efree(ocname);
...
NCerror applyclientparams34(NCDRNO *drno) {
  int i, len;
  ...
  limit = getlimitnumber(value);
  if (limit > 0)
    drno->cdf.cache.cachecount = limit;
...
void freecdfroot34(CDFnode *root) {
  int i;
  CDFtree *tree;
  NCDRNO *drno;
  if (root == NULL)
...
*/
int nodematch34(CDFnode *node1, CDFnode *node2) {
  if (node1 == NULL)
    return (node2 == NULL);
  if (node2 == NULL)
    return 0;
  ...
  if (node1->nctype == NC_Dataset)
    return 1;
  if (node1->nctype == NC_Primitive && node1->etype != node2->etype)
    return 0;
  if (node1->name != NULL && node2->name != NULL &&
      strcmp(node1->name, node2->name) != 0)
    return 0;
...
int simplenodematch34(CDFnode *node1, CDFnode *node2) {
  if (node1 == NULL)
    return (node2 == NULL);
  if (node2 == NULL)
    return 0;
  ...
  if (nclistlength(node1->array.dimensions0) !=
      nclistlength(node2->array.dimensions0))
    return 0;
  return 1;
}
...
#ifdef IGNORE
static CDFnode *findxnode34r(NClist *path, int depth, CDFnode *xnode) {
  unsigned int i;
  CDFnode *pathnode;
...
CDFnode *findxnode34(CDFnode *target, CDFnode *xroot) {
...
static void setattach(CDFnode *target, CDFnode *srcnode) {
...
static NCerror attachdims34(CDFnode *xnode, CDFnode *ddsnode) {
  unsigned int i;
...
static NCerror attachall34r(CDFnode *dstnode, CDFnode *srcnode) {
  unsigned int i;
  NCerror ncstat = NC_NOERR;
...
but not always.
*/
NCerror attachall34(CDFnode *dstroot, CDFnode *srcroot) {
  NCerror ncstat = NC_NOERR;

  if (dstroot->attachment)
    unattach34(dstroot);
  if (srcroot != NULL && srcroot->attachment)
    unattach34(srcroot);
  if (!nodematch34(dstroot, srcroot)) {
    THROWCHK(ncstat = NC_EINVAL);
    goto done;
  }
...
static NCerror attach34r(CDFnode *xnode, NClist *path, int depth) {
  ...
  if (lastnode)
    goto done; /* We have the match and are done */
...
NCerror attach34(CDFnode *xroot, CDFnode *ddstarget) {
  NCerror ncstat = NC_NOERR;
  NClist *path = nclistnew();
  ...
  if (xroot->attachment)
    unattach34(xroot);
  if (ddsroot != NULL && ddsroot->attachment)
    unattach34(ddsroot);
...
NCerror attachsubset34(CDFnode *dstroot, CDFnode *srcroot) {
  NCerror ncstat = NC_NOERR;

  if (srcroot == NULL) {
    THROWCHK(ncstat = NC_NOERR);
    goto done;
  }
  if (!nodematch34(dstroot, srcroot)) {
    THROWCHK(ncstat = NC_EINVAL);
...
static NCerror attachsubset34r(CDFnode *dstnode, CDFnode *srcnode) {
  unsigned int i;
  NCerror ncstat = NC_NOERR;
  int fieldindex;
  ...
  fieldindex = 0;
  for (fieldindex = 0, i = 0; i < nclistlength(srcnode->subnodes) &&
                              fieldindex < nclistlength(dstnode->subnodes);
       i++) {
    CDFnode *srcsubnode = (CDFnode *)nclistget(srcnode->subnodes, i);
