...
/* Forward */
static int occompile1(OCstate *state, OCnode *node, OCmemdata **memdatap,
                      XDR *);
static int occompilefields(OCstate *state, OCnode *node, OCmemdata **memdatap,
                           XDR *);
static int occompileprim(OCstate *state, OCnode *node, OCmemdata **memdatap,
                         XDR *);
...
int occompile(OCstate *state, OCnode *xroot) {
...
static int occompile1(OCstate *state, OCnode *xnode, OCmemdata **memdatap,
                      XDR *xdrs) {
  ...
  default:
    OCPANIC1("ocmap: encountered unexpected node type: %x", xnode->octype);
    break;
  }
...
static int occompilefields(OCstate *state, OCnode *xnode, OCmemdata **memdatap,
                           XDR *xdrs) {
  OCmemdata *structdata = NULL;
  OCmemdata **qmem;
  OCerror ocstat = OC_NOERR;
...
static int occompileprim(OCstate *state, OCnode *xnode, OCmemdata **memdatap,
                         XDR *xdrs) {
  ...
  case OC_String:
  case OC_URL: { /* Get the array of strings and store pointers in buf */
    char **dst = (char **)memdata->data.data;
    for (i = 0; i < xdrcount; i++) {
      char *s = NULL;
      if (!xdr_string(xdrs, (void *)&s, OC_INT32_MAX)) {
        ocstat = OC_EXDR;
        goto fail;
      }
      dst[i] = s;
  ...
  case OC_Char: {
    if (xnode->array.rank == 0) { /* Single unpacked character/byte */
      union {
        unsigned int i;
        char c[BYTES_PER_XDR_UNIT];
      } u;
      if (!xdr_opaque(xdrs, u.c, BYTES_PER_XDR_UNIT)) {
        ocstat = OC_EXDR;
        goto fail;
      }
      u.i = ocntoh(u.i);
      ...
      if (!xdr_opaque(xdrs, dst, xdrcount)) {
        ocstat = OC_EXDR;
        goto fail;
      }
    }
  ...
  case OC_UInt16: {
    ...
    if (!xdr_opaque(xdrs, (char *)src, xdrsize)) {
      ocfree(src);
      ocstat = OC_EXDR;
      goto fail;
    }
  ...
  case OC_Float32: {
    ...
    size_t xdrsize = xdrcount * BYTES_PER_XDR_UNIT;
    if (!xdr_opaque(xdrs, (char *)dst, xdrsize)) {
      ocstat = OC_EXDR;
      goto fail;
    }
    if (!oc_network_order) {
...
static size_t instancesize(OCtype etype) {
  switch (etype) {
  case OC_Char:
    return sizeof(char);
  case OC_Byte:
  ...
  case OC_Grid:
  case OC_Structure:
    return sizeof(char *); /* represented as pointer */
  default:
    break;
  }
  return 0;
...
static OCmemdata *makememdata(OCtype octype, OCtype etype,
                              unsigned long count) {
  ...
  if (memdata->etype > 107)
    OCPANIC("help");
  memdata->count = count;
  return memdata;
}

void freeocmemdata(OCmemdata *md) {
  char **sp;
  OCmemdata **mdp;
  unsigned int i;
  if (md == NULL)
