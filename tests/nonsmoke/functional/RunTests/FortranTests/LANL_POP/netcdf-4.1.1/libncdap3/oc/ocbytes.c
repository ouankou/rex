...
OCbytes *ocbytesnew(void) {
  OCbytes *bb = (OCbytes *)malloc(sizeof(OCbytes));
  if (bb == NULL)
    return (OCbytes *)ocbytesfail();
  bb->alloc = 0;
  bb->length = 0;
  bb->content = NULL;
  bb->nonextendible = 0;
...
int ocbytessetalloc(OCbytes *bb, unsigned int sz) {
  char *newcontent;
  if (bb == NULL)
    return ocbytesfail();
  if (sz <= 0) {
...
int ocbytessetlength(OCbytes *bb, unsigned int sz) {
  if (bb == NULL)
    return ocbytesfail();
  if (sz > bb->alloc) {
    if (!ocbytessetalloc(bb, sz))
      return ocbytesfail();
  }
  bb->length = sz;
  return TRUE;
...
int ocbytesfill(OCbytes *bb, char fill) {
  unsigned int i;
  if (bb == NULL)
    return ocbytesfail();
...
int ocbytesget(OCbytes *bb, unsigned int index) {
  if (bb == NULL)
    return -1;
  if (index >= bb->length)
    return -1;
...
int ocbytesset(OCbytes *bb, unsigned int index, char elem) {
  if (bb == NULL)
    return ocbytesfail();
  if (index >= bb->length)
    return ocbytesfail();
...
int ocbytesappend(OCbytes *bb, char elem) {
  if (bb == NULL)
    return ocbytesfail();
  /* We need space for the char + null */
  while (bb->length + 1 >= bb->alloc) {
    if (!ocbytessetalloc(bb, 0))
      return ocbytesfail();
  }
...
int ocbytescat(OCbytes *bb, char *s) {
...
int ocbytesappendn(OCbytes *bb, void *elem, unsigned int n) {
...
int ocbytesprepend(OCbytes *bb, char elem) {
...
char *ocbytesextract(OCbytes *bb) {
...
int ocbytessetcontents(OCbytes *bb, char *contents, unsigned int alloc) {
  if (bb == NULL)
    return ocbytesfail();
  ocbytesclear(bb);
  if (!bb->nonextendible && bb->content != NULL)
