 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/constraints3.c,v 1.34 2010/04/10
 *20:56:37 dmh Exp $
 *********************************************************************/
#include "ncdap3.h"
...
#ifdef NCCONSTRAINTS
static NCerror convertncconstraint3(NCDRNO *, CDFnode *, char *, char *,
                                    NCconstraint *);
#endif
static NCerror convertdapconstraint3(NCDRNO *, CDFnode *, char *, char *,
                                     NCconstraint *);
...
*/
NCerror buildconstraints3(NCDRNO *drno) {
  ...
  if (dapprojectionstring != NULL || dapselectionstring != NULL) {
    ncstat = convertdapconstraint3(drno, ddsroot, dapprojectionstring,
                                   dapselectionstring, &dapconstraint);
    if (ncstat != NC_NOERR)
      goto done;
  ...
  if (ncconstraintstring != NULL) {
    ...
    ncstat = convertncconstraint3(drno, ddsroot, ncprojectionstring,
                                  ncselectionstring, &ncconstraint);
    if (ncstat != NC_NOERR)
      goto done;
  ...
  */
...
#ifdef NCCONSTRAINTS
  ncstat =
      mergeprojections3(drno, pmerge, drno->dap.netcdfconstraint.projections);
#endif
  ...
  if (drno->dap.netcdfconstraint.selections != NULL) {
    ...
    for (i = 0; i < nclistlength(drno->dap.netcdfconstraint.selections); i++) {
      NCselection *sel =
          (NCselection *)nclistget(drno->dap.netcdfconstraint.selections, i);
...
static NCerror convertdapconstraint3(NCDRNO *drno, CDFnode *root,
                                     char *dapprojectionstring,
                                     char *dapselectionstring,
  ...
  for (i = 0; i < nclistlength(dapselections); i++) {
    ...
    for (j = 0; j < nclistlength(sel->values); j++) {
      ...
      if (value->kind == ST_VAR) {
        ncstat = matchpartialname3(nodes, value->value.var.path,
                                   &value->value.var.node);
        if (ncstat)
          goto done;
      }
    }
...
#ifdef NCCONSTRAINTS
static NCerror convertncconstraint3(NCDRNO *drno, CDFnode *ncroot,
                                    char *ncprojectionstring,
                                    char *ncselectionstring,
  ...
  if (nclistlength(ncselections) > 0) {
    oc_log(OCLOGERR, "NETCDF selection are not supported: %s",
           ncselectionstring);
    freencselections(ncselections);
    ncselections = nclistnew();
  }
...
/* Fill in:
    ...
    3. selection path
*/
static NCerror qualifyconstraints3(NCDRNO *drno, NCconstraint *constraint) {
  NCerror ncstat = NC_NOERR;
  int i;
  if (constraint != NULL) {
...
*/
static NCerror qualifyprojectionnames3(NCDRNO *drno, NCprojection *proj) {
  int i, j;
  NCerror ncstat = NC_NOERR;
...
#ifdef DEBUG
  fprintf(stderr, "qualify: %s -> ", dumpprojection1(proj));
#endif
...
static NCerror qualifyprojectionsizes3(NCDRNO *drno, NCprojection *proj) {
  int i, j;
...
static NCerror assignslices3(NCDRNO *drno, NClist *projections) {
  ...
  for (i = 0; i < nclistlength(projections); i++) {
    ...
    */
    if (p->leaf->nctype == NC_Primitive && p->leaf->etype == NC_CHAR &&
        index + 1 != rank0) {
...
*/
static NCerror completeslicing3(NCDRNO *drno, NClist *projections) {
  int i;
  NCerror ncstat = NC_NOERR;
  for (i = 0; i < nclistlength(projections); i++) {
    NCprojection *p = (NCprojection *)nclistget(projections, i);
...
static NCerror completeslicing31(NCDRNO *drno, NCprojection *proj) {
  int i, j;
  NCerror ncstat = NC_NOERR;
  NClist *segments = proj->segments;
...
static NCerror makesimpleprojection(NCDRNO *drno, NCprojection *proj) {
  int i, j;
  NCerror ncstat = NC_NOERR;
  NClist *segments = proj->segments;
...
static NCerror qualifyselectionnames3(NCDRNO *drno, NCselection *sel) {
  NCerror ncstat = NC_NOERR;
  int i;
  NClist *path = NULL;
...
static NCerror matchpartialname3(NClist *nodes, NClist *path, CDFnode **nodep) {
  int i, pathlen;
  NCerror ncstat = NC_NOERR;
  CDFnode *exact = NULL;
  ...
  */
  ...
  for (i = 0; i < nclistlength(nodes); i++) {
    ...
    if (node->nctype != NC_Sequence && node->nctype != NC_Structure &&
        node->nctype != NC_Grid && node->nctype != NC_Primitive)
      continue;
  ...
  for (i = nclistlength(matches) - 1; i >= 0; i--) { /* walk backwards */
    ...
    for (j = 0; j < pathlen; j++) {
      ...
      if (strcmp(n->name, seg) != 0) {
        match = 0;
        break;
      }
    }
    if (!match)
      nclistremove(matches, i);
    /* check for exact match */
    if (exact == NULL && startpoint == 0) {
      exact = node;
      exactcount++;
    }
  }
...
static CDFnode *subpathmatch(NClist *path, CDFnode *node, int depth) {
  ...
  if (depth >= nclistlength(path))
    return node; /* => 0..|path|-1 match */
...
static void parsepath3(char *path, NClist *segments) {
  int c;
  char *p = path;
  char *q = path;
  size_t len;
...
*/
int iscached(NCDRNO *drno, CDFnode *target, NCcachenode **cachenodep) {
  int i, j, found, index;
  NCcache *cache;
...
*/
void restrictprojection3(NCDRNO *drno, NClist *varlist, NClist *projections) {
  int i, j, len;
  ...
  if (nclistlength(varlist) == 0)
    goto done; /* nothing to add or remove */
  ...
  if (nclistlength(projections) == 0) {
    ...
    for (i = len - 1; i >= 0; i--) { /* Walk backward to facilitate removal*/
      ...
      for (j = 0; j < nclistlength(varlist); j++) {
        ...
        if (treecontains3(var, proj->leaf) || treecontains3(proj->leaf, var)) {
          intersect = 1;
          break;
        }
      }
  ...
  for (;;) {
    ...
    for (i = 0; i < nclistlength(containerset); i++) {
      ...
      for (allfound = 1, j = 0; j < nclistlength(container->subnodes); j++) {
        ...
        if (!nclistcontains(varlist, (ncelem)subnode)) {
          allfound = 0;
          break;
        }
    ...
    if (!changed)
      break; /* apparently we have reached a stable situation */
  }
...
NCprojection *cloneprojection1(NCprojection *p) {
  int i, j;
  NCprojection *newp;
  if (p == NULL)
    return NULL;
  ...
  *newp = *p;
  ...
  for (i = 0; i < nclistlength(p->segments); i++) {
    ...
    *newseg = *seg;
    ...
    for (j = 0; j < seg->slicerank; j++)
      newseg->slices[j] = seg->slices[j];
    nclistpush(newp->segments, (ncelem)newseg);
  }
  return newp;
}

NClist *cloneprojections(NClist *projectionlist) {
  int i;
...
NCselection *cloneselection1(NCselection *s) {
  int i, j;
  NCselection *news;
  if (s == NULL)
    return NULL;
...
NClist *cloneselections(NClist *selectionlist) {
  int i;
  NClist *clone = nclistnew();
...
NCconstraint cloneconstraint(NCconstraint *con) {
  NCconstraint clone;
...
NCerror slicemerge3(NCslice *dst, NCslice *src) {
...
*/
NCerror mergeprojections3(NCDRNO *drno, NClist *dst, NClist *src) {
  int i;
...
static NCerror mergeprojection31(NCDRNO *drno, NCprojection *dst,
                                 NCprojection *src) {
  NCerror ncstat = NC_NOERR;
  int i, j;
...
overlapping projections into acct.
*/
static NCerror mergesingleprojection3(NCDRNO *drno, NClist *dst,
                                      NCprojection *src) {
  NCerror stat;
...
char *makeprojectionstring3(NClist *projections) {
  int i;
...
static void makeprojectionstring13(NCprojection *p, NCbytes *buf,
                                   char *separator) {
  int i;
  for (i = 0; i < nclistlength(p->segments); i++) {
...
#endif
    if (i > 0)
      ncbytescat(buf, separator);
    ncbytescat(buf, (segment->segment ? segment->segment : "<unknown>"));
    if (segment->node->nctype == NC_Sequence)
      continue;
...
static void makeslicestring3(NCslice *slice, unsigned int rank, NCbytes *buf) {
  int i;
  ...
  for (i = 0; i < rank; i++, slice++) {
    ...
    if (slice->stride == 1) {
      snprintf(tmp, sizeof(tmp), "[%lu:%lu]", (unsigned long)slice->first,
               (unsigned long)last);
    } else {
      snprintf(tmp, sizeof(tmp), "[%lu:%lu:%lu]", (unsigned long)slice->first,
               (unsigned long)slice->stride, (unsigned long)last);
    }
    ncbytescat(buf, tmp);
  }
}
...
char *makeselectionstring3(NClist *selections) {
  int i;
  NCbytes *buf = ncbytesnew();
  char *sstring;
  for (i = 0; i < nclistlength(selections); i++) {
    ...
    for (j = 0; j < nclistlength(sel->values); j++) {
      ...
      case ST_VAR:
        ...
        break;
      default:
        PANIC1("unexpected tag: %d", (int)value->kind);
      }
    }
    if (sel->operator == ST_FCN)
      ncbytescat(buf, "(");
...
char *makeconstraintstring3(NCconstraint *con) {
  NCbytes *buf = ncbytesnew();
  char *result = NULL;
...
void freencsegment(NCsegment *seg) {
  efree(seg->segment);
  efree(seg);
}

void freencprojection1(NCprojection *p) {
  int i;
  if (p == NULL)
    return;
  for (i = 0; i < nclistlength(p->segments); i++) {
...
void freencprojections(NClist *plist) {
  int i;
  if (plist == NULL)
    return;
...
void freencselection1(NCselection *s) {
  int i;
  if (s == NULL)
    return;
  freepath(s->path);
...
void freencselections(NClist *slist) {
  int i;
  if (slist == NULL)
    return;
...
void clearncconstraint(NCconstraint *con) {
  if (con->projections)
    freencprojections(con->projections);
  if (con->selections)
    freencselections(con->selections);
  con->projections = NULL;
  con->selections = NULL;
}

NCsegment *createncsegment(void) {
  NCsegment *mem = (NCsegment *)emalloc(sizeof(NCsegment));
  memset((void *)mem, 0, sizeof(NCsegment));
  return mem;
}

NCprojection *createncprojection(void) {
  NCprojection *mem = (NCprojection *)emalloc(sizeof(NCprojection));
  memset((void *)mem, 0, sizeof(NCprojection));
  return mem;
}

NCselection *createncselection(void) {
...
void dereference3(NCconstraint *constraint) {
  int i, j;
  for (i = 0; i < nclistlength(constraint->projections); i++) {
    NCprojection *p = (NCprojection *)nclistget(constraint->projections, i);
    p->leaf = NULL;
  ...
  for (i = 0; i < nclistlength(constraint->selections); i++) {
    ...
    for (j = 0; j < nclistlength(s->values); j++) {
      ...
      if (v->kind == ST_VAR)
        v->value.var.node = NULL;
    }
  }
}

static NCerror fillsegmentpath(NCprojection *p, NClist *nodes) {
  int i;
  NCerror ncstat = NC_NOERR;
  NClist *path = nclistnew();
...
static NCerror fillselectionpath(NCselection *s, NClist *nodes) {
  int i;
  NCerror ncstat = NC_NOERR;
  NClist *path = nclistnew();
  ...
  for (i = 0; i < nclistlength(s->values); i++) {
    ...
    if (v->kind != ST_VAR)
      continue;
    ncstat = matchpartialname3(nodes, v->value.var.path, &v->value.var.node);
    if (ncstat)
      goto done;
  }
...
*/
NCerror rereference3(NCconstraint *constraint, NClist *nodeset) {
  int i;
  NCerror ncstat = NC_NOERR;
  for (i = 0; i < nclistlength(constraint->projections); i++) {
    NCprojection *p = (NCprojection *)nclistget(constraint->projections, i);
...
NCerror buildvaraprojection3(NCDRNO *drno, Getvara *getvar,
  ...
  for (i = 0; i < nclistlength(segments); i++) {
    ...
    if (segment->node->array.stringdim != NULL)
      localrank++;
    if (segment->node->array.seqdim != NULL)
      localrank++;
  ...
  nclistfree(path);
  if (ncstat)
    freencprojection1(projection);
  return ncstat;
}

/* Compute the set of prefetched data */
NCerror prefetchdata3(NCDRNO *drno) {
  int i, j;
  NCerror ncstat = NC_NOERR;
...
NCerror buildcachenode3(NCDRNO *drno, NCconstraint *constraint, NClist *varlist,
                        NCcachenode **cachep, int isprefetch) {
  NCerror ncstat = NC_NOERR;
  OCerror ocstat = OC_NOERR;
...
void clearnccache(NCDRNO *drno, NCcache *cache) {
  int i;
  if (cache == NULL)
    return;
...
#ifdef IGNORE
static NCerror verifyprojectionrank3(NCsegment *seg) {
  unsigned int rank;
  if (seg->node->array.ncdimensions != NULL)
    rank = nclistlength(seg->node->array.ncdimensions);
  else
    rank = nclistlength(seg->node->array.dimensions);
  if (rank != seg->slicerank)
    return THROW(NC_EDAPCONSTRAINT);
  return NC_NOERR;
}
#endif

int iswholeslice(NCslice *slice, CDFnode *dim) {
  if (slice->first != 0 || slice->stride != 1)
    return 0;
  if (dim != NULL) {
    if (slice->stop != dim->dim.declsize)
      return 0;
  } else if (dim == NULL) {
    if (slice->declsize == 0 || slice->count != slice->declsize)
      return 0;
  }
  return 1;
}

int iswholesegment(NCsegment *seg) {
  int i, whole;
...
int iswholeprojection(NCprojection *proj) {
  int i, whole;
...
void makewholeslice3(NCslice *slice, CDFnode *dim) {
  slice->first = 0;
  slice->stride = 1;
  slice->length = dim->dim.declsize;
  slice->stop = dim->dim.declsize;
...
void makewholesegment3(NCsegment *seg, CDFnode *node) {
  int i;
  NClist *dimset = NULL;
  unsigned int rank;
