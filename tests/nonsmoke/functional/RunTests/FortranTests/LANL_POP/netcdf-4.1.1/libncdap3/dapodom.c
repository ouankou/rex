 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/dapodom.c,v 1.11 2009/11/14
 *22:33:20 dmh Exp $
 *********************************************************************/
...
Dapodometer *newdapodometer(NCslice *slices, unsigned int first,
                            unsigned int rank) {
  int i;
...
Dapodometer *newsimpledapodometer(NCsegment *segment, unsigned int rank) {
  int i;
  Dapodometer *odom = (Dapodometer *)emalloc(sizeof(Dapodometer));
  MEMCHECK(odom, NULL);
  odom->rank = rank;
...
Dapodometer *newdapodometer1(unsigned int count) {
  Dapodometer *odom = (Dapodometer *)emalloc(sizeof(Dapodometer));
  MEMCHECK(odom, NULL);
  odom->rank = 1;
...
void freedapodometer(Dapodometer *odom) {
  if (odom)
    free(odom);
}

char *dapodometerprint(Dapodometer *odom) {
  int i;
  ...
  if (odom->rank == 0) {
    ...
    for (i = 0; i < odom->rank; i++) {
      sprintf(tmp, "[%lu/%lu:%lu:%lu]", (unsigned long)odom->index[i],
...
int dapodometermore(Dapodometer *odom) {
  return (odom->index[0] < odom->slices[0].stop);
}

void dapodometerreset(Dapodometer *odom) {
  int rank = odom->rank;
  while (rank-- > 0) {
    odom->index[rank] = odom->slices[rank].first;
  }
}

/* Convert current dapodometer settings to a single integer count*/
size_t dapodometercount(Dapodometer *odom) {
  int i;
  size_t offset = 0;
  for (i = 0; i < odom->rank; i++) {
...
size_t dapodometerspace(Dapodometer *odom, unsigned int wheel) {
...
size_t dapodometerpoints(Dapodometer *odom) {
  unsigned int i, rank = odom->rank;
  size_t count = 1;
...
*/
Dapodometer *dapodometersplit(Dapodometer *odom, int tail) {
  int i;
  ...
  for (i = 0; i < tail; i++) {
    split->slices[i] = odom->slices[odom->rank + i];
  }
  return split;
}
#endif

int dapodometerincr(Dapodometer *odom) { return dapodometerincrith(odom, -1); }

int dapodometerincrith(Dapodometer *odom, int wheel) {
  ...
  for (slice = odom->slices + (wheel), i = wheel; i >= 0; i--, slice--) {
    ...
    if (odom->index[i] < slice->stop)
      break;
    if (i == 0)
      return 0;                    /* leave the 0th entry if it overflows*/
...
/**************************************************/
int dapodometervarmcount(Dapodometer *odom, const ptrdiff_t *steps,
                         const size_t *declsizes) {
  int i;
  size_t offset = 0;
  for (i = 0; i < odom->rank; i++) {
    size_t tmp;
...
size_t *dapodometerindices(Dapodometer *odom) {
  if (odom == NULL)
    return NULL;
  return odom->index;
}

Dapodometer *newdapodometer2(const size_t *start, const size_t *count,
                             const ptrdiff_t *stride, unsigned int first,
                             unsigned int rank) {
  int i;
  Dapodometer *odom = (Dapodometer *)emalloc(sizeof(Dapodometer));
  MEMCHECK(odom, NULL);
...
Dapodometer *newdapodometer3(int rank, size_t *dimsizes) {
  int i;
  Dapodometer *odom = (Dapodometer *)emalloc(sizeof(Dapodometer));
  MEMCHECK(odom, NULL);
  odom->rank = rank;
