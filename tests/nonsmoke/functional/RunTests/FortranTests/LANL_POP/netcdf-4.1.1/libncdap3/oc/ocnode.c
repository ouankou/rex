...
void computeocsemantics(OClist *ocnodes) {
  unsigned int i;
...
void computeocfullnames(OCnode *root) {
  unsigned int i;
  if (root->name != NULL)
    computefullname(root);
...
static char *pathtostring(OClist *path, char *separator, int usecdfname) {
  int slen, i, len;
  char *pathname;
  if (path == NULL || (len = oclistlength(path)) == 0)
    return NULL;
  for (slen = 0, i = 0; i < len; i++) {
    OCnode *node = (OCnode *)oclistget(path, i);
    if (node->container == NULL || node->name == NULL)
      continue;
...
void collectpathtonode(OCnode *node, OClist *path) {
  if (node == NULL)
    return;
...
OCnode *makeocnode(char *name, OCtype ptype, OCnode *root) {
...
OCattribute *makeattribute(char *name, OCtype ptype, OClist *values) {
  OCattribute *att =
      (OCattribute *)ocmalloc(sizeof(OCattribute)); /* ocmalloc zeros*/
...
static void marklostattribute(OCnode *att) {
  oc_log(LOGWARN, "Lost attribute: %s", att->name);
}

void *oclinearize(OCtype etype, unsigned int nstrings, char **strings) {
  int i;
  size_t typesize;
  char *memp;
  char *memory;
...
static int converttype(OCtype etype, char *value, char *memory) {
  ...
  case OC_Char:
    ...
    *((char *)memory) = c[0];
    break;
  case OC_Byte:
    if (sscanf(value, "%ld", &iv) != 1)
      goto fail;
    else if (iv > OC_BYTE_MAX || iv < OC_BYTE_MIN) {
      iv = OC_BYTE_MAX;
      outofrange = 1;
    }
    *((signed char *)memory) = (signed char)iv;
    break;
  case OC_UByte:
    if (sscanf(value, "%lu", &uiv) != 1)
      goto fail;
    else if (uiv > OC_UBYTE_MAX) {
      uiv = OC_UBYTE_MAX;
      outofrange = 1;
    }
  ...
  case OC_Int16:
    if (sscanf(value, "%ld", &iv) != 1)
      goto fail;
    else if (iv > OC_INT16_MAX || iv < OC_INT16_MIN) {
      iv = OC_INT16_MAX;
      outofrange = 1;
    }
    *((signed short *)memory) = (signed short)iv;
    break;
  case OC_UInt16:
    if (sscanf(value, "%ld", &uiv) != 1)
      goto fail;
    else if (uiv > OC_UINT16_MAX) {
      uiv = OC_UINT16_MAX;
      outofrange = 1;
    }
  ...
  case OC_Int32:
    if (sscanf(value, "%ld", &iv) != 1)
      goto fail;
    else if (iv > OC_INT32_MAX || iv < OC_INT32_MIN) {
      iv = OC_INT32_MAX;
      outofrange = 1;
    }
...
size_t ocsetsize(OCnode *node) {
  ...
  if (node->subnodes != NULL) {
    ...
    for (i = 0; i < oclistlength(node->subnodes); i++) {
      ...
      size_t subsize =
          ocsetsize(subnode); /* includes subnode dimension counts*/
      if (subsize == 0)
        nonuniform = 1;
      subnodesum += subsize;
    }
  ...
  case OC_Primitive:
    ...
    case OC_Char:
      ...
      /* We have to watch out for the fact that packed instances have padding in
       * the xdr packet*/
      if (!isscalar) { /* padding to multiple of BYTE_PER_XDR_UNIT*/
        ...
        if (rem > 0)
          dimsize += (BYTES_PER_XDR_UNIT - rem);
  ...
  default:
    OCPANIC1("ocmap: encountered unexpected node type: %x", node->octype);
    break;
  }
...
void ocfreetree(OCtree *tree) {
  ...
  if (tree->dxdclass == OCDATA) {
    ...
    if (tree->data.file != NULL)
      fclose(tree->data.file);
  }
#else
...
void ocfreenodes(OClist *nodes) {
  ...
  for (i = 0; i < oclistlength(nodes); i++) {
    ...
    if (node->array.dimensions != NULL)
      oclistfree(node->array.dimensions);
    if (node->subnodes != NULL)
      oclistfree(node->subnodes);
    if (node->att.values != NULL)
      oclistfree(node->att.values);
    if (node->attributes != NULL)
      oclistfree(node->attributes);
    ocfree(node);
  }
  oclistfree(nodes);
}
...
int ocddsdasmerge(OCstate *state, OCnode *dasroot, OCnode *ddsroot) {
  ...
  for (i = 0; i < oclistlength(dasroot->tree->nodes); i++) {
    ...
    for (j = 0; j < oclistlength(das->subnodes); j++) {
      ...
      if (subnode->octype == OC_Attribute) {
        hasattributes = 1;
        break;
      }
    }
    if (hasattributes) {
      ...
      for (j = 0; j < oclistlength(dasnodes); j++) {
        ...
        if (das->name == NULL || das2->name == NULL)
          continue;
        if (strcmp(das->name, das2->name) == 0) {
          oc_log(LOGWARN, "oc_mergedas: potentially ambiguous DAS name: %s",
                 das->name);
        }
      }
  ...
  for (i = 0; i < oclistlength(ddsnodes); i++) {
    ...
    if (dds->octype == OC_Primitive)
      oclistpush(varnodes, (ocelem)dds);
  }
...
static int mergedas1(OCnode *dds, OCnode *das) {
  ...
  int stat = OC_NOERR;
  if (das == NULL)
    return OC_NOERR; /* nothing to do */
  if (dds->attributes == NULL)
    dds->attributes = oclistnew();
  ...
  for (i = 0; i < oclistlength(das->subnodes); i++) {
    ...
    if (attnode->octype == OC_Attribute) {
      OCattribute *att =
          makeattribute(attnode->name, attnode->etype, attnode->att.values);
...
static void ocuncorrelate(OCnode *root) {
...
static OCerror occorrelater(OCnode *dds, OCnode *dxd) {
  int i, j;
  OCerror ocstat = OC_NOERR;

  if (dds->octype != dxd->octype) {
    THROWCHK((ocstat = OC_EINVAL));
    goto fail;
  }
  ...
  case OC_Primitive:
    break;
  default:
    OCPANIC1("unexpected node type: %d", dds->octype);
  }
...
OCerror occorrelate(OCnode *dds, OCnode *dxd) {
  if (dds == NULL || dxd == NULL)
    return OC_EINVAL;
  ocuncorrelate(dds);
