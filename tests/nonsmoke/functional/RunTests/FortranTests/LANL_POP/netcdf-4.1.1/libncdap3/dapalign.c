 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/dapalign.c,v 1.5 2009/09/23
 *22:26:00 dmh Exp $
 *********************************************************************/
...
static char *ncctypenames[NCCTYPECOUNT] = {
    (char *)NULL,     "char",      "unsigned char",      "short",
    "unsigned short", "int",       "unsigned int",       "long",
    "unsigned long",  "long long", "unsigned long long", "float",
    "double",         "void*",     "alignvlen_t"};
#endif
...
unsigned int ncctypealignment(int nctype) {
  ...
  case NC_UINT:
    ...
    break;
  case NC_INT64:
    index = NCCTYPELONGLONG;
    break;
  case NC_UINT64:
    index = NCCTYPEULONGLONG;
    break;
  case NC_STRING:
    index = NCCTYPEPTR;
    break;
...
void compute_nccalignments(void) {
...
/* Compute padding */
int nccpadding(unsigned long offset, int alignment) {
  int pad, rem;
...
static void verify(NCtypealignvec *vec) {
  int i, j;
  ...
  for (i = 0; i < NCCTYPECOUNT; i++) {
    printf("%s: size=%2d  alignment=%2d\n", padname(vec[i].typename), sizes8[i],
           vec[i].alignment);
  }
  for (i = 0; i < NCCTYPECOUNT; i++) {
    printf("short vs %s: size=%2d  alignment=%2d\n", padname(vec[i].typename),
           sizes16[i], vec16[i].alignment);
  }
  for (i = 0; i < NCCTYPECOUNT; i++) {
    printf("int vs %s: size=%2d  alignment=%2d\n", padname(vec[i].typename),
           sizes32[i], vec32[i].alignment);
  }
}
