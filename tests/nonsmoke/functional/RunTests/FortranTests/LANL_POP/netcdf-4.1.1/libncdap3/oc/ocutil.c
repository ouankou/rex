...
char *ocstrndup(const char *s, size_t len) {
  char *dup;
  if (s == NULL)
    return NULL;
...
void ocfreeprojectionclause(OCprojectionclause *clause) {
...
static void freeAttributes(OClist *attset) {
  unsigned int i, j;
...
void freeOCnode(OCnode *cdf, int deep) {
  unsigned int i;
  if (cdf == NULL)
    return;
  if (cdf->name != NULL)
    free(cdf->name);
  if (cdf->fullname != NULL)
    free(cdf->fullname);
  if (cdf->attributes != NULL)
    freeAttributes(cdf->attributes);
...
int findbod(OCbytes *buffer, size_t *bodp, size_t *ddslenp) {
  unsigned int i;
  char *content;
...
size_t totaldimsize(OCnode *node) {
  unsigned int i;
  size_t count = 1;
...
char *octypetostring(OCtype octype) {
  switch (octype) {
  case OC_NAT:
    return "OC_NAT";
  case OC_Char:
...
char *octypetoddsstring(OCtype octype) {
  switch (octype) {
  case OC_Byte:
    return "Byte";
  case OC_Int16:
...
OCerror octypeprint(OCtype etype, char *buf, size_t bufsize, void *value) {
  if (buf == NULL || bufsize == 0 || value == NULL)
    return OC_EINVAL;
  buf[0] = '\0';
  switch (etype) {
  case OC_Char:
...
int xdr_skip(XDR *xdrs, unsigned int len) {
  unsigned int pos;
  if (len <= 0)
    return 1; /* ignore*/
  pos = xdr_getpos(xdrs);
...
int xdr_skip_strings(XDR *xdrs, unsigned int n) {
  while (n-- > 0) {
    unsigned int slen;
    if (!xdr_u_int(xdrs, &slen))
      return xdrerror();
    if (xdr_skip(xdrs, RNDUP(slen)))
      return xdrerror();
  }
...
unsigned int xdr_roundup(unsigned int n) {
  ...
  if (rem > 0)
    n += (BYTES_PER_XDR_UNIT - rem);
  return n;
}

unsigned int ocbyteswap(unsigned int i) {
...
OCerror ocsvcerrordata(OCstate *state, char **codep, char **msgp, long *httpp) {
  if (codep)
    *codep = state->error.code;
  if (msgp)
    *msgp = state->error.message;
  if (httpp)
    *httpp = state->error.httpcode;
  return OC_NOERR;
}
...
*/
void ocdataddsmsg(OCstate *state, OCtree *tree) {
...
#define ERRTAG "Error {"
  ...
  for (p = chunk, i = 0; i < (count - strlen(ERRTAG)); i++, p++) {
    if (strncmp(p, ERRTAG, strlen(ERRTAG)) == 0) {
      ...
      oc_log(LOGERR, "DATADDS failure, possible message: '%s'", chunk + i);
      return;
    }
  }
}
