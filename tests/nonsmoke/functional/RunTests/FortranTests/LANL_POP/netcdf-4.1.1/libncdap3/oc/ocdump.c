...
/* support [dd] leader*/
static char *dent2(int n) { return dent(n + 4); }

void ocdumpnode(OCnode *node) {
  if (node != NULL) {
    dumpocnode1(node, 0);
  } else {
    fprintf(stderr, "<NULL>\n");
...
static void dumpocnode1(OCnode *node, int depth) {
  ...
  case OC_Structure: {
    fprintf(stderr, "[%2d]%s ", depth, dent(depth));
    fprintf(stderr, "struct %s", (node->name ? node->name : ""));
    dumpdimensions(node);
    fprintf(stderr, " @%lx", (unsigned long)node);
    fprintf(stderr, "\n");
    for (n = 0; n < oclistlength(node->subnodes); n++) {
  ...
  case OC_Sequence: {
    fprintf(stderr, "[%2d]%s ", depth, dent(depth));
    fprintf(stderr, "sequence %s", (node->name ? node->name : ""));
    dumpdimensions(node);
    fprintf(stderr, " @%lx", (unsigned long)node);
    fprintf(stderr, "\n");
    for (n = 0; n < oclistlength(node->subnodes); n++) {
  ...
  case OC_Grid: {
    unsigned int i;
    fprintf(stderr, "[%2d]%s ", depth, dent(depth));
    fprintf(stderr, "grid %s", (node->name ? node->name : ""));
    dumpdimensions(node);
    fprintf(stderr, " @%lx", (unsigned long)node);
    fprintf(stderr, "\n");
  ...
  case OC_Attribute: {
    fprintf(stderr, "[%2d]%s ", depth, dent(depth));
    if (node->name == NULL)
      OCPANIC("Attribute without name");
    fprintf(stderr, "%s %s", octypetostring(node->etype), node->name);
...
static void dumpdimensions(OCnode *node) {
  ...
  for (i = 0; i < node->array.rank; i++) {
    ...
    fprintf(stderr, "[%s=%lu]", (dim->name ? dim->name : "?"),
            (unsigned long)dim->dim.declsize);
  }
}

static void dumpattvalue(OCtype nctype, char **strings, int index) {
...
void ocdumpslice(OCslice *slice) {
  ...
  if (slice->stride > 1)
    fprintf(stderr, ":%lu", (unsigned long)slice->stride);
  fprintf(stderr, ":%lu", (unsigned long)(slice->first + slice->count) - 1);
  fprintf(stderr, "]");
}

void ocdumpclause(OCprojectionclause *ref) {
  unsigned int i;
  ...
  for (i = 0; i < oclistlength(path); i++) {
    ...
    if (node->tree != NULL)
      continue; /* leave off the root node*/
...
static void dumpmem2(char *s, char *accum, int align) {
  int len, rem;
  strcat(accum, "|");
  strcat(accum, s);
  len = strlen(s);
...
static void dumpmem1(int index, unsigned int n, unsigned int n1) {
  int i;
  char s[1024];
  char tmp[32];
  union {
...
static void dumpmemory0(char *memory, int len, int fromxdr, int bod) {
...
void ocdumppacket(char *memory, int len, int bod) {
  dumpmemory0(memory, len, 1, bod);
}

void ocdumpmemory(char *memory, int len) { dumpmemory0(memory, len, 0, 0); }

void ocdumpfile(FILE *file, int datastart) {
  int i, count, rem, len;
  long pos;
  char dds[4096];
  char hdr[1024];
...
void ocdumpmemdata(OCmemdata *memdata, OCbytes *buf) {
  if (memdata == NULL || buf == NULL)
    return;
  ocdumpmemdata1(memdata, buf, 0);
}

static char *ocmodestr(OCmode mode) {
  switch (mode) {
  case Emptymode:
    return "Empty";
  case Nullmode:
...
static void ocdumpmemdata1(OCmemdata *md, OCbytes *buf, int depth) {
  OCmemdata **mdp;
  unsigned int i;
  char tmp[1024];
  ...
  case OC_Dataset:
    sprintf(tmp, "%s%s/%s (%lu) {\n", dent(depth),
            octypetostring((OCtype)md->octype), ocmodestr(md->mode),
  ...
  case OC_Primitive: {
    ...
    sprintf(tmp, "%s%s/%s (%lu) {", dent(depth), octypetostring(etype),
            ocmodestr(md->mode), (unsigned long)md->count);
    ocbytescat(buf, tmp);
    for (i = 0; i < md->count; i++) {
...
void ocdd(OCstate *state, OCnode *root) {
