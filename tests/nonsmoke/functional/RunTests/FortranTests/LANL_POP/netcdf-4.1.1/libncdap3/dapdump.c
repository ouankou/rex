 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/dapdump.c,v 1.18 2010/04/10
 *20:56:37 dmh Exp $
 *********************************************************************/
#include "config.h"
...
int dumpmetadata(int ncid, NChdr **hdrp) {
  int stat, i, j, k;
  ...
  stat = RENAME3(inq)(hdr->ncid, &hdr->ndims, &hdr->nvars, &hdr->ngatts,
                      &hdr->unlimid);
  CHECK(stat);
  if (ncdap3debug > 0) {
  ...
  for (i = 0; i < hdr->ngatts; i++) {
    ...
    fprintf(stdout, "\t[%d]: name=%s type=%s values(%lu)=", i, att->name,
            nctypetostring(octypetonc(att->etype)), (unsigned long)nvalues);
    if (nctype == NC_CHAR) {
  ...
  for (i = 0; i < hdr->ndims; i++) {
    hdr->dims[i].dimid = i;
    stat = RENAME3(inq_dim)(hdr->ncid, hdr->dims[i].dimid, hdr->dims[i].name,
                            &hdr->dims[i].size);
    CHECK(stat);
    fprintf(stdout, "dim[%d]: name=%s size=%lu\n", i, hdr->dims[i].name,
            (unsigned long)hdr->dims[i].size);
  }
  ...
  for (i = 0; i < hdr->nvars; i++) {
    ...
    nc_type nctype;
    var->varid = i;
    stat = RENAME3(inq_var)(hdr->ncid, var->varid, var->name, &nctype,
                            &var->ndims, var->dimids, &var->natts);
    CHECK(stat);
    var->nctype = (nctype);
    fprintf(stdout, "var[%d]: name=%s type=%s |dims|=%d", i, var->name,
            nctypetostring(var->nctype), var->ndims);
    fprintf(stdout, " dims={");
    ...
    for (j = 0; j < var->natts; j++) {
      ...
      att->name = nulldup(attname);
      stat =
          RENAME3(inq_att)(hdr->ncid, var->varid, att->name, &nctype, &nvalues);
      CHECK(stat);
      att->etype = nctypetodap(nctype);
      ...
      CHECK(stat);
      fprintf(stdout, "\tattr[%d]: name=%s type=%s values(%lu)=", j, att->name,
              nctypetostring(octypetonc(att->etype)), (unsigned long)nvalues);
      for (k = 0; k < nvalues; k++) {
        fprintf(stdout, " ");
        dumpdata1(octypetonc(att->etype), k, values);
...
void dumpdata1(nc_type nctype, size_t index, char *data) {
  switch (nctype) {
  case NC_CHAR:
...
*/
char *dumpprojections(NClist *projections) {
  int i;
...
char *dumpprojection1(NCprojection *p) {
  int i;
  NCbytes *buf;
  char *pstring;
  if (p == NULL)
  ...
  for (i = 0; i < nclistlength(p->segments); i++) {
    ...
    char tmp[1024];
    snprintf(tmp, sizeof(tmp), "%s%s/%lu", (i > 0 ? "." : ""),
  ...
  if (iswholeprojection(p))
    ncbytescat(buf, "*");
  ncbytescat(buf, "(");
  if (p->leaf != NULL)
    ncbytescat(buf, p->leaf->name);
  ncbytescat(buf, ")");
  pstring = ncbytesdup(buf);
  ncbytesfree(buf);
  return pstring;
...
char *dumpselections(NClist *selections) {
  int i;
  NCbytes *buf = ncbytesnew();
  char *sstring;
  if (nclistlength(selections) == 0)
    return nulldup("");
...
char *dumpselection1(NCselection *sel) {
  NCbytes *buf = ncbytesnew();
  char *sstring;
  NClist *path = NULL;
...
char *dumpconstraint(NCconstraint *con) {
  NCbytes *buf = ncbytesnew();
  char *result = NULL;
...
char *dumpsegments(NClist *segments) {
  int i;
  NCbytes *buf = ncbytesnew();
  char *sstring;
...
char *dumpsegment(NCsegment *segment) {
  NCbytes *buf;
  char *result;
  if (segment == NULL)
    return nulldup("(nullsegment)");
  buf = ncbytesnew();
...
static void dumpindent(int indent, NCbytes *buf) {
  static char *indentstr = "  ";
  int i;
  for (i = 0; i < indent; i++)
    ncbytescat(buf, indentstr);
}

static void dumptreer(CDFnode *root, NCbytes *buf, int indent, int visible);

static void dumptreer1(CDFnode *root, NCbytes *buf, int indent, char *tag,
                       int visible) {
  int i;
...
char *dumpalign(NCalignment *ncalign) {
  char *result;
  char tmp[1024];
  if (ncalign == NULL)
...
char *dumpcachenode(NCcachenode *node) {
  char *result = NULL;
  char tmp[8192];
  int i;
  NCbytes *buf;
...
char *dumpcache(NCcache *cache) {
  ...
  if (cache == NULL)
    return strdup("cache{null}");
  buf = ncbytesnew();
  snprintf(tmp, sizeof(tmp), "cache{limit=%lu; size=%lu;\n",
           (unsigned long)cache->cachelimit, (unsigned long)cache->cachesize);
  ncbytescat(buf, tmp);
  if (cache->prefetch) {
...
char *dumpslices(NCslice *slice, unsigned int rank) {
  int i;
  NCbytes *buf;
  char *result = NULL;
