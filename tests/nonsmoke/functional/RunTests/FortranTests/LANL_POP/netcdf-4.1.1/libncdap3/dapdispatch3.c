 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/dapdispatch3.c,v 1.19 2009/11/29
 *00:16:26 dmh Exp $
 *********************************************************************/
#include "config.h"
...
extern int nc3d_getvarx(int ncid, int varid, const size_t *startp,
                        const size_t *countp, const ptrdiff_t *stridep,
                        void *value, nc_type dsttype);

/* Forward*/
static int nc3d_getvar1(int ncid, int varid, const size_t *, void *, nc_type);

static int nc3d_getvars(int ncid, int varid, const size_t *start,
...
static void setnc3dispatch(int ncid) {
  NC *ncp;
...
int RENAME3(_open_mp)(const char *path, int mode, int basepe,
...
int nc3d_redef(int ncid) { return THROW(NC_EPERM); }

int nc3d__enddef(int ncid, size_t h_minfree, size_t v_align, size_t v_minfree,
                 size_t r_align) {
  return THROW(NC_EPERM);
}

int nc3d_enddef(int ncid) { return THROW(NC_EPERM); }

int nc3d_sync(int ncid) { return THROW(NC_EINVAL); }

int nc3d_abort(int ncid) { return THROW(NC_NOERR); }
...
int nc3d_get_var1(int ncid, int varid, const size_t *indexp, void *value) {
  return THROW(nc3d_getvar1(ncid, varid, indexp, value, NC_NAT));
}

int nc3d_get_vara(int ncid, int varid, const size_t *start, const size_t *count,
                  void *value) {
  return THROW(nc3d_getvara(ncid, varid, start, count, value, NC_NAT));
}

int nc3d_get_vars(int ncid, int varid, const size_t *start, const size_t *count,
                  const ptrdiff_t *stride, void *value) {
  return THROW(nc3d_getvars(ncid, varid, start, count, stride, value, NC_NAT));
}

int nc3d_get_varm(int ncid, int varid, const size_t *start, const size_t *count,
                  const ptrdiff_t *stride, const ptrdiff_t *imapp,
                  void *value) {
  return THROW(
      nc3d_getvarmx(ncid, varid, start, count, stride, imapp, value, NC_NAT));
}
...
#define DEFGETVARA(T1, T2, T3)                                                 \
  int nc3d_get_vara_##T1(int ncid, int varid, const size_t *start,             \
                         const size_t *count, T2 *ip) {                        \
...
#define DEFGETVARS(T1, T2, T3)                                                 \
  int nc3d_get_vars_##T1(int ncid, int varid, const size_t *start,             \
                         const size_t *count, const ptrdiff_t *stridep,        \
                         T2 *ip) {                                             \
...
#define DEFGETVARM(T1, T2, T3)                                                 \
  int nc3d_get_varm_##T1(int ncid, int varid, const size_t *start,             \
                         const size_t *count, const ptrdiff_t *stridep,        \
                         const ptrdiff_t *imapp, T2 *ip) {                     \
    return THROW(                                                              \
        nc3d_getvarmx(ncid, varid, start, count, stridep, imapp, ip, T3));     \
  }
...
static int nc3d_getvar1(int ncid, int varid, const size_t *indexp, void *value,
                        nc_type dsttype) {
  /* utilize the more general varx routine*/
  return THROW(nc3d_getvarx(ncid, varid, indexp, dapsinglecount3,
                            dapsinglestride3, value, dsttype));
}

int nc3d_getvara(int ncid, int varid, const size_t *start, const size_t *count,
                 void *value, nc_type dsttype) {
  /* utilize the more general varx routine*/
  return THROW(nc3d_getvarx(ncid, varid, start, count, dapsinglestride3, value,
                            dsttype));
}
...
int nc3d_getvar(int ncid, int varid, void *value, nc_type dsttype) {
  /* utilize the more general varx routine*/
  /* Use null count to force varx to compute total # of elements */
  return THROW(nc3d_getvarx(ncid, varid, dapzerostart3, NULL, dapsinglestride3,
                            value, dsttype));
}
...
int RENAME3(_create_mp)(const char *path, int cmode, size_t initialsz,
                        int basepe, size_t *chunksizehintp, int *ncidp) {
  int stat;
  stat = RENAMEDAP(_create_mp)(path, cmode, initialsz, basepe, chunksizehintp,
                               ncidp);
  if (stat == NC_NOERR)
    setnc3dispatch(*ncidp);
  return THROW(stat);
...
int nc3d__open_mp(const char *path, int mode, int basepe,
                  size_t *chunksizehintp, int *ncidp) {
...
int nc3d_def_dim(int ncid, const char *name, size_t len, int *idp) {
  return RENAME3(def_dim)(ncid, name, len, idp);
}

int nc3d_put_att(int ncid, int varid, const char *name, nc_type datatype,
                 size_t len, const void *value) {
  return RENAME3(put_att)(ncid, varid, name, datatype, len, value);
}

int nc3d_put_att_text(int ncid, int varid, const char *name, size_t len,
                      const char *op) {
  return RENAME3(put_att_text)(ncid, varid, name, len, op);
}

int nc3d_def_var(int ncid, const char *name, nc_type xtype, int ndims,
...
#define ULONGCAST                                                              \
  (sizeof(unsigned long) == sizeof(unsigned int) ? NC_UINT : NC_UINT64)
#else
#define LONGCAST (sizeof(long) == sizeof(int) ? NC_INT : NC_INT64)
#define ULONGCAST                                                              \
  (sizeof(unsigned long) == sizeof(unsigned int) ? NC_UINT : NC_UINT64)
...
int nc3d_get_var1_long(int ncid, int varid, const size_t *indexp, long *ip) {
  return THROW(nc3d_getvar1(ncid, varid, indexp, ip, LONGCAST));
}

int nc3d_get_vara_long(int ncid, int varid, const size_t *start,
                       const size_t *count, long *ip) {
  return THROW(nc3d_getvara(ncid, varid, start, count, ip, LONGCAST));
}

int nc3d_get_vars_long(int ncid, int varid, const size_t *start,
                       const size_t *count, const ptrdiff_t *stridep,
                       long *ip) {
  return THROW(nc3d_getvars(ncid, varid, start, count, stridep, ip, LONGCAST));
}

int nc3d_get_varm_long(int ncid, int varid, const size_t *start,
                       const size_t *count, const ptrdiff_t *stridep,
                       const ptrdiff_t *imapp, long *ip) {
  return THROW(
      nc3d_getvarmx(ncid, varid, start, count, stridep, imapp, ip, LONGCAST));
}

int nc3d_get_var_long(int ncid, int varid, long *ip) {
...
int nc3d_get_varm_long(int ncid, int varid, const size_t *start,
                       ...
                       const ptrdiff_t *imapp, long *ip) {
  return THROW(
      nc3d_getvarmx(ncid, varid, start, count, stridep, imapp, ip, ULONGCAST));
}
