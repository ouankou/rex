...
OCcontent *ocnewcontent(OCstate *state) {
  OCcontent *content;
  if (state == NULL)
    return NULL;
  content = state->contentlist;
  /* Search for an unused content node*/
  while (content != NULL && content->mode != Emptymode) {
...
void ocfreecontent(OCstate *state, OCcontent *content) {
  if (content != NULL) {
    content->mode = Emptymode;
  }
}

OCcontent *ocresetcontent(struct OCstate *state, OCcontent *content) {
  content->state = state;
...
OCcontent *occlonecontent(OCstate *state, OCcontent *content) {
...
int ocrootcontent(OCstate *state, OCnode *root, OCcontent *content) {
  OCtree *tree;
...
int ocarraycontent(OCstate *state, OCcontent *content, OCcontent *newcontent,
                   size_t index) {
  unsigned int i;
  int stat = OC_NOERR;
  XDR *xdrs;
  ...
  if (state == NULL || content == NULL)
    return THROW(OC_EINVAL);
  if (content->mode != Dimmode)
    return THROW(OC_EINVAL);
  if (content->node->array.rank == 0)
    return THROW(OC_EINVAL);
  ...
  if (!xdr_setpos(xdrs, content->xdrpos.offset))
    return xdrerror();

  /* Collect the dimension count from the xdr data packet*/
  if (!xdr_u_int(xdrs, &xdrcount))
    return xdrerror();
  if (xdrcount < index)
    return THROW(OC_ENODATA);
...
int ocrecordcontent(OCstate *state, OCcontent *content,
                    OCcontent *recordcontent, size_t index) {
  unsigned int i;
  int stat = OC_NOERR;
  XDR *xdrs;
  ...
  case OC_Sequence:
    for (i = 0; i < index; i++) {
      ...
      if (!xdr_opaque(xdrs, tmp, sizeof(tmp)))
        return xdrerror();
...
*/
int ocfieldcontent(OCstate *state, OCcontent *content, OCcontent *fieldcontent,
                   size_t index) {
  unsigned int i;
  int stat = OC_NOERR;
  XDR *xdrs;
  ...
  case OC_Dataset:
  case OC_Structure:
    if (index >= oclistlength(content->node->subnodes))
      return THROW(OC_EINVALCOORDS);
    for (i = 0; i < index; i++) {
...
int ocgetcontent(OCstate *state, OCcontent *content, void *memory,
...
#endif
  stat = ocxdrread(xdrs, (char *)memory, memsize, packed, content->node->etype,
                   start, count);
  if (!xdr_setpos(xdrs, content->xdrpos.offset))
    return xdrerror(); /* restore location*/
done:
  return THROW(stat);
}

static int ocgetmemdata(OCstate *state, OCcontent *content, void *memory,
                        size_t memsize, size_t start, size_t count) {
  OCmemdata *md = content->memdata;
  unsigned short *d16;
  unsigned int *d32;
  ...
  default:
    OCPANIC1("unexpected etype: %d", content->node->etype);
  }
  return THROW(OC_NOERR);
}

size_t ocfieldcount(OCstate *state, OCcontent *content) {
...
size_t ocarraycount(OCstate *state, OCcontent *content) {
  unsigned int count;
...
size_t ocrecordcount(OCstate *state, OCcontent *content) {
  int stat = OC_NOERR;
  size_t count;
  OCnode *node = content->node;
  XDR *xdrs;
...
static OCmode modetransition(OCnode *node, OCmode srcmode) {
  switch (srcmode) {
  case Dimmode:
    if (node->octype == OC_Sequence)
      return Recordmode;
...
OCmode occontentmode(OCstate *conn, OCcontent *content) {
  return ((content)->mode);
}

OCnode *occontentnode(OCstate *conn, OCcontent *content) {
  return ((content)->node);
}

size_t occontentindex(OCstate *conn, OCcontent *content) {
