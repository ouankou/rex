 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/daputil.c,v 1.42 2010/04/10
 *20:56:38 dmh Exp $
 *********************************************************************/
#include "config.h"
...
char *cdflegalname3(char *dapname) {
#ifndef IGNORE
...
*/
nc_type nctypeconvert(NCDRNO *drno, nc_type nctype) {
...
nc_type octypetonc(OCtype etype) {
  switch (etype) {
  case OC_Char:
    return NC_CHAR;
  case OC_Byte:
...
OCtype nctypetodap(nc_type nctype) {
  switch (nctype) {
  case NC_CHAR:
    return OC_Char;
  case NC_BYTE:
...
size_t nctypesizeof(nc_type nctype) {
  switch (nctype) {
  case NC_CHAR:
    return sizeof(char);
  case NC_BYTE:
...
char *nctypetostring(nc_type nctype) {
  switch (nctype) {
  case NC_NAT:
    return "NC_NAT";
  case NC_BYTE:
...
*/
void collectnode0path3(CDFnode *node, NClist *path, int withdataset) {
  /* Move to dds0 space */
...
void collectnodepath3(CDFnode *node, NClist *path, int withdataset) {
  if (node == NULL)
    return;
  /* stop at the dataset container as well*/
  if (node->nctype != NC_Dataset)
    collectnodepath3(node->container, path, withdataset);
...
void dividepath(NClist *path, NClist *prefix) {
  int i, divide;
...
/* Pad a buffer */
int alignbuffer3(NCbytes *buf, int alignment) {
  int pad;
  unsigned long len;
  if (buf == NULL)
    return 0;
...
size_t dimproduct3(NClist *dimensions) {
  size_t size = 1;
  unsigned int i;
  if (dimensions == NULL)
    return size;
...
*/
int paramcheck34(NCDRNO *drno, const char *param, const char *substring) {
  const char *value;
  const char *sh;
  unsigned int splen;
  if (drno == NULL || param == NULL)
    ;
  ...
  for (sh = value; *sh; sh++) {
    if (strncmp(sh, substring, splen) == 0) {
      ...
      if (sh == value || strchr(checkseps, cpre) != NULL)
        match++;
      if (cpost == '\0' || strchr(checkseps, cpost) != NULL)
        match++;
      if (match == 2)
        return 1;
    }
...
int nclistconcat(NClist *l1, NClist *l2) {
  unsigned int i;
  for (i = 0; i < nclistlength(l2); i++)
    nclistpush(l1, nclistget(l2, i));
  return 1;
}
...
int nclistminus(NClist *l1, NClist *l2) {
  ...
  found = 0;
  for (i = 0; i < len; i++) {
    if (nclistdeleteall(l1, nclistget(l2, i)))
      found = 1;
  }
  return found;
}

int nclistdeleteall(NClist *l, ncelem elem) {
...
char *makecdfpathstring3(CDFnode *var, const char *separator) {
...
char *makesimplepathstring3(CDFnode *var) {
  int slen, i, len, first;
  char *pathname;
  NClist *path = nclistnew();
...
char *makeocpathstring3(OCconnection conn, OCobject var,
...
NCerror collectocpath(OCconnection conn, OCobject node, NClist *path) {
  OCobject container;
  OCtype octype;
  if (node == OCNULL)
    return NC_NOERR;
...
void clonenodenamepath3(CDFnode *node, NClist *path, int withdataset) {
  if (node == NULL)
    return;
  /* stop at the dataset container as well*/
  if (node->nctype != NC_Dataset)
...
char *simplepathstring3(NClist *names, char *separator) {
  int i;
  size_t len;
  char *result;
  if (nclistlength(names) == 0)
    return nulldup("");
  ...
  if (gv->walk != NULL) {
    for (i = 0; i < nclistlength(gv->walk->segments); i++) {
      ...
      if (segment->slicerank == 0)
        ...
        sprintf(tmp, "[%lu:%lu:%lu]", (unsigned long)segment->slices[i].first,
...
BOOL dapgridarray(CDFnode *node) {
  if (node != NULL && node->container != NULL &&
      node->container->nctype == NC_Grid) {
    CDFnode *array = (CDFnode *)nclistget(node->container->subnodes, 0);
    return (node == array);
...
BOOL dapgridelement(CDFnode *node) {
  return dapgridarray(node) || dapgridmap(node);
}

/* Is node a top-level grid node? */
BOOL daptopgrid(CDFnode *grid) {
  if (grid == NULL || grid->nctype != NC_Grid)
    return FALSE;
  return daptoplevel(grid);
}

/* Is node a top-level sequence node? */
BOOL daptopseq(CDFnode *seq) {
  if (seq == NULL || seq->nctype != NC_Sequence)
    return FALSE;
  return daptoplevel(seq);
}

/* Is node a top-level node? */
BOOL daptoplevel(CDFnode *node) {
  if (node->container == NULL || node->container->nctype != NC_Dataset)
    return FALSE;
  return TRUE;
}
...
NClist *dapparamdecode(char *params0) {
  char *cp;
  char *cq;
  int c;
  int i;
...
const char *dapparamlookup(NClist *params, const char *clientparam) {
  int i;
  if (params == NULL || clientparam == NULL)
    return NULL;
...
void dapparamfree(NClist *params) {
  int i;
  if (params == NULL)
    return;
...
unsigned int modeldecode(int translation, const char *smodel,
                         const struct NCTMODEL *models, unsigned int dfalt) {
  for (; models->translation; models++) {
    if (translation != models->translation)
      continue;
...
unsigned long getlimitnumber(const char *limit) {
  size_t slen;
...
void dapexpandescapes(char *termstring) {
  char *s, *t, *endp;
...
OCerror dap_oc_fetch(NCDRNO *drno, OCconnection conn, const char *ce, OCdxd dxd,
                     OCobject *rootp) {
  OCerror ocstat;
  char *ext;
  if (dxd == OCDDS)
