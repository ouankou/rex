 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap3/cdf3.c,v 1.33 2009/12/03 03:42:37
 *dmh Exp $
 *********************************************************************/
#include "config.h"
...
NCerror computecdfnodesets3(NCDRNO *drno) {
  unsigned int i;
  NClist *varnodes = nclistnew();
  NClist *allnodes = drno->cdf.ddsroot->tree->nodes;

  if (drno->cdf.seqnodes == NULL)
    drno->cdf.seqnodes = nclistnew();
  if (drno->cdf.gridnodes == NULL)
    drno->cdf.gridnodes = nclistnew();
...
NCerror computevarnodes3(NCDRNO *drno, NClist *allnodes, NClist *varnodes) {
  unsigned int i, len;
...
*/
NCerror computecdfvarnames3(NCDRNO *drno, CDFnode *root, NClist *varnodes) {
  unsigned int i, j, d;
    ...
    for (i = 0; i < nclistlength(varnodes); i++) {
      ...
      for (j = 0; j < i; j++) {
        ...
        if (strcmp(var->ncfullname, testnode->ncfullname) != 0)
          ...
          for (d = 0; d < nclistlength(testnode->array.dimensions); d++) {
  ...
  for (i = 0; i < nclistlength(varnodes); i++) {
    ...
    if (var1->array.basevar != NULL)
      continue;
    for (j = 0; j < i; j++) {
      CDFnode *var2 = (CDFnode *)nclistget(varnodes, j);
      if (var2->array.basevar != NULL)
        continue;
...
#ifdef IGNORE
static NClist *hoist(NCDRNO *drno, CDFnode *container) {
  int i, j;
  ...
  for (i = 0; i < nclistlength(container->subnodes); i++) {
    ...
    case NC_Primitive:
      ...
      break;
    default:
      PANIC1("computecdfvarnames: bad node type: %d", sub->nctype);
    }
  }
...
#ifdef UNUSED
static void makevarnames(NCDRNO *drno, NClist *vars) {
  int i;
  for (i = 0; i < nclistlength(vars); i++) {
    CDFnode *var = (CDFnode *)nclistget(vars, i);
    efree(var->ncfullname);
    var->ncfullname = makecdfpathstring3(var, drno->cdf.separator);
    if (var == v4node && var->ncfullname[0] != 'Q')
      dappanic("");
    if (ncdap3debug > 1)
...
NCerror sequencecheck3(NCDRNO *drno) {
...
static NCerror sequencecheck3r(CDFnode *node, NClist *vars, CDFnode *topseq) {
  ...
  if (topseq == NULL && nclistlength(node->array.dimensions) > 0) {
    ...
    for (i = 0; i < nclistlength(node->subnodes); i++) {
      ...
      if (err == NC_NOERR)
        ok = 1; /* there is at least 1 usable var below */
    }
    if (topseq == NULL && ok == 1) {
...
NCerror regrid3(CDFnode *ddsroot, CDFnode *template, NClist *projections) {
  int i;
...
static void projection3r(CDFnode *node) {
  int i;
  NClist *path = nclistnew();
...
*/
static NCerror regrid3r(CDFnode *node, CDFnode *template, NClist *structnodes) {
...
*/
static NCerror testregrid3(CDFnode *node, CDFnode *template,
                           NClist *structnodes) {
  int i, match;
  NCerror ncstat = NC_NOERR;
    ...
    for (match = 0, i = 0; i < nclistlength(template->subnodes); i++) {
      ...
      if (ncstat == NC_NOERR) {
        ...
        for (i = 0; i < nclistlength(structnodes); i++) {
          ...
          if (newstruct->template == template)
            break;
          newstruct = NULL;
        }
        if (newstruct == NULL) {
...
static NCerror regridinsert(CDFnode *newstruct, CDFnode *node) {
  int i;
  CDFnode *parent;
...
static NCerror regridremove(CDFnode *newstruct, CDFnode *node) {
  int i;
  CDFnode *parent;
...
#ifdef IGNORE
static void regridclean3(NClist *nodes, struct NCregrid *gridnodes) {
  int i;
  NCregrid *ncregrid;
  ...
  for (i = 0; i < nclistlength(nodes); i++) {
    ...
    ncregrid = gridnodes;
    while (ncregrid->newgrid != NULL && ncregrid->newgrid != node)
      ncregrid++;
    if (ncregrid->newgrid == node)
      continue; /* no need to process this */
    for (i = nclistlength(node->subnodes) - 1; i >= 0;
         i--) { /* walk backwards */
      CDFnode *subnode = (CDFnode *)nclistget(node->subnodes, i);
      /* Remove any subnodes of this node that now point to a regrid node */
      ncregrid = gridnodes;
      while (ncregrid->newgrid != NULL && ncregrid->newgrid != subnode)
        ncregrid++;
      /* Found, remove from this list */
      if (ncregrid->newgrid != NULL)
        nclistremove(node->subnodes, i);
    }
  }
}
#endif
...
CDFnode *findddsnode0(CDFnode *node) {
...
NCerror imprint3(CDFnode *dstroot, CDFnode *srcroot) {
  NCerror ncstat = NC_NOERR;
...
static NCerror imprint3r(CDFnode *dstnode, CDFnode *srcnode, int depth) {
  unsigned int i, j;
  NCerror ncstat = NC_NOERR;
  ...
  ASSERT((nclistlength(dstnode->array.dimensions) ==
          nclistlength(srcnode->array.dimensions)));
...
static NCerror imprintdims3(CDFnode *dstnode, CDFnode *srcnode) {
  unsigned int i;
...
void setvisible(CDFnode *root, int visible) {
  unsigned int i;
  CDFtree *tree = root->tree;
  for (i = 0; i < nclistlength(tree->nodes); i++) {
    CDFnode *node = (CDFnode *)nclistget(tree->nodes, i);
...
NCerror imprintself3(CDFnode *root) {
  setvisible(root, 1);
  return NC_NOERR;
}
