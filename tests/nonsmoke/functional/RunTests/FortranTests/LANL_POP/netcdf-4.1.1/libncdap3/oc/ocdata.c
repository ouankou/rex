...
int ocskip(OCnode *node, XDR *xdrs) {
  ...
  case OC_Primitive:
    ...
    if (node->etype == OC_String || node->etype == OC_URL) {
      ...
      xdrcount = 1;
      if (rank > 0 && !xdr_u_int(xdrs, &xdrcount))
        return xdrerror();
      len = xdrcount;
      for (i = 0; i < xdrcount; i++) {
        if (!xdr_u_int(xdrs, &len))
          return xdrerror();
        if (!xdr_skip(xdrs, len))
          return xdrerror();
      }
    } else { /* uniform => do a direct skip*/
      OCASSERT((node->dap.arraysize > 0 && node->dap.instancesize > 0));
      if (node->array.rank > 0) {
        if (!xdr_skip(xdrs, node->dap.arraysize))
          return xdrerror();
      } else {
...
int ocskipinstance(OCnode *node, XDR *xdrs) {
  unsigned int i;
  int stat = OC_NOERR;
  unsigned int xdrcount;
...
*/
int ocxdrread(XDR *xdrs, char *memory, size_t memsize, int packed,
              OCtype octype, unsigned int start, size_t count) {
  int stat = OC_NOERR;
  unsigned int i;
  ...
  if (packed) {
    ...
    if (!xdr_opaque(xdrs, (char *)localmem, readsize))
      return xdrerror();
    memcpy((void *)memory, (void *)(localmem + start), count);
    if (readsize > LOCALMEMMAX)
      ocfree(localmem);
    if (!xdr_setpos(xdrs, xdrckp))
      return xdrerror(); /* revert to beginning*/
  ...
  if (octype == OC_String || octype == OC_URL) {
  ...
  } else if (octype == OC_Float64 || octype == OC_UInt64 ||
             octype == OC_Int64) {
    unsigned int *p;
    ...
    MEMCHECK(localmem, OC_ENOMEM);
    if (!xdr_opaque(xdrs, (char *)localmem, xdrsize))
      return xdrerror();
    if (!oc_network_order) {
  ...
  case OC_URL: {
    ...
    for (i = 0; i < start; i++) {
      ...
      if (!xdr_string(xdrs, &s, OC_INT32_MAX))
        return xdrerror();
      ocfree(s);
    }
    /* Read count strings */
...
int countrecords(OCnode *node, XDR *xdrs, size_t *nrecordsp) {
  int stat = OC_NOERR;
  size_t nrecords = 0;
