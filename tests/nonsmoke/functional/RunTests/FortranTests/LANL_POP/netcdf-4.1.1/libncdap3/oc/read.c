...
static int readfiletofile(char *path, char *suffix, FILE *stream,
                          unsigned long *);
#endif
...
int readDDS(OCstate *state, OCtree *tree) {
  ...
  status =
      readpacket(state->curl, &state->url, state->packet, OCDDS, &lastmodified);
  if (status == OC_NOERR)
    state->ddslastmodified = lastmodified;
  return status;
}

int readDAS(OCstate *state, OCtree *tree) {
  dapurlsetconstraints(&state->url, tree->constraint);
  return readpacket(state->curl, &state->url, state->packet, OCDAS, NULL);
}

int readversion(CURL *curl, DAPURL *url, OCbytes *packet) {
  return readpacket(curl, url, packet, OCVER, NULL);
}

static char *ocdxdextension[] = {
    ".dds",  /*OCDDS*/
    ".das",  /*OCDAS*/
    ".dods", /*OCDATADDS*/
    ".vers", /*OCVERS*/
...
static int readpacket(CURL *curl, DAPURL *url, OCbytes *packet, OCdxd dxd,
                      long *lastmodified) {
  int stat;
  int fileprotocol = 0;
...
int readDATADDS(OCstate *state, OCtree *tree) {
  int stat;
  long lastmod = -1;
#ifndef OC_DISK_STORAGE
  dapurlsetconstraints(&state->url, tree->constraint);
  stat =
      readpacket(state->curl, &state->url, state->packet, OCDATADDS, &lastmod);
  if (stat == OC_NOERR)
  ...
  if (fileprotocol && !oc_curl_file_supported) {
    stat = readfiletofile(url->base, ".dods", tree->data.file,
                          &tree->data.datasize);
  } else {
    char *fetchurl;
