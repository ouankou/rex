...
void *dapcalloc(size_t size, size_t nelems) { return dapmalloc(size * nelems); }

void *dapmalloc(size_t size) {
...
int dapbreakpoint(int err) { return err; }

int dapthrow(int err) {
  if (err == 0)
    return err;
