...
NC_new_var()
 */
static NC_var *new_NC_var(const char *uname, nc_type type, size_t ndims,
                          const int *dimids) {
  NC_string *strp;
  NC_var *varp;
...
NC_hvarid
 */
int NC_findvar(const NC_vararray *ncap, const char *uname, NC_var **varpp) {
  NC_var **loc;
  size_t slen;
  int varid;
  char *name;
...
out:
  if (varp->xsz <=
      (X_UINT_MAX - 1) / product) /* if integer multiply will not overflow */
  {
...
int DISPNAME(def_var)(int ncid, const char *name, nc_type type, int ndims,
...
int DISPNAME(inq_varid)(int ncid, const char *name, int *varid_ptr) {
  int status;
  NC *ncp;
  NC_var *varp;
  int varid;
...
int DISPNAME(inq_var)(int ncid, int varid, char *name, nc_type *typep,
...
int DISPNAME(rename_var)(int ncid, int varid, const char *unewname) {
  int status;
  NC *ncp;
  NC_var *varp;
