...
int RENAME(_create_mp)(const char *path, int ioflags, size_t initialsz,
                       int basepe, size_t *chunksizehintp, int *ncid_ptr) {
  NC *ncp;
  int status;
  void *xp = NULL;
  int sizeof_off_t = 0;
...
int RENAME(_open_mp)(const char *path, int ioflags, int basepe,
                     size_t *chunksizehintp, int *ncid_ptr) {
  NC *ncp;
  int status;
...
int RENAME(_enddef)(int ncid, size_t h_minfree, size_t v_align,
                    size_t v_minfree, size_t r_align) {
  int status;
  NC *ncp;
...
int DISPNAME(inq)(int ncid, int *ndimsp, int *nvarsp, int *nattsp,
                  int *xtendimp) {
  int status;
  NC *ncp;
...
int DISPNAME(set_fill)(int ncid, int fillmode, int *old_mode_ptr) {
  int status;
  NC *ncp;
  int oldmode;
...
/*ARGSUSED*/
int DISPNAME(set_base_pe)(int ncid, int pe) {
#if _CRAYMPP && defined(LOCKNUMREC)
  int status;
  NC *ncp;
  shmem_t numrecs;
...
/*ARGSUSED*/
int DISPNAME(inq_base_pe)(int ncid, int *pe) {
#if _CRAYMPP && defined(LOCKNUMREC)
  int status;
  NC *ncp;
...
int DISPNAME(inq_format)(int ncid, int *formatp) {
  int status;
  NC *ncp;
  ...
  /* only need to check for netCDF-3 variants, since this is never called for
     netCDF-4 files */
  *formatp =
      fIsSet(ncp->flags, NC_64BIT_OFFSET) ? NC_FORMAT_64BIT : NC_FORMAT_CLASSIC;
  return NC_NOERR;
}
...
void drno_add_to_NCList(NC *ncp) { add_to_NCList(ncp); }

void drno_del_from_NCList(NC *ncp) { del_from_NCList(ncp); }

void drno_free_NC(NC *ncp) { free_NC(ncp); }

NC *drno_new_NC(const size_t *chunkp) { return new_NC(chunkp); }

void drno_set_numrecs(NC *ncp, size_t size) { NC_set_numrecs(ncp, size); }

size_t drno_get_numrecs(NC *ncp) { return NC_get_numrecs(ncp); }

int drno_ncio_open(NC *ncp, const char *path, int mode) {
  int ncstat;
  ncio *nciop = NULL;
...
int DISPNAME(inq_type)(int ncid, nc_type typeid, char *name, size_t *size) {
  int atomic_size[NUM_ATOMIC_TYPES] = {NC_BYTE_LEN,  NC_CHAR_LEN,
                                       NC_SHORT_LEN, NC_INT_LEN,
                                       NC_FLOAT_LEN, NC_DOUBLE_LEN};
