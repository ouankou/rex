...
#include <string.h>/* Insertion by O. R. Heudecker, AWI-Bremerhaven 12.3.98 (1 line)*/
#include <unistd.h>
...
static int ffio_pgout(ncio *const nciop, off_t const offset,
                      const size_t extent, const void *const vp, off_t *posp) {
#ifdef X_ALIGN
...
static int ffio_pgin(ncio *const nciop, off_t const offset, const size_t extent,
...
static int ncio_ffio_rel(ncio *const nciop, off_t offset, int rflags) {
  ...
  if (fIsSet(rflags, RGN_MODIFIED)) {
    ...
    status =
        ffio_pgout(nciop, ffp->bf_offset, ffp->bf_cnt, ffp->bf_base, &ffp->pos);
...
static int ncio_ffio_get(ncio *const nciop, off_t offset, size_t extent,
  ...
  status =
      ffio_pgin(nciop, offset, extent, ffp->bf_base, &ffp->bf_cnt, &ffp->pos);
  if (status != ENOERR)
    return status;
  ...
  if (ffp->bf_cnt < extent) {
    (void)memset((char *)ffp->bf_base + ffp->bf_cnt, 0, extent - ffp->bf_cnt);
    ffp->bf_cnt = extent;
  }
...
static int ncio_ffio_move(ncio *const nciop, off_t to, off_t from,
  ...
  status =
      ncio_ffio_get(nciop, lower, extent, RGN_WRITE | rflags, (void **)&base);
 ...
 * RKO 06/26/98
 */
static int ncio_ffio_sync_noffflush(ncio *const nciop) {
 ...
 */
static int ncio_ffio_global_test(const char *ControlString) {
...
static int ncio_ffio_sync(ncio *const nciop) {
#ifdef __crayx1
  struct ffsw stat;
...
static void ncio_ffio_free(void *const pvt) {
...
static int ncio_ffio_init2(ncio *const nciop, size_t *sizehintp) {
  ncio_ffio *ffp = (ncio_ffio *)nciop->pvt;
...
static void ncio_ffio_init(ncio *const nciop) {
  ncio_ffio *ffp = (ncio_ffio *)nciop->pvt;
...
#ifdef __crayx1
void ASNQFILE(const char *filename, const char *attribute, int *istat, int flen,
              int alen);
void ASNFILE(const char *filename, const char *attribute, int *istat, int flen,
             int alen);
#else
...
#define BUFLEN 256
static const char *ncio_ffio_assign(const char *filename) {
...
static const size_t NCIO_MAXBLOCKSIZE =
    268435456; /* sanity check, about X_SIZE_T_MAX/8 */
...
int ncio_create(const char *path, int ioflags, size_t initialsz, off_t igeto,
  ...
  if (strstr(ControlString, "global") != (char *)NULL) {
    ...
    *((ncio_syncfunc **)&nciop->sync) = ncio_ffio_sync_noffflush;
  }
#endif
