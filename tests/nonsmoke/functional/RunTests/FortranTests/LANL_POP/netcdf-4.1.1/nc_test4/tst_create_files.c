...
#define D2 2000
    ...
    if (!(data = (float *)malloc(D1 * D2 * sizeof(float))))
      ERR;
    for (i = 0; i < D1 * D2; i++)
      data[i] = (float)rand();

    /* User TEMP_LARGE as the directory. */
    if (strlen(TEMP_LARGE) + strlen(LARGE_FILE) > NC_MAX_NAME * 2)
      ERR;
    ...
    if (nc_create(file_name, NC_NETCDF4 | NC_CLASSIC_MODEL, &ncid))
      ERR;
    if (nc_def_dim(ncid, "d0", D0, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, "d1", D1, &dimids[1]))
      ERR;
    if (nc_def_dim(ncid, "d2", D2, &dimids[2]))
      ERR;
    if (nc_def_var(ncid, LARGE_VAR_NAME, NC_FLOAT, NDIMS3, dimids, &varid))
      ERR;
    if (nc_def_var_chunking(ncid, varid, NULL, chunk_sizes, NULL))
      ERR;
    if (nc_enddef(ncid))
      ERR;
...
#define DIMLEN 10000
    ...
    if (nc_create(FILE_NAME, 0, &ncid))
      ERR;
    if (nc_def_dim(ncid, DIM_NAME, DIMLEN, dimids))
      ERR;
    if (nc_def_var(ncid, var_name, NC_FLOAT, NUMDIMS, dimids, &varid))
      ERR;
    if (nc_enddef(ncid))
      ERR;
    ...
    if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid))
      ERR;
    if (ndims != NUMDIMS || nvars != 1 || natts != 0 || unlimdimid != -1)
      ERR;
    if (nc_inq_dimids(ncid, &ndims, dimids, 1))
      ERR;
    if (ndims != 1 || dimids[0] != 0)
      ERR;
    if (nc_inq_dim(ncid, 0, name_in, &len))
      ERR;
    if (strcmp(name_in, DIM_NAME) || len != DIMLEN)
      ERR;
    ...
    if (strcmp(name_in, var_name) || xtype != NC_FLOAT || ndims != 1 ||
        dimids[0] != 0 || natts != 0)
      ERR;
    if (nc_get_var_float(ncid, 0, data_in))
      ERR;
    for (i = 0; i < DIMLEN; i++)
      if (data_in[i] != data[i])
        ERR;
...
#define MAX_TYPES 3
    ...
    for (t = 0; t < MAX_TYPES; t++) {
      ...
      for (ndims = 1; ndims <= MAX_DIMS; ndims++) {
        ...
        for (len = pow(TOTAL_SIZE, (float)1 / ndims), d = 0; d < ndims; d++) {
          ...
          if (nc_def_dim(ncid, dim_name, len, &dimids[d]))
            ERR;
        }
        if (nc_def_var(ncid, VAR_NAME, typeid[t], ndims, dimids, &varid))
          ERR;
        if (nc_enddef(ncid))
          ERR;
        if (nc_put_var(ncid, varid, data[t]))
          ERR;
        if (nc_close(ncid))
          ERR;
      } /* next ndims. */
  ...
  SUMMARIZE_ERR;
  printf("*** Creating file like in "
         "http://hdfeos.org/workshops/ws06/presentations/Pourmal/"
         "HDF5_IO_Perf.pdf...");
  {
#define XLEN 256
#define YLEN 256
...
#define E_TYPE_SIZE 4
    ...
    if (nc_create(ELENA_FILE_NAME, NC_CLOBBER, &ncid))
      ERR;
    if (nc_def_dim(ncid, "z", ZLEN, &dimids[0]))
      ERR;
    if (nc_def_dim(ncid, "y", YLEN, &dimids[1]))
      ERR;
    if (nc_def_dim(ncid, "x", XLEN, &dimids[2]))
      ERR;
    if (nc_def_var(ncid, E_VAR_NAME, NC_INT, NDIMS, dimids, &varid))
      ERR;
    if (nc_enddef(ncid))
      ERR;

    /* Write the data. */
    if (nc_put_var(ncid, varid, idata))
      ERR;
    if (nc_close(ncid))
      ERR;
  }
...
#define S_FILE_NAME "tst_simple.nc"
    ...
    if (nc_create(S_FILE_NAME, NC_CLOBBER, &ncid))
      ERR;
    if (nc_def_dim(ncid, "a", DIM_LEN, &dimids[0]))
      ERR;
    if (nc_def_var(ncid, S_VAR_NAME, NC_INT, NDIMS1, dimids, &varid))
      ERR;
    if (nc_enddef(ncid))
      ERR;
...
#define MAX_TYPES 3
    ...
    for (t = 0; t < MAX_TYPES; t++) {
      ...
      for (ndims = 1; ndims <= MAX_DIMS; ndims++) {
        ...
        if (nc_def_var(ncid, SIMPLE_VAR_NAME, typeid[t], ndims, dimids, &varid))
          ERR;
        if (nc_enddef(ncid))
          ERR;
        if (nc_put_var(ncid, varid, data[t]))
