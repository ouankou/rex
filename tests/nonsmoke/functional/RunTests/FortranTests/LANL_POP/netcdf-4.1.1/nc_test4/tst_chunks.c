...
#define TIMING_END(seconds)                                                    \
  ...
  printf("%-45.45s %7.2g sec", time_mess, seconds);
...
void parse_args(int argc, char *argv[], /* from command-line invocation */
                ...
                float *cache_prep) /* returned: cache preemption policy (0-1) */
{
  ...
  if (argc > 11) {
    printf("Usage: %s [def_level] [dim1] [chunk1] [dim2] [chunk2] [dim3] "
           "[chunk3] [cache_size] [cache_nelems] [cache_pre]\n",
           argv[0]);
    exit(1);
  }
  return;
...
#define RANK_var1 3
  ...
  /* From args, get parameters for timing, including variable and
     ...
     filter. */
  parse_args(argc, argv, &deflate_level, &shuffle, dims, chunks, &cache_size,
             &cache_hash, &cache_pre);
  ...
  if ((stat = nc_set_chunk_cache(cache_size, cache_hash, cache_pre)))
    ERR1(stat);
  printf("cache: %3.2f MBytes  %ld objs  %3.2f preempt, ", cache_size / 1.e6,
         cache_hash, cache_pre);
  ...
  var_dims[1] = dim2id;
  var_dims[2] = dim3id;
  if ((stat = nc_def_var(ncid, "var_contiguous", NC_FLOAT, RANK_var1, var_dims,
                         &varid_g)))
    ERR1(stat);
  if ((stat = nc_def_var(ncid, "var_chunked", NC_FLOAT, RANK_var1, var_dims,
                         &varid_k)))
    ERR1(stat);
  if ((stat = nc_def_var(ncid, "var_compressed", NC_FLOAT, RANK_var1, var_dims,
                         &varid_x)))
    ERR1(stat);
  ...
  if (deflate_level != 0) {
    if ((stat = nc_def_var_deflate(ncid, varid_x, shuffle, NC_COMPRESSED,
                                   deflate_level)))
      ERR1(stat);
  }
  ...
  sprintf(time_mess, "  contiguous write %3ld %3ld %3ld", 1, dims[1], dims[2]);
  TIMING_START;
  for (i = 0; i < dims[0]; i++) {
    start[0] = i;
  ...
  sprintf(time_mess, "  chunked    write %3ld %3ld %3ld  %3ld %3ld %3ld", 1,
          dims[1], dims[2], chunks[0], chunks[1], chunks[2]);
  TIMING_START;
  for (i = 0; i < dims[0]; i++) {
    start[0] = i;
  ...
  sprintf(time_mess, "  compressed write %3ld %3ld %3ld  %3ld %3ld %3ld", 1,
          dims[1], dims[2], chunks[0], chunks[1], chunks[2]);
  TIMING_START;
  for (i = 0; i < dims[0]; i++) {
    start[0] = i;
  ...
  sprintf(time_mess, "  contiguous write %3ld %3ld %3ld", dims[0], 1, dims[2]);
  TIMING_START;
  for (i = 0; i < dims[1]; i++) {
    start[1] = i;
  ...
  sprintf(time_mess, "  contiguous write %3ld %3ld %3ld", dims[0], dims[1], 1);
  TIMING_START;
  for (i = 0; i < dims[2]; i++) {
    start[2] = i;
  ...
  sprintf(time_mess, "  contiguous  read %3ld %3ld %3ld", 1, dims[1], dims[2]);
  TIMING_START;
  for (i = 0; i < dims[0]; i++) {
    start[0] = i;
  ...
  sprintf(time_mess, "  chunked     read %3ld %3ld %3ld  %3ld %3ld %3ld", 1,
          dims[1], dims[2], chunks[0], chunks[1], chunks[2]);
  TIMING_START;
  for (i = 0; i < dims[0]; i++) {
    start[0] = i;
  ...
  sprintf(time_mess, "  compressed  read %3ld %3ld %3ld  %3ld %3ld %3ld", 1,
          dims[1], dims[2], chunks[0], chunks[1], chunks[2]);
  TIMING_START;
  for (i = 0; i < dims[0]; i++) {
    start[0] = i;
  ...
  sprintf(time_mess, "  contiguous  read %3ld %3ld %3ld", dims[0], 1, dims[2]);
  TIMING_START;
  for (i = 0; i < dims[1]; i++) {
    start[1] = i;
  ...
  sprintf(time_mess, "  contiguous  read %3ld %3ld %3ld", dims[0], dims[1], 1);
  TIMING_START;
  for (i = 0; i < dims[2]; i++) {
    start[2] = i;
