...
Foundation\n", " and the Office of Biological and Environmental Research of\n",
               ...
               "In addition, the words \'Community Climate System Model\'
...
include this data\n", " acknowledgement statement." ; :realization = 2 ;
...
#define NELEMS 7919
  if (nc_set_chunk_cache(cache, NELEMS, PREEMPTION))
    ERR;
  if (nc_open(argv[0], 0, &ncid))
    ERR;

  /* Check to make sure that all the dimension information is
   * correct. */
  if (nc_inq_varid(ncid, DATA_VAR_NAME, &varid))
    ERR;
  if (nc_inq_dim(ncid, LON_DIMID, name_in, &len))
    ERR;
  if (strcmp(name_in, "lon") || len != LON_LEN)
    ERR;
  if (nc_inq_dim(ncid, LAT_DIMID, name_in, &len))
    ERR;
  if (strcmp(name_in, "lat") || len != LAT_LEN)
    ERR;
  if (nc_inq_dim(ncid, DEPTH_DIMID, name_in, &len))
    ERR;
  if (strcmp(name_in, "depth") || len != DEPTH_LEN)
    ERR;
  if (nc_inq_dim(ncid, BNDS_DIMID, name_in, &len))
    ERR;
  if (strcmp(name_in, "bnds") || len != BNDS_LEN)
    ERR;
  if (nc_inq_dim(ncid, TIME_DIMID, name_in, &len))
    ERR;
  if (strcmp(name_in, "time") || len != TIME_LEN)
    ERR;
  if (nc_inq_var(ncid, varid, NULL, NULL, &ndims, dimid, NULL))
    ERR;
  if (ndims != NDIMS4 || dimid[0] != TIME_DIMID || dimid[1] != DEPTH_DIMID ||
      dimid[2] != LAT_DIMID || dimid[3] != LON_DIMID)
    ERR;

  /* Get info about the main data var. */
  if (nc_inq_var_chunking(ncid, varid, &storage, cs))
    ERR;
  if (nc_inq_var_deflate(ncid, varid, &shuffle, &deflate, &deflate_level))
    ERR;
  ...
  if (timeseries) {
    ...
    if (gettimeofday(&start_time, NULL))
      ERR;
    if (nc_get_vara_float(ncid, varid, start, count, ts_data))
      ERR_RET;
    ...
    if (gettimeofday(&start_time, NULL))
      ERR;
    for (start[1] = 0; read_count < MAX_READ_COUNT && start[1] < LAT_LEN;
         start[1]++)
      for (start[2] = 1; read_count < MAX_READ_COUNT && start[2] < LON_LEN;
           start[2]++)
        for (start[3] = 1; read_count < MAX_READ_COUNT && start[3] < DEPTH_LEN;
             start[3]++) {
          if (nc_get_vara_float(ncid, varid, start, count, ts_data))
            ERR_RET;
          read_count++;
    ...
    if (timeval_subtract(&diff_time, &end_time, &start_time))
      ERR;
    num_reads = (read_count == MAX_READ_COUNT)
                    ? MAX_READ_COUNT
                    : (LAT_LEN * LON_LEN * DEPTH_LEN);
    avg_read_us =
        ((int)diff_time.tv_sec * MILLION + (int)diff_time.tv_usec + read_1_us) /
        num_reads;
  } else if (vertical_profile) {
    ...
    if (gettimeofday(&start_time, NULL))
      ERR;
    if (nc_get_vara_float(ncid, varid, start, count, vert_data))
      ERR_RET;
    if (gettimeofday(&end_time, NULL))
      ERR;
    if (timeval_subtract(&diff_time, &end_time, &start_time))
    ...
    if (gettimeofday(&start_time, NULL))
      ERR;
    /*       for (start[0] = 0; read_count < MAX_READ_COUNT && start[1] <
     * LAT_LEN; start[1]++) */
    /*       for (start[1] = 0; read_count < MAX_READ_COUNT && start[1] <
     * LAT_LEN; start[1]++) */
    /* 	 for (start[2] = 1; read_count < MAX_READ_COUNT && start[2] < LON_LEN;
     * start[2]++) */
    /* 	    for (start[] = 1; read_count < MAX_READ_COUNT && start[3] <
     * DEPTH_LEN; start[3]++) */
    /* 	    { */
    /* 	       if (nc_get_vara_float(ncid, varid, start, count, vert_data))
     * ERR_RET; */
    ...
    if (gettimeofday(&start_time, NULL))
      ERR;
    if (nc_get_vara_float(ncid, varid, start, count, hor_data))
      ERR_RET;
    if (gettimeofday(&end_time, NULL))
      ERR;
    if (timeval_subtract(&diff_time, &end_time, &start_time))
    ...
    if (gettimeofday(&start_time, NULL))
      ERR;
    for (start[0] = 0; read_count < MAX_READ_COUNT && start[0] < TIME_LEN;
         start[0]++)
      for (start[1] = 1; read_count < MAX_READ_COUNT && start[1] < DEPTH_LEN;
           start[1]++) {
        if (nc_get_vara_float(ncid, varid, start, count, hor_data))
          ERR_RET;
        read_count++;
  ...
  /* Print results. */
  printf("%d\t%d\t%d\t%.1f\t\t%d\t%d\t\t", (int)cs[0], (int)cs[1], (int)cs[2],
         (storage == NC_CHUNKED) ? (cache / (float)MEGABYTE) : 0, deflate,
         shuffle);
  if (timeseries)
