...
int main(int argc, char **argv) {
  ...
  if (nc_create_par(file_name, NC_PNETCDF, comm, info, &ncid))
    ERR;

  /* A global attribute holds the number of processors that created
   * the file. */
  if (nc_put_att_int(ncid, NC_GLOBAL, "num_processors", NC_INT, 1, &mpi_size))
    ERR;

  /* Create three dimensions. */
  if (nc_def_dim(ncid, DIM1_NAME, NUM_SLABS, dimids))
    ERR;
  if (nc_def_dim(ncid, DIM2_NAME, DIMSIZE, &dimids[1]))
    ERR;
  if (nc_def_dim(ncid, DIM3_NAME, DIMSIZE, &dimids[2]))
    ERR;
  ...
  if (nc_var_par_access(ncid, varid, NC_INDEPENDENT))
    ERR;
  ...
  if (!mpi_rank) {
    ...
    printf("%d\t%g\t%g\n", mpi_size, total_time,
           DIMSIZE * DIMSIZE * NUM_SLABS * sizeof(int) / total_time);
  }
  ...
  if (nc_open_par(file_name, NC_NOWRITE, comm, info, &ncid))
    ERR;
  ...
  for (i = 0; i < NUM_SLABS / mpi_size; i++) {
    ...
    if (nc_get_vara_int(ncid, varid, start, count, data_in))
      ERR;
