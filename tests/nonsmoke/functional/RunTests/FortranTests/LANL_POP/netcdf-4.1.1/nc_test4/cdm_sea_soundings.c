...
int main(int argc, char **argv) {
  ...
  if (nc_def_vlen(ncid, "temp_vlen", NC_FLOAT, &temp_typeid))
    ERR;

  /* Create the compound type to hold a sea sounding. */
  if (nc_def_compound(ncid, sizeof(struct sea_sounding), "sea_sounding",
                      &sounding_typeid))
    ERR;
  ...
  if (nc_def_dim(ncid, DIM_NAME, DIM_LEN, &dimid))
    ERR;
  if (nc_def_var(ncid, "fun_soundings", sounding_typeid, 1, &dimid, &varid))
    ERR;
