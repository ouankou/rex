 ...
 * and writes. */
static int get_starts_counts(int ndims, size_t *dimlen, int p, int my_rank,
  ...
  if (use_scs) {
    ...
    *slice_len = *start_inc;
    ...
    if (start[0] > dimlen[0]) {
      fprintf(stderr,
              "slow_count too large for this many processors, "
...
static size_t file_size(char *name) {
  struct stat stbuf;
  stat(name, &stbuf);
...
static int check_att(int ncid1, int ncid2, int varid, int a) {
  int typeid, typeid2;
...
static int cmp_file(char *file1, char *file2, int *meta_read_us,
                    int *data_read_us, int use_par, int par_access, int do_cmp,
                    int p, int my_rank, int slow_count, int verbose, int num_vo,
                    VAR_OPTS_T *vo, int use_scs) {
  int ncid1, ncid2;
  ...
  if (use_par) {
    ...
    *meta_read_us += (MPI_Wtime() - ftime) * MILLION;
    ...
    if (gettimeofday(&end_time, NULL))
      ERR;
    if (timeval_subtract(&diff_time, &end_time, &start_time))
      ERR;
  ...
  for (v = 0; v < nvars; v++) {
    ...
    /* Check var metadata. */
    if (strcmp(name, name2) || xtype != xtype2 || ndims != ndims2 ||
        natts != natts2)
      return BAD;
    for (d = 0; d < ndims; d++)
    ...
    for (step = 0; !ret && step < num_steps; step++) {
      ...
      if (nc_get_vara(ncid1, v, start, count, data))
        ERR;
      ...
      if (gettimeofday(&start_time, NULL))
        ERR;
#endif
      if (nc_get_vara(ncid2, v, start, count, data2))
        ERR;
#ifdef USE_PARALLEL
...
static int copy_file(char *file_name_in, char *file_name_out, int cmode_out,
                     int num_vo, VAR_OPTS_T *vo, int *meta_read_us,
                     int *meta_write_us, int *data_read_us, int *data_write_us,
                     int *in_format, int use_par, int par_access,
                     long long *num_bytes, int p, int my_rank, int slow_count,
                     int verbose, int use_scs, int endianness,
                     int convert_unlim) {
...
#ifdef USE_PARALLEL
    ftime = MPI_Wtime();
    if ((ret = nc_open_par(file_name_in, 0, MPI_COMM_WORLD, MPI_INFO_NULL,
                           &ncid_in)))
      ERR1(ret);
    *meta_read_us += (MPI_Wtime() - ftime) * MILLION;
#else
...
#define NELEMS 7919
  ...
  if (strlen(file_name_out)) {
    ...
    for (d = 0; d < ndims; d++) {
      ...
      if (convert_unlim) {
        ...
        if ((ret = nc_def_dim(ncid_out, name,
                              (d == unlimdimid) ? NC_UNLIMITED : len, NULL)))
          ERR1(ret);
      }
    }
    ...
    /* Copy global atts. */
    for (a = 0; a < natts; a++) {
      if (nc_inq_attname(ncid_in, NC_GLOBAL, a, name))
        ERR;
      if (nc_copy_att(ncid_in, NC_GLOBAL, name, ncid_out, NC_GLOBAL))
        ERR;
    }
    ...
    for (v = 0; v < nvars; v++) {
      ...
      if (nc_def_var(ncid_out, name, xtype, ndims, dimids, &varid_out))
        ERR;
      ...
      if (endianness)
        if (nc_def_var_endian(ncid_out, varid_out, endianness))
          ERR;
      ...
      for (o1 = 0; o1 < num_vo; o1++)
        if (vo[o1].varid == v) {
          if (vo[o1].chunksize[0]) {
            if (nc_def_var_chunking(ncid_out, v, 0, vo[o1].chunksize))
              ERR;
          } else {
            if (nc_def_var_chunking(ncid_out, v, 1, NULL))
              ERR;
          }
          if (vo[o1].deflate_num != -1)
            if (nc_def_var_deflate(ncid_out, v, vo[o1].shuffle, 1,
                                   vo[o1].deflate_num))
              ERR;
          break;
        }
      ...
      /* Copy the attributes. */
      for (a = 0; a < natts; a++) {
        if (nc_inq_attname(ncid_in, v, a, att_name))
          ERR;
        if (nc_copy_att(ncid_in, v, att_name, ncid_out, varid_out))
          ERR;
      }
    }
    ...
    *meta_write_us += (int)diff_time.tv_sec * MILLION + (int)diff_time.tv_usec;
    ...
    if (verbose)
      printf("%d: copying %d vars, %d global atts, and %d dims took %d "
             "micro-seconds\n",
      ...
      if (strlen(file_name_out)) {
        ...
        *data_write_us += (MPI_Wtime() - ftime) * MILLION;
        ...
        if (timeval_subtract(&diff_time, &end_time, &start_time))
          ERR;
        *data_write_us +=
            (int)diff_time.tv_sec * MILLION + (int)diff_time.tv_usec;
#endif
        if (verbose)
          printf("%d: writing step %d, var %d took %d micro-seconds\n", my_rank,
                 step, v, *data_write_us);
      }
...
static void usage(void) {
  fprintf(
      stderr,
      "bm_file -v [-s N]|[-t V:S:S:S -u V:C:C:C -r V:I:I:I] -o file_out -f N -h"
...
int main(int argc, char **argv) {
  ...
  int tmeta_read_us = 0, tmeta_write_us = 0, tdata_read_us = 0,
      tdata_write_us = 0;
      ...
      for (v = 0; v < num_vo; v++) {
        ...
        for (d = 0; d < vo[v].ndims; d++)
          printf("start[%d]=%ld, count[%d]=%ld, inc[%d]=%ld\n", d,
                 vo[v].start[d], d, vo[v].count[d], d, vo[v].inc[d]);
      }
    else
      printf("slow_count=%d, doublecheck=%d\n", slow_count, doublecheck);
  }

  /* Copy the file, keeping track of the read and write times for metadata and
   * data. */
  if ((ret =
           copy_file(file_in, file_out, cmode, num_vo, vo, &meta_read_us,
                     &meta_write_us, &data_read_us, &data_write_us, &in_format,
                     use_par, par_access, &num_bytes, p, my_rank, slow_count,
                     verbose, use_scs, endianness, convert_unlim)))
    return ret;
  ...
  if (use_par) {
#ifdef USE_PARALLEL
    MPI_Reduce(&meta_read_us, &tmeta_read_us, 1, MPI_INT, MPI_MAX, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&meta_write_us, &tmeta_write_us, 1, MPI_INT, MPI_MAX, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&data_read_us, &tdata_read_us, 1, MPI_INT, MPI_MAX, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&data_write_us, &tdata_write_us, 1, MPI_INT, MPI_MAX, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&data_read2_us, &tdata_read2_us, 1, MPI_INT, MPI_MAX, 0,
               MPI_COMM_WORLD);
#else
    return NC_EPARINIT;
#endif
  ...
  if (verbose)
    printf("%d: read rate %g, write rate %g, reread_rate %g\n", my_rank,
           read_rate, write_rate, reread_rate);

  /* Print some output. */
  if (!my_rank) {
    /* Does the user want a text header for the data? */
    if (header) {
      printf("input format, output_format, input size, output size, meta read "
             "time, "
    ...
    printf("%d, %d, %ld, %ld, %d, %d, %d, %d, %d, ", in_format, out_format,
           file_size(file_in), file_size(file_out), tmeta_read_us,
           tmeta_write_us, tdata_read_us, tdata_write_us, endianness);
    if (doublecheck)
      printf("%d, %d, %g, %g, %g, ", tmeta_read2_us, tdata_read2_us, read_rate,
    ...
    for (o1 = 0; o1 < num_vo; o1++) {
      printf("%d, %d, %d, %d, %d, %d ", vo[o1].deflate_num, vo[o1].shuffle,
             (int)vo[o1].chunksize[0], (int)vo[o1].chunksize[1],
             (int)vo[o1].chunksize[2], (int)vo[o1].chunksize[3]);
      if (o1 != num_vo - 1)
        printf(", ");
    }
    printf("\n");
