...
char *usage[] = {"",
                 "  river_regrid --mosaic mosaic_grid --river_src "
                 "river_src_file [--output output_file] ",
                 "                                                             "
                 "                       ",
                 "river_regrid will remap river network data from global "
                 "regular lat-lon grid onto any  ",
                 "other grid (includes regular lat-lon grid and cubic grid ), "
                 "which is specified        ",
                 "through option --mosaic. The river network source data is "
                 ...
                 "                                                             "
                 "                         ",
                 "--mosaic    mosaic_grid     specify the mosaic file of "
                 "destination grid. This mosaic  ",
                 "                            file should be a coupler mosaic "
                 "file, which contains link ",
                 "                            to land solo mosaic and the "
                 "exchange grid file.           ",
                 "                                                             "
                 "                         ",
                 "--river_src river_src_file  specify the river network source "
                 "data file. The data is   ",
                 "                            assumed on regular lat-lon grid "
                 "and the longitude is      ",
                 "                            assumed from 0 to 360 degree and "
                 "latitude is assumed      ",
                 ...
                 "                                                             "
                 "                         ",
                 "--output output_file        specify the output file base "
                 "name. the suffix '.nc'       ",
                 "                            should not be included in the "
                 "output_file. The default    ",
                 "                            value is river_output. For one "
                 "tile mosaic, the actual    ",
                 "                            result will be $output_file.nc. "
                 "For multiple tile mosaic, ",
                 "                            the result will be "
                 "$output.tile#.nc.                      ",
                 "                                                             "
                 "                         ",
...
int main(int argc, char *argv[]) {
  ...
  if (mpp_npes() > 1)
    mpp_error("river_regrid: parallel is not supported yet, try running one "
              "single processor");
...
/*   mpp_get_var_att(fid, vid, "missing_value",
 * &(river_data->celllength_missing) ); */
...
/*   for(i=1; i<nx; i++ ) river_data->xb[i] =
 * 0.5*(river_data->xt[i-1]+river_data->xt[i]); */
/*   for(j=1; j<ny; j++ ) river_data->yb[j] =
 * 0.5*(river_data->yt[j-1]+river_data->yt[j]); */
...
/*   /\* make sure the xb is in the range [0,360] and yb is in the range
 * [-90,90] *\/ */
/*   if(fabs(river_data->xb[0]) > EPSLN) mpp_error("river_regrid: The starting
 * longitude of grid bound is not 0"); */
/*   if(fabs(river_data->xb[nx]-360) > EPSLN) mpp_error("river_regrid: The
 * ending longitude of grid bound is not 360");   */
/*   if(fabs(river_data->yb[0]+90) > EPSLN) mpp_error("river_regrid: The
 * starting latitude of grid bound is not -90"); */
/*   if(fabs(river_data->yb[ny]-90) > EPSLN) mpp_error("river_regrid: The ending
 * latitude of grid bound is not 90");   */
/*   river_data->xb[0]  = 0.; */
...
/*    --------------------------------------------------------------------*\/ */
/* void get_mosaic_grid(const char *coupler_mosaic, const char *land_mosaic, int
 * ntiles, river_type *river_data, unsigned int *opcode) */
/* { */
...
/*   /\* coupler_mosaic, land_mosaic, and exchange grid file should be located
...
/*     mpp_close(g_fid); */
/*     if( ni%x_refine != 0 ) mpp_error("river_regrid: supergrid x-size can not
 * be divided by x_refine"); */
/*     if( nj%y_refine != 0 ) mpp_error("river_regrid: supergrid y-size can not
 * be divided by y_refine"); */
/*     nx   = ni/x_refine; */
/*     ny   = nj/y_refine; */
/*     nxp  = nx + 1; */
/*     nyp  = ny + 1; */
...
/*       if(fabs(river_data[n].landfrac[m]-1) < EPSLN) river_data[n].landfrac[m]
 * = 1; */
/*       if(fabs(river_data[n].landfrac[m])   < EPSLN) river_data[n].landfrac[m]
 * = 0; */
...
/*   for(n=1; n<ntiles; n++) { */
/*     if(river_data[n].nx != river_data[0].nx || river_data[n].ny !=
 * river_data[0].ny ) */
/*       mpp_error("river_regrid: all the tiles should have the same grid
 * size"); */
/*   } */
...
/* 	if(ntiles !=1) mpp_error("river_regrid: number of tiles must be 1 for
 * single contact"); */
/* 	for(n=0; n<ncontact; n++) { */
/* 	  if(istart1[n] == iend1[n] && istart2[n] == iend2[n] ) /\* x_cyclic *\/
 */
/* 	    *opcode |= X_CYCLIC; */
/* 	  else if(jstart1[n] == jend1[n] && istart2[n] == iend2[n] ) /\*
 * x_cyclic *\/ */
/* 	    *opcode |= Y_CYCLIC; */
/* 	  else */
/* 	    mpp_error("river_regrid: for one-tile mosaic, the boundary condition
 * should be either x-cyclic or y-cyclic"); */
/* 	} */
/*       } */
/*       else if(ncontact == 12) { */
/* 	if(ntiles != 6) mpp_error("river_regrid: the mosaic must be a 6-tile
 * cubic grid for 12 contacts."); */
/* 	*opcode |= CUBIC_GRID; */
/*       } */
/*       else */
/* 	mpp_error("river_regrid: the number of contact should be either 0, 1, 2
 * or 6"); */
/*     } */
...
/*   ----------------------------------------------------------------------------*\/
 */
/* void init_river_data(int ntiles, river_type *river_out, const river_type *
 * const river_in) */
/* { */
...
/* /\*------------------------------------------------------------------------------
 */
/*   void update_halo_double(int ntiles, double *data, int nx, int ny, unsigned
 * int opcode) */
...
/*       if(n%2) { /\* tile 2 4 6 *\/ */
/* 	tw = (n+5)%ntiles; te = (n+2)%ntiles; ts = (n+4)%ntiles; tn =
 * (n+1)%ntiles; */
/* 	for(j=1; j<nyp1; j++) { */
...
/* 	} */
/*       } else {  /\* tile 1, 3, 5 *\/ */
/*        tw = (n+4)%ntiles; te = (n+1)%ntiles; ts = (n+5)%ntiles; tn =
 * (n+2)%ntiles; */
/* 	for(j=1; j<nyp1; j++) { */
...
/* void update_halo_int(int ntiles, int **data, int nx, int ny, unsigned int
 * opcode) */
/* { */
/*   double **ldata; */
/*   int n, i, j, nxp2, nyp2; */
...
/* 	if(subA_me > suba_cutoff ) { /\* tocell will be land cell with larger
 * subA value, instead of neighboring ocean cell. *\/ */
 ...
 * cellarea. */
/*   For each basin, one and only one river point will have tocell = 0, this
 * point will */
/*   have travel = 1. */
/*   ----------------------------------------------------------------------------*\/
 */
 ...
 * river_data[n].area[j*nx+i]*river_data[n].landfrac[j*nx+i]; */
...
/* 	    i_dest = ii + di[dir]; */
/* 	    j_dest = jj + dj[dir]; */
/* 	    if(i_dest == i && j_dest == j) psubA[n][j*nxp2+i] +=
 * psubA[n][jj*nxp2+ii]; */
/* 	  } */
/* 	} */
/*       } */
...
/* 	  printf("At ocean points (i=%d,j=%d), subA = %f, tocell = %d, travel =
 * %d, basin = %d.\n ", */
/* 		 i, j, subA, tocell, travel, basin); */
/* 	  mpp_error("river_regrid, subA, tocell, travel, or basin is not missing
 * value for some ocean points"); */
/* 	} */
/*       } */
/*       else { */
/* 	if(tocell == tocell_missing || travel == travel_missing || */
/* 	   basin == basin_missing || subA == subA_missing) { */
/* 	  printf("At river points (i=%d,j=%d), subA = %f, tocell = %d, travel =
 * %d, basin = %d.\n ", */
/* 		 i, j, subA, tocell, travel, basin); */
...
/* 	      ncoast_full_land++; */
/* 	      printf("At point (%d,%d), tocell = 0 and landfrac = 1 is a coast
 * point\n", i, j); */
/* 	      goto done_check; */
/* 	    } */
/* 	  } */
/* 	} */
/* 	 printf("At point (%d,%d), tocell = 0 and landfrac = 1 is a sink
 * point\n", i, j); */
/*         nsink++; */
/*       done_check: continue;	 */
/*       } */
...
/*   if(nsink > 0) */
/*     printf("Warning from river_regrid: there are %d sink points is a full
 * land cell\n", nsink); */
/*   else */
/*     printf("NOTE from river_regrid: there are no sink points is a full land
 * cell\n"); */

/*   /\* check river travel to make sure there is one and only one point with
 * travel = 0. *\/ */
...
/*     } */
/*     else if(ncoast[n] >1) { */
/*       printf("river with basin = %d has more than one point with travel =
 * 0.\n", n+1); */
/*       mpp_error("river_regrid: some river has more than one point with travel
 * = 0"); */
/*     } */
/*   } */
/*   free(ncoast); */
...
/*       if( basin > maxbasin || basin < 1) mpp_error("river_regrid: basin
 * should be between 1 and maxbasin"); */
...
/*   for(n=0; n<maxbasin; n++) { */
/*     if(maxsuba[n] == subA_missing) mpp_error("river_regrid: maxsuba is not
 * assigned for some basin"); */
/*   } */
...
/*       if( basin > maxbasin || basin < 1) mpp_error("river_regrid: basin
 * should be a positive integer no larger than maxbasin"); */
...
/*     id_celllength = mpp_def_var(fid, celllength_name, MPP_DOUBLE, 2, dimid ,
 * 0); */
...
/* 			      "long_name", "Geographic longitude", "units",
 * "degree_east"); */
/*     id_y        = mpp_def_var(fid, y_name, MPP_DOUBLE, 2, dimid, 2, */
/* 			      "long_name", "Geographic latitude", "units",
 * "degree_north");     */
/*     mpp_end_def(fid); */
 ...
 */
/*   void qsort_index(double array[], int start, int end, int rank[]) */
/*   sort array in increasing order and get the rank of each member in the
 * original array */
...
/*       swap(array[start], array[pivot_index], tmp_double); /\*** choose
 * arbitrary pivot ***\/ */
