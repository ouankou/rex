...
void filter_topo(int nx, int ny, int num_pass, int smooth_topo_allow_deepening,
                 double *depth);
void set_depth(int nx, int ny, const double *xbnd, const double *ybnd,
               double alat1, double slon1, double elon1, double alat2,
...
/*********************************************************************
   ...
   Constructing a rectangular basin with a flat bottom, basin_depth can be 0 (
 all land).
 ********************************************************************/
void create_rectangular_topog(int nx, int ny, double basin_depth,
                              double *depth) {
  int i;
 ...
 ********************************************************************/
void create_bowl_topog(int nx, int ny, const double *x, const double *y,
                       double bottom_depth, double min_depth, double bowl_east,
                       double bowl_south, double bowl_west, double bowl_north,
                       double *depth) {
  double bottom, xx, yy;
  int i, j, nxp, nyp;
  ...
  for (j = 0; j < ny; j++) {
    for (i = 0; i < nx; i++) {
      xx = (x[j * nxp + i] + x[j * nxp + i + 1] + x[(j + 1) * nxp + i] +
            x[(j + 1) * nxp + i + 1]) *
           0.25;
      yy = (y[j * nxp + i] + y[j * nxp + i + 1] + y[(j + 1) * nxp + i] +
            y[(j + 1) * nxp + i + 1]) *
           0.25;
      if (xx <= bowl_west || xx >= bowl_east || yy <= bowl_south ||
          yy >= bowl_north)
        bottom = min_depth;
      else
        bottom = min_depth + bottom_depth *
                                 (1.0 - exp(-pow((yy - bowl_south) / 2.0, 2))) *
 ...
 ********************************************************************/
void create_gaussian_topog(int nx, int ny, const double *x, const double *y,
                           double bottom_depth, double min_depth,
  ...
  for (j = 0; j < ny; j++) {
    for (i = 0; i < nx; i++) {
      xt[j * nx + i] = (x[j * nxp + i] + x[j * nxp + i + 1] +
                        x[(j + 1) * nxp + i] + x[(j + 1) * nxp + i + 1]) *
                       0.25;
      yt[j * nx + i] = (y[j * nxp + i] + y[j * nxp + i + 1] +
                        y[(j + 1) * nxp + i] + y[(j + 1) * nxp + i + 1]) *
                       0.25;
    }
  }
  ...
  printf("Constructing a gaussian bump of height = %f meters with a scale "
         "width of %f degrees.\n",
         bump_height, bump_scale);
  printf("The bump is centered at (lon,lat) = (%f,%f) deg.\n", xcent, ycent);
  printf("The ocean floor rises with a slope of %f meters/deg towards the east "
         "and %f "
...
/*********************************************************************
  ...
  void create_idealized_topog
   ...
   Note: this routine needs to be re-thought for generalized curvilinear
 coordinates
...
void create_idealized_topog(int nx, int ny, const double *x, const double *y,
  ...
  for (i = 0; i < nx; i++)
    xbnd[i] = (x[i] + x[i + 1]) * 0.5;
  for (j = 0; j < ny; j++)
    ybnd[j] = (y[j * nxp] + y[(j + 1) * nxp]) * 0.5;

  set_depth(nx, ny, xbnd, ybnd, -90.0, 0.0, 360.0, -80.0, 0.0, 360.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, -80.0, 360.0 - 25.0, 360.0, -70.0, 360.0, 360.0,
            0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, -80.0, 0.0, 360.0, -70.0, 0.0, 170.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, -80.0, 360.0 - 135.0, 360.0 - 60.0, -68.0,
            360.0 - 75.0, 360.0 - 60.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, -70.0, 0.0, 155.0, -67.0, 50.0, 145.0, 0.0,
            depth);

  // australia

  set_depth(nx, ny, xbnd, ybnd, -35.0, 116.0, 120.0, -31.0, 114.0, 130.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, -38.0, 140.0, 151.0, -31.0, 130.0, 151.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, -31.0, 115.0, 153.0, -20.0, 113.0, 149.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, -20.0, 113.0, 149.0, -11.0, 131.0, 143.0, 0.0,
            depth);

  // south america

  set_depth(nx, ny, xbnd, ybnd, -50.0, 360.0 - 74.0, 360.0 - 68.0, -40.0,
            360.0 - 73.0, 360.0 - 62.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, -40.0, 360.0 - 73.0, 360.0 - 62.0, -20.0,
            360.0 - 70.0, 360.0 - 40.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, -20.0, 360.0 - 70.0, 360.0 - 40.0, -16.0,
            360.0 - 81.0, 360.0 - 35.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, -16.0, 360.0 - 81.0, 360.0 - 35.0, 0.0,
            360.0 - 80.0, 360.0 - 50.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 0.0, 360.0 - 80.0, 360.0 - 50.0, 11.0,
            360.0 - 75.0, 360.0 - 60.0, 0.0, depth);

  // central america

  set_depth(nx, ny, xbnd, ybnd, 6.0, 360.0 - 78.0, 360.0 - 75.0, 20.0,
            360.0 - 105.0, 360.0 - 97.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 20.0, 360.0 - 105.0, 360.0 - 97.0, 30.0,
            360.0 - 115.0, 360.0 - 94.0, 0.0, depth);

  // north america

  set_depth(nx, ny, xbnd, ybnd, 25.0, 360.0 - 82.0, 360.0 - 80.0, 30.0,
            360.0 - 85.0, 360.0 - 81.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 30.0, 360.0 - 115.0, 360.0 - 80.0, 40.0,
            360.0 - 124.0, 360.0 - 74.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 40.0, 360.0 - 124.0, 360.0 - 74.0, 50.0,
            360.0 - 124.0, 360.0 - 57.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 50.0, 360.0 - 124.0, 360.0 - 57.0, 60.0,
            360.0 - 140.0, 360.0 - 64.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 60.0, 360.0 - 165.0, 360.0 - 64.0, 65.0,
            360.0 - 140.0, 360.0 - 64.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 65.0, 360.0 - 140.0, 360.0 - 64.0, 70.0,
            360.0 - 162.0, 360.0 - 72.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 70.0, 360.0 - 162.0, 360.0 - 140.0, 72.0,
            360.0 - 157.0, 360.0 - 157.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 70.0, 360.0 - 130.0, 360.0 - 70.0, 75.0,
            360.0 - 120.0, 360.0 - 80.0, 0.0, depth);

  // greenland

  set_depth(nx, ny, xbnd, ybnd, 60.0, 360.0 - 45.0, 360.0 - 45.0, 75.0,
            360.0 - 58.0, 360.0 - 19.0, 0.0, depth);
  ...
  set_depth(nx, ny, xbnd, ybnd, 6.0, 360.0 - 10.0, 360.0, 18.0, 360.0 - 18.0,
            360.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 18.0, 360.0 - 18.0, 360.0, 26.0, 360.0 - 15.0,
            360.0, 0.0, depth);

  // northern africa and europe and asia

  set_depth(nx, ny, xbnd, ybnd, 26.0, 360.0 - 15.0, 360.0, 40.0, 360.0 - 7.0,
            360.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 40.0, 360.0 - 7.0, 360.0, 50.0, 360.0, 360.0,
            0.0, depth);

  set_depth(nx, ny, xbnd, ybnd, 8.0, 77.0, 78.0, 26.0, 65.0, 90.0, 0.0, depth);
  set_depth(nx, ny, xbnd, ybnd, 4.0, 99.0, 100.0, 26.0, 90.0, 115.0, 0.0,
            depth);
  ...
  set_depth(nx, ny, xbnd, ybnd, 60.0, 8.0, 163.0, 65.0, 13.0, 180.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, 65.0, 13.0, 188.0, 70.0, 20.0, 180.0, 0.0,
            depth);
  set_depth(nx, ny, xbnd, ybnd, 70.0, 70.0, 180.0, 75.0, 90.0, 100.0, 0.0,
            depth);
  ...
  for (j = 0; j < ny; j++) {
    for (i = 0; i < nx; i++) {
      if (depth[j * nx + i] > 0.0) {
        arg = bottom_depth * (1 - 0.4 * fabs(cos(((j + 1) * M_PI) / nyp) *
                                             sin(((i + 1) * 2 * M_PI) / nxp)));
  ...
  set_depth(nx, ny, xbnd, ybnd, -20.0, 360.0 - 20.0, 360.0 - 10.0, 30.0,
            360.0 - 45.0, 360.0 - 35.0, arg, depth);
  set_depth(nx, ny, xbnd, ybnd, 30.0, 360.0 - 45.0, 360.0 - 35.0, 60.0,
            360.0 - 20.0, 360.0 - 30.0, arg, depth);
  set_depth(nx, ny, xbnd, ybnd, -60.0, 360.0 - 100.0, 360.0 - 130.0, 40.0,
            360.0 - 160.0, 180.0, arg, depth);

  arg = 0.5 * bottom_depth;

  set_depth(nx, ny, xbnd, ybnd, -50.0, 360.0 - 120.0, 360.0 - 120.0, 30.0,
            190.0, 190.0, arg, depth);
...
/*********************************************************************
   void set_depth(double alat1, double slon1, double elon1, double alat2,
        ...
        set the topography depth "depth[i][j](i,j)" = "depth_in" within the area
 of the trapezoid bounded by vertices: (alat1,slon1), (alat1,elon1),
 (alat2,slon2), and (alat2,elon2)
 ...
 ********************************************************************/
void create_realistic_topog(int nx_dst, int ny_dst, const double *x_dst,
                            const double *y_dst, const char *topog_file,
                            const char *topog_field, double scale_factor,
                            double fill_first_row, int filter_topog,
                            int num_filter_pass,
                            int smooth_topo_allow_deepening, int round_shallow,
                            int fill_shallow, int deepen_shallow,
  ...
  for (j = 0; j < nyp_src; j++) {
    for (i = 1; i < nx_src; i++)
      x_src[j * nxp_src + i] = (xt_src[i - 1] + xt_src[i]) * 0.5;
    x_src[j * nxp_src] = 2 * xt_src[0] - x_src[j * nxp_src + 1];
    x_src[j * nxp_src + nx_src] =
        2 * xt_src[nx_src - 1] - x_src[j * nxp_src + nx_src - 1];
  }
  for (i = 0; i < nxp_src; i++) {
    for (j = 1; j < ny_src; j++)
      y_src[j * nxp_src + i] = (yt_src[j - 1] + yt_src[j]) * 0.5;
    y_src[i] = 2 * yt_src[0] - y_src[nxp_src + i];
    y_src[ny_src * nxp_src + i] =
        2 * yt_src[ny_src - 1] - y_src[(ny_src - 1) * nxp_src + i];
  }
  ...
  if (filter_topog)
    filter_topo(nx_dst, ny_dst, num_filter_pass, smooth_topo_allow_deepening,
                depth);
  /* enforce minimum depth when one of round_shallow, deepen_shallow or
     fill_shallow is true ( At most one ).         */
  count = 0;
  if (round_shallow)
    count++;
  if (fill_shallow)
  ...
  if (deepen_shallow)
    count++;
  if (count > 1)
    mpp_error("topog: at most one of round_shallow/deepen_shallow/fill_shallow "
              "can be set to true");

  if (count > 0)
    enforce_min_depth(nx_dst, ny_dst, round_shallow, deepen_shallow,
                      fill_shallow, min_depth, depth);
        ...
        for (ip = -1; ip <= 1; ip++) {
          for (jp = -1; jp <= 1; jp++) {
            ...
            if (rmask[(j + jp) * nx + i + ip] == 0.0)
              ...
              s[j * nx + i] =
                  s[j * nx + i] + depth[(j + jp) * nx + i + ip] * f[m];
          }
        }
...
void enforce_min_depth(int nx, int ny, int round_shallow, int deepen_shallow,
  ...
  if (mpp_pe() == mpp_root_pe())
    printf(" Enforcing the minimum topography depth to be %f\n", min_depth);
      ...
      for (i = 0; i < nx; i++) {
        ...
        if (depth[l] != 0 && depth[l] < min_depth) {
          ...
          if (depth[l] < 0.5 * min_depth) {
            ...
            printf("Setting location i,j,depth= %d,%d,%f to minimum ocean "
                   "depth.\n",
                   i, j, depth[l]);
            depth[l] = min_depth;
          }
    ...
    for (j = 0; j < ny; j++) {
      for (i = 0; i < nx; i++) {
        ...
        if (depth[l] != 0 && depth[l] < min_depth) {
          n = n + 1;
          printf(
              "Setting location i,j,depth= %d,%d,%f to minimum ocean depth.\n",
              i, j, depth[l]);
          depth[l] = min_depth;
        }
      }
