...
void cubic_spline(int size1, int size2, const double *grid1,
                  const double *grid2, const double *data1, double *data2,
  ...
  for (i = 1; i < size1; i++) {
    if (grid1[i] <= grid1[i - 1])
      error_handler("cubic_spline: grid1 is not monotonic increasing");
  }

  for (i = 0; i < size2; i++) {
    if (grid2[i] < grid1[0] || grid2[i] > grid1[size1 - 1])
      error_handler("cubic_spline: grid2 lies outside grid1");
  }

  if (size1 < 2)
    error_handler("cubic_spline: the size of input grid should be at least 2");
  if (size1 ==
      2) { /* when size1 is 2, it just reduced to a linear interpolation */
    p = (data1[1] - data1[0]) / (grid1[1] - grid1[0]);
    for (i = 0; i < size2; i++)
      data2[i] = p * (grid2[i] - grid1[0]) + data1[0];
    return;
  }
  ...
  if (yp1 > .99e30) {
    ...
    y2[0] = -0.5;
    u[0] = (3. / (grid1[1] - grid1[0])) *
           ((data1[1] - data1[0]) / (grid1[1] - grid1[0]) - yp1);
  }
  ...
  for (i = 1; i < size1 - 1; i++) {
    ...
    y2[i] = (sig - 1.) / p;
    u[i] = (6. *
                ((data1[i + 1] - data1[i]) / (grid1[i + 1] - grid1[i]) -
                 (data1[i] - data1[i - 1]) / (grid1[i] - grid1[i - 1])) /
                (grid1[i + 1] - grid1[i - 1]) -
            sig * u[i - 1]) /
           p;
  }
  ...
  if (ypn > .99e30) {
    ...
    qn = 0.5;
    un = (3. / (grid1[size1 - 1] - grid1[size1 - 2])) *
         (ypn - (data1[size1 - 1] - data1[size1 - 2]) /
                    (grid1[size1 - 1] - grid1[size1 - 2]));
  }
  ...
  for (k = 0; k < size2; k++) {
    ...
    data2[k] = a * data1[klo] + b * data1[khi] +
               ((pow(a, 3.0) - a) * y2[klo] + (pow(b, 3.0) - b) * y2[khi]) *
                   (pow(h, 2.0)) / 6;
  }
...
/*------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
void conserve_interp(int nx_src, int ny_src, int nx_dst, int ny_dst,
                     const double *x_src, const double *y_src,
                     const double *x_dst, const double *y_dst,
                     const double *mask_src, const double *data_src,
                     double *data_dst) {
  int n, nxgrid;
  ...
  nxgrid = create_xgrid_2dx2d_order1(&nx_src, &ny_src, &nx_dst, &ny_dst, x_src,
                                     y_src, x_dst, y_dst, mask_src, xgrid_i1,
                                     xgrid_j1, xgrid_i2, xgrid_j2, xgrid_area);
  get_grid_area(&nx_dst, &ny_dst, x_dst, y_dst, dst_area);
  area_frac = (double *)malloc(nxgrid * sizeof(double));
  for (n = 0; n < nxgrid; n++)
    area_frac[n] = xgrid_area[n] / dst_area[xgrid_j2[n] * nx_dst + xgrid_i2[n]];

  for (n = 0; n < nx_dst * ny_dst; n++) {
    data_dst[n] = 0;
  }
  for (n = 0; n < nxgrid; n++) {
    data_dst[xgrid_j2[n] * nx_dst + xgrid_i2[n]] +=
        data_src[xgrid_j1[n] * nx_src + xgrid_i1[n]] * area_frac[n];
  }
