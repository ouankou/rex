/* This is part of the libcf package. Copyright 2007 University
   ...
   $Id: tst_subsets.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
#define FILE_NAME_SIMPLE_SIGMA "tst_subsets_simple_sigma.nc"
  ...
  printf("*** testing index size stuff...");
  {
    ...
    if (nc_create(FILE_NAME_SIMPLE_SIGMA, 0, &ncid))
      ERR;
    if (nccf_def_file(ncid, TITLE, HISTORY))
      ERR;
    if (nccf_def_latitude(ncid, NLATS, NC_INT, &lat_did, &lat_vid))
      ERR;
    if (nccf_def_longitude(ncid, NLONS, NC_INT, &lon_did, &lon_vid))
      ERR;

    /* Now define sigma level coordinage var and dim. */
    if (nccf_def_lvl_sigma(ncid, "sigma", NC_FLOAT, NLVLS, &lvl_did, &lvl_vid))
      ERR;

    /* Now define the time coordinate. */
    if (nccf_def_time(ncid, "time", NC_UNLIMITED, NC_FLOAT, TIME_UNITS, "time",
                      &time_did, &time_vid))
      ERR;
    ...
    dimids[0] = lat_did;
    dimids[1] = lon_did;
    if (nc_def_var(ncid, "ps", NC_FLOAT, 2, dimids, &ps_vid))
      ERR;
    if (nc_def_var(ncid, "PTOP", NC_FLOAT, 0, NULL, &ptop_vid))
      ERR;

    /* Save the ps and ptop info in the formula terms attribute. */
    if (nccf_def_ft_sigma(ncid, lvl_vid, ps_vid, ptop_vid))
      ERR;
    ...
    dimids[2] = lat_did;
    dimids[3] = lon_did;
    if (nc_def_var(ncid, "data", NC_FLOAT, NDIMS, dimids, &data_vid))
      ERR;
    ...
    /* Write lats and lons. */
    if (nc_put_var_int(ncid, lat_vid, lat))
      ERR;
    if (nc_put_var_int(ncid, lon_vid, lon))
      ERR;
    ...
    for (start[0] = 0; start[0] < MAX_REC; start[0]++)
      if (nc_put_vara(ncid, data_vid, start, count, &data))
        ERR;
    ...
    /* Check the file. */
    if (nc_open(FILE_NAME_SIMPLE_SIGMA, 0, &ncid))
      ERR;
    if (nc_inq_varid(ncid, "data", &data_vid_in))
      ERR;
    if (data_vid_in != data_vid)
      ERR;
    if (nccf_get_vara(ncid, data_vid, NULL, &nlat_in, NULL, &nlon_in, 0, 0,
                      data_in))
      ERR;
    if (nlat_in != NLATS || nlon_in != NLONS)
      ERR;
    if (nccf_get_vara(ncid, data_vid, bad_lat_bounds, &nlat_in, bad_lon_bounds,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 0 || nlon_in != 0)
      ERR;
    if (nccf_get_vara(ncid, data_vid, lat_bounds, &nlat_in, lon_bounds,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 1 || nlon_in != 1)
      ERR;
...
#define FILE_NAME_NW "tst_subsets_nw.nc"
  {
    ...
    float simple_lat_range2[] = {50.0, 50.5},
          simple_lon_range2[] = {-179.0, -178.5};
    ...
    /* Create a file and use nccd_def_file to annotate it. */
    if (nc_create(FILE_NAME_NW, 0, &ncid))
      ERR;
    if (nccf_def_file(ncid, TITLE, HISTORY))
      ERR;
    if (nccf_def_latitude(ncid, NLATS_N, NC_INT, &lat_did, &lat_vid))
      ERR;
    if (nccf_def_longitude(ncid, NLONS_W, NC_INT, &lon_did, &lon_vid))
      ERR;
    ...
    if (nc_put_var_float(ncid, data_vid, &data[0][0]))
      ERR;
    ...
    /* Check the file. */
    if (nc_open(FILE_NAME_NW, 0, &ncid))
      ERR;
    if (nc_inq_varid(ncid, DATA_VAR_NAME_SIMPLE, &data_vid))
      ERR;

    /* All lons, one lat. */
    if (nccf_get_vara(ncid, data_vid, simple_lat_range, &nlat_in, NULL,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 1 || nlon_in != NLONS_W)
      ERR;

    /* All lats, one lon. */
    if (nccf_get_vara(ncid, data_vid, NULL, &nlat_in, simple_lon_range,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != NLATS_N || nlon_in != 1)
      ERR;

    /* These should all yeild no subset. */
    if (nccf_get_vara(ncid, data_vid, simple_lat_range, &nlat_in, bad_lon_range,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 1 || nlon_in != 0)
      ERR;
    if (nccf_get_vara(ncid, data_vid, bad_lat_range, &nlat_in, NULL, &nlon_in,
                      0, 0, data_in))
      ERR;
    if (nlat_in != 0 || nlon_in != 180)
      ERR;
    if (nccf_get_vara(ncid, data_vid, bad_lat_range, &nlat_in, bad_lon_range,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 0 || nlon_in != 0)
      ERR;
    if (nccf_get_vara(ncid, data_vid, bad_lat_range, &nlat_in, simple_lon_range,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 0 || nlon_in != 1)
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, NULL, &nlat_in, NULL, &nlon_in, 0, 0,
                      data_in))
      ERR;
    if (nlat_in != NLATS_N || nlon_in != NLONS_W)
      ERR;

    /* Read a small subset (1 value). */
    if (nccf_get_vara(ncid, data_vid, simple_lat_range, &nlat_in,
                      simple_lon_range, &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != NLATS_SIMPLE || nlon_in != NLONS_SIMPLE)
      ERR;
    if (data_in[0][0] !=
        data[(int)simple_lat_range[0]][abs((int)simple_lon_range[0])])
      ERR;

    /* Read a different small subset (1 value). */
    if (nccf_get_vara(ncid, data_vid, simple_lat_range2, &nlat_in,
                      simple_lon_range2, &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != NLATS_SIMPLE || nlon_in != NLONS_SIMPLE)
      ERR;
    if (data_in[0][0] !=
        data[(int)simple_lat_range2[0]][abs((int)simple_lon_range2[0])])
      ERR;

    /* Read a big subset (approximate CONUS). */
    if (nccf_get_vara(ncid, data_vid, conus_lat_range, &nlat_in,
                      conus_lon_range, &nlon_in, 0, 0, data_conus))
      ERR;
    if (nlat_in != NLATS_CONUS || nlon_in != NLONS_CONUS)
      ERR;
    for (j = 0; j < conus_lat_index[1] - conus_lat_index[0]; j++)
      for (k = 0; k < conus_lat_index[1] - conus_lat_index[0]; k++)
        if (data_conus[j][k] !=
            data[j + conus_lat_index[0]][k + conus_lon_index[0]])
          ERR;
  ...
  printf("*** testing simple subsetting case, with 2D data around South "
         "America...");
#define NLONS_W 180
#define NLATS_S 90
#define NLONS_SIMPLE 1
...
#define FILE_NAME_SW "tst_subsets_sw.nc"
  {
    ...
    float simple_lat_range[] = {-30.0, -29.5},
          simple_lon_range[] = {-1.0, -0.5};
    float simple_lat_range2[] = {-50.0, -49.5},
          simple_lon_range2[] = {-179.0, -178.5};
    ...
    /* Create a file and use nccd_def_file to annotate it. */
    if (nc_create(FILE_NAME_SW, 0, &ncid))
      ERR;
    if (nccf_def_file(ncid, TITLE, HISTORY))
      ERR;
    if (nccf_def_latitude(ncid, NLATS_S, NC_INT, &lat_did, &lat_vid))
      ERR;
    if (nccf_def_longitude(ncid, NLONS_W, NC_INT, &lon_did, &lon_vid))
      ERR;
    ...
    /* Check the file. */
    if (nc_open(FILE_NAME_SW, 0, &ncid))
      ERR;
    if (nc_inq_varid(ncid, DATA_VAR_NAME_SIMPLE, &data_vid))
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, NULL, &nlat_in, NULL, &nlon_in, 0, 0,
                      data_in))
      ERR;
    if (nlat_in != NLATS_S || nlon_in != NLONS_W)
      ERR;
    ...
    if (nlat_in != NLATS_SIMPLE || nlon_in != NLONS_SIMPLE)
      ERR;
    if (data_in[0][0] !=
        data[abs((int)simple_lat_range[0])][abs((int)simple_lon_range[0])])
      ERR;
    ...
    if (nlat_in != NLATS_SIMPLE || nlon_in != NLONS_SIMPLE)
      ERR;
    if (data_in[0][0] !=
        data[abs((int)simple_lat_range2[0])][abs((int)simple_lon_range2[0])])
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, sa_lat_range, &nlat_in, sa_lon_range,
                      &nlon_in, 0, 0, data_sa))
      ERR;
    if (nlat_in != NLATS_SA || nlon_in != NLONS_SA)
      ERR;
    for (j = 0; j < NLATS_SA; j++)
      for (k = 0; k < NLONS_SA; k++)
        if (data_sa[j][k] != data[j + sa_lat_index[0]][k + sa_lon_index[0]])
          ERR;
  ...
  printf(
      "*** testing simple subsetting case, with 2D data around Australia...");
#define NLONS_HEMI 180
#define NLATS_HEMI 90
#define NLONS_SIMPLE 1
#define NLATS_SIMPLE 1
...
#define FILE_NAME_SE "tst_subsets_se.nc"
  {
    ...
    /* Create a file and use nccd_def_file to annotate it. */
    if (nc_create(FILE_NAME_SE, 0, &ncid))
      ERR;
    if (nccf_def_file(ncid, TITLE, HISTORY))
      ERR;
    if (nccf_def_latitude(ncid, NLATS_HEMI, NC_INT, &lat_did, &lat_vid))
      ERR;
    if (nccf_def_longitude(ncid, NLONS_HEMI, NC_INT, &lon_did, &lon_vid))
      ERR;
    ...
    /* Check the file. */
    if (nc_open(FILE_NAME_SE, 0, &ncid))
      ERR;
    if (nc_inq_varid(ncid, DATA_VAR_NAME_SIMPLE, &data_vid))
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, simple_lat_range, &nlat_in, NULL,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != 1 || nlon_in != NLONS_HEMI)
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, NULL, &nlat_in, simple_lon_range,
                      &nlon_in, 0, 0, data_in))
      ERR;
    if (nlat_in != NLATS_HEMI || nlon_in != 1)
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, NULL, &nlat_in, NULL, &nlon_in, 0, 0,
                      data_in))
      ERR;
    if (nlat_in != NLATS_HEMI || nlon_in != NLONS_HEMI)
      ERR;
    ...
    if (nccf_get_vara(ncid, data_vid, aus_lat_range, &nlat_in, aus_lon_range,
                      &nlon_in, 0, 0, data_aus))
      ERR;
    if (nlat_in != NLATS_AUS || nlon_in != NLONS_AUS)
      ERR;
...
#define FILE_NAME_NE "tst_subsets_ne.nc"
  {
    ...
    /* Create a file and use nccd_def_file to annotate it. */
    if (nc_create(FILE_NAME_NE, 0, &ncid))
      ERR;
    if (nccf_def_file(ncid, TITLE, HISTORY))
      ERR;
    if (nccf_def_latitude(ncid, NLATS_HEMI, NC_INT, &lat_did, &lat_vid))
    ...
    /* Check the file. */
    if (nc_open(FILE_NAME_NE, 0, &ncid))
      ERR;
    if (nc_inq_varid(ncid, DATA_VAR_NAME_SIMPLE, &data_vid))
      ERR;
    ...
    /* Read a big subset (approximatly Australia). */
    if (nccf_get_vara(ncid, data_vid, japan_lat_range, &nlat_in,
                      japan_lon_range, &nlon_in, 0, 0, data_aus))
      ERR;
    if (nlat_in != NLATS_JAPAN || nlon_in != NLONS_JAPAN)
      ERR;
