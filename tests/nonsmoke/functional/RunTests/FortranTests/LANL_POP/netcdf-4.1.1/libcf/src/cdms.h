 ...
 *              drach@llnl.gov
 *
 * Version:     $Id: cdms.h,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
 *
 * Revision History:
 *
 * $Log: cdms.h,v $
...
typedef enum cdIntersectPolicy {
  cdRoundNearest = CW_ROUND_NEAREST,
  cdRange = CW_RANGE
} cdIntersectPolicy;

typedef enum cdRepresent {
  cdExternalArray = 1,
  cdInternalArray,
  cdLinearRep
} cdRepresent;
typedef enum cdOpenMode { cdReadOnly = 1, cdReadWrite } cdOpenMode;
typedef enum cdGridType {
  cdRectangular = 1,
  cdZonal,
  cdMeridional,
  cdProjected,
 ...
 * =================================================================
 */
extern int cdDimInq(long dsetid, long dimid, char *name, char *alias,
                    cdType *datatype, int *natts, cdDimType *type, int *subtype,
                    long *length, cdDimShape *topology);
extern int cdDsetClose(long dsetid);
extern int cdDsetInq(long dsetid, int *ndims, int *nvars, int *ngrids,
                     int *ngatts);
extern int cdGridInq(long dsetid, long gridid, char *name, cdGridType *type,
                     int *ndims, long dim[], int *natts);
...
extern int cdVarGetIndexRange(long dsetid, long varid, long dimid, long *index1,
                              long *index2);
extern int cdVarInq(long dsetid, long varid, char *name, char *alias,
                    cdType *datatype, int *ndims, long dim[], int *natts);
extern int cdVarSetCoordRange(long dsetid, long varid, long dimid,
                              double coord1, double coord2, cdIntersectOpt opt,
                              long *len);
extern int cdVarSetIndex(long dsetid, long varid, long dimid, long nindices,
                         long index[], long *len);
extern int cdVarSetIndexRange(long dsetid, long varid, long dimid, long index1,
                              long index2, long *len);
...
extern void cdChar2Comp(cdCalenType timetype, char *chartime,
                        cdCompTime *comptime);
extern void cdChar2Rel(cdCalenType timetype, char *chartime, char *relunits,
                       double *reltime);
extern void cdComp2Char(cdCalenType timetype, cdCompTime comptime, char *time);
extern void cdComp2Rel(cdCalenType timetype, cdCompTime comptime,
                       char *relunits, double *reltime);
extern void cdRel2Char(cdCalenType timetype, char *relunits, double reltime,
                       char *chartime);
extern void cdRel2Comp(cdCalenType timetype, char *relunits, double reltime,
                       cdCompTime *comptime);
extern void cdRel2Rel(cdCalenType timetype, char *relunits, double reltime,
                      char *outunits, double *outtime);
extern int cdAbs2Comp(char *absunits, void *abstime, cdType abstimetype,
                      cdCompTime *comptime, double *frac);
extern int cdComp2Abs(cdCompTime comptime, char *absunits, cdType abstimetype,
                      double frac, void *abstime);
extern int cdDecodeRelativeTime(cdCalenType timetype, char *units, double time,
                                cdCompTime *comptime);
extern int cdDecodeAbsoluteTime(char *units, void *time, cdType abstimetype,
                                cdCompTime *comptime, double *fraction);

extern long cdSlabRead(long dsetid, long varid, const long order[],
                       const double first[], const double last[],
                       const double modulus[], cdIntersectPolicy policy,
                       cdType usertype, void *values);
extern long cdNdimIntersect(long dsetid, long varid, const long order[],
                            const double first[], const double last[],
                            const double modulus[], cdIntersectPolicy policy,
...
extern int cevargeta(int fileid, int varid, const long start[],
                     const long count[], void *value);
extern int cemajority(CeMajority majority);
extern int cehyperslabinq(int fileid, int varid, int rank, long *order,
                          double *fe, double *le, double *cycle,
                          long *data_arraysize, int *dim_arraylengths);
extern int cehyperslab(int fileid, int varid, int rank, long *order, double *fe,
                       double *le, double *cycle, double *dimarray, void *data);
extern int cehyperslaba(int fileid, int varid, int rank, long *order,
                        double *fe, double *le, double *cycle, long dimsize,
                        double *dimarray, void *data);
extern int cehyperslabi(int fileid, int varid, int rank, long *order, long *fe,
                        long *le, double *cycle, void **data, int *dimsize,
                        double *dimarray[]);
