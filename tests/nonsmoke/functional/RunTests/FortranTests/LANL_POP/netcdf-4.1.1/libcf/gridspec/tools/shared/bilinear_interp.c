...
void redu2x(const double *varfin, const double *yfin, int nxfin, int nyfin,
            double *varcrs, int nxcrs, int nycrs, int nz, int has_missing,
            double missvalue);
void do_latlon_coarsening(const double *var_latlon, const double *ylat,
                          int nlon, int nlat, int nz, double *var_latlon_crs,
                          int finer_steps, int has_missing, double missvalue);
void do_c2l_interp(const Interp_config *interp, int nx_in, int ny_in, int nz,
                   const Field_config *field_in, int nx_out, int ny_out,
                   double *data_out, int has_missing, double missing,
                   int fill_missing);
void sort_index(int ntiles, int *index, double *shortest);
int get_index(const Grid_config *grid_in, const Grid_config *grid_out,
              int *index, int i_in, int j_in, int l_in, int i_out, int j_out);
/*******************************************************************************
  void setup_bilinear_interp( )
    ...
    !------------------------------------------------------------------!
*******************************************************************************/
void setup_bilinear_interp(int ntiles_in, const Grid_config *grid_in,
                           int ntiles_out, const Grid_config *grid_out,
  ...
  if (ntiles_in != 6)
    mpp_error(
        "Error from bilinear_interp: source mosaic should be cubic mosaic "
  ...
  if ((opcode & READ) && interp->file_exist) { /* reading from file */
    ...
    printf("NOTE: reading index and weight for bilinear interpolation from "
           "file.\n");
    ...
    if (nx2 != nx_out || ny2 != ny_out)
      mpp_error("bilinear_interp: size mismatch between grid size and remap "
                "file size");
    ...
    for (l = 0; l < ntiles_in; l++) {
      for (jc = 1; jc <= ny_in; jc++)
        for (ic = 1; ic <= nx_in; ic++) {
          ...
          j_min =
              max(1, floor((grid_in[l].latt[n1] - dcub + 0.5 * M_PI) / dlat) -
                         iter + 1);
          j_max = min(ny_out,
                      ceil((grid_in[l].latt[n1] + dcub + 0.5 * M_PI) / dlat) +
                          iter - 1);
          if (j_min == 1 || j_max == ny_out) {
            ...
            i_min =
                max(1, floor((grid_in[l].lont[n1] - dcub) / dlon - iter + 1));
            i_max = min(nx_out,
                        ceil((grid_in[l].lont[n1] + dcub) / dlon + iter - 1));
          }
            ...
            for (i = i_min - 1; i < i_max; i++) {
              ...
              if (!found[n0]) {
                ...
                /*------------------------------------------------
                  ...
                  ------------------------------------------------*/
                found[n0] = get_closest_index(
                    &(grid_in[l]), grid_out,
                    &(interp->index[3 * (j * nx_out + i)]), index[3 * l],
  ...
  for (j = 0; j < ny_out; j++)
    for (i = 0; i < nx_out; i++) {
      ...
      if (!found[n0]) {
        printf(
            "**************************************************************\n");
        printf("WARNING: didn't find lower left corner for (ilon,jlat) = "
               "(%d,%d)\n",
               i, j);
        ...
        for (l = 0; l < ntiles_in; l++) {
          if (!found[n0]) {
            found[n0] = get_index(
                &(grid_in[l]), grid_out, &(interp->index[3 * (j * nx_out + i)]),
                index[3 * l], index[3 * l + 1], index[3 * l + 2], i, j);
            if (found[n0])
              break;
          }
        }
        if (!found[n0])
          mpp_error(
              "error from bilinear_interp: couldn't find lower left corner");
      }
      /*------------------------------------------------------------
      ...
      if (ic == nx_in && jc == ny_in) {
        ...
        sum = interp->weight[m1] + interp->weight[m1 + 1] +
              interp->weight[m1 + 3];
        ...
        sum = interp->weight[m1] + interp->weight[m1 + 2] +
              interp->weight[m1 + 3];
        interp->weight[m1] /= sum;
        ...
        sum = interp->weight[m1] + interp->weight[m1 + 1] +
              interp->weight[m1 + 2] + interp->weight[m1 + 3];
        interp->weight[m1] /= sum;
        interp->weight[m1 + 1] /= sum;
        interp->weight[m1 + 2] /= sum;
        interp->weight[m1 + 3] /= sum;
...
/*----------------------------------------------------------------------------
   void do_scalar_bilinear_interp(Mosaic_config *input, Mosaic_config *output,
   int varid ) interpolate scalar data to latlon, !
   --------------------------------------------------------------------------*/
void do_scalar_bilinear_interp(const Interp_config *interp, int vid,
                               int ntiles_in, const Grid_config *grid_in,
                               const Grid_config *grid_out,
                               const Field_config *field_in,
                               Field_config *field_out, int finer_step,
                               int fill_missing) {
  ...
  do_c2l_interp(interp, nx_in, ny_in, nz, field_in, nx_out, ny_out, data_fine,
                has_missing, missing, fill_missing);
  do_latlon_coarsening(data_fine, grid_out->latt1D_fine, nx_out, ny_out, nz,
                       field_out->data, finer_step, has_missing, missing);
  free(data_fine);
...
/*----------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------*/
void do_vector_bilinear_interp(Interp_config *interp, int vid, int ntiles_in,
                               const Grid_config *grid_in, int ntiles_out,
                               const Grid_config *grid_out,
                               const Field_config *u_in,
                               const Field_config *v_in, Field_config *u_out,
                               Field_config *v_out, int finer_step,
                               int fill_missing) {
  ...
  for (n = 0; n < ntiles_in; n++)
    var_cubsph[n].data =
        (double *)malloc((nx_in + 2) * (ny_in + 2) * nz * sizeof(double));
      ...
      for (j = 0; j < nyd; j++)
        for (i = 0; i < nxd; i++) {
          ...
          var_cubsph[n].data[n1] =
              u_in[n].data[n1] * grid_in[n].vlon_t[3 * n2] +
              v_in[n].data[n1] * grid_in[n].vlat_t[3 * n2];
        }
  }

  do_c2l_interp(interp, nx_in, ny_in, nz, var_cubsph, nx_out, ny_out, x_latlon,
                has_missing, missing, fill_missing);
    ...
    for (k = 0; k < nz; k++)
      for (j = 0; j < nyd; j++)
        for (i = 0; i < nxd; i++) {
          ...
          n2 = j * nxd + i;
          var_cubsph[n].data[n1] =
              u_in[n].data[n1] * grid_in[n].vlon_t[3 * n2 + 1] +
              v_in[n].data[n1] * grid_in[n].vlat_t[3 * n2 + 1];
        }
  }
  do_c2l_interp(interp, nx_in, ny_in, nz, var_cubsph, nx_out, ny_out, y_latlon,
                has_missing, missing, fill_missing);
    ...
    for (k = 0; k < nz; k++)
      for (j = 0; j < nyd; j++)
        for (i = 0; i < nxd; i++) {
          ...
          n2 = j * nxd + i;
          var_cubsph[n].data[n1] =
              u_in[n].data[n1] * grid_in[n].vlon_t[3 * n2 + 2] +
              v_in[n].data[n1] * grid_in[n].vlat_t[3 * n2 + 2];
        }
  }

  do_c2l_interp(interp, nx_in, ny_in, nz, var_cubsph, nx_out, ny_out, z_latlon,
                has_missing, missing, fill_missing);

  for (n = 0; n < ntiles_in; n++)
    free(var_cubsph[n].data);
  free(var_cubsph);
  ...
  for (k = 0; k < nz; k++)
    for (j = 0; j < ny_out; j++)
      for (i = 0; i < nx_out; i++) {
        ...
        n2 = j * nx_out + i;
        var_latlon[n1] = x_latlon[n1] * grid_out->vlon_t[3 * n2] +
                         y_latlon[n1] * grid_out->vlon_t[3 * n2 + 1] +
                         z_latlon[n1] * grid_out->vlon_t[3 * n2 + 2];
      }
  do_latlon_coarsening(var_latlon, grid_out->latt1D_fine, nx_out, ny_out, nz,
                       u_out->data, finer_step, has_missing, missing);
  ...
  for (k = 0; k < nz; k++)
    for (j = 0; j < ny_out; j++)
      for (i = 0; i < nx_out; i++) {
        n1 = k * nx_out * ny_out + j * nx_out + i;
        n2 = j * nx_out + i;
        var_latlon[n1] = x_latlon[n1] * grid_out->vlat_t[3 * n2] +
                         y_latlon[n1] * grid_out->vlat_t[3 * n2 + 1] +
                         z_latlon[n1] * grid_out->vlat_t[3 * n2 + 2];
      }

  do_latlon_coarsening(var_latlon, grid_out->latt1D_fine, nx_out, ny_out, nz,
                       v_out->data, finer_step, has_missing, missing);
    ...
    for (k = 0; k < nz; k++)
      for (j = 0; j < ny_out; j++)
        for (i = 0; i < nx_out; i++) {
          ...
          d_in[2] =
              field_in[tile].data[k * nxd * nyd + (jc + 1) * nxd + ic + 1];
          d_in[3] = field_in[tile].data[k * nxd * nyd + jc * nxd + ic + 1];
          if (d_in[0] == missing || d_in[1] == missing || d_in[3] == missing ||
              d_in[4] == missing) {
            if (fill_missing) {
            ...
            data_out[k * nx_out * ny_out + n1] =
                d_in[0] * interp->weight[4 * n1] +
                d_in[1] * interp->weight[4 * n1 + 1] +
...
void sort_index(int ntiles, int *index, double *shortest) {
  ...
  for (l = 0; l < 3 * ntiles; l++)
    index_sort[l] = 0;
  for (l = 0; l < ntiles; l++)
    shortest_sort[l] = M_PI + M_PI;
  for (l = 0; l < ntiles; l++) {
  ...
  for (l = 0; l < 3 * ntiles; l++)
    index[l] = index_sort[l];
  for (l = 0; l < ntiles; l++)
    shortest[l] = shortest_sort[l];
...
/*-------------------------------------------------------------
  ...
  --------------------------------------------------------------*/
int get_closest_index(const Grid_config *grid_in, const Grid_config *grid_out,
                      int *index, int i_in, int j_in, int l_in, int i_out,
                      int j_out) {
  int found;
...
void do_latlon_coarsening(const double *var_latlon, const double *ylat,
  ...
  case 0:
    if (nlon_crs != nlon || nlat_crs != nlat)
      mpp_error(
          "bilinear_interp(do_latlon_coarsening): grid dimensions don't match");
    for (i = 0; i < nlon * nlat * nz; i++)
      var_latlon_crs[i] = var_latlon[i];
    break;
  case 1:
    redu2x(var_latlon, ylat, nlon, nlat, var_latlon_crs, nlon_crs, nlat_crs, nz,
           has_missing, missvalue);
    break;
  default:
    nlon_new = nlon;
    nlat_new = nlat;
    for (steps = 1; steps <= finer_steps; steps++) {
      ...
      nlat_old = nlat_new;
      var_latlon_old =
          (double *)malloc(nlon_old * nlat_old * nz * sizeof(double));
      ylat_old = (double *)malloc(nlat_old * sizeof(double));
      if (steps == 1) {
        for (i = 0; i < nlat; i++)
          ylat_old[i] = ylat[i];
        for (i = 0; i < nlon_old * nlat_old * nz; i++)
          var_latlon_old[i] = var_latlon[i];
      } else {
        dlat = M_PI / (nlat_old - 1);
        ylat_old[0] = -0.5 * M_PI;
        ylat_old[nlat_old - 1] = 0.5 * M_PI;
        for (j = 1; j < nlat_old - 1; j++)
          ylat_old[j] = ylat_old[0] + j * dlat;
        for (i = 0; i < nlon_old * nlat_old * nz; i++)
          var_latlon_old[i] = var_latlon_new[i];
        free(var_latlon_new);
      }
      ...
      var_latlon_new =
          (double *)malloc(nlon_new * nlat_new * nz * sizeof(double));
      redu2x(var_latlon_old, ylat_old, nlon_old, nlat_old, var_latlon_new,
             nlon_new, nlat_new, nz, has_missing, missvalue);
      free(var_latlon_old);
      free(ylat_old);
    }
    for (i = 0; i < nlon_new * nlat_new * nz; i++)
      var_latlon_crs[i] = var_latlon_new[i];
    free(var_latlon_new);
  }
...
void redu2x(const double *varfin, const double *yfin, int nxfin, int nyfin,
  ...
  if (has_missing) {
    for (k = 0; k < nz; k++)
      for (j = 1; j < nyfin - 1; j++) {
        ...
        if (varfin[n1 + nxfin - 1] == missvalue || varfin[n1] == missvalue ||
            varfin[n1 + 1] == missvalue)
          vartmp[n2] = missvalue;
        else
          vartmp[n2] = 0.25 * (varfin[n1 + nxfin - 1] + 2. * varfin[n1] +
                               varfin[n1 + 1]);
        for (i = 2; i < nxfin - 1; i += 2) {
          i2 = i / 2;
          if (varfin[n1 + i - 1] == missvalue || varfin[n1 + i] == missvalue ||
              varfin[n1 + i + 1] == missvalue)
            vartmp[n2 + i2] = missvalue;
          else
            vartmp[n2 + i2] = 0.25 * (varfin[n1 + i - 1] + 2. * varfin[n1 + i] +
                                      varfin[n1 + i + 1]);
        }
      }
  } else {
    for (k = 0; k < nz; k++)
      for (j = 1; j < nyfin - 1; j++) {
        ...
        n2 = k * nxcrs * nyfin + j * nxcrs;
        vartmp[n2] =
            0.25 * (varfin[n1 + nxfin - 1] + 2. * varfin[n1] + varfin[n1 + 1]);
        for (i = 2; i < nxfin - 1; i += 2) {
          i2 = i / 2;
          vartmp[n2 + i2] = 0.25 * (varfin[n1 + i - 1] + 2. * varfin[n1 + i] +
                                    varfin[n1 + i + 1]);
  ...
  /*---------------------------------------------------------------------
    ...
    Winds at poles are wave-1; the follwoing is quick & dirty yet the correct
    way The skipping method. A more rigorous way is to recompute the wave-1
    components for the coarser grid.
    --------------------------------------------------------------------*/
  for (k = 0; k < nz; k++)
    for (i = 0; i < nxcrs; i++) {
      i2 = i * 2;
      ...
      varcrs[n1 + (nycrs - 1) * nxcrs + i] =
          varfin[n1 + (nyfin - 1) * nxfin + i2];
    }
  /*----------------------------------------------------------------
    y-sweep
    ----------------------------------------------------------------*/
    ...
    for (k = 0; k < nz; k++)
      for (j = 2; j < nyfin - 2; j += 2) {
        ...
        for (i = 0; i < nxcrs; i++) {
          n1 = k * nxcrs * nyfin + i;
          n2 = k * nxcrs * nycrs + i;
          if (vartmp[n1 + j * nxcrs] == missvalue ||
            ...
            varcrs[n2 + j2 * nxcrs] =
                acosp[j] *
                (vartmp[n1 + j * nxcrs] + 0.5 * (vartmp[n1 + (j - 1) * nxcrs] +
                                                 vartmp[n1 + (j + 1) * nxcrs]));
        }
      }
  } else {
