...
int mpp_open(const char *file, int action) {
  ...
  if (action == MPP_WRITE && mpp_pe() != mpp_root_pe())
    return -1;
  /*if file is not ended with .nc add .nc at the end. */
  strcpy(curfile, file);
  if (strstr(curfile, ".nc") == NULL)
    strcat(curfile, ".nc");
  ...
  if (fid > -1) {
    if (files[n].action == MPP_WRITE) {
      sprintf(errmsg, "mpp_io(mpp_open): %s is already created for write",
              file);
      mpp_error(errmsg);
    }
    if (files[n].status) {
    ...
    if (nfiles > MAXFILE)
      mpp_error(
          "mpp_io(mpp_open): nfiles is larger than MAXFILE, increase MAXFILE");
  ...
  default:
    sprintf(errmsg,
            "mpp_io(mpp_open): the action should be MPP_WRITE or MPP_READ when "
            "opening file %s",
            file);
    mpp_error(errmsg);
  }
...
void mpp_close(int fid) {
  ...
  if (fid == -1 && mpp_pe() != mpp_root_pe())
    return;

  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_close): invalid id number, id should be "
              "a nonnegative integer that less than nfiles");

  status = nc_close(files[fid].ncid);
  if (status != NC_NOERR) {
    sprintf(errmsg, "mpp_io(mpp_close): error in closing files %s ",
            files[fid].name);
    netcdf_error(errmsg, status);
  }
  files[fid].ncid = 0;
...
int mpp_get_varid(int fid, const char *varname) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_varid): invalid id number, id should be "
              "a nonnegative integer that less than nfiles");

  for (n = 0; n < files[fid].nvar; n++) {
    if (!strcmp(files[fid].var[n].name, varname))
      return n;
  }

  vid = files[fid].nvar;
  files[fid].nvar++;
  if (files[fid].nvar > MAXVAR)
    mpp_error(
        "mpp_io(mpp_get_varid): nvar is larger than MAXVAR, increase MAXVAR");

  status = nc_inq_varid(files[fid].ncid, varname, &fldid);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_varid): error in get field_id of variable %s from "
            "file %s",
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_varid): Error in getting type of of field %s in "
            "file %s ",
...
int mpp_get_dimlen(int fid, const char *name) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_dimlen): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  ncid = files[fid].ncid;
  ...
  if (status != NC_NOERR) {
    sprintf(
        errmsg,
        "mpp_io(mpp_get_dimlen): error in inquiring dimid of %s from file %s",
        name, files[fid].name);
    netcdf_error(errmsg, status);
  }
  ...
  if (status != NC_NOERR) {
    sprintf(
        errmsg,
        "mpp_io(mpp_get_dimlen): error in inquiring dimlen of %s from file %s",
        name, files[fid].name);
    netcdf_error(errmsg, status);
  }
  len = size;
...
void mpp_get_var_value(int fid, int vid, void *data) {
  ...
  case NC_FLOAT:
    status =
        nc_get_var_double(files[fid].ncid, files[fid].var[vid].fldid, data);
    break;
  case NC_INT:
  ...
  default:
    sprintf(
        ...
        "the type should be NC_DOUBLE, NC_FLOAT",
        files[fid].var[vid].name, files[fid].name);
    mpp_error(errmsg);
  }
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var_value_block): Error in getting value of "
...
/*********************************************************************
  void mpp_get_var_value_block(int fid, int vid, const size_t *start, const
size_t *nread, void *data) read part of var data, the part is defined by start
and nread.
*********************************************************************/
void mpp_get_var_value_block(int fid, int vid, const size_t *start,
                             const size_t *nread, void *data) {
  int status;
  char errmsg[512];
  ...
  case NC_DOUBLE:
  case NC_FLOAT:
    status = nc_get_vara_double(files[fid].ncid, files[fid].var[vid].fldid,
                                start, nread, data);
    break;
  case NC_INT:
    status = nc_get_vara_int(files[fid].ncid, files[fid].var[vid].fldid, start,
                             nread, data);
    break;
  case NC_CHAR:
    status = nc_get_vara_text(files[fid].ncid, files[fid].var[vid].fldid, start,
                              nread, data);
    break;
  default:
    sprintf(
...
void mpp_get_var_att(int fid, int vid, const char *name, void *val) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_var_att): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (vid < 0 || vid >= files[fid].nvar)
    mpp_error("mpp_io(mpp_get_var_att): invalid vid number, vid should be "
              "a nonnegative integer that less than nvar");

  status =
      nc_inq_atttype(files[fid].ncid, files[fid].var[vid].fldid, name, &type);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var_att): Error in getting type of attribute %s of "
            "field %s in file %s ",
  ...
  case NC_DOUBLE:
  case NC_FLOAT:
    status = nc_get_att_double(files[fid].ncid, files[fid].var[vid].fldid, name,
                               val);
    break;
  default:
    sprintf(errmsg,
            "mpp_io(mpp_get_var_att): attribute %s of field %s in file %s has "
            "an invalid type, "
            "the type should be NC_DOUBLE, NC_FLOAT",
            name, files[fid].var[vid].name, files[fid].name);
    mpp_error(errmsg);
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var_att): Error in getting value of attribute %s "
            "of variable %s from file %s",
            name, files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
}
...
void mpp_get_global_att(int fid, const char *name, void *val) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_global_att): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  status = nc_inq_atttype(files[fid].ncid, NC_GLOBAL, name, &type);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_global_att): Error in getting type of global "
  ...
  default:
    sprintf(errmsg,
            "mpp_io(mpp_get_global_att): global attribute %s in file %s has an "
            "invalid type, "
            "the type should be NC_DOUBLE, NC_FLOAT",
            name, files[fid].name);
    mpp_error(errmsg);
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_global_att): Error in getting value of global "
            "attribute %s from file %s",
            name, files[fid].name);
    netcdf_error(errmsg, status);
  }
}
...
int mpp_get_var_ndim(int fid, int vid) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_var_ndim): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (vid < 0 || vid >= files[fid].nvar)
    mpp_error("mpp_io(mpp_get_var_ndim): invalid vid number, vid should be "
              "a nonnegative integer that less than nvar");

  status = nc_inq_varndims(files[fid].ncid, files[fid].var[vid].fldid, &ndim);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var_ndim): Error in getting ndims of var %s from "
            "file %s",
            files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
...
nc_type mpp_get_var_type(int fid, int vid) {
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var_type): Error in getting type of var %s from "
            "file %s",
            files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
...
For each dimension we are assuming there is a 1-d field have the same name as
the dimension.
*********************************************************************/
void mpp_get_var_dimname(int fid, int vid, int ind, char *name) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_var_dimname): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (vid < 0 || vid >= files[fid].nvar)
    mpp_error("mpp_io(mpp_get_var_dimname): invalid vid number, vid should be "
              "a nonnegative integer that less than nvar");
  ncid = files[fid].ncid;
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var2D_dimname): Error in getting ndims of var %s "
            "from file %s",
            files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
  ...
  if (ind < 0 || ind >= ndims)
    mpp_error("mpp_io(mpp_get_var_dimname): invalid ind value, ind should be "
              "between 0 and ndim-1");

  status = nc_inq_vardimid(ncid, fldid, dims);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var2D_dimname): Error in getting dimid of var %s "
            "from file %s",
            files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var2D_dimname): Error in getting %d dimension name "
            "of var %s from file %s",
...
char mpp_get_var_cart(int fid, int vid) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_get_var_cart): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (vid < 0 || vid >= files[fid].nvar)
    mpp_error("mpp_io(mpp_get_var_cart): invalid vid number, vid should be "
              "a nonnegative integer that less than nvar");
  cart = 'N';
  ...
  if (status != NC_NOERR)
    status = nc_get_att_text(ncid, fldid, "axis", &cart);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_get_var_cart): Error in getting attribute "
            "cartesian_axis/axis of "
...
/***************************************************************************
 ...
 Get the bound name of dimension variable if it exist, otherwise the value will
 be 'none' for time axis, the bounds may be 'climatology'
 **************************************************************************/
void mpp_get_var_bndname(int fid, int vid, char *bndname) {
  int ncid, fldid, status;
...
int mpp_var_att_exist(int fid, int vid, const char *att) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_var_att_exist): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (vid < 0 || vid >= files[fid].nvar)
    mpp_error("mpp_io(mpp_var_att_exist): invalid vid number, vid should be "
              "a nonnegative integer that less than nvar");

  status = nc_inq_att(files[fid].ncid, files[fid].var[vid].fldid, att, &atttype,
                      &attlen);
  if (status == NC_NOERR)
    return 1;
  else
...
int mpp_def_dim(int fid, const char *name, int size) {
  ...
  if (mpp_pe() != mpp_root_pe())
    return 0;
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_def_dim): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");

  status = nc_def_dim(files[fid].ncid, name, size, &dimid);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_def_dim): Error in defining dimension %s of file %s",
            name, files[fid].name);
    netcdf_error(errmsg, status);
  }
  return dimid;
...
/********************************************************************
 int mpp_def_var(nt fid, const char* name, int type, int ndim, int *dims, int
natts ... ) define metadata of field.
********************************************************************/
int mpp_def_var(int fid, const char *name, nc_type type, int ndim,
                const int *dims, int natts, ...) {
  ...
  if (mpp_pe() != mpp_root_pe())
    return 0;
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_def_var): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  ...
  if (files[fid].nvar > MAXVAR)
    mpp_error(
        "mpp_io(mpp_def_var): nvar is larger than MAXVAR, increase MAXVAR");
  files[fid].var[vid].fldid = fldid;
  ...
  for (i = 0; i < natts; i++) {
    ...
    if (attname == NULL || attval == NULL) {
      mpp_error("mpp_io: attribute name and attribute value not defined "
                "suitably, check the arguments list.");
    }
    status = nc_put_att_text(ncid, fldid, attname, strlen(attval), attval);
    if (status != NC_NOERR) {
      sprintf(
          errmsg,
          "mpp_io(mpp_def_var): Error in put attribute %s of var %s of file %s",
...
void mpp_def_global_att(int fid, const char *name, const char *val) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_def_global_att): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  status = nc_put_att_text(files[fid].ncid, NC_GLOBAL, name, strlen(val), val);
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_def_global_att): Error in put glboal attribute %s of "
            "file %s",
            name, files[fid].name);
    netcdf_error(errmsg, status);
  }
...
void mpp_copy_var_att(int fid_in, int vid_in, int fid_out, int vid_out) {
  ...
  if (fid_in < 0 || fid_in >= nfiles)
    mpp_error("mpp_io(mpp_copy_var_att): invalid fid_in number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (fid_out < 0 || fid_out >= nfiles)
    mpp_error("mpp_io(mpp_copy_var_att): invalid fid_out number, fid should be "
              "a nonnegative integer that less than nfiles");
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_copy_var_att): Error in inquiring natts of var %s of "
            "file %s",
  ...
  for (i = 0; i < natt; i++) {
    ...
    if (status != NC_NOERR) {
      sprintf(errmsg,
              "mpp_io(mpp_copy_var_att): Error in inquiring %d attname of var "
              "%s of file %s",
              i, files[fid_in].var[vid_in].name, files[fid_in].name);
      netcdf_error(errmsg, status);
    }
    status = nc_copy_att(ncid_in, fldid_in, name, ncid_out, fldid_out);
    if (status != NC_NOERR) {
      sprintf(errmsg,
              "mpp_io(mpp_copy_var_att): Error in copying att %s of var %s of "
              "file %s",
              name, files[fid_in].var[vid_in].name, files[fid_in].name);
      netcdf_error(errmsg, status);
    }
  }
...
void mpp_copy_global_att(int fid_in, int fid_out) {
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_copy_global_att): Error in inquiring natts(global) of "
            "file %s",
            files[fid_in].name);
    netcdf_error(errmsg, status);
  }
  ...
  for (i = 0; i < natt; i++) {
    ...
    if (status != NC_NOERR) {
      sprintf(errmsg,
              "mpp_io(mpp_copy_global_att): Error in inquiring %d global "
              "attname of file %s",
              i, files[fid_in].name);
      netcdf_error(errmsg, status);
    }
    ...
    if (status != NC_NOERR) {
      sprintf(errmsg,
              "mpp_io(mpp_copy_global_att): Error in copying %d global att %s "
              "of file %s",
              i, name, files[fid_in].name);
      netcdf_error(errmsg, status);
    }
  }
...
void mpp_put_var_value(int fid, int vid, const void *data) {
  ...
  if (fid < 0 || fid >= nfiles)
    mpp_error("mpp_io(mpp_put_var_value): invalid fid number, fid should be "
              "a nonnegative integer that less than nfiles");
  if (vid < 0 || vid >= files[fid].nvar)
    mpp_error("mpp_io(mpp_put_var_value): invalid vid number, vid should be "
              "a nonnegative integer that less than nvar");
  ...
  case NC_FLOAT:
    status =
        nc_put_var_double(files[fid].ncid, files[fid].var[vid].fldid, data);
    break;
  case NC_INT:
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_put_var_value): Error in putting value of variable %s "
            "from file %s",
            files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
...
/*********************************************************************
  void mpp_put_var_value_block(int fid, int vid, const size_t *start, const
size_t *nread, void *data) read part of var data, the part is defined by start
and nread.
*********************************************************************/
void mpp_put_var_value_block(int fid, int vid, const size_t *start,
                             const size_t *nwrite, const void *data) {
  int status;
  char errmsg[512];
  ...
  case NC_DOUBLE:
  case NC_FLOAT:
    status = nc_put_vara_double(files[fid].ncid, files[fid].var[vid].fldid,
                                start, nwrite, data);
    break;
  case NC_INT:
    status = nc_put_vara_int(files[fid].ncid, files[fid].var[vid].fldid, start,
                             nwrite, data);
    break;
  case NC_CHAR:
    status = nc_put_vara_text(files[fid].ncid, files[fid].var[vid].fldid, start,
                              nwrite, data);
    break;
  default:
    sprintf(errmsg,
            "mpp_io(mpp_get_var_value_block): field %s in file %s has an "
            "invalid type, "
            "the type should be NC_DOUBLE, NC_FLOAT",
            files[fid].var[vid].name, files[fid].name);
    mpp_error(errmsg);
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg,
            "mpp_io(mpp_put_var_value_block): Error in putting value of "
            "variable %s from file %s",
            files[fid].var[vid].name, files[fid].name);
    netcdf_error(errmsg, status);
  }
...
void mpp_end_def(int fid) {
  ...
  if (status != NC_NOERR) {
    sprintf(errmsg, "mpp_io(mpp_end_def): Error in end definition of file %s",
            files[fid].name);
    netcdf_error(errmsg, status);
  }
} /* mpp_end_def */
...
int mpp_file_exist(const char *file) {
  ...
  if (status == NC_NOERR) {
    ...
    if (status != NC_NOERR)
      netcdf_error("mpp_file_exist(mpp_io):in closing file", status);
    return 1;
  } else
    return 0;
};
