/* This is part of the libcf package. Copyright 2006 University
   ...
   $Id: tst_cvars.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
#define NLATS 20
  ...
  printf("*** testing coordinate variable creation...");
  {
    ...
    if (nc_create(FILE_NAME, 0, &ncid))
      ERR;
    if (nccf_def_latitude(ncid, NLATS, NC_INT, &dimid, &varid))
      ERR;

    /* Check the file. */
    if (nccf_inq_latitude(ncid, &len_in, &xtype_in, &dimid_in, &varid_in))
      ERR;
    if (len_in != NLATS || xtype_in != NC_INT || dimid_in != 0 || varid_in != 0)
      ERR;
