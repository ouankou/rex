/* This is part of the netCDF package. Copyright 2005 University
   ...
   $Id: tst_vars.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
#define DIM_NAME "Lines_of_code_per_cube_per_second"
  ...
  printf("*** testing variable notes...");
  {
    ...
    if (nc_create(FILE_NAME, 0, &ncid))
      ERR;
    if (nc_def_dim(ncid, DIM_NAME, NC_UNLIMITED, dimids))
      ERR;
    if (nc_def_var(ncid, VAR_NAME, NC_INT, NDIMS, dimids, &varid))
      ERR;
    if (nccf_def_notes(ncid, varid, INSTITUTION, SOURCE, COMMENT, REFERENCES))
      ERR;

    /* Check the file. */
    if (nccf_inq_notes(ncid, varid, &ilen, inst_in, &slen, source_in, &clen,
                       comment_in, &rlen, ref_in))
      ERR;
    if (ilen != strlen(INSTITUTION) + 1 || slen != strlen(SOURCE) + 1 ||
        clen != strlen(COMMENT) + 1 || rlen != strlen(REFERENCES) + 1)
      ERR;
    if (strncmp(inst_in, INSTITUTION, ilen) ||
        strncmp(source_in, SOURCE, slen) ||
        strncmp(comment_in, COMMENT, clen) || strncmp(ref_in, REFERENCES, rlen))
      ERR;
...
#define VAR2_NAME "manager_salary"
  ...
  printf("*** testing missing values...");
  {
    ...
    if (nc_def_var(ncid, VAR_NAME, NC_INT, NDIMS, dimids, &varid))
      ERR;
    if (nc_def_var(ncid, VAR2_NAME, NC_INT, NDIMS, dimids, &varid2))
      ERR;
    if (nccf_def_var_missing(ncid, varid, &fill_value, &valid_min, &valid_max))
      ERR;

    /* Check the file. */
    if (nccf_inq_var_missing(ncid, varid, &fill_value_in, &valid_min_in,
                             &valid_max_in))
      ERR;
    if (fill_value_in != fill_value || valid_min_in != valid_min ||
        valid_max_in != valid_max)
      ERR;
    if (nccf_inq_var_missing(ncid, varid2, &fill_value_in, &valid_min_in,
                             &valid_max_in))
      ERR;
    if (fill_value_in != NC_FILL_INT || valid_min_in != CF_INT_MIN ||
        valid_max_in != CF_INT_MAX)
      ERR;
