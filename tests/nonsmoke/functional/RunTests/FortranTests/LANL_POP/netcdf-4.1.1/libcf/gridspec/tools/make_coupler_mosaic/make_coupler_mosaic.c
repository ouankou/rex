 ...
 *  Warning, when the atmos grid is cubic grid, the number of model points
 * should be even to avoid tiling error. I will come back to solve this issue in
 * the future.
 *
 */
...
char *usage[] = {
    "",
    "  make_coupler_mosaic --atmos_mosaic atmos_mosaic.nc --ocean_mosaic "
    "ocean_mosaic.nc ",
    "              --ocean_topog ocean_topog.nc [--land_mosaic "
    "land_mosaic.nc] ",
    "              [--sea_level #]  [--interp_method #] [--mosaic_name "
    "mosaic_name] ",
    "              [--check ]                                                  "
    "     ",
    " ",
    ...
    "The output ",
    "file includes exchange grid files for fluxes between atmosphere and "
    "surface (sea ice ",
    "and land), exchange grid files for runoff between land and sea ice. There "
    "might be more ",
    "than one exchange grid files between two model solo mosaic because there "
    "might be ",
    "multiple tiles in a solo mosaic. All the exchange grid information are "
    "between model ",
    "grid, not between supergrid. We assume the refinement ratio between model "
    "grid and ",
    "supergrid is 2. Currently we only output the exchange grid on T-cell. ",
    "Besides generate the exchange grid files, make_coupler_mosaic also "
    "generate the ",
    "coupler mosaic file (the file name will be mosaic_name.nc) which contains "
    "the atmos, ",
    "land and ocean mosaic path, ocean mosaic topog path and exchange grid "
    "file path. ",
    ...
    "REQUIRED:",
    "",
    "--atmos_mosaic atmos_mosaic.nc specify the atmosphere mosaic information. "
    "This file",
    "                               contains list of tile files which specify "
    "the grid ",
    "                               information for each tile. Each grid is "
    "required to be ",
    "                               regular lon/lat grid. The file name can "
    "not be 'mosaic.nc' ",
    "",
    "--ocean_mosaic ocean_mosaic.nc specify the ocean mosaic information. This "
    "file",
    "                               contains list of tile files which specify "
    "the grid ",
    "                               information for each tile. The file name "
    "can not be 'mosaic.nc' ",
    " ",
    "--ocean_topog ocean_topog.nc   specify the topography information for "
    "ocean mosaic.",
    "                               The field name of the topography is "
    "depth_tile# or depth when ",
    "                               ntiles = 1, The topography data is "
    "positive down.",
    " ",
    "OPTIONAL FLAGS",
    "",
    "--land_mosaic land_mosaic.nc   specify the land mosaic information. This "
    "file",
    "                               contains list of tile files which specify "
    "the grid ",
    "                               information for each tile. Each grid is "
    "required to be ",
    "                               regular lon/lat grid. When land_mosaic is "
    "not specified,",
    "                               atmosphere mosaic will be used to specify "
    "land mosaic.",
    "                               The file name can not be 'mosaic.nc'.",
    " ",
    "--interp_order #               specify the order of conservative "
    "interplation. Its value "
    "                               can be 1 ( linear order ) or 2 ( second "
    "order ) with default "
    "                               value 2.                                   "
    "                  "
    "                                                                          "
    "                  "
    "--sea_level #                  specify the sea level ( in meters ) and "
    "its value will be used",
    "                               to determine land/sea mask. When "
    "topography of  ",
    "                               a grid cell is less than sea level, this "
    "grid cell will be land,",
    "                               otherwise it will be ocean. Default value "
    "is 0",
    " ",
    "--mosaic_name mosaic_name      coupler mosaic name. The output coupler "
    "mosaic file will be ",
    "                               mosaic_name.nc. default value is "
    "'mosaic'. ",
    " ",
    "--check                        check the tiling error",
    "",
    "A sample call to make_coupler_mosaic that makes exchange grids for "
...
int main(int argc, char *argv[]) {
  ...
  if (interp_order != 1 && interp_order != 2)
    mpp_error("make_coupler_mosaic: interp_order should be 1 or 2");
  ...
  ***************************************************************************************/
  ...
  if (mpp_pe() == mpp_root_pe())
    printf("\n***** Congratulation! You have successfully run "
           "make_coupler_mosaic\n");
  mpp_end();
