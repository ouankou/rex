/*
  ...
  $Id: cfsubsets.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
static int find_coord_idx(int ncid, int vid, size_t len, float *range,
                          int *nidxp, size_t *startp, size_t *endp) {
  int i;
  float *coord;
...
int nccf_get_vara(int ncid, int varid, float *lat_range, int *nlat,
  ...
  if ((ret = nccf_inq_latitude(ncid, &len[LAT], &xtype[LAT], &did[LAT],
                               &vid[LAT])))
    return ret;
  if ((ret = nccf_inq_longitude(ncid, &len[LON], &xtype[LON], &did[LON],
                                &vid[LON])))
    return ret;
