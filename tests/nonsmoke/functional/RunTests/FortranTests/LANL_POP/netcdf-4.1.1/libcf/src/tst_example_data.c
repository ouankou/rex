/* This is part of the libcf package. Copyright 2007 University
   ...
   $Id: tst_example_data.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
#define EXAMPLE_FILE                                                           \
  ...
  printf("*** looking at %s...", EXAMPLE_FILE);
  {
    ...
    /* Check the file. */
    if (nc_open(EXAMPLE_FILE, 0, &ncid))
      ERR;
    if (nccf_inq_latitude(ncid, &len, &xtype, &did, &vid))
      ERR;
    if (len != 91 || xtype != NC_DOUBLE || did != 0 || vid != 1)
      ERR;
    if (nccf_inq_longitude(ncid, &len, &xtype, &did, &vid))
      ERR;
    if (len != 144 || xtype != NC_DOUBLE || did != 1 || vid != 2)
      ERR;
    if (nccf_inq_lvl(ncid, name, &len, &xtype, &ft_len, formula_terms,
                     &positive_up, &did, &vid))
      ERR;
    if (len != 26 || xtype != NC_DOUBLE || did != 4 || vid != 6 ||
        positive_up != 0)
      ERR;
