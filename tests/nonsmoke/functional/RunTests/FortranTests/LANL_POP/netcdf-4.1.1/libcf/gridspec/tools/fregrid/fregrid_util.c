...
void update_halo(int nx, int ny, int nz, double *data, Bound_config *bound,
                 Data_holder *dHold);
void setup_boundary(const char *mosaic_file, int ntiles, Grid_config *grid,
                    Bound_config *bound, int halo, int position);
...
void set_mosaic_data_file(int ntiles, const char *mosaic_file, const char *dir,
                          File_config *file, const char *filename) {
  ...
  if (ntiles > 1) {
    if (!mosaic_file)
      mpp_error("fregrid_util: when ntiles is greater than 1, mosaic_file "
                "should be defined");
...
/*******************************************************************************
  void set_scalar_var()
*******************************************************************************/
void set_field_struct(int ntiles, Field_config *field, int nvar, char *varname,
                      File_config *file) {
  int n, i;
...
*******************************************************************************/
void get_input_grid(int ntiles, Grid_config *grid, Bound_config *bound_T,
                    const char *mosaic_file, unsigned int opcode) {
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (nx[n] % 2)
      mpp_error("fregrid_util(get_input_grid): the size of dimension nx should "
                "be even (on supergrid)");
    if (ny[n] % 2)
      mpp_error("fregrid_util(get_input_grid): the size of dimension ny should "
                "be even (on supergrid)");
    nx[n] /= 2;
    ny[n] /= 2;
    grid[n].halo = halo;
    ...
    grid[n].lonc = (double *)malloc((nx[n] + 1 + 2 * halo) *
                                    (ny[n] + 1 + 2 * halo) * sizeof(double));
    grid[n].latc = (double *)malloc((nx[n] + 1 + 2 * halo) *
                                    (ny[n] + 1 + 2 * halo) * sizeof(double));
    if (halo > 0) {
    ...
    if (opcode & CONSERVE_ORDER2 || opcode & BILINEAR) {
      grid[n].vlon_t = (double *)malloc(3 * (nx[n] + 2 * halo) *
                                        (ny[n] + 2 * halo) * sizeof(double));
      grid[n].vlat_t = (double *)malloc(3 * (nx[n] + 2 * halo) *
                                        (ny[n] + 2 * halo) * sizeof(double));
    ...
    if (opcode & VECTOR) {
      if (opcode & AGRID) {
        double *angle;
        angle = (double *)malloc((2 * nx[n] + 1) * (2 * ny[n] + 1) *
                                 sizeof(double));
        ...
        for (j = 0; j < ny[n]; j++)
          for (i = 0; i < nx[n]; i++) {
            ...
            if (fabs(grid[n].sinrot[m1]) > EPSLN)
              grid[n].rotate = 1;
          }
        free(angle);
      }
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (nbound > 0) {
      ...
      for (l = 0; l < nbound; l++)
        dHold[l].data = grid[bound_T[n].tile2[l]].latt;
  ...
  if (opcode & BILINEAR) {
    ...
    for (n = 0; n < ntiles; n++) {
      nlon = grid[n].nx;
      nlat = grid[n].ny;
      latlon2xyz((nlon + 2) * (nlat + 2), grid[n].lont, grid[n].latt,
                 grid[n].xt, grid[n].yt, grid[n].zt);
      unit_vect_latlon((nlon + 2) * (nlat + 2), grid[n].lont, grid[n].latt,
                       grid[n].vlon_t, grid[n].vlat_t);
      ...
      if (nbound > 0) {
        ...
        update_halo(nlon + 1 + 2 * halo, nlat + 1 + 2 * halo, 1, grid[n].lonc,
                    &(bound_C[n]), dHold);
        for (l = 0; l < nbound; l++)
          dHold[l].data = grid[bound_C[n].tile2[l]].latc;

        update_halo(nlon + 1 + 2 * halo, nlat + 1 + 2 * halo, 1, grid[n].latc,
                    &(bound_C[n]), dHold);
        for (l = 0; l < nbound; l++)
          dHold[l].data = NULL;
        free(dHold);
    ...
    for (n = 0; n < ntiles; n++) {
      ...
      calc_c2l_grid_info(&nlon, &nlat, grid[n].lont, grid[n].latt, grid[n].lonc,
                         grid[n].latc, grid[n].dx, grid[n].dy, grid[n].area,
                         grid[n].edge_w, grid[n].edge_e, grid[n].edge_s,
                         grid[n].edge_n, grid[n].en_n, grid[n].en_e,
                         grid[n].vlon_t, grid[n].vlat_t, &is_true, &is_true,
                         &is_true, &is_true);
    }
  }

  if (opcode & BILINEAR)
    delete_bound_memory(ntiles, bound_C);
  free(bound_C);
  free(nx);
  free(ny);
}; /* get_input_grid */
...
*******************************************************************************/
void get_output_grid_from_mosaic(int ntiles, Grid_config *grid,
                                 const char *mosaic_file, unsigned int opcode) {
  ...
  if (opcode & BILINEAR)
    mpp_error("fregrid_util: for bilinear interpolation, output grid can not "
              "got from mosaic file");
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    ny[n] = mpp_get_dimlen(g_fid, "ny");
    if (nx[n] % 2)
      mpp_error("fregrid_util(get_output_grid_from_mosaic): the size of "
                "dimension nx should be even (on supergrid)");
    if (ny[n] % 2)
      mpp_error("fregrid_util(get_output_grid_from_mosaic): the size of "
                "dimension ny should be even (on supergrid)");
    nx[n] /= 2;
    ny[n] /= 2;
    grid[n].nx = nx[n];
    grid[n].ny = ny[n];
    ...
    layout[0] = 1;
    layout[1] = npes;
    mpp_define_domain2d(grid[n].nx, grid[n].ny, layout, 0, 0,
                        &(grid[n].domain));
    mpp_get_compute_domain2d(grid[n].domain, &(grid[n].isc), &(grid[n].iec),
                             &(grid[n].jsc), &(grid[n].jec));
    grid[n].nxc = grid[n].iec - grid[n].isc + 1;
    grid[n].nyc = grid[n].jec - grid[n].jsc + 1;

    grid[n].lonc = (double *)malloc((grid[n].nxc + 1) * (grid[n].nyc + 1) *
                                    sizeof(double));
    grid[n].latc = (double *)malloc((grid[n].nxc + 1) * (grid[n].nyc + 1) *
                                    sizeof(double));
    ...
    for (j = 0; j <= grid[n].nyc; j++)
      for (i = 0; i <= grid[n].nxc; i++) {
        jj = 2 * (j + grid[n].jsc);
        ii = 2 * (i + grid[n].isc);
        grid[n].lonc[j * (grid[n].nxc + 1) + i] =
            x[jj * (2 * nx[n] + 1) + ii] * D2R;
        grid[n].latc[j * (grid[n].nxc + 1) + i] =
            y[jj * (2 * nx[n] + 1) + ii] * D2R;
      }
    for (i = 0; i < nx[n] + 1; i++)
      grid[n].lonc1D[i] = x[i] * D2R;
    for (j = 0; j < ny[n] + 1; j++)
      grid[n].latc1D[j] = y[j * (nx[n] + 1)] * D2R;

    for (i = 0; i < nx[n]; i++)
      grid[n].lont1D[i] = x[2 * nx[n] + 1 + 2 * i + 1] * D2R;
    for (j = 0; j < ny[n]; j++)
      grid[n].latt1D[j] = y[(2 * j + 1) * (2 * nx[n] + 1) + 1] * D2R;
    free(x);
    free(y);
      ...
      if (opcode & AGRID) {
        ...
        for (j = 0; j < grid[n].nyc; j++)
          for (i = 0; i < grid[n].nxc; i++) {
            jj = 2 * (j + grid[n].jsc);
            ii = 2 * (i + grid[n].isc);
            grid[n].cosrot[j * grid[n].nxc + i] =
                cos(angle[jj * (2 * nx[n] + 1) + ii] * D2R);
            grid[n].sinrot[j * grid[n].nxc + i] =
                sin(angle[jj * (2 * nx[n] + 1) + ii] * D2R);
            if (fabs(grid[n].sinrot[j * grid[n].nxc + i]) > EPSLN)
              grid[n].rotate = 1;
          }
        free(angle);
      }
...
/*******************************************************************************
  void get_output_grid_by_size(Mosaic_config *mosaic, int nlon, int nlat, int
finer_steps, unsigned int opcode) calculate output grid based on nlon, nlat and
finer steps.

*******************************************************************************/
void get_output_grid_by_size(int ntiles, Grid_config *grid, double lonbegin,
                             double lonend, double latbegin, double latend,
                             int nlon, int nlat, int finer_steps, int center_y,
                             unsigned int opcode) {
  ...
  if (ntiles != 1)
    mpp_error("fregrid_utils: ntiles of output mosaic must be 1 for bilinear "
              "interpolation");
  if (finer_steps && !(opcode & BILINEAR))
    mpp_error("fregrid_util: finer_steps must be 0 when interp_method is not "
              "bilinear");
  ...
  dlon = lon_range / nlon;
  for (i = 0; i < nlon; i++)
    grid->lont1D[i] = (lonbegin + (i + 0.5) * dlon) * D2R;
  for (i = 0; i <= nlon; i++)
    grid->lonc1D[i] = (lonbegin + i * dlon) * D2R;

  layout[0] = 1;
  layout[1] = mpp_npes();
  mpp_define_domain2d(grid->nx, grid->ny, layout, 0, 0, &(grid->domain));
  mpp_get_compute_domain2d(grid->domain, &(grid->isc), &(grid->iec),
                           &(grid->jsc), &(grid->jec));
  ...
  if (center_y) {
    dlat = lat_range / nlat;
    for (j = 0; j < nlat; j++)
      grid->latt1D[j] = (latbegin + (j + 0.5) * dlat) * D2R;
    for (j = 0; j <= nlat; j++)
      grid->latc1D[j] = (latbegin + j * dlat) * D2R;
  } else {
    dlat = lat_range / (nlat - 1);
    for (j = 0; j < nlat; j++)
      grid->latt1D[j] = (latbegin + j * dlat) * D2R;
    for (j = 0; j <= nlat; j++)
      grid->latc1D[j] = (latbegin + (j - 0.5) * dlat) * D2R;
  }
  ...
  if (opcode & BILINEAR) {
    ...
    for (i = 0; i < nx_fine; i++) {
      ...
      for (j = 0; j < ny_fine; j++)
        grid->lont[j * nx_fine + i] = lon_fine;
    }
    if (center_y) {
      dlat = lat_range / ny_fine;
      for (j = 0; j < ny_fine; j++)
        grid->latt1D_fine[j] = (latbegin + (j + 0.5) * dlat) * D2R;
    } else {
      dlat = lat_range / (ny_fine - 1);
      for (j = 0; j < ny_fine; j++)
        grid->latt1D_fine[j] = (latbegin + j * dlat) * D2R;
    }
    for (j = 0; j < ny_fine; j++)
      for (i = 0; i < nx_fine; i++) {
    ...
    latlon2xyz(nx_fine * ny_fine, grid->lont, grid->latt, grid->xt, grid->yt,
               grid->zt);

    unit_vect_latlon(nx_fine * ny_fine, grid->lont, grid->latt, grid->vlon_t,
                     grid->vlat_t);
...
*******************************************************************************/
void get_input_metadata(int ntiles, int nfiles, File_config *file1,
                        File_config *file2, Field_config *scalar,
                        Field_config *u_comp, Field_config *v_comp,
                        const Grid_config *grid, int kbegin, int kend,
                        int lbegin, int lend) {
  ...
  nfield = (nfiles == 1) ? (nscalar + 2 * nvector)
                         : nvector; /* when nfiles = 2, no scalar */
  ...
  for (m = 0; m < nfiles; m++) {
    ...
    for (n = 0; n < ntiles; n++) {
      for (l = 0; l < nfield; l++) {
        ...
        field[n].var[ll].vid =
            mpp_get_varid(file[n].fid, field[n].var[ll].name);
        field[n].var[ll].type =
            mpp_get_var_type(file[n].fid, field[n].var[ll].vid);
        /* check if time_avg_info attribute existed in any variables */
        if (!file[n].has_tavg_info) {
          if (mpp_var_att_exist(file[n].fid, field[n].var[ll].vid,
                                "time_avg_info"))
            file[n].has_tavg_info = 1;
        }
        ...
        if (ndim < 2 || ndim > 4)
          mpp_error("get_input_metadata(fregrid_util.c): ndim should be no "
                    "less than 2 and no larger than 4");
        if (cart[ndim - 1] != 'X')
          mpp_error("get_input_metadata(fregrid_util.c): the last dimension "
                    "cartesian should be 'X'");
        if (cart[ndim - 2] != 'Y')
          mpp_error("get_input_metadata(fregrid_util.c): the second last "
                    "dimension cartesian should be 'Y'");
        if (dimsize[ndim - 1] != grid[n].nx)
          mpp_error("get_input_metadata(fregrid_util.c): x-size in grid file "
                    "in not the same as in data file");
        if (dimsize[ndim - 2] != grid[n].ny)
          mpp_error("get_input_metadata(fregrid_util.c): y-size in grid file "
                    "in not the same as in data file");
        if (cart[0] == 'Z' || cart[1] == 'Z') {
          ...
          if (kend > field[n].var[ll].nz) {
            sprintf(errmsg,
                    "get_input_metadata(fregrid_util.c): KlevelEnd should be "
                    "no larger than "
        ...
        if (cart[0] == 'T') {
          ...
          if (lend > dimsize[0]) {
            sprintf(errmsg,
                    "get_input_metadata(fregrid_util.c): LstepEnd should be no "
                    "larger than "
        ...
        for (i = 0; i < ndim; i++) {
          /* loop through all the file dimensions to see if the dimension
          ...
          if (!found) {
            ...
            file[n].ndim++;
            if (file[n].ndim > MAXDIM)
              mpp_error("get_input_metadata(fregrid_util.c):ndim is greater "
                        "than MAXDIM");
            ...
            file[n].axis[j].data =
                (double *)malloc(file[n].axis[j].size * sizeof(double));
            nread[0] = file[n].axis[j].size;
            mpp_get_var_value_block(file[n].fid, file[n].axis[j].vid, start,
                                    nread, file[n].axis[j].data);
            ...
            if (strcmp(bndname[i], "none")) {
              ...
              if (mpp_get_var_ndim(file[n].fid, file[n].axis[j].bndid) == 1) {
                ...
                file[n].axis[j].bnddata = (double *)malloc(
                    (file[n].axis[j].size + 1) * sizeof(double));
                nread[0] = file[n].axis[j].size + 1;
              } else {
                file[n].axis[j].bndtype = 2;
                file[n].axis[j].bnddata =
                    (double *)malloc(2 * file[n].axis[j].size * sizeof(double));
                nread[0] = file[n].axis[j].size;
                nread[1] = 2;
              }
              mpp_get_var_value_block(file[n].fid, file[n].axis[j].bndid, start,
                                      nread, file[n].axis[j].bnddata);
            }
          }
        } /*ndim*/
      } /* nvar */
    ...
    for (n = 1; n < ntiles; n++) {
      if (file[n].has_tavg_info != file[0].has_tavg_info)
        mpp_error("get_input_metadata(fregrid_util.c): mismatch between tiles "
                  "for field attribute has_tavg_info");
      ...
      for (l = 0; l < file[n].ndim; l++) {
        if (strcmp(file[n].axis[l].name, file[0].axis[l].name))
          mpp_error("get_input_metadata(fregrid_util.c): mismatch between "
                    "tiles for file axis name");
      }
    ...
    for (n = 0; n < ntiles; n++) {
      if (file[n].has_tavg_info) {
        ...
        nread[0] = file[n].nt;
        nread[1] = 1;
        mpp_get_var_value_block(file[n].fid, file[n].id_t1, start, nread,
                                file[n].t1);
        mpp_get_var_value_block(file[n].fid, file[n].id_t2, start, nread,
                                file[n].t2);
        mpp_get_var_value_block(file[n].fid, file[n].id_dt, start, nread,
                                file[n].dt);
      }
    }
...
void set_output_metadata(int ntiles_in, int nfiles, const File_config *file1_in,
                         const File_config *file2_in,
                         const Field_config *scalar_in,
                         const Field_config *u_in, const Field_config *v_in,
                         int ntiles_out, File_config *file1_out,
                         File_config *file2_out, Field_config *scalar_out,
                         Field_config *u_out, Field_config *v_out,
                         const Grid_config *grid_out, const char *history,
                         const char *tagname) {
  ...
  for (n = 0; n < ntiles_out; n++) {
    for (l = 0; l < nscalar; l++) {
      ...
      for (i = 0; i < scalar_in[0].var[l].ndim; i++)
        scalar_out[n].var[l].index[i] = scalar_in[0].var[l].index[i];
    }

    for (l = 0; l < nvector; l++) {
      u_out[n].var[l].ndim = u_in[0].var[l].ndim;
      v_out[n].var[l].ndim = v_in[0].var[l].ndim;
      for (i = 0; i < u_in[0].var[l].ndim; i++)
        u_out[n].var[l].index[i] = u_in[0].var[l].index[i];
      for (i = 0; i < v_in[0].var[l].ndim; i++)
        v_out[n].var[l].index[i] = v_in[0].var[l].index[i];
  ...
  for (m = 0; m < nfiles; m++) {
    ...
    for (n = 0; n < ntiles_out; n++) {
      ...
      for (i = 0; i < ndim; i++) {
        ...
        if (file_out[n].axis[i].cart == 'X')
          file_out[n].axis[i].size = grid_out[n].nx;
        if (file_out[n].axis[i].cart == 'Y')
          file_out[n].axis[i].size = grid_out[n].ny;
      ...
      for (i = 0; i < ndim; i++) {
        file_out[n].axis[i].data =
            (double *)malloc(file_out[n].axis[i].size * sizeof(double));
        if (file_out[n].axis[i].cart == 'X') { /* x-axis */
          for (l = 0; l < file_out[n].axis[i].size; l++)
            file_out[n].axis[i].data[l] =
                grid_out[n].lont1D[l] * R2D;          /* T-cell center */
        } else if (file_out[n].axis[i].cart == 'Y') { /* y-axis */
          for (l = 0; l < file_out[n].axis[i].size; l++)
            file_out[n].axis[i].data[l] =
                grid_out[n].latt1D[l] * R2D; /* T-cell center */
        } else {                             /* z or t axis */
          for (l = 0; l < file_out[n].axis[i].size; l++)
        ...
        case 1:
          file_out[n].axis[i].bnddata =
              (double *)malloc((file_out[n].axis[i].size + 1) * sizeof(double));
          if (file_out[n].axis[i].cart == 'X') { /* x-axis */
            for (l = 0; l <= file_out[n].axis[i].size; l++)
        ...
        case 2:
          file_out[n].axis[i].bnddata =
              (double *)malloc(2 * file_out[n].axis[i].size * sizeof(double));
          if (file_out[n].axis[i].cart == 'X') { /* x-axis */
            for (l = 0; l < file_out[n].axis[i].size; l++) {
              file_out[n].axis[i].bnddata[2 * l] = grid_out[n].lonc[l] * R2D;
              file_out[n].axis[i].bnddata[2 * l + 1] =
                  grid_out[n].lonc[l + 1] * R2D;
            }
          } else if (file_out[n].axis[i].cart == 'Y') {
            for (l = 0; l < file_out[n].axis[i].size; l++) {
              file_out[n].axis[i].bnddata[2 * l] = grid_out[n].latc[l] * R2D;
              file_out[n].axis[i].bnddata[2 * l + 1] =
                  grid_out[n].latc[l + 1] * R2D;
            }
          } else {
            for (l = 0; l < file_out[n].axis[i].size; l++) {
              file_out[n].axis[i].bnddata[2 * l] =
                  file_in[0].axis[i].bnddata[2 * l];
              file_out[n].axis[i].bnddata[2 * l + 1] =
                  file_in[0].axis[i].bnddata[2 * l + 1];
            }
          }
          break;
        }
      ...
      if (mpp_pe() == mpp_root_pe()) {
        ...
        for (i = 0; i < ndim; i++) {
          if (file_out[n].axis[i].cart == 'T') {
            file_out[n].axis[i].dimid = mpp_def_dim(
                file_out[n].fid, file_out[n].axis[i].name, NC_UNLIMITED);
            dim_time = file_out[n].axis[i].dimid;
          } else
            file_out[n].axis[i].dimid =
                mpp_def_dim(file_out[n].fid, file_out[n].axis[i].name,
                            file_out[n].axis[i].size);
          file_out[n].axis[i].vid = mpp_def_var(
              file_out[n].fid, file_out[n].axis[i].name,
              file_out[n].axis[i].type, 1, &(file_out[n].axis[i].dimid), 0);
          mpp_copy_var_att(file_in[0].fid, file_in[0].axis[i].vid,
                           file_out[n].fid, file_out[n].axis[i].vid);
          switch (file_out[n].axis[i].bndtype) {
          case 1:
            dims[0] = mpp_def_dim(file_out[n].fid, file_out[n].axis[i].bndname,
                                  file_out[n].axis[i].size + 1);
            file_out[n].axis[i].bndid =
                mpp_def_var(file_out[n].fid, file_out[n].axis[i].bndname,
                            file_out[n].axis[i].type, 1, dims, 0);
            mpp_copy_var_att(file_in[0].fid, file_in[0].axis[i].bndid,
                             file_out[n].fid, file_out[n].axis[i].bndid);
            break;
          case 2:
        ...
        for (l = 0; l < nscalar; l++) {
          ...
          scalar_out[n].var[l].vid = mpp_def_var(
              file_out[n].fid, scalar_out[n].var[l].name,
              scalar_out[n].var[l].type, scalar_out[n].var[l].ndim, dims, 0);
          mpp_copy_var_att(file_in[0].fid, scalar_in[0].var[l].vid,
                           file_out[n].fid, scalar_out[n].var[l].vid);
        }
        for (l = 0; l < nvector; l++) {
          if (m == 0) {
            u_out[n].var[l].type = u_in[0].var[l].type;
            for (i = 0; i < u_out[n].var[l].ndim; i++)
              dims[i] = file_out[n].axis[u_out[n].var[l].index[i]].dimid;
            u_out[n].var[l].vid = mpp_def_var(
                file_out[n].fid, u_out[n].var[l].name, u_out[n].var[l].type,
                u_out[n].var[l].ndim, dims, 0);
            mpp_copy_var_att(file_in[0].fid, u_in[0].var[l].vid,
                             file_out[n].fid, u_out[n].var[l].vid);
          }
          if (m == 1 || nfiles == 1) {
            v_out[n].var[l].type = v_in[0].var[l].type;
            for (i = 0; i < v_out[n].var[l].ndim; i++)
              dims[i] = file_out[n].axis[v_out[n].var[l].index[i]].dimid;
            v_out[n].var[l].vid = mpp_def_var(
                file_out[n].fid, v_out[n].var[l].name, v_out[n].var[l].type,
                v_out[n].var[l].ndim, dims, 0);
            mpp_copy_var_att(file_in[0].fid, v_in[0].var[l].vid,
                             file_out[n].fid, v_out[n].var[l].vid);
          }
        }
        /* define time avg info variables */
        if (file_out[n].has_tavg_info) {
          int ll;
          file_out[n].id_t1 = mpp_def_var(file_out[n].fid, "average_T1",
                                          NC_DOUBLE, 1, &dim_time, 0);
          mpp_copy_var_att(file_in[0].fid, file_in[0].id_t1, file_out[n].fid,
                           file_out[n].id_t1);
          file_out[n].id_t2 = mpp_def_var(file_out[n].fid, "average_T2",
                                          NC_DOUBLE, 1, &dim_time, 0);
          mpp_copy_var_att(file_in[0].fid, file_in[0].id_t2, file_out[n].fid,
                           file_out[n].id_t2);
          file_out[n].id_dt = mpp_def_var(file_out[n].fid, "average_DT",
                                          NC_DOUBLE, 1, &dim_time, 0);
          mpp_copy_var_att(file_in[0].fid, file_in[0].id_dt, file_out[n].fid,
                           file_out[n].id_dt);
        ...
        for (i = 0; i < ndim; i++) {
          if (file_out[n].axis[i].cart == 'T')
            continue;
          mpp_put_var_value(file_out[n].fid, file_out[n].axis[i].vid,
                            file_out[n].axis[i].data);
          if (strcmp(file_out[n].axis[i].bndname, "none"))
            mpp_put_var_value(file_out[n].fid, file_out[n].axis[i].bndid,
                              file_out[n].axis[i].bnddata);
        }
      }
    }
...
void get_field_missing(int ntiles, Field_config *field) {
  ...
  for (l = 0; l < nfield; l++) {
    for (n = 0; n < ntiles; n++) {
      ...
      field[n].var[l].vid =
          mpp_get_varid(*(field[n].fid), field[n].var[l].name);
      if (field[n].var[l].has_missing = mpp_var_att_exist(
              *(field[n].fid), field[n].var[l].vid, "missing_value")) {
        mpp_get_var_att(*(field[n].fid), field[n].var[l].vid, "missing_value",
                        (void *)(&(field[n].var[l].missing)));
...
void set_remap_file(int ntiles, const char *mosaic_file, const char *remap_file,
  ...
  if (len >= STRING)
    mpp_error("setoutput_remap_file(fregrid_util): length of remap_file should "
              "be less than STRING");
...
void write_output_time(int ntiles, File_config *file, int level) {
  ...
  if (mpp_pe() == mpp_root_pe()) {
    for (n = 0; n < ntiles; n++) {
      ...
      if (file[n].has_tavg_info) {
        nwrite[1] = 1;
        mpp_put_var_value_block(file[n].fid, file[n].id_t1, start, nwrite,
                                &(file[n].t1[level]));
        mpp_put_var_value_block(file[n].fid, file[n].id_t2, start, nwrite,
                                &(file[n].t2[level]));
        mpp_put_var_value_block(file[n].fid, file[n].id_dt, start, nwrite,
                                &(file[n].dt[level]));
      }
    }
  }
...
/*---------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------*/
void get_input_data(int ntiles, Field_config *field, Grid_config *grid,
                    Bound_config *bound, int varid, int level,
                    unsigned int opcode) {
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (halo == 0) {
      mpp_get_var_value_block(*(field[n].fid), field[n].var[varid].vid, start,
                              nread, field[n].data);
    } else {
      ...
      for (k = 0; k < nz; k++)
        for (j = 0; j < ny; j++)
          for (i = 0; i < nx; i++) {
            i1 = k * (nx + 2 * halo) * (ny + 2 * halo) +
                 (j + halo) * (nx + 2 * halo) + i + halo;
            i2 = k * nx * ny + j * nx + i;
  ...
  if (halo > 0) {
    for (n = 0; n < ntiles; n++) {
      ...
      if (nbound > 0) {
        ...
        update_halo(grid[n].nx + 2, grid[n].ny + 2, nz, field[n].data,
                    &(bound[n]), dHold);
        for (l = 0; l < nbound; l++)
          dHold[l].data = NULL;
        free(dHold);
    ...
    if (opcode & CONSERVE_ORDER2) {
      for (n = 0; n < ntiles; n++) {
        ...
        for (k = 0; k < nz; k++) {
          grad_c2l(&(grid[n].nx), &(grid[n].ny),
                   field[n].data + k * (nx + 2) * (ny + 2), grid[n].dx,
                   grid[n].dy, grid[n].area, grid[n].edge_w, grid[n].edge_e,
                   grid[n].edge_s, grid[n].edge_n, grid[n].en_n, grid[n].en_e,
                   grid[n].vlon_t, grid[n].vlat_t,
                   field[n].grad_x + k * nx * ny, field[n].grad_y + k * nx * ny,
                   &is_true, &is_true, &is_true, &is_true);
        }
        /* where there is missing and using second order conservative
         * interpolation, need to calculate mask for gradient */
          ...
          for (k = 0; k < nz; k++)
            for (j = 0; j < ny; j++)
              for (i = 0; i < nx; i++) {
                ...
                jm1 = jj - 1;
                kk = k * (nx + 2) * (ny + 2);
                if (field[n].data[kk + jm1 * (nx + 2) + im1] == missing ||
                    field[n].data[kk + jm1 * (nx + 2) + ii] == missing ||
                    field[n].data[kk + jm1 * (nx + 2) + ip1] == missing ||
                    field[n].data[kk + jj * (nx + 2) + im1] == missing ||
                    field[n].data[kk + jj * (nx + 2) + ip1] == missing ||
                    field[n].data[kk + jp1 * (nx + 2) + im1] == missing ||
                    field[n].data[kk + jp1 * (nx + 2) + ii] == missing ||
                    field[n].data[kk + jp1 * (nx + 2) + ip1] == missing)
...
/*---------------------------------------------------------------------------
  ...
  get the input data for the number l variable.
  -------------------------------------------------------------------------*/
void get_test_input_data(char *test_case, double test_param, int ntiles,
                         Field_config *field, Grid_config *grid,
                         Bound_config *bound, unsigned int opcode) {
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    ny = grid[n].ny;
    field[n].data =
        (double *)malloc((nx + 2 * halo) * (ny + 2 * halo) * sizeof(double));
    data = (double *)malloc(nx * ny * sizeof(double));
    if (!strcmp(test_case, "tanh_cosphi_costheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] =
              tanh(test_param * cos(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) *
    ...
    if (!strcmp(test_case, "tanh_sinphi_sintheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] =
              tanh(test_param * sin(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) *
                   sin(grid[n].latt[(j + 1) * (nx + 2) + i + 1]));
        }
    } else if (!strcmp(test_case, "cosphi_costheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] = cos(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) *
                             cos(grid[n].latt[(j + 1) * (nx + 2) + i + 1]);
        }
    } else if (!strcmp(test_case, "sinphi_costheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] = sin(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) *
                             cos(grid[n].latt[(j + 1) * (nx + 2) + i + 1]);
        }
    } else
  ...
  if (halo > 0) {
    for (n = 0; n < ntiles; n++) {
      ...
      if (nbound > 0) {
        ...
        update_halo(grid[n].nx + 2, grid[n].ny + 2, 1, field[n].data,
                    &(bound[n]), dHold);
        for (l = 0; l < nbound; l++)
          dHold[l].data = NULL;
        free(dHold);
    ...
    if (opcode & CONSERVE_ORDER2) {
      for (n = 0; n < ntiles; n++) {
        int is_true = 1;
        field[n].grad_x = (double *)malloc((grid[n].nx + 2) * (grid[n].ny + 2) *
                                           sizeof(double));
        field[n].grad_y = (double *)malloc((grid[n].nx + 2) * (grid[n].ny + 2) *
                                           sizeof(double));
        grad_c2l(&(grid[n].nx), &(grid[n].ny), field[n].data, grid[n].dx,
                 grid[n].dy, grid[n].area, grid[n].edge_w, grid[n].edge_e,
                 grid[n].edge_s, grid[n].edge_n, grid[n].en_n, grid[n].en_e,
                 grid[n].vlon_t, grid[n].vlat_t, field[n].grad_x,
                 field[n].grad_y, &is_true, &is_true, &is_true, &is_true);
      }
    }
  }
...
/*-------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------*/
void write_field_data(int ntiles, Field_config *field, Grid_config *grid, int l,
                      int level) {
  double *gdata;
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    nwrite[ndim - 1] = nx;
    nwrite[ndim - 2] = ny;
    if (field[n].var[l].has_zaxis)
      nwrite[ndim - 3] = nz;
    if (mpp_npes() == 1) {
      mpp_put_var_value_block(*(field[n].fid), field[n].var[l].vid, start,
                              nwrite, field[n].data);
    } else {
       ...
       */
      /* 			      field[n].data+k*grid[n].nxc*grid[n].nyc,
       * gdata+k*nx*ny); */
      /*    } */
...
void get_contact_direction(int ncontact, const int *tile, const int *istart,
                           const int *iend, const int *jstart, const int *jend,
                           int *dir) {
  int n;

  for (n = 0; n < ncontact; n++) {
    if (istart[n] == iend[n] && jstart[n] == jend[n])
      mpp_error("fregrid_util: istart = iend and jstart = jend can not be both "
                "true for one contact");
    if (istart[n] != iend[n] && jstart[n] != jend[n])
      mpp_error("fregrid_util: either istart = iend or jstart = jend need to "
                "be true");
    if (istart[n] == iend[n]) {
      if (istart[n] == 0)
        dir[n] = WEST;
...
void setup_boundary(const char *mosaic_file, int ntiles, Grid_config *grid,
  ...
  read_mosaic_contact(mosaic_file, tile, tile + ncontacts, istart, iend, jstart,
                      jend, istart + ncontacts, iend + ncontacts,
                      jstart + ncontacts, jend + ncontacts);
  for (n = 0; n < 2 * ncontacts; n++)
    --tile[n];
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (nbound > 0) {
      ...
      for (l = 0; l < 2 * ncontacts; l++) {
        ...
        if (dir[l] == WEST && dir[l2] == NORTH)
          bound[n].rotate[nb] = NINETY;
        if (dir[l] == EAST && dir[l2] == SOUTH)
          bound[n].rotate[nb] = NINETY;
        if (dir[l] == SOUTH && dir[l2] == EAST)
          bound[n].rotate[nb] = MINUS_NINETY;
        if (dir[l] == NORTH && dir[l2] == WEST)
          bound[n].rotate[nb] = MINUS_NINETY;
        if (dir[l] == NORTH && dir[l2] == NORTH)
          bound[n].rotate[nb] = ONE_HUNDRED_EIGHTY;
        /* make sure the size match at the boundary */
        if ((bound[n].ie2[nb] - bound[n].is2[nb] + 1) *
                (bound[n].je2[nb] - bound[n].js2[nb] + 1) !=
            (bound[n].ie1[nb] - bound[n].is1[nb] + 1) *
                (bound[n].je1[nb] - bound[n].js1[nb] + 1))
...
void init_halo(double *var, int nx, int ny, int nz, int halo) {
  ...
  for (k = 0; k < nz; k++) {
    for (j = 0; j < nyd; j++)
      for (i = 0; i < halo; i++)
        var[k * nall + j * nxd + i] = 0; /* west halo */
    for (j = 0; j < nyd; j++)
      for (i = nx + halo; i < nxd; i++)
        var[k * nall + j * nxd + i] = 0; /* east halo */
    for (j = 0; j < halo; j++)
      for (i = 0; i < nxd; i++)
        var[k * nall + j * nxd + i] = 0; /* south halo */
    for (j = ny + halo; j < nyd; j++)
      for (i = 0; i < nxd; i++)
        var[k * nall + j * nxd + i] = 0; /* north halo */
  }
