...
/*******************************************************************************
   void get_string_data_level(const char *file, const char *name, const size_t
*start, const size_t *nread, char *data) get string data of field with "name"
from "file".
******************************************************************************/
void get_string_data_level(const char *file, const char *name, char *data,
                           const int *level) {
...
/******************************************************************************
   void get_var_text_att(const char *file, const char *name, const char
*attname, char *att) get text attribute of field 'name' from 'file
******************************************************************************/
void get_var_text_att(const char *file, const char *name, const char *attname,
                      char *att) {
  int ncid, varid, status;
  char msg[512];
  ...
  if (status != NC_NOERR) {
    sprintf(msg, "in getting attribute %s of %s from file %s.", attname, name,
            file);
    handle_netcdf_error(msg, status);
  }
  status = nc_close(ncid);
...
/****************************************************************************/
#ifndef __AIX
void read_mosaic_xgrid_order1_(const char *xgrid_file, int *i1, int *j1,
                               int *i2, int *j2, double *area) {
  read_mosaic_xgrid_order1(xgrid_file, i1, j1, i2, j2, area);
};
#endif

void read_mosaic_xgrid_order1(const char *xgrid_file, int *i1, int *j1, int *i2,
                              int *j2, double *area) {
  int ncells, n;
...
/****************************************************************************/
#ifndef __AIX
void read_mosaic_xgrid_order2_(const char *xgrid_file, int *i1, int *j1,
                               int *i2, int *j2, double *area, double *di,
                               double *dj) {
  read_mosaic_xgrid_order2(xgrid_file, i1, j1, i2, j2, area, di, dj);
};
#endif

void read_mosaic_xgrid_order2(const char *xgrid_file, int *i1, int *j1, int *i2,
                              int *j2, double *area, double *di, double *dj) {
  int ncells, n;
  int *tile1_cell, *tile2_cell;
...
/*****************************************************************************
  ...
  read mosaic grid size of each tile, currently we are assuming the refinement
is 2. We assume the grid files are located at the same directory as mosaic_file.
*****************************************************************************/
#ifndef __AIX
...
******************************************************************************/
#ifndef __AIX
void read_mosaic_contact_(const char *mosaic_file, int *tile1, int *tile2,
...
void read_mosaic_contact(const char *mosaic_file, int *tile1, int *tile2,
                         int *istart1, int *iend1, int *jstart1, int *jend1,
                         int *istart2, int *iend2, int *jstart2, int *jend2) {
  ...
  for (n = 0; n < ncontacts; n++) {
    ...
    if (istart1[n] == iend1[n]) {
      ...
      if (jend1[n] > jstart1[n]) {
        ...
        error_handler("Error from read_mosaic_contact: jstart1 and jend1 "
                      "should not be equal when istart1=iend1");

      if (jstart1[n] % y_refine || jend1[n] % y_refine)
        error_handler("Error from read_mosaic_contact: mismatch between "
                      "y_refine and jstart1/jend1 when istart1=iend1");
      jstart1[n] /= y_refine;
      jend1[n] /= y_refine;
      ...
      if (iend1[n] > istart1[n]) {
        ...
        error_handler("Error from read_mosaic_contact: istart1 and iend1 "
                      "should not be equal when jstart1=jend1");

      if (istart1[n] % x_refine || iend1[n] % x_refine)
        error_handler("Error from read_mosaic_contact: mismatch between "
                      "x_refine and istart1/iend1 when jstart1=jend1");
      istart1[n] /= x_refine;
      iend1[n] /= x_refine;
    } else {
      error_handler("Error from read_mosaic_contact: only line contact is "
                    "supported now, contact developer");
    }
    if (istart2[n] == iend2[n]) {
      ...
      if (jend2[n] > jstart2[n]) {
        ...
        error_handler("Error from read_mosaic_contact: jstart2 and jend2 "
                      "should not be equal when istart2=iend2");

      if (jstart2[n] % y_refine || jend2[n] % y_refine)
        error_handler("Error from read_mosaic_contact: mismatch between "
                      "y_refine and jstart2/jend2 when istart2=iend2");
      ...
      if (iend2[n] > istart2[n]) {
        ...
        error_handler("Error from read_mosaic_contact: istart2 and iend2 "
                      "should not be equal when jstart2=jend2");

      if (istart2[n] % x_refine || iend2[n] % x_refine)
        error_handler("Error from read_mosaic_contact: mismatch between "
                      "x_refine and istart2/iend2 when jstart2=jend2");
      istart2[n] /= x_refine;
      iend2[n] /= x_refine;
    } else {
      error_handler("Error from read_mosaic_contact: only line contact is "
...
/******************************************************************************
  void read_mosaic_grid_data(const char *mosaic_file, const char *name, int nx,
int ny, double *data, int level, int ioff, int joff) read mosaic grid
information onto model grid. We assume the refinement is 2 right now. We may
remove this restriction in the future. nx and ny are model grid size. level is
the tile number. ioff and joff to indicate grid location. ioff =0 and joff = 0
  for C-cell. ioff=0 and joff=1 for E-cell, ioff=1 and joff=0 for N-cell,
  ioff=1 and joff=1 for T-cell
******************************************************************************/
void read_mosaic_grid_data(const char *mosaic_file, const char *name, int nx,
                           int ny, double *data, int level, int ioff,
                           int joff) {
  ...
  if (ni != nx * 2 || nj != ny * 2)
    error_handler("supergrid size should be double of the model grid size");
  ...
  for (j = 0; j < nyp; j++)
    for (i = 0; i < nxp; i++)
      data[j * nxp + i] = tmp[(2 * j + joff) * (ni + 1) + 2 * i + ioff];
  free(tmp);
