...
*********************************************************************/
double bipolar_dist(double x1, double y1, double x2, double y2, double bpeq,
                    double bpsp, double bpnp, double rp) {
  ...
  for (n = 0; n < 2; n++) {
    bp_lon[n] = bp_lam(x[n], y[n], bpeq,
                       rp); /* longitude (degrees) in bipolar grid system */
    bp_lat[n] = bp_phi(x[n], y[n], bpsp,
                       bpnp); /* latitude (degrees) in bipolar grid system */
    ...
    if (fabs(y[n] - 90.0) < SMALL || fabs(bp_lon[n] * D2R) >= SMALL ||
      ...
      metric[n] = rp * (1 / pow(cos(chic / 2), 2)) /
                  (1 + (pow(rp, 2)) * (pow(tan(chic / 2), 2))); /* eq 3 */
    }
  }
  ...
  if (x1 == x2)
    ...
    mpp_error(
        "tool_util: This tripolar grid not transformed from rectangular grid");
 ...
 ********************************************************************/
double spherical_area(double x1, double y1, double x2, double y2, double x3,
...
/*********************************************************************
   ...
   bipolar grid  area
 ********************************************************************/
double bipolar_area(double x1, double y1, double x2, double y2, double x3,
                    double y3, double x4, double y4) {
...
void tp_trans(double *lon, double *lat, double lon_ref, double lon_start,
  ...
  if (fabs(lamc) < SMALL && fabs(phic) < SMALL) {
    ...
    *lon = lon_ref;
    ...
    lams = fmod(lam0 + M_PI + M_PI / 2 - atan2(sin(lamc), tan(phic)),
                2 * M_PI);                            /* eqn. 5 */
    chic = acos(cos(lamc) * cos(phic));               /* eqn. 6 */
    phis = M_PI * 0.5 - 2 * atan(rp * tan(chic / 2)); /* eqn. 5 */
    *lon = lams * R2D;
...
/********************************************************************************
  void compute_grid_bound(int nb, const couble *bnds, const int *npts, int
*grid_size, const char *center_cell) compute the 1-D grid location.
********************************************************************************/
double *compute_grid_bound(int nb, const double *bnds, const int *npts,
                           int *grid_size, const char *center) {
  int refine, i, n, np;
  ...
  for (i = 1; i < nb; i++) {
    if (npts[i - 1] % refine)
      mpp_error("tool_util: when center_cell is not 'none', npts should be "
                "divided by 2");
    n += npts[i - 1] / refine;
    grid1[i] = n + 1;
  }
  ...
  *grid_size = n * refine;
  ...
  if (!strcmp(center, "none")) {
    ...
    for (i = 0; i < np; i++)
      grid[2 * i] = tmp[i];
    for (i = 0; i < n; i++)
      grid[2 * i + 1] = 0.5 * (tmp[i] + tmp[i + 1]);
  } else if (!strcmp(center, "c_cell")) {
    for (i = 0; i < np; i++)
      grid[2 * i] = tmp[i];
    grid[1] = 0.5 * (tmp[0] + tmp[1]);
    for (i = 1; i < n; i++)
      grid[2 * i + 1] = 2 * grid[2 * i] - grid[2 * i - 1];
  }
...
int gs_make_hgrid(char *grid_type, int *nlat, int *nlon, int nxbnds0,
                  int nybnds0, int nxbnds1, int nybnds1, int nxbnds2,
                  int nybnds2, double lat_join, int nratio, double simple_dx,
                  double simple_dy, int ntilex, int ntiley, char *gridname,
                  char *center, char *history, double *xbnds, double *ybnds) {
  ...
  double *x = NULL, *y = NULL, *dx = NULL, *dy = NULL, *angle_dx = NULL,
         *angle_dy = NULL, *area = NULL;
  ...
  if (strcmp(grid_type, "regular_lonlat_grid") == 0) {
    nxbnds = nxbnds0;
    nybnds = nybnds0;
    if (nxbnds < 2 || nybnds < 2)
      mpp_error("make_hgrid: grid type is 'regular_lonlat_grid', "
    ...
    nybnds = nybnds0;
    if (nxbnds != 2)
      mpp_error("make_hgrid: grid type is 'tripolar_grid', nxbnds should be 2");
    if (nybnds < 2)
      mpp_error("make_hgrid: grid type is 'tripolar_grid', nybnds should be no "
                "less than 2");
    if (nxbnds != nxbnds1 || nxbnds != nxbnds2 + 1)
      mpp_error("make_hgrid: grid type is 'tripolar_grid', nxbnds does not "
                "match number of entry in xbnds or nlon");
    if (nybnds != nybnds1 || nybnds != nybnds2 + 1)
      mpp_error("make_hgrid: grid type is 'tripolar_grid', nybnds does not "
                "match number of entry in ybnds or nlat");
    ...
    */
    ...
    if (ntiles_file == 0)
      mpp_error("make_hgrid: grid_type is 'from_file', but my_grid_file is not "
                "specified");
    ntiles = ntiles_file;
    for (n = 0; n < ntiles; n++) {
      if (strstr(my_grid_file[n], ".nc")) {
        /* get the grid size for each tile, the grid is on model grid, should
        ...
        if (nxbnds2 != ntiles || nybnds2 != ntiles)
          mpp_error(
              "make_hgrid: grid type is 'from_file', number entry entered "
              "through --nlon and --nlat should be equal to number of files "
              "specified through --my_grid_file");
      }
    }
    /* for simplify purpose, currently we assume all the tile have the same grid
     * size */
    for (n = 1; n < ntiles; n++) {
    ...
    if (nxbnds1 != 2 || nybnds1 != 2)
      mpp_error("make_hgrid: grid type is 'simple_cartesian_grid', number "
                "entry entered "
    ...
    if (nxbnds2 != 1 || nybnds2 != 1)
      mpp_error(
          "make_hgrid: grid type is 'spectral_grid', number entry entered "
          "through --nlon and --nlat should be 1");
  } else if (strcmp(grid_type, "conformal_cubic_grid") == 0) {
    strcpy(projection, "cube_gnomonic");
    strcpy(conformal, "FALSE");
    if (nxbnds2 != 1)
      mpp_error("make_hgrid: grid type is 'conformal_cubic_grid', number entry "
                "entered "
                "through --nlon should be 1");
    if (nratio < 1)
      mpp_error("make_hgrid: grid type is 'conformal_cubic_grid', nratio "
                "should be a positive integer");
  } else if (!strcmp(grid_type, "gnomonic_ed")) {
    strcpy(projection, "cube_gnomonic");
    strcpy(conformal, "FALSE");
    if (nxbnds2 != 1)
      mpp_error("make_hgrid: grid type is 'gnomonic_cubic_grid', number entry "
                "entered "
                "through --nlon should be 1");
  } else {
    mpp_error(
        "make_hgrid: only grid_type = 'regular_lonlat_grid', 'tripolar_grid', "
        "'from_file', "
  ...
  if (!strcmp(grid_type, "gnomonic_ed") ||
      !strcmp(grid_type, "conformal_cubic_grid")) {
    nx = nlon[0];
    ny = nx;
  } else {
    nx = 0;
  ...
  if (!strcmp(grid_type, "gnomonic_ed") ||
    ...
    if (mpp_npes() > 1)
      mpp_error("make_hgrid: cubic grid generation must be run one processor, "
                "contact developer");
  }
  /* Currently we restrict nx can be divided by ndivx and ny can be divided by
   * ndivy */
  if (ntilex > 0 && ntilex != ntiles)
    mpp_error("make_hgrid: number of entry specified through --ndivx does not "
              "equal ntiles");
  if (ntiley > 0 && ntiley != ntiles)
    mpp_error("make_hgrid: number of entry specified through --ndivy does not "
              "equal ntiles");
  for (n = 0; n < ntiles; n++) {
    if (nx % ndivx[n])
      mpp_error("make_hgrid: nx can not be divided by ndivx");
    if (ny % ndivy[n])
      mpp_error("make_hgrid: ny can not be divided by ndivy");
  }

  if (strcmp(center, "none") && strcmp(center, "c_cell") &&
      strcmp(center, "t_cell"))
  ...
  */
  ...
  if (strcmp(conformal, "true") != 0)
    angle_dy = (double *)malloc(nxp * nyp * ntiles * sizeof(double));

  if (strcmp(grid_type, "regular_lonlat_grid") == 0)
    create_regular_lonlat_grid(&nxbnds, &nybnds, xbnds, ybnds, nlon, nlat, &isc,
                               &iec, &jsc, &jec, x, y, dx, dy, area, angle_dx,
                               center);
  else if (strcmp(grid_type, "tripolar_grid") == 0)
    create_tripolar_grid(&nxbnds, &nybnds, xbnds, ybnds, nlon, nlat, &lat_join,
                         &isc, &iec, &jsc, &jec, x, y, dx, dy, area, angle_dx,
                         center);
  else if (strcmp(grid_type, "from_file") == 0) {
    for (n = 0; n < ntiles; n++) {
      int n1, n2, n3, n4;
      ...
      n3 = n * nxp * ny;
      n4 = n * nx * ny;
      create_grid_from_file(my_grid_file[n], &nx, &ny, x + n1, y + n1, dx + n2,
                            dy + n3, area + n4, angle_dx + n1);
    }
  } else if (strcmp(grid_type, "simple_cartesian_grid") == 0)
    create_simple_cartesian_grid(xbnds, ybnds, &nx, &ny, &simple_dx, &simple_dy,
                                 &isc, &iec, &jsc, &jec, x, y, dx, dy, area,
                                 angle_dx);
  else if (strcmp(grid_type, "spectral_grid") == 0)
    create_spectral_grid(&nx, &ny, &isc, &iec, &jsc, &jec, x, y, dx, dy, area,
                         angle_dx);
  else if (strcmp(grid_type, "conformal_cubic_grid") == 0)
    create_conformal_cubic_grid(&nx, &nratio, method, orientation, x, y, dx, dy,
                                area, angle_dx, angle_dy);
  else if (strcmp(grid_type, "gnomonic_ed") == 0)
    create_gnomonic_cubic_grid(grid_type, &nx, x, y, dx, dy, area, angle_dx,
                               angle_dy);

  /* write out data */
  {
    int fid, id_tile, id_x, id_y, id_dx, id_dy, id_area, id_angle_dx,
        id_angle_dy, id_arcx;
    ...
    for (n = 0; n < ntiles; n++) {
      for (j = 0; j < ndivy[n]; j++) {
        for (i = 0; i < ndivx[n]; i++) {
          ...
          /* define variable */
          if (strcmp(north_pole_tile, "none") ==
              0) /* no north pole, then no projection */
            id_tile = mpp_def_var(fid, "tile", MPP_CHAR, 1, dimlist, 4,
                                  "standard_name", "grid_tile_spec", "geometry",
                                  geometry, "discretization", discretization,
                                  "conformal", conformal);
          else if (strcmp(projection, "none") == 0)
            id_tile = mpp_def_var(fid, "tile", MPP_CHAR, 1, dimlist, 5,
                                  "standard_name", "grid_tile_spec", "geometry",
                                  geometry, "north_pole", north_pole_tile,
                                  "discretization", discretization, "conformal",
                                  conformal);
          else
            id_tile = mpp_def_var(fid, "tile", MPP_CHAR, 1, dimlist, 6,
                                  "standard_name", "grid_tile_spec", "geometry",
                                  geometry, "north_pole", north_pole_tile,
                                  "projection", projection, "discretization",
                                  discretization, "conformal", conformal);
          dims[0] = dimlist[4];
          dims[1] = dimlist[3];
          id_x = mpp_def_var(fid, "x", MPP_DOUBLE, 2, dims, 2, "standard_name",
                             "geographic_longitude", "units", "degree_east");
          id_y = mpp_def_var(fid, "y", MPP_DOUBLE, 2, dims, 2, "standard_name",
                             "geographic_latitude", "units", "degree_north");
          dims[0] = dimlist[4];
          dims[1] = dimlist[1];
          id_dx =
              mpp_def_var(fid, "dx", MPP_DOUBLE, 2, dims, 2, "standard_name",
                          "grid_edge_x_distance", "units", "meters");
          dims[0] = dimlist[2];
          dims[1] = dimlist[3];
          id_dy =
              mpp_def_var(fid, "dy", MPP_DOUBLE, 2, dims, 2, "standard_name",
                          "grid_edge_y_distance", "units", "meters");
          dims[0] = dimlist[2];
          dims[1] = dimlist[1];
          id_area =
              mpp_def_var(fid, "area", MPP_DOUBLE, 2, dims, 2, "standard_name",
                          "grid_cell_area", "units", "m2");
          dims[0] = dimlist[4];
          dims[1] = dimlist[3];
          id_angle_dx = mpp_def_var(fid, "angle_dx", MPP_DOUBLE, 2, dims, 2,
                                    "standard_name",
                                    "grid_vertex_x_angle_WRT_geographic_east",
                                    "units", "degrees_east");
          if (strcmp(conformal, "true") != 0)
            id_angle_dy = mpp_def_var(
                fid, "angle_dy", MPP_DOUBLE, 2, dims, 2, "standard_name",
                "grid_vertex_y_angle_WRT_geographic_north", "units",
                "degrees_north");
          if (strcmp(north_pole_arcx, "none") == 0)
            id_arcx = mpp_def_var(fid, "arcx", MPP_CHAR, 1, dimlist, 1,
                                  "standard_name", "grid_edge_x_arc_type");
          else
            id_arcx = mpp_def_var(fid, "arcx", MPP_CHAR, 1, dimlist, 2,
                                  "standard_name", "grid_edge_x_arc_type",
          ...
          tmp = get_subregion(nxp, x + n * nxp * nyp, i * ni, (i + 1) * ni,
                              j * nj, (j + 1) * nj);
          mpp_put_var_value(fid, id_x, tmp);
          free(tmp);
          tmp = get_subregion(nxp, y + n * nxp * nyp, i * ni, (i + 1) * ni,
                              j * nj, (j + 1) * nj);
          mpp_put_var_value(fid, id_y, tmp);
          free(tmp);
          gdata = (double *)malloc(nx * nyp * sizeof(double));
          mpp_global_field_double(domain, nxc, nyc + 1, dx + n * nx * nyp,
                                  gdata);
          tmp = get_subregion(nx, gdata, i * ni, (i + 1) * ni - 1, j * nj,
                              (j + 1) * nj);
          mpp_put_var_value(fid, id_dx, tmp);
          free(tmp);
          free(gdata);
          gdata = (double *)malloc(nxp * ny * sizeof(double));
          mpp_global_field_double(domain, nxc + 1, nyc, dy + n * nxp * ny,
                                  gdata);
          tmp = get_subregion(nxp, gdata, i * ni, (i + 1) * ni, j * nj,
                              (j + 1) * nj - 1);
          ...
          mpp_global_field_double(domain, nxc + 1, nyc + 1,
                                  angle_dx + n * nxp * nyp, gdata);
          tmp = get_subregion(nxp, gdata, i * ni, (i + 1) * ni, j * nj,
                              (j + 1) * nj);
...
int gs_fregrid(char *history, char *mosaic_in, char *mosaic_out, char *dir_in,
               ...
               int nfiles_out, char *remap_file, char **scalar_name,
               int nscalar, char **u_name, int nvector, char **v_name,
               int nvector2, char *interp_method, char *test_case,
               double test_param, unsigned int opcode, int grid_type,
               unsigned int finer_step, int fill_missing, int nlon, int nlat,
               int check_conserve, int y_at_center, double lonbegin,
               double lonend, double latbegin, double latend, int kbegin,
               int kend, int lbegin, int lend) {
  ...
  /* check the arguments */
  if (!mosaic_in)
    mpp_error("fregrid: input_mosaic is not specified");
  if (!mosaic_out) {
    if (nlon == 0 || nlat == 0)
      mpp_error("fregrid: when output_mosaic is not specified, nlon and nlat "
                "should be specified");
    if (lonend <= lonbegin)
      mpp_error("fregrid: when output_mosaic is not specified, lonEnd should "
                "be larger than lonBegin");
    if (latend <= latbegin)
      mpp_error("fregrid: when output_mosaic is not specified, latEnd should "
                "be larger than latBegin");
  } else {
    if (nlon != 0 || nlat != 0)
      mpp_error("fregrid: when output_mosaic is specified, nlon and nlat "
                "should not be specified");
  }

  if (nfiles == 0) {
    if (nvector > 0 || nscalar > 0 || nvector2 > 0)
      mpp_error("fregrid: when --input_file is not specified, --scalar_field, "
                "--u_field and --v_field should also not be specified");
    if (!remap_file)
      mpp_error("fregrid: when --input_file is not specified, remap_file must "
                "be specified to save weight information");
    save_weight_only = 1;
    if (mpp_pe() == mpp_root_pe())
      printf("NOTE: No input file specified in this run, no data file will be "
             "regridded "
             "and only weight information is calculated.\n");
  } else if (nfiles == 1 || nfiles == 2) {
    if (nvector != nvector2)
      mpp_error("fregrid: number of fields specified in u_field must be the "
                "same as specified in v_field");
    if (nscalar + nvector == 0)
      mpp_error(
          "fregrid: both scalar_field and vector_field are not specified");
    /* when nvector =2 and nscalar=0, nfiles can be 2 otherwise nfiles must be 1
     */
    if (nscalar && nfiles != 1)
      mpp_error(
          "fregrid: when scalar_field is specified, number of files must be 1");
    if (nfiles_out == 0) {
      for (i = 0; i < nfiles; i++)
        strcpy(output_file[i], input_file[i]);
    } else if (nfiles_out != nfiles)
      mpp_error(
          "fregrid:number of input file is not equal to number of output file");
  } else
  ...
  if (lbegin != 0 || lend != -1) { /* at least one of lbegin and lend is set */
    if (lbegin < 1 || lend < lbegin)
      mpp_error("fregrid:LstepBegin should be a positive integer and no larger "
  ...
  if (!strcmp(interp_method, "conserve_order1")) {
    if (mpp_pe() == mpp_root_pe())
      printf("****fregrid: first order conservative scheme will be used for "
             "regridding.\n");
    opcode |= CONSERVE_ORDER1;
  } else if (!strcmp(interp_method, "conserve_order2")) {
    if (mpp_pe() == mpp_root_pe())
      printf("****fregrid: second order conservative scheme will be used for "
             "regridding.\n");
    opcode |= CONSERVE_ORDER2;
  } else if (!strcmp(interp_method, "bilinear")) {
    if (mpp_pe() == mpp_root_pe())
      printf("****fregrid: bilinear remapping scheme will be used for "
             "regridding.\n");
    opcode |= BILINEAR;
  } else
    mpp_error("fregrid: interp_method must be 'conserve_order1', "
              "'conserve_order2' or 'bilinear'");

  if (test_case) {
    if (nfiles != 1)
      mpp_error("fregrid: when test_case is specified, nfiles should be 1");
    sprintf(output_file[0], "%s.%s.output", test_case, interp_method);
  }

  if (check_conserve)
    opcode |= CHECK_CONSERVE;

  if (opcode & BILINEAR) {
    int ncontact;
    ncontact = read_mosaic_ncontacts(mosaic_in);
    if (nlon == 0 || nlat == 0)
      mpp_error("fregrid: when interp_method is bilinear, nlon and nlat should "
                "be specified");
    if (ntiles_in != 6)
      mpp_error("fregrid: when interp_method is bilinear, the input mosaic "
                "should be 6 tile cubic grid");
    if (ncontact != 12)
      mpp_error("fregrid: when interp_method is bilinear, the input mosaic "
                "should be 12 contact cubic grid");
    if (mpp_npes() > 1)
      mpp_error("fregrid: parallel is not implemented for bilinear remapping");
  } else
    y_at_center = 1;
  ...
  if (mosaic_out)
    ...
    get_output_grid_by_size(ntiles_out, grid_out, lonbegin, lonend, latbegin,
                            latend, nlon, nlat, finer_step, y_at_center,
                            opcode);

  if (remap_file)
    set_remap_file(ntiles_out, mosaic_out, remap_file, interp, &opcode,
                   save_weight_only);

  /* preparing for the interpolation, if remapping information exist, read it
     from remap_file, otherwise create the remapping information and write it to
     remap_file
  */
  if (opcode & BILINEAR) /* bilinear interpolation from cubic to lalon */
    setup_bilinear_interp(ntiles_in, grid_in, ntiles_out, grid_out, interp,
                          opcode);
  else
    setup_conserve_interp(ntiles_in, grid_in, ntiles_out, grid_out, interp,
                          opcode);

  if (save_weight_only) {
    if (mpp_pe() == mpp_root_pe()) {
      printf("NOTE: Successfully running fregrid and the following files which "
  ...
  set_mosaic_data_file(ntiles_out, mosaic_out, dir_out, file_out,
                       output_file[0]);
  if (nfiles == 2) {
    set_mosaic_data_file(ntiles_in, mosaic_in, dir_in, file2_in, input_file[1]);
    set_mosaic_data_file(ntiles_out, mosaic_out, dir_out, file2_out,
                         output_file[1]);
  }
  ...
  get_input_metadata(ntiles_in, nfiles, file_in, file2_in, scalar_in, u_in,
                     v_in, grid_in, kbegin, kend, lbegin, lend);

  set_output_metadata(ntiles_in, nfiles, file_in, file2_in, scalar_in, u_in,
                      v_in, ntiles_out, file_out, file2_out, scalar_out, u_out,
                      v_out, grid_out, history, TAGNAME);

  if (nscalar > 0)
    get_field_missing(ntiles_in, scalar_in);
  if (nvector > 0) {
  ...
  if (test_case) {
    if (nscalar != 1 || nvector != 0)
      mpp_error("fregrid: when test_case is specified, nscalar must be 1 and "
                "nvector must be 0");
    if (scalar_in->var->nz != 1)
      mpp_error("fregrid: when test_case is specified, number of vertical "
                "level must be 1");
    file_in->nt = 1;
    file_out->nt = 1;
  }
  ...
  for (m = 0; m < file_in->nt; m++) {
    ...
    if (nfiles > 1)
      write_output_time(ntiles_out, file2_out, m);

    /* first interp scalar variable */
    for (l = 0; l < nscalar; l++) {
      if (!scalar_in->var[l].has_taxis && m > 0)
        continue;
      level = m + scalar_in->var[l].lstart;
      if (test_case)
        get_test_input_data(test_case, test_param, ntiles_in, scalar_in,
                            grid_in, bound_T, opcode);
      else
        get_input_data(ntiles_in, scalar_in, grid_in, bound_T, l, level,
                       opcode);
      allocate_field_data(ntiles_out, scalar_out, grid_out, l);
      if (opcode & BILINEAR)
        do_scalar_bilinear_interp(interp, l, ntiles_in, grid_in, grid_out,
                                  scalar_in, scalar_out, finer_step,
                                  fill_missing);
      else
        do_scalar_conserve_interp(interp, l, ntiles_in, grid_in, ntiles_out,
                                  grid_out, scalar_in, scalar_out, opcode);
      write_field_data(ntiles_out, scalar_out, grid_out, l, m);
      if (opcode & CONSERVE_ORDER2) {
        for (n = 0; n < ntiles_in; n++) {
          free(scalar_in[n].grad_x);
          free(scalar_in[n].grad_y);
          if (scalar_in[n].var[l].has_missing)
            free(scalar_in[n].grad_mask);
        }
      }
      for (n = 0; n < ntiles_in; n++)
        free(scalar_in[n].data);
      for (n = 0; n < ntiles_out; n++)
        free(scalar_out[n].data);
    }

    /* then interp vector field */
    for (l = 0; l < nvector; l++) {
      if (!u_in[n].var[l].has_taxis && m > 0)
        continue;
      ...
      if (opcode & BILINEAR)
        do_vector_bilinear_interp(interp, l, ntiles_in, grid_in, ntiles_out,
                                  grid_out, u_in, v_in, u_out, v_out,
                                  finer_step, fill_missing);
      else
        do_vector_conserve_interp(interp, l, ntiles_in, grid_in, ntiles_out,
                                  grid_out, u_in, v_in, u_out, v_out, opcode);
  ...
  if (mpp_pe() == mpp_root_pe()) {
    printf("Successfully running fregrid and the following output file are "
           "generated.\n");
    for (n = 0; n < ntiles_out; n++) {
...
int gs_make_topog(char *history, char *mosaic_file, char *topog_type,
                  ...
                  char *topog_file, double bottom_depth, double min_depth,
                  double scale_factor, int num_filter_pass, double gauss_amp,
                  double gauss_scale, double slope_x, double slope_y,
                  double bowl_south, double bowl_north, double bowl_west,
                  double bowl_east, int fill_first_row, int filter_topog,
                  int round_shallow, int fill_shallow, int deepen_shallow,
                  int smooth_topo_allow_deepening, char *output_file) {
  char *topog_field = NULL;
  int option_index, i, c;

  if (x_refine != 2 || y_refine != 2)
    mpp_error("Error from make_topog: x_refine and y_refine should be 2, "
              "contact developer");
  if (mpp_pe() == mpp_root_pe())
    printf("NOTE from make_topog ==> x_refine is %d, y_refine is %d\n",
           x_refine, y_refine);

  if (strcmp(topog_type, "rectangular_basin") == 0) {
    if (mpp_pe() == mpp_root_pe())
      printf("NOTE from make_topog ==> the basin depth is %f\n", basin_depth);
    ...
    if (!topog_file || !topog_field)
      mpp_error("Error from make_topog: when topog_type is realistic, "
    ...
    if (mpp_pe() == mpp_root_pe()) {
      ...
      printf("NOTE from make_topog ==> num_filter_pass is: %d\n",
             num_filter_pass);
      if (fill_first_row)
        printf("NOTE from make_topog ==>make first row of ocean model all land "
               "points.\n");
      if (filter_topog)
        printf("NOTE from make_topog ==>will apply filter to topography.\n");
      if (round_shallow)
        printf(
            "NOTE from make_topog ==>Make cells land if depth is less than 1/2 "
            "mimumim depth, otherwise make ocean.\n");
      if (fill_shallow)
        printf("NOTE from make_topog ==>Make cells less than minimum depth "
               "land.\n");
      if (deepen_shallow)
        printf("NOTE from make_topog ==>Make cells less than minimum depth "
               "equal to minimum depth.\n");
      if (smooth_topo_allow_deepening)
        printf("NOTE from make_topog ==>allow filter to deepen cells.\n");
    }
  } else {
    mpp_error("make_topog: topog_type should be rectangular_basin, gaussian, "
              "bowl, idealized or realistic");
  }
...
#define STRING 255
    ...
    */
    ...
    /* loop through each tile to get tile information and set up meta data for
     * output file */
    ...
    for (n = 0; n < ntiles; n++) {
      ...
      if (nx[n] % x_refine != 0)
        mpp_error(
            "make_topog: supergrid x-size can not be divided by x_refine");
      if (ny[n] % y_refine != 0)
        mpp_error(
            "make_topog: supergrid y-size can not be divided by y_refine");
      nx[n] /= x_refine;
      ny[n] /= y_refine;
      nxp[n] = nx[n] + 1;
      nyp[n] = ny[n] + 1;
      ...
      id_depth[n] =
          mpp_def_var(fid, depth_name, NC_DOUBLE, 2, dims, 2, "standard_name",
    ...
    for (n = 0; n < ntiles; n++) {
      ...
      tmp = (double *)malloc((nxc * x_refine + 1) * (nyc * y_refine + 1) *
                             sizeof(double));
      start[0] = jsc * y_refine;
      start[1] = isc * x_refine;
      nread[0] = nyc * y_refine + 1;
      nread[1] = nxc * x_refine + 1;
      ni = nxc * x_refine + 1;
      g_fid = mpp_open(tile_files[n], MPP_READ);
      ...
      if (strcmp(topog_type, "rectangular_basin") == 0)
        ...
        create_bowl_topog(nx[n], ny[n], x, y, bottom_depth, min_depth,
                          bowl_east, bowl_south, bowl_west, bowl_north, depth);
      else if (strcmp(topog_type, "idealized") == 0)
        create_idealized_topog(nx[n], ny[n], x, y, bottom_depth, min_depth,
                               depth);
      else if (strcmp(topog_type, "realistic") == 0)
        create_realistic_topog(nxc, nyc, x, y, topog_file, topog_field,
                               scale_factor, fill_first_row, filter_topog,
                               num_filter_pass, smooth_topo_allow_deepening,
...
int gs_make_solo_mosaic(char *history, int ntiles, char *mosaic_name,
                        char *grid_descriptor, char **tilefile, double periodx,
                        double periody, char *dir) {
  char *pch = NULL;
  ...
  /*--- if file name is not specified through -f, file name will be
    ...
    if (ntiles == 1) {
      ...
      for (n = 0; n < ntiles; n++) {
        sprintf(tilefile[n], "horizontal_grid.tile%d.nc", n + 1);
            ...
            */
    if (nfiles != ntiles)
      mpp_error("make_solo_mosaic: number of grid files specified through -n "
  ...
  /*find the contact region between tiles, currently assume the contact region
    are align-contact There should be no contact between same directions of two
    tiles ( w-w, e-e, s-s, n-n) We assume no contact between w-s, e-n ( will
    added in if needed ) . */
  ncontact = 0;
  for (n = 0; n < ntiles; n++) {
    for (m = n; m < ntiles; m++) {
      int count;
      int istart1[MAXCONTACT], iend1[MAXCONTACT], jstart1[MAXCONTACT],
          jend1[MAXCONTACT];
      int istart2[MAXCONTACT], iend2[MAXCONTACT], jstart2[MAXCONTACT],
          jend2[MAXCONTACT];
      count =
          get_align_contact(n + 1, m + 1, nxp[n], nyp[n], nxp[m], nyp[m], x[n],
                            y[n], x[m], y[m], periodx, periody, istart1, iend1,
                            jstart1, jend1, istart2, iend2, jstart2, jend2);
      if (ncontact + count > MAXCONTACT)
        mpp_error(
            "make_solo_mosaic: number of contacts is more than MAXCONTACT");
      for (l = 0; l < count; l++) {
  ...
  /* write out data */
  {
    ...
    int fid, dim_ntiles, dim_ncontact, dim_string, id_mosaic, id_gridtiles,
        id_contacts;
    ...
    /* define variable */
    id_mosaic = mpp_def_var(fid, "mosaic", MPP_CHAR, 1, &dim_string, 4,
                            "standard_name", "grid_mosaic_spec", "children",
                            "gridtiles", "contact_regions", "contacts",
    ...
    if (ncontact > 0) {
      dim[0] = dim_ncontact;
      dim[1] = dim_string;
      id_contacts = mpp_def_var(
          fid, "contacts", MPP_CHAR, 2, dim, 5, "standard_name",
          "grid_contact_spec", "contact_type", "boundary", "alignment", "true",
          "contact_index", "contact_index", "orientation", "orient");
      id_contact_index = mpp_def_var(fid, "contact_index", MPP_CHAR, 2, dim, 1,
                                     "standard_name",
    ...
    for (n = 0; n < ncontact; n++) {
      sprintf(str, "%s:%s::%s:%s", mosaic_name, tile_name[contact_tile1[n]],
              mosaic_name, tile_name[contact_tile2[n]]);
      start[0] = n;
      nwrite[1] = strlen(str);
      mpp_put_var_value_block(fid, id_contacts, start, nwrite, str);
      sprintf(str, "%d:%d,%d:%d::%d:%d,%d:%d", contact_tile1_istart[n],
              contact_tile1_iend[n], contact_tile1_jstart[n],
              contact_tile1_jend[n], contact_tile2_istart[n],
              contact_tile2_iend[n], contact_tile2_jstart[n],
              contact_tile2_jend[n]);
      nwrite[1] = strlen(str);
...
int gs_river_regrid(char *history, char *mosaic_file, char *river_src_file,
                    char *output_file) {
  ...
  /* check the arguments */
  if (!mosaic_file)
    mpp_error("fregrid: mosaic_grid is not specified");
  if (!river_src_file)
    mpp_error("fregrid: river_source_file is not specified");
  ...
  printf("Successfully running river_regrid and the following output file are "
         "generated.\n");
  for (n = 0; n < ntiles; n++)
...
void get_source_data(const char *src_file, river_type *river_data) {
  ...
  for (i = 1; i < nx; i++)
    river_data->xb[i] = 0.5 * (river_data->xt[i - 1] + river_data->xt[i]);
  for (j = 1; j < ny; j++)
    river_data->yb[j] = 0.5 * (river_data->yt[j - 1] + river_data->yt[j]);
  ...
  if (fabs(river_data->xb[0]) > EPSLN)
    mpp_error("river_regrid: The starting longitude of grid bound is not 0");
  if (fabs(river_data->xb[nx] - 360) > EPSLN)
    mpp_error("river_regrid: The ending longitude of grid bound is not 360");
  if (fabs(river_data->yb[0] + 90) > EPSLN)
    mpp_error("river_regrid: The starting latitude of grid bound is not -90");
  if (fabs(river_data->yb[ny] - 90) > EPSLN)
    mpp_error("river_regrid: The ending latitude of grid bound is not 90");
  river_data->xb[0] = 0.;
  river_data->xb[nx] = 360.;
  river_data->yb[0] = -90.;
  river_data->yb[ny] = 90.;
  for (j = 0; j < nyp; j++)
    river_data->yb_r[j] = river_data->yb[j] * D2R;
  for (i = 0; i < nxp; i++)
    river_data->xb_r[i] = river_data->xb[i] * D2R;
}
...
void get_mosaic_grid(const char *coupler_mosaic, const char *land_mosaic,
  ...
  /* coupler_mosaic, land_mosaic, and exchange grid file should be located in
   * the same directory */
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (ni % x_refine != 0)
      mpp_error(
          "river_regrid: supergrid x-size can not be divided by x_refine");
    if (nj % y_refine != 0)
      mpp_error(
          "river_regrid: supergrid y-size can not be divided by y_refine");
    nx = ni / x_refine;
    ny = nj / y_refine;
    nxp = nx + 1;
    nyp = ny + 1;
    ...
    for (j = 0; j < ny; j++)
      for (i = 0; i < nx; i++) {
        ii = i + 1;
        jj = j + 1;
        river_data[n].xt[jj * nxp2 + ii] =
            x[(j * y_refine + 1) * nip + i * x_refine + 1];
        river_data[n].yt[jj * nxp2 + ii] =
            y[(j * y_refine + 1) * nip + i * x_refine + 1];
      }
    for (j = 0; j < nyp; j++)
      for (i = 0; i < nxp; i++) {
    ...
    get_grid_area(&nx, &ny, river_data[n].xb_r, river_data[n].yb_r,
                  river_data[n].area);
    ...
    for (m = 0; m < n_xgrid_files; m++) {
      if (strstr(xgrid_file[m], tilename)) {
        ...
        for (l = 0; l < nxgrid; l++)
          area[j2[l] * nx + i2[l]] += xgrid_area[l];
        free(i1);
        free(j1);
        free(i2);
        free(j2);
    ...
    for (m = 0; m < nx * ny; m++)
      area[m] *= 4 * M_PI * RADIUS * RADIUS;
    for (m = 0; m < nx * ny; m++) {
      river_data[n].landfrac[m] = area[m] / river_data[n].area[m];
      /* consider truncation error */
      if (fabs(river_data[n].landfrac[m] - 1) < EPSLN)
        river_data[n].landfrac[m] = 1;
      if (fabs(river_data[n].landfrac[m]) < EPSLN)
        river_data[n].landfrac[m] = 0;
  ...
  for (n = 1; n < ntiles; n++) {
    if (river_data[n].nx != river_data[0].nx ||
        river_data[n].ny != river_data[0].ny)
    ...
    --------------------------------------------------------------------------*/
  {
    ...
    if (ncontact > 0) {
      read_mosaic_contact(land_mosaic, tile1, tile2, istart1, iend1, jstart1,
                          jend1, istart2, iend2, jstart2, jend2);
      if (ncontact <= 2) { /* x-cyclic of y-cyclic */
        if (ntiles != 1)
          mpp_error(
              "river_regrid: number of tiles must be 1 for single contact");
        for (n = 0; n < ncontact; n++) {
          if (istart1[n] == iend1[n] && istart2[n] == iend2[n]) /* x_cyclic */
            *opcode |= X_CYCLIC;
          else if (jstart1[n] == jend1[n] &&
                   istart2[n] == iend2[n]) /* x_cyclic */
            *opcode |= Y_CYCLIC;
          else
            mpp_error("river_regrid: for one-tile mosaic, the boundary "
                      "condition should be either x-cyclic or y-cyclic");
        }
      } else if (ncontact == 12) {
        if (ntiles != 6)
          mpp_error("river_regrid: the mosaic must be a 6-tile cubic grid for "
                    "12 contacts.");
        *opcode |= CUBIC_GRID;
      } else
        mpp_error("river_regrid: the number of contact should be either 0, 1, "
                  "2 or 6");
    }
...
/*------------------------------------------------------------------------------
  ...
  allocate memory to river data and initialize these river data with missing
  value
  ----------------------------------------------------------------------------*/
void init_river_data(int ntiles, river_type *river_out,
                     const river_type *const river_in) {
...
/*------------------------------------------------------------------------------
  void calc_max_subA(const river_type *river_in, river_type *river_out, int
  ntiles) find max value of suba in each model grid cell.
  ----------------------------------------------------------------------------*/
void calc_max_subA(const river_type *river_in, river_type *river_out,
    ...
    for (j = 0; j < ny_out; j++)
      for (i = 0; i < nx_out; i++) {
        ...
        for (jj = 0; jj < ny_in; jj++) {
          ll_y = yb_in[jj];
          ur_y = yb_in[jj + 1];
          if ((yv1[0] <= ll_y) && (yv1[1] <= ll_y) && (yv1[2] <= ll_y) &&
              (yv1[3] <= ll_y))
            continue;
          if ((yv1[0] >= ur_y) && (yv1[1] >= ur_y) && (yv1[2] >= ur_y) &&
              (yv1[3] >= ur_y))
            continue;

          for (ii = 0; ii < nx_in; ii++) {
            if (river_in->subA[jj * nx_in + ii] == missing)
              continue;
            ll_x = xb_in[ii];
            ur_x = xb_in[ii + 1];
            /* adjust xv1 to make sure it is in the range as ll_x and ur_x */
            adjust_lon(xv1, 0.5 * (ll_x + ur_x));
            if ((xv1[0] <= ll_x) && (xv1[1] <= ll_x) && (xv1[2] <= ll_x) &&
                (xv1[3] <= ll_x))
              continue;
            if ((xv1[0] >= ur_x) && (xv1[1] >= ur_x) && (xv1[2] >= ur_x) &&
                (xv1[3] >= ur_x))
              continue;
            if ((n_out = clip(xv1, yv1, 4, ll_x, ll_y, ur_x, ur_y, xv2, yv2)) >
                0) {
              double xarea;
              xarea = poly_area(xv2, yv2, n_out);
              if (xarea / river_out[n].area[j * nx_out + i] < MIN_AREA_RATIO)
                continue;
              if (river_in->subA[jj * nx_in + ii] >
                  river_out[n].subA[j1 * nxp2 + i1])
                river_out[n].subA[j1 * nxp2 + i1] =
                    river_in->subA[jj * nx_in + ii];
            }
          }
        }
      }
  ...
  for (n = 0; n < ntiles; n++)
    psubA[n] = river_out[n].subA;
  update_halo_double(ntiles, psubA, river_out->nx, river_out->ny, opcode);
  free(psubA);
}; /* calc_max_subA */

/*------------------------------------------------------------------------------
  void update_halo_double(int ntiles, double *data, int nx, int ny, unsigned int
  opcode) We assume all the tiles have the same size.
  -----------------------------------------------------------------------------*/
void update_halo_double(int ntiles, double **data, int nx, int ny,
                        unsigned int opcode) {
...
void update_halo_int(int ntiles, int **data, int nx, int ny,
                     unsigned int opcode) {
  double **ldata;
  int n, i, j, nxp2, nyp2;
    ...
    for (j = 1; j <= ny; j++)
      for (i = 1; i <= nx; i++) {
        ...
        if (river_data[n].landfrac[jm1 * nx + im1] == 0) {
          ...
          jget = -1;
          if (subA_me >
              suba_cutoff) { /* tocell will be land cell with larger subA value,
                                instead of neighboring ocean cell. */
...
/*------------------------------------------------------------------------------
  ...
  calculate travel and basin according to tocell, as well as celllength,
  cellarea. For each basin, one and only one river point will have tocell = 0,
  this point will have travel = 1.
  ----------------------------------------------------------------------------*/
    ...
    for (j = 0; j < ny; j++)
      for (i = 0; i < nx; i++) {
        if (river_data[n].landfrac[j * nx + i] > 0) {
          ...
          river_data[n].cellarea[j * nx + i] =
              river_data[n].area[j * nx + i] *
              river_data[n].landfrac[j * nx + i];
          dir = pdir[n][jj * nxp2 + ii];
          if (dir >= 0) {
            i_dest = ii + di[dir];
            j_dest = jj + dj[dir];
            river_data[n].celllength[j * nx + i] =
                distance(river_data[n].xt[jj * nxp2 + ii],
                         river_data[n].yt[jj * nxp2 + ii],
          ...
          if (dir >= 0 && pbasin[n][j * nxp2 + i] == basin_missing) {
            ...
            if (ptravel[n][j_dest * nxp2 + i_dest] == cur_travel) {
              ...
              if (pbasin[n][j_dest * nxp2 + i_dest] == basin_missing) {
                mpp_error(
                    "river_grid: the tocell should have valid basin value");
              }
              ...
              for (ioff = 0; ioff < ncells; ioff++) {
                ...
                if (dir >= 0) {
                  if (opcode & CUBIC_GRID) {
                    if (ii == 0) { /* west */
                      if (n % 2 == 0)
                        dir = (dir + 2) % 8; /* tile 1 3 5 */
                    } else if (ii == nxp) {  /*east */
                      if (n % 2 == 1)
                        dir = (dir + 2) % 8; /* tile 2 4 6 */
                    } else if (jj == 0) {    /* south */
                      if (n % 2 == 1)
                        dir = (dir + 6) % 8; /* tile 2 4 6 */
                    } else if (jj == nyp) {  /*north */
                      if (n % 2 == 0)
                        dir = (dir + 6) % 8; /* tile 1 3 5 */
                    }
                  }
                  i_dest = ii + di[dir];
                  j_dest = jj + dj[dir];
                  if (i_dest == i && j_dest == j)
                    psubA[n][j * nxp2 + i] += psubA[n][jj * nxp2 + ii];
                }
              }
          }
        }
...
/*------------------------------------------------------------------------------
  ...
  check to make sure all the river points have been assigned travel and basin
  value and all the ocean points will have missing value.
  ----------------------------------------------------------------------------*/
...
void check_river_data(int ntiles, river_type *river_data) {
  ...
  printf("==> NOTE from river_regrid: maximum travel is %d and maximum basin "
         "is %d.\n",
         maxtravel, maxbasin);
      ...
      for (i = 1; i <= nx; i++) {
        ...
        if (river_data[n].landfrac[jm1 * nx + im1] == 0) {
          if (tocell != tocell_missing || travel != travel_missing ||
            ...
            printf("At ocean points (i=%d,j=%d), subA = %f, tocell = %d, "
                   "travel = %d, basin = %d.\n ",
                   i, j, subA, tocell, travel, basin);
            mpp_error("river_regrid, subA, tocell, travel, or basin is not "
                      "missing value for some ocean points");
          }
        } else {
          if (tocell == tocell_missing || travel == travel_missing ||
              basin == basin_missing || subA == subA_missing) {
            printf("At river points (i=%d,j=%d), subA = %f, tocell = %d, "
                   "travel = %d, basin = %d.\n ",
                   i, j, subA, tocell, travel, basin);
            mpp_error("river_regrid, subA, tocell, travel, or basin is missing "
                      "value for some river points");
          }
        }
        /* check if the points with land_frac == 1 and tocell=0, those points
         * should be sink.*/
        if (river_data[n].landfrac[jm1 * nx + im1] == 1 && tocell == 0) {
          for (joff = 0; joff < ncells; joff++) {
            ...
            for (ioff = 0; ioff < ncells; ioff++) {
              ...
              if (river_data[n].tocell[jj * nxp2 + ii] == tocell_missing) {
                ncoast_full_land++;
                printf("At point (%d,%d), tocell = 0 and landfrac = 1 is a "
                       "coast point\n",
                       i, j);
                goto done_check;
              }
          ...
          printf(
              "At point (%d,%d), tocell = 0 and landfrac = 1 is a sink point\n",
              i, j);
          nsink++;
        done_check:
          continue;
  ...
  if (ncoast_full_land > 0)
    printf("Warning from river_regrid: there are %d coast points is a full "
           "land cell\n",
           ncoast_full_land);
  else
    printf("NOTE from river_regrid: there are no coast points is a full land "
           "cell\n");

  if (nsink > 0)
    printf("Warning from river_regrid: there are %d sink points is a full land "
           "cell\n",
           nsink);
  else
    printf("NOTE from river_regrid: there are no sink points is a full land "
           "cell\n");

  /* check river travel to make sure there is one and only one point with travel
   * = 0. */
  ...
  for (n = 0; n < ntiles; n++) {
    for (j = 1; j <= ny; j++)
      for (i = 1; i <= nx; i++) {
        if (river_data[n].last_point[(j - 1) * nx + i - 1])
          ++ncoast[river_data[n].basin[j * nxp2 + i] - 1];
      }
  }
  for (n = 0; n < maxbasin; n++) {
    if (ncoast[n] <= 0) {
      ...
      printf("river with basin = %d has more than one point with travel = 0.\n",
             n + 1);
      mpp_error(
          "river_regrid: some river has more than one point with travel = 0");
    }
  }
  free(ncoast);
...
void sort_basin(int ntiles, river_type *river_data) {
    ...
    for (j = 1; j <= ny; j++)
      for (i = 1; i <= nx; i++) {
        if (river_data[n].last_point[(j - 1) * nx + i - 1]) { /* coast point */
          ...
          if (basin > maxbasin || basin < 1)
            mpp_error("river_regrid: basin should be between 1 and maxbasin");
  ...
  for (n = 0; n < maxbasin; n++) {
    if (maxsuba[n] == subA_missing)
      mpp_error("river_regrid: maxsuba is not assigned for some basin");
  }
  ...
  for (n = 0; n < maxbasin; n++) {
    if (rank[n] < 0)
      mpp_error("river_regrid: rank should be no less than 0");
  }
    ...
    for (j = 1; j <= ny; j++)
      for (i = 1; i <= nx; i++) {
        ...
        if (basin != basin_missing) {
          if (basin > maxbasin || basin < 1)
            mpp_error("river_regrid: basin should be a positive integer no "
                      "larger than maxbasin");
...
void write_river_data(const char *river_src_file, const char *output_file,
                      river_type *river_data, const char *history, int ntiles) {
  double *subA, *yt, *xt;
  int *tocell, *travel, *basin;
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    id_x = mpp_def_var(fid, x_name, MPP_DOUBLE, 2, dimid, 2, "long_name",
                       "Geographic longitude", "units", "degree_east");
    id_y = mpp_def_var(fid, y_name, MPP_DOUBLE, 2, dimid, 2, "long_name",
    ...
    */
    if (ntiles == 1) {
      for (i = 0; i < nx; i++)
        xt[i] = river_data[n].xt[nxp2 + i + 1];
      for (j = 0; j < ny; j++)
        yt[j] = river_data[n].yt[(j + 1) * nxp2 + 1];
...
/*------------------------------------------------------------------------------
  ...
  sort array in increasing order and get the rank of each member in the original
  array the array size of array and rank will be size.
...
void qsort_index(double array[], int start, int end, int rank[]) {
  ...
  if (end > start) {
    ...
    swap(array[start], array[pivot_index],
         tmp_double); /*** choose arbitrary pivot ***/
...
int gs_transfer_to_mosaic(char *old_file, char *mosaic_dir) {
  ...
  char atmos_name[128] = "atmos", land_name[128] = "land",
       ocean_name[128] = "ocean";
  ...
  if (mpp_field_exist(old_file, "AREA_ATMxOCN"))
    is_coupled_grid = 1;
  if (mpp_field_exist(old_file, "AREA_ATM"))
    is_ocean_only = 0;
  if (mpp_field_exist(old_file, "DI_ATMxOCN"))
    interp_order = 2;
  // -----------------ocean_hgrid.nc---------------------------------[
  // Ocean
  {
    ...
    vid = mpp_get_varid(fid_old, "x_C");
    mpp_get_var_value(fid_old, vid, x_C);
    vid = mpp_get_varid(fid_old, "y_C");
    mpp_get_var_value(fid_old, vid, y_C);
    vid = mpp_get_varid(fid_old, "x_T");
    mpp_get_var_value(fid_old, vid, x_T);
    vid = mpp_get_varid(fid_old, "y_T");
    mpp_get_var_value(fid_old, vid, y_T);
    vid = mpp_get_varid(fid_old, "x_vert_T");
    mpp_get_var_value(fid_old, vid, x_vert_T);
    vid = mpp_get_varid(fid_old, "y_vert_T");
    mpp_get_var_value(fid_old, vid, y_vert_T);
    vid = mpp_get_varid(fid_old, "ds_01_11_C");
    mpp_get_var_value(fid_old, vid, ds_01_11_C);
    vid = mpp_get_varid(fid_old, "ds_10_11_C");
    mpp_get_var_value(fid_old, vid, ds_10_11_C);
    vid = mpp_get_varid(fid_old, "ds_11_21_C");
    mpp_get_var_value(fid_old, vid, ds_11_21_C);
    vid = mpp_get_varid(fid_old, "ds_11_12_C");
    mpp_get_var_value(fid_old, vid, ds_11_12_C);
    vid = mpp_get_varid(fid_old, "ds_01_11_T");
    mpp_get_var_value(fid_old, vid, ds_01_11_T);
    vid = mpp_get_varid(fid_old, "ds_10_11_T");
    mpp_get_var_value(fid_old, vid, ds_10_11_T);
    vid = mpp_get_varid(fid_old, "ds_11_21_T");
    mpp_get_var_value(fid_old, vid, ds_11_21_T);
    vid = mpp_get_varid(fid_old, "ds_11_12_T");
    mpp_get_var_value(fid_old, vid, ds_11_12_T);
    vid = mpp_get_varid(fid_old, "ds_02_22_T");
    mpp_get_var_value(fid_old, vid, ds_02_22_T);
    vid = mpp_get_varid(fid_old, "ds_20_22_T");
    mpp_get_var_value(fid_old, vid, ds_20_22_T);
    vid = mpp_get_varid(fid_old, "angle_C");
    mpp_get_var_value(fid_old, vid, angle_C);
    mpp_close(fid_old);
    ...
    for (j = 0; j < ny_T; j++) {
      for (i = 0; i < nx_T; i++) {
        ...
        */
        ...
        if (i > 0)
          dx[(2 * j + 2) * nx + (2 * i)] = ds_11_21_C[j * nx_T + (i - 1)];
        ...
        if (j == 0) {
          ...
          dy[(2 * j) * nxp + (2 * i + 2)] =
              ds_20_22_T[j * nx_T + i] - ds_10_11_C[j * nx_T + i];
        ...
        if (i == 0) {
          ...
          dx[(2 * j + 2) * nx + (2 * i)] =
              ds_02_22_T[j * nx_T + i] - ds_01_11_C[j * nx_T + i];
          dy[(2 * j) * nxp] = unknown;
    ...
    // if(angle_dx[j*nxp+i]>0.0) printf("%d %d %f\n ",i,j,angle_dx[j*nxp+i]); }
    // } for(j = 0; j<ny_T; j++) { printf("%f ",y_C[j*nx_T+(nx_T+0)/4]); } for(i
    // = 0; i<nx_T; i++) { if(y_T[(ny_T-1)*nx_T+i]>85.) printf("%f
    // ",y_T[(ny_T-1)*nx_T+i]); } printf("\n\n"); for(j = 0; j<nyp; j++) {
    // printf("%f ",y[j*nxp+(nxp-1)/4]); } for(i = 0; i<nxp; i++) {
    // if(y[(nyp-1)*nxp+i]>85.) printf("%f ",y[(nyp-1)*nxp+i]); }
    ...
    /*-------ocean_hgrid.nc-------*/
    {
      ...
      char tile_spec_version[] = "0.2", geometry[] = "spherical",
           discretization[] = "logically_rectangular", conformal[] = "true";
      ...
      id_tile = mpp_def_var(
          fid, "tile", MPP_CHAR, 1, dimlist, 5, "standard_name",
          "grid_tile_spec", "tile_spec_version", tile_spec_version, "geometry",
          geometry, "discretization", discretization, "conformal", conformal);
      ...
      dims[0] = dimlist[2];
      dims[1] = dimlist[3];
      id_dx = mpp_def_var(fid, "dx", MPP_DOUBLE, 2, dims, 2, "standard_name",
                          "grid_edge_x_distance", "units", "meters");
      dims[0] = dimlist[4];
      dims[1] = dimlist[1];
      id_dy = mpp_def_var(fid, "dy", MPP_DOUBLE, 2, dims, 2, "standard_name",
                          "grid_edge_y_distance", "units", "meters");

      dims[0] = dimlist[2];
      dims[1] = dimlist[1];
      id_angle_dx = mpp_def_var(
          fid, "angle_dx", MPP_DOUBLE, 2, dims, 2, "standard_name",
          "grid_vertex_x_angle_WRT_geographic_east", "units", "degrees_east");

      dims[0] = dimlist[4];
      dims[1] = dimlist[3];
      id_area = mpp_def_var(fid, "area", MPP_DOUBLE, 2, dims, 2,
                            "standard_name", "grid_cell_area", "units", "m2");
  ...
  if (is_coupled_grid) {
    ...
    vid = mpp_get_varid(fid_old, "xba");
    mpp_get_var_value(fid_old, vid, xba);
    vid = mpp_get_varid(fid_old, "yba");
    mpp_get_var_value(fid_old, vid, yba);
    vid = mpp_get_varid(fid_old, "xta");
    mpp_get_var_value(fid_old, vid, xta);
    vid = mpp_get_varid(fid_old, "yta");
    mpp_get_var_value(fid_old, vid, yta);
    mpp_close(fid_old);
    ...
    for (j = 0; j < nyba; j++)
      y_atmos[2 * j * nxap] = yba[j];
    for (j = 0; j < nyta; j++)
      y_atmos[(2 * j + 1) * nxap] = yta[j];

    for (j = 1; j < nyap; j++)
      for (i = 0; i < nxap; i++)
        x_atmos[j * nxap + i] = x_atmos[i];
    for (i = 1; i < nxap; i++)
      for (j = 0; j < nyap; j++)
        y_atmos[j * nxap + i] = y_atmos[j * nxap];
  ...
  if (is_coupled_grid) {
    ...
    vid = mpp_get_varid(fid_old, "xbl");
    mpp_get_var_value(fid_old, vid, xbl);
    vid = mpp_get_varid(fid_old, "ybl");
    mpp_get_var_value(fid_old, vid, ybl);
    vid = mpp_get_varid(fid_old, "xtl");
    mpp_get_var_value(fid_old, vid, xtl);
    vid = mpp_get_varid(fid_old, "ytl");
    mpp_get_var_value(fid_old, vid, ytl);
    mpp_close(fid_old);
    ...
    for (j = 0; j < nybl; j++)
      y_land[2 * j * nxlp] = ybl[j];
    for (j = 0; j < nytl; j++)
      y_land[(2 * j + 1) * nxlp] = ytl[j];

    for (j = 1; j < nylp; j++)
      for (i = 0; i < nxlp; i++)
        x_land[j * nxlp + i] = x_land[i];
  ...
  //-------------------------------ocean_vgrid.nc----------------------------------
  {
    ...
    vid = mpp_get_varid(fid_old, "zb");
    mpp_get_var_value(fid_old, vid, zb);
    vid = mpp_get_varid(fid_old, "zt");
    mpp_get_var_value(fid_old, vid, zt);
    mpp_close(fid_old);
...

    {
      ...
      varid = mpp_def_var(fid, "zeta", MPP_DOUBLE, 1, &dim, 2, "standard_name",
                          "vertical_grid_vertex", "units", "meters");
      mpp_end_def(fid);
  ...
  //-------------------------------topog.nc----------------------------------
  {
    ...
    mpp_get_var_value(fid_old, vid, depth_t);
    mpp_close(fid_old);

    {
      ...
      varid =
          mpp_def_var(fid, "depth", MPP_DOUBLE, 2, dim, 2, "standard_name",
                      "topographic depth at T-cell centers", "units", "meters");
      mpp_end_def(fid);
  ...
  if (is_coupled_grid) {
    ...
    contact_tile1_istart[0] = nxba - 1;
    contact_tile1_iend[0] = nxba - 1;
    contact_tile1_jstart[0] = 1;
    contact_tile1_jend[0] = nyba - 1;

    contact_tile2_istart[0] = 1;
    contact_tile2_iend[0] = 1;
    contact_tile2_jstart[0] = 1;
    contact_tile2_jend[0] = nyba - 1;
    //--------Initilize values --------------
    ...
    id_mosaic =
        mpp_def_var(fid, "mosaic", MPP_CHAR, 1, &dim_string, 5, "standard_name",
  ...
  if (is_coupled_grid) {
    ...
    //--------Initilize values --------------
    ntiles = 1, ncontact = 1;
    contact_tile1_istart[1] = 1;
    contact_tile1_iend[1] = 1;
    contact_tile1_jstart[1] = 1;
    contact_tile1_jend[1] = nytl;

    contact_tile2_istart[1] = nxtl;
    contact_tile2_iend[1] = nxtl;
    contact_tile2_jstart[1] = 1;
    contact_tile2_jend[1] = nytl;
    ...
    contact_tile1_istart[0] = nxbl - 1;
    contact_tile1_iend[0] = nxbl - 1;
    contact_tile1_jstart[0] = 1;
    contact_tile1_jend[0] = nybl - 1;

    contact_tile2_istart[0] = 1;
    contact_tile2_iend[0] = 1;
    contact_tile2_jstart[0] = 1;
    contact_tile2_jend[0] = nybl - 1;
    //--------Initilize values --------------
  ...
  /* ...................... ocean_mosaic.nc .................*/
  {
    char str[STRING], outfile[STRING], x_boundary_type1[255] = "",
                                       y_boundary_type1[255] = "";
    int fid, dim_ntiles, dim_ncontact, dim_string, id_mosaic, id_gridtiles,
        id_contacts;
    ...
    if (strcmp(x_boundary_type1, "cyclic") == 0) {
      n = ncontact;
      ncontact = ncontact + 1;
      contact_tile1_istart[n] = 360;
      contact_tile1_iend[n] = 360;
      contact_tile1_jstart[n] = 1;
      contact_tile1_jend[n] = 180;

      contact_tile2_istart[n] = 1;
      contact_tile2_iend[n] = 1;
      contact_tile2_jstart[n] = 1;
      contact_tile2_jend[n] = 180;
    ...
    if (strcmp(y_boundary_type1, "fold_north_edge") == 0) {
      n = ncontact;
      ncontact = ncontact + 1;
      contact_tile1_istart[n] = 1;
      contact_tile1_iend[n] = 180;
      contact_tile1_jstart[n] = 180;
      contact_tile1_jend[n] = 180;

      contact_tile2_istart[n] = 360;
      contact_tile2_iend[n] = 181;
      contact_tile2_jstart[n] = 180;
      contact_tile2_jend[n] = 180;
  ...
  if (is_coupled_grid) {
    ...
    strcpy(atile_name, "tile1");
    strcpy(ltile_name, "tile1");
    strcpy(otile_name, "tile1");
    strcpy(amosaic_name, "atmos");
    strcpy(lmosaic_name, "land");
    strcpy(omosaic_name, "ocean");
    ...
    if (naxl > 0) {
      ...
      sprintf(contact, "%s_mosaic:%s::%s_mosaic:%s", amosaic_name, atile_name,
              lmosaic_name, ltile_name);
      ...
      if (interp_order == 2) {
        id_contact = mpp_def_var(
            fid, "contact", MPP_CHAR, 1, &dim_string, 8, "standard_name",
            "grid_contact_spec", "contact_spec_version", version,
            "contact_type", "exchange", "parent1_cell", "tile1_cell",
            "parent2_cell", "tile2_cell", "xgrid_area_field", "xgrid_area",
            "distant_to_parent1_centroid", "tile1_distance",
            "distant_to_parent2_centroid", "tile2_distance");
      } else {
        id_contact = mpp_def_var(
            fid, "contact", MPP_CHAR, 1, &dim_string, 6, "standard_name",
            "grid_contact_spec", "contact_spec_version", version,
            "contact_type", "exchange", "parent1_cell", "tile1_cell",
            "parent2_cell", "tile2_cell", "xgrid_area_field", "xgrid_area");
      }
      ...
      dims[1] = dim_two;
      id_tile1_cell =
          mpp_def_var(fid, "tile1_cell", MPP_INT, 2, dims, 1, "standard_name",
                      "parent_cell_indices_in_mosaic1");
      id_tile2_cell =
          mpp_def_var(fid, "tile2_cell", MPP_INT, 2, dims, 1, "standard_name",
                      "parent_cell_indices_in_mosaic2");
      id_xgrid_area =
          mpp_def_var(fid, "xgrid_area", MPP_DOUBLE, 1, &dim_ncells, 2,
                      "standard_name", "exchange_grid_area", "units", "m2");
      if (interp_order == 2) {
        id_tile1_dist =
            mpp_def_var(fid, "tile1_distance", MPP_DOUBLE, 2, dims, 1,
                        "standard_name", "distance_from_parent1_cell_centroid");
        id_tile2_dist =
            mpp_def_var(fid, "tile2_distance", MPP_DOUBLE, 2, dims, 1,
      ...
      vid = mpp_get_varid(fid_old, "AREA_ATMxLND");
      mpp_get_var_value(fid_old, vid, atmxlnd_area);
      vid = mpp_get_varid(fid_old, "I_ATM_ATMxLND");
      mpp_get_var_value(fid_old, vid, atmxlnd_ia);
      vid = mpp_get_varid(fid_old, "J_ATM_ATMxLND");
      mpp_get_var_value(fid_old, vid, atmxlnd_ja);
      vid = mpp_get_varid(fid_old, "I_LND_ATMxLND");
      mpp_get_var_value(fid_old, vid, atmxlnd_il);
      vid = mpp_get_varid(fid_old, "J_LND_ATMxLND");
      mpp_get_var_value(fid_old, vid, atmxlnd_jl);

      for (i = 0; i < naxl; i++)
        atmxlnd_area[i] = atmxlnd_area[i] * 4.0 * PI * RADIUS * RADIUS;
      ...
      if (interp_order == 2) {
        ...
        vid = mpp_get_varid(fid_old, "DI_ATMxLND");
        mpp_get_var_value(fid_old, vid, atmxlnd_di);
        vid = mpp_get_varid(fid_old, "DJ_ATMxLND");
        mpp_get_var_value(fid_old, vid, atmxlnd_dj);
        start[1] = 0;
  ...
  if (is_coupled_grid) {
    ...
    if (naxo > 0) {
      ...
      vid = mpp_get_varid(fid_old, "AREA_ATMxOCN");
      mpp_get_var_value(fid_old, vid, atmxocn_area);
      vid = mpp_get_varid(fid_old, "I_ATM_ATMxOCN");
      mpp_get_var_value(fid_old, vid, atmxocn_ia);
      vid = mpp_get_varid(fid_old, "J_ATM_ATMxOCN");
      mpp_get_var_value(fid_old, vid, atmxocn_ja);
      vid = mpp_get_varid(fid_old, "I_OCN_ATMxOCN");
      mpp_get_var_value(fid_old, vid, atmxocn_io);
      vid = mpp_get_varid(fid_old, "J_OCN_ATMxOCN");
      mpp_get_var_value(fid_old, vid, atmxocn_jo);

      for (i = 0; i < naxo; i++)
        atmxocn_area[i] = atmxocn_area[i] * 4.0 * PI * RADIUS * RADIUS;
      ...
      if (interp_order == 2) {
        ...
        vid = mpp_get_varid(fid_old, "DI_ATMxOCN");
        mpp_get_var_value(fid_old, vid, atmxocn_di);
        vid = mpp_get_varid(fid_old, "DJ_ATMxOCN");
        mpp_get_var_value(fid_old, vid, atmxocn_dj);
        start[1] = 0;
  ...
  if (is_coupled_grid) {
    ...
    if (nlxo > 0) {
      ...
      sprintf(contact, "%s_mosaic:%s::%s_mosaic:%s", lmosaic_name, ltile_name,
              omosaic_name, otile_name);
      ...
      vid = mpp_get_varid(fid_old, "AREA_LNDxOCN");
      mpp_get_var_value(fid_old, vid, lndxocn_area);
      vid = mpp_get_varid(fid_old, "I_LND_LNDxOCN");
      mpp_get_var_value(fid_old, vid, lndxocn_il);
      vid = mpp_get_varid(fid_old, "J_LND_LNDxOCN");
      mpp_get_var_value(fid_old, vid, lndxocn_jl);
      vid = mpp_get_varid(fid_old, "I_OCN_LNDxOCN");
      mpp_get_var_value(fid_old, vid, lndxocn_io);
      vid = mpp_get_varid(fid_old, "J_OCN_LNDxOCN");
      mpp_get_var_value(fid_old, vid, lndxocn_jo);

      for (i = 0; i < nlxo; i++)
        lndxocn_area[i] = lndxocn_area[i] * 4.0 * PI * RADIUS * RADIUS;
      ...
      if (interp_order == 2) {
        ...
        vid = mpp_get_varid(fid_old, "DI_LNDxOCN");
        mpp_get_var_value(fid_old, vid, lndxocn_di);
        vid = mpp_get_varid(fid_old, "DJ_LNDxOCN");
        mpp_get_var_value(fid_old, vid, lndxocn_dj);
        start[1] = 0;
  ...
  /* ...................... mosaic.nc .................*/
  {
    ...
    id_amosaic_dir =
        mpp_def_var(fid, "atm_mosaic_dir", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "directory_storing_atmosphere_mosaic");
    id_amosaic_file =
        mpp_def_var(fid, "atm_mosaic_file", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "atmosphere_mosaic_file_name");
    id_amosaic = mpp_def_var(fid, "atm_mosaic", MPP_CHAR, 1, &dim_string, 1,
                             "standard_name", "atmosphere_mosaic_name");
    id_lmosaic_dir =
        mpp_def_var(fid, "lnd_mosaic_dir", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "directory_storing_land_mosaic");
    id_lmosaic_file =
        mpp_def_var(fid, "lnd_mosaic_file", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "land_mosaic_file_name");
    id_lmosaic = mpp_def_var(fid, "lnd_mosaic", MPP_CHAR, 1, &dim_string, 1,
                             "standard_name", "land_mosaic_name");
    id_omosaic_dir =
        mpp_def_var(fid, "ocn_mosaic_dir", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "directory_storing_ocean_mosaic");
    id_omosaic_file =
        mpp_def_var(fid, "ocn_mosaic_file", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "ocean_mosaic_file_name");
    id_omosaic = mpp_def_var(fid, "ocn_mosaic", MPP_CHAR, 1, &dim_string, 1,
                             "standard_name", "ocean_mosaic_name");
    id_otopog_dir =
        mpp_def_var(fid, "ocn_topog_dir", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "directory_storing_ocean_topog");
    id_otopog_file =
        mpp_def_var(fid, "ocn_topog_file", MPP_CHAR, 1, &dim_string, 1,
                    "standard_name", "ocean_topog_file_name");
    /* since exchange grid is created in this tool, we may add command line
     * option to specify where to store the output */
    /*    id_xgrids_dir = mpp_def_var(fid, "xgrids_dir", MPP_CHAR, 1,
       &dim_string, 1, "standard_name", "directory_storing_xgrids"); */
    dims[0] = dim_axo;
    dims[1] = dim_string;
    id_axo_file = mpp_def_var(fid, "aXo_file", MPP_CHAR, 2, dims, 1,
                              "standard_name", "atmXocn_exchange_grid_file");
    dims[0] = dim_axl;
    dims[1] = dim_string;
    id_axl_file = mpp_def_var(fid, "aXl_file", MPP_CHAR, 2, dims, 1,
                              "standard_name", "atmXlnd_exchange_grid_file");
    dims[0] = dim_lxo;
    dims[1] = dim_string;
    id_lxo_file = mpp_def_var(fid, "lXo_file", MPP_CHAR, 2, dims, 1,
                              "standard_name", "lndXocn_exchange_grid_file");
    mpp_end_def(fid);

    strcpy(otopog_file, "topog.nc");

    sprintf(amosaic_name, "%s_mosaic", atmos_name);
    sprintf(lmosaic_name, "%s_mosaic", land_name);
    sprintf(omosaic_name, "%s_mosaic", ocean_name);
    sprintf(amosaic_file, "%s.nc", amosaic_name);
    sprintf(lmosaic_file, "%s.nc", lmosaic_name);
    sprintf(omosaic_file, "%s.nc", omosaic_name);
...
int gs_make_vgrid(char *history, int nbnds, int *bnds, int n1, int n2, int *nz,
  ...
  /* check the command-line arguments to make sure the value are suitable */
  if (nbnds < 2)
    mpp_error("number of bounds specified through -nbnd should be an integer "
              "greater than 1");
  if (nbnds != n1)
    mpp_error("nbnds does not equal number entry specified through -bnd");
  if (nbnds - 1 != n2)
    mpp_error("nbnds-1 does not match number entry specified through -nz");
  ...
  /* write out vertical grid into a netcdf file */
  {
    ...
    varid = mpp_def_var(fid, "zeta", NC_DOUBLE, 1, &dim, 2, "standard_name",
                        "vertical_grid_vertex", "units", "meters");
    mpp_def_global_att(fid, "grid_version", GRID_VERSION);
    mpp_def_global_att(fid, "code_version", TAGNAME);
    mpp_def_global_att(fid, "history", history);
  ...
  if (mpp_pe() == mpp_root_pe())
    printf("Successfully generate vertical grid file %s\n", filename);
  return 0;
}
...
int gs_make_coupler_mosaic(char *history, char *amosaic, char *lmosaic,
  ...
  int *nxo = NULL, *nyo = NULL, *nxa = NULL, *nya = NULL, *nxl = NULL,
      *nyl = NULL;
  double **xocn = NULL, **yocn = NULL, **xatm = NULL, **yatm = NULL,
         **xlnd = NULL, **ylnd = NULL;
  ...
  /*mosaic_file can not have the same name as amosaic, lmosaic or omosaic, also
    the file name of amosaic, lmosaic, omosaic can not be "mosaic.nc"
  */
  ...
  if (!strcmp(mosaic_file, amosaic_file) ||
      !strcmp(mosaic_file, lmosaic_file) || !strcmp(mosaic_file, omosaic_file))
    mpp_error("make_coupler_mosaic: mosaic_file can not have the same name as "
              "amosaic, lmosaic or omosaic");
  if (!strcmp(amosaic_file, "mosaic.nc") ||
      !strcmp(lmosaic_file, "mosaic.nc") || !strcmp(omosaic_file, "mosaic.nc"))
    mpp_error("make_coupler_mosaic: the file name of amosaic, lmosaic or "
              "omosaic can not be mosaic.nc");
   ...
   */
  {
    ...
    for (n = 0; n < ntile_atm; n++) {
      ...
      if (nxa[n] % x_refine != 0)
        mpp_error("make_coupler_mosaic: atmos supergrid x-size can not be "
                  "divided by x_refine");
      if (nya[n] % y_refine != 0)
        mpp_error("make_coupler_mosaic: atmos supergrid y-size can not be "
                  "divided by y_refine");
      nxa[n] /= x_refine;
      nya[n] /= y_refine;
      ...
      tmpx = (double *)malloc((nxa[n] * x_refine + 1) *
                              (nya[n] * y_refine + 1) * sizeof(double));
      tmpy = (double *)malloc((nxa[n] * x_refine + 1) *
                              (nya[n] * y_refine + 1) * sizeof(double));
      vid = mpp_get_varid(g_fid, "x");
      ...
      for (j = 0; j < nya[n] + 1; j++)
        for (i = 0; i < nxa[n] + 1; i++) {
          xatm[n][j * (nxa[n] + 1) + i] =
              tmpx[(j * y_refine) * (nxa[n] * x_refine + 1) + i * x_refine];
          yatm[n][j * (nxa[n] + 1) + i] =
              tmpy[(j * y_refine) * (nxa[n] * x_refine + 1) + i * x_refine];
        }
      free(tmpx);
      free(tmpy);
      /*scale grid from degree to radian, because create_xgrid assume the grid
       * is in radians */
   ...
   */
  if (strcmp(lmosaic,
             amosaic)) { /* land mosaic is different from atmosphere mosaic */
    int n, m_fid, g_fid, vid, gid, tid;
    size_t start[4], nread[4];
    char dir[STRING], filename[STRING], file[2 * STRING];
    ...
    for (n = 0; n < ntile_lnd; n++) {
      ...
      if (nxl[n] % x_refine != 0)
        mpp_error("make_coupler_mosaic: land supergrid x-size can not be "
                  "divided by x_refine");
      if (nyl[n] % y_refine != 0)
        mpp_error("make_coupler_mosaic: land supergrid y-size can not be "
                  "divided by y_refine");
      nxl[n] /= x_refine;
      nyl[n] /= y_refine;
      ...
      tmpx = (double *)malloc((nxl[n] * x_refine + 1) *
                              (nyl[n] * y_refine + 1) * sizeof(double));
      tmpy = (double *)malloc((nxl[n] * x_refine + 1) *
                              (nyl[n] * y_refine + 1) * sizeof(double));
      vid = mpp_get_varid(g_fid, "x");
      mpp_get_var_value(g_fid, vid, tmpx);
      vid = mpp_get_varid(g_fid, "y");
      ...
      for (j = 0; j < nyl[n] + 1; j++)
        for (i = 0; i < nxl[n] + 1; i++) {
          xlnd[n][j * (nxl[n] + 1) + i] =
              tmpx[(j * y_refine) * (nxl[n] * x_refine + 1) + i * x_refine];
          ylnd[n][j * (nxl[n] + 1) + i] =
              tmpy[(j * y_refine) * (nxl[n] * x_refine + 1) + i * x_refine];
        }
      free(tmpx);
      free(tmpy);
      /*scale grid from degree to radian, because create_xgrid assume the grid
  ...
  /*
   * Read ocean grid boundaries and mask (where water is) for each tile within
   * the mosaic.
   */
  {
    ...
    for (n = 0; n < ntile_ocn; n++) {
      ...
      if (nxo[n] % x_refine != 0)
        mpp_error("make_coupler_mosaic: ocean supergrid x-size can not be "
                  "divided by x_refine");
      if (nyo[n] % y_refine != 0)
        mpp_error("make_coupler_mosaic: ocean supergrid y-size can not be "
                  "divided by y_refine");
      ...
      /* sometimes the ocean is only covered part of atmosphere, especially not
         cover the south pole region. In order to get all the exchange grid
         between atmosXland, we need to extend one point to cover the whole
         atmosphere. This need the assumption of one-tile ocean. Also we assume
         the latitude is the along j=0
      */
      if (ntile_ocn == 1) {
        int na;
        for (i = 1; i <= nxo[n]; i++)
          if (tmpy[i] != tmpy[i - 1])
            mpp_error("make_coupler_mosaic: latitude is not uniform along j=0");
        ...
        for (na = 0; na < ntile_atm; na++) {
          ...
          if (min_atm_lat > min_lat)
            min_atm_lat = min_lat;
        }
        if (tmpy[0] * D2R >
            min_atm_lat + TINY_VALUE) { /* extend one point in south direction*/
          ocn_south_ext = 1;
        }
      }
      nxo[n] /= x_refine;
      ...
      for (j = 0; j < nyo[n] + 1; j++)
        for (i = 0; i < nxo[n] + 1; i++) {
          xocn[n][(j + ocn_south_ext) * (nxo[n] + 1) + i] =
              tmpx[(j * y_refine) * (nxo[n] * x_refine + 1) + i * x_refine] *
              D2R;
          yocn[n][(j + ocn_south_ext) * (nxo[n] + 1) + i] =
              tmpy[(j * y_refine) * (nxo[n] * x_refine + 1) + i * x_refine] *
              D2R;
        }
      if (ocn_south_ext == 1) {
        for (i = 0; i < nxo[n] + 1; i++) {
    ...
    if (ntile_ocn != ntiles)
      mpp_error("make_coupler_mosaic: dimlen ntiles in mosaic file is not the "
                "same as dimlen in topog file");
    ...
    for (n = 0; n < ntile_ocn; n++) {
      ...
      if (nx != nxo[n] || ny + ocn_south_ext != nyo[n])
        mpp_error("make_coupler_mosaic: grid size mismatch between mosaic file "
                  "and topog file");
      if (ntiles == 1)
        strcpy(name, "depth");
      else
      ...
      for (i = 0; i < nxo[n] * nyo[n]; i++)
        omask[n][i] = 0;
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          if (depth[j * nx + i] > sea_level)
            omask[n][(j + ocn_south_ext) * nx + i] = 1;
        }
      free(depth);
    }
    mpp_close(t_fid);
  ...
  */
  if (strcmp(omosaic_name,
             amosaic_name)) { /* omosaic is different from amosaic */
    if (!strcmp(omosaic_name, lmosaic_name))
      mpp_error("make_coupler_mosaic: omosaic is the same as lmosaic, "
                "but different from amosaic.");
    same_mosaic = 0;
  } else { /* omosaic is same as amosaic */
    if (strcmp(omosaic_name, lmosaic_name))
      mpp_error("make_coupler_mosaic: omosaic is the same as amosaic, "
  ...
  ***************************************************************************************/
  ...
  nfile_lxo = 0;
  {
    ...
    int ***atmxlnd_ia, ***atmxlnd_ja, ***atmxlnd_il, ***atmxlnd_jl;
    int ***atmxocn_ia, ***atmxocn_ja, ***atmxocn_io, ***atmxocn_jo;
    double ***atmxlnd_area, ***atmxlnd_dia, ***atmxlnd_dja, ***atmxlnd_dil,
        ***atmxlnd_djl;
    double ***atmxocn_area, ***atmxocn_dia, ***atmxocn_dja, ***atmxocn_dio,
        ***atmxocn_djo;
      ...
      for (la = is; la <= ie; la++) {
          ...
          for (jl = 0; jl < nyl[nl]; jl++)
            for (il = 0; il < nxl[nl]; il++) {
              ...
              */
              ...
              if (xa_min >= xl_max || xa_max <= xl_min)
                continue;
              if ((n_out = clip_2dx2d(xa, ya, na_in, xl, yl, nl_in, x_out,
                                      y_out)) > 0) {
                xarea = poly_area(x_out, y_out, n_out);
                min_area =
                    min(area_lnd[nl][jl * nxl[nl] + il], area_atm[na][la]);
                if (xarea / min_area > AREA_RATIO_THRESH) {
                  ...
                  ++count;
                  if (count > MX)
                    mpp_error("make_coupler_mosaic: count is greater than MX, "
                              "increase MX");
                }
              }
            }
          ...
          for (jo = 0; jo < nyo[no]; jo++)
            for (io = 0; io < nxo[no]; io++) {
              ...
              if (omask[no][jo * nxo[no] + io] > 0.5) { /* over sea/ice */
                /* xo should in the same range as xa after lon_fix, so no need
                   to consider cyclic condition
                */
                if (xa_min >= xo_max || xa_max <= xo_min || yo_min >= ya_max ||
                    yo_max <= ya_min)
                  continue;

                if ((n_out = clip_2dx2d(xa, ya, na_in, xo, yo, no_in, x_out,
                                        y_out)) > 0) {
                  xarea = poly_area(x_out, y_out, n_out);
                  min_area =
                      min(area_ocn[no][jo * nxo[no] + io], area_atm[na][la]);
                  if (xarea / min_area > AREA_RATIO_THRESH) {
                    ...
                    if (interp_order == 2) {
                      atmxocn_clon[na][no][naxo[na][no]] =
                          poly_ctrlon(x_out, y_out, n_out, xa_avg);
                      atmxocn_clat[na][no][naxo[na][no]] =
                          poly_ctrlat(x_out, y_out, n_out);
                    }
                    ++(naxo[na][no]);
                    if (naxo[na][no] > MAXXGRID)
                      mpp_error(
                ...
                for (l = 0; l < count; l++) {
                  if (axl_xmin[l] >= xo_max || axl_xmax[l] <= xo_min ||
                      axl_ymin[l] >= ya_max || axl_ymax[l] <= ya_min)
                    continue;
                  if ((n_out = clip_2dx2d(atmxlnd_x[l], atmxlnd_y[l], num_v[l],
                                          xo, yo, no_in, x_out, y_out)) > 0) {
                    xarea = poly_area(x_out, y_out, n_out);
                    min_area = min(
                        area_lnd[axl_t[l]][axl_j[l] * nxl[axl_t[l]] + axl_i[l]],
                        area_atm[na][la]);
                    if (xarea / min_area > AREA_RATIO_THRESH) {
                      axl_area[l] += xarea;
                      if (interp_order == 2) {
        ...
        for (l = 0; l < count; l++) {
          ...
          if (axl_area[l] / min_area > AREA_RATIO_THRESH) {
            ...
            ++(naxl[na][nl]);
            if (naxl[na][nl] > MAXXGRID)
              mpp_error("naxl is greater than MAXXGRID, increase MAXXGRID");
          }
        }
      } /* end of la loop */
     ...
     */
    {
        ...
        for (na = 0; na < ntile_atm; na++) {
          for (nl = 0; nl < ntile_lnd; nl++) {
            ...
            if (nxgrid > 0) {
              ...
              mpp_gather_field_double(naxl[na][nl], atmxlnd_area[na][nl],
                                      g_area);
              for (i = 0; i < nxgrid; i++) {
          ...
          for (no = 0; no < ntile_ocn; no++) {
            ...
            if (nxgrid > 0) {
              ...
              mpp_gather_field_double(naxo[na][no], atmxocn_area[na][no],
                                      g_area);
              for (i = 0; i < nxgrid; i++) {
        ...
        for (na = 0; na < ntile_atm; na++) {
          ...
          for (nl = 0; nl < ntile_lnd; nl++) {
            ...
            if (nxgrid > 0) {
              ...
              mpp_gather_field_double(naxl[na][nl], atmxlnd_area[na][nl],
                                      g_area);
              mpp_gather_field_double(naxl[na][nl], atmxlnd_clon[na][nl],
                                      g_clon);
              mpp_gather_field_double(naxl[na][nl], atmxlnd_clat[na][nl],
                                      g_clat);
              for (i = 0; i < nxgrid; i++) {
          ...
          for (no = 0; no < ntile_ocn; no++) {
            ...
            if (nxgrid > 0) {
              ...
              mpp_gather_field_double(naxo[na][no], atmxocn_area[na][no],
                                      g_area);
              mpp_gather_field_double(naxo[na][no], atmxocn_clon[na][no],
                                      g_clon);
              mpp_gather_field_double(naxo[na][no], atmxocn_clat[na][no],
                                      g_clat);
              for (i = 0; i < nxgrid; i++) {
                ii = g_ja[i] * nxa[na] + g_ia[i];
                a_area[ii] += g_area[i];
          ...
          /* substract atmos centroid to get the centroid distance between atmos
           * grid and exchange grid. */
          for (nl = 0; nl < ntile_lnd; nl++) {
            for (i = 0; i < naxl[na][nl]; i++) {
              la = atmxlnd_ja[na][nl][i] * nxa[na] + atmxlnd_ia[na][nl][i];
              atmxlnd_dia[na][nl][i] =
                  atmxlnd_clon[na][nl][i] / atmxlnd_area[na][nl][i] -
                  a_clon[la];
              atmxlnd_dja[na][nl][i] =
                  atmxlnd_clat[na][nl][i] / atmxlnd_area[na][nl][i] -
                  a_clat[la];
            }
          }
          for (no = 0; no < ntile_ocn; no++) {
            for (i = 0; i < naxo[na][no]; i++) {
              ...
              atmxocn_dia[na][no][i] =
                  atmxocn_clon[na][no][i] / atmxocn_area[na][no][i] -
                  a_clon[la];
              atmxocn_dja[na][no][i] =
                  atmxocn_clat[na][no][i] / atmxocn_area[na][no][i] -
                  a_clat[la];
            }
          }
      ...
      /* also write out ocn_frac */
      {
        ...
        for (no = 0; no < ntile_ocn; no++) {
          ...
          for (jo = 0; jo < ny; jo++)
            for (io = 0; io < nxo[no]; io++) {
              ...
              if (fabs(omask[no][i] - ocn_frac) > TOLORENCE) {
                printf("at ocean point (%d,%d), omask = %f, ocn_frac = %f, "
                       "diff = %f\n",
          ...
          id_mask =
              mpp_def_var(fid, "mask", MPP_DOUBLE, 2, dims, 2, "standard_name",
    ...
    for (na = 0; na < ntile_atm; na++) {
      ...
      for (nl = 0; nl < ntile_lnd; nl++) {
        ...
        if (nxgrid > 0) {
          ...
          if (same_mosaic)
            sprintf(axl_file[nfile_axl], "atm_%s_%sXlnd_%s_%s.nc", amosaic_name,
                    atile_name[na], lmosaic_name, ltile_name[nl]);
          else
            sprintf(axl_file[nfile_axl], "%s_%sX%s_%s.nc", amosaic_name,
                    atile_name[na], lmosaic_name, ltile_name[nl]);
          sprintf(contact, "%s:%s::%s:%s", amosaic_name, atile_name[na],
                  lmosaic_name, ltile_name[nl]);
          ...
          if (interp_order == 2) {
            id_contact = mpp_def_var(
                fid, "contact", MPP_CHAR, 1, &dim_string, 7, "standard_name",
                "grid_contact_spec", "contact_type", "exchange", "parent1_cell",
                "tile1_cell", "parent2_cell", "tile2_cell", "xgrid_area_field",
                "xgrid_area", "distant_to_parent1_centroid", "tile1_distance",
                "distant_to_parent2_centroid", "tile2_distance");
          } else {
            id_contact = mpp_def_var(fid, "contact", MPP_CHAR, 1, &dim_string,
                                     5, "standard_name", "grid_contact_spec",
                                     "contact_type", "exchange", "parent1_cell",
                                     "tile1_cell", "parent2_cell", "tile2_cell",
                                     "xgrid_area_field", "xgrid_area");
          }

          dims[0] = dim_ncells;
          dims[1] = dim_two;
          id_tile1_cell =
              mpp_def_var(fid, "tile1_cell", MPP_INT, 2, dims, 1,
                          "standard_name", "parent_cell_indices_in_mosaic1");
          id_tile2_cell =
              mpp_def_var(fid, "tile2_cell", MPP_INT, 2, dims, 1,
                          "standard_name", "parent_cell_indices_in_mosaic2");
          id_xgrid_area =
              mpp_def_var(fid, "xgrid_area", MPP_DOUBLE, 1, &dim_ncells, 2,
                          "standard_name", "exchange_grid_area", "units", "m2");
          if (interp_order == 2) {
            id_tile1_dist = mpp_def_var(fid, "tile1_distance", MPP_DOUBLE, 2,
                                        dims, 1, "standard_name",
                                        "distance_from_parent1_cell_centroid");
            id_tile2_dist = mpp_def_var(fid, "tile2_distance", MPP_DOUBLE, 2,
                                        dims, 1, "standard_name",
          ...
          if (interp_order == 2) {
            start[1] = 0;
            mpp_gather_field_double(naxl[na][nl], atmxlnd_dia[na][nl],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
            mpp_gather_field_double(naxl[na][nl], atmxlnd_dil[na][nl],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile2_dist, start, nwrite,
                                    gdata_dbl);
            start[1] = 1;
            mpp_gather_field_double(naxl[na][nl], atmxlnd_dja[na][nl],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
            mpp_gather_field_double(naxl[na][nl], atmxlnd_djl[na][nl],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile2_dist, start, nwrite,
                                    gdata_dbl);
          }
      ...
      for (no = 0; no < ntile_ocn; no++) {
        ...
        if (nxgrid > 0) {
          ...
          if (same_mosaic)
            sprintf(axo_file[nfile_axo], "atm_%s_%sXocn_%s_%s.nc", amosaic_name,
                    atile_name[na], omosaic_name, otile_name[no]);
          else
            sprintf(axo_file[nfile_axo], "%s_%sX%s_%s.nc", amosaic_name,
                    atile_name[na], omosaic_name, otile_name[no]);

          sprintf(contact, "%s:%s::%s:%s", amosaic_name, atile_name[na],
                  omosaic_name, otile_name[no]);
          ...
          for (i = 0; i < naxo[na][no]; i++) {
            ...
            ++(atmxocn_io[na][no][i]);
            atmxocn_jo[na][no][i] +=
                1 - ocn_south_ext; /* possible one artificial j-level is added
                                      at south end */
          }
          ...
          if (interp_order == 2) {
            start[1] = 0;
            mpp_gather_field_double(naxo[na][no], atmxocn_dia[na][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
            mpp_gather_field_double(naxo[na][no], atmxocn_dio[na][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile2_dist, start, nwrite,
                                    gdata_dbl);
            start[1] = 1;
            mpp_gather_field_double(naxo[na][no], atmxocn_dja[na][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
            mpp_gather_field_double(naxo[na][no], atmxocn_djo[na][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile2_dist, start, nwrite,
                                    gdata_dbl);
          }
  ...
  if (mpp_pe() == mpp_root_pe())
    printf("\nNOTE from make_coupler_mosaic: Complete the process to create "
           "exchange grids "
  ...
  ***************************************************************************************/
  ...
  if (!lnd_same_as_atm) {
          ...
          for (jo = 0; jo < nyo[no]; jo++)
            for (io = 0; io < nxo[no]; io++)
              if (omask[no][jo * nxo[no] + io] > 0.5) {
                ...
                yo_min = minval_double(4, yo);
                yo_max = maxval_double(4, yo);
                if (yo_min >= yl_max || yo_max <= yl_min)
                  continue;
                ...
                /* xo should in the same range as xa after lon_fix, so no need
                   to consider cyclic condition
                */
                if (xl_min >= xo_max || xl_max <= xo_min)
                  continue;
                if ((n_out = clip_2dx2d(xl, yl, nl_in, xo, yo, no_in, x_out,
                                        y_out)) > 0) {
                  xarea = poly_area(x_out, y_out, n_out);
                  min_area =
                      min(area_ocn[no][jo * nxo[no] + io], area_lnd[nl][ll]);
                  if (xarea / min_area > AREA_RATIO_THRESH) {
                    ...
                    if (interp_order == 2) {
                      lndxocn_clon[nl][no][nlxo[nl][no]] =
                          poly_ctrlon(x_out, y_out, n_out, xl_avg);
                      lndxocn_clat[nl][no][nlxo[nl][no]] =
                          poly_ctrlat(x_out, y_out, n_out);
                    }
                    ++(nlxo[nl][no]);
                    if (nlxo[nl][no] > MAXXGRID)
                      mpp_error(
    ...
    if (interp_order == 2) {
      ...
      for (nl = 0; nl < ntile_lnd; nl++) {
        ...
        /* substract land centroid to get the centroid distance between land
         * grid and exchange grid. */
        for (no = 0; no < ntile_ocn; no++) {
          for (i = 0; i < nlxo[nl][no]; i++) {
            ll = lndxocn_jl[nl][no][i] * nxl[nl] + lndxocn_il[nl][no][i];
            lndxocn_dil[nl][no][i] =
                lndxocn_clon[nl][no][i] / lndxocn_area[nl][no][i] - l_clon[ll];
            lndxocn_djl[nl][no][i] =
                lndxocn_clat[nl][no][i] / lndxocn_area[nl][no][i] - l_clat[ll];
          }
        }
      ...
      for (no = 0; no < ntile_ocn; no++) {
        ...
        for (nl = 0; nl < ntile_lnd; nl++) {
          for (i = 0; i < nlxo[nl][no]; i++) {
            ...
            lndxocn_dio[nl][no][i] =
                lndxocn_clon[nl][no][i] / lndxocn_area[nl][no][i] -
                o_clon[no][lo];
            lndxocn_djo[nl][no][i] =
                lndxocn_clat[nl][no][i] / lndxocn_area[nl][no][i] -
                o_clat[no][lo];
          }
        }
        free(o_area[no]);
    ...
    for (nl = 0; nl < ntile_lnd; nl++) {
      for (no = 0; no < ntile_ocn; no++) {
        ...
        if (nxgrid > 0) {
          ...
          if (same_mosaic)
            sprintf(lxo_file[nfile_lxo], "lnd_%s_%sXocn_%s_%s.nc", lmosaic_name,
                    ltile_name[nl], omosaic_name, otile_name[no]);
          else
            sprintf(lxo_file[nfile_lxo], "%s_%sX%s_%s.nc", lmosaic_name,
                    ltile_name[nl], omosaic_name, otile_name[no]);
          sprintf(contact, "%s:%s::%s:%s", lmosaic_name, ltile_name[nl],
                  omosaic_name, otile_name[no]);
          ...
          dims[0] = dim_ncells;
          dims[1] = dim_two;
          id_tile1_cell = mpp_def_var(fid, "tile1_cell", MPP_INT, 2, dims, 1,
                                      "standard_name", "parent1_cell_indices");
          id_tile2_cell = mpp_def_var(fid, "tile2_cell", MPP_INT, 2, dims, 1,
                                      "standard_name", "parent2_cell_indices");
          id_xgrid_area =
              mpp_def_var(fid, "xgrid_area", MPP_DOUBLE, 1, &dim_ncells, 2,
                          "standard_name", "exchange_grid_area", "units", "m2");
          ...
          for (i = 0; i < nlxo[nl][no]; i++) {
            ...
            ++(lndxocn_io[nl][no][i]);
            lndxocn_jo[nl][no][i] +=
                1 - ocn_south_ext; /* one artificial j-level may be added in the
                                      south end */
          }
          ...
          mpp_gather_field_double(nlxo[nl][no], lndxocn_area[nl][no],
                                  gdata_dbl);
          mpp_put_var_value(fid, id_xgrid_area, gdata_dbl);
          ...
          if (interp_order == 2) {
            start[1] = 0;
            mpp_gather_field_double(nlxo[nl][no], lndxocn_dil[nl][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
            mpp_gather_field_double(nlxo[nl][no], lndxocn_dio[nl][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile2_dist, start, nwrite,
                                    gdata_dbl);
            start[1] = 1;
            mpp_gather_field_double(nlxo[nl][no], lndxocn_djl[nl][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
            mpp_gather_field_double(nlxo[nl][no], lndxocn_djo[nl][no],
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile2_dist, start, nwrite,
                                    gdata_dbl);
          }
    ...
    nfile_lxo = nfile_axo;
    for (i = 0; i < nfile_axo; i++)
      strcpy(lxo_file[i], axo_file[i]);
    if (mpp_pe() == mpp_root_pe())
      printf(
          "\nNOTE from make_coupler_mosaic: Since lmosaic is the same as "
          "amosaic, "
  ...
  if (mpp_pe() == mpp_root_pe()) {
    ...
    if (check) {
      /* for cubic grid, when number of model points is odd, some grid cell area
         will be negative, need to think about how to solve this issue in the
         future */
