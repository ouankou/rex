...
static char *vert_std_name[] = {
    STD_ATM_LN,        STD_SIGMA,         STD_HYBRID_SIGMA,
    STD_HYBRID_HEIGHT, STD_SLEVE,         STD_OCEAN_SIGMA,
    STD_OCEAN_S,       STD_OCEAN_SIGMA_Z, STD_OCEAN_DBL_SIGMA};
...
static int def_coord_var(int ncid, const char *name, size_t len, nc_type xtype,
  ...
  if (units)
    if ((ret = nc_put_att_text(ncid, vid, CF_UNITS, strlen(units) + 1, units)))
      return ret;
   ...
   * Y, Z or T.*/
  if (axis) {
    if ((ret = nc_put_att_text(ncid, vid, CF_AXIS, strlen(axis) + 1, axis)))
      return ret;
    ...
    if (!strncmp(axis, CF_LEVEL_AXIS, strlen(CF_LEVEL_AXIS))) {
      if (positive_up) {
        if ((ret = nc_put_att_text(ncid, vid, CF_POSITIVE, strlen(CF_UP) + 1,
                                   CF_UP)))
          return ret;
      } else {
        if ((ret = nc_put_att_text(ncid, vid, CF_POSITIVE, strlen(CF_DOWN) + 1,
                                   CF_DOWN)))
          return ret;
      }
    }
...
static int inq_coord_var(int ncid, int nvalid,
                         const char valid_units[][CF_MAX_LEN + 1],
                         const char *valid_standard_name, char *name,
                         size_t *lenp, nc_type *xtypep, size_t *ft_lenp,
                         char *formula_terms, int *positive_upp, int *dimidp,
                         int *varidp) {
  ...
  for (varid = 0; varid < nvars; varid++) {
     ...
     * var, move on. */
    if ((ret = nc_inq_att(ncid, varid, target, &att_type_in, &att_len_in)) ==
        NC_ENOTATT)
      continue;
  ...
  if (ft_lenp || formula_terms) {
    ret = nc_inq_att(ncid, varid, CF_FORMULA_TERMS, &att_type_in, &att_len_in);
    if (ret == NC_ENOTATT) {
      if (ft_lenp)
        *ft_lenp = 0;
 ...
 * recomended attribute accessories.*/
int nccf_def_latitude(int ncid, size_t len, nc_type xtype, int *lat_dimidp,
                      int *lat_varidp) {
  return def_coord_var(ncid, CF_LATITUDE_NAME, len, xtype, CF_LATITUDE_UNITS,
                       CF_LATITUDE_AXIS, 0, CF_LATITUDE_NAME, NULL,
...
int nccf_inq_latitude(int ncid, size_t *lenp, nc_type *xtypep, int *lat_dimidp,
  ...
  const char val[NLAT_UNITS][CF_MAX_LEN + 1] = {"degrees_north", "degree_north",
                                                "degree_N",      "degrees_N",
                                                "degreeN",       "degreesN"};
  return inq_coord_var(ncid, NLAT_UNITS, val, NULL, NULL, lenp, xtypep, NULL,
                       NULL, NULL, lat_dimidp, lat_varidp);
}

/* Define a longitude dimension and variable with all the CF
 * recomended attribute accessories.*/
int nccf_def_longitude(int ncid, size_t len, nc_type xtype, int *lon_dimidp,
                       int *lon_varidp) {
  return def_coord_var(ncid, CF_LONGITUDE_NAME, len, xtype, CF_LONGITUDE_UNITS,
                       CF_LONGITUDE_AXIS, 0, CF_LONGITUDE_NAME, NULL,
...
int nccf_inq_longitude(int ncid, size_t *lenp, nc_type *xtypep, int *lon_dimidp,
                       int *lon_varidp) {
  const char val[NLON_UNITS][CF_MAX_LEN + 1] = {"degrees_east", "degree_east",
                                                "degree_E",     "degrees_E",
                                                "degreeE",      "degreesE"};

  return inq_coord_var(ncid, NLON_UNITS, val, NULL, NULL, lenp, xtypep, NULL,
                       NULL, NULL, lon_dimidp, lon_varidp);
}
...
int nccf_def_lvl(int ncid, const char *name, size_t len, nc_type xtype,
                 ...
                 const char *formula_terms, int cdm_axis_type, int *lvl_dimidp,
                 int *lvl_varidp) {
  return def_coord_var(ncid, name, len, xtype, units, CF_LEVEL_AXIS,
                       positive_up, standard_name, formula_terms, cdm_axis_type,
...
int nccf_inq_lvl(int ncid, char *name, size_t *lenp, nc_type *xtypep,
  ...
  const char val[NLEVEL_UNITS][CF_MAX_LEN + 1] = {"bar",
                                                  "standard_atmosphere",
...
int nccf_inq_time(int ncid, char *name, size_t *lenp, nc_type *xtypep,
                  int *time_dimidp, int *time_varidp) {
  const char val[NTIME_UNITS][CF_MAX_LEN + 1] = {"second",
                                                 "day",
                                                 "hour",
                                                 "minute",
                                                 "s",
...
static int def_vert_var(int ncid, const char *name, nc_type xtype, size_t len,
  ...
  for (v = 0; v < nterms; v++) {
    ...
    else if ((ret = nc_inq_varname(ncid, term_id[v], term_name[v])))
      return ret;
  }
  ...
  case 2:
    ...
    break;
  case 3:
    sprintf(formula_terms, ft_format, term_name[0], term_name[1], term_name[2]);
    break;
  case 4:
    sprintf(formula_terms, ft_format, term_name[0], term_name[1], term_name[2],
            term_name[3]);
  ...
  case 5:
    sprintf(formula_terms, ft_format, term_name[0], term_name[1], term_name[2],
            term_name[3], term_name[4]);
    break;
  case 6:
    sprintf(formula_terms, ft_format, term_name[0], term_name[1], term_name[2],
            term_name[3], term_name[4], term_name[5]);
    break;
  case 7:
    sprintf(formula_terms, ft_format, term_name[0], term_name[1], term_name[2],
            term_name[3], term_name[4], term_name[5], term_name[6]);
    break;
  default:
    return CF_EINVAL;
  }
  ...
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_SIGMA, formula_terms, NCCF_GEOZ, lvl_dimidp,
                       lvl_varidp);
}

static int nccf_set_ft(int ncid, int varid, int nterms, int *ft_varids,
...
static int inq_vert_var(int ncid, const char *standard_name, int nterms,
  ...
  if ((ret =
           inq_coord_var(ncid, 0, NULL, standard_name, name, lenp, xtypep,
                         &ft_len, formula_terms, NULL, lvl_dimidp, lvl_varidp)))
    return ret;
  ...
  case 1:
    ...
    break;
  case 2:
    if (sscanf(formula_terms, ft_format, term_name[0], term_name[1]) != nterms)
      return CF_EBADFT;
    break;
  case 3:
...
*/
int nccf_def_lvl_atm_ln(int ncid, const char *name, nc_type xtype, size_t len,
                        int *lvl_dimidp, int *lvl_varidp) {
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_ATM_LN, NULL, NCCF_GEOZ, lvl_dimidp, lvl_varidp);
}
...
/* Atmosphere natural log pressure coordinate. */
int nccf_inq_lvl_atm_ln(int ncid, char *name, nc_type *xtypep, size_t *lenp,
...
*/
int nccf_def_lvl_sigma(int ncid, const char *name, nc_type xtype, size_t len,
                       int *lvl_dimidp, int *lvl_varidp) {
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_SIGMA, NULL, NCCF_GEOZ, lvl_dimidp, lvl_varidp);
}
...
int nccf_def_lvl_hybrid_sigma(int ncid, const char *name, nc_type xtype,
   ...
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_HYBRID_SIGMA, NULL, NCCF_GEOZ, lvl_dimidp,
                       lvl_varidp);
}

/* Set the formula_terms atribute to contain all this information. */
int nccf_def_ft_hybrid_sigma(int ncid, int lvl_vid, int a_vid, int b_vid,
                             int ps_vid, int p0_vid) {
  int ft_varids[FT_MAX_TERMS];
...
/* { */
/*    return nccf_def_coord_system(ncid, CF_VERT_HYBRID_SIGMA, names, lens,
 * types, time_units,  */
/* 				FT_HYBRID_SIGMA_TERMS, term_names, term_types,
 * term_ndims, */
/* 				term_dimids, did, vids, term_vids); */
/* } */

/* int */
/* nccf_def_coord_system(int ncid, int vert_axis_type, char
 * name[CF_4D][NC_MAX_NAME+1],  */
...
/*    for (t = 0; t < nterms; t++) */
/*    { */
/*       if ((ret = nc_def_var(ncid, term_names[t], term_types[t], 1, did,
 * &a_vid))) */
/* 	 return ret; */
/*    } */
...
/* #define PS_NDIMS 3 */
/* int  */
/* nccf_def_lvl_hybrid_sigma_full(int ncid, const char *name, nc_type xtype,
 * size_t len,  */
/* 			       const char *a_name, nc_type a_type, int *a_varid,
 * const char *a_units, */
/* 			       const char *b_name, nc_type b_type, int *b_varid,
 * const char *b_units, */
/* 			       const char *ps_name, nc_type ps_type, int
 * *ps_varid, const char *ps_units, */
/* 			       const char *p0_name, nc_type p0_type, int
 * *p0_varid, const char *p0_units, */
/* 			       int *lvl_dimidp, int *lvl_varidp) */
/* { */
/*    int termids[FT_HYBRID_SIGMA_TERMS] = {a_varid, b_varid, ps_varid,
 * p0_varid}; */
/*    int ret; */
...
int nccf_inq_lvl_hybrid_sigma(int ncid, char *name, nc_type *xtypep,
  ...
  if ((ret = inq_vert_var(ncid, STD_HYBRID_SIGMA, FT_HYBRID_SIGMA_TERMS,
                          FT_HYBRID_SIGMA_FORMAT, termids, name, xtypep, lenp,
                          lvl_dimidp, lvl_varidp)))
    return ret;
...
/* Atmosphere hybrid height coordinate
   ...
   is needed.
*/
int nccf_def_lvl_hybrid_height(int ncid, const char *name, nc_type xtype,
                               size_t len, int *lvl_dimidp, int *lvl_varidp) {
  /* Create the coordinate variable and dimension with no
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_HYBRID_HEIGHT, NULL, NCCF_GEOZ, lvl_dimidp,
                       lvl_varidp);
}
...
int nccf_inq_lvl_hybrid_height(int ncid, char *name, nc_type *xtypep,
                               size_t *lenp, int *a_varidp, int *b_varidp,
                               int *orog_varidp, int *lvl_dimidp,
                               int *lvl_varidp) {
  ...
  if ((ret = inq_vert_var(ncid, STD_HYBRID_HEIGHT, FT_HYBRID_HEIGHT_TERMS,
                          FT_HYBRID_HEIGHT_FORMAT, termids, name, xtypep, lenp,
                          lvl_dimidp, lvl_varidp)))
    return ret;
...
int nccf_def_lvl_sleve(int ncid, const char *name, nc_type xtype, size_t len,
   ...
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_SLEVE, NULL, NCCF_GEOZ, lvl_dimidp, lvl_varidp);
}
...
int nccf_inq_lvl_sleve(int ncid, char *name, nc_type *xtypep, size_t *lenp,
                       int *a_varidp, int *b1_varidp, int *b2_varidp,
                       int *ztop_varidp, int *zsurf1_varidp, int *zsurf2_varidp,
                       int *lvl_dimidp, int *lvl_varidp) {
...
*/
int nccf_def_lvl_ocean_sigma(int ncid, const char *name, nc_type xtype,
                             size_t len, int *lvl_dimidp, int *lvl_varidp) {
  /* Create the coordinate variable and dimension with no
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_OCEAN_SIGMA, NULL, NCCF_GEOZ, lvl_dimidp,
                       lvl_varidp);
}

int nccf_def_ft_ocean_sigma(int ncid, int varid, int eta_varid,
                            int depth_varid) {
...
int nccf_inq_lvl_ocean_sigma(int ncid, char *name, nc_type *xtypep,
                             size_t *lenp, int *eta_varidp, int *depth_varidp,
                             int *lvl_dimidp, int *lvl_varidp) {
  int termids[FT_OCEAN_SIGMA_TERMS];
  int ret;

  /* Get most of the info from inq_vert_var. */
  if ((ret = inq_vert_var(ncid, STD_OCEAN_SIGMA, FT_OCEAN_SIGMA_TERMS,
                          FT_OCEAN_SIGMA_FORMAT, termids, name, xtypep, lenp,
                          lvl_dimidp, lvl_varidp)))
    return ret;
...
int nccf_def_lvl_ocean_s(int ncid, const char *name, nc_type xtype, size_t len,
   ...
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_OCEAN_S, NULL, NCCF_GEOZ, lvl_dimidp, lvl_varidp);
}
...
int nccf_inq_lvl_ocean_s(int ncid, char *name, nc_type *xtypep, size_t *lenp,
                         int *eta_varidp, int *depth_varidp, int *a_varidp,
                         int *b_varidp, int *depth_c_varidp, int *lvl_dimidp,
                         int *lvl_varidp) {
  ...
  /* Get most of the info from inq_vert_var. */
  if ((ret =
           inq_vert_var(ncid, STD_OCEAN_S, FT_OCEAN_S_TERMS, FT_OCEAN_S_FORMAT,
                        termids, name, xtypep, lenp, lvl_dimidp, lvl_varidp)))
    return ret;
...
int nccf_def_lvl_ocean_sigma_z(int ncid, const char *name, nc_type xtype,
   ...
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_OCEAN_SIGMA_Z, NULL, NCCF_GEOZ, lvl_dimidp,
                       lvl_varidp);
}

int nccf_def_ft_ocean_sigma_z(int ncid, int varid, int eta_varid,
                              int depth_varid, int depth_c_varid,
                              int nsigma_varid, int zlev_varid) {
...
int nccf_inq_lvl_ocean_sigma_z(int ncid, char *name, nc_type *xtypep,
                               size_t *lenp, int *eta_varidp, int *depth_varidp,
                               int *depth_c_varidp, int *nsigma_varidp,
                               int *zlev_varidp, int *lvl_dimidp,
                               int *lvl_varidp) {
  ...
  if ((ret = inq_vert_var(ncid, STD_OCEAN_SIGMA_Z, FT_OCEAN_SIGMA_Z_TERMS,
                          FT_OCEAN_SIGMA_Z_FORMAT, termids, name, xtypep, lenp,
                          lvl_dimidp, lvl_varidp)))
    return ret;
...
int nccf_def_lvl_ocean_dbl_sigma(int ncid, const char *name, nc_type xtype,
                                 size_t len, int *lvl_dimidp, int *lvl_varidp) {
  /* Create the coordinate variable and dimension with no
   * formula_terms. */
  return def_coord_var(ncid, name, len, xtype, NULL, CF_LEVEL_AXIS, 0,
                       STD_OCEAN_DBL_SIGMA, NULL, NCCF_GEOZ, lvl_dimidp,
                       lvl_varidp);
}

int nccf_def_ft_ocean_dbl_sigma(int ncid, int varid, int depth_varid,
                                int z1_varid, int z2_varid, int a_varid,
                                int href_varid, int k_c_varid) {
...
int nccf_inq_lvl_ocean_dbl_sigma(int ncid, char *name, nc_type *xtypep,
                                 size_t *lenp, int *depth_varidp,
                                 int *z1_varidp, int *z2_varidp, int *a_varidp,
                                 int *href_varidp, int *k_c_varidp,
                                 int *lvl_dimidp, int *lvl_varidp) {
