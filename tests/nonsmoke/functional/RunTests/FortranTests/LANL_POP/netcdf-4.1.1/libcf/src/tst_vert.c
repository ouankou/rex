/* This is part of the libcf package. Copyright 2006 University
   ...
   $Id: tst_vert.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
int main(int argc, char **argv) {
  ...
  printf("*** creating file %s...", file_name);
  {
    ...
    if (nccf_def_file(ncid, TITLE, HISTORY))
      ERR;
    if (nccf_def_latitude(ncid, NLAT, NC_FLOAT, &lat_did, &lat_vid))
      ERR;
    if (nccf_def_longitude(ncid, NLON, NC_FLOAT, &lon_did, &lon_vid))
      ERR;
    ...
    /* Check the file. */
    if (nccf_inq_file(ncid, &title_len, title_in, &history_len, history_in))
      ERR;
    if (title_len != strlen(TITLE) + 1 ||
        strncmp(title_in, TITLE, strlen(TITLE)))
      ERR;
    if (nccf_inq_lvl_vert(ncid, name_in, &xtype_in, &len_in, &lvl_type_in,
                          &dimid_in, &varid_in))
      ERR;
    if (!strcmp(name_in, "sigma") || xtype_in != NC_FLOAT || len_in != NLVL ||
        lvl_type_in != CF_VERT_SIGMA || dimid_in != lvl_did ||
        varid_in != lvl_vid)
      ERR;
    if (nccf_inq_latitude(ncid, &len_in, &xtype_in, &dimid_in, &varid_in))
      ERR;
    if (len_in != NLAT || xtype_in != NC_FLOAT || dimid_in != lat_did ||
        varid_in != lat_vid)
      ERR;
    if (nccf_inq_longitude(ncid, &len_in, &xtype_in, &dimid_in, &varid_in))
      ERR;
    if (len_in != NLON || xtype_in != NC_FLOAT || dimid_in != lon_did ||
        varid_in != lon_vid)
      ERR;
    if (nccf_inq_lvl_sigma(ncid, name_in, &xtype_in, &len_in, &ps_varid_in,
                           &ptop_varid_in, &dimid_in, &varid_in))
      ERR;
    if (len_in != NLVL || xtype_in != NC_FLOAT || dimid_in != lvl_did ||
        varid_in != lvl_vid || ps_varid_in != ps_vid ||
        ptop_varid_in != ptop_vid)
      ERR;
    if (nccf_inq_time(ncid, name_in, &len_in, &xtype_in, &dimid_in, &varid_in))
      ERR;
    if (len_in != NC_UNLIMITED || xtype_in != NC_FLOAT ||
        dimid_in != time_did || varid_in != time_vid)
      ERR;
  ...
  /*       if (nccf_inq_lvl_hybrid_sigma(ncid, name_in, &xtype_in, &len_in,
   * &a_varid_in, &b_varid_in, &ps_varid_in, */
  ...
  /*       if (len_in != NC_UNLIMITED || xtype_in != NC_FLOAT || dimid_in !=
   * time_did || */
