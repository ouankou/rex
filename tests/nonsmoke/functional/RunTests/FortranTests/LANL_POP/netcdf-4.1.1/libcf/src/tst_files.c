/* This is part of the netCDF package. Copyright 2005 University
   ...
   $Id: tst_files.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
int main(int argc, char **argv) {
  ...
  printf("*** testing file convention...");
  {
    ...
    if (nccf_inq_convention(ncid, &cf_convention))
      ERR;
    if (cf_convention)
      ERR;

    if (nccf_def_convention(ncid))
      ERR;
...
#define HISTORY                                                                \
  "This file was generated by a grant from the Obscurity Foundation"
  ...
  printf("*** testing file annotation...");
  {
    ...
    if (!cf_convention)
      ERR;
    if (nccf_inq_file(ncid, &tlen, NULL, &hlen, NULL))
      ERR;
    if (tlen != strlen(TITLE) + 1 || hlen != strlen(HISTORY) + 20)
      ERR;
    if (!(history_in = malloc(hlen)))
      ERR;
    if (!(title_in = malloc(tlen)))
      ERR;
    if (nccf_inq_file(ncid, NULL, title_in, NULL, history_in))
      ERR;
    if (strncmp(title_in, TITLE, tlen) ||
        strncmp(history_in + 18, HISTORY, strlen(HISTORY)))
      ERR;
...
#define SOURCE                                                                 \
  "These data were generated from the feverish imagination of a diseased "     \
  "mind."
...
#define MAX_LEN 200
  ...
  printf("*** testing file notes...");
  {
    ...
    if (nc_create(FILE_NAME, 0, &ncid))
      ERR;
    if (nccf_def_notes(ncid, NC_GLOBAL, INSTITUTION, SOURCE, COMMENT,
                       REFERENCES))
      ERR;

    /* Check the file. */
    if (nccf_inq_notes(ncid, NC_GLOBAL, &ilen, inst_in, &slen, source_in, &clen,
