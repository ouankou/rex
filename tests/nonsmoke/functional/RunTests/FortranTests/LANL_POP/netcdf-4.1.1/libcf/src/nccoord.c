...
$Id: nccoord.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
int nccf_inq_axis_type(int ncid, int varid, int *axis_type) {
  ...
  if ((ret = nc_get_att_text(ncid, varid, COORDINATE_AXIS_TYPE, value)))
    return ret;
  ...
  if (i == NCCF_HEIGHT_UP) {
    if ((ret = nc_get_att_text(ncid, varid, COORDINATE_Z_IS_POSITIVE, value)))
      return ret;
...
EXTERNL int nccf_inq_coord_system(int ncid, int system_varid, char *name,
  ...
  if ((ret = nccf_parse_coords(ncid, system_varid, COORDINATE_AXES, naxes,
                               axis_varids)))
    return ret;
...
int nccf_assign_coord_system(int ncid, int varid, int system_varid) {
  ...
  systems_str[0] = 0;
  if (!ret) {
    if ((ret = nc_get_att_text(ncid, varid, COORDINATE_SYSTEMS, systems_str)))
      return ret;
    strcat(systems_str, " ");
  }
...
EXTERNL int nccf_inq_transform(int ncid, int transform_varid, char *name,
                               size_t *type_len, char *transform_type,
                               size_t *name_len, char *transform_name) {
  int ret;
  ...
  if (type_len)
    if ((ret = nc_inq_attlen(ncid, transform_varid, TRANSFORM_TYPE, type_len)))
      return ret;

  /* If the user wants the transform type string, get it. */
  if (transform_type)
    if ((ret = nc_get_att_text(ncid, transform_varid, TRANSFORM_TYPE,
                               transform_type)))
      return ret;

  /* If the user wants the length of the transform_name, find it. */
  if (type_len)
    if ((ret = nc_inq_attlen(ncid, transform_varid, TRANSFORM_NAME, name_len)))
      return ret;

  /* If the user wants the transform name string, get it. */
  if (transform_name)
    if ((ret = nc_get_att_text(ncid, transform_varid, TRANSFORM_NAME,
                               transform_name)))
      return ret;
