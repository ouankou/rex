    ...
    if (string[i] != ' ' && string[i] != '\t') {
      ...
      if (found) {
        ...
        *(pstring + nvar * varlen + j++) = string[i];
        if (j >= varlen)
          error_handler("error from tokenize: variable name length exceeds "
                        "limit during tokenization");
      }
    }
  }
  *(pstring + nvar * varlen + j) = 0;
...
******************************************************************************/
void latlon2xyz(int size, const double *lon, const double *lat, double *x,
                double *y, double *z) {
  int n;
...
/*------------------------------------------------------------
   ...
   ----------------------------------------------------------*/
void xyz2latlon(int np, const double *x, const double *y, const double *z,
...
double poly_area(const double x[], const double y[], int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (fabs(lat1 - lat2) <
        SMALL_VALUE) /* cheap area calculation along latitude */
...
int insert_vtx(double x[], double y[], int n, int n_ins, double lon_in,
               double lat_in) {
  int i;
...
double great_circle_distance(double *p1, double *p2) {
  ...
  dist = RADIUS*ACOS(SIN(p1[1])*SIN(p2[1]) +
  COS(p1[1])*COS(p2[1])*COS(p1[0]-p2[0]));
  */
  beta =
      2. *
      asin(sqrt(sin((p1[1] - p2[1]) / 2.) * sin((p1[1] - p2[1]) / 2.) +
                cos(p1[1]) * cos(p2[1]) *
                    (sin((p1[0] - p2[0]) / 2.) * sin((p1[0] - p2[0]) / 2.))));
  dist = RADIUS * beta;
  return dist;
...
double spherical_excess_area(const double *p_ll, const double *p_ul,
                             const double *p_lr, const double *p_ur,
                             double radius) {
...
/*------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------*/
void unit_vect_latlon(int size, const double *lon, const double *lat,
