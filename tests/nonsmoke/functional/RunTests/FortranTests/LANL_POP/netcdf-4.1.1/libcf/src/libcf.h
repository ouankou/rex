...
$Id: libcf.h,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
#define CF_EEXISTS                                                             \
  -404 /* Attempting to define var, dim, or att that already exists. */
...
#define FT_OCEAN_SIGMA_Z_FORMAT                                                \
  "sigma: %s eta: %s depth: %s depth_c: %s nsigma: %s zlev: %s"
#define FT_OCEAN_SIGMA_Z_TERMS 6

#define CF_VERT_OCEAN_DBL_SIGMA 8
#define STD_OCEAN_DBL_SIGMA "ocean_double_sigma_coordinate"
#define FT_OCEAN_DBL_SIGMA_FORMAT                                              \
  "sigma: %s depth: %s z1: %s z2: %s a: %s href: %s k_c: %s"
...
int nccf_inq_notes(int ncid, int varid, size_t *institution_lenp,
                   ...
                   size_t *comment_lenp, char *comment, size_t *references_lenp,
                   char *references);
 ...
 * system_varid. */
int nccf_def_coord_system(int ncid, const char *name, int naxes,
                          int *axis_varids, int *system_varid);

/* Find out about a coordinate system, it's name, number of axes, and
 * the varid of each axis coordinate var. */
int nccf_inq_coord_system(int ncid, int system_varid, char *name, int *naxes,
                          int *axis_varids);
...
int nccf_inq_transform(int ncid, int transform_varid, char *name,
                       size_t *type_len, char *transform_type, size_t *name_len,
                       char *transform_name);
...
int nccf_get_vara(int ncid, int varid, float *lat_range, int *nlat,
                  float *lon_range, int *nlon, int lvl_index, int timestep,
                  void *data);
...
int cdAbs2Comp(char *absunits, void *abstime, cdType abstimetype,
               cdCompTime *comptime, double *frac);
...
void cdChar2Rel(cdCalenType timetype, char *chartime, char *relunits,
                double *reltime);
int cdComp2Abs(cdCompTime comptime, char *absunits, cdType abstimetype,
               double frac, void *abstime);
void cdComp2Char(cdCalenType timetype, cdCompTime comptime, char *time);
void cdComp2Rel(cdCalenType timetype, cdCompTime comptime, char *relunits,
                double *reltime);
void cdRel2Char(cdCalenType timetype, char *relunits, double reltime,
                char *chartime);
void cdRel2Comp(cdCalenType timetype, char *relunits, double reltime,
                cdCompTime *comptime);
