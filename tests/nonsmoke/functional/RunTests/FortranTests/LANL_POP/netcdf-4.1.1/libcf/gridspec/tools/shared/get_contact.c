...
/************************************************************************************************
  ...
  This routine will return number of algined contacts bewteen two tiles (line
contact). This routine assume the starting and ending points of the  contact
line are coincidence with the grid points of both tiles. lrg_rectangle tiles are
assumed. It will return the contact information, which includes
...
int get_align_contact(int tile1, int tile2, int nx1, int ny1, int nx2, int ny2,
  ...
  if (get_contact_index(ny1, ny2, xb1, yb1, xb2, yb2, periodx, 0.0, &start1,
                        &end1, &start2, &end2)) {
  ...
  /* East bound of tile1 and SOUTH bound of tile2, tile1 and tile must be
   * different tile */
  if (tile1 != tile2) {
    free(xb2);
    free(yb2);
    ...
    if (get_contact_index(ny1, nx2, xb1, yb1, xb2, yb2, 0.0, 0.0, &start1,
                          &end1, &start2, &end2)) {
      istart1[ncontact] = nx1 - 1;
      iend1[ncontact] = nx1 - 1;
  ...
  if (get_contact_index(nx1, nx2, xb1, yb1, xb2, yb2, 0.0, periody, &start1,
                        &end1, &start2, &end2)) {
  ...
  /* South bound of tile1 and East bound of tile2, tile1 and tile must be
   * different tile*/
  if (tile1 != tile2) {
    free(xb2);
    free(yb2);
    ...
    if (get_contact_index(nx1, ny2, xb1, yb1, xb2, yb2, 0.0, 0.0, &start1,
                          &end1, &start2, &end2)) {
      istart1[ncontact] = start1;
      iend1[ncontact] = end1;
  ...
  /* to avoid duplicate, the following will be done only when tile1 not equal to
   * tile2 */
  if (tile1 != tile2) {
  ...
  /* when tile1 = tile2, we need to consider about folded. Only foled north is
   * considered here */
  if (tile1 == tile2) {
    int i, folded = 1;
    double dx;
...
int get_contact_index(int size1, int size2, double *x1, double *y1, double *x2,
                      double *y2, double periodx, double periody, int *start1,
