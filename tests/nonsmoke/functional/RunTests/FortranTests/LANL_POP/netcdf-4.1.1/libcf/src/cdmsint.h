 ...
 *              drach@llnl.gov
 *
 * Version:     $Id: cdmsint.h,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
 *
 * Revision History:
 *
 * $Log: cdmsint.h,v $
...
#define cdSymtabNentries(table)                                                \
  (table)->nentries /* number of entries in symbol table */
#define cdDsetVar(dset, varid)                                                 \
  (cdVar *)((dset)->vars->entries) +                                           \
      ((varid) - 1) /* dset variable with id varid */
#define cdDsetDim(dset, dimid)                                                 \
  (cdDim *)((dset)->dims->entries) +                                           \
      ((dimid) - CD_DIM_OFFSET - 1) /* dset dimension with id dimid */
#define cdDsetGrid(dset, gridid)                                               \
  (cdGrid *)((dset)->grids->entries) +                                         \
      ((gridid) - CD_GRID_OFFSET - 1) /* dset dimension with id dimid */

#define cdSetByte(vp, val)                                                     \
  {                                                                            \
    unsigned char *ucp;                                                        \
    ucp = (unsigned char *)(vp);                                               \
    *ucp = (val);                                                              \
  }
#define cdSetShort(vp, val)                                                    \
  {                                                                            \
    short *sp;                                                                 \
    sp = (short *)(vp);                                                        \
    *sp = (val);                                                               \
  }
#define cdSetInt(vp, val)                                                      \
  {                                                                            \
    int *ip;                                                                   \
    ip = (int *)(vp);                                                          \
    *ip = (val);                                                               \
  }
#define cdSetLong(vp, val)                                                     \
  {                                                                            \
    long *lp;                                                                  \
    lp = (long *)(vp);                                                         \
    *lp = (val);                                                               \
  }
#define cdSetFloat(vp, val)                                                    \
  {                                                                            \
    float *fp;                                                                 \
    fp = (float *)(vp);                                                        \
    *fp = (val);                                                               \
...
#define cdSetCharTime(vp, val)                                                 \
  {                                                                            \
    char *cp;                                                                  \
    cp = (char *)(vp);                                                         \
    strncpy(cp, (val), CD_MAX_CHARTIME);                                       \
    cp[CD_MAX_CHARTIME - 1] = '\0';                                            \
  }

/* For Dean's DRS-like extensions */

#define CE_FLOAT_NULL 1.0e20 /* Null float arg */
#define CE_FLOAT_DELTA                                                         \
  1.0e14 /* Treat floats as null if = CE_FLOAT_NULL +/- CE_FLOAT_DELTA */
...
typedef struct {
  ...
  char delta[CD_MAX_TIME_DELTA]; /* Delta, for linear rep (datatype is double,
                                    long, or chartime) */
  ...
  char initVal[CD_MAX_CHARTIME]; /* Initial value, for linear rep (datatype is
                                    double, long, or chartime)*/
...
extern int cdSymLookupAlias(cdSymtab *table, char *alias, int *loc,
                            void **entry);

/* Basic time routines */
int cdParseRelunits(cdCalenType timetype, char *relunits, cdUnitTime *unit,
                    cdCompTime *base_comptime);
int cdParseDeltaTime(cdCalenType timetype, char *deltaTime, double *value,
                     cdUnitTime *unit);
...
extern void Cdc2e(char *ctime, CdTimeType timeType, double *etime,
                  long *baseYear);
extern void Cdc2h(char *ctime, CdTimeType timeType, CdTime *htime);
extern void Cde2c(double etime, CdTimeType timeType, long baseYear,
                  char *ctime);
extern void Cde2h(double etime, CdTimeType timeType, long baseYear,
                  CdTime *htime);
...
extern int ce_lookup_with_policy(double tab[], long n, double x,
                                 CeRoundPolicy policy, double delta, long *k);
extern int ce_lookup_cycle(double tab[], long n, double x, double cycle,
                           CeRoundPolicy policy, double delta, long *k,
                           long *icycle);
extern int ce_dimmap(int fileid, int dimid, double df, double dl,
                     CeRoundPolicy policy, double delta, int isCycle,
                     double cycle, double **dp, long *idf, long *idl,
                     double *xdf, double *xdl);
extern int ce_dimmapi(int fileid, int dimid, double df, double dl,
                      CeRoundPolicy policy, double delta, int isCycle,
                      double cycle, double **dp, long *idf, long *idl,
                      double *xdf, double *xdl);
extern int cevarinq(int fileid, int varid, char *name, CuType *datatype,
...
typedef enum { CdFirst = 1, CdLast, CdNext, CdPrev } CdCursorOp;
...
/* indicates that the dimension is not applicable. For direction, a positive
 * value */
...
extern CdDset CdOpenDset(char *path,         /* Dataset path name */
                         CdOpenMode openMode /* Open mode */
);

/* Close a dataset */

extern void CdCloseDset(CdDset dset); /* Dataset handle */

/* Get a variable handle */

extern int CdLookupVar(
    CdDset dset,       /* Dataset handle */
...
extern int CdSetLon(CdVar *var,        /* variable */
                    double firstLon,   /* first longitude in range */
                    double lastLon,    /* last longitude in range */
                    CdRegion *region); /* region to set */

extern int CdSetLat(CdVar *var, /* variable */
                    double firstLat, double lastLat, CdRegion *region);

extern int CdSetLevel(CdVar *var, /* variable */
                      double firstLevel, double lastLevel, CdRegion *region);

extern int CdSetTime(CdVar *var, /* variable */
                     CdTime *start, CdTime *end, CdRegion *region);

/* Get dimension ranges from a region */

extern int CdGetLon(CdVar *var, /* variable */
                    CdRegion *region, double *firstLon, double *lastLon);

extern int CdGetLat(CdVar *var, /* variable */
                    CdRegion *region, double *firstLat, double *lastLat);

extern int CdGetLevel(CdVar *var, /* variable */
                      CdRegion *region, double *firstLevel, double *lastLevel);

extern int CdGetTime(CdVar *var, /* variable */
                     CdRegion *region, CdTime *start, CdTime *end);

/* Get region size, returns number of elements */

extern long CdRegionSize(CdRegion *region, CdType dataType);

/* Set the order structure from human-readable form */
extern int CdSetOrder(
    CdVar *var,          /* Variable handle */
...
extern int CdGetSpctGeom(CdVar *var,               /* Variable */
...
/* Fetch a variable */
extern CdVar CdFetchVar(CdCursorOp op); /* Cursor operation */
