...
$Id: cffiles.c,v 1.2 2009/08/21 16:23:48 ed Exp $
*/
 ...
 * it doesn't exist. */
static int nccf_append_att(int ncid, const char *name, const char *string) {
  char *att_str = NULL;
  size_t len, new_len;
  int ret;
  ...
  if (ret == NC_ENOTATT) {
     ...
     * attribute. */
    if ((ret = nc_put_att_text(ncid, NC_GLOBAL, name, strlen(string) + 1,
                               string)))
      return ret;
    ...
    if ((ret = nc_put_att_text(ncid, NC_GLOBAL, name, strlen(att_str) + 1,
                               att_str)))
      BAIL(ret);
  }
...
int nccf_inq_convention(int ncid, int *cf_convention) {
  ...
  if (ret == NC_NOERR) {
    ...
    if ((ret = nc_get_att_text(ncid, NC_GLOBAL, CF_CONVENTIONS, existing_att)))
      BAIL(CF_ENETCDF);
...
int nccf_def_file(int ncid, const char *title, const char *history) {
  ...
  if (title)
    if ((ret = nc_put_att_text(ncid, NC_GLOBAL, CF_TITLE, strlen(title) + 1,
                               title)))
      return ret;
...
int nccf_def_notes(int ncid, int varid, const char *institution,
  ...
  if (source)
    if ((ret = nc_put_att_text(ncid, varid, CF_SOURCE, strlen(source) + 1,
                               source)))
      return ret;

  if (comment)
    if ((ret = nc_put_att_text(ncid, varid, CF_COMMENT, strlen(comment) + 1,
                               comment)))
      return ret;
...
int nccf_inq_notes(int ncid, int varid, size_t *institution_lenp,
  ...
  if (institution_lenp)
    if ((ret = nc_inq_attlen(ncid, varid, CF_INSTITUTION, institution_lenp)))
      return ret;

  if (institution)
    if ((ret = nc_get_att_text(ncid, varid, CF_INSTITUTION, institution)))
      return ret;

  if (source_lenp)
    if ((ret = nc_inq_attlen(ncid, varid, CF_SOURCE, source_lenp)))
      return ret;

  if (source)
    if ((ret = nc_get_att_text(ncid, varid, CF_SOURCE, source)))
      return ret;

  if (comment_lenp)
    if ((ret = nc_inq_attlen(ncid, varid, CF_COMMENT, comment_lenp)))
      return ret;

  if (comment)
    if ((ret = nc_get_att_text(ncid, varid, CF_COMMENT, comment)))
      return ret;

  if (references_lenp)
    if ((ret = nc_inq_attlen(ncid, varid, CF_REFERENCES, references_lenp)))
      return ret;

  if (references)
    if ((ret = nc_get_att_text(ncid, varid, CF_REFERENCES, references)))
      return ret;
