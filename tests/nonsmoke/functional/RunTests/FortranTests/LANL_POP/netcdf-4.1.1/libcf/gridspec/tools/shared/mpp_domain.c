...
void mpp_define_domain_1d(int npts, int ndivs, domain1D *domain) {
  ...
  if (ndivs > npts) {
    mpp_error(
        "mpp_define_domain1d: more divisions requested than rows available. ");
  }
  npts_left = npts;
  pos = 0;
  for (n = 0; n < ndivs; n++) {
...
/************************************************************
  void define_domain(int ni, int nj, int layout[], int xhalo, int yhalo,
domain2D *domain  ) define 2D domain decomposition
************************************************************/

void mpp_define_domain2d(int ni, int nj, int layout[], int xhalo, int yhalo,
                         domain2D *domain) {
  domain1D domx, domy;
  int i, j, posx, posy, n;
...
/***********************************************************
     void get_get_compute_domain(omain2D domain, int *is, int *ie, int *js, int
*je) get the compute domain decomposition
***********************************************************/

void mpp_get_compute_domain2d(domain2D domain, int *is, int *ie, int *js,
                              int *je) {
  *is = domain.isc;
  *ie = domain.iec;
  *js = domain.jsc;
...
void mpp_get_compute_domains2d(domain2D domain, int *is, int *ie, int *js,
                               int *je) {
  int n;
...
void mpp_get_data_domain2d(domain2D domain, int *is, int *ie, int *js,
                           int *je) {
  *is = domain.isd;
  *ie = domain.ied;
  *js = domain.jsd;
...
****************************************************************/
void mpp_get_shift(domain2D domain, int sizex, int sizey, int *ishift,
                   int *jshift) {
  int nxc, nyc, nxd, nyd;
  ...
  if ((sizex == nxc && sizey == nyc) || (sizex == nxd && sizey == nyd)) {
    *ishift = 0;
    *jshift = 0;
  } else if ((sizex == nxc + 1 && sizey == nyc) ||
             (sizex == nxd + 1 && sizey == nyd)) {
    *ishift = 1;
    *jshift = 0;
  } else if ((sizex == nxc && sizey == nyc + 1) ||
             (sizex == nxd && sizey == nyd + 1)) {
    *ishift = 0;
    *jshift = 1;
  } else if ((sizex == nxc + 1 && sizey == nyc + 1) ||
             (sizex == nxd + 1 && sizey == nyd + 1)) {
    *ishift = 1;
    *jshift = 1;
  } else
...
/*************************************************************
    mpp_global_field_double(domain2D domain, int sizex, int sizey, const double
*ldata, double *gdata) get the global data on root pe. ldata is on compute
domain and gdata is on global domain
************************************************************/
void mpp_global_field_double(domain2D domain, int sizex, int sizey,
    ...
    for (p = 0; p < npes; p++) {
      if (p == root_pe) {
        if (sizex == nxc && sizey == nyc) { /* data is on compute domain */
          ...
          for (j = js; j <= je; j++) {
            for (i = is; i <= ie; i++)
              gdata[j * nxg + i] = ldata[n++];
          }
        } else {
          for (j = js; j <= je; j++) {
            for (i = is; i <= ie; i++)
              gdata[j * nxg + i] = ldata[(j - jsd) * nxd + (i - isd)];
          }
        }
      } else {
        recv_size = (domain.ieclist[p] - domain.isclist[p] + 1 + ishift) *
                    (domain.jeclist[p] - domain.jsclist[p] + 1 + jshift);
...
************************************************************/
void mpp_global_field_double_3D(domain2D domain, int sizex, int sizey,
                                int sizez, const double *ldata, double *gdata) {
  double *send_buffer = NULL, *recv_buffer = NULL;
    ...
    if (sizex == nxc && sizey == nyc) { /* data is on compute domain */
      ...
      for (k = 0; k < sizez; k++)
        for (j = js; j <= je; j++) {
          for (i = is; i <= ie; i++)
            send_buffer[n++] =
                ldata[k * nxd * nyd + (j - jsd) * nxd + (i - isd)];
        }
      ...
      if (p == root_pe) {
        if (sizex == nxc && sizey == nyc) { /* data is on compute domain */
          ...
          for (k = 0; k < sizez; k++)
            for (j = js; j <= je; j++) {
              for (i = is; i <= ie; i++)
                gdata[k * nxg * nyg + j * nxg + i] = ldata[n++];
            }
        } else {
          for (k = 0; k < sizez; k++)
            for (j = js; j <= je; j++) {
              for (i = is; i <= ie; i++)
                gdata[k * nxg * nyg + j * nxg + i] =
                    ldata[k * nxd * nyd + (j - jsd) * nxd + (i - isd)];
            }
        }
      } else {
