...
void conformal_map_coords2xyz(int ni, int nj, double *lx, double *ly, double *X,
                              double *Y, double *Z);
void map_xyz2lonlat(int ni, int nj, double *X, double *Y, double *Z,
                    double *lon, double *lat);
void rotate_about_xaxis(int ni, int nj, double *X, double *Y, double *Z,
                        double angle);
void permutiles(int ni, int nj, double *b, int num);
void calc_fvgrid(int nx, int ny, int nratio, double *dx, double *dy,
                 double *area);
double *angle_between_vectors(int ni, int nj, double *vec1, double *vec2);
double *excess_of_quad(int ni, int nj, double *vec1, double *vec2, double *vec3,
                       double *vec4);
double *plane_normal(int ni, int nj, double *P1, double *P2);
void calc_rotation_angle(int nxp, int nyp, double *x, double *y,
                         double *angle_dx, double *angle_dy);

/*******************************************************************************
  void create_conformal_cubic_grid( int *npoints, int *nratio, char *method,
char *orientation, double *x, double *y, double *dx, double *dy, double *area,
double *angle_dx, double *angle_dy ) create cubic grid. All six tiles grid will
be generated.
*******************************************************************************/
void create_conformal_cubic_grid(int *npts, int *nratio, char *method,
                                 char *orientation, double *x, double *y,
...
void calc_geocoords_centerpole(int nx, int ny, double *x, double *y) {
  ...
  for (j = 0; j < nyh; j++) {
    for (i = 0; i < nxh; i++) {
      lonP[j * nxp + i] =
          (tmp[j * nxh + i] - 3. / 2. * M_PI - tmp[i * nxh + j]) * 0.5;
      if (i == j)
        lonP[j * nxp + i] = -3.0 / 4.0 * M_PI;
    }
  }
...
/*************************************************************************
  void conformal_map_coords2xyz ( double *lx, double * ly, double *X, double *Y,
double *Z ) Conformal mapping of a cube onto a sphere maps (lx, ly) on the
north-pole face of a cube to (X,Y,Z) coordinates in physical space. Face is
oriented normal to Z-axis with  X and Y increasing with lx and ly valid ranges:
-1 < lx < 1   -1 < ly < 1 Based on scripts from Alistair  ????
**************************************************************************/
...
void conformal_map_coords2xyz(int ni, int nj, double *lx, double *ly, double *X,
  ...
  const int order = 29;
  const double A[] = {
      1.47713057321600,  -0.38183513110512, -0.05573055466344,
...
void calc_fvgrid(int nx, int ny, int nratio, double *dx, double *dy,
  ...
  for (j = 0; j < njf; j++) {
    for (i = 0; i < nif - 1; i++)
      dxl[j * (nif - 1) + i] =
          (dxl[j * (nif - 1) + i] + dyl[i * nif + j]) * 0.5;
  }

  for (j = 0; j < njf - 1; j++) {
    for (i = 0; i < nif; i++)
      dyl[j * nif + i] = dxl[i * (nif - 1) + j];
  }
  ...
  for (j = 0; j < njf - 1; j++) {
    for (i = 0; i < nif - 1; i++) {
      if (j < i)
        areal[j * (nif - 1) + i] =
            (areal[j * (nif - 1) + i] + areal[i * (nif - 1) + j]) * 0.5;
      else if (j > i)
  ...
  for (j = 0; j < nyh; j++) {
    for (i = 0; i < nxh - 1; i++) {
      ar = 0;
      for (n = 0; n < nratio; n++)
        ar = ar + dxl[j * nratio * (nif - 1) + i * nratio + n];
      dx[j * nx + i] = ar * RADIUS;
    }
  }
  ...
  for (j = 0; j < nyh - 1; j++) {
    for (i = 0; i < nxh - 1; i++) {
      ...
      for (n = 0; n < nratio; n++)
        for (m = 0; m < nratio; m++)
          ar += areal[(j * nratio + n) * (nif - 1) + i * nratio + m];
...
double *angle_between_vectors(int ni, int nj, double *vec1, double *vec2) {
  ...
  for (n = 0; n < ni * nj; n++) {
    vector_prod = vec1[n] * vec2[n] + vec1[ni * nj + n] * vec2[ni * nj + n] +
                  vec1[2 * ni * nj + n] * vec2[2 * ni * nj + n];
    nrm1 = pow(vec1[n], 2) + pow(vec1[ni * nj + n], 2) +
           pow(vec1[2 * ni * nj + n], 2);
    nrm2 = pow(vec2[n], 2) + pow(vec2[ni * nj + n], 2) +
           pow(vec2[2 * ni * nj + n], 2);
...
double *plane_normal(int ni, int nj, double *P1, double *P2) {
  ...
  for (j = 0; j < nj; j++) {
    for (i = 0; i < ni; i++) {
      n = j * ni + i;
      p1 = P1[ni * nj + n] * P2[2 * ni * nj + n] -
           P1[2 * ni * nj + n] * P2[ni * nj + n];
  ...
  for (n = 0; n < ntiles; n++) {
    for (j = 0; j < nyp; j++) {
      for (i = 0; i < nxp; i++) {
        ...
        angle_dx[n * nxp * nyp + j * nxp + i] =
            atan2(y[tp1 * nxp * nyp + jp1 * nxp + ip1] -
                      y[tm1 * nxp * nyp + jm1 * nxp + im1],
                  (x[tp1 * nxp * nyp + jp1 * nxp + ip1] -
                   x[tm1 * nxp * nyp + jm1 * nxp + im1]) *
                      lon_scale) *
            R2D;
        tp1 = n;
        tm1 = n;
