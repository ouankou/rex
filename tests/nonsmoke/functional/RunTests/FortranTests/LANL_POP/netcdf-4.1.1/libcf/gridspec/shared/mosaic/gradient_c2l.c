...
#include <stdio.h>
void a2b_ord2(int nx, int ny, const double *qin, const double *edge_w,
              const double *edge_e, const double *edge_s, const double *edge_n,
              double *qout, int on_west_edge, int on_east_edge,
              int on_south_edge, int on_north_edge);
...
/*------------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------*/
void grad_c2l_(const int *nlon, const int *nlat, const double *pin,
               const double *dx, const double *dy, const double *area,
               const double *edge_w, const double *edge_e, const double *edge_s,
               const double *edge_n, const double *en_n, const double *en_e,
               const double *vlon, const double *vlat, double *grad_x,
               double *grad_y, const int *on_west_edge, const int *on_east_edge,
               const int *on_south_edge, const int *on_north_edge) {
  grad_c2l(nlon, nlat, pin, dx, dy, area, edge_w, edge_e, edge_s, edge_n, en_n,
           en_e, vlon, vlat, grad_x, grad_y, on_west_edge, on_east_edge,
           on_south_edge, on_north_edge);
}

void grad_c2l(const int *nlon, const int *nlat, const double *pin,
              const double *dx, const double *dy, const double *area,
              const double *edge_w, const double *edge_e, const double *edge_s,
              const double *edge_n, const double *en_n, const double *en_e,
              const double *vlon, const double *vlat, double *grad_x,
  ...
  a2b_ord2(nx, ny, pin, edge_w, edge_e, edge_s, edge_n, pb, *on_west_edge,
  ...
  for (j = 0; j < nyp; j++)
    for (i = 0; i < nx; i++) {
      ...
      for (n = 0; n < 3; n++) {
        pdx[3 * m0 + n] =
            0.5 * (pb[m1] + pb[m1 + 1]) * dx[m0] * en_n[3 * m0 + n];
      }
    }
  ...
  for (j = 0; j < ny; j++)
    for (i = 0; i < nxp; i++) {
      ...
      for (n = 0; n < 3; n++) {
        pdy[3 * m0 + n] =
            0.5 * (pb[m0] + pb[m0 + nxp]) * dy[m0] * en_e[3 * m0 + n];
      }
    }
  ...
  for (j = 0; j < ny; j++)
    for (i = 0; i < nx; i++) {
      m0 = 3 * (j * nx + i);
      for (n = 0; n < 3; n++) {
        grad3[m0 + n] = pdx[3 * ((j + 1) * nx + i) + n] - pdx[m0 + n] -
                        pdy[3 * (j * nxp + i) + n] +
                        pdy[3 * (j * nxp + i + 1) + n];
      }
    }
  ...
  for (j = 0; j < ny; j++)
    for (i = 0; i < nx; i++) {
      ...
      m1 = 3 * m0;
      /* dq / d(Lamda)*/
      grad_x[m0] = (vlon[m1] * grad3[m1] + vlon[m1 + 1] * grad3[m1 + 1] +
                    vlon[m1 + 2] * grad3[m1 + 2]) /
                   area[m0];
      grad_x[m0] *= RADIUS;
      /* dq / d(theta) */
      grad_y[m0] = (vlat[m1] * grad3[m1] + vlat[m1 + 1] * grad3[m1 + 1] +
                    vlat[m1 + 2] * grad3[m1 + 2]) /
                   area[m0];
      grad_y[m0] *= RADIUS;
    }
...
void a2b_ord2(int nx, int ny, const double *qin, const double *edge_w,
  ...
  for (j = jstart; j < jend; j++)
    for (i = istart; i < iend; i++) {
      qout[j * nxp + i] =
          0.25 *
          (qin[j * (nx + 2) + i] + qin[j * (nx + 2) + i + 1] +
           qin[(j + 1) * (nx + 2) + i] + qin[(j + 1) * (nx + 2) + i + 1]);
    }

  /* Fix the 4 Corners */
  if (on_west_edge && on_south_edge)
    qout[0] = r3 * (qin[1 * (nx + 2) + 1] + qin[1 * (nx + 2)] +
                    qin[1]); /* sw_corner */
  if (on_east_edge && on_south_edge)
    qout[nx] = r3 * (qin[1 * (nx + 2) + nx] + qin[nx] +
                     qin[1 * (nx + 2) + nxp]); /* se_corner */
  if (on_east_edge && on_north_edge)
    qout[ny * nxp + nx] =
        r3 * (qin[ny * (nx + 2) + nx] + qin[ny * (nx + 2) + nxp] +
              qin[nyp * (nx + 2) + nx]); /* ne_corner */
  if (on_west_edge && on_north_edge)
    qout[ny * nxp] = r3 * (qin[ny * (nx + 2) + 1] + qin[ny * (nx + 2)] +
                           qin[nyp * (nx + 2) + 1]); /* nw_corner */
...
void get_edge(int nx, int ny, const double *lont, const double *latt,
              const double *lonc, const double *latc, double *edge_w,
              double *edge_e, double *edge_s, double *edge_n, int on_west_edge,
              int on_east_edge, int on_south_edge, int on_north_edge) {
  int i, j, nxp, nyp;
  int istart, iend, jstart, jend;
  double p1[2], p2[2];
  double *py, *px;
  ...
  if (on_west_edge) {
    ...
    for (j = jstart; j <= jend; j++) {
      ...
      p1[0] = lont[j * (nx + 2) + i];
      p1[1] = latt[j * (nx + 2) + i];
      p2[0] = lont[j * (nx + 2) + i + 1];
      p2[1] = latt[j * (nx + 2) + i + 1];
  ...
  if (on_south_edge) {
    ...
    for (i = istart; i <= iend; i++) {
      p1[0] = lont[j * (nx + 2) + i];
      p1[1] = latt[j * (nx + 2) + i];
      p2[0] = lont[(j + 1) * (nx + 2) + i];
      p2[1] = latt[(j + 1) * (nx + 2) + i];
...
/**********************************************************************************************
  This routine is used to calculate grid information for second order
conservative interpolation from cubic grid to other grid the size of xt     will
be (nx+2,ny+2), T-cell center, with halo = 1 the size of yt     will be
(nx+2,ny+2), T-cell center, with halo = 1 the size of xc     will be
(nx+1,ny+1), C-cell center the size of yc     will be (nx+1,ny+1), C-cell center
  the size of dx     will be (nx, ny+1),  N-cell center
  the size of dy     will be (nx+1, ny),  E-cell center
  the size of area   will be (nx, ny),    T-cell center.
...
**********************************************************************************************/
void calc_c2l_grid_info_(int *nx_pt, int *ny_pt, const double *xt,
                         const double *yt, const double *xc, const double *yc,
                         double *dx, double *dy, double *area, double *edge_w,
                         double *edge_e, double *edge_s, double *edge_n,
                         double *en_n, double *en_e, double *vlon, double *vlat,
                         int *on_west_edge, int *on_east_edge,
                         int *on_south_edge, int *on_north_edge) {
  calc_c2l_grid_info(nx_pt, ny_pt, xt, yt, xc, yc, dx, dy, area, edge_w, edge_e,
                     edge_s, edge_n, en_n, en_e, vlon, vlat, on_west_edge,
                     on_east_edge, on_south_edge, on_north_edge);
}

void calc_c2l_grid_info(int *nx_pt, int *ny_pt, const double *xt,
                        const double *yt, const double *xc, const double *yc,
                        double *dx, double *dy, double *area, double *edge_w,
                        double *edge_e, double *edge_s, double *edge_n,
                        double *en_n, double *en_e, double *vlon, double *vlat,
  ...
  get_edge(nx, ny, xt, yt, xc, yc, edge_w, edge_e, edge_s, edge_n,
