...
*******************************************************************************/
void setup_conserve_interp(int ntiles_in, const Grid_config *grid_in,
                           int ntiles_out, Grid_config *grid_out,
  ...
  if (opcode & READ) {
    for (n = 0; n < ntiles_out; n++) {
      if (interp[n].file_exist) { /* reading from file */
        ...
        if (opcode & CONSERVE_ORDER1)
          read_mosaic_xgrid_order1(interp[n].remap_file, i_in, j_in, i_out,
                                   j_out, xgrid_area);
        else
          read_mosaic_xgrid_order2(interp[n].remap_file, i_in, j_in, i_out,
                                   j_out, xgrid_area, xgrid_clon, xgrid_clat);
        ...
        mpp_close(fid);
        /*distribute the exchange grid on each pe according to target grid
         * index*/
        interp[n].nxgrid = 0;
        for (i = 0; i < nxgrid; i++) {
    ...
    if (mpp_pe() == mpp_root_pe())
      printf("NOTE: Finish reading index and weight for conservative "
      ...
      for (m = 0; m < ntiles_in; m++) {
        ...
        if (opcode & CONSERVE_ORDER1) {
          nxgrid = create_xgrid_2dx2d_order1(
              ...
              grid_in[m].latc, grid_out[n].lonc, grid_out[n].latc, mask, i_in,
              j_in, i_out, j_out, xgrid_area);
          ...
          /* For the purpose of bitiwise reproducing, the following operation is
           * needed. */
          g_nxgrid = nxgrid;
          ...
          mpp_error("conserve_interp: interp_method should be CONSERVE_ORDER1 "
                    "or CONSERVE_ORDER2");
        ...
        if (nxgrid > 0) {
          ...
          if (nxgrid_prev == 0) {
            ...
            interp[n].j_out = (int *)malloc(interp[n].nxgrid * sizeof(int));
            interp[n].area =
                (double *)malloc(interp[n].nxgrid * sizeof(double));
            interp[n].t_in = (int *)malloc(interp[n].nxgrid * sizeof(int));
            for (i = 0; i < interp[n].nxgrid; i++) {
              interp[n].t_in[i] = m;
    ...
    if (opcode & CONSERVE_ORDER2) {
      /* subtrack the grid_in clon and clat to get the distance between xgrid
       * and grid_in */
      for (n = 0; n < ntiles_in; n++) {
        ...
        get_grid_area(&nx_in, &ny_in, grid_in[n].lonc, grid_in[n].latc,
                      area_in);
        for (j = 0; j < ny_in; j++)
          for (i = 0; i < nx_in; i++) {
            ii = j * nx_in + i;
            if (cell_in[n].area[ii] > 0) {
              if (fabs(cell_in[n].area[ii] - area_in[ii]) / area_in[ii] <
                  AREA_RATIO) {
    ...
    if (opcode & WRITE) { /* write out remapping information */
      for (n = 0; n < ntiles_out; n++) {
        ...
        if (nxgrid > 0) {
          ...
          id_tile1_cell =
              mpp_def_var(fid, "tile1_cell", NC_INT, 2, dims, 1,
                          "standard_name", "parent_cell_indices_in_mosaic1");
          id_tile2_cell =
              mpp_def_var(fid, "tile2_cell", NC_INT, 2, dims, 1,
                          "standard_name", "parent_cell_indices_in_mosaic2");
          id_xgrid_area =
              mpp_def_var(fid, "xgrid_area", NC_DOUBLE, 1, &dim_ncells, 2,
                          "standard_name", "exchange_grid_area", "units", "m2");
          if (opcode & CONSERVE_ORDER2)
            id_tile1_dist = mpp_def_var(fid, "tile1_distance", NC_DOUBLE, 2,
          ...
          if (interp[n].nxgrid > 0)
            ldata_int = (int *)malloc(interp[n].nxgrid * sizeof(int));
          ...
          if (opcode & CONSERVE_ORDER2) {
            mpp_gather_field_double(interp[n].nxgrid, interp[n].di_in,
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
          }
          for (i = 0; i < interp[n].nxgrid; i++)
            ldata_int[i] = interp[n].i_out[i] + grid_out[n].isc + 1;
          ...
          mpp_put_var_value_block(fid, id_tile1_cell, start, nwrite, gdata_int);
          if (opcode & CONSERVE_ORDER2) {
            mpp_gather_field_double(interp[n].nxgrid, interp[n].dj_in,
                                    gdata_dbl);
            mpp_put_var_value_block(fid, id_tile1_dist, start, nwrite,
                                    gdata_dbl);
          }
          for (i = 0; i < interp[n].nxgrid; i++)
            ldata_int[i] = interp[n].j_out[i] + grid_out[n].jsc + 1;
          mpp_gather_field_int(interp[n].nxgrid, ldata_int, gdata_int);
          mpp_put_var_value_block(fid, id_tile2_cell, start, nwrite, gdata_int);
          free(gdata_int);
          free(gdata_dbl);
    ...
    if (mpp_pe() == mpp_root_pe())
      printf("NOTE: done calculating index and weight for conservative "
             "interpolation\n");
  }
  ...
  if (opcode & TARGET) {
    for (n = 0; n < ntiles_out; n++) {
      ...
      get_grid_area(&nx_out, &ny_out, grid_out[n].lonc, grid_out[n].latc,
                    grid_out[n].area);
    }
  }
  free(i_in);
...
*******************************************************************************/
void do_scalar_conserve_interp(Interp_config *interp, int varid, int ntiles_in,
        ...
        if (has_missing) {
          for (n = 0; n < interp[m].nxgrid; n++) {
            ...
            if (field_in[tile].data[k * nx1 * ny1 + j1 * nx1 + i1] != missing) {
              field_out[m].data[kk + j2 * nx2 + i2] +=
                  field_in[tile].data[k * nx1 * ny1 + j1 * nx1 + i1] * area;
          ...
          for (n = 0; n < interp[m].nxgrid; n++) {
            ...
            if (field_in[tile].data[n2] != missing) {
              ...
              if (field_in[tile].grad_mask[n1]) { /* use zero gradient */
                field_out[m].data[kk + j2 * nx2 + i2] +=
                    field_in[tile].data[n2] * area;
              } else {
                field_out[m].data[kk + j2 * nx2 + i2] +=
                    (field_in[tile].data[n2] + field_in[tile].grad_x[n1] * di +
  ...
  if (opcode & CHECK_CONSERVE) {
    ...
    for (n = 0; n < ntiles_in; n++) {
      ...
      for (k = 0; k < nz; k++) {
        ...
        for (j = 0; j < ny1; j++)
          for (i = 0; i < nx1; i++) {
            dd =
                field_in[n].data[kk + (j + halo) * (nx1 + 2 * halo) + i + halo];
            if (dd != missing)
              gsum_in += dd * area[j * nx1 + i];
          }
      }
      free(area);
    }
    ...
    if (mpp_pe() == mpp_root_pe())
      printf("the flux(data*area) sum of %s: input = %g, output = %g, diff = "
             "%g. \n",
...
/*******************************************************************************
 ...
 doing conservative interpolation
*******************************************************************************/
void do_vector_conserve_interp(Interp_config *interp, int varid, int ntiles_in,
                               const Grid_config *grid_in, int ntiles_out,
                               const Grid_config *grid_out,
                               const Field_config *u_in,
                               const Field_config *v_in, Field_config *u_out,
                               Field_config *v_out, unsigned int opcode) {
      ...
      for (k = 0; k < nz; k++) {
        for (i = 0; i < nx1 * ny1; i++) {
          ...
          if (tmp_x != missing && tmp_y != missing) {
            u_in[n].data[k * nx1 * ny1 + i] =
                tmp_x * grid_in[n].cosrot[i] - tmp_y * grid_in[n].sinrot[i];
            v_in[n].data[k * nx1 * ny1 + i] =
                tmp_x * grid_in[n].sinrot[i] + tmp_y * grid_in[n].cosrot[i];
          }
        }
      }
    }
  ...
  for (m = 0; m < ntiles_out; m++) {
    ...
    for (k = 0; k < nz; k++) {
      ...
      for (n = 0; n < interp[m].nxgrid; n++) {
        ...
        if (tmp_x != missing && tmp_y != missing) {
          u_out[m].data[kk + j2 * nx2 + i2] +=
              u_in[tile].data[k * nx1 * ny1 + j1 * nx1 + i1] * area;
          v_out[m].data[kk + j2 * nx2 + i2] +=
              v_in[tile].data[k * nx1 * ny1 + j1 * nx1 + i1] * area;
          out_area[j2 * nx2 + i2] += area;
        }
      }
      if (opcode & TARGET) {
      ...
      if (grid_out[m].rotate) {
        for (i = 0; i < nx2 * ny2; i++) {
          ...
          if (tmp_x != missing && tmp_y != missing) {
            u_out[m].data[k * nx2 * ny2 + i] =
                tmp_x * grid_out[m].cosrot[i] + tmp_y * grid_out[m].sinrot[i];
            v_out[m].data[k * nx2 * ny2 + i] =
                -tmp_x * grid_out[m].sinrot[i] + tmp_y * grid_out[m].cosrot[i];
          }
        }
      }
    }
