...
int mpp_npes() { return npes; }; /* mpp_npes */
...
int mpp_root_pe() { return root_pe; }; /* mpp_root_pe */
...
int *mpp_get_pelist() { return pelist; }; /* mpp_get_pelist */
...
void mpp_send_double(const double *data, int size, int to_pe) {
  ...
  MPI_Isend(data, size, MPI_DOUBLE, to_pe, tag, MPI_COMM_WORLD,
            request + to_pe);
#endif
...
#ifdef use_libMPI
  MPI_Status status;
  MPI_Recv(data, size, MPI_DOUBLE, from_pe, MPI_ANY_TAG, MPI_COMM_WORLD,
           &status);
#endif
}; /* mpp_recv_double */
