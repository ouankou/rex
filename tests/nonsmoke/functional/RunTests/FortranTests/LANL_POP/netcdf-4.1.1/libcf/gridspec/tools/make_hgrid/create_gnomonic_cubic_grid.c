...
void cartesian_to_spherical(double x, double y, double z, double *lon,
                            double *lat, double *r);
void spherical_to_cartesian(double lon, double lat, double r, double *x,
                            double *y, double *z);
void symm_ed(int ni, double *lamda, double *theta);
void mirror_grid(int ni, int ntiles, double *x, double *y);
void mirror_latlon(double lon1, double lat1, double lon2, double lat2,
                   double lon0, double lat0, double *lon, double *lat);
void rot_3d(int axis, double x1in, double y1in, double z1in, double angle,
            double *x2out, double *y2out, double *z2out, int degrees,
            int convert);
double excess_of_quad2(const double *vec1, const double *vec2,
                       const double *vec3, const double *vec4);
...
void calc_rotation_angle2(int nxp, double *x, double *y, double *angle_dx,
                          double *angle_dy);
void cell_center(int ni, const double *lonc, const double *latc, double *lont,
                 double *latt);
void cell_east(int ni, const double *lonc, const double *latc, double *lone,
               double *late);
void cell_north(int ni, const double *lonc, const double *latc, double *lonn,
                double *latn);
/*******************************************************************************
  void create_gnomonic_cubic_grid( int *npoints, int *nratio, char *method, char
*orientation, double *x, double *y, double *dx, double *dy, double *area, double
*angle_dx, double *angle_dy ) create nomomic cubic grid. All six tiles grid will
be generated.
*******************************************************************************/
void create_gnomonic_cubic_grid(char *grid_type, int *npts, double *x,
                                double *y, double *dx, double *dy, double *area,
                                double *angle_dx, double *angle_dy) {
  const int ntiles = 6;
  int nx, nxp, ni, nip;
  int i, j, n;
  double p1[2], p2[2];
  ...
  if (nx % 2)
    mpp_error(
        "create_gnomonic_cubic_grid: supergrid size should be divided by 2");
  ni = nx / 2;
  nip = ni + 1;
  nxp = nx + 1;
  ...
  if (strcmp(grid_type, "gnomonic_ed") == 0)
    ...
    mpp_error("create_gnomonic_cubic_grid: grid type should be 'gnomonic_ed', "
  ...
  for (j = 0; j < nip; j++) {
    ...
    xc[3 * nip * nip + j * nip] =
        xc[2 * nip * nip + j * nip + ni]; /* 3E -> 4W */
    yc[3 * nip * nip + j * nip] =
        yc[2 * nip * nip + j * nip + ni]; /* 3E -> 4W */
    xc[4 * nip * nip + j * nip] =
        xc[2 * nip * nip + ni * nip + ni - j]; /* 3N -> 5W */
    yc[4 * nip * nip + j * nip] =
        yc[2 * nip * nip + ni * nip + ni - j]; /* 3N -> 5W */
  }
  for (i = 0; i < nip; i++) {
    xc[4 * nip * nip + i] = xc[3 * nip * nip + ni * nip + i]; /* 4N -> 5S */
    yc[4 * nip * nip + i] = yc[3 * nip * nip + ni * nip + i]; /* 4N -> 5S */
    xc[5 * nip * nip + i] =
        xc[3 * nip * nip + (ni - i) * nip + ni]; /* 4E -> 6S */
    yc[5 * nip * nip + i] =
        yc[3 * nip * nip + (ni - i) * nip + ni]; /* 4E -> 6S */
  }
  for (j = 0; j < nip; j++) {
    xc[5 * nip * nip + j * nip] =
        xc[4 * nip * nip + j * nip + ni]; /* 5E -> 6W */
    yc[5 * nip * nip + j * nip] =
        yc[4 * nip * nip + j * nip + ni]; /* 5E -> 6W */
  }

  /* calculate grid box center location */

  for (n = 0; n < ntiles; n++) {
    cell_center(ni, xc + n * nip * nip, yc + n * nip * nip, xt + n * ni * ni,
                yt + n * ni * ni);
    cell_east(ni, xc + n * nip * nip, yc + n * nip * nip, xe + n * nip * ni,
              ye + n * nip * ni);
    cell_north(ni, xc + n * nip * nip, yc + n * nip * nip, xn + n * ni * nip,
               yn + n * ni * nip);
  }
  ...
  for (n = 0; n < ntiles; n++)
    for (j = 0; j < nip; j++)
      for (i = 0; i < nip; i++) {
        x[n * nxp * nxp + j * 2 * nxp + i * 2] =
            xc[n * nip * nip + j * nip + i];
        y[n * nxp * nxp + j * 2 * nxp + i * 2] =
            yc[n * nip * nip + j * nip + i];
      }
  ...
  for (n = 0; n < ntiles; n++)
    for (j = 0; j < ni; j++)
      for (i = 0; i < ni; i++) {
        x[n * nxp * nxp + (j * 2 + 1) * nxp + i * 2 + 1] =
            xt[n * ni * ni + j * ni + i];
        y[n * nxp * nxp + (j * 2 + 1) * nxp + i * 2 + 1] =
            yt[n * ni * ni + j * ni + i];
      }
  ...
  for (n = 0; n < ntiles; n++)
    for (j = 0; j < ni; j++)
      for (i = 0; i < nip; i++) {
        x[n * nxp * nxp + (j * 2 + 1) * nxp + i * 2] =
            xe[n * nip * ni + j * nip + i];
        y[n * nxp * nxp + (j * 2 + 1) * nxp + i * 2] =
            ye[n * nip * ni + j * nip + i];
      }
  ...
  for (n = 0; n < ntiles; n++)
    for (j = 0; j < nip; j++)
      for (i = 0; i < ni; i++) {
        x[n * nxp * nxp + j * 2 * nxp + i * 2 + 1] =
            xn[n * ni * nip + j * ni + i];
        y[n * nxp * nxp + j * 2 * nxp + i * 2 + 1] =
            yn[n * ni * nip + j * ni + i];
      }
  ...
  for (n = 0; n < ntiles; n++) {
    for (j = 0; j < nxp; j++) {
      for (i = 0; i < nx; i++)
        dy[n * nx * nxp + i * nxp + j] = dx[n * nx * nxp + j * nx + i];
    }
  }
  ...
  for (j = 0; j < nx; j++) {
    ...
    dy[nxp * nx + j * nxp + nx] =
        dx[3 * nx * nxp + (nx - j - 1)];                          /* 4S -> 2E */
    dy[2 * nxp * nx + j * nxp] = dx[nx * nx + nx - j - 1];        /* 1N -> 3W */
    dy[2 * nxp * nx + j * nxp + nx] = dy[3 * nxp * nx + j * nxp]; /* 4W -> 3E */
    dy[3 * nxp * nx + j * nxp + nx] =
        dx[5 * nx * nxp + (nx - j - 1)]; /* 4S -> 2E */
    dy[4 * nxp * nx + j * nxp] =
        dx[2 * nx * nxp + nx * nx + nx - j - 1];                  /* 3N -> 5W */
    dy[4 * nxp * nx + j * nxp + nx] = dy[5 * nxp * nx + j * nxp]; /* 6W -> 5E */
    dy[5 * nxp * nx + j * nxp + nx] =
        dx[nx * nxp + (nx - j - 1)]; /* 2S -> 6E */
  }
  ...
  for (j = 0; j < nx; j++) {
    for (i = 0; i < nx; i++) {
      ...
      p_lr[0] = x[j * nxp + i + 1];
      p_lr[1] = y[j * nxp + i + 1];
      p_ur[0] = x[(j + 1) * nxp + i + 1];
      p_ur[1] = y[(j + 1) * nxp + i + 1];
  ...
  /*calculate rotation angle, just some workaround, will modify this in the
   * future. */
...
void gnomonic_ed(int ni, double *lamda, double *theta) {
  ...
  for (i = 1; i < ni; i++) {
    mirror_latlon(lamda[0], theta[0], lamda[ni * nip + ni],
                  theta[ni * nip + ni], lamda[i * nip], theta[i * nip],
                  &lamda[i], &theta[i]);
  ...
  latlon2xyz(1, &lamda[ni * nip], &theta[ni * nip], &x[ni * nip], &y[ni * nip],
             &z[ni * nip]);
  latlon2xyz(1, &lamda[ni * nip + ni], &theta[ni * nip + ni], &x[ni * nip + ni],
             &y[ni * nip + ni], &z[ni * nip + ni]);
...
void mirror_grid(int ni, int ntiles, double *x, double *y) {
  ...
  for (j = 0; j < ceil(nip / 2.); j++) {
    ...
    for (i = 0; i < ceil(nip / 2.); i++) {
      ip = ni - i;
      x1 = 0.25 * (fabs(x[j * nip + i]) + fabs(x[j * nip + ip]) +
                   fabs(x[jp * nip + i]) + fabs(x[jp * nip + ip]));
      x[j * nip + i] = x1 * (x[j * nip + i] >= 0 ? 1 : -1);
      x[j * nip + ip] = x1 * (x[j * nip + ip] >= 0 ? 1 : -1);
      x[jp * nip + i] = x1 * (x[jp * nip + i] >= 0 ? 1 : -1);
      x[jp * nip + ip] = x1 * (x[jp * nip + ip] >= 0 ? 1 : -1);

      y1 = 0.25 * (fabs(y[j * nip + i]) + fabs(y[j * nip + ip]) +
                   fabs(y[jp * nip + i]) + fabs(y[jp * nip + ip]));
  ...
  for (nt = 1; nt < ntiles; nt++) {
    for (j = 0; j < nip; j++) {
      for (i = 0; i < nip; i++) {
        ...
        case 1: /* tile 2 */
          ang = -90.;
          rot_3d(3, x1, y1, z1, ang, &x2, &y2, &z2, 1,
        ...
        case 2: /* tile 3 */
          ...
          rot_3d(3, x1, y1, z1, ang, &x2, &y2, &z2, 1,
                 1); /* rotate about the z-axis */
          ang = 90.;
          rot_3d(1, x2, y2, z2, ang, &x1, &y1, &z1, 1,
                 1); /* rotate about the z-axis */
          x2 = x1;
          y2 = y1;
          z2 = z1;
        ...
        case 4: /* tile 5 */
          ...
          rot_3d(3, x1, y1, z1, ang, &x2, &y2, &z2, 1,
                 1); /* rotate about the z-axis */
          ang = 90.;
          rot_3d(2, x2, y2, z2, ang, &x1, &y1, &z1, 1,
                 1); /* rotate about the z-axis */
          x2 = x1;
          y2 = y1;
          z2 = z1;
        ...
        case 5: /* tile 6 */
          ang = 90.;
          rot_3d(2, x1, y1, z1, ang, &x2, &y2, &z2, 1,
                 1); /* rotate about the z-axis */
          ang = 0.;
          rot_3d(3, x2, y2, z2, ang, &x1, &y1, &z1, 1,
                 1); /* rotate about the z-axis */
          x2 = x1;
          y2 = y1;
          z2 = z1;
  ...
  for (n = 0; n < ntiles; n++) {
    for (j = 0; j < nxp; j++) {
      for (i = 0; i < nxp; i++) {
        ...
        angle_dx[n * nxp * nxp + j * nxp + i] =
            atan2(y[tp1 * nxp * nxp + jp1 * nxp + ip1] -
                      y[tm1 * nxp * nxp + jm1 * nxp + im1],
                  (x[tp1 * nxp * nxp + jp1 * nxp + ip1] -
                   x[tm1 * nxp * nxp + jm1 * nxp + im1]) *
                      lon_scale) *
            R2D;
        tp1 = n;
        tm1 = n;
