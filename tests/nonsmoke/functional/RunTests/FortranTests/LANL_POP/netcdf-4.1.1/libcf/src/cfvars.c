...
$Id: cfvars.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
*/
...
int nccf_def_var(int ncid, int varid, const char *units, const char *long_name,
                 const char *standard_name, int ncoord_vars,
                 int *coord_varids) {
  ...
  if (units)
    if ((ret =
             nc_put_att_text(ncid, varid, CF_UNITS, strlen(units) + 1, units)))
      return ret;

  /* Write long_name. */
  if (long_name)
    if ((ret = nc_put_att_text(ncid, varid, CF_LONG_NAME, strlen(long_name) + 1,
                               long_name)))
      return ret;
...
int nccf_inq_var(int ncid, int varid, size_t *units_lenp, char *units,
  ...
  if (long_name_lenp)
    if ((ret = nc_inq_attlen(ncid, varid, CF_LONG_NAME, long_name_lenp)))
      return ret;
  ...
  if (standard_name_lenp)
    if ((ret =
             nc_inq_attlen(ncid, varid, CF_STANDARD_NAME, standard_name_lenp)))
      return ret;
   ...
   * desired. */
  if ((ret = nccf_parse_coords(ncid, varid, CF_COORDINATES, ncoord_vars,
                               coord_varids)))
    return ret;
...
int nccf_def_var_missing(int ncid, int varid, const void *fill_value,
   ...
   * valid_max. */
  if ((valid_min && !valid_max) || (!valid_min && valid_max))
    return CF_EMINMAX;
  ...
  if (fill_value)
    if ((ret = nc_put_att(ncid, varid, CF_FILL_VALUE, xtype, 1, fill_value)))
      return ret;

  /* Write a valid_min if the user provided one. */
  if (valid_min)
    if ((ret = nc_put_att(ncid, varid, CF_VALID_MIN, xtype, 1, valid_min)))
      return ret;

  /* Write a valid_max value if the user provided one. */
  if (valid_max)
    if ((ret = nc_put_att(ncid, varid, CF_VALID_MAX, xtype, 1, valid_max)))
      return ret;
...
static int nccf_get_default_missing(nc_type xtype, void *fill_value,
