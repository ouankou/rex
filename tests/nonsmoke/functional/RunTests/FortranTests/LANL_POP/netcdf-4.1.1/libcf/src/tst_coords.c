...
static void test_axis(const char *testfile) {
  ...
  /* Create a file. */
  if (nc_create(testfile, NC_CLOBBER, &ncid))
    ERR;

  /* Create an dimension and a coordinate var to go with it. */
  if (nc_def_dim(ncid, DIM1_NAME, DIM_LEN, &dimid))
    ERR;
  if (dimid != 0)
    ERR;
  if (nc_def_var(ncid, DIM1_NAME, NC_FLOAT, 1, dimids, &varid))
    ERR;
  if (varid != 0)
    ERR;
  if (nccf_def_axis_type(ncid, varid, NCCF_LONGITUDE))
    ERR;
  if (nccf_inq_axis_type(ncid, varid, &axis_type))
    ERR;
  if (axis_type != NCCF_LONGITUDE)
    ERR;
  ...
  if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid))
    ERR;
  if (ndims != 1 && nvars != 2 && natts != 0 && unlimdimid != -1)
    ERR;
  if (nccf_inq_axis_type(ncid, 0, &axis_type))
    ERR;
  if (axis_type != NCCF_LONGITUDE)
    ERR;
  ...
  if (varid != 0)
    ERR;
  if (nccf_def_axis_type(ncid, varid, NCCF_HEIGHT_UP))
    ERR;
  if (nccf_inq_axis_type(ncid, varid, &axis_type))
    ERR;
  ...
  if (axis_type != NCCF_HEIGHT_UP)
    ERR;
  if (nc_get_att_text(ncid, varid, COORDINATE_Z_IS_POSITIVE, value))
    ERR;
  if (strcmp(value, Z_UP))
    ERR;
...
static void test_system(const char *testfile) {
  ...
  if (nc_def_dim(ncid, DIM1_NAME, DIM_LEN, &dimid))
    ERR;
  if (nc_def_var(ncid, DIM1_NAME, NC_FLOAT, 1, dimids, &axis_varid))
    ERR;
  if (axis_varid != 0)
    ERR;
  if (nccf_def_coord_system(ncid, COORD_SYSTEM, 1, axes_varids, &system_varid))
    ERR;
  if (system_varid != 1)
    ERR;
  if (nccf_inq_coord_system(ncid, 1, name_in, &naxes_in, axes_varids_in))
    ERR;
  if (strcmp(name_in, COORD_SYSTEM) || naxes_in != 1 ||
      axes_varids_in[0] != axes_varids[0])
    ERR;
...
static void test_system_assign(const char *testfile) {
  ...
  for (d = 0; d < NDIMS; d++) {
    if (nc_def_dim(ncid, dim_name[d], dim_len[d], &dimids[d]))
      ERR;
    if (nc_def_var(ncid, dim_name[d], NC_FLOAT, 1, &dimids[d], &axis_varids[d]))
      ERR;
  }

  /* Create two data vars, earth and air. (Don't know what happened
   * to fire and water Aristotle!) */
  if (nc_def_var(ncid, EARTH, NC_FLOAT, NDIMS, dimids, &earth_varid))
    ERR;
  if (nc_def_var(ncid, AIR, NC_FLOAT, NDIMS, dimids, &air_varid))
    ERR;

  /* Unite our 4 dims in a coordinate system. */
  if (nccf_def_coord_system(ncid, LAT_LON_COORDINATE_SYSTEM, NDIMS, axis_varids,
                            &system_varid))
    ERR;
  ...
  if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid))
    ERR;
  if (ndims != 4 && nvars != 7 && natts != 0 && unlimdimid != dimids[3])
    ERR;
  if (nc_inq_varid(ncid, LAT_LON_COORDINATE_SYSTEM, &varid_in))
    ERR;
  if (varid_in != system_varid)
    ERR;
  if (nc_inq_var(ncid, system_varid, name_in, &type_in, &ndims_in, dimids_in,
                 &natts_in))
    ERR;
  if (strcmp(name_in, LAT_LON_COORDINATE_SYSTEM) || type_in != NC_CHAR ||
      ndims_in != 0 || natts_in != 1)
    ;
  if (nc_inq_att(ncid, system_varid, COORDINATE_AXES, &type_in, &len_in))
    ERR;
  if (type_in != NC_CHAR)
    ERR;
  if (nccf_inq_coord_system(ncid, system_varid, name_in, &naxes_in,
                            axis_varids_in))
    ERR;
  if (strcmp(name_in, LAT_LON_COORDINATE_SYSTEM) || naxes_in != NDIMS)
    ERR;
  for (d = 0; d < NDIMS; d++)
    if (axis_varids_in[d] != axis_varids[d])
      ERR;

  /* Assign the system to the earth and air data variables. */
  if (nccf_assign_coord_system(ncid, earth_varid, system_varid))
    ERR;
  if (nccf_assign_coord_system(ncid, air_varid, system_varid))
    ERR;

  /* Check to ensure that the assignments happened. */
  if (nc_inq_varnatts(ncid, earth_varid, &natts_in))
    ERR;
  if (natts_in != 1)
    ERR;
  if (nc_inq_att(ncid, earth_varid, COORDINATE_SYSTEMS, &type_in, &len_in))
    ERR;
  if (type_in != NC_CHAR)
    ERR;
  if (nc_inq_varnatts(ncid, air_varid, &natts_in))
    ERR;
  if (natts_in != 1)
    ERR;
  if (nc_get_att_text(ncid, earth_varid, COORDINATE_SYSTEMS, systems_in))
    ERR;
  if (strcmp(systems_in, LAT_LON_COORDINATE_SYSTEM))
    ERR;
  if (nc_inq_att(ncid, air_varid, COORDINATE_SYSTEMS, &type_in, &len_in))
    ERR;
  if (type_in != NC_CHAR)
    ERR;
  if (nc_get_att_text(ncid, air_varid, COORDINATE_SYSTEMS, systems_in))
    ERR;
  if (strcmp(systems_in, LAT_LON_COORDINATE_SYSTEM))
    ERR;
...
static void test_transform(const char *testfile) {
  ...
  /* Create a transform. */
  if (nccf_def_transform(ncid, TRANSFORM_NAME, TRANSFORM_TYPE1, TRANSFORM_NAME,
                         &transform_varid))
    ERR;
  if (transform_varid != 0)
    ERR;
  if (nccf_inq_transform(ncid, transform_varid, name_in, &type_len,
                         transform_type_in, &name_len, transform_name_in))
    ERR;
  if (strcmp(name_in, TRANSFORM_NAME) ||
      type_len != strlen(TRANSFORM_TYPE1) + 1 ||
      strcmp(transform_type_in, TRANSFORM_TYPE1) ||
      name_len != strlen(TRANSFORM_NAME) + 1 ||
      strcmp(transform_name_in, TRANSFORM_NAME))
    ERR;
 ...
 * John's example 4. */
static void test_transform_assign(const char *testfile) {
  int ncid, axis_varids[NDIMS], system_varid, dimids[NDIMS];
  int transform_varid;
  ...
  size_t dim_len[NDIMS] = {NC_UNLIMITED, LEVEL_LEN, LAT_LEN, LON_LEN};
  int axis_type[NDIMS] = {NCCF_TIME, NCCF_HEIGHT_DOWN, NCCF_LATITUDE,
                          NCCF_LONGITUDE};
  int earth_varid, air_varid;
  int d;
  ...
  for (d = 0; d < NDIMS; d++) {
    ...
    if (nc_def_var(ncid, dim_name[d], NC_FLOAT, 1, &dimids[d], &axis_varids[d]))
      ERR;
    if (nccf_def_axis_type(ncid, axis_varids[d], axis_type[d]))
      ERR;
  }
  ...
  if (nccf_def_transform(ncid, LAMBERT_CONFORMAL_PROJECTION, PROJECTION,
                         LAMBERT_CONFORMAL_CONIC, &transform_varid))
    ERR;
  if (nccf_inq_transform(ncid, transform_varid, name_in, &len_in,
                         transform_type_in, &name_len, transform_name_in))
    ERR;
  if (strcmp(name_in, LAMBERT_CONFORMAL_PROJECTION) ||
      len_in != strlen(PROJECTION) + 1 ||
      strcmp(transform_type_in, PROJECTION) ||
      name_len != strlen(LAMBERT_CONFORMAL_CONIC) + 1 ||
      strcmp(transform_name_in, LAMBERT_CONFORMAL_CONIC))
    ERR;

  /* Assign the transform to the coordinate system. */
  if (nccf_assign_transform(ncid, system_varid, transform_varid))
    ERR;

  /* Check it out. */
  if (nc_get_att_text(ncid, system_varid, COORDINATE_TRANSFORMS, transform_in))
    ERR;
  if (strcmp(transform_in, LAMBERT_CONFORMAL_PROJECTION))
    ERR;
