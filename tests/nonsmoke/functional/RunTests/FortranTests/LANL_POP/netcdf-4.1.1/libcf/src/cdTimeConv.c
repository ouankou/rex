 ...
 *              drach@llnl.gov
 *
 * Version:     $Id: cdTimeConv.c,v 1.1.1.1 2009/07/06 15:06:30 ed Exp $
 *
 * Revision History:
 *
 * $Log: cdTimeConv.c,v $
...
void cdComp2RelMixed(cdCompTime ct, cdUnitTime unit, cdCompTime basetime,
                     double *reltime);
void cdRel2CompMixed(double reltime, cdUnitTime unit, cdCompTime basetime,
                     cdCompTime *comptime);

#define CD_DEFAULT_BASEYEAR                                                    \
  "1979" /* Default base year for relative time (no 'since' clause) */
...
int cdToOldTimetype(cdCalenType newtype, CdTimeType *oldtype) {
  switch (newtype) {
  case cdStandard:
    *oldtype = CdChron;
    break;
  ...
  case cdClim360:
    *oldtype = CdClim360;
    break;
  default:
    cdError("Error on relative units conversion, invalid timetype = %d",
            newtype);
    return 1;
  }
  return 0;
  ...
  if (!strncmp(charunits, "sec", 3) || !strcmp(charunits, "s")) {
    ...
    *unit = cdSeason;
    ...
    if (!(timetype & cdStandardCal)) {
      cdError("Error on relative units conversion: climatological units cannot "
              "be 'years'.\n");
      return 1;
    }
    *unit = cdYear;
...
int cdParseDeltaTime(cdCalenType timetype, char *deltaTime, double *value,
  ...
  if (!strncmp(charunits, "sec", 3) || !strcmp(charunits, "s")) {
    ...
    *unit = cdSeason;
    ...
    if (!(timetype & cdStandardCal)) {
      cdError("Error on delta time conversion: climatological units cannot be "
              "'years'.");
      return 1;
    }
    *unit = cdYear;
...
void cdChar2Comp(cdCalenType timetype, char *chartime, cdCompTime *comptime) {
  ...
  if (timetype & cdStandardCal) {
    nconv = sscanf(chartime, "%ld-%hd-%hd %d:%d:%lf", &year, &month, &day, &ihr,
                   &imin, &sec);
    if (nconv == EOF || nconv == 0) {
    ...
    nconv =
        sscanf(chartime, "%hd-%hd %d:%d:%lf", &month, &day, &ihr, &imin, &sec);
    if (nconv == EOF || nconv == 0) {
...
void cdComp2Char(cdCalenType timetype, cdCompTime comptime, char *time) {
  ...
  if (timetype & cdStandardCal) {
    if (nskip == 0)
      sprintf(time, "%ld-%hd-%hd %d:%d:%lf", comptime.year, comptime.month,
              comptime.day, ihr, imin, sec);
    else if (nskip == 1)
      sprintf(time, "%ld-%hd-%hd %d:%d", comptime.year, comptime.month,
              comptime.day, ihr, imin);
    else
      sprintf(time, "%ld-%hd-%hd %d:0", comptime.year, comptime.month,
              comptime.day, ihr);
  } else { /* Climatological */
    if (nskip == 0)
      sprintf(time, "%hd-%hd %d:%d:%lf", comptime.month, comptime.day, ihr,
              imin, sec);
    else if (nskip == 1)
...
void cdComp2Rel(cdCalenType timetype, cdCompTime comptime, char *relunits,
  ...
  case cdSecond:
    ...
    if (!(timetype & cdStandardCal)) { /* Climatological time */
      hoursInYear = (timetype & cd365Days)   ? 8760.
                    : (timetype & cdHasLeap) ? 8784.
                                             : 8640.;
  ...
  case cdMonth:
  case cdSeason:
  case cdYear: /* Already in correct units */
    if (timetype & cdStandardCal)
...
void cdRel2Rel(cdCalenType timetype, char *relunits, double reltime,
...
int cdParseAbsunits(char *absunits, cdUnitTime *unit, int *ncompon,
                    cdUnitTime compon[]) {
  int nconv;
...
int cdAbs2Comp(char *absunits, void *abstime, cdType abstimetype,
  ...
  default:
    cdError(
        "Error converting absolute to component time: invalid datatype = %d\n",
        abstimetype);
    return 1;
  }
...
int cdComp2Abs(cdCompTime comptime, char *absunits, cdType abstimetype,
  ...
  default:
    cdError(
        "Error converting component to absolute time: invalid datatype = %d\n",
        abstimetype);
    return 1;
  }
  return 0;
...
/* components */
int cdDecodeRelativeTime(cdCalenType timetype, char *units, double time,
                         cdCompTime *comptime) {
...
/* time, 0 if not.*/
int cdDecodeAbsoluteTime(char *units, void *time, cdType abstimetype,
...
double cdToHours(double value, cdUnitTime unit) {
...
void cdCompAdd(cdCompTime comptime, double value, cdCalenType calendar,
               cdCompTime *result) {
...
/* Result is in hours. */
double cdDiffJulian(cdCompTime ca, cdCompTime cb) {
...
/* Result is in hours. */
double cdDiffGregorian(cdCompTime ca, cdCompTime cb) {
...
/* Result is in hours. */
double cdDiffMixed(cdCompTime ca, cdCompTime cb) {
