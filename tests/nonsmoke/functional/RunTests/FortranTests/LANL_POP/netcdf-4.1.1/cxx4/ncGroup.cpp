...
// comparator operator
bool operator<(const NcGroup &lhs, const NcGroup &rhs) { return false; }

// comparator operator
bool operator>(const NcGroup &lhs, const NcGroup &rhs) { return true; }
} // namespace netCDF
...
NcGroup::NcGroup() : nullObject(true) {}

// constructor
NcGroup::NcGroup(const int groupId) : nullObject(false), myId(groupId) {}
...
NcGroup::NcGroup(const NcGroup &rhs)
    : nullObject(rhs.nullObject), myId(rhs.myId) {}
...
//  !=  operator
bool NcGroup::operator!=(const NcGroup &rhs) const { return !(*this == rhs); }
...
string NcGroup::getName(bool fullName) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getName on a Null group",
                    __FILE__, __LINE__);
  string groupName;
  if (fullName) {
...
NcGroup NcGroup::getParentGroup() const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getParentGroup on a Null group",
                    __FILE__, __LINE__);
  try {
    int parentId;
...
int NcGroup::getId() const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getId on a Null group",
                    __FILE__, __LINE__);
  return myId;
}
...
int NcGroup::getGroupCount(NcGroup::GroupLocation location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getGroupCount on a Null group",
                    __FILE__, __LINE__);
  ...
  if (location == ChildrenGrps || location == AllChildrenGrps ||
      location == AllGrps) {
    int numgrps;
    int *ncids = NULL;
  ...
  if (location == ParentsGrps || location == ParentsAndCurrentGrps ||
      location == AllGrps) {
  ...
  if (location == ChildrenOfChildrenGrps || location == AllChildrenGrps ||
      location == AllGrps) {
...
multimap<std::string, NcGroup>
NcGroup::getGroups(NcGroup::GroupLocation location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getGroups on a Null group",
                    __FILE__, __LINE__);
  ...
  if (location == ChildrenGrps || location == AllChildrenGrps ||
    ...
    for (int i = 0; i < groupCount; i++) {
      ...
      ncGroups.insert(
          pair<const string, NcGroup>(tmpGroup.getName(), tmpGroup));
    }
  }
    ...
    if (!tmpGroup.isRootGroup()) {
      while (1) {
        ...
        if (parentGroup.isNull())
          break;
        ncGroups.insert(
            pair<const string, NcGroup>(parentGroup.getName(), parentGroup));
...
NcGroup NcGroup::getGroup(const string &name,
                          NcGroup::GroupLocation location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getGroup on a Null group",
                    __FILE__, __LINE__);
  multimap<string, NcGroup> ncGroups(getGroups(location));
  pair<multimap<string, NcGroup>::iterator, multimap<string, NcGroup>::iterator>
      ret;
...
NcGroup NcGroup::addGroup(const string &name) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::addGroup on a Null group",
                    __FILE__, __LINE__);
  int new_ncid;
  ncCheck(nc_def_grp(myId, const_cast<char *>(name.c_str()), &new_ncid),
          __FILE__, __LINE__);
...
int NcGroup::getVarCount(NcGroup::Location location) const {
  ...
  if (location == ChildrenAndCurrent || location == Children ||
      location == All) {
    multimap<string, NcGroup>::iterator it;
...
multimap<std::string, NcVar>
NcGroup::getVars(NcGroup::Location location) const {
  ...
  if (location == Parents || location == ParentsAndCurrent || location == All) {
    ...
    while (!tmpGroup.isNull()) {
      ...
      // now get the name of each NcVar object and populate the ncVars
      // container.
      int *nvars = NULL;
      vector<int> varids(varCount);
      ncCheck(nc_inq_varids(tmpGroup.getId(), nvars, &varids[0]), __FILE__,
              __LINE__);
      for (int i = 0; i < varCount; i++) {
...
set<NcVar> NcGroup::getVars(const string &name,
                            NcGroup::Location location) const {
  // get the set of ncVars in this group and above.
  multimap<std::string, NcVar> ncVars(getVars(location));
  pair<multimap<string, NcVar>::iterator, multimap<string, NcVar>::iterator>
      ret;
...
NcVar NcGroup::addVar(const string &name, const string &typeName,
  ...
  if (tmpType.isNull())
    throw NcNullType("Attempt to invoke NcGroup::addVar failed: typeName must "
                     "be defined in either the current group or a parent group",
                     __FILE__, __LINE__);

  // get a NcDim object with the given dimension name
  NcDim tmpDim(getDim(dimName, NcGroup::ParentsAndCurrent));
  if (tmpDim.isNull())
    throw NcNullDim("Attempt to invoke NcGroup::addVar failed: dimName must be "
                    "defined in either the current group or a parent group",
                    __FILE__, __LINE__);

  // finally define a new netCDF  variable
  int varId;
  int dimId(tmpDim.getId());
  ncCheck(nc_def_var(myId, name.c_str(), tmpType.getId(), 1, &dimId, &varId),
          __FILE__, __LINE__);
...
// Add a new netCDF variable.
NcVar NcGroup::addVar(const string &name, const NcType &ncType,
                      const NcDim &ncDim) const {

  // check NcType object is valid
  if (ncType.isNull())
    throw NcNullType(
        "Attempt to invoke NcGroup::addVar with a Null NcType object", __FILE__,
        __LINE__);
  NcType tmpType(getType(ncType.getName(), NcGroup::ParentsAndCurrent));
  if (tmpType.isNull())
    throw NcNullType("Attempt to invoke NcGroup::addVar failed: NcType must be "
                     "defined in either the current group or a parent group",
                     __FILE__, __LINE__);
  ...
  if (ncDim.isNull())
    throw NcNullDim(
        "Attempt to invoke NcGroup::addVar with a Null NcDim object", __FILE__,
        __LINE__);
  NcDim tmpDim(getDim(ncDim.getName(), NcGroup::ParentsAndCurrent));
  if (tmpDim.isNull())
    throw NcNullDim("Attempt to invoke NcGroup::addVar failed: NcDim must be "
                    "defined in either the current group or a parent group",
                    __FILE__, __LINE__);
...
NcVar NcGroup::addVar(const string &name, const string &typeName,
  ...
  for (size_t i = 0; i < dimNames.size(); i++) {
    ...
    if (tmpDim.isNull())
      throw NcNullDim(
          "Attempt to invoke NcGroup::addVar failed: dimNames must be defined "
          "in either the current group or a parent group",
          __FILE__, __LINE__);
  ...
  // finally define a new netCDF variable
  int varId;
  ncCheck(nc_def_var(myId, name.c_str(), tmpType.getId(), dimIds.size(),
                     &dimIds[0], &varId),
          __FILE__, __LINE__);
  // return an NcVar object for this new variable
  return NcVar(*this, varId);
...
// Add a new netCDF multi-dimensional variable.
NcVar NcGroup::addVar(const string &name, const NcType &ncType,
                      const vector<NcDim> &ncDimVector) const {
...
multimap<std::string, NcGroupAtt>
NcGroup::getAtts(NcGroup::Location location) const {
  ...
  if ((location == ParentsAndCurrent || location == ChildrenAndCurrent ||
    ...
    for (int i = 0; i < attCount; i++) {
      char charName[NC_MAX_NAME + 1];
      ncCheck(nc_inq_attname(tmpGroup.getId(), NC_GLOBAL, i, charName),
              __FILE__, __LINE__);
  ...
  if (location == ChildrenAndCurrent || location == Children ||
    ...
    multimap<string, NcGroup> groups(getGroups());
    for (it = groups.begin(); it != groups.end(); it++) {
      multimap<string, NcGroupAtt> atts =
          it->second.getAtts(ChildrenAndCurrent);
...
NcGroupAtt NcGroup::getAtt(const std::string &name,
  ...
  pair<multimap<string, NcGroupAtt>::iterator,
       multimap<string, NcGroupAtt>::iterator>
      ret;
...
set<NcGroupAtt> NcGroup::getAtts(const string &name,
                                 NcGroup::Location location) const {
...
NcGroupAtt NcGroup::putAtt(const string &name, const string &dataValues) const {
  ncCheck(nc_put_att_text(myId, NC_GLOBAL, name.c_str(), dataValues.size(),
                          dataValues.c_str()),
          __FILE__, __LINE__);
...
//  Creates a new NetCDF group attribute or if already exisiting replaces it.
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                       dataValues),
            __FILE__, __LINE__);
  else
    ncCheck(nc_put_att_uchar(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                             dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_schar(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                             dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
//  Creates a new NetCDF group attribute or if already exisiting replaces it.
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
                           short datumValue) const {
  NcType::ncType typeClass(type.getTypeClass());
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
      typeClass == NcType::nc_ENUM | typeClass == NcType::nc_COMPOUND)
    ncCheck(
        nc_put_att(myId, NC_GLOBAL, name.c_str(), type.getId(), 1, &datumValue),
        __FILE__, __LINE__);
  else
    ncCheck(nc_put_att_short(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                             &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_int(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                           &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_long(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                            &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_float(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                             &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_double(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                              &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_ushort(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                              &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_uint(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                            &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_longlong(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                                &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_ulonglong(myId, NC_GLOBAL, name.c_str(), type.getId(), 1,
                                 &datumValue),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_short(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                             dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_int(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                           dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_long(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                            dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_float(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                             dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_double(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                              dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_ushort(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                              dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_uint(myId, NC_GLOBAL, name.c_str(), type.getId(), len,
                            dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_longlong(myId, NC_GLOBAL, name.c_str(), type.getId(),
                                len, dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  if (typeClass == NcType::nc_VLEN | typeClass == NcType::nc_OPAQUE |
    ...
    ncCheck(nc_put_att_ulonglong(myId, NC_GLOBAL, name.c_str(), type.getId(),
                                 len, dataValues),
            __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
...
//  Creates a new NetCDF group attribute or if already exisiting replaces it.
NcGroupAtt NcGroup::putAtt(const string &name, size_t len,
                           const char **dataValues) const {
  ncCheck(nc_put_att_string(myId, NC_GLOBAL, name.c_str(), len, dataValues),
          __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
}
...
NcGroupAtt NcGroup::putAtt(const string &name, const NcType &type, size_t len,
  ...
  ncCheck(
      nc_put_att(myId, NC_GLOBAL, name.c_str(), type.getId(), len, dataValues),
      __FILE__, __LINE__);
  // finally instantiate this attribute and return
  return getAtt(name);
}
...
int NcGroup::getDimCount(NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getDimCount on a Null group",
                    __FILE__, __LINE__);
  ...
  // search in current group
  if (location == Current || location == ParentsAndCurrent ||
      location == ChildrenAndCurrent || location == All) {
    int ndimsp;
  ...
  if (location == Children || location == ChildrenAndCurrent ||
      location == All) {
    multimap<string, NcGroup>::iterator it;
...
multimap<string, NcDim> NcGroup::getDims(NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getDims on a Null group",
                    __FILE__, __LINE__);
  ...
  if (location == Current || location == ParentsAndCurrent ||
    ...
    ncCheck(nc_inq_dimids(getId(), &dimCount, &dimids[0], 0), __FILE__,
            __LINE__);
...
NcDim NcGroup::getDim(const string &name, NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getDim on a Null group",
                    __FILE__, __LINE__);
  multimap<string, NcDim> ncDims(getDims(location));
  pair<multimap<string, NcDim>::iterator, multimap<string, NcDim>::iterator>
      ret;
...
set<NcDim> NcGroup::getDims(const string &name,
                            NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getDims on a Null group",
                    __FILE__, __LINE__);
...
NcDim NcGroup::addDim(const string &name, size_t dimSize) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::addDim on a Null group",
                    __FILE__, __LINE__);
  int dimId;
...
NcDim NcGroup::addDim(const string &name) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::addDim on a Null group",
                    __FILE__, __LINE__);
  int dimId;
  ncCheck(nc_def_dim(myId, name.c_str(), NC_UNLIMITED, &dimId), __FILE__,
          __LINE__);
  // finally return NcDim object for this new variable
  return NcDim(*this, dimId);
}
...
int NcGroup::getTypeCount(NcGroup::Location location) const {
  ...
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getTypeCount on a Null group",
                    __FILE__, __LINE__);
...
int NcGroup::getTypeCount(NcType::ncType enumType,
  ...
  if (location == Current || location == ParentsAndCurrent ||
    ...
    for (int i = 0; i < ntypesp; i++) {
      ...
      if (tmpType.getTypeClass() == enumType)
        ntypes++;
    }
  }
...
multimap<string, NcType> NcGroup::getTypes(NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getTypes on a Null group",
                    __FILE__, __LINE__);
  ...
  if (location == Current || location == ParentsAndCurrent ||
    ...
    ncCheck(nc_inq_typeids(getId(), &typeCount, &typeids[0]), __FILE__,
            __LINE__);
...
set<NcType> NcGroup::getTypes(const string &name,
                              NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getTypes on a Null group",
                    __FILE__, __LINE__);
  ...
  // return argument of equal_range: iterators to lower and upper bounds of the
  // range.
  pair<multimap<string, NcType>::iterator, multimap<string, NcType>::iterator>
      ret;
...
set<NcType> NcGroup::getTypes(const string &name, NcType::ncType enumType,
                              NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getTypes on a Null group",
                    __FILE__, __LINE__);
...
NcType NcGroup::getType(const string &name, NcGroup::Location location) const {
  if (isNull())
    throw NcNullGrp("Attempt to invoke NcGroup::getType on a Null group",
                    __FILE__, __LINE__);
  if (name == "byte")
    return ncByte;
  if (name == "ubyte")
...
NcEnumType NcGroup::addEnumType(const string &name,
                                NcEnumType::ncEnumType baseType) const {
  nc_type typeId;
  ncCheck(nc_def_enum(myId, baseType, name.c_str(), &typeId), __FILE__,
          __LINE__);
...
NcVlenType NcGroup::addVlenType(const string &name, NcType &baseType) const {
  nc_type typeId;
  ncCheck(nc_def_vlen(myId, const_cast<char *>(name.c_str()), baseType.getId(),
                      &typeId),
          __FILE__, __LINE__);
...
NcOpaqueType NcGroup::addOpaqueType(const string &name, size_t size) const {
  nc_type typeId;
  ncCheck(nc_def_opaque(myId, size, const_cast<char *>(name.c_str()), &typeId),
          __FILE__, __LINE__);
...
map<string, NcGroup> NcGroup::getCoordVars(NcGroup::Location location) const {
  ...
  while (1) {
    ...
    for (itD = dimTmp.begin(); itD != dimTmp.end(); itD++) {
      ...
      if (itV != varTmp.end()) {
        coordVars.insert(
            pair<const string, NcGroup>(string(coordName), tmpGroup));
      }
    }
    if (location != ParentsAndCurrent || location != All ||
        tmpGroup.isRootGroup()) {
      break;
    }
    // continue loop with the parent.
    tmpGroup = tmpGroup.getParentGroup();
...
void NcGroup::getCoordVar(string &coordVarName, NcDim &ncDim, NcVar &ncVar,
                          NcGroup::Location location) const {
