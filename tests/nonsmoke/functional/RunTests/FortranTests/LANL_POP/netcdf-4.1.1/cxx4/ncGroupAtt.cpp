...
// comparator operator
bool operator<(const NcGroupAtt &lhs, const NcGroupAtt &rhs) { return false; }

// comparator operator
bool operator>(const NcGroupAtt &lhs, const NcGroupAtt &rhs) { return true; }
} // namespace netCDF
...
NcGroupAtt::NcGroupAtt(const NcGroup &grp, const int index) : NcAtt(false) {
  groupId = grp.getId();
  varId = NC_GLOBAL;
