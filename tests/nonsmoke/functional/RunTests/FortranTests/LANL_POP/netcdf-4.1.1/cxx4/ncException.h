...
/*!
  ...
  generates an exception using an object inheriting this class.  All other
  netCDF-related errors  including those originating in the C++ binding,
  generates an NcException.
*/
class NcException : public std::exception {
public:
  NcException(const std::string &exceptionName, const std::string &complaint,
              const char *file, int line);
  virtual ~NcException() throw();
  void what() {
    //	std::cout<<fileName<<":"<<lnumber<<" "<<message<<std::endl;
    std::cout << fileName << ":" << lnumber << " thrown exception "
              << exceptionName << ".\n"
              << message << std::endl;
  }
...
/*!
  ...
  data. The exception is thrown if the specified indices were out of range for
  the rank of the specified variable. For example, a negative index or an index
  that is larger than the corresponding dimension length will cause an error.
*/
...
/*!
  ...
  This exception is thrown if an operation on a NcGroup object is requested
  which is empty. To test if the object is empty used NcGroup::isNull()
 */
...
/*!
  ...
  This exception is thrown if an operation on a NcType object is requested which
  is empty. To test if the object is empty used NcType::isNull()
 */
...
/*!
  ...
  This exception is thrown if an operation on a NcDim object is requested which
  is empty. To test if the object is empty used NcDim::isNull()
 */
...
/*!
  Thrown if an operation to set the chunking, endianness, fill of a NcVar object
  is issued after a call to NcVar::getVar or NcVar::putVar has been made.
*/
