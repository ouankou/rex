...
NcOpaqueType &NcOpaqueType::operator=(const NcType &rhs) {
  if (&rhs != this) {
    ...
    if (getTypeClass() != NC_OPAQUE)
      throw NcException("NcException",
                        "The NcType object must be the base of an Opaque type.",
                        __FILE__, __LINE__);
    // assign base class parts
    NcType::operator=(rhs);
  }
...
NcOpaqueType::NcOpaqueType(const NcOpaqueType &rhs) : NcType(rhs) {}
...
// constructor
NcOpaqueType::NcOpaqueType(const NcType &ncType) : NcType(ncType) {
