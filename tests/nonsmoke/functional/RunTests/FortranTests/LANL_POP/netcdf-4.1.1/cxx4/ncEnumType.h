...
public:
  ...
  /*!
    Constructor.
    The enum Type must already exist in the netCDF file. New netCDF enum types
    can be added using NcGroup::addNcEnumType();
  ...
  */
  ...
  /*!
    ...
    NcType is not the base of an Enum type.
  ...
  */
  ...
  /*!
    Assignment operator.
   This assigns from the base type NcType object. Will throw an exception if the
   NcType is not the base of an Enum type.
  */
  ...
  */
  template <class T> void addMember(const std::string &name, T memberValue) {
    ncCheck(nc_insert_enum(groupId, myId, name.c_str(), (void *)&memberValue),
            __FILE__, __LINE__);
  }
  ...
  std::string getMemberNameFromValue(const T memberValue) const {
    ...
    ncCheck(nc_inq_enum_ident(groupId, myId,
                              static_cast<long long>(memberValue), charName),
            __FILE__, __LINE__);
  ...
  */
  template <class T> void getMemberValue(int index, T &memberValue) const {
    char *charName = NULL;
    ncCheck(nc_inq_enum_member(groupId, myId, index, charName, &memberValue),
            __FILE__, __LINE__);
  }
