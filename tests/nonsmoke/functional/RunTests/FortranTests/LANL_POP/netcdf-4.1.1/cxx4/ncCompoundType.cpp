...
NcCompoundType &NcCompoundType::operator=(const NcType &rhs) {
  if (&rhs != this) {
    ...
    if (getTypeClass() != nc_COMPOUND)
      throw NcException(
          "NcException",
          "The NcType object must be the base of a Compound type.", __FILE__,
          __LINE__);
...
NcCompoundType::NcCompoundType(const NcCompoundType &rhs) : NcType(rhs) {}
...
void NcCompoundType::addMember(const string &memberName,
                               const NcType &newMemberType, size_t offset) {
  ncCheck(nc_insert_compound(groupId, myId,
                             const_cast<char *>(memberName.c_str()), offset,
                             newMemberType.getId()),
          __FILE__, __LINE__);
}
...
void NcCompoundType::addMember(const string &memberName,
  ...
  ncCheck(nc_insert_array_compound(groupId, myId,
                                   const_cast<char *>(memberName.c_str()),
                                   offset, newMemberType.getId(), shape.size(),
...
size_t NcCompoundType::getMemberCount() const {
  size_t nfieldsp;
  ncCheck(nc_inq_compound_nfields(groupId, myId, &nfieldsp), __FILE__,
          __LINE__);
  return nfieldsp;
}
...
NcType NcCompoundType::getMember(int memberIndex) const {
  ...
  ncCheck(nc_inq_compound_fieldtype(groupId, myId, memberIndex, &fieldtypeidp),
          __FILE__, __LINE__);
  switch (fieldtypeidp) {
  case NC_BYTE:
    return ncByte;
...
int NcCompoundType::getMemberDimCount(int memberIndex) const {
  int ndimsp;
  ncCheck(nc_inq_compound_fieldndims(groupId, myId, memberIndex, &ndimsp),
          __FILE__, __LINE__);
  return ndimsp;
}
...
vector<int> NcCompoundType::getMemberShape(int memberIndex) const {
  ...
  ncCheck(
      nc_inq_compound_fielddim_sizes(groupId, myId, memberIndex, &dim_size[0]),
      __FILE__, __LINE__);
  return dim_size;
}
...
size_t NcCompoundType::getMemberOffset(const int index) const {
  size_t offsetp;
  ncCheck(nc_inq_compound_fieldoffset(groupId, myId, index, &offsetp), __FILE__,
          __LINE__);
  return offsetp;
}
