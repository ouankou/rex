...
class NcVar {
public:
  /*! Used for chunking specifications (see NcVar::setChunking,
   * NcVar::getChunkingParameters). */
  enum ChunkMode {
    /*!
      Chunked storage is used for this variable.
    */
    nc_CHUNKED = NC_CHUNKED,
    /*! Contiguous storage is used for this variable. Variables with one or more
      unlimited dimensions cannot use contiguous storage. If contiguous storage
      is turned on, the chunkSizes parameter is ignored.
    */
    nc_CONTIGUOUS = NC_CONTIGUOUS
  };

  /*!
    Used to specifying the endianess of the data, (see NcVar::setEndianness,
    NcVar::getEndianness). By default this is NC_ENDIAN_NATIVE.
  */
  enum EndianMode {
  ...
  /*! Used for checksum specification (see NcVar::setChecksum,
   * NcVar::getChecksum). */
  enum ChecksumMode {
  ...
  /*! Constructor for a variable .
    ...
    The variable must already exist in the netCDF file. New netCDF variables can
    be added using NcGroup::addNcVar();
  ...
  */
  ...
  /*! \overload
   */
  NcVarAtt putAtt(const std::string &name, size_t len,
  ...
  /*! \overload
   */
  NcVarAtt putAtt(const std::string &name, const NcType &type, size_t len,
  ...
  /*! \overload
   */
  NcVarAtt putAtt(const std::string &name, const NcType &type,
  ...
  */
  ...
  /*! Sets chunking parameters.
    \param chunkMode   Enumeration type. Allowable parameters are:
    "nc_CONTIGUOUS", "nc_CHUNKED"
  ...
  */
  ...
  /*! Gets the chunking parameters
    \param chunkMode   On return contains either: "nc_CONTIGUOUS" or
    "nc_CHUNKED"
    \param chunksizes  On return contains shape of chunking, used if
    ChunkMode=nc_CHUNKED.
  */
  ...
  */
  ...
  /*! Sets the fill parameters
    ...
    Must be the same type as the variable. Ignored if fillMode=.false.
  */
  template <class T> void setFill(bool fillMode, T fillValue) const {
    ncCheck(
  ...
  */
  void setCompression(bool enableShuffleFilter, bool enableDeflateFilter,
                      int deflateLevel) const;

  /*! Gets the compression parameters
    \param enableShuffleFilter  On return set to true if the shuffle filter is
    enabled.
    \param enableDeflateFilter  On return set to true if the deflate filter is
    enabled.
  ...
  */
  ...
  /*! Sets the endianness of the variable.
    \param Endianness enumeration type. Allowable parameters are:
    "nc_ENDIAN_NATIVE" (the default), "nc_ENDIAN_LITTLE", "nc_ENDIAN_BIG"
  */
  ...
  */
  ...
  /*! Sets the checksum parameters of a variable.
    \param ChecksumMode Enumeration type. Allowable parameters are:
    "nc_NOCHECKSUM", "nc_FLETCHER32".
  */
  void setChecksum(ChecksumMode checksumMode) const;

  /*! Gets the checksum parameters of the variable.
    \return ChecksumMode Enumeration type. Allowable parameters are:
    "nc_NOCHECKSUM", "nc_FLETCHER32".
  */
  ...
  */
  ...
  /*!
    ...
    This is the simplest interface to use for reading the value of a scalar
    variable or when all the values of a multidimensional variable can be read
    at once. The values are read into consecutive locations with the last
    ...
    \param dataValues Pointer to the location into which the data value is read.
  ...
  */
  ...
  /*! \overload
   */
  void getVar(const std::vector<size_t> &index,
  ...
  void getVar(const std::vector<size_t> &index,
              unsigned long long *datumValue) const;
  /*! Reads a single datum value from a variable of an open netCDF dataset.
    The value is converted from the external data type of the variable, if
    necessary.

    \param index       Vector specifying the index of the data value to be read.
    The indices are relative to 0, so for example, the first data value of a
    ...
    variable, the first index is the record number.

    \param datumValue Pointer to the location into which the data value is read.
    If the type of data value differs from the netCDF variable type, type
    conversion will occur. (However, no type conversion is carried out for
    variables using the user-defined data types: nc_Vlen, nc_Opaque, nc_Compound
    and nc_Enum.)
  */
  ...
  /*!
    ...
    the type of the data in memory must match the type of the variable.
  */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, void *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, char **dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, char *dataValues) const;
   ...
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, signed char *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, short *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, int *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, long *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, float *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, double *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
   ...
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, unsigned int *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
  ...
  /*!
    ...
    \param start
    Vector specifying the index in the variable where the first of the data
    values will be read. The indices are relative to 0, so for example, the
    first data value of a variable would have index (0, 0, ... , 0). The length
    of start must be the same as the number of dimensions of the specified
    variable. The elements of start correspond, in order, to the variable's
    dimensions. Hence, if the variable is a record variable, the first index
    would correspond to the starting record number for reading the data values.

    \param count
    Vector specifying the edge lengths along each dimension of the block of data
    values to be read. To read a single value, for example, specify count as (1,
    1, ... , 1). The length of count is the number of dimensions of the
    specified variable. The elements of count correspond, in order, to the
    variable's dimensions. Hence, if the variable is a record variable, the
    first element of count corresponds to a count of the number of records to
    read. Note: setting any element of the count array to zero causes the
  ...
  */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count, long long *dataValues) const;

  //////////////////////

  // Reads a subsampled (strided) array section of values from a netCDF
  // variable.
  /*!
    This is an overloaded member function, provided for convenience.
    It differs from the above function in what argument(s) it accepts.
  ...
  */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, void *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, char **dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
  ...
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, short *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, int *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, long *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, float *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride, double *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
  ...
  /*!
    ...
    \param stride
    Vector specifying the interval between selected indices. The elements of the
    stride vector correspond, in order, to the variable's dimensions. A value of
    1 accesses adjacent values of the netCDF variable in the corresponding
    dimension; a value of 2 accesses every other value of the netCDF variable in
    the corresponding dimension; and so on. A NULL stride argument is treated as
    (1, 1, ... , 1).
  ...
  */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, void *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, char **dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
  ...
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, short *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, int *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, long *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, float *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, double *dataValues) const;
  /*! \overload
   */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
  ...
  /*!
    Reads a mapped array section of values from a netCDF variable.
    The mapped array section is specified by giving a corner, a vector of edge
    ...
    data array. No assumptions are made about the ordering or length of the
  ...
  */
  void getVar(const std::vector<size_t> &start,
              const std::vector<size_t> &count,
              const std::vector<ptrdiff_t> &stride,
              const std::vector<ptrdiff_t> &imap, long long *dataValues) const;
  ...
  */
  ...
  /*!
    Writes the entire data into the netCDF variable.
    This is the simplest interface to use for writing a value in a scalar
    variable or whenever all the values of a multidimensional variable can all
    be written at once. The values to be written are associated with the netCDF
    variable by assuming that the last dimension of the netCDF variable varies
    fastest in the C interface.
    ...
    \param dataValues The data values. The order in which the data will be
  ...
  */
  ...
  /*!
    ...
    \param index      Vector specifying the index where the data values will be
    written. The indices are relative to 0, so for example, the first data value
    of a two-dimensional variable would have index (0,0). The elements of index
    must correspond to the variable's dimensions. Hence, if the variable uses
    the unlimited dimension, the first index would correspond to the unlimited
    dimension.

    \param datumValue The data value. If the type of data values differs from
    the netCDF variable type, type conversion will occur. (However, no type
    conversion is carried out for variables using the user-defined data types:
  ...
  */
  ...
  /*!
    ...
    the type of the data in memory must match the type of the variable.
  */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const void *dataValues) const;
  /*! \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const char **dataValues) const;
  /*!  \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const char *dataValues) const;
   ...
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const short *dataValues) const;
  /*!  \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const int *dataValues) const;
  /*!  \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const long *dataValues) const;
  /*!  \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp, const float *dataValues) const;
  /*!  \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp,
  ...
  /*!
    Writes an array of values into the netCDF variable.
    The portion of the netCDF variable to write is specified by giving a corner
    and a vector of edge lengths that refer to an array section of the netCDF
    variable. The values to be written are associated with the netCDF variable
    by assuming that the last dimension of the netCDF variable varies fastest.

    \param startp  Vector specifying the index where the first data values will
    ...
    \param countp  Vector specifying the number of indices selected along each
  ...
  */
  ...
  /*!
    ...
    \param stridep  A vector of ptrdiff_t integers that specifies the sampling
  ...
  */
  ...
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp,
              const std::vector<ptrdiff_t> &stridep,
              const std::vector<ptrdiff_t> &imapp, const int *dataValues) const;
  /*!  \overload
   */
  void putVar(const std::vector<size_t> &startp,
              const std::vector<size_t> &countp,
  ...
  /*!
   ...
   count corresponds to a count of the number of records to write. Note: setting
   any element of the count array to zero causes the function to exit without
   ...
   (stride[0] gives the sampling interval along the most slowly varying
   dimension of the netCDF variable). Sampling intervals are specified in
   type-independent units of elements (a value of 1 selects consecutive elements
   of the netCDF variable along the corresponding dimension, a value of 2
   selects every other element, etc.). A NULL stride argument is treated as (1,
   1, ... , 1).

    \param imap Vector  specifies the mapping between the dimensions of a netCDF
   variable and the in-memory structure of the internal data array. The elements
   of the index mapping vector correspond, in order, to the netCDF variable's
   dimensions (imap[0] gives the distance between elements of the internal array
   corresponding to the most slowly varying dimension of the netCDF variable).
   Distances between elements are specified in type-independent units of
   elements (the distance between internal elements that occupy adjacent memory
   locations is 1 and not the element's byte-length as in netCDF 2). A NULL
   argument means the memory-resident values have the same structure as the
