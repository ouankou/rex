...
NcEnumType &NcEnumType::operator=(const NcType &rhs) {
  if (&rhs != this) {
    ...
    if (getTypeClass() != NC_ENUM)
      throw NcException("NcException",
                        "The NcType object must be the base of an Enum type.",
                        __FILE__, __LINE__);
...
NcEnumType::NcEnumType(const NcEnumType &rhs) : NcType(rhs) {}
...
// constructor
NcEnumType::NcEnumType(const NcType &ncType) : NcType(ncType) {
...
NcType NcEnumType::getBaseType() const {
  ...
  ncCheck(nc_inq_enum(groupId, myId, charName, &base_nc_typep, base_sizep,
                      num_membersp),
          __FILE__, __LINE__);
...
string NcEnumType::getMemberNameFromIndex(int index) const {
  void *value = NULL;
  char charName[NC_MAX_NAME + 1];
  ncCheck(nc_inq_enum_member(groupId, myId, index, charName, value), __FILE__,
          __LINE__);
