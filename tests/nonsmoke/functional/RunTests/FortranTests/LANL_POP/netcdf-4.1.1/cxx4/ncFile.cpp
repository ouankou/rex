...
// destructor
NcFile::~NcFile() { ncCheck(nc_close(myId), __FILE__, __LINE__); }
...
NcFile::NcFile(const string &filePath, const FileMode fMode) {
  ...
  case NcFile::read:
    ...
    break;
  case NcFile::newFile:
    ncCheck(nc_create(filePath.c_str(), NC_NETCDF4 | NC_NOCLOBBER, &myId),
            __FILE__, __LINE__);
    break;
  case NcFile::replace:
    ncCheck(nc_create(filePath.c_str(), NC_NETCDF4 | NC_CLOBBER, &myId),
            __FILE__, __LINE__);
    break;
  }
  nullObject = false;
...
NcFile::NcFile(const string &filePath, const FileMode fMode,
  ...
  case NcFile::read:
    ...
    break;
  case NcFile::newFile:
    ncCheck(nc_create(filePath.c_str(), format | NC_NOCLOBBER, &myId), __FILE__,
            __LINE__);
    break;
  case NcFile::replace:
    ncCheck(nc_create(filePath.c_str(), format | NC_CLOBBER, &myId), __FILE__,
            __LINE__);
    break;
  }
  nullObject = false;
