...
static NCerror buildattribute4a(NCDRNO *drno, NCattribute *att, int varid,
                                int ncid);
...
static void setnetcdf4dispatch(int ncid) {
  ...
  if (nc != NULL)
    nc->dispatch = &netcdf4lib;
}

static void setncdap4dispatch(int ncid) {
  NC_FILE_INFO_T *nc = nc4_find_nc_file(ncid);
  if (nc != NULL)
    nc->dispatch = &ncdap4lib;
}

int nc4d_open_file(const char *path, int mode, int basepe,
                   size_t *chunksizehintp, int use_parallel, MPI_Comm comm,
  ...
  */
  ...
  if ((mode & NC_CLASSIC_MODEL) != 0)
    ...
    if (dapparamlookup(params, "netcdf4") || dapparamlookup(params, "netcdf-4"))
      translation = 4;
  ...
  if (translation_flags == 0 || (translation_flags & NCF_NC3) != 0) {
    /* call the regular netcdf-4 open, which in turn will call
       netcdf-3 */
    dapurlclear(&tmpurl);
    ncstat = l4nc_open_file(path, mode, basepe, chunksizehintp, use_parallel,
                            comm, info, &ncid);
    if (ncstat == NC_NOERR)
      setnetcdf4dispatch(ncid);
    *ncidp = ncid;
    return THROW(ncstat);
  }
  ...
  if ((mode & NC_WRITE) != 0)
    ncstat = NC_EINVAL;
  else if (mode & (NC_WRITE | NC_CLOBBER))
    ncstat = NC_EPERM;
  if (ncstat != NC_NOERR) {
    THROWCHK(ncstat);
    goto fail;
...
int nc_create_file(const char *path, int cmode, size_t initialsz, int basepe,
                   size_t *chunksizehintp, MPI_Comm comm, MPI_Info info,
                   int *ncidp) {
  NC_GRP_INFO_T *grp;
  NC_FILE_INFO_T *nc;
  NC_HDF5_FILE_INFO_T *h5;
  ...
  ncstat = l4nc_create_file(path, cmode, initialsz, basepe, chunksizehintp,
                            comm, info, ncidp);
  if (ncstat != NC_NOERR)
    return ncstat;
  /* Find our metadata for this file. */
...
int nc4d_sync(int ncid) {
  LOG((1, "nc_sync: ncid 0x%x", ncid));
  return NC_NOERR;
}

int nc4d_abort(int ncid) {
  LOG((1, "nc_abort: ncid 0x%x", ncid));
  /* Turn into close */
  return nc4d_close(ncid);
}

int nc4d_put_vara(NC_FILE_INFO_T *nc, int ncid, int varid, const size_t *startp,
                  const size_t *countp, nc_type mem_nc_type, int is_long,
                  void *data) {
...
static void nc4dinitialize() {
  int i;
  nc3dinitialize();
  for (i = 0; i < NC_MAX_DIMS; i++)
    dapsinglestride4[i] = 1;
  nc4dinitialized = 1;
}

NCerror freeNCDRNO4(NCDRNO *drno) { return freeNCDRNO3(drno); }
...
static NCerror builddims4(NCDRNO *drno) {
  unsigned int i, j;
  NCerror ncstat = NC_NOERR;
  int dimid;
...
static NCerror buildtypes4(NCDRNO *drno) {
  unsigned int i;
  NCerror ncstat = NC_NOERR;
...
static NCerror buildtypes4r(NCDRNO *drno, CDFnode *tnode) {
  ...
  case NC_Sequence:
    ...
    */
    if (tnode->singleton) {
      ...
      ASSERT((prim != NULL));
      ncstat =
          nc_def_vlen(getncid(drno), tnode->vlenname, prim->etype, &typeid);
      if (ncstat != NC_NOERR) {
        THROWCHK(ncstat);
        goto done;
      }
  ...
  case NC_Structure:
    ...
    for (i = 0; i < nclistlength(tnode->subnodes); i++) {
      ...
      if (nclistlength(field->array.dimensions) == 0) {
        ncstat =
            nc_insert_compound(getncid(drno), typeid, field->ncbasename,
                               field->typesize.field.offset, field->typeid);
        if (ncstat != NC_NOERR) {
          THROWCHK(ncstat);
          goto done;
        }
        ...
        ncstat = nc_insert_array_compound(
            getncid(drno), typeid, field->ncbasename,
            field->typesize.field.offset, field->typeid,
            nclistlength(field->array.dimensions), dimsizes);
        if (ncstat != NC_NOERR) {
          THROWCHK(ncstat);
          goto done;
        }
    ...
    if (tnode->nctype == NC_Sequence) {
      ncstat =
          nc_def_vlen(getncid(drno), tnode->vlenname, tnode->typeid, &typeid);
      if (ncstat != NC_NOERR) {
        THROWCHK(ncstat);
        goto done;
      }
  ...
  default:
    PANIC1("unexpected nctype: %d", tnode->nctype);
  }
...
static NCerror buildvars4(NCDRNO *drno) {
  ...
  for (i = 0; i < nclistlength(drno->cdf.varnodes); i++) {
    ...
    ASSERT((var->typeid > 0));
    ncstat = nc_def_var(getncid(drno), var->ncfullname, var->typeid,
...
static NCerror buildglobalattrs4(NCDRNO *drno, int ncid, CDFnode *root) {
  int i;
  const char *txt;
  char *nltxt, *p;
  NCerror ncstat = NC_NOERR;
  ...
  if (paramcheck34(drno, "show", "translate")) {
    ...
    ncstat = nc_put_att_text(ncid, NC_GLOBAL, "_translate", strlen("netcdf-4"),
                             "netcdf-4");
  }
...
static NCerror showprojection4(NCDRNO *drno, CDFnode *var) {
  ...
  ncstat =
      nc_put_att_text(getncid(drno), var->ncid, "_projection",
                      ncbyteslength(projection), ncbytescontents(projection));
  return ncstat;
}

static unsigned long cdftotalsize4(NClist *dimensions) {
  unsigned int i;
...
static size_t estimatesizes4r(NCDRNO *drno, CDFnode *node) {
  ...
  fprintf(stderr, "estimatedsize: %s%s/%u = %lu (= %lu = %lu * %lu)\n",
          (node->visible ? "" : "*"), node->name, rank, totalsize,
          node->estimatedsize, size, dimsize);
#endif
