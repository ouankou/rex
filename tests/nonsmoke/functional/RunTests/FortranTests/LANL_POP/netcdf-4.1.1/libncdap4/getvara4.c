...
static NCerror getcontent4r(NCDRNO *, Getvara *, CDFnode *tnode, OCdata,
                            NCbytes *);
static NCerror getcontent4prim(NCDRNO *drno, Getvara *, CDFnode *tnode,
                               NCsegment *, OCdata currentcontent,
                               NCbytes *memory);
static int findfield(CDFnode *node, CDFnode *subnode);
static int contiguousdims(Dapodometer *odom);
static NCerror ncdap4convert(nc_type srctype, nc_type dsttype, char *memory,
                             char *value, size_t);

int nc4d_get_vara(NC_FILE_INFO_T *nc, int ncid, int varid, const size_t *startp,
                  const size_t *countp, nc_type externaltype0, int is_long,
                  void *data) {
  unsigned int i;
  ...
  if (is_long && externaltype0 < NC_STRING && sizeof(long) != sizeof(int)) {
    ...
    case NC_INT:
    case NC_INT64:
      externaltype = NC_INT64;
      break;
    case NC_UINT:
    case NC_UINT64:
      externaltype = NC_UINT64;
      break;
    default:
      PANIC("Illegal nc_get_varX_long type");
    }
  }
...
#ifdef DEBUG
  {
    ...
    fprintf(stderr, "getvarx: %s/%d", cdfvar->ncfullname,
            (int)nclistlength(dims));
    if (nclistlength(dims) > 0) {
      int i;
      for (i = 0; i < nclistlength(dims); i++)
        fprintf(stderr, "[%lu:%lu]", (unsigned long)startp[i],
                (unsigned long)countp[i]);
      fprintf(stderr, " -> ");
      for (i = 0; i < nclistlength(dims); i++)
        fprintf(stderr, "[%lu:%lu]", (unsigned long)startp[i],
                (unsigned long)((startp[i] + countp[i]) - 1));
    }
    fprintf(stderr, "\n");
  }
#endif
  ...
  if (externaltype == NC_NAT)
    externaltype = cdfvar->externaltype;

  /* Validate any implied type conversion*/
  if (cdfvar->nctype == NC_Primitive && cdfvar->etype != externaltype &&
      externaltype == NC_CHAR) {
  ...
  ncstat = buildvaraprojection4(drno, varainfo, startp, countp,
                                dapsinglestride4, &varaprojection);
  if (ncstat != NC_NOERR) {
    THROWCHK(ncstat);
    goto fail;
  }
...
static NCerror getcontent4(NCDRNO *drno, Getvara *xgetvar, CDFnode *xroot,
                           void *data) {
  NCerror ncstat = NC_NOERR;
  OCerror ocstat = OC_NOERR;
  NCbytes *memory;
  ...
  if (seqvar) {
    ...
    ncstat =
        getcontent4prim(drno, xgetvar, tnode, segment, fieldcontent, memory);
...
static NCerror getcontent4r(NCDRNO *drno, Getvara *xgetvar,
  ...
  if (tnode->nctype == NC_Primitive) {
    ...
    ncstat =
        getcontent4prim(drno, xgetvar, tnode, &seg, currentcontent, memory);
    goto done;
  }
  ...
  case OCRECORDMODE:
    ...
    fprintf(stderr, "getcontent4r: record: memory=%lx |memory|=%lu\n",
            ...
            (unsigned long)ncbyteslength(memory));
#endif
    break;
...
static NCerror getcontent4prim(NCDRNO *drno, Getvara *xgetvar, CDFnode *tnode,
                               NCsegment *segment, OCdata currentcontent,
                               NCbytes *memory) {
  OCerror ocstat = OC_NOERR;
  NCerror ncstat = NC_NOERR;
  unsigned int rank;
  ...
  fprintf(
      stderr,
      "getcontent4prim: tnode=%s/%d segment=%s internaltype=%s externaltype=%s "
      "memory=%lx internaltypesize=%lu externaltypesize=%lu\n",
      tnode->name, rank, dumpsegment(segment), nctypetostring(internaltype),
      nctypetostring(externaltype), (unsigned long)ncbytescontents(memory),
      (unsigned long)internaltypesize, (unsigned long)externaltypesize);
#endif
  alignbuffer3(memory, ncctypealignment(
                           externaltype)); /* Align to the proper boundary */
  memoffset = ncbyteslength(memory);
#ifdef READCHECK
  fprintf(stderr, "getcontent4prim: alignment=%d memory.after=%lx\n",
          ncctypealignment(externaltype),
          (unsigned long)ncbytescontents(memory));
#endif
  if (rank == 0) {  /* singleton data item*/
    ...
    if (!ncbytessetlength(memory, memoffset + arraysize))
      return NC_ENOMEM;
    memdata = ncbytescontents(memory) + memoffset;
...
#ifdef TEST
    {
      ...
      printf("read: %s: memoffset=%lu externaltypesize=%lu dimcount=%lu "
             "requested=%lu arraysize=%lu\n",
             tnode->name, (unsigned long)memoffset,
             (unsigned long)externaltypesize, (unsigned long)dimcount,
             (unsigned long)requested, arraysize);
      fflush(stdout);
    }
#endif
    ...
    if (contiguous < odom->rank && internaltype == externaltype) {
      ...
      if (contiguous == 0) {
        ...
        */
        ...
        ocstat = oc_data_get(conn, currentcontent, memdata, arraysize, 0,
                             chunkcount);
        if (ocstat != OC_NOERR) {
          THROWCHK(ocstat);
          goto fail;
        ...
        while (dapodometermore(odom)) {
          ...
          ocstat = oc_data_get(conn, currentcontent, memdata, arraysize,
      ...
      char *s = (char *)emalloc(requested +
                                1); /* bytes will go into a single string */
      ...
      *sp = s;
      ...
      while (dapodometermore(odom)) {
        ...
        ocstat = oc_data_get(conn, currentcontent, value, sizeof(value),
                             dimoffset, 1);
        if (ocstat != OC_NOERR) {
          THROWCHK(ocstat);
          goto fail;
...
static int contiguousdims(Dapodometer *odom) {
  ...
  for (i = 0; i < odom->rank; i++) {
    ...
    if (odom->slices[i].stride != 1 || odom->slices[i].first > 0 ||
...
is contained.
*/
static NCerror buildvarprojection4(NCDRNO *drno, Getvara *getvar,
                                   NCbytes *buf) {
  int i, dimdex;
  CDFnode *node;
  ...
  for (dimdex = 0, i = 0; i < nclistlength(path); i++) {
    ...
    if (i > 0)
      ncbytescat(buf, "."); /* we are past the first projection*/
    ...
    if (node->nctype == NC_Sequence) {
      ...
      makevarprojection3(getvar->slices, dimdex,
                         nclistlength(node->array.dimensions), buf);
      dimdex += nclistlength(node->array.dimensions);
    }
#endif
  }
#ifdef DEBUG
  fprintf(stderr, "projection=|%s|", ncbytescontents(buf));
  fflush(stderr);
#endif
  nclistfree(path);
  return NC_NOERR;
...
static void makevarprojection3(NCslice *slices, int dimdex, int ndims,
                               NCbytes *buf) {
  int i;
  char tmp[1024];
  ...
  for (i = 0; i < ndims; i++, slice++) {
    snprintf(tmp, sizeof(tmp), "[%lu:%lu:%lu]", (unsigned long)slice->first,
             (unsigned long)slice->stride, (unsigned long)(slice->stop - 1));
    ncbytescat(buf, tmp);
  }
}
#endif
