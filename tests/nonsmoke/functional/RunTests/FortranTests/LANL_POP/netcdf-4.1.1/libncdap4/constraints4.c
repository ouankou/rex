 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap4/constraints4.c,v 1.8 2010/04/02
 *23:18:41 dmh Exp $
 *********************************************************************/
#include "ncdap4.h"
...
is contained.
*/
NCerror buildvaraprojection4(NCDRNO *drno, Getvara *getvar,
...
NCerror prefetchdata4(NCDRNO *drno) {
  int i, j;
  NCerror ncstat = NC_NOERR;
...
static void computevarset4(NCDRNO *drno, Getvara *getvar, NClist *varlist) {
  int i;
  nclistclear(varlist);
  for (i = 0; i < nclistlength(drno->cdf.varnodes); i++) {
    ...
    for (j = 0; j < nclistlength(var->array.ncdimensions); j++) {
      ...
      if (dim->dim.declsize == NC_UNLIMITED) {
        ok = 0;
        break;
      }
    }
