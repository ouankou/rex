 ...
 *   $Header: /shared/CVS/netcdf-3/libncdap4/cdf4.c,v 1.15 2009/11/14 22:33:23
 *dmh Exp $
 *********************************************************************/
#include "config.h"
...
NCerror computecdfnodesets4(NCDRNO *drno) {
  unsigned int i;
...
static NCerror computevarnodes4(NCDRNO *drno, NClist *varnodes) {
  unsigned int i;
...
NCerror fixgrids4(NCDRNO *drno) {
  ...
  for (i = 0; i < nclistlength(gridnodes); i++) {
    ...
    if (daptoplevel(grid))
      nclistpush(topgrids, (ncelem)grid);
...
#ifdef IGNORE
NCerror computecdfdimnames4(NCDRNO *drno) {
  int i, j;
  ...
  */
  for (i = 0; i < nclistlength(drno->cdf.varnodes); i++) {
    ...
    for (j = 0; j < nclistlength(var->array.dimensions); j++) {
      ...
      if (nclistcontains(topdims, (ncelem)dim))
        continue;
...
void setvarbasetype(NCDRNO *drno, CDFnode *field) {
  switch (field->nctype) {
  case NC_Primitive: {
...
static NCerror computetypesizes4(NCDRNO *drno, CDFnode *tnode) {
  ...
  case NC_Structure:
    ...
    tnode->typesize.field.size =
        tnode->typesize.instance.size * cdftotalsize(tnode->array.dimensions);
#ifdef ALIGNCHECK
...
NCerror computeusertypes4(NCDRNO *drno) {
  NClist *toplevel = drno->cdf.ddsroot->subnodes;
  unsigned int i;
...
static NCerror computeusertypes4r(NCDRNO *drno, CDFnode *tnode,
                                  NClist *usertypes) {
  unsigned int i;
...
int singletonsequence(CDFnode *node) {
  CDFnode *thefield;
  if (node == NULL)
    return 0;
  if (node->nctype == NC_Primitive)
    return singletonsequence(node->container);
...
CDFnode *getsingletonfield(NClist *list) {
  int i, fieldcount = 0;
  ...
  for (i = 0; i < nclistlength(list); i++) {
    ...
    if (!field->visible)
      continue;
    fieldcount++;
    if (field->nctype == NC_Primitive &&
        nclistlength(field->array.dimensions) == 0)
      thefield = field;
  }
  if (fieldcount != 1)
    thefield = NULL; /* not the right type of field */
  return thefield;
}
...
static void reversepath(CDFnode *node, NClist *rpath) {
  nclistclear(rpath);
...
static void reversepathstring(unsigned int count, NClist *path, NCbytes *name,
                              const char *separator) {
  unsigned int i, depth;
  ncbytesclear(name);
  if (count > nclistlength(path))
    count = nclistlength(path);
  for (depth = (count - 1), i = 0; i < count; i++, depth--) {
    CDFnode *node = (CDFnode *)nclistget(path, depth);
    if (i > 0)
      ncbytescat(name, (char *)separator);
    ncbytescat(name, node->ncbasename);
  }
}

NCerror shortentypenames4(NCDRNO *drno) {
  ...
  */
  ...
  for (i = 0; i < nclistlength(drno->cdf.usertypes); i++) {
    ...
    case NC_Grid:
      efree(tnode->typename);
      tnode->typename =
          (char *)emalloc(strlen(tnode->ncfullname) + strlen("_t") + 1);
    ...
    case NC_Sequence:
      efree(tnode->typename);
      tnode->typename =
          (char *)emalloc(strlen(tnode->ncfullname) + strlen("_record_t") + 1);
      MEMCHECK(tnode->typename, NC_ENOMEM);
      strcpy(tnode->typename, tnode->ncfullname);
