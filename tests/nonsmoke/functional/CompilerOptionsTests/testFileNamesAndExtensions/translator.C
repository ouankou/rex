...
int main(int argc, char *argv[]) {
  ...
  // Usage:
  //    translator --command=CMD NAMES [-- SWITCHES...]         -- runs: CMD
  //    SWITCHES NAMES translator NAMES -- SWITCHES                        --
  //    runs ROSE on SWITCHES NAMES
  ...
  if (args[0].compare(0, 10, "--command=") ==
  ...
  for (Strings::iterator arg = args.begin(); arg != args.end(); ++arg) {
    ...
    for (size_t i = 0; i < arg->size(); ++i) {
      size_t endPlus;
      if ((*arg)[i] == '+' &&
          (endPlus = arg->find('+', i + 1)) != std::string::npos) {
  ...
  // If there's a '--' argument, then create a stub C source file for each
  // argument before the '--', move them all to the end, and remove the '--'.
  // I.e., if the arguments are "a b -- c d" then create files "a" and "b" and
  // then rearrange the arguments to read "c d a b".  If there's no "--", then
  // create source files for all the arguments but don't rearrange anything.
