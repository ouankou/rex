...
#include "machine.h"
/* BP : 11/01/2001, can't seem to find strdup for some reason */
#include <strings.h>
   }
...
Array_Domain_Type::Initialize_Domain ( 
  ...
  // initialization (all others call this function)
     ...
     if (APP_DEBUG > 1)
        ...
        think this
        // is compiler dependent)!  Also on legacy vector systems we could have
        an example static descriptor
...
// endif
   ...
   // for the view were called too early as is the case on a legacy compiler.
   // The problem appeared in the P6 = -12 + (P5+P4)(all,all,3) test in
   // the testcode.C program that Kristi wrote.
   // What this means is that a view of a temporary is not a temporary in the
   // A++ sense because the view is on the local scope and its destructor is
   // called by the compiler where as an A++ temporary is on the heap and its
   // destructor is called by A++.  If we didn't have it this way then the
   // compiler could delete something allocated within local scope as a local
   // variable.
