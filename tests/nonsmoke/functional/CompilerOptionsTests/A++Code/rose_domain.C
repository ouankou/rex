...
void Array_Domain_Type::Initialize_Domain(int Number_Of_Valid_Dimensions,Internal_Index * const * Internal_Index_List)
   ...
   // initialization (all others call this function)
     ...
     /* It is faster to put the initialization of the data members into the body
of the
// constructor rather than into the preinitialization list (though I think this
// is compiler dependent)!  Also on some legacy systems we could have an example
static descriptor
// and then initialize the new one using the static one in a loop that would
vectorize
// and then to the few scalar operations to fixup the new descriptor data that
was
