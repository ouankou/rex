...
template <class CudaFn> __global__ void launcher(CudaFn fn) {
  int idx = threadIdx.x + blockIdx.x * blockDim.x;

  fn(idx);
}

template <class Fn> void run(int sz, Fn fn) {
...
int main() {
  ...
  // host device lambda
  run(sz, [matrix] __host__ __device__(int idx) -> void { matrix[idx] = 0; });
  return 0;
}
