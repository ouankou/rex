...
// Make sure that the Intel compiler backend is seeing the include file where
// __m128 is defined.
#include <xmmintrin.h>
#else
// A declaration of __m128 can only appear in xmmintrin.h (enforced by Intel
// compiler).

// DQ (1/25/2017): I think this should be (__vector_size__ (16) for at least
// GNU 4.8 in C++11 mode. typedef float __m128 __attribute__ ((__vector_size__
// (16), __may_alias__));
...
// DQ (1/25/2017): I think this should be (__vector_size__ (16) for at least
// GNU 4.8 in C++11 mode. typedef float __m128 __attribute__ ((__vector_size__
// (16), __may_alias__)); typedef float __m128 __attribute__ ((__vector_size__
// (32), __may_alias__));
typedef float __m128 __attribute__((__vector_size__(16), __may_alias__));
#endif
