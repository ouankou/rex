...
OptionDeclaration option2("PACKAGE", "IndexObject");
...
protected:
  inline Internal_Index()
      : Base(0), Count(0), Stride(1), Bound(-1), Index_Mode(All_Index) {
...
public:
  inline Internal_Index(int i)
      : Base(i), Count(1), Stride(1), Bound(i), Index_Mode(Index_Triplet) {
  ...
  inline Internal_Index(int Input_Base, int Input_Count, int Input_Stride,
                        Index_Mode_Type Input_Index_Mode)
      : Base(Input_Base), Count(Input_Count), Stride(Input_Stride),
        Bound(Input_Base + (Input_Count - 1) * Input_Stride),
        Index_Mode(Input_Index_Mode) {
    // I think this should only be set to a NULL_Index if the count is ZERO AND
    // the input mode is not All_Index
    if (Count == 0)
      Index_Mode = Null_Index;
  ...
  inline Internal_Index(const Internal_Index &X)
      : Base(X.Base), Count(X.Count), Stride(X.Stride), Bound(X.Bound),
  ...
  // For Index I(a,b,s) and scalars p and q  ----  p*I+q == Index (p*a+q, b,
  // p*s) So n-I should be (n-a,b,-s).  This is constant with the rule above
  // with n-I being (-1)*I + n Within the definition for operator/ integer
  // division is assumed
  ...
  inline friend Internal_Index operator*(const Internal_Index &Lhs, int i) {
    return Internal_Index(Lhs.Base * i, Lhs.Count, Lhs.Stride * i,
                          Lhs.Index_Mode);
  }
  inline friend Internal_Index operator*(int i, const Internal_Index &Rhs) {
    return Internal_Index(i * Rhs.Base, Rhs.Count, i * Rhs.Stride,
                          Rhs.Index_Mode);
  }
  ...
  inline Index_Mode_Type getMode() const { return Index_Mode; }
  ...
  inline bool operator==(const Internal_Index &X) const {
    ...
    if (Index_Mode == X.Index_Mode) {
      if ((Index_Mode == Null_Index) || (Index_Mode == All_Index))
        ...
        Return_Value =
            ((Base == X.Base) && (Bound == X.Bound) && (Stride == X.Stride));
      }
    }
...
public:
  ...
  // Internally arrays of pointers to Index objects are initialized and we have
  // to avoid taking the address of the member function's returned value.
  Internal_Index *getPointerToLocalPart(const SerialArray_Domain_Type &X,
                                        int Axis) const;

private:
  // used internally in the getLocalPart and getPointerToLocalPart member
  // functions
  void getLocalPartData(const SerialArray_Domain_Type &X, int Axis,
                        int &Output_Base, int &Output_Count,
                        int &Output_Stride) const;
