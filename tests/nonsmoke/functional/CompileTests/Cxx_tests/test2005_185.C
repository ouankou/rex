...
// type modifiers extensions; which are not supported by legacy frontend.
...
// where the backend header files are taken and modified for use by legacy
// frontend. This processing is specific to g++ header files, where:
