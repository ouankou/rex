...
#else
  // Here legacy frontend will normalize the string representation for the
  // template declaration that it saves. But the defining template declaration
  // can still be represented in the AST.  In thise case template instantiations
  // can identify the template declaration.
  friend bool operator== <Key, T, Hash, Pred, Alloc>(unordered_map &, int) {
    return true;
  }
#endif
