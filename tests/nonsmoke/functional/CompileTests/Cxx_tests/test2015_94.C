...
struct to_long_double_functor : boost::static_visitor<long double> {
  template <class T> long double operator()(const T &v) const {
    // Lexical cast has many optimizations including optimizations for
    // situations that usually occur in generic programming, like std::string to
    // std::string or arithmetic type to arithmetic type conversion.
...
// to `long double`
template <class Variant> long double to_long_double(const Variant &v) {
...
int main() {
  ...
  long double sum =
      to_long_double(v1) + to_long_double(v2) + to_long_double(v3);
