...
void test() {
  ...
  // (typedefs are stripped too aggressively in the legacy frontend/ROSE
  // translation). It appears as a name qualification bug because the
  // previous name qualification was never applied to more than the input
  // files (not headers).  This is a bug in ROSE, but nothing to do with name
  // qualification.
  // std::vector< int ,std::allocator< int  > > ::iterator k = std::find<
  // __gnu_cxx::__normal_iterator< std::vector< int ,std::allocator< int  > >
  // ::pointer  , std::vector< int ,std::allocator< int  > > ::vector_type  >
  // , int  > (neighbors. begin (),neighbors. end (),(5));
