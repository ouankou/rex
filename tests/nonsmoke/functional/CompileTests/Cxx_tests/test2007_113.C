...
#define JS_PROTO(name, code, init) JSProto_##name = code,
  JS_PROTO(Null, 0, js_InitNullClass) JS_PROTO(
      Object, 1,
      js_InitFunctionAndObjectClasses) JS_PROTO(Function, 2,
                                                js_InitFunctionAndObjectClasses)
      JS_PROTO(Array, 3, js_InitArrayClass) JS_PROTO(
          Boolean, 4, js_InitBooleanClass) JS_PROTO(Call, 5, js_InitCallClass)
          JS_PROTO(Date, 6, js_InitDateClass) JS_PROTO(
              Math, 7, js_InitMathClass) JS_PROTO(Number, 8, js_InitNumberClass)
              JS_PROTO(String, 9, js_InitStringClass) JS_PROTO(
                  RegExp, 10,
                  js_InitRegExpClass) JS_PROTO(Script, 11,
                                               SCRIPT_INIT) JS_PROTO(XML, 12,
                                                                     XML_INIT)
                  JS_PROTO(Namespace, 13, NAMESPACE_INIT) JS_PROTO(
                      QName, 14,
                      QNAME_INIT) JS_PROTO(AnyName, 15,
                                           ANYNAME_INIT) JS_PROTO(AttributeName,
                                                                  16,
                                                                  ATTRIBUTE_INIT)
                      JS_PROTO(Error, 17, js_InitExceptionClasses) JS_PROTO(
                          InternalError, 18,
                          js_InitExceptionClasses) JS_PROTO(EvalError, 19,
                                                            js_InitExceptionClasses)
                          JS_PROTO(RangeError, 20, js_InitExceptionClasses) JS_PROTO(
                              ReferenceError, 21, js_InitExceptionClasses)
                              JS_PROTO(SyntaxError, 22, js_InitExceptionClasses)
                                  JS_PROTO(TypeError, 23,
                                           js_InitExceptionClasses)
                                      JS_PROTO(URIError, 24,
                                               js_InitExceptionClasses)
                                          JS_PROTO(Generator, 25,
                                                   GENERATOR_INIT)
                                              JS_PROTO(Iterator, 26,
                                                       js_InitIteratorClasses)
                                                  JS_PROTO(
                                                      StopIteration, 27,
                                                      js_InitIteratorClasses)
                                                      JS_PROTO(UnusedProto28,
                                                               28,
                                                               js_InitNullClass)
                                                          JS_PROTO(File, 29,
                                                                   FILE_INIT)
...
typedef JSBool (*JSPropertyOp)(JSContext *cx, JSObject *obj, jsval id,
                               jsval *vp);

typedef JSBool (*JSNewEnumerateOp)(JSContext *cx, JSObject *obj,
...
typedef JSBool (*JSNewResolveOp)(JSContext *cx, JSObject *obj, jsval id,
...
typedef JSBool (*JSDefinePropOp)(JSContext *cx, JSObject *obj, jsid id,
                                 jsval value, JSPropertyOp getter,
                                 JSPropertyOp setter, uintN attrs,
                                 JSProperty **propp);

typedef JSBool (*JSPropertyIdOp)(JSContext *cx, JSObject *obj, jsid id,
                                 jsval *vp);
...
typedef JSBool (*JSSetObjectSlotOp)(JSContext *cx, JSObject *obj, uint32 slot,
                                    JSObject *pobj);

typedef jsval (*JSGetRequiredSlotOp)(JSContext *cx, JSObject *obj, uint32 slot);

typedef JSBool (*JSSetRequiredSlotOp)(JSContext *cx, JSObject *obj, uint32 slot,
                                      jsval v);

typedef JSObject *(*JSGetMethodOp)(JSContext *cx, JSObject *obj, jsid id,
                                   jsval *vp);

typedef JSBool (*JSSetMethodOp)(JSContext *cx, JSObject *obj, jsid id,
                                jsval *vp);

typedef JSBool (*JSEnumerateValuesOp)(JSContext *cx, JSObject *obj,
                                      JSIterateOp enum_op, jsval *statep,
                                      jsid *idp, jsval *vp);

typedef JSBool (*JSEqualityOp)(JSContext *cx, JSObject *obj, jsval v,
                               JSBool *bp);

typedef JSBool (*JSConcatenateOp)(JSContext *cx, JSObject *obj, jsval v,
                                  jsval *vp);

/* Typedef for native functions called by the JS VM. */

typedef JSBool (*JSNative)(JSContext *cx, JSObject *obj, uintN argc,
...
typedef void (*JSErrorReporter)(JSContext *cx, const char *message,
...
typedef const JSErrorFormatString *(*JSErrorCallback)(void *userRef,
                                                      const char *locale,
                                                      const uintN errorNumber);

typedef JSBool (*JSLocaleCompare)(JSContext *cx, JSString *src1, JSString *src2,
                                  jsval *rval);
...
extern JS_EXPORT_API(void) JS_ClearScope(JSContext *cx, JSObject *obj);
}
...
extern JS_EXPORT_API(void) JS_HashTableDestroy(JSHashTable *ht);

extern JS_EXPORT_API(JSHashEntry **)
    JS_HashTableRawLookup(JSHashTable *ht, JSHashNumber keyHash,
                          const void *key);
...
extern JS_EXPORT_API(intN)
    JS_HashTableEnumerateEntries(JSHashTable *ht, JSHashEnumerator f,
                                 void *arg);
...
extern JS_EXPORT_API(JSHashNumber) JS_HashString(const void *key);

extern JS_EXPORT_API(intN) JS_CompareValues(const void *v1, const void *v2);
}
...
void Clear(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
