...
// EDG 4.7, forcing us to tell EDG we are an EDG 4.2 compiler to avoid a
// third-party header bug (see test2013_246.C). DQ (9/12/2009):
// std::__uninitialized_copy_aux is not present in GNU g++ version 4.3. #if (
// (__GNUC__ == 3) || (__GNUC__ == 4) && (__GNUC_MINOR__ < 3) ) #if ( (__GNUC__
// == 3) || (__GNUC__ == 4) && (__GNUC_MINOR__ < 3) ) &&
// !defined(LIE_ABOUT_GNU_VERSION_TO_EDG)
