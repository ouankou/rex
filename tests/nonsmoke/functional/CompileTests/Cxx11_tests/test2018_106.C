...
//
// This is not directly a problem in Areas, but was identified ando not
// completely isolated in the process of building a reducer for an issue
// specific to Ares.
...
namespace __detail {
template <typename _Value> struct _Hash_node {
...
public:
  ...
  using __value_alloc_type = typename __alloctr_rebind<
      _NodeAlloc, typename _NodeAlloc::value_type::value_type>::__type;
  using __value_alloc_traits = std::allocator_traits<__value_alloc_type>;
};
} // namespace __detail

template <typename _Value, typename _Alloc> class _Hashtable {
  using __node_alloc_type =
      typename __alloctr_rebind<_Alloc, __detail::_Hash_node<_Value>>::__type;
...
template <class _Key> class unordered_map {
  // NEEDED:
