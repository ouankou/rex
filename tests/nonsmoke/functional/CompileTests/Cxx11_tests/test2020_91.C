...
// better solution.  The fix for this test code is in frontend_bridge.C
...
// function (in frontend_bridge.c).  An issue appears to be that legacy frontend
// has two scope for something previously processed and the scope_cache that is
// maintained can not be used to lookup the ROSE scope associated with the
// dsecond legacy frontend scope (that was not previously processed).  In these
// cases we force the processing of the previously unseen scope (specifically
// the type that that scope represents).  In test2020_92.C this causes a second
// version of a function to be built.  The fix works great and uses the ROSE AST
// to resolve scopes that have not previoously been seen within legacy frontend,
