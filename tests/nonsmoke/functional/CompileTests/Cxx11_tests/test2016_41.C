...
using LatticeUnaryExpression = std::pair<Op, std::tuple<T1>>;
template <typename Op, typename T1, typename T2>
using LatticeBinaryExpression = std::pair<Op, std::tuple<T1, T2>>;
template <typename Op, typename T1, typename T2, typename T3>
using LatticeTrinaryExpression = std::pair<Op, std::tuple<T1, T2, T3>>;

template <class obj> class Lattice {
public:
  int _osites;
  ...
  template <typename Op, typename T1>
  inline Lattice<obj> &operator=(const LatticeUnaryExpression<Op, T1> &expr) {
    for (int ss = 0; ss < _osites; ss++) {
  ...
  template <typename Op, typename T1, typename T2, typename T3>
  inline Lattice<obj> &
  operator=(const LatticeTrinaryExpression<Op, T1, T2, T3> &expr) {
    for (int ss = 0; ss < _osites; ss++) {
      _odata[ss] = eval(ss, expr);
    }
    return *this;
...
template <class obj>
std::ostream &operator<<(std::ostream &stream, const Lattice<obj> &o) {
  int N = o._osites;
  stream << "{";
  for (int s = 0; s < N - 1; s++) {
    stream << o._odata[s];
...
#define DEFINE_UNOP(op, name)                                                  \
  ...
  inline auto op(const T1 &arg)                                                \
      -> decltype(LatticeUnaryExpression<UNOP(name), const T1 &>(              \
          std::make_pair(UNOP(name)(), std::forward_as_tuple(arg)))) {         \
    return LatticeUnaryExpression<UNOP(name), const T1 &>(                     \
        std::make_pair(UNOP(name)(), std::forward_as_tuple(arg)));             \
  }
...
#define DEFINE_BINOP(op, name)                                                 \
  ...
  inline auto op(const T1 &lhs, const T2 &rhs)                                 \
      -> decltype(LatticeBinaryExpression<BINOP(name), const T1 &,             \
                                          const T2 &>(                         \
          std::make_pair(BINOP(name)(), std::forward_as_tuple(lhs, rhs)))) {   \
    return LatticeBinaryExpression<BINOP(name), const T1 &, const T2 &>(       \
        std::make_pair(BINOP(name)(), std::forward_as_tuple(lhs, rhs)));       \
  }
...
#define DEFINE_TRINOP(op, name)                                                \
  ...
  inline auto op(const T1 &pred, const T2 &lhs, const T3 &rhs)                 \
      -> decltype(LatticeTrinaryExpression<BINOP(name), const T1 &,            \
                                           const T2 &, const T3 &>(            \
          std::make_pair(TRINOP(name)(),                                       \
                         std::forward_as_tuple(pred, lhs, rhs)))) {            \
    return LatticeTrinaryExpression<BINOP(name), const T1 &, const T2 &,       \
                                    const T3 &>(std::make_pair(                \
        TRINOP(name)(), std::forward_as_tuple(pred, lhs, rhs)));               \
  }
...
int main(int argc, char **argv) {
  ...
  Lattice<double> v1(length);
  v1 = 1.0;
  Lattice<double> v2(length);
  v2 = 2.0;
  Lattice<double> v3(length);
  v3 = 10.0;

  BinaryAdd<double, double> tmp;
  LatticeBinaryExpression<BinaryAdd<double, double>, Lattice<double> &,
                          Lattice<double> &>
      expr(std::make_pair(tmp, std::forward_as_tuple(v1, v2)));
