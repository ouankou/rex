...
typedef NestedPolicy<ExecList<omp_parallel_for_exec, seq_exec, simd_exec>> EXEC;

// restrict qualifier does not seem to help
void rhs4sg_forallN(
    int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast, int nk,
    int *onesided, const float_sw4 *__restrict__ a_acof,
    const float_sw4 *__restrict__ a_bope, const float_sw4 *__restrict__ a_ghcof,
    float_sw4 *__restrict__ a_lu, const float_sw4 *__restrict__ a_u,
    const float_sw4 *__restrict__ a_mu, const float_sw4 *__restrict__ a_lambda,
    float_sw4 h, const float_sw4 *__restrict__ a_strx,
...
#define ghcof(i) a_ghcof[i - 1]
...

  {
    forallN<EXEC, int, int, int>(
        RangeSegment(k1, k2 + 1), RangeSegment(jfirst + 2, jlast - 1),
