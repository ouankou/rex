...
protected:
  ...
  LoopAlgorithm() {};
  LoopAlgorithm(const int algorithmId, const StructuredDomain *domain,
  ...
  void compareArrays(const double *exp, const double *comp, const int begin,
                     const int end, double &minErr, double &maxErr);
  void compareRealZoneArrays(const double *exp, const double *comp,
                             const StructuredDomain *domain, double &minErr,
                             double &maxErr);
...
public:
  ...
  void getNodeDimensions(int node_dims[3],
                         const bool include_ghost = true) const;
  void getZoneDimensions(int zone_dims[3],
                         const bool include_ghost = true) const;
  ...
  bool withinRealExtent(const int i, const int j, const int k) {
    return ((i >= m_imin) && (i <= m_imax) && (j >= m_jmin) && (j <= m_jmax) &&
            (k >= m_kmin) && (k <= m_kmax));
  }
  void generateSyntheticDataSet(const int nx, const int ny, const int nz,
                                const int nreg, const int matLayout,
                                const bool mixed, const int ngleft,
                                const int ngright);
...
template <typename T> struct is_unsigned_int {
  constexpr static const bool value =
      std::is_unsigned<T>::value && std::is_integral<T>::value;
};

template <typename T> struct is_signed_int {
  constexpr static const bool value =
      !std::is_unsigned<T>::value && std::is_integral<T>::value;
};

template <typename T, bool GPU = false> struct larger {};

template <> struct larger<uint8_t> {
  using type = uint16_t;
};

template <> struct larger<uint16_t> {
  using type = uint32_t;
};

template <> struct larger<uint32_t> {
  using type = uint64_t;
};

template <> struct larger<int8_t> {
  using type = int16_t;
};

template <> struct larger<int16_t> {
  using type = int32_t;
};

template <> struct larger<int32_t> {
  using type = int64_t;
};

template <> struct larger<float> {
  using type = double;
};

template <> struct larger<double> {
...
template <typename T> struct largest<T, true, false, false> {
  using type = uint64_t;
};

template <typename T> struct largest<T, true, true, false> {
  using type = int64_t;
};

template <typename T> struct largest<T, false, false, true, false> {
  using type = long double;
};

template <typename T> struct largest<T, false, false, true, true> {
  using type = double;
};
} // namespace detail

template <typename T, bool gpu = false> struct largest {
  using type =
      typename detail::largest<T, std::is_integral<T>::value,
                               std::is_signed<T>::value,
                               std::is_floating_point<T>::value, gpu>::type;
};
...
template <typename T, typename U, bool lhsLarger> struct larger_of {};

template <typename T, typename U> struct larger_of<T, U, true> {
  using type = T;
};

template <typename T, typename U> struct larger_of<T, U, false> {
  using type = U;
};
} // namespace detail

template <typename T, typename U> struct larger_of {
  using type =
      typename detail::larger_of<T, U,
...
struct plus : public detail::binary_function<Arg1, Arg2, Ret>,
  ...
  Ret operator()(const Arg1 &lhs, const Arg2 &rhs) { return Ret{lhs} + rhs; }
...
struct logical_and : public detail::comparison_function<Arg1, Arg2>,
                     detail::associative_tag {
  bool operator()(const Arg1 &lhs, const Arg2 &rhs) { return lhs && rhs; }
...
struct bit_or : public detail::binary_function<Arg1, Arg2, Ret> {
  Ret operator()(const Arg1 &lhs, const Arg2 &rhs) { return lhs | rhs; }
};
...
namespace RAJA {
template <typename TYPE> class IndexValue {
public:
  inline __attribute__((always_inline)) constexpr IndexValue() : value(0) {}

  inline
      __attribute__((always_inline)) constexpr explicit IndexValue(Index_type v)
      : value(v) {}

  inline __attribute__((always_inline)) Index_type operator*(void) const {
    return value;
  }
  ...
  inline __attribute__((always_inline)) TYPE operator+(Index_type a) const {
    return TYPE(value + a);
  }
...
using IteratorCategoryOf =
    typename std::iterator_traits<typename std::remove_reference<
        Container>::type::iterator>::iterator_category;

template <typename Container>
using OffersRAI = std::is_base_of<std::random_access_iterator_tag,
                                  IteratorCategoryOf<Container>>;

template <typename Type, typename DifferenceType = std::ptrdiff_t,
...
template <typename Type = Index_type, typename DifferenceType = Index_type,
          typename PointerType = Type *>
...
public:
  ...
  inline numeric_iterator &operator+=(const difference_type &rhs) {
    base::val += rhs;
    return *this;
  }
  inline numeric_iterator &operator-=(const difference_type &rhs) {
  ...
  inline numeric_iterator &operator+=(const numeric_iterator &rhs) {
    base::val += rhs.val;
    return *this;
  }
  inline numeric_iterator &operator-=(const numeric_iterator &rhs) {
  ...
  inline difference_type operator+(const numeric_iterator &rhs) const {
  ...
  friend constexpr numeric_iterator operator+(difference_type lhs,
  ...
  inline Type operator->() const { return base::val; }
  constexpr Type operator[](difference_type rhs) const {
    return base::val + rhs;
  }
};
...
public:
  ...
  constexpr strided_numeric_iterator(const Type &rhs, DifferenceType stride = 1)
  ...
  inline strided_numeric_iterator &operator+=(const difference_type &rhs) {
    base::val += rhs * stride;
    return *this;
  }
  inline strided_numeric_iterator &operator-=(const difference_type &rhs) {
  ...
  inline strided_numeric_iterator operator+(const difference_type &rhs) const {
...
template <typename Iterator> class Enumerater : public numeric_iterator<> {
public:
  template <typename First, typename Second> class InnerPair {
  public:
    InnerPair() = delete;
  ...
  Enumerater() = delete;
  constexpr Enumerater(const Iterator &rhs, std::ptrdiff_t val = 0,
...
template <>
inline __attribute__((always_inline)) void swap(RAJA::IndexSet &a,
                                                RAJA::IndexSet &b) {
  a.swap(b);
}
} // namespace std
...
ListSegment::ListSegment(const T &indx)
    : BaseSegment(_ListSeg_), m_indx(0), m_len(indx.size()),
      m_indx_own(Unowned) {
  if (!indx.empty()) {
...
template <>
inline __attribute__((always_inline)) void swap(RAJA::ListSegment &a,
                                                RAJA::ListSegment &b) {
  a.swap(b);
}
} // namespace std
...
inline __attribute__((always_inline)) void forall_Icount(const IndexSet &c,
                                                         LOOP_BODY loop_body) {
...
forall_Icount(Container &&c, Index_type icount, LOOP_BODY loop_body) {
...
template <typename EXEC_POLICY_T, typename Container, typename LOOP_BODY>
inline __attribute__((always_inline)) void forall(Container &&c,
                                                  LOOP_BODY loop_body) {
  using category =
...
forall_Icount(Index_type begin, Index_type end, Index_type stride,
  ...
  forall_Icount(EXEC_POLICY_T(), RangeStrideSegment(begin, end, stride), icount,
                loop_body);
}
template <typename EXEC_POLICY_T, typename LOOP_BODY>
inline __attribute__((always_inline)) void
...
forall_Icount(const Index_type *idx, Index_type len, Index_type icount,
  ...
  forall_Icount<EXEC_POLICY_T>(ListSegment(idx, len, Unowned), icount,
                               loop_body);
}

} // namespace RAJA

static_assert(201103L >= 201103L, "C++ standards below 2011 are not supported"
                                  "__cplusplus");
namespace VarOps {
using std::get;
...
template <typename Op, typename... Rest> struct foldl_impl;

template <typename Op, typename Arg1> struct foldl_impl<Op, Arg1> {
  using Ret = Arg1;
};
...
foldl(Op &&operation, Arg1 &&arg1, Arg2 &&arg2) ->
  ...
  return forward<Op &&>(operation)(forward<Arg1 &&>(arg1),
                                   forward<Arg2 &&>(arg2));
}
...
foldl(Op &&operation, Arg1 &&arg1, Arg2 &&arg2, Arg3 &&arg3, Rest &&...rest) ->
  ...
  return foldl(forward<Op &&>(operation),
               forward<Op &&>(operation)(
                   forward<Op &&>(operation)(forward<Arg1 &&>(arg1),
                                             forward<Arg2 &&>(arg2)),
                   forward<Arg3 &&>(arg3)),
               forward<Rest &&>(rest)...);
}
...
struct adder {
  ...
  inline __attribute__((always_inline)) constexpr Result
  operator()(const Result &l, const Result &r) const {
    return l + r;
  }
};
...
inline __attribute__((always_inline)) constexpr auto
rotate_left_one(const Seq<First, Ints...>) -> Seq<Ints..., First> {
  return Seq<Ints..., First>{};
}

template <size_t... Ints> constexpr size_t integer_sequence<Ints...>::size;
...
template <size_t... Ints> using index_sequence = integer_sequence<Ints...>;

template <typename Fn, size_t... Sequence, typename TupleLike>
inline __attribute__((always_inline)) constexpr auto
invoke_with_order(TupleLike &&t, Fn &&f, index_sequence<Sequence...>)
    -> decltype(f(get<Sequence>(t)...)) {
  return f(get<Sequence>(t)...);
}

template <typename Fn, typename TupleLike>
inline __attribute__((always_inline)) constexpr auto invoke(TupleLike &&t,
                                                            Fn &&f)
    -> decltype(invoke_with_order(
        t, f, make_index_sequence<tuple_size<TupleLike>::value>{})) {
  return invoke_with_order(t, f,
...
inline __attribute__((always_inline)) void
assign(ToT &&dst, FromT src, index_sequence<To...>, index_sequence<From...>) {
  ignore_args((dst[To] = src[From])...);
}

template <size_t... To, typename ToT, typename... Args>
inline __attribute__((always_inline)) void
assign_args(ToT &&dst, index_sequence<To...>, Args... args) {
  ignore_args((dst[To] = args)...);
}

template <size_t index, size_t first, size_t... rest> struct get_at {
  static constexpr size_t value = get_at<index - 1, rest...>::value;
};

template <size_t first, size_t... rest> struct get_at<0, first, rest...> {
  static constexpr size_t value = first;
};

template <size_t index, typename first, typename... rest> struct get_type_at {
...
struct get_offset_impl {
  static constexpr size_t value =
      get_offset_impl<match - get_at<off + 1, rest...>::value, off + 1, match,
                      rest...>::value;
};
...
template <size_t index> struct get_arg_at {
  template <typename First, typename... Rest>
  inline __attribute__((always_inline)) static constexpr auto
...
template <> struct get_arg_at<0> {
  template <typename First, typename... Rest>
  inline __attribute__((always_inline)) static constexpr auto
  value(First &&first, Rest &&...) -> decltype(VarOps::forward<First>(first)) {
...
public:
  ...
  LayoutBase_impl(const LayoutBase_impl<IndexRange, IdxLin> &rhs)
      : sizes{rhs.sizes[RangeInts]...}, strides{rhs.strides[RangeInts]...},
        mods{rhs.mods[RangeInts]...} {}

  template <typename... Types>
  constexpr inline __attribute__((always_inline))
  LayoutBase_impl(const std::array<IdxLin, n_dims> &sizes_in,
  ...
  inline __attribute__((always_inline)) constexpr IdxLin
...
template <size_t... Ints> using Perm = VarOps::index_sequence<Ints...>;
template <size_t N> using MakePerm = VarOps::make_index_sequence<N>;
...
template <typename Range, typename IdxLin> struct OffsetLayout_impl;
...
struct OffsetLayout_impl<VarOps::index_sequence<RangeInts...>, IdxLin> {
  ...
  static inline
      __attribute__((always_inline)) OffsetLayout_impl<IndexRange, IdxLin>
...
template <typename DataType, typename LayoutT> struct View {
  LayoutT const layout;
  DataType *data;

  template <typename... Args>
  inline __attribute__((always_inline)) constexpr View(DataType *data_ptr,
                                                       Args... dim_sizes)
  ...
  template <typename... Args>
  inline __attribute__((always_inline)) DataType &
...
struct TypedView {
  ...
  template <typename... Args>
  inline __attribute__((always_inline)) constexpr TypedView(DataType *data_ptr,
                                                            Args... dim_sizes)
  ...
  inline __attribute__((always_inline)) DataType &
  operator()(IndexTypes... args) const {
...
inline __attribute__((always_inline)) void
executeRangeList_forall(const IndexSetSegInfo *seg_info,
                        LOOP_BODY &&loop_body) {
...
struct rangeListExecutor {
  ...
  inline __attribute__((always_inline)) void
  operator()(const IndexSetSegInfo &seg_info) {
...
template <typename SEG_EXEC_POLICY_T, typename LOOP_BODY>
constexpr inline __attribute__((always_inline))
rangeListExecutor<SEG_EXEC_POLICY_T, LOOP_BODY>
makeRangeListExecutor(LOOP_BODY &&body) {
  return rangeListExecutor<SEG_EXEC_POLICY_T, LOOP_BODY>(body);
}

template <typename SEG_IT_POLICY_T, typename SEG_EXEC_POLICY_T,
          typename LOOP_BODY>
inline __attribute__((always_inline)) void
forall(IndexSet::ExecPolicy<SEG_IT_POLICY_T, SEG_EXEC_POLICY_T>,
...
template <typename SEG_EXEC_POLICY_T, typename LOOP_BODY>
constexpr inline __attribute__((always_inline))
rangeListIcountExecutor<SEG_EXEC_POLICY_T, LOOP_BODY>
...
template <typename SEG_IT_POLICY_T, typename SEG_EXEC_POLICY_T,
          typename LOOP_BODY>
inline __attribute__((always_inline)) void
forall_Icount(IndexSet::ExecPolicy<SEG_IT_POLICY_T, SEG_EXEC_POLICY_T>,
              const IndexSet &iset, LOOP_BODY loop_body) {

  forall<SEG_IT_POLICY_T>(
...
inline __attribute__((always_inline)) void
forall(const PolicyBase &, const RangeSegment &iter, Func &&loop_body) {
...
inline __attribute__((always_inline)) void
forall(const PolicyBase &, Iterable &&iter, Func &&loop_body) {
...
template <typename Iterable, typename Func>
inline __attribute__((always_inline)) void
forall_Icount(const PolicyBase &, Iterable &&iter, Index_type icount,
              Func &&loop_body) {
...
template <typename T> T *allocate_aligned_type(size_t alignment, size_t size) {
...
public:
  explicit ReduceMin(T init_m_val) : m_parent(__null), m_val(init_m_val) {}
...
public:
  explicit ReduceMax(T init_m_val) : m_parent(__null), m_val(init_m_val) {}
...
public:
  explicit ReduceSum(T init_m_val, T initializer = 0)
      : m_parent(__null), m_val(init_m_val), m_custom_init(initializer) {}
...
void inclusive_inplace(const ::RAJA::seq_exec &, Iter begin, Iter end,
                       BinFn f) {
...
void exclusive_inplace(const ::RAJA::seq_exec &, Iter begin, Iter end, BinFn f,
                       T v) {
  using Value = typename ::std::iterator_traits<Iter>::value_type;
...
void inclusive(const ::RAJA::seq_exec &, Iter begin, Iter end, OutIter out,
               BinFn f) {
  using Value = typename ::std::iterator_traits<Iter>::value_type;
  Value agg = *begin;
  *out++ = agg;
...
void exclusive(const ::RAJA::seq_exec &, Iter begin, Iter end, OutIter out,
               BinFn f, T v) {
  using Value = typename ::std::iterator_traits<Iter>::value_type;
  Value agg = v;
  OutIter o = out;
...
template <typename Iterable, typename Func>
inline __attribute__((always_inline)) void
forall_Icount(const simd_exec &, Iterable &&iter, Index_type icount,
              Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
namespace RAJA {
template <typename InnerPolicy> struct omp_parallel_exec {};
struct omp_for_exec {};
struct omp_parallel_for_exec : public omp_parallel_exec<omp_for_exec> {};
template <size_t ChunkSize> struct omp_for_static {};
...
inline __attribute__((always_inline)) void
forall(const omp_parallel_exec<InnerPolicy> &, Iterable &&iter,
       Func &&loop_body) {
...
template <typename Iterable, typename InnerPolicy, typename Func>
inline __attribute__((always_inline)) void
forall_Icount(const omp_parallel_exec<InnerPolicy> &, Iterable &&iter,
              Index_type icount, Func &&loop_body) {

#pragma omp parallel

  {
    typename std::remove_reference<decltype(loop_body)>::type body = loop_body;
    forall_Icount<InnerPolicy>(std::forward<Iterable>(iter), icount,
...
template <typename Iterable, typename Func>
inline __attribute__((always_inline)) void
forall(const omp_for_nowait_exec &, Iterable &&iter, Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
template <typename Iterable, typename Func>
inline __attribute__((always_inline)) void
forall_Icount(const omp_for_nowait_exec &, Iterable &&iter, Index_type icount,
              Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
template <typename Iterable, typename Func>
inline __attribute__((always_inline)) void
forall(const omp_for_exec &, Iterable &&iter, Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
template <typename Iterable, typename Func>
inline __attribute__((always_inline)) void
forall_Icount(const omp_for_exec &, Iterable &&iter, Index_type icount,
              Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
inline __attribute__((always_inline)) void
forall(const omp_for_static<ChunkSize> &, Iterable &&iter, Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
template <typename Iterable, typename Func, size_t ChunkSize>
inline __attribute__((always_inline)) void
forall_Icount(const omp_for_static<ChunkSize> &, Iterable &&iter,
              Index_type icount, Func &&loop_body) {
  auto begin = std::begin(iter);
  auto end = std::end(iter);
  auto distance = std::distance(begin, end);
...
forall(IndexSet::ExecPolicy<omp_taskgraph_segit, SEG_EXEC_POLICY_T>,
  ...
  if (!iset.dependencyGraphSet()) {
    std::cerr << "\n RAJA IndexSet dependency graph not set , "
              << "FILE: "
              << "/home/liao6/workspace/raja/installraja2017-01-27/include/"
                 "RAJA/internal/exec-openmp/forall_openmp.hxx"
              << " line: " << 240 << std::endl;
...
public:
  explicit ReduceMin(T init_val) : m_parent(__null), m_val(init_val) {}
...
#pragma omp critical
      ...
      {
        m_parent->m_val =
            (((m_val) < (m_parent->m_val)) ? (m_val) : (m_parent->m_val));
      }
    }
  }
...
public:
  explicit ReduceMax(T init_val) : m_parent(__null), m_val(init_val) {}
...
template <typename T> class ReduceMin<omp_reduce_ordered, T> {
public:
...
template <typename T> class ReduceMinLoc<omp_reduce_ordered, T> {
public:
  explicit ReduceMinLoc(T init_val, Index_type init_loc) {
    m_is_copy = false;
...
template <typename T> class ReduceMax<omp_reduce_ordered, T> {
public:
...
template <typename T> class ReduceMaxLoc<omp_reduce_ordered, T> {
public:
  explicit ReduceMaxLoc(T init_val, Index_type init_loc) {
    m_is_copy = false;
...
template <typename T> class ReduceSum<omp_reduce_ordered, T> {
public:
...
inline __attribute__((always_inline)) int firstIndex(int n, int p, int pid) {
  return static_cast<size_t>(n * pid) / p;
}

template <typename Iter, typename BinFn>
void inclusive_inplace(const ::RAJA::omp_parallel_for_exec &, Iter begin,
                       Iter end, BinFn f) {
  using Value = typename ::std::iterator_traits<Iter>::value_type;
  const int n = end - begin;
...
void exclusive_inplace(const ::RAJA::omp_parallel_for_exec &, Iter begin,
                       Iter end, BinFn f, ValueT v) {
  using Value = typename ::std::iterator_traits<Iter>::value_type;
  const int n = end - begin;
  const int p = omp_get_max_threads();
...
template <typename Iter, typename OutIter, typename BinFn>
void inclusive(const ::RAJA::omp_parallel_for_exec &exec, Iter begin, Iter end,
               OutIter out, BinFn f) {
...
void exclusive(const ::RAJA::omp_parallel_for_exec &exec, Iter begin, Iter end,
               OutIter out, BinFn f, ValueT v) {
  ::std::copy(begin, end, out);
...
template <typename policy> struct policy_traits {
  typedef RAJA::seq_exec raja_policy;

  typedef RAJA::IndexSet::ExecPolicy<RAJA::seq_segit, RAJA::seq_exec>
      raja_indexset_policy;
  static const bool valid = false;
};

template <> struct policy_traits<execution_policy::serial> {
  typedef RAJA::seq_exec raja_policy;
...
template <> struct policy_traits<execution_policy::parallel> {
...
namespace core {
template <typename policy, typename loop_type, typename index_type,
          typename kernel_type>
inline void for_all(loop_type, const index_type *, const index_type &,
                    const index_type &, kernel_type) {
  std::cerr << ("for_all", "ambiguous call!");
}
template <typename policy, typename index_type, typename kernel_type>
inline void for_all(loop_over::index_only, const index_type *ndx_array,
                    const index_type &begin, const index_type &end,
                    kernel_type kernel) {
  ((begin <= end) ? static_cast<void>(0)
                  : __assert_fail("begin <= end",
                                  "/home/liao6/workspace/raja/example/"
                                  "loopsuite.git/raja_api/core/for_all.hxx",
                                  94, __PRETTY_FUNCTION__));
  ((ndx_array != __null)
       ? static_cast<void>(0)
       : __assert_fail("ndx_array != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/core/for_all.hxx",
                       95, __PRETTY_FUNCTION__));

  RAJA::forall<policy_traits<policy>::raja_policy>(
      begin, end, [=](int i) { kernel(ndx_array[i]); });
}
template <typename policy, typename index_type, typename kernel_type>
inline void for_all(loop_over::index, const index_type *ndx_array,
                    const index_type &begin, const index_type &end,
                    kernel_type kernel) {
  ((begin <= end) ? static_cast<void>(0)
                  : __assert_fail("begin <= end",
  ...
  RAJA::forall<policy_traits<policy>::raja_policy>(
      begin, end, [=](int i) { kernel(ndx_array[i], i); });
}
...
namespace core {
template <typename policy, typename loop_type, typename kernel_type>
inline void for_all_nodes(Domain_t *, loop_type, kernel_type) {
  std::cerr << "for_all_nodes", "ambiguous call!\n";
}
template <typename policy, typename kernel_type>
inline void for_all_nodes(Domain_t *domain, loop_over::node_index_only,
                          kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/core/for_all_nodes.hxx",
                       79, __PRETTY_FUNCTION__));

  RAJA::IndexSet real_nodes;
  RajaIndexSetBuilder::buildRealNodesIndexSet(domain, real_nodes);

  RAJA::forall<policy_traits<policy>::raja_indexset_policy>(real_nodes, kernel);
}
template <typename policy, typename kernel_type>
inline void for_all_nodes(Domain_t *domain, loop_over::ij, kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
...
template <typename policy, typename kernel_type>
inline void for_all_nodes(Domain_t *domain, loop_over::ijk,
                          kernel_type kernel) {
  ((domain->m_dimension == 3)
       ? static_cast<void>(0)
       : __assert_fail("domain->m_dimension==3",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/core/for_all_nodes.hxx",
...
template <typename policy, typename kernel_type>
inline void for_all_nodes(Domain_t *domain, loop_over::index,
                          kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
  ...
  RAJA::forall_Icount<policy_traits<policy>::raja_indexset_policy>(
      real_nodes, [=](int icount, int nodeIdx) { kernel(nodeIdx, icount); });
}
...
namespace core {
template <typename policy, typename loop_type, typename kernel_type>
inline void for_all_zones(Domain_t *, loop_type, kernel_type) {
  std::cerr << "for_all_zones", "ambiguous call!";
}
template <typename policy, typename kernel_type>
inline void for_all_zones(Domain_t *domain, loop_over::zone_index_only,
                          kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/core/for_all_zones.hxx",
                       83, __PRETTY_FUNCTION__));

  RAJA::IndexSet real_zones;
  RajaIndexSetBuilder::buildRealZonesIndexSet(domain, real_zones);

  RAJA::forall<typename policy_traits<policy>::raja_indexset_policy>(real_zones,
                                                                     kernel);
}
template <typename policy, typename kernel_type>
inline void for_all_zones(Domain_t *domain, loop_over::ij, kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
...
template <typename policy, typename kernel_type>
inline void for_all_zones(Domain_t *domain, loop_over::ijk,
                          kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
...
template <typename policy, typename kernel_type>
inline void for_all_zones(Domain_t *domain, loop_over::index,
                          kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
  ...
  RAJA::forall_Icount<policy_traits<policy>::raja_indexset_policy>(
      real_zones, [=](int icount, int zoneIdx) { kernel(zoneIdx, icount); });
}
...
namespace core {
template <typename policy, typename loop_type, typename kernel_type>
inline void for_all_mixed_slots(loop_type, Domain_t *, int, kernel_type) {
...
inline void for_all_mixed_slots(loop_over::zone_and_slot_index,
  ...
  ((domain != __null)
       ...
       : __assert_fail("domain != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/core/for_all_mixed_slots.hxx",
                       87, __PRETTY_FUNCTION__));
  ((ir >= 1 && ir <= domain->m_nreg)
       ? static_cast<void>(0)
       : __assert_fail("ir >=1 && ir <= domain->m_nreg",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/core/for_all_mixed_slots.hxx",
  ...
  RAJA::forall<typename policy_traits<policy>::raja_policy>(
      0, rlenmix, [=](int i) { kernel(rgrdmix[i], rndxmix[i]); });
}
template <typename policy, typename kernel_type>
inline void for_all_mixed_slots(loop_over::slot_index_only, Domain_t *domain,
                                int ir, kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
  ...
  const int rlenmix = domain->m_rlenmix[ir];
  RAJA::forall<policy_traits<policy>::raja_policy>(
      0, rlenmix, [=](int i) { kernel(rndxmix[i]); });
}
template <typename policy, typename kernel_type>
inline void for_all_mixed_slots(loop_over::index, Domain_t *domain, int ir,
                                kernel_type kernel) {
  ((domain != __null)
       ? static_cast<void>(0)
       : __assert_fail("domain != __null",
  ...
  const int rlenmix = domain->m_rlenmix[ir];
  RAJA::forall<policy_traits<policy>::raja_policy>(
      0, rlenmix, [=](int i) { kernel(rndxmix[i], i); });
}
...
namespace loopsuite {
template <typename policy, typename index_type, typename kernel_type>
inline void for_all(const index_type &begin, const index_type &end,
                    kernel_type kernel) {
  ((begin <= end) ? static_cast<void>(0)
                  : __assert_fail("begin <= end",
                                  "/home/liao6/workspace/raja/example/"
                                  "loopsuite.git/raja_api/RajaAPI.h",
                                  68, __PRETTY_FUNCTION__));
  ((policy_traits<policy>::valid)
       ? static_cast<void>(0)
       : __assert_fail("policy_traits< policy >::valid",
                       "/home/liao6/workspace/raja/example/loopsuite.git/"
                       "raja_api/RajaAPI.h",
...
template <typename policy, typename loop_type = loop_over::index_only,
          typename index_type, typename kernel_type>
inline void for_all(const index_type *ndx_array, const index_type &begin,
                    const index_type &end, kernel_type kernel) {
  ((begin <= end) ? static_cast<void>(0)
                  : __assert_fail("begin <= end",
                                  "/home/liao6/workspace/raja/example/"
                                  "loopsuite.git/raja_api/RajaAPI.h",
                                  126, __PRETTY_FUNCTION__));
  ((ndx_array != __null) ? static_cast<void>(0)
                         : __assert_fail("ndx_array != __null",
                                         "/home/liao6/workspace/raja/example/"
                                         "loopsuite.git/raja_api/RajaAPI.h",
                                         127, __PRETTY_FUNCTION__));

  core::for_all<policy, index_type, kernel_type>(loop_type(), ndx_array, begin,
                                                 end, kernel);
}
template <typename policy, typename loop_type = loop_over::zone_index_only,
          typename kernel_type>
inline void for_all_zones(Domain_t *domain, kernel_type kernel) {
  ((domain != __null) ? static_cast<void>(0)
                      : __assert_fail("domain != __null",
                                      "/home/liao6/workspace/raja/example/"
                                      "loopsuite.git/raja_api/RajaAPI.h",
                                      195, __PRETTY_FUNCTION__));
  core::for_all_zones<policy, kernel_type>(domain, loop_type(), kernel);
}
template <typename policy, typename kernel_type>
inline void for_all_mixed_zones(Domain_t *domain, kernel_type kernel) {}
template <typename policy, typename loop_type = loop_over::node_index_only,
          typename kernel_type>
...
template <typename policy, typename kernel_type>
inline void for_all_region_zones(Domain_t *domain, int ir, kernel_type kernel) {
}
...
PdVWorkRajaAPI::~PdVWorkRajaAPI() { this->tearDown(); }
