...
// Reference member functions (class and member function names) that are
// specific to the DSL.
extern std::vector<std::pair<std::string, std::string>>
    dsl_member_function_names;

class AstAttribute {};

// Example attributes that we need to have generated to support array DSL
// example.
...
// References to dsl attributes in a map inexed by the name of the dsl
// abstraction.
...
public:
  // Depending on the compile time semantics we want to leverage we may not need
  // an object here.
  array value;
};
...
// }; std::vector<std::pair<std::string,dsl_attribute*> > dsl_attribute_list = {
// {"array", new array_dsl_attribute()} };
// std::vector<std::pair<std::string,dsl_attribute> > dsl_attribute_list = {
// {"array", array_dsl_attribute()} };
