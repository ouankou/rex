...
//! start from a CFG node 'n', collect all other CFG nodes which can be reached
//! from 'n'
void getReachableNodes(CFGNode n, set<CFGNode> &s) {
  if (s.find(n) != s.end())
    return; // n is already in s
  s.insert(n);
...
void testCFG(SgFunctionDefinition *stmt) {
  ...
  // Dump graph of CFG
  {
    ...
    string file_func_name =
        file_name + "_" + stmt->get_mangled_name().getString();
  ...
  for (set<CFGNode>::const_iterator i = nodes.begin(); i != nodes.end(); ++i) {
    ...
    for (vector<CFGEdge>::const_iterator j = forwardEdgesFromN.begin();
      ...
      if (std::find(backwardEdgesFromN.begin(), backwardEdgesFromN.end(), *j) ==
          backwardEdgesFromN.end()) {
        cerr << "Found edge in forward set which is not in backward set: "
             << j->source().toStringForDebugging() << " -> "
             << j->target().toStringForDebugging() << " "
             << j->toStringForDebugging() << endl;
        anyMismatches = true;
      }
    }
    for (vector<CFGEdge>::const_iterator j = backwardEdgesFromN.begin();
         j != backwardEdgesFromN.end(); ++j) {
      if (std::find(forwardEdgesFromN.begin(), forwardEdgesFromN.end(), *j) ==
          forwardEdgesFromN.end()) {
        cerr << "Found edge in backward set which is not in forward set: "
             << j->source().toStringForDebugging() << " -> "
             << j->target().toStringForDebugging() << " "
             << j->toStringForDebugging() << endl;
        anyMismatches = true;
...
int main(int argc, char *argv[]) {
  ...
  NodeQuerySynthesizedAttributeType functions =
      NodeQuery::querySubTree(sageProject, V_SgFunctionDefinition);
  for (NodeQuerySynthesizedAttributeType::const_iterator i = functions.begin();
       i != functions.end(); ++i) {
