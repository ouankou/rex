// ngx_http_request_combined.c

# 1 "src/http/ngx_http_request.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "src/http/ngx_http_request.c"
...
# 9 "src/http/ngx_http_request.c" 2
...
# 10 "src/http/ngx_http_request.c" 2
...
# 11 "src/http/ngx_http_request.c" 2
...
ngx_http_header_t ngx_http_headers_in[] = {
    { { sizeof("Host") - 1, (u_char *) "Host" }, __builtin_offsetof (ngx_http_headers_in_t, host),
                 ngx_http_process_host },

    { { sizeof("Connection") - 1, (u_char *) "Connection" }, __builtin_offsetof (ngx_http_headers_in_t, connection),
                 ngx_http_process_connection },

    { { sizeof("If-Modified-Since") - 1, (u_char *) "If-Modified-Since" },
                 __builtin_offsetof (ngx_http_headers_in_t, if_modified_since),
                 ngx_http_process_unique_header_line },

    { { sizeof("If-Unmodified-Since") - 1, (u_char *) "If-Unmodified-Since" },
                 __builtin_offsetof (ngx_http_headers_in_t, if_unmodified_since),
                 ngx_http_process_unique_header_line },

    { { sizeof("User-Agent") - 1, (u_char *) "User-Agent" }, __builtin_offsetof (ngx_http_headers_in_t, user_agent),
                 ngx_http_process_user_agent },

    { { sizeof("Referer") - 1, (u_char *) "Referer" }, __builtin_offsetof (ngx_http_headers_in_t, referer),
                 ngx_http_process_header_line },

    { { sizeof("Content-Length") - 1, (u_char *) "Content-Length" },
                 __builtin_offsetof (ngx_http_headers_in_t, content_length),
                 ngx_http_process_unique_header_line },

    { { sizeof("Content-Type") - 1, (u_char *) "Content-Type" },
                 __builtin_offsetof (ngx_http_headers_in_t, content_type),
                 ngx_http_process_header_line },

    { { sizeof("Range") - 1, (u_char *) "Range" }, __builtin_offsetof (ngx_http_headers_in_t, range),
                 ngx_http_process_header_line },

    { { sizeof("If-Range") - 1, (u_char *) "If-Range" },
                 __builtin_offsetof (ngx_http_headers_in_t, if_range),
                 ngx_http_process_unique_header_line },

    { { sizeof("Transfer-Encoding") - 1, (u_char *) "Transfer-Encoding" },
                 __builtin_offsetof (ngx_http_headers_in_t, transfer_encoding),
                 ngx_http_process_header_line },

    { { sizeof("Expect") - 1, (u_char *) "Expect" },
                 __builtin_offsetof (ngx_http_headers_in_t, expect),
                 ngx_http_process_unique_header_line },


    { { sizeof("Accept-Encoding") - 1, (u_char *) "Accept-Encoding" },
                 __builtin_offsetof (ngx_http_headers_in_t, accept_encoding),
                 ngx_http_process_header_line },

    { { sizeof("Via") - 1, (u_char *) "Via" }, __builtin_offsetof (ngx_http_headers_in_t, via),
                 ngx_http_process_header_line },


    { { sizeof("Authorization") - 1, (u_char *) "Authorization" },
                 __builtin_offsetof (ngx_http_headers_in_t, authorization),
                 ngx_http_process_unique_header_line },

    { { sizeof("Keep-Alive") - 1, (u_char *) "Keep-Alive" }, __builtin_offsetof (ngx_http_headers_in_t, keep_alive),
                 ngx_http_process_header_line },


    { { sizeof("X-Forwarded-For") - 1, (u_char *) "X-Forwarded-For" },
                 __builtin_offsetof (ngx_http_headers_in_t, x_forwarded_for),
                 ngx_http_process_header_line },
# 176 "src/http/ngx_http_request.c"
    { { sizeof("Cookie") - 1, (u_char *) "Cookie" }, 0, ngx_http_process_cookie },
...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    if (rev->ready) {
        ...
        if (ngx_use_accept_mutex) {
            ; if (rev->prev == ((void *)0)) { rev->next = (ngx_event_t *) *&ngx_posted_events; rev->prev = (ngx_event_t **) &ngx_posted_events; *&ngx_posted_events = rev; if (rev->next) { rev->next->prev = &rev->next; } ; } else { ; }; ;;
            return;
        }
...
# 262 "src/http/ngx_http_request.c"
    c = rev->data;

    if (rev->timedout) {
        if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 110, "client timed out");
    ...
    if (r) {
        (void) memset(r, 0, sizeof(ngx_http_request_t));
    ...
    if (port->naddrs > 1) {
        ...
        if (ngx_connection_local_sockaddr(c, ((void *)0), 0) != 0) {
            ngx_http_close_connection(c);
            return;
        }

        switch (c->local_sockaddr->sa_family) {
# 350 "src/http/ngx_http_request.c"
        default:
        ...
        switch (c->local_sockaddr->sa_family) {
# 379 "src/http/ngx_http_request.c"
        default:
            addr = port->addrs;
...
# 434 "src/http/ngx_http_request.c"
    clcf = (r)->loc_conf[ngx_http_core_module.ctx_index];
    c->log->file = clcf->error_log->file;
    if (!(c->log->log_level & 0x80000000)) {
...
# 707 "src/http/ngx_http_request.c"
static void
    ...
    for ( ;; ) {
        ...
        if (rc == 0) {
            ...
            if (r->complex_uri || r->quoted_uri) {
                ...
                if (rc == 11) {
                    if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent invalid request");
...
# 864 "src/http/ngx_http_request.c"
            ;
            ...
            if (r->host_start && r->host_end) {
                ...
                if (n == 0) {
                    if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent invalid host in request line");
        ...
        if (rc != -2) {
            ...
            if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, ngx_http_client_errors[rc - 10]);
        ...
        if (r->header_in->pos == r->header_in->end) {
            ...
            if (rv == -5) {
                ...
                if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent too long URI");

                ngx_http_finalize_request(r, 414);
                return;
            }
        }
    }
    ...
    for ( ;; ) {
        ...
        if (rc == -2) {
            ...
            if (r->header_in->pos == r->header_in->end) {
                ...
                if (rv == -5) {
                    ...
                    if (p == ((void *)0)) {
                        if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent too large request");
                    ...
                    if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent too long header line: \"%*s\"", len, r->header_name_start);
        ...
        if (rc == 0) {
            ...
            if (r->invalid_header && cscf->ignore_invalid_headers) {
                ...
                if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent invalid header line: \"%*s\"", r->header_end - r->header_name_start, r->header_name_start);
        ...
        if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent invalid header line: \"%*s\\r...\"", r->header_end - r->header_name_start, r->header_name_start);
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (n == 0) {
        if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client prematurely closed connection");
    ...
    ngx_uint_t request_line)
{
    ...
    (void) memcpy(new, old, r->header_in->pos - old);
    ...
    ngx_uint_t offset)
{
    ...
    if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent duplicate header line: \"%V: %V\", " "previous value: \"%V: %V\"", &h->key, &h->value, &(*ph)->key, &(*ph)->value);
    ...
    ngx_uint_t offset)
{
    ...
    if (len == 0) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent invalid host header");
...
ngx_http_process_request_header(ngx_http_request_t *r)
{
    ...
    if (r->headers_in.host == ((void *)0) && r->http_version > 1000) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent HTTP/1.1 request without \"Host\" header");
    ...
    if (r->headers_in.content_length) {
        ...
        if (r->headers_in.content_length_n == -1) {
            if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent invalid \"Content-Length\" header");

            ngx_http_finalize_request(r, 411);
            return -1;
        }
    }

    if (r->method & 0x0010 && r->headers_in.content_length_n == -1) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent %V method without \"Content-Length\" header", &r->method_name);
    ...
    if (r->method & 0x8000) {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent TRACE method");

        ngx_http_finalize_request(r, 405);
        return -1;
    }
    ...
    if (r->headers_in.transfer_encoding
        && ngx_strcasestrn(r->headers_in.transfer_encoding->value.data,
                           "chunked", 7 - 1))
    {
        if ((r->connection->log)->log_level >= 7) ngx_log_error_core(7, r->connection->log, 0, "client sent \"Transfer-Encoding: chunked\" header");
...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    if (r->plain_http) {
        if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, 0, "client sent plain HTTP request to HTTPS port");

        ngx_http_finalize_request(r, 497);
        return;
    }
# 1670 "src/http/ngx_http_request.c"
    if (c->read->timer_set) {
        ngx_event_del_timer(c->read);
    }
# 1681 "src/http/ngx_http_request.c"
...
found:
    ...
    if (!(r->connection->log->log_level & 0x80000000)) {
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (r != r->main) {
        ...
        if (r->buffered || r->postponed) {
            ...
            if (ngx_http_set_write_handler(r) != 0) {
        ...
        if (r == c->data) {
            ...
            if (!r->logged) {
                ...
                if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, 0, "subrequest: \"%V?%V\" logged again", &r->uri, &r->args);
        ...
        if (ngx_http_post_request(pr, ((void *)0)) != 0) {
            r->main->count++;
            ngx_http_terminate_request(r, 0);
            return;
        }
    ...
    if (r != c->data) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, 0, "http finalize non-active request: \"%V?%V\"", &r->uri, &r->args);
...
ngx_http_finalize_connection(ngx_http_request_t *r)
{
    ...
    if (r->main->count != 1) {
        ...
        if (r->discard_body) {
            ...
            if (r->lingering_time == 0) {
                r->lingering_time = ngx_cached_time->sec
    ...
    if (clcf->lingering_close == 2
        || (clcf->lingering_close == 1
            && (r->lingering_close
...
ngx_http_set_write_handler(ngx_http_request_t *r)
{
    ...
    if (ngx_handle_write_event(wev, clcf->send_lowat) != 0) {
        ngx_http_close_request(r, 0);
        return -1;
    }
...
ngx_http_writer(ngx_http_request_t *r)
{
    ...
    clcf = (r->main)->loc_conf[ngx_http_core_module.ctx_index];
    ...
    if (wev->timedout) {
        if (!wev->delayed) {
            ...
            ngx_http_finalize_request(r, 408);
            return;
        }
...
ngx_http_block_reading(ngx_http_request_t *r)
{
    ...
    if ((ngx_event_flags & 0x00000001)
        && r->connection->read->active)
    {
        if (ngx_event_actions.del(r->connection->read, EPOLLIN, 0) != 0) {
            ngx_http_close_request(r, 0);
        }
    }
}
...
ngx_http_test_reading(ngx_http_request_t *r)
{
    ...
    ;
# 2401 "src/http/ngx_http_request.c"
    ...
    if ((ngx_event_flags & 0x00000001) && rev->active) {

        if (ngx_event_actions.del(rev, EPOLLIN, 0) != 0) {
            ngx_http_close_request(r, 0);
        }
    }
...
closed:
    ...
    if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, err, "client prematurely closed connection");
...
ngx_http_set_keepalive(ngx_http_request_t *r)
{
    ...
    if (r->discard_body) {
        r->write_event_handler = ngx_http_request_empty_handler;
        r->lingering_time = ngx_cached_time->sec + (time_t) (clcf->lingering_time / 1000);
    ...
    if (b->pos < b->last) {
        ...
        if (b != c->buffer) {
# 2490 "src/http/ngx_http_request.c"
            cscf = (r)->srv_conf[ngx_http_core_module.ctx_index];
    ...
    hc->pipeline = 0;
# 2556 "src/http/ngx_http_request.c"
    if (ngx_pfree(c->pool, r) == 0) {
        hc->request = ((void *)0);
    }

    b = c->buffer;

    if (ngx_pfree(c->pool, b->start) == 0) {
    ...
    if (wev->active && (ngx_event_flags & 0x00000001)) {
        if (ngx_event_actions.del(wev, EPOLLOUT, 0) != 0) {
            ngx_http_close_connection(c);
            return;
        }
    }
    ...
    if (c->tcp_nopush == NGX_TCP_NOPUSH_SET) {
        if (ngx_tcp_push(c->fd) == -1) {
            ngx_connection_error(c, (*__errno_location ()), "setsockopt(!TCP_CORK)" " failed");
            ngx_http_close_connection(c);
            return;
        }
...
# 2708 "src/http/ngx_http_request.c"
    b = c->buffer;
    size = b->end - b->start;
    ...
    if (n == 0) {
        if ((c->log)->log_level >= 7) ngx_log_error_core(7, c->log, (*__errno_location ()), "client %V closed keepalive connection", &c->addr_text);
...
ngx_http_lingering_close_handler(ngx_event_t *rev)
{
    ...
    timer = (ngx_msec_t) (r->lingering_time - ngx_cached_time->sec);
    if (timer <= 0) {
        ngx_http_close_request(r, 0);
        return;
    }
...
ngx_http_close_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (r->count == 0) {
        if ((c->log)->log_level >= 2) ngx_log_error_core(2, c->log, 0, "http request count is zero");
    }
...
ngx_http_free_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (r->pool == ((void *)0)) {
        if ((log)->log_level >= 2) ngx_log_error_core(2, log, 0, "http request already closed");
        return;
    }
...
# 3031 "src/http/ngx_http_request.c"
    ...
    if (r->connection->timedout) {
        ...
        if (clcf->reset_timedout_connection) {
            ...
            if (setsockopt(r->connection->fd, 1, 13,
                           (const void *) &linger, sizeof(struct linger)) == -1)
            {
                if ((log)->log_level >= 2) ngx_log_error_core(2, log, (*__errno_location ()), "setsockopt(SO_LINGER) failed");
...
ngx_http_close_connection(ngx_connection_t *c)
{
    ...
    ;
# 3110 "src/http/ngx_http_request.c"
    c->destroyed = 1;
    ...
    u_char *buf, size_t len)
{
    ...
    if (r->request_line.data == ((void *)0) && r->request_start) {
        for (p = r->request_start; p < r->header_in->last; p++) {
            if (*p == (u_char) 13 || *p == (u_char) 10) {
                break;
            }
        }
    ...
    if (u && u->peer.name) {
        ...
        if (u->peer.sockaddr && u->peer.sockaddr->sa_family == 1) {
            uri_separator = ":";
        }
