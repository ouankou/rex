/* The following test code is all ANSI C (showing old-style and new-style
function definitions (compiles using gcc -std=c99, for example, but does not
compile with g++). The old-style C represented in the function definitions
finally works in ROSE. Note that the filename suffix must be ".c" and not ".C"
to be compiled with gcc using old-style syntax (not the case for legacy
frontend).

   These tests were prepared as part of the new old-style function declaration
handling in ROSE (required to compile older laboratory application written in
C).
*/
...
/* Note that "float" in place of "double", in foo7, is permitted by GNU
   ...
   onto the stack as a double under GNU (or is it C (old-style only)?). foo7
   case is an error with legacy frontend (but works with GNU gcc).  It seems
   impossible to pass a float using legacy frontend as a C compiler foo6 is an
   error with GNU gcc, and also with legacy frontend.
 */
#ifdef DEMO_ERROR_CASE
/* Example compiles fine with GNU gcc:
   Error in legacy frontend (ROSE): declaration is incompatible with "int
   foo7(int, float, int *)" This is only an error for the old-style function
   declarations (passing a float with the new-style compiles fine (though it
   might still be ignored internally)).
*/
