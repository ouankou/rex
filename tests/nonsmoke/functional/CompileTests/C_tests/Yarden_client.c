...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    printf("usage : %s <server> <tcp threads num> <udp threads num> [optional: "
           "1 = shutdown server]\n",
           argv[0]);
    exit(1);
  }
  ...
  if (tcp_threads > 0) {
    ...
    for (i = 0; i < tcp_threads; i++) {
      if (pthread_create(&tcp_threadVec[i], NULL, thread_func_tcp,
                         (void *)(i + 1)) != 0) {
  ...
  if (udp_threads > 0) {
    ...
    for (i = 0; i < udp_threads; i++) {
      if (pthread_create(&udp_threadVec[i], NULL, thread_func_udp,
                         (void *)(i + 1)) != 0) {
        printf("failed while creating threads\n");
        exit(1);
      }
  ...
  if (argc >= 5) {
    ...
    if (!strcmp(argv[4], "1")) {
      ...
      for (j = 0; j < 10; j++) {
        rc = sendto(sd, "quit", strlen("quit") + 1, 0,
...
void *thread_func_udp(void *p) {
  ...
  for (j = i; j < i + 100; j++) {
    sprintf(pNum, "%d", j);
    rc = sendto(sd, pNum, strlen(pNum) + 1, 0,
                (struct sockaddr *)&remoteServAddr, sizeof(remoteServAddr));
    if (rc < 0) {
...
void *thread_func_tcp(void *p) {
  ...
  memcpy((char *)&remoteTCPServAddr.sin_addr.s_addr, h->h_addr_list[0],
         h->h_length);
  remoteTCPServAddr.sin_port = htons(REMOTE_SERVER_TCP_PORT);
  //  printf("before connect \n");
  rc = connect(stcp, (struct sockaddr *)&remoteTCPServAddr,
               sizeof(remoteTCPServAddr));
  if (rc != 0) {
