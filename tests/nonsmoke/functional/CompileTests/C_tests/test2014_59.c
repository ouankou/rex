...
// DQ: Note that this will build the names of variables (data members) based on
// the line number.
...
// DQ: Note that this will reference the names of variables based on the line
// number.
static const gint16 opidx[] = {
#define WRAPPER(a, b)                                                          \
  [MONO_WRAPPER_##a] = offsetof(struct msgstr_t, MSGSTRFIELD(__LINE__)),
#if ROSE_USE_INCLUDES
#include <wrapper-types.h>
#else

    // DQ: I had to preprocess the original file to get the source code that
    // would have matching line numbers!
    ...
    [MONO_WRAPPER_DELEGATE_BEGIN_INVOKE] =
        __builtin_offsetof(struct msgstr_t, str8),
    [MONO_WRAPPER_DELEGATE_END_INVOKE] =
        __builtin_offsetof(struct msgstr_t, str9),
    [MONO_WRAPPER_RUNTIME_INVOKE] = __builtin_offsetof(struct msgstr_t, str10),
    [MONO_WRAPPER_NATIVE_TO_MANAGED] =
        __builtin_offsetof(struct msgstr_t, str11),
    [MONO_WRAPPER_MANAGED_TO_NATIVE] =
        __builtin_offsetof(struct msgstr_t, str12),
    [MONO_WRAPPER_MANAGED_TO_MANAGED] =
        __builtin_offsetof(struct msgstr_t, str13),
    [MONO_WRAPPER_REMOTING_INVOKE] = __builtin_offsetof(struct msgstr_t, str14),
    [MONO_WRAPPER_REMOTING_INVOKE_WITH_CHECK] =
        __builtin_offsetof(struct msgstr_t, str15),
    [MONO_WRAPPER_XDOMAIN_INVOKE] = __builtin_offsetof(struct msgstr_t, str16),
    [MONO_WRAPPER_XDOMAIN_DISPATCH] =
        __builtin_offsetof(struct msgstr_t, str17),
    ...
    [MONO_WRAPPER_COMINTEROP_INVOKE] =
        __builtin_offsetof(struct msgstr_t, str32),
