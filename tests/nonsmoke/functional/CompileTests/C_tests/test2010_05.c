/* Example failing code reported 7/18/2010): by King
   ...
   (actually legacy frontend) catches it at compile time.
   ...
   legacy frontend is detecting this problem, though for this single
   ...
   definition of a modifiable object with static storage duration, and share not
   contain a reference to an identifier with internal linkage." The problem here
   is that "hash" is the name of an internal linkage function within "test"
   which is an external linage inlined function. Thus legacy frontend catches
   the problem, and GNU gcc does not.
 */
