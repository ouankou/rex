...
#include <$header>\nint main(){ return 0; }\n" > ${TEST_HEADER_C}
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "${ASAN_OPTIONS} $TOOL2 $TOOL2_BACKEND $BS_INCLUDE ${TEST_HEADER_PP_C} -std=$LANG_STANDARD" >> $LOGFILE
        {
            ...
            $TOOL2 $TOOL2_BACKEND $BS_INCLUDE  ${TEST_HEADER_PP_C} -std=$LANG_STANDARD
        } >> $LOGFILE 2>&1
        if [ $? -eq 0 ]; then
            ...
            then
                ...
                echo -en " ${RED}FAIL${COLOREND} "
                echo -en "[no file generated by: $ASAN_OPTIONS $TOOL2 $TOOL2_BACKEND $BS_INCLUDE  ${TEST_HEADER_PP_C} -std=$LANG_STANDARD ]"
                ((T1_FAIL+=1))
            fi
        else
            echo -en "${RED}FAIL ----${COLOREND}" # 1
            # reinvoke the rose tool to show the failing front end issue
            echo -en " [ $ASAN_OPTIONS $TOOL2 $TOOL2_BACKEND $BS_INCLUDE  ${TEST_HEADER_PP_C} -std=$LANG_STANDARD ]"
            ((T1_FAIL+=1))
        fi
    else
