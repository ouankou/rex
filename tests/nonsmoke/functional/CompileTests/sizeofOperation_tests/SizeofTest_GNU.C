...
void checkEqual(ValueType const &computedSize, ValueType const &directSize) {
  ...
  if (isSgSizeOfOp(currentNode) != NULL) {
    ...
    if (isSgValueExp(parent) != NULL) {
      ...
      if (isSgUnsignedIntVal(parent) != NULL) {
        ...
        unsigned long long computedSize =
            ((SgUnsignedLongLongIntVal *)parent)->get_value();
        unsigned long long directSize;
        fscanf(fp, "%llu", &directSize);
        checkEqual(computedSize, directSize);
      } else {
        ROSE_ASSERT(false &&
                    "SgSizeOfOp is folded into an unhandled value type");
      }
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  system("rm -f test");
  // Run the input file first to get all the sizeof results and write them into
  // a file.
  ...
  // Read the file serially and compare each sizeof result with the value in the
  // extra IR node.
  fp = fopen("test", "r");
  if (fp == NULL) {
