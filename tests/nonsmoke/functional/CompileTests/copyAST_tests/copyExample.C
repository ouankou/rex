...
// DQ (2/26/2009): The copyGraph.[hC] files have been moved to
// src/midend/astDump. directory and are not included in librose.so. #include
// "copyGraph.C"

// DQ (12/31/2005): This is OK if not declared in a header file
using namespace std;

static void requireExactExplicitInstantiationOwners(SgNode *root) {
  ROSE_ASSERT(root != NULL);
  Rose_STL_Container<SgNode *> directives = NodeQuery::querySubTree(
      root, V_SgTemplateInstantiationDirectiveStatement);
  for (SgNode *node : directives) {
    ...
    size_t edges = 0;
    for (const auto &successor : directive->returnDataMemberPointers()) {
...
SgNode *copyAST(SgNode *node) {
  ...
  // This triggers a bug with test2005_152.C (the unparsed code fails for
  // g++ 4.1.2, but not 3.5.6)
  ...
  if (SgProject::get_verbose() > 0)
    printf(
        "Running the AST Post Processing Phase on the new copy of the AST! \n");

  AstPostProcessing(copyOfNode);
  requireExactExplicitInstantiationOwners(copyOfNode);

  if (SgProject::get_verbose() > 0)
    printf("DONE: Running the AST Post Processing Phase on the new copy of the "
           "AST! \n");

  if (SgProject::get_verbose() > 0)
    printf("\n\nCompare two generated ASTs ... \n");

  std::vector<SgNode *> intersectionNodeList =
      SageInterface::astIntersection(node, copyOfNode, &restrictedCopyType);
...
void printOutTemplateDeclarations() {
  // Debugging support
  ...
  public:
    ...
    void visit(SgNode *node) {
      ...
      if (derivedDeclaration != NULL) {
        printf("case V_SgTemplateDeclaration: derivedDeclaration            "
               "   = %p \n",
               derivedDeclaration);
        printf("case V_SgTemplateDeclaration: "
               "derivedDeclaration->get_parent() = %p = %s \n",
               derivedDeclaration->get_parent(),
               derivedDeclaration->get_parent()->class_name().c_str());
        printf("case V_SgTemplateDeclaration: "
               "derivedDeclaration->get_scope()  = %p = %s \n",
               derivedDeclaration->get_scope(),
               derivedDeclaration->get_scope()->class_name().c_str());
        printf("case V_SgTemplateDeclaration: "
               "derivedDeclaration->get_declarationModifier().isFriend() = "
               "%s \n",
               derivedDeclaration->get_declarationModifier().isFriend()
                   ? "true"
                   : "false");
        printf("case V_SgTemplateDeclaration: "
               "derivedDeclaration->get_name()   = %s \n",
               derivedDeclaration->get_name().str());
        printf("case V_SgTemplateDeclaration: "
               "derivedDeclaration->get_template_kind() = %d \n",
               static_cast<int>(derivedDeclaration->get_template_kind()));
      }
    }
  ...
  printf("Friend template function declarations are not properly marked as "
         "friends \n");
...
int main(int argc, char *argv[]) {
  ...
  // DQ (7/7/2005): Only output the performance report if verbose is set
  // (greater than zero)
  if (project->get_verbose() > 0) {
    // Output any saved performance data (see
    // ROSE/src/astDiagnostics/AstPerformance.h)
