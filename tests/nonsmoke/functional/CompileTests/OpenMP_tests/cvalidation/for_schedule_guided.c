...
#define CFSMAX_SIZE                                                            \
  150 /*choose small iteration space for small sync. overhead*/
#define MAX_TIME 5
...
#pragma omp critical
      ...
      /* if it is not our turn we wait
         ...
         b) we are at the end of the loop (first thread finished and set
         notout=0 OR c) timeout arrived */
...
int crosscheck_for_schedule_guided(FILE *logFile) {
  ...
  int tid;
/*
Since it takes quite long to finish the check_x(), I skip the cross_check_X()
here. Liao
*/
#pragma omp parallel
  {
