...
#pragma omp master
  ...
  for (iter = 0; iter < 100; iter++) {
    my_random =
        rand(); /* random number generator is called inside serial region */

    /* the first parallel region is used to initialiye myvalue and the array
     * with my_random+rank */
#pragma omp parallel
    {
      int rank;
    ...
    /* the second parallel region verifies that the value of "myvalue" is
     * retained */
