...
- `rodinia_gaussian_like`: three-kernel lowering shape with direct `__tgt_target_kernel` launch and `collapse(2)`.
...
- `rodinia_nn_like`: single-kernel map-list lowering integrity and
  `rex_offload_init()` ordering before timed declarations, without automatic
  `rex_offload_fini()` insertion at process exit.
