...
double tracepath(int tsb1, int tsb2, int *print_ptr, int *last_print,
                 int *displ, int seq1, int seq2);
...
#pragma omp task untied private(i, gg, len2, mm_score)                         \
    firstprivate(m, n, si, sj, len1)                                           \
    shared(nseqs, bench_output, seqlen_array, seq_array, gap_pos1, gap_pos2,   \
               pw_ge_penalty, pw_go_penalty, mat_avscore)
            {
              ...
              mm_score = tracepath(sb1, sb2, &print_ptr, &last_print, displ,
                                   seq1, seq2);
