...
double diff_ratio(double val, double ref, int significant_digits) {
  ...
  double upper_limit =
      pow(0.1, significant_digits); // 1.0/(double(10^significant_digits)) ;
  printf("value :%E  ref_value: %E  diff_ratio: %E upper_limit: %E \n", val,
         ref, diff_ratio, upper_limit);
...
// Must split the omp for into two parallel for regions since the translation
// focuses on parallel to generate the outlined kernel We need two CUDA kernels
// for implementing global synchronization so we have to have two omp parallel
// directives!!
// #pragma omp target map(to:n, m, omega, ax, ay, u[0:n][0:m],f[0:n][0:m])
// map(alloc:uold[0:n][0:m]) #pragma omp parallel
//    {
#pragma omp target map(to : n, m, u[0 : n][0 : m])                             \
    map(from : uold[0 : n][0 : m])
#pragma omp parallel for private(j, i)
    for (i = 0; i < n; i++)
      for (j = 0; j < m; j++)
        uold[i][j] = u[i][j];

#pragma omp target map(to : n, m, omega, ax, ay, b, f[0 : n][0 : m],           \
                           uold[0 : n][0 : m]) map(from : u[0 : n][0 : m])
