// Using target data to promote data allocation to higher level, enabling
// reusing in iterations
#include <assert.h>
...
void jacobi() {
  ...
  // An optimization on top of naive coding: promoting data handling outside the
  // while loop data properties may change since the scope is bigger:
#pragma omp target data map(to : n, m, omega, ax, ay, b, f[0 : n][0 : m])      \
    map(tofrom : u[0 : n][0 : m]) map(alloc : uold[0 : n][0 : m])
