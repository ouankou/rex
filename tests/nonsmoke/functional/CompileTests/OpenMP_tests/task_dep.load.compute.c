...
//
//  load a[0] -> compute a[0] -> load a[1] -> compute a[2] -> load a[3] ->
//  compute a[3]
//
...
#pragma omp single
  {
    for (i = 0; i < MYLEN; i++) {
// MUST USE firstprivate(i) here. Otherwise it inherits the shared attribute
// from enclosing region.
// #pragma omp task depend(out:i) firstprivate(i) // wrong, must dep on a[i] to
// have distinct dep pairs.
