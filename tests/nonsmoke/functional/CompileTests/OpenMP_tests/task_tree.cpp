...
// DQ (9/12/2009): GNU g++ 4.4 requires stdio.h so that printf() will be
// defined. Note that this is not required for GNU g++ version 4.3.
#include <stdio.h>
...
public:
  ...
  node(int id) : id(id) {
    left = 0;
    right = 0;
  } // must initialize left and right children, otherwise may cause seg fault!!
