...
REAL error_ref = 9.213041E-04,
     resid_ref =
         2.355794E-08; // depending on MSIZE and precision (double vs. float) !!

void initialize(REAL *u, REAL *uold, REAL *f, REAL *dx, REAL *dy, int m,
                int n) {
...
void jacobi_GPU(REAL *u, REAL *uold, REAL *f, REAL dx, REAL dy, int offset,
                int m, int n, double *error) {
  double omega;
  int i, j, k;
  ...
  // #pragma omp parallel
  {
// #pragma omp target device (tid) map(tofrom: u[offset*m:m*n]) map(to:
// uold[offset*m:m*n],f[offset*m:m*n],m,n, offset,ax,ay,b,omega)
#pragma omp parallel for shared(uold, u, f, offset, ax, ay, b, omega, n)       \
    private(resid, j, i) reduction(+ : err_tmp)
    for (i = offset + 1; i < offset + (n - 1); i++) {
      for (j = 1; j < (m - 1); j++) {
        resid = (ax * (uold[j + (i - 1) * m] + uold[j + (i + 1) * m]) +
                 ay * (uold[j - 1 + i * m] + uold[j + 1 + i * m]) +
                 b * uold[j + i * m] - f[j + i * m]) /
                b;
...
#pragma omp parallel shared(GPU_N, u, uold, f, m, n, error, error_sum)         \
    private(idev)
  //    for (idev = 0; idev < GPU_N; idev++)
  {
    ...
    printf("thread %d working on GPU devices %d with size %d copying data from "
           "y_ompacc with offset %d\n",
           tid, tid, size, offset);
    int k;
    k = 1;
    error_sum = 10.0 * tol;
...
#pragma omp master
  ...
  //    n * n / (1.0e6 * (seq_elapsed)), maxerror(C_seq, C_seq, n));
  printf("OMP For         :  %4f \t\t %4f\t\t\n", omp_for_elapsed,
         2.0 * n * n * n / (1.0e6 * (omp_for_elapsed)));
  printf("ACC For         :  %4f \t\t %4f\t\t\n", acc_elapsed,
         2.0 * n * n * n / (1.0e6 * (acc_elapsed)));
