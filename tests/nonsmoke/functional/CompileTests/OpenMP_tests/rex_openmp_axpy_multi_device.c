...
void axpy_ompacc(REAL *x, REAL *y, int n, REAL a) {
  int i;
  /* this one defines both the target device name and data environment to map
     to, I think here we need mechanism to tell the compiler the device type
     (could be multiple) so that compiler can generate the codes of different
     versions; we also need to let the runtime know what the target device is so
     the runtime will chose the right function to call if the code are generated
...
#pragma omp parallel num_threads(ndev)
  {
    ...
    if (devid <
        remain) { /* each of the first remain dev has one more element */
      partsize = esize + 1;
...
#pragma omp target device(devid) map(tofrom : y[starti : endi])                \
    map(to : x[starti : endi], a, partsize)
