...
class MyInsert : public SgSimpleProcessing {
public:
  void visit(SgNode *astNode);
};

void MyInsert::visit(SgNode *astNode) {
  ...
  if (SgIfStmt *ifst = isSgIfStmt(astNode)) {
    ...
    SgVariableDeclaration *stmt =
        new SgVariableDeclaration(finfo, temporary, new SgTypeInt());
