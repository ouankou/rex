...
int main() {
  ...
  // for (std::vector< int , std::allocator< int >  > ::iterator i =
  // (integerVector.begin()); i__gnu_cxx::!=((integerVector.end())); i++)
  for (vector<int>::iterator i = integerVector.begin();
       i != integerVector.end(); i++) {
    sumOverVector += *i;
  }

  // Try to use a map to draw out error in compiling stl_map.h (error in
  // processing Kull)
  map<int, int> xmap;
