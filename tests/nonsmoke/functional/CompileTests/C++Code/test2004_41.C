...
template <class _Tp, class _Ref, class _Ptr> struct _List_iterator {
...
template <class _Tp, class _Ref, class _Ptr>
inline ptrdiff_t *distance_type(const _List_iterator<_Tp, _Ref, _Ptr> &) {
  return 0;
}
...
template <class _Tp, class _Allocator, bool _IsStatic> class _List_alloc_base {
public:
  typedef
...
template <class _Tp, class _Alloc> void _List_base<_Tp, _Alloc>::clear() {
...
protected:
  _Node *_M_create_node(const _Tp &__x) {
    ...
    __STL_TRY { construct(&__p->_M_data, __x); }
  ...
  _Node *_M_create_node() {
    _Node *__p = _M_get_node();
    __STL_TRY { construct(&__p->_M_data); }
    __STL_UNWIND(_M_put_node(__p));
    return __p;
  }
...
public:
  ...
  friend bool operator== __STL_NULL_TMPL_ARGS(const list &__x, const list &__y);
};
...
template <class _Tp, class _Alloc>
void list<_Tp, _Alloc>::insert(iterator __position, size_type __n,
                               const _Tp &__x) {
  for (; __n > 0; --__n)
...
template <class _Tp, class _Alloc> void list<_Tp, _Alloc>::reverse() {
