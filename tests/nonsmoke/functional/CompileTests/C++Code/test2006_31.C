...
template <class A, class B> struct Add : public Expr<Add<A, B>> {
...
template <class A, class B> struct Mult : public Expr<Mult<A, B>> {
  static inline double Give(int i) { return (A::Give(i) * B::Give(i)); }
};

template <class A, class B>
inline Mult<A, B> operator*(const Expr<A> &a, const Expr<B> &b) {
  return Mult<A, B>();
}

template <int num> class Vector : public Expr<Vector<num>> {
private:
  int n_;
  static double *data_;
...
public:
  ...
  template <class A> inline void operator=(const Expr<A> &a) {
    int N = n_, i = 0;
    double *dd = data_;
    for (i = 0; i < N; ++i)
      dd[i] = A::Give(i);
