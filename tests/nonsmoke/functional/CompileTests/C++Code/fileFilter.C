...
// This program demonstrates the required mechanism for processing the input
// ASCII file within preprocessors built by ROSE.  If it detects illeagal
// characters in the input file then it generates a new file as a backup and
// then overwrites the original file. This could be dangerous, so we might want
// to attempt an alternative approach, later.

// The default behaviour within ROSE preprocessors could be to just quit if
// illegal characters are found in the input code. But this is not a great way
// to handle the expansion of tabs (tabs are not illegal characters, just
// annoying ones).
...
bool fileContainsProblemCharaters(string filename, bool ignoreTabs) {
  ...
  return foundCarriageReturn || foundHorizontalTab || foundVerticalTab ||
         foundBackSpace || foundFormFeed || foundBell;
}

void filterInputFile(string filename, string outputFilename) {
