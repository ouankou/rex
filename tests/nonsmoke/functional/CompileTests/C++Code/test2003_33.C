...
void MyTraversal::visit(SgNode *astNode) {
  ...
  if (bb != NULL) {
    ...
    SgVariableDeclaration *sgdecl =
        new SgVariableDeclaration(finfo, sgnm, ptype);
    bb->prepend_statement(sgdecl);
    // MiddleLevelRewrite::insert(bb,
    // sgdecl->unparseToString(),MidLevelCollectionTypedefs::TopOfCurrentScope);
  } else {
    if (sg != NULL) {
