...
// mechanism
class MySynthesizedAttribute : public HighLevelRewrite::SynthesizedAttribute {
public:
  MySynthesizedAttribute() { generationObjectCreatedInScope = true; };
...
class MyTraversal
    : public HighLevelRewrite::RewriteTreeTraversal<MyInheritedAttribute,
...
public:
  ...
  MySynthesizedAttribute evaluateRewriteSynthesizedAttribute(
      SgNode *astNode, MyInheritedAttribute inheritedAttribute,
...
// Functions required by the tree traversal mechanism
MyInheritedAttribute MyTraversal::evaluateRewriteInheritedAttribute(
    SgNode *astNode, MyInheritedAttribute inheritedAttribute) {
...
MySynthesizedAttribute MyTraversal::evaluateRewriteSynthesizedAttribute(
  ...
  // if(startinf) cout << " start info, file: " << startinf->get_filename() <<
  // ", line:" << startinf->get_line() << endl;
  ...
  case V_SgGlobal: {
    string includeString(
        "#ifndef EMPTYINCLUDE \n #define EMPTYINCLUDE \n #include "
        "\"/home/saebjorn/temp/printDatastructureNEW/emptyInclude.h\" \n "
        "#endif\n");
  ...
  case V_SgBasicBlock: {
    ...
    // queryFindCommentsInScope("//pragma","GenPrintCode_",isSgScopeStatement(astNode));
    ...
    string bottomOfScopeString(
        "//" + classPointerName + "->writeToFileAsGraph(\"" + filename +
    ...
    returnAttribute.insert(astNode, bottomOfScopeString,
                           HighLevelCollectionTypedefs::LocalScope,
                           HighLevelCollectionTypedefs::BottomOfScope);
  }
  }
