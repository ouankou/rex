...
static void printHelp(const char *program_name) {
  std::cout
      << "Feature Vector: This program prints out the feature\n"
...
void nodeTraversal::getNamedTypeInfo(SgNamedType *namedType) {
  ...
  SgTemplateInstantiationDecl *templateInstDecl =
      isSgTemplateInstantiationDecl(namedType->get_declaration());
  if (templateInstDecl) {
    SgTemplateArgumentPtrList argPtrList =
        templateInstDecl->get_templateArguments();
    // cout << "have a class decl:" << argPtrList.size() << endl;
    for (SgTemplateArgumentPtrList::iterator tempArgId = argPtrList.begin();
         tempArgId != argPtrList.end(); ++tempArgId) {
      SgTemplateArgument *tempArg = *tempArgId;
      // cout << "template argument type: " << tempArg->get_argumentType() << "
      // Mangled name: " << tempArg->get_mangled_name() << endl;
      if (tempArg->get_argumentType() == SgTemplateArgument::type_argument) {
        ...
        // if(enable_verbose || enable_debug)
        {
          ...
          if (enable_verbose)
            cout << o.str() << endl;
          // cout << "## " << n->variantT() <<":" << n->class_name() << " " <<
          // locatedNode->getFilenameString()  << endl;
          debugVector.push_back(o.str());
        }
        tempArgType = (isSgTypedefType(tempArgType) != nullptr)
                          ? isSgTypedefType(tempArgType)->get_base_type()
                          : tempArgType;
        ...
        {
          std::ostringstream o;
          o << "template argument mangled name:" << hashStringVal << ":"
            << mangledNameString;
          ;
          if (enable_verbose)
            cout << o.str() << endl;
...
void nodeTraversal::visit(SgNode *n) {
  ...
  if (locatedNode)
    if (sourcefilename.compare(locatedNode->getFilenameString()) == 0) {
      ...
      if (info) {
        std::ostringstream o;
        for (AttachedPreprocessingInfoType::iterator i = info->begin();
             i != info->end(); i++) {
          PreprocessingInfo::DirectiveType directive =
              (*i)->getTypeOfDirective();
          // std::cout << PreprocessingInfo::directiveTypeName (directive) << "
          // : " << (*i)->getString() << std::endl; o << n->variantT() << ":" <<
          // n->class_name() << " (" <<
          // PreprocessingInfo::relativePositionName((*i)->getRelativePosition())
          // << "):" << PreprocessingInfo::directiveTypeName (directive) << ":"
          // << (*i)->getString();
          o << n->class_name() << " ("
            ...
            << "):" << PreprocessingInfo::directiveTypeName(directive) << ":"
            << (*i)->getString();
          /*  Keep the following code in case we need to print only content from
             partial of the list

                    switch(directive)
                    {
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorUnknownDeclaration:
                      case PreprocessingInfo::DirectiveType::C_StyleComment:
                      case
             PreprocessingInfo::DirectiveType::CplusplusStyleComment: case
             PreprocessingInfo::DirectiveType::FortranStyleComment: case
             PreprocessingInfo::DirectiveType::F90StyleComment: case
             PreprocessingInfo::DirectiveType::CpreprocessorBlankLine: case
             PreprocessingInfo::DirectiveType::CpreprocessorIncludeDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorIncludeNextDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorDefineDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorUndefDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorIfdefDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorIfndefDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorIfDeclaration: case
             PreprocessingInfo::DirectiveType::CpreprocessorDeadIfDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorElseDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorElifDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorEndifDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorLineDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorErrorDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorWarningDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorEmptyDeclaration:
                      case PreprocessingInfo::DirectiveType::CSkippedToken:
                      case PreprocessingInfo::DirectiveType::CMacroCall:
                      case
             PreprocessingInfo::DirectiveType::CMacroCallStatement: case
             PreprocessingInfo::DirectiveType::CpreprocessorIdentDeclaration:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorCompilerGeneratedLinemarker:
                      case
             PreprocessingInfo::DirectiveType::CpreprocessorEnd_ifDeclaration:
                      case PreprocessingInfo::DirectiveType::LastDirectiveType:
                        o << n->variantT() << ":" << n->class_name() << " (" <<
             PreprocessingInfo::relativePositionName((*i)->getRelativePosition())
             << "):" << PreprocessingInfo::directiveTypeName (directive) << ":"
             << (*i)->getString(); break; default :
                        ;
                    }
          */
        }
      ...
      if (varDecl) {
        ...
        for (SgInitializedNamePtrList::iterator nameId = nameList.begin();
             nameId != nameList.end(); ++nameId) {
      ...
      if (funcDecl) {
        ...
        for (SgInitializedNamePtrList::iterator argId = funcArgList.begin();
             argId != funcArgList.end(); ++argId) {
          ...
          {
            std::ostringstream o;
            // o <<  varType->variantT() <<":funcArgType:" <<
            // varType->class_name();
...
int main(int argc, char *argv[]) {
  ...
  Rose_STL_Container<std::string> filenames =
      project->getAbsolutePathFileNames();
  for (Rose_STL_Container<std::string>::iterator it = filenames.begin();
       it != filenames.end(); it++)
