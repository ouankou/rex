...
make will try to compile every file that can be tried and show as many
...
3. src/frontend/SageIII/sage_support/keep_going.h /.cpp
 ...
 which internally uses -rose:keep_going and keeps track of successfully
processed or failed files, and save such information into log files.
...
int main(int argc, char *argv[]) {
  ...
  /*
     "  --list-filenames                Prints the filename(s), computed by the
     ROSE commandline handling,\n" "                                  onto
     standard out. Each filename is separated by a new line.\n"
     "\n"
     ...
     expected to fail\n" "  --expected-passes=<filename>    File containing
     filenames that are expected to pass\n" "  --strip-path-prefix=<filename>
     Normalize filenames by stripping this path prefix from them\n"
     "\n"
     "  --enable-ast-tests              Enables the internal ROSE AST
     consistency tests\n"
     "\n"
     "  -h,--help                       Show this help message\n"
     "  --verbose                       Enables debugging output, e.g. outputs
     successful files\n" "  --silent                        Disables debugging
     output\n"
     */
  ...
  } else // put your midend code here
  {
    if (Rose::KeepGoing::enable_ast_tests) {
      ...
      if (Rose::KeepGoing::verbose) {
        std::cerr << "[INFO] "
                  << "Skipping AST consistency tests; turn them on with "
                     "'--enable-ast-tests'"
                  << std::endl;
      }
    }
  ...
  // IMPORTANT: Note that this is split into two steps, with generate_reports()
  // in between; Otherwise the backend errors will not be reported in the file
  // report. return backend(project);
