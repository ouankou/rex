...
int main(int argc, char *argv[]) {
  ...
  if (project->get_verbose() > 0) {
    MLOG_INFO_CXX("dotGeneratorWholeASTGraph")
        << "Generating AST tree (" << numberOfNodes() << " nodes) in file "
        << filename << ".dot.\n";
  }

  generateWholeGraphOfAST(
      filename, new CustomMemoryPoolDOTGeneration::s_Filter_Flags(argvList));
