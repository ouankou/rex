...
void MyTraversal::visit(SgNode *node) {

  //! test for base class information of AST node
  if (dynamic_cast<SgBinaryOp *>(node)) {
    //! the number of binary operators is accumulated in the member variable
    //! binaryOpCount
    binaryOpCount++;
  }
...
unsigned int MyTraversal::getBinaryOpCount() { return binaryOpCount; }
