...
public:
  ...
  bool verificationSucceeded() const { return success; }

  void printReport(std::ostream &outputStream) const { outputStream << report; }
...
protected:
  ...
  void analyzeSubtree(SgFunctionDeclaration *func, Conditions *conditions) {
  ...
  void addSerializedState(std::pair<int, void *> state) {
    ...
    if (otherSuccess == false)
      success = false;
    const char *otherReport = (const char *)state.second + sizeof otherSuccess;
    report += otherReport;
  }
...
protected:
  Conditions *evaluateInheritedAttribute(SgNode *node,
    ...
    if (isSgClassDefinition(node)) {
      ...
      conditions = combineConditions(prePostConditions, inheritedConditions);
    }
    return conditions;
  }
};
