...
public:
  ...
  InheritedAttribute(const InheritedAttribute &X)
      : isLoop(X.isLoop), isVulnerability(X.isVulnerability) {}
};
...
InheritedAttribute
BugSeeding::evaluateInheritedAttribute(SgNode *astNode,
                                       InheritedAttribute inheritedAttribute) {
  // Use this if we only want to seed bugs in loops
  bool isLoop =
      inheritedAttribute.isLoop || (isSgForStatement(astNode) != NULL) ||
      (isSgWhileStmt(astNode) != NULL) || (isSgDoWhileStmt(astNode) != NULL);
  // Add Fortran support
  ...
  if (isLoop == true || applyEveryWhere == true) {
    // The inherited attribute is true iff we are inside a loop and this is a
    // SgPntrArrRefExp.
    ...
    if (arrayReference != NULL) {
      ...
      SgTreeCopy copyHelp;
      // Make a copy of the expression used to hold the array size in the array
      // declaration.
      ...
      // Build a new expression: "array[n]" --> "array[n+arraySizeCopy]", where
      // the arraySizeCopy is a size of "array"
      SgExpression *newIndexExpression = buildAddOp(
          indexExpression, arraySizeCopy, indexExpression->get_type());
