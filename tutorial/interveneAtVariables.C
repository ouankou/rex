...
SgFunctionCallExp *InstrumentationFunction::getCallExp() const {
  ...
  SgFunctionRefExp *functionRefExpression =
      new SgFunctionRefExp(new Sg_File_Info(*file_info), symbol, type);
  SgExprListExp *expressionList =
      new SgExprListExp(new Sg_File_Info(*file_info));
  SgFunctionCallExp *functionCallExp =
      new SgFunctionCallExp(file_info, functionRefExpression, expressionList,
...
void InstrumentationFunction::buildDeclaration(SgProject *project) {
  ...
  SgFunctionDeclaration *functionDeclaration =
      new SgFunctionDeclaration(file_info, name, type);
  ...
  SgInitializedName *var1_init_name = new SgInitializedName(var1_name,
  pointer_type);
...
void instrumentWrite(SgVarRefExp *varRef) {
  ...
  Sg_File_Info *file_info =
      Sg_File_Info::generateDefaultFileInfoForTransformationNode();
  SgCommaOpExp *commaOp = new SgCommaOpExp(file_info, beforeWrite.getCallExp(),
                                           varRef, varRef->get_type());
...
void instrumentRead(SgVarRefExp *varRef) {
  ...
  SgCommaOpExp *commaOp = new SgCommaOpExp(file_info, beforeRead.getCallExp(),
                                           varRef, varRef->get_type());
  ...
  if (uOp != NULL) {
    ...
    if (bOp != NULL) {
      ...
      if (expList != NULL) {
        ...
        for (SgExpressionPtrList::iterator iter = expressions.begin();;
             iter++) {
        ...
        cerr << "unexpected parent expression: " << parent->class_name()
             << endl;
        ROSE_ABORT();
      }
    }
...
class Traversal : public SgTopDownBottomUpProcessing<InheritedAttribute,
...
SynthesizedAttribute Traversal::evaluateSynthesizedAttribute(
  ...
  SgVarRefExp *varRef = NULL; // set to a var ref child of astNode iff this
                              // child is to be instrumented
  if (isSgVarRefExp(astNode)) {
    returnAttribute.isVarRef = true;
  } else {
    if (isSgAssignOp(astNode)) { // add other assignment operatrors
      SynthesizedAttribute leftChild =
          synthesizedAttributeList[SgBinaryOp_lhs_operand_i];
    ...
    if (varRef == NULL) { // not else! could be right-hand-side of assignment
      for (SubTreeSynthesizedAttributes::iterator iter =
               synthesizedAttributeList.begin();
