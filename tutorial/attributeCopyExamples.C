...
// This is an example of inherited attributes.  Here we prevent the traversal
// going any deeper than 20 levels.
...
// This is an example of synthesized attributes.  Here we keep a list of all the
// global veriable declarations that are created during the copy.
...
public:
  SgNode *copyAst(const SgNode *n) {
    ...
    if (!isSgFunctionDefinition(newN) && !isSgClassDefinition(newN)) {
      varDecls.insert(varDecls.end(), subCopy.varDecls.begin(),
                      subCopy.varDecls.end());
    }
...
// This is an example of accumulator attributes.  Here we count the number of if
// statements found during the copy.
