...
public:
  ...
  // SgFunctionSymbol* buildNewFunctionDeclaration ( SgProject* project,
  // SgFunctionType* previousFunctionType );
  SgFunctionSymbol *
  buildNewFunctionDeclaration(SgStatement *statementLocation,
                              SgFunctionType *previousFunctionType);
...
SgFunctionSymbol *SimpleInstrumentation::buildNewFunctionDeclaration(
    SgStatement *statementLocation, SgFunctionType *previousFunctionType) {
  // *****************************************************
  // Create the functionDeclaration
  // *****************************************************

  // Must mark the newly built node to be a part of a transformation so that it
  // will be unparsed!
  ...
  SgFunctionDeclaration *functionDeclaration =
      new SgFunctionDeclaration(file_info, function_name, previousFunctionType);
  ...
  // ********************************************************************
  const SgTypePtrList &argList = previousFunctionType->get_arguments();
  SgTypePtrList::const_iterator i = argList.begin();
  while (i != argList.end()) {
    SgName var_name = "";
  ...
  // Add any additional function arguments here! Make sure that the function
  // type is consistant.
  ...
  globalScope->insert_statement(functionDeclarationInGlobalScope,
                                functionDeclaration, inFront);
  ...
  case V_SgFunctionCallExp: {
    ...
    case V_SgFunctionRefExp: {
      ...
      if (symbol == functionSymbol) {
        ...
        // We only want to build the decalration once (and insert it into the
        // global scope) after that we save the symbol and reuse it.
        if (newFunctionSymbol == NULL) {
          ...
          newFunctionSymbol = buildNewFunctionDeclaration(
              SageInterface::getEnclosingStatement(astNode),
  ...
  case V_SgFunctionDeclaration: {
    ...
    if (functionName == "send") {
      ...
      if (functionType->get_return_type()->unparseToString() == "ssize_t") {
        const SgTypePtrList &argumentList = functionType->get_arguments();
        SgTypePtrList::const_iterator i = argumentList.begin();
      ...
      if (foundFunction == true) {
        ...
        functionSymbol =
            scope->lookup_function_symbol(functionName, functionType);
      }
    }
    break;
  }
