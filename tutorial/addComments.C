...
void visitorTraversal::visit(SgNode *n) {
  ...
  if (valueExp != NULL) {
    // Check if there is an expression tree from the original unfolded
    // expression. This is a trivial example ouf the output of an analysis
    // result.
    string comment = string("Auto-comment value: ") +
                     ((valueExp->get_originalExpressionTree() != NULL)
                          ? " this IS a constant folded value"
                          : " this is NOT a constant folded value");
