...
static const char *description =
    ...
    "statements with a function call to a new function containing those "
    "statements. "
...
// Switches for this tool. Tools with lots of switches will probably want these
// to be in some Settings struct mirroring the approach used by some analyses
// that have lots of settings. So we'll do that here too even though it looks
// funny.
struct Settings {
  bool showOutlinerSettings; // should we show the outliner settings instead of
                             // running it?

  Settings() : showOutlinerSettings(true) {}
} settings;
...
int main(int argc, char *argv[]) {
  ...
  // Some analyses validate their user-defined settings before running, but
  // that's not true (yet) for Outliner, so we call it explicitly.
  ...
  // Ideally, this logging would be part of the Outliner, not every tool that
  // uses it. And the outliner would have its own diagnostic facility that could
  // be turned on and off from the command-line (see --log).
  MLOG_INFO_CXX("tutorial") << "outlining...\n";
  size_t count = Outliner::outlineAll(proj);
  MLOG_INFO_CXX("tutorial")
      << "outlining processed " << count << " outline directive(s)" << "\n";
  return backend(proj);
}
