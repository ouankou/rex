...
void foo() {
  ...
  // Use of "sprintf" with a buffer where the length is an expression
  ...
  // Notice, that the explicit use of "snprintf" could be incorrect, we could
  // check this!.
  snprintf(global_buffer, n, "ASCI \n");

  // More interesting case where buffer is dynamically allocated.
  // Should generate: "snprintf(sNewName,sizeof(char)*(strlen(sName)+10),"%s =
  // %d",sName,7);"
  char bufferName[10];
  char *newBufferName =
      (char *)malloc(sizeof(char) * (strlen(bufferName) + 10));
