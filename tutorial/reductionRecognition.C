...
int main(int argc, char *argv[]) {
  ...
  //  std::set<  std::pair <SgInitializedName*, VariantT> >::const_iterator
  //  iter;
  std::set<std::pair<SgInitializedName *, OmpSupport::omp_construct_enum>>
      reductions;
  std::set<std::pair<SgInitializedName *,
                     OmpSupport::omp_construct_enum>>::const_iterator iter;
  ...
  for (iter = reductions.begin(); iter != reductions.end(); iter++) {
    ...
    cout << "\t variable: " << item.first->unparseToString()
         << "\t operation:" << OmpSupport::toString(item.second) << endl;
    ;
  }
