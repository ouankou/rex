...
/*
   ...
   Properties of instrumented source code:
       1) added declaration for coverage support function
          (either forward function declaration or a #include to include a header
   file). 2) Each function in the source program is instrumented to include a
   call to the coverage support function/
*/

// Global variables so that the global function declaration can be reused to
// build each function call expression in the AST traversal to instrument all
// functions.
...
// Simple ROSE traversal class: This allows us to visit all the functions and
// add new code to instrument/record their use.
...
// Code to build function declaration: This declares Shmuel's function call
// which will be inserted (as a function call) into each function body of the
// input application.
void buildFunctionDeclaration(SgProject *project) {
  ...
  SgFunctionType *function_type =
      new SgFunctionType(function_return_type, false);
  SgFunctionDeclaration *functionDeclaration =
      new SgFunctionDeclaration(file_info, function_name, function_type);
  ...
  // DQ (9/8/2007): We have not build a non-defining declaration, so this should
  // be NULL.
  ...
  printf(
      "Fixing up the symbol table in scope = %p = %s for function = %p = %s \n",
      globalScope, globalScope->class_name().c_str(), functionDeclaration,
  ...
  ROSE_ASSERT(globalScope->lookup_function_symbol(
                  functionDeclaration->get_name()) != NULL);
  ...
  SgInitializedName *var1_init_name =
      new SgInitializedName(var1_name, pointer_type);
  var1_init_name->set_file_info(
      Sg_File_Info::generateDefaultFileInfoForTransformationNode());
  ...
  // This can't be done by the AstPostProcessing (unless we relax some
  // constraints)
  functionDeclaration->set_scope(globalScope);

  // If it is not a forward declaration then the unparser will skip the ";" at
  // the end (need to fix this better)
...
void SimpleInstrumentation::visit(SgNode *astNode) {
  ...
  if (block != NULL) {
    // It is up to the user to link the implementations of these functions link
    // time
    ...
    // Note that we should not be sharing the same Sg_File_Info object in
    // multiple IR nodes.
    Sg_File_Info *newCallfileInfo =
        Sg_File_Info::generateDefaultFileInfoForTransformationNode();

    ROSE_ASSERT(functionSymbol != NULL);
    SgFunctionRefExp *functionRefExpression = new SgFunctionRefExp(
        newCallfileInfo, functionSymbol, globalFunctionType);
    SgExprListExp *expressionList = new SgExprListExp(newCallfileInfo);

    string codeLocation =
        fileName + " " + StringUtility::numberToString(lineNum);
    SgStringVal *functionNameStringValue =
        new SgStringVal(newCallfileInfo, (char *)codeLocation.c_str());
    expressionList->append_expression(functionNameStringValue);
    SgFunctionCallExp *functionCallExp = new SgFunctionCallExp(
        newCallfileInfo, functionRefExpression, expressionList,
        globalFunctionType->get_return_type());

    // create an expression type
    // SgTypeVoid* expr_type = new SgTypeVoid();
    // create an expression root
    // SgExpressionRoot * expr_root = new
    // SgExpressionRoot(newCallfileInfo,functionCallExp,expr_type);

    // create an expression statement
    SgExprStatement *new_stmt =
        new SgExprStatement(newCallfileInfo, functionCallExp);
...
int main(int argc, char *argv[]) {
  ...
  // Call function to declare function to be called to recode use of all
  ...
  // Generate Code and compile it with backend (vendor) compiler to generate
  // object code or executable (as specified on commandline using vendor
  // compiler's command line). Returns error code form compilation using
  // vendor's compiler.
