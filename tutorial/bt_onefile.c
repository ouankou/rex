...
c      problem_size:  12, 64, 102, 162 (for cclass T, A, B, C)
...
c  It sets the number of processors and the cclass of the NPB
...
#define COMPILETIME "05 Oct 2004"
...
static double forcing[IMAX / 2 * 2 + 1][JMAX / 2 * 2 + 1][KMAX / 2 * 2 + 1]
                     [5 + 1];
static double u[(IMAX + 1) / 2 * 2 + 1][(JMAX + 1) / 2 * 2 + 1]
               [(KMAX + 1) / 2 * 2 + 1][5];
static double rhs[IMAX / 2 * 2 + 1][JMAX / 2 * 2 + 1][KMAX / 2 * 2 + 1][5];
static double lhs[IMAX / 2 * 2 + 1][JMAX / 2 * 2 + 1][KMAX / 2 * 2 + 1][3][5]
                 [5];
...
#pragma omp master
  ...
  c_print_results("BT", cclass, grid_points[0], grid_points[1], grid_points[2],
...
#pragma omp for private(i, j, k, m)
  /* #pragma omp for ,by Liao, j k m will be shared by default, race condtion*/
...
c-------------------------------------------------------------------*/
#pragma omp for private(k, i, m, eta, zeta, dtpp, im1, ip1, xi)
  for (j = 1; j < grid_points[1] - 1; j++) {
    eta = (double)j * dnym1;
    ...
    for (k = 1; k < grid_points[2] - 1; k++) {
      ...
      for (m = 0; m < 5; m++) {
        ...
        i = 2;
        forcing[i][j][k][m] =
            forcing[i][j][k][m] - dssp * (-4.0 * ue[i - 1][m] + 6.0 * ue[i][m] -
      ...
      for (m = 0; m < 5; m++) {
        i = grid_points[0] - 3;
        forcing[i][j][k][m] =
            forcing[i][j][k][m] - dssp * (ue[i - 2][m] - 4.0 * ue[i - 1][m] +
...
c-------------------------------------------------------------------*/
#pragma omp for private(k, j, m, xi, eta, zeta, dtpp, jm1, jp1)
  for (i = 1; i < grid_points[0] - 1; i++) {
    xi = (double)i * dnxm1;
    ...
    for (k = 1; k < grid_points[2] - 1; k++) {
      ...
      for (m = 0; m < 5; m++) {
        ...
        j = 2;
        forcing[i][j][k][m] =
            forcing[i][j][k][m] - dssp * (-4.0 * ue[j - 1][m] + 6.0 * ue[j][m] -
      ...
      for (m = 0; m < 5; m++) {
        j = grid_points[1] - 3;
        forcing[i][j][k][m] =
            forcing[i][j][k][m] - dssp * (ue[j - 2][m] - 4.0 * ue[j - 1][m] +
...
c-------------------------------------------------------------------*/
#pragma omp for private(xi, j, eta, zeta, k, m, km1, kp1, dtpp)
  for (i = 1; i < grid_points[0] - 1; i++) {
    xi = (double)i * dnxm1;
    ...
    for (j = 1; j < grid_points[1] - 1; j++) {
      ...
      for (m = 0; m < 5; m++) {
        ...
        k = 2;
        forcing[i][j][k][m] =
            forcing[i][j][k][m] - dssp * (-4.0 * ue[k - 1][m] + 6.0 * ue[k][m] -
      ...
      for (m = 0; m < 5; m++) {
        k = grid_points[2] - 3;
        forcing[i][j][k][m] =
            forcing[i][j][k][m] - dssp * (ue[k - 2][m] - 4.0 * ue[k - 1][m] +
...
#pragma omp for private(xi, eta, zeta, j, k, ix, iy, iz, Pxi, m, Peta, Pzeta)
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
  ...
  i = 0;
  xi = 0.0;
#pragma omp for nowait private(eta, k, zeta, m)
  for (j = 0; j < grid_points[1]; j++) {
    eta = (double)j * dnym1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
  ...
  i = grid_points[0] - 1;
  xi = 1.0;
#pragma omp for private(eta, k, zeta, m)
  for (j = 0; j < grid_points[1]; j++) {
    eta = (double)j * dnym1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
  ...
  j = 0;
  eta = 0.0;
#pragma omp for nowait private(xi, k, zeta, m)
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
  ...
  j = grid_points[1] - 1;
  eta = 1.0;
#pragma omp for private(xi, k, m, zeta)
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
    for (k = 0; k < grid_points[2]; k++) {
      zeta = (double)k * dnzm1;
...
#pragma omp for nowait
  ...
  k = grid_points[2] - 1;
  zeta = 1.0;
#pragma omp for private(xi, eta, j, m)
  for (i = 0; i < grid_points[0]; i++) {
    xi = (double)i * dnxm1;
    for (j = 0; j < grid_points[1]; j++) {
      eta = (double)j * dnym1;
...
c-------------------------------------------------------------------*/
#pragma omp for private(k, i, tmp1, tmp2, tmp3)
  for (j = 1; j < grid_points[1] - 1; j++) {
    for (k = 1; k < grid_points[2] - 1; k++) {
      for (i = 0; i < grid_points[0]; i++) {
...
c-------------------------------------------------------------------*/
#pragma omp for private(j, k, tmp1, tmp2, tmp3)
  for (i = 1; i < grid_points[0] - 1; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 1; k < grid_points[2] - 1; k++) {
...
c-------------------------------------------------------------------*/
#pragma omp for private(j, k, tmp1, tmp2)
  for (i = 1; i < grid_points[0] - 1; i++) {
    for (j = 1; j < grid_points[1] - 1; j++) {
      for (k = 1; k < grid_points[2] - 1; k++) {
...
c-------------------------------------------------------------------*/
#pragma omp for nowait private(j, k, rho_inv)
  for (i = 0; i < grid_points[0]; i++) {
    for (j = 0; j < grid_points[1]; j++) {
      for (k = 0; k < grid_points[2]; k++) {
...
#pragma omp for private(j, k, uijk, up1, um1)
  ...
  c-------------------------------------------------------------------*/
  i = 1;
#pragma omp for nowait private(k, m)
  for (j = 1; j < grid_points[1] - 1; j++) {
    for (k = 1; k < grid_points[2] - 1; k++) {
      for (m = 0; m < 5; m++) {
...
#pragma omp for nowait private(j, k, m)
...
c-------------------------------------------------------------------*/
#pragma omp for private(j, k, vijk, vp1, vm1)
  for (i = 1; i < grid_points[0] - 1; i++) {
    for (j = 1; j < grid_points[1] - 1; j++) {
      for (k = 1; k < grid_points[2] - 1; k++) {
        vijk = vs[i][j][k];
...
c-------------------------------------------------------------------*/
#pragma omp for private(j, k, wijk, wp1, wm1)
  for (i = 1; i < grid_points[0] - 1; i++) {
    for (j = 1; j < grid_points[1] - 1; j++) {
      for (k = 1; k < grid_points[2] - 1; k++) {
        wijk = ws[i][j][k];
  ...
  c-------------------------------------------------------------------*/
  k = 1;
#pragma omp for nowait private(j, m)
  for (i = 1; i < grid_points[0] - 1; i++) {
    for (j = 1; j < grid_points[1] - 1; j++) {
      for (m = 0; m < 5; m++) {
        rhs[i][j][k][m] = rhs[i][j][k][m] -
...
void c_print_results(char *name, char cclass, int n1, int n2, int n3, int niter,
