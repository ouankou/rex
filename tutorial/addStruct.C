...
buildClassDeclarationAndDefinition(string name, SgScopeStatement *scope) {
  ...
  SgClassDeclaration *classDeclaration = new SgClassDeclaration(
      SOURCE_POSITION, name.c_str(), SgClassDeclaration::e_struct, NULL,
      classDefinition);
  ...
  SgClassDeclaration *nondefiningClassDeclaration = new SgClassDeclaration(
      SOURCE_POSITION, name.c_str(), SgClassDeclaration::e_struct, NULL, NULL);
  assert(classDeclaration != NULL);
  nondefiningClassDeclaration->set_endOfConstruct(SOURCE_POSITION);
  nondefiningClassDeclaration->set_scope(
      scope); // scope is needed for createType()
...
buildStructVariable(SgScopeStatement *scope, vector<SgType *> memberTypes,
                    vector<string> memberNames, string structName = "",
                    string varName = "",
                    SgAggregateInitializer *initializer = NULL) {
  ROSE_ASSERT(memberTypes.size() == memberNames.size());
  SgClassDeclaration *classDeclaration =
      buildClassDeclarationAndDefinition(structName, scope);
  ...
  while (typeIterator != memberTypes.end()) {
    ...
    SgVariableDeclaration *memberDeclaration = new SgVariableDeclaration(
        SOURCE_POSITION, *memberNameIterator, *typeIterator, NULL);
    ...
    classDeclaration->get_definition()->insert_symbol(*memberNameIterator,
                                                      variableSymbol);
  ...
  SgVariableDeclaration *variableDeclaration = new SgVariableDeclaration(
      SOURCE_POSITION, varName, classType, initializer);
  ...
  // The declarator, not the declaration wrapper, owns its initializer.
  ...
  variableDeclaration->set_baseTypeDefiningDeclaration(
...
int main(int argc, char *argv[]) {
  ...
  SgAggregateInitializer *structureInitializer =
      new SgAggregateInitializer(SOURCE_POSITION, initializerList);
  structureInitializer->set_endOfConstruct(SOURCE_POSITION);

  // Build the data member initializers for the structure (one
  // SgAssignInitializer for each data member)
  for (unsigned int i = 0; i < memberNames.size(); i++) {
    ...
    SgAssignInitializer *memberInitializer =
        new SgAssignInitializer(SOURCE_POSITION, value, memberTypes[i]);
  ...
  SgVariableDeclaration *variableDeclaration =
      buildStructVariable(file->get_globalScope(), memberTypes, memberNames,
                          "X", "x", structureInitializer);
