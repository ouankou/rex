...
public:
  std::string label;
  Edge(std::string label = "default edge")
...
template <class NodeType, class EdgeType>
void GraphBuilder<NodeType, EdgeType>::addEdge(NodeType *src, NodeType *snk,
                                               EdgeType *edge) {
...
/***********************************************************************************************
 * The function
 *     void TranslateGraph(_GraphStructure& graph, _NodePredicate& _nodePred,
 *_EdgePredicate& _edgePred); performas the action _nodePred to every node and
 *_edgePred to every edge in the graph 'graph'. This work is general and should
 *work on any graph constructed from VirtualGraphCreateTemplate which is the
 *graph all the program analysis work returns.
 ***********************************************************************************************/
template <typename _GraphStructure, typename _NodePredicate,
          typename _EdgePredicate>
void translateGraph(_GraphStructure &graph, _NodePredicate _nodePred,
                    _EdgePredicate _edgePred) {
  ...
  while (!it1.ReachEnd()) {
    ...
    it1++;
    typename _GraphStructure::EdgeIterator it2 =
        graph.GetNodeEdgeIterator(x1, GraphAccess::EdgeOut);
...
int main(int argc, char *argv[]) {
  ...
  graph.addNode(docs);
  Node *scripts = new Node("scripts");
  graph.addNode(scripts);
  Node *src = new Node("src");
  graph.addNode(src);
  Node *tests = new Node("tests");
  graph.addNode(tests);
  Node *tools = new Node("tools");
  graph.addNode(tools);
  Node *tutorial = new Node("tutorial");
  graph.addNode(tutorial);
  Node *frontend = new Node("frontend");
  graph.addNode(frontend);
  Node *midend = new Node("midend");
  graph.addNode(midend);
  Node *backend = new Node("backend");
  graph.addNode(backend);
  Node *thirdPartyLibraries = new Node("3rdPartyLibraries");
  graph.addNode(thirdPartyLibraries);
  Node *roseExtensions = new Node("RoseExtensions");
  graph.addNode(roseExtensions);
  Node *roseIndependentSupport = new Node("RoseIndependentSupport");
  graph.addNode(roseIndependentSupport);
  Node *rosetta = new Node("ROSETTA");
  graph.addNode(rosetta);
