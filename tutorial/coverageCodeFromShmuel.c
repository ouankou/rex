 ...
 * Returns NULL if failed. change buffer options here, if desired. Note fflush
 * below.
 */
static FILE *openCoverageFile() {
  char timestamp[TIMESTAMP_LEN];
  char name[6 + TIMESTAMP_LEN + 6 +
            1]; /* synch_ = 6 + timestamp = 20 + .trace = 6 + null=1 */
  FILE *file = NULL;
  struct timeval val;
  ...
  // DQ (9/13/2011): Static analysis reports that this is a problem and that the
  // size of the buffer should be 68 or greater. We back this up by also using a
  // safer version of sprintf() to limit the number of characters written to the
  // buffer to be the size of the buffer. sprintf(timestamp,
  // "%02d_%02d_%02d_%02d_%02d_%02d_%02d",ts->tm_year % 100, ts->tm_mon,
  // ts->tm_mday, ts->tm_hour, ts->tm_min,
  // ts->tm_sec,getHundredthOfSeconds(&val));
  snprintf(timestamp, TIMESTAMP_LEN, "%02d_%02d_%02d_%02d_%02d_%02d_%02d",
           ts->tm_year % 100, ts->tm_mon, ts->tm_mday, ts->tm_hour, ts->tm_min,
           ts->tm_sec, getHundredthOfSeconds(&val));
...
/*
 * Print a coverage task to the file, as one line. The line is composed of the
 * four arguments separated by spaces. The format is: source file name, space,
 * line number, space, type, sp. The file is then flushed. File is opened upon
 * first invocation, and never closed explicitly.
 */
static void reportTask(const char *str) {
  static bool fileErrorOccured =
      FALSE; /*once it is turned on, no further attempt to print will be done.*/
  if (!fileErrorOccured) {
...
/*
 * Handle a newly found coverage task: check if it is in the set. If not, print
 * it (using reportTask()), and add it to the set. If needed, resize the
 * hashtable used for the set. Return non-zero if some problem occured. 1 is out
 * of memory. Assumed to be called within internal lock.
 */
static int taskEncountered(const char *str) {
  ...
  static int capacity = 64; /*initial hash table size. When testing, try setting
                               it at very low value (e.g. 3), so that*/
                            /*resize will occur and will be tested.*/
  static taskListCell **taskSet =
      NULL; /*hashtable: array of lists of taskListCell. It is initialized to
               size*/
            /*capacity, but can be reallocated (resize), hence it is allocated
             * dynamically.*/
  static int setSize;
  ...
  if (setSize > HASH_LOAD_FACTOR * capacity) {
    ...
    /*go over all tasks in the old taskSet. Insert each one to the new table,
     * according to it's hash code.*/
    for (i = 0; i < capacity; i++) {
      for (iter = taskSet[i]; iter != NULL;) {

        taskListCell *next = iter->next; /*don't do it in the for(), since
                                            iter->next will change in the*/
                                         /*iteration block!*/
