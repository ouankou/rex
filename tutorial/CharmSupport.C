...
// transformation to do a transformation specific for Charm++.
...
buildListOfGlobalVariables(SgSourceFile *file) {
  ...
  while (i != globalScope->get_declarations().end()) {
    ...
    if (variableDeclaration != NULL) {
      Rose_STL_Container<SgInitializedName *> &variableList =
          variableDeclaration->get_variables();
      Rose_STL_Container<SgInitializedName *>::iterator var =
          variableList.begin();
...
buildListOfGlobalVariables(SgProject *project) {
  ...
  while (file != fileList.end()) {
    Rose_STL_Container<SgInitializedName *> fileGlobalVariableList =
        buildListOfGlobalVariables(isSgSourceFile(*file));

    // DQ (9/26/2007): Moved from std::list to std::vector
    // globalVariableList.merge(fileGlobalVariableList);
    globalVariableList.insert(globalVariableList.begin(),
                              fileGlobalVariableList.begin(),
                              fileGlobalVariableList.end());
...
buildListOfVariableReferenceExpressionsUsingGlobalVariables(SgNode *node) {
  // This function builds a list of "uses" of variables (SgVarRefExp IR nodes)
  // within the AST.
  ...
  while (i != nodeList.end()) {
    ...
    assert(variableReferenceExpression->get_symbol()->get_declaration() !=
           NULL);
    assert(variableReferenceExpression->get_symbol()
               ->get_declaration()
               ->get_scope() != NULL);

    // Note that variableReferenceExpression->get_symbol()->get_declaration()
    // returns the SgInitializedName (not the SgVariableDeclaration where it was
    // declared)!
...
buildClassDeclarationAndDefinition(string name, SgScopeStatement *scope) {
  ...
  SgClassDeclaration *classDeclaration = new SgClassDeclaration(
      fileInfo, name.c_str(), SgClassDeclaration::e_struct, NULL,
      classDefinition);
  ...
  printf("Fixing up the symbol table in scope = %p = %s for class = %p = %s \n",
         scope, scope->class_name().c_str(), classDeclaration,
...
SgVariableSymbol *putGlobalVariablesIntoClass(
    Rose_STL_Container<SgInitializedName *> &globalVariables,
    SgClassDeclaration *classDeclaration) {
  // This function iterates over the list of global variables and inserts them
  // into the iput class definition

  SgVariableSymbol *globalClassVariableSymbol = NULL;

  for (Rose_STL_Container<SgInitializedName *>::iterator var =
           globalVariables.begin();
       var != globalVariables.end(); var++) {
    // printf ("Appending global variable = %s to new globalVariableContainer
    // \n",(*var)->get_name().str());
    SgVariableDeclaration *globalVariableDeclaration =
        isSgVariableDeclaration((*var)->get_parent());
    ...
    if (var == globalVariables.begin()) {
      // This is the first time in this loop, replace the first global variable
      // with the class declaration/definition containing all the global
      // variables! Note that initializers in the global variable declarations
      // require modification of the preinitialization list in the class's
      // constructor!  I am ignoring this for now!
      globalScope->replace_statement(globalVariableDeclaration,
                                     classDeclaration);
      ...
      SgClassType *variableType =
          new SgClassType(classDeclaration->get_firstNondefiningDeclaration());
      assert(variableType != NULL);
      SgVariableDeclaration *variableDeclaration =
          new SgVariableDeclaration(fileInfo, "AMPI_globals", variableType);
      assert(variableDeclaration != NULL);

      globalScope->insert_statement(classDeclaration, variableDeclaration,
                                    false);

      assert(variableDeclaration->get_variables().empty() == false);
      SgInitializedName *variableName =
      ...
      // DQ (9/8/2007): Need to add the symbol to the global scope (new testing
      // requires this).
      globalScope->insert_symbol(variableName->get_name(),
                                 globalClassVariableSymbol);
      ROSE_ASSERT(globalScope->lookup_variable_symbol(
                      variableName->get_name()) != NULL);
    } else {
    ...
    classDeclaration->get_definition()->append_member(
...
void fixupReferencesToGlobalVariables(
    Rose_STL_Container<SgVarRefExp *> &variableReferenceList,
    SgVariableSymbol *globalClassVariableSymbol) {
  // Now fixup the SgVarRefExp to reference the global variables through a
  // struct
  for (Rose_STL_Container<SgVarRefExp *>::iterator var =
           variableReferenceList.begin();
    ...
    // If this is not an expression then is likely a meaningless statement such
    // as ("x;")
    SgExpression *parentExpression = isSgExpression(parent);
    assert(parentExpression != NULL);

    // Build the reference through the global class variable ("x" -->
    // "AMPI_globals.x")
    ...
    SgDotExp *globalVariableReference =
        new SgDotExp(fileInfo, lhs, *var, (*var)->get_type());
      ...
      if (unaryOperator != NULL) {
        ...
        if (binaryOperator != NULL) {
          ...
          switch (parentExpression->variantT()) {
            // Where the variable appers in the function argument list the
            // parent is a SgExprListExp
          case V_SgExprListExp: {
            printf("Sorry not implemented, case of global variable in function "
                   "argument list ... \n");
            ROSE_ABORT();
          }
          ...
          case V_SgVarArgOp:
          default: {
            printf("Error: default reached in switch  parentExpression = %p = "
                   "%s \n",
...
void transformGlobalVariablesToUseStruct(SgSourceFile *file) {
  ...
  // These are the global variables in the input program (provided as helpful
  // information)
  Rose_STL_Container<SgInitializedName *> globalVariables =
      buildListOfGlobalVariables(file);
  ...
  // get the global scope within the first file (currently ignoring all other
  // files)
  SgGlobal *globalScope = file->get_globalScope();
  assert(globalScope != NULL);
  ...
  SgClassDeclaration *classDeclaration =
      buildClassDeclarationAndDefinition("AMPI_globals_t", globalScope);

  // Put the global variables intothe class
  SgVariableSymbol *globalClassVariableSymbol =
      putGlobalVariablesIntoClass(globalVariables, classDeclaration);

  // Their associated symbols will be located within the project's AST
  // (where they occur in variable reference expressions).
  Rose_STL_Container<SgVarRefExp *> variableReferenceList =
      buildListOfVariableReferenceExpressionsUsingGlobalVariables(file);
  ...
  // Fixup all references to global variable to access the variable through the
  // class ("x" --> "AMPI_globals.x")
  fixupReferencesToGlobalVariables(variableReferenceList,
                                   globalClassVariableSymbol);
}

void transformGlobalVariablesToUseStruct(SgProject *project) {
