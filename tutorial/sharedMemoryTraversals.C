...
int main(int argc, char **argv) {
  ...
  parallelTraversal.addTraversal(
      new NodeTypeTraversal(V_SgForStatement, "for loop"));
  parallelTraversal.addTraversal(
      new NodeTypeTraversal(V_SgIntVal, "int constant"));
  parallelTraversal.addTraversal(
      new NodeTypeTraversal(V_SgVariableDeclaration, "variable declaration"));
...
int main() {
  std::cout << "parallel traversal is not supported in this configuration "
               "(user does not want ROSE to be thread-aware)\n";
  return 0;
}
