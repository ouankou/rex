...
/*
Your basic visitor traversal subclassed from SgGraphTraversal on the CFGforT
template as defined above
*/
class visitorTraversal : public SgGraphTraversal<CFGforT> {
public:
  int paths;
  /* This is the function run by the algorithm on every path, VertexID is a type
   * implemented in SgGraphTemplate.h */
  void analyzePath(vector<VertexID> &pth);
};

/* defining the analyzePath function. This simply counts paths as should be
 * obvious. Again, VertexID is defined in SgGraphTemplate.h */
void visitorTraversal::analyzePath(vector<VertexID> &pth) { paths++; }
