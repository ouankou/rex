...
int main(int argc, char *argv[]) {
  ...
  // Process all function definition bodies for virtual control flow graph
  // generation
  ...
  for (Rose_STL_Container<SgNode *>::const_iterator i = functions.begin();
    ...
    string fileName = StringUtility::stripPathFromFileName(
        proc->get_file_info()->get_filenameString());
    string dotFileName1 =
        fileName + "." + proc->get_declaration()->get_name() + ".debug.dot";
    string dotFileName2 = fileName + "." + proc->get_declaration()->get_name() +
                          ".interesting.dot";
