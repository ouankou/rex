...
void SimpleInstrumentation::visit(SgNode *astNode) {
  ...
  if (globalScope != NULL) {
    ...
    // ********************************************************************
    // Create the InitializedName for a parameter within the parameter
    // list
    // ********************************************************************
    ...
    SgInitializedName *var1_init_name =
        new SgInitializedName(var1_name, ref_type);
    ...
    SgPlusPlusOp *pp_expression = new SgPlusPlusOp(
        TRANSFORMATION_FILE_INFO, var_ref, var_ref->get_type());
